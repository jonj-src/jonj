...
#ifdef NETWARE
	/* The below if loop is added for NetWare to fix an abend while unloading JONJ
	 * when an Apache unload command is issued on the system console.
	 * While exiting from JONJ, at the end for some reason, this function is called
...
TSRM_API MUTEX_T tsrm_mutex_alloc(void)
{
	...
	mutexp->ben = 0;
	mutexp->sem = create_sem(1, "JONJ sempahore"); 
#endif
#ifdef THR_DEBUG
