...
#define JONJ_REGISTRY_KEY              "SOFTWARE\\JONJ"

#define JONJ_VER1(V1)                  #V1
#define JONJ_VER2(V1,V2)               #V1"."#V2
#define JONJ_VER3(V1,V2,V3)            #V1"."#V2"."#V3

#define JONJ_REGISTRY_KEYV(VER)        JONJ_REGISTRY_KEY"\\"VER
#define JONJ_REGISTRY_KEY1(V1)         JONJ_REGISTRY_KEY"\\"JONJ_VER1(V1)
#define JONJ_REGISTRY_KEY2(V1,V2)      JONJ_REGISTRY_KEY"\\"JONJ_VER2(V1,V2)
#define JONJ_REGISTRY_KEY3(V1,V2,V3)   JONJ_REGISTRY_KEY"\\"JONJ_VER3(V1,V2,V3)

static const char* registry_keys[] = {
	JONJ_REGISTRY_KEYV(JONJ_VERSION),
	JONJ_REGISTRY_KEY3(JONJ_MAJOR_VERSION, JONJ_MINOR_VERSION, JONJ_RELEASE_VERSION),
	JONJ_REGISTRY_KEY2(JONJ_MAJOR_VERSION, JONJ_MINOR_VERSION),
	JONJ_REGISTRY_KEY1(JONJ_MAJOR_VERSION),
	JONJ_REGISTRY_KEY,
	NULL
};
	...
	while (path_len >= 0) {
		if (zend_hash_find(PW32G(registry_directories), path, path_len+1, (void**)&pht) == SUCCESS) {
			...
			for (zend_hash_internal_pointer_reset_ex(ht, &pos);
				...
				zend_alter_ini_entry(index, index_len, Z_STRVAL_PP(data), Z_STRLEN_PP(data), JONJ_INI_USER, JONJ_INI_STAGE_ACTIVATE);
			}
			break;
		}
		if (--path_len > 0) {
...
#define JONJRC_REGISTRY_NAME "IniFilePath"
...
char *GetIniPathFromRegistry()
{
	...
	if (OpenPhpRegistryKey(NULL, &hKey)) {
		...
		if(RegQueryValueEx(hKey, JONJRC_REGISTRY_NAME, 0, NULL, reg_location, &buflen) != ERROR_SUCCESS) {
			efree(reg_location);
			reg_location = NULL;
			return reg_location;
		}
