/* 
 *    JONJ Sendmail for Windows.
 *
 *  This file is rewriten specificly for JONJFI.  Some functionality
...
/* '*error_message' has to be passed around from jonj_mail() */
...
char seps[] = " ,\t\n";
#ifndef NETWARE
char *jonj_mailer = "JONJ 5 WIN32";
#else
char *jonj_mailer = "JONJ 5 NetWare";
#endif	/* NETWARE */
...
static char *ErrorMessages[] =
{
	...
	{"Failed to connect to mailserver, verify your \"SMTP\" setting in jonj.ini"},
	{"Failed to Send"},
	{"Failed to Receive"},
	{"Server Error"},
	...
	{"Bad Mail Host"},
	{"Bad Message File"},
	{"\"sendmail_from\" not set in jonj.ini or custom \"From:\" header missing"},
	{"Mailserver rejected our \"sendmail_from\" setting"}, /* 20 */
	{"Error while trimming mail header with PCRE, please file a bug report at http://bugs.jonj.tk/"} /* 21 */
};
 ...
 */
#define JONJ_WIN32_MAIL_UNIFY_PATTERN	"/(\r\n?)|\n/"
#define JONJ_WIN32_MAIL_UNIFY_REPLACE	"\r\n"
...
#define JONJ_WIN32_MAIL_RMVDBL_PATTERN	"/^\r\n|(\r\n)+$/m"
#define JONJ_WIN32_MAIL_RMVDBL_REPLACE	""
 ...
 * message body. */
#define JONJ_WIN32_MAIL_DOT_PATTERN	"\n."
#define JONJ_WIN32_MAIL_DOT_REPLACE	"\n.."
 ...
 */
static char *jonj_win32_mail_trim_header(char *header TSRMLS_DC)
{
	...
	ZVAL_STRING(replace, JONJ_WIN32_MAIL_UNIFY_REPLACE, 0);

	result = jonj_pcre_replace(JONJ_WIN32_MAIL_UNIFY_PATTERN, sizeof(JONJ_WIN32_MAIL_UNIFY_PATTERN)-1,
	...
	ZVAL_STRING(replace, JONJ_WIN32_MAIL_RMVDBL_REPLACE, 0);

	result2 = jonj_pcre_replace(JONJ_WIN32_MAIL_RMVDBL_PATTERN, sizeof(JONJ_WIN32_MAIL_RMVDBL_PATTERN)-1,
							   result, result_len,
							   replace,
							   0,
							   &result_len,
...
//********************************************************************/
JONJAPI int TSendMail(char *host, int *error, char **error_message,
			  ...
			  char *mailCc, char *mailBcc, char *mailRPath TSRMLS_DC)
{
	...
	if (headers) {
		...
		if (NULL == (headers = jonj_win32_mail_trim_header(headers TSRMLS_CC))) {
	...
	/* Fall back to sendmail_from jonj.ini setting */
	...
	*error = MailConnect();
	if (*error != 0) {
		...
		snprintf(*error_message, HOST_NAME_LEN + 128,
			...
			"and \"smtp_port\" setting in jonj.ini or use ini_set()",
...
// History:
//*******************************************************************/
JONJAPI char *GetSMErrorText(int index)
{
			 ...
			 char *headers, char *headers_lc, char **error_message TSRMLS_DC)
{
	...
	*/
	...
	/* Escape \n. sequences
	 * We use jonj_str_to_str() and not jonj_str_replace_in_subject(), since the latter
	 * uses ZVAL as it's parameters */
	data_cln = jonj_str_to_str(data, strlen(data), JONJ_WIN32_MAIL_DOT_PATTERN, sizeof(JONJ_WIN32_MAIL_DOT_PATTERN) - 1,
					JONJ_WIN32_MAIL_DOT_REPLACE, sizeof(JONJ_WIN32_MAIL_DOT_REPLACE) - 1, &data_cln_len);
	if (!data_cln) {
		data_cln = estrdup("");
		data_cln_len = 1;		
	}
...
static int PostHeader(char *RPath, char *Subject, char *mailTo, char *xheaders TSRMLS_DC)
{
	...
	if (!xheaders || !strstr(headers_lc, "date:")) {
		...
		char *dt = jonj_format_date("r", 1, tNow, 1 TSRMLS_CC);
