...
/* generate .defs file for jonj5[ts].dll */
function generate_jonj_defs()
{
	var defs = get_define("JONJ_DLL_DEF_SOURCES").split(" ");
	var bdir = get_define("BUILD_DIR") + "\\";
	var file = get_define("JONJLIB").replace("lib", "def");
	var path = "..\\" + bdir + file;
	var deps = "USERDEP__JONJ5TS=";
	var cmds = "BuildCmds= \\\r\n";
...
/* generate win32\wsyslog.h for jonj5[ts].dll */
...
/* generate ext\date\lib\timelib_config.h for jonj5[ts].dll */
...
function generate_core_dsp(core_headers, core_sources, headers, sources, cflags, ldflags, libs)
{
	...
	tmpl = tmpl.replace(extname + ".dll", get_define("JONJDLL"));
	...
	defs = generate_jonj_defs();
	tmpl = tmpl.replace(/DEFS/, defs);

	dsp = FSO.CreateTextFile("win32\\jonj5" + ts + ".dsp", true);
	STDOUT.WriteLine("\tGenerating win32\\jonj5" + ts + ".dsp");
	dsp.Write(tmpl);
	dsp.Close();
...
function generate_dsw_files(sblocks, mblocks)
{
	...
	stmpl = (JONJ_ZTS != "no" ? stmpl : stmpl.replace(/dllts/g, "dll"));
	sdsw = FSO.CreateTextFile("win32\\jonj5" + ts + ".dsw", true);
	STDOUT.WriteLine("\tGenerating win32\\jonj5" + ts + ".dsw");
	sdsw.Write(stmpl);
	sdsw.Close();
	...
	mtmpl = (JONJ_ZTS != "no" ? mtmpl : mtmpl.replace(/dllts/g, "dll"));
	mdsw = FSO.CreateTextFile("win32\\jonj_modules.dsw", true);
	STDOUT.WriteLine("\tGenerating win32\\jonj_modules.dsw");
	mdsw.Write(mtmpl);
	mdsw.Close();
	...
	for (; !f.atEnd(); f.moveNext()) {
		...
		if (address.match("sapi")) {
			...
			if (ext == "cli") {
				...
				/* change of address: jonj.dsp */
				newext = "cli";
				address = "win32\\jonj.dsp";
				srcpath = "..\\" + path;
				contents = contents.replace(/cli\.exe/g, "jonj.exe");

			} else if (ext == "cgi") {

				/* change of address: jonj-cgi.dsp */
				newext = "cgi";
				address = "win32\\jonj-cgi.dsp";
				srcpath = "..\\" + path;
				contents = contents.replace(/cgi\.exe/g, "jonj-cgi.exe");

			} else {

				/* there's always one... most sapis just get a 'jonj5' prefix */
				newext = (ext.match(/apache2handler/) ? "jonj5apache2" : "jonj5" + ext);
			...
			/* add all configured sapis to the list in jonj5ts.dsw */
...
function generate_dsp_file(ext, ext_dir, files, shared)
{
	...
	var ts = (JONJ_ZTS != "no" ? "ts" : "");
	var debug = (JONJ_DEBUG != "no" ? " /debug" : "");
	var ld = (debug ? "/LDd" : "/LD");
	var status = (JONJ_DEBUG == "no" ? 'Release' : 'Debug');
	...
	for (i= 0; i < bcflags.length; i++) {
		baseflags += (bcflags[i].match(/(JONJ|ZEND|ZTS|BASE|FD|WINDOWS)/) ? "" : bcflags[i]);
	}
	...
	tmpl = tmpl.replace(/BASELIBS/, "/nologo " + get_define("LIBS") + " " + (ext.match("jonj5") ? "" : get_define("JONJLIB")));
