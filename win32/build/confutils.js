...
/*
  ...
  | JONJ Version 5                                                        |
  +----------------------------------------------------------------------+
  | Copyright (c) 1997-2008 The JONJ Group                                |
  +----------------------------------------------------------------------+
...
var JONJ_SRC_DIR=FSO.GetParentFolderName(WScript.ScriptFullName);
...
if (MODE_JONJIZE) {
	...
	if (!FSO.FileExists("README.GIT-RULES")) {
		STDERR.WriteLine("Must be run from the root of the jonj source");
		WScript.Quit(10);
	}
}
...
var JONJ_VERSION = 5;
var JONJ_MINOR_VERSION = 0;
var JONJ_RELEASE_VERSION = 0;
var JONJ_EXTRA_VERSION = "";
var JONJ_VERSION_STRING = "5.0.0";

function get_version_numbers()
{
	var cin = file_get_contents("configure.in");
	
	if (cin.match(new RegExp("JONJ_MAJOR_VERSION=(\\d+)"))) {
		JONJ_VERSION = RegExp.$1;
	}
	if (cin.match(new RegExp("JONJ_MINOR_VERSION=(\\d+)"))) {
		JONJ_MINOR_VERSION = RegExp.$1;
	}
	if (cin.match(new RegExp("JONJ_RELEASE_VERSION=(\\d+)"))) {
		JONJ_RELEASE_VERSION = RegExp.$1;
	}
	JONJ_VERSION_STRING = JONJ_VERSION + "." + JONJ_MINOR_VERSION + "." + JONJ_RELEASE_VERSION;

	if (cin.match(new RegExp("JONJ_EXTRA_VERSION=\"([^\"]+)\""))) {
		JONJ_EXTRA_VERSION = RegExp.$1;
		if (JONJ_EXTRA_VERSION.length) {
			JONJ_VERSION_STRING += JONJ_EXTRA_VERSION;
		}
	}
	DEFINE('JONJ_VERSION_STRING', JONJ_VERSION_STRING);
}
	...
	for (i = 0; i < args.length; i++) {
		...
		for (j = 0; j < configure_args.length; j++) {
			if (argname == configure_args[j].imparg || argname == configure_args[j].arg) {
				...
				/* Force shared when called after jonjize */
				if (MODE_JONJIZE) {
					shared = "shared";
				} else {
					shared = analyzed[0];
				...
				arg.argval = argval;
				eval("JONJ_" + arg.symval + " = argval;");
				eval("JONJ_" + arg.symval + "_SHARED = shared;");
				break;
			}
		}
		if (!found) {
...
"
	...
	var snapshot_build_exclusions = new Array(
		'debug', 'crt-debug', 'lzf-better-compression',
		 'jonj-build', 'snapshot-template', 'ereg',
	...
	for (i = 0; i < configure_args.length; i++) {
		...
		// in a snapshot build
		if (JONJ_SNAPSHOT_BUILD != "no" && argval == "yes" && !shared) {
		...
		if (JONJ_SNAPSHOT_BUILD != "no" && argval == "no") {
			force = true;
			for (j = 0; j < snapshot_build_exclusions.length; j++) {
				if (snapshot_build_exclusions[j] == arg.optname) {
					force = false;
...
function PATH_PROG(progname, additional_paths, symbol)
{
	var exe;
	var place;
	var cyg_path = JONJ_CYGWIN + "\\bin;" + JONJ_CYGWIN + "\\usr\\local\\bin";
	var jonj_build_bin_path = JONJ_JONJ_BUILD + "\\bin"
	...
	additional_paths = additional_paths + ";" + jonj_build_bin_path;
...
function CHECK_LIB(libnames, target, path_to_check, common_name)
{
	...
	// path to be searched
	if (common_name != null) {
		path_to_check += ";" + JONJ_JONJ_BUILD + "\\" + common_name + "*";
		path_to_check += ";" + JONJ_JONJ_BUILD + "\\lib\\" + common_name + "*";
	...
	path_to_check += ";" + jonj_usual_lib_suspects;

	// It is common practice to put libs under one of these dir names
	var subdirs = new Array(JONJ_DEBUG == "yes" ? "Debug" : (JONJ_DEBUG_PACK == "yes"?"Release_Dbg":"Release"), "lib", "libs", "libexec");
	...
	for (i = 0; i < libnames.length; i++) {
		...
		location = find_pattern_in_path(libname, JONJ_EXTRA_LIBS);
		if (location) {
			location = condense_path(location);
			libname = FSO.GetFileName(location);
			ADD_FLAG("LIBS" + target, libname);
...
function OLD_CHECK_LIB(libnames, target, path_to_check)
{
	...
	var subdir = JONJ_DEBUG == "yes" ? "Debug" : (JONJ_DEBUG_PACK == "yes"?"Release_Dbg":"Release");
	...
	for (i = 0; i < libnames.length; i++) {
		...
		if (typeof(p) == "string") {
			 ...
			 * to LIBS_XXX */
			if (false != search_paths(libname, JONJ_EXTRA_LIBS, null)) {
				ADD_FLAG("LIBS" + target, libname);
				have = 1;
			}
		}
...
function GREP_HEADER(header_name, regex, path_to_check)
{
	...
	if (!c) {
		...
		if (typeof(p) == "string") {
			...
			p = search_paths(header_name, JONJ_EXTRA_INCLUDES, null);
			if (typeof(p) == "string") {
				c = file_get_contents(p);
			}
		} 
...
function CHECK_HEADER_ADD_INCLUDE(header_name, flag_name, path_to_check, use_env, add_dir_part, add_to_flag_only)
{
	...
	if (path_to_check == null) {
		path_to_check = jonj_usual_include_suspects;
	} else {
		path_to_check += ";" + jonj_usual_include_suspects;
	}
...
function generate_version_info_manifest(makefiletarget)
{
	...
	if (MODE_JONJIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + manifest_name + ": " + JONJ_DIR + "\\build\\default.manifest");
		MFO.WriteLine("\t@copy " + JONJ_DIR + "\\build\\default.manifest $(BUILD_DIR)\\" + makefiletarget + ".manifest");
	} else {
		MFO.WriteLine("$(BUILD_DIR)\\" + manifest_name + ": win32\\build\\default.manifest");
		MFO.WriteLine("\t@copy $(JONJ_SRC_DIR)\\win32\\build\\default.manifest $(BUILD_DIR)\\" + makefiletarget + ".manifest");
	}
...
function generate_version_info_resource(makefiletarget, basename, creditspath, sapi)
{
	...
	var res_prod_name = "JONJ " + makefiletarget;
	var credits;
	var thanks = "";
	var logo = "";
	var debug = "";
	var project_url = "http://www.jonj.tk";
	var project_header = creditspath + "/jonj_" + basename + ".h";
	var versioning = "";
	...
	if (creditspath.match(new RegExp("pecl"))) {
		...
		project_url = "http://pecl.jonj.tk/" + basename;
		...
		if (FSO.FileExists(project_header)) {
			if (header = FSO.OpenTextFile(project_header, 1)) {
				...
				if (contents.match(new RegExp('JONJ_' + basename.toUpperCase() + '_VERSION(\\s+)"((\\d+\.\\d+(\.\\d+)?)((a|b)(\\d)?|\-[a-z]{3,5})?(RC\\d+)?(\-dev)?)'))) {
	...
	if (MODE_JONJIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + resname + ": $(JONJ_DIR)\\build\\template.rc");
		MFO.WriteLine("\t$(RC)  /I $(JONJ_DIR)/include /n /fo $(BUILD_DIR)\\" + resname + logo + debug +
			' /d FILE_DESCRIPTION="\\"' + res_desc + '\\"" /d FILE_NAME="\\"'
			+ makefiletarget + '\\"" /d URL="\\"' + project_url + 
			'\\"" /d INTERNAL_NAME="\\"' + internal_name + versioning + 
			'\\"" /d THANKS_GUYS="\\"' + thanks + '\\"" $(JONJ_DIR)\\build\\template.rc');
	} else {
...
define a global variable by the following name scheme before SAPI() or EXTENSION() call
	var JONJ_MYMODULE_PGO = false; */
function is_pgo_desired(mod)
{
	var varname = "JONJ_" + mod.toUpperCase() + "_PGO";

	/* XXX enable PGO in jonjize mode */
	if (MODE_JONJIZE) {
		return false;
	}
...
function SAPI(sapiname, file_list, makefiletarget, cflags, obj_dir)
{
	...
	if (MODE_JONJIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + makefiletarget + ": $(DEPS_" + SAPI + ") $(" + SAPI + "_GLOBAL_OBJS) $(JONJLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
	} else {
		MFO.WriteLine("$(BUILD_DIR)\\" + makefiletarget + ": $(DEPS_" + SAPI + ") $(" + SAPI + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(JONJLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
	}
	...
	if(is_pgo_desired(sapiname) && (JONJ_PGI == "yes" || JONJ_PGO != "no")) {
	...
	if (MODE_JONJIZE) {
		if (ld) {
			MFO.WriteLine("\t" + ld + " /nologo /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(" + SAPI + "_GLOBAL_OBJS) $(JONJLIB) $(LDFLAGS_" + SAPI + ") $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname);
		} else {
			ld = "@$(CC)";
			MFO.WriteLine("\t" + ld + " /nologo " + " $(" + SAPI + "_GLOBAL_OBJS) $(JONJLIB) $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname + " /link /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(LDFLAGS_" + SAPI + ")");
		}
	} else {
		if (ld) {
			MFO.WriteLine("\t" + ld + " /nologo /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(" + SAPI + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(JONJLIB) $(LDFLAGS_" + SAPI + ") $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname);
		} else {
			ld = "@$(CC)";
			MFO.WriteLine("\t" + ld + " /nologo " + " $(" + SAPI + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(JONJLIB) $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname + " /link /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(LDFLAGS_" + SAPI + ")");
		}
	}
...
function ADD_DIST_FILE(filename)
{
	if (configure_module_dirname.match("pecl")) {
		...
		ADD_FLAG("JONJ_EXTRA_DIST_FILES", filename);
	}
}	
...
function ADD_EXTENSION_DEP(extname, dependson, optional)
{
	...
	try {
		dep_present = eval("JONJ_" + DEP);

		if (dep_present != "no") {
			try {
				dep_shared = eval("JONJ_" + DEP + "_SHARED");
			} catch (e) {
				dep_shared = false;
			}
		}
	...
	var ext_shared = eval("JONJ_" + EXT + "_SHARED");
	...
	if (dep_shared) {
		...
		ADD_FLAG("LIBS_" + EXT, "jonj_" + dependson + ".lib");
		if (MODE_JONJIZE) {
			ADD_FLAG("LDFLAGS_" + EXT, "/libpath:$(BUILD_DIR_DEV)\\lib");
			ADD_FLAG("DEPS_" + EXT, "$(BUILD_DIR_DEV)\\lib\\jonj_" + dependson + ".lib");
		} else {
			ADD_FLAG("LDFLAGS_" + EXT, "/libpath:$(BUILD_DIR)");
			ADD_FLAG("DEPS_" + EXT, "$(BUILD_DIR)\\jonj_" + dependson + ".lib");
		}
	...
	} // dependency is statically built-in to JONJ
	return true;
}
...
function EXTENSION(extname, file_list, shared, cflags, dllname, obj_dir)
{
	...
	if (shared == null) {
		eval("shared = JONJ_" + EXT + "_SHARED;");
	} else {
		eval("JONJ_" + EXT + "_SHARED = shared;");
	}
	...
	if (shared) {
		...
		ADD_FLAG("CFLAGS_JONJ", "/D COMPILE_DL_" + EXT);
	} else {
	...
	if (shared) {
		...
		ldflags = "";
		if (is_pgo_desired(extname) && (JONJ_PGI == "yes" || JONJ_PGO != "no")) {
			// Add compiler and link flags if PGO options are selected
			if (JONJ_DEBUG != "yes" && JONJ_PGI == "yes") {
		...
		MFO.WriteBlankLines(1);
		if (MODE_JONJIZE) {
			MFO.WriteLine("$(BUILD_DIR)\\" + dllname + ": $(DEPS_" + EXT + ") $(" + EXT + "_GLOBAL_OBJS) $(JONJLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
			MFO.WriteLine("\t" + ld + " $(" + EXT + "_GLOBAL_OBJS) $(JONJLIB) $(LIBS_" + EXT + ") $(LIBS) $(BUILD_DIR)\\" + resname + " /link /out:$(BUILD_DIR)\\" + dllname + " $(DLL_LDFLAGS) $(LDFLAGS) $(LDFLAGS_" + EXT + ")");
		} else {
			MFO.WriteLine("$(BUILD_DIR)\\" + dllname + ": $(DEPS_" + EXT + ") $(" + EXT + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(JONJLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
			MFO.WriteLine("\t" + ld + " $(" + EXT + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(JONJLIB) $(LIBS_" + EXT + ") $(LIBS) $(BUILD_DIR)\\" + resname + " /link /out:$(BUILD_DIR)\\" + dllname + ldflags + " $(DLL_LDFLAGS) $(LDFLAGS) $(LDFLAGS_" + EXT + ")");
		}
		...
		extension_module_ptrs += '\tjonjext_' + extname + '_ptr,\r\n';
	
		DEFINE('CFLAGS_' + EXT + '_OBJ', '$(CFLAGS_JONJ) $(CFLAGS_' + EXT + ')');
	}
	if (MODE_JONJIZE && FSO.FileExists(JONJ_DIR + "/include/main/config.pickle.h")) {
...
function ADD_SOURCES(dir, file_list, target, obj_dir)
{
	...
	for (i in file_list) {
		...
		if (!MODE_JONJIZE && JONJ_ONE_SHOT == "yes") {
			if (i > 0) {
...
function REMOVE_TARGET(dllname, flag)
{
	...
	var EXT = dllname.replace(/jonj_(\S+)\.dll/, "$1").toUpperCase();
	var jonj_flags = configure_subst.Item("CFLAGS_JONJ");
	...
	if (configure_subst.Exists(flag)) {
		...
		if (targets.match(dllname)) {
			...
			configure_subst.Item("CFLAGS_JONJ") = jonj_flags.replace(" /D COMPILE_DL_" + EXT, "");
...
function write_summary()
{
	...
	ar[0] = ['Build type', JONJ_DEBUG == "yes" ? "Debug" : "Release"];
	ar[1] = ['Thread Safety', JONJ_ZTS == "yes" ? "Yes" : "No"];
	ar[2] = ['Compiler', VC_VERSIONS[VCVERS]];
	ar[3] = ['Architecture', X64 ? 'x64' : 'x86'];
	if (JONJ_PGO == "yes") {
		ar[4] = ['Optimization', "PGO"];
	} else if (JONJ_PGI == "yes") {
		ar[4] = ['Optimization', "PGI"];
	} else {
		ar[4] = ['Optimization', JONJ_DEBUG == "yes" ? "disabled" : "PGO disabled"];
	}
	if (JONJ_ANALYZER == "vs") {
...
function generate_files()
{
	...
	if (JONJ_SNAPSHOT_BUILD != "no") {
		STDOUT.WriteLine("Type 'nmake snap' to build a JONJ snapshot");
	} else {
		STDOUT.WriteLine("Type 'nmake' to build JONJ");
	}
}
...
function generate_ext_pickle()
{
	...
	var build_var_name = function(name) {
		return "JONJ_" + name.toUpperCase();
	}

	STDOUT.WriteLine("Generating pickle deps");
	dest = JONJ_DIR + "/script/";
...
function generate_config_pickle_h()
{
	...
	dest = JONJ_DIR + "/include/main";

	var ignore_key = function(key) {
		var ignores = [ "CONFIGURE_COMMAND", "JONJ_COMPILER_ID", "COMPILER", "ARCHITECTURE", "HAVE_STRNLEN", "JONJ_DIR" ];
...
function generate_config_h()
{
	...
	prefix = JONJ_PREFIX.replace(new RegExp("\\\\", "g"), "\\\\");
...
function generate_jonjize()
{
	STDOUT.WriteLine("Generating jonjize");
	...
	var MF = FSO.CreateTextFile(dest + "/jonjize.js", true);
	var DEPS = FSO.CreateTextFile(dest + "/ext_deps.js", true);
	
	prefix = get_define("JONJ_PREFIX");
	prefix = prefix.replace(new RegExp("/", "g"), "\\");
	prefix = prefix.replace(new RegExp("\\\\", "g"), "\\\\");
	MF.WriteLine("var JONJ_PREFIX=" + '"' + prefix + '"');
	MF.WriteLine("var JONJ_ZTS=" + '"' + (JONJ_ZTS.toLowerCase() == "yes" ? "Yes" : "No") + '"');
	MF.WriteLine("var VC_VERSION=" + VCVERS);
	MF.WriteLine("var JONJ_VERSION=" + JONJ_VERSION);
	MF.WriteLine("var JONJ_MINOR_VERSION=" + JONJ_MINOR_VERSION);
	MF.WriteLine("var JONJ_RELEASE_VERSION=" + JONJ_RELEASE_VERSION);
	...
	for (i in extensions_enabled) {
		out = "JONJ_" + extensions_enabled[i][0].toUpperCase() + "_SHARED=" + (extensions_enabled[i][1] == 'shared' ? 'true' : 'false') + ";";
		DEPS.WriteLine("JONJ_" + extensions_enabled[i][0].toUpperCase() + "=true;");
		DEPS.WriteLine(out);
		MF.WriteLine(out);
	}

	MF.WriteBlankLines(2);
	MF.WriteLine("/* Genereted win32/build/jonjize.js.in */");
	MF.WriteBlankLines(1);
	MF.Write(file_get_contents("win32/build/jonjize.js.in"));
	MF.Close();
	DEPS.Close();

	/* Generate flags file */
	/* spit out variable definitions */
	CJ = FSO.CreateTextFile(dest + "/config.jonjize.js");

	CJ.WriteLine("var JONJ_ZTS =" + '"' + JONJ_ZTS + '"');
	CJ.WriteLine("var JONJ_DLL_LIB =" + '"' + get_define('JONJLIB') + '"');
	CJ.WriteLine("var JONJ_DLL =" + '"' + get_define('JONJDLL') + '"');

	/* The corresponding configure options aren't enabled through jonjize,
		thus these dummy declarations are required. */
	CJ.WriteLine("var JONJ_ANALYZER =" + '"no"');
	CJ.WriteLine("var JONJ_PGO =" + '"no"');
	CJ.WriteLine("var JONJ_PGI =" + '"no"');
	CJ.WriteLine("var JONJ_ALL_SHARED =" + '"no"');
...
function generate_makefile()
{
	...
	var i;
	MF.WriteLine("JONJ_SRC_DIR =" + JONJ_SRC_DIR);
	for (i in keys) {
	...
	MF.WriteBlankLines(1);
	if (MODE_JONJIZE) {
		var TF = FSO.OpenTextFile(JONJ_DIR + "/script/Makefile.jonjize", 1);
	} else {
	...
	if (MODE_JONJIZE) {
		...
		for (var i in extensions_enabled) {
			var lib = "jonj_" + extensions_enabled[i][0] + ".lib";
			var dll = "jonj_" + extensions_enabled[i][0] + ".dll";
			MF.WriteLine("	@copy $(BUILD_DIR)\\" + lib + " $(BUILD_DIR_DEV)\\lib");
			MF.WriteLine("  @if not exist $(JONJ_PREFIX) mkdir $(JONJ_PREFIX) >nul");
			MF.WriteLine("  @if not exist $(JONJ_PREFIX)\\ext mkdir $(JONJ_PREFIX)\\ext >nul");
			MF.WriteLine("	@copy $(BUILD_DIR)\\" + dll + " $(JONJ_PREFIX)\\ext");
		}
	} else {
		MF.WriteBlankLines(1);
		...
		for (var i in extensions_enabled) {
			...
			lib = "jonj_" + extensions_enabled[i][0] + "*.lib";
...
function ADD_FLAG(name, flags, target)
{
	...
	if (JONJ_DSP != "no") {
		if (flags && (name.substr(name.length-3) != "JONJ") && (name.substr(0, 7) == "CFLAGS_")) {
...
{
	if (!configure_subst.Exists("JONJDEF")) {
		DEFINE("JONJDEF", "$(BUILD_DIR)\\$(JONJDLL).def");
		ADD_FLAG("JONJ_LDFLAGS", "/def:$(JONJDEF)");
	}
	ADD_FLAG("JONJ_DLL_DEF_SOURCES", name);
}
...
function JONJ_INSTALL_HEADERS(dir, headers_list)
{
