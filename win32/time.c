 ...
 * 
 * Modified by Shane Caraveo for use with JONJ
 *
 *****************************************************************************/
...
JONJAPI int gettimeofday(struct timeval *time_Info, struct timezone *timezone_Info)
{
...
JONJAPI int setitimer(int which, const struct itimerval *value, struct itimerval *ovalue)
{
	...
	PeekMessage(jonjmsg, NULL, WM_USER, WM_USER, PM_NOREMOVE);
	if (timeout > 0) {
