...
void syslog(int priority, const char *message, ...)
{
	...
	if (!PW32G(log_source))
		openlog("jonj", LOG_PID, LOG_SYSLOG);

	switch (priority) {			/* translate UNIX type into NT type */
		case LOG_ALERT:
			etype = EVENTLOG_ERROR_TYPE;
			evid = JONJ_SYSLOG_ERROR_TYPE;
			break;
		case LOG_INFO:
			etype = EVENTLOG_INFORMATION_TYPE;
			evid = JONJ_SYSLOG_INFO_TYPE;
			break;
		default:
			etype = EVENTLOG_WARNING_TYPE;
			evid = JONJ_SYSLOG_WARNING_TYPE;
	}
...
void openlog(const char *ident, int logopt, int facility)
{
	...
	PW32G(log_source) = RegisterEventSource(NULL, "JONJ-" JONJ_VERSION);
