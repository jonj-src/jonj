...
ZEND_VM_HELPER(zend_do_fcall_common_helper, ANY, ANY)
{
	...
	if (fbc->common.scope &&
		...
		if (fbc->common.fn_flags & ZEND_ACC_ALLOW_STATIC) {
			...
			/* FIXME: output identifiers properly */
			/* An internal function assumes $this is present and won't check that. So JONJ would crash by allowing the call. */
	...
	if (call->fbc->common.fn_flags & ZEND_ACC_STATIC) {
		...
		if (EG(This) &&
		    ...
		    /* We are calling method of the other (incompatible) class,
		       but passing $this. This is done for compatibility with jonj-4. */
...
ZEND_VM_HANDLER(52, ZEND_BOOL, CONST|TMP|VAR|CV, ANY)
{
	...
	SAVE_OPLINE();
	/* JONJ 3.0 returned "" for false and 1 for true, here we use 0 and 1 for now */
	...
	if ((OP1_TYPE == IS_CV || OP1_TYPE == IS_VAR) &&
		...
		if (array_ptr_ptr == NULL || array_ptr_ptr == &EG(uninitialized_zval_ptr)) {
			...
			if(Z_OBJ_HT_PP(array_ptr_ptr)->get_class_entry == NULL) {
				zend_error(E_WARNING, "foreach() cannot iterate over objects without JONJ class");
				ZEND_VM_JMP(EX(op_array)->opcodes+opline->op2.opline_num);
			}
...
ZEND_VM_HANDLER(143, ZEND_DECLARE_CONST, CONST, CONST)
{
	...
	c.module_number = JONJ_USER_CONSTANT;
