...
# define jonj_sys_stat(path, buf) jonj_sys_stat_ex(path, buf, 0)
# define jonj_sys_lstat(path, buf) jonj_sys_stat_ex(path, buf, 1)
CWD_API int jonj_sys_readlink(const char *link, char *target, size_t target_len);
#else
# define jonj_sys_stat stat
# define jonj_sys_lstat lstat
# ifdef HAVE_SYMLINK
# define jonj_sys_readlink(link, target, target_len) readlink(link, target, target_len)
# endif
#endif
...
#define REALPATH_CACHE_SIZE 0      /* disabled while jonj.ini isn't loaded */
...
#define VCWD_STAT(path, buff) jonj_sys_stat(path, buff)
