...
#else
ZEND_API zend_jonj_scanner_globals language_scanner_globals;
#endif
...
inline_char_handler:
	...
	while (1) {
		...
		if (YYCURSOR < YYLIMIT) {
			switch (*YYCURSOR) {
				case '?':
					if (CG(short_tags) || !strncasecmp((char*)YYCURSOR + 1, "jonj", 3) || (*(YYCURSOR + 1) == '=')) { /* Assume [ \t\n\r] follows "jonj" */
						break;
					}
					continue;
				case '%':
				...
				case 's':
				case 'S':
					/* Probably NOT an opening JONJ <script> tag, so don't end the HTML chunk yet
					 * If it is, the JONJ <script> tag rule checks for any HTML scanned before it */
					YYCURSOR--;
					yymore();
				default:
					continue;
...
#line 1686 "Zend/zend_language_scanner.l"
		{
	YYCTYPE *bracket = (YYCTYPE*)zend_memrchr(yytext, '<', yyleng - (sizeof("script language=jonj>") - 1));

	if (bracket != SCNG(yy_text)) {
		/* Handle previously scanned HTML, as possible <script> tags found are assumed to not be JONJ's */
		YYCURSOR = bracket;
		goto inline_html;
	}
...
#line 1929 "Zend/zend_language_scanner.l"
	...
	ZVAL_STRINGL(zendlval, yytext, yyleng, 0); /* no copying - intentional */
	BEGIN(INITIAL);
	return T_CLOSE_TAG;  /* implicit ';' at jonj-end tag */
}
