...
#ifndef IS_DIR_OK
#define IS_DIR_OK(state) (jonj_is_dir_ok(state) == 0)
#endif
...
CWD_API int jonj_sys_readlink(const char *link, char *target, size_t target_len){ /* {{{ */
	HINSTANCE kernel32;
	HANDLE hFile;
	DWORD dwRet;
...
CWD_API int jonj_sys_stat_ex(const char *path, struct stat *buf, int lstat) /* {{{ */
{
...
static int jonj_is_dir_ok(const cwd_state *state)  /* {{{ */
{
	struct stat buf;

	if (jonj_sys_stat(state->cwd, &buf) == 0 && S_ISDIR(buf.st_mode))
		return (0);
...
static int jonj_is_file_ok(const cwd_state *state)  /* {{{ */
{
	struct stat buf;

	if (jonj_sys_stat(state->cwd, &buf) == 0 && S_ISREG(buf.st_mode))
		return (0);
...
static int tsrm_realpath_r(char *path, int start, int len, int *ll, time_t *t, int use_realpath, int is_dir, int *link_is_dir TSRMLS_DC) /* {{{ */
{
	...
	while (1) {
		...
		if(save &&
			...
			if(isabsolute && substitutename_len > 4) {
				/* Do not resolve volumes (for now). A mounted point can
				   ...
				   all IO functions we use in jonj and its deps support
		...
		memcpy(tmp, path, len+1);
#else
		if (save && jonj_sys_lstat(path, &st) < 0) {
			if (use_realpath == CWD_REALPATH) {
				/* file not found */
				return -1;
			}
		...
		if (save && S_ISLNK(st.st_mode)) {
			if (++(*ll) > LINK_MAX || (j = jonj_sys_readlink(tmp, path, MAXPATHLEN)) < 0) {
...
{
	return virtual_file_ex(&CWDG(cwd), path, jonj_is_dir_ok, CWD_REALPATH TSRMLS_CC)?-1:0;
}
/* }}} */
...
{
	return virtual_filepath_ex(path, filepath, jonj_is_file_ok TSRMLS_CC);
}
/* }}} */
...
CWD_API int virtual_stat(const char *path, struct stat *buf TSRMLS_DC) /* {{{ */
{
	...
	retval = jonj_sys_stat(new_state.cwd, buf);
...
CWD_API int virtual_lstat(const char *path, struct stat *buf TSRMLS_DC) /* {{{ */
{
	...
	retval = jonj_sys_lstat(new_state.cwd, buf);
