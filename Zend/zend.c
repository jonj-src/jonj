...
static void jonj_scanner_globals_ctor(zend_jonj_scanner_globals *scanner_globals_p TSRMLS_DC) /* {{{ */
{
	memset(scanner_globals_p, 0, sizeof(*scanner_globals_p));
}
/* }}} */
...
static zend_bool jonj_auto_globals_create_globals(const char *name, uint name_len TSRMLS_DC) /* {{{ */
{
	zval *globals;
...
int zend_startup(zend_utility_functions *utility_functions, char **extensions TSRMLS_DC) /* {{{ */
{
	...
	extern zend_jonj_scanner_globals language_scanner_globals;
#endif
...
/* build with dtrace support */
	...
	ts_allocate_id(&language_scanner_globals_id, sizeof(zend_jonj_scanner_globals), (ts_allocate_ctor) jonj_scanner_globals_ctor, NULL);
	...
	*executor_globals->zend_constants = *GLOBAL_CONSTANTS_TABLE;
	...
	jonj_scanner_globals_ctor(&language_scanner_globals TSRMLS_CC);
	zend_set_default_compile_time_values(TSRMLS_C);
	EG(user_error_handler) = NULL;
	EG(user_exception_handler) = NULL;
#endif
	...
	zend_register_auto_global("GLOBALS", sizeof("GLOBALS") - 1, 1, jonj_auto_globals_create_globals TSRMLS_CC);
...
# endif
			...
			/* User error handler may include() additinal JONJ files.
			 * If an error was generated during comilation JONJ will compile
