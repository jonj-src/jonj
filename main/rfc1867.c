...
#if defined(JONJ_WIN32) && !defined(HAVE_ATOLL)
...
static char *jonj_ap_getword(const zend_encoding *encoding, char **line, char stop TSRMLS_DC);
static char *jonj_ap_getword_conf(const zend_encoding *encoding, char *str TSRMLS_DC);

static jonj_rfc1867_encoding_translation_t jonj_rfc1867_encoding_translation = dummy_encoding_translation;
static jonj_rfc1867_get_detect_order_t jonj_rfc1867_get_detect_order = NULL;
static jonj_rfc1867_set_input_encoding_t jonj_rfc1867_set_input_encoding = NULL;
static jonj_rfc1867_getword_t jonj_rfc1867_getword = jonj_ap_getword;
static jonj_rfc1867_getword_conf_t jonj_rfc1867_getword_conf = jonj_ap_getword_conf;
static jonj_rfc1867_basename_t jonj_rfc1867_basename = NULL;

JONJAPI int (*jonj_rfc1867_callback)(unsigned int event, void *event_data, void **extra TSRMLS_DC) = NULL;

static void safe_jonj_register_variable(char *var, char *strval, int val_len, zval *track_vars_array, zend_bool override_protection TSRMLS_DC);
...
void jonj_rfc1867_register_constants(TSRMLS_D) /* {{{ */
{
...
static void safe_jonj_register_variable(char *var, char *strval, int val_len, zval *track_vars_array, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	if (override_protection || !is_protected_variable(var TSRMLS_CC)) {
		jonj_register_variable_safe(var, strval, val_len, track_vars_array TSRMLS_CC);
	}
}
/* }}} */

static void safe_jonj_register_variable_ex(char *var, zval *val, zval *track_vars_array, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	if (override_protection || !is_protected_variable(var TSRMLS_CC)) {
		jonj_register_variable_ex(var, val, track_vars_array TSRMLS_CC);
	}
}
/* }}} */
...
{
	safe_jonj_register_variable(strvar, val, strlen(val), http_post_files, override_protection TSRMLS_CC);
}
/* }}} */
...
{
	safe_jonj_register_variable_ex(var, val, http_post_files, override_protection TSRMLS_CC);
}
/* }}} */
...
static multipart_buffer *multipart_buffer_new(char *boundary, int boundary_len TSRMLS_DC)
{
	...
	if (jonj_rfc1867_encoding_translation(TSRMLS_C)) {
		jonj_rfc1867_get_detect_order(&self->detect_order, &self->detect_order_size TSRMLS_CC);
	} else {
...
/* Free header entry */
static void jonj_free_hdr_entry(mime_header_entry *h)
{
	if (h->key) {
		efree(h->key);
	}
...
static char *jonj_mime_get_hdr_value(zend_llist header, char *key)
{
...
static char *jonj_ap_basename(const zend_encoding *encoding, char *path TSRMLS_DC)
{
	char *s = strrchr(path, '\\');
 ...
 */
static void *jonj_ap_memstr(char *haystack, int haystacklen, char *needle, int needlen, int partial)
{
	int len = haystacklen;
	char *ptr = haystack;
...
static int multipart_buffer_read(multipart_buffer *self, char *buf, int bytes, int *end TSRMLS_DC)
{
	...
	if ((bound = jonj_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 1))) {
		max = bound - self->buf_begin;
		if (end && jonj_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 0)) {
			*end = 1;
		}
	} else {
...
SAPI_API SAPI_POST_HANDLER_FUNC(rfc1867_post_handler) /* {{{ */
{
	...
	jonj_rfc1867_getword_t getword;
	jonj_rfc1867_getword_conf_t getword_conf;
	jonj_rfc1867_basename_t _basename;
	long count = 0;

	if (jonj_rfc1867_encoding_translation(TSRMLS_C) && internal_encoding) {
		getword = jonj_rfc1867_getword;
		getword_conf = jonj_rfc1867_getword_conf;
		_basename = jonj_rfc1867_basename;
	} else {
		getword = jonj_ap_getword;
		getword_conf = jonj_ap_getword_conf;
		_basename = jonj_ap_basename;
	}
	...
	if (!boundary) {
		...
		jonj_strtolower(content_type_lcase, content_type_len);
	...
	zend_llist_init(&header, sizeof(mime_header_entry), (llist_dtor_func_t) jonj_free_hdr_entry, 0);

	if (jonj_rfc1867_callback != NULL) {
		multipart_event_start event_start;

		event_start.content_length = SG(request_info).content_length;
		if (jonj_rfc1867_callback(MULTIPART_EVENT_START, &event_start, &event_extra_data TSRMLS_CC) == FAILURE) {
	...
	while (!multipart_buffer_eof(mbuff TSRMLS_CC))
	{
		...
		if ((cd = jonj_mime_get_hdr_value(header, "Content-Disposition"))) {
			char *pair = NULL;
			int end = 0;
			...
			if (!filename && param) {
				...
				if (++count <= PG(max_input_vars) && sapi_module.input_filter(PARSE_POST, param, &value, value_len, &new_val_len TSRMLS_CC)) {
					if (jonj_rfc1867_callback != NULL) {
						...
						if (jonj_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data TSRMLS_CC) == FAILURE) {
							efree(param);
							efree(value);
							continue;
						}
					...
					safe_jonj_register_variable(param, value, new_val_len, array_ptr, 0 TSRMLS_CC);
				} else {
					...
					if (jonj_rfc1867_callback != NULL) {
						...
						jonj_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data TSRMLS_CC);
					}
				}
			...
			if (!skip_upload && jonj_rfc1867_callback != NULL) {
				multipart_event_file_start event_file_start;

				event_file_start.post_bytes_processed = SG(read_post_bytes);
				event_file_start.name = param;
				event_file_start.filename = &filename;
				if (jonj_rfc1867_callback(MULTIPART_EVENT_FILE_START, &event_file_start, &event_extra_data TSRMLS_CC) == FAILURE) {
					temp_filename = "";
					efree(param);
					efree(filename);
					continue;
			...
			if (!cancel_upload) {
				...
				{
#endif
					fd = jonj_open_temporary_fd_ex(PG(upload_tmp_dir), "jonj", &temp_filename, 1 TSRMLS_CC);
					upload_cnt--;
					if (fd == -1) {
			...
			while (!cancel_upload && (blen > 0))
			{
				if (jonj_rfc1867_callback != NULL) {
					...
					if (jonj_rfc1867_callback(MULTIPART_EVENT_FILE_DATA, &event_file_data, &event_extra_data TSRMLS_CC) == FAILURE) {
			...
			if (jonj_rfc1867_callback != NULL) {
				...
				if (jonj_rfc1867_callback(MULTIPART_EVENT_FILE_END, &event_file_end, &event_extra_data TSRMLS_CC) == FAILURE) {
					cancel_upload = UPLOAD_ERROR_X;
				}
			}
			...
			if (!is_anonymous) {
				safe_jonj_register_variable(lbuf, s, strlen(s), NULL, 0 TSRMLS_CC);
			}
			...
			if (cancel_upload || !(cd = jonj_mime_get_hdr_value(header, "Content-Type"))) {
				cd = "";
			} else {
				/* fix for Opera 6.01 */
				s = strchr(cd, ';');
			...
			if (!is_anonymous) {
				safe_jonj_register_variable(lbuf, cd, strlen(cd), NULL, 0 TSRMLS_CC);
			}
...

			{
				...
				if (!is_anonymous) {
					...
					safe_jonj_register_variable_ex(param, &zfilename, NULL, 1 TSRMLS_CC);
				}
...

			{
				...
				if (!is_anonymous) {
					...
					safe_jonj_register_variable_ex(lbuf, &file_size, NULL, size_overflow TSRMLS_CC);
				}
...
fileupload_done:
	if (jonj_rfc1867_callback != NULL) {
		...
		jonj_rfc1867_callback(MULTIPART_EVENT_END, &event_end, &event_extra_data TSRMLS_CC);
	}
...
SAPI_API void jonj_rfc1867_set_multibyte_callbacks(
					jonj_rfc1867_encoding_translation_t encoding_translation,
					jonj_rfc1867_get_detect_order_t get_detect_order,
					jonj_rfc1867_set_input_encoding_t set_input_encoding,
					jonj_rfc1867_getword_t getword,
					jonj_rfc1867_getword_conf_t getword_conf,
					jonj_rfc1867_basename_t basename) /* {{{ */
{
	jonj_rfc1867_encoding_translation = encoding_translation;
	jonj_rfc1867_get_detect_order = get_detect_order;
	jonj_rfc1867_set_input_encoding = set_input_encoding;
	jonj_rfc1867_getword = getword;
	jonj_rfc1867_getword_conf = getword_conf;
	jonj_rfc1867_basename = basename;
}
/* }}} */
