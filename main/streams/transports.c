...
#define ERR_REPORT(out_err, fmt, arg) \
	...
	else { jonj_error_docref(NULL TSRMLS_CC, E_WARNING, fmt, arg); }

#define ERR_RETURN(out_err, local_err, fmt) \
	if (out_err) { *out_err = local_err; } \
	else { jonj_error_docref(NULL TSRMLS_CC, E_WARNING, fmt, local_err ? local_err : "Unspecified error"); \
		...
		STREAMS_DC TSRMLS_DC)
{
	...
	jonj_stream_transport_factory *factory = NULL;
	...
	if (persistent_id) {
		switch(jonj_stream_from_persistent_id(persistent_id, &stream TSRMLS_CC)) {
			case JONJ_STREAM_PERSISTENT_SUCCESS:
				/* use a 0 second timeout when checking if the socket
				 * has already died */
				if (JONJ_STREAM_OPTION_RETURN_OK == jonj_stream_set_option(stream, JONJ_STREAM_OPTION_CHECK_LIVENESS, 0, NULL)) {
					return stream;
				}
				/* dead - kill it */
	...
	if (protocol) {
		...
		if (FAILURE == zend_hash_find(&xport_hash, (char*)tmp, n + 1, (void**)&factory)) {
			...
			ERR_REPORT(error_string, "Unable to find the socket transport \"%s\" - did you forget to enable it when you configured JONJ?",
					wrapper_name);
	...
	if (factory == NULL) {
		/* should never happen */
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not find a factory !?");
		return NULL;
	}
	...
	if (stream) {
		...
		if ((flags & STREAM_XPORT_SERVER) == 0) {
			...
			if (flags & (STREAM_XPORT_CONNECT|STREAM_XPORT_CONNECT_ASYNC)) {
				if (-1 == jonj_stream_xport_connect(stream, name, namelen,
			...
			if (flags & STREAM_XPORT_BIND) {
				if (0 != jonj_stream_xport_bind(stream, name, namelen, &error_text TSRMLS_CC)) {
					...
					if (stream->context && jonj_stream_context_get_option(stream->context, "socket", "backlog", &zbacklog) == SUCCESS) {
						zval *ztmp = *zbacklog;
					...
					if (0 != jonj_stream_xport_listen(stream, backlog, &error_text TSRMLS_CC)) {
		...
		TSRMLS_DC)
{
	...
	ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_XPORT_API, 0, &param);

	if (ret == JONJ_STREAM_OPTION_RETURN_OK) {
		if (error_text) {
...
/* Prepare to listen */
JONJAPI int jonj_stream_xport_listen(jonj_stream *stream, int backlog, char **error_text TSRMLS_DC)
{
	jonj_stream_xport_param param;
	int ret;
...
{
	jonj_stream_xport_crypto_param param;
	int ret;
	...
	ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_CRYPTO_API, 0, &param);

	if (ret == JONJ_STREAM_OPTION_RETURN_OK) {
		return param.outputs.returncode;
	}

	jonj_error_docref("streams.crypto" TSRMLS_CC, E_WARNING, "this stream does not support SSL/crypto");
		...
		TSRMLS_DC)
{
	...
	if (flags == 0 && addr == NULL) {
		return jonj_stream_read(stream, buf, buflen);
	}

	if (stream->readfilters.head) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot peek or fetch OOB data from a filtered stream");
		return -1;
	}
		...
		long flags, void *addr, socklen_t addrlen TSRMLS_DC)
{
...
#if 0
	if (flags == 0 && addr == NULL) {
		return jonj_stream_write(stream, buf, buflen);
	}
#endif
	...
	if ((oob || addr) && stream->writefilters.head) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot write OOB data, or data to a targeted address on a filtered stream");
		return -1;
	}
