...
#include "ext/standard/jonj_string.h" /* for jonj_memnstr, used by jonj_stream_get_record() */
#include <stddef.h>
#include <fcntl.h>
...
static int _jonj_stream_release_context(zend_rsrc_list_entry *le, void *pContext TSRMLS_DC)
{
	if (le->ptr == pContext) {
...
fprintf(stderr, "forget_persistent: %s:%p\n", stream->ops->label, stream);
	...
	if (stream->context) {
		zend_hash_apply_with_argument(&EG(regular_list),
				(apply_func_arg_t) _jonj_stream_release_context,
...
{
	jonj_stream *orig = enclosed->enclosing_stream;

	jonj_stream_auto_cleanup(enclosed);
...
JONJAPI int jonj_stream_from_persistent_id(const char *persistent_id, jonj_stream **stream TSRMLS_DC)
{
	...
	if (zend_hash_find(&EG(persistent_list), (char*)persistent_id, strlen(persistent_id)+1, (void*) &le) == SUCCESS) {
		if (Z_TYPE_P(le) == le_pstream) {
			...
			return JONJ_STREAM_PERSISTENT_SUCCESS;
		}
		return JONJ_STREAM_PERSISTENT_FAILURE;
	}
	return JONJ_STREAM_PERSISTENT_NOT_EXIST;
}

/* }}} */

static zend_llist *jonj_get_wrapper_errors_list(jonj_stream_wrapper *wrapper TSRMLS_DC)
{
...
void jonj_stream_display_wrapper_errors(jonj_stream_wrapper *wrapper, const char *path, const char *caption TSRMLS_DC)
{
	...
	if (wrapper) {
		zend_llist *err_list = jonj_get_wrapper_errors_list(wrapper TSRMLS_CC);
		if (err_list) {
			size_t l = 0;
			int brlen;
			int i;
			...
			if (wrapper == &jonj_plain_files_wrapper) {
	...
	jonj_strip_url_passwd(tmp);
	jonj_error_docref1(NULL TSRMLS_CC, tmp, E_WARNING, "%s: %s", caption, msg);
	efree(tmp);
	if (free_msg) {
		efree(msg);
	}
...
void jonj_stream_tidy_wrapper_error_log(jonj_stream_wrapper *wrapper TSRMLS_DC)
{
...
JONJAPI void jonj_stream_wrapper_log_error(jonj_stream_wrapper *wrapper, int options TSRMLS_DC, const char *fmt, ...)
{
	va_list args;
	char *buffer = NULL;
	...
	if (options & REPORT_ERRORS || wrapper == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", buffer);
		efree(buffer);
	} else {
		zend_llist *list = NULL;
		if (!FG(wrapper_errors)) {
...
JONJAPI jonj_stream *_jonj_stream_alloc(jonj_stream_ops *ops, void *abstract, const char *persistent_id, const char *mode STREAMS_DC TSRMLS_DC) /* {{{ */
{
	jonj_stream *ret;

	ret = (jonj_stream*) pemalloc_rel_orig(sizeof(jonj_stream), persistent_id ? 1 : 0);

	memset(ret, 0, sizeof(jonj_stream));
...
fprintf(stderr, "stream_alloc: %s:%p persistent=%s\n", ops->label, ret, persistent_id);
	...
	if (FG(auto_detect_line_endings)) {
		ret->flags |= JONJ_STREAM_FLAG_DETECT_EOL;
	}
...
{
	return _jonj_stream_free(stream_enclosed,
		close_options | JONJ_STREAM_FREE_IGNORE_ENCLOSING TSRMLS_CC);
}
/* }}} */

#if STREAM_DEBUG
static const char *_jonj_stream_pretty_free_options(int close_options, char *out)
{
	if (close_options & JONJ_STREAM_FREE_CALL_DTOR)
		strcat(out, "CALL_DTOR, ");
	if (close_options & JONJ_STREAM_FREE_RELEASE_STREAM)
		strcat(out, "RELEASE_STREAM, ");
	if (close_options & JONJ_STREAM_FREE_PRESERVE_HANDLE)
		strcat(out, "PREVERSE_HANDLE, ");
	if (close_options & JONJ_STREAM_FREE_RSRC_DTOR)
		strcat(out, "RSRC_DTOR, ");
	if (close_options & JONJ_STREAM_FREE_PERSISTENT)
		strcat(out, "PERSISTENT, ");
	if (close_options & JONJ_STREAM_FREE_IGNORE_ENCLOSING)
...
static int _jonj_stream_free_persistent(zend_rsrc_list_entry *le, void *pStream TSRMLS_DC)
{
...
{
	int ret = 1;
	int preserve_handle = close_options & JONJ_STREAM_FREE_PRESERVE_HANDLE ? 1 : 0;
	int release_cast = 1;
	jonj_stream_context *context = NULL;
	...
	if (stream->flags & JONJ_STREAM_FLAG_NO_CLOSE) {
		preserve_handle = 1;
	}
...
#if STREAM_DEBUG
	{
		...
		fprintf(stderr, "stream_free: %s:%p[%s] in_free=%d opts=%s\n",
			stream->ops->label, stream, stream->orig_path, stream->in_free, _jonj_stream_pretty_free_options(close_options, out));
	}
	...
	if (stream->in_free) {
		...
		if ((stream->in_free == 1) && (close_options & JONJ_STREAM_FREE_IGNORE_ENCLOSING) && (stream->enclosing_stream == NULL)) {
			close_options |= JONJ_STREAM_FREE_RSRC_DTOR; /* restore flag */
		} else {
	...
	if ((close_options & JONJ_STREAM_FREE_RSRC_DTOR) &&
			!(close_options & JONJ_STREAM_FREE_IGNORE_ENCLOSING) &&
			(close_options & (JONJ_STREAM_FREE_CALL_DTOR | JONJ_STREAM_FREE_RELEASE_STREAM)) && /* always? */
			(stream->enclosing_stream != NULL)) {
		jonj_stream *enclosing_stream = stream->enclosing_stream;
		stream->enclosing_stream = NULL;
		/* we force JONJ_STREAM_CALL_DTOR because that's from where the
		...
		return _jonj_stream_free(enclosing_stream,
			(close_options | JONJ_STREAM_FREE_CALL_DTOR) & ~JONJ_STREAM_FREE_RSRC_DTOR TSRMLS_CC);
	}
	 ...
	 * */
	if (preserve_handle) {
		if (stream->fclose_stdiocast == JONJ_STREAM_FCLOSE_FOPENCOOKIE) {
...
fprintf(stderr, "stream_free: %s:%p[%s] preserve_handle=%d release_cast=%d remove_rsrc=%d\n",
		...
		(close_options & JONJ_STREAM_FREE_RSRC_DTOR) == 0);
#endif

	/* make sure everything is saved */
	_jonj_stream_flush(stream, 1 TSRMLS_CC);

	/* If not called from the resource dtor, remove the stream from the resource list. */
	if ((close_options & JONJ_STREAM_FREE_RSRC_DTOR) == 0) {
		 ...
		 * not only when JONJ_STREAM_FREE_RELEASE_STREAM is set */
		while (zend_list_delete(stream->rsrc_id) == SUCCESS) {}
	}

	if (close_options & JONJ_STREAM_FREE_CALL_DTOR) {
		if (release_cast && stream->fclose_stdiocast == JONJ_STREAM_FCLOSE_FOPENCOOKIE) {
			/* calling fclose on an fopencookied stream will ultimately
				...
				we can be sure that we only reach here when JONJ code calls
				jonj_stream_free.
		...
		if (release_cast && stream->fclose_stdiocast == JONJ_STREAM_FCLOSE_FDOPEN && stream->stdiocast) {
	...
	if (close_options & JONJ_STREAM_FREE_RELEASE_STREAM) {
		while (stream->readfilters.head) {
			jonj_stream_filter_remove(stream->readfilters.head, 1 TSRMLS_CC);
		}
		while (stream->writefilters.head) {
			jonj_stream_filter_remove(stream->writefilters.head, 1 TSRMLS_CC);
		}
		...
		if (stream->is_persistent && (close_options & JONJ_STREAM_FREE_PERSISTENT)) {
			/* we don't work with *stream but need its value for comparison */
			zend_hash_apply_with_argument(&EG(persistent_list), (apply_func_arg_t) _jonj_stream_free_persistent, stream TSRMLS_CC);
		}
#if ZEND_DEBUG
		if ((close_options & JONJ_STREAM_FREE_RSRC_DTOR) && (stream->__exposed == 0) && (EG(error_reporting) & E_WARNING)) {
...
JONJAPI void _jonj_stream_fill_read_buffer(jonj_stream *stream, size_t size TSRMLS_DC)
{
	...
	if (stream->readfilters.head) {
		char *chunk_buf;
		int err_flag = 0;
		jonj_stream_bucket_brigade brig_in = { NULL, NULL }, brig_out = { NULL, NULL };
		jonj_stream_bucket_brigade *brig_inp = &brig_in, *brig_outp = &brig_out, *brig_swap;

		/* Invalidate the existing cache, otherwise reads can fail, see note in
		   main/streams/filter.c::_jonj_stream_filter_append */
		...
		while (!stream->eof && !err_flag && (stream->writepos - stream->readpos < (off_t)size)) {
			...
			jonj_stream_filter_status_t status = PSFS_ERR_FATAL;
			...
			if (justread && justread != (size_t)-1) {
				bucket = jonj_stream_bucket_new(stream, chunk_buf, justread, 0, 0 TSRMLS_CC);

				/* after this call, bucket is owned by the brigade */
				jonj_stream_bucket_append(brig_inp, bucket TSRMLS_CC);
...
JONJAPI size_t _jonj_stream_read(jonj_stream *stream, char *buf, size_t size TSRMLS_DC)
{
	...
	while (size > 0) {
		...
		if (!stream->readfilters.head && (stream->flags & JONJ_STREAM_FLAG_NO_BUFFER || stream->chunk_size == 1)) {
			...
			jonj_stream_fill_read_buffer(stream, size);
		...
		if (stream->wrapper != &jonj_plain_files_wrapper) {
			break;
		}
	}
...
JONJAPI int _jonj_stream_eof(jonj_stream *stream TSRMLS_DC)
{
	...
	if (!stream->eof && JONJ_STREAM_OPTION_RETURN_ERR ==
		   	jonj_stream_set_option(stream, JONJ_STREAM_OPTION_CHECK_LIVENESS,
		   	0, NULL)) {
		stream->eof = 1;
	}
...
JONJAPI int _jonj_stream_putc(jonj_stream *stream, int c TSRMLS_DC)
{
	...
	if (jonj_stream_write(stream, &buf, 1) > 0) {
		return 1;
	}
	return EOF;
}
...
JONJAPI int _jonj_stream_getc(jonj_stream *stream TSRMLS_DC)
{
	...
	if (jonj_stream_read(stream, &buf, 1) > 0) {
		return buf & 0xff;
	}
	return EOF;
}
...
JONJAPI int _jonj_stream_puts(jonj_stream *stream, const char *buf TSRMLS_DC)
{
	...
	if (len > 0 && jonj_stream_write(stream, buf, len) && jonj_stream_write(stream, newline, 1)) {
		return 1;
	}
	return 0;
}
...
JONJAPI const char *jonj_stream_locate_eol(jonj_stream *stream, const char *buf, size_t buf_len TSRMLS_DC)
{
	...
	/* Look for EOL */
	if (stream->flags & JONJ_STREAM_FLAG_DETECT_EOL) {
		cr = memchr(readptr, '\r', avail);
		lf = memchr(readptr, '\n', avail);

		if (cr && lf != cr + 1 && !(lf && lf < cr)) {
			/* mac */
			stream->flags ^= JONJ_STREAM_FLAG_DETECT_EOL;
			stream->flags |= JONJ_STREAM_FLAG_EOL_MAC;
			eol = cr;
	...
	} else if (stream->flags & JONJ_STREAM_FLAG_EOL_MAC) {
 ...
 * permitting */
JONJAPI char *_jonj_stream_get_line(jonj_stream *stream, char *buf, size_t maxlen,
		size_t *returned_len TSRMLS_DC)
{
	...
	for (;;) {
		...
		if (avail > 0) {
			...
			readptr = stream->readbuf + stream->readpos;
			eol = jonj_stream_locate_eol(stream, NULL, 0 TSRMLS_CC);
			...
			jonj_stream_fill_read_buffer(stream, toread);
...
static const char *_jonj_stream_search_delim(jonj_stream *stream,
											size_t maxlen,
											size_t skiplen,
											...
											size_t delim_len TSRMLS_DC)
{
	...
	if (delim_len == 1) {
		...
		return jonj_memnstr((char*)&stream->readbuf[stream->readpos + skiplen],
				delim, delim_len,
...
JONJAPI char *jonj_stream_get_record(jonj_stream *stream, size_t maxlen, size_t *returned_len, const char *delim, size_t delim_len TSRMLS_DC)
{
	...
	if (has_delim) {
		found_delim = _jonj_stream_search_delim(
	...
	while (!found_delim && buffered_len < maxlen) {
		...
		jonj_stream_fill_read_buffer(stream, buffered_len + to_read_now);
	...
	/* jonj_stream_read will not call ops->read here because the necessary
	 * data is guaranteedly buffered */
	*returned_len = jonj_stream_read(stream, ret_buf, tent_ret_len);
...
static size_t _jonj_stream_write_buffer(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (stream->ops->seek && (stream->flags & JONJ_STREAM_FLAG_NO_SEEK) == 0 && stream->readpos != stream->writepos) {
 ...
 * */
static size_t _jonj_stream_write_filtered(jonj_stream *stream, const char *buf, size_t count, int flags TSRMLS_DC)
{
	size_t consumed = 0;
	jonj_stream_bucket *bucket;
	jonj_stream_bucket_brigade brig_in = { NULL, NULL }, brig_out = { NULL, NULL };
	...
	if (buf) {
		bucket = jonj_stream_bucket_new(stream, (char *)buf, count, 0, 0 TSRMLS_CC);
		jonj_stream_bucket_append(&brig_in, bucket TSRMLS_CC);
	}
	...
	switch (status) {
		case PSFS_PASS_ON:
			...
			while (brig_inp->head) {
				bucket = brig_inp->head;
				_jonj_stream_write_buffer(stream, bucket->buf, bucket->buflen TSRMLS_CC);
...
JONJAPI int _jonj_stream_flush(jonj_stream *stream, int closing TSRMLS_DC)
{
	...
	if (stream->writefilters.head) {
		_jonj_stream_write_filtered(stream, NULL, 0, closing ? PSFS_FLAG_FLUSH_CLOSE : PSFS_FLAG_FLUSH_INC  TSRMLS_CC);
	}
...
JONJAPI size_t _jonj_stream_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (stream->writefilters.head) {
		return _jonj_stream_write_filtered(stream, buf, count, PSFS_FLAG_NORMAL TSRMLS_CC);
	} else {
		return _jonj_stream_write_buffer(stream, buf, count TSRMLS_CC);
	}
}

JONJAPI size_t _jonj_stream_printf(jonj_stream *stream TSRMLS_DC, const char *fmt, ...)
{
	size_t count;
	char *buf;
	va_list ap;
	...
	count = jonj_stream_write(stream, buf, count);
	efree(buf);
...
JONJAPI int _jonj_stream_seek(jonj_stream *stream, off_t offset, int whence TSRMLS_DC)
{
	...
	if ((stream->flags & JONJ_STREAM_FLAG_NO_BUFFER) == 0) {
		switch(whence) {
			case SEEK_CUR:
	...
	if (stream->ops->seek && (stream->flags & JONJ_STREAM_FLAG_NO_SEEK) == 0) {
		...
		if (stream->writefilters.head) {
			_jonj_stream_flush(stream, 0 TSRMLS_CC);
		}
		...
		if (((stream->flags & JONJ_STREAM_FLAG_NO_SEEK) == 0) || ret == 0) {
			if (ret == 0) {
				stream->eof = 0;
			}
	...
	if (whence == SEEK_CUR && offset >= 0) {
		...
		size_t didread;
		while(offset > 0) {
			if ((didread = jonj_stream_read(stream, tmp, MIN(offset, sizeof(tmp)))) == 0) {
				return -1;
			}
			offset -= didread;
		}
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "stream does not support seeking");
...
{
	int ret = JONJ_STREAM_OPTION_RETURN_NOTIMPL;

	if (stream->ops->set_option) {
		ret = stream->ops->set_option(stream, option, value, ptrparam TSRMLS_CC);
	}

	if (ret == JONJ_STREAM_OPTION_RETURN_NOTIMPL) {
		switch(option) {
			case JONJ_STREAM_OPTION_SET_CHUNK_SIZE:
				ret = stream->chunk_size;
				stream->chunk_size = value;
				return ret;

			case JONJ_STREAM_OPTION_READ_BUFFER:
				/* try to match the buffer mode as best we can */
				if (value == JONJ_STREAM_BUFFER_NONE) {
					stream->flags |= JONJ_STREAM_FLAG_NO_BUFFER;
				} else if (stream->flags & JONJ_STREAM_FLAG_NO_BUFFER) {
					stream->flags ^= JONJ_STREAM_FLAG_NO_BUFFER;
				}
				ret = JONJ_STREAM_OPTION_RETURN_OK;
				break;
...
{
	return jonj_stream_set_option(stream, JONJ_STREAM_OPTION_TRUNCATE_API, JONJ_STREAM_TRUNCATE_SET_SIZE, &newsize);
}

JONJAPI size_t _jonj_stream_passthru(jonj_stream * stream STREAMS_DC TSRMLS_DC)
{
	size_t bcount = 0;
	char buf[8192];
	int b;

	if (jonj_stream_mmap_possible(stream)) {
		char *p;
		size_t mapped;

		p = jonj_stream_mmap_range(stream, jonj_stream_tell(stream), JONJ_STREAM_MMAP_ALL, JONJ_STREAM_MAP_MODE_SHARED_READONLY, &mapped);

		if (p) {
			do {
				/* output functions return int, so pass in int max */
				if (0 < (b = JONJWRITE(p + bcount, MIN(mapped - bcount, INT_MAX)))) {
					bcount += b;
				}
			} while (b > 0 && mapped > bcount);

			jonj_stream_mmap_unmap_ex(stream, mapped);

			return bcount;
		}
	}

	while ((b = jonj_stream_read(stream, buf, sizeof(buf))) > 0) {
		JONJWRITE(buf, b);
		bcount += b;
	}
...
JONJAPI size_t _jonj_stream_copy_to_mem(jonj_stream *src, char **buf, size_t maxlen, int persistent STREAMS_DC TSRMLS_DC)
{
	size_t ret = 0;
	char *ptr;
	...
	jonj_stream_statbuf ssbuf;

	if (maxlen == 0) {
		return 0;
	}

	if (maxlen == JONJ_STREAM_COPY_ALL) {
		maxlen = 0;
	}

	if (maxlen > 0) {
		ptr = *buf = pemalloc_rel_orig(maxlen + 1, persistent);
		while ((len < maxlen) && !jonj_stream_eof(src)) {
			ret = jonj_stream_read(src, ptr, maxlen - len);
			if (!ret) {
				break;
			}
			len += ret;
	 ...
	 * 2K).  */
	if (jonj_stream_stat(src, &ssbuf) == 0 && ssbuf.sb.st_size > 0) {
	...
	while((ret = jonj_stream_read(src, ptr, max_len - len)))	{
		len += ret;
...
JONJAPI int _jonj_stream_copy_to_stream_ex(jonj_stream *src, jonj_stream *dest, size_t maxlen, size_t *len STREAMS_DC TSRMLS_DC)
{
	...
	if (jonj_stream_stat(src, &ssbuf) == 0) {
	...
	if (jonj_stream_mmap_possible(src)) {
		char *p;
		size_t mapped;

		p = jonj_stream_mmap_range(src, jonj_stream_tell(src), maxlen, JONJ_STREAM_MAP_MODE_SHARED_READONLY, &mapped);

		if (p) {
			didwrite = jonj_stream_write(dest, p, mapped);

			jonj_stream_mmap_unmap_ex(src, mapped);
	...
	while(1) {
		...
		didread = jonj_stream_read(src, buf, readchunk);
		...
		if (didread) {
			...
			while(towrite) {
				didwrite = jonj_stream_write(dest, writeptr, towrite);
				if (didwrite == 0) {
 ...
 * Deprecated in favor of jonj_stream_copy_to_stream_ex() */
ZEND_ATTRIBUTE_DEPRECATED
JONJAPI size_t _jonj_stream_copy_to_stream(jonj_stream *src, jonj_stream *dest, size_t maxlen STREAMS_DC TSRMLS_DC)
{
	size_t len;
	int ret = _jonj_stream_copy_to_stream_ex(src, dest, maxlen, &len STREAMS_REL_CC TSRMLS_CC);
...
{
	jonj_stream *stream = (jonj_stream*)rsrc->ptr;
	/* set the return value for pclose */
	FG(pclose_ret) = jonj_stream_free(stream, JONJ_STREAM_FREE_CLOSE | JONJ_STREAM_FREE_RSRC_DTOR);
}
...
int jonj_init_stream_wrappers(int module_number TSRMLS_DC)
{
	...
	return (
			...
			&&
			zend_hash_init(jonj_get_stream_filters_hash_global(), 0, NULL, NULL, 1) == SUCCESS
			&&
			zend_hash_init(jonj_stream_xport_get_hash(), 0, NULL, NULL, 1) == SUCCESS
			&&
			jonj_stream_xport_register("tcp", jonj_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
			&&
			jonj_stream_xport_register("udp", jonj_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
#if defined(AF_UNIX) && !(defined(JONJ_WIN32) || defined(__riscos__) || defined(NETWARE))
			&&
			jonj_stream_xport_register("unix", jonj_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
			&&
			jonj_stream_xport_register("udg", jonj_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
#endif
		) ? SUCCESS : FAILURE;
}
...
int jonj_shutdown_stream_wrappers(int module_number TSRMLS_DC)
{
	...
	zend_hash_destroy(jonj_get_stream_filters_hash_global());
	zend_hash_destroy(jonj_stream_xport_get_hash());
	return SUCCESS;
}
 ...
 */
static inline int jonj_stream_wrapper_scheme_validate(const char *protocol, unsigned int protocol_len)
{
	unsigned int i;
...
JONJAPI int jonj_register_url_stream_wrapper(const char *protocol, jonj_stream_wrapper *wrapper TSRMLS_DC)
{
	...
	if (jonj_stream_wrapper_scheme_validate(protocol, protocol_len) == FAILURE) {
		return FAILURE;
	}
...
/* {{{ jonj_stream_locate_url_wrapper */
JONJAPI jonj_stream_wrapper *jonj_stream_locate_url_wrapper(const char *path, const char **path_for_open, int options TSRMLS_DC)
{
	...
	jonj_stream_wrapper **wrapperpp = NULL;
	...
	if (options & IGNORE_URL) {
		return (options & STREAM_LOCATE_WRAPPERS_ONLY) ? NULL : &jonj_plain_files_wrapper;
	}
	...
	if (protocol) {
		...
		if (FAILURE == zend_hash_find(wrapper_hash, (char*)tmp, n + 1, (void**)&wrapperpp)) {
			...
			if (FAILURE == zend_hash_find(wrapper_hash, (char*)tmp, n + 1, (void**)&wrapperpp)) {
				...
				JONJ_STRLCPY(wrapper_name, protocol, sizeof(wrapper_name), n);

				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find the wrapper \"%s\" - did you forget to enable it when you configured JONJ?", wrapper_name);
	...
	if (!protocol || !strncasecmp(protocol, "file", n))	{
		...
		jonj_stream_wrapper *plain_files_wrapper = &jonj_plain_files_wrapper;
		...
		if (protocol) {
			...
			if (localhost == 0 && path[n+3] != '\0' && path[n+3] != '/') {
				...
				if (options & REPORT_ERRORS) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "remote host file access not supported, %s", path);
				}
				return NULL;
			}
		...
		/* The file:// wrapper may have been disabled/overridden */
			...
			if (options & REPORT_ERRORS) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "file:// wrapper is disabled in the server configuration");
			}
			return NULL;
		}
	...
	if (wrapperpp && (*wrapperpp)->is_url &&
		...
		if (options & REPORT_ERRORS) {
			...
			if (!PG(allow_url_fopen)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s:// wrapper is disabled in the server configuration by allow_url_fopen=0", protocol_dup);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s:// wrapper is disabled in the server configuration by allow_url_include=0", protocol_dup);
			}
			efree(protocol_dup);
		}
		return NULL;
...
{
	jonj_stream_wrapper *wrapper = NULL;

	wrapper = jonj_stream_locate_url_wrapper(path, NULL, 0 TSRMLS_CC);
...
JONJAPI int _jonj_stream_stat_path(const char *path, int flags, jonj_stream_statbuf *ssb, jonj_stream_context *context TSRMLS_DC)
{
	...
	if (!(flags & JONJ_STREAM_URL_STAT_NOCACHE)) {
		/* Try to hit the cache first */
		if (flags & JONJ_STREAM_URL_STAT_LINK) {
			if (BG(CurrentLStatFile) && strcmp(path, BG(CurrentLStatFile)) == 0) {
				memcpy(ssb, &BG(lssb), sizeof(jonj_stream_statbuf));
				return 0;
			}
		} else {
			if (BG(CurrentStatFile) && strcmp(path, BG(CurrentStatFile)) == 0) {
				memcpy(ssb, &BG(ssb), sizeof(jonj_stream_statbuf));
				return 0;
			}
		}
	}

	wrapper = jonj_stream_locate_url_wrapper(path, &path_to_open, 0 TSRMLS_CC);
	if (wrapper && wrapper->wops->url_stat) {
		...
		if (ret == 0) {
				...
				if (flags & JONJ_STREAM_URL_STAT_LINK) {
					...
					memcpy(&BG(lssb), ssb, sizeof(jonj_stream_statbuf));
				} else {
					if (BG(CurrentStatFile)) {
						efree(BG(CurrentStatFile));
					}
					BG(CurrentStatFile) = estrdup(path);
					memcpy(&BG(ssb), ssb, sizeof(jonj_stream_statbuf));
				}
			}
		}
		return ret;
...
/* {{{ jonj_stream_opendir */
JONJAPI jonj_stream *_jonj_stream_opendir(const char *path, int options,
		jonj_stream_context *context STREAMS_DC TSRMLS_DC)
{
	...
	wrapper = jonj_stream_locate_url_wrapper(path, &path_to_open, options TSRMLS_CC);
	...
	if (wrapper && wrapper->wops->dir_opener) {
		...
		if (stream) {
			...
			stream->flags |= JONJ_STREAM_FLAG_NO_BUFFER | JONJ_STREAM_FLAG_IS_DIR;
		}
	} else if (wrapper) {
		jonj_stream_wrapper_log_error(wrapper, options ^ REPORT_ERRORS TSRMLS_CC, "not implemented");
	}
	if (stream == NULL && (options & REPORT_ERRORS)) {
		jonj_stream_display_wrapper_errors(wrapper, path, "failed to open dir" TSRMLS_CC);
	}
	jonj_stream_tidy_wrapper_error_log(wrapper TSRMLS_CC);

	return stream;
}
/* }}} */

/* {{{ _jonj_stream_readdir */
JONJAPI jonj_stream_dirent *_jonj_stream_readdir(jonj_stream *dirstream, jonj_stream_dirent *ent TSRMLS_DC)
{

	if (sizeof(jonj_stream_dirent) == jonj_stream_read(dirstream, (char*)ent, sizeof(jonj_stream_dirent))) {
		return ent;
	}
...
/* {{{ jonj_stream_open_wrapper_ex */
JONJAPI jonj_stream *_jonj_stream_open_wrapper_ex(const char *path, const char *mode, int options,
		char **opened_path, jonj_stream_context *context STREAMS_DC TSRMLS_DC)
{
	...
	if (options & STREAM_USE_URL && (!wrapper || !wrapper->is_url)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "This function may only be used against URLs");
		if (resolved_path) {
			efree(resolved_path);
		}
		return NULL;
	...
	if (wrapper) {
		if (!wrapper->wops->stream_opener) {
			jonj_stream_wrapper_log_error(wrapper, options ^ REPORT_ERRORS TSRMLS_CC,
	...
	if (stream != NULL && (options & STREAM_MUST_SEEK)) {
		...
		switch(jonj_stream_make_seekable_rel(stream, &newstream,
					(options & STREAM_WILL_CAST)
						? JONJ_STREAM_PREFER_STDIO : JONJ_STREAM_NO_PREFERENCE)) {
			case JONJ_STREAM_UNCHANGED:
				if (resolved_path) {
					efree(resolved_path);
				}
				return stream;
			case JONJ_STREAM_RELEASED:
			...
			default:
				...
				if (options & REPORT_ERRORS) {
					char *tmp = estrdup(path);
					jonj_strip_url_passwd(tmp);
					jonj_error_docref1(NULL TSRMLS_CC, tmp, E_WARNING, "could not make seekable - %s",
							tmp);
					efree(tmp);
	...
	if (stream && stream->ops->seek && (stream->flags & JONJ_STREAM_FLAG_NO_SEEK) == 0 && strchr(mode, 'a') && stream->position == 0) {
		off_t newpos = 0;
	...
	if (stream == NULL && (options & REPORT_ERRORS)) {
		jonj_stream_display_wrapper_errors(wrapper, path, "failed to open stream" TSRMLS_CC);
...
{
	jonj_stream_context *oldcontext = stream->context;
	TSRMLS_FETCH();
...
JONJAPI void jonj_stream_context_free(jonj_stream_context *context)
{
	...
	if (context->notifier) {
		jonj_stream_notification_free(context->notifier);
...
JONJAPI jonj_stream_context *jonj_stream_context_alloc(TSRMLS_D)
{
	...
	context = ecalloc(1, sizeof(jonj_stream_context));
	context->notifier = NULL;
	MAKE_STD_ZVAL(context->options);
	array_init(context->options);

	context->rsrc_id = ZEND_REGISTER_RESOURCE(NULL, context, jonj_le_stream_context(TSRMLS_C));
	return context;
}

JONJAPI jonj_stream_notifier *jonj_stream_notification_alloc(void)
{
	return ecalloc(1, sizeof(jonj_stream_notifier));
}
			  ...
			  int (*compare) (const char **a, const char **b) TSRMLS_DC)
{
	...
	stream = jonj_stream_opendir(dirname, REPORT_ERRORS, context);
	if (!stream) {
		return FAILURE;
	}

	while (jonj_stream_readdir(stream, &sdp)) {
