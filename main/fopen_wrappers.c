...
JONJAPI ZEND_INI_MH(OnUpdateBaseDir)
{
	...
	if (stage == JONJ_INI_STAGE_STARTUP || stage == JONJ_INI_STAGE_SHUTDOWN || stage == JONJ_INI_STAGE_ACTIVATE || stage == JONJ_INI_STAGE_DEACTIVATE) {
		/* We're in a JONJ_INI_SYSTEM context, no restrictions */
		*p = new_value;
		return SUCCESS;
	}
	...
	while (ptr && *ptr) {
		...
		if (jonj_check_open_basedir_ex(ptr, 0 TSRMLS_CC) != 0) {
...
/* {{{ jonj_check_specific_open_basedir
...
JONJAPI int jonj_check_specific_open_basedir(const char *basedir, const char *path TSRMLS_DC)
{
	...
	while (VCWD_REALPATH(path_tmp, resolved_name) == NULL) {
#if defined(JONJ_WIN32) || defined(HAVE_SYMLINK)
		...
		if (EG(windows_version_info).dwMajorVersion > 5) {
			...
			if (nesting_level == 0) {
				...
				ret = jonj_sys_readlink(path_tmp, buf, MAXPATHLEN - 1);
				if (ret < 0) {
	...
	if (expand_filepath(local_open_basedir, resolved_basedir TSRMLS_CC) != NULL) {
...
#if defined(JONJ_WIN32) || defined(NETWARE)
		if (basedir[strlen(basedir) - 1] == JONJ_DIR_SEPARATOR || basedir[strlen(basedir) - 1] == '/') {
#else
		if (basedir[strlen(basedir) - 1] == JONJ_DIR_SEPARATOR) {
#endif
			if (resolved_basedir[resolved_basedir_len - 1] != JONJ_DIR_SEPARATOR) {
				resolved_basedir[resolved_basedir_len] = JONJ_DIR_SEPARATOR;
		...
		if (path_tmp[path_len - 1] == JONJ_DIR_SEPARATOR) {
			if (resolved_name[resolved_name_len - 1] != JONJ_DIR_SEPARATOR) {
				resolved_name[resolved_name_len] = JONJ_DIR_SEPARATOR;
		...
		if (strncmp(resolved_basedir, resolved_name, resolved_basedir_len) == 0) {
			...
			if (resolved_name_len > resolved_basedir_len &&
				resolved_name[resolved_basedir_len - 1] != JONJ_DIR_SEPARATOR) {
				return -1;
			} else {
			...
			if (resolved_basedir_len == (resolved_name_len + 1) && resolved_basedir[resolved_basedir_len - 1] == JONJ_DIR_SEPARATOR) {
#if defined(JONJ_WIN32) || defined(NETWARE)
...
{
	return jonj_check_open_basedir_ex(path, 1 TSRMLS_CC);
}

/* {{{ jonj_check_open_basedir
 */
	...
	if (PG(open_basedir) && *PG(open_basedir)) {
		...
		* message. */
		if (strlen(path) > (MAXPATHLEN - 1)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "File name is longer than the maximum allowed path length on this platform (%d): %s", MAXPATHLEN, path);
			errno = EINVAL;
			return -1;
		}
		...
		while (ptr && *ptr) {
			...
			if (jonj_check_specific_open_basedir(ptr, path TSRMLS_CC) == 0) {
				efree(pathbuf);
				return 0;
			}
		...
		if (warn) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "open_basedir restriction in effect. File(%s) is not within the allowed path(s): (%s)", path, PG(open_basedir));
		}
		efree(pathbuf);
...
/* {{{ jonj_fopen_and_set_opened_path
 */
static FILE *jonj_fopen_and_set_opened_path(const char *path, const char *mode, char **opened_path TSRMLS_DC)
{
	FILE *fp;

	if (jonj_check_open_basedir((char *)path TSRMLS_CC)) {
		return NULL;
	}
...
JONJAPI int jonj_fopen_primary_script(zend_file_handle *file_handle TSRMLS_DC)
{
...
#endif
			if (pw && pw->pw_dir) {
				spprintf(&filename, 0, "%s%c%s%c%s", pw->pw_dir, JONJ_DIR_SEPARATOR, PG(user_dir), JONJ_DIR_SEPARATOR, s + 1); /* Safe */
			} else {
	...
	if (PG(doc_root) && path_info && (length = strlen(PG(doc_root))) &&
		...
		if (filename) {
			...
			if (!IS_SLASH(filename[length - 1])) {	/* length is never 0 */
				filename[length++] = JONJ_DIR_SEPARATOR;
			}
	...
	if (!resolved_path) {
		 ...
		 * jonj_destroy_request_info assumes that it will get
 ...
 */
JONJAPI char *jonj_resolve_path(const char *filename, int filename_length, const char *path TSRMLS_DC)
{
	...
	if ((*p == ':') && (p - filename > 1) && (p[1] == '/') && (p[2] == '/')) {
		wrapper = jonj_stream_locate_url_wrapper(filename, &actual_path, STREAM_OPEN_FOR_INCLUDE TSRMLS_CC);
	...
	while (ptr && *ptr) {
		...
		if (is_stream_wrapper) {
			wrapper = jonj_stream_locate_url_wrapper(trypath, &actual_path, STREAM_OPEN_FOR_INCLUDE TSRMLS_CC);
			if (!wrapper) {
				continue;
			} else if (wrapper != &jonj_plain_files_wrapper) {
...
JONJAPI FILE *jonj_fopen_with_path(const char *filename, const char *mode, const char *path, char **opened_path TSRMLS_DC)
{
	...
	/* Absolute path open */
	 ...
	 || (!path || (path && !*path))
		...
		return jonj_fopen_and_set_opened_path(filename, mode, opened_path TSRMLS_CC);
	}
	...
	while (ptr && *ptr) {
		...
		fp = jonj_fopen_and_set_opened_path(trypath, mode, opened_path TSRMLS_CC);
		if (fp) {
			efree(pathbuf);
			return fp;
		}
...
/* {{{ jonj_strip_url_passwd
 */
JONJAPI char *jonj_strip_url_passwd(char *url)
{
 ...
 */
JONJAPI char *expand_filepath_with_mode(const char *filepath, char *real_path, const char *relative_to, size_t relative_to_len, int realpath_mode TSRMLS_DC)
{
	cwd_state new_state;
	char cwd[MAXPATHLEN];
	int copy_len;
