...
#ifdef JONJ_WIN32
# include "win32/jonj_inttypes.h"
#endif

/* for systems that need to override reading of environment variables */
void _jonj_import_environment_variables(zval *array_ptr TSRMLS_DC);
JONJAPI void (*jonj_import_environment_variables)(zval *array_ptr TSRMLS_DC) = _jonj_import_environment_variables;

JONJAPI void jonj_register_variable(char *var, char *strval, zval *track_vars_array TSRMLS_DC)
{
	jonj_register_variable_safe(var, strval, strlen(strval), track_vars_array TSRMLS_CC);
}

/* binary-safe version */
JONJAPI void jonj_register_variable_safe(char *var, char *strval, int str_len, zval *track_vars_array TSRMLS_DC)
{
	zval new_entry;
	assert(strval != NULL);
	...
	jonj_register_variable_ex(var, &new_entry, track_vars_array TSRMLS_CC);
}

JONJAPI void jonj_register_variable_ex(char *var_name, zval *val, zval *track_vars_array TSRMLS_DC)
{
	char *p = NULL;
		...
		while (1) {
			...
			if(++nest_level > PG(max_input_nesting_level)) {
				...
				if (!PG(display_errors)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Input variable nesting level exceeded %ld. To increase the limit change max_input_nesting_level in jonj.ini.", PG(max_input_nesting_level));
				}
				free_alloca(var_orig, use_heap);
				return;
			}
			...
			if (*ip==']') {
				...
				ip = strchr(ip, ']');
				if (!ip) {
					/* JONJ variables cannot contain '[' in their names, so we replace the character with a '_' */
					*(index_s - 1) = '_';
...
static zend_bool add_post_var(zval *arr, post_var_data_t *var, zend_bool eof TSRMLS_DC)
{
	...
	jonj_url_decode(var->ptr, klen);

	val = estrndup(ksep, vlen);
	if (vlen) {
		vlen = jonj_url_decode(val, vlen);
	}

	if (sapi_module.input_filter(PARSE_POST, var->ptr, &val, vlen, &new_vlen TSRMLS_CC)) {
		jonj_register_variable_safe(var->ptr, val, new_vlen, arr TSRMLS_CC);
	}
	efree(val);
	...
	while (add_post_var(arr, vars, eof TSRMLS_CC)) {
		if (++vars->cnt > max_vars) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
					...
					"To increase the limit change max_input_vars in jonj.ini.",
					max_vars);
			return FAILURE;
		}
	}
...
{
	zval *arr = (zval *) arg;
	jonj_stream *s = SG(request_info).request_body;
	post_var_data_t post_data;

	if (s && SUCCESS == jonj_stream_rewind(s)) {
		memset(&post_data, 0, sizeof(post_data));

		while (!jonj_stream_eof(s)) {
			char buf[SAPI_POST_HANDLER_BUFSIZ] = {0};
			size_t len = jonj_stream_read(s, buf, SAPI_POST_HANDLER_BUFSIZ);
...
SAPI_API SAPI_INPUT_FILTER_FUNC(jonj_default_input_filter)
{
...
SAPI_API SAPI_TREAT_DATA_FUNC(jonj_default_treat_data)
{
	...
	var = jonj_strtok_r(res, separator, &strtok_buf);
	...
	while (var) {
		...
		if (++count > PG(max_input_vars)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Input variables exceeded %ld. To increase the limit change max_input_vars in jonj.ini.", PG(max_input_vars));
			break;
		}
		...
		if (val) { /* have a value */
			...
			*val++ = '\0';
			jonj_url_decode(var, strlen(var));
			val_len = jonj_url_decode(val, strlen(val));
...
next_cookie:
		var = jonj_strtok_r(NULL, separator, &strtok_buf);
	}
...
void _jonj_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
	...
	for (env = environ; env != NULL && *env != NULL; env++) {
		...
		memcpy(t, *env, nlen);
		t[nlen] = '\0';
		jonj_register_variable(t, p + 1, array_ptr TSRMLS_CC);
	}
	if (t != buf && t != NULL) {
		efree(t);
	}
...
zend_bool jonj_std_auto_global_callback(char *name, uint name_len TSRMLS_DC)
{
	zend_printf("%s\n", name);
...
/* {{{ jonj_build_argv
 */
static void jonj_build_argv(char *s, zval *track_vars_array TSRMLS_DC)
{
	zval *arr, *argc, *tmp;
	int count = 0;
	char *ss, *space;
...
/* {{{ jonj_register_server_variables
 */
static inline void jonj_register_server_variables(TSRMLS_D)
{
	zval *array_ptr = NULL;
	...
	/* JONJ Authentication support */
	if (SG(request_info).auth_user) {
		jonj_register_variable("JONJ_AUTH_USER", SG(request_info).auth_user, array_ptr TSRMLS_CC);
	}
	if (SG(request_info).auth_password) {
		jonj_register_variable("JONJ_AUTH_PW", SG(request_info).auth_password, array_ptr TSRMLS_CC);
	}
	if (SG(request_info).auth_digest) {
		jonj_register_variable("JONJ_AUTH_DIGEST", SG(request_info).auth_digest, array_ptr TSRMLS_CC);
	}
	/* store request init time */
	{
		...
		jonj_register_variable_ex("REQUEST_TIME_FLOAT", &request_time_float, array_ptr TSRMLS_CC);
		Z_TYPE(request_time_long) = IS_LONG;
		Z_LVAL(request_time_long) = zend_dval_to_lval(Z_DVAL(request_time_float));
		jonj_register_variable_ex("REQUEST_TIME", &request_time_long, array_ptr TSRMLS_CC);
	}

}
/* }}} */

/* {{{ jonj_autoglobal_merge
 */
static void jonj_autoglobal_merge(HashTable *dest, HashTable *src TSRMLS_DC)
{
	...
	while (zend_hash_get_current_data_ex(src, (void **)&src_entry, &pos) == SUCCESS) {
		...
		if (Z_TYPE_PP(src_entry) != IS_ARRAY
			...
			jonj_autoglobal_merge(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_PP(src_entry) TSRMLS_CC);
		}
...
static zend_bool jonj_auto_globals_create_server(const char *name, uint name_len TSRMLS_DC);
static zend_bool jonj_auto_globals_create_env(const char *name, uint name_len TSRMLS_DC);
static zend_bool jonj_auto_globals_create_request(const char *name, uint name_len TSRMLS_DC);

/* {{{ jonj_hash_environment
 */
JONJAPI int jonj_hash_environment(TSRMLS_D)
{
	...
	if (PG(register_argc_argv)) {
		jonj_build_argv(SG(request_info).query_string, PG(http_globals)[TRACK_VARS_SERVER] TSRMLS_CC);
	}
	return SUCCESS;
}
/* }}} */

static zend_bool jonj_auto_globals_create_get(const char *name, uint name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool jonj_auto_globals_create_post(const char *name, uint name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool jonj_auto_globals_create_cookie(const char *name, uint name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool jonj_auto_globals_create_files(const char *name, uint name_len TSRMLS_DC)
{
	zval *vars;
...
static zend_bool jonj_auto_globals_create_server(const char *name, uint name_len TSRMLS_DC)
{
	if (PG(variables_order) && (strchr(PG(variables_order),'S') || strchr(PG(variables_order),'s'))) {
		jonj_register_server_variables(TSRMLS_C);
...
static zend_bool jonj_auto_globals_create_env(const char *name, uint name_len TSRMLS_DC)
{
	...
	if (PG(variables_order) && (strchr(PG(variables_order),'E') || strchr(PG(variables_order),'e'))) {
		jonj_import_environment_variables(PG(http_globals)[TRACK_VARS_ENV] TSRMLS_CC);
	}
...
static zend_bool jonj_auto_globals_create_request(const char *name, uint name_len TSRMLS_DC)
{
	...
	for (; p && *p; p++) {
		switch (*p) {
			...
			case 'G':
				if (!_gpc_flags[0]) {
					jonj_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_GET]) TSRMLS_CC);
					_gpc_flags[0] = 1;
				}
				break;
			case 'p':
			case 'P':
				if (!_gpc_flags[1]) {
					jonj_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_POST]) TSRMLS_CC);
					_gpc_flags[1] = 1;
				}
				break;
			case 'c':
			case 'C':
				if (!_gpc_flags[2]) {
					jonj_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_COOKIE]) TSRMLS_CC);
					_gpc_flags[2] = 1;
				}
				break;
		}
...
{
	zend_register_auto_global(ZEND_STRL("_GET"), 0, jonj_auto_globals_create_get TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_POST"), 0, jonj_auto_globals_create_post TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_COOKIE"), 0, jonj_auto_globals_create_cookie TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_SERVER"), PG(auto_globals_jit), jonj_auto_globals_create_server TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_ENV"), PG(auto_globals_jit), jonj_auto_globals_create_env TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_REQUEST"), PG(auto_globals_jit), jonj_auto_globals_create_request TSRMLS_CC);
	zend_register_auto_global(ZEND_STRL("_FILES"), 0, jonj_auto_globals_create_files TSRMLS_CC);
}
