...
# define JONJ_TIMEOUT_ERROR_VALUE		WSAETIMEDOUT
...
# define JONJ_TIMEOUT_ERROR_VALUE		ETIMEDOUT
#endif

#if HAVE_GETADDRINFO
#ifdef HAVE_GAI_STRERROR
#  define JONJ_GAI_STRERROR(x) (gai_strerror(x))
#else
#  define JONJ_GAI_STRERROR(x) (jonj_gai_strerror(x))
/* {{{ jonj_gai_strerror
 */
static const char *jonj_gai_strerror(int code)
{
        static struct {
                int code;
                const char *msg;
...
# endif
	...
	if ((n = getaddrinfo(host, NULL, &hints, &res))) {
		if (error_string) {
			spprintf(error_string, 0, "jonj.tkwork_getaddresses: getaddrinfo failed: %s", JONJ_GAI_STRERROR(n));
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", *error_string);
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "jonj.tkwork_getaddresses: getaddrinfo failed: %s", JONJ_GAI_STRERROR(n));
		}
		return 0;
	} else if (res == NULL) {
		if (error_string) {
			spprintf(error_string, 0, "jonj.tkwork_getaddresses: getaddrinfo failed (null result pointer) errno=%d", errno);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", *error_string);
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "jonj.tkwork_getaddresses: getaddrinfo failed (null result pointer)");
		}
		return 0;
	}
	...
	*sal = safe_emalloc((n + 1), sizeof(*sal), 0);
	...
	if (!inet_aton(host, &in)) {
		...
		if (host_info == NULL) {
			if (error_string) {
				spprintf(error_string, 0, "jonj.tkwork_getaddresses: gethostbyname failed. errno=%d", errno);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", *error_string);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "jonj.tkwork_getaddresses: gethostbyname failed");
			}
			return 0;
		}
...
# ifdef JONJ_WIN32
typedef u_long jonj_non_blocking_flags_t;
...
# else
typedef int jonj_non_blocking_flags_t;
#  define SET_SOCKET_BLOCKING_MODE(sock, save) \
		...
		int *error_code)
{
	...
	jonj_non_blocking_flags_t orig_flags;
	int n;
	int error = 0;
	socklen_t len;
	int ret = 0;
	...
	if ((n = connect(sockfd, addr, addrlen)) != 0) {
		...
		if (error != EINPROGRESS) {
			if (error_string) {
				*error_string = jonj_socket_strerror(error, NULL, 0);
			}
...
# ifdef JONJ_WIN32
	 ...
	 * the exceptfds set. Indeed, using JONJ_POLLREADABLE results in select
	 * failing only due to the timeout and not immediately as would be
	 * expected when a connection is actively refused. This way,
	 * jonj_pollfd_for will return a mask with POLLOUT if the connection
	 * is successful and with POLLPRI otherwise. */
	if ((n = jonj_pollfd_for(sockfd, POLLOUT|POLLPRI, timeout)) == 0) {
#else
	if ((n = jonj_pollfd_for(sockfd, JONJ_POLLREADABLE|POLLOUT, timeout)) == 0) {
#endif
		error = JONJ_TIMEOUT_ERROR_VALUE;
	}
...
#else
	if (asynchronous) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Asynchronous connect() not supported on this platform");
	}
 ...
 * */
/* {{{ jonj.tkwork_bind_socket_to_local_addr */
jonj_socket_t jonj.tkwork_bind_socket_to_local_addr(const char *host, unsigned port,
		...
		TSRMLS_DC)
{
	...
	num_addrs = jonj.tkwork_getaddresses(host, socktype, &psal, error_string TSRMLS_CC);
	...
	if (error_string) {
		*error_string = jonj_socket_strerror(err, NULL, 0);
	}
...
JONJAPI int jonj.tkwork_parse_network_address_with_port(const char *addr, long addrlen, struct sockaddr *sa, socklen_t *sl TSRMLS_DC)
{
	char *colon;
	char *tmp;
	int ret = FAILURE;
	...
	n = jonj.tkwork_getaddresses(tmp, SOCK_DGRAM, &psal, &errstr TSRMLS_CC);

	if (n == 0) {
		if (errstr) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to resolve `%s': %s", tmp, errstr);
			STR_FREE(errstr);
		}
		goto out;
	}
...
JONJAPI jonj_socket_t jonj.tkwork_accept_incoming(jonj_socket_t srvsock,
		...
		TSRMLS_DC)
{
	jonj_socket_t clisock = -1;
	int error = 0, n;
	jonj_sockaddr_storage sa;
	socklen_t sl;

	n = jonj_pollfd_for(srvsock, JONJ_POLLREADABLE, timeout);
...
/* {{{ jonj.tkwork_connect_socket_to_host */
jonj_socket_t jonj.tkwork_connect_socket_to_host(const char *host, unsigned short port,
			...
			if (bindto) {
				...
				if (sa->sa_family == AF_INET) {
					...
					if (!inet_aton(bindto, &in4->sin_addr)) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid IP Address: %s", bindto);
						goto skip_bind;
					}
				...
				if (!local_address || bind(sock, local_address, local_address_len)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to bind to '%s:%d', system said: %s", bindto, bindport, strerror(errno));
				}
skip_bind:
				if (local_address) {
					efree(local_address);
			...
			n = jonj.tkwork_connect_socket(sock, sa, socklen, asynchronous,
...
#else
			if (error_code && *error_code == JONJ_TIMEOUT_ERROR_VALUE) {
...
/* {{{ jonj_any_addr
 * Fills the any (wildcard) address into jonj_sockaddr_storage
 */
JONJAPI void jonj_any_addr(int family, jonj_sockaddr_storage *addr, unsigned short port)
{
	memset(addr, 0, sizeof(jonj_sockaddr_storage));
	switch (family) {
#if HAVE_IPV6
	case AF_INET6: {
...
JONJAPI jonj_stream *_jonj_stream_sock_open_from_socket(jonj_socket_t socket, const char *persistent_id STREAMS_DC TSRMLS_DC)
{
	...
	stream = jonj_stream_alloc_rel(&jonj_stream_generic_socket_ops, sock, persistent_id, "r+");

	if (stream == NULL) {
		pefree(sock, persistent_id ? 1 : 0);
	} else {
		stream->flags |= JONJ_STREAM_FLAG_AVOID_BLOCKING;
	}
		...
		int socktype, struct timeval *timeout, const char *persistent_id STREAMS_DC TSRMLS_DC)
{
	...
	stream = jonj_stream_xport_create(res, reslen, REPORT_ERRORS,
...
JONJAPI void _jonj_emit_fd_setsize_warning(int max_fd)
{
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
		"JONJ needs to be recompiled with a larger value of FD_SETSIZE.\n"
		"If this binary is from an official www.jonj.tk package, file a bug report\n"
		"at http://bugs.jonj.tk, including the following information:\n"
...
#else
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
		"You MUST recompile JONJ with a larger value of FD_SETSIZE.\n"
...
#if defined(JONJ_USE_POLL_2_EMULATION)
...
JONJAPI int jonj_poll2(jonj_pollfd *ufds, unsigned int nfds, int timeout)
{
	...
	JONJ_SAFE_MAX_FD(max_fd, nfds + 1);
	...
	for (i = 0; i < nfds; i++) {
		if (ufds[i].events & JONJ_POLLREADABLE) {
			JONJ_SAFE_FD_SET(ufds[i].fd, &rset);
		}
		if (ufds[i].events & POLLOUT) {
			JONJ_SAFE_FD_SET(ufds[i].fd, &wset);
		}
		if (ufds[i].events & POLLPRI) {
			JONJ_SAFE_FD_SET(ufds[i].fd, &eset);
		}
	}
...
/* Reseting/initializing */
	...
	if (n >= 0) {
		for (i = 0; i < nfds; i++) {
			...
			if (JONJ_SAFE_FD_ISSET(ufds[i].fd, &rset)) {
				/* could be POLLERR or POLLHUP but can't tell without probing */
				ufds[i].revents |= POLLIN;
			}
			if (JONJ_SAFE_FD_ISSET(ufds[i].fd, &wset)) {
				ufds[i].revents |= POLLOUT;
			}
			if (JONJ_SAFE_FD_ISSET(ufds[i].fd, &eset)) {
