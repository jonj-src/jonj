...
static void sapi_globals_ctor(sapi_globals_struct *sapi_globals TSRMLS_DC)
{
	...
	jonj_setup_sapi_content_types(TSRMLS_C);
}
...
callback_failed:
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not call the sapi_header_callback");
	}
...
SAPI_API SAPI_POST_READER_FUNC(sapi_read_standard_form_data)
{
	if ((SG(post_max_size) > 0) && (SG(request_info).content_length > SG(post_max_size))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "POST Content-Length of %ld bytes exceeds the limit of %ld bytes",
					SG(request_info).content_length, SG(post_max_size));
		return;
	}


	SG(request_info).request_body = jonj_stream_temp_create_ex(TEMP_STREAM_DEFAULT, SAPI_POST_BLOCK_SIZE, PG(upload_tmp_dir));
	...
	if (sapi_module.read_post) {
		...
		for (;;) {
			...
			if (read_bytes > 0) {
				if (jonj_stream_write(SG(request_info).request_body, buffer, read_bytes) != read_bytes) {
					/* if parts of the stream can't be written, purge it completely */
					jonj_stream_truncate_set_size(SG(request_info).request_body, 0);
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "POST data can't be buffered; all data discarded");
					break;
				}
			}

			if ((SG(post_max_size) > 0) && (SG(read_post_bytes) > SG(post_max_size))) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Actual POST length does not match Content-Length, and exceeds %ld bytes", SG(post_max_size));
				break;
			}
	...
	if (*mimetype != NULL) {
		if (*charset && strncmp(*mimetype, "text/", 5) == 0 && strstr(*mimetype, "charset=") == NULL) {
			...
			newtype = emalloc(newlen + 1);
	 		JONJ_STRLCPY(newtype, *mimetype, newlen + 1, len);
...
/*
 * Called from jonj_request_startup() for every request.
 */
			...
			*colon_offset = 0;
			if (!STRCASECMP(header_line, "Content-Type")) {
				...
				if (!strncmp(ptr, "image/", sizeof("image/")-1)) {
					zend_alter_ini_entry("zlib.output_compression", sizeof("zlib.output_compression"), "0", sizeof("0") - 1, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME);
				}
				...
				if (newlen != 0) {
					...
					JONJ_STRLCPY(newheader, "Content-type: ", newlen, sizeof("Content-type: ")-1);
				...
				zend_alter_ini_entry("zlib.output_compression", sizeof("zlib.output_compression"),
					"0", sizeof("0") - 1, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME);
