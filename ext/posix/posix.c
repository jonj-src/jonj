...
static JONJ_MINFO_FUNCTION(posix);
...
const zend_function_entry posix_functions[] = {
    /* POSIX.1, 3.3 */
	JONJ_FE(posix_kill,		arginfo_posix_kill)

	/* POSIX.1, 4.1 */
	JONJ_FE(posix_getpid,	arginfo_posix_getpid)
	JONJ_FE(posix_getppid,	arginfo_posix_getppid)

	/* POSIX.1,  4.2 */
	JONJ_FE(posix_getuid,	arginfo_posix_getuid)
	JONJ_FE(posix_setuid,	arginfo_posix_setuid)
	JONJ_FE(posix_geteuid,	arginfo_posix_geteuid)
#ifdef HAVE_SETEUID
	JONJ_FE(posix_seteuid,	arginfo_posix_seteuid)
#endif
	JONJ_FE(posix_getgid,	arginfo_posix_getgid)
	JONJ_FE(posix_setgid,	arginfo_posix_setgid)
	JONJ_FE(posix_getegid,	arginfo_posix_getegid)
#ifdef HAVE_SETEGID
	JONJ_FE(posix_setegid,	arginfo_posix_setegid)
#endif
#ifdef HAVE_GETGROUPS
	JONJ_FE(posix_getgroups,	arginfo_posix_getgroups)
#endif
#ifdef HAVE_GETLOGIN
	JONJ_FE(posix_getlogin,	arginfo_posix_getlogin)
#endif

	/* POSIX.1, 4.3 */
	JONJ_FE(posix_getpgrp,	arginfo_posix_getpgrp)
#ifdef HAVE_SETSID
	JONJ_FE(posix_setsid,	arginfo_posix_setsid)
#endif
	JONJ_FE(posix_setpgid,	arginfo_posix_setpgid)
	/* Non-Posix functions which are common */
#ifdef HAVE_GETPGID
	JONJ_FE(posix_getpgid,	arginfo_posix_getpgid)
#endif /* HAVE_GETPGID */
#ifdef HAVE_GETSID
	JONJ_FE(posix_getsid,	arginfo_posix_getsid)
#endif /* HAVE_GETSID */

	/* POSIX.1, 4.4 */
	JONJ_FE(posix_uname,		arginfo_posix_uname)

	/* POSIX.1, 4.5 */
	JONJ_FE(posix_times,		arginfo_posix_times)

	/* POSIX.1, 4.7 */
#ifdef HAVE_CTERMID
	JONJ_FE(posix_ctermid,	arginfo_posix_ctermid)
#endif
	JONJ_FE(posix_ttyname,	arginfo_posix_ttyname)
	JONJ_FE(posix_isatty,	arginfo_posix_isatty)

    /* POSIX.1, 5.2 */
	JONJ_FE(posix_getcwd,	arginfo_posix_getcwd)

	/* POSIX.1, 5.4 */
#ifdef HAVE_MKFIFO
	JONJ_FE(posix_mkfifo,	arginfo_posix_mkfifo)
#endif
#ifdef HAVE_MKNOD
	JONJ_FE(posix_mknod,		arginfo_posix_mknod)
#endif

	/* POSIX.1, 5.6 */
	JONJ_FE(posix_access,	arginfo_posix_access)
	/* POSIX.1, 9.2 */
	JONJ_FE(posix_getgrnam,	arginfo_posix_getgrnam)
	JONJ_FE(posix_getgrgid,	arginfo_posix_getgrgid)
	JONJ_FE(posix_getpwnam,	arginfo_posix_getpwnam)
	JONJ_FE(posix_getpwuid,	arginfo_posix_getpwuid)

#ifdef HAVE_GETRLIMIT
	JONJ_FE(posix_getrlimit,	arginfo_posix_getrlimit)
#endif

	JONJ_FE(posix_get_last_error,					arginfo_posix_get_last_error)
	JONJ_FALIAS(posix_errno, posix_get_last_error,	arginfo_posix_get_last_error)
	JONJ_FE(posix_strerror,							arginfo_posix_strerror)
#ifdef HAVE_INITGROUPS
	JONJ_FE(posix_initgroups,	arginfo_posix_initgroups)
#endif
...
static JONJ_GINIT_FUNCTION(posix) /* {{{ */
{
	posix_globals->last_error = 0;
}
/* }}} */

/* {{{ JONJ_MINIT_FUNCTION(posix)
 */
static JONJ_MINIT_FUNCTION(posix)
{
...
#define JONJ_POSIX_NO_ARGS	if (zend_parse_parameters_none() == FAILURE) return;

#define JONJ_POSIX_RETURN_LONG_FUNC(func_name)	\
	JONJ_POSIX_NO_ARGS	\
	RETURN_LONG(func_name());

#define JONJ_POSIX_SINGLE_ARG_FUNC(func_name)	\
	long val;	\
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getpid);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getppid);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getuid);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getgid);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(geteuid);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getegid);
}
/* }}} */
...
{
	JONJ_POSIX_SINGLE_ARG_FUNC(setuid);
}
/* }}} */
...
{
	JONJ_POSIX_SINGLE_ARG_FUNC(setgid);
}
/* }}} */
...
{
	JONJ_POSIX_SINGLE_ARG_FUNC(seteuid);
}
#endif
/* }}} */
...
{
	JONJ_POSIX_SINGLE_ARG_FUNC(setegid);
}
#endif
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(getpgrp);
}
/* }}} */
...
{
	JONJ_POSIX_RETURN_LONG_FUNC(setsid);
}
#endif
/* }}} */
...
static int jonj_posix_stream_get_fd(zval *zfp, int *fd TSRMLS_DC) /* {{{ */
{
	jonj_stream *stream;

	jonj_stream_from_zval_no_verify(stream, &zfp);

	if (stream == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "expects argument 1 to be a valid stream resource");
		return 0;
	}
	if (jonj_stream_can_cast(stream, JONJ_STREAM_AS_FD_FOR_SELECT) == SUCCESS) {
		jonj_stream_cast(stream, JONJ_STREAM_AS_FD_FOR_SELECT, (void*)fd, 0);
	} else if (jonj_stream_can_cast(stream, JONJ_STREAM_AS_FD) == SUCCESS) {
		jonj_stream_cast(stream, JONJ_STREAM_AS_FD, (void*)fd, 0);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "could not use stream of type '%s'", 
				stream->ops->label);
		return 0;
	}
	return 1;
...
JONJ_FUNCTION(posix_ttyname)
{
	...
	switch (Z_TYPE_PP(z_fd)) {
		case IS_RESOURCE:
			if (!jonj_posix_stream_get_fd(*z_fd, &fd TSRMLS_CC)) {
				RETURN_FALSE;
			}
			break;
		default:
...
JONJ_FUNCTION(posix_mkfifo)
{
	...
	if (jonj_check_open_basedir_ex(path, 0 TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(posix_mknod)
{
	...
	if ((mode & S_IFCHR) || (mode & S_IFBLK)) {
		if (ZEND_NUM_ARGS() == 2) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "For S_IFCHR and S_IFBLK you need to pass a major device kernel identifier");
			RETURN_FALSE;
		}
		if (major == 0) {
			...
			jonj_dev = makedev(major, minor);
#else
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create a block or character device, creating a normal file instead");
#endif
		}
	}

	result = mknod(path, mode, jonj_dev);
	if (result < 0) {
		POSIX_G(last_error) = errno;
		RETURN_FALSE;
	}
...
int jonj_posix_group_to_array(struct group *g, zval *array_group) /* {{{ */
{
	zval *array_members;
	int count;
...
/*
	...
	POSIX.1, 5.6.x stat(), chmod(), utime() already supported by JONJ.
*/
...
/*
	POSIX.1, 6.x most I/O functions already supported by JONJ.
...
JONJ_FUNCTION(posix_getgrnam)
{
	...
	if (!jonj_posix_group_to_array(g, return_value)) {
		zval_dtor(return_value);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to convert posix group to array");
		RETVAL_FALSE;
	}
#if defined(ZTS) && defined(HAVE_GETGRNAM_R) && defined(_SC_GETGR_R_SIZE_MAX)
	efree(buf);
...
JONJ_FUNCTION(posix_getgrgid)
{
	...
	if (!jonj_posix_group_to_array(g, return_value)) {
		zval_dtor(return_value);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to convert posix group struct to array");
		RETVAL_FALSE;
	}
#if defined(ZTS) && defined(HAVE_GETGRGID_R) && defined(_SC_GETGR_R_SIZE_MAX)
	efree(grbuf);
...
int jonj_posix_passwd_to_array(struct passwd *pw, zval *return_value) /* {{{ */
{
	if (NULL == pw)
		return 0;
...
JONJ_FUNCTION(posix_getpwnam)
{
	...
	if (!jonj_posix_passwd_to_array(pw, return_value)) {
		zval_dtor(return_value);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to convert posix passwd struct to array");
		RETVAL_FALSE;
	}
#if defined(ZTS) && defined(_SC_GETPW_R_SIZE_MAX) && defined(HAVE_GETPWNAM_R)
	efree(buf);
