...
MYSQLND_METHOD(mysqlnd_debug, open)(MYSQLND_DEBUG * self, zend_bool reopen)
{
	...
	self->stream = jonj_stream_open_wrapper(self->file_name,
								   ...
								   unsigned int level, const char * type, const char * message)
{
	...
	ret = jonj_stream_write(self->stream, message_line, message_line_len)? PASS:FAIL;
...
MYSQLND_METHOD(mysqlnd_debug, close)(MYSQLND_DEBUG * self)
{
	...
	if (self->stream) {
		...
		jonj_stream_free(self->stream, JONJ_STREAM_FREE_CLOSE);
		self->stream = NULL;
	}
		...
		switch (mode[i]) {
			...
			case ':':
				...
				if (state != PARSER_WAIT_COLON) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Consecutive semicolons at position %u", i);
				}
#endif
...
#if 0
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unrecognized format '%c'", mode[i]);
#endif
					if (i+1 < mode_len && mode[i+1] == ',') {
						i+= 2;
						while (i < mode_len) {
...
#if 0
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Colon expected, '%c' found", mode[i]);
#endif
				}
				break;
		}
