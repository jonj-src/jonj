...
/*	result->m.row_decoder = jonj_mysqlnd_rowp_read_binary_protocol; */
							 				 ...
							 				 const char * const data, unsigned long length TSRMLS_DC)
{
	...
	MYSQLND_CONN_DATA * conn;
	zend_uchar * cmd_buf;
	enum jonj_mysqlnd_server_command cmd = COM_STMT_SEND_LONG_DATA;
	...
	if (CONN_GET_STATE(conn) == CONN_READY) {
		...
		*/
		...
		if ((packet_len = conn->net->m.consume_uneaten_data(conn->net, cmd TSRMLS_CC))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "There was an error "
	...
	if (stmt->field_count) {
		...
		for (i = 0; i < stmt->field_count; i++) {
			...
			/*
			  ...
			  Otherwise either 009.jonjt or mysqli_stmt_bind_result.jonjt
			  will fail.
			*/
	...
	switch (attr_type) {
		case STMT_ATTR_UPDATE_MAX_LENGTH:{
			...
			/*
			  ...
			  and mysqlnd won't be used out of the scope of JONJ -> use ulong.
			*/
