...
#define	PACKET_READ_HEADER_AND_BODY(packet, conn, buf, buf_size, packet_type_as_text, packet_type) \
	{ \
		...
		if (FAIL == mysqlnd_read_header((conn)->net, &((packet)->header), (conn)->stats, ((conn)->error_info) TSRMLS_CC)) {\
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", mysqlnd_server_gone); \
...
#define BAIL_IF_NO_MORE_DATA \
	if ((size_t)(p - begin) > packet->header.size) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Premature end of data (mysqlnd_wireprotocol.c:%u)", __LINE__); \
		goto premature_end; \
	} \
...
/* {{{ jonj_mysqlnd_net_field_length
   Get next field's length */
unsigned long
jonj_mysqlnd_net_field_length(zend_uchar **packet)
{
...
/* {{{ jonj_mysqlnd_net_field_length_ll
   Get next field's length */
uint64_t
jonj_mysqlnd_net_field_length_ll(zend_uchar **packet)
{
	register zend_uchar *p= (zend_uchar *)*packet;
...
/* {{{ jonj_mysqlnd_net_store_length */
zend_uchar *
jonj_mysqlnd_net_store_length(zend_uchar *packet, uint64_t length)
{
...
/* {{{ jonj_mysqlnd_net_store_length_size */
size_t 
jonj_mysqlnd_net_store_length_size(uint64_t length)
{
	if (length < (uint64_t) L64(251)) {
		return 1;
	}
...
/* {{{ jonj_mysqlnd_read_error_from_line */
static enum_func_status
jonj_mysqlnd_read_error_from_line(zend_uchar *buf, size_t buf_len,
								...
								unsigned int *error_no, char *sqlstate TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_read_error_from_line");
					...
					MYSQLND_STATS * conn_stats, MYSQLND_ERROR_INFO * error_info TSRMLS_DC)
{
	...
	jonj_error(E_WARNING, "Packets out of order. Expected %u received %u. Packet size="MYSQLND_SZ_T_SPEC,
			  net->packet_no, header->packet_no, header->size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_greet_read */
static enum_func_status
jonj_mysqlnd_greet_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	zend_uchar buf[2048];
	zend_uchar *p = buf;
	...
	DBG_ENTER("jonj_mysqlnd_greet_read");
	...
	if (ERROR_MARKER == packet->protocol_version) {
		jonj_mysqlnd_read_error_from_line(p, packet->header.size - 1,
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "GREET packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
...
/* {{{ jonj_mysqlnd_greet_free_mem */
static
void jonj_mysqlnd_greet_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_auth_write */
static
size_t jonj_mysqlnd_auth_write(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_auth_write");
	...
	if (packet->send_auth_data || packet->is_change_user_packet) {
		...
		*p++ = '\0';
		...
		if (packet->auth_data_len > 0xFF) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", msg);
			DBG_RETURN(0);
		}		
		...
		if (packet->connect_attr && zend_hash_num_elements(packet->connect_attr)) {
			...
			while (SUCCESS == zend_hash_get_current_data_ex(packet->connect_attr, (void **)&entry_value, &pos_value)) {
				...
				if (HASH_KEY_IS_STRING == zend_hash_get_current_key_ex(packet->connect_attr, &s_key, &s_len, &num_key, 0, &pos_value)) {
					ca_payload_len += jonj_mysqlnd_net_store_length_size(s_len);
					ca_payload_len += s_len;
					ca_payload_len += jonj_mysqlnd_net_store_length_size(value_len);
					ca_payload_len += value_len;
				}
				zend_hash_move_forward_ex(conn->options->connect_attr, &pos_value);
			}

			if ((sizeof(buffer) - (p - buffer)) >= (ca_payload_len + jonj_mysqlnd_net_store_length_size(ca_payload_len))) {
				p = jonj_mysqlnd_net_store_length(p, ca_payload_len);
				...
				while (SUCCESS == zend_hash_get_current_data_ex(packet->connect_attr, (void **)&entry_value, &pos_value)) {
					...
					if (HASH_KEY_IS_STRING == zend_hash_get_current_key_ex(packet->connect_attr, &s_key, &s_len, &num_key, 0, &pos_value)) {
						/* copy key */
						p = jonj_mysqlnd_net_store_length(p, s_len);
						memcpy(p, s_key, s_len);
						p+= s_len;
						/* copy value */
						p = jonj_mysqlnd_net_store_length(p, value_len);
...
/* {{{ jonj_mysqlnd_auth_free_mem */
static
void jonj_mysqlnd_auth_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	if (!stack_allocation) {
...
/* {{{ jonj_mysqlnd_auth_response_read */
static enum_func_status
jonj_mysqlnd_auth_response_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_auth_response_read");
	...
	*/
	...
	if (0xFE == packet->response_code) {
		...
		/* Everything was fine! */
		packet->affected_rows  = jonj_mysqlnd_net_field_length_ll(&p);
		BAIL_IF_NO_MORE_DATA;

		packet->last_insert_id = jonj_mysqlnd_net_field_length_ll(&p);
		BAIL_IF_NO_MORE_DATA;
		...
		/* There is a message */
		if (packet->header.size > (size_t) (p - buf) && (i = jonj_mysqlnd_net_field_length(&p))) {
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "AUTH_RESPONSE packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_auth_response_free_mem */
static void
jonj_mysqlnd_auth_response_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_change_auth_response_write */
static size_t
jonj_mysqlnd_change_auth_response_write(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_change_auth_response_write");
...
/* {{{ jonj_mysqlnd_change_auth_response_free_mem */
static void
jonj_mysqlnd_change_auth_response_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	if (!stack_allocation) {
...
/* {{{ jonj_mysqlnd_ok_read */
static enum_func_status
jonj_mysqlnd_ok_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_ok_read");
...
premature_end:
	DBG_ERR_FMT("OK packet %d bytes shorter than expected", p - begin - packet->header.size);
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OK packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_ok_free_mem */
static void
jonj_mysqlnd_ok_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_eof_read */
static enum_func_status
jonj_mysqlnd_eof_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	/*
	...
	*/
	...
	DBG_ENTER("jonj_mysqlnd_eof_read");
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "EOF packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_eof_free_mem */
static
void jonj_mysqlnd_eof_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	if (!stack_allocation) {
...
/* {{{ jonj_mysqlnd_cmd_write */
size_t jonj_mysqlnd_cmd_write(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_cmd_write");
	/*
...
/* {{{ jonj_mysqlnd_cmd_free_mem */
static
void jonj_mysqlnd_cmd_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	if (!stack_allocation) {
...
/* {{{ jonj_mysqlnd_rset_header_read */
static enum_func_status
jonj_mysqlnd_rset_header_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_rset_header_read");
	...
	*/
	...
	packet->field_count = jonj_mysqlnd_net_field_length(&p);
	BAIL_IF_NO_MORE_DATA;
	...
	switch (packet->field_count) {
		...
		case 0x00:
			...
			if (packet->header.size  > (size_t) (p - buf) && (len = jonj_mysqlnd_net_field_length(&p))) {
				packet->info_or_local_file = mnd_emalloc(len + 1);
				if (packet->info_or_local_file) {
					memcpy(packet->info_or_local_file, p, len);
					packet->info_or_local_file[len] = '\0';
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "RSET_HEADER packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_rset_header_free_mem */
static
void jonj_mysqlnd_rset_header_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	MYSQLND_PACKET_RSET_HEADER *p= (MYSQLND_PACKET_RSET_HEADER *) _packet;
	DBG_ENTER("jonj_mysqlnd_rset_header_free_mem");
...
/* {{{ jonj_mysqlnd_rset_field_read */
static enum_func_status
jonj_mysqlnd_rset_field_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_rset_field_read");
	...
	for (i = 0; i < field_count; i += 2) {
		len = jonj_mysqlnd_net_field_length(&p);
		BAIL_IF_NO_MORE_DATA;
		switch ((len)) {
			case 0:
	...
	if (12 != *p) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Protocol error. Server sent false length. Expected 12");
	}
	p++;
	BAIL_IF_NO_MORE_DATA;
...
faulty_or_fake:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Protocol error. Server sent NULL_LENGTH."
					 " The server is faulty");
	DBG_RETURN(FAIL);
premature_end:
	DBG_ERR_FMT("RSET field packet %d bytes shorter than expected", p - begin - packet->header.size);
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Result set field packet "MYSQLND_SZ_T_SPEC" bytes "
...
/* {{{ jonj_mysqlnd_rset_field_free_mem */
static
void jonj_mysqlnd_rset_field_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_read_row_ex */
static enum_func_status
jonj_mysqlnd_read_row_ex(MYSQLND_CONN_DATA * conn, MYSQLND_MEMORY_POOL * result_set_memory_pool,
						...
						unsigned int prealloc_more_bytes TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_read_row_ex");
	...
	while (1) {
		...
		*data_size += header.size;
		...
		if (PASS != (ret = conn->net->data->m.receive_ex(conn->net, p, header.size, conn->stats, conn->error_info TSRMLS_CC))) {
			...
			jonj_error(E_WARNING, "Empty row packet body");
			break;
		}
...
/* {{{ jonj_mysqlnd_rowp_read_binary_protocol */
enum_func_status
jonj_mysqlnd_rowp_read_binary_protocol(MYSQLND_MEMORY_POOL_CHUNK * row_buffer, zval ** fields,
									  ...
									  zend_bool as_int_or_float, MYSQLND_STATS * stats TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_rowp_read_binary_protocol");
...
/* {{{ jonj_mysqlnd_rowp_read_text_protocol */
enum_func_status
jonj_mysqlnd_rowp_read_text_protocol_aux(MYSQLND_MEMORY_POOL_CHUNK * row_buffer, zval ** fields,
									unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
									zend_bool as_int_or_float, zend_bool copy_data, MYSQLND_STATS * stats TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_rowp_read_text_protocol_aux");
	...
	for (i = 0, current_field = start_field; current_field < end_field; current_field++, i++) {
		...
		/* jonj_mysqlnd_net_field_length() call should be after *this_field_len_pos = p; */
		const unsigned long len = jonj_mysqlnd_net_field_length(&p);

		if (len != MYSQLND_NULL_LENGTH && ((p + len) > packet_end)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Malformed server packet. Field length pointing "MYSQLND_SZ_T_SPEC
		...
		if (len == MYSQLND_NULL_LENGTH) {
			...
			if (as_int_or_float && perm_bind.jonj_type == IS_LONG) {
...
#error Need fix for this architecture
				...
				*(p + len) = save;
			} else if (as_int_or_float && perm_bind.jonj_type == IS_DOUBLE) {
				zend_uchar save = *(p + len);
				/* We have to make it ASCIIZ temporarily */
				*(p + len) = '\0';
			...
			if (fields_metadata[i].type == MYSQL_TYPE_BIT) {
				/*
				  ...
				  see jonj_mysqlnd_rowp_read(). And we add the strings at the end.
...
/* {{{ jonj_mysqlnd_rowp_read_text_protocol_zval */
enum_func_status
jonj_mysqlnd_rowp_read_text_protocol_zval(MYSQLND_MEMORY_POOL_CHUNK * row_buffer, zval ** fields,
									unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
									zend_bool as_int_or_float, MYSQLND_STATS * stats TSRMLS_DC)
{
	enum_func_status ret;
	DBG_ENTER("jonj_mysqlnd_rowp_read_text_protocol_zval");
	ret = jonj_mysqlnd_rowp_read_text_protocol_aux(row_buffer, fields, field_count, fields_metadata, as_int_or_float, FALSE, stats TSRMLS_CC);
	DBG_RETURN(ret);
}
/* }}} */


/* {{{ jonj_mysqlnd_rowp_read_text_protocol_c */
enum_func_status
jonj_mysqlnd_rowp_read_text_protocol_c(MYSQLND_MEMORY_POOL_CHUNK * row_buffer, zval ** fields,
									unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
									zend_bool as_int_or_float, MYSQLND_STATS * stats TSRMLS_DC)
{
	enum_func_status ret;
	DBG_ENTER("jonj_mysqlnd_rowp_read_text_protocol_c");
	ret = jonj_mysqlnd_rowp_read_text_protocol_aux(row_buffer, fields, field_count, fields_metadata, as_int_or_float, TRUE, stats TSRMLS_CC);
	DBG_RETURN(ret);
}
/* }}} */
...
jonj_mysqlnd_rowp_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_rowp_read");
	...
	ret = jonj_mysqlnd_read_row_ex(conn, packet->result_set_memory_pool, &packet->row_buffer, &data_size,
	...
	if (ERROR_MARKER == (*(p = packet->row_buffer->ptr))) {
		...
		*/
		ret = FAIL;
		jonj_mysqlnd_read_error_from_line(p + 1, data_size - 1,
...
/* {{{ jonj_mysqlnd_rowp_free_mem */
static void
jonj_mysqlnd_rowp_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	MYSQLND_PACKET_ROW *p;

	DBG_ENTER("jonj_mysqlnd_rowp_free_mem");
...
/* {{{ jonj_mysqlnd_stats_read */
static enum_func_status
jonj_mysqlnd_stats_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	MYSQLND_PACKET_STATS *packet= (MYSQLND_PACKET_STATS *) _packet;
	size_t buf_len = conn->net->cmd_buffer.length;
	zend_uchar *buf = (zend_uchar *) conn->net->cmd_buffer.buffer;

	DBG_ENTER("jonj_mysqlnd_stats_read");
...
/* {{{ jonj_mysqlnd_stats_free_mem */
static
void jonj_mysqlnd_stats_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_prepare_read */
static enum_func_status
jonj_mysqlnd_prepare_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_prepare_read");
	...
	if (ERROR_MARKER == packet->error_code) {
		jonj_mysqlnd_read_error_from_line(p, data_size - 1,
										 packet->error_info.error,
										 sizeof(packet->error_info.error),
										 &packet->error_info.error_no,
										 packet->error_info.sqlstate
	...
	if (data_size != PREPARE_RESPONSE_SIZE_41 &&
		...
		jonj_error(E_WARNING, "Wrong COM_STMT_PREPARE response size. Received %u", data_size);
		DBG_RETURN(FAIL);
	}
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "PREPARE packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_prepare_free_mem */
static void
jonj_mysqlnd_prepare_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_chg_user_read */
static enum_func_status
jonj_mysqlnd_chg_user_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_chg_user_read");
...
premature_end:
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CHANGE_USER packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
						 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_chg_user_free_mem */
static void
jonj_mysqlnd_chg_user_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
/* {{{ jonj_mysqlnd_sha256_pk_request_write */
static
size_t jonj_mysqlnd_sha256_pk_request_write(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	zend_uchar buffer[MYSQLND_HEADER_SIZE + 1];
	size_t sent;

	DBG_ENTER("jonj_mysqlnd_sha256_pk_request_write");
...
/* {{{ jonj_mysqlnd_sha256_pk_request_free_mem */
static
void jonj_mysqlnd_sha256_pk_request_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
	if (!stack_allocation) {
...
/* {{{ jonj_mysqlnd_sha256_pk_request_response_read */
static enum_func_status
jonj_mysqlnd_sha256_pk_request_response_read(void * _packet, MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	DBG_ENTER("jonj_mysqlnd_sha256_pk_request_response_read");
...
premature_end:
	DBG_ERR_FMT("OK packet %d bytes shorter than expected", p - begin - packet->header.size);
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SHA256_PK_REQUEST_RESPONSE packet "MYSQLND_SZ_T_SPEC" bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ jonj_mysqlnd_sha256_pk_request_response_free_mem */
static void
jonj_mysqlnd_sha256_pk_request_response_free_mem(void * _packet, zend_bool stack_allocation TSRMLS_DC)
{
...
mysqlnd_packet_methods packet_methods[PROT_LAST] =
{
	...
	}, /* PROT_AUTH_PACKET */
	{
		...
		jonj_mysqlnd_auth_response_read, /* read */
		NULL, /* write */
		jonj_mysqlnd_auth_response_free_mem,
	}, /* PROT_AUTH_RESP_PACKET */
	{
		...
		NULL, /* read */
		jonj_mysqlnd_change_auth_response_write, /* write */
		jonj_mysqlnd_change_auth_response_free_mem,
	}, /* PROT_CHANGE_AUTH_RESP_PACKET */
	{
		sizeof(MYSQLND_PACKET_OK),
		jonj_mysqlnd_ok_read, /* read */
		NULL, /* write */
		jonj_mysqlnd_ok_free_mem,
	}, /* PROT_OK_PACKET */
	{
		sizeof(MYSQLND_PACKET_EOF),
		jonj_mysqlnd_eof_read, /* read */
		NULL, /* write */
		jonj_mysqlnd_eof_free_mem,
	}, /* PROT_EOF_PACKET */
	{
