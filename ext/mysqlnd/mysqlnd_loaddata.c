...
int mysqlnd_local_infile_init(void ** ptr, char * filename, void ** userdata TSRMLS_DC)
{
	...
	*ptr = info;
	...
	/* check open_basedir */
	if (PG(open_basedir)) {
		if (jonj_check_open_basedir_ex(filename, 0 TSRMLS_CC) == -1) {
	...
	info->fd = jonj_stream_open_wrapper_ex((char *)filename, "r", 0, NULL, context);
...
int mysqlnd_local_infile_read(void * ptr, zend_uchar * buf, unsigned int buf_len TSRMLS_DC)
{
	...
	count = (int)jonj_stream_read(info->fd, (char *) buf, buf_len);
...
void mysqlnd_local_infile_end(void * ptr TSRMLS_DC)
{
	...
	if (info) {
		/* jonj_stream_close segfaults on NULL */
		if (info->fd) {
			jonj_stream_close(info->fd);
			info->fd = NULL;
		}
		mnd_efree(info);
	}
...
mysqlnd_handle_local_infile(MYSQLND_CONN_DATA * conn, const char * filename, zend_bool * is_warning TSRMLS_DC)
{
	...
	if (!(conn->options->flags & CLIENT_LOCAL_FILES)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "LOAD DATA LOCAL INFILE forbidden");
