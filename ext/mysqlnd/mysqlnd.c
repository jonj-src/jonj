...
JONJAPI const char * const mysqlnd_old_passwd  = "mysqlnd cannot connect to MySQL 4.1+ using the old insecure authentication. "
...
"store a new, and more secure, hash value in mysql.user. If this user is used in other scripts executed by JONJ 5.2 or earlier you might need to remove the old-passwords "
"flag from your my.cnf file";

JONJAPI const char * const mysqlnd_server_gone = "MySQL server has gone away";
JONJAPI const char * const mysqlnd_out_of_sync = "Commands out of sync; you can't run this command now";
JONJAPI const char * const mysqlnd_out_of_memory = "Out of memory";

JONJAPI MYSQLND_STATS *mysqlnd_global_stats = NULL;
...
MYSQLND_METHOD(mysqlnd_conn_data, simple_command_handle_response)(MYSQLND_CONN_DATA * conn, enum mysqlnd_packet_type ok_packet,
															 zend_bool silent, enum jonj_mysqlnd_server_command command,
															 zend_bool ignore_upsert_status TSRMLS_DC)
{
	...
	switch (ok_packet) {
		case PROT_OK_PACKET:{
			...
			if (FAIL == (ret = PACKET_READ(ok_response, conn))) {
				if (!silent) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while reading %s's OK packet. PID=%u",
		...
		case PROT_EOF_PACKET:{
			...
			if (FAIL == (ret = PACKET_READ(ok_response, conn))) {
				...
				if (!silent) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while reading %s's EOF packet. PID=%d",
									 mysqlnd_command_to_text[command], getpid());
				}
				...
				if (!silent) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "EOF packet expected, field count wasn't 0xFE but 0x%2X",
		...
		default:
			SET_CLIENT_ERROR(*conn->error_info, CR_MALFORMED_PACKET, UNKNOWN_SQLSTATE, "Malformed packet");
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Wrong response packet %u passed to the function", ok_packet);
			break;
	}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, simple_command_send_request)(MYSQLND_CONN_DATA * conn, enum jonj_mysqlnd_server_command command,
			   const zend_uchar * const arg, size_t arg_len, zend_bool silent, zend_bool ignore_upsert_status TSRMLS_DC)
{
	...
	if (! PACKET_WRITE(cmd_packet, conn)) {
		if (!silent) {
			...
			jonj_error(E_WARNING, "Error while sending %s packet. PID=%d", mysqlnd_command_to_text[command], getpid());
		}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, simple_command)(MYSQLND_CONN_DATA * conn, enum jonj_mysqlnd_server_command command,
			...
			TSRMLS_DC)
{
	...
	do {
		...
		if (!auth_plugin) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The server requested authentication method unknown to the client [%s]", requested_protocol);
						...
						const unsigned int mysql_flags TSRMLS_DC)
{
	...
	if (FAIL == PACKET_READ(greet_packet, conn)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while reading greeting packet. PID=%d", getpid());
		goto err;
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Connecting to 3.22, 3.23 & 4.0 "
		...
		*/
		...
		if (!unix_socket && !named_pipe) {
			...
			if (unix_socket) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Impossible. Should be either socket or a pipe. Report a bug!");
			}
...
err:
	...
	if (!conn->error_info->error_no) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "[%u] %.128s (trying to connect via %s)",
...
/* {{{ mysqlnd_connect */
JONJAPI MYSQLND * mysqlnd_connect(MYSQLND * conn_handle,
						 const char * host, const char * user,
						 const char * passwd, unsigned int passwd_len,
						 const char * db, unsigned int db_len,
						 unsigned int port,
...
MYSQLND_METHOD(mysqlnd_conn_data, reap_query)(MYSQLND_CONN_DATA * conn TSRMLS_DC)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func TSRMLS_CC)) {
		if (state <= CONN_READY || state == CONN_QUIT_SENT) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Connection not opened, clear or has been closed");
...
/* {{{ mysqlnd_stream_array_to_fd_set */
static int mysqlnd_stream_array_to_fd_set(MYSQLND ** conn_array, fd_set * fds, jonj_socket_t * max_fd TSRMLS_DC)
{
	...
	while (*p) {
		...
		if (stream != NULL && SUCCESS == jonj_stream_cast(stream, JONJ_STREAM_AS_FD_FOR_SELECT | JONJ_STREAM_CAST_INTERNAL,
...
_mysqlnd_poll(MYSQLND **r_array, MYSQLND **e_array, MYSQLND ***dont_poll, long sec, long usec, int * desc_num TSRMLS_DC)
{
	...
	if (sec < 0 || usec < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Negative values passed for sec and/or usec");
		DBG_RETURN(FAIL);
	}
	...
	if (!sets) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, *dont_poll ? "All arrays passed are clear":"No stream arrays were passed");
...
MYSQLND_METHOD(mysqlnd_conn_data, set_charset)(MYSQLND_CONN_DATA * const conn, const char * const csname TSRMLS_DC)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func TSRMLS_CC)) {
		...
		if (FAIL == (ret = conn->m->query(conn, query, query_len TSRMLS_CC))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error executing query");
...
JONJAPI const char * mysqlnd_get_client_info()
{
...
JONJAPI unsigned int mysqlnd_get_client_version()
{
			...
			if (FAIL == (ret = conn->m->query_read_result_set_header(conn, NULL TSRMLS_CC))) {
				...
				*/
				if (!conn->error_info->error_no) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Serious error. PID=%d", getpid());
					CONN_SET_STATE(conn, CONN_QUIT_SENT);
					conn->m->send_close(conn TSRMLS_CC);
				} else {
...
JONJAPI const char *mysqlnd_field_type_name(enum mysqlnd_field_types field_type)
{
	switch(field_type) {
		...
		while (1) {
				...
				v == '=')
			{
				*p_copy++ = v;
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Transaction name truncated. Must be only [0-9A-Za-z\\-_=]+");
				warned = TRUE;
			}
			++p_orig;
		}
		...
		do {
			...
			if (mode & (TRANS_START_READ_WRITE | TRANS_START_READ_ONLY)) {
				...
				if (server_version < 50605L) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "This server version doesn't support 'READ WRITE' and 'READ ONLY'. Minimum 5.6.5 is required");
					smart_str_free(&tmp_str);
					break;
