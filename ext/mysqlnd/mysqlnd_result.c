...
mysqlnd_query_read_result_set_header(MYSQLND_CONN_DATA * conn, MYSQLND_STMT * s TSRMLS_DC)
{
	...
	do {
		...
		if (FAIL == (ret = PACKET_READ(rset_header, conn))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error reading result set's header");
			break;
		}
				...
				if (FAIL == (ret = PACKET_READ(fields_eof, conn))) {
					...
					*/
					...
					if (to_log) {
						...
						jonj_log_err(backtrace TSRMLS_CC);
						efree(backtrace);
#endif
					}
...
/*
  Do lazy initialization for buffered results. As JONJ strings have
  length inside, this function makes not much sense in the context
  of JONJ, to be called as separate function. But let's have it for
  completeness.
*/
													...
													zend_bool binary_protocol TSRMLS_DC)
{
	...
	row_packet->skip_extraction = TRUE; /* let jonj_mysqlnd_rowp_read() not allocate row_packet->fields, we will do it */
										...
										enum_mysqlnd_extension extension TSRMLS_DC ZEND_FILE_LINE_DC)
{
	...
	*/
	...
	if (FAIL == result->m.fetch_row(result, (void *)return_value, flags, &fetched_anything TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while reading a row");
...
MYSQLND_METHOD(mysqlnd_res, fetch_row_c)(MYSQLND_RES * result TSRMLS_DC)
{
	...
	if (result->stored_data && result->stored_data->m.fetch_row == MYSQLND_METHOD(mysqlnd_result_buffered_zval, fetch_row)) {
		...
		ret = NULL;
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "result->m.fetch_row has invalid value. Report to the developers");
	}
	DBG_RETURN(ret);
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_res, fetch_all)(MYSQLND_RES * result, const unsigned int flags, zval *return_value TSRMLS_DC ZEND_FILE_LINE_DC)
{
	...
	if ((!result->unbuf && !set)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "fetch_all can be used only with buffered sets");
		if (result->conn) {
...
JONJAPI MYSQLND_RES_UNBUFFERED *
mysqlnd_result_unbuffered_init(unsigned int field_count, zend_bool ps, zend_bool persistent TSRMLS_DC)
{
	...
	if (ps) {
		...
		ret->m.row_decoder	= jonj_mysqlnd_rowp_read_binary_protocol;
	} else {
		ret->m.row_decoder	= jonj_mysqlnd_rowp_read_text_protocol_zval;
	}
...
JONJAPI MYSQLND_RES_BUFFERED_ZVAL *
...
JONJAPI MYSQLND_RES_BUFFERED_C *
mysqlnd_result_buffered_c_init(unsigned int field_count, zend_bool ps, zend_bool persistent TSRMLS_DC)
{
	...
	if (ps) {
		...
		ret->m.row_decoder	= jonj_mysqlnd_rowp_read_text_protocol_c;
	}
