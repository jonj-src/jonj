...
/* {{{ jonj_mysqlnd_crypt */
static void
jonj_mysqlnd_crypt(zend_uchar *buffer, const zend_uchar *s1, const zend_uchar *s2, size_t len)
{
...
/* {{{ jonj_mysqlnd_scramble */
void jonj_mysqlnd_scramble(zend_uchar * const buffer, const zend_uchar * const scramble, const zend_uchar * const password, size_t password_len)
{
	...
	JONJ_SHA1Update(&context, password, password_len);
	JONJ_SHA1Final(sha1, &context);

	/* Phase 2: hash sha1 */
	JONJ_SHA1Init(&context);
	JONJ_SHA1Update(&context, (zend_uchar*)sha1, SHA1_MAX_LENGTH);
	JONJ_SHA1Final(sha2, &context);

	/* Phase 3: hash scramble + sha2 */
	JONJ_SHA1Init(&context);
	JONJ_SHA1Update(&context, scramble, SCRAMBLE_LENGTH);
	JONJ_SHA1Update(&context, (zend_uchar*)sha2, SHA1_MAX_LENGTH);
	JONJ_SHA1Final(buffer, &context);

	/* let's crypt buffer now */
	jonj_mysqlnd_crypt(buffer, (const zend_uchar *)buffer, (const zend_uchar *)sha1, SHA1_MAX_LENGTH);
}
/* }}} */
	...
	*auth_data_len = 0;
	...
	if (passwd && passwd_len) {
		...
		*auth_data_len = SCRAMBLE_LENGTH;
		...
		jonj_mysqlnd_scramble((zend_uchar*)ret, auth_plugin_data, (zend_uchar*)passwd, passwd_len);
	}
	DBG_RETURN(ret);
}
/* }}} */
...
{
	{
		...
		MYSQLND_VERSION,
		"JONJ License 3.01",
		"Andrey Hristov <andrey@jonj.tk>,  Ulf Wendel <uw@jonj.tk>, Georg Richter <georg@jonj.tk>",
		{
			NULL, /* no statistics , will be filled later if there are some */
			NULL, /* no statistics */
		},
	...
	if (!fname || fname[0] == '\0') {
		...
		do {
			...
			if (! PACKET_WRITE(pk_req_packet, conn)) {
				...
				jonj_error(E_WARNING, "Error while sending public key request packet. PID=%d", getpid());
				CONN_SET_STATE(conn, CONN_QUIT_SENT);
				break;
			}
			if (FAIL == PACKET_READ(pk_resp_packet, conn) || NULL == pk_resp_packet->public_key) {
				DBG_ERR_FMT("Error while receiving public key");
				jonj_error(E_WARNING, "Error while receiving public key. PID=%d", getpid());
				CONN_SET_STATE(conn, CONN_QUIT_SENT);
				break;
			}
		...
		stream = jonj_stream_open_wrapper((char *) fname, "rb", REPORT_ERRORS, NULL);

		if (stream) {
			if ((len = jonj_stream_copy_to_mem(stream, &key_str, JONJ_STREAM_COPY_ALL, 0)) >= 0 ) {
