...
/* {{{ jonj_mysqlnd_free_field_metadata */
static void
jonj_mysqlnd_free_field_metadata(MYSQLND_FIELD *meta, zend_bool persistent TSRMLS_DC)
{
	if (meta) {
		if (meta->root) {
...
MYSQLND_METHOD(mysqlnd_res_meta, free)(MYSQLND_RES_METADATA * meta TSRMLS_DC)
{
	...
	if ((fields = meta->fields)) {
		...
		i = meta->field_count;
		while (i--) {
			jonj_mysqlnd_free_field_metadata(fields++, meta->persistent TSRMLS_CC);
		}
...
JONJAPI struct st_mysqlnd_res_meta_methods *
