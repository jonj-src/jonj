...
static int
mysqlnd_set_sock_no_delay(jonj_stream * stream TSRMLS_DC)
{

	int socketd = ((jonj.tkstream_data_t*)stream->abstract)->socket;
	int ret = SUCCESS;
	int flag = 1;
...
static int
mysqlnd_set_sock_keepalive(jonj_stream * stream TSRMLS_DC)
{
											 ...
											 MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	jonj_stream * net_stream = net->data->m.get_stream(net TSRMLS_CC);
	...
	while (to_read) {
		if (!(ret = jonj_stream_read(net_stream, (char *) p, to_read))) {
											  ...
											  MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	ret = jonj_stream_write(net->data->m.get_stream(net TSRMLS_CC), (char *)buffer, count);
	DBG_RETURN(ret);
}
/* }}} */
									   ...
									   MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	net_stream = jonj_stream_open_wrapper((char*) scheme + sizeof("pipe://") - 1, "r+", streams_options, NULL);
	if (!net_stream) {
											  ...
											  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	DBG_INF_FMT("calling jonj_stream_xport_create");
	net_stream = jonj_stream_xport_create(scheme, scheme_len, streams_options, streams_flags,
												  ...
												  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	if (net_stream) {
		if (net->data->options.timeout_read) {
			struct timeval tv;
			DBG_INF_FMT("setting %u as JONJ_STREAM_OPTION_READ_TIMEOUT", net->data->options.timeout_read);
			tv.tv_sec = net->data->options.timeout_read;
			tv.tv_usec = 0;
			jonj_stream_set_option(net_stream, JONJ_STREAM_OPTION_READ_TIMEOUT, 0, &tv);
		}
										...
										MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	if (open_stream) {
		jonj_stream * net_stream = open_stream(net, scheme, scheme_len, persistent, conn_stats, error_info TSRMLS_CC);
		if (net_stream) {
...
#ifdef MYSQLND_COMPRESSION_ENABLED
/* {{{ jonj_mysqlnd_read_buffer_is_empty */
static zend_bool
jonj_mysqlnd_read_buffer_is_empty(MYSQLND_READ_BUFFER * buffer)
{
...
/* {{{ jonj_mysqlnd_read_buffer_read */
static void
jonj_mysqlnd_read_buffer_read(MYSQLND_READ_BUFFER * buffer, size_t count, zend_uchar * dest)
{
	if (buffer->len >= count) {
...
/* {{{ jonj_mysqlnd_read_buffer_bytes_left */
static size_t
jonj_mysqlnd_read_buffer_bytes_left(MYSQLND_READ_BUFFER * buffer)
{
	return buffer->len;
}
/* }}} */
...
/* {{{ jonj_mysqlnd_read_buffer_free */
static void
jonj_mysqlnd_read_buffer_free(MYSQLND_READ_BUFFER ** buffer TSRMLS_DC)
{
	DBG_ENTER("jonj_mysqlnd_read_buffer_free");
	if (*buffer) {
...
/* {{{ jonj_mysqlnd_create_read_buffer */
...
mysqlnd_create_read_buffer(size_t count TSRMLS_DC)
{
	...
	ret->is_empty = jonj_mysqlnd_read_buffer_is_empty;
	ret->read = jonj_mysqlnd_read_buffer_read;
	ret->bytes_left = jonj_mysqlnd_read_buffer_bytes_left;
	ret->free_buffer = jonj_mysqlnd_read_buffer_free;
...
size_t 
MYSQLND_METHOD(mysqlnd_net, consume_uneaten_data)(MYSQLND_NET * const net, enum jonj_mysqlnd_server_command cmd TSRMLS_DC)
{
	...
	*/
	...
	int opt = JONJ_STREAM_OPTION_BLOCKING;
	jonj_stream * net_stream = net->data->get_stream(net TSRMLS_CC);
	int was_blocked = net_stream->ops->set_option(net_stream, opt, 0, NULL TSRMLS_CC);

	DBG_ENTER("mysqlnd_net::consume_uneaten_data");

	if (JONJ_STREAM_OPTION_RETURN_ERR != was_blocked) {
		/* Do a read of 1 byte */
		int bytes_consumed;

		do {
			skipped_bytes += (bytes_consumed = jonj_stream_read(net_stream, tmp_buf, sizeof(tmp_buf)));
		...
		if (bytes_consumed) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Skipped %u bytes. Last command %s hasn't "
...
MYSQLND_METHOD(mysqlnd_net, enable_ssl)(MYSQLND_NET * const net TSRMLS_DC)
{
	...
	jonj_stream_context * context = jonj_stream_context_alloc(TSRMLS_C);
	jonj_stream * net_stream = net->data->m.get_stream(net TSRMLS_CC);
	...
	if (net->data->options.ssl_key) {
		...
		jonj_stream_context_set_option(context, "ssl", "local_pk", &key_zval);
		any_flag = TRUE;
	}
	if (net->data->options.ssl_cert) {
		zval cert_zval;
		ZVAL_STRING(&cert_zval, net->data->options.ssl_cert, 0);
		jonj_stream_context_set_option(context, "ssl", "local_cert", &cert_zval);
		if (!net->data->options.ssl_key) {
			jonj_stream_context_set_option(context, "ssl", "local_pk", &cert_zval);
		}
		any_flag = TRUE;
	}
	if (net->data->options.ssl_ca) {
		...
		jonj_stream_context_set_option(context, "ssl", "cafile", &cafile_zval);
		any_flag = TRUE;
	}
	if (net->data->options.ssl_capath) {
		zval capath_zval;
		ZVAL_STRING(&capath_zval, net->data->options.ssl_capath, 0);
		jonj_stream_context_set_option(context, "ssl", "capath", &capath_zval);
		any_flag = TRUE;
	}
	if (net->data->options.ssl_passphrase) {
		zval passphrase_zval;
		ZVAL_STRING(&passphrase_zval, net->data->options.ssl_passphrase, 0);
		jonj_stream_context_set_option(context, "ssl", "passphrase", &passphrase_zval);
		any_flag = TRUE;
	}
	if (net->data->options.ssl_cipher) {
		zval cipher_zval;
		ZVAL_STRING(&cipher_zval, net->data->options.ssl_cipher, 0);
		jonj_stream_context_set_option(context, "ssl", "ciphers", &cipher_zval);
		any_flag = TRUE;
	}
	{
		...
		jonj_stream_context_set_option(context, "ssl", "verify_peer", &verify_peer_zval);
		jonj_stream_context_set_option(context, "ssl", "verify_peer_name", &verify_peer_zval);
		if (net->data->options.ssl_verify_peer == MYSQLND_SSL_PEER_DONT_VERIFY) {
			ZVAL_TRUE(&verify_peer_zval);
			jonj_stream_context_set_option(context, "ssl", "allow_self_signed", &verify_peer_zval);
		}
	}

	jonj_stream_context_set(net_stream, context);
	if (jonj_stream_xport_crypto_setup(net_stream, STREAM_CRYPTO_METHOD_TLS_CLIENT, NULL TSRMLS_CC) < 0 ||
	    jonj_stream_xport_crypto_enable(net_stream, 1 TSRMLS_CC) < 0)
	{
		DBG_ERR("Cannot connect to MySQL by using SSL");
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot connect to MySQL by using SSL");
		DBG_RETURN(FAIL);
	}
	net->data->ssl = TRUE;
	/*
	...
	*/
	jonj_stream_context_set(net_stream, NULL);
...
MYSQLND_METHOD(mysqlnd_net, close_stream)(MYSQLND_NET * const net, MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info TSRMLS_DC)
{
	...
	if (net && (net_stream = net->data->m.get_stream(net TSRMLS_CC))) {
		...
		if (pers) {
			if (EG(active)) {
				jonj_stream_free(net_stream, JONJ_STREAM_FREE_CLOSE_PERSISTENT | JONJ_STREAM_FREE_RSRC_DTOR);
			} else {
				/*
				  otherwise we will crash because the EG(persistent_list) has been freed already,
				  before the modules are shut down
				*/
				jonj_stream_free(net_stream, JONJ_STREAM_FREE_CLOSE | JONJ_STREAM_FREE_RSRC_DTOR);
			}
		} else {
			jonj_stream_free(net_stream, JONJ_STREAM_FREE_CLOSE);
		}
...
static jonj_stream *
MYSQLND_METHOD(mysqlnd_net, set_stream)(MYSQLND_NET * const net, jonj_stream * net_stream TSRMLS_DC)
{
	jonj_stream * ret = NULL;
