...
enum {
	JONJ_PCRE_NO_ERROR = 0,
	JONJ_PCRE_INTERNAL_ERROR,
	JONJ_PCRE_BACKTRACK_LIMIT_ERROR,
	JONJ_PCRE_RECURSION_LIMIT_ERROR,
	JONJ_PCRE_BAD_UTF8_ERROR,
	JONJ_PCRE_BAD_UTF8_OFFSET_ERROR
};
...
static void pcre_handle_exec_error(int pcre_code TSRMLS_DC) /* {{{ */
{
	...
	switch (pcre_code) {
		case PCRE_ERROR_MATCHLIMIT:
			preg_code = JONJ_PCRE_BACKTRACK_LIMIT_ERROR;
			break;

		case PCRE_ERROR_RECURSIONLIMIT:
			preg_code = JONJ_PCRE_RECURSION_LIMIT_ERROR;
			break;

		case PCRE_ERROR_BADUTF8:
			preg_code = JONJ_PCRE_BAD_UTF8_ERROR;
			break;

		case PCRE_ERROR_BADUTF8_OFFSET:
			preg_code = JONJ_PCRE_BAD_UTF8_OFFSET_ERROR;
			break;

		default:
			preg_code = JONJ_PCRE_INTERNAL_ERROR;
			break;
	}
...
static void jonj_free_pcre_cache(void *data) /* {{{ */
{
...
static JONJ_GINIT_FUNCTION(pcre) /* {{{ */
{
	zend_hash_init(&pcre_globals->pcre_cache, 0, NULL, jonj_free_pcre_cache, 1);
	pcre_globals->backtrack_limit = 0;
	pcre_globals->recursion_limit = 0;
	pcre_globals->error_code      = JONJ_PCRE_NO_ERROR;
}
/* }}} */

static JONJ_GSHUTDOWN_FUNCTION(pcre) /* {{{ */
{
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("pcre.backtrack_limit", "1000000", JONJ_INI_ALL, OnUpdateLong, backtrack_limit, zend_pcre_globals, pcre_globals)
	STD_JONJ_INI_ENTRY("pcre.recursion_limit", "100000", JONJ_INI_ALL, OnUpdateLong, recursion_limit, zend_pcre_globals, pcre_globals)
JONJ_INI_END()


/* {{{ JONJ_MINFO_FUNCTION(pcre) */
static JONJ_MINFO_FUNCTION(pcre)
{
	jonj_info_print_table_start();
	jonj_info_print_table_row(2, "PCRE (Perl Compatible Regular Expressions) Support", "enabled" );
	jonj_info_print_table_row(2, "PCRE Library Version", pcre_version() );
...
/* {{{ JONJ_MINIT_FUNCTION(pcre) */
static JONJ_MINIT_FUNCTION(pcre)
{
	...
	REGISTER_LONG_CONSTANT("PREG_NO_ERROR", JONJ_PCRE_NO_ERROR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PREG_INTERNAL_ERROR", JONJ_PCRE_INTERNAL_ERROR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PREG_BACKTRACK_LIMIT_ERROR", JONJ_PCRE_BACKTRACK_LIMIT_ERROR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PREG_RECURSION_LIMIT_ERROR", JONJ_PCRE_RECURSION_LIMIT_ERROR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PREG_BAD_UTF8_ERROR", JONJ_PCRE_BAD_UTF8_ERROR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PREG_BAD_UTF8_OFFSET_ERROR", JONJ_PCRE_BAD_UTF8_OFFSET_ERROR, CONST_CS | CONST_PERSISTENT);
...
/* {{{ JONJ_MSHUTDOWN_FUNCTION(pcre) */
static JONJ_MSHUTDOWN_FUNCTION(pcre)
{
...
static char **make_subpats_table(int num_subpats, pcre_cache_entry *pce TSRMLS_DC)
{
	...
	if (rc < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Internal pcre_fullinfo() error %d", rc);
		efree(subpat_names);
		return NULL;
	}
	if (name_cnt > 0) {
		...
		while (ni++ < name_cnt) {
			...
			if (is_numeric_string(subpat_names[name_idx], strlen(subpat_names[name_idx]), NULL, NULL, 0) > 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Numeric named subpatterns are not allowed");
				efree(subpat_names);
				return NULL;
			}
...
# endif
	...
	if (isalnum((int)*(unsigned char *)&delimiter) || delimiter == '\\') {
		jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "Delimiter must not be alphanumeric or backslash");
		return NULL;
	}
	...
	if (*pp == 0) {
		if (pp < regex + regex_len) {
			jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "Null byte in regex");
		} else if (start_delimiter == end_delimiter) {
			jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "No ending delimiter '%c' found", delimiter);
		} else {
			jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "No ending matching delimiter '%c' found", delimiter);
		}
		return NULL;
	}
	...
	/* In  PCRE,  by  default, \d, \D, \s, \S, \w, and \W recognize only ASCII
       ...
       the PCRE_UCP option. */
			...
			default:
				if (pp[-1]) {
					jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "Unknown modifier '%c'", pp[-1]);
				} else {
					jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "Null byte in regex");
				}
				efree(pattern);
	...
	if (re == NULL) {
		jonj_error_docref(NULL TSRMLS_CC,E_WARNING, "Compilation failed: %s at offset %d", error, erroffset);
		efree(pattern);
		if (tables) {
	...
	if (do_study) {
		...
		if (error != NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while studying pattern");
		}
	} else {
		extra = NULL;
	}
 ...
 */
JONJAPI pcre* pcre_get_compiled_regex(char *regex, pcre_extra **extra, int *preg_options TSRMLS_DC)
{
 ...
 */
JONJAPI pcre* pcre_get_compiled_regex_ex(char *regex, pcre_extra **extra, int *preg_options, int *compile_options TSRMLS_DC)
{
	pcre_cache_entry * pce = pcre_get_compiled_regex_cache(regex, strlen(regex) TSRMLS_CC);
...
static void jonj_do_pcre_match(INTERNAL_FUNCTION_PARAMETERS, int global) /* {{{ */
{
	/* parameters */
	...
	pce->refcount++;
	jonj_pcre_match_impl(pce, subject, subject_len, return_value, subpats,
		global, ZEND_NUM_ARGS() >= 4, flags, start_offset TSRMLS_CC);
	pce->refcount--;
}
/* }}} */

/* {{{ jonj_pcre_match_impl() */
	...
	zval *subpats, int global, int use_flags, long flags, long start_offset TSRMLS_DC)
{
	...
	if (use_flags) {
		...
		if ((global && (subpats_order < PREG_PATTERN_ORDER || subpats_order > PREG_SET_ORDER)) ||
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid flags specified");
			return;
		}
	} else {
		offset_capture = 0;
	...
	matched = 0;
	PCRE_G(error_code) = JONJ_PCRE_NO_ERROR;
	...
	do {
		...
		if (count == 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Matched, but too many substrings");
		...
		if (count > 0) {
			...
			if (subpats != NULL) {
				...
				if ((offsets[1] - offsets[0] < 0) || pcre_get_substring_list(subject, offsets, count, &stringlist) < 0) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Get subpatterns list failed");
					RETURN_FALSE;
				}
	...
	if (PCRE_G(error_code) == JONJ_PCRE_NO_ERROR) {
		RETVAL_LONG(matched);
	} else {
		RETVAL_FALSE;
	}
...
static JONJ_FUNCTION(preg_match)
{
	jonj_do_pcre_match(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ proto int preg_match_all(string pattern, string subject [, array &subpatterns [, int flags [, int offset]]])
   Perform a Perl-style global regular expression match */
static JONJ_FUNCTION(preg_match_all)
{
	jonj_do_pcre_match(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
						...
						int *offsets, int count, char **result TSRMLS_DC)
{
		...
		if ('\\' == *walk || '$' == *walk) {
			...
			if (preg_get_backref(&walk, &backref)) {
				if (backref < count) {
					...
					if (match_len) {
						esc_match = jonj_addslashes(match, match_len, &esc_match_len, 0 TSRMLS_CC);
					} else {
						esc_match = match;
						esc_match_len = 0;
					}
	...
	if (zend_eval_stringl(code.c, code.len, &retval, compiled_string_description TSRMLS_CC) == FAILURE) {
		...
		jonj_error_docref(NULL TSRMLS_CC,E_ERROR, "Failed evaluating code: %s%s", JONJ_EOL, code.c);
...
/* {{{ jonj_pcre_replace
 */
JONJAPI char *jonj_pcre_replace(char *regex,   int regex_len,
							  ...
							  int *result_len, int limit, int *replace_count TSRMLS_DC)
{
	...
	pce->refcount++;
	result = jonj_pcre_replace_impl(pce, subject, subject_len, replace_val,
...
/* {{{ jonj_pcre_replace_impl() */
JONJAPI char *jonj_pcre_replace_impl(pcre_cache_entry *pce, char *subject, int subject_len, zval *replace_val,
	int is_callable_replace, int *result_len, int limit, int *replace_count TSRMLS_DC)
{
	...
	if (is_callable_replace) {
		if (eval) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Modifier /e cannot be used with replacement callback");
			return NULL;
		}
	} else {
	...
	if (eval) {
		jonj_error_docref(NULL TSRMLS_CC, E_DEPRECATED, "The /e modifier is deprecated, use preg_replace_callback instead");
	}
	...
	*result_len = 0;
	...
	if(result && (size_t)(*result_len) > INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Result is too big, max is %d", INT_MAX);
		efree(result);
		result = NULL;
	}
...
/* {{{ jonj_replace_in_subject
 */
static char *jonj_replace_in_subject(zval *regex, zval *replace, zval **subject, int *result_len, int limit, int is_callable_replace, int *replace_count TSRMLS_DC)
{
	zval		**regex_entry,
				**replace_entry = NULL,
				 *replace_value,
	...
	if (Z_TYPE_P(regex) == IS_ARRAY) {
		...
		*result_len = subject_len;
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(regex), (void **)&regex_entry) == SUCCESS) {
			...
			if ((result = jonj_pcre_replace(Z_STRVAL_PP(regex_entry),
		...
		result = jonj_pcre_replace(Z_STRVAL_P(regex),
								  Z_STRLEN_P(regex),
								  Z_STRVAL_PP(subject),
								  Z_STRLEN_PP(subject),
								  replace,
...
static void preg_replace_impl(INTERNAL_FUNCTION_PARAMETERS, int is_callable_replace, int is_filter)
{
	...
	if (!is_callable_replace && Z_TYPE_PP(replace) == IS_ARRAY && Z_TYPE_PP(regex) != IS_ARRAY) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parameter mismatch, pattern is a string while replacement is an array");
		RETURN_FALSE;
	}
	...
	if (is_callable_replace) {
		if (!zend_is_callable(*replace, 0, &callback_name TSRMLS_CC)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Requires argument 2, '%s', to be a valid callback", callback_name);
			efree(callback_name);
	...
	if (Z_TYPE_PP(subject) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_PP(subject), (void **)&subject_entry) == SUCCESS) {
			...
			if ((result = jonj_replace_in_subject(*regex, *replace, subject_entry, &result_len, limit_val, is_callable_replace, &replace_count TSRMLS_CC)) != NULL) {
		...
		old_replace_count = replace_count;
		if ((result = jonj_replace_in_subject(*regex, *replace, subject, &result_len, limit_val, is_callable_replace, &replace_count TSRMLS_CC)) != NULL) {
			if (!is_filter || replace_count > old_replace_count) {
...
static JONJ_FUNCTION(preg_replace)
{
...
static JONJ_FUNCTION(preg_replace_callback)
{
...
static JONJ_FUNCTION(preg_filter)
{
...
static JONJ_FUNCTION(preg_split)
{
	char				*regex;			/* Regular expression */
	char				*subject;		/* String to match against */
	int					 regex_len;
	...
	pce->refcount++;
	jonj_pcre_split_impl(pce, subject, subject_len, return_value, limit_val, flags TSRMLS_CC);
	pce->refcount--;
}
/* }}} */
...
static JONJ_FUNCTION(preg_quote)
{
	int		 in_str_len;
...
static JONJ_FUNCTION(preg_grep)
{
	char				*regex;			/* Regular expression */
	int				 	 regex_len;
	...
	pce->refcount++;
	jonj_pcre_grep_impl(pce, input, return_value, flags TSRMLS_CC);
	pce->refcount--;
}
/* }}} */

JONJAPI void  jonj_pcre_grep_impl(pcre_cache_entry *pce, zval *input, zval *return_value, long flags TSRMLS_DC) /* {{{ */
{
...
static JONJ_FUNCTION(preg_last_error)
{
...
static const zend_function_entry pcre_functions[] = {
	JONJ_FE(preg_match,				arginfo_preg_match)
	JONJ_FE(preg_match_all,			arginfo_preg_match_all)
	JONJ_FE(preg_replace,			arginfo_preg_replace)
	JONJ_FE(preg_replace_callback,	arginfo_preg_replace_callback)
	JONJ_FE(preg_filter,				arginfo_preg_replace)
	JONJ_FE(preg_split,				arginfo_preg_split)
	JONJ_FE(preg_quote,				arginfo_preg_quote)
	JONJ_FE(preg_grep,				arginfo_preg_grep)
	JONJ_FE(preg_last_error,			arginfo_preg_last_error)
	JONJ_FE_END
};
