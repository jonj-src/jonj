								   ...
								   zval ***args, unsigned int start, const char * const types TSRMLS_DC)
{
	...
	for (i = start; i < argc; i++) {
		...
		switch (types[ofs]) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Undefined fieldtype %c (parameter %d)", types[ofs], i+1);
				rc = 1;
				goto end_1;
		}
		ofs++;
	...
	for (i = 0; i < (argc - start); i++) {
		...
		switch (types[i]) {
			...
			default:
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Undefined fieldtype %c (parameter %d)", types[i], i + start + 1);
				ret = FAIL;
...
JONJ_FUNCTION(mysqli_stmt_bind_param)
{
	...
	if (!types_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid type or no types specified");
		RETURN_FALSE;
	}

	if (types_len != argc - start) {
		/* number of bind variables doesn't match number of elements in type definition string */
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of elements in type definition string doesn't match number of bind variables");
		RETURN_FALSE;
	}

	if (types_len != mysql_stmt_param_count(stmt->stmt)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of variables doesn't match number of parameters in prepared statement");
		RETURN_FALSE;
	}
...
mysqli_stmt_bind_result_do_bind(MY_STMT *stmt, zval ***args, unsigned int argc, unsigned int start TSRMLS_DC)
{
	...
	for (i=start; i < var_cnt + start ; i++) {
		...
		switch (col_type) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Server returned unknown type %ld. Probably your client library is incompatible with the server version you use!", col_type);
				break;
		}
	}
...
JONJ_FUNCTION(mysqli_stmt_bind_result)
{
	...
	if ((argc - start) != mysql_stmt_field_count(stmt->stmt)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of bind variables doesn't match number of fields in prepared statement");
		RETURN_FALSE;
	}
...
/* {{{ jonj_mysqli_close */
void jonj_mysqli_close(MY_MYSQL * mysql, int close_type, int resource_status TSRMLS_DC)
{
	...
	if (!mysql->persistent) {
		...
		if (zend_hash_find(&EG(persistent_list), mysql->hash_key, strlen(mysql->hash_key) + 1, (void **)&le) == SUCCESS) {
			if (Z_TYPE_P(le) == jonj_le_pmysqli()) {
...
JONJ_FUNCTION(mysqli_close)
{
	...
	jonj_mysqli_close(mysql, MYSQLI_CLOSE_EXPLICIT, ((MYSQLI_RESOURCE *)((mysqli_object *)zend_object_store_get_object(mysql_link TSRMLS_CC))->ptr)->status TSRMLS_CC);
...
JONJ_FUNCTION(mysqli_data_seek)
{
	...
	if (mysqli_result_is_unbuffered(result)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Function cannot be used with MYSQL_USE_RESULT");
		RETURN_FALSE;
	}
...
#ifndef MYSQLI_USE_MYSQLND
/* {{{ jonj_mysqli_stmt_copy_it */
static void
jonj_mysqli_stmt_copy_it(zval *** copies, zval *original, uint param_count, uint current)
{
	if (!*copies) {
...
JONJ_FUNCTION(mysqli_stmt_execute)
{
		...
		for (i = 0; i < stmt->param.var_cnt; i++) {
			for (j = i + 1; j < stmt->param.var_cnt; j++) {
				...
				if (stmt->param.vars[j] == stmt->param.vars[i] && stmt->param.vars[i]) {
					jonj_mysqli_stmt_copy_it(&copies, stmt->param.vars[i], stmt->param.var_cnt, i);
					break;
				}
			}
		}
	...
	if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
		jonj_mysqli_report_index(stmt->query, mysqli_stmt_server_status(stmt->stmt) TSRMLS_CC);
	}
}
/* }}} */
...
/* {{{  jonj_add_field_properties */
static void jonj_add_field_properties(zval *value, const MYSQL_FIELD *field TSRMLS_DC)
{
...
JONJ_FUNCTION(mysqli_fetch_field)
{
	...
	jonj_add_field_properties(return_value, field TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mysqli_fetch_fields)
{
	...
	for (i = 0; i < mysql_num_fields(result); i++) {
		...
		jonj_add_field_properties(obj, field TSRMLS_CC);
...
JONJ_FUNCTION(mysqli_fetch_field_direct)
{
	...
	if (offset < 0 || offset >= (long) mysql_num_fields(result)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Field offset is invalid for resultset");
		RETURN_FALSE;
	}
...
{
	jonj_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MYSQLI_NUM, 0);
}
/* }}} */
...
JONJ_FUNCTION(mysqli_field_seek)
{
	...
	if (fieldnr < 0 || fieldnr >= mysql_num_fields(result)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid field offset");
		RETURN_FALSE;
	}
...
{
	jonj_mysqli_init(INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
/* }}} */
...
JONJ_FUNCTION(mysqli_kill)
{
	...
	if (processid <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "processid should have positive value");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mysqli_next_result) {
	...
	if (!mysql_more_results(mysql->mysql)) {
		jonj_error_docref(NULL TSRMLS_CC, E_STRICT, "There is no next result set. "
...
#endif /* mysqlnd @ JONJ 5.3.2 */
#ifdef MYSQL_OPT_SSL_VERIFY_SERVER_CERT
	REGISTER_LONG_CONSTANT("MYSQLI_OPT_SSL_VERIFY_SERVER_CERT", MYSQL_OPT_SSL_VERIFY_SERVER_CERT, CONST_CS | CONST_PERSISTENT);
#endif /* MySQL 5.1.1., mysqlnd @ JONJ 5.3.3 */
...
JONJ_FUNCTION(mysqli_prepare)
{
	...
	if (mysql->mysql->status == MYSQL_STATUS_GET_RESULT) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "All data must be fetched before a new statement prepare takes place");
		RETURN_FALSE;
	}
#endif
...
JONJ_FUNCTION(mysqli_real_query)
{
	...
	if (!mysql_field_count(mysql->mysql)) {
		if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
			jonj_mysqli_report_index(query, mysqli_server_status(mysql->mysql) TSRMLS_CC);
		}
	}
...
JONJ_FUNCTION(mysqli_stmt_send_long_data)
{
	...
	if (param_nr < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parameter number");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mysqli_stmt_data_seek)
{
	...
	if (offset < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset must be positive");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mysqli_stmt_attr_set)
{
	...
	if (mode_in < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mode should be non-negative, %ld passed", mode_in);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mysqli_stmt_store_result)
{
...
#if !defined(MYSQLI_USE_MYSQLND)
	{
		/*
		  ...
		  BLOB/TEXT columns after calling store_result() the memory usage of JONJ will
...
JONJ_FUNCTION(mysqli_store_result)
{
	...
	if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
		jonj_mysqli_report_index("from previous query", mysqli_server_status(mysql->mysql) TSRMLS_CC);
	}
