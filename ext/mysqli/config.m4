...
JONJ_ARG_WITH(mysqli, for MySQLi support,
[  --with-mysqli[=FILE]      Include MySQLi support.  FILE is the path
                          to mysql_config.  If no value or mysqlnd is passed 
                          as FILE, the MySQL native driver will be used])

JONJ_ARG_ENABLE(embedded_mysqli, whether to enable embedded MySQLi support,
[  --enable-embedded-mysqli  
                          MYSQLi: Enable embedded support
                          Note: Does not work with MySQL native driver!], no, no)

if test "$JONJ_MYSQLI" = "yes" || test "$JONJ_MYSQLI" = "mysqlnd"; then
  dnl This needs to be set in any extension which wishes to use mysqlnd
  JONJ_MYSQLND_ENABLED=yes

elif test "$JONJ_MYSQLI" != "no"; then

  MYSQL_CONFIG=$JONJ_MYSQLI

  MYSQL_LIB_NAME='mysqlclient'
  if test "$JONJ_EMBEDDED_MYSQLI" = "yes"; then
  ...
  dnl
  JONJ_CHECK_LIBRARY($MYSQL_LIB_NAME, mysql_set_server_option,
  [
    JONJ_EVAL_INCLINE($MYSQLI_INCLINE)
    JONJ_EVAL_LIBLINE($MYSQLI_LIBLINE, MYSQLI_SHARED_LIBADD)
    AC_DEFINE(HAVE_MYSQLILIB, 1, [ ])
    JONJ_CHECK_LIBRARY($MYSQL_LIB_NAME, mysql_set_character_set,
    [ ],[
  ...
  dnl
  JONJ_CHECK_LIBRARY($MYSQL_LIB_NAME, mysql_stmt_next_result,
  [
...
if test "$JONJ_MYSQLI" != "no"; then
  ...
  JONJ_NEW_EXTENSION(mysqli, $mysqli_sources, $ext_shared)
  JONJ_SUBST(MYSQLI_SHARED_LIBADD)
  JONJ_INSTALL_HEADERS([ext/mysqli/jonj_mysqli_structs.h])

  if test "$JONJ_MYSQLI" = "yes" || test "$JONJ_MYSQLI" = "mysqlnd"; then
    JONJ_ADD_EXTENSION_DEP(mysqli, mysqlnd)
    AC_DEFINE([MYSQLI_USE_MYSQLND], 1, [Whether mysqlnd is enabled])
    JONJ_INSTALL_HEADERS([ext/mysqli/mysqli_mysqlnd.h])
  else
    JONJ_INSTALL_HEADERS([ext/mysqli/mysqli_libmysql.h])
  fi
fi
