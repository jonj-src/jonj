...
#ifndef JONJ_MYSQLI_STRUCTS_H
#define JONJ_MYSQLI_STRUCTS_H
...
#ifdef JONJ_WIN32
#define JONJ_MYSQLI_API __declspec(dllexport)
...
#  define JONJ_MYSQLI_API __attribute__ ((visibility("default")))
# else
#  define JONJ_MYSQLI_API
# endif
...
extern zend_object_iterator *jonj_mysqli_result_get_iterator(zend_class_entry *ce, zval *object, int by_ref TSRMLS_DC);

extern void jonj_mysqli_fetch_into_hash_aux(zval *return_value, MYSQL_RES * result, long fetchtype TSRMLS_DC);
...
{ \
	...
	if (__check && my_res->status < __check) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid object or resource %s\n", intern->zo.ce->name); \
		RETURN_NULL();\
	}\
}
