...
static JONJ_GINIT_FUNCTION(mysqli);
...
void jonj_mysqli_dtor_p_elements(void *data)
{
...
ZEND_RSRC_DTOR_FUNC(jonj_mysqli_dtor)
{
	if (rsrc->ptr) {
		mysqli_plist_entry *plist = (mysqli_plist_entry *) rsrc->ptr;
		zend_ptr_stack_clean(&plist->free_links, jonj_mysqli_dtor_p_elements, 0);
...
int jonj_le_pmysqli(void)
{
	return le_pmysqli;
}

#ifndef MYSQLI_USE_MYSQLND
/* {{{ jonj_free_stmt_bind_buffer */
void jonj_free_stmt_bind_buffer(BIND_BUFFER bbuf, int type)
{
	unsigned int i;
...
/* {{{ jonj_clear_stmt_bind */
void jonj_clear_stmt_bind(MY_STMT *stmt TSRMLS_DC)
{
	if (stmt->stmt) {
		if (mysqli_stmt_close(stmt->stmt, TRUE)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error occurred while closing statement");
			return;
		}
	}
...
#ifndef MYSQLI_USE_MYSQLND
	/* Clean param bind */
	jonj_free_stmt_bind_buffer(stmt->param, FETCH_SIMPLE);
	/* Clean output bind */
	jonj_free_stmt_bind_buffer(stmt->result, FETCH_RESULT);
...
/* {{{ jonj_clear_mysql */
void jonj_clear_mysql(MY_MYSQL *mysql) {
	if (mysql->hash_key) {
...
/* mysqli_link_free_storage partly doubles the work of JONJ_FUNCTION(mysqli_close) */
...
static void mysqli_link_free_storage(void *object TSRMLS_DC)
{
	...
	if (my_res && my_res->ptr) {
		...
		if (mysql->mysql) {
			jonj_mysqli_close(mysql, MYSQLI_CLOSE_EXPLICIT, my_res->status TSRMLS_CC);
		}
		jonj_clear_mysql(mysql);
		efree(mysql);
...
static void mysqli_stmt_free_storage(void *object TSRMLS_DC)
{
	...
	if (my_res && my_res->ptr) {
		...
		jonj_clear_stmt_bind(stmt TSRMLS_CC);
	}
	mysqli_objects_free_storage(object TSRMLS_CC);
}
/* }}} */
...
static void mysqli_warning_free_storage(void *object TSRMLS_DC)
{
	...
	if (my_res && my_res->ptr) {
		jonj_clear_warnings((MYSQLI_WARNING *)my_res->info);
		my_res->ptr = NULL;
	}
	mysqli_objects_free_storage(object TSRMLS_CC);
}
...
static int mysqli_object_has_property(zval *object, zval *member, int has_set_exists, const zend_literal *key TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_find(obj->prop_handler, Z_STRVAL_P(member), Z_STRLEN_P(member) + 1, (void **)&p) == SUCCESS) {
		switch (has_set_exists) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid value for has_set_exists");
		}
	} else {
		zend_object_handlers * std_hnd = zend_get_std_object_handlers();
...
#if JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 3
...
/* {{{ mysqli_objects_new
 */
JONJ_MYSQLI_EXPORT(zend_object_value) mysqli_objects_new(zend_class_entry *class_type TSRMLS_DC)
{
...
static MYSQLND *mysqli_convert_zv_to_mysqlnd(zval * zv TSRMLS_DC)
{
	if (Z_TYPE_P(zv) == IS_OBJECT && instanceof_function(Z_OBJCE_P(zv), mysqli_link_class_entry TSRMLS_CC)) {
		...
		if (!(my_res = (MYSQLI_RESOURCE *)intern->ptr)) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Couldn't fetch %s", intern->zo.ce->name);
			return NULL;
		}
...
*/
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY_EX("mysqli.max_links",			"-1",	JONJ_INI_SYSTEM,		OnUpdateLong,		max_links,			zend_mysqli_globals,		mysqli_globals, display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mysqli.max_persistent",		"-1",	JONJ_INI_SYSTEM,		OnUpdateLong,		max_persistent,		zend_mysqli_globals,		mysqli_globals,	display_link_numbers)
	STD_JONJ_INI_BOOLEAN("mysqli.allow_persistent",		"1",	JONJ_INI_SYSTEM,		OnUpdateLong,		allow_persistent,	zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_BOOLEAN("mysqli.rollback_on_cached_plink",	"0",JONJ_INI_SYSTEM,		OnUpdateBool,		rollback_on_cached_plink,	zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_ENTRY("mysqli.default_host",			NULL,	JONJ_INI_ALL,		OnUpdateString,		default_host,		zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_ENTRY("mysqli.default_user",			NULL,	JONJ_INI_ALL,		OnUpdateString,		default_user,		zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_ENTRY("mysqli.default_pw",				NULL,	JONJ_INI_ALL,		OnUpdateString,		default_pw,			zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_ENTRY("mysqli.default_port",			"3306",	JONJ_INI_ALL,		OnUpdateLong,		default_port,		zend_mysqli_globals,		mysqli_globals)
#ifdef JONJ_MYSQL_UNIX_SOCK_ADDR
	STD_JONJ_INI_ENTRY("mysqli.default_socket",			MYSQL_UNIX_ADDR,JONJ_INI_ALL,OnUpdateStringUnempty,	default_socket,	zend_mysqli_globals,		mysqli_globals)
#else
	STD_JONJ_INI_ENTRY("mysqli.default_socket",			NULL,	JONJ_INI_ALL,		OnUpdateStringUnempty,	default_socket,	zend_mysqli_globals,		mysqli_globals)
#endif
	STD_JONJ_INI_BOOLEAN("mysqli.reconnect",				"0",	JONJ_INI_SYSTEM,		OnUpdateLong,		reconnect,			zend_mysqli_globals,		mysqli_globals)
	STD_JONJ_INI_BOOLEAN("mysqli.allow_local_infile",	"1",	JONJ_INI_SYSTEM,		OnUpdateLong,		allow_local_infile,	zend_mysqli_globals,		mysqli_globals)
JONJ_INI_END()
/* }}} */
...
JONJ_MINIT_FUNCTION(mysqli)
{
	...
	le_pmysqli = zend_register_list_destructors_ex(NULL, jonj_mysqli_dtor,
	...
	mysqli_result_class_entry->get_iterator = jonj_mysqli_result_get_iterator;
	mysqli_result_class_entry->iterator_funcs.funcs = &jonj_mysqli_result_iterator_funcs;
 ...
 */
JONJ_MSHUTDOWN_FUNCTION(mysqli)
{
#ifndef MYSQLI_USE_MYSQLND
#if MYSQL_VERSION_ID >= 40000
#ifdef JONJ_WIN32
...
static void jonj_mysqli_persistent_helper_for_every(void *p)
{
	TSRMLS_FETCH();
...
static int jonj_mysqli_persistent_helper_once(zend_rsrc_list_entry *le TSRMLS_DC)
{
	if (le->type == jonj_le_pmysqli()) {
		mysqli_plist_entry *plist = (mysqli_plist_entry *) le->ptr;
		zend_ptr_stack_apply(&plist->free_links, jonj_mysqli_persistent_helper_for_every);
	}
 ...
 */
JONJ_RSHUTDOWN_FUNCTION(mysqli)
{
	...
	zend_hash_apply(&EG(persistent_list), (apply_func_t) jonj_mysqli_persistent_helper_once TSRMLS_CC);
#endif
	return SUCCESS;
}
/* }}} */
...
/* {{{ JONJ_MINFO_FUNCTION
 */
JONJ_MINFO_FUNCTION(mysqli)
{
	char buf[32];

	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "MysqlI Support", "enabled");
	jonj_info_print_table_row(2, "Client API library version", mysql_get_client_info());
	...
	jonj_info_print_table_row(2, "Inactive Persistent Links", buf);
	snprintf(buf, sizeof(buf), "%ld", MyG(num_links));
	jonj_info_print_table_row(2, "Active Links", buf);
#if !defined(MYSQLI_USE_MYSQLND)
	jonj_info_print_table_row(2, "Client API header version", MYSQL_SERVER_VERSION);
	jonj_info_print_table_row(2, "MYSQLI_SOCKET", MYSQL_UNIX_ADDR);
#endif
...
#if defined(HAVE_SPL) && ((JONJ_MAJOR_VERSION > 5) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 1))
...
zend_module_entry mysqli_module_entry = {
	...
	mysqli_functions,
	JONJ_MINIT(mysqli),
	JONJ_MSHUTDOWN(mysqli),
	JONJ_RINIT(mysqli),
	JONJ_RSHUTDOWN(mysqli),
	JONJ_MINFO(mysqli),
	"0.1", /* Replace with version number for your extension */
	JONJ_MODULE_GLOBALS(mysqli),
	JONJ_GINIT(mysqli),
	NULL,
	NULL,
...
JONJ_FUNCTION(mysqli_result_construct)
{
	...
	switch (resmode) {
		...
		case MYSQLI_USE_RESULT:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid value for resultmode");
	}
...
/* {{{ jonj_mysqli_fetch_into_hash_aux
 */
void jonj_mysqli_fetch_into_hash_aux(zval *return_value, MYSQL_RES * result, long fetchtype TSRMLS_DC)
{
#if !defined(MYSQLI_USE_MYSQLND)
	MYSQL_ROW row;
	unsigned int	i;
		...
		if (row[i]) {
...
#endif
			{
				...
				if (PG(magic_quotes_runtime)) {
					...
					Z_STRVAL_P(res) = jonj_addslashes(row[i], field_len[i], &Z_STRLEN_P(res), 0 TSRMLS_CC);
				} else {
#endif
...
/* {{{ jonj_mysqli_fetch_into_hash
 */
void jonj_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAMETERS, int override_flags, int into_object)
{
	MYSQL_RES		*result;
	zval			*mysql_result;
	long			fetchtype;
	...
	if (fetchtype < MYSQLI_ASSOC || fetchtype > MYSQLI_BOTH) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The result type should be either MYSQLI_NUM, MYSQLI_ASSOC or MYSQLI_BOTH");
		RETURN_FALSE;
	}

	jonj_mysqli_fetch_into_hash_aux(return_value, result, fetchtype TSRMLS_CC);
