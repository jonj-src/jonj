...
void jonj_mysqli_throw_sql_exception(char *sqlstate, int errorno TSRMLS_DC, char *format, ...)
{
	zval	*sql_ex;
	va_list arg;
	char 	*message;
	...
	if (!(MyG(report_mode) & MYSQLI_REPORT_STRICT)) {
	 	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "(%s/%d): %s", sqlstate, errorno, message);
		efree(message);
		return;
	}
