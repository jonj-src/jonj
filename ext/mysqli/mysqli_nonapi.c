...
/* {{{ jonj_mysqli_set_error
 */
static void jonj_mysqli_set_error(long mysql_errno, char *mysql_err TSRMLS_DC)
{
...
void mysqli_common_connect(INTERNAL_FUNCTION_PARAMETERS, zend_bool is_real_connect, zend_bool in_ctor)
{
		...
		(mysqli_resource->status > MYSQLI_STATUS_INITIALIZED))
	{
		...
		jonj_mysqli_close(mysql, MYSQLI_CLOSE_IMPLICIT, mysqli_resource->status TSRMLS_CC);
	}

	if (strlen(SAFE_STR(hostname)) > 2 && !strncasecmp(hostname, "p:", 2)) {
		hostname += 2;
		if (!MyG(allow_persistent)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Persistent connections are disabled. Downgrading to normal");
		} else {
	...
	if (MyG(max_links) != -1 && MyG(num_links) >= MyG(max_links)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open links (%ld)", MyG(num_links));
		goto err;
	}
	...
	{
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open persistent links (%ld)",
...
#endif
	{
		...
		jonj_mysqli_set_error(mysql_errno(mysql->mysql), (char *) mysql_error(mysql->mysql) TSRMLS_CC);
		jonj_mysqli_throw_sql_exception((char *)mysql_sqlstate(mysql->mysql), mysql_errno(mysql->mysql) TSRMLS_CC,
...
{
	jonj_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */
...
{
	jonj_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MYSQLI_ASSOC, 0);
}
/* }}} */
...
JONJ_FUNCTION(mysqli_fetch_all)
{
	...
	if (!mode || (mode & ~MYSQLND_FETCH_BOTH)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Mode can be only MYSQLI_FETCH_NUM, "
...
{
	jonj_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MYSQLI_ASSOC, 1);
}
/* }}} */
...
JONJ_FUNCTION(mysqli_query)
{
	...
	if (!query_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty query");
		RETURN_FALSE;
	}
#ifdef MYSQLI_USE_MYSQLND
	...
	*out_array = ecalloc(zend_hash_num_elements(Z_ARRVAL_P(in_array)) + 1, sizeof(MYSQLND *));
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(in_array));
		...
		if (Z_TYPE_PP(elem) != IS_OBJECT ||
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parameter %d not a mysqli object", i);
		} else {
			MY_MYSQL *mysql;
			MYSQLI_RESOURCE *my_res;
			mysqli_object *intern = (mysqli_object *)zend_object_store_get_object(*elem TSRMLS_CC);
			if (!(my_res = (MYSQLI_RESOURCE *)intern->ptr)) {
		  		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "[%d] Couldn't fetch %s", i, intern->zo.ce->name);
				continue;
		  	}
			mysql = (MY_MYSQL*) my_res->ptr;
			if (MYSQLI_STATUS_VALID && my_res->status < MYSQLI_STATUS_VALID) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid object %d or resource %s", i, intern->zo.ce->name);
				continue;
			}
...
JONJ_FUNCTION(mysqli_reap_async_query)
{
	...
	if (!mysql_field_count(mysql->mysql)) {
		/* no result set - not a SELECT */
		if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
/*			jonj_mysqli_report_index("n/a", mysqli_server_status(mysql->mysql) TSRMLS_CC); */
		}
		RETURN_TRUE;
	}
...
#ifdef MYSQLI_USE_MYSQLND
		w = jonj_get_warnings(mysql->mysql->data TSRMLS_CC);
#else
		w = jonj_get_warnings(mysql->mysql TSRMLS_CC);
#endif
	} else {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mysqli_stmt_get_warnings)
{
	...
	if (mysqli_stmt_warning_count(stmt->stmt)) {
		w = jonj_get_warnings(mysqli_stmt_get_connection(stmt->stmt) TSRMLS_CC);
	} else {
		RETURN_FALSE;
	}
	mysqli_resource = (MYSQLI_RESOURCE *)ecalloc (1, sizeof(MYSQLI_RESOURCE));
...
JONJ_FUNCTION(mysqli_get_charset)
{
	...
	if (!cs) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The connection has no charset associated");
		RETURN_NULL();
	}
	name = cs->name;
...
JONJ_FUNCTION(mysqli_begin_transaction)
{
	...
	MYSQLI_FETCH_RESOURCE_CONN(mysql, &mysql_link, MYSQLI_STATUS_VALID);
	if (flags < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid value for parameter flags (%ld)", flags);
		err = TRUE;
	}
	if (!name_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Savepoint name cannot be empty");
		err = TRUE;
	}
	if (TRUE == err) {
		RETURN_FALSE;			
...
{
	if (ZEND_NUM_ARGS()) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no parameters expected");
		return;	
	}
	array_init(return_value);
