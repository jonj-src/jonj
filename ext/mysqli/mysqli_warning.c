...
/* Define these in the JONJ5 tree to make merging easy process */
...
/* {{{ void jonj_clear_warnings() */
void jonj_clear_warnings(MYSQLI_WARNING *w)
{
	MYSQLI_WARNING *n;
...
/* {{{ MYSQLI_WARNING *jonj_new_warning */
static
MYSQLI_WARNING *jonj_new_warning(const char *reason, int errorno TSRMLS_DC)
{
	MYSQLI_WARNING *w;
...
/* {{{ MYSQLI_WARNING *jonj_get_warnings(MYSQL *mysql TSRMLS_DC) */
MYSQLI_WARNING *jonj_get_warnings(MYSQL *mysql TSRMLS_DC)
{
	...
	while ((row = mysql_fetch_row(result))) {
		w = jonj_new_warning(row[2], atoi(row[1]) TSRMLS_CC);
		if (!first) {
			first = w;
		}
		if (prev) {
...
/* {{{ MYSQLI_WARNING *jonj_new_warning */
static
MYSQLI_WARNING *jonj_new_warning(const zval * reason, int errorno TSRMLS_DC)
{
	MYSQLI_WARNING *w;
...
/* {{{ MYSQLI_WARNING *jonj_get_warnings(MYSQL *mysql TSRMLS_DC) */
MYSQLI_WARNING * jonj_get_warnings(MYSQLND_CONN_DATA * mysql TSRMLS_DC)
{
	MYSQLI_WARNING	*w, *first = NULL, *prev = NULL;
	MYSQL_RES		*result;
	zval			*row;
	...
	for (;;) {
		...
		w = jonj_new_warning(*entry, errno TSRMLS_CC);
		/*
...
JONJ_METHOD(mysqli_warning, next)
{
	MYSQLI_WARNING 	*w;
	zval  			*mysqli_warning;
...
JONJ_METHOD(mysqli_warning, __construct)
{
	...
	if (obj->zo.ce == mysqli_link_class_entry) {
		...
		if (mysql_warning_count(mysql->mysql)) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No warnings found");
			RETURN_FALSE;
		}
		...
		if (mysql_warning_count(hdl)) {
			w = jonj_get_warnings(hdl TSRMLS_CC);
#else
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid class argument");
		RETURN_FALSE;
	}
...
const zend_function_entry mysqli_warning_methods[] = {
	JONJ_ME(mysqli_warning, __construct,		NULL, ZEND_ACC_PROTECTED)
	JONJ_ME(mysqli_warning, next, 			NULL, ZEND_ACC_PUBLIC)
	{NULL, NULL, NULL}
};
/* }}} */
