...
#define JONJ_BZ_ERRNO   0
#define JONJ_BZ_ERRSTR  1
#define JONJ_BZ_ERRBOTH 2

static JONJ_MINIT_FUNCTION(bz2);
static JONJ_MSHUTDOWN_FUNCTION(bz2);
static JONJ_MINFO_FUNCTION(bz2);
static JONJ_FUNCTION(bzopen);
static JONJ_FUNCTION(bzread);
static JONJ_FUNCTION(bzerrno);
static JONJ_FUNCTION(bzerrstr);
static JONJ_FUNCTION(bzerror);
static JONJ_FUNCTION(bzcompress);
static JONJ_FUNCTION(bzdecompress);
...
static const zend_function_entry bz2_functions[] = {
	JONJ_FE(bzopen,       arginfo_bzopen)
	JONJ_FE(bzread,       arginfo_bzread)
	JONJ_FALIAS(bzwrite,   fwrite,		arginfo_bzwrite)
	JONJ_FALIAS(bzflush,   fflush,		arginfo_bzflush)
	JONJ_FALIAS(bzclose,   fclose,		arginfo_bzflush)
	JONJ_FE(bzerrno,      arginfo_bzerrno)
	JONJ_FE(bzerrstr,     arginfo_bzerrstr)
	JONJ_FE(bzerror,      arginfo_bzerror)
	JONJ_FE(bzcompress,   arginfo_bzcompress)
	JONJ_FE(bzdecompress, arginfo_bzdecompress)
	JONJ_FE_END
};
...
static size_t jonj_bz2iop_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	struct jonj_bz2_stream_data_t *self = (struct jonj_bz2_stream_data_t *) stream->abstract;
	int bz2_ret;
...
static size_t jonj_bz2iop_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	struct jonj_bz2_stream_data_t *self = (struct jonj_bz2_stream_data_t *) stream->abstract;
...
static int jonj_bz2iop_close(jonj_stream *stream, int close_handle TSRMLS_DC)
{
	struct jonj_bz2_stream_data_t *self = (struct jonj_bz2_stream_data_t *)stream->abstract;
	int ret = EOF;
	...
	if (self->stream) {
		jonj_stream_free(self->stream, JONJ_STREAM_FREE_CLOSE | (close_handle == 0 ? JONJ_STREAM_FREE_PRESERVE_HANDLE : 0));
	}
...
static int jonj_bz2iop_flush(jonj_stream *stream TSRMLS_DC)
{
	struct jonj_bz2_stream_data_t *self = (struct jonj_bz2_stream_data_t *)stream->abstract;
...
jonj_stream_ops jonj_stream_bz2io_ops = {
	jonj_bz2iop_write, jonj_bz2iop_read,
	jonj_bz2iop_close, jonj_bz2iop_flush,
	"BZip2",
	NULL, /* seek */
	NULL, /* cast */
	NULL, /* stat */
...
JONJ_BZ2_API jonj_stream *_jonj_stream_bz2open_from_BZFILE(BZFILE *bz,
														const char *mode, jonj_stream *innerstream STREAMS_DC TSRMLS_DC)
{
	struct jonj_bz2_stream_data_t *self;
	...
	return jonj_stream_alloc_rel(&jonj_stream_bz2io_ops, self, 0, mode);
}

JONJ_BZ2_API jonj_stream *_jonj_stream_bz2open(jonj_stream_wrapper *wrapper,
											const char *path,
											const char *mode,
											int options,
											char **opened_path,
...
{
	jonj_stream *retstream = NULL, *stream = NULL;
	char *path_copy = NULL;
	BZFILE *bz_file = NULL;
	...
	if (jonj_check_open_basedir(path_copy TSRMLS_CC)) {
#ifdef VIRTUAL_DIR
		efree(path_copy);
#endif
		return NULL;
	...
	if (bz_file == NULL) {
		...
		stream = jonj_stream_open_wrapper(path, mode, options | STREAM_WILL_CAST, opened_path);

		if (stream) {
			jonj_socket_t fd;
			if (SUCCESS == jonj_stream_cast(stream, JONJ_STREAM_AS_FD, (void **) &fd, REPORT_ERRORS)) {
				bz_file = BZ2_bzdopen(fd, mode);
			}
		}

		/* remove the file created by jonj_stream_open_wrapper(), it is not needed since BZ2 functions
		 * failed.
		 */
	...
	if (bz_file) {
		retstream = _jonj_stream_bz2open_from_BZFILE(bz_file, mode, stream STREAMS_REL_CC TSRMLS_CC);
		if (retstream) {
			return retstream;
		}
...
static jonj_stream_wrapper_ops bzip2_stream_wops = {
	_jonj_stream_bz2open,
	NULL, /* close */
	NULL, /* fstat */
	NULL, /* stat */
	NULL, /* opendir */
...
static jonj_stream_wrapper jonj_stream_bzip2_wrapper = {
	&bzip2_stream_wops,
	NULL,
	0 /* is_url */
};

static void jonj_bz2_error(INTERNAL_FUNCTION_PARAMETERS, int);

static JONJ_MINIT_FUNCTION(bz2)
{
	jonj_register_url_stream_wrapper("compress.bzip2", &jonj_stream_bzip2_wrapper TSRMLS_CC);
	jonj_stream_filter_register_factory("bzip2.*", &jonj_bz2_filter_factory TSRMLS_CC);
	return SUCCESS;
}

static JONJ_MSHUTDOWN_FUNCTION(bz2)
{
	jonj_unregister_url_stream_wrapper("compress.bzip2" TSRMLS_CC);
	jonj_stream_filter_unregister_factory("bzip2.*" TSRMLS_CC);
...
static JONJ_MINFO_FUNCTION(bz2)
{
	...
	jonj_info_print_table_row(2, "BZip2 Support", "Enabled");
	jonj_info_print_table_row(2, "Stream Wrapper support", "compress.bzip2://");
	jonj_info_print_table_row(2, "Stream Filter support", "bzip2.decompress, bzip2.compress");
	jonj_info_print_table_row(2, "BZip2 Version", (char *) BZ2_bzlibVersion());
...
static JONJ_FUNCTION(bzread)
{
	...
	jonj_stream_from_zval(stream, &bz);

	if ((len + 1) < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "length may not be negative");
		RETURN_FALSE;
	}
	...
	if (Z_STRLEN_P(return_value) < 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "could not read valid bz2 data from stream");
		RETURN_FALSE;
	}
...
static JONJ_FUNCTION(bzopen)
{
	...
	if (mode_len != 1 || (mode[0] != 'r' && mode[0] != 'w')) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'%s' is not a valid mode for bzopen(). Only 'w' and 'r' are supported.", mode);
		RETURN_FALSE;
	}
	...
	if (Z_TYPE_PP(file) == IS_STRING) {
		...
		stream = jonj_stream_bz2open(NULL,
									Z_STRVAL_PP(file),
									mode,
									REPORT_ERRORS,
									NULL);
		...
		jonj_stream_from_zval(stream, file);
		stream_mode_len = strlen(stream->mode);

		if (stream_mode_len != 1 && !(stream_mode_len == 2 && memchr(stream->mode, 'b', 2))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot use stream opened in mode '%s'", stream->mode);
			RETURN_FALSE;
		...
		switch(mode[0]) {
			case 'r':
				...
				if (stream->mode[0] != mode[0] && !(stream_mode_len == 2 && stream->mode[1] != mode[0])) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot read from a stream opened in write only mode");
					RETURN_FALSE;
				}
				break;
			case 'w':
				...
				if (stream->mode[0] != mode[0] && !(stream_mode_len == 2 && stream->mode[1] != mode[0])
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot write to a stream opened in read only mode");
					RETURN_FALSE;
				}
				break;
			default:
		...
		if (FAILURE == jonj_stream_cast(stream, JONJ_STREAM_AS_FD, (void *) &fd, REPORT_ERRORS)) {
			RETURN_FALSE;
		}

		bz = BZ2_bzdopen(fd, mode);

		stream = jonj_stream_bz2open_from_BZFILE(bz, mode, stream);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "first parameter has to be string or file-resource");
		RETURN_FALSE;
	}
...
static JONJ_FUNCTION(bzerrno)
{
	jonj_bz2_error(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_BZ_ERRNO);
}
/* }}} */
...
static JONJ_FUNCTION(bzerrstr)
{
	jonj_bz2_error(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_BZ_ERRSTR);
}
/* }}} */
...
static JONJ_FUNCTION(bzerror)
{
	jonj_bz2_error(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_BZ_ERRBOTH);
}
/* }}} */
...
static JONJ_FUNCTION(bzdecompress)
{
	...
	if (error == BZ_STREAM_END || error == BZ_OK) {
		size = (bzs.total_out_hi32 * (unsigned int) -1) + bzs.total_out_lo32;
		if (size > INT_MAX) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Decompressed size too big, max is %d", INT_MAX);
			efree(dest);
...
static void jonj_bz2_error(INTERNAL_FUNCTION_PARAMETERS, int opt)
{
	...
	jonj_stream_from_zval(stream, &bzp);

	if (!jonj_stream_is(stream, JONJ_STREAM_IS_BZIP2)) {
		RETURN_FALSE;
	}

	self = (struct jonj_bz2_stream_data_t *) stream->abstract;
