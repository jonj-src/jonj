...
#else
jonj_shmop_globals shmop_globals;
#endif
...
const zend_function_entry shmop_functions[] = {
	JONJ_FE(shmop_open, 		arginfo_shmop_open)
	JONJ_FE(shmop_read, 		arginfo_shmop_read)
	JONJ_FE(shmop_close, 	arginfo_shmop_close)
	JONJ_FE(shmop_size, 		arginfo_shmop_size)
	JONJ_FE(shmop_write, 	arginfo_shmop_write)
	JONJ_FE(shmop_delete, 	arginfo_shmop_delete)
	JONJ_FE_END
};
/* }}} */
...
#define JONJ_SHMOP_GET_RES \
	shmop = zend_list_find(shmid, &type);	\
	if (!shmop) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no shared memory segment with an id of [%lu]", shmid);	\
		RETURN_FALSE;	\
	} else if (type != shm_type) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "not a shmop resource");	\
		RETURN_FALSE;	\
	}	\
...
{
	struct jonj_shmop *shmop = (struct jonj_shmop *)rsrc->ptr;
...
JONJ_MINFO_FUNCTION(shmop)
{
	...
	jonj_info_print_table_row(2, "shmop support", "enabled");
...
JONJ_FUNCTION(shmop_open)
{
	...
	if (flags_len != 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s is not a valid flag", flags);
		RETURN_FALSE;
	}

	shmop = emalloc(sizeof(struct jonj_shmop));
	memset(shmop, 0, sizeof(struct jonj_shmop));
	...
	switch (flags[0]) 
	{
		...
		case 'w':
			...
			*/
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid access mode");
			goto err;
	}

	if (shmop->shmflg & IPC_CREAT && shmop->size < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Shared memory segment size must be greater than zero");
		goto err;
	}

	shmop->shmid = shmget(shmop->key, shmop->size, shmop->shmflg);
	if (shmop->shmid == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to attach or create shared memory segment");
		goto err;
	}

	if (shmctl(shmop->shmid, IPC_STAT, &shm)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to get shared memory segment information");
		goto err;
	}	

	shmop->addr = shmat(shmop->shmid, 0, shmop->shmatflg);
	if (shmop->addr == (char*) -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to attach to shared memory segment");
		goto err;
	}
...
JONJ_FUNCTION(shmop_read)
{
	...
	if (start < 0 || start > shmop->size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "start is out of range");
		RETURN_FALSE;
	}

	if (count < 0 || start > (INT_MAX - count) || start + count > shmop->size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "count is out of range");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(shmop_write)
{
	...
	if ((shmop->shmatflg & SHM_RDONLY) == SHM_RDONLY) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "trying to write to a read only segment");
		RETURN_FALSE;
	}

	if (offset < 0 || offset > shmop->size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "offset out of range");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(shmop_delete)
{
	...
	if (shmctl(shmop->shmid, IPC_RMID, NULL)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "can't mark segment for deletion (are you the owner?)");
		RETURN_FALSE;
	}
