...
JONJ_COM_DOTNET_API OLECHAR *jonj_com_string_to_olestring(char *string, uint string_len, int codepage TSRMLS_DC)
{
	...
	if (!ok) {
		char *msg = jonj_win32_error_to_msg(GetLastError());
...
JONJ_COM_DOTNET_API char *jonj_com_olestring_to_string(OLECHAR *olestring, uint *string_len, int codepage TSRMLS_DC)
{
	char *string;
	uint length = 0;
	BOOL ok;
