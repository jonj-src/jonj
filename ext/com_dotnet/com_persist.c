 ...
 * Implements: IStream* wrapper for JONJ streams.
...
/* {{{ expose jonj_stream as a COM IStream */
...
static void istream_destructor(jonj_istream *stm TSRMLS_DC);

static void istream_dtor(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	jonj_istream *stm = (jonj_istream *)rsrc->ptr;
	istream_destructor(stm TSRMLS_CC);
}

#define FETCH_STM()	\
	jonj_istream *stm = (jonj_istream*)This; \
	TSRMLS_FETCH(); \
...
static HRESULT STDMETHODCALLTYPE stm_read(IStream *This, void *pv, ULONG cb, ULONG *pcbRead)
{
	...
	nread = jonj_stream_read(stm->stream, pv, cb);
...
static HRESULT STDMETHODCALLTYPE stm_write(IStream *This, void const *pv, ULONG cb, ULONG *pcbWritten)
{
	...
	nwrote = jonj_stream_write(stm->stream, pv, cb);
		...
		DWORD dwOrigin, ULARGE_INTEGER *plibNewPosition)
{
	...
	ret = jonj_stream_seek(stm->stream, offset, whence);
...
static HRESULT STDMETHODCALLTYPE stm_set_size(IStream *This, ULARGE_INTEGER libNewSize)
{
	...
	if (jonj_stream_truncate_supported(stm->stream)) {
		int ret = jonj_stream_truncate_set_size(stm->stream, (size_t)libNewSize.QuadPart);
...
static struct IStreamVtbl jonj_istream_vtbl = {
	stm_queryinterface,
	stm_addref,
	stm_release,
	stm_read,
...
JONJ_COM_DOTNET_API IStream *jonj_com_wrapper_export_stream(jonj_stream *stream TSRMLS_DC)
{
	jonj_istream *stm = (jonj_istream*)CoTaskMemAlloc(sizeof(*stm));
...
#define CPH_ME(fname, arginfo)	JONJ_ME(com_persist, fname, arginfo, ZEND_ACC_PUBLIC)
#define CPH_SME(fname, arginfo)	JONJ_ME(com_persist, fname, arginfo, ZEND_ACC_ALLOW_STATIC|ZEND_ACC_PUBLIC)
#define CPH_METHOD(fname)		static JONJ_METHOD(com_persist, fname)
	
#define CPH_FETCH()				jonj_com_persist_helper *helper = (jonj_com_persist_helper*)zend_object_store_get_object(getThis() TSRMLS_CC);

#define CPH_NO_OBJ()			if (helper->unk == NULL) { jonj_com_throw_exception(E_INVALIDARG, "No COM object is associated with this helper instance" TSRMLS_CC); return; }
...
typedef struct {
	...
	IPersistFile		*ipf;
} jonj_com_persist_helper;

static zend_object_handlers helper_handlers;
static zend_class_entry *helper_ce;

static inline HRESULT get_persist_stream(jonj_com_persist_helper *helper)
{
...
static inline HRESULT get_persist_stream_init(jonj_com_persist_helper *helper)
{
...
static inline HRESULT get_persist_file(jonj_com_persist_helper *helper)
{
...
CPH_METHOD(GetCurFileName)
{
	...
	if (helper->ipf) {
		...
		if (res == S_OK) {
			...
			Z_STRVAL_P(return_value) = jonj_com_olestring_to_string(olename,
...
CPH_METHOD(SaveToFile)
{
	...
	if (helper->ipf) {
		...
		if (filename) {
			...
			olefilename = jonj_com_string_to_olestring(filename, strlen(fullpath), helper->codepage TSRMLS_CC);
			efree(fullpath);
		}
...
CPH_METHOD(LoadFromFile)
{
	...
	if (helper->ipf) {
		...
		olefilename = jonj_com_string_to_olestring(fullpath, strlen(fullpath), helper->codepage TSRMLS_CC);
		efree(fullpath);
...
CPH_METHOD(LoadFromStream)
{
	...
	jonj_stream_from_zval_no_verify(stream, &zstm);
	
	if (stream == NULL) {
		jonj_com_throw_exception(E_INVALIDARG, "expected a stream" TSRMLS_CC);
		return;
	}

	stm = jonj_com_wrapper_export_stream(stream TSRMLS_CC);
	if (stm == NULL) {
		jonj_com_throw_exception(E_UNEXPECTED, "failed to wrap stream" TSRMLS_CC);
		return;
	}
	...
	if (helper->unk == NULL) {
		...
		if (SUCCEEDED(res)) {
			jonj_com_wrap_dispatch(return_value, disp, COMG(code_page) TSRMLS_CC);	
		}
	} else {
		res = get_persist_stream_init(helper);
		if (helper->ipsi) {
...
CPH_METHOD(__construct)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|O!",
				&zobj, jonj_com_variant_class_entry)) {
		jonj_com_throw_exception(E_INVALIDARG, "invalid arguments" TSRMLS_CC);
		return;
	}
	...
	if (V_VT(&obj->v) != VT_DISPATCH || V_DISPATCH(&obj->v) == NULL) {
		jonj_com_throw_exception(E_INVALIDARG, "parameter must represent an IDispatch COM object" TSRMLS_CC);
		return;
	}
...
{
	jonj_com_persist_helper *object = (jonj_com_persist_helper*)obj;
...
{
	jonj_com_persist_helper *clone, *object = (jonj_com_persist_helper*)obj;
...
{
	jonj_com_persist_helper *helper;
