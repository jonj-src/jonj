...
typedef struct {
	...
	zval *proxy_obj;
	jonj_com_saproxy *proxy;
	LONG key;
	LONG imin, imax;
	LONG *indices;
} jonj_com_saproxy_iter;

#define SA_FETCH(zv)			(jonj_com_saproxy*)zend_object_store_get_object(zv TSRMLS_CC)

static inline void clone_indices(jonj_com_saproxy *dest, jonj_com_saproxy *src, int ndims)
{
	int i;
...
static zval *saproxy_property_read(zval *object, zval *member, int type, const zend_literal *key TSRMLS_DC)
{
	...
	jonj_com_throw_exception(E_INVALIDARG, "safearray has no properties" TSRMLS_CC);
...
{
	jonj_com_saproxy *proxy = SA_FETCH(object);
	zval *return_value;
	UINT dims, i;
	SAFEARRAY *sa;
	LONG ubound, lbound;
	...
	if (V_VT(&proxy->obj->v) == VT_DISPATCH) {
		...
		res = jonj_com_do_invoke(proxy->obj, Z_STRVAL_P(proxy->indices[0]),
			   	Z_STRLEN_P(proxy->indices[0]), DISPATCH_METHOD|DISPATCH_PROPERTYGET, &v,
			   	proxy->dimensions, args, 0 TSRMLS_CC);

		if (res == SUCCESS) {
			jonj_com_zval_from_variant(return_value, &v, proxy->obj->code_page TSRMLS_CC);
			VariantClear(&v);
		...
		jonj_com_throw_exception(E_INVALIDARG, "invalid read from com proxy object" TSRMLS_CC);
		return return_value;
	}
	...
	if ((UINT) proxy->dimensions >= dims) {
		/* too many dimensions */
		jonj_com_throw_exception(E_INVALIDARG, "too many dimensions!" TSRMLS_CC);
		return return_value;
	}
	...
	if (Z_LVAL_P(offset) < lbound || Z_LVAL_P(offset) > ubound) {
		jonj_com_throw_exception(DISP_E_BADINDEX, "index out of bounds" TSRMLS_CC);
		return return_value;
	}
	...
	if (dims - 1 == proxy->dimensions) {
		...
		if (SUCCEEDED(res)) {
			jonj_com_wrap_variant(return_value, &v, proxy->obj->code_page TSRMLS_CC);
		} else {
...
static void saproxy_write_dimension(zval *object, zval *offset, zval *value TSRMLS_DC)
{
	...
	if (V_VT(&proxy->obj->v) == VT_DISPATCH) {
		...
		convert_to_string(proxy->indices[0]);
		VariantInit(&v);
		if (SUCCESS == jonj_com_do_invoke(proxy->obj, Z_STRVAL_P(proxy->indices[0]),
		...
		VariantInit(&v);
		jonj_com_variant_from_zval(&v, value, proxy->obj->code_page TSRMLS_CC);
		...
		jonj_com_throw_exception(E_NOTIMPL, "invalid write to com proxy object" TSRMLS_CC);
	}
}
...
{
	return jonj_com_saproxy_class_entry;
}

static int saproxy_class_name_get(const zval *object, const char **class_name, zend_uint *class_name_len, int parent TSRMLS_DC)
{
	*class_name = estrndup(jonj_com_saproxy_class_entry->name, jonj_com_saproxy_class_entry->name_length);
	*class_name_len = jonj_com_saproxy_class_entry->name_length;
	return 0;
}
...
zend_object_handlers jonj_com_saproxy_handlers = {
...
{
	jonj_com_saproxy *proxy = (jonj_com_saproxy *)object;
	int i;
...
{
	jonj_com_saproxy *proxy, *rel = NULL;

	proxy = ecalloc(1, sizeof(*proxy));
	proxy->dimensions = 1;

	if (Z_OBJCE_P(com_object) == jonj_com_saproxy_class_entry) {
	...
	*proxy->indices[proxy->dimensions-1] = *index;
	...
	Z_OBJ_HT_P(proxy_out) = &jonj_com_saproxy_handlers;
...
{
	jonj_com_saproxy_iter *I = (jonj_com_saproxy_iter*)iter->data;
...
static void saproxy_iter_get_data(zend_object_iterator *iter, zval ***data TSRMLS_DC)
{
	...
	MAKE_STD_ZVAL(return_value);
	jonj_com_wrap_variant(return_value, &v, I->proxy->obj->code_page TSRMLS_CC);
	VariantClear(&v);
