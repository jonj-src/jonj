...
void jonj_com_throw_exception(HRESULT code, char *message TSRMLS_DC)
{
	int free_msg = 0;
	if (message == NULL) {
		message = jonj_win32_error_to_msg(code);
		free_msg = 1;
	}
	zend_throw_exception(jonj_com_exception_class_entry, message, (long)code TSRMLS_CC);
	if (free_msg) {
		LocalFree(message);
	}
}
		...
		int codepage TSRMLS_DC)
{
	...
	obj->ce = jonj_com_variant_class_entry;
	obj->zo.ce = jonj_com_variant_class_entry;
	...
	z->value.obj.handle = zend_objects_store_put(obj, NULL, jonj_com_object_free_storage, jonj_com_object_clone TSRMLS_CC);
	z->value.obj.handlers = &jonj_com_object_handlers;
}
