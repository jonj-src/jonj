...
static zval *com_property_read(zval *object, zval *member, int type, const zend_literal *key TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		res = jonj_com_do_invoke(obj, Z_STRVAL_P(member), Z_STRLEN_P(member),
				DISPATCH_METHOD|DISPATCH_PROPERTYGET, &v, 0, NULL, 1 TSRMLS_CC);

		if (res == SUCCESS) {
			jonj_com_zval_from_variant(return_value, &v, obj->code_page TSRMLS_CC);
			VariantClear(&v);
		} else if (res == DISP_E_BADPARAMCOUNT) {
			jonj_com_saproxy_create(object, return_value, member TSRMLS_CC);
		}
	} else {
		jonj_com_throw_exception(E_INVALIDARG, "this variant has no properties" TSRMLS_CC);
	}
...
static void com_property_write(zval *object, zval *member, zval *value, const zend_literal *key TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		convert_to_string_ex(&member);
		if (SUCCESS == jonj_com_do_invoke(obj, Z_STRVAL_P(member), Z_STRLEN_P(member),
...
static zval *com_read_dimension(zval *object, zval *offset, int type TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		if (SUCCESS == jonj_com_do_invoke_by_id(obj, DISPID_VALUE,
		...
		if (SafeArrayGetDim(V_ARRAY(&obj->v)) == 1) {	
			if (jonj_com_safearray_get_elem(&obj->v, &v, Z_LVAL_P(offset) TSRMLS_CC)) {
				jonj_com_wrap_variant(return_value, &v, obj->code_page TSRMLS_CC);
				VariantClear(&v);
			}
		} else {
			jonj_com_saproxy_create(object, return_value, offset TSRMLS_CC);
		}

	} else {
		jonj_com_throw_exception(E_INVALIDARG, "this variant is not an array type" TSRMLS_CC);
	}
...
static void com_write_dimension(zval *object, zval *offset, zval *value TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		if (SafeArrayGetDim(V_ARRAY(&obj->v)) == 1) {	
			...
			VariantInit(&v);
			jonj_com_variant_from_zval(&v, value, obj->code_page TSRMLS_CC);
			...
			jonj_com_throw_exception(DISP_E_BADINDEX, "this variant has multiple dimensions; you can't set a new value without specifying *all* dimensions" TSRMLS_CC);
		}
...
static int com_property_exists(zval *object, zval *member, int check_empty, const zend_literal *key TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		convert_to_string_ex(&member);
		if (SUCCEEDED(jonj_com_get_id_of_name(obj, Z_STRVAL_P(member), Z_STRLEN_P(member), &dispid TSRMLS_CC))) {
...
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Operation not yet supported on a COM object");
	return 0;
}

static void com_property_delete(zval *object, zval *member, const zend_literal *key TSRMLS_DC)
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot delete properties from a COM object");
}
...
static JONJ_FUNCTION(com_method_handler)
{
...
static union _zend_function *com_method_get(zval **object_ptr, char *name, int len, const zend_literal *key TSRMLS_DC)
{
	...
	if (FAILED(jonj_com_get_id_of_name(obj, name, len, &dummy TSRMLS_CC))) {
		return NULL;
	}
	...
	if (obj->method_cache == NULL || FAILURE == zend_hash_find(obj->method_cache, name, len, (void**)&fptr)) {
		...
		f.handler = JONJ_FN(com_method_handler);
		...
		if (obj->typeinfo) {
			...
			if (SUCCEEDED(ITypeInfo_GetTypeComp(obj->typeinfo, &comp))) {
				olename = jonj_com_string_to_olestring(name, len, obj->code_page TSRMLS_CC);
...
static int com_call_method(const char *method, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (SUCCESS == jonj_com_do_invoke_byref(obj, (char*)method, -1, DISPATCH_METHOD|DISPATCH_PROPERTYGET, &v, nargs, args TSRMLS_CC)) {
		jonj_com_zval_from_variant(return_value, &v, obj->code_page TSRMLS_CC);
		ret = SUCCESS;
		VariantClear(&v);
	}
...
{
	jonj_com_dotnet_object *obja, *objb;
	int ret;
...
static int com_object_cast(zval *readobj, zval *writeobj, int type TSRMLS_DC)
{
	...
	if (SUCCEEDED(res)) {
		jonj_com_zval_from_variant(writeobj, &v, obj->code_page TSRMLS_CC);
	}
...
{
	jonj_com_dotnet_object *obj = (jonj_com_dotnet_object*)object;
...
{
	jonj_com_dotnet_object *cloneobj, *origobject;

	origobject = (jonj_com_dotnet_object*)object;
	cloneobj = (jonj_com_dotnet_object*)emalloc(sizeof(jonj_com_dotnet_object));
...
zend_object_value jonj_com_object_new(zend_class_entry *ce TSRMLS_DC)
{
	...
	retval.handle = zend_objects_store_put(obj, NULL, jonj_com_object_free_storage, jonj_com_object_clone TSRMLS_CC);
	retval.handlers = &jonj_com_object_handlers;
