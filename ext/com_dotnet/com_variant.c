...
/* create an automation SafeArray from a JONJ array.
 * Only creates a single-dimensional array of variants.
 * The keys of the JONJ hash MUST be numeric.  If the array
...
static void safe_array_from_zval(VARIANT *v, zval *z, int codepage TSRMLS_DC)
{
	...
	for (;; zend_hash_move_forward_ex(HASH_OF(z), &pos)) {
		...
		jonj_com_variant_from_zval(&va[intindex], *item, codepage TSRMLS_CC);		
	}
...
bogus:
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "COM: converting from JONJ array to VARIANT array; only arrays with numeric keys are allowed");
...
JONJ_COM_DOTNET_API void jonj_com_variant_from_zval(VARIANT *v, zval *z, int codepage TSRMLS_DC)
{
	...
	switch (ztype) {
		...
		case IS_OBJECT:
			if (jonj_com_is_valid_object(z TSRMLS_CC)) {
				obj = CDNO_FETCH(z);
				...
				/* export the JONJ object using our COM wrapper */
				V_VT(v) = VT_DISPATCH;
				V_DISPATCH(v) = jonj_com_wrapper_export(z TSRMLS_CC);
			}
			break;
		...
		case IS_STRING:
			V_VT(v) = VT_BSTR;
			olestring = jonj_com_string_to_olestring(Z_STRVAL_P(z), Z_STRLEN_P(z), codepage TSRMLS_CC);
			if (CP_UTF8 == codepage) {
...
JONJ_COM_DOTNET_API int jonj_com_zval_from_variant(zval *z, VARIANT *v, int codepage TSRMLS_DC)
{
	...
	switch (V_VT(v)) {
		...
		case VT_BSTR:
			...
			if (olestring) {
				...
				Z_STRVAL_P(z) = jonj_com_olestring_to_string(olestring,
		...
		case VT_UNKNOWN:
			if (V_UNKNOWN(v) != NULL) {
				...
				if (SUCCEEDED(IUnknown_QueryInterface(V_UNKNOWN(v), &IID_IDispatch, &disp))) {
					jonj_com_wrap_dispatch(z, disp, codepage TSRMLS_CC);
		...
		case VT_DISPATCH:
			if (V_DISPATCH(v) != NULL) {
				jonj_com_wrap_dispatch(z, V_DISPATCH(v), codepage TSRMLS_CC);
			}
			break;

		case VT_VARIANT:
			/* points to another variant */
			return jonj_com_zval_from_variant(z, V_VARIANTREF(v), codepage TSRMLS_CC);
			
		default:
			jonj_com_wrap_variant(z, v, codepage TSRMLS_CC);
	}
	...
	if (ret == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "variant->zval: conversion from 0x%x ret=%d", V_VT(v), ret);
	}
...
JONJ_COM_DOTNET_API int jonj_com_copy_variant(VARIANT *dstvar, VARIANT *srcvar TSRMLS_DC)
{
	int ret = SUCCESS;
	...
	case VT_VARIANT:
		return jonj_com_copy_variant(V_VARIANTREF(dstvar), srcvar TSRMLS_CC);
		
	default:
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "variant->variant: failed to copy from 0x%x to 0x%x", V_VT(dstvar), V_VT(srcvar));
		ret = FAILURE;
	}
	return ret;
}
...
JONJ_FUNCTION(com_variant_create_instance)
{
	...
	if (zvalue) {
		jonj_com_variant_from_zval(&obj->v, zvalue, obj->code_page TSRMLS_CC);
	}
...
JONJ_FUNCTION(variant_set)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
			"Oz!", &zobj, jonj_com_variant_class_entry, &zvalue)) {
		return;
	}
	...
	if (obj->sink_dispatch) {
		jonj_com_object_enable_event_sink(obj, FALSE TSRMLS_CC);
...
static void variant_binary_operation(enum variant_binary_opcode op, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	...
	if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "OO", &zleft, jonj_com_variant_class_entry,
			&zright, jonj_com_variant_class_entry)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
	} else if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "Oz!", &zleft, jonj_com_variant_class_entry,
			&zright)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		vright = &right_val;
		jonj_com_variant_from_zval(vright, zright, codepage TSRMLS_CC);
	} else if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "z!O", &zleft, &zright, jonj_com_variant_class_entry)) {
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
		vleft = &left_val;
		jonj_com_variant_from_zval(vleft, zleft, codepage TSRMLS_CC);
	...
	if (SUCCEEDED(result)) {
		jonj_com_wrap_variant(return_value, &vres, codepage TSRMLS_CC);
	} else {
		jonj_com_throw_exception(result, NULL TSRMLS_CC);
	}
...
static void variant_unary_operation(enum variant_unary_opcode op, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	...
	if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "O", &zleft, jonj_com_variant_class_entry)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
	} else if (SUCCESS == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
			"z!", &zleft)) {
...
JONJ_FUNCTION(variant_round)
{
	...
	if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "Ol", &zleft, jonj_com_variant_class_entry, &decimals)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
	} else if (SUCCESS == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
			"z!l", &zleft, &decimals)) {
...
JONJ_FUNCTION(variant_cmp)
{
	...
	if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "OO|ll", &zleft, jonj_com_variant_class_entry,
			&zright, jonj_com_variant_class_entry, &lcid, &flags)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
	} else if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
			ZEND_NUM_ARGS() TSRMLS_CC, "Oz!|ll", &zleft, jonj_com_variant_class_entry,
			&zright, &lcid, &flags)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		vright = &right_val;
		jonj_com_variant_from_zval(vright, zright, codepage TSRMLS_CC);
			...
			ZEND_NUM_ARGS() TSRMLS_CC, "z!O|ll", &zleft, &zright, jonj_com_variant_class_entry,
			&lcid, &flags)) {
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
		vleft = &left_val;
...
JONJ_FUNCTION(variant_date_to_timestamp)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
		"O", &zleft, jonj_com_variant_class_entry)) {
		return;
	}
	obj = CDNO_FETCH(zleft);
...
JONJ_FUNCTION(variant_date_from_timestamp)
{
	...
	if (timestamp < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Timestamp value must be a positive value.");
		RETURN_FALSE;
	}
	...
	jonj_com_wrap_variant(return_value, &res, CP_ACP TSRMLS_CC);
...
JONJ_FUNCTION(variant_get_type)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
		"O", &zobj, jonj_com_variant_class_entry)) {
		return;
	}
	obj = CDNO_FETCH(zobj);
...
JONJ_FUNCTION(variant_set_type)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
		"Ol", &zobj, jonj_com_variant_class_entry, &vt)) {
		return;
	}
	obj = CDNO_FETCH(zobj);
...
JONJ_FUNCTION(variant_cast)
{
	...
	if (SUCCEEDED(res)) {
		jonj_com_wrap_variant(return_value, &vres, obj->code_page TSRMLS_CC);
	} else {
		char *werr, *msg;
