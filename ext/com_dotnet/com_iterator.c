...
{
	struct jonj_com_iterator *I = (struct jonj_com_iterator*)iter->data;
...
static int com_iter_move_forwards(zend_object_iterator *iter TSRMLS_DC)
{
	...
	if (I->ev) {
		...
		I->key++;
		if (jonj_com_safearray_get_elem(&I->safe_array, &I->v, (LONG)I->key TSRMLS_CC) == 0) {
			I->key = (ulong)-1;
			return FAILURE;
		}
	}
	...
	MAKE_STD_ZVAL(ptr);
	jonj_com_zval_from_variant(ptr, &I->v, I->code_page TSRMLS_CC);
	/* jonj_com_wrap_variant(ptr, &I->v, I->code_page TSRMLS_CC); */
	I->zdata = ptr;
	return SUCCESS;
}
...
zend_object_iterator *jonj_com_iter_get(zend_class_entry *ce, zval *object, int by_ref TSRMLS_DC)
{
	...
	if (V_VT(&obj->v) != VT_DISPATCH && !V_ISARRAY(&obj->v)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "variant is not an object or array VT=%d", V_VT(&obj->v));
		return NULL;
	}
	...
	I = (struct jonj_com_iterator*)ecalloc(1, sizeof(*I));
	...
	if (V_ISARRAY(&obj->v)) {
		...
		/* same semantics as foreach on a JONJ array;
		...
		if (jonj_com_safearray_get_elem(&I->safe_array, &I->v, bound TSRMLS_CC)) {
			I->key = bound;
			MAKE_STD_ZVAL(ptr);
			jonj_com_zval_from_variant(ptr, &I->v, I->code_page TSRMLS_CC);
			I->zdata = ptr;
