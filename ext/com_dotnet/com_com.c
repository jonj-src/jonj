...
JONJ_FUNCTION(com_create_instance)
{
	...
	if (FAILURE == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET,
		...
		jonj_com_throw_exception(E_INVALIDARG, "Could not create COM object - invalid arguments!" TSRMLS_CC);
		ZVAL_NULL(object);
		return;
	}
	...
	if (server_name && !COMG(allow_dcom)) {
		jonj_com_throw_exception(E_ERROR, "DCOM has been disabled by your administrator [com.allow_dcom=0]" TSRMLS_CC);
		return;
	}

	moniker = jonj_com_string_to_olestring(module_name, module_name_len, obj->code_page TSRMLS_CC);
	...
	if (server_name) {
		info.dwReserved1 = 0;
		info.dwReserved2 = 0;
		info.pwszName = jonj_com_string_to_olestring(server_name, server_name_len, obj->code_page TSRMLS_CC);

		if (user_name) {
			authid.User = jonj_com_string_to_olestring(user_name, -1, obj->code_page TSRMLS_CC);
	...
	if (FAILED(res)) {
		...
		werr = jonj_win32_error_to_msg(res);
		spprintf(&msg, 0, "Failed to create COM object `%s': %s", module_name, werr);
		LocalFree(werr);

		jonj_com_throw_exception(res, msg TSRMLS_CC);
		efree(msg);
		ZVAL_NULL(object);
		return;
	}
	...
	if (FAILED(IDispatch_GetTypeInfo(V_DISPATCH(&obj->v), 0, LANG_NEUTRAL, &obj->typeinfo)) && typelib_name) {
		...
		TL = jonj_com_load_typelib_via_cache(typelib_name, obj->code_page, &cached TSRMLS_CC);

		if (TL) {
			if (COMG(autoreg_on) && !cached) {
				jonj_com_import_typelib(TL, mode, obj->code_page TSRMLS_CC);
			}
		...
		if (SUCCEEDED(ITypeInfo_GetContainingTypeLib(obj->typeinfo, &TL, &idx))) {
			...
			if (SUCCEEDED(ITypeLib_GetDocumentation(TL, -1, &name, NULL, NULL, NULL))) {
				typelib_name = jonj_com_olestring_to_string(name, &typelib_name_len, obj->code_page TSRMLS_CC);

				if (SUCCESS == zend_ts_hash_add(&jonj_com_typelibraries, typelib_name, typelib_name_len+1, (void*)&TL, sizeof(ITypeLib*), NULL)) {
					jonj_com_import_typelib(TL, mode, obj->code_page TSRMLS_CC);
...
JONJ_FUNCTION(com_get_active_object)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l",
		...
		jonj_com_throw_exception(E_INVALIDARG, "Invalid arguments!" TSRMLS_CC);
		return;
	}

	module = jonj_com_string_to_olestring(module_name, module_name_len, code_page TSRMLS_CC);

	res = CLSIDFromString(module, &clsid);

	if (FAILED(res)) {
		jonj_com_throw_exception(res, NULL TSRMLS_CC);
	} else {
		...
		if (FAILED(res)) {
			...
			if (FAILED(res)) {
				...
				jonj_com_wrap_dispatch(return_value, obj, code_page TSRMLS_CC);
			}
		}
	}
	...
	if (silent == 0 && FAILED(hr)) {
		...
		switch (hr) {
			case DISP_E_EXCEPTION:
				if (e.bstrSource) {
					source = jonj_com_olestring_to_string(e.bstrSource, &source_len, obj->code_page TSRMLS_CC);
					SysFreeString(e.bstrSource);
				}
				if (e.bstrDescription) {
					desc = jonj_com_olestring_to_string(e.bstrDescription, &desc_len, obj->code_page TSRMLS_CC);
			...
			case DISP_E_TYPEMISMATCH:
				desc = jonj_win32_error_to_msg(hr);
		...
		if (msg) {
			jonj_com_throw_exception(hr, msg TSRMLS_CC);
			efree(msg);
		}
	}
		...
		int namelen, DISPID *dispid TSRMLS_DC)
{
	...
	olename = jonj_com_string_to_olestring(name, namelen, obj->code_page TSRMLS_CC);
		...
		WORD flags,	VARIANT *v, int nargs, zval ***args TSRMLS_DC)
{
	...
	hr = jonj_com_get_id_of_name(obj, name, namelen, &dispid TSRMLS_CC);

	if (FAILED(hr)) {
		char *winerr = NULL;
		char *msg = NULL;
		winerr = jonj_win32_error_to_msg(hr);
	...
	if (byref_count) {
		...
		for (j = 0, i = 0; i < nargs; i++) {
			if (f->arg_info[nargs - i - 1].pass_by_reference) {
				...
				jonj_com_variant_from_zval(&byref_vals[j], *args[nargs - i - 1], obj->code_page TSRMLS_CC);
				...
				jonj_com_variant_from_zval(&vargs[i], *args[nargs - i - 1], obj->code_page TSRMLS_CC);
			}
		}
	...
	hr = jonj_com_invoke_helper(obj, dispid, flags, &disp_params, v, 0, 0 TSRMLS_CC);	
		...
		WORD flags,	VARIANT *v, int nargs, zval **args, int silent, int allow_noarg TSRMLS_DC)
{
	...
	/* this will create an exception if needed */
	hr = jonj_com_invoke_helper(obj, dispid, flags, &disp_params, v, silent, allow_noarg TSRMLS_CC);	
		...
		WORD flags,	VARIANT *v, int nargs, zval **args, int allow_noarg TSRMLS_DC)
{
	...
	return jonj_com_do_invoke_by_id(obj, dispid, flags, v, nargs, args, 0, allow_noarg TSRMLS_CC);
}
...
JONJ_FUNCTION(com_create_guid)
{
	...
	if (CoCreateGuid(&retval) == S_OK && StringFromCLSID(&retval, &guid_string) == S_OK) {
		...
		Z_STRVAL_P(return_value) = jonj_com_olestring_to_string(guid_string, &Z_STRLEN_P(return_value), CP_ACP TSRMLS_CC);
...
/* {{{ proto bool com_event_sink(object comobject, object sinkobject [, mixed sinkinterface])
   Connect events from a COM object to a JONJ object */
JONJ_FUNCTION(com_event_sink)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "Oo|z/",
			&object, jonj_com_variant_class_entry, &sinkobject, &sink)) {
		RETURN_FALSE;
	}
	...
	typeinfo = jonj_com_locate_typeinfo(typelibname, obj, dispname, 1 TSRMLS_CC);
	...
	if (typeinfo) {
		...
		if (jonj_com_process_typeinfo(typeinfo, id_to_name, 0, &obj->sink_id, obj->code_page TSRMLS_CC)) {

			/* Create the COM wrapper for this sink */
			obj->sink_dispatch = jonj_com_wrapper_export_as_sink(sinkobject, &obj->sink_id, id_to_name TSRMLS_CC);

			/* Now hook it up to the source */
			jonj_com_object_enable_event_sink(obj, TRUE TSRMLS_CC);
			RETVAL_TRUE;
...
/* {{{ proto bool com_print_typeinfo(object comobject | string typelib, string dispinterface, bool wantsink)
   Print out a JONJ class definition for a dispatchable interface */
JONJ_FUNCTION(com_print_typeinfo)
{
	...
	jonj_com_dotnet_object *obj = NULL;
	ITypeInfo *typeinfo;
	...
	typeinfo = jonj_com_locate_typeinfo(typelibname, obj, ifacename, wantsink ? 1 : 0 TSRMLS_CC);
	if (typeinfo) {
		jonj_com_process_typeinfo(typeinfo, NULL, 1, NULL, obj ? obj->code_page : COMG(code_page) TSRMLS_CC);
		ITypeInfo_Release(typeinfo);
		RETURN_TRUE;
	} else {
...
JONJ_FUNCTION(com_load_typelib)
{
	...
	jonj_com_initialize(TSRMLS_C);
	pTL = jonj_com_load_typelib_via_cache(name, codepage, &cached TSRMLS_CC);
	if (pTL) {
		if (cached) {
			RETVAL_TRUE;
		} else if (jonj_com_import_typelib(pTL, cs ? CONST_CS : 0, codepage TSRMLS_CC) == SUCCESS) {
			RETVAL_TRUE;
		}
