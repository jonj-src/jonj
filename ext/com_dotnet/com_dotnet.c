...
JONJ_FUNCTION(com_dotnet_create_instance)
{
	...
	if (stuff == NULL) {
		...
		if (FAILED(hr)) {
			char buf[1024];
			char *err = jonj_win32_error_to_msg(hr);
			snprintf(buf, sizeof(buf), "Failed to init .Net runtime [%s] %s", where, err);
			if (err)
				LocalFree(err);
			jonj_com_throw_exception(hr, buf TSRMLS_CC);
			ZVAL_NULL(object);
			return;
		}
		stuff = (struct dotnet_runtime_stuff*)COMG(dotnet_runtime_stuff);
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|l",
			...
			&obj->code_page)) {
		jonj_com_throw_exception(E_INVALIDARG, "Could not create .Net object - invalid arguments!" TSRMLS_CC);
		ZVAL_NULL(object);
		return;
	}

	oletype = jonj_com_string_to_olestring(datatype_name, datatype_name_len, obj->code_page TSRMLS_CC);
	oleassembly = jonj_com_string_to_olestring(assembly_name, assembly_name_len, obj->code_page TSRMLS_CC);
