...
/* This module exports a JONJ object as a COM object by wrapping it
 * using IDispatchEx */
...
static void disp_destructor(jonj_dispatchex *disp TSRMLS_DC);

static void dispatch_dtor(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	jonj_dispatchex *disp = (jonj_dispatchex *)rsrc->ptr;
...
#define FETCH_DISP(methname)																			\
	jonj_dispatchex *disp = (jonj_dispatchex*)This; 														\
	TSRMLS_FETCH();																						\
	if (COMG(rshutdown_started)) {																		\
		trace(" JONJ Object:%p (name:unknown) %s\n", disp->object,  methname); 							\
	} else {																							\
		trace(" JONJ Object:%p (name:%s) %s\n", disp->object, Z_OBJCE_P(disp->object)->name, methname); 	\
	}																									\
	...
	/* [size_is][out] */ DISPID *rgDispId)
{
	...
	for (i = 0; i < cNames; i++) {
		...
		name = jonj_com_olestring_to_string(rgszNames[i], &namelen, COMG(code_page) TSRMLS_CC);
	...
	/* [out] */ DISPID *pid)
{
	...
	name = jonj_com_olestring_to_string(bstrName, &namelen, COMG(code_page) TSRMLS_CC);
	...
	if (SUCCESS == zend_hash_index_find(disp->dispid_to_name, id, (void**)&name)) {
		...
		if (pdp->cArgs) {
			...
			for (i = 0; i < pdp->cArgs; i++) {
				...
				jonj_com_wrap_variant(zarg, arg, COMG(code_page) TSRMLS_CC);
		...
		/* TODO: if JONJ raises an exception here, we should catch it
		...
		if (wFlags & DISPATCH_PROPERTYGET) {
			...
			zend_try {
				if (SUCCESS == call_user_function_ex(EG(function_table), &disp->object, *name,
					...
					for (i = 0; i < pdp->cArgs; i++) {
						jonj_com_dotnet_object *obj = CDNO_FETCH(*params[i]);
						...
						if ((V_VT(dstvar) & VT_BYREF) && obj->modified ) {
							...
							jonj_com_copy_variant(dstvar, srcvar TSRMLS_CC);   
						}
					}
				} else {
		...
		if (retval) {
			if (pvarRes) {
				VariantInit(pvarRes);
				jonj_com_variant_from_zval(pvarRes, retval, COMG(code_page) TSRMLS_CC);
			}
	...
	/* [out] */ BSTR *pbstrName)
{
	...
	if (SUCCESS == zend_hash_index_find(disp->dispid_to_name, id, (void**)&name)) {
		OLECHAR *olestr = jonj_com_string_to_olestring(Z_STRVAL_P(name), Z_STRLEN_P(name), COMG(code_page) TSRMLS_CC);
...
static struct IDispatchExVtbl jonj_dispatch_vtbl = {
	disp_queryinterface,
	disp_addref,
	disp_release,
 ...
 * dispatch ids */
static void generate_dispids(jonj_dispatchex *disp TSRMLS_DC)
{
	HashPosition pos;
	char *name = NULL;
	zval *tmp;
...
static jonj_dispatchex *disp_constructor(zval *object TSRMLS_DC)
{
	jonj_dispatchex *disp = (jonj_dispatchex*)CoTaskMemAlloc(sizeof(jonj_dispatchex));

	trace("constructing a COM wrapper for JONJ object %p (%s)\n", object, Z_OBJCE_P(object)->name);
	
	if (disp == NULL)
		return NULL;

	memset(disp, 0, sizeof(jonj_dispatchex));

	disp->engine_thread = GetCurrentThreadId();
	disp->lpVtbl = &jonj_dispatch_vtbl;
	disp->refcount = 1;
...
{	
	...
	if (COMG(rshutdown_started)) {
		trace("destroying COM wrapper for JONJ object %p (name:unknown)\n", disp->object);
	} else {
		trace("destroying COM wrapper for JONJ object %p (name:%s)\n", disp->object, Z_OBJCE_P(disp->object)->name);
	}
...
JONJ_COM_DOTNET_API IDispatch *jonj_com_wrapper_export_as_sink(zval *val, GUID *sinkid,
	   HashTable *id_to_name TSRMLS_DC)
{
	jonj_dispatchex *disp = disp_constructor(val TSRMLS_CC);
	HashPosition pos;
	char *name = NULL;
	zval *tmp, **ntmp;
	int namelen;
...
{
	jonj_dispatchex *disp = NULL;

	if (Z_TYPE_P(val) != IS_OBJECT) {
		return NULL;
	}

	if (jonj_com_is_valid_object(val TSRMLS_CC)) {
		/* pass back its IDispatch directly */
		jonj_com_dotnet_object *obj = CDNO_FETCH(val);
