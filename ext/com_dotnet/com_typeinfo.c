...
JONJ_COM_DOTNET_API ITypeLib *jonj_com_load_typelib(char *search_string, int codepage TSRMLS_DC)
{
	...
	search_string = jonj_strtok_r(search_string, ",", &strtok_buf);

	if (search_string == NULL) {
		return NULL;
	}

	major = jonj_strtok_r(NULL, ",", &strtok_buf);
	minor = jonj_strtok_r(NULL, ",", &strtok_buf);

	p = jonj_com_string_to_olestring(search_string, strlen(search_string), codepage TSRMLS_CC);
							...
							for (j = 0; j < VersionCount; j++) {
								...
								if (ERROR_SUCCESS == RegQueryValue(hsubkey, version, libname, &libnamelen)) {
									if (0 == stricmp(libname, search_string)) {
										...
										/* recurse */
										TL = jonj_com_load_typelib(str, codepage TSRMLS_CC);
...
JONJ_COM_DOTNET_API int jonj_com_import_typelib(ITypeLib *TL, int mode, int codepage TSRMLS_DC)
{
	int i, j, interfaces;
	TYPEKIND pTKind;
	ITypeInfo *TypeInfo;
	...
	for (i = 0; i < interfaces; i++) {
		...
		if (pTKind == TKIND_ENUM) {
			...
			for (j = 0; ; j++) {
				...
				const_name = jonj_com_olestring_to_string(bstr_ids, &c.name_len, codepage TSRMLS_CC);
				...
				if (zend_get_constant(c.name, c.name_len - 1, &exists TSRMLS_CC)) {
					if (COMG(autoreg_verbose) && !compare_function(&results, &c.value, &exists TSRMLS_CC)) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type library constant %s is already defined", c.name);
					}
					free(c.name);
					ITypeInfo_ReleaseVarDesc(TypeInfo, pVarDesc);
					continue;
				...
				jonj_com_zval_from_variant(&value, pVarDesc->lpvarValue, codepage TSRMLS_CC);
	...
	int codepage, int *cached TSRMLS_DC)
{
	...
	if (zend_ts_hash_find(&jonj_com_typelibraries, search_string, l+1,
			(void**)&TLp) == SUCCESS) {
		*cached = 1;
	...
	*cached = 0;
	...
	TL = jonj_com_load_typelib(name_dup, codepage TSRMLS_CC);
	efree(name_dup);

	if (TL) {
		if (SUCCESS == zend_ts_hash_update(&jonj_com_typelibraries,
	...
	if (obj) {
		if (dispname == NULL && sink) {
			...
			if (!gotguid && SUCCEEDED(IDispatch_QueryInterface(V_DISPATCH(&obj->v), &IID_IProvideClassInfo, (void**)&pci))) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "IProvideClassInfo: this code not yet written!");
		...
		typelib = jonj_com_load_typelib(typelibname, CP_THREAD_ACP TSRMLS_CC);
	} 

	if (!gotguid && dispname && typelib) {
		unsigned short cfound;
		MEMBERID memid;
		OLECHAR *olename = jonj_com_string_to_olestring(dispname, strlen(dispname), CP_ACP TSRMLS_CC);
...
static char *jonj_com_string_from_clsid(const CLSID *clsid, int codepage TSRMLS_DC)
{
	LPOLESTR ole_clsid;
	char *clsid_str;

	StringFromCLSID(clsid, &ole_clsid);
	clsid_str = jonj_com_olestring_to_string(ole_clsid, NULL, codepage TSRMLS_CC);
	LocalFree(ole_clsid);
...
int jonj_com_process_typeinfo(ITypeInfo *typeinfo, HashTable *id_to_name, int printdef, GUID *guid, int codepage TSRMLS_DC)
{
	...
	if (id_to_name == NULL || attr->typekind == TKIND_DISPATCH) {
		...
		if (printdef) {
			...
			ansiname = jonj_com_olestring_to_string(olename, &ansinamelen, codepage TSRMLS_CC);
			SysFreeString(olename);

			guidstring = jonj_com_string_from_clsid(&attr->guid, codepage TSRMLS_CC);
			jonj_printf("class %s { /* GUID=%s */\n", ansiname, guidstring);
			efree(guidstring);
			...
			if (!isprop || lastid != func->memid) {
				...
				if (printdef) {
					...
					if (isprop) {
						...
						if (olename) {
							funcdesc = jonj_com_olestring_to_string(olename, &funcdesclen, codepage TSRMLS_CC);
							SysFreeString(olename);
							jonj_printf("\t/* %s */\n", funcdesc);
							efree(funcdesc);
						}
						...
						jonj_printf("\tfunction %s(\n", ansiname);

						for (j = 0; j < func->cParams; j++) {
							ELEMDESC *elem = &func->lprgelemdescParam[j];

							jonj_printf("\t\t/* %s [%d] ", vt_to_string(elem->tdesc.vt), elem->tdesc.vt);
							...
							if (j+1 < (int)cnames) {
								funcdesc = jonj_com_olestring_to_string(names[j+1], &funcdesclen, codepage TSRMLS_CC);
