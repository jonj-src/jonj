	...
	if (obj->ptr != NULL) {
		if (hnd->read_char_func) {
			...
			if (hnd->read_int_func) {
				...
				if (retint == -1) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Internal libxml error returned");
					return FAILURE;
				}
			}
		}
...
void xmlreader_write_property(zval *object, zval *member, zval *value, const zend_literal *key TSRMLS_DC)
{
	...
	if (ret == SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot write to read-only property");
	} else {
		std_hnd = zend_get_std_object_handlers();
...
zend_module_entry xmlreader_module_entry = {
	...
	"xmlreader",
	NULL,
	JONJ_MINIT(xmlreader),
	JONJ_MSHUTDOWN(xmlreader),
	NULL,
	NULL,
	JONJ_MINFO(xmlreader),
...
/* {{{ jonj_xmlreader_string_arg */
static void jonj_xmlreader_string_arg(INTERNAL_FUNCTION_PARAMETERS, xmlreader_read_one_char_t internal_function) {
	zval *id;
	int name_len = 0;
	char *retchar = NULL;
	xmlreader_object *intern;
	...
	if (!name_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument cannot be an empty string");
		RETURN_FALSE;
	}
...
/* {{{ jonj_xmlreader_no_arg */
static void jonj_xmlreader_no_arg(INTERNAL_FUNCTION_PARAMETERS, xmlreader_read_int_t internal_function) {
	zval *id;
	int retval;
	xmlreader_object *intern;
...
/* {{{ jonj_xmlreader_no_arg_string */
static void jonj_xmlreader_no_arg_string(INTERNAL_FUNCTION_PARAMETERS, xmlreader_read_char_t internal_function) {
	zval *id;
	char *retchar = NULL;
	xmlreader_object *intern;
...
/* {{{ jonj_xmlreader_set_relaxng_schema */
static void jonj_xmlreader_set_relaxng_schema(INTERNAL_FUNCTION_PARAMETERS, int type) {
#ifdef LIBXML_SCHEMAS_ENABLED
	zval *id;
	...
	if (source != NULL && !source_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Schema data source is required");
		RETURN_FALSE;
	}
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set schema. This must be set prior to reading or schema contains errors.");

	RETURN_FALSE;
#else
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No Schema support built into libxml.");
...
JONJ_METHOD(xmlreader, getAttribute)
{
	jonj_xmlreader_string_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderGetAttribute);
}
/* }}} */

/* {{{ proto string XMLReader::getAttributeNo(int index)
Get value of an attribute at index from current element */
JONJ_METHOD(xmlreader, getAttributeNo)
{
	zval *id;
	long attr_pos;
	char *retchar = NULL;
...
JONJ_METHOD(xmlreader, getAttributeNs)
{
	zval *id;
	...
	if (name_len == 0 || ns_uri_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attribute Name and Namespace URI cannot be empty");
		RETURN_FALSE;
	}
...
JONJ_METHOD(xmlreader, getParserProperty)
{
	zval *id;
	long property;
	int retval = -1;
	...
	if (retval == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parser property");
		RETURN_FALSE;
	}
...
JONJ_METHOD(xmlreader, isValid)
{
	jonj_xmlreader_no_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderIsValid);
}
/* }}} */

/* {{{ proto string XMLReader::lookupNamespace(string prefix)
Return namespaceURI for associated prefix on current node */
JONJ_METHOD(xmlreader, lookupNamespace)
{
	jonj_xmlreader_string_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderLookupNamespace);
}
/* }}} */
...
JONJ_METHOD(xmlreader, moveToAttribute)
{
	zval *id;
...
JONJ_METHOD(xmlreader, moveToAttributeNo)
{
	zval *id;
	long attr_pos;
	int retval;
...
Returns TRUE on success and FALSE on failure */
JONJ_METHOD(xmlreader, moveToAttributeNs)
{
	zval *id;
...
JONJ_METHOD(xmlreader, moveToElement)
{
	jonj_xmlreader_no_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderMoveToElement);
}
/* }}} */

/* {{{ proto boolean XMLReader::moveToFirstAttribute()
Moves the position of the current instance to the first attribute associated with the current node. */
JONJ_METHOD(xmlreader, moveToFirstAttribute)
{
	jonj_xmlreader_no_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderMoveToFirstAttribute);
}
/* }}} */

/* {{{ proto boolean XMLReader::moveToNextAttribute()
Moves the position of the current instance to the next attribute associated with the current node. */
JONJ_METHOD(xmlreader, moveToNextAttribute)
{
	jonj_xmlreader_no_arg(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderMoveToNextAttribute);
}
/* }}} */

/* {{{ proto boolean XMLReader::read()
Moves the position of the current instance to the next node in the stream. */
JONJ_METHOD(xmlreader, read)
{
	zval *id;
	int retval;
	xmlreader_object *intern;
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Load Data before trying to read");
	RETURN_FALSE;
}
/* }}} */
...
JONJ_METHOD(xmlreader, open)
{
	...
	if (reader == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to open source data");
		RETURN_FALSE;
	}
...
JONJ_METHOD(xmlreader, resetState)
{
...
JONJ_METHOD(xmlreader, readInnerXml)
{
	jonj_xmlreader_no_arg_string(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderReadInnerXml);
}
/* }}} */

/* {{{ proto boolean XMLReader::readOuterXml()
Reads the contents of the current node, including child nodes and markup. */
JONJ_METHOD(xmlreader, readOuterXml)
{
	jonj_xmlreader_no_arg_string(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderReadOuterXml);
}
/* }}} */

/* {{{ proto boolean XMLReader::readString()
Reads the contents of an element or a text node as a string. */
JONJ_METHOD(xmlreader, readString)
{
	jonj_xmlreader_no_arg_string(INTERNAL_FUNCTION_PARAM_PASSTHRU, xmlTextReaderReadString);
}
/* }}} */

/* {{{ proto boolean XMLReader::setSchema(string filename)
Use W3C XSD schema to validate the document as it is processed. Activation is only possible before the first Read(). */
JONJ_METHOD(xmlreader, setSchema)
{
#ifdef LIBXML_SCHEMAS_ENABLED
	zval *id;
	int source_len = 0, retval = -1;
...
JONJ_METHOD(xmlreader, setParserProperty)
{
	zval *id;
	long property;
	int retval = -1;
...
JONJ_METHOD(xmlreader, setRelaxNGSchema)
{
	jonj_xmlreader_set_relaxng_schema(INTERNAL_FUNCTION_PARAM_PASSTHRU, XMLREADER_LOAD_FILE);
}
/* }}} */

/* {{{ proto boolean XMLReader::setRelaxNGSchemaSource(string source)
Sets the string that the XMLReader will parse. */
JONJ_METHOD(xmlreader, setRelaxNGSchemaSource)
{
	jonj_xmlreader_set_relaxng_schema(INTERNAL_FUNCTION_PARAM_PASSTHRU, XMLREADER_LOAD_STRING);
}
/* }}} */
...
/* Get the URI of the current script so that we can set the base directory in libxml */
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to load source data");
	RETURN_FALSE;
}
/* }}} */
...
JONJ_METHOD(xmlreader, expand)
{
	...
	xmlNode *node, *nodec;
	xmlDocPtr docp = NULL;
	jonj_libxml_node_object *domobj = NULL;
	...
	if (intern && intern->ptr) {
		...
		if (node == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An Error Occurred while expanding ");
			RETURN_FALSE;
		} else {
			nodec = xmlDocCopyNode(node, docp, 1);
			if (nodec == NULL) {
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Cannot expand this node type");
				RETURN_FALSE;
			} else {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Load Data before trying to expand");
		RETURN_FALSE;
	}
#else
	jonj_error(E_WARNING, "DOM support is not enabled");
	return;
#endif
}
/* }}} */
...
static const zend_function_entry xmlreader_functions[] = {
	JONJ_ME(xmlreader, close, arginfo_xmlreader_close, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, getAttribute, arginfo_xmlreader_getAttribute, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, getAttributeNo, arginfo_xmlreader_getAttributeNo, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, getAttributeNs, arginfo_xmlreader_getAttributeNs, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, getParserProperty, arginfo_xmlreader_getParserProperty, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, isValid, arginfo_xmlreader_isValid, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, lookupNamespace, arginfo_xmlreader_lookupNamespace, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToAttributeNo, arginfo_xmlreader_moveToAttributeNo, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToAttribute, arginfo_xmlreader_moveToAttribute, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToAttributeNs, arginfo_xmlreader_moveToAttributeNs, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToElement, arginfo_xmlreader_moveToElement, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToFirstAttribute, arginfo_xmlreader_moveToFirstAttribute, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, moveToNextAttribute, arginfo_xmlreader_moveToNextAttribute, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, open, arginfo_xmlreader_open, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_ME(xmlreader, read, arginfo_xmlreader_read, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, next, arginfo_xmlreader_next, ZEND_ACC_PUBLIC)
#if LIBXML_VERSION >= 20620
	JONJ_ME(xmlreader, readInnerXml, arginfo_xmlreader_readInnerXml, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, readOuterXml, arginfo_xmlreader_readOuterXml, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, readString, arginfo_xmlreader_readString, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, setSchema, arginfo_xmlreader_setSchema, ZEND_ACC_PUBLIC)
#endif
/* Not Yet Implemented though defined in libxml as of 2.6.9dev
	JONJ_ME(xmlreader, resetState, NULL, ZEND_ACC_PUBLIC)
*/
	JONJ_ME(xmlreader, setParserProperty, arginfo_xmlreader_setParserProperty, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, setRelaxNGSchema, arginfo_xmlreader_setRelaxNGSchema, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, setRelaxNGSchemaSource, arginfo_xmlreader_setRelaxNGSchemaSource, ZEND_ACC_PUBLIC)
	JONJ_ME(xmlreader, XML, arginfo_xmlreader_XML, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_ME(xmlreader, expand, arginfo_xmlreader_expand, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
...
JONJ_MINFO_FUNCTION(xmlreader)
{
	...
	{
		jonj_info_print_table_row(2, "XMLReader", "enabled");
	}
