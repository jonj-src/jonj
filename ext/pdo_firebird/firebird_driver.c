...
static int pdo_firebird_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC) /* {{{ */
{
	...
	jonj_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, 3);
	...
	if (!dbh->methods) {
		...
		zend_throw_exception_ex(jonj_pdo_get_exception(), H->isc_status[1] TSRMLS_CC, "SQLSTATE[%s] [%d] %s",
