...
#include "jonj_sqlite3.h"
#include "jonj_sqlite3_structs.h"
#include "main/SAPI.h"
...
static JONJ_GINIT_FUNCTION(sqlite3);
static int jonj_sqlite3_authorizer(void *autharg, int access_type, const char *arg3, const char *arg4, const char *arg5, const char *arg6);
static void sqlite3_param_dtor(void *data);
static int jonj_sqlite3_compare_stmt_zval_free(jonj_sqlite3_free_list **free_list, zval *statement);

/* {{{ Error Handler
*/
static void jonj_sqlite3_error(jonj_sqlite3_db_object *db_obj, char *format, ...)
{
	va_list arg;
	char 	*message;
	TSRMLS_FETCH();
	...
	if (db_obj && db_obj->exception) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", message);
	}
...
#define SQLITE3_CHECK_INITIALIZED(db_obj, member, class_name) \
	if (!(db_obj) || !(member)) { \
		jonj_sqlite3_error(db_obj, "The " #class_name " object has not been correctly initialised"); \
		RETURN_FALSE; \
	}

#define SQLITE3_CHECK_INITIALIZED_STMT(member, class_name) \
	if (!(member)) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The " #class_name " object has not been correctly initialised"); \
		RETURN_FALSE; \
	}
...
*/
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("sqlite3.extension_dir",  NULL, JONJ_INI_SYSTEM, OnUpdateString, extension_dir, zend_sqlite3_globals, sqlite3_globals)
JONJ_INI_END()
/* }}} */
...
/* Class entries */
zend_class_entry *jonj_sqlite3_sc_entry;
zend_class_entry *jonj_sqlite3_stmt_entry;
zend_class_entry *jonj_sqlite3_result_entry;
...
JONJ_METHOD(sqlite3, open)
{
	...
	db_obj = (jonj_sqlite3_db_object *)zend_object_store_get_object(object TSRMLS_CC);
	...
	if (filename_len != 0 && (filename_len != sizeof(":memory:")-1 ||
		...
		if (PG(safe_mode) && (!jonj_checkuid(fullpath, NULL, CHECKUID_CHECK_FILE_AND_DIR))) {
		...
		if (jonj_check_open_basedir(fullpath TSRMLS_CC)) {
...
#endif
		sqlite3_set_authorizer(db_obj->db, jonj_sqlite3_authorizer, NULL);
	}
	...
	if (db_obj->initialised) {
		...
		if(db_obj->db) {
            ...
            if (errcode != SQLITE_OK) {
			    jonj_sqlite3_error(db_obj, "Unable to close database: %d, %s", errcode, sqlite3_errmsg(db_obj->db));
                RETURN_FALSE;
		    }
        }
...
JONJ_METHOD(sqlite3, exec)
{
	...
	if (sqlite3_exec(db_obj->db, sql, NULL, NULL, &errtext) != SQLITE_OK) {
		jonj_sqlite3_error(db_obj, "%s", errtext);
...
JONJ_METHOD(sqlite3, lastInsertRowID)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	db_obj = (jonj_sqlite3_db_object *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3, lastErrorCode)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	db_obj = (jonj_sqlite3_db_object *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3, lastErrorMsg)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	db_obj = (jonj_sqlite3_db_object *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3, busyTimeout)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	long ms;
	...
	if (return_code != SQLITE_OK) {
		jonj_sqlite3_error(db_obj, "Unable to set busy timeout: %d, %s", return_code, sqlite3_errmsg(db_obj->db));
		RETURN_FALSE;
	}
...
JONJ_METHOD(sqlite3, loadExtension)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	...
	) {		jonj_sqlite3_error(db_obj, "Not supported in multithreaded Web servers");
		RETURN_FALSE;
	}
#endif

	if (!SQLITE3G(extension_dir)) {
		jonj_sqlite3_error(db_obj, "SQLite Extension are disabled");
		RETURN_FALSE;
	}

	if (extension_len == 0) {
		jonj_sqlite3_error(db_obj, "Empty string as an extension");
		RETURN_FALSE;
	}
	...
	if (!VCWD_REALPATH(lib_path, fullpath)) {
		jonj_sqlite3_error(db_obj, "Unable to load extension at '%s'", lib_path);
		efree(lib_path);
		RETURN_FALSE;
	}
	...
	if (strncmp(fullpath, extension_dir, extension_dir_len) != 0) {
		jonj_sqlite3_error(db_obj, "Unable to open extensions outside the defined directory");
		RETURN_FALSE;
	}
...
JONJ_METHOD(sqlite3, escapeString)
{
	char *sql, *ret;
	int sql_len;
...
JONJ_METHOD(sqlite3, prepare)
{
	...
	char *sql;
	int sql_len, errcode;
	jonj_sqlite3_free_list *free_item;
	...
	object_init_ex(return_value, jonj_sqlite3_stmt_entry);
	stmt_obj = (jonj_sqlite3_stmt *)zend_object_store_get_object(return_value TSRMLS_CC);
	...
	if (errcode != SQLITE_OK) {
		jonj_sqlite3_error(db_obj, "Unable to prepare statement: %d, %s", errcode, sqlite3_errmsg(db_obj->db));
	...
	free_item = emalloc(sizeof(jonj_sqlite3_free_list));
...
JONJ_METHOD(sqlite3, query)
{
	jonj_sqlite3_db_object *db_obj;
	jonj_sqlite3_result *result;
	jonj_sqlite3_stmt *stmt_obj;
	zval *object = getThis();
	zval *stmt = NULL;
	char *sql, *errtext = NULL;
	...
	object_init_ex(stmt, jonj_sqlite3_stmt_entry);
	stmt_obj = (jonj_sqlite3_stmt *)zend_object_store_get_object(stmt TSRMLS_CC);
	stmt_obj->db_obj = db_obj;
	stmt_obj->db_obj_zval = getThis();
	...
	if (return_code != SQLITE_OK) {
		jonj_sqlite3_error(db_obj, "Unable to prepare statement: %d, %s", return_code, sqlite3_errmsg(db_obj->db));
		zval_ptr_dtor(&stmt);
		RETURN_FALSE;
	}
	...
	object_init_ex(return_value, jonj_sqlite3_result_entry);
	result = (jonj_sqlite3_result *)zend_object_store_get_object(return_value TSRMLS_CC);
	...
	switch (return_code) {
		...
		default:
			if (!EG(exception)) {
				jonj_sqlite3_error(db_obj, "Unable to execute statement: %s", sqlite3_errmsg(db_obj->db));
			}
...
JONJ_METHOD(sqlite3, querySingle)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	char *sql, *errtext = NULL;
...
static int sqlite3_do_callback(struct jonj_sqlite3_fci *fc, zval *cb, int argc, sqlite3_value **argv, sqlite3_context *context, int is_agg TSRMLS_DC) /* {{{ */
{
	zval ***zargs = NULL;
	zval *retval = NULL;
	int i;
	int ret;
	int fake_argc;
	jonj_sqlite3_agg_context *agg_context = NULL;
	...
	if (is_agg) {
		...
		agg_context = (jonj_sqlite3_agg_context *)sqlite3_aggregate_context(context, sizeof(jonj_sqlite3_agg_context));
	...
	if ((ret = zend_call_function(&fc->fci, &fc->fcc TSRMLS_CC)) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the callback");
	}
...
static void jonj_sqlite3_callback_func(sqlite3_context *context, int argc, sqlite3_value **argv) /* {{{ */
{
	jonj_sqlite3_func *func = (jonj_sqlite3_func *)sqlite3_user_data(context);
	TSRMLS_FETCH();
...
static void jonj_sqlite3_callback_step(sqlite3_context *context, int argc, sqlite3_value **argv) /* {{{ */
{
	jonj_sqlite3_func *func = (jonj_sqlite3_func *)sqlite3_user_data(context);
	jonj_sqlite3_agg_context *agg_context = (jonj_sqlite3_agg_context *)sqlite3_aggregate_context(context, sizeof(jonj_sqlite3_agg_context));
...
static void jonj_sqlite3_callback_final(sqlite3_context *context) /* {{{ */
{
	jonj_sqlite3_func *func = (jonj_sqlite3_func *)sqlite3_user_data(context);
	jonj_sqlite3_agg_context *agg_context = (jonj_sqlite3_agg_context *)sqlite3_aggregate_context(context, sizeof(jonj_sqlite3_agg_context));
...
static int jonj_sqlite3_callback_compare(void *coll, int a_len, const void *a, int b_len, const void* b) /* {{{ */
{
	jonj_sqlite3_collation *collation = (jonj_sqlite3_collation*)coll;
	zval ***zargs = NULL;
	zval *retval = NULL;
	int ret;
	...
	if ((ret = zend_call_function(&collation->fci.fci, &collation->fci.fcc TSRMLS_CC)) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the compare callback");
	}
	...
	if (!retval) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the compare callback (invalid return type).  Collation behaviour is undefined.");
	} else {
		ret = Z_LVAL_P(retval);
	}
...
/* {{{ proto bool SQLite3::createFunction(string name, mixed callback [, int argcount])
   Allows registration of a JONJ function as a SQLite UDF that can be called within SQL statements. */
JONJ_METHOD(sqlite3, createFunction)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	jonj_sqlite3_func *func;
	...
	if (!zend_is_callable(callback_func, 0, &callback_name TSRMLS_CC)) {
		jonj_sqlite3_error(db_obj, "Not a valid callback function %s", callback_name);
		efree(callback_name);
		RETURN_FALSE;
	}
	efree(callback_name);

	func = (jonj_sqlite3_func *)ecalloc(1, sizeof(*func));

	if (sqlite3_create_function(db_obj->db, sql_func, sql_func_num_args, SQLITE_UTF8, func, jonj_sqlite3_callback_func, NULL, NULL) == SQLITE_OK) {
...
/* {{{ proto bool SQLite3::createAggregate(string name, mixed step, mixed final [, int argcount])
   Allows registration of a JONJ function for use as an aggregate. */
JONJ_METHOD(sqlite3, createAggregate)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	jonj_sqlite3_func *func;
	...
	if (sqlite3_create_function(db_obj->db, sql_func, sql_func_num_args, SQLITE_UTF8, func, NULL, jonj_sqlite3_callback_step, jonj_sqlite3_callback_final) == SQLITE_OK) {
		func->func_name = estrdup(sql_func);
...
/* {{{ proto bool SQLite3::createCollation(string name, mixed callback)
   Registers a JONJ function as a comparator that can be used with the SQL COLLATE operator. Callback must accept two strings and return an integer (as strcmp()). */
JONJ_METHOD(sqlite3, createCollation)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	jonj_sqlite3_collation *collation;
	...
	collation = (jonj_sqlite3_collation *)ecalloc(1, sizeof(*collation));
	if (sqlite3_create_collation(db_obj->db, collation_name, SQLITE_UTF8, collation, jonj_sqlite3_callback_compare) == SQLITE_OK) {
...
typedef struct {
	...
	size_t		 position;
	size_t       size;
} jonj_stream_sqlite3_data;

static size_t jonj_sqlite3_stream_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
/*	jonj_stream_sqlite3_data *sqlite3_stream = (jonj_stream_sqlite3_data *) stream->abstract; */
	
	return 0;
}

static size_t jonj_sqlite3_stream_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	jonj_stream_sqlite3_data *sqlite3_stream = (jonj_stream_sqlite3_data *) stream->abstract;
...
static int jonj_sqlite3_stream_close(jonj_stream *stream, int close_handle TSRMLS_DC)
{
	jonj_stream_sqlite3_data *sqlite3_stream = (jonj_stream_sqlite3_data *) stream->abstract;
...
static int jonj_sqlite3_stream_flush(jonj_stream *stream TSRMLS_DC)
{
	/* do nothing */
	return 0;
}
...
/* {{{ */
static int jonj_sqlite3_stream_seek(jonj_stream *stream, off_t offset, int whence, off_t *newoffs TSRMLS_DC)
{
	jonj_stream_sqlite3_data *sqlite3_stream = (jonj_stream_sqlite3_data *) stream->abstract;
...
static int jonj_sqlite3_stream_cast(jonj_stream *stream, int castas, void **ret TSRMLS_DC)
{
	return FAILURE;
}

static int jonj_sqlite3_stream_stat(jonj_stream *stream, jonj_stream_statbuf *ssb TSRMLS_DC)
{
	jonj_stream_sqlite3_data *sqlite3_stream = (jonj_stream_sqlite3_data *) stream->abstract;
...
static jonj_stream_ops jonj_stream_sqlite3_ops = {
	jonj_sqlite3_stream_write,
	jonj_sqlite3_stream_read,
	jonj_sqlite3_stream_close,
	jonj_sqlite3_stream_flush,
	"SQLite3",
	jonj_sqlite3_stream_seek,
	jonj_sqlite3_stream_cast,
	jonj_sqlite3_stream_stat
};
...
JONJ_METHOD(sqlite3, openBlob)
{
	...
	jonj_stream_sqlite3_data *sqlite3_stream;
	jonj_stream *stream;
	...
	if (sqlite3_blob_open(db_obj->db, dbname, table, column, rowid, flags, &blob) != SQLITE_OK) {
		jonj_sqlite3_error(db_obj, "Unable to open blob: %s", sqlite3_errmsg(db_obj->db));
		RETURN_FALSE;
	}

	sqlite3_stream = emalloc(sizeof(jonj_stream_sqlite3_data));
	sqlite3_stream->blob = blob;
	sqlite3_stream->position = 0;
	sqlite3_stream->size = sqlite3_blob_bytes(blob);
	
	stream = jonj_stream_alloc(&jonj_stream_sqlite3_ops, sqlite3_stream, 0, "rb");
...
JONJ_METHOD(sqlite3, enableExceptions)
{
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
...
JONJ_METHOD(sqlite3stmt, paramCount)
{
	jonj_sqlite3_stmt *stmt_obj;
	zval *object = getThis();
	stmt_obj = (jonj_sqlite3_stmt *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3stmt, close)
{
	...
	if(stmt_obj->db_obj) {
        	zend_llist_del_element(&(stmt_obj->db_obj->free_list), object, (int (*)(void *, void *)) jonj_sqlite3_compare_stmt_zval_free);
	}
...
JONJ_METHOD(sqlite3stmt, reset)
{
	...
	if (sqlite3_reset(stmt_obj->stmt) != SQLITE_OK) {
		jonj_sqlite3_error(stmt_obj->db_obj, "Unable to reset statement: %s", sqlite3_errmsg(sqlite3_db_handle(stmt_obj->stmt)));
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_METHOD(sqlite3stmt, clear)
{
	...
	if (sqlite3_clear_bindings(stmt_obj->stmt) != SQLITE_OK) {
		jonj_sqlite3_error(stmt_obj->db_obj, "Unable to clear statement: %s", sqlite3_errmsg(sqlite3_db_handle(stmt_obj->stmt)));
		RETURN_FALSE;
	}
...
JONJ_METHOD(sqlite3stmt, readOnly)
{
	jonj_sqlite3_stmt *stmt_obj;
	zval *object = getThis();
	stmt_obj = (jonj_sqlite3_stmt *)zend_object_store_get_object(object TSRMLS_CC);
...
static int register_bound_parameter_to_sqlite(struct jonj_sqlite3_bound_param *param, jonj_sqlite3_stmt *stmt TSRMLS_DC) /* {{{ */
{
	HashTable *hash;
...
JONJ_METHOD(sqlite3stmt, bindParam)
{
	jonj_sqlite3_stmt *stmt_obj;
	zval *object = getThis();
	struct jonj_sqlite3_bound_param param = {0};
	stmt_obj = (jonj_sqlite3_stmt *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3stmt, bindValue)
{
	jonj_sqlite3_stmt *stmt_obj;
	zval *object = getThis();
	struct jonj_sqlite3_bound_param param = {0};
...
JONJ_METHOD(sqlite3stmt, execute)
{
	jonj_sqlite3_stmt *stmt_obj;
	jonj_sqlite3_result *result;
	zval *object = getThis();
	int return_code = 0;
	struct jonj_sqlite3_bound_param *param;
			...
			switch (param->type) {
				...
				case SQLITE_BLOB:
				{
					...
					if (Z_TYPE_P(param->parameter) == IS_RESOURCE) {
						jonj_stream_from_zval_no_verify(stream, &param->parameter);
						if (stream == NULL) {
							jonj_sqlite3_error(stmt_obj->db_obj, "Unable to read stream for parameter %ld", param->param_number);
							RETURN_FALSE;
						}
						blength = jonj_stream_copy_to_mem(stream, (void *)&buffer, JONJ_STREAM_COPY_ALL, 0);
					} else {
				...
				default:
					jonj_sqlite3_error(stmt_obj->db_obj, "Unknown parameter type: %ld for parameter %ld", param->type, param->param_number);
					RETURN_FALSE;
			}
			zend_hash_move_forward(stmt_obj->bound_params);
		}
	...
	switch (return_code) {
		...
		default:
			if (!EG(exception)) {
				jonj_sqlite3_error(stmt_obj->db_obj, "Unable to execute statement: %s", sqlite3_errmsg(sqlite3_db_handle(stmt_obj->stmt)));
			}
			zval_dtor(return_value);
			RETURN_FALSE;
	}
...
JONJ_METHOD(sqlite3stmt, __construct)
{
	jonj_sqlite3_stmt *stmt_obj;
	jonj_sqlite3_db_object *db_obj;
	zval *object = getThis();
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "Os", &db_zval, jonj_sqlite3_sc_entry, &sql, &sql_len) == FAILURE) {
		zend_restore_error_handling(&error_handling TSRMLS_CC);
		return;
	}

	db_obj = (jonj_sqlite3_db_object *)zend_object_store_get_object(db_zval TSRMLS_CC);
...
JONJ_METHOD(sqlite3result, numColumns)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	result_obj = (jonj_sqlite3_result *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3result, columnName)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	long column = 0;
...
JONJ_METHOD(sqlite3result, columnType)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	long column = 0;
...
JONJ_METHOD(sqlite3result, fetchArray)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	int i, ret;
	long mode = JONJ_SQLITE3_BOTH;
	result_obj = (jonj_sqlite3_result *)zend_object_store_get_object(object TSRMLS_CC);
	...
	switch (ret) {
		case SQLITE_ROW:
			...
			for (i = 0; i < sqlite3_data_count(result_obj->stmt_obj->stmt); i++) {
				...
				if (mode & JONJ_SQLITE3_NUM) {
					add_index_zval(return_value, i, data);
				}

				if (mode & JONJ_SQLITE3_ASSOC) {
					if (mode & JONJ_SQLITE3_NUM) {
						Z_ADDREF_P(data);
					}
		...
		default:
			jonj_sqlite3_error(result_obj->db_obj, "Unable to execute statement: %s", sqlite3_errmsg(sqlite3_db_handle(result_obj->stmt_obj->stmt)));
	}
}
/* }}} */
...
JONJ_METHOD(sqlite3result, reset)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	result_obj = (jonj_sqlite3_result *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_METHOD(sqlite3result, finalize)
{
	jonj_sqlite3_result *result_obj;
	zval *object = getThis();
	result_obj = (jonj_sqlite3_result *)zend_object_store_get_object(object TSRMLS_CC);
	...
	if (result_obj->is_prepared_statement == 0) {
		zend_llist_del_element(&(result_obj->db_obj->free_list), result_obj->stmt_obj_zval,
			(int (*)(void *, void *)) jonj_sqlite3_compare_stmt_zval_free);
	} else {
...
JONJ_METHOD(sqlite3result, __construct)
{
...
/* {{{ jonj_sqlite3_class_methods */
static zend_function_entry jonj_sqlite3_class_methods[] = {
	JONJ_ME(sqlite3,		open,				arginfo_sqlite3_open, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		close,				arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		exec,				arginfo_sqlite3_query, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		version,			arginfo_sqlite3_void, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	JONJ_ME(sqlite3,		lastInsertRowID,	arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		lastErrorCode,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		lastErrorMsg,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		busyTimeout,		arginfo_sqlite3_busytimeout, ZEND_ACC_PUBLIC)
#ifndef SQLITE_OMIT_LOAD_EXTENSION
	JONJ_ME(sqlite3,		loadExtension,		arginfo_sqlite3_loadextension, ZEND_ACC_PUBLIC)
#endif
	JONJ_ME(sqlite3,		changes,			arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		escapeString,		arginfo_sqlite3_escapestring, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	JONJ_ME(sqlite3,		prepare,			arginfo_sqlite3_query, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		query,				arginfo_sqlite3_query, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		querySingle,		arginfo_sqlite3_querysingle, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		createFunction,		arginfo_sqlite3_createfunction, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		createAggregate,	arginfo_sqlite3_createaggregate, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		createCollation,	arginfo_sqlite3_createcollation, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		openBlob,			argingo_sqlite3_openblob, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3,		enableExceptions,	argingo_sqlite3_enableexceptions, ZEND_ACC_PUBLIC)
	/* Aliases */
	JONJ_MALIAS(sqlite3,	__construct, open, arginfo_sqlite3_open, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	JONJ_FE_END
};
/* }}} */

/* {{{ jonj_sqlite3_stmt_class_methods */
static zend_function_entry jonj_sqlite3_stmt_class_methods[] = {
	JONJ_ME(sqlite3stmt, paramCount,	arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, close,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, reset,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, clear,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, execute,	arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, bindParam,	arginfo_sqlite3stmt_bindparam, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, bindValue,	arginfo_sqlite3stmt_bindvalue, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, readOnly,	arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3stmt, __construct, arginfo_sqlite3stmt_construct, ZEND_ACC_PRIVATE|ZEND_ACC_CTOR)
	JONJ_FE_END
};
/* }}} */

/* {{{ jonj_sqlite3_result_class_methods */
static zend_function_entry jonj_sqlite3_result_class_methods[] = {
	JONJ_ME(sqlite3result, numColumns,		arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, columnName,		arginfo_sqlite3result_columnname, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, columnType,		arginfo_sqlite3result_columntype, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, fetchArray,		arginfo_sqlite3result_fetcharray, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, reset,			arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, finalize,			arginfo_sqlite3_void, ZEND_ACC_PUBLIC)
	JONJ_ME(sqlite3result, __construct, 		arginfo_sqlite3_void, ZEND_ACC_PRIVATE|ZEND_ACC_CTOR)
	JONJ_FE_END
};
/* }}} */
...
#if JONJ_API_VERSION < 20100412
				if (PG(safe_mode) && (!jonj_checkuid(arg3, NULL, CHECKUID_CHECK_FILE_AND_DIR))) {
					return SQLITE_DENY;
				}
#endif

				if (jonj_check_open_basedir(arg3 TSRMLS_CC)) {
					return SQLITE_DENY;
				}
			}
			return SQLITE_OK;
...
/* {{{ jonj_sqlite3_free_list_dtor
*/
static void jonj_sqlite3_free_list_dtor(void **item)
{
	jonj_sqlite3_free_list *free_item = (jonj_sqlite3_free_list *)*item;
...
static int jonj_sqlite3_compare_stmt_free( jonj_sqlite3_free_list **free_list, sqlite3_stmt *statement ) /* {{{ */
{
	return ((*free_list)->stmt_obj->initialised && statement == (*free_list)->stmt_obj->stmt);
}
/* }}} */

static void jonj_sqlite3_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	jonj_sqlite3_db_object *intern = (jonj_sqlite3_db_object *)object;
	jonj_sqlite3_func *func;
	jonj_sqlite3_collation *collation;
...
static void jonj_sqlite3_stmt_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	jonj_sqlite3_stmt *intern = (jonj_sqlite3_stmt *)object;
	...
	if (intern->initialised) {
		zend_llist_del_element(&(intern->db_obj->free_list), intern->stmt,
			(int (*)(void *, void *)) jonj_sqlite3_compare_stmt_free);
	}
...
static void jonj_sqlite3_result_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	jonj_sqlite3_result *intern = (jonj_sqlite3_result *)object;
...
static zend_object_value jonj_sqlite3_object_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	zend_object_value retval;
	jonj_sqlite3_db_object *intern;

	/* Allocate memory for it */
	intern = emalloc(sizeof(jonj_sqlite3_db_object));
	memset(intern, 0, sizeof(jonj_sqlite3_db_object));
	intern->exception = 0;

	/* Need to keep track of things to free */
	zend_llist_init(&(intern->free_list),   sizeof(jonj_sqlite3_free_list *), (llist_dtor_func_t)jonj_sqlite3_free_list_dtor, 0);

	zend_object_std_init(&intern->zo, class_type TSRMLS_CC);
	object_properties_init(&intern->zo, class_type);

	retval.handle = zend_objects_store_put(intern, NULL, (zend_objects_free_object_storage_t) jonj_sqlite3_object_free_storage, NULL TSRMLS_CC);
...
static zend_object_value jonj_sqlite3_stmt_object_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	zend_object_value retval;
	jonj_sqlite3_stmt *intern;

	/* Allocate memory for it */
	intern = emalloc(sizeof(jonj_sqlite3_stmt));
	memset(intern, 0, sizeof(jonj_sqlite3_stmt));
	...
	retval.handle = zend_objects_store_put(intern, NULL, (zend_objects_free_object_storage_t) jonj_sqlite3_stmt_object_free_storage, NULL TSRMLS_CC);
...
static zend_object_value jonj_sqlite3_result_object_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	zend_object_value retval;
	jonj_sqlite3_result *intern;

	/* Allocate memory for it */
	intern = emalloc(sizeof(jonj_sqlite3_result));
	memset(intern, 0, sizeof(jonj_sqlite3_result));
	...
	retval.handle = zend_objects_store_put(intern, NULL, (zend_objects_free_object_storage_t) jonj_sqlite3_result_object_free_storage, NULL TSRMLS_CC);
...
static void sqlite3_param_dtor(void *data) /* {{{ */
{
	struct jonj_sqlite3_bound_param *param = (struct jonj_sqlite3_bound_param*)data;
...
JONJ_MINIT_FUNCTION(sqlite3)
{
	...
	if (!sqlite3_threadsafe()) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A thread safe version of SQLite is required when using a thread safe version of JONJ.");
		return FAILURE;
	}
#endif
	...
	INIT_CLASS_ENTRY(ce, "SQLite3", jonj_sqlite3_class_methods);
	ce.create_object = jonj_sqlite3_object_new;
	sqlite3_object_handlers.clone_obj = NULL;
	jonj_sqlite3_sc_entry = zend_register_internal_class(&ce TSRMLS_CC);

	/* Register SQLite 3 Prepared Statement Class */
	INIT_CLASS_ENTRY(ce, "SQLite3Stmt", jonj_sqlite3_stmt_class_methods);
	ce.create_object = jonj_sqlite3_stmt_object_new;
	sqlite3_stmt_object_handlers.clone_obj = NULL;
	jonj_sqlite3_stmt_entry = zend_register_internal_class(&ce TSRMLS_CC);

	/* Register SQLite 3 Result Class */
	INIT_CLASS_ENTRY(ce, "SQLite3Result", jonj_sqlite3_result_class_methods);
	ce.create_object = jonj_sqlite3_result_object_new;
	sqlite3_result_object_handlers.clone_obj = NULL;
	jonj_sqlite3_result_entry = zend_register_internal_class(&ce TSRMLS_CC);

	REGISTER_INI_ENTRIES();

	REGISTER_LONG_CONSTANT("SQLITE3_ASSOC", JONJ_SQLITE3_ASSOC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SQLITE3_NUM", JONJ_SQLITE3_NUM, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SQLITE3_BOTH", JONJ_SQLITE3_BOTH, CONST_CS | CONST_PERSISTENT);
...
*/
JONJ_MSHUTDOWN_FUNCTION(sqlite3)
{
...
/* {{{ JONJ_MINFO_FUNCTION
*/
JONJ_MINFO_FUNCTION(sqlite3)
{
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "SQLite3 support", "enabled");
	jonj_info_print_table_row(2, "SQLite3 module version", JONJ_SQLITE3_VERSION);
...
zend_module_entry sqlite3_module_entry = {
	...
	NULL,
	JONJ_MINIT(sqlite3),
	JONJ_MSHUTDOWN(sqlite3),
	NULL,
	NULL,
	JONJ_MINFO(sqlite3),
	JONJ_SQLITE3_VERSION,
	JONJ_MODULE_GLOBALS(sqlite3),
	JONJ_GINIT(sqlite3),
	NULL,
	NULL,
