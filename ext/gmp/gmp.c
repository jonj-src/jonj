/*
   ...
   +----------------------------------------------------------------------+
   | Author: Stanislav Malyshev <stas@jonj.tk>                            |
   +----------------------------------------------------------------------+
 */
...
static void shift_operator_helper(gmp_binary_ui_op_t op, zval *return_value, zval *op1, zval *op2 TSRMLS_DC) {
	...
	if (Z_LVAL_P(op2) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Shift cannot be negative");
		RETVAL_FALSE;
	} else {
...
static int gmp_serialize(zval *object, unsigned char **buffer, zend_uint *buf_len, zend_serialize_data *data TSRMLS_DC) /* {{{ */
{
	...
	smart_str buf = {0};
	zval zv, *zv_ptr = &zv;
	jonj_serialize_data_t serialize_data = (jonj_serialize_data_t) data;

	JONJ_VAR_SERIALIZE_INIT(serialize_data);
	INIT_PZVAL(zv_ptr);

	gmp_strval(zv_ptr, gmpnum, 10);
	jonj_var_serialize(&buf, &zv_ptr, &serialize_data TSRMLS_CC);
	zval_dtor(zv_ptr);
	...
	JONJ_VAR_SERIALIZE_DESTROY(serialize_data);
...
static int gmp_unserialize(zval **object, zend_class_entry *ce, const unsigned char *buf, zend_uint buf_len, zend_unserialize_data *data TSRMLS_DC) /* {{{ */
{
	...
	zval *zv_ptr;
	int retval = FAILURE;
	jonj_unserialize_data_t unserialize_data = (jonj_unserialize_data_t) data;

	JONJ_VAR_UNSERIALIZE_INIT(unserialize_data);
	...
	if (!jonj_var_unserialize(&zv_ptr, &p, max, &unserialize_data TSRMLS_CC)
...
exit:
	var_push_dtor_no_addref(&unserialize_data, &zv_ptr);
	JONJ_VAR_UNSERIALIZE_DESTROY(unserialize_data);
	return retval;
}
/* }}} */
...
ZEND_MODULE_INFO_D(gmp)
{
	...
	jonj_info_print_table_row(2, "gmp support", "enabled");
#ifdef mpir_version
	jonj_info_print_table_row(2, "MPIR version", mpir_version);
#else
	jonj_info_print_table_row(2, "GMP version", gmp_version);
#endif
...
static inline void gmp_zval_binary_ui_op(zval *return_value, zval *a_arg, zval *b_arg, gmp_binary_op_t gmp_op, gmp_binary_ui_op_t gmp_ui_op, int check_b_zero TSRMLS_DC)
{
	...
	if (check_b_zero) {
		...
		if (b_is_zero) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Zero operand not allowed");
			FREE_GMP_TEMP(temp_a);
			FREE_GMP_TEMP(temp_b);
			RETURN_FALSE;
		}
...
ZEND_FUNCTION(gmp_init)
{
	...
	if (base && (base < 2 || base > MAX_BASE)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad base for conversion: %ld (should be between 2 and %d)", base, MAX_BASE);
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(gmp_strval)
{
	...
	if ((base < 2 && base > -2) || base > MAX_BASE || base < -36) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad base for conversion: %ld (should be between 2 and %d or -2 and -36)", base, MAX_BASE);
#else
...
ZEND_FUNCTION(gmp_div_qr)
{
	...
	case GMP_ROUND_MINUSINF:
		...
		break;
	default:
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid rounding mode");
		RETURN_FALSE;
	}
}
/* }}} */
...
ZEND_FUNCTION(gmp_fact)
{
	...
	if (IS_GMP(a_arg)) {
		...
		if (mpz_sgn(gmpnum_tmp) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number has to be greater than or equal to 0");
			RETURN_FALSE;
		}
	} else {
...
ZEND_FUNCTION(gmp_pow)
{
	...
	if (exp < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Negative exponent not supported");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(gmp_powm)
{
	...
	if (Z_TYPE_P(exp_arg) == IS_LONG && Z_LVAL_P(exp_arg) >= 0) {
		...
		if (mpz_sgn(gmpnum_exp) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Second parameter cannot be less than 0");
			FREE_GMP_TEMP(temp_base);
	...
	if (!mpz_cmp_ui(gmpnum_mod, 0)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Modulus may not be zero");
		FREE_GMP_TEMP(temp_base);
		FREE_GMP_TEMP(temp_exp);
...
ZEND_FUNCTION(gmp_root)
{
	...
	if (nth <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The root must be positive");
		RETURN_FALSE;
	}
	...
	if (nth % 2 == 0 && mpz_sgn(gmpnum_a) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't take even root of negative number");
		FREE_GMP_TEMP(temp_a);
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(gmp_random_bits)
{
	...
	if (bits <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The number of bits must be positive");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(gmp_random_range)
{
	...
	if (Z_TYPE_P(min_arg) == IS_LONG && Z_LVAL_P(min_arg) >= 0) {
		if (mpz_cmp_ui(gmpnum_max, Z_LVAL_P(min_arg)) <= 0) {
			FREE_GMP_TEMP(temp_a);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The minimum value must be less than the maximum value");
			RETURN_FALSE;
		}
...
ZEND_FUNCTION(gmp_setbit)
{
	...
	if (index < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Index must be greater than or equal to zero");
		return;
	}
...
ZEND_FUNCTION(gmp_scan0)
{
	...
	if (start < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Starting index must be greater than or equal to zero");
		RETURN_FALSE;
	}
