...
#define JONJ_ODBC_TYPE ODBC_TYPE
...
const zend_function_entry odbc_functions[] = {
	JONJ_FE(odbc_autocommit, arginfo_odbc_autocommit)
	JONJ_FE(odbc_binmode, arginfo_odbc_binmode)
	JONJ_FE(odbc_close, arginfo_odbc_close)
	JONJ_FE(odbc_close_all, arginfo_odbc_close_all)
	JONJ_FE(odbc_columns, arginfo_odbc_columns)
	JONJ_FE(odbc_commit, arginfo_odbc_commit)
	JONJ_FE(odbc_connect, arginfo_odbc_connect)
	JONJ_FE(odbc_cursor, arginfo_odbc_cursor)
#ifdef HAVE_SQLDATASOURCES
	JONJ_FE(odbc_data_source, arginfo_odbc_data_source)
#endif
	JONJ_FE(odbc_execute, arginfo_odbc_execute)
	JONJ_FE(odbc_error, arginfo_odbc_error)
	JONJ_FE(odbc_errormsg, arginfo_odbc_errormsg)
	JONJ_FE(odbc_exec, arginfo_odbc_exec)
#ifdef JONJ_ODBC_HAVE_FETCH_HASH
	JONJ_FE(odbc_fetch_array, arginfo_odbc_fetch_array)
	JONJ_FE(odbc_fetch_object, arginfo_odbc_fetch_object)
#endif
	JONJ_FE(odbc_fetch_row, arginfo_odbc_fetch_row)
	JONJ_FE(odbc_fetch_into, arginfo_odbc_fetch_into)
	JONJ_FE(odbc_field_len, arginfo_odbc_field_len)
	JONJ_FE(odbc_field_scale, arginfo_odbc_field_scale)
	JONJ_FE(odbc_field_name, arginfo_odbc_field_name)
	JONJ_FE(odbc_field_type, arginfo_odbc_field_type)
	JONJ_FE(odbc_field_num, arginfo_odbc_field_num)
	JONJ_FE(odbc_free_result, arginfo_odbc_free_result)
	JONJ_FE(odbc_gettypeinfo, arginfo_odbc_gettypeinfo)
	JONJ_FE(odbc_longreadlen, arginfo_odbc_longreadlen)
#if !defined(HAVE_SOLID) && !defined(HAVE_SOLID_30)
	JONJ_FE(odbc_next_result, arginfo_odbc_next_result)
#endif
	JONJ_FE(odbc_num_fields, arginfo_odbc_num_fields)
	JONJ_FE(odbc_num_rows, arginfo_odbc_num_rows)
	JONJ_FE(odbc_pconnect, arginfo_odbc_pconnect)
	JONJ_FE(odbc_prepare, arginfo_odbc_prepare)
	JONJ_FE(odbc_result, arginfo_odbc_result)
	JONJ_FE(odbc_result_all, arginfo_odbc_result_all)
	JONJ_FE(odbc_rollback, arginfo_odbc_rollback)
	JONJ_FE(odbc_setoption, arginfo_odbc_setoption)
	JONJ_FE(odbc_specialcolumns, arginfo_odbc_specialcolumns)
	JONJ_FE(odbc_statistics, arginfo_odbc_statistics)
	JONJ_FE(odbc_tables, arginfo_odbc_tables)
	JONJ_FE(odbc_primarykeys, arginfo_odbc_primarykeys)
	...
	JONJ_FE(odbc_columnprivileges, arginfo_odbc_columnprivileges)
	JONJ_FE(odbc_tableprivileges, arginfo_odbc_tableprivileges)
#endif
#if !defined(HAVE_SOLID) && !defined(HAVE_SOLID_30) && !defined(HAVE_SOLID_35) /* not supported */
	JONJ_FE(odbc_foreignkeys, arginfo_odbc_foreignkeys)
	JONJ_FE(odbc_procedures, arginfo_odbc_procedures)
#if !defined(HAVE_BIRDSTEP)
	JONJ_FE(odbc_procedurecolumns, arginfo_odbc_procedurecolumns)
#endif
#endif
	JONJ_FALIAS(odbc_do, odbc_exec, arginfo_odbc_exec)
	JONJ_FALIAS(odbc_field_precision, odbc_field_len, arginfo_odbc_field_len)
	JONJ_FE_END
};
/* }}} */

ZEND_DECLARE_MODULE_GLOBALS(odbc)
static JONJ_GINIT_FUNCTION(odbc);
...
static void _free_odbc_result(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	...
	if (res) {
		...
		if (res->stmt && !(PG(connection_status) & JONJ_CONNECTION_TIMEOUT)) {
#if defined(HAVE_SOLID) || defined(HAVE_SOLID_30) || defined(HAVE_SOLID_35)
...
static void _close_odbc_conn(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	...
	/* If aborted via timer expiration, don't try to call any unixODBC function */
	if (!(PG(connection_status) & JONJ_CONNECTION_TIMEOUT)) {
...
/* {{{ JONJ_INI_DISP(display_link_nums)
 */
static JONJ_INI_DISP(display_link_nums)
{
	char *value;
	TSRMLS_FETCH();
...
/* {{{ JONJ_INI_DISP(display_defPW)
 */
static JONJ_INI_DISP(display_defPW)
{
	char *value;
	TSRMLS_FETCH();
...
/* {{{ JONJ_INI_DISP(display_binmode)
 */
static JONJ_INI_DISP(display_binmode)
{
	char *value;
	TSRMLS_FETCH();
...
/* {{{ JONJ_INI_DISP(display_lrl)
 */
static JONJ_INI_DISP(display_lrl)
{
	char *value;
	TSRMLS_FETCH();
	...
	if (value) {
		if (atoi(value) <= 0) {
			...
			jonj_printf("return up to %s bytes", value);
		}
	}
}
/* }}} */
...
/* {{{ JONJ_INI_DISP(display_cursortype)
 */
static JONJ_INI_DISP(display_cursortype)
{
	char *value;
	TSRMLS_FETCH();
		...
		switch (atoi (value))
		  {
			...
			default:
				jonj_printf("Unknown cursor model %s", value);
				break;
		  }
	}
}
 ...
 */
JONJ_INI_BEGIN()
	STD_JONJ_INI_BOOLEAN("odbc.allow_persistent", "1", JONJ_INI_SYSTEM, OnUpdateLong,
			allow_persistent, zend_odbc_globals, odbc_globals)
	STD_JONJ_INI_ENTRY_EX("odbc.max_persistent",  "-1", JONJ_INI_SYSTEM, OnUpdateLong,
			max_persistent, zend_odbc_globals, odbc_globals, display_link_nums)
	STD_JONJ_INI_ENTRY_EX("odbc.max_links", "-1", JONJ_INI_SYSTEM, OnUpdateLong,
			max_links, zend_odbc_globals, odbc_globals, display_link_nums)
	STD_JONJ_INI_ENTRY("odbc.default_db", NULL, JONJ_INI_ALL, OnUpdateString,
			defDB, zend_odbc_globals, odbc_globals)
	STD_JONJ_INI_ENTRY("odbc.default_user", NULL, JONJ_INI_ALL, OnUpdateString,
			defUser, zend_odbc_globals, odbc_globals)
	STD_JONJ_INI_ENTRY_EX("odbc.default_pw", NULL, JONJ_INI_ALL, OnUpdateString,
			defPW, zend_odbc_globals, odbc_globals, display_defPW)
	STD_JONJ_INI_ENTRY_EX("odbc.defaultlrl", "4096", JONJ_INI_ALL, OnUpdateLong,
			defaultlrl, zend_odbc_globals, odbc_globals, display_lrl)
	STD_JONJ_INI_ENTRY_EX("odbc.defaultbinmode", "1", JONJ_INI_ALL, OnUpdateLong,
			defaultbinmode, zend_odbc_globals, odbc_globals, display_binmode)
	STD_JONJ_INI_BOOLEAN("odbc.check_persistent", "1", JONJ_INI_SYSTEM, OnUpdateLong,
			check_persistent, zend_odbc_globals, odbc_globals)
	STD_JONJ_INI_ENTRY_EX("odbc.default_cursortype", "3", JONJ_INI_ALL, OnUpdateLong, 
...
JONJ_MINIT_FUNCTION(odbc)
{
	...
	REGISTER_STRING_CONSTANT("ODBC_TYPE", JONJ_ODBC_TYPE, CONST_CS | CONST_PERSISTENT);
	...
	*/
	...
	/* atexit() handler in the DB2/AIX library segfaults in JONJ CLI */
...
JONJ_MINFO_FUNCTION(odbc)
{
	...
	jonj_info_print_table_header(2, "ODBC Support", "enabled");
	...
	jonj_info_print_table_row(2, "ODBC library", JONJ_ODBC_TYPE);
#ifndef JONJ_WIN32
	jonj_info_print_table_row(2, "ODBC_INCLUDE", JONJ_ODBC_INCLUDE);
	jonj_info_print_table_row(2, "ODBC_LFLAGS", JONJ_ODBC_LFLAGS);
	jonj_info_print_table_row(2, "ODBC_LIBS", JONJ_ODBC_LIBS);
#endif
...
void odbc_sql_error(ODBC_SQL_ERROR_PARAMS)
{
	...
	if (func) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SQL error: %s, SQL state %s in %s", errormsg, state, func);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SQL error: %s, SQL state %s", errormsg, state);
	}
	/*		
...
/* {{{ jonj_odbc_fetch_attribs */
void jonj_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	odbc_result *result;
	zval *pv_res;
	long flag;
...
int odbc_bindcols(odbc_result *result TSRMLS_DC)
{
	...
	for(i = 0; i < result->numcols; i++) {
		...
		rc = JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), JONJ_ODBC_SQL_DESC_NAME,
				result->values[i].name, sizeof(result->values[i].name), &colnamelen, 0);
		rc = JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), SQL_COLUMN_TYPE, 
...
#endif
			default:
				rc = JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), colfieldid,
...
void odbc_column_lengths(INTERNAL_FUNCTION_PARAMETERS, int type)
{
	...
	if (result->numcols == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No tuples available at this result index");
		RETURN_FALSE;
	}

	if (pv_num > result->numcols) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Field index larger than number of fields");
		RETURN_FALSE;
	}

	if (pv_num < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Field numbering starts at 1");
		RETURN_FALSE;
	}

	JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)pv_num, (SQLUSMALLINT) (type?SQL_COLUMN_SCALE:SQL_COLUMN_PRECISION), NULL, 0, NULL, &len);
...
{
	jonj_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(odbc_prepare)
{
	...
	rc = JONJ_ODBC_SQLALLOCSTMT(conn->hdbc, &(result->stmt));
	if (rc == SQL_INVALID_HANDLE) {
		efree(result);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SQLAllocStmt error 'Invalid Handle'");
		RETURN_FALSE;
	}
...
{ 
	...
	if (result->numparams > 0 && numArgs == 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No parameters to SQL statement given");
		RETURN_FALSE;
	}

	if (result->numparams > 0) {
		if ((ne = zend_hash_num_elements(Z_ARRVAL_P(pv_param_arr))) < result->numparams) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING,"Not enough parameters (%d should be %d) given", ne, result->numparams);
			RETURN_FALSE;
		}
		...
		for(i = 1; i <= result->numparams; i++) {
			...
			if (Z_STRLEN_PP(tmp) > 2 &&
				...
				if ((params[i-1].fp = open(filename,O_RDONLY)) == -1) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING,"Can't open file %s", filename);
					SQLFreeStmt(result->stmt, SQL_RESET_PARAMS);
					for (i = 0; i < result->numparams; i++) {
						if (params[i].fp != -1) {
							close(params[i].fp);
	...
	if (max_len > 0) {
		...
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
			...
			if (!strncmp(state,"S1015",5)) {
				snprintf(cursorname, max_len+1, "jonj_curs_%d", (int)result->stmt);
...
JONJ_FUNCTION(odbc_data_source)
{
	...
	if (!(fetch_type == SQL_FETCH_FIRST || fetch_type == SQL_FETCH_NEXT)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid fetch type (%d)", fetch_type);
		RETURN_FALSE;
	}
...
/* {{{ jonj_odbc_fetch_hash */
static void jonj_odbc_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, int result_type)
{
	int i;
	odbc_result *result;
	RETCODE rc;
...
{
	jonj_odbc_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, ODBC_OBJECT);
...
/* {{{ proto bool solid_fetch_prev(resource result_id)
   */ 
#if defined(HAVE_SOLID) || defined(HAVE_SOLID_30) || defined(HAVE_SOLID_35)
JONJ_FUNCTION(solid_fetch_prev)
{
	odbc_result *result;
	RETCODE rc;
	zval *pv_res;
...
JONJ_FUNCTION(odbc_result)
{
	...
	if (field != NULL) {
		if (result->values == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Result set contains no data");
			RETURN_FALSE;
		}
		...
		if (field_ind >= result->numcols || field_ind < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Field index is larger than the number of fields");
			RETURN_FALSE;
		}
	}
	...
	switch(result->values[field_ind].coltype) {
		...
		case SQL_WLONGVARCHAR:
			...
			if (IS_SQL_LONG(result->values[field_ind].coltype)) {
			   ...
			   JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(field_ind + 1), 
...
/* If we come here, output unbound LONG and/or BINARY column data to the client */
	...
	while(1) {
		...
		JONJWRITE(field,(rc == SQL_SUCCESS_WITH_INFO) ? 4095 : result->values[field_ind].vallen);
...
JONJ_FUNCTION(odbc_result_all)
{
	...
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
		jonj_printf("<h2>No rows found</h2>\n");
		RETURN_LONG(0);
	}
	
	/* Start table tag */
	if (ZEND_NUM_ARGS() == 1) {
		jonj_printf("<table><tr>");
	} else {
		jonj_printf("<table %s ><tr>", pv_format);
	}
	
	for (i = 0; i < result->numcols; i++) {
		jonj_printf("<th>%s</th>", result->values[i].name);
	}
	...
	while(rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
		...
		for(i = 0; i < result->numcols; i++) {
			...
			switch(result->values[i].coltype) {
				...
				case SQL_LONGVARBINARY:
					if (result->binmode <= 0) {
						jonj_printf("<td>Not printable</td>");
						break;
					}
					if (result->binmode <= 1) sql_c_type = SQL_C_BINARY; 
				case SQL_LONGVARCHAR:
				...
				case SQL_WLONGVARCHAR:
					...
					if (rc == SQL_ERROR) {
						odbc_sql_error(result->conn_ptr, result->stmt, "SQLGetData");
						jonj_printf("</td></tr></table>");
						efree(buf);
						RETURN_FALSE;
					}
					if (rc == SQL_SUCCESS_WITH_INFO) {
						JONJWRITE(buf, result->longreadlen);
					} else if (result->values[i].vallen == SQL_NULL_DATA) {
						jonj_printf("<td>NULL</td>");
						break;
					} else {
						JONJWRITE(buf, result->values[i].vallen);
					}
					jonj_printf("</td>");
					break;
				default:
					if (result->values[i].vallen == SQL_NULL_DATA) {
						...
						jonj_printf("<td>%s</td>", result->values[i].value);
					}
					break;
			}
		}
...
#if defined(HAVE_EMPRESS) || defined(HAVE_UNIXODBC) || defined(JONJ_WIN32) || defined (HAVE_IODBC)
...
void odbc_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	if (ZEND_NUM_ARGS() > 3) {
		...
		if (! (cur_opt == SQL_CUR_USE_IF_NEEDED || 
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid Cursor type (%d)", cur_opt);
			RETURN_FALSE;
		}
	}
	...
	if (persistent) {
		...
		if (zend_hash_find(&EG(persistent_list), hashed_details, hashed_len + 1, (void **) &le)	== FAILURE) {
			...
			if (ODBCG(max_links) != -1 && ODBCG(num_links) >= ODBCG(max_links)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open links (%ld)", ODBCG(num_links));
				efree(hashed_details);
				RETURN_FALSE;
			}
			if (ODBCG(max_persistent) != -1 && ODBCG(num_persistent) >= ODBCG(max_persistent)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING,"Too many open persistent links (%ld)", ODBCG(num_persistent));
				efree(hashed_details);
				RETURN_FALSE;
			}
		...
		if (ODBCG(max_links) != -1 && ODBCG(num_links) >= ODBCG(max_links)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING,"Too many open connections (%ld)",ODBCG(num_links));
			efree(hashed_details);
			RETURN_FALSE;
		}
...
JONJ_FUNCTION(odbc_field_type)
{
	...
	JONJ_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)pv_num, SQL_COLUMN_TYPE_NAME, tmp, 31, &tmplen, NULL);
	RETURN_STRING(tmp,1)
}
/* }}} */
...
/* {{{ jonj_odbc_lasterror */
static void jonj_odbc_lasterror(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	odbc_connection *conn;
	zval *pv_handle;
	char *ptr;
...
{
	jonj_odbc_lasterror(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_odbc_lasterror(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(odbc_setoption)
{
	...
	switch (pv_which) {
		case 1:		/* SQLSetConnectOption */
			...
			if (conn->persistent) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set option for persistent connection");
				RETURN_FALSE;
			}
		...
		case 2:		/* SQLSetStmtOption */
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown option type");
			RETURN_FALSE;
			break;
	}
