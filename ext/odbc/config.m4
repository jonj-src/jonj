...
AC_DEFUN([JONJ_ODBC_CHECK_HEADER],[
...
dnl
AC_DEFUN([JONJ_ODBC_FIND_SOLID_LIBS],[
...
AC_DEFUN([JONJ_ODBC_FIND_EMPRESS_LIBS],[
...
AC_DEFUN([JONJ_ODBC_FIND_EMPRESS_BCS_LIBS],[
...
JONJ_ARG_WITH(adabas,,
[  --with-adabas[=DIR]       Include Adabas D support [/usr/local]])

  AC_MSG_CHECKING([for Adabas support])
  if test "$JONJ_ADABAS" != "no"; then
    if test "$JONJ_ADABAS" = "yes"; then
      JONJ_ADABAS=/usr/local
    fi
    JONJ_ADD_INCLUDE($JONJ_ADABAS/incl)
    JONJ_ADD_LIBPATH($JONJ_ADABAS/$JONJ_LIBDIR)
    ODBC_OBJS="$JONJ_ADABAS/$JONJ_LIBDIR/odbclib.a"
    ODBC_LIB="$abs_builddir/ext/odbc/libodbc_adabas.a"
    $srcdir/build/shtool mkdir -f -p ext/odbc
    rm -f "$ODBC_LIB"
    cp "$ODBC_OBJS" "$ODBC_LIB"
    JONJ_ADD_LIBRARY(sqlptc)
    JONJ_ADD_LIBRARY(sqlrte)
    JONJ_ADD_LIBRARY_WITH_PATH(odbc_adabas, $abs_builddir/ext/odbc)
    ODBC_TYPE=adabas
    ODBC_INCDIR=$JONJ_ADABAS/incl
    JONJ_ODBC_CHECK_HEADER(sqlext.h)
...
[  --with-sapdb[=DIR]        Include SAP DB support [/usr/local]])
  ...
  if test "$JONJ_SAPDB" != "no"; then
    if test "$JONJ_SAPDB" = "yes"; then
      JONJ_SAPDB=/usr/local
    fi
    JONJ_ADD_INCLUDE($JONJ_SAPDB/incl)
    JONJ_ADD_LIBPATH($JONJ_SAPDB/$JONJ_LIBDIR)
    JONJ_ADD_LIBRARY(sqlod)
    ODBC_TYPE=sapdb
...
[  --with-solid[=DIR]        Include Solid support [/usr/local/solid]])
  ...
  if test "$JONJ_SOLID" != "no"; then
    if test "$JONJ_SOLID" = "yes"; then
      JONJ_SOLID=/usr/local/solid
    fi
    ODBC_INCDIR=$JONJ_SOLID/include
    ODBC_LIBDIR=$JONJ_SOLID/$JONJ_LIBDIR
    ...
    AC_MSG_RESULT([$ext_output])
    JONJ_ODBC_FIND_SOLID_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(ibm-db2,,
[  --with-ibm-db2[=DIR]      Include IBM DB2 support [/home/db2inst1/sqllib]])

  AC_MSG_CHECKING(for IBM DB2 support)
  if test "$JONJ_IBM_DB2" != "no"; then
    if test "$JONJ_IBM_DB2" = "yes"; then
      ODBC_INCDIR=/home/db2inst1/sqllib/include
      ODBC_LIBDIR=/home/db2inst1/sqllib/lib
    else
      ODBC_INCDIR=$JONJ_IBM_DB2/include
      ODBC_LIBDIR=$JONJ_IBM_DB2/$JONJ_LIBDIR
    fi

    JONJ_ODBC_CHECK_HEADER(sqlcli1.h)
...
You need to source your DB2 environment before running JONJ configure:
# . \$IBM_DB2/db2profile
])
    ], [
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(ODBCRouter,,
[  --with-ODBCRouter[=DIR]   Include ODBCRouter.com support [/usr]])

  AC_MSG_CHECKING(for ODBCRouter.com support)
  if test "$JONJ_ODBCROUTER" != "no"; then
    if test "$JONJ_ODBCROUTER" = "yes"; then
      JONJ_ODBCROUTER=/usr
    fi
    ODBC_INCDIR=$JONJ_ODBCROUTER/include
    ODBC_LIBDIR=$JONJ_ODBCROUTER/lib
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
    ODBC_LIBS=-lodbcsdk
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(empress,,
[  --with-empress[=DIR]      Include Empress support [\$EMPRESSPATH]
                          (Empress Version >= 8.60 required)])

  AC_MSG_CHECKING(for Empress support)
  if test "$JONJ_EMPRESS" != "no"; then
    if test "$JONJ_EMPRESS" = "yes"; then
      ODBC_INCDIR=$EMPRESSPATH/include/odbc
      ODBC_LIBDIR=$EMPRESSPATH/shlib
    else
      ODBC_INCDIR=$JONJ_EMPRESS/include/odbc
      ODBC_LIBDIR=$JONJ_EMPRESS/shlib
    fi
    ODBC_INCLUDE=-I$ODBC_INCDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_TYPE=empress
    ...
    AC_MSG_RESULT([$ext_output])
    JONJ_ODBC_FIND_EMPRESS_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(empress-bcs,,
[  --with-empress-bcs[=DIR]
                          Include Empress Local Access support [\$EMPRESSPATH]
                          (Empress Version >= 8.60 required)])

  AC_MSG_CHECKING(for Empress local access support)
  if test "$JONJ_EMPRESS_BCS" != "no"; then
    if test "$JONJ_EMPRESS_BCS" = "yes"; then
      ODBC_INCDIR=$EMPRESSPATH/include/odbc
      ODBC_LIBDIR=$EMPRESSPATH/shlib
    else
      ODBC_INCDIR=$JONJ_EMPRESS_BCS/include/odbc
      ODBC_LIBDIR=$JONJ_EMPRESS_BCS/shlib
    fi
    ...
    ODBC_LIBS="-lempjonjbcs -lms -lmscfg -lbasic -lbasic_os -lnlscstab -lnlsmsgtab -lm -ldl -lcrypt"
    ODBC_TYPE=empress-bcs
    AC_DEFINE(HAVE_EMPRESS,1,[ ])
    AC_MSG_RESULT([$ext_output])
    JONJ_ODBC_FIND_EMPRESS_BCS_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(birdstep,,
[  --with-birdstep[=DIR]     Include Birdstep support [/usr/local/birdstep]])
  
  AC_MSG_CHECKING(for Birdstep support)
  if test "$JONJ_BIRDSTEP" != "no"; then
    if test "$JONJ_BIRDSTEP" = "yes"; then
        ODBC_INCDIR=/usr/local/birdstep/include
        ODBC_LIBDIR=/usr/local/birdstep/lib
    else
        ODBC_INCDIR=$JONJ_BIRDSTEP/include
        ODBC_LIBDIR=$JONJ_BIRDSTEP/$JONJ_LIBDIR
    fi
...
[  --with-custom-odbc[=DIR]  Include user defined ODBC support. DIR is ODBC install base
  ...
  if test "$JONJ_CUSTOM_ODBC" != "no"; then
    if test "$JONJ_CUSTOM_ODBC" = "yes"; then
      JONJ_CUSTOM_ODBC=/usr/local
    fi
    ODBC_INCDIR=$JONJ_CUSTOM_ODBC/include
    ODBC_LIBDIR=$JONJ_CUSTOM_ODBC/$JONJ_LIBDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
...
[  --with-iodbc[=DIR]        Include iODBC support [/usr/local]])
  ...
  if test "$JONJ_IODBC" != "no"; then
    if test -z "$PKG_CONFIG"; then
      AC_PATH_PROG(PKG_CONFIG, pkg-config, no)
    fi 
    if test -x "$PKG_CONFIG" && $PKG_CONFIG --exists libiodbc ; then
      JONJ_ADD_LIBRARY_WITH_PATH(iodbc, $JONJ_IODBC/$JONJ_LIBDIR)
      ODBC_TYPE=iodbc
      ...
      JONJ_EVAL_INCLINE($ODBC_INCLUDE)
      AC_DEFINE(HAVE_IODBC,1,[ ])
      AC_DEFINE(HAVE_ODBC2,1,[ ])
      AC_MSG_RESULT([$ext_output])
    else
      if test "$JONJ_IODBC" = "yes"; then
        JONJ_IODBC=/usr/local
      fi
      JONJ_ADD_LIBRARY_WITH_PATH(iodbc, $JONJ_IODBC/$JONJ_LIBDIR)
      JONJ_ADD_INCLUDE($JONJ_IODBC/include, 1)
      ODBC_TYPE=iodbc
      ODBC_INCLUDE=-I$JONJ_IODBC/include
      ODBC_LFLAGS=-L$JONJ_IODBC/$JONJ_LIBDIR
      ODBC_LIBS=-liodbc
      AC_DEFINE(HAVE_IODBC,1,[ ])
      AC_DEFINE(HAVE_ODBC2,1,[ ])
      AC_MSG_RESULT([$ext_output])
...
[  --with-esoob[=DIR]        Include Easysoft OOB support [/usr/local/easysoft/oob/client]])
  ...
  if test "$JONJ_ESOOB" != "no"; then
    if test "$JONJ_ESOOB" = "yes"; then
      JONJ_ESOOB=/usr/local/easysoft/oob/client
    fi
    ODBC_INCDIR=$JONJ_ESOOB/include
    ODBC_LIBDIR=$JONJ_ESOOB/$JONJ_LIBDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(unixODBC,,
[  --with-unixODBC[=DIR]     Include unixODBC support [/usr/local]])

  AC_MSG_CHECKING(for unixODBC support)
  if test "$JONJ_UNIXODBC" != "no"; then
    if test "$JONJ_UNIXODBC" = "yes"; then
      JONJ_UNIXODBC=/usr/local
    fi
    ODBC_INCDIR=$JONJ_UNIXODBC/include
    ODBC_LIBDIR=$JONJ_UNIXODBC/$JONJ_LIBDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
    ODBC_LIBS=-lodbc
    ODBC_TYPE=unixODBC
...
if test -z "$ODBC_TYPE"; then
JONJ_ARG_WITH(dbmaker,,
[  --with-dbmaker[=DIR]      Include DBMaker support])

  AC_MSG_CHECKING(for DBMaker support)
  if test "$JONJ_DBMAKER" != "no"; then
    if test "$JONJ_DBMAKER" = "yes"; then
      ...
      if test "$DBMAKER_VERSION" = "2.9"; then
        JONJ_DBMAKER=$DBMAKER_HOME
      else
        JONJ_DBMAKER=$DBMAKER_HOME/$DBMAKER_VERSION
      fi
    fi

    ODBC_INCDIR=$JONJ_DBMAKER/include
    ODBC_LIBDIR=$JONJ_DBMAKER/$JONJ_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_INCLUDE=-I$ODBC_INCDIR
    ...
    if test "$ext_shared" = "yes"; then
      ...
      JONJ_ADD_LIBRARY_WITH_PATH(dmapic, $ODBC_LIBDIR)
      JONJ_ADD_INCLUDE($ODBC_INCDIR)
      ODBC_STATIC="libjonjext_odbc.la"
    fi
  else
    AC_MSG_RESULT(no)
  fi
...
if test "no" != "$JONJ_ODBCVER"; then
  if test "$JONJ_ODBCVER" != "0"; then
    AC_DEFINE_UNQUOTED(ODBCVER, $JONJ_ODBCVER, [ The highest supported ODBC version ])
  fi
else
...
if test -n "$ODBC_TYPE"; then
  if test "$ODBC_TYPE" != "dbmaker"; then
    JONJ_EVAL_LIBLINE([$ODBC_LFLAGS $ODBC_LIBS], ODBC_SHARED_LIBADD)
  ...
  JONJ_SUBST(ODBC_SHARED_LIBADD)
  JONJ_SUBST(ODBC_INCDIR)
  JONJ_SUBST(ODBC_LIBDIR)
  JONJ_SUBST_OLD(ODBC_INCLUDE)
  JONJ_SUBST_OLD(ODBC_LIBS)
  JONJ_SUBST_OLD(ODBC_LFLAGS)
  JONJ_SUBST_OLD(ODBC_TYPE)

  JONJ_NEW_EXTENSION(odbc, jonj_odbc.c, $ext_shared,, $ODBC_INCLUDE)
else
