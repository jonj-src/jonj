...
#include "jonj_wddx.h"
#include "jonj_wddx_api.h"

#define JONJ_XML_INTERNAL
#include "ext/xml/jonj_xml.h"
#include "ext/standard/jonj_incomplete_class.h"
...
#define JONJ_CLASS_NAME_VAR		"jonj_class_name"
...
#define jonj_wddx_deserialize(a,b) \
	jonj_wddx_deserialize_ex((a)->value.str.val, (a)->value.str.len, (b))
...
static void jonj_wddx_process_data(void *user_data, const XML_Char *s, int len);
...
const zend_function_entry wddx_functions[] = {
	JONJ_FE(wddx_serialize_value, arginfo_wddx_serialize_value)
	JONJ_FE(wddx_serialize_vars,	arginfo_wddx_serialize_vars)
	JONJ_FE(wddx_packet_start,	arginfo_wddx_serialize_start)
	JONJ_FE(wddx_packet_end,		arginfo_wddx_packet_end)
	JONJ_FE(wddx_add_vars,		arginfo_wddx_add_vars)
	JONJ_FE(wddx_deserialize,	arginfo_wddx_deserialize)
	JONJ_FE_END
};
/* }}} */

JONJ_MINIT_FUNCTION(wddx);
JONJ_MINFO_FUNCTION(wddx);
...
PS_SERIALIZER_ENCODE_FUNC(wddx)
{
	...
	packet = jonj_wddx_constructor();

	jonj_wddx_packet_start(packet, NULL, 0);
	jonj_wddx_add_chunk_static(packet, WDDX_STRUCT_S);

	PS_ENCODE_LOOP(
		jonj_wddx_serialize_var(packet, *struc, key, key_length TSRMLS_CC);
	);

	jonj_wddx_add_chunk_static(packet, WDDX_STRUCT_E);
	jonj_wddx_packet_end(packet);
	*newstr = jonj_wddx_gather(packet);
	jonj_wddx_destructor(packet);
...
PS_SERIALIZER_DECODE_FUNC(wddx)
{
	...
	if ((ret = jonj_wddx_deserialize_ex((char *)val, vallen, retval)) == SUCCESS) {
		...
		for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(retval));
			...
			switch (hash_type) {
				...
				case HASH_KEY_IS_STRING:
					jonj_set_session_var(key, key_length-1, *ent, NULL TSRMLS_CC);
					PS_ADD_VAR(key);
			}
		}
	}
...
#if HAVE_JONJ_SESSION && !defined(COMPILE_DL_SESSION)
	jonj_session_register_serializer("wddx",
...
#if HAVE_JONJ_SESSION && !defined(COMPILE_DL_SESSION)
	jonj_info_print_table_header(2, "WDDX Support", "enabled" );
	jonj_info_print_table_row(2, "WDDX Session Serializer", "enabled" );
#else
	jonj_info_print_table_row(2, "WDDX Support", "enabled" );
#endif
	jonj_info_print_table_end();
}
/* }}} */

/* {{{ jonj_wddx_packet_start
 */
void jonj_wddx_packet_start(wddx_packet *packet, char *comment, int comment_len)
{
	jonj_wddx_add_chunk_static(packet, WDDX_PACKET_S);
	if (comment) {
		char *escaped;
		size_t escaped_len;
		TSRMLS_FETCH();
		escaped = jonj_escape_html_entities(
			comment, comment_len, &escaped_len, 0, ENT_QUOTES, NULL TSRMLS_CC);

		jonj_wddx_add_chunk_static(packet, WDDX_HEADER_S);
		jonj_wddx_add_chunk_static(packet, WDDX_COMMENT_S);
		jonj_wddx_add_chunk_ex(packet, escaped, escaped_len);
		jonj_wddx_add_chunk_static(packet, WDDX_COMMENT_E);
		jonj_wddx_add_chunk_static(packet, WDDX_HEADER_E);

		str_efree(escaped);
	} else {
		jonj_wddx_add_chunk_static(packet, WDDX_HEADER);
	}
	jonj_wddx_add_chunk_static(packet, WDDX_DATA_S);
}
/* }}} */

/* {{{ jonj_wddx_packet_end
 */
void jonj_wddx_packet_end(wddx_packet *packet)
{
	jonj_wddx_add_chunk_static(packet, WDDX_DATA_E);
	jonj_wddx_add_chunk_static(packet, WDDX_PACKET_E);
}
/* }}} */

#define FLUSH_BUF()                               \
	if (l > 0) {                                  \
		jonj_wddx_add_chunk_ex(packet, buf, l);    \
		l = 0;                                    \
	}

/* {{{ jonj_wddx_serialize_string
 */
static void jonj_wddx_serialize_string(wddx_packet *packet, zval *var TSRMLS_DC)
{
	jonj_wddx_add_chunk_static(packet, WDDX_STRING_S);

	if (Z_STRLEN_P(var) > 0) {
		char *buf;
		size_t buf_len;

		buf = jonj_escape_html_entities(Z_STRVAL_P(var), Z_STRLEN_P(var), &buf_len, 0, ENT_QUOTES, NULL TSRMLS_CC);

		jonj_wddx_add_chunk_ex(packet, buf, buf_len);

		str_efree(buf);
	}
	jonj_wddx_add_chunk_static(packet, WDDX_STRING_E);
}
/* }}} */

/* {{{ jonj_wddx_serialize_number
 */
static void jonj_wddx_serialize_number(wddx_packet *packet, zval *var)
{
	...
	jonj_wddx_add_chunk(packet, tmp_buf);
}
/* }}} */

/* {{{ jonj_wddx_serialize_boolean
 */
static void jonj_wddx_serialize_boolean(wddx_packet *packet, zval *var)
{
	jonj_wddx_add_chunk(packet, Z_LVAL_P(var) ? WDDX_BOOLEAN_TRUE : WDDX_BOOLEAN_FALSE);
}
/* }}} */

/* {{{ jonj_wddx_serialize_unset
 */
static void jonj_wddx_serialize_unset(wddx_packet *packet)
{
	jonj_wddx_add_chunk_static(packet, WDDX_NULL);
}
/* }}} */

/* {{{ jonj_wddx_serialize_object
 */
static void jonj_wddx_serialize_object(wddx_packet *packet, zval *obj)
{
/* OBJECTS_FIXME */
	...
	zend_class_entry *ce;
	JONJ_CLASS_ATTRIBUTES;
	TSRMLS_FETCH();

	JONJ_SET_CLASS_ATTRIBUTES(obj);
	ce = Z_OBJCE_P(obj);
	if (!ce || ce->serialize || ce->unserialize) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Class %s can not be serialized", class_name);
		JONJ_CLEANUP_CLASS_ATTRIBUTES();
		return;
	}
	...
	if (call_user_function_ex(CG(function_table), &obj, fname, &retval, 0, 0, 1, NULL TSRMLS_CC) == SUCCESS) {
		if (retval && (sleephash = HASH_OF(retval))) {
			jonj_wddx_add_chunk_static(packet, WDDX_STRUCT_S);
			snprintf(tmp_buf, WDDX_BUF_LEN, WDDX_VAR_S, JONJ_CLASS_NAME_VAR);
			jonj_wddx_add_chunk(packet, tmp_buf);
			jonj_wddx_add_chunk_static(packet, WDDX_STRING_S);
			jonj_wddx_add_chunk_ex(packet, class_name, name_len);
			jonj_wddx_add_chunk_static(packet, WDDX_STRING_E);
			jonj_wddx_add_chunk_static(packet, WDDX_VAR_E);
			...
			for (zend_hash_internal_pointer_reset(sleephash);
				...
				if (Z_TYPE_PP(varname) != IS_STRING) {
					jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "__sleep should return an array only containing the names of instance-variables to serialize.");
					continue;
				}

				if (zend_hash_find(objhash, Z_STRVAL_PP(varname), Z_STRLEN_PP(varname)+1, (void **)&ent) == SUCCESS) {
					jonj_wddx_serialize_var(packet, *ent, Z_STRVAL_PP(varname), Z_STRLEN_PP(varname) TSRMLS_CC);
				}
			}
		...
		for (zend_hash_internal_pointer_reset(objhash);
			...
			if (zend_hash_get_current_key_ex(objhash, &key, &key_len, &idx, 0, NULL) == HASH_KEY_IS_STRING) {
				...
				jonj_wddx_serialize_var(packet, *ent, prop_name, strlen(prop_name)+1 TSRMLS_CC);
			} else {
				key_len = slprintf(tmp_buf, sizeof(tmp_buf), "%ld", idx);
				jonj_wddx_serialize_var(packet, *ent, tmp_buf, key_len TSRMLS_CC);
			}
		}
		jonj_wddx_add_chunk_static(packet, WDDX_STRUCT_E);
	}
...
/* {{{ jonj_wddx_serialize_array
 */
static void jonj_wddx_serialize_array(wddx_packet *packet, zval *arr)
{
	zval **ent;
	char *key;
	uint key_len;
	...
	for (zend_hash_internal_pointer_reset(target_hash);
		...
		if (is_struct) {
			...
			if (ent_type == HASH_KEY_IS_STRING) {
				jonj_wddx_serialize_var(packet, *ent, key, key_len TSRMLS_CC);
			} else {
				key_len = slprintf(tmp_buf, sizeof(tmp_buf), "%ld", idx);
				jonj_wddx_serialize_var(packet, *ent, tmp_buf, key_len TSRMLS_CC);
			}
		} else {
			jonj_wddx_serialize_var(packet, *ent, NULL, 0 TSRMLS_CC);
		}
	}
	...
	if (is_struct) {
		...
		jonj_wddx_add_chunk_static(packet, WDDX_ARRAY_E);
	}
}
/* }}} */

/* {{{ jonj_wddx_serialize_var
 */
void jonj_wddx_serialize_var(wddx_packet *packet, zval *var, char *name, int name_len TSRMLS_DC)
{
	...
	if (name) {
		...
		name_esc = jonj_escape_html_entities(name, name_len, &name_esc_len, 0, ENT_QUOTES, NULL TSRMLS_CC);
	...
	switch(Z_TYPE_P(var)) {
		case IS_STRING:
			jonj_wddx_serialize_string(packet, var TSRMLS_CC);
			break;

		case IS_LONG:
		case IS_DOUBLE:
			jonj_wddx_serialize_number(packet, var);
			break;

		case IS_BOOL:
			jonj_wddx_serialize_boolean(packet, var);
			break;

		case IS_NULL:
			jonj_wddx_serialize_unset(packet);
			break;

		case IS_ARRAY:
			ht = Z_ARRVAL_P(var);
			if (ht->nApplyCount > 1) {
				jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "WDDX doesn't support circular references");
				return;
			}
			ht->nApplyCount++;
			jonj_wddx_serialize_array(packet, var);
			ht->nApplyCount--;
			break;
		...
		case IS_OBJECT:
			...
			ht->nApplyCount++;
 			jonj_wddx_serialize_object(packet, var);
			ht->nApplyCount--;
			break;
	}
...
/* {{{ jonj_wddx_add_var
 */
static void jonj_wddx_add_var(wddx_packet *packet, zval *name_var)
{
	zval **val;
	HashTable *target_hash;
	TSRMLS_FETCH();
	...
	if (Z_TYPE_P(name_var) == IS_STRING) {
		...
		if (zend_hash_find(EG(active_symbol_table), Z_STRVAL_P(name_var),
			...
			jonj_wddx_serialize_var(packet, *val, Z_STRVAL_P(name_var), Z_STRLEN_P(name_var) TSRMLS_CC);
		}
...
/* {{{ jonj_wddx_push_element
 */
static void jonj_wddx_push_element(void *user_data, const XML_Char *name, const XML_Char **atts)
{
	st_entry ent;
	...
	if (!strcmp(name, EL_PACKET)) {
		...
		if (atts) for (i = 0; atts[i]; i++) {
			if (!strcmp(atts[i], EL_CHAR_CODE) && atts[i+1] && atts[i+1][0]) {
				...
				jonj_wddx_process_data(user_data, tmp_buf, strlen(tmp_buf));
				break;
			}
		}
		...
		if (atts) for (i = 0; atts[i]; i++) {
			if (!strcmp(atts[i], EL_VALUE) && atts[i+1] && atts[i+1][0]) {
				...
				SET_STACK_VARNAME;
				wddx_stack_push((wddx_stack *)stack, &ent, sizeof(st_entry));
				jonj_wddx_process_data(user_data, atts[i+1], strlen(atts[i+1]));
				break;
			}
		} else {
			ALLOC_ZVAL(ent.data);
		...
		if (atts) for (i = 0; atts[i]; i++) {
			if (!strcmp(atts[i], "fieldNames") && atts[i+1] && atts[i+1][0]) {
				...
				p1 = (char *)atts[i];
				while ((p2 = jonj_memnstr(p1, ",", sizeof(",")-1, endp)) != NULL) {
...
/* {{{ jonj_wddx_pop_element
 */
static void jonj_wddx_pop_element(void *user_data, const XML_Char *name)
{
	st_entry 			*ent1, *ent2;
	wddx_stack 			*stack = (wddx_stack *)user_data;
	HashTable 			*target_hash;
...
/* OBJECTS_FIXME */
	...
	if (!strcmp(name, EL_STRING) || !strcmp(name, EL_NUMBER) ||
		...
		if (!strcmp(name, EL_BINARY)) {
			...
			new_str = jonj_base64_decode(Z_STRVAL_P(ent1->data), Z_STRLEN_P(ent1->data), &new_len);
		...
		if (stack->top > 1) {
			...
			if (Z_TYPE_P(ent2->data) == IS_ARRAY || Z_TYPE_P(ent2->data) == IS_OBJECT) {
				...
				if (ent1->varname) {
					if (!strcmp(ent1->varname, JONJ_CLASS_NAME_VAR) &&
						...
						if (pce != &JONJ_IC_ENTRY && ((*pce)->serialize || (*pce)->unserialize)) {
							zval_ptr_dtor(&ent2->data);
							ent2->data = NULL;
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Class %s can not be unserialized", Z_STRVAL_P(ent1->data));
						} else {
							...
							if (incomplete_class) {
								jonj_store_class_name(obj, Z_STRVAL_P(ent1->data), Z_STRLEN_P(ent1->data));
							}
	...
	if (!wddx_stack_is_empty(stack) && !stack->done) {
		...
		switch (ent->type) {
			...
			case ST_DATETIME: {
				...
				Z_LVAL_P(ent->data) = jonj_parse_date(tmp, NULL);
...
/* {{{ jonj_wddx_deserialize_ex
 */
int jonj_wddx_deserialize_ex(char *value, int vallen, zval *return_value)
{
	wddx_stack stack;
	XML_Parser parser;
	st_entry *ent;
	...
	XML_SetElementHandler(parser, jonj_wddx_push_element, jonj_wddx_pop_element);
	XML_SetCharacterDataHandler(parser, jonj_wddx_process_data);
...
JONJ_FUNCTION(wddx_serialize_value)
{
	...
	jonj_wddx_packet_start(packet, comment, comment_len);
	jonj_wddx_serialize_var(packet, var, NULL, 0 TSRMLS_CC);
	jonj_wddx_packet_end(packet);
...
JONJ_FUNCTION(wddx_serialize_vars)
{
	...
	for (i=0; i<num_args; i++) {
		...
		jonj_wddx_add_var(packet, *args[i]);
	}
...
/* {{{ proto mixed wddx_deserialize(mixed packet)
   Deserializes given packet and returns a JONJ value */
JONJ_FUNCTION(wddx_deserialize)
{
	...
	if (Z_TYPE_P(packet) == IS_STRING) {
		...
		jonj_stream_from_zval(stream, &packet);
		if (stream) {
			payload_len = jonj_stream_copy_to_mem(stream, &payload, JONJ_STREAM_COPY_ALL, 0);
		}
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Expecting parameter 1 to be a string or a stream");
		return;
	}
	...
	jonj_wddx_deserialize_ex(payload, payload_len, return_value);
