/*
  ...
  +----------------------------------------------------------------------+
  | Authors: Sterling Hughes <sterling@jonj.tk>                          |
...
#include "jonj_simplexml_exports.h"
...
#define SXE_ME(func, arg_info, flags) JONJ_ME(simplexml_element, func, arg_info, flags)
#define SXE_MALIAS(func, alias, arg_info, flags) JONJ_MALIAS(simplexml_element, func, alias, arg_info, flags)

#define SXE_METHOD(func) JONJ_METHOD(simplexml_element, func)

static jonj_sxe_object* jonj_sxe_object_new(zend_class_entry *ce TSRMLS_DC);
static zend_object_value jonj_sxe_register_object(jonj_sxe_object * TSRMLS_DC);
static xmlNodePtr jonj_sxe_reset_iterator(jonj_sxe_object *sxe, int use_data TSRMLS_DC);
static xmlNodePtr jonj_sxe_iterator_fetch(jonj_sxe_object *sxe, xmlNodePtr node, int use_data TSRMLS_DC);
static zval *sxe_get_value(zval *z TSRMLS_DC);
static void jonj_sxe_iterator_dtor(zend_object_iterator *iter TSRMLS_DC);
static int jonj_sxe_iterator_valid(zend_object_iterator *iter TSRMLS_DC);
static void jonj_sxe_iterator_current_data(zend_object_iterator *iter, zval ***data TSRMLS_DC);
static void jonj_sxe_iterator_current_key(zend_object_iterator *iter, zval *key TSRMLS_DC);
static void jonj_sxe_iterator_move_forward(zend_object_iterator *iter TSRMLS_DC);
static void jonj_sxe_iterator_rewind(zend_object_iterator *iter TSRMLS_DC);

/* {{{ _node_as_zval()
 */
static void _node_as_zval(jonj_sxe_object *sxe, xmlNodePtr node, zval *value, SXE_ITER itertype, char *name, const xmlChar *nsprefix, int isprefix TSRMLS_DC)
{
	jonj_sxe_object *subnode;

	subnode = jonj_sxe_object_new(sxe->zo.ce TSRMLS_CC);
	...
	jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)subnode, node, NULL TSRMLS_CC);

	value->type = IS_OBJECT;
	value->value.obj = jonj_sxe_register_object(subnode TSRMLS_CC);
}
/* }}} */
...
static xmlNodePtr jonj_sxe_get_first_node(jonj_sxe_object *sxe, xmlNodePtr node TSRMLS_DC) /* {{{ */
{
	jonj_sxe_object *intern;
	xmlNodePtr retnode = NULL;

	if (sxe && sxe->iter.type != SXE_ITER_NONE) {
		jonj_sxe_reset_iterator(sxe, 1 TSRMLS_CC);
		if (sxe->iter.data) {
			intern = (jonj_sxe_object *)zend_object_store_get_object(sxe->iter.data TSRMLS_CC);
...
static inline int match_ns(jonj_sxe_object *sxe, xmlNodePtr node, xmlChar *name, int prefix) /* {{{ */
{
...
static xmlNodePtr sxe_get_element_by_offset(jonj_sxe_object *sxe, long offset, xmlNodePtr node, long *cnt) /* {{{ */
{
	long nodendx = 0;
...
static xmlNodePtr sxe_find_element_by_name(jonj_sxe_object *sxe, xmlNodePtr node, xmlChar *name TSRMLS_DC) /* {{{ */
{
	while (node) {
		SKIP_TEXT(node)
		if (node->type == XML_ELEMENT_NODE && match_ns(sxe, node, sxe->iter.nsprefix, sxe->iter.isprefix)) {
...
static xmlNodePtr sxe_get_element_by_name(jonj_sxe_object *sxe, xmlNodePtr node, char **name, SXE_ITER *type TSRMLS_DC) /* {{{ */
{
	int         orgtype;
	...
	if (sxe->iter.type != SXE_ITER_ATTRLIST)
	{
		...
		node = jonj_sxe_get_first_node(sxe, node TSRMLS_CC);
...
static zval * sxe_prop_dim_read(zval *object, zval *member, zend_bool elements, zend_bool attribs, int type TSRMLS_DC)
{
	...
	sxe = jonj_sxe_fetch_object(object TSRMLS_CC);
	...
	if (!member || Z_TYPE_P(member) == IS_LONG) {
		if (sxe->iter.type != SXE_ITER_ATTRLIST) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Cannot create unnamed attribute");
			return NULL;
		}
		name = NULL;
	} else {
	...
	if (node) {
		...
		if (elements) {
			if (!sxe->node) {
				jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)sxe, node, NULL TSRMLS_CC);
			}
			if (!member || Z_TYPE_P(member) == IS_LONG) {
				long cnt = 0;
				...
				if (sxe->iter.type == SXE_ITER_NONE) {
					if (member && Z_LVAL_P(member) > 0) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add element %s number %ld when only 0 such elements exist", mynode->name, Z_LVAL_P(member));
					}
				} else if (member) {
				...
				if (node) {
					...
					if (member && cnt < Z_LVAL_P(member)) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add element %s number %ld when only %ld such elements exist", mynode->name, Z_LVAL_P(member), cnt);
					}
...
static void change_node_zval(xmlNodePtr node, zval *value TSRMLS_DC)
{
	...
	switch (Z_TYPE_P(value)) {
		...
		case IS_STRING:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "It is not possible to assign complex types to nodes");
			break;
	}
}
/* }}} */
...
static int sxe_prop_dim_write(zval *object, zval *member, zval *value, zend_bool elements, zend_bool attribs, xmlNodePtr *pnewnode TSRMLS_DC)
{
	...
	if (!member || Z_TYPE_P(member) == IS_LONG) {
		...
		if (Z_TYPE_P(member) != IS_STRING) {
			...
			jonj_trim(Z_STRVAL(trim_zv), Z_STRLEN(trim_zv), NULL, 0, &tmp_zv, 3 TSRMLS_CC);
			zval_dtor(&trim_zv);
			member = &tmp_zv;
		}

		if (!Z_STRLEN_P(member)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot write or create unnamed %s", attribs ? "attribute" : "element");
			if (member == &tmp_zv) {
				zval_dtor(&tmp_zv);
			}
			return FAILURE;
		...
		if (elements) {
			if (!member || Z_TYPE_P(member) == IS_LONG) {
				if (node->type == XML_ATTRIBUTE_NODE) {
					jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Cannot create duplicate attribute");
					return FAILURE;
				}
		...
		if (counter == 1) {
			...
			if (value) {
				while ((tempnode = (xmlNodePtr) newnode->children)) {
					...
					jonj_libxml_node_free_resource((xmlNodePtr) tempnode TSRMLS_CC);
				}
				change_node_zval(newnode, value TSRMLS_CC);
			}
		} else if (counter > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot assign to an array of nodes (duplicate subnodes or attr detected)");
			retval = FAILURE;
		} else if (elements) {
			if (!node) {
				if (!member || Z_TYPE_P(member) == IS_LONG) {
			...
			if (Z_TYPE_P(member) == IS_LONG) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot change attribute number %ld when only %d attributes exist", Z_LVAL_P(member), nodendx);
				retval = FAILURE;
			} else {
...
static zval** sxe_property_get_adr(zval *object, zval *member, int fetch_type, const zend_literal *key TSRMLS_DC) /* {{{ */
{
	...
	sxe = jonj_sxe_fetch_object(return_value TSRMLS_CC);
	if (sxe->tmp) {
	...
	if (node) {
				...
				while (attr && nodendx <= Z_LVAL_P(member)) {
					if ((!test || !xmlStrcmp(attr->name, sxe->iter.name)) && match_ns(sxe, (xmlNodePtr) attr, sxe->iter.nsprefix, sxe->iter.isprefix)) {
						if (nodendx == Z_LVAL_P(member)) {
							...
							jonj_libxml_node_free_resource((xmlNodePtr) attr TSRMLS_CC);
							break;
						}
						nodendx++;
					}
		...
		if (elements) {
			if (Z_TYPE_P(member) == IS_LONG) {
				...
				if (node) {
					xmlUnlinkNode(node);
					jonj_libxml_node_free_resource(node TSRMLS_CC);
				}
			} else {
				node = node->children;
				while (node) {
 ...
 */
static void _get_base_node_value(jonj_sxe_object *sxe_ref, xmlNodePtr node, zval **value, xmlChar *nsprefix, int isprefix TSRMLS_DC)
{
	jonj_sxe_object *subnode;
	xmlChar        *contents;
	...
	if (node->children && node->children->type == XML_TEXT_NODE && !xmlIsBlankNode(node->children)) {
		...
		subnode = jonj_sxe_object_new(sxe_ref->zo.ce TSRMLS_CC);
	...
	if (node && sxe->iter.type != SXE_ITER_ATTRLIST) {
		if (node->type == XML_ATTRIBUTE_NODE) {
			...
			if ( sxe->iter.type == SXE_ITER_NONE || !node->children || !node->parent || !node->next || node->children->next || node->children->children || node->parent->children == node->parent->last ) {
				...
				node = jonj_sxe_reset_iterator(sxe, 0 TSRMLS_CC);
...
next_iter:
			if ( use_iter ) {
				node = jonj_sxe_iterator_fetch(sxe, node->next, 0 TSRMLS_CC);
			} else {
				node = node->next;
			}
		}
...
{
	jonj_sxe_object *sxe1;
	jonj_sxe_object *sxe2;

	sxe1 = jonj_sxe_fetch_object(object1 TSRMLS_CC);
	sxe2 = jonj_sxe_fetch_object(object2 TSRMLS_CC);
...
SXE_METHOD(xpath)
{
	...
	sxe = jonj_sxe_fetch_object(getThis() TSRMLS_CC);
	...
	if (!sxe->node) {
		jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)sxe, xmlDocGetRootElement((xmlDocPtr) sxe->document->ptr), NULL TSRMLS_CC);
		if (!sxe->node) {
			RETURN_FALSE;
		}
	}

	nodeptr = jonj_sxe_get_first_node(sxe, sxe->node->node TSRMLS_CC);
...
static void sxe_add_namespaces(jonj_sxe_object *sxe, xmlNodePtr node, zend_bool recursive, zval *return_value TSRMLS_DC) /* {{{ */
{
	xmlAttrPtr  attr;
...
static void sxe_add_registered_namespaces(jonj_sxe_object *sxe, xmlNodePtr node, zend_bool recursive, zval *return_value TSRMLS_DC) /* {{{ */
{
	xmlNsPtr ns;
...
SXE_METHOD(addChild)
{
	...
	if (qname_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Element name is required");
		return;
	}
	...
	if (sxe->iter.type == SXE_ITER_ATTRLIST) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add element to attributes");
		return;
	}
	...
	if (node == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add child. Parent is not a permanent member of the XML tree");
		return;
	}
...
SXE_METHOD(addAttribute)
{
	...
	if (qname_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attribute name is required");
		return;
	}
	...
	if (node == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to locate parent Element");
		return;
	}
	...
	if (localname == NULL) {
		if (nsuri_len > 0) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attribute requires prefix for namespace");
			return;
		}
		localname = xmlStrdup((xmlChar *)qname);
	}
	...
	if (attrp != NULL && attrp->type != XML_ATTRIBUTE_DECL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attribute already exists");
		return;
	}
...
static int sxe_object_cast(zval *readobj, zval *writeobj, int type TSRMLS_DC)
{
	...
	sxe = jonj_sxe_fetch_object(readobj TSRMLS_CC);

	if (type == IS_BOOL) {
		node = jonj_sxe_get_first_node(sxe, NULL TSRMLS_CC);
...
static int jonj_sxe_count_elements_helper(jonj_sxe_object *sxe, long *count TSRMLS_DC) /* {{{ */
{
	xmlNodePtr       node;
	zval            *data;
...
{
	jonj_sxe_object  *intern;
	intern = jonj_sxe_fetch_object(object TSRMLS_CC);
	...
	return jonj_sxe_count_elements_helper(intern, count TSRMLS_CC);
}
/* }}} */
...
{
	long count = 0;
	jonj_sxe_object *sxe = jonj_sxe_fetch_object(getThis() TSRMLS_CC);

	if (zend_parse_parameters_none() == FAILURE) {
		return;
	}

	jonj_sxe_count_elements_helper(sxe, &count TSRMLS_CC);
...
{
	jonj_sxe_object *sxe = (jonj_sxe_object *) object;
	jonj_sxe_object *clone;
	xmlNodePtr nodep = NULL;
	xmlDocPtr docp = NULL;

	clone = jonj_sxe_object_new(sxe->zo.ce TSRMLS_CC);
	...
	jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)clone, nodep, NULL TSRMLS_CC);
...
static void sxe_object_free_storage(void *object TSRMLS_DC)
{
	...
	jonj_libxml_node_decrement_resource((jonj_libxml_node_object *)sxe TSRMLS_CC);
...
static jonj_sxe_object* jonj_sxe_object_new(zend_class_entry *ce TSRMLS_DC)
{
	...
	intern = ecalloc(1, sizeof(jonj_sxe_object));
...
/* {{{ jonj_sxe_register_object
 */
static zend_object_value
jonj_sxe_register_object(jonj_sxe_object *intern TSRMLS_DC)
{
...
/* {{{ sxe_object_new()
 */
JONJ_SXE_API zend_object_value
sxe_object_new(zend_class_entry *ce TSRMLS_DC)
{
	jonj_sxe_object    *intern;

	intern = jonj_sxe_object_new(ce TSRMLS_CC);
	return jonj_sxe_register_object(intern TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(simplexml_load_file)
{
	jonj_sxe_object *sxe;
	char           *filename;
	int             filename_len;
	...
	sxe = jonj_sxe_object_new(ce TSRMLS_CC);
	sxe->iter.nsprefix = ns_len ? xmlStrdup((xmlChar *)ns) : NULL;
	sxe->iter.isprefix = isprefix;
	jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)sxe, docp TSRMLS_CC);
	jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)sxe, xmlDocGetRootElement(docp), NULL TSRMLS_CC);

	return_value->type = IS_OBJECT;
	return_value->value.obj = jonj_sxe_register_object(sxe TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(simplexml_load_string)
{
	jonj_sxe_object *sxe;
	char           *data;
	int             data_len;
...
SXE_METHOD(__construct)
{
	...
	if (!docp) {
		((jonj_libxml_node_object *)sxe)->document = NULL;
...
zend_object_iterator_funcs jonj_sxe_iterator_funcs = { /* {{{ */
	jonj_sxe_iterator_dtor,
	jonj_sxe_iterator_valid,
	jonj_sxe_iterator_current_data,
	jonj_sxe_iterator_current_key,
	jonj_sxe_iterator_move_forward,
	jonj_sxe_iterator_rewind,
};
/* }}} */
...
static xmlNodePtr jonj_sxe_reset_iterator(jonj_sxe_object *sxe, int use_data TSRMLS_DC) /* {{{ */
{
	...
	if (node) {
		...
		return jonj_sxe_iterator_fetch(sxe, node, use_data TSRMLS_CC);
	}
	return NULL;
}
/* }}} */

zend_object_iterator *jonj_sxe_get_iterator(zend_class_entry *ce, zval *object, int by_ref TSRMLS_DC) /* {{{ */
{
	jonj_sxe_iterator *iterator;

	if (by_ref) {
		zend_error(E_ERROR, "An iterator cannot be used with foreach by reference");
	}
	iterator = emalloc(sizeof(jonj_sxe_iterator));

	Z_ADDREF_P(object);
	iterator->intern.data = (void*)object;
	iterator->intern.funcs = &jonj_sxe_iterator_funcs;
	iterator->sxe = jonj_sxe_fetch_object(object TSRMLS_CC);
...
static void jonj_sxe_iterator_dtor(zend_object_iterator *iter TSRMLS_DC) /* {{{ */
{
	jonj_sxe_iterator *iterator = (jonj_sxe_iterator *)iter;
...
static void jonj_sxe_iterator_current_key(zend_object_iterator *iter, zval *key TSRMLS_DC) /* {{{ */
{
	...
	jonj_sxe_object *intern = (jonj_sxe_object *)zend_object_store_get_object(curobj TSRMLS_CC);

	xmlNodePtr curnode = NULL;
	if (intern != NULL && intern->node != NULL) {
		curnode = (xmlNodePtr)((jonj_libxml_node_ptr *)intern->node)->node;
	}
...
JONJ_SXE_API void jonj_sxe_move_forward_iterator(jonj_sxe_object *sxe TSRMLS_DC) /* {{{ */
{
	...
	if (node) {
		jonj_sxe_iterator_fetch(sxe, node->next, 1 TSRMLS_CC);
	}
}
/* }}} */
...
{
	jonj_sxe_iterator *iterator = (jonj_sxe_iterator *)iter;
	jonj_sxe_move_forward_iterator(iterator->sxe TSRMLS_CC);
}
/* }}} */
...
void *simplexml_export_node(zval *object TSRMLS_DC) /* {{{ */
{
	...
	sxe = jonj_sxe_fetch_object(object TSRMLS_CC);
	GET_NODE(sxe, node);
	return jonj_sxe_get_first_node(sxe, node TSRMLS_CC);
}
/* }}} */

/* {{{ proto simplemxml_element simplexml_import_dom(domNode node [, string class_name])
   Get a simplexml_element object from dom to allow for processing */
JONJ_FUNCTION(simplexml_import_dom)
{
	jonj_sxe_object *sxe;
	zval *node;
	jonj_libxml_node_object *object;
	xmlNodePtr		nodep = NULL;
	zend_class_entry *ce= sxe_class_entry;
	...
	object = (jonj_libxml_node_object *)zend_object_store_get_object(node TSRMLS_CC);

	nodep = jonj_libxml_import_node(node TSRMLS_CC);

	if (nodep) {
		if (nodep->doc == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Imported Node must have associated Document");
			RETURN_NULL();
		}
	...
	if (nodep && nodep->type == XML_ELEMENT_NODE) {
		...
		jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)sxe, nodep->doc TSRMLS_CC);
		jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)sxe, nodep, NULL TSRMLS_CC);

		return_value->type = IS_OBJECT;
		return_value->value.obj = jonj_sxe_register_object(sxe TSRMLS_CC);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid Nodetype to import");
		RETVAL_NULL();
	}
}
/* }}} */
...
const zend_function_entry simplexml_functions[] = { /* {{{ */
	JONJ_FE(simplexml_load_file, 	arginfo_simplexml_load_file)
	JONJ_FE(simplexml_load_string,	arginfo_simplexml_load_string)
	JONJ_FE(simplexml_import_dom,	arginfo_simplexml_import_dom)
	JONJ_FE_END
};
/* }}} */
...
zend_module_entry simplexml_module_entry = { /* {{{ */
	...
	"SimpleXML",
	simplexml_functions,
	JONJ_MINIT(simplexml),
	JONJ_MSHUTDOWN(simplexml),
	NULL,
	NULL,
	JONJ_MINFO(simplexml),
	"0.1",
...
JONJ_MINIT_FUNCTION(simplexml)
{
	...
	sxe_class_entry->get_iterator = jonj_sxe_get_iterator;
	sxe_class_entry->iterator_funcs.funcs = &jonj_sxe_iterator_funcs;
	...
	jonj_libxml_register_export(sxe_class_entry, simplexml_export_node);

	JONJ_MINIT(sxe)(INIT_FUNC_ARGS_PASSTHRU);
...
JONJ_MINFO_FUNCTION(simplexml)
{
	...
	jonj_info_print_table_header(2, "Simplexml support", "enabled");
	jonj_info_print_table_row(2, "Revision", "$Id$");
	jonj_info_print_table_row(2, "Schema support",
