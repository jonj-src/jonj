...
static int readline_shell_ub_write(const char *str, uint str_length TSRMLS_DC) /* {{{ */
{
	...
	jonj_last_char = str[str_length-1];
	return -1;
}
/* }}} */
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("cli.pager", "", JONJ_INI_ALL, OnUpdateString, pager, zend_cli_readline_globals, cli_readline_globals)
	STD_JONJ_INI_ENTRY("cli.prompt", DEFAULT_PROMPT, JONJ_INI_ALL, OnUpdateString, prompt, zend_cli_readline_globals, cli_readline_globals)
JONJ_INI_END()
...
static char *cli_get_prompt(char *block, char prompt TSRMLS_DC) /* {{{ */
{
	...
	do {
		if (*prompt_spec == '\\') {
			...
			case 'v':
				smart_str_appends(&retval, JONJ_VERSION);
				prompt_spec++;
				break;
			case 'b':
			...
			if (prompt_end) {
				...
				zend_try {
					zend_eval_stringl(code, prompt_end - prompt_spec - 1, NULL, "jonj prompt code" TSRMLS_CC);
				} zend_end_try();
...
static int cli_is_valid_code(char *code, int len, char **prompt TSRMLS_DC) /* {{{ */
{
	...
	switch (code_type) {
		default:
			if (brace_count) {
				*prompt = cli_get_prompt("jonj", '(' TSRMLS_CC);
...
- future: respect scope ("jonj > function foo() { $[tab]" should only expand to local variables...)
*/
	char *retval = NULL;
...
static int readline_shell_run(TSRMLS_D) /* {{{ */
{
	...
	char *prompt = cli_get_prompt("jonj", '>' TSRMLS_CC);
	char *history_file;
	...
	history_file = tilde_expand("~/.jonj_history");
	...
	while ((line = readline(prompt)) != NULL) {
		...
		if (line[0] == '#') {
			...
			if (param) {
				...
				zend_alter_ini_entry_ex(cmd, cmd_len + 1, param, strlen(param), JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
				efree(cmd);
				add_history(line);
		...
		zend_try {
			zend_eval_stringl(code, pos, NULL, "jonj shell code" TSRMLS_CC);
		} zend_end_try();

		pos = 0;
					
		if (!pager_pipe && jonj_last_char != '\0' && jonj_last_char != '\n') {
			jonj_write("\n", 1 TSRMLS_CC);
		}
...
#define GET_SHELL_CB(cb) \
	do { \
		...
		get_callbacks = dlsym(RTLD_DEFAULT, "jonj_cli_get_shell_callbacks"); \
		if (get_callbacks) { \
			(cb) = get_callbacks(); \
		} \
	} while(0)
/*#else
#define GET_SHELL_CB(cb) (cb) = jonj_cli_get_shell_callbacks()
#endif*/

JONJ_MINIT_FUNCTION(cli_readline)
{
...
JONJ_MSHUTDOWN_FUNCTION(cli_readline)
{
	cli_shell_callbacks_t *cb;
...
JONJ_MINFO_FUNCTION(cli_readline)
{
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "Readline Support", "enabled");
	jonj_info_print_table_row(2, "Readline library", (rl_library_version ? rl_library_version : "Unknown"));
