...
JONJ_FUNCTION(readline);
JONJ_FUNCTION(readline_add_history);
JONJ_FUNCTION(readline_info);
JONJ_FUNCTION(readline_clear_history);
#ifndef HAVE_LIBEDIT
JONJ_FUNCTION(readline_list_history);
#endif
JONJ_FUNCTION(readline_read_history);
JONJ_FUNCTION(readline_write_history);
JONJ_FUNCTION(readline_completion_function);

#if HAVE_RL_CALLBACK_READ_CHAR
JONJ_FUNCTION(readline_callback_handler_install);
JONJ_FUNCTION(readline_callback_read_char);
JONJ_FUNCTION(readline_callback_handler_remove);
JONJ_FUNCTION(readline_redisplay);
JONJ_FUNCTION(readline_on_new_line);
...
JONJ_MINIT_FUNCTION(readline);
JONJ_MSHUTDOWN_FUNCTION(readline);
JONJ_RSHUTDOWN_FUNCTION(readline);
JONJ_MINFO_FUNCTION(readline);
...
/* {{{ module stuff */
static const zend_function_entry jonj_readline_functions[] = {
	JONJ_FE(readline,	   		        arginfo_readline)
	JONJ_FE(readline_info,  	            arginfo_readline_info)
	JONJ_FE(readline_add_history, 		arginfo_readline_add_history)
	JONJ_FE(readline_clear_history, 		arginfo_readline_clear_history)
#ifndef HAVE_LIBEDIT
	JONJ_FE(readline_list_history, 		arginfo_readline_list_history)
#endif
	JONJ_FE(readline_read_history, 		arginfo_readline_read_history)
	JONJ_FE(readline_write_history, 		arginfo_readline_write_history)
	JONJ_FE(readline_completion_function,arginfo_readline_completion_function)
#if HAVE_RL_CALLBACK_READ_CHAR
	JONJ_FE(readline_callback_handler_install, arginfo_readline_callback_handler_install)
	JONJ_FE(readline_callback_read_char,			arginfo_readline_callback_read_char)
	JONJ_FE(readline_callback_handler_remove,	arginfo_readline_callback_handler_remove)
	JONJ_FE(readline_redisplay, arginfo_readline_redisplay)
#endif
#if HAVE_RL_ON_NEW_LINE
	JONJ_FE(readline_on_new_line, arginfo_readline_on_new_line)
#endif
	JONJ_FE_END
};
...
zend_module_entry readline_module_entry = { 
	...
	"readline", 
	jonj_readline_functions, 
	JONJ_MINIT(readline), 
	JONJ_MSHUTDOWN(readline),
	NULL,
	JONJ_RSHUTDOWN(readline),
	JONJ_MINFO(readline), 
	JONJ_VERSION,
...
JONJ_MINIT_FUNCTION(readline)
{
		...
		using_history();
#endif
    	return JONJ_MINIT(cli_readline)(INIT_FUNC_ARGS_PASSTHRU);
}

JONJ_MSHUTDOWN_FUNCTION(readline)
{
	return JONJ_MSHUTDOWN(cli_readline)(SHUTDOWN_FUNC_ARGS_PASSTHRU);
}
...
JONJ_MINFO_FUNCTION(readline)
{
	JONJ_MINFO(cli_readline)(ZEND_MODULE_INFO_FUNC_ARGS_PASSTHRU);
}
...
JONJ_FUNCTION(readline_read_history)
{
	...
	if (arg && jonj_check_open_basedir(arg TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(readline_completion_function)
{
	...
	if (!zend_is_callable(arg, 0, &name TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s is not callable", name);
		efree(name);
		RETURN_FALSE;
	}
	efree(name);
...
static void jonj_rl_callback_handler(char *the_line)
{
	zval *params[1];
	zval dummy;
	TSRMLS_FETCH();
...
JONJ_FUNCTION(readline_callback_handler_install)
{
	...
	rl_callback_handler_install(prompt, jonj_rl_callback_handler);
