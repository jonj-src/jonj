...
const zend_function_entry sysvsem_functions[] = {
	JONJ_FE(sem_get,			arginfo_sem_get)
	JONJ_FE(sem_acquire,		arginfo_sem_acquire)
	JONJ_FE(sem_release,		arginfo_sem_release)
	JONJ_FE(sem_remove,		arginfo_sem_remove)
	JONJ_FE_END
};
/* }}} */
...
THREAD_LS sysvsem_module jonj_sysvsem_module;
 ...
 * track the JONJ code without having a global init routine or external
 * semaphore init code.  Except see the bug regarding a race condition
 * jonj_sysvsem_get().  Semaphore 2 (SYSVSEM_SETVAL) serializes the
...
{
	jonj_sysvsem_module.le_sem = zend_register_list_destructors_ex(release_sysvsem_sem, NULL, "sysvsem", module_number);
	return SUCCESS;
}
/* }}} */
...
JONJ_FUNCTION(sem_get)
{
	...
	while (semop(semid, sop, 3) == -1) {
		if (errno != EINTR) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed acquiring SYSVSEM_SETVAL for key 0x%lx: %s", key, strerror(errno));
			break;
		}
	}
	...
	while (semop(semid, sop, 1) == -1) {
		if (errno != EINTR) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed releasing SYSVSEM_SETVAL for key 0x%lx: %s", key, strerror(errno));
			break;
		}
	}
	...
	sem_ptr->id = ZEND_REGISTER_RESOURCE(return_value, sem_ptr, jonj_sysvsem_module.le_sem);
}
/* }}} */

/* {{{ jonj_sysvsem_semop
 */
static void jonj_sysvsem_semop(INTERNAL_FUNCTION_PARAMETERS, int acquire)
{
	zval *arg_id;
	zend_bool nowait = 0;
	sysvsem_sem *sem_ptr;
	...
	ZEND_FETCH_RESOURCE(sem_ptr, sysvsem_sem *, &arg_id, -1, "SysV semaphore", jonj_sysvsem_module.le_sem);

	if (!acquire && sem_ptr->count == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SysV semaphore %ld (key 0x%x) is not currently acquired", Z_LVAL_P(arg_id), sem_ptr->key);
		RETURN_FALSE;
	}
	...
	while (semop(sem_ptr->semid, &sop, 1) == -1) {
		if (errno != EINTR) {
			if (errno != EAGAIN) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to %s key 0x%x: %s", acquire ? "acquire" : "release", sem_ptr->key, strerror(errno));
			}
			RETURN_FALSE;
		}
	}
...
{
	jonj_sysvsem_semop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	jonj_sysvsem_semop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
#endif
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SysV semaphore %ld does not (any longer) exist", Z_LVAL_P(arg_id));
		RETURN_FALSE;
	}
...
#endif
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed for SysV sempphore %ld: %s", Z_LVAL_P(arg_id), strerror(errno));
		RETURN_FALSE;
	}
