...
#define JONJ_MSG_IPC_NOWAIT	1
#define JONJ_MSG_NOERROR		2
#define JONJ_MSG_EXCEPT		4
...
const zend_function_entry sysvmsg_functions[] = {
	JONJ_FE(msg_get_queue,				arginfo_msg_get_queue)
	JONJ_FE(msg_send,					arginfo_msg_send)
	JONJ_FE(msg_receive,					arginfo_msg_receive)
	JONJ_FE(msg_remove_queue,			arginfo_msg_remove_queue)
	JONJ_FE(msg_stat_queue,				arginfo_msg_stat_queue)
	JONJ_FE(msg_set_queue,				arginfo_msg_set_queue)
	JONJ_FE(msg_queue_exists,			arginfo_msg_queue_exists)
	JONJ_FE_END
};
/* }}} */
...
JONJ_MINIT_FUNCTION(sysvmsg)
{
	...
	REGISTER_LONG_CONSTANT("MSG_IPC_NOWAIT", JONJ_MSG_IPC_NOWAIT, CONST_PERSISTENT|CONST_CS);
	...
	REGISTER_LONG_CONSTANT("MSG_NOERROR",    JONJ_MSG_NOERROR,    CONST_PERSISTENT|CONST_CS);
	REGISTER_LONG_CONSTANT("MSG_EXCEPT",     JONJ_MSG_EXCEPT,     CONST_PERSISTENT|CONST_CS);
	return SUCCESS;
}
/* }}} */
...
JONJ_MINFO_FUNCTION(sysvmsg)
{
	...
	jonj_info_print_table_row(2, "sysvmsg support", "enabled");
...
JONJ_FUNCTION(msg_get_queue)
{
	...
	if (mq->id < 0)	{
		...
		if (mq->id < 0)	{
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed for key 0x%lx: %s", key, strerror(errno));
			efree(mq);
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(msg_receive)
{
	...
	sysvmsg_queue_t *mq = NULL;
	struct jonj_msgbuf *messagebuffer = NULL; /* buffer to transmit */
	int result;
	...
	if (maxsize <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "maximum size of the message has to be greater than zero");
		return;
	}

	if (flags != 0) {
		if (flags & JONJ_MSG_EXCEPT) {
#ifndef MSG_EXCEPT
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "MSG_EXCEPT is not supported on your system");
			RETURN_FALSE;
#else
			realflags |= MSG_EXCEPT;
#endif
		}
		if (flags & JONJ_MSG_NOERROR) {
			realflags |= MSG_NOERROR;
		}
		if (flags & JONJ_MSG_IPC_NOWAIT) {
	...
	messagebuffer = (struct jonj_msgbuf *) safe_emalloc(maxsize, 1, sizeof(struct jonj_msgbuf));
	...
	if (result >= 0) {
		...
		if (do_unserialize)	{
			...
			if (!jonj_var_unserialize(&tmp, &p, p + result, &var_hash TSRMLS_CC)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "message corrupted");
				RETVAL_FALSE;
			} else {
...
JONJ_FUNCTION(msg_send)
{
	...
	if (do_serialize) {
		...
		jonj_var_serialize(&msg_var, &message, &var_hash TSRMLS_CC);
		JONJ_VAR_SERIALIZE_DESTROY(var_hash);

		/* NB: jonj_msgbuf is 1 char bigger than a long, so there is no need to
		 * allocate the extra byte. */
		messagebuffer = safe_emalloc(msg_var.len, 1, sizeof(struct jonj_msgbuf));
		...
		switch (Z_TYPE_P(message)) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Message parameter must be either a string or a number.");
				RETURN_FALSE;
		}

		messagebuffer = safe_emalloc(message_len, 1, sizeof(struct jonj_msgbuf));
	...
	if (result == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "msgsnd failed: %s", strerror(errno));
		if (zerror) {
