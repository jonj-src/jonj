...
const zend_function_entry sysvshm_functions[] = {
	JONJ_FE(shm_attach,		arginfo_shm_attach)
	JONJ_FE(shm_remove,		arginfo_shm_detach)
	JONJ_FE(shm_detach, 		arginfo_shm_remove)
	JONJ_FE(shm_put_var,		arginfo_shm_put_var)
	JONJ_FE(shm_has_var,		arginfo_shm_has_var)
	JONJ_FE(shm_get_var,		arginfo_shm_get_var)
	JONJ_FE(shm_remove_var,	arginfo_shm_remove_var)
	JONJ_FE_END
};
/* }}} */
...
#define SHM_FETCH_RESOURCE(shm_ptr, z_ptr) ZEND_FETCH_RESOURCE(shm_ptr, sysvshm_shm *, &z_ptr, -1, JONJ_SHM_RSRC_NAME, jonj_sysvshm.le_shm)

THREAD_LS sysvshm_module jonj_sysvshm;

static int jonj_put_shm_data(sysvshm_chunk_head *ptr, long key, const char *data, long len);
static long jonj_check_shm_data(sysvshm_chunk_head *ptr, long key);
static int jonj_remove_shm_data(sysvshm_chunk_head *ptr, long shm_varpos);

/* {{{ jonj_release_sysvshm
 */
static void jonj_release_sysvshm(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
{
	jonj_sysvshm.le_shm = zend_register_list_destructors_ex(jonj_release_sysvshm, NULL, JONJ_SHM_RSRC_NAME, module_number);

	if (cfg_get_long("sysvshm.init_mem", &jonj_sysvshm.init_mem) == FAILURE) {
		jonj_sysvshm.init_mem=10000;
	}
	return SUCCESS;
}
/* }}} */
...
JONJ_FUNCTION(shm_attach)
{
	...
	long shm_key, shm_id, shm_size = jonj_sysvshm.init_mem, shm_flag = 0666;
	...
	if (shm_size < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Segment size must be greater than zero");
		RETURN_FALSE;
  	}
	...
	if ((shm_id = shmget(shm_key, 0, 0)) < 0) {
		if (shm_size < sizeof(sysvshm_chunk_head)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed for key 0x%lx: memorysize too small", shm_key);
			efree(shm_list_ptr);
			RETURN_FALSE;
		}
		if ((shm_id = shmget(shm_key, shm_size, shm_flag | IPC_CREAT | IPC_EXCL)) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed for key 0x%lx: %s", shm_key, strerror(errno));
			efree(shm_list_ptr);
			RETURN_FALSE;
		}
	}
	...
	if (strcmp((char*) &(chunk_ptr->magic), "JONJ_SM") != 0) {
		strcpy((char*) &(chunk_ptr->magic), "JONJ_SM");	
	...
	ZEND_REGISTER_RESOURCE(return_value, shm_list_ptr, jonj_sysvshm.le_shm);
}
/* }}} */
...
JONJ_FUNCTION(shm_remove)
{
	...
	if (shmctl(shm_list_ptr->id, IPC_RMID, NULL) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed for key 0x%x, id %ld: %s", shm_list_ptr->key, Z_LVAL_P(shm_id), strerror(errno));
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(shm_put_var)
{
	...
	jonj_var_serialize(&shm_var, &arg_var, &var_hash TSRMLS_CC);
	JONJ_VAR_SERIALIZE_DESTROY(var_hash);
	
	shm_list_ptr = zend_fetch_resource(&shm_id TSRMLS_CC, -1, JONJ_SHM_RSRC_NAME, NULL, 1, jonj_sysvshm.le_shm);
	if (!shm_list_ptr) {
	...
	ret = jonj_put_shm_data(shm_list_ptr->ptr, shm_key, shm_var.c, shm_var.len);

	/* free string */
	smart_str_free(&shm_var);
	
	if (ret == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "not enough shared memory left");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(shm_get_var)
{
	...
	shm_varpos = jonj_check_shm_data((shm_list_ptr->ptr), shm_key);

	if (shm_varpos < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "variable key %ld doesn't exist", shm_key);
		RETURN_FALSE;
	}
	...
	if (jonj_var_unserialize(&return_value, (const unsigned char **) &shm_data, (unsigned char *) shm_data + shm_var->length, &var_hash TSRMLS_CC) != 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "variable data in shared memory is corrupted");
		RETVAL_FALSE;
	}
...
JONJ_FUNCTION(shm_has_var)
{
	...
	SHM_FETCH_RESOURCE(shm_list_ptr, shm_id);
	RETURN_BOOL(jonj_check_shm_data(shm_list_ptr->ptr, shm_key) >= 0);
}
/* }}} */
...
JONJ_FUNCTION(shm_remove_var)
{
	...
	jonj_remove_shm_data((shm_list_ptr->ptr), shm_varpos);
	RETURN_TRUE;
}
/* }}} */
...
static int jonj_put_shm_data(sysvshm_chunk_head *ptr, long key, const char *data, long len)
{
	...
	if ((shm_varpos = jonj_check_shm_data(ptr, key)) > 0) {
		jonj_remove_shm_data(ptr, shm_varpos);
	}
