...
/* {{{ function prototypes */
JONJ_MINIT_FUNCTION(xml);
JONJ_MINFO_FUNCTION(xml);
static JONJ_GINIT_FUNCTION(xml);
...
const zend_function_entry xml_functions[] = {
	JONJ_FE(xml_parser_create,					arginfo_xml_parser_create)
	JONJ_FE(xml_parser_create_ns,				arginfo_xml_parser_create_ns)
	JONJ_FE(xml_set_object, 						arginfo_xml_set_object)
	JONJ_FE(xml_set_element_handler,				arginfo_xml_set_element_handler)
	JONJ_FE(xml_set_character_data_handler,		arginfo_xml_set_character_data_handler)
	JONJ_FE(xml_set_processing_instruction_handler, 	arginfo_xml_set_processing_instruction_handler)
	JONJ_FE(xml_set_default_handler, 				arginfo_xml_set_default_handler)
	JONJ_FE(xml_set_unparsed_entity_decl_handler,arginfo_xml_set_unparsed_entity_decl_handler)
	JONJ_FE(xml_set_notation_decl_handler,		arginfo_xml_set_notation_decl_handler)
	JONJ_FE(xml_set_external_entity_ref_handler,	arginfo_xml_set_external_entity_ref_handler)
	JONJ_FE(xml_set_start_namespace_decl_handler,arginfo_xml_set_start_namespace_decl_handler)
	JONJ_FE(xml_set_end_namespace_decl_handler,	arginfo_xml_set_end_namespace_decl_handler)
	JONJ_FE(xml_parse,							arginfo_xml_parse)
	JONJ_FE(xml_parse_into_struct, 				arginfo_xml_parse_into_struct)
	JONJ_FE(xml_get_error_code,					arginfo_xml_get_error_code)
	JONJ_FE(xml_error_string,					arginfo_xml_error_string)
	JONJ_FE(xml_get_current_line_number,			arginfo_xml_get_current_line_number)
	JONJ_FE(xml_get_current_column_number,		arginfo_xml_get_current_column_number)
	JONJ_FE(xml_get_current_byte_index,			arginfo_xml_get_current_byte_index)
	JONJ_FE(xml_parser_free, 					arginfo_xml_parser_free)
	JONJ_FE(xml_parser_set_option, 				arginfo_xml_parser_set_option)
	JONJ_FE(xml_parser_get_option,				arginfo_xml_parser_get_option)
	JONJ_FE(utf8_encode, 						arginfo_utf8_encode)
	JONJ_FE(utf8_decode, 						arginfo_utf8_decode)
	JONJ_FE_END
};
...
zend_module_entry xml_module_entry = {
	...
	JONJ_MINIT(xml),       /* extension-wide startup function */
	NULL,                 /* extension-wide shutdown function */
	NULL,                 /* per-request startup function */
	NULL,                 /* per-request shutdown function */
	JONJ_MINFO(xml),       /* information function */
    NO_VERSION_YET,
    JONJ_MODULE_GLOBALS(xml), /* globals descriptor */
    JONJ_GINIT(xml),          /* globals ctor */
    NULL,                    /* globals dtor */
...
static XML_Memory_Handling_Suite jonj_xml_mem_hdlrs;
...
static void *jonj_xml_malloc_wrapper(size_t sz)
{
	return emalloc(sz);
}

static void *jonj_xml_realloc_wrapper(void *ptr, size_t sz)
{
	return erealloc(ptr, sz);
}

static void jonj_xml_free_wrapper(void *ptr)
{
	if (ptr != NULL) {
		efree(ptr);
	}
...
JONJ_MINIT_FUNCTION(xml)
{
	...
	REGISTER_LONG_CONSTANT("XML_OPTION_CASE_FOLDING", JONJ_XML_OPTION_CASE_FOLDING, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("XML_OPTION_TARGET_ENCODING", JONJ_XML_OPTION_TARGET_ENCODING, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("XML_OPTION_SKIP_TAGSTART", JONJ_XML_OPTION_SKIP_TAGSTART, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("XML_OPTION_SKIP_WHITE", JONJ_XML_OPTION_SKIP_WHITE, CONST_CS|CONST_PERSISTENT);

	/* this object should not be pre-initialised at compile time,
	   as the order of members may vary */

	jonj_xml_mem_hdlrs.malloc_fcn = jonj_xml_malloc_wrapper;
	jonj_xml_mem_hdlrs.realloc_fcn = jonj_xml_realloc_wrapper;
	jonj_xml_mem_hdlrs.free_fcn = jonj_xml_free_wrapper;
...
JONJ_MINFO_FUNCTION(xml)
{
	...
	jonj_info_print_table_row(2, "XML Support", "active");
	jonj_info_print_table_row(2, "XML Namespace Support", "active");
#if defined(LIBXML_DOTTED_VERSION) && defined(LIBXML_EXPAT_COMPAT)
	jonj_info_print_table_row(2, "libxml2 Version", LIBXML_DOTTED_VERSION);
#else
	jonj_info_print_table_row(2, "EXPAT Version", XML_ExpatVersion());
#endif
	...
	if (parser && handler && !EG(exception)) {
		...
		if (result == FAILURE) {
			...
			if (Z_TYPE_P(handler) == IS_STRING) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call handler %s::%s()", Z_OBJCE_PP(obj)->name, Z_STRVAL_PP(method));
			} else
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call handler");
		}
...
/* {{{ xml_utf8_decode */
JONJAPI char *xml_utf8_decode(const XML_Char *s, int len, int *newlen, const XML_Char *encoding)
{
	size_t pos = 0;
	...
	*newlen = 0;
	...
	while (pos < (size_t)len) {
		int status = FAILURE;
		c = jonj_next_utf8_char((const unsigned char*)s, (size_t) len, &pos, &status);
...
static char *_xml_decode_tag(xml_parser *parser, const char *tag)
{
	...
	if (parser->case_folding) {
		jonj_strtoupper(newstr, out_len);
	}
	...
	if (parser) {
		...
		if (parser->data) {
			if (parser->level <= XML_MAXLEVEL)  {
				...
				TSRMLS_FETCH();
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Maximum depth exceeded - Results truncated");
			}
		}
...
static void jonj_xml_parser_create_impl(INTERNAL_FUNCTION_PARAMETERS, int ns_support) /* {{{ */
{
	xml_parser *parser;
	int auto_detect = 0;
	...
	if (encoding_param != NULL) {
		...
		if (encoding_param_len == 0) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unsupported source encoding \"%s\"", encoding_param);
			RETURN_FALSE;
		}
	} else {
		encoding = XML(default_encoding);
...
{
	jonj_xml_parser_create_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_xml_parser_create_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(xml_parser_free)
{
	...
	if (parser->isparsing == 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parser cannot be freed while it is parsing.");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(xml_parser_set_option)
{
	...
	switch (opt) {
		case JONJ_XML_OPTION_CASE_FOLDING:
			convert_to_long_ex(val);
			parser->case_folding = Z_LVAL_PP(val);
			break;
		case JONJ_XML_OPTION_SKIP_TAGSTART:
			convert_to_long_ex(val);
			parser->toffset = Z_LVAL_PP(val);
			if (parser->toffset < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "tagstart ignored, because it is out of range");
				parser->toffset = 0;
			}
			break;
		case JONJ_XML_OPTION_SKIP_WHITE:
			convert_to_long_ex(val);
			parser->skipwhite = Z_LVAL_PP(val);
			break;
		case JONJ_XML_OPTION_TARGET_ENCODING: {
			xml_encoding *enc;
			convert_to_string_ex(val);
			enc = xml_get_encoding(Z_STRVAL_PP(val));
			if (enc == NULL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unsupported target encoding \"%s\"", Z_STRVAL_PP(val));
				RETURN_FALSE;
			}
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown option");
			RETURN_FALSE;
			break;
	}
	RETVAL_TRUE;
