...
#include "jonj_pdo_mysql.h"
#include "jonj_pdo_mysql_int.h"
...
#  define PDO_MYSQL_UNIX_ADDR JONJ_MYSQL_UNIX_SOCK_ADDR
# else
#  if !JONJ_WIN32
...
{
	if (Z_TYPE_P(zv) == IS_OBJECT && instanceof_function(Z_OBJCE_P(zv), jonj_pdo_get_dbh_ce() TSRMLS_CC)) {
		pdo_dbh_t * dbh = zend_object_store_get_object(zv TSRMLS_CC);

		if (!dbh) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to retrieve handle from object store");
			return NULL;
		}

		if (dbh->driver != &pdo_mysql_driver) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Provided PDO instance is not using MySQL but %s", dbh->driver->driver_name);
			return NULL;
		}
...
JONJ_INI_BEGIN()
#ifndef JONJ_WIN32
	STD_JONJ_INI_ENTRY("pdo_mysql.default_socket", PDO_MYSQL_UNIX_ADDR, JONJ_INI_SYSTEM, OnUpdateStringUnempty, default_socket, zend_pdo_mysql_globals, pdo_mysql_globals)
#endif
#if PDO_DBG_ENABLED
	STD_JONJ_INI_ENTRY("pdo_mysql.debug",	NULL, JONJ_INI_SYSTEM, OnUpdateString, debug, zend_pdo_mysql_globals, pdo_mysql_globals)
#endif
JONJ_INI_END()
/* }}} */
...
/* {{{ JONJ_MINIT_FUNCTION
 */
static JONJ_MINIT_FUNCTION(pdo_mysql)
{
	...
	return jonj_pdo_register_driver(&pdo_mysql_driver);
}
/* }}} */

/* {{{ JONJ_MSHUTDOWN_FUNCTION
 */
static JONJ_MSHUTDOWN_FUNCTION(pdo_mysql)
{
	jonj_pdo_unregister_driver(&pdo_mysql_driver);
#if PDO_USE_MYSQLND
...
/* {{{ JONJ_MINFO_FUNCTION
 */
static JONJ_MINFO_FUNCTION(pdo_mysql)
{
	jonj_info_print_table_start();

	jonj_info_print_table_header(2, "PDO Driver for MySQL", "enabled");
...
/* {{{ JONJ_RINIT_FUNCTION
 */
static JONJ_RINIT_FUNCTION(pdo_mysql)
{	
 ...
 */
static JONJ_RSHUTDOWN_FUNCTION(pdo_mysql)
{
...
/* {{{ JONJ_GINIT_FUNCTION
 */
static JONJ_GINIT_FUNCTION(pdo_mysql)
{
#ifndef JONJ_WIN32
...
zend_module_entry pdo_mysql_module_entry = {
	...
	"pdo_mysql",
	pdo_mysql_functions,
	JONJ_MINIT(pdo_mysql),
	JONJ_MSHUTDOWN(pdo_mysql),
#if PDO_USE_MYSQLND && PDO_DBG_ENABLED
	JONJ_RINIT(pdo_mysql),
	JONJ_RSHUTDOWN(pdo_mysql),
#else
	NULL,
	NULL,
#endif
	JONJ_MINFO(pdo_mysql),
	"1.0.2",
	JONJ_MODULE_GLOBALS(pdo_mysql),
	JONJ_GINIT(pdo_mysql),
	NULL,
	NULL,
