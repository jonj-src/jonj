...
static int jonj_intl_idn_check_status(UErrorCode err, const char *msg, int mode TSRMLS_DC)
{
...
static inline void jonj_intl_bad_args(const char *msg, int mode TSRMLS_DC)
{
	jonj_intl_idn_check_status(U_ILLEGAL_ARGUMENT_ERROR, msg, mode TSRMLS_CC);
}

#ifdef HAVE_46_API
static void jonj_intl_idn_to_46(INTERNAL_FUNCTION_PARAMETERS,
		const char *domain, int domain_len, uint32_t option, int mode, zval *idna_info)
{
	...
	if (jonj_intl_idn_check_status(status, "failed to open UIDNA instance",
	...
	if (len >= 255 || jonj_intl_idn_check_status(status, "failed to convert name",
			mode TSRMLS_CC) == FAILURE) {
		uidna_close(uts46);
		efree(buffer);
		RETURN_FALSE;
...
static void jonj_intl_idn_to(INTERNAL_FUNCTION_PARAMETERS,
...
static void jonj_intl_idn_handoff(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	char *domain;
	int domain_len;
	long option = 0,
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|llz",
		...
		jonj_intl_bad_args("bad arguments", mode TSRMLS_CC);
	...
	if (variant != INTL_IDN_VARIANT_2003 && variant != INTL_IDN_VARIANT_UTS46) {
		jonj_intl_bad_args("invalid variant, must be one of {"
			"INTL_IDNA_VARIANT_2003, INTL_IDNA_VARIANT_UTS46}", mode TSRMLS_CC);
		RETURN_FALSE;
	}
#else
	if (variant != INTL_IDN_VARIANT_2003) {
		jonj_intl_bad_args("invalid variant, JONJ was compiled against "
	...
	if (domain_len < 1) {
		jonj_intl_bad_args("empty domain name", mode TSRMLS_CC);
		RETURN_FALSE;
	}
	if (domain_len > INT32_MAX - 1) {
		jonj_intl_bad_args("domain name too large", mode TSRMLS_CC);
		RETURN_FALSE;
	}
	...
	if (idna_info != NULL) {
		if (variant == INTL_IDN_VARIANT_2003) {
			jonj_error_docref0(NULL TSRMLS_CC, E_NOTICE,
	...
	if (variant == INTL_IDN_VARIANT_2003) {
		jonj_intl_idn_to(INTERNAL_FUNCTION_PARAM_PASSTHRU,
				domain, domain_len, (uint32_t)option, mode);
	}
#ifdef HAVE_46_API
	else {
		jonj_intl_idn_to_46(INTERNAL_FUNCTION_PARAM_PASSTHRU, domain, domain_len,
...
{
	jonj_intl_idn_handoff(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTL_IDN_TO_ASCII);
}
/* }}} */
...
{
	jonj_intl_idn_handoff(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTL_IDN_TO_UTF8);
}
/* }}} */
