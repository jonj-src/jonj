...
static void _jonj_intlgregcal_constructor_body(INTERNAL_FUNCTION_PARAMETERS)
{
	zval		**tz_object	= NULL;
	zval		**args_a[6] = {0},
				***args		= &args_a[0];
	...
	if (variant <= 2) {
		...
		if (tzstr.isBogus()) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
				...
				"from JONJ's default timezone name (see date_default_timezone_get())",
				0 TSRMLS_CC);
			delete gcal;
			RETURN_NULL();
		}
...
U_CFUNC JONJ_FUNCTION(intlgregcal_create_instance)
{
	zval orig;
	intl_error_reset(NULL TSRMLS_CC);
	...
	_jonj_intlgregcal_constructor_body(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
U_CFUNC JONJ_METHOD(IntlGregorianCalendar, __construct)
{
...
U_CFUNC JONJ_FUNCTION(intlgregcal_set_gregorian_change)
{
	double date;
...
U_CFUNC JONJ_FUNCTION(intlgregcal_get_gregorian_change)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC JONJ_FUNCTION(intlgregcal_is_leap_year)
{
	long year;
	CALENDAR_METHOD_INIT_VARS;
