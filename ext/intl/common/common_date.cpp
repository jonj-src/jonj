												...
												const char *func TSRMLS_DC)
{
	...
	switch (type) {
		case TIMELIB_ZONETYPE_ID:
			id = is_datetime
				? ((jonj_date_obj*)object)->time->tz_info->name
				: ((jonj_timezone_obj*)object)->tzi.tz->name;
			id_len = strlen(id);
			break;
		case TIMELIB_ZONETYPE_OFFSET: {
			int offset_mins = is_datetime
				? -((jonj_date_obj*)object)->time->z
				: -(int)((jonj_timezone_obj*)object)->tzi.utc_offset,
		...
		case TIMELIB_ZONETYPE_ABBR:
			id = is_datetime
				? ((jonj_date_obj*)object)->time->tz_abbr
				: ((jonj_timezone_obj*)object)->tzi.z.abbr;
			id_len = strlen(id);
			break;
	}
		...
		intl_error *err, const char *func TSRMLS_DC)
{
	...
	if (tz) {
		...
		datetime = (jonj_date_obj*)zend_object_store_get_object(z TSRMLS_CC);
		if (!datetime->time) {
...
U_CFUNC double intl_zval_to_millis(zval *z, intl_error *err, const char *func TSRMLS_DC)
{
	...
	case IS_DOUBLE:
		...
		break;
	case IS_OBJECT:
		if (instanceof_function(Z_OBJCE_P(z), jonj_date_get_date_ce() TSRMLS_CC)) {
		...
		break;
	default:
		spprintf(&message, 0, "%s: invalid JONJ type for date", func);
		intl_errors_set(err, U_ILLEGAL_ARGUMENT_ERROR,
			message, 1 TSRMLS_CC);
		efree(message);
		break;
