...
JONJ_FUNCTION(grapheme_strpos)
{
	...
	*/
	found = (unsigned char *)jonj_memnstr((char *)haystack + noffset, (char *)needle, needle_len, (char *)haystack + haystack_len);
...
JONJ_FUNCTION(grapheme_stripos)
{
	...
	if ( is_ascii ) {
		...
		jonj_strtolower((char *)needle_dup, needle_len);
		haystack_dup = (unsigned char *)estrndup((char *)haystack, haystack_len);
		jonj_strtolower((char *)haystack_dup, haystack_len);

		found = (unsigned char*) jonj_memnstr((char *)haystack_dup + noffset, (char *)needle_dup, needle_len, (char *)haystack_dup + haystack_len);
...
static void strstr_common_handler(INTERNAL_FUNCTION_PARAMETERS, int f_ignore_case)
{
	...
	if ( !f_ignore_case ) {
		 ...
		 * I realize that 'offset' is 'grapheme count offset' but will work in spite of that
		*/
		found = (unsigned char *)jonj_memnstr((char *)haystack, (char *)needle, needle_len, (char *)haystack + haystack_len);
