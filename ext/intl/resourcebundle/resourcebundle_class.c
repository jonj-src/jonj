 ...
 */
JONJ_METHOD( ResourceBundle, __construct )
{
...
zval *resourcebundle_array_get(zval *object, zval *offset, int type TSRMLS_DC)
{
	...
	if(offset == NULL) {
		jonj_error( E_ERROR, "Cannot apply [] to ResourceBundle object" );
	}
...
static zend_function_entry ResourceBundle_class_functions[] = {
	JONJ_ME( ResourceBundle, __construct, arginfo_resourcebundle___construct, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR )
