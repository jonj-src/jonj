...
err:
			...
			jonj_error_docref( NULL TSRMLS_CC, E_ERROR, "%s", err_msg );
...
#endif
{
	...
	if( ( type != BP_VAR_R && type != BP_VAR_IS ) &&
		...
		Z_STRVAL_P( member ), Z_STRLEN_P( member ) ) == 0 ) )
	{
		jonj_error_docref0( NULL TSRMLS_CC, E_WARNING, "The property \"id\" is read-only" );
...
zend_function_entry Transliterator_class_functions[] = {
	JONJ_ME( Transliterator,			__construct,						ainfo_trans_void,				ZEND_ACC_PRIVATE | ZEND_ACC_CTOR | ZEND_ACC_FINAL )
	JONJ_ME_MAPPING( create,			transliterator_create,				ainfo_trans_create,				ZEND_ACC_STATIC |ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( createFromRules,transliterator_create_from_rules,	ainfo_trans_create_from_rules,	ZEND_ACC_STATIC | ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( createInverse,	transliterator_create_inverse,		ainfo_trans_void,				ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( listIDs,		transliterator_list_ids,			ainfo_trans_void,				ZEND_ACC_STATIC | ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( transliterate,	transliterator_transliterate,		ainfo_trans_me_transliterate,	ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( getErrorCode,	transliterator_get_error_code,		ainfo_trans_void,				ZEND_ACC_PUBLIC )
	JONJ_ME_MAPPING( getErrorMessage,transliterator_get_error_message,	ainfo_trans_void,				ZEND_ACC_PUBLIC )
	JONJ_FE_END
};
/* }}} */
