								...
								int *formatted_len TSRMLS_DC)
{
	...
	string_arg:
					 ...
					 * and JONJ ends up with a fatal error and calls longjmp
					 * as a result of that.
					 */
...
retry_klong:
					if (Z_TYPE_PP(elem) == IS_DOUBLE) {
						if (Z_DVAL_PP(elem) > (double)INT32_MAX ||
							...
							intl_errors_set(&err, U_ILLEGAL_ARGUMENT_ERROR,
								"Found JONJ float with absolute value too large for "
						...
						if (Z_LVAL_PP(elem) > INT32_MAX ||
							...
							intl_errors_set(&err, U_ILLEGAL_ARGUMENT_ERROR,
								"Found JONJ integer with absolute value too large "
