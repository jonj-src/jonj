...
zval* collator_convert_zstr_utf16_to_utf8( zval* utf16_zval )
{
	...
	if( U_FAILURE( status ) )
		jonj_error( E_WARNING, "Error converting utf16 to utf8 in collator_convert_zval_utf16_to_utf8()" );
...
zval* collator_convert_zstr_utf8_to_utf16( zval* utf8_zval )
{
	...
	intl_convert_utf8_to_utf16(
			...
			&status );
	if( U_FAILURE( status ) )
		jonj_error( E_WARNING, "Error casting object to string in collator_convert_zstr_utf8_to_utf16()" );
...
zval* collator_convert_object_to_string( zval* obj TSRMLS_DC )
{
	...
	intl_convert_utf8_to_utf16(
			...
			&status );
	if( U_FAILURE( status ) )
		jonj_error( E_WARNING, "Error casting object to string in collator_convert_object_to_string()" );
