...
static int collator_regular_compare_function(zval *result, zval *op1, zval *op2 TSRMLS_DC)
{
	 ...
	 * then use ICU-compare. Otherwise JONJ-compare. */
	...
	else
	{
		...
		if( num1 )
		{
			...
			{
				/* str1 is numeric strings => passthru to JONJ-compare. */
				zval_add_ref( &num1 );
				norm1 = num1;

				/* str2 is numeric strings => passthru to JONJ-compare. */
				zval_add_ref( &num2 );
				norm2 = num2;
			}
		}
 ...
 * Taken from JONJ5 source (array_data_compare).
 */
 ...
 * Equivalent to standard JONJ sort using Collator.
