...
inline BreakIterator *_breakiter_prolog(zend_object_iterator *iter TSRMLS_DC)
{
	...
	if (bio->biter == NULL) {
		intl_errors_set(BREAKITER_ERROR_P(bio), U_INVALID_STATE_ERROR,
			"The BreakIterator object backing the JONJ iterator is not "
...
U_CFUNC JONJ_METHOD(IntlPartsIterator, getBreakIterator)
{
...
static const zend_function_entry IntlPartsIterator_class_functions[] = {
	JONJ_ME(IntlPartsIterator,	getBreakIterator,	ainfo_parts_it_void,	ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
