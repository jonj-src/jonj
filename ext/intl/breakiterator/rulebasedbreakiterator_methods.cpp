...
static void _jonj_intlrbbi_constructor_body(INTERNAL_FUNCTION_PARAMETERS)
{
	zval		*object		= getThis();
	char		*rules;
	int			rules_len;
...
U_CFUNC JONJ_METHOD(IntlRuleBasedBreakIterator, __construct)
{
	zval	orig_this		= *getThis();

	return_value = getThis();
	//changes this to IS_NULL (without first destroying) if there's an error
	_jonj_intlrbbi_constructor_body(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
U_CFUNC JONJ_FUNCTION(rbbi_get_rules)
{
...
U_CFUNC JONJ_FUNCTION(rbbi_get_rule_status)
{
	BREAKITER_METHOD_INIT_VARS;
	object = getThis();
...
U_CFUNC JONJ_FUNCTION(rbbi_get_rule_status_vec)
{
	BREAKITER_METHOD_INIT_VARS;
	object = getThis();
...
#if U_ICU_VERSION_MAJOR_NUM * 10 + U_ICU_VERSION_MINOR_NUM >= 48
U_CFUNC JONJ_FUNCTION(rbbi_get_binary_rules)
{
	BREAKITER_METHOD_INIT_VARS;
	object = getThis();
