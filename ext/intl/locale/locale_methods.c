...
JONJ_NAMED_FUNCTION(zif_locale_set_default)
{
	...
	zend_alter_ini_entry(LOCALE_INI_NAME, sizeof(LOCALE_INI_NAME), locale_name, len, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME);
...
/* {{{
* Gets the value from ICU , called when JONJ userspace function is called
* common code shared by get_primary_language,get_script or get_region or get_variant
*/
static void get_icu_value_src_jonj( char* tag_name, INTERNAL_FUNCTION_PARAMETERS)
{
...
{
	get_icu_value_src_jonj( LOC_SCRIPT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
	get_icu_value_src_jonj( LOC_REGION_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
	get_icu_value_src_jonj( LOC_LANG_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
static void get_icu_disp_value_src_jonj( char* tag_name, INTERNAL_FUNCTION_PARAMETERS)
{
	const char* loc_name        	= NULL;
	int         loc_name_len    	= 0;
...
{
    get_icu_disp_value_src_jonj( DISP_NAME , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
    get_icu_disp_value_src_jonj( LOC_LANG_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
    get_icu_disp_value_src_jonj( LOC_SCRIPT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
    get_icu_disp_value_src_jonj( LOC_REGION_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
    get_icu_disp_value_src_jonj( LOC_VARIANT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
{
	get_icu_value_src_jonj( LOC_CANONICALIZE_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
static int add_array_entry(const char* loc_name, zval* hash_arr, char* key_name TSRMLS_DC)
{
	...
	if( (strcmp(key_name , LOC_PRIVATE_TAG)==0) ||
		...
		if( result > 0 && key_value){
			...
			token = jonj_strtok_r( key_value , DELIMITER ,&last_ptr);
			if( cur_key_name ){
				efree( cur_key_name);
			}
			...
			while( (token = jonj_strtok_r(NULL , DELIMITER , &last_ptr)) && (strlen(token)>1) ){
				sprintf( cur_key_name , "%s%d", key_name , cnt++);
				add_assoc_string( hash_arr, cur_key_name , token , TRUE );
			}
/*
...
JONJ_FUNCTION(locale_get_all_variants)
{
	...
	/* Call ICU variant */
		...
		if( result > 0 && variant){
			/* Tokenize on the "_" or "-" */
			token = jonj_strtok_r( variant , DELIMITER , &saved_ptr);
			add_next_index_stringl( return_value, token , strlen(token) ,TRUE );
			/* tokenize on the "_" or "-" and stop  at singleton if any	*/
			while( (token = jonj_strtok_r(NULL , DELIMITER, &saved_ptr)) && (strlen(token)>1) ){
 				add_next_index_stringl( return_value, token , strlen(token) ,TRUE );
			}
		}
		if( variant ){
...
/* {{{
* returns the lookup result to lookup_loc_range_src_jonj
* internal function
*/
