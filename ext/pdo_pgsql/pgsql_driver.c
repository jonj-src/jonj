...
static char * _pdo_pgsql_escape_credentials(char *str TSRMLS_DC)
{
	...
	if (str) {
		return jonj_addcslashes(str, strlen(str), &len, 0, "\\'", sizeof("\\'") TSRMLS_CC);
	}
...
int _pdo_pgsql_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, int errcode, const char *sqlstate, const char *msg, const char *file, int line TSRMLS_DC) /* {{{ */
{
	...
	if (!dbh->methods) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), einfo->errcode TSRMLS_CC, "SQLSTATE[%s] [%d] %s",
...
static size_t pgsql_lob_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	struct pdo_pgsql_lob_self *self = (struct pdo_pgsql_lob_self*)stream->abstract;
	return lo_write(self->conn, self->lfd, (char*)buf, count);
}

static size_t pgsql_lob_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	struct pdo_pgsql_lob_self *self = (struct pdo_pgsql_lob_self*)stream->abstract;
	return lo_read(self->conn, self->lfd, buf, count);
}

static int pgsql_lob_close(jonj_stream *stream, int close_handle TSRMLS_DC)
{
	struct pdo_pgsql_lob_self *self = (struct pdo_pgsql_lob_self*)stream->abstract;
	...
	efree(self);
	jonj_pdo_dbh_delref(dbh TSRMLS_CC);
	return 0;
}

static int pgsql_lob_flush(jonj_stream *stream TSRMLS_DC)
{
	return 0;
}

static int pgsql_lob_seek(jonj_stream *stream, off_t offset, int whence,
...
jonj_stream_ops pdo_pgsql_lob_stream_ops = {
	pgsql_lob_write,
	pgsql_lob_read,
	pgsql_lob_close,
	pgsql_lob_flush,
...
jonj_stream *pdo_pgsql_create_lob_stream(pdo_dbh_t *dbh, int lfd, Oid oid TSRMLS_DC)
{
	jonj_stream *stm;
	...
	stm = jonj_stream_alloc(&pdo_pgsql_lob_stream_ops, self, 0, "r+b");
...
static int pgsql_handle_preparer(pdo_dbh_t *dbh, const char *sql, long sql_len, pdo_stmt_t *stmt, zval *driver_options TSRMLS_DC)
{
	...
	if (scrollable) {
		...
		if (pdo_attr_lval(driver_options, PDO_PGSQL_ATTR_DISABLE_NATIVE_PREPARED_STATEMENT, H->disable_native_prepares TSRMLS_CC) == 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_DEPRECATED, "PDO::PGSQL_ATTR_DISABLE_NATIVE_PREPARED_STATEMENT is deprecated, use PDO::ATTR_EMULATE_PREPARES instead");
			emulate = 1;
		}
...
static char *pdo_pgsql_last_insert_id(pdo_dbh_t *dbh, const char *name, unsigned int *len TSRMLS_DC)
{
	...
	if (name == NULL) {
		...
		if (savepoint) {
			...
			(void)PQexec(H->server, "SAVEPOINT _jonj_lastid_savepoint");
		}
	...
	if (res && (status == PGRES_TUPLES_OK)) {
		...
		*len = PQgetlength(res, 0, 0);
		...
		if (savepoint) {
			(void)PQexec(H->server, "ROLLBACK TO SAVEPOINT _jonj_lastid_savepoint");
		}
	...
	if (savepoint) {
		(void)PQexec(H->server, "RELEASE SAVEPOINT _jonj_lastid_savepoint");
	}
...
static JONJ_METHOD(PDO, pgsqlCopyFromArray)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	...
	if (!zend_hash_num_elements(Z_ARRVAL_P(pg_rows))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot copy from an empty array");
		RETURN_FALSE;
	}
...
/* {{{ proto string PDO::pgsqlCopyFromFile(string $table_name , string $filename [, string $delimiter [, string $null_as ] [, string $fields])
   Returns true if the copy worked fine or false if error */
static JONJ_METHOD(PDO, pgsqlCopyFromFile)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	...
	stream = jonj_stream_open_wrapper_ex(filename, "rb", ENFORCE_SAFE_MODE, NULL, FG(default_context));
	if (!stream) {
	...
	if (status == PGRES_COPY_IN && pgsql_result) {
		...
		PQclear(pgsql_result);
		while ((buf = jonj_stream_get_line(stream, NULL, 0, &line_len)) != NULL) {
...
/* {{{ proto string PDO::pgsqlCopyToFile(string $table_name , $filename, [string $delimiter [, string $null_as [, string $fields]]])
   Returns true if the copy worked fine or false if error */
static JONJ_METHOD(PDO, pgsqlCopyToFile)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	...
	stream = jonj_stream_open_wrapper_ex(filename, "wb", ENFORCE_SAFE_MODE, NULL, FG(default_context));
	if (!stream) {
	...
	if (status == PGRES_COPY_OUT && pgsql_result) {
		...
		while (1) {
			...
			if (ret == -1) {
				...
				if (jonj_stream_write(stream, csv, ret) != ret) {
...
/* {{{ proto string PDO::pgsqlCopyToArray(string $table_name , [string $delimiter [, string $null_as [, string $fields]]])
   Returns true if the copy worked fine or false if error */
static JONJ_METHOD(PDO, pgsqlCopyToArray)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
...
static JONJ_METHOD(PDO, pgsqlLOBCreate)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	Oid lfd;
...
static JONJ_METHOD(PDO, pgsqlLOBOpen)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	Oid oid;
	...
	if (lfd >= 0) {
		jonj_stream *stream = pdo_pgsql_create_lob_stream(dbh, lfd, oid TSRMLS_CC);
		if (stream) {
...
static JONJ_METHOD(PDO, pgsqlLOBUnlink)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	Oid oid;
...
static JONJ_METHOD(PDO, pgsqlGetNotify)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
	...
	if (ms_timeout < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid timeout");
 		RETURN_FALSE;
	}
	...
	if (ms_timeout && !pgsql_notify) {
		jonj_pollfd_for_ms(PQsocket(H->server), JONJ_POLLREADABLE, ms_timeout);
...
static JONJ_METHOD(PDO, pgsqlGetPid)
{
	pdo_dbh_t *dbh;
	pdo_pgsql_db_handle *H;
...
static const zend_function_entry dbh_methods[] = {
	JONJ_ME(PDO, pgsqlLOBCreate, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlLOBOpen, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlLOBUnlink, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlCopyFromArray, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlCopyFromFile, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlCopyToArray, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, pgsqlCopyToFile, NULL, ZEND_ACC_PUBLIC)
        JONJ_ME(PDO, pgsqlGetNotify, NULL, ZEND_ACC_PUBLIC)
        JONJ_ME(PDO, pgsqlGetPid, NULL, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
...
static int pdo_pgsql_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC) /* {{{ */
{
	...
	if (PQstatus(H->server) != CONNECTION_OK) {
		pdo_pgsql_error(dbh, PGRES_FATAL_ERROR, JONJ_PDO_PGSQL_CONNECTION_FAILURE_SQLSTATE);
		goto cleanup;
	}
