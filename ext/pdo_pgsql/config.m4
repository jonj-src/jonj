...
JONJ_ARG_WITH(pdo-pgsql,for PostgreSQL support for PDO,
[  --with-pdo-pgsql[=DIR]    PDO: PostgreSQL support.  DIR is the PostgreSQL base
                          install directory or the path to pg_config])

if test "$JONJ_PDO_PGSQL" != "no"; then

  if test "$JONJ_PDO" = "no" && test "$ext_shared" = "no"; then
    AC_MSG_ERROR([PDO is not enabled! Add --enable-pdo to your configure line.])
  fi

  JONJ_EXPAND_PATH($PGSQL_INCLUDE, PGSQL_INCLUDE)

  AC_MSG_CHECKING(for pg_config)
  for i in $JONJ_PDO_PGSQL $JONJ_PDO_PGSQL/bin /usr/local/pgsql/bin /usr/local/bin /usr/bin ""; do
  ...
  if test -n "$PG_CONFIG"; then
    ...
    if test "$JONJ_PDO_PGSQL" = "yes"; then
      PGSQL_SEARCH_PATHS="/usr /usr/local /usr/local/pgsql"
    else
      PGSQL_SEARCH_PATHS=$JONJ_PDO_PGSQL
    fi
    ...
    for i in $PGSQL_SEARCH_PATHS; do
      ...
      for j in $JONJ_LIBDIR $JONJ_LIBDIR/pgsql $JONJ_LIBDIR/postgres $JONJ_LIBDIR/postgresql ""; do
  ...
  JONJ_ADD_LIBRARY_WITH_PATH(pq, $PGSQL_LIBDIR, PDO_PGSQL_SHARED_LIBADD)
  JONJ_SUBST(PDO_PGSQL_SHARED_LIBADD)

  JONJ_ADD_INCLUDE($PGSQL_INCLUDE)
  ...
  JONJ_NEW_EXTENSION(pdo_pgsql, pdo_pgsql.c pgsql_driver.c pgsql_statement.c, $ext_shared,,-I$pdo_cv_inc_path $PDO_PGSQL_CFLAGS)
  ifdef([JONJ_ADD_EXTENSION_DEP],
  [
    JONJ_ADD_EXTENSION_DEP(pdo_pgsql, pdo)
  ])
fi
