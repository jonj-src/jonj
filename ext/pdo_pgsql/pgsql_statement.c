...
stmt_retry:
			...
			switch (status) {
				...
				default: {
					 ...
					 * jonj disconnects. if jonj dies (no chance to run RSHUTDOWN) during execution it has no
			...
			case PDO_PARAM_EVT_EXEC_PRE:
				...
				if (param->paramno >= 0) {
					...
					if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_LOB &&
						...
						if (stm) {
							if (jonj_stream_is(stm, &pdo_pgsql_lob_stream_ops)) {
								...
								if ((len = jonj_stream_copy_to_mem(stm, &Z_STRVAL_P(param->parameter), JONJ_STREAM_COPY_ALL, 0)) > 0) {
				...
				if (S->cols[colno].pgsql_type == OIDOID) {
					...
					*len = 0;
					...
					if (!tmp_len) {
						...
						*ptr = (char *)jonj_stream_memory_open(TEMP_STREAM_READONLY, "", 0);
						PQfreemem(tmp_ptr);
						*len = 0;
					} else {
