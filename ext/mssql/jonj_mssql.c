...
static void jonj_mssql_get_column_content_with_type(mssql_link *mssql_ptr,int offset,zval *result, int column_type TSRMLS_DC);
static void jonj_mssql_get_column_content_without_type(mssql_link *mssql_ptr,int offset,zval *result, int column_type TSRMLS_DC);
...
const zend_function_entry mssql_functions[] = {
	JONJ_FE(mssql_connect,				arginfo_mssql_connect)
	JONJ_FE(mssql_pconnect,				arginfo_mssql_connect)
	JONJ_FE(mssql_close,					arginfo_mssql_close)
	JONJ_FE(mssql_select_db,				arginfo_mssql_select_db)
	JONJ_FE(mssql_query,					arginfo_mssql_query)
	JONJ_FE(mssql_fetch_batch,			arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_rows_affected,			arginfo_mssql_rows_affected)
	JONJ_FE(mssql_free_result,			arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_get_last_message,		arginfo_mssql_get_last_message)
	JONJ_FE(mssql_num_rows,				arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_num_fields,			arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_fetch_field,			arginfo_mssql_fetch_field)
	JONJ_FE(mssql_fetch_row,				arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_fetch_array,			arginfo_mssql_fetch_array)
	JONJ_FE(mssql_fetch_assoc,			arginfo_mssql_fetch_assoc)
	JONJ_FE(mssql_fetch_object,			arginfo_mssql_fetch_batch)
	JONJ_FE(mssql_field_length,			arginfo_mssql_field_length)
	JONJ_FE(mssql_field_name,			arginfo_mssql_field_length)
	JONJ_FE(mssql_field_type,			arginfo_mssql_field_length)
	JONJ_FE(mssql_data_seek,				arginfo_mssql_data_seek)
	JONJ_FE(mssql_field_seek,			arginfo_mssql_fetch_field)
	JONJ_FE(mssql_result,				arginfo_mssql_result)
	JONJ_FE(mssql_next_result,			arginfo_mssql_fetch_assoc)
	JONJ_FE(mssql_min_error_severity,	arginfo_mssql_min_error_severity)
	JONJ_FE(mssql_min_message_severity,	arginfo_mssql_min_error_severity)
 	JONJ_FE(mssql_init,					arginfo_mssql_init)
 	JONJ_FE(mssql_bind,					arginfo_mssql_bind)
 	JONJ_FE(mssql_execute,				arginfo_mssql_execute)
	JONJ_FE(mssql_free_statement,		arginfo_mssql_free_statement)
 	JONJ_FE(mssql_guid_string,			arginfo_mssql_guid_string)
	JONJ_FE_END
};
/* }}} */

ZEND_DECLARE_MODULE_GLOBALS(mssql)
static JONJ_GINIT_FUNCTION(mssql);
...
zend_module_entry mssql_module_entry = 
{
	...
	mssql_functions, 
	JONJ_MINIT(mssql), 
	JONJ_MSHUTDOWN(mssql), 
	JONJ_RINIT(mssql), 
	JONJ_RSHUTDOWN(mssql), 
	JONJ_MINFO(mssql), 
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(mssql),
	JONJ_GINIT(mssql),
	NULL,
	NULL,
...
/* {{{ JONJ_INI_DISP
*/
static JONJ_INI_DISP(display_text_size)
{
	char *value;
	TSRMLS_FETCH();
	
    if (type == JONJ_INI_DISPLAY_ORIG && ini_entry->modified) {
...
*/
JONJ_INI_BEGIN()
	STD_JONJ_INI_BOOLEAN("mssql.allow_persistent",		"1",	JONJ_INI_SYSTEM,	OnUpdateBool,	allow_persistent,			zend_mssql_globals,		mssql_globals)
	STD_JONJ_INI_ENTRY_EX("mssql.max_persistent",		"-1",	JONJ_INI_SYSTEM,	OnUpdateLong,	max_persistent,				zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mssql.max_links",				"-1",	JONJ_INI_SYSTEM,	OnUpdateLong,	max_links,					zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mssql.min_error_severity",	"10",	JONJ_INI_ALL,	OnUpdateLong,	cfg_min_error_severity,		zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mssql.min_message_severity",	"10",	JONJ_INI_ALL,	OnUpdateLong,	cfg_min_message_severity,	zend_mssql_globals,		mssql_globals,	display_link_numbers)
	/*
	...
	*/
	STD_JONJ_INI_BOOLEAN("mssql.compatability_mode",		"0",	JONJ_INI_ALL,	OnUpdateBool,	compatibility_mode,			zend_mssql_globals,		mssql_globals)
	STD_JONJ_INI_BOOLEAN("mssql.compatibility_mode",		"0",	JONJ_INI_ALL,	OnUpdateBool,	compatibility_mode,			zend_mssql_globals,		mssql_globals)
	STD_JONJ_INI_ENTRY_EX("mssql.connect_timeout",    	"5",	JONJ_INI_ALL,	OnUpdateLong,	connect_timeout,			zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mssql.timeout",      			"60",	JONJ_INI_ALL,	OnUpdateLong,	timeout,					zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("mssql.textsize",   			"-1",	JONJ_INI_ALL,	OnUpdateLong,	textsize,					zend_mssql_globals,		mssql_globals,	display_text_size)
	STD_JONJ_INI_ENTRY_EX("mssql.textlimit",   			"-1",	JONJ_INI_ALL,	OnUpdateLong,	textlimit,					zend_mssql_globals,		mssql_globals,	display_text_size)
	STD_JONJ_INI_ENTRY_EX("mssql.batchsize",   			"0",	JONJ_INI_ALL,	OnUpdateLong,	batchsize,					zend_mssql_globals,		mssql_globals,	display_link_numbers)
	STD_JONJ_INI_BOOLEAN("mssql.datetimeconvert",  		"1",	JONJ_INI_ALL,	OnUpdateBool,	datetimeconvert,			zend_mssql_globals,		mssql_globals)
	STD_JONJ_INI_BOOLEAN("mssql.secure_connection",		"0",	JONJ_INI_SYSTEM, OnUpdateBool,	secure_connection,			zend_mssql_globals,		mssql_globals)
	STD_JONJ_INI_ENTRY_EX("mssql.max_procs",				"-1",	JONJ_INI_ALL,	OnUpdateLong,	max_procs,					zend_mssql_globals,		mssql_globals,	display_link_numbers)
#ifdef HAVE_FREETDS
	STD_JONJ_INI_ENTRY("mssql.charset",					"",		JONJ_INI_ALL,	OnUpdateString,	charset,					zend_mssql_globals,		mssql_globals)
#endif
JONJ_INI_END()
/* }}} */

/* error handler */
static int jonj_mssql_error_handler(DBPROCESS *dbproc, int severity, int dberr, int oserr, char *dberrstr, char *oserrstr)
{
	TSRMLS_FETCH();

	if (severity >= MS_SQL_G(min_error_severity)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s (severity %d)", dberrstr, severity);
	}
	return INT_CANCEL;  
}

/* {{{ jonj_mssql_message_handler
*/
/* message handler */
static int jonj_mssql_message_handler(DBPROCESS *dbproc, DBINT msgno,int msgstate, int severity,char *msgtext,char *srvname, char *procname,DBUSMALLINT line)
{
	TSRMLS_FETCH();

	if (severity >= MS_SQL_G(min_message_severity)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "message: %s (severity %d)", msgtext, severity);
	}
...
/* {{{ jonj_mssql_set_defaullt_link
*/
static void jonj_mssql_set_default_link(int id TSRMLS_DC)
{
...
static JONJ_GINIT_FUNCTION(mssql)
{
	...
	mssql_globals->get_column_content = jonj_mssql_get_column_content_with_type;
	if (cfg_get_long("mssql.compatibility_mode", &compatibility_mode) == SUCCESS) {
		if (compatibility_mode) {
			mssql_globals->get_column_content = jonj_mssql_get_column_content_without_type;	
		}
	}
}
/* }}} */
...
JONJ_RINIT_FUNCTION(mssql)
{
	...
	MS_SQL_G(appname) = estrndup("JONJ 5", 5);
	MS_SQL_G(server_message) = NULL;
...
JONJ_MINFO_FUNCTION(mssql)
{
	...
	jonj_info_print_table_header(2, "MSSQL Support", "enabled");
	...
	jonj_info_print_table_row(2, "Library version", MSSQL_VERSION);
...
/* {{{ jonj_mssql_do_connect
*/
static void jonj_mssql_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	if (hashed_details == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Out of memory");
		RETURN_FALSE;
	}
	...
	if ((mssql.login = dblogin()) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to allocate login record");
		RETURN_FALSE;
	}
	
	DBERRHANDLE(mssql.login, (EHANDLEFUNC) jonj_mssql_error_handler);
	DBMSGHANDLE(mssql.login, (MHANDLEFUNC) jonj_mssql_message_handler);
...
/*	DBSETLTIME(mssql.login, TIMEOUT_INFINITE); */
	...
	if (persistent) {
		...
		if (new_link || zend_hash_find(&EG(persistent_list), hashed_details, hashed_details_length + 1, (void **) &le)==FAILURE) {  /* we don't */
			...
			if (MS_SQL_G(max_links) != -1 && MS_SQL_G(num_links) >= MS_SQL_G(max_links)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open links (%ld)", MS_SQL_G(num_links));
				efree(hashed_details);
				dbfreelogin(mssql.login);
				RETURN_FALSE;
			}
			if (MS_SQL_G(max_persistent) != -1 && MS_SQL_G(num_persistent) >= MS_SQL_G(max_persistent)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open persistent links (%ld)", MS_SQL_G(num_persistent));
				efree(hashed_details);
				dbfreelogin(mssql.login);
				RETURN_FALSE;
			}
			...
			if ((mssql.link = dbopen(mssql.login, host)) == FAIL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to connect to server: %s", (host == NULL ? "" : host));
				efree(hashed_details);
				dbfreelogin(mssql.login);
				RETURN_FALSE;
			}
			...
			if (Z_TYPE_P(le) != le_plink) {
				...
				log_error("JONJ/MS SQL: Hashed persistent link is not a MS SQL link!",jonj_rqst->server);
#endif
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Hashed persistent link is not a MS SQL link!");
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (DBDEAD(mssql_ptr->link) == TRUE) {
				dbclose(mssql_ptr->link);
#if BROKEN_MSSQL_PCONNECTS
				log_error("JONJ/MS SQL: Persistent link died, trying to reconnect...",jonj_rqst->server);
#endif
				if ((mssql_ptr->link=dbopen(mssql_ptr->login,host))==NULL) {
#if BROKEN_MSSQL_PCONNECTS
					log_error("JONJ/MS SQL: Unable to reconnect!",jonj_rqst->server);
#endif
...
#if BROKEN_MSSQL_PCONNECTS
				log_error("JONJ/MS SQL: Reconnect successful!",jonj_rqst->server);
#endif
				if (DBSETOPT(mssql_ptr->link, DBBUFFER, "2")==FAIL) {
#if BROKEN_MSSQL_PCONNECTS
					log_error("JONJ/MS SQL: Unable to set required options",jonj_rqst->server);
#endif
					zend_hash_del(&EG(persistent_list), hashed_details, hashed_details_length + 1);
					efree(hashed_details);
					dbfreelogin(mssql_ptr->login);
		...
		if (!new_link && zend_hash_find(&EG(regular_list), hashed_details, hashed_details_length + 1,(void **) &index_ptr)==SUCCESS) {
			...
			if (ptr && (type==le_link || type==le_plink)) {
				...
				jonj_mssql_set_default_link(link TSRMLS_CC);
	...
	efree(hashed_details);
	jonj_mssql_set_default_link(Z_LVAL_P(return_value) TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_mssql_get_default_link
*/
static int jonj_mssql_get_default_link(INTERNAL_FUNCTION_PARAMETERS)
{
	if (MS_SQL_G(default_link)==-1) { /* no link opened yet, implicitly open one */
		ht = 0;
		jonj_mssql_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU,0);
	}
...
{
	jonj_mssql_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU,1);
}
/* }}} */
...
JONJ_FUNCTION(mssql_close)
{
	...
	if (mssql_link_index == NULL) {
		id = jonj_mssql_get_default_link(INTERNAL_FUNCTION_PARAM_PASSTHRU);
		CHECK_LINK(id);
	}
...
JONJ_FUNCTION(mssql_select_db)
{
	...
	if (dbuse(mssql_ptr->link, db)==FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to select database:  %s", db);
		RETURN_FALSE;
	} else {
		RETURN_TRUE;
	}
...
/* {{{ jonj_mssql_get_column_content_with_type
*/
static void jonj_mssql_get_column_content_with_type(mssql_link *mssql_ptr,int offset,zval *result, int column_type  TSRMLS_DC)
{
	...
	switch (column_type)
	{
		...
		default: {
			if (dbwillconvert(column_type,SQLCHAR)) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "column %d has unknown data type (%d)", offset, coltype(offset));
				ZVAL_FALSE(result);
			}
		}
	}
...
/* {{{ jonj_mssql_get_column_content_without_type
*/
static void jonj_mssql_get_column_content_without_type(mssql_link *mssql_ptr,int offset,zval *result, int column_type TSRMLS_DC)
{
		...
		for (i = 1; i <= num_rets; i++) {
			...
			if (statement->binds != NULL) {	/*	Maybe a non-parameter sp	*/
				if (zend_hash_find(statement->binds, parameter, strlen(parameter), (void**)&bind)==SUCCESS) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An output parameter variable was not provided");
				}
			}
		}
	}
	if (statement->binds != NULL) {	/*	Maybe a non-parameter sp	*/
		if (zend_hash_find(statement->binds, "RETVAL", 6, (void**)&bind)==SUCCESS) {
			if (dbhasretstat(mssql_ptr->link)) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "stored procedure has no return value. Nothing was returned into RETVAL");
			}
		}
	}
}
...
JONJ_FUNCTION(mssql_query)
{
	...
	if (dbcmd(mssql_ptr->link, query)==FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set query");
		RETURN_FALSE;
	}
	if (dbsqlexec(mssql_ptr->link)==FAIL || (retvalue = dbresults(mssql_ptr->link))==FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Query failed");
...
/* {{{ jonj_mssql_fetch_hash
*/
static void jonj_mssql_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, int result_type)
{
	zval *mssql_result_index;
	mssql_result *result;
	int i;
...
{
	jonj_mssql_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MSSQL_NUM);
}
/* }}} */
...
{
	jonj_mssql_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MSSQL_ASSOC);
...
{
	jonj_mssql_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MSSQL_BOTH);
}
/* }}} */
...
JONJ_FUNCTION(mssql_data_seek)
{
	...
	if (offset < 0 || offset >= result->num_rows) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad row offset");
		RETURN_FALSE;
	}
...
/* {{{ jonj_mssql_get_field_name
*/
static char *jonj_mssql_get_field_name(int type)
{
	switch (type) {
		case SQLBINARY:
		case SQLVARBINARY:
...
JONJ_FUNCTION(mssql_fetch_field)
{
	...
	if (field_offset<0 || field_offset >= result->num_fields) {
		if (ZEND_NUM_ARGS()==2) { /* field specified explicitly */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad column offset");
		}
		RETURN_FALSE;
	}
	...
	add_property_string(return_value, "type", jonj_mssql_get_field_name(Z_TYPE(result->fields[field_offset])), 1);
}
/* }}} */
...
JONJ_FUNCTION(mssql_field_type)
{
	...
	RETURN_STRINGL(jonj_mssql_get_field_name(Z_TYPE(result->fields[field_offset])), strlen(jonj_mssql_get_field_name(Z_TYPE(result->fields[field_offset]))), 1);
}
/* }}} */
...
JONJ_FUNCTION(mssql_result)
{
	...
	if (row < 0 || row >= result->num_rows) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad row offset (%ld)", row);
		RETURN_FALSE;
	}
	...
	switch(Z_TYPE_PP(field)) {
		case IS_STRING: {
			...
			if (i>=result->num_fields) { /* no match found */
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s field not found in result", Z_STRVAL_PP(field));
				RETURN_FALSE;
			}
			break;
		}
...
JONJ_FUNCTION(mssql_init)
{
	...
	if (dbrpcinit(mssql_ptr->link, sp_name,0)==FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to init stored procedure");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mssql_bind)
{
	...
	if ( (type==SQLVARCHAR) || (type==SQLCHAR) || (type==SQLTEXT) )	{	/* variable-length type */
		...
		switch (type) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unsupported type");
				RETURN_FALSE;
				break;
		}
	}
	...
	if (zend_hash_exists(statement->binds, param_name, param_name_len)) {
		...
		if ( strcmp("RETVAL", param_name)!=0 ) {						
			if (dbrpcparam(mssql_ptr->link, param_name, (BYTE)status, type, maxlen, datalen, (LPBYTE)value)==FAIL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set parameter");
				RETURN_FALSE;
			}
		}
	}
...
JONJ_FUNCTION(mssql_execute)
{
	...
	if (exec_retval == FAIL || dbsqlok(mssql_ptr->link) == FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "stored procedure execution failed");
	...
	if (retval_results==FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "could not retrieve results");
		dbcancel(mssql_ptr->link);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mssql_guid_string)
{
	...
	if (dbconvert(NULL, SQLBINARY, (BYTE*) binary, MIN(16, binary_len), SQLCHAR, buffer, (DBINT) -1) == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "could not convert binary string to GUID string");
		RETURN_FALSE;
	}

	if (sf) {
		jonj_strtoupper(buffer, 32);
