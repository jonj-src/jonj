...
const zend_function_entry calendar_functions[] = {
	JONJ_FE(jdtogregorian, arginfo_jdtogregorian)
	JONJ_FE(gregoriantojd, arginfo_gregoriantojd)
	JONJ_FE(jdtojulian, arginfo_jdtojulian)
	JONJ_FE(juliantojd, arginfo_juliantojd)
	JONJ_FE(jdtojewish, arginfo_jdtojewish)
	JONJ_FE(jewishtojd, arginfo_jewishtojd)
	JONJ_FE(jdtofrench, arginfo_jdtofrench)
	JONJ_FE(frenchtojd, arginfo_frenchtojd)
	JONJ_FE(jddayofweek, arginfo_jddayofweek)
	JONJ_FE(jdmonthname, arginfo_jdmonthname)
	JONJ_FE(easter_date, arginfo_easter_date)
	JONJ_FE(easter_days, arginfo_easter_days)
	JONJ_FE(unixtojd, arginfo_unixtojd)
	JONJ_FE(jdtounix, arginfo_jdtounix)
	JONJ_FE(cal_to_jd, arginfo_cal_to_jd)
	JONJ_FE(cal_from_jd, arginfo_cal_from_jd)
	JONJ_FE(cal_days_in_month, arginfo_cal_days_in_month)
	JONJ_FE(cal_info, arginfo_cal_info)
	JONJ_FE_END
};
...
JONJ_MINFO_FUNCTION(calendar)
{
	...
	jonj_info_print_table_row(2, "Calendar support", "enabled");
	jonj_info_print_table_end();
}

static void _jonj_cal_info(int cal, zval **ret)
{
	zval *months, *smonths;
	int i;
...
JONJ_FUNCTION(cal_info)
{
	...
	if (cal != -1 && (cal < 0 || cal >= CAL_NUM_CALS)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid calendar ID %ld.", cal);
		RETURN_FALSE;
	}

	_jonj_cal_info(cal, &return_value);
...
JONJ_FUNCTION(cal_days_in_month)
{
	...
	if (sdn_start == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid date.");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(jdtojewish)
{
	...
	if (!heb) {
		...
		if (year <= 0 || year > 9999) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Year out of range (0-9999).");
			RETURN_FALSE;
		}
