...
ub4 _oci_error(OCIError *err, pdo_dbh_t *dbh, pdo_stmt_t *stmt, char *what, sword status, int isinit, const char *file, int line TSRMLS_DC) /* {{{ */
{
	...
	if (!dbh->methods) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), einfo->errcode TSRMLS_CC, "SQLSTATE[%s]: %s", *pdo_err, einfo->errmsg);
	}
...
static long oci_handle_doer(pdo_dbh_t *dbh, const char *sql, long sql_len TSRMLS_DC) /* {{{ */
{
	...
	if (stmt_type == OCI_STMT_SELECT) {
		...
		OCIHandleFree(stmt, OCI_HTYPE_STMT);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "issuing a SELECT query here is invalid");
		return -1;
	}
...
static int oci_handle_get_attribute(pdo_dbh_t *dbh, long attr, zval *return_value TSRMLS_DC)  /* {{{ */
{
	...
	switch (attr) {
		...
		case PDO_ATTR_CLIENT_VERSION:
		{
			...
			ZVAL_STRING(return_value, verstr, 1);
#elif defined(JONJ_PDO_OCI_CLIENT_VERSION)
			/* Compile time client version */
			ZVAL_STRING(return_value, JONJ_PDO_OCI_CLIENT_VERSION, 1);
#else
			return FALSE;
...
static int pdo_oci_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC) /* {{{ */
{
	...
	jonj_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, 2);
	...
	if (vars[0].optval) {
		...
		if (!H->charset) {
			...
			if (OCIEnvNlsCreate(&H->env, PDO_OCI_INIT_MODE, 0, NULL, NULL, NULL, 0, NULL, H->charset, H->charset) != OCI_SUCCESS) {
				oci_init_error("OCIEnvNlsCreate: Check the character set is valid and that JONJ has access to Oracle libraries and NLS data");
				goto cleanup;
			}
		}
	}
