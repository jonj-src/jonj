...
static jonj_stream *oci_create_lob_stream(pdo_stmt_t *stmt, OCILobLocator *lob TSRMLS_DC);
...
static sb4 oci_bind_input_cb(dvoid *ctx, OCIBind *bindp, ub4 iter, ub4 index, dvoid **bufpp, ub4 *alenp, ub1 *piecep, dvoid **indpp) /* {{{ */
{
	...
	if (!param || !param->parameter) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "param is NULL in oci_bind_input_cb; this should not happen");
		return OCI_ERROR;
	}
...
static sb4 oci_bind_output_cb(dvoid *ctx, OCIBind *bindp, ub4 iter, ub4 index, dvoid **bufpp, ub4 **alenpp, ub1 *piecep, dvoid **indpp, ub2 **rcodepp) /* {{{ */
{
	...
	if (!param || !param->parameter) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "param is NULL in oci_bind_output_cb; this should not happen");
		return OCI_ERROR;
	}
				...
				if (P->used_for_output) {
					...
					if (Z_TYPE_P(param->parameter) == IS_NULL) {
						...
						if (stm) {
							...
							jonj_stream_to_zval(stm, param->parameter);
							P->thing = NULL;
						}
					} else {
						...
						jonj_stream_from_zval_no_verify(stm, &param->parameter);
						if (stm) {
							OCILobOpen(S->H->svc, S->err, (OCILobLocator*)P->thing, OCI_LOB_READWRITE);
							do {
								char buf[8192];
								n = jonj_stream_read(stm, buf, sizeof(buf));
								if ((int)n <= 0) {
									break;
								}
								consume = buf;
...
static size_t oci_blob_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
...
static size_t oci_blob_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
	ub4 amt;
	sword r;
...
static int oci_blob_close(jonj_stream *stream, int close_handle TSRMLS_DC)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
...
static int oci_blob_flush(jonj_stream *stream TSRMLS_DC)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
...
static int oci_blob_seek(jonj_stream *stream, off_t offset, int whence, off_t *newoffset TSRMLS_DC)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
	...
	if (offset >= PDO_OCI_LOBMAXSIZE) {
		...
		self->offset = offset + 1;  /* Oracle LOBS are 1-based, but JONJ is 0-based */
		return 0;
	}
}

static jonj_stream_ops oci_blob_stream_ops = {
	oci_blob_write,
	oci_blob_read,
	oci_blob_close,
	oci_blob_flush,
...
static jonj_stream *oci_create_lob_stream(pdo_stmt_t *stmt, OCILobLocator *lob TSRMLS_DC)
{
	...
	stm = jonj_stream_alloc(&oci_blob_stream_ops, self, 0, "r+b");

	if (stm) {
		jonj_pdo_stmt_addref(stmt TSRMLS_CC);
		return stm;
	}
...
static int oci_stmt_get_col(pdo_stmt_t *stmt, int colno, char **ptr, unsigned long *len, int *caller_frees TSRMLS_DC) /* {{{ */
{
	...
	if (C->indicator == -1) {
		...
		*len = C->fetched_len;
		...
		/* it was truncated */
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "column %d data was too large for buffer and was truncated to fit it", colno);
