...
static char *accel_gen_system_id(void)
{
	...
	JONJ_MD5Update(&context, JONJ_VERSION, sizeof(JONJ_VERSION)-1);
	JONJ_MD5Update(&context, ZEND_EXTENSION_BUILD_ID, sizeof(ZEND_EXTENSION_BUILD_ID)-1);
	JONJ_MD5Update(&context, ZEND_BIN_ID, sizeof(ZEND_BIN_ID)-1);
	if (strstr(JONJ_VERSION, "-dev") != 0) {
		/* Development versions may be changed from build to build */
		JONJ_MD5Update(&context, __DATE__, sizeof(__DATE__)-1);
		JONJ_MD5Update(&context, __TIME__, sizeof(__TIME__)-1);
	}
