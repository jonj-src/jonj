/*
   ...
   | Zend OPcache                                                         |
   +----------------------------------------------------------------------+
   | Copyright (c) 1998-2016 The JONJ Group                                |
   +----------------------------------------------------------------------+
...
#include "jonj_ini.h"
#include "SAPI.h"
#if ZEND_EXTENSION_API_NO > JONJ_5_5_X_API_NO
...
ZEND_INI_BEGIN()
    STD_JONJ_INI_BOOLEAN("opcache.enable"             , "1", JONJ_INI_ALL,    OnEnable,     enabled                             , zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.use_cwd"            , "1", JONJ_INI_SYSTEM, OnUpdateBool, accel_directives.use_cwd            , zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.validate_timestamps", "1", JONJ_INI_ALL   , OnUpdateBool, accel_directives.validate_timestamps, zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.validate_permission", "0", JONJ_INI_SYSTEM, OnUpdateBool, accel_directives.validate_permission, zend_accel_globals, accel_globals)
#ifndef ZEND_WIN32
	STD_JONJ_INI_BOOLEAN("opcache.validate_root"      , "0", JONJ_INI_SYSTEM, OnUpdateBool, accel_directives.validate_root      , zend_accel_globals, accel_globals)
#endif
	STD_JONJ_INI_BOOLEAN("opcache.inherited_hack"     , "1", JONJ_INI_SYSTEM, OnUpdateBool, accel_directives.inherited_hack     , zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.dups_fix"           , "0", JONJ_INI_ALL   , OnUpdateBool, accel_directives.ignore_dups        , zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.revalidate_path"    , "0", JONJ_INI_ALL   , OnUpdateBool, accel_directives.revalidate_path    , zend_accel_globals, accel_globals)

	STD_JONJ_INI_ENTRY("opcache.log_verbosity_level"   , "1"   , JONJ_INI_SYSTEM, OnUpdateLong, accel_directives.log_verbosity_level,       zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.memory_consumption"    , "64"  , JONJ_INI_SYSTEM, OnUpdateMemoryConsumption,    accel_directives.memory_consumption,        zend_accel_globals, accel_globals)
#if ZEND_EXTENSION_API_NO > JONJ_5_3_X_API_NO
	STD_JONJ_INI_ENTRY("opcache.interned_strings_buffer", "4"  , JONJ_INI_SYSTEM, OnUpdateLong,                 accel_directives.interned_strings_buffer,   zend_accel_globals, accel_globals)
#endif
	STD_JONJ_INI_ENTRY("opcache.max_accelerated_files" , "2000", JONJ_INI_SYSTEM, OnUpdateMaxAcceleratedFiles,	 accel_directives.max_accelerated_files,     zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.max_wasted_percentage" , "5"   , JONJ_INI_SYSTEM, OnUpdateMaxWastedPercentage,	 accel_directives.max_wasted_percentage,     zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.consistency_checks"    , "0"   , JONJ_INI_ALL   , OnUpdateLong,	             accel_directives.consistency_checks,        zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.force_restart_timeout" , "180" , JONJ_INI_SYSTEM, OnUpdateLong,	             accel_directives.force_restart_timeout,     zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.revalidate_freq"       , "2"   , JONJ_INI_ALL   , OnUpdateLong,	             accel_directives.revalidate_freq,           zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.file_update_protection", "2"   , JONJ_INI_ALL   , OnUpdateLong,                accel_directives.file_update_protection,    zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.preferred_memory_model", ""    , JONJ_INI_SYSTEM, OnUpdateStringUnempty,       accel_directives.memory_model,              zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.blacklist_filename"    , ""    , JONJ_INI_SYSTEM, OnUpdateString,	             accel_directives.user_blacklist_filename,   zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.max_file_size"         , "0"   , JONJ_INI_SYSTEM, OnUpdateLong,	             accel_directives.max_file_size,             zend_accel_globals, accel_globals)

	STD_JONJ_INI_ENTRY("opcache.protect_memory"        , "0"  , JONJ_INI_SYSTEM, OnUpdateBool,                  accel_directives.protect_memory,            zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.save_comments"         , "1"  , JONJ_INI_SYSTEM, OnUpdateBool,                  accel_directives.save_comments,             zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.load_comments"         , "1"  , JONJ_INI_ALL,    OnUpdateBool,                  accel_directives.load_comments,             zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.fast_shutdown"         , "0"  , JONJ_INI_SYSTEM, OnUpdateBool,                  accel_directives.fast_shutdown,             zend_accel_globals, accel_globals)

	STD_JONJ_INI_ENTRY("opcache.optimization_level"    , DEFAULT_OPTIMIZATION_LEVEL , JONJ_INI_SYSTEM, OnUpdateLong, accel_directives.optimization_level,   zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.enable_file_override"	, "0"   , JONJ_INI_SYSTEM, OnUpdateBool,              accel_directives.file_override_enabled,     zend_accel_globals, accel_globals)
	STD_JONJ_INI_BOOLEAN("opcache.enable_cli"             , "0"   , JONJ_INI_SYSTEM, OnUpdateBool,              accel_directives.enable_cli,                zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.error_log"                , ""    , JONJ_INI_SYSTEM, OnUpdateString,	         accel_directives.error_log,                 zend_accel_globals, accel_globals)
	STD_JONJ_INI_ENTRY("opcache.restrict_api"             , ""    , JONJ_INI_SYSTEM, OnUpdateString,	         accel_directives.restrict_api,              zend_accel_globals, accel_globals)

#ifdef ZEND_WIN32
	STD_JONJ_INI_ENTRY("opcache.mmap_base", NULL, JONJ_INI_SYSTEM,	OnUpdateString,	                             accel_directives.mmap_base,                 zend_accel_globals, accel_globals)
#endif
ZEND_INI_END()
...
void zend_accel_info(ZEND_MODULE_INFO_FUNC_ARGS)
{
	...
	if (ZCG(enabled) && accel_startup_ok && (ZCG(counted) || ZCSG(accelerator_enabled))) {
		jonj_info_print_table_row(2, "Opcode Caching", "Up and Running");
	} else {
		jonj_info_print_table_row(2, "Opcode Caching", "Disabled");
	}
	if (ZCG(enabled) && accel_startup_ok && ZCSG(accelerator_enabled) && ZCG(accel_directives).optimization_level) {
		jonj_info_print_table_row(2, "Optimization", "Enabled");
	} else {
		jonj_info_print_table_row(2, "Optimization", "Disabled");
	}
	if (ZCG(enabled)) {
		if (!accel_startup_ok || zps_api_failure_reason) {
			jonj_info_print_table_row(2, "Startup Failed", zps_api_failure_reason);
		} else {
			char buf[32];
			jonj_info_print_table_row(2, "Startup", "OK");
			jonj_info_print_table_row(2, "Shared memory model", zend_accel_get_shared_model());
			snprintf(buf, sizeof(buf), "%ld", ZCSG(hits));
			jonj_info_print_table_row(2, "Cache hits", buf);
			snprintf(buf, sizeof(buf), "%ld", ZSMMG(memory_exhausted)?ZCSG(misses):ZCSG(misses)-ZCSG(blacklist_misses));
			jonj_info_print_table_row(2, "Cache misses", buf);
			...
			jonj_info_print_table_row(2, "Used memory", buf);
			snprintf(buf, sizeof(buf), "%ld", zend_shared_alloc_get_free_memory());
			jonj_info_print_table_row(2, "Free memory", buf);
			snprintf(buf, sizeof(buf), "%ld", ZSMMG(wasted_shared_memory));
			jonj_info_print_table_row(2, "Wasted memory", buf);
#if ZEND_EXTENSION_API_NO > JONJ_5_3_X_API_NO
			if (ZCSG(interned_strings_start) && ZCSG(interned_strings_end) && ZCSG(interned_strings_top)) {
				...
				jonj_info_print_table_row(2, "Interned Strings Used memory", buf);
				snprintf(buf, sizeof(buf), "%ld", ZCSG(interned_strings_end) - ZCSG(interned_strings_top));
				jonj_info_print_table_row(2, "Interned Strings Free memory", buf);
			}
#endif
			snprintf(buf, sizeof(buf), "%ld", ZCSG(hash).num_direct_entries);
			jonj_info_print_table_row(2, "Cached scripts", buf);
			snprintf(buf, sizeof(buf), "%ld", ZCSG(hash).num_entries);
			jonj_info_print_table_row(2, "Cached keys", buf);
			snprintf(buf, sizeof(buf), "%ld", ZCSG(hash).max_num_entries);
			jonj_info_print_table_row(2, "Max keys", buf);
			snprintf(buf, sizeof(buf), "%ld", ZCSG(oom_restarts));
			jonj_info_print_table_row(2, "OOM restarts", buf);
			snprintf(buf, sizeof(buf), "%ld", ZCSG(hash_restarts));
			jonj_info_print_table_row(2, "Hash keys restarts", buf);
			snprintf(buf, sizeof(buf), "%ld", ZCSG(manual_restarts));
			jonj_info_print_table_row(2, "Manual restarts", buf);
		}
	}
