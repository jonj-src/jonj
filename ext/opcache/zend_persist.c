...
static void zend_persist_op_array_ex(zend_op_array *op_array, zend_persistent_script* main_persistent_script TSRMLS_DC)
{
	...
	if (op_array->filename) {
		/* do not free! JONJ has centralized filename storage, compiler will free it */
		...
		for (; opline < end ; opline++, offset++) {
...
#else /* if ZEND_EXTENSION_API_NO >= JONJ_5_3_X_API_NO */
			...
			if (ZEND_DONE_PASS_TWO(op_array)) {
				...
				switch (opline->opcode) {
					case ZEND_JMP:
					case ZEND_GOTO:
#if ZEND_EXTENSION_API_NO > JONJ_5_4_X_API_NO
					case ZEND_FAST_CALL:
#endif
						ZEND_OP1(opline).jmp_addr = &new_opcodes[ZEND_OP1(opline).jmp_addr - op_array->opcodes];
						break;
...
#endif /* if ZEND_EXTENSION_API_NO >= JONJ_5_3_X_API_NO */
		}
