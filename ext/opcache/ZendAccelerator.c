...
/* Points to the structure shared across all JONJ processes */
...
#if ZEND_EXTENSION_API_NO < JONJ_5_3_X_API_NO
static char *accel_jonj_resolve_path(const char *filename, int filename_length, const char *path TSRMLS_DC);
#endif
...
/* O+ overrides JONJ chdir() function and remembers the current working directory
...
/* O+ disables creation of interned strings by regular JONJ compiler, instead,
 * it creates interned strings in shared memory when saves a script.
 * Such interned strings are shared across all JONJ processes
 */
static const char *accel_new_interned_string_for_jonj(const char *str, int len, int free_src TSRMLS_DC)
{
	return str;
}

static void accel_interned_strings_snapshot_for_jonj(TSRMLS_D)
{
}

static void accel_interned_strings_restore_for_jonj(TSRMLS_D)
{
}
...
#ifndef ZTS
/* Copy JONJ interned strings from JONJ process memory into the shared memory */
...
static int zend_get_stream_timestamp(const char *filename, struct stat *statbuf TSRMLS_DC)
{
	...
	jonj_stream_statbuf stream_statbuf;
	int ret, er;
	...
	wrapper = jonj_stream_locate_url_wrapper(filename, NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC);
	if (!wrapper) {
		return FAILURE;
	}
	...
	zend_try {
		ret = wrapper->wops->url_stat(wrapper, (char*)filename, JONJ_STREAM_URL_STAT_QUIET, &stream_statbuf, NULL TSRMLS_CC);
	} zend_catch {
		ret = -1;
	} zend_end_try();
...
static accel_time_t zend_get_file_handle_timestamp(zend_file_handle *file_handle, size_t *size TSRMLS_DC)
{
	...
	switch (file_handle->type) {
		...
		case ZEND_HANDLE_STREAM:
			{
				jonj_stream *stream = (jonj_stream *)file_handle->handle.stream.handle;
				jonj_stream_statbuf sb;
				int ret, er;
...
#if ZEND_EXTENSION_API_NO < JONJ_5_3_X_API_NO
		full_path_ptr = accel_jonj_resolve_path(file_handle->filename, strlen(file_handle->filename), ZCG(include_path) TSRMLS_CC);
#else
...
#if ZEND_EXTENSION_API_NO < JONJ_5_3_X_API_NO
	realpath = accel_jonj_resolve_path(filename, filename_len, ZCG(include_path) TSRMLS_CC);
#else
			...
			if (!EG(opline_ptr) || !*EG(opline_ptr) ||
				...
				if (zend_hash_quick_add(&EG(included_files), persistent_script->full_path, persistent_script->full_path_len + 1, persistent_script->hash_value, &dummy, sizeof(void *), NULL) == SUCCESS) {
					...
					if (strstr(persistent_script->full_path, ".phar") && !strstr(persistent_script->full_path, "://")) {
						...
						jonj_stream_stat_path(fname, &ssb);
						efree(fname);
					}
				}
			}
...
/* Taken from jonj-5.2.5 because early versions don't have correct version */
static char *accel_tsrm_realpath(const char *path, int path_len TSRMLS_DC)
{
	cwd_state new_state;
#if ZEND_EXTENSION_API_NO < JONJ_5_6_X_API_NO
	char *real_path;
#endif
	char *cwd;
	int cwd_len;
...
/* zend_stream_open_function() replacement for JONJ 5.2 */
	...
	if (ZCG(enabled) && accel_startup_ok &&
		...
		if (EG(opline_ptr) && *EG(opline_ptr)) {
            ...
            if (opline->opcode == ZEND_INCLUDE_OR_EVAL &&
				...
				resolved_path = accel_jonj_resolve_path(filename, filename_len, ZCG(include_path) TSRMLS_CC);
...
/* zend_stream_open_function() replacement for JONJ 5.3 and above */
static int persistent_stream_open_function(const char *filename, zend_file_handle *handle TSRMLS_DC)
{
	if (ZCG(enabled) && accel_startup_ok &&
	    (ZCG(counted) || ZCSG(accelerator_enabled)) &&
...
/* zend_resolve_path() replacement for JONJ 5.3 and above */
...
static void accel_activate(void)
{
	...
	SHM_UNPROTECT();
	/* JONJ-5.4 and above return "double", but we use 1 sec precision */
...
#if (ZEND_EXTENSION_API_NO > JONJ_5_3_X_API_NO) && !defined(ZTS)
...
#endif
#if ZEND_EXTENSION_API_NO <= JONJ_5_5_API_NO
...
# endif
	...
	zend_new_interned_string = accel_new_interned_string_for_jonj;
	zend_interned_strings_snapshot = accel_interned_strings_snapshot_for_jonj;
	zend_interned_strings_restore = accel_interned_strings_restore_for_jonj;
