...
JONJAPI zend_class_entry *reflector_ptr;
JONJAPI zend_class_entry *reflection_exception_ptr;
JONJAPI zend_class_entry *reflection_ptr;
JONJAPI zend_class_entry *reflection_function_abstract_ptr;
JONJAPI zend_class_entry *reflection_function_ptr;
JONJAPI zend_class_entry *reflection_parameter_ptr;
JONJAPI zend_class_entry *reflection_class_ptr;
JONJAPI zend_class_entry *reflection_object_ptr;
JONJAPI zend_class_entry *reflection_method_ptr;
JONJAPI zend_class_entry *reflection_property_ptr;
JONJAPI zend_class_entry *reflection_extension_ptr;
JONJAPI zend_class_entry *reflection_zend_extension_ptr;
...
#define METHOD_NOTSTATIC(ce)                                                                                \
	if (!this_ptr || !instanceof_function(Z_OBJCE_P(this_ptr), ce TSRMLS_CC)) {                             \
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "%s() cannot be called statically", get_active_function_name(TSRMLS_C));        \
		return;                                                                                             \
	}                                                                                                       \
...
#define GET_REFLECTION_OBJECT_PTR(target)                                                                   \
	...
	if (intern == NULL || intern->ptr == NULL) {                                                            \
		RETURN_ON_EXCEPTION                                                                                 \
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Internal error: Failed to retrieve the reflection object");                    \
	}                                                                                                       \
	target = intern->ptr;                                                                                   \
...
ZEND_METHOD(reflection, export)
{
	...
	if (!retval_ptr) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s::__toString() did not return anything", Z_OBJCE_P(object)->name);
		RETURN_FALSE;
	}
...
ZEND_METHOD(reflection_class, newInstance)
{
	...
	if (constructor) {
		...
		if (zend_call_function(&fci, &fcc TSRMLS_CC) == FAILURE) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invocation of %s's constructor failed", ce->name);
			zval_dtor(return_value);
			RETURN_NULL();
		}
		if (retval_ptr) {
		...
		case IS_OBJECT:
			if (instanceof_function(Z_OBJCE_P(class_name), reflection_class_ptr TSRMLS_CC)) {
				...
				if (argument == NULL || argument->ptr == NULL) {
					jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Internal error: Failed to retrieve the argument's reflection object");
					/* Bails out */
				}
...
ZEND_METHOD(reflection_property, getValue)
{
	...
	if ((ref->prop.flags & ZEND_ACC_STATIC)) {
		...
		if (!CE_STATIC_MEMBERS(intern->ce)[ref->prop.offset]) {
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Internal error: Could not find the property %s::%s", intern->ce->name, ref->prop.name);
			/* Bails out */
		}
...
/* {{{ proto public void ReflectionExtension::info()
       Prints jonjinfo block for the extension */
...
static const zend_function_entry reflection_function_abstract_functions[] = {
	...
	JONJ_ABSTRACT_ME(reflection_function, __toString, arginfo_reflection__void)
...
JONJ_MINIT_FUNCTION(reflection) /* {{{ */
{
...
JONJ_MINFO_FUNCTION(reflection) /* {{{ */
{
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "Reflection", "enabled");

	jonj_info_print_table_row(2, "Version", "$Id$");
...
zend_module_entry reflection_module_entry = { /* {{{ */
	...
	JONJ_MINIT(reflection),
	NULL,
	NULL,
	NULL,
	JONJ_MINFO(reflection),
	"$Id$",
