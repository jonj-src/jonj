...
static JONJ_GINIT_FUNCTION(sybase);
static JONJ_GSHUTDOWN_FUNCTION(sybase);
...
const zend_function_entry sybase_functions[] = {
	JONJ_FE(sybase_connect,				arginfo_sybase_connect)
	JONJ_FE(sybase_pconnect,				arginfo_sybase_pconnect)
	JONJ_FE(sybase_close, 				arginfo_sybase_close)
	JONJ_FE(sybase_select_db, 			arginfo_sybase_select_db)
	JONJ_FE(sybase_query,				arginfo_sybase_query)
	JONJ_FE(sybase_unbuffered_query,		arginfo_sybase_unbuffered_query)
	JONJ_FE(sybase_free_result,			arginfo_sybase_free_result)
	JONJ_FE(sybase_get_last_message,		arginfo_sybase_get_last_message)
	JONJ_FE(sybase_num_rows,				arginfo_sybase_num_rows)
	JONJ_FE(sybase_num_fields,			arginfo_sybase_num_fields)
	JONJ_FE(sybase_fetch_row,			arginfo_sybase_fetch_row)
	JONJ_FE(sybase_fetch_array,			arginfo_sybase_fetch_array)
	JONJ_FE(sybase_fetch_assoc,			arginfo_sybase_fetch_assoc)
	JONJ_FE(sybase_fetch_object,			arginfo_sybase_fetch_object)
	JONJ_FE(sybase_data_seek,			arginfo_sybase_data_seek)
	JONJ_FE(sybase_fetch_field,			arginfo_sybase_fetch_field)
	JONJ_FE(sybase_field_seek, 			arginfo_sybase_field_seek)
	JONJ_FE(sybase_result,				arginfo_sybase_result)
	JONJ_FE(sybase_affected_rows, 		arginfo_sybase_affected_rows)
	JONJ_FE(sybase_min_client_severity,	arginfo_sybase_min_client_severity)
	JONJ_FE(sybase_min_server_severity,	arginfo_sybase_min_server_severity)
	JONJ_FE(sybase_set_message_handler,	arginfo_sybase_set_message_handler)
	JONJ_FE(sybase_deadlock_retry_count, arginfo_sybase_deadlock_retry_count)

#if !defined(JONJ_WIN32) && !defined(HAVE_MSSQL)
	JONJ_FALIAS(mssql_connect, 	sybase_connect, 	arginfo_sybase_connect)
	JONJ_FALIAS(mssql_pconnect, 	sybase_pconnect, 	arginfo_sybase_pconnect)
	JONJ_FALIAS(mssql_close, 	sybase_close, 		arginfo_sybase_close)
	JONJ_FALIAS(mssql_select_db, sybase_select_db, 	arginfo_sybase_select_db)
	JONJ_FALIAS(mssql_query, 	sybase_query, 		arginfo_sybase_query)
	JONJ_FALIAS(mssql_unbuffered_query, 	sybase_unbuffered_query, 	arginfo_sybase_unbuffered_query)
	JONJ_FALIAS(mssql_free_result, 		sybase_free_result, 		arginfo_sybase_free_result)
	JONJ_FALIAS(mssql_get_last_message, 	sybase_get_last_message, 	arginfo_sybase_get_last_message)
	JONJ_FALIAS(mssql_num_rows,	 	sybase_num_rows, 		arginfo_sybase_num_rows)
	JONJ_FALIAS(mssql_num_fields, 	sybase_num_fields, 		arginfo_sybase_num_fields)
	JONJ_FALIAS(mssql_fetch_row, 	sybase_fetch_row, 		arginfo_sybase_fetch_row)
	JONJ_FALIAS(mssql_fetch_array, 	sybase_fetch_array, 	arginfo_sybase_fetch_array)
	JONJ_FALIAS(mssql_fetch_assoc, 	sybase_fetch_assoc, 	arginfo_sybase_fetch_assoc)
	JONJ_FALIAS(mssql_fetch_object, 	sybase_fetch_object, 	arginfo_sybase_fetch_object)
	JONJ_FALIAS(mssql_data_seek, 	sybase_data_seek, 		arginfo_sybase_data_seek)
	JONJ_FALIAS(mssql_fetch_field, 	sybase_fetch_field, 	arginfo_sybase_fetch_field)
	JONJ_FALIAS(mssql_field_seek, 	sybase_field_seek, 		arginfo_sybase_field_seek)
	JONJ_FALIAS(mssql_result, 		sybase_result, 			arginfo_sybase_result)
	JONJ_FALIAS(mssql_affected_rows, sybase_affected_rows, 	arginfo_sybase_affected_rows)
	JONJ_FALIAS(mssql_min_client_severity,   sybase_min_client_severity, 	arginfo_sybase_min_client_severity)
	JONJ_FALIAS(mssql_min_server_severity, 	sybase_min_server_severity, 	arginfo_sybase_min_server_severity)
	JONJ_FALIAS(mssql_set_message_handler, 	sybase_set_message_handler, 	arginfo_sybase_set_message_handler)
	JONJ_FALIAS(mssql_deadlock_retry_count, 	sybase_deadlock_retry_count, 	arginfo_sybase_deadlock_retry_count)
#endif
	JONJ_FE_END
};
...
zend_module_entry sybase_module_entry = {
	...
	sybase_functions,
	JONJ_MINIT(sybase),
	JONJ_MSHUTDOWN(sybase),
	JONJ_RINIT(sybase),
	JONJ_RSHUTDOWN(sybase),
	JONJ_MINFO(sybase),
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(sybase),
	JONJ_GINIT(sybase),
	JONJ_GSHUTDOWN(sybase),
	NULL,
...
#define CHECK_LINK(link) { if (link==-1) { jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  A link to the server could not be established"); RETURN_FALSE; } }
...
/* Forward declaration */
static int jonj_sybase_finish_results (sybase_result *result TSRMLS_DC);

static void jonj_free_sybase_result(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	...
	if (result->sybase_ptr && result->sybase_ptr->active_result_index) {
		...
		jonj_sybase_finish_results(result TSRMLS_CC);
	}
...
static void _close_sybase_link(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	...
	if (ct_con_props(sybase_ptr->connection, CS_GET, CS_CON_STATUS,
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to get connection status on close");
		/* Assume the worst. */
...
static CS_RETCODE CS_PUBLIC _client_message_handler(CS_CONTEXT *context, CS_CONNECTION *connection, CS_CLIENTMSG *errmsg)
{
	...
	if (CS_SEVERITY(errmsg->msgnumber) >= SybCtG(min_client_severity)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Client message:  %s (severity %ld)", errmsg->msgstring, (long)CS_SEVERITY(errmsg->msgnumber));
	}
...
static int _call_message_handler(zval *callback_name, CS_SERVERMSG *srvmsg TSRMLS_DC)
{
	...
	if (call_user_function_ex(EG(function_table), NULL, callback_name, &retval, 5, args, 0, NULL TSRMLS_CC) == FAILURE) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Cannot call the messagehandler %s", Z_STRVAL(expr_copy));
		zval_dtor(&expr_copy);
	}
...
static CS_RETCODE CS_PUBLIC _server_message_handler(CS_CONTEXT *context, CS_CONNECTION *connection, CS_SERVERMSG *srvmsg)
{
	...
	if (!handled) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Server message:  %s (severity %ld, procedure %s)",
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_BOOLEAN("sybct.allow_persistent", "1", JONJ_INI_SYSTEM, OnUpdateLong, allow_persistent, zend_sybase_globals, sybase_globals)
	STD_JONJ_INI_ENTRY_EX("sybct.max_persistent", "-1", JONJ_INI_SYSTEM, OnUpdateLong, max_persistent, zend_sybase_globals, sybase_globals, display_link_numbers)
	STD_JONJ_INI_ENTRY_EX("sybct.max_links", "-1", JONJ_INI_SYSTEM, OnUpdateLong, max_links, zend_sybase_globals, sybase_globals, display_link_numbers)
	STD_JONJ_INI_ENTRY("sybct.min_server_severity", "10", JONJ_INI_ALL, OnUpdateLong, min_server_severity, zend_sybase_globals, sybase_globals)
	STD_JONJ_INI_ENTRY("sybct.min_client_severity", "10", JONJ_INI_ALL, OnUpdateLong, min_client_severity, zend_sybase_globals, sybase_globals)
	STD_JONJ_INI_ENTRY("sybct.login_timeout", "-1", JONJ_INI_ALL, OnUpdateLong, login_timeout, zend_sybase_globals, sybase_globals)
	STD_JONJ_INI_ENTRY("sybct.hostname", NULL, JONJ_INI_ALL, OnUpdateString, hostname, zend_sybase_globals, sybase_globals)
	STD_JONJ_INI_ENTRY_EX("sybct.deadlock_retry_count", "0", JONJ_INI_ALL, OnUpdateLong, deadlock_retry_count, zend_sybase_globals, sybase_globals, display_link_numbers)
JONJ_INI_END()
...
static JONJ_GINIT_FUNCTION(sybase)
{
	...
	if (ct_callback(sybase_globals->context, NULL, CS_SET, CS_SERVERMSG_CB, (CS_VOID *)_server_message_handler)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to set server message handler");
	}

	if (ct_callback(sybase_globals->context, NULL, CS_SET, CS_CLIENTMSG_CB, (CS_VOID *)_client_message_handler)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to set client message handler");
	}
	 ...
	 */
	{
		...
		if (cs_dt_info(sybase_globals->context, CS_SET, NULL, CS_DT_CONVFMT, CS_UNUSED, &dt_convfmt, sizeof(dt_convfmt), NULL)!=CS_SUCCEED) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to set datetime conversion format");
		}
	}
	...
	if (cfg_get_long("sybct.timeout", &opt)==SUCCESS) {
		...
		if (ct_config(sybase_globals->context, CS_SET, CS_TIMEOUT, &cs_timeout, CS_UNUSED, NULL)!=CS_SUCCEED) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to update the timeout");
		}
	}
...
JONJ_MINIT_FUNCTION(sybase)
{
	...
	le_result = zend_register_list_destructors_ex(jonj_free_sybase_result, NULL, "sybase-ct result", module_number);
...
JONJ_RINIT_FUNCTION(sybase)
{
	...
	SybCtG(appname) = estrndup("JONJ " JONJ_VERSION, sizeof("JONJ " JONJ_VERSION));
...
static int jonj_sybase_do_connect_internal(sybase_link *sybase, char *host, char *user, char *passwd, char *charset, char *appname TSRMLS_DC)
{
	...
	if (ct_con_alloc(SybCtG(context), &sybase->connection)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to allocate connection record");
		return 0;
	}
	...
	if (ct_con_props(sybase->connection, CS_SET, CS_USERDATA, &sybase, CS_SIZEOF(sybase), NULL)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to set userdata");
	...
	if (charset) {
		if (cs_loc_alloc(SybCtG(context), &tmp_locale)!=CS_SUCCEED) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase: Unable to allocate locale information");
		} else {
			if (cs_locale(SybCtG(context), CS_SET, tmp_locale, CS_LC_ALL, NULL, CS_NULLTERM, NULL)!=CS_SUCCEED) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase: Unable to load default locale data");
			} else {
				if (cs_locale(SybCtG(context), CS_SET, tmp_locale, CS_SYB_CHARSET, charset, CS_NULLTERM, NULL)!=CS_SUCCEED) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase: Unable to update character set");
				} else {
					if (ct_con_props(sybase->connection, CS_SET, CS_LOC_PROP, tmp_locale, CS_UNUSED, NULL)!=CS_SUCCEED) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase: Unable to update connection properties");
					}
				}
			}
		}
	...
	if (cfg_get_long("sybct.packet_size", &packetsize) == SUCCESS) {
		if (ct_con_props(sybase->connection, CS_SET, CS_PACKETSIZE, (CS_VOID *)&packetsize, CS_UNUSED, NULL) != CS_SUCCEED) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase: Unable to update connection packetsize");
		}
	}
	...
	if (SybCtG(login_timeout) != -1) {
		...
		if (ct_config(SybCtG(context), CS_SET, CS_LOGIN_TIMEOUT, &cs_login_timeout, CS_UNUSED, NULL)!=CS_SUCCEED) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to update the login timeout");
		}
	}
	...
	if (ct_connect(sybase->connection, host, CS_NULLTERM)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to connect");
		ct_con_drop(sybase->connection);
		return 0;
	}

	if (ct_cmd_alloc(sybase->connection, &sybase->cmd)!=CS_SUCCEED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to allocate command record");
...
static void jonj_sybase_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	if (persistent) {
		...
		if (zend_hash_find(&EG(persistent_list), hashed_details, hashed_details_length+1, (void **) &le)==FAILURE) {  /* we don't */
			...
			if (SybCtG(max_links)!=-1 && SybCtG(num_links)>=SybCtG(max_links)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Too many open links (%ld)", SybCtG(num_links));
				efree(hashed_details);
				RETURN_FALSE;
			}
			if (SybCtG(max_persistent)!=-1 && SybCtG(num_persistent)>=SybCtG(max_persistent)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Too many open persistent links (%ld)", SybCtG(num_persistent));
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (!jonj_sybase_do_connect_internal(sybase_ptr, host, user, passwd, charset, appname TSRMLS_CC)) {
				free(sybase_ptr);
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (ct_con_props(sybase_ptr->connection, CS_GET, CS_CON_STATUS,
							 &con_status, CS_UNUSED, NULL)!=CS_SUCCEED) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Unable to get connection status");
				efree(hashed_details);
				RETURN_FALSE;
			}
...
static int jonj_sybase_get_default_link(INTERNAL_FUNCTION_PARAMETERS)
{
	if (SybCtG(default_link)==-1) { /* no link opened yet, implicitly open one */
		ht = 0;
		jonj_sybase_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
	}
...
{
	jonj_sybase_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}

/* }}} */

inline static int jonj_sybase_connection_id(zval *sybase_link_index, int *id TSRMLS_DC)
{
...
JONJ_FUNCTION(sybase_close)
{
	...
	if (jonj_sybase_connection_id(sybase_link_index, &id TSRMLS_CC) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  No connection to close");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(sybase_select_db)
{
	...
	if (jonj_sybase_connection_id(sybase_link_index, &id TSRMLS_CC) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  No connection");
		RETURN_FALSE;
	}
...
static int jonj_sybase_finish_results(sybase_result *result TSRMLS_DC) 
{
	...
	while ((retcode = ct_results(result->sybase_ptr->cmd, &restype))==CS_SUCCEED) {
		switch ((int) restype) {
			...
			case CS_CMD_FAIL:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Command failed, canceling rest");
				ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
				fail = 1;
				break;
			...
			case CS_ROW_RESULT:
				...
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Sybase:  Unexpected results, canceling current");
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Sybase:  Unexpected results, canceling all");
				ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
				break;
		}
...
static int jonj_sybase_fetch_result_row(sybase_result *result, int numrows TSRMLS_DC)
{
	int i, j;
	CS_INT retcode;
	...
	if (retcode==CS_ROW_FAIL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Error reading row %d", result->num_rows);
		return retcode;
	}
	result->last_retcode= retcode;
	switch (retcode) {
		case CS_END_DATA:
			retcode = jonj_sybase_finish_results(result TSRMLS_CC);
			break;
...
static sybase_result * jonj_sybase_fetch_result_set(sybase_link *sybase_ptr, int buffered, int store TSRMLS_DC)
{
	int num_fields;
	...
	if (buffered) {
		...
		if ((retcode = jonj_sybase_fetch_result_row(result, -1 TSRMLS_CC)) == CS_FAIL) {
			return NULL;
		}
	}
...
static void jonj_sybase_query (INTERNAL_FUNCTION_PARAMETERS, int buffered)
{
	zval *sybase_link_index = NULL;
	zend_bool store = 1;
	char *query;
	...
	if (!store && !buffered) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Sybase:  Cannot use non-storing mode with buffered queries");
		store = 1;
	}
	...
	if (sybase_ptr->active_result_index) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Sybase:  Called without first fetching all rows from a previous unbuffered query");
		if (sybase_ptr->cmd) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
		}
	...
	for (;;) {
		...
		if (ct_command(sybase_ptr->cmd, CS_LANG_CMD, query, CS_NULLTERM, CS_UNUSED)!=CS_SUCCEED) {
			 ...
			 */
			sybase_ptr->dead = 1;
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Connection is dead");
			RETURN_FALSE;
		}
		...
		if (ct_send(sybase_ptr->cmd)!=CS_SUCCEED) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
			sybase_ptr->dead = 1;
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Cannot send command");
			RETURN_FALSE;
		}
		...
		if (ct_results(sybase_ptr->cmd, &restype)!=CS_SUCCEED) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
			sybase_ptr->dead = 1;
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Cannot read results");
			RETURN_FALSE;
		}
		switch ((int) restype) {
			case CS_CMD_FAIL:
			...
			case CS_ROW_RESULT:
			case CS_STATUS_RESULT:
				result = jonj_sybase_fetch_result_set(sybase_ptr, buffered, store TSRMLS_CC);
				if (result == NULL) {
					ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
					RETURN_FALSE;
				}
		...
		if (sybase_ptr->deadlock && SybCtG(deadlock_retry_count) != -1 && ++deadlock_count > SybCtG(deadlock_retry_count)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Retried deadlock %d times [max: %ld], giving up", deadlock_count- 1, SybCtG(deadlock_retry_count));
			FREE_SYBASE_RESULT(result);
			break;
		}
...
{
	jonj_sybase_query(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_sybase_query(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
...
JONJ_FUNCTION(sybase_free_result)
{
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS) {
		/* jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  canceling the rest of the results"); */
		ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
		jonj_sybase_finish_results(result TSRMLS_CC);
	}
...
JONJ_FUNCTION(sybase_fetch_row)
{
	...
	/* Unbuffered? */
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS) {
		jonj_sybase_fetch_result_row(result, 1 TSRMLS_CC);
	}
...
static void jonj_sybase_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, int numerics)
{
	zval *sybase_result_index = NULL;
	sybase_result *result;
	int i, j;
...
JONJ_FUNCTION(sybase_fetch_object)
{
		...
		switch (Z_TYPE_P(object)) {
			...
			default: {
				...
				if (zend_lookup_class(Z_STRVAL_P(object), Z_STRLEN_P(object), &pce TSRMLS_CC) == FAILURE) {
					jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Sybase:  Class %s has not been declared", Z_STRVAL_P(object));
					/* Use default (ZEND_STANDARD_CLASS_DEF_PTR) */
				} else {
					ce = *pce;
				}
	...
	ht= 1;
	jonj_sybase_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
...
{
	jonj_sybase_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(sybase_data_seek)
{
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS && offset >= result->num_rows) {
		jonj_sybase_fetch_result_row(result, offset+ 1 TSRMLS_CC);
	}
	
	if (offset < 0 || offset >= result->num_rows) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Bad row offset %ld, must be betweem 0 and %d", offset, result->num_rows - 1);
		RETURN_FALSE;
	}
...
static char *jonj_sybase_get_field_name(CS_INT type)
{
	switch (type) {
		case CS_CHAR_TYPE:
		case CS_VARCHAR_TYPE:
...
JONJ_FUNCTION(sybase_fetch_field)
{
	...
	if (field_offset < 0 || field_offset >= result->num_fields) {
		if (ZEND_NUM_ARGS() == 2) { /* field specified explicitly */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Bad column offset");
		}
		RETURN_FALSE;
	}
	...
	add_property_string(return_value, "type", jonj_sybase_get_field_name(Z_TYPE(result->fields[field_offset])), 1);
}
/* }}} */
...
JONJ_FUNCTION(sybase_result)
{
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS && row >= result->num_rows) {
		jonj_sybase_fetch_result_row(result, row TSRMLS_CC);
	}

	if (row < 0 || row >= result->num_rows) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Bad row offset (%ld)", row);
		RETURN_FALSE;
	}
	...
	switch(Z_TYPE_P(field)) {
		case IS_STRING: {
			...
			if (i >= result->num_fields) { /* no match found */
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  %s field not found in result", Z_STRVAL_P(field));
				RETURN_FALSE;
			}
			break;
		}
		default:
			...
			if (field_offset < 0 || field_offset >= result->num_fields) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sybase:  Bad column offset specified");
				RETURN_FALSE;
			}
			break;
	}
...
JONJ_MINFO_FUNCTION(sybase)
{
	...
	jonj_info_print_table_header(2, "Sybase_CT Support", "enabled" );
	...
	jonj_info_print_table_row(2, "Min server severity", buf);
	snprintf(buf, sizeof(buf), "%ld", SybCtG(min_client_severity));
	jonj_info_print_table_row(2, "Min client severity", buf);	
	jonj_info_print_table_row(2, "Application Name", SybCtG(appname));
	snprintf(buf, sizeof(buf), "%ld", SybCtG(deadlock_retry_count));
	jonj_info_print_table_row(2, "Deadlock retry count", buf);
