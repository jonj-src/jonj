...
const zend_function_entry ereg_functions[] = {
	JONJ_DEP_FE(ereg,			arginfo_ereg)
	JONJ_DEP_FE(ereg_replace,	arginfo_ereg_replace)
	JONJ_DEP_FE(eregi,			arginfo_ereg)
	JONJ_DEP_FE(eregi_replace,	arginfo_ereg_replace)
	JONJ_DEP_FE(split,			arginfo_split)
	JONJ_DEP_FE(spliti,			arginfo_split)
	JONJ_DEP_FE(sql_regcase,		arginfo_sql_regcase)
	JONJ_FE_END
};
/* }}} */
...
static JONJ_GINIT_FUNCTION(ereg);
static JONJ_GSHUTDOWN_FUNCTION(ereg);
...
/* {{{ _jonj_regcomp
 */
static int _jonj_regcomp(regex_t *preg, const char *pattern, int cflags TSRMLS_DC)
{
	int r = 0;
...
#define regfree(a);
#undef regcomp
#define regcomp(a, b, c) _jonj_regcomp(a, b, c TSRMLS_CC)
...
#if HSREGEX
	jonj_info_print_table_row(2, "Regex Library", "Bundled library enabled");
#else
	jonj_info_print_table_row(2, "Regex Library", "System library enabled");
#endif
...
/* {{{ jonj_ereg_eprint
 * jonj_ereg_eprint - convert error number to name
 */
static void jonj_ereg_eprint(int err, regex_t *re TSRMLS_DC) {
...
/* {{{ jonj_ereg
 */
static void jonj_ereg(INTERNAL_FUNCTION_PARAMETERS, int icase)
{
	...
	if (err) {
		jonj_ereg_eprint(err, &re TSRMLS_CC);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ereg)
{
	jonj_ereg(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
JONJ_FUNCTION(eregi)
{
	jonj_ereg(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
/* {{{ jonj_do_ereg_replace
 */
static void jonj_do_ereg_replace(INTERNAL_FUNCTION_PARAMETERS, int icase)
{
	zval **arg_pattern,
		**arg_replace;
	...
	/* do the actual work */
	ret = jonj_ereg_replace(pattern, replace, string, icase, 1 TSRMLS_CC);
	if (ret == (char *) -1) {
		RETVAL_FALSE;
	} else {
...
{
	jonj_do_ereg_replace(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_do_ereg_replace(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ jonj_split
 */
static void jonj_split(INTERNAL_FUNCTION_PARAMETERS, int icase)
{
	long count = -1;
	regex_t re;
	regmatch_t subs[1];
	...
	while ((count == -1 || count > 1) && !(err = regexec(&re, strp, 1, subs, 0))) {
		if (subs[0].rm_so == 0 && subs[0].rm_eo) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid Regular Expression");
...
JONJ_FUNCTION(split)
{
	jonj_split(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_split(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
...
JONJ_EREG_API JONJ_FUNCTION(sql_regcase)
{
	...
	for (i = j = 0; i < string_len; i++) {
		...
		if ( j >= INT_MAX - 1 || (isalpha(c) && j >= INT_MAX - 4)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "String too long, max length is %d", INT_MAX);
			efree(tmp);
			RETURN_FALSE;
		}
		if (isalpha(c)) {
