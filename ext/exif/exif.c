...
#include "ext/standard/jonj_image.h"
...
const zend_function_entry exif_functions[] = {
	JONJ_FE(exif_read_data, arginfo_exif_read_data)
	JONJ_FALIAS(read_exif_data, exif_read_data, arginfo_exif_read_data)
	JONJ_FE(exif_tagname, arginfo_exif_tagname)
	JONJ_FE(exif_thumbnail, arginfo_exif_thumbnail)
	JONJ_FE(exif_imagetype, arginfo_exif_imagetype)
	JONJ_FE_END
};
/* }}} */
...
/* {{{ JONJ_MINFO_FUNCTION
 */
JONJ_MINFO_FUNCTION(exif)
{
	jonj_info_print_table_start();
	jonj_info_print_table_row(2, "EXIF Support", "enabled");
	jonj_info_print_table_row(2, "EXIF Version", EXIF_VERSION);
	jonj_info_print_table_row(2, "Supported EXIF Version", "0220");
	jonj_info_print_table_row(2, "Supported filetypes", "JPEG,TIFF");
...
ZEND_INI_MH(OnUpdateEncode)
{
	...
	&return_list, &return_size, 0 TSRMLS_CC)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal encoding ignored: '%s'", new_value);
			return FAILURE;
		}
		efree(return_list);
	}
...
JONJ_INI_BEGIN()
    STD_JONJ_INI_ENTRY("exif.encode_unicode",          "ISO-8859-15", JONJ_INI_ALL, OnUpdateEncode, encode_unicode,    zend_exif_globals, exif_globals)
    STD_JONJ_INI_ENTRY("exif.decode_unicode_motorola", "UCS-2BE",     JONJ_INI_ALL, OnUpdateDecode, decode_unicode_be, zend_exif_globals, exif_globals)
    STD_JONJ_INI_ENTRY("exif.decode_unicode_intel",    "UCS-2LE",     JONJ_INI_ALL, OnUpdateDecode, decode_unicode_le, zend_exif_globals, exif_globals)
    STD_JONJ_INI_ENTRY("exif.encode_jis",              "",            JONJ_INI_ALL, OnUpdateEncode, encode_jis,        zend_exif_globals, exif_globals)
    STD_JONJ_INI_ENTRY("exif.decode_jis_motorola",     "JIS",         JONJ_INI_ALL, OnUpdateDecode, decode_jis_be,     zend_exif_globals, exif_globals)
    STD_JONJ_INI_ENTRY("exif.decode_jis_intel",        "JIS",         JONJ_INI_ALL, OnUpdateDecode, decode_jis_le,     zend_exif_globals, exif_globals)
JONJ_INI_END()
/* }}} */

/* {{{ JONJ_GINIT_FUNCTION
 */
static JONJ_GINIT_FUNCTION(exif)
{
...
static size_t jonj_strnlen(char* str, size_t maxlen) {
	size_t len = 0;
...
*/
static int jonj_tiff_bytes_per_format[] = {0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8, 1};
...
*/
static int jonj_jpg_get16(void *value)
{
...
static int jonj_ifd_get16u(void *value, int motorola_intel)
{
	if (motorola_intel) {
		return (((uchar *)value)[0] << 8) | ((uchar *)value)[1];
	} else {
...
static signed short jonj_ifd_get16s(void *value, int motorola_intel)
{
	return (signed short)jonj_ifd_get16u(value, motorola_intel);
}
/* }}} */

/* {{{ jonj_ifd_get32s
 * Convert a 32 bit signed value from file's native byte order */
static int jonj_ifd_get32s(void *value, int motorola_intel)
{
	if (motorola_intel) {
...
static unsigned jonj_ifd_get32u(void *value, int motorola_intel)
{
	return (unsigned)jonj_ifd_get32s(value, motorola_intel) & 0xffffffff;
}
/* }}} */

/* {{{ jonj_ifd_set16u
 * Write 16 bit unsigned value to data */
static void jonj_ifd_set16u(char *data, unsigned int value, int motorola_intel)
{
	if (motorola_intel) {
...
static void jonj_ifd_set32u(char *data, size_t value, int motorola_intel)
{
	if (motorola_intel) {
	...
	while(components > 0) {
		switch(format) {
			...
			case TAG_FMT_SSHORT:
				...
				snprintf(dump + len, 6 + 1, "0x%04X", jonj_ifd_get16s(value_ptr, motorola_intel));
				len += 6;
				value_ptr += 2;
				break;
			case TAG_FMT_ULONG:
			case TAG_FMT_SLONG:
				dump = erealloc(dump, len + 6 + 1);
				snprintf(dump + len, 6 + 1, "0x%04X", jonj_ifd_get32s(value_ptr, motorola_intel));
				len += 6;
				value_ptr += 4;
				break;
			...
			case TAG_FMT_SRATIONAL:
				...
				snprintf(dump + len, 13 + 1, "0x%04X/0x%04X", jonj_ifd_get32s(value_ptr, motorola_intel), jonj_ifd_get32s(value_ptr+4, motorola_intel));
				len += 13;
				value_ptr += 8;
				break;
		}
...
static double exif_convert_any_format(void *value, int format, int motorola_intel TSRMLS_DC)
{
	...
	switch(format) {
		...
		case TAG_FMT_USHORT:    return jonj_ifd_get16u(value, motorola_intel);
		case TAG_FMT_ULONG:     return jonj_ifd_get32u(value, motorola_intel);

		case TAG_FMT_URATIONAL:
			u_den = jonj_ifd_get32u(4+(char *)value, motorola_intel);
			if (u_den == 0) {
				return 0;
			} else {
				return (double)jonj_ifd_get32u(value, motorola_intel) / u_den;
			}

		case TAG_FMT_SRATIONAL:
			s_den = jonj_ifd_get32s(4+(char *)value, motorola_intel);
			if (s_den == 0) {
				return 0;
			} else {
				return (double)jonj_ifd_get32s(value, motorola_intel) / s_den;
			}

		case TAG_FMT_SSHORT:    return (signed short)jonj_ifd_get16u(value, motorola_intel);
		case TAG_FMT_SLONG:     return jonj_ifd_get32s(value, motorola_intel);

		/* Not sure if this is correct (never seen float used in Exif format) */
		case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Found value of type single");
#endif
			return (double)*(float *)value;
		case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Found value of type double");
#endif
			return *(double *)value;
	}
	return 0;
	...
	switch(format) {
		...
		case TAG_FMT_URATIONAL:
			...
			if (u_den == 0) {
				...
				return jonj_ifd_get32u(value, motorola_intel) / u_den;
			}
		...
		case TAG_FMT_SRATIONAL:
			...
			if (s_den == 0) {
				...
				return (size_t)((double)jonj_ifd_get32s(value, motorola_intel) / s_den);
			}

		case TAG_FMT_SSHORT:    return jonj_ifd_get16u(value, motorola_intel);
		case TAG_FMT_SLONG:     return jonj_ifd_get32s(value, motorola_intel);
...
static void exif_error_docref(const char *docref EXIFERR_DC, const image_info_type *ImageInfo, int type, const char *format, ...)
{
...
#ifdef EXIF_DEBUG
	{
		...
		jonj_verror(docref, ImageInfo->FileName?ImageInfo->FileName:"", type, buf, args TSRMLS_CC);
		efree(buf);
	}
#else
	jonj_verror(docref, ImageInfo->FileName?ImageInfo->FileName:"", type, format, args TSRMLS_CC);
#endif
	va_end(args);
}
/* }}} */
...
static void exif_iif_add_value(image_info_type *image_info, int section_index, char *name, int tag, int format, int length, void* value, int motorola_intel TSRMLS_DC)
{
	...
	switch (format) {
		case TAG_FMT_STRING:
			if (value) {
				length = jonj_strnlen(value, length);
		...
		case TAG_FMT_UNDEFINED:
			if (value) {
				if (tag == TAG_MAKER_NOTE) {
					length = (int) jonj_strnlen(value, length);
				}
		...
		case TAG_FMT_DOUBLE:
			...
			for (idex=0,vptr=value; idex<(size_t)length; idex++,vptr=(char *) vptr + jonj_tiff_bytes_per_format[format]) {
				if (length>1) {
					info_value = &info_data->value.list[idex];
				}
				switch (format) {
					case TAG_FMT_USHORT:
						info_value->u = jonj_ifd_get16u(vptr, motorola_intel);
						break;

					case TAG_FMT_ULONG:
						info_value->u = jonj_ifd_get32u(vptr, motorola_intel);
						break;

					case TAG_FMT_URATIONAL:
						info_value->ur.num = jonj_ifd_get32u(vptr, motorola_intel);
						info_value->ur.den = jonj_ifd_get32u(4+(char *)vptr, motorola_intel);
						break;

					case TAG_FMT_SSHORT:
						info_value->i = jonj_ifd_get16s(vptr, motorola_intel);
						break;

					case TAG_FMT_SLONG:
						info_value->i = jonj_ifd_get32s(vptr, motorola_intel);
						break;

					case TAG_FMT_SRATIONAL:
						info_value->sr.num = jonj_ifd_get32u(vptr, motorola_intel);
						info_value->sr.den = jonj_ifd_get32u(4+(char *)vptr, motorola_intel);
						break;

					case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Found value of type single");
#endif
						info_value->f = *(float *)value;

					case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Found value of type double");
#endif
...
#ifdef EXIF_DEBUG
/*		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Adding %d infos from section %s", image_info->info_list[section_index].count, exif_get_sectionname(section_index));*/
#endif
	if (image_info->info_list[section_index].count) {
		if (sub_array) {
			MAKE_STD_ZVAL(tmpi);
...
#ifdef EXIF_DEBUG
/*		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Adding infos: tag(0x%04X,%12s,L=0x%04X): %s", info_tag, exif_get_tagname(info_tag, buffer, -12, exif_get_tag_table(section_index) TSRMLS_CC), info_data->length, info_data->format==TAG_FMT_STRING?(info_value&&info_value->s?info_value->s:"<no data>"):exif_get_tagformat(info_data->format));*/
#endif
	...
	if (length>3) {
		switch(value[2]) {
			...
			case 1:
				...
				break;
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Undefined JPEG2000 comment encoding");
				break;
		}
	} else {
		exif_iif_add_tag(image_info, SECTION_COMMENT, "Comment", TAG_COMPUTED_VALUE, TAG_FMT_UNDEFINED, 0, NULL);
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "JPEG2000 comment section too small");
	}
}
#endif
/* }}} */
...
/* 0xFF SOSn SectLen(2) Bits(1) Height(2) Width(2) Channels(1)  3*Channels (1)  */
	...
	result->height          = jonj_jpg_get16(Data+3);
	result->width           = jonj_jpg_get16(Data+5);
...
static void* exif_ifd_make_value(image_info_data *info_data, int motorola_intel TSRMLS_DC) {
	...
	byte_count = jonj_tiff_bytes_per_format[info_data->format] * info_data->length;
	  ...
	  || (byte_count>1 && (info_data->format == TAG_FMT_BYTE || info_data->format == TAG_FMT_SBYTE))
		...
		*value_ptr = info_data->value.i;
		...
		for(i=0; i<info_data->length; i++) {
			...
			switch(info_data->format) {
				case TAG_FMT_USHORT:
					jonj_ifd_set16u(data_ptr, info_value->u, motorola_intel);
					data_ptr += 2;
					break;
				case TAG_FMT_ULONG:
					jonj_ifd_set32u(data_ptr, info_value->u, motorola_intel);
					data_ptr += 4;
					break;
				case TAG_FMT_SSHORT:
					jonj_ifd_set16u(data_ptr, info_value->i, motorola_intel);
					data_ptr += 2;
					break;
				case TAG_FMT_SLONG:
					jonj_ifd_set32u(data_ptr, info_value->i, motorola_intel);
					data_ptr += 4;
					break;
				case TAG_FMT_URATIONAL:
					jonj_ifd_set32u(data_ptr,   info_value->sr.num, motorola_intel);
					jonj_ifd_set32u(data_ptr+4, info_value->sr.den, motorola_intel);
					data_ptr += 8;
					break;
				case TAG_FMT_SRATIONAL:
					jonj_ifd_set32u(data_ptr,   info_value->ur.num, motorola_intel);
					jonj_ifd_set32u(data_ptr+4, info_value->ur.den, motorola_intel);
					data_ptr += 8;
					break;
				case TAG_FMT_SINGLE:
...
static void exif_thumbnail_build(image_info_type *ImageInfo TSRMLS_DC) {
	...
	switch(ImageInfo->Thumbnail.filetype) {
		...
		case IMAGE_FILETYPE_TIFF_MM:
			...
			new_data += 8;
			jonj_ifd_set16u(new_data, info_list->count, ImageInfo->motorola_intel);
			new_data += 2;
			for (i=0; i<info_list->count; i++) {
				info_data  = &info_list->list[i];
				byte_count = jonj_tiff_bytes_per_format[info_data->format] * info_data->length;
				...
				if (info_data->tag==TAG_STRIP_OFFSETS || info_data->tag==TAG_JPEG_INTERCHANGE_FORMAT) {
					jonj_ifd_set16u(new_data + 0, info_data->tag,    ImageInfo->motorola_intel);
					jonj_ifd_set16u(new_data + 2, TAG_FMT_ULONG,     ImageInfo->motorola_intel);
					jonj_ifd_set32u(new_data + 4, 1,                 ImageInfo->motorola_intel);
					jonj_ifd_set32u(new_data + 8, new_move,          ImageInfo->motorola_intel);
				} else {
					jonj_ifd_set16u(new_data + 0, info_data->tag,    ImageInfo->motorola_intel);
					jonj_ifd_set16u(new_data + 2, info_data->format, ImageInfo->motorola_intel);
					jonj_ifd_set32u(new_data + 4, info_data->length, ImageInfo->motorola_intel);
					value_ptr  = exif_ifd_make_value(info_data, ImageInfo->motorola_intel TSRMLS_CC);
					if (byte_count <= 4) {
						memmove(new_data+8, value_ptr, 4);
					} else {
						jonj_ifd_set32u(new_data+8, new_value, ImageInfo->motorola_intel);
#ifdef EXIF_DEBUG
...
static int exif_process_string(char **result, char *value, size_t byte_count TSRMLS_DC) {
	 ...
	 * memory page...so we use jonj_strnlen in conjunction with memcpy and add the NUL
	 * char.
	 * estrdup would sometimes allocate more memory and does not return length
	 */
	if ((byte_count=jonj_strnlen(value, byte_count)) > 0) {
...
static int exif_process_IFD_in_MAKERNOTE(image_info_type *ImageInfo, char * value_ptr, int value_len, char *offset_base, size_t IFDlength, size_t displacement TSRMLS_DC)
{
	...
	NumDirEntries = jonj_ifd_get16u(dir_start, ImageInfo->motorola_intel);
	...
	switch (maker_note->offset_mode) {
		...
		case MN_OFFSET_GUESS:
			...
			offset_diff = 2 + NumDirEntries*12 + 4 - jonj_ifd_get32u(dir_start+10, ImageInfo->motorola_intel);
#ifdef EXIF_DEBUG
...
/*	NextDirOffset (must be NULL) = jonj_ifd_get32u(dir_start+2+12*de, ImageInfo->motorola_intel);*/
#ifdef EXIF_DEBUG
...
static int exif_process_IFD_TAG(image_info_type *ImageInfo, char *dir_entry, char *offset_base, size_t IFDlength, size_t displacement, int section_index, int ReadNextIFD, tag_table_type tag_table TSRMLS_DC)
{
	...
	tag = jonj_ifd_get16u(dir_entry, ImageInfo->motorola_intel);
	format = jonj_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
	components = jonj_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel);
	...
	byte_count_signed = (int64_t)components * jonj_tiff_bytes_per_format[format];
	...
	if (byte_count > 4) {
		offset_val = jonj_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
        ...
        */
		if (byte_count > IFDlength || offset_val > IFDlength-byte_count || value_ptr < dir_entry || offset_val < (size_t)(dir_entry-offset_base)) {
			...
			fpos = jonj_stream_tell(ImageInfo->infile);
			jonj_stream_seek(ImageInfo->infile, displacement+offset_val, SEEK_SET);
			fgot = jonj_stream_tell(ImageInfo->infile);
			...
			fgot = jonj_stream_read(ImageInfo->infile, value_ptr, byte_count);
			jonj_stream_seek(ImageInfo->infile, fpos, SEEK_SET);
			if (fgot<byte_count) {
				EFREE_IF(outside);
	...
	if (section_index==SECTION_THUMBNAIL) {
		...
		switch(tag) {
			case TAG_COPYRIGHT:
				...
				if (byte_count>1 && (length=jonj_strnlen(value_ptr, byte_count)) > 0) {
			...
			case TAG_INTEROP_IFD_POINTER:
				if (ReadNextIFD) {
					...
					Subdir_start = offset_base + jonj_ifd_get32u(value_ptr, ImageInfo->motorola_intel);
...
static int exif_process_IFD_in_JPEG(image_info_type *ImageInfo, char *dir_start, char *offset_base, size_t IFDlength, size_t displacement, int section_index TSRMLS_DC)
{
	...
	NextDirOffset = jonj_ifd_get32u(dir_start+2+12*de, ImageInfo->motorola_intel);
	if (NextDirOffset) {
...
static void exif_process_TIFF_in_JPEG(image_info_type *ImageInfo, char *CharBuf, size_t length, size_t displacement TSRMLS_DC)
{
	...
	exif_value_2a = jonj_ifd_get16u(CharBuf+2, ImageInfo->motorola_intel);
	offset_of_ifd = jonj_ifd_get32u(CharBuf+4, ImageInfo->motorola_intel);
...
static int exif_scan_JPEG_header(image_info_type *ImageInfo TSRMLS_DC)
{
	...
	for(section=0;;section++) {
		...
		do {
			if ((marker = jonj_stream_getc(ImageInfo->infile)) == EOF) {
		...
		if ((lh = jonj_stream_getc(ImageInfo->infile)) == EOF) {
			EXIF_ERRLOG_CORRUPT(ImageInfo)
			return FALSE;
		}
		if ((ll = jonj_stream_getc(ImageInfo->infile)) == EOF) {
			EXIF_ERRLOG_CORRUPT(ImageInfo)
			return FALSE;
		}
		...
		got = jonj_stream_read(ImageInfo->infile, (char*)(Data+2), itemlen-2); /* Read the whole section. */
		if (got != itemlen-2) {
		...
		switch(marker) {
			case M_SOS:   /* stop before hitting compressed data  */
				...
				if (ImageInfo->read_all) {
					...
					Data = ImageInfo->file.list[sn].data;
					got = jonj_stream_read(ImageInfo->infile, (char*)Data, size);
					if (got != size) {
						EXIF_ERRLOG_FILEEOF(ImageInfo)
						return FALSE;
					}
...
static int exif_scan_thumbnail(image_info_type *ImageInfo TSRMLS_DC)
{
	...
	for (;;) {
		...
		if (c == 0xFF)
			return FALSE;
		marker = c;
		length = jonj_jpg_get16(data+pos);
...
static int exif_process_IFD_in_TIFF(image_info_type *ImageInfo, size_t dir_offset, int section_index TSRMLS_DC)
{
	...
	if (ImageInfo->FileSize >= dir_offset+2) {
...
#endif
		jonj_stream_seek(ImageInfo->infile, dir_offset, SEEK_SET); /* we do not know the order of sections */
		jonj_stream_read(ImageInfo->infile, (char*)ImageInfo->file.list[sn].data, 2);
		num_entries = jonj_ifd_get16u(ImageInfo->file.list[sn].data, ImageInfo->motorola_intel);
		...
		if (ImageInfo->FileSize >= dir_offset+dir_size) {
			...
			jonj_stream_read(ImageInfo->infile, (char*)(ImageInfo->file.list[sn].data+2), dir_size-2);
			/*exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "Dump: %s", exif_char_dump(ImageInfo->file.list[sn].data, dir_size, 0));*/
			next_offset = jonj_ifd_get32u(ImageInfo->file.list[sn].data + dir_size - 4, ImageInfo->motorola_intel);
#ifdef EXIF_DEBUG
			...
			for(i=0;i<num_entries;i++) {
				...
				entry_tag    = jonj_ifd_get16u(dir_entry+0, ImageInfo->motorola_intel);
				entry_type   = jonj_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
				if (entry_type > NUM_FORMATS) {
					...
					/* jonj_ifd_set16u(dir_entry+2, entry_type, ImageInfo->motorola_intel);*/
				}
				entry_length = jonj_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel) * jonj_tiff_bytes_per_format[entry_type];
				if (entry_length <= 4) {
					switch(entry_type) {
						case TAG_FMT_USHORT:
							entry_value  = jonj_ifd_get16u(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_SSHORT:
							entry_value  = jonj_ifd_get16s(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_ULONG:
							entry_value  = jonj_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_SLONG:
							entry_value  = jonj_ifd_get32s(dir_entry+8, ImageInfo->motorola_intel);
							break;
					}
					switch(entry_tag) {
						case TAG_IMAGEWIDTH:
					...
					entry_offset = jonj_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
			...
			if (ImageInfo->FileSize >= dir_offset + ImageInfo->file.list[sn].size) {
...
#endif
					jonj_stream_read(ImageInfo->infile, (char*)(ImageInfo->file.list[sn].data+dir_size), ifd_size-dir_size);
#ifdef EXIF_DEBUG
				...
				for(i=0;i<num_entries;i++) {
					...
					entry_tag    = jonj_ifd_get16u(dir_entry+0, ImageInfo->motorola_intel);
					entry_type   = jonj_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
					/*entry_length = jonj_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel);*/
						...
						if (section_index!=SECTION_THUMBNAIL && entry_tag==TAG_SUB_IFD) {
							...
							&&  ImageInfo->read_thumbnail
								...
								if (!ImageInfo->Thumbnail.data) {
									...
									jonj_stream_seek(ImageInfo->infile, ImageInfo->Thumbnail.offset, SEEK_SET);
									fgot = jonj_stream_read(ImageInfo->infile, ImageInfo->Thumbnail.data, ImageInfo->Thumbnail.size);
...
static int exif_scan_FILE_header(image_info_type *ImageInfo TSRMLS_DC)
{
	...
	if (ImageInfo->FileSize >= 2) {
		jonj_stream_seek(ImageInfo->infile, 0, SEEK_SET);
		if (jonj_stream_read(ImageInfo->infile, (char*)file_header, 2) != 2) {
			return FALSE;
		}
		if ((file_header[0]==0xff) && (file_header[1]==M_SOI)) {
			...
			if (jonj_stream_read(ImageInfo->infile, (char*)(file_header+2), 6) != 6) {
				return FALSE;
			}
			if (!memcmp(file_header, "II\x2A\x00", 4)) {
				...
				if (exif_process_IFD_in_TIFF(ImageInfo,
											 jonj_ifd_get32u(file_header + 4, ImageInfo->motorola_intel),
...
static int exif_read_file(image_info_type *ImageInfo, char *FileName, int read_thumbnail, int read_all TSRMLS_DC)
{
	...
	ImageInfo->infile = jonj_stream_open_wrapper(FileName, "rb", STREAM_MUST_SEEK|IGNORE_PATH, NULL);
	if (!ImageInfo->infile) {
		exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_WARNING, "Unable to open file");
		return FALSE;
	}

	if (jonj_stream_is(ImageInfo->infile, JONJ_STREAM_IS_STDIO)) {
		if (VCWD_STAT(FileName, &st) >= 0) {
			if ((st.st_mode & S_IFMT) != S_IFREG) {
				exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_WARNING, "Not a file");
				jonj_stream_close(ImageInfo->infile);
				return FALSE;
			}
		...
		if (!ImageInfo->FileSize) {
			jonj_stream_seek(ImageInfo->infile, 0, SEEK_END);
			ImageInfo->FileSize = jonj_stream_tell(ImageInfo->infile);
			jonj_stream_seek(ImageInfo->infile, 0, SEEK_SET);
		}
	}

	jonj_basename(FileName, strlen(FileName), NULL, 0, &(ImageInfo->FileName), NULL TSRMLS_CC);
...
JONJ_FUNCTION(exif_read_data)
{
	...
	exif_iif_add_str(&ImageInfo, SECTION_FILE, "MimeType",      (char*)jonj_image_type_to_mime_type(ImageInfo.FileType) TSRMLS_CC);
	...
	if (ImageInfo.Thumbnail.size) {
		...
		exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Thumbnail.MimeType", (char*)jonj_image_type_to_mime_type(ImageInfo.Thumbnail.filetype) TSRMLS_CC);
	}
	if (ImageInfo.Thumbnail.width && ImageInfo.Thumbnail.height) {
...
#ifdef EXIF_DEBUG
	jonj_error_docref1(NULL TSRMLS_CC, Z_STRVAL_PP(p_name), E_NOTICE, "done");
#endif
}
/* }}} */
...
#ifdef EXIF_DEBUG
	jonj_error_docref1(NULL TSRMLS_CC, p_name, E_NOTICE, "Done");
#endif
}
/* }}} */
...
JONJ_FUNCTION(exif_imagetype)
{
	...
	stream = jonj_stream_open_wrapper(imagefile, "rb", IGNORE_PATH|REPORT_ERRORS, NULL);

	if (stream == NULL) {
		RETURN_FALSE;
	}

	itype = jonj_getimagetype(stream, NULL TSRMLS_CC);
