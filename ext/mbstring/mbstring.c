...
/*
 * JONJ 4 Multibyte String module "mbstring"
 *
 * History:
 *   2000.5.19  Release jonj-4.0RC2_jstring-1.0
 *   2001.4.1   Release jonj4_jstring-1.0.91
 *   2001.4.30  Release jonj4_jstring-1.1 (contribute to The JONJ Group)
...
#include "SAPI.h"
#include "jonj_unicode.h"
#include "TSRM.h"

#include "mb_gpc.h"

#if HAVE_MBREGEX
#include "jonj_mbregex.h"
#endif

#include "zend_multibyte.h"

#if HAVE_ONIG
#include "jonj_onig_compat.h"
#include <oniguruma.h>
#undef UChar
...
static JONJ_GINIT_FUNCTION(mbstring);
static JONJ_GSHUTDOWN_FUNCTION(mbstring);

static void jonj_mb_populate_current_detect_order_list(TSRMLS_D);

static int jonj_mb_encoding_translation(TSRMLS_D);

static void jonj_mb_gpc_get_detect_order(const zend_encoding ***list, size_t *list_size TSRMLS_DC);

static void jonj_mb_gpc_set_input_encoding(const zend_encoding *encoding TSRMLS_DC);

/* }}} */

/* {{{ jonj_mb_default_identify_list */
typedef struct _jonj_mb_nls_ident_list {
	enum mbfl_no_language lang;
	const enum mbfl_no_encoding *list;
	size_t list_size;
} jonj_mb_nls_ident_list;

static const enum mbfl_no_encoding jonj_mb_default_identify_list_ja[] = {
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_cn[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_tw_hk[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_kr[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_ru[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_hy[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
	mbfl_no_encoding_armscii8
};

static const enum mbfl_no_encoding jonj_mb_default_identify_list_tr[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
...
static const enum mbfl_no_encoding jonj_mb_default_identify_list_ua[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8,
	mbfl_no_encoding_koi8u
};

static const enum mbfl_no_encoding jonj_mb_default_identify_list_neut[] = {
	mbfl_no_encoding_ascii,
	mbfl_no_encoding_utf8
};


static const jonj_mb_nls_ident_list jonj_mb_default_identify_list[] = {
	{ mbfl_no_language_japanese, jonj_mb_default_identify_list_ja, sizeof(jonj_mb_default_identify_list_ja) / sizeof(jonj_mb_default_identify_list_ja[0]) },
	{ mbfl_no_language_korean, jonj_mb_default_identify_list_kr, sizeof(jonj_mb_default_identify_list_kr) / sizeof(jonj_mb_default_identify_list_kr[0]) },
	{ mbfl_no_language_traditional_chinese, jonj_mb_default_identify_list_tw_hk, sizeof(jonj_mb_default_identify_list_tw_hk) / sizeof(jonj_mb_default_identify_list_tw_hk[0]) },
	{ mbfl_no_language_simplified_chinese, jonj_mb_default_identify_list_cn, sizeof(jonj_mb_default_identify_list_cn) / sizeof(jonj_mb_default_identify_list_cn[0]) },
	{ mbfl_no_language_russian, jonj_mb_default_identify_list_ru, sizeof(jonj_mb_default_identify_list_ru) / sizeof(jonj_mb_default_identify_list_ru[0]) },
	{ mbfl_no_language_armenian, jonj_mb_default_identify_list_hy, sizeof(jonj_mb_default_identify_list_hy) / sizeof(jonj_mb_default_identify_list_hy[0]) },
	{ mbfl_no_language_turkish, jonj_mb_default_identify_list_tr, sizeof(jonj_mb_default_identify_list_tr) / sizeof(jonj_mb_default_identify_list_tr[0]) },
	{ mbfl_no_language_ukrainian, jonj_mb_default_identify_list_ua, sizeof(jonj_mb_default_identify_list_ua) / sizeof(jonj_mb_default_identify_list_ua[0]) },
	{ mbfl_no_language_neutral, jonj_mb_default_identify_list_neut, sizeof(jonj_mb_default_identify_list_neut) / sizeof(jonj_mb_default_identify_list_neut[0]) }
};
...
const zend_function_entry mbstring_functions[] = {
	JONJ_FE(mb_convert_case,			arginfo_mb_convert_case)
	JONJ_FE(mb_strtoupper,			arginfo_mb_strtoupper)
	JONJ_FE(mb_strtolower,			arginfo_mb_strtolower)
	JONJ_FE(mb_language,				arginfo_mb_language)
	JONJ_FE(mb_internal_encoding,	arginfo_mb_internal_encoding)
	JONJ_FE(mb_http_input,			arginfo_mb_http_input)
	JONJ_FE(mb_http_output,			arginfo_mb_http_output)
	JONJ_FE(mb_detect_order,			arginfo_mb_detect_order)
	JONJ_FE(mb_substitute_character,	arginfo_mb_substitute_character)
	JONJ_FE(mb_parse_str,			arginfo_mb_parse_str)
	JONJ_FE(mb_output_handler,		arginfo_mb_output_handler)
	JONJ_FE(mb_preferred_mime_name,	arginfo_mb_preferred_mime_name)
	JONJ_FE(mb_strlen,				arginfo_mb_strlen)
	JONJ_FE(mb_strpos,				arginfo_mb_strpos)
	JONJ_FE(mb_strrpos,				arginfo_mb_strrpos)
	JONJ_FE(mb_stripos,				arginfo_mb_stripos)
	JONJ_FE(mb_strripos,				arginfo_mb_strripos)
	JONJ_FE(mb_strstr,				arginfo_mb_strstr)
	JONJ_FE(mb_strrchr,				arginfo_mb_strrchr)
	JONJ_FE(mb_stristr,				arginfo_mb_stristr)
	JONJ_FE(mb_strrichr,				arginfo_mb_strrichr)
	JONJ_FE(mb_substr_count,			arginfo_mb_substr_count)
	JONJ_FE(mb_substr,				arginfo_mb_substr)
	JONJ_FE(mb_strcut,				arginfo_mb_strcut)
	JONJ_FE(mb_strwidth,				arginfo_mb_strwidth)
	JONJ_FE(mb_strimwidth,			arginfo_mb_strimwidth)
	JONJ_FE(mb_convert_encoding,		arginfo_mb_convert_encoding)
	JONJ_FE(mb_detect_encoding,		arginfo_mb_detect_encoding)
	JONJ_FE(mb_list_encodings,		arginfo_mb_list_encodings)
	JONJ_FE(mb_encoding_aliases,		arginfo_mb_encoding_aliases)
	JONJ_FE(mb_convert_kana,			arginfo_mb_convert_kana)
	JONJ_FE(mb_encode_mimeheader,	arginfo_mb_encode_mimeheader)
	JONJ_FE(mb_decode_mimeheader,	arginfo_mb_decode_mimeheader)
	JONJ_FE(mb_convert_variables,	arginfo_mb_convert_variables)
	JONJ_FE(mb_encode_numericentity,	arginfo_mb_encode_numericentity)
	JONJ_FE(mb_decode_numericentity,	arginfo_mb_decode_numericentity)
	JONJ_FE(mb_send_mail,			arginfo_mb_send_mail)
	JONJ_FE(mb_get_info,				arginfo_mb_get_info)
	JONJ_FE(mb_check_encoding,		arginfo_mb_check_encoding)
#if HAVE_MBREGEX
...
zend_module_entry mbstring_module_entry = {
	...
	mbstring_functions,
	JONJ_MINIT(mbstring),
	JONJ_MSHUTDOWN(mbstring),
	JONJ_RINIT(mbstring),
	JONJ_RSHUTDOWN(mbstring),
	JONJ_MINFO(mbstring),
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(mbstring),
	JONJ_GINIT(mbstring),
	JONJ_GSHUTDOWN(mbstring),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};
/* }}} */

/* {{{ static sapi_post_entry jonj_post_entries[] */
static sapi_post_entry jonj_post_entries[] = {
	{ DEFAULT_POST_CONTENT_TYPE, sizeof(DEFAULT_POST_CONTENT_TYPE)-1, sapi_read_standard_form_data,	jonj_std_post_handler },
...
/* {{{ allocators */
static void *_jonj_mb_allocators_malloc(unsigned int sz)
{
	return emalloc(sz);
}

static void *_jonj_mb_allocators_realloc(void *ptr, unsigned int sz)
{
	return erealloc(ptr, sz);
}

static void *_jonj_mb_allocators_calloc(unsigned int nelems, unsigned int szelem)
{
	return ecalloc(nelems, szelem);
}

static void _jonj_mb_allocators_free(void *ptr)
{
	efree(ptr);
}

static void *_jonj_mb_allocators_pmalloc(unsigned int sz)
{
	return pemalloc(sz, 1);
}

static void *_jonj_mb_allocators_prealloc(void *ptr, unsigned int sz)
{
	return perealloc(ptr, sz, 1);
}

static void _jonj_mb_allocators_pfree(void *ptr)
{
	pefree(ptr, 1);
}

static mbfl_allocators _jonj_mb_allocators = {
	_jonj_mb_allocators_malloc,
	_jonj_mb_allocators_realloc,
	_jonj_mb_allocators_calloc,
	_jonj_mb_allocators_free,
	_jonj_mb_allocators_pmalloc,
	_jonj_mb_allocators_prealloc,
	_jonj_mb_allocators_pfree
};
/* }}} */

/* {{{ static sapi_post_entry mbstr_post_entries[] */
static sapi_post_entry mbstr_post_entries[] = {
	{ DEFAULT_POST_CONTENT_TYPE, sizeof(DEFAULT_POST_CONTENT_TYPE)-1, sapi_read_standard_form_data, jonj_mb_post_handler },
	{ MULTIPART_CONTENT_TYPE,    sizeof(MULTIPART_CONTENT_TYPE)-1,    NULL,                         rfc1867_post_handler },
	{ NULL, 0, NULL, NULL }
};
/* }}} */

/* {{{ static int jonj_mb_parse_encoding_list()
 *  Return 0 if input contains any illegal encoding, otherwise 1.
 *  Even if any illegal encoding is detected the result may contain a list
 *  of parsed encodings.
 */
static int
jonj_mb_parse_encoding_list(const char *value, size_t value_length, const mbfl_encoding ***return_list, size_t *return_size, int persistent TSRMLS_DC)
{
	...
	if (value == NULL || value_length <= 0) {
		...
		n = 1;
		p1 = tmpstr;
		while ((p2 = jonj_memnstr(p1, ",", 1, endp)) != NULL) {
			p1 = p2 + 1;
			n++;
		}
...
/* {{{ static int jonj_mb_parse_encoding_array()
 *  Return 0 if input contains any illegal encoding, otherwise 1.
 *  Even if any illegal encoding is detected the result may contain a list
 *  of parsed encodings.
 */
static int
jonj_mb_parse_encoding_array(zval *array, const mbfl_encoding ***return_list, size_t *return_size, int persistent TSRMLS_DC)
{
	zval **hash_entry;
...
static const zend_encoding* jonj_mb_zend_encoding_fetcher(const char *encoding_name TSRMLS_DC)
{
	return (const zend_encoding*)mbfl_name2encoding(encoding_name);
}

static const char *jonj_mb_zend_encoding_name_getter(const zend_encoding *encoding)
{
	return ((const mbfl_encoding *)encoding)->name;
}

static int jonj_mb_zend_encoding_lexer_compatibility_checker(const zend_encoding *_encoding)
{
...
static const zend_encoding *jonj_mb_zend_encoding_detector(const unsigned char *arg_string, size_t arg_length, const zend_encoding **list, size_t list_size TSRMLS_DC)
{
	mbfl_string string;
...
static size_t jonj_mb_zend_encoding_converter(unsigned char **to, size_t *to_length, const unsigned char *from, size_t from_length, const zend_encoding *encoding_to, const zend_encoding *encoding_from TSRMLS_DC)
{
...
static int jonj_mb_zend_encoding_list_parser(const char *encoding_list, size_t encoding_list_len, const zend_encoding ***return_list, size_t *return_size, int persistent TSRMLS_DC)
{
	return jonj_mb_parse_encoding_list(encoding_list, encoding_list_len, (const mbfl_encoding ***)return_list, return_size, persistent TSRMLS_CC);
}

static const zend_encoding *jonj_mb_zend_internal_encoding_getter(TSRMLS_D)
{
	return (const zend_encoding *)MBSTRG(internal_encoding);
}

static int jonj_mb_zend_internal_encoding_setter(const zend_encoding *encoding TSRMLS_DC)
{
	MBSTRG(internal_encoding) = (const mbfl_encoding *)encoding;
	return SUCCESS;
}

static zend_multibyte_functions jonj_mb_zend_multibyte_functions = {
	"mbstring",
	jonj_mb_zend_encoding_fetcher,
	jonj_mb_zend_encoding_name_getter,
	jonj_mb_zend_encoding_lexer_compatibility_checker,
	jonj_mb_zend_encoding_detector,
	jonj_mb_zend_encoding_converter,
	jonj_mb_zend_encoding_list_parser,
	jonj_mb_zend_internal_encoding_getter,
	jonj_mb_zend_internal_encoding_setter
};
/* }}} */

static void *_jonj_mb_compile_regex(const char *pattern TSRMLS_DC);
static int _jonj_mb_match_regex(void *opaque, const char *str, size_t str_len);
static void _jonj_mb_free_regex(void *opaque);

#if HAVE_ONIG
/* {{{ _jonj_mb_compile_regex */
static void *_jonj_mb_compile_regex(const char *pattern TSRMLS_DC)
{
	jonj_mb_regex_t *retval;
	...
	if ((err_code = onig_new(&retval,
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s: %s", pattern, err_str);
		retval = NULL;
	}
	return retval;
}
...
/* {{{ _jonj_mb_match_regex */
static int _jonj_mb_match_regex(void *opaque, const char *str, size_t str_len)
{
	return onig_search((jonj_mb_regex_t *)opaque, (const OnigUChar *)str,
...
static void _jonj_mb_free_regex(void *opaque)
{
	onig_free((jonj_mb_regex_t *)opaque);
}
/* }}} */
#elif HAVE_PCRE || HAVE_BUNDLED_PCRE
/* {{{ _jonj_mb_compile_regex */
static void *_jonj_mb_compile_regex(const char *pattern TSRMLS_DC)
{
	...
	if (!(retval = pcre_compile(pattern,
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s (offset=%d): %s", pattern, err_offset, err_str);
	}
	return retval;
}
/* }}} */
...
/* {{{ jonj_mb_nls_get_default_detect_order_list */
static int jonj_mb_nls_get_default_detect_order_list(enum mbfl_no_language lang, enum mbfl_no_encoding **plist, size_t *plist_size)
{
	size_t i;

	*plist = (enum mbfl_no_encoding *) jonj_mb_default_identify_list_neut;
	*plist_size = sizeof(jonj_mb_default_identify_list_neut) / sizeof(jonj_mb_default_identify_list_neut[0]);

	for (i = 0; i < sizeof(jonj_mb_default_identify_list) / sizeof(jonj_mb_default_identify_list[0]); i++) {
		if (jonj_mb_default_identify_list[i].lang == lang) {
			*plist = (enum mbfl_no_encoding *)jonj_mb_default_identify_list[i].list;
			*plist_size = jonj_mb_default_identify_list[i].list_size;
			return 1;
		}
	}
	return 0;
...
static char *jonj_mb_rfc1867_substring_conf(const zend_encoding *encoding, char *start, int len, char quote TSRMLS_DC)
{
	...
	for (i = 0; i < len && start[i] != quote; ++i) {
		if (start[i] == '\\' && (start[i + 1] == '\\' || (quote && start[i + 1] == quote))) {
			*resp++ = start[++i];
			...
			size_t j = jonj_mb_mbchar_bytes_ex(start+i, (const mbfl_encoding *)encoding);
...
static char *jonj_mb_rfc1867_getword(const zend_encoding *encoding, char **line, char stop TSRMLS_DC) /* {{{ */
{
	char *pos = *line, quote;
	char *res;
	...
	while (*pos && *pos != stop) {
		if ((quote = *pos) == '"' || quote == '\'') {
			...
			pos += jonj_mb_mbchar_bytes_ex(pos, (const mbfl_encoding *)encoding);
...
static char *jonj_mb_rfc1867_getword_conf(const zend_encoding *encoding, char *str TSRMLS_DC) /* {{{ */
{
	...
	if (*str == '"' || *str == '\'') {
		...
		str++;
		return jonj_mb_rfc1867_substring_conf(encoding, str, strlen(str), quote TSRMLS_CC);
	} else {
		char *strend = str;
		...
		return jonj_mb_rfc1867_substring_conf(encoding, str, strend - str, 0 TSRMLS_CC);
	}
}
/* }}} */

static char *jonj_mb_rfc1867_basename(const zend_encoding *encoding, char *filename TSRMLS_DC) /* {{{ */
{
	char *s, *s2;
	...
	s = jonj_mb_safe_strrchr_ex(filename, '\\', filename_len, (const mbfl_encoding *)encoding);
	s2 = jonj_mb_safe_strrchr_ex(filename, '/', filename_len, (const mbfl_encoding *)encoding);
...
/* {{{ jonj.ini directive handler */
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_language) */
static JONJ_INI_MH(OnUpdate_mbstring_language)
{
	...
	jonj_mb_nls_get_default_detect_order_list(no_language, &MBSTRG(default_detect_order_list), &MBSTRG(default_detect_order_list_size));
	return SUCCESS;
}
/* }}} */

/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_detect_order) */
static JONJ_INI_MH(OnUpdate_mbstring_detect_order)
{
	...
	if (FAILURE == jonj_mb_parse_encoding_list(new_value, new_value_length, &list, &size, 1 TSRMLS_CC)) {
		return FAILURE;
	}
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_http_input) */
static JONJ_INI_MH(OnUpdate_mbstring_http_input)
{
	const mbfl_encoding **list;
	size_t size;
	...
	if (!new_value || !new_value_length) {
		...
		if (SUCCESS == jonj_mb_parse_encoding_list(get_input_encoding(TSRMLS_C), strlen(get_input_encoding(TSRMLS_C))+1, &list, &size, 1 TSRMLS_CC)) {
	...
	if (stage & (JONJ_INI_STAGE_ACTIVATE | JONJ_INI_STAGE_RUNTIME)) {
		jonj_error_docref("ref.mbstring" TSRMLS_CC, E_DEPRECATED, "Use of mbstring.http_input is deprecated");
	}
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_http_output) */
static JONJ_INI_MH(OnUpdate_mbstring_http_output)
{
	...
	if (stage & (JONJ_INI_STAGE_ACTIVATE | JONJ_INI_STAGE_RUNTIME)) {
		jonj_error_docref("ref.mbstring" TSRMLS_CC, E_DEPRECATED, "Use of mbstring.http_output is deprecated");
	}
...
/* {{{ static _jonj_mb_ini_mbstring_internal_encoding_set */
int _jonj_mb_ini_mbstring_internal_encoding_set(const char *new_value, uint new_value_length TSRMLS_DC)
{
...
#if HAVE_MBREGEX
	{
		...
		if (FAILURE == jonj_mb_regex_set_default_mbctype(enc_name TSRMLS_CC)) {
			/* falls back to UTF-8 if an unknown encoding name is given */
			enc_name = "UTF-8";
			jonj_mb_regex_set_default_mbctype(enc_name TSRMLS_CC);
		}
		jonj_mb_regex_set_mbctype(new_value TSRMLS_CC);
	}
#endif
	return SUCCESS;
}
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_internal_encoding) */
static JONJ_INI_MH(OnUpdate_mbstring_internal_encoding)
{
	if (stage & (JONJ_INI_STAGE_ACTIVATE | JONJ_INI_STAGE_RUNTIME)) {
		jonj_error_docref("ref.mbstring" TSRMLS_CC, E_DEPRECATED, "Use of mbstring.internal_encoding is deprecated");
	}
	...
	if (stage & (JONJ_INI_STAGE_STARTUP | JONJ_INI_STAGE_SHUTDOWN | JONJ_INI_STAGE_RUNTIME)) {
		if (new_value && new_value_length) {
			return _jonj_mb_ini_mbstring_internal_encoding_set(new_value, new_value_length TSRMLS_CC);
		} else {
			return _jonj_mb_ini_mbstring_internal_encoding_set(get_internal_encoding(TSRMLS_C), strlen(get_internal_encoding(TSRMLS_C))+1 TSRMLS_CC);
		}
	} else {
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_substitute_character) */
static JONJ_INI_MH(OnUpdate_mbstring_substitute_character)
{
	int c;
	char *endptr = NULL;
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_encoding_translation) */
static JONJ_INI_MH(OnUpdate_mbstring_encoding_translation)
{
	if (new_value == NULL) {
		return FAILURE;
...
/* {{{ static JONJ_INI_MH(OnUpdate_mbstring_http_output_conv_mimetypes */
static JONJ_INI_MH(OnUpdate_mbstring_http_output_conv_mimetypes)
{
	zval tmp;
	void *re = NULL;
	...
	jonj_trim(new_value, new_value_length, NULL, 0, &tmp, 3 TSRMLS_CC);

	if (Z_STRLEN(tmp) > 0) {
		if (!(re = _jonj_mb_compile_regex(Z_STRVAL(tmp) TSRMLS_CC))) {
			zval_dtor(&tmp);
			return FAILURE;
		}
	}
	...
	if (MBSTRG(http_output_conv_mimetypes)) {
		_jonj_mb_free_regex(MBSTRG(http_output_conv_mimetypes));
	}
...
/* {{{ jonj.ini directive registration */
JONJ_INI_BEGIN()
	JONJ_INI_ENTRY("mbstring.language", "neutral", JONJ_INI_ALL, OnUpdate_mbstring_language)
	JONJ_INI_ENTRY("mbstring.detect_order", NULL, JONJ_INI_ALL, OnUpdate_mbstring_detect_order)
	JONJ_INI_ENTRY("mbstring.http_input", NULL, JONJ_INI_ALL, OnUpdate_mbstring_http_input)
	JONJ_INI_ENTRY("mbstring.http_output", NULL, JONJ_INI_ALL, OnUpdate_mbstring_http_output)
	STD_JONJ_INI_ENTRY("mbstring.internal_encoding", NULL, JONJ_INI_ALL, OnUpdate_mbstring_internal_encoding, internal_encoding_name, zend_mbstring_globals, mbstring_globals)
	JONJ_INI_ENTRY("mbstring.substitute_character", NULL, JONJ_INI_ALL, OnUpdate_mbstring_substitute_character)
	STD_JONJ_INI_ENTRY("mbstring.func_overload", "0",
	JONJ_INI_SYSTEM, OnUpdateLong, func_overload, zend_mbstring_globals, mbstring_globals)

	STD_JONJ_INI_BOOLEAN("mbstring.encoding_translation", "0",
		JONJ_INI_SYSTEM | JONJ_INI_PERDIR,
		OnUpdate_mbstring_encoding_translation,
		encoding_translation, zend_mbstring_globals, mbstring_globals)
	JONJ_INI_ENTRY("mbstring.http_output_conv_mimetypes",
		"^(text/|application/xhtml\\+xml)",
		JONJ_INI_ALL,
		OnUpdate_mbstring_http_output_conv_mimetypes)

	STD_JONJ_INI_BOOLEAN("mbstring.strict_detection", "0",
		JONJ_INI_ALL,
		OnUpdateLong,
...
static JONJ_GINIT_FUNCTION(mbstring)
{
	...
	mbstring_globals->default_detect_order_list = (enum mbfl_no_encoding *) jonj_mb_default_identify_list_neut;
	mbstring_globals->default_detect_order_list_size = sizeof(jonj_mb_default_identify_list_neut) / sizeof(jonj_mb_default_identify_list_neut[0]);
...
#if HAVE_MBREGEX
	mbstring_globals->mb_regex_globals = jonj_mb_regex_globals_alloc(TSRMLS_C);
#endif
}
/* }}} */
...
static JONJ_GSHUTDOWN_FUNCTION(mbstring)
{
	...
	if (mbstring_globals->http_output_conv_mimetypes) {
		_jonj_mb_free_regex(mbstring_globals->http_output_conv_mimetypes);
	}
#if HAVE_MBREGEX
	jonj_mb_regex_globals_free(mbstring_globals->mb_regex_globals TSRMLS_CC);
#endif
}
/* }}} */
...
JONJ_MINIT_FUNCTION(mbstring)
{
	__mbfl_allocators = &_jonj_mb_allocators;
	...
	REGISTER_LONG_CONSTANT("MB_CASE_UPPER", JONJ_UNICODE_CASE_UPPER, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("MB_CASE_LOWER", JONJ_UNICODE_CASE_LOWER, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("MB_CASE_TITLE", JONJ_UNICODE_CASE_TITLE, CONST_CS | CONST_PERSISTENT);

#if HAVE_MBREGEX
	JONJ_MINIT(mb_regex) (INIT_FUNC_ARGS_PASSTHRU);
#endif

	if (FAILURE == zend_multibyte_set_functions(&jonj_mb_zend_multibyte_functions TSRMLS_CC)) {
		return FAILURE;
	}

	jonj_rfc1867_set_multibyte_callbacks(
		jonj_mb_encoding_translation,
		jonj_mb_gpc_get_detect_order,
		jonj_mb_gpc_set_input_encoding,
		jonj_mb_rfc1867_getword,
		jonj_mb_rfc1867_getword_conf,
		jonj_mb_rfc1867_basename);
...
#if HAVE_MBREGEX
	JONJ_MSHUTDOWN(mb_regex) (INIT_FUNC_ARGS_PASSTHRU);
#endif
...
JONJ_RINIT_FUNCTION(mbstring)
{
	...
	jonj_mb_populate_current_detect_order_list(TSRMLS_C);
		...
		while (p->type > 0) {
			if ((MBSTRG(func_overload) & p->type) == p->type &&
				...
				if (zend_hash_find(EG(function_table), p->orig_func, strlen(p->orig_func)+1, (void **)&orig) != SUCCESS) {
					jonj_error_docref("ref.mbstring" TSRMLS_CC, E_WARNING, "mbstring couldn't find function %s.", p->orig_func);
					return FAILURE;
				} else {
					...
					if (zend_hash_update(EG(function_table), p->orig_func, strlen(p->orig_func)+1, func, sizeof(zend_function),
						NULL) == FAILURE) {
						jonj_error_docref("ref.mbstring" TSRMLS_CC, E_WARNING, "mbstring couldn't replace function %s.", p->orig_func);
						return FAILURE;
					}
				}
			}
...
#if HAVE_MBREGEX
	JONJ_RINIT(mb_regex) (INIT_FUNC_ARGS_PASSTHRU);
#endif
...
#if HAVE_MBREGEX
	JONJ_RSHUTDOWN(mb_regex) (INIT_FUNC_ARGS_PASSTHRU);
#endif
...
JONJ_MINFO_FUNCTION(mbstring)
{
	...
	jonj_info_print_table_row(2, "Multibyte Support", "enabled");
	jonj_info_print_table_row(2, "Multibyte string engine", "libmbfl");
	jonj_info_print_table_row(2, "HTTP input encoding translation", MBSTRG(encoding_translation) ? "enabled": "disabled");
	{
		char tmp[256];
		snprintf(tmp, sizeof(tmp), "%d.%d.%d", MBFL_VERSION_MAJOR, MBFL_VERSION_MINOR, MBFL_VERSION_TEENY);
		jonj_info_print_table_row(2, "libmbfl version", tmp);
	}
	jonj_info_print_table_end();

	jonj_info_print_table_start();
	jonj_info_print_table_header(1, "mbstring extension makes use of \"streamable kanji code filter and converter\", which is distributed under the GNU Lesser General Public License version 2.1.");
	jonj_info_print_table_end();

#if HAVE_MBREGEX
	JONJ_MINFO(mb_regex)(ZEND_MODULE_INFO_FUNC_ARGS_PASSTHRU);
#endif
...
JONJ_FUNCTION(mb_language)
{
	...
	if (name == NULL) {
		...
		if (FAILURE == zend_alter_ini_entry(
				...
				name, name_len, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown language \"%s\"", name);
			RETVAL_FALSE;
		} else {
			RETVAL_TRUE;
		}
...
JONJ_FUNCTION(mb_internal_encoding)
{
	...
	if (name == NULL) {
		...
		if (!encoding) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", name);
			RETURN_FALSE;
		} else {
			MBSTRG(current_internal_encoding) = encoding;
			RETURN_TRUE;
...
JONJ_FUNCTION(mb_detect_order)
{
	...
	if (!arg1) {
		...
		case IS_ARRAY:
			if (FAILURE == jonj_mb_parse_encoding_array(*arg1, &list, &size, 0 TSRMLS_CC)) {
				if (list) {
					efree(list);
				}
				RETURN_FALSE;
		...
		default:
			...
			if (FAILURE == jonj_mb_parse_encoding_list(Z_STRVAL_PP(arg1), Z_STRLEN_PP(arg1), &list, &size, 0 TSRMLS_CC)) {
				if (list) {
					efree(list);
				}
				RETURN_FALSE;
		...
		case IS_STRING:
			if (strncasecmp("none", Z_STRVAL_PP(arg1), Z_STRLEN_PP(arg1)) == 0) {
				...
				if (Z_LVAL_PP(arg1) < 0xffff && Z_LVAL_PP(arg1) > 0x0) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown character.");
					RETURN_FALSE;
				}
			}
			break;
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &name, &name_len) == FAILURE) {
		...
		if (no_encoding == mbfl_no_encoding_invalid) {
			...
			if (preferred_name == NULL || *preferred_name == '\0') {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No MIME preferred name corresponding to \"%s\"", name);
				RETVAL_FALSE;
			} else {
...
JONJ_FUNCTION(mb_parse_str)
{
	...
	char *encstr = NULL;
	int encstr_len;
	jonj_mb_encoding_handler_info_t info;
	...
	if (track_vars_array != NULL) {
		detected = _jonj_mb_encoding_handler_ex(&info, track_vars_array, encstr TSRMLS_CC);
	} else {
		zval tmp;
		...
		detected = _jonj_mb_encoding_handler_ex(&info, &tmp, encstr TSRMLS_CC);
	}
...
JONJ_FUNCTION(mb_output_handler)
{
	...
	encoding = MBSTRG(current_http_output_encoding);
 	...
 	/* start phase only */
 	if ((arg_status & JONJ_OUTPUT_HANDLER_START) != 0) {
 	...
 	/* flag */
 	last_feed = ((arg_status & JONJ_OUTPUT_HANDLER_END) != 0);
 	/* mode */
...
JONJ_FUNCTION(mb_strlen)
{
	...
	if (enc_name == NULL) {
		...
		if (string.no_encoding == mbfl_no_encoding_invalid) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", enc_name);
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(mb_strpos)
{
	...
	if (offset < 0 || offset > mbfl_strlen(&haystack)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset not contained in string");
		RETURN_FALSE;
	}
	if (needle.len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty delimiter");
		RETURN_FALSE;
	}
	...
	if (n >= 0) {
		...
		case 1:
			break;
		case 2:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Needle has not positive length");
			break;
		case 4:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding or conversion error");
			break;
		case 8:
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Argument is empty");
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown error in mb_strpos");
			break;
		}
		RETVAL_FALSE;
	}
...
JONJ_FUNCTION(mb_strrpos)
{
...

	{
		...
		if ((offset > 0 && offset > haystack_char_len) ||
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset is greater than the length of haystack string");
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(mb_stripos)
{
	...
	n = jonj_mb_stripos(0, (char *)haystack.val, haystack.len, (char *)needle.val, needle.len, offset, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_strripos)
{
	...
	n = jonj_mb_stripos(1, (char *)haystack.val, haystack.len, (char *)needle.val, needle.len, offset, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_stristr)
{
	...
	if (haystack.no_encoding == mbfl_no_encoding_invalid) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", from_encoding);
		RETURN_FALSE;
	}

	n = jonj_mb_stripos(0, (char *)haystack.val, haystack.len, (char *)needle.val, needle.len, 0, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_strrichr)
{
	...
	n = jonj_mb_stripos(1, (char *)haystack.val, haystack.len, (char *)needle.val, needle.len, 0, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_substr_count)
{
	...
	if (needle.len <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty substring");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mb_substr)
{
	...
	if (argc == 4) {
		...
		if (string.no_encoding == mbfl_no_encoding_invalid) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", encoding);
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(mb_strimwidth)
{
	...
	if (from < 0 || from > str_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Start position is out of range");
		RETURN_FALSE;
	}

	if (width < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Width is negative value");
		RETURN_FALSE;
	}
...
MBSTRING_API char * jonj_mb_convert_encoding(const char *input, size_t length, const char *_to_encoding, const char *_from_encodings, size_t *output_len TSRMLS_DC)
{
	mbfl_string string, result, *ret;
	...
	if (_to_encoding && strlen(_to_encoding)) {
		...
		if (!to_encoding) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", _to_encoding);
			return NULL;
		}
	} else {
	...
	if (_from_encodings) {
		list = NULL;
		size = 0;
		jonj_mb_parse_encoding_list(_from_encodings, strlen(_from_encodings), &list, &size, 0 TSRMLS_CC);
		if (size == 1) {
			from_encoding = *list;
			string.no_encoding = from_encoding->no_encoding;
		} else if (size > 1) {
			...
			if (from_encoding) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to detect character encoding");
				from_encoding = &mbfl_encoding_pass;
				to_encoding = from_encoding;
				string.no_encoding = from_encoding->no_encoding;
			}
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal character encoding specified");
		}
		if (list != NULL) {
			efree((void *)list);
		}
	...
	if (convd == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to create character encoding converter");
		return NULL;
	}
	mbfl_buffer_converter_illegal_mode(convd, MBSTRG(current_filter_illegal_mode));
	mbfl_buffer_converter_illegal_substchar(convd, MBSTRG(current_filter_illegal_substchar));
...
JONJ_FUNCTION(mb_convert_encoding)
{
	...
	/* new encoding */
	ret = jonj_mb_convert_encoding(arg_str, str_len, arg_new, _from_encodings, &size TSRMLS_CC);
	if (ret != NULL) {
...
JONJ_FUNCTION(mb_convert_case)
{
	...
	newstr = jonj_unicode_convert_case(case_mode, str, (size_t) str_len, &ret_len, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_strtoupper)
{
	...
	newstr = jonj_unicode_convert_case(JONJ_UNICODE_CASE_UPPER, str, (size_t) str_len, &ret_len, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_strtolower)
{
	...
	newstr = jonj_unicode_convert_case(JONJ_UNICODE_CASE_LOWER, str, (size_t) str_len, &ret_len, from_encoding TSRMLS_CC);
...
JONJ_FUNCTION(mb_detect_encoding)
{
	...
	if (ZEND_NUM_ARGS() >= 2 && !ZVAL_IS_NULL(encoding_list)) {
		...
		case IS_ARRAY:
			if (FAILURE == jonj_mb_parse_encoding_array(encoding_list, &list, &size, 0 TSRMLS_CC)) {
				if (list) {
					efree(list);
					list = NULL;
					size = 0;
		...
		default:
			...
			if (FAILURE == jonj_mb_parse_encoding_list(Z_STRVAL_P(encoding_list), Z_STRLEN_P(encoding_list), &list, &size, 0 TSRMLS_CC)) {
				if (list) {
					efree(list);
					list = NULL;
					size = 0;
		...
		if (size <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal argument");
		}
	}
...
JONJ_FUNCTION(mb_encode_mimeheader)
{
	...
	if (charset_name != NULL) {
		...
		if (charset == mbfl_no_encoding_invalid) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", charset_name);
			RETURN_FALSE;
		}
	} else {
...
JONJ_FUNCTION(mb_convert_kana)
{
	...
	if (encname != NULL) {
		...
		if (string.no_encoding == mbfl_no_encoding_invalid) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", encname);
			RETURN_FALSE;
		}
	}
...
#define JONJ_MBSTR_STACK_BLOCK_SIZE 32
...
JONJ_FUNCTION(mb_convert_variables)
{
	...
	if (!to_encoding) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown encoding \"%s\"", to_enc);
		efree(args);
		RETURN_FALSE;
	}
	...
	case IS_ARRAY:
		jonj_mb_parse_encoding_array(*zfrom_enc, &elist, &elistsz, 0 TSRMLS_CC);
		break;
	default:
		convert_to_string_ex(zfrom_enc);
		jonj_mb_parse_encoding_list(Z_STRVAL_PP(zfrom_enc), Z_STRLEN_PP(zfrom_enc), &elist, &elistsz, 0 TSRMLS_CC);
		break;
	}
	if (elistsz <= 0) {
		from_encoding = &mbfl_encoding_pass;
		...
		/* auto detect */
		from_encoding = NULL;
		stack_max = JONJ_MBSTR_STACK_BLOCK_SIZE;
...
detect_end:
		...
		if (recursion_error) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot handle recursive references");
			RETURN_FALSE;
		}
		efree(stack);

		if (!from_encoding) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to detect encoding");
			from_encoding = &mbfl_encoding_pass;
		}
	}
	if (elist != NULL) {
	...
	if (from_encoding != &mbfl_encoding_pass) {
		...
		if (convd == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to create converter");
			RETURN_FALSE;
		}
		mbfl_buffer_converter_illegal_mode(convd, MBSTRG(current_filter_illegal_mode));
		mbfl_buffer_converter_illegal_substchar(convd, MBSTRG(current_filter_illegal_substchar));
...
/* {{{ HTML numeric entity */
/* {{{ static void jonj_mb_numericentity_exec() */
static void
jonj_mb_numericentity_exec(INTERNAL_FUNCTION_PARAMETERS, int type)
{
	char *str, *encoding;
...
{
	jonj_mb_numericentity_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_mb_numericentity_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
/* }}} */
...
static int _jonj_mbstr_parse_mail_headers(HashTable *ht, const char *str, size_t str_len)
{
	const char *ps;
	size_t icnt;
	int state = 0;
					...
					case 3:
						if (crlf_state == -1) {
							...
							if (fld_name.c != NULL && fld_val.c != NULL) {
								...
								jonj_strtoupper(fld_name.c, fld_name.len);
...
JONJ_FUNCTION(mb_send_mail)
{
	...
	if (headers != NULL) {
		_jonj_mbstr_parse_mail_headers(&ht_headers, headers, headers_len);
	}
	...
	if (zend_hash_find(&ht_headers, "CONTENT-TYPE", sizeof("CONTENT-TYPE") - 1, (void **)&s) == SUCCESS) {
		...
		if (p != NULL) {
			...
			if (*p != '\0') {
				if ((param_name = jonj_strtok_r(p, "= ", &tmp)) != NULL) {
					if (strcasecmp(param_name, "charset") == 0) {
						enum mbfl_no_encoding _tran_cs = tran_cs;

						charset = jonj_strtok_r(NULL, "= \"", &tmp);
						if (charset != NULL) {
							_tran_cs = mbfl_name2no_encoding(charset);
						}

						if (_tran_cs == mbfl_no_encoding_invalid) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unsupported charset \"%s\" - will be regarded as ascii", charset);
	...
	if (zend_hash_find(&ht_headers, "CONTENT-TRANSFER-ENCODING", sizeof("CONTENT-TRANSFER-ENCODING") - 1, (void **)&s) == SUCCESS) {
		...
		switch (_body_enc) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unsupported transfer encoding \"%s\" - will be regarded as 8bit", s->c);
	...
	if (to != NULL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing To: field");
		err = 1;
	}
	...
	if (message != NULL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty message body");
		message = NULL;
	}

	/* other headers */
#define JONJ_MBSTR_MAIL_MIME_HEADER1 "MIME-Version: 1.0"
#define JONJ_MBSTR_MAIL_MIME_HEADER2 "Content-Type: text/plain"
#define JONJ_MBSTR_MAIL_MIME_HEADER3 "; charset="
#define JONJ_MBSTR_MAIL_MIME_HEADER4 "Content-Transfer-Encoding: "
	if (headers != NULL) {
		p = headers;
		n = headers_len;
	...
	if (!zend_hash_exists(&ht_headers, "MIME-VERSION", sizeof("MIME-VERSION") - 1)) {
		mbfl_memory_device_strncat(&device, JONJ_MBSTR_MAIL_MIME_HEADER1, sizeof(JONJ_MBSTR_MAIL_MIME_HEADER1) - 1);
		mbfl_memory_device_strncat(&device, "\n", 1);
	}

	if (!suppressed_hdrs.cnt_type) {
		mbfl_memory_device_strncat(&device, JONJ_MBSTR_MAIL_MIME_HEADER2, sizeof(JONJ_MBSTR_MAIL_MIME_HEADER2) - 1);

		p = (char *)mbfl_no2preferred_mime_name(tran_cs);
		if (p != NULL) {
			mbfl_memory_device_strncat(&device, JONJ_MBSTR_MAIL_MIME_HEADER3, sizeof(JONJ_MBSTR_MAIL_MIME_HEADER3) - 1);
			mbfl_memory_device_strcat(&device, p);
		}
		mbfl_memory_device_strncat(&device, "\n", 1);
	}
	if (!suppressed_hdrs.cnt_trans_enc) {
		mbfl_memory_device_strncat(&device, JONJ_MBSTR_MAIL_MIME_HEADER4, sizeof(JONJ_MBSTR_MAIL_MIME_HEADER4) - 1);
	...
	if (!err && jonj_mail(to_r, subject, message, headers, extra_cmd TSRMLS_CC)) {
		RETVAL_TRUE;
	} else {
		RETVAL_FALSE;
	}
...
#undef JONJ_MBSTR_MAIL_MIME_HEADER1
#undef JONJ_MBSTR_MAIL_MIME_HEADER2
#undef JONJ_MBSTR_MAIL_MIME_HEADER3
#undef JONJ_MBSTR_MAIL_MIME_HEADER4
/* }}} */
...
JONJ_FUNCTION(mb_check_encoding)
{
	...
	if (enc != NULL) {
		...
		if (!encoding || encoding == &mbfl_encoding_pass) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid encoding \"%s\"", enc);
			RETURN_FALSE;
		}
	}
...
/* {{{ jonj_mb_populate_current_detect_order_list */
static void jonj_mb_populate_current_detect_order_list(TSRMLS_D)
{
...
/* {{{ static int jonj_mb_encoding_translation() */
static int jonj_mb_encoding_translation(TSRMLS_D)
{
...
/* {{{ MBSTRING_API size_t jonj_mb_mbchar_bytes_ex() */
...
/* {{{ MBSTRING_API size_t jonj_mb_mbchar_bytes() */
MBSTRING_API size_t jonj_mb_mbchar_bytes(const char *s TSRMLS_DC)
{
	return jonj_mb_mbchar_bytes_ex(s, MBSTRG(internal_encoding));
}
/* }}} */

/* {{{ MBSTRING_API char *jonj_mb_safe_strrchr_ex() */
MBSTRING_API char *jonj_mb_safe_strrchr_ex(const char *s, unsigned int c, size_t nbytes, const mbfl_encoding *enc)
{
	...
	if (nbytes == (size_t)-1) {
		...
		while (*p != '\0') {
			if (nb == 0) {
				...
				nb = jonj_mb_mbchar_bytes_ex(p, enc);
				if (nb == 0) {
		...
		while (bcnt > 0) {
			...
			nbytes_char = jonj_mb_mbchar_bytes_ex(p, enc);
			if (bcnt < nbytes_char) {
				return NULL;
			}
			p += nbytes_char;
...
/* {{{ MBSTRING_API char *jonj_mb_safe_strrchr() */
MBSTRING_API char *jonj_mb_safe_strrchr(const char *s, unsigned int c, size_t nbytes TSRMLS_DC)
{
	return jonj_mb_safe_strrchr_ex(s, c, nbytes, MBSTRG(internal_encoding));
}
/* }}} */

/* {{{ MBSTRING_API int jonj_mb_stripos()
 */
MBSTRING_API int jonj_mb_stripos(int mode, const char *old_haystack, unsigned int old_haystack_len, const char *old_needle, unsigned int old_needle_len, long offset, const char *from_encoding TSRMLS_DC)
{
	...
	do {
		size_t len = 0;
		haystack.val = (unsigned char *)jonj_unicode_convert_case(JONJ_UNICODE_CASE_UPPER, (char *)old_haystack, old_haystack_len, &len, from_encoding TSRMLS_CC);
		haystack.len = len;
		...
		needle.val = (unsigned char *)jonj_unicode_convert_case(JONJ_UNICODE_CASE_UPPER, (char *)old_needle, old_needle_len, &len, from_encoding TSRMLS_CC);
		needle.len = len;
