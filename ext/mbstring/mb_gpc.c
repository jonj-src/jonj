/*
   ...
   +----------------------------------------------------------------------+
   | Author: Rui Hirokawa <hirokawa@jonj.tk>                              |
...
MBSTRING_API SAPI_TREAT_DATA_FUNC(mbstr_treat_data)
{
	...
	if (arg != PARSE_STRING) {
		...
		_jonj_mb_ini_mbstring_internal_encoding_set(value, value ? strlen(value): 0 TSRMLS_CC);
	}

	if (!MBSTRG(encoding_translation)) {
		jonj_default_treat_data(arg, str, destArray TSRMLS_CC);
		return;
	}
	...
	detected = _jonj_mb_encoding_handler_ex(&info, array_ptr, res TSRMLS_CC);
...
/* {{{ mbfl_no_encoding _jonj_mb_encoding_handler_ex() */
const mbfl_encoding *_jonj_mb_encoding_handler_ex(const jonj_mb_encoding_handler_info_t *info, zval *arg, char *res TSRMLS_DC)
{
	char *var, *val;
	const char *s1, *s2;
	...
	n = 0;
	strtok_buf = NULL;
	var = jonj_strtok_r(res, info->separator, &strtok_buf);
	while (var)  {
		val = strchr(var, '=');
		if (val) { /* have a value */
			len_list[n] = jonj_url_decode(var, val-var);
			val_list[n] = var;
			n++;
			
			*val++ = '\0';
			val_list[n] = val;
			len_list[n] = jonj_url_decode(val, strlen(val));
		} else {
			len_list[n] = jonj_url_decode(var, strlen(var));
			val_list[n] = var;
			n++;
		...
		n++;
		var = jonj_strtok_r(NULL, info->separator, &strtok_buf);
	} 
...
/* {{{ SAPI_POST_HANDLER_FUNC(jonj_mb_post_handler) */
SAPI_POST_HANDLER_FUNC(jonj_mb_post_handler)
{
	const mbfl_encoding *detected;
	jonj_mb_encoding_handler_info_t info;
	...
	jonj_stream_rewind(SG(request_info).request_body);
	jonj_stream_copy_to_mem(SG(request_info).request_body, &post_data_str, JONJ_STREAM_COPY_ALL, 0);
	detected = _jonj_mb_encoding_handler_ex(&info, arg, post_data_str TSRMLS_CC);
