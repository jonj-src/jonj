...
#include "jonj_onig_compat.h" /* must come prior to the oniguruma header */
#include <oniguruma.h>
#undef UChar
...
/* {{{ static void jonj_mb_regex_free_cache() */
static void jonj_mb_regex_free_cache(jonj_mb_regex_t **pre)
{
	onig_free(*pre);
}
/* }}} */

/* {{{ _jonj_mb_regex_globals_ctor */
static int _jonj_mb_regex_globals_ctor(zend_mb_regex_globals *pglobals TSRMLS_DC)
{
	pglobals->default_mbctype = ONIG_ENCODING_UTF8;
	pglobals->current_mbctype = ONIG_ENCODING_UTF8;
	zend_hash_init(&(pglobals->ht_rc), 0, NULL, (void (*)(void *)) jonj_mb_regex_free_cache, 1);
	pglobals->search_str = (zval*) NULL;
	pglobals->search_re = (jonj_mb_regex_t*)NULL;
...
/* {{{ _jonj_mb_regex_globals_dtor */
static void _jonj_mb_regex_globals_dtor(zend_mb_regex_globals *pglobals TSRMLS_DC)
{
	zend_hash_destroy(&pglobals->ht_rc);
}
/* }}} */

/* {{{ jonj_mb_regex_globals_alloc */
zend_mb_regex_globals *jonj_mb_regex_globals_alloc(TSRMLS_D)
{
	...
	if (SUCCESS != _jonj_mb_regex_globals_ctor(pglobals TSRMLS_CC)) {
		pefree(pglobals, 1);
		return NULL;
	}
	return pglobals;
...
/* {{{ jonj_mb_regex_globals_free */
void jonj_mb_regex_globals_free(zend_mb_regex_globals *pglobals TSRMLS_DC)
{
	if (!pglobals) {
		return;
	}
	_jonj_mb_regex_globals_dtor(pglobals TSRMLS_CC);
	pefree(pglobals, 1);
}
/* }}} */
...
JONJ_MINFO_FUNCTION(mb_regex)
{
	...
	jonj_info_print_table_row(2, "Multibyte (japanese) regex support", "enabled");
	snprintf(buf, sizeof(buf), "%d.%d.%d",
			ONIGURUMA_VERSION_MAJOR,
			ONIGURUMA_VERSION_MINOR,
			ONIGURUMA_VERSION_TEENY);
#ifdef JONJ_ONIG_BUNDLED
#ifdef USE_COMBINATION_EXPLOSION_CHECK
	jonj_info_print_table_row(2, "Multibyte regex (oniguruma) backtrack check", "On");
#else	/* USE_COMBINATION_EXPLOSION_CHECK */
	jonj_info_print_table_row(2, "Multibyte regex (oniguruma) backtrack check", "Off");
#endif	/* USE_COMBINATION_EXPLOSION_CHECK */
#endif /* JONJ_BUNDLED_ONIG */
	jonj_info_print_table_row(2, "Multibyte regex (oniguruma) version", buf);
...
/* {{{ encoding name map */
typedef struct _jonj_mb_regex_enc_name_map_t {
	const char *names;
	OnigEncoding code;
} jonj_mb_regex_enc_name_map_t;

jonj_mb_regex_enc_name_map_t enc_name_map[] = {
...
#endif
#if defined(ONIG_ENCODING_KOI8) && !JONJ_ONIG_BAD_KOI8_ENTRY
	{
		"KOI8\0KOI-8\0",
		ONIG_ENCODING_KOI8
	},
...
/* {{{ jonj_mb_regex_name2mbctype */
static OnigEncoding _jonj_mb_regex_name2mbctype(const char *pname)
{
	const char *p;
	jonj_mb_regex_enc_name_map_t *mapping;
...
/* {{{ jonj_mb_regex_mbctype2name */
static const char *_jonj_mb_regex_mbctype2name(OnigEncoding mbctype)
{
	jonj_mb_regex_enc_name_map_t *mapping;
...
/* {{{ jonj_mb_regex_set_mbctype */
int jonj_mb_regex_set_mbctype(const char *encname TSRMLS_DC)
{
	OnigEncoding mbctype = _jonj_mb_regex_name2mbctype(encname);
...
/* {{{ jonj_mb_regex_set_default_mbctype */
...
/* {{{ jonj_mb_regex_get_mbctype */
const char *jonj_mb_regex_get_mbctype(TSRMLS_D)
{
	return _jonj_mb_regex_mbctype2name(MBREX(current_mbctype));
}
/* }}} */

/* {{{ jonj_mb_regex_get_default_mbctype */
const char *jonj_mb_regex_get_default_mbctype(TSRMLS_D)
{
	return _jonj_mb_regex_mbctype2name(MBREX(default_mbctype));
}
/* }}} */
 ...
 * regex cache
 */
/* {{{ jonj_mbregex_compile_pattern */
static jonj_mb_regex_t *jonj_mbregex_compile_pattern(const char *pattern, int patlen, OnigOptionType options, OnigEncoding enc, OnigSyntaxType *syntax TSRMLS_DC)
{
	int err_code = 0;
	int found = 0;
	jonj_mb_regex_t *retval = NULL, **rc = NULL;
	...
	if (found == FAILURE || (*rc)->options != options || (*rc)->enc != enc || (*rc)->syntax != syntax) {
		if ((err_code = onig_new(&retval, (OnigUChar *)pattern, (OnigUChar *)(pattern + patlen), options, enc, syntax, &err_info)) != ONIG_NORMAL) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mbregex compile err: %s", err_str);
			retval = NULL;
			goto out;
		}
...
/* {{{ _jonj_mb_regex_get_option_string */
static size_t _jonj_mb_regex_get_option_string(char *str, size_t len, OnigOptionType option, OnigSyntaxType *syntax)
{
	size_t len_left = len;
	size_t len_req = 0;
	char *p = str;
...
/* {{{ _jonj_mb_regex_init_options */
static void
_jonj_mb_regex_init_options(const char *parg, int narg, OnigOptionType *option, OnigSyntaxType **syntax, int *eval)
{
	int n;
	char c;
	int optm = 0;
...
JONJ_FUNCTION(mb_regex_encoding)
{
	...
	if (argc == 0) {
		const char *retval = _jonj_mb_regex_mbctype2name(MBREX(current_mbctype));
		...
		mbctype = _jonj_mb_regex_name2mbctype(encoding);
...
/* {{{ _jonj_mb_regex_ereg_exec */
static void _jonj_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAMETERS, int icase)
{
	...
	if (!Z_STRVAL_PP(arg_pattern) || Z_STRLEN_PP(arg_pattern) == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "empty pattern");
		RETVAL_FALSE;
		goto out;
	}

	re = jonj_mbregex_compile_pattern(Z_STRVAL_PP(arg_pattern), Z_STRLEN_PP(arg_pattern), options, MBREX(current_mbctype), MBREX(regex_default_syntax) TSRMLS_CC);
	if (re == NULL) {
		RETVAL_FALSE;
		goto out;
	}
...
{
	_jonj_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	_jonj_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ _jonj_mb_regex_ereg_replace_exec */
static void _jonj_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAMETERS, OnigOptionType options, int is_callable)
{
...

	{
		...
		current_enc_name = _jonj_mb_regex_mbctype2name(MBREX(current_mbctype));
	...
	eval = 0;
	{
		...
		if (option_str != NULL) {
			_jonj_mb_regex_init_options(option_str, option_str_len, &options, &syntax, &eval);
		} else {
			options |= MBREX(regex_default_options);
	...
	re = jonj_mbregex_compile_pattern(arg_pattern, arg_pattern_len, options, MBREX(current_mbctype), syntax TSRMLS_CC);
	if (re == NULL) {
		RETURN_FALSE;
	}
	...
	if (is_callable) {
		if (eval) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Option 'e' cannot be used with replacement callback");
			RETURN_FALSE;
		}
	}
	...
	while (err >= 0) {
		...
		if (err <= -2) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mbregex search failure in jonj_mbereg_replace_exec(): %s", err_str);
			break;
		}
		if (err >= 0) {
#if moriyoshi_0
			if (regs->beg[0] == regs->end[0]) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty regular expression");
				break;
			}
#endif
			...
			if (!is_callable) {
				...
				p = replace;
				while (i < replace_len) {
					int fwd = (int) jonj_mb_mbchar_bytes_ex(p, enc);
					n = -1;
			...
			if (eval) {
				...
				if (zend_eval_stringl(eval_buf.c, eval_buf.len, &v, description TSRMLS_CC) == FAILURE) {
					efree(description);
					jonj_error_docref(NULL TSRMLS_CC,E_ERROR, "Failed evaluating code: %s%s", JONJ_EOL, eval_buf.c);
				...
				if (zend_call_function(&arg_replace_fci, &arg_replace_fci_cache TSRMLS_CC) == SUCCESS && arg_replace_fci.retval_ptr_ptr && retval_ptr) {
					...
					if (!EG(exception)) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call custom replacement function");
					}
				}
...
{
	_jonj_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */
...
{
	_jonj_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, ONIG_OPTION_IGNORECASE, 0);
}
/* }}} */
...
{
	_jonj_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 1);
}
/* }}} */
...
JONJ_FUNCTION(mb_split)
{
	...
	/* create regex pattern buffer */
	if ((re = jonj_mbregex_compile_pattern(arg_pattern, arg_pattern_len, MBREX(regex_default_options), MBREX(current_mbctype), MBREX(regex_default_syntax) TSRMLS_CC)) == NULL) {
		RETURN_FALSE;
	}
	...
	if (err <= -2) {
		OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
		onig_error_code_to_str(err_str, err);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mbregex search failure in mbsplit(): %s", err_str);
...
JONJ_FUNCTION(mb_ereg_match)
{
...

	{
		...
		if (option_str != NULL) {
			_jonj_mb_regex_init_options(option_str, option_str_len, &option, &syntax, NULL);
		} else {
	...
	if ((re = jonj_mbregex_compile_pattern(arg_pattern, arg_pattern_len, option, MBREX(current_mbctype), syntax TSRMLS_CC)) == NULL) {
		RETURN_FALSE;
	}
...
/* regex search */
/* {{{ _jonj_mb_regex_ereg_search_exec */
static void
_jonj_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	size_t argc = ZEND_NUM_ARGS();
	...
	if (argc == 2) {
		option = 0;
		_jonj_mb_regex_init_options(arg_options, arg_options_len, &option, &syntax, NULL);
	}

	if (argc > 0) {
		/* create regex pattern buffer */
		if ((MBREX(search_re) = jonj_mbregex_compile_pattern(arg_pattern, arg_pattern_len, option, MBREX(current_mbctype), MBREX(regex_default_syntax) TSRMLS_CC)) == NULL) {
			RETURN_FALSE;
		}
	}
	...
	if (MBREX(search_re) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No regex given");
		RETURN_FALSE;
	}

	if (str == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No string given");
		RETURN_FALSE;
	}
	...
	if (err == ONIG_MISMATCH) {
		...
		OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
		onig_error_code_to_str(err_str, err);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mbregex search failure in mbregex_search(): %s", err_str);
		RETVAL_FALSE;
	} else {
		switch (mode) {
		case 1:
...
{
	_jonj_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	_jonj_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	_jonj_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */
...
JONJ_FUNCTION(mb_ereg_search_init)
{
	...
	if (argc > 1) {
		/* create regex pattern buffer */
		if ((MBREX(search_re) = jonj_mbregex_compile_pattern(arg_pattern, arg_pattern_len, option, MBREX(current_mbctype), syntax TSRMLS_CC)) == NULL) {
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(mb_ereg_search_setpos)
{
	...
	if (position < 0 || (MBREX(search_str) != NULL && Z_TYPE_P(MBREX(search_str)) == IS_STRING && position > Z_STRLEN_P(MBREX(search_str)))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Position is out of range");
		MBREX(search_pos) = 0;
		RETURN_FALSE;
	}
...
/* {{{ jonj_mb_regex_set_options */
static void _jonj_mb_regex_set_options(OnigOptionType options, OnigSyntaxType *syntax, OnigOptionType *prev_options, OnigSyntaxType **prev_syntax TSRMLS_DC)
{
...
JONJ_FUNCTION(mb_regex_set_options)
{
	...
	if (string != NULL) {
		opt = 0;
		syntax = NULL;
		_jonj_mb_regex_init_options(string, string_len, &opt, &syntax, NULL);
		_jonj_mb_regex_set_options(opt, syntax, NULL, NULL TSRMLS_CC);
	} else {
		opt = MBREX(regex_default_options);
		syntax = MBREX(regex_default_syntax);
	}
	_jonj_mb_regex_get_option_string(buf, sizeof(buf), opt, syntax);
