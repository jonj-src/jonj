...
typedef struct _jonj_zlib_filter_data {
	int persistent;
	z_stream strm;
	char *inbuf;
	size_t inbuf_len;
...
{
	return (voidpf)safe_pemalloc(items, size, 0, ((jonj_zlib_filter_data*)opaque)->persistent);
}

static void jonj_zlib_free(voidpf opaque, voidpf address)
{
	pefree((void*)address, ((jonj_zlib_filter_data*)opaque)->persistent);
}
/* }}} */

/* {{{ zlib.inflate filter implementation */

static jonj_stream_filter_status_t jonj_zlib_inflate_filter(
	jonj_stream *stream,
	...
	TSRMLS_DC)
{
	...
	data = (jonj_zlib_filter_data *)(thisfilter->abstract);
...
static void jonj_zlib_inflate_dtor(jonj_stream_filter *thisfilter TSRMLS_DC)
{
	if (thisfilter && thisfilter->abstract) {
		jonj_zlib_filter_data *data = thisfilter->abstract;
		if (!data->finished) {
			inflateEnd(&(data->strm));
		}
...
static jonj_stream_filter_ops jonj_zlib_inflate_ops = {
	jonj_zlib_inflate_filter,
	jonj_zlib_inflate_dtor,
	"zlib.inflate"
};
/* }}} */
...
static jonj_stream_filter_status_t jonj_zlib_deflate_filter(
	jonj_stream *stream,
	jonj_stream_filter *thisfilter,
	jonj_stream_bucket_brigade *buckets_in,
	jonj_stream_bucket_brigade *buckets_out,
...
static void jonj_zlib_deflate_dtor(jonj_stream_filter *thisfilter TSRMLS_DC)
{
	if (thisfilter && thisfilter->abstract) {
		jonj_zlib_filter_data *data = thisfilter->abstract;
...
static jonj_stream_filter_ops jonj_zlib_deflate_ops = {
	jonj_zlib_deflate_filter,
	jonj_zlib_deflate_dtor,
	"zlib.deflate"
};
...
static jonj_stream_filter *jonj_zlib_filter_create(const char *filtername, zval *filterparams, int persistent TSRMLS_DC)
{
	...
	/* Create this filter */
	data = pecalloc(1, sizeof(jonj_zlib_filter_data), persistent);
	if (!data) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed allocating %zd bytes", sizeof(jonj_zlib_filter_data));
		return NULL;
	}
	...
	data->strm.zalloc = (alloc_func) jonj_zlib_alloc;
	data->strm.zfree = (free_func) jonj_zlib_free;
	data->strm.avail_out = data->outbuf_len = data->inbuf_len = 0x8000;
	data->strm.next_in = data->inbuf = (Bytef *) pemalloc(data->inbuf_len, persistent);
	if (!data->inbuf) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed allocating %zd bytes", data->inbuf_len);
	...
	if (!data->outbuf) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed allocating %zd bytes", data->outbuf_len);
		...
		if (filterparams) {
			...
			if ((Z_TYPE_P(filterparams) == IS_ARRAY || Z_TYPE_P(filterparams) == IS_OBJECT) &&
				...
				if (Z_LVAL(tmp) < -MAX_WBITS || Z_LVAL(tmp) > MAX_WBITS + 32) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parameter give for window size. (%ld)", Z_LVAL(tmp));
				} else {
				...
				case IS_OBJECT:
					if (zend_hash_find(HASH_OF(filterparams), "memory", sizeof("memory"), (void**) &tmpzval) == SUCCESS) {
						...
						if (Z_LVAL(tmp) < 1 || Z_LVAL(tmp) > MAX_MEM_LEVEL) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parameter give for memory level. (%ld)", Z_LVAL(tmp));
						} else {
							memLevel = Z_LVAL(tmp);
						}
					}
...
factory_setlevel:
					...
					if (Z_LVAL(tmp) < -1 || Z_LVAL(tmp) > 9) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid compression level specified. (%ld)", Z_LVAL(tmp));
					} else {
						level = Z_LVAL(tmp);
					}
					break;
				default:
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid filter parameter, ignored");
			}
		}
		status = deflateInit2(&(data->strm), level, Z_DEFLATED, windowBits, memLevel, 0);
		fops = &jonj_zlib_deflate_ops;
	} else {
		status = Z_DATA_ERROR;
	}
...
jonj_stream_filter_factory jonj_zlib_filter_factory = {
	jonj_zlib_filter_create
};
/* }}} */
