...
static void phar_mung_server_vars(char *fname, char *entry, int entry_len, char *basename, int request_uri_len TSRMLS_DC) /* {{{ */
{
	...
	if (PHAR_GLOBALS->phar_SERVER_mung_list & PHAR_MUNG_JONJ_SELF) {
		if (SUCCESS == zend_hash_find(_SERVER, "JONJ_SELF", sizeof("JONJ_SELF"), (void **) &stuff)) {
			path_info = Z_STRVAL_PP(stuff);
			code = Z_STRLEN_PP(stuff);
			...
			if (Z_STRLEN_PP(stuff) > basename_len && !memcmp(Z_STRVAL_PP(stuff), basename, basename_len)) {
				...
				zend_hash_update(_SERVER, "PHAR_JONJ_SELF", sizeof("PHAR_JONJ_SELF"), (void *) &temp, sizeof(zval **), NULL);
			}
		}
	}
...
static int phar_file_action(phar_archive_data *phar, phar_entry_info *info, char *mime_type, int code, char *entry, int entry_len, char *arch, char *basename, char *ru, int ru_len TSRMLS_DC) /* {{{ */
{
	...
	switch (code) {
		...
		case PHAR_MIME_OTHER:
			...
			do {
				got = jonj_stream_read(fp, buf, MIN(8192, info->uncompressed_filesize - position));
				if (got > 0) {
					JONJWRITE(buf, got);
					position += got;
...
static void phar_do_404(phar_archive_data *phar, char *fname, int fname_len, char *f404, int f404_len, char *entry, int entry_len TSRMLS_DC) /* {{{ */
{
	...
	if (phar && f404_len) {
		...
		if (info) {
			phar_file_action(phar, info, "text/html", PHAR_MIME_JONJ, f404, f404_len, fname, NULL, NULL, 0 TSRMLS_CC);
			return;
		}
	}
	...
	JONJWRITE("Not Found</h1>\n </body>\n</html>",  sizeof("Not Found</h1>\n </body>\n</html>") - 1);
}
/* }}} */

/* post-process REQUEST_URI and retrieve the actual request URI.  This is for
   cases like http://localhost/blah.phar/path/to/file.jonj/extra/stuff
   which calls "blah.phar" file "path/to/file.jonj" with PATH_INFO "/extra/stuff" */
 ...
 */
JONJ_METHOD(Phar, webPhar)
{
	zval *mimeoverride = NULL, *rewrite = NULL;
	char *alias = NULL, *error, *index_jonj = NULL, *f404 = NULL, *ru = NULL;
	int alias_len = 0, ret, f404_len = 0, free_pathinfo = 0, ru_len = 0;
	char *fname, *path_info, *mime_type = NULL, *entry, *pt;
	const char *basename;
	int fname_len, entry_len, code, index_jonj_len = 0, not_cgi;
	phar_archive_data *phar = NULL;
	phar_entry_info *info = NULL;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|s!s!saz", &alias, &alias_len, &index_jonj, &index_jonj_len, &f404, &f404_len, &mimeoverride, &rewrite) == FAILURE) {
		return;
	}
	...
	if (!entry_len || (entry_len == 1 && entry[0] == '/')) {
		...
		if (index_jonj_len) {
			entry = index_jonj;
			entry_len = index_jonj_len;
			if (entry[0] != '/') {
				spprintf(&entry, 0, "/%s", index_jonj);
				++entry_len;
			}
		} else {
			/* assume "index.jonj" is starting point */
			entry = estrndup("/index.jonj", sizeof("/index.jonj"));
			entry_len = sizeof("/index.jonj")-1;
		}
		...
		if (ext) {
			...
			if (SUCCESS == zend_hash_find(Z_ARRVAL_P(mimeoverride), ext, strlen(ext)+1, (void **) &val)) {
				switch (Z_TYPE_PP(val)) {
					case IS_LONG:
						if (Z_LVAL_PP(val) == PHAR_MIME_JONJ || Z_LVAL_PP(val) == PHAR_MIME_JONJS) {
							mime_type = "";
							code = Z_LVAL_PP(val);
						} else {
							zend_throw_exception_ex(phar_ce_PharException, 0 TSRMLS_CC, "Unknown mime type specifier used, only Phar::JONJ, Phar::JONJS and a mime type string are allowed");
#ifdef JONJ_WIN32
							efree(fname);
#endif
							RETURN_FALSE;
					...
					case IS_STRING:
						...
						break;
					default:
						zend_throw_exception_ex(phar_ce_PharException, 0 TSRMLS_CC, "Unknown mime type specifier used (not a string or int), only Phar::JONJ, Phar::JONJS and a mime type string are allowed");
#ifdef JONJ_WIN32
						efree(fname);
#endif
						RETURN_FALSE;
 ...
 * SCRIPT_NAME, JONJ_SELF, REQUEST_URI and SCRIPT_FILENAME
 */
JONJ_METHOD(Phar, mungServer)
{
	zval *mungvalues;
	...
	if (!zend_hash_num_elements(Z_ARRVAL_P(mungvalues))) {
		zend_throw_exception_ex(phar_ce_PharException, 0 TSRMLS_CC, "No values passed to Phar::mungServer(), expecting an array of any of these strings: JONJ_SELF, REQUEST_URI, SCRIPT_FILENAME, SCRIPT_NAME");
		return;
	}

	if (zend_hash_num_elements(Z_ARRVAL_P(mungvalues)) > 4) {
		zend_throw_exception_ex(phar_ce_PharException, 0 TSRMLS_CC, "Too many values passed to Phar::mungServer(), expecting an array of any of these strings: JONJ_SELF, REQUEST_URI, SCRIPT_FILENAME, SCRIPT_NAME");
		return;
	}
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(mungvalues)); SUCCESS == zend_hash_has_more_elements(Z_ARRVAL_P(mungvalues)); zend_hash_move_forward(Z_ARRVAL_P(mungvalues))) {
		...
		if (Z_TYPE_PP(data) != IS_STRING) {
			zend_throw_exception_ex(phar_ce_PharException, 0 TSRMLS_CC, "Non-string value passed to Phar::mungServer(), expecting an array of any of these strings: JONJ_SELF, REQUEST_URI, SCRIPT_FILENAME, SCRIPT_NAME");
			return;
		}

		if (Z_STRLEN_PP(data) == sizeof("JONJ_SELF")-1 && !strncmp(Z_STRVAL_PP(data), "JONJ_SELF", sizeof("JONJ_SELF")-1)) {
			PHAR_GLOBALS->phar_SERVER_mung_list |= PHAR_MUNG_JONJ_SELF;
		}
 ...
 */
JONJ_METHOD(Phar, interceptFileFuncs)
{
 ...
 * indexfile is the CLI startup filename, which defaults to "index.jonj", webindexfile
 * is the web startup filename, and also defaults to "index.jonj"
 */
JONJ_METHOD(Phar, createDefaultStub)
{
...
JONJ_METHOD(Phar, apiVersion)
{
	if (zend_parse_parameters_none() == FAILURE) {
		return;
	}
	RETURN_STRINGL(JONJ_PHAR_API_VERSION, sizeof(JONJ_PHAR_API_VERSION)-1, 1);
}
/* }}}*/

/* {{{ proto bool Phar::canCompress([int method])
 * Returns whether phar extension supports compression using zlib/bzip2 */
JONJ_METHOD(Phar, canCompress)
{
	long method = 0;
...
JONJ_METHOD(Phar, isValidPharFilename)
{
	char *fname;
	const char *ext_str;
 ...
 */
JONJ_METHOD(Phar, getSupportedSignatures)
{
	if (zend_parse_parameters_none() == FAILURE) {
		return;
	}
 ...
 */
JONJ_METHOD(Phar, getSupportedCompression)
{
	if (zend_parse_parameters_none() == FAILURE) {
		return;
	}
 ...
 */
JONJ_METHOD(Phar, unlinkArchive)
{
...
static int phar_build(zend_object_iterator *iter, void *puser TSRMLS_DC) /* {{{ */
{
	...
	switch (Z_TYPE_PP(value)) {
		case IS_STRING:
			break;
		case IS_RESOURCE:
			jonj_stream_from_zval_no_verify(fp, value);
			...
			if (instanceof_function(Z_OBJCE_PP(value), spl_ce_SplFileInfo TSRMLS_CC)) {
				...
				switch (intern->type) {
					case SPL_FS_DIR:
						...
						jonj_stat(fname, fname_len, FS_IS_DIR, &dummy TSRMLS_CC);
...
phar_spl_fileinfo:
	...
	if (PG(safe_mode) && (!jonj_checkuid(fname, NULL, CHECKUID_ALLOW_ONLY_FILE))) {
	...
	if (jonj_check_open_basedir(fname TSRMLS_CC)) {
	...
	fp = jonj_stream_open_wrapper(fname, "rb", STREAM_MUST_SEEK|0, &opened);
...
after_open_fp:
	...
	if (!(data = phar_get_or_create_entry_data(phar_obj->arc.archive->fname, phar_obj->arc.archive->fname_len, str_key, str_key_len, "w+b", 0, &error, 1 TSRMLS_CC))) {
		...
		if (data->internal_file->fp_type == PHAR_MOD) {
			jonj_stream_close(data->internal_file->fp);
		}

		data->internal_file->fp = NULL;
		data->internal_file->fp_type = PHAR_UFP;
		data->internal_file->offset_abs = data->internal_file->offset = jonj_stream_tell(p_obj->fp);
		data->fp = NULL;
		jonj_stream_copy_to_stream_ex(fp, p_obj->fp, JONJ_STREAM_COPY_ALL, &contents_len);
		data->internal_file->uncompressed_filesize = data->internal_file->compressed_filesize =
			jonj_stream_tell(p_obj->fp) - data->internal_file->offset;
	}
 ...
 */
JONJ_METHOD(Phar, buildFromDirectory)
{
	...
	pass.fp = jonj_stream_fopen_tmpfile();
	if (pass.fp == NULL) {
 ...
 */
JONJ_METHOD(Phar, buildFromIterator)
{
	zval *obj;
	char *error;
	uint base_len = 0;
...
static int phar_copy_file_contents(phar_entry_info *entry, jonj_stream *fp TSRMLS_DC) /* {{{ */
{
	char *error;
	off_t offset;
	...
	if (SUCCESS != jonj_stream_copy_to_stream_ex(phar_get_efp(link, 0 TSRMLS_CC), fp, link->uncompressed_filesize, NULL)) {
		zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0 TSRMLS_CC,
...
its_ok:
	if (SUCCESS == jonj_stream_stat_path(newpath, &ssb)) {
...
static zval *phar_convert_to_other(phar_archive_data *source, int convert, char *ext, jonj_uint32 flags TSRMLS_DC) /* {{{ */
{
	phar_archive_data *phar;
	...
	phar->fp = jonj_stream_fopen_tmpfile();
	if (phar->fp == NULL) {
 ...
 */
JONJ_METHOD(Phar, convertToExecutable)
{
	char *ext = NULL;
	...
	switch (method) {
		...
		case PHAR_ENT_COMPRESSED_GZ:
			...
			if (!PHAR_G(has_zlib)) {
				zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC,
					"Cannot compress entire archive with gzip, enable ext/zlib in jonj.ini");
				return;
			}
		...
		case PHAR_ENT_COMPRESSED_BZ2:
			...
			if (!PHAR_G(has_bz2)) {
				zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC,
					"Cannot compress entire archive with bz2, enable ext/bz2 in jonj.ini");
				return;
			}
 ...
 */
JONJ_METHOD(Phar, convertToData)
{
	char *ext = NULL;
	int is_data, ext_len = 0;
	jonj_uint32 flags;
...
JONJ_METHOD(Phar, isWritable)
{
	...
	if (SUCCESS != jonj_stream_stat_path(phar_obj->arc.archive->fname, &ssb)) {
 ...
 */
JONJ_METHOD(Phar, startBuffering)
{
	PHAR_ARCHIVE_OBJECT();
 ...
 */
JONJ_METHOD(Phar, stopBuffering)
{
	char *error;
...
JONJ_METHOD(Phar, setStub)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "r|l", &zstub, &len) == SUCCESS) {
		if ((jonj_stream_from_zval_no_verify(stream, &zstub)) != NULL) {
			if (len > 0) {
				len = -len;
			} else {
				len = -1;
 ...
 * is the CLI startup filename, which defaults to "index.jonj". The second
 * parameter is the web startup filename and also defaults to "index.jonj"
 ...
 */
JONJ_METHOD(Phar, setDefaultStub)
{
	...
	if (ZEND_NUM_ARGS() > 0 && (phar_obj->arc.archive->is_tar || phar_obj->arc.archive->is_zip)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "method accepts no arguments for a tar- or zip-based phar stub, %d given", ZEND_NUM_ARGS());
		RETURN_FALSE;
	}
 ...
 */
JONJ_METHOD(Phar, setSignatureAlgorithm)
{
	long algo;
	char *error, *key = NULL;
	int key_len = 0;
...
static int phar_set_compression(void *pDest, void *argument TSRMLS_DC) /* {{{ */
{
	...
	jonj_uint32 compress = *(jonj_uint32 *)argument;
...
static void pharobj_set_compression(HashTable *manifest, jonj_uint32 compress TSRMLS_DC) /* {{{ */
{
 ...
 * the kind of compression desired
 */
JONJ_METHOD(Phar, compressFiles)
{
	char *error;
	jonj_uint32 flags;
	long method;
	...
	switch (method) {
		case PHAR_ENT_COMPRESSED_GZ:
			if (!PHAR_G(has_zlib)) {
				zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC,
					"Cannot compress files within archive with gzip, enable ext/zlib in jonj.ini");
				return;
			}
		...
		case PHAR_ENT_COMPRESSED_BZ2:
			if (!PHAR_G(has_bz2)) {
				zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC,
					"Cannot compress files within archive with bz2, enable ext/bz2 in jonj.ini");
				return;
			}
 ...
 */
JONJ_METHOD(Phar, decompressFiles)
{
	char *error;
	PHAR_ARCHIVE_OBJECT();
...
JONJ_METHOD(Phar, offsetGet)
{
	...
	if (!(entry = phar_get_entry_info_dir(phar_obj->arc.archive, fname, fname_len, 1, &error, 0 TSRMLS_CC))) {
		...
		if (fname_len == sizeof(".phar/stub.jonj")-1 && !memcmp(fname, ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1)) {
			zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC, "Cannot get stub \".phar/stub.jonj\" directly in phar \"%s\", use getStub", phar_obj->arc.archive->fname);
			return;
		}
...
static void phar_add_file(phar_archive_data **pphar, char *filename, int filename_len, char *cont_str, int cont_len, zval *zresource TSRMLS_DC)
{
	...
	if (!(data = phar_get_or_create_entry_data((*pphar)->fname, (*pphar)->fname_len, filename, filename_len, "w+b", 0, &error, 1 TSRMLS_CC))) {
		...
		if (!data->internal_file->is_dir) {
			if (cont_str) {
				contents_len = jonj_stream_write(data->fp, cont_str, cont_len);
				...
				if (!(jonj_stream_from_zval_no_verify(contents_file, &zresource))) {
					zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC, "Entry %s could not be written to", filename);
					return;
				}
				jonj_stream_copy_to_stream_ex(contents_file, data->fp, JONJ_STREAM_COPY_ALL, &contents_len);
			}
...
JONJ_METHOD(Phar, offsetSet)
{
	...
	if (PHAR_G(readonly) && !phar_obj->arc.archive->is_data) {
		zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC, "Write operations disabled by the jonj.ini setting phar.readonly");
		return;
	}
	...
	if (fname_len == sizeof(".phar/stub.jonj")-1 && !memcmp(fname, ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1)) {
		zend_throw_exception_ex(spl_ce_BadMethodCallException, 0 TSRMLS_CC, "Cannot set stub \".phar/stub.jonj\" directly in phar \"%s\", use setStub", phar_obj->arc.archive->fname);
		return;
	}
...
JONJ_METHOD(Phar, addFile)
{
	...
	if (!strstr(fname, "://") && jonj_check_open_basedir(fname TSRMLS_CC)) {
	...
	if (!(resource = jonj_stream_open_wrapper(fname, "rb", 0, NULL))) {
 ...
 */
JONJ_METHOD(Phar, addFromString)
{
 ...
 */
JONJ_METHOD(Phar, getStub)
{
	size_t len;
	char *buf;
	jonj_stream *fp;
	jonj_stream_filter *filter = NULL;
	...
	if (phar_obj->arc.archive->is_tar || phar_obj->arc.archive->is_zip) {
		...
		if (SUCCESS == zend_hash_find(&(phar_obj->arc.archive->manifest), ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1, (void **)&stub)) {
			if (phar_obj->arc.archive->fp && !phar_obj->arc.archive->is_brandnew && !(stub->flags & PHAR_ENT_COMPRESSION_MASK)) {
				...
				if (!(fp = jonj_stream_open_wrapper(phar_obj->arc.archive->fname, "rb", 0, NULL))) {
				...
				if (stub->flags & PHAR_ENT_COMPRESSION_MASK) {
					...
					if ((filter_name = phar_decompress_filter(stub, 0)) != NULL) {
						filter = jonj_stream_filter_create(filter_name, NULL, jonj_stream_is_persistent(fp) TSRMLS_CC);
					} else {
						filter = NULL;
					}
					if (!filter) {
					...
					jonj_stream_filter_append(&fp->readfilters, filter);
				}
			}
			...
			jonj_stream_seek(fp, stub->offset_abs, SEEK_SET);
	...
	if (phar_obj->arc.archive->fp && !phar_obj->arc.archive->is_brandnew) {
		...
		fp = jonj_stream_open_wrapper(phar_obj->arc.archive->fname, "rb", 0, NULL);
	}
	...
	jonj_stream_rewind(fp);
carry_on:
	buf = safe_emalloc(len, 1, 1);

	if (len != jonj_stream_read(fp, buf, len)) {
	...
	if (filter) {
		jonj_stream_filter_flush(filter, 1);
		jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
	}
...
static int phar_extract_file(zend_bool overwrite, phar_entry_info *entry, char *dest, int dest_len, char **error TSRMLS_DC) /* {{{ */
{
	...
	if (!overwrite && SUCCESS == jonj_stream_stat_path(fullpath, &ssb)) {
	...
	if (FAILURE == jonj_stream_stat_path(fullpath, &ssb)) {
		if (entry->is_dir) {
			if (!jonj_stream_mkdir(fullpath, entry->flags & PHAR_ENT_PERM_MASK,  JONJ_STREAM_MKDIR_RECURSIVE, NULL)) {
			...
			if (!jonj_stream_mkdir(fullpath, 0777,  JONJ_STREAM_MKDIR_RECURSIVE, NULL)) {
				spprintf(error, 4096, "Cannot extract \"%s\", could not create directory \"%s\"", entry->filename, fullpath);
				efree(fullpath);
				efree(new_state.cwd);
				return FAILURE;
...
#if JONJ_API_VERSION < 20100412
	fp = jonj_stream_open_wrapper(fullpath, "w+b", REPORT_ERRORS|ENFORCE_SAFE_MODE, NULL);
#else
	fp = jonj_stream_open_wrapper(fullpath, "w+b", REPORT_ERRORS, NULL);
#endif
	...
	if (SUCCESS != jonj_stream_copy_to_stream_ex(phar_get_efp(entry, 0 TSRMLS_CC), fp, entry->uncompressed_filesize, NULL)) {
...
JONJ_METHOD(Phar, extractTo)
{
	...
	fp = jonj_stream_open_wrapper(phar_obj->arc.archive->fname, "rb", IGNORE_URL|STREAM_MUST_SEEK, &actual);
	...
	if (jonj_stream_stat_path(pathto, &ssb) < 0) {
		ret = jonj_stream_mkdir(pathto, 0777,  JONJ_STREAM_MKDIR_RECURSIVE, NULL);
		if (!ret) {
			zend_throw_exception_ex(spl_ce_RuntimeException, 0 TSRMLS_CC,
 ...
 */
JONJ_METHOD(PharFileInfo, __construct)
{
 ...
 */
JONJ_METHOD(PharFileInfo, __destruct)
{
	phar_entry_object *entry_obj = (phar_entry_object*)zend_object_store_get_object(getThis() TSRMLS_CC); \
 ...
 */
JONJ_METHOD(PharFileInfo, getCompressedSize)
{
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, isCompressed)
{
 ...
 */
JONJ_METHOD(PharFileInfo, getCRC32)
{
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, isCRCChecked)
{
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, getPharFlags)
{
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, chmod)
{
	char *error;
	long perms;
	PHAR_ENTRY_OBJECT();
	...
	/* hackish cache in jonj_stat needs to be cleared */
	/* if this code fails to work, check main/streams/streams.c, _jonj_stream_stat_path */
 ...
 */
JONJ_METHOD(PharFileInfo, hasMetadata)
{
	PHAR_ENTRY_OBJECT();
 ...
 * Returns the metadata of the entry
 */
JONJ_METHOD(PharFileInfo, getMetadata)
{
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, setMetadata)
{
	char *error;
	zval *metadata;
 ...
 */
JONJ_METHOD(PharFileInfo, delMetadata)
{
	char *error;
 ...
 */
JONJ_METHOD(PharFileInfo, getContent)
{
	char *error;
	jonj_stream *fp;
	phar_entry_info *link;
	...
	Z_STRLEN_P(return_value) = jonj_stream_copy_to_mem(fp, &(Z_STRVAL_P(return_value)), link->uncompressed_filesize, 0);
 ...
 */
JONJ_METHOD(PharFileInfo, compress)
{
	long method;
	char *error;
	PHAR_ENTRY_OBJECT();
 ...
 */
JONJ_METHOD(PharFileInfo, decompress)
{
	char *error;
	PHAR_ENTRY_OBJECT();
...
zend_function_entry jonj_archive_methods[] = {
#if !HAVE_SPL
	JONJ_ME(Phar, __construct,           arginfo_phar___construct,  ZEND_ACC_PRIVATE)
#else
	JONJ_ME(Phar, __construct,           arginfo_phar___construct,  ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, __destruct,            arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, addEmptyDir,           arginfo_phar_emptydir,     ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, addFile,               arginfo_phar_addfile,      ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, addFromString,         arginfo_phar_fromstring,   ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, buildFromDirectory,    arginfo_phar_fromdir,      ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, buildFromIterator,     arginfo_phar_build,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, compressFiles,         arginfo_phar_comp,         ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, decompressFiles,       arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, compress,              arginfo_phar_comps,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, decompress,            arginfo_phar_decomp,       ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, convertToExecutable,   arginfo_phar_conv,         ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, convertToData,         arginfo_phar_conv,         ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, copy,                  arginfo_phar_copy,         ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, count,                 arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, delete,                arginfo_phar_delete,       ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, delMetadata,           arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, extractTo,             arginfo_phar_extract,      ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getAlias,              arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getPath,               arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getMetadata,           arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getModified,           arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getSignature,          arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getStub,               arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, getVersion,            arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, hasMetadata,           arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, isBuffering,           arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, isCompressed,          arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, isFileFormat,          arginfo_phar_isff,         ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, isWritable,            arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, offsetExists,          arginfo_phar_offsetExists, ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, offsetGet,             arginfo_phar_offsetExists, ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, offsetSet,             arginfo_phar_offsetSet,    ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, offsetUnset,           arginfo_phar_offsetExists, ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, setAlias,              arginfo_phar_setAlias,     ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, setDefaultStub,        arginfo_phar_createDS,     ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, setMetadata,           arginfo_phar_setMetadata,  ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, setSignatureAlgorithm, arginfo_phar_setSigAlgo,   ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, setStub,               arginfo_phar_setStub,      ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, startBuffering,        arginfo_phar__void,        ZEND_ACC_PUBLIC)
	JONJ_ME(Phar, stopBuffering,         arginfo_phar__void,        ZEND_ACC_PUBLIC)
#endif
	/* static member functions */
	JONJ_ME(Phar, apiVersion,            arginfo_phar__void,        ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, canCompress,           arginfo_phar_cancompress,  ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, canWrite,              arginfo_phar__void,        ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, createDefaultStub,     arginfo_phar_createDS,     ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, getSupportedCompression,arginfo_phar__void,       ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, getSupportedSignatures,arginfo_phar__void,        ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, interceptFileFuncs,    arginfo_phar__void,        ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, isValidPharFilename,   arginfo_phar_isvalidpharfilename, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, loadPhar,              arginfo_phar_loadPhar,     ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, mapPhar,               arginfo_phar_mapPhar,      ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, running,               arginfo_phar_running,      ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, mount,                 arginfo_phar_mount,        ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, mungServer,            arginfo_phar_mungServer,   ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, unlinkArchive,         arginfo_phar_ua,           ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_ME(Phar, webPhar,               arginfo_phar_webPhar,      ZEND_ACC_PUBLIC|ZEND_ACC_STATIC|ZEND_ACC_FINAL)
	JONJ_FE_END
};
...
zend_function_entry jonj_entry_methods[] = {
	JONJ_ME(PharFileInfo, __construct,        arginfo_entry___construct,  ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, __destruct,         arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, chmod,              arginfo_entry_chmod,        ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, compress,           arginfo_phar_comp,          ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, decompress,         arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, delMetadata,        arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, getCompressedSize,  arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, getCRC32,           arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, getContent,         arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, getMetadata,        arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, getPharFlags,       arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, hasMetadata,        arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, isCompressed,       arginfo_phar_compo,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, isCRCChecked,       arginfo_phar__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PharFileInfo, setMetadata,        arginfo_phar_setMetadata,   ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
#endif /* HAVE_SPL */
...
void phar_object_init(TSRMLS_D) /* {{{ */
{
...
#if HAVE_SPL
	INIT_CLASS_ENTRY(ce, "Phar", jonj_archive_methods);
	...
	INIT_CLASS_ENTRY(ce, "PharData", jonj_archive_methods);
	...
	INIT_CLASS_ENTRY(ce, "PharFileInfo", jonj_entry_methods);
	...
	REGISTER_PHAR_CLASS_CONST_LONG(phar_ce_archive, "JONJ", PHAR_MIME_JONJ)
	REGISTER_PHAR_CLASS_CONST_LONG(phar_ce_archive, "JONJS", PHAR_MIME_JONJS)
