...
#define PHAR_FUNC(name) \
	static JONJ_NAMED_FUNCTION(name)
...
PHAR_FUNC(phar_opendir) /* {{{ */
{
	...
	if (!IS_ABSOLUTE_PATH(filename, filename_len) && !strstr(filename, "://")) {
		...
		if (SUCCESS == phar_split_fname(fname, fname_len, &arch, &arch_len, &entry, &entry_len, 2, 0 TSRMLS_CC)) {
			...
			stream = jonj_stream_opendir(name, REPORT_ERRORS, context);
			efree(name);
			if (!stream) {
				RETURN_FALSE;
			}
...
phar_it:
			...
			stream = jonj_stream_open_wrapper_ex(name, "rb", 0 | REPORT_ERRORS, NULL, context);
			efree(name);
			...
			if ((len = jonj_stream_copy_to_mem(stream, &contents, maxlen, 0)) > 0) {
				...
				if (PG(magic_quotes_runtime)) {
					int newlen;
					contents = jonj_addslashes(contents, len, &newlen, 1 TSRMLS_CC); /* 1 = free source string */
					len = newlen;
				}
#endif
...
notfound:
		...
		efree(arch);
		context = jonj_stream_context_from_zval(zcontext, 0);
		stream = jonj_stream_open_wrapper_ex(name, mode, 0 | REPORT_ERRORS, NULL, context);
		efree(name);
		if (stream == NULL) {
			RETURN_FALSE;
		}
...
static void phar_fancy_stat(struct stat *stat_sb, int type, zval *return_value TSRMLS_DC)
{
	...
	case FS_TYPE:
		...
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Unknown file type (%u)", stat_sb->st_mode & S_IFMT);
...
static void phar_file_stat(const char *filename, jonj_stat_len filename_length, int type, void (*orig_stat_func)(INTERNAL_FUNCTION_PARAMETERS), INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	if (!filename_length) {
		RETURN_FALSE;
	}
...
void fname(INTERNAL_FUNCTION_PARAMETERS) { \
	if (!PHAR_G(intercepted)) { \
		...
		\
		phar_file_stat(filename, (jonj_stat_len) filename_len, funcnum, PHAR_G(orig), INTERNAL_FUNCTION_PARAM_PASSTHRU); \
	} \
}
/* }}} */
