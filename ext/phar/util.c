...
#else
static int phar_call_openssl_signverify(int is_sign, jonj_stream *fp, off_t end, char *key, int key_len, char **signature, int *signature_len TSRMLS_DC);
#endif
...
jonj_stream *phar_get_efp(phar_entry_info *entry, int follow_links TSRMLS_DC) /* {{{ */
{
	...
	if (phar_get_fp_type(entry TSRMLS_CC) == PHAR_FP) {
		...
		if (!entry->fp) {
			entry->fp = jonj_stream_open_wrapper(entry->tmp, "rb", STREAM_MUST_SEEK|0, NULL);
		}
		return entry->fp;
	}
}
...
{
	jonj_stream *fp = phar_get_efp(entry, follow_links TSRMLS_CC);
	off_t temp, eoffset;
	...
	return jonj_stream_seek(fp, temp, SEEK_SET);
}
/* }}} */
...
int phar_mount_entry(phar_archive_data *phar, char *filename, int filename_len, char *path, int path_len TSRMLS_DC) /* {{{ */
{
	...
	if (PG(safe_mode) && !is_phar && (!jonj_checkuid(entry.tmp, NULL, CHECKUID_CHECK_FILE_AND_DIR))) {
		efree(entry.tmp);
	...
	if (!is_phar && jonj_check_open_basedir(filename TSRMLS_CC)) {
		efree(entry.tmp);
		efree(entry.filename);
		return FAILURE;
	}
	...
	if (SUCCESS != jonj_stream_stat_path(filename, &ssb)) {
		efree(entry.tmp);
		efree(entry.filename);
		return FAILURE;
	}
...
splitted:
	...
	efree(arch);
	ret = jonj_resolve_path(filename, filename_len, path TSRMLS_CC);
	efree(path);
...
phar_entry_data *phar_get_or_create_entry_data(char *fname, int fname_len, char *path, int path_len, const char *mode, char allow_dir, char **error, int security TSRMLS_DC) /* {{{ */
{
	...
	etemp.fp = jonj_stream_fopen_tmpfile();
...
int phar_open_archive_fp(phar_archive_data *phar TSRMLS_DC) /* {{{ */
{
	...
	if (jonj_check_open_basedir(phar->fname TSRMLS_CC)) {
		return FAILURE;
	}

	phar_set_pharfp(phar, jonj_stream_open_wrapper(phar->fname, "rb", IGNORE_URL|STREAM_MUST_SEEK|0, NULL) TSRMLS_CC);
...
int phar_copy_entry_fp(phar_entry_info *source, phar_entry_info *dest, char **error TSRMLS_DC) /* {{{ */
{
	...
	dest->offset = 0;
	dest->is_modified = 1;
	dest->fp = jonj_stream_fopen_tmpfile();
	if (dest->fp == NULL) {
		spprintf(error, 0, "phar error: unable to create temporary file");
		return EOF;
	}
	...
	if (SUCCESS != jonj_stream_copy_to_stream_ex(phar_get_efp(link, 0 TSRMLS_CC), dest->fp, link->uncompressed_filesize, NULL)) {
		jonj_stream_close(dest->fp);
...
int phar_open_entry_fp(phar_entry_info *entry, char **error, int follow_links TSRMLS_DC) /* {{{ */
{
	...
	if (!phar_get_entrypufp(entry TSRMLS_CC)) {
		phar_set_entrypufp(entry, jonj_stream_fopen_tmpfile() TSRMLS_CC);
		if (!phar_get_entrypufp(entry TSRMLS_CC)) {
	...
	if ((filtername = phar_decompress_filter(entry, 0)) != NULL) {
		filter = jonj_stream_filter_create(filtername, NULL, 0 TSRMLS_CC);
	} else {
		filter = NULL;
	}
	...
	jonj_stream_seek(ufp, 0, SEEK_END);
	loc = jonj_stream_tell(ufp);
	jonj_stream_filter_append(&ufp->writefilters, filter);
	jonj_stream_seek(phar_get_entrypfp(entry TSRMLS_CC), phar_get_fp_offset(entry TSRMLS_CC), SEEK_SET);

	if (entry->uncompressed_filesize) {
		if (SUCCESS != jonj_stream_copy_to_stream_ex(phar_get_entrypfp(entry TSRMLS_CC), ufp, entry->compressed_filesize, NULL)) {
	...
	jonj_stream_flush(ufp);
	jonj_stream_filter_remove(filter, 1 TSRMLS_CC);

	if (jonj_stream_tell(ufp) - loc != (off_t) entry->uncompressed_filesize) {
		spprintf(error, 4096, "phar error: internal corruption of phar \"%s\" (actual filesize mismatch on file \"%s\")", phar->fname, entry->filename);
		return FAILURE;
	}
...
int phar_create_writeable_entry(phar_archive_data *phar, phar_entry_info *entry, char **error TSRMLS_DC) /* {{{ */
{
	if (entry->fp_type == PHAR_MOD) {
		...
		jonj_stream_truncate_set_size(entry->fp, 0);
	...
	if (phar->mounted_dirs.arBuckets && zend_hash_num_elements(&phar->mounted_dirs)) {
		...
		while (FAILURE != zend_hash_has_more_elements(&phar->mounted_dirs)) {
			...
			if ((int)keylen >= path_len || strncmp(str_key, path, keylen)) {
				...
				if (SUCCESS != jonj_stream_stat_path(test, &ssb)) {
					efree(test);
					return NULL;
				}
...
static int phar_call_openssl_signverify(int is_sign, jonj_stream *fp, off_t end, char *key, int key_len, char **signature, int *signature_len TSRMLS_DC) /* {{{ */
{
	...
	if (end != (off_t) jonj_stream_copy_to_mem(fp, &(Z_STRVAL_P(zdata)), (size_t) end, 0)) {
		zval_dtor(zdata);
		zval_dtor(zsig);
		zval_dtor(zkey);
		zval_dtor(openssl);
...
int phar_verify_signature(jonj_stream *fp, size_t end_of_phar, jonj_uint32 sig_type, char *sig, int sig_len, char *fname, char **signature, int *signature_len, char **error TSRMLS_DC) /* {{{ */
{
	int read_size, len;
	off_t read_len;
	...
	switch (sig_type) {
		case PHAR_SIG_OPENSSL: {
			...
			pfp = jonj_stream_open_wrapper(pfile, "rb", 0, NULL);
			efree(pfile);

#if JONJ_MAJOR_VERSION > 5
			if (!pfp || !(pubkey_len = jonj_stream_copy_to_mem(pfp, (void **) &pubkey, JONJ_STREAM_COPY_ALL, 0)) || !pubkey) {
#else
			if (!pfp || !(pubkey_len = jonj_stream_copy_to_mem(pfp, &pubkey, JONJ_STREAM_COPY_ALL, 0)) || !pubkey) {
#endif
				if (pfp) {
					jonj_stream_close(pfp);
				}
				if (error) {
			...
			while (read_size && (len = jonj_stream_read(fp, (char*)buf, read_size)) > 0) {
		...
		case PHAR_SIG_SHA512: {
			...
			while ((len = jonj_stream_read(fp, (char*)buf, read_size)) > 0) {
				JONJ_SHA512Update(&context, buf, len);
				read_len -= (off_t)len;
				if (read_len < read_size) {
					read_size = (int)read_len;
				}
			...
			JONJ_SHA512Final(digest, &context);
		...
		case PHAR_SIG_SHA256: {
			...
			while ((len = jonj_stream_read(fp, (char*)buf, read_size)) > 0) {
				JONJ_SHA256Update(&context, buf, len);
				read_len -= (off_t)len;
				if (read_len < read_size) {
					read_size = (int)read_len;
				}
			...
			JONJ_SHA256Final(digest, &context);
		...
		case PHAR_SIG_SHA1: {
			...
			while ((len = jonj_stream_read(fp, (char*)buf, read_size)) > 0) {
				JONJ_SHA1Update(&context, buf, len);
				read_len -= (off_t)len;
				if (read_len < read_size) {
					read_size = (int)read_len;
				}
		...
		case PHAR_SIG_MD5: {
			...
			while ((len = jonj_stream_read(fp, (char*)buf, read_size)) > 0) {
				JONJ_MD5Update(&context, buf, len);
				read_len -= (off_t)len;
				if (read_len < read_size) {
					read_size = (int)read_len;
				}
...
int phar_create_signature(phar_archive_data *phar, jonj_stream *fp, char **signature, int *signature_length, char **error TSRMLS_DC) /* {{{ */
{
	unsigned char buf[1024];
	int sig_len;
	...
	switch(phar->sig_flags) {
		...
		case PHAR_SIG_SHA512: {
			...
			while ((sig_len = jonj_stream_read(fp, (char*)buf, sizeof(buf))) > 0) {
				JONJ_SHA512Update(&context, buf, sig_len);
			}
		...
		case PHAR_SIG_SHA256: {
			...
			while ((sig_len = jonj_stream_read(fp, (char*)buf, sizeof(buf))) > 0) {
				JONJ_SHA256Update(&context, buf, sig_len);
			}
		...
		case PHAR_SIG_OPENSSL: {
			...
			if (FAILURE == phar_call_openssl_signverify(1, fp, jonj_stream_tell(fp), PHAR_G(openssl_privatekey), PHAR_G(openssl_privatekey_len), (char **)&sigbuf, &siglen TSRMLS_CC)) {
				if (error) {
					spprintf(error, 0, "unable to write phar \"%s\" with requested openssl signature", phar->fname);
				}
				return FAILURE;
		...
		case PHAR_SIG_SHA1: {
			...
			while ((sig_len = jonj_stream_read(fp, (char*)buf, sizeof(buf))) > 0) {
				JONJ_SHA1Update(&context, buf, sig_len);
			}
		...
		case PHAR_SIG_MD5: {
			...
			while ((sig_len = jonj_stream_read(fp, (char*)buf, sizeof(buf))) > 0) {
				JONJ_MD5Update(&context, buf, sig_len);
			}
