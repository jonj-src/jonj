...
jonj_stream_wrapper_ops phar_stream_wops = {
...
jonj_stream_wrapper jonj_stream_phar_wrapper = {
	&phar_stream_wops,
	NULL,
	0 /* is_url */
};
...
jonj_url* phar_parse_url(jonj_stream_wrapper *wrapper, const char *filename, const char *mode, int options TSRMLS_DC) /* {{{ */
{
	...
	if (mode[0] == 'a') {
		if (!(options & JONJ_STREAM_URL_STAT_QUIET)) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: open mode append not supported");
		}
		return NULL;
	}
	if (phar_split_fname(filename, strlen(filename), &arch, &arch_len, &entry, &entry_len, 2, (mode[0] == 'w' ? 2 : 0) TSRMLS_CC) == FAILURE) {
		if (!(options & JONJ_STREAM_URL_STAT_QUIET)) {
			if (arch && !entry) {
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: no directory in \"%s\", must have at least phar://%s/ for root directory (always use full path to a new phar)", filename, arch);
				arch = NULL;
			} else {
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: invalid url or non-existent phar \"%s\"", filename);
			}
		}
		return NULL;
	}
	resource = ecalloc(1, sizeof(jonj_url));
...
/*			fprintf(stderr, "Fragment:  %s\n", resource->fragment);*/
	...
	if (mode[0] == 'w' || (mode[0] == 'r' && mode[1] == '+')) {
		...
		if (PHAR_G(readonly) && (!pphar || !(*pphar)->is_data)) {
			if (!(options & JONJ_STREAM_URL_STAT_QUIET)) {
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: write operations disabled by the jonj.ini setting phar.readonly");
			}
		...
		if (phar_open_or_create_filename(resource->host, arch_len, NULL, 0, 0, options, &phar, &error TSRMLS_CC) == FAILURE)
		{
			if (error) {
				if (!(options & JONJ_STREAM_URL_STAT_QUIET)) {
					jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "%s", error);
				}
				efree(error);
			}
			jonj_url_free(resource);
...
static jonj_stream * phar_wrapper_open_url(jonj_stream_wrapper *wrapper, const char *path, const char *mode, int options, char **opened_path, jonj_stream_context *context STREAMS_DC TSRMLS_DC) /* {{{ */
{
	...
	/* we must have at the very least phar://alias.phar/internalfile.jonj */
	if (!resource->scheme || !resource->host || !resource->path) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: invalid url \"%s\"", path);
		return NULL;
	}

	if (strcasecmp("phar", resource->scheme)) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: not a phar stream url \"%s\"", path);
		return NULL;
	}
	...
	if (mode[0] == 'w' || (mode[0] == 'r' && mode[1] == '+')) {
		if (NULL == (idata = phar_get_or_create_entry_data(resource->host, host_len, internal_file, strlen(internal_file), mode, 0, &error, 1 TSRMLS_CC))) {
			if (error) {
				...
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: file \"%s\" could not be created in phar \"%s\"", internal_file, resource->host);
			}
			efree(internal_file);
			jonj_url_free(resource);
			return NULL;
		...
		fpf = jonj_stream_alloc(&phar_ops, idata, NULL, mode);
		jonj_url_free(resource);
		efree(internal_file);
		...
		if (!*internal_file && (options & STREAM_OPEN_FOR_INCLUDE)) {
			...
			if (FAILURE == phar_get_archive(&phar, resource->host, host_len, NULL, 0, NULL TSRMLS_CC)) {
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "file %s is not a valid phar archive", resource->host);
				efree(internal_file);
				jonj_url_free(resource);
				return NULL;
			}
			if (phar->is_tar || phar->is_zip) {
				if ((FAILURE == phar_get_entry_data(&idata, resource->host, host_len, ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1, "r", 0, &error, 0 TSRMLS_CC)) || !idata) {
					goto idata_error;
				}
				efree(internal_file);
				if (opened_path) {
...
idata_error:
			if (error) {
				...
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: \"%s\" is not a file in phar \"%s\"", internal_file, resource->host);
			}
			efree(internal_file);
			jonj_url_free(resource);
			return NULL;
	...
	if (!PHAR_G(cwd_init) && options & STREAM_OPEN_FOR_INCLUDE) {
		...
		PHAR_G(cwd_init) = 1;
		if ((idata->phar->is_tar || idata->phar->is_zip) && idata->internal_file->filename_len == sizeof(".phar/stub.jonj")-1 && !strncmp(idata->internal_file->filename, ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1)) {
...
static size_t phar_stream_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC) /* {{{ */
{
	...
	jonj_stream_seek(data->fp, data->position + data->zero, SEEK_SET);

	got = jonj_stream_read(data->fp, buf, MIN(count, entry->uncompressed_filesize - data->position));
	data->position = jonj_stream_tell(data->fp) - data->zero;
...
static int phar_stream_seek(jonj_stream *stream, off_t offset, int whence, off_t *newoffset TSRMLS_DC) /* {{{ */
{
	...
	res = jonj_stream_seek(data->fp, temp, SEEK_SET);
	*newoffset = jonj_stream_tell(data->fp) - data->zero;
...
static size_t phar_stream_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC) /* {{{ */
{
	...
	jonj_stream_seek(data->fp, data->position, SEEK_SET);
	if (count != jonj_stream_write(data->fp, buf, count)) {
		jonj_stream_wrapper_log_error(stream->wrapper, stream->flags TSRMLS_CC, "phar error: Could not write %d characters to \"%s\" in phar \"%s\"", (int) count, data->internal_file->filename, data->phar->fname);
		return -1;
	}
	data->position = jonj_stream_tell(data->fp);
...
static int phar_stream_flush(jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	...
	if (data->internal_file->is_modified) {
		...
		if (error) {
			jonj_stream_wrapper_log_error(stream->wrapper, REPORT_ERRORS TSRMLS_CC, "%s", error);
			efree(error);
		}
		return ret;
	} else {
 ...
 */
void phar_dostat(phar_archive_data *phar, phar_entry_info *data, jonj_stream_statbuf *ssb, zend_bool is_temp_dir TSRMLS_DC)
{
 ...
 */
static int phar_wrapper_stat(jonj_stream_wrapper *wrapper, const char *url, int flags,
				  jonj_stream_statbuf *ssb, jonj_stream_context *context TSRMLS_DC) /* {{{ */
{
	...
	if ((resource = phar_parse_url(wrapper, url, "r", flags|JONJ_STREAM_URL_STAT_QUIET TSRMLS_CC)) == NULL) {
		return FAILURE;
	}
	...
	if (phar->mounted_dirs.arBuckets && zend_hash_num_elements(&phar->mounted_dirs)) {
		...
		for (zend_hash_internal_pointer_reset_ex(&phar->mounted_dirs, &pos);
			...
			if ((int)keylen >= internal_file_len || strncmp(str_key, internal_file, keylen)) {
				...
				if (SUCCESS != jonj_stream_stat_path(test, &ssbi)) {
					efree(test);
					continue;
				}
...
static int phar_wrapper_unlink(jonj_stream_wrapper *wrapper, const char *url, int options, jonj_stream_context *context TSRMLS_DC) /* {{{ */
{
	...
	if ((resource = phar_parse_url(wrapper, url, "rb", options TSRMLS_CC)) == NULL) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: unlink failed");
		return 0;
	}
	...
	if (!resource->scheme || !resource->host || !resource->path) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: invalid url \"%s\"", url);
		return 0;
	}

	if (strcasecmp("phar", resource->scheme)) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: not a phar stream url \"%s\"", url);
		return 0;
	}
	...
	if (FAILURE == phar_get_entry_data(&idata, resource->host, host_len, internal_file, internal_file_len, "r", 0, &error, 1 TSRMLS_CC)) {
		...
		if (error) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "unlink of \"%s\" failed: %s", url, error);
			efree(error);
		} else {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "unlink of \"%s\" failed, file does not exist", url);
		}
		efree(internal_file);
		jonj_url_free(resource);
		return 0;
	...
	if (idata->internal_file->fp_refcount > 1) {
		...
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: \"%s\" in phar \"%s\", has open file pointers, cannot unlink", internal_file, resource->host);
		efree(internal_file);
		jonj_url_free(resource);
		phar_entry_delref(idata TSRMLS_CC);
		return 0;
...
{
	jonj_url *resource_from, *resource_to;
	char *error;
	...
	if ((resource_from = phar_parse_url(wrapper, url_from, "wb", options|JONJ_STREAM_URL_STAT_QUIET TSRMLS_CC)) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": invalid or non-writable url \"%s\"", url_from, url_to, url_from);
		return 0;
	}
	...
	if (PHAR_G(readonly) && (!pfrom || !pfrom->is_data)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: Write operations disabled by the jonj.ini setting phar.readonly");
		return 0;
	}

	if ((resource_to = phar_parse_url(wrapper, url_to, "wb", options|JONJ_STREAM_URL_STAT_QUIET TSRMLS_CC)) == NULL) {
		jonj_url_free(resource_from);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": invalid or non-writable url \"%s\"", url_from, url_to, url_to);
		return 0;
	}
	...
	if (strcmp(resource_from->host, resource_to->host)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\", not within the same phar archive", url_from, url_to);
		return 0;
	}
	...
	if (!resource_from->scheme || !resource_from->host || !resource_from->path) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": invalid url \"%s\"", url_from, url_to, url_from);
		return 0;
	}
	...
	if (!resource_to->scheme || !resource_to->host || !resource_to->path) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": invalid url \"%s\"", url_from, url_to, url_to);
		return 0;
	}
	...
	if (strcasecmp("phar", resource_from->scheme)) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": not a phar stream url \"%s\"", url_from, url_to, url_from);
		return 0;
	}
	...
	if (strcasecmp("phar", resource_to->scheme)) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": not a phar stream url \"%s\"", url_from, url_to, url_to);
		return 0;
	}
	...
	if (SUCCESS != phar_get_archive(&phar, resource_from->host, host_len, NULL, 0, &error TSRMLS_CC)) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": %s", url_from, url_to, error);
		efree(error);
		return 0;
	}
	...
	if (phar->is_persistent && FAILURE == phar_copy_on_write(&phar TSRMLS_CC)) {
		jonj_url_free(resource_from);
		jonj_url_free(resource_to);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\": could not make cached phar writeable", url_from, url_to);
		return 0;
	}
	...
	if (SUCCESS == zend_hash_find(&(phar->manifest), resource_from->path+1, strlen(resource_from->path)-1, (void **)&entry)) {
		...
		if (entry->is_deleted) {
			jonj_url_free(resource_from);
			jonj_url_free(resource_to);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\" from extracted phar archive, source has been deleted", url_from, url_to);
			return 0;
		}
		...
		if (!is_dir) {
			...
			jonj_url_free(resource_from);
			jonj_url_free(resource_to);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "phar error: cannot rename \"%s\" to \"%s\" from extracted phar archive, source does not exist", url_from, url_to);
			return 0;
