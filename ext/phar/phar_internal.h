...
#include "zend_vm.h"
#include "main/jonj_streams.h"
#include "main/streams/jonj_stream_plain_wrapper.h"
#include "main/SAPI.h"
#include "main/jonj_main.h"
#include "main/jonj_open_temporary_file.h"
...
#include "jonj_phar.h"
#ifdef PHAR_HASH_OK
#include "ext/hash/jonj_hash.h"
#include "ext/hash/jonj_hash_sha.h"
#endif

/* JONJ_ because this is public information via MINFO */
#define JONJ_PHAR_API_VERSION      "1.1.1"
/* x.y.z maps to 0xyz0 */
...
#define PHAR_MUNG_JONJ_SELF			(1<<0)
...
#ifndef jonj_uint16
# if SIZEOF_SHORT == 2
#  define jonj_uint16 unsigned short
# else
#  define jonj_uint16 uint16_t
# endif
#endif
#include "pharzip.h"
...
typedef struct _phar_entry_info {
	...
	jonj_uint32               uncompressed_filesize;
	jonj_uint32               timestamp;
	jonj_uint32               compressed_filesize;
	jonj_uint32               crc32;
	jonj_uint32               flags;
	/* remainder */
	/* when changing compression, save old flags in case fp is NULL */
	jonj_uint32               old_flags;
	zval                     *metadata;
	int                      metadata_len; /* only used for cached manifests */
	jonj_uint32               filename_len;
	char                     *filename;
...
struct _phar_archive_data {
	...
	jonj_uint32               flags;
	jonj_uint32               min_timestamp;
	jonj_uint32               max_timestamp;
	jonj_stream               *fp;
	/* decompressed file contents are stored here */
	jonj_stream               *ufp;
	int                      refcount;
	jonj_uint32               sig_flags;
	int                      sig_len;
	char                     *signature;
	zval                     *metadata;
	int                      metadata_len; /* only used for cached manifests */
...
static inline jonj_stream *phar_get_entrypfp(phar_entry_info *entry TSRMLS_DC)
{
...
static inline jonj_stream *phar_get_entrypufp(phar_entry_info *entry TSRMLS_DC)
{
	if (!entry->is_persistent) {
...
static inline void phar_set_entrypfp(phar_entry_info *entry, jonj_stream *fp TSRMLS_DC)
{
...
static inline void phar_set_entrypufp(phar_entry_info *entry, jonj_stream *fp TSRMLS_DC)
{
	if (!entry->phar->is_persistent) {
		entry->phar->ufp =  fp;
		return;
...
static inline jonj_stream *phar_get_pharfp(phar_archive_data *phar TSRMLS_DC)
{
...
static inline jonj_stream *phar_get_pharufp(phar_archive_data *phar TSRMLS_DC)
{
	if (!phar->is_persistent) {
		return phar->ufp;
	}
...
static inline void phar_set_pharfp(phar_archive_data *phar, jonj_stream *fp TSRMLS_DC)
{
	if (!phar->is_persistent) {
		phar->fp =  fp;
		return;
...
static inline void phar_set_pharufp(phar_archive_data *phar, jonj_stream *fp TSRMLS_DC)
{
	if (!phar->is_persistent) {
		phar->ufp =  fp;
		return;
...
#define PHAR_MIME_JONJ '\0'
#define PHAR_MIME_JONJS '\1'
...
extern jonj_stream_wrapper jonj_stream_phar_wrapper;
#else
