...
BEGIN_EXTERN_C()
void phar_dostat(phar_archive_data *phar, phar_entry_info *data, jonj_stream_statbuf *ssb, zend_bool is_dir TSRMLS_DC);
END_EXTERN_C()

jonj_stream_ops phar_dir_ops = {
...
static size_t phar_dir_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC) /* {{{ */
{
	...
	memset(buf, 0, sizeof(jonj_stream_dirent));
	memcpy(((jonj_stream_dirent *) buf)->d_name, str_key, to_read);
	((jonj_stream_dirent *) buf)->d_name[to_read + 1] = '\0';
 ...
 */
static jonj_stream *phar_make_dirstream(char *dir, HashTable *manifest TSRMLS_DC) /* {{{ */
{
	HashTable *data;
	...
	if ((*dir == '/' && dirlen == 1 && (manifest->nNumOfElements == 0)) || (dirlen >= sizeof(".phar")-1 && !memcmp(dir, ".phar", sizeof(".phar")-1))) {
		...
		efree(dir);
		return jonj_stream_alloc(&phar_dir_ops, data, NULL, "r");
	}
...
jonj_stream *phar_wrapper_open_dir(jonj_stream_wrapper *wrapper, const char *path, const char *mode, int options, char **opened_path, jonj_stream_context *context STREAMS_DC TSRMLS_DC) /* {{{ */
{
	...
	if ((resource = phar_parse_url(wrapper, path, mode, options TSRMLS_CC)) == NULL) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar url \"%s\" is unknown", path);
		return NULL;
	}
	...
	if (!resource->scheme || !resource->host || !resource->path) {
		if (resource->host && !resource->path) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: no directory in \"%s\", must have at least phar://%s/ for root directory (always use full path to a new phar)", path, resource->host);
			jonj_url_free(resource);
			return NULL;
		}
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: invalid url \"%s\", must have at least phar://%s/", path, path);
		return NULL;
	}

	if (strcasecmp("phar", resource->scheme)) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: not a phar url \"%s\"", path);
		return NULL;
	}
	...
	if (FAILURE == phar_get_archive(&phar, resource->host, host_len, NULL, 0, &error TSRMLS_CC)) {
		if (error) {
			...
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar file \"%s\" is unknown", resource->host);
		}
		jonj_url_free(resource);
		return NULL;
	}
	...
	if (SUCCESS == zend_hash_find(&phar->manifest, internal_file, strlen(internal_file), (void**)&entry) && !entry->is_dir) {
		...
		if (entry->is_mounted) {
			jonj_url_free(resource);
			return jonj_stream_opendir(entry->tmp, options, context);
		}
...
int phar_wrapper_mkdir(jonj_stream_wrapper *wrapper, const char *url_from, int mode, int options, jonj_stream_context *context TSRMLS_DC) /* {{{ */
{
	...
	if (FAILURE == phar_split_fname(url_from, strlen(url_from), &arch, &arch_len, &entry2, &entry_len, 2, 2 TSRMLS_CC)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\", no phar archive specified", url_from);
		return 0;
	}
	...
	if (PHAR_G(readonly) && (!phar || !phar->is_data)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\", write operations disabled", url_from);
		return 0;
	}
	...
	if (!resource->scheme || !resource->host || !resource->path) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: invalid url \"%s\"", url_from);
		return 0;
	}

	if (strcasecmp("phar", resource->scheme)) {
		jonj_url_free(resource);
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: not a phar stream url \"%s\"", url_from);
		return 0;
	}
	...
	if (FAILURE == phar_get_archive(&phar, resource->host, host_len, NULL, 0, &error TSRMLS_CC)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", error retrieving phar information: %s", resource->path+1, resource->host, error);
		efree(error);
		jonj_url_free(resource);
		return 0;
	}
	...
	if ((e = phar_get_entry_info_dir(phar, resource->path + 1, strlen(resource->path + 1), 2, &error, 1 TSRMLS_CC))) {
		...
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", directory already exists", resource->path+1, resource->host);
		jonj_url_free(resource);
		return 0;
	}

	if (error) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", %s", resource->path+1, resource->host, error);
		efree(error);
		jonj_url_free(resource);
		return 0;
	}
	...
	if (phar_get_entry_info_dir(phar, resource->path + 1, strlen(resource->path + 1), 0, &error, 1 TSRMLS_CC)) {
		...
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", file already exists", resource->path+1, resource->host);
		jonj_url_free(resource);
		return 0;
	}
	...
	if (SUCCESS != zend_hash_add(&phar->manifest, entry.filename, entry.filename_len, (void*)&entry, sizeof(phar_entry_info), NULL)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", adding to manifest failed", entry.filename, phar->fname);
		efree(error);
	...
	if (error) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot create directory \"%s\" in phar \"%s\", %s", entry.filename, phar->fname, error);
...
int phar_wrapper_rmdir(jonj_stream_wrapper *wrapper, const char *url, int options, jonj_stream_context *context TSRMLS_DC) /* {{{ */
{
	...
	if (FAILURE == phar_split_fname(url, strlen(url), &arch, &arch_len, &entry2, &entry_len, 2, 2 TSRMLS_CC)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot remove directory \"%s\", no phar archive specified, or phar archive does not exist", url);
		return 0;
	}
	...
	if (PHAR_G(readonly) && (!phar || !phar->is_data)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot rmdir directory \"%s\", write operations disabled", url);
		return 0;
	}
	...
	if (FAILURE == phar_get_archive(&phar, resource->host, host_len, NULL, 0, &error TSRMLS_CC)) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot remove directory \"%s\" in phar \"%s\", error retrieving phar information: %s", resource->path+1, resource->host, error);
		efree(error);
		jonj_url_free(resource);
		return 0;
	}
	...
	if (!(entry = phar_get_entry_info_dir(phar, resource->path + 1, path_len, 2, &error, 1 TSRMLS_CC))) {
		if (error) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot remove directory \"%s\" in phar \"%s\", %s", resource->path+1, resource->host, error);
			efree(error);
		} else {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot remove directory \"%s\" in phar \"%s\", directory does not exist", resource->path+1, resource->host);
		}
		jonj_url_free(resource);
		return 0;
	}
	...
	if (!entry->is_deleted) {
		for (zend_hash_internal_pointer_reset(&phar->manifest);
			...
			if (key_len > path_len && 
				...
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: Directory not empty");
	...
	if (entry->is_temp_dir) {
		...
		if (error) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "phar error: cannot remove directory \"%s\" in phar \"%s\", %s", entry->filename, phar->fname, error);
			jonj_url_free(resource);
			efree(error);
			return 0;
		}
