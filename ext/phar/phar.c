...
static void phar_split_cache_list(TSRMLS_D) /* {{{ */
{
	...
	for (key = jonj_strtok_r(tmp, ds, &lasts);
			key;
			key = jonj_strtok_r(NULL, ds, &lasts)) {
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_BOOLEAN("phar.readonly", "1", JONJ_INI_ALL, phar_ini_modify_handler, readonly, zend_phar_globals, phar_globals)
	STD_JONJ_INI_BOOLEAN("phar.require_hash", "1", JONJ_INI_ALL, phar_ini_modify_handler, require_hash, zend_phar_globals, phar_globals)
	STD_JONJ_INI_ENTRY("phar.cache_list", "", JONJ_INI_SYSTEM, phar_ini_cache_list, cache_list, zend_phar_globals, phar_globals)
JONJ_INI_END()
...
# define PHAR_GET_32(buffer, var) \
	...
	buffer += 4
# define PHAR_GET_16(buffer, var) \
	var = *(jonj_uint16*)(buffer); \
	buffer += 2
#endif
#define PHAR_ZIP_16(var) ((jonj_uint16)((((jonj_uint16)var[0]) & 0xff) | \
	(((jonj_uint16)var[1]) & 0xff) << 8))
#define PHAR_ZIP_32(var) ((jonj_uint32)((((jonj_uint32)var[0]) & 0xff) | \
	(((jonj_uint32)var[1]) & 0xff) << 8 | \
	(((jonj_uint32)var[2]) & 0xff) << 16 | \
	(((jonj_uint32)var[3]) & 0xff) << 24))
	...
	if (SUCCESS == phar_get_archive(&phar, fname, fname_len, alias, alias_len, error TSRMLS_CC)
		...
		if (!is_data) {
			...
			if (!phar->halt_offset && !phar->is_brandnew && (phar->is_tar || phar->is_zip)) {
				if (PHAR_G(readonly) && FAILURE == zend_hash_find(&(phar->manifest), ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1, (void **)&stub)) {
					if (error) {
 ...
 */
int phar_parse_metadata(char **buffer, zval **metadata, jonj_uint32 zip_metadata_len TSRMLS_DC) /* {{{ */
{
	...
	if (zip_metadata_len) {
		...
		if (!jonj_var_unserialize(metadata, &p, p + zip_metadata_len, &var_hash TSRMLS_CC)) {
			efree(p_buff);
 ...
 * directly.
 */
static int phar_parse_pharfile(jonj_stream *fp, char *fname, int fname_len, char *alias, int alias_len, long halt_offset, phar_archive_data** pphar, jonj_uint32 compression, char **error TSRMLS_DC) /* {{{ */
{
	char b32[4], *buffer, *endbuffer, *savebuf;
	phar_archive_data *mydata = NULL;
	phar_entry_info entry;
	jonj_uint32 manifest_len, manifest_count, manifest_flags, manifest_index, tmp_len, sig_flags;
	jonj_uint16 manifest_ver;
	jonj_uint32 len;
	long offset;
	...
	if (-1 == jonj_stream_seek(fp, halt_offset, SEEK_SET)) {
		MAPPHAR_ALLOC_FAIL("cannot seek to __HALT_COMPILER(); location in phar \"%s\"")
	}

	buffer = b32;

	if (3 != jonj_stream_read(fp, buffer, 3)) {
	...
	if ((*buffer == ' ' || *buffer == '\n') && *(buffer + 1) == '?' && *(buffer + 2) == '>') {
		int nextchar;
		halt_offset += 3;
		if (EOF == (nextchar = jonj_stream_getc(fp))) {
			MAPPHAR_ALLOC_FAIL("internal corruption of phar \"%s\" (truncated manifest at stub end)")
		}

		if ((char) nextchar == '\r') {
			/* if we have an \r we require an \n as well */
			if (EOF == (nextchar = jonj_stream_getc(fp)) || (char)nextchar != '\n') {
				MAPPHAR_ALLOC_FAIL("internal corruption of phar \"%s\" (truncated manifest at stub end)")
			}
			++halt_offset;
		}
	...
	if (4 != jonj_stream_read(fp, buffer, 4)) {
	...
	if (manifest_len < 10 || manifest_len != jonj_stream_read(fp, buffer, manifest_len)) {
	...
	if (manifest_flags & PHAR_HDR_SIGNATURE) {
		...
		if (-1 == jonj_stream_seek(fp, -8, SEEK_END)
		|| (read_len = jonj_stream_tell(fp)) < 20
		|| 8 != jonj_stream_read(fp, sig_buf, 8)
		...
		switch(sig_flags) {
			case PHAR_SIG_OPENSSL: {
				jonj_uint32 signature_len;
				char *sig;
				off_t whence;

				/* we store the signature followed by the signature length */
				if (-1 == jonj_stream_seek(fp, -12, SEEK_CUR)
				|| 4 != jonj_stream_read(fp, sig_buf, 4)) {
					efree(savebuf);
					jonj_stream_close(fp);
					if (error) {
				...
				if (-1 == jonj_stream_seek(fp, whence, SEEK_CUR)
				|| !(end_of_phar = jonj_stream_tell(fp))
				|| signature_len != jonj_stream_read(fp, sig, signature_len)) {
					efree(savebuf);
					efree(sig);
					jonj_stream_close(fp);
					if (error) {
			...
			case PHAR_SIG_SHA512: {
				...
				jonj_stream_seek(fp, -(8 + 64), SEEK_END);
				read_len = jonj_stream_tell(fp);

				if (jonj_stream_read(fp, (char*)digest, sizeof(digest)) != sizeof(digest)) {
					efree(savebuf);
					jonj_stream_close(fp);
					if (error) {
						spprintf(error, 0, "phar \"%s\" has a broken signature", fname);
			...
			case PHAR_SIG_SHA256: {
				...
				jonj_stream_seek(fp, -(8 + 32), SEEK_END);
				read_len = jonj_stream_tell(fp);
			...
			case PHAR_SIG_SHA1: {
				...
				jonj_stream_seek(fp, -(8 + 20), SEEK_END);
				read_len = jonj_stream_tell(fp);
			...
			case PHAR_SIG_MD5: {
				...
				jonj_stream_seek(fp, -(8 + 16), SEEK_END);
				read_len = jonj_stream_tell(fp);
...
check_file:
	if (phar_open_parsed_phar(fname, fname_len, alias, alias_len, is_data, options, test, &my_error TSRMLS_CC) == SUCCESS) {
		...
		if (PHAR_G(readonly) && !(*test)->is_data && ((*test)->is_tar || (*test)->is_zip)) {
			phar_entry_info *stub;
			if (FAILURE == zend_hash_find(&((*test)->manifest), ".phar/stub.jonj", sizeof(".phar/stub.jonj")-1, (void **)&stub)) {
				spprintf(error, 0, "'%s' is not a phar archive. Use PharData::__construct() for a standard zip or tar archive", fname);
				return FAILURE;
			}
		}
...
int phar_create_or_parse_filename(char *fname, int fname_len, char *alias, int alias_len, int is_data, int options, phar_archive_data** pphar, char **error TSRMLS_DC) /* {{{ */
{
	...
	fp = jonj_stream_open_wrapper(fname, "rb", IGNORE_URL|STREAM_MUST_SEEK|0, &actual);
	...
	if (PHAR_G(readonly) && !is_data) {
		if (options & REPORT_ERRORS) {
			if (error) {
				spprintf(error, 0, "creating archive \"%s\" disabled by the jonj.ini setting phar.readonly", fname);
			}
		}
		return FAILURE;
	}
	...
	zend_hash_init(&mydata->virtual_dirs, sizeof(char *),
		zend_get_hash_value, NULL, (zend_bool)mydata->is_persistent);
	mydata->fname_len = fname_len;
	snprintf(mydata->version, sizeof(mydata->version), "%s", JONJ_PHAR_API_VERSION);
...
int phar_open_from_filename(char *fname, int fname_len, char *alias, int alias_len, int options, phar_archive_data** pphar, char **error TSRMLS_DC) /* {{{ */
{
	...
	fp = jonj_stream_open_wrapper(fname, "rb", IGNORE_URL|STREAM_MUST_SEEK, &actual);
 ...
 * or FAILURE is returned and pphar is set to a pointer to the phar's manifest
 */
static int phar_open_from_fp(jonj_stream* fp, char *fname, int fname_len, char *alias, int alias_len, int options, phar_archive_data** pphar, int is_data, char **error TSRMLS_DC) /* {{{ */
{
	...
	long halt_offset;
	size_t got;
	jonj_uint32 compression = PHAR_FILE_COMPRESSED_NONE;

	if (error) {
		*error = NULL;
	}

	if (-1 == jonj_stream_rewind(fp)) {
	...
	while(!jonj_stream_eof(fp)) {
		if ((got = jonj_stream_read(fp, buffer+tokenlen, readsize)) < (size_t) tokenlen) {
		...
		if (!test) {
			...
			if (!memcmp(pos, gz_magic, 3)) {
				...
				if (!PHAR_G(has_zlib)) {
					MAPPHAR_ALLOC_FAIL("unable to decompress gzipped phar archive \"%s\" to temporary file, enable zlib extension in jonj.ini")
				}
...
#define MAX_WBITS 15
				...
				if (!(temp = jonj_stream_fopen_tmpfile())) {
					MAPPHAR_ALLOC_FAIL("unable to create temporary file for decompression of gzipped phar archive \"%s\"")
				}

				jonj_stream_rewind(fp);
				filter = jonj_stream_filter_create("zlib.inflate", &filterparams, jonj_stream_is_persistent(fp) TSRMLS_CC);
				...
				if (!filter) {
					...
					if (!filter) {
						jonj_stream_close(temp);
						MAPPHAR_ALLOC_FAIL("unable to decompress gzipped phar archive \"%s\", ext/zlib is buggy in JONJ versions older than 5.2.6")
					}
				} else {
					zval_dtor(&filterparams);
				}

				jonj_stream_filter_append(&temp->writefilters, filter);

				if (SUCCESS != jonj_stream_copy_to_stream_ex(fp, temp, JONJ_STREAM_COPY_ALL, NULL)) {
					if (err) {
						jonj_stream_close(temp);
						MAPPHAR_ALLOC_FAIL("unable to decompress gzipped phar archive \"%s\", ext/zlib is buggy in JONJ versions older than 5.2.6")
					}
				...
				if (!PHAR_G(has_bz2)) {
					MAPPHAR_ALLOC_FAIL("unable to decompress bzipped phar archive \"%s\" to temporary file, enable bz2 extension in jonj.ini")
				}
				...
				jonj_stream_rewind(fp);
				filter = jonj_stream_filter_create("bzip2.decompress", NULL, jonj_stream_is_persistent(fp) TSRMLS_CC);
			...
			if (!memcmp(pos, zip_magic, 4)) {
				jonj_stream_seek(fp, 0, SEEK_END);
...
static int phar_analyze_path(const char *fname, const char *ext, int ext_len, int for_create TSRMLS_DC) /* {{{ */
{
	...
	if (SUCCESS == jonj_stream_stat_path((char *) filename, &ssb)) {
		...
		if (SUCCESS != jonj_stream_stat_path((char *) filename, &ssb)) {
			if (!slash) {
				...
				if (SUCCESS != jonj_stream_stat_path(realpath, &ssb)) {
					efree(realpath);
					efree(filename);
					return FAILURE;
				}
...
static int jonj_check_dots(const char *element, int n) /* {{{ */
{
	for(n--; n >= 0; --n) {
		if (element[n] != '.') {
			return 1;
...
#define IS_DIRECTORY_UP(element, len) \
	(len >= 2 && !jonj_check_dots(element, len))
...
int phar_open_executed_filename(char *alias, int alias_len, char **error TSRMLS_DC) /* {{{ */
{
	...
	if (!strcmp(fname, "[no active file]")) {
		if (error) {
			spprintf(error, 0, "cannot initialize a phar outside of JONJ execution");
		}
		return FAILURE;
	}
	...
	fp = jonj_stream_open_wrapper(fname, "rb", IGNORE_URL|STREAM_MUST_SEEK|REPORT_ERRORS, &actual);
 ...
 */
int phar_postprocess_file(phar_entry_data *idata, jonj_uint32 crc32, char **error, int process_zip TSRMLS_DC) /* {{{ */
{
	jonj_uint32 crc = ~0;
	int len = idata->internal_file->uncompressed_filesize;
	jonj_stream *fp = idata->fp;
	...
	if (entry->is_zip && process_zip > 0) {
		...
		jonj_stream_seek(phar_get_entrypfp(idata->internal_file TSRMLS_CC), entry->header_offset, SEEK_SET);

		if (sizeof(local) != jonj_stream_read(phar_get_entrypfp(idata->internal_file TSRMLS_CC), (char *) &local, sizeof(local))) {
		...
		if (((PHAR_ZIP_16(local.flags)) & 0x8) == 0x8) {
			jonj_stream_seek(phar_get_entrypfp(idata->internal_file TSRMLS_CC),
			...
			if (sizeof(desc) != jonj_stream_read(phar_get_entrypfp(idata->internal_file TSRMLS_CC),
	...
	jonj_stream_seek(fp, idata->zero, SEEK_SET);

	while (len--) {
		CRC32(crc, jonj_stream_getc(fp));
	}
...
char *phar_create_default_stub(const char *index_jonj, const char *web_index, size_t *len, char **error TSRMLS_DC) /* {{{ */
{
	char *stub = NULL;
	...
	if (!index_jonj) {
		index_jonj = "index.jonj";
	}

	if (!web_index) {
		web_index = "index.jonj";
	}

	index_len = strlen(index_jonj);
	web_len = strlen(web_index);

	if (index_len > 400) {
		/* ridiculous size not allowed for index.jonj startup filename */
		if (error) {
	...
	phar_get_stub(index_jonj, web_index, len, &stub, index_len+1, web_len+1 TSRMLS_CC);
	return stub;
}
/* }}} */
...
int phar_flush(phar_archive_data *phar, char *user_stub, long len, int convert, char **error TSRMLS_DC) /* {{{ */
{
	...
	long offset;
	size_t wrote;
	jonj_uint32 manifest_len, mytime, loc, new_manifest_count;
	jonj_uint32 newcrc32;
	jonj_stream *file, *oldfile, *newfile, *stubfile;
	jonj_stream_filter *filter;
	jonj_serialize_data_t metadata_hash;
	...
	if (phar->fp && !phar->is_brandnew) {
		oldfile = phar->fp;
		closeoldfile = 0;
		jonj_stream_rewind(oldfile);
	} else {
		oldfile = jonj_stream_open_wrapper(phar->fname, "rb", 0, NULL);
		closeoldfile = oldfile != NULL;
	}
	newfile = jonj_stream_fopen_tmpfile();
	if (!newfile) {
		if (error) {
			spprintf(error, 0, "unable to create temporary file");
		}
		if (closeoldfile) {
			jonj_stream_close(oldfile);
		}
		return EOF;
	}
	...
	if (user_stub) {
		if (len < 0) {
			/* resource passed in */
			if (!(jonj_stream_from_zval_no_verify(stubfile, (zval **)user_stub))) {
				if (closeoldfile) {
					jonj_stream_close(oldfile);
				}
				jonj_stream_close(newfile);
				if (error) {
			...
			if (len == -1) {
				len = JONJ_STREAM_COPY_ALL;
			} else {
				len = -len;
			}
			user_stub = 0;

			if (!(len = jonj_stream_copy_to_mem(stubfile, &user_stub, len, 0)) || !user_stub) {
				if (closeoldfile) {
					jonj_stream_close(oldfile);
				}
				jonj_stream_close(newfile);
		...
		if ((pos = jonj_stristr(tmp, halt_stub, len, sizeof(halt_stub) - 1)) == NULL) {
			efree(tmp);
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
		...
		if ((size_t)len != jonj_stream_write(newfile, user_stub, len)
		||			  5 != jonj_stream_write(newfile, " ?>\r\n", 5)) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
			jonj_stream_close(newfile);
		...
		if (!user_stub && phar->halt_offset && oldfile && !phar->is_brandnew) {
			jonj_stream_copy_to_stream_ex(oldfile, newfile, phar->halt_offset, &written);
			newstub = NULL;
		} else {
			...
			written = jonj_stream_write(newfile, newstub, phar->halt_offset);
		}
	...
	manifest_ftell = jonj_stream_tell(newfile);
	...
	if (phar->metadata) {
		JONJ_VAR_SERIALIZE_INIT(metadata_hash);
		jonj_var_serialize(&main_metadata_str, &phar->metadata, &metadata_hash TSRMLS_CC);
		JONJ_VAR_SERIALIZE_DESTROY(metadata_hash);
	} else {
	...
	for (zend_hash_internal_pointer_reset(&phar->manifest);
		...
		if (entry->metadata) {
			...
			JONJ_VAR_SERIALIZE_INIT(metadata_hash);
			jonj_var_serialize(&entry->metadata_str, &entry->metadata, &metadata_hash TSRMLS_CC);
			JONJ_VAR_SERIALIZE_DESTROY(metadata_hash);
		} else {
			if (entry->metadata_str.c) {
				smart_str_free(&entry->metadata_str);
		...
		for (loc = 0;loc < mytime; ++loc) {
			CRC32(newcrc32, jonj_stream_getc(file));
		}
		...
		filter = jonj_stream_filter_create(phar_compress_filter(entry, 0), NULL, 0 TSRMLS_CC);
		if (!filter) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
		...
		in read count */
		entry->cfp = jonj_stream_fopen_tmpfile();
		if (!entry->cfp) {
			if (error) {
				spprintf(error, 0, "unable to create temporary file");
			}
		...
		jonj_stream_filter_append((&entry->cfp->writefilters), filter);
		if (SUCCESS != jonj_stream_copy_to_stream_ex(file, entry->cfp, entry->uncompressed_filesize, NULL)) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
			jonj_stream_close(newfile);
		...
		jonj_stream_filter_flush(filter, 1);
		jonj_stream_flush(entry->cfp);
		jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
		jonj_stream_seek(entry->cfp, 0, SEEK_END);
		entry->compressed_filesize = (jonj_uint32) jonj_stream_tell(entry->cfp);
		/* generate crc on compressed file */
		jonj_stream_rewind(entry->cfp);
	...
	if (sizeof(manifest) != jonj_stream_write(newfile, manifest, sizeof(manifest))
	|| (size_t)phar->alias_len != jonj_stream_write(newfile, phar->alias, phar->alias_len)) {
	...
	if (4 != jonj_stream_write(newfile, manifest, 4) || (main_metadata_str.len
	&& main_metadata_str.len != jonj_stream_write(newfile, main_metadata_str.c, main_metadata_str.len))) {
	...
	for (zend_hash_internal_pointer_reset(&phar->manifest);
		...
		if (4 != jonj_stream_write(newfile, entry_buffer, 4)
		|| entry->filename_len != jonj_stream_write(newfile, entry->filename, entry->filename_len)
		|| (entry->is_dir && 1 != jonj_stream_write(newfile, "/", 1))) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
			jonj_stream_close(newfile);
		...
		*/
		...
		if (sizeof(entry_buffer) != jonj_stream_write(newfile, entry_buffer, sizeof(entry_buffer))
		|| entry->metadata_str.len != jonj_stream_write(newfile, entry->metadata_str.c, entry->metadata_str.len)) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
	...
	/* Hack - see bug #65028, add padding byte to the end of the manifest */
	if(manifest_hack) {
		if(1 != jonj_stream_write(newfile, manifest, 1)) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
	...
	offset = jonj_stream_tell(newfile);
	for (zend_hash_internal_pointer_reset(&phar->manifest);
		zend_hash_has_more_elements(&phar->manifest) == SUCCESS;
		zend_hash_move_forward(&phar->manifest)) {
		...
		entry->offset = entry->offset_abs = offset;
		offset += entry->compressed_filesize;
		if (jonj_stream_copy_to_stream_ex(file, newfile, entry->compressed_filesize, &wrote) == FAILURE) {
			if (closeoldfile) {
				jonj_stream_close(oldfile);
			}
	...
	if (global_flags & PHAR_HDR_SIGNATURE) {
		...
		switch(phar->sig_flags) {
			...
			default: {
				...
				jonj_stream_write(newfile, digest, digest_len);
				efree(digest);
				if (phar->sig_flags == PHAR_SIG_OPENSSL) {
					phar_set_32(sig_buf, digest_len);
					jonj_stream_write(newfile, sig_buf, 4);
				}
				break;
			}
		}
		...
		jonj_stream_write(newfile, sig_buf, 4);
		jonj_stream_write(newfile, "GBMB", 4);
	}
	...
	*/
	...
	if (phar->donotflush) {
		...
		phar->fp = jonj_stream_open_wrapper(phar->fname, "w+b", IGNORE_URL|STREAM_MUST_SEEK|REPORT_ERRORS, NULL);
		if (!phar->fp) {
			phar->fp = newfile;
			if (error) {
		...
		if (phar->flags & PHAR_FILE_COMPRESSED_GZ) {
			...
			filter = jonj_stream_filter_create("zlib.deflate", &filterparams, jonj_stream_is_persistent(phar->fp) TSRMLS_CC);
			zval_dtor(&filterparams);

			if (!filter) {
				if (error) {
					spprintf(error, 4096, "unable to compress all contents of phar \"%s\" using zlib, JONJ versions older than 5.2.6 have a buggy zlib", phar->fname);
				}
				return EOF;
			}

			jonj_stream_filter_append(&phar->fp->writefilters, filter);
			jonj_stream_copy_to_stream_ex(newfile, phar->fp, JONJ_STREAM_COPY_ALL, NULL);
			jonj_stream_filter_flush(filter, 1);
			jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
			jonj_stream_close(phar->fp);
			...
			filter = jonj_stream_filter_create("bzip2.compress", NULL, jonj_stream_is_persistent(phar->fp) TSRMLS_CC);
			jonj_stream_filter_append(&phar->fp->writefilters, filter);
			jonj_stream_copy_to_stream_ex(newfile, phar->fp, JONJ_STREAM_COPY_ALL, NULL);
			jonj_stream_filter_flush(filter, 1);
			jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
	...
	if (-1 == jonj_stream_seek(phar->fp, phar->halt_offset, SEEK_SET)) {
		if (error) {
...
{
	return jonj_stream_read(phar_get_pharfp((phar_archive_data*)handle TSRMLS_CC), buf, len);
}
/* }}} */
	...
	if (strstr(file_handle->filename, ".phar") && !strstr(file_handle->filename, "://")) {
		if (SUCCESS == phar_open_from_filename((char*)file_handle->filename, strlen(file_handle->filename), NULL, 0, 0, &phar, NULL TSRMLS_CC)) {
			if (phar->is_zip || phar->is_tar) {
				...
				spprintf(&name, 4096, "phar://%s/%s", file_handle->filename, ".phar/stub.jonj");
				...
				phar->is_persistent ?
					jonj_stream_rewind(PHAR_GLOBALS->cached_fp[phar->phar_pos].fp) :
					jonj_stream_rewind(phar->fp);
...
#define PHAR_SET_MIME(mimetype, ret, fileext) \
	...
	PHAR_SET_MIME("text/html", PHAR_MIME_JONJS, "jonjs")
	...
	PHAR_SET_MIME("", PHAR_MIME_JONJ, "jonj")
	PHAR_SET_MIME("", PHAR_MIME_JONJ, "inc")
...
JONJ_GSHUTDOWN_FUNCTION(phar) /* {{{ */
{
	zend_hash_destroy(&phar_globals->mime_types);
}
/* }}} */

JONJ_MINIT_FUNCTION(phar) /* {{{ */
{
	...
	return jonj_register_url_stream_wrapper("phar", &jonj_stream_phar_wrapper TSRMLS_CC);
}
/* }}} */

JONJ_MSHUTDOWN_FUNCTION(phar) /* {{{ */
{
	jonj_unregister_url_stream_wrapper("phar" TSRMLS_CC);
		...
		if (PHAR_GLOBALS->cached_fp) {
			for (i = 0; i < zend_hash_num_elements(&cached_phars); ++i) {
				if (PHAR_GLOBALS->cached_fp[i].fp) {
					jonj_stream_close(PHAR_GLOBALS->cached_fp[i].fp);
				}
				if (PHAR_GLOBALS->cached_fp[i].ufp) {
					jonj_stream_close(PHAR_GLOBALS->cached_fp[i].ufp);
				}
...
JONJ_MINFO_FUNCTION(phar) /* {{{ */
{
	phar_request_initialize(TSRMLS_C);
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "Phar: JONJ Archive support", "enabled");
	jonj_info_print_table_row(2, "Phar EXT version", JONJ_PHAR_VERSION);
	jonj_info_print_table_row(2, "Phar API version", JONJ_PHAR_API_VERSION);
	jonj_info_print_table_row(2, "SVN revision", "$Id$");
	jonj_info_print_table_row(2, "Phar-based phar archives", "enabled");
	jonj_info_print_table_row(2, "Tar-based phar archives", "enabled");
	jonj_info_print_table_row(2, "ZIP-based phar archives", "enabled");

	if (PHAR_G(has_zlib)) {
		jonj_info_print_table_row(2, "gzip compression", "enabled");
	} else {
		jonj_info_print_table_row(2, "gzip compression", "disabled (install ext/zlib)");
	}

	if (PHAR_G(has_bz2)) {
		jonj_info_print_table_row(2, "bzip2 compression", "enabled");
	} else {
		jonj_info_print_table_row(2, "bzip2 compression", "disabled (install pecl/bz2)");
	}
#ifdef PHAR_HAVE_OPENSSL
	jonj_info_print_table_row(2, "Native OpenSSL support", "enabled");
#else
	if (zend_hash_exists(&module_registry, "openssl", sizeof("openssl"))) {
		jonj_info_print_table_row(2, "OpenSSL support", "enabled");
	} else {
		jonj_info_print_table_row(2, "OpenSSL support", "disabled (install ext/openssl)");
	}
#endif
	jonj_info_print_table_end();

	jonj_info_print_box_start(0);
	PUTS("Phar based on pear/JONJ_Archive, original concept by Davey Shafik.");
	PUTS(!sapi_module.jonjinfo_as_text?"<br />":"\n");
...
zend_module_entry phar_module_entry = {
	...
	JONJ_MINFO(phar),
	JONJ_PHAR_VERSION,
	JONJ_MODULE_GLOBALS(phar),   /* globals descriptor */
	JONJ_GINIT(phar),            /* globals ctor */
	JONJ_GSHUTDOWN(phar),        /* globals dtor */
