...
#include "jonj_pcntl.h"
#include "jonj_signal.h"
...
static JONJ_GINIT_FUNCTION(pcntl);
...
const zend_function_entry pcntl_functions[] = {
	JONJ_FE(pcntl_fork,			arginfo_pcntl_void)
	JONJ_FE(pcntl_waitpid,		arginfo_pcntl_waitpid)
	JONJ_FE(pcntl_wait,			arginfo_pcntl_wait)
	JONJ_FE(pcntl_signal,		arginfo_pcntl_signal)
	JONJ_FE(pcntl_signal_dispatch,	arginfo_pcntl_void)
	JONJ_FE(pcntl_wifexited,		arginfo_pcntl_wifexited)
	JONJ_FE(pcntl_wifstopped,	arginfo_pcntl_wifstopped)
	JONJ_FE(pcntl_wifsignaled,	arginfo_pcntl_wifsignaled)
	JONJ_FE(pcntl_wexitstatus,	arginfo_pcntl_wifexitstatus)
	JONJ_FE(pcntl_wtermsig,		arginfo_pcntl_wtermsig)
	JONJ_FE(pcntl_wstopsig,		arginfo_pcntl_wstopsig)
	JONJ_FE(pcntl_exec,			arginfo_pcntl_exec)
	JONJ_FE(pcntl_alarm,			arginfo_pcntl_alarm)
	JONJ_FE(pcntl_get_last_error,	arginfo_pcntl_void)
	JONJ_FALIAS(pcntl_errno, pcntl_get_last_error,	NULL)
	JONJ_FE(pcntl_strerror,		arginfo_pcntl_strerror)
#ifdef HAVE_GETPRIORITY
	JONJ_FE(pcntl_getpriority,	arginfo_pcntl_getpriority)
#endif
#ifdef HAVE_SETPRIORITY
	JONJ_FE(pcntl_setpriority,	arginfo_pcntl_setpriority)
#endif
#ifdef HAVE_SIGPROCMASK
	JONJ_FE(pcntl_sigprocmask,	arginfo_pcntl_sigprocmask)
#endif
#if HAVE_SIGWAITINFO && HAVE_SIGTIMEDWAIT
	JONJ_FE(pcntl_sigwaitinfo,	arginfo_pcntl_sigwaitinfo)
	JONJ_FE(pcntl_sigtimedwait,	arginfo_pcntl_sigtimedwait)
#endif
	JONJ_FE_END
};
...
zend_module_entry pcntl_module_entry = {
	...
	pcntl_functions,
	JONJ_MINIT(pcntl),
	JONJ_MSHUTDOWN(pcntl),
	JONJ_RINIT(pcntl),
	JONJ_RSHUTDOWN(pcntl),
	JONJ_MINFO(pcntl),
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(pcntl),
	JONJ_GINIT(pcntl),
	NULL,
	NULL,
...
void jonj_register_signal_constants(INIT_FUNC_ARGS)
{
...
static void jonj_pcntl_register_errno_constants(INIT_FUNC_ARGS)
{
#ifdef EINTR
...
{
	zend_hash_init(&PCNTL_G(jonj_signal_table), 16, NULL, ZVAL_PTR_DTOR, 0);
...
{
	jonj_register_signal_constants(INIT_FUNC_ARGS_PASSTHRU);
	jonj_pcntl_register_errno_constants(INIT_FUNC_ARGS_PASSTHRU);
	jonj_add_tick_function(pcntl_signal_dispatch);
...
JONJ_MINFO_FUNCTION(pcntl)
{
	...
	jonj_info_print_table_header(2, "pcntl support", "enabled");
...
JONJ_FUNCTION(pcntl_fork)
{
	...
	if (id == -1) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error %d", errno);
	}
	...
	if ( ZEND_NUM_ARGS() == 3 ) {
		...
		*(pair) = NULL;
		...
		if (execve(path, argv, envp) == -1) {
			PCNTL_G(last_error) = errno;
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error has occurred: (errno %d) %s", errno, strerror(errno));
		}
...
/* {{{ proto bool pcntl_signal(int signo, callback handle [, bool restart_syscalls])
   Assigns a system signal handler to a JONJ function */
JONJ_FUNCTION(pcntl_signal)
{
	...
	if (signo < 1 || signo > 32) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid signal");
		RETURN_FALSE;
	}
	...
	if (!PCNTL_G(spares)) {
		...
		int i;
		for (i = 0; i < 32; i++) {
			struct jonj_pcntl_pending_signal *psig;
	...
	if (Z_TYPE_P(handle) == IS_LONG) {
		if (Z_LVAL_P(handle) != (long) SIG_DFL && Z_LVAL_P(handle) != (long) SIG_IGN) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid value for handle argument specified");
			RETURN_FALSE;
		}
		if (jonj_signal(signo, (Sigfunc *) Z_LVAL_P(handle), (int) restart_syscalls) == SIG_ERR) {
			PCNTL_G(last_error) = errno;
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error assigning signal");
			RETURN_FALSE;
		}
		zend_hash_index_del(&PCNTL_G(jonj_signal_table), signo);
		RETURN_TRUE;
	}

	if (!zend_is_callable(handle, 0, &func_name TSRMLS_CC)) {
		PCNTL_G(last_error) = EINVAL;
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s is not a callable function name error", func_name);
		efree(func_name);
		RETURN_FALSE;
	}
	efree(func_name);
	...
	zend_hash_index_update(&PCNTL_G(jonj_signal_table), signo, (void **) &handle, sizeof(zval *), (void **) &dest_handle);
	if (dest_handle) zval_add_ref(dest_handle);

	if (jonj_signal4(signo, pcntl_signal_handler, (int) restart_syscalls, 1) == SIG_ERR) {
		PCNTL_G(last_error) = errno;
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error assigning signal");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(pcntl_getpriority)
{
	...
	if (errno) {
		...
		switch (errno) {
			case ESRCH:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error %d: No process was located using the given parameters", errno);
				break;
			case EINVAL:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error %d: Invalid identifier flag", errno);
				break;
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown error %d has occurred", errno);
				break;
		}
		RETURN_FALSE;
	}
	...
	if (setpriority(who, pid, pri)) {
		...
		switch (errno) {
			...
			case EINVAL:
				...
				break;
			case EPERM:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error %d: A process was located, but neither its effective nor real user ID matched the effective user ID of the caller", errno);
				break;
			case EACCES:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error %d: Only a super user may attempt to increase the process priority", errno);
				break;
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown error %d has occurred", errno);
				break;
...
/* Our custom signal handler that calls the appropriate jonj_function */
...
void pcntl_signal_dispatch()
{
	...
	struct jonj_pcntl_pending_signal *queue, *next;
	sigset_t mask;
	sigset_t old_mask;
	TSRMLS_FETCH();
	...
	while (queue) {
		if (zend_hash_index_find(&PCNTL_G(jonj_signal_table), queue->signo, (void **) &handle)==SUCCESS) {
			MAKE_STD_ZVAL(retval);
			MAKE_STD_ZVAL(param);
			ZVAL_NULL(retval);
			ZVAL_LONG(param, queue->signo);

			/* Call jonj signal handler - Note that we do not report errors, and we ignore the return value */
