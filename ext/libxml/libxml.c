...
#include "jonj_libxml.h"

#define JONJ_LIBXML_ERROR 0
#define JONJ_LIBXML_CTX_ERROR 1
#define JONJ_LIBXML_CTX_WARNING 2

/* a true global for initialization */
static int _jonj_libxml_initialized = 0;
static int _jonj_libxml_per_request_initialization = 1;
static xmlExternalEntityLoader _jonj_libxml_default_entity_loader;

typedef struct _jonj_libxml_func_handler {
	jonj_libxml_export_node export_func;
} jonj_libxml_func_handler;

static HashTable jonj_libxml_exports;

static ZEND_DECLARE_MODULE_GLOBALS(libxml)
static JONJ_GINIT_FUNCTION(libxml);

static JONJ_FUNCTION(libxml_set_streams_context);
static JONJ_FUNCTION(libxml_use_internal_errors);
static JONJ_FUNCTION(libxml_get_last_error);
static JONJ_FUNCTION(libxml_clear_errors);
static JONJ_FUNCTION(libxml_get_errors);
static JONJ_FUNCTION(libxml_set_external_entity_loader);
static JONJ_FUNCTION(libxml_disable_entity_loader);
...
/* {{{ function prototypes */
static JONJ_MINIT_FUNCTION(libxml);
static JONJ_RINIT_FUNCTION(libxml);
static JONJ_MSHUTDOWN_FUNCTION(libxml);
static JONJ_MINFO_FUNCTION(libxml);
static int jonj_libxml_post_deactivate(void);
...
static const zend_function_entry libxml_functions[] = {
	JONJ_FE(libxml_set_streams_context, arginfo_libxml_set_streams_context)
	JONJ_FE(libxml_use_internal_errors, arginfo_libxml_use_internal_errors)
	JONJ_FE(libxml_get_last_error, arginfo_libxml_get_last_error)
	JONJ_FE(libxml_clear_errors, arginfo_libxml_clear_errors)
	JONJ_FE(libxml_get_errors, arginfo_libxml_get_errors)
	JONJ_FE(libxml_disable_entity_loader, arginfo_libxml_disable_entity_loader)
	JONJ_FE(libxml_set_external_entity_loader, arginfo_libxml_set_external_entity_loader)
	JONJ_FE_END
};
...
zend_module_entry libxml_module_entry = {
	...
	JONJ_MINIT(libxml),       /* extension-wide startup function */
	JONJ_MSHUTDOWN(libxml),   /* extension-wide shutdown function */
	JONJ_RINIT(libxml),       /* per-request startup function */
	NULL,                    /* per-request shutdown function */
	JONJ_MINFO(libxml),       /* information function */
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(libxml), /* globals descriptor */
	JONJ_GINIT(libxml),          /* globals ctor */
	NULL,                       /* globals dtor */
	jonj_libxml_post_deactivate, /* post deactivate */
...
static int jonj_libxml_clear_object(jonj_libxml_node_object *object TSRMLS_DC)
{
	if (object->properties) {
		object->properties = NULL;
	}
	jonj_libxml_decrement_node_ptr(object TSRMLS_CC);
	return jonj_libxml_decrement_doc_ref(object TSRMLS_CC);
}

static int jonj_libxml_unregister_node(xmlNodePtr nodep TSRMLS_DC)
{
	jonj_libxml_node_object *wrapper;

	jonj_libxml_node_ptr *nodeptr = nodep->_private;

	if (nodeptr != NULL) {
		wrapper = nodeptr->_private;
		if (wrapper) {
			jonj_libxml_clear_object(wrapper TSRMLS_CC);
		} else {
...
static void jonj_libxml_node_free(xmlNodePtr node)
{
	if(node) {
		if (node->_private != NULL) {
			((jonj_libxml_node_ptr *) node->_private)->node = NULL;
		}
		switch (node->type) {
...
static void jonj_libxml_node_free_list(xmlNodePtr node TSRMLS_DC)
{
	xmlNodePtr curnode;
		...
		while (curnode != NULL) {
			...
			switch (node->type) {
				...
				case XML_ENTITY_REF_NODE:
					jonj_libxml_node_free_list((xmlNodePtr) node->properties TSRMLS_CC);
					break;
				case XML_ATTRIBUTE_NODE:
				...
				case XML_NAMESPACE_DECL:
				case XML_TEXT_NODE:
					jonj_libxml_node_free_list(node->children TSRMLS_CC);
					break;
				default:
					jonj_libxml_node_free_list(node->children TSRMLS_CC);
					jonj_libxml_node_free_list((xmlNodePtr) node->properties TSRMLS_CC);
			...
			curnode = node->next;
			xmlUnlinkNode(node);
			if (jonj_libxml_unregister_node(node TSRMLS_CC) == 0) {
				node->doc = NULL;
			}
			jonj_libxml_node_free(node);
		}
	}
}
...
static void _jonj_libxml_destroy_fci(zend_fcall_info *fci)
{
	if (fci->size > 0) {
...
static void *jonj_libxml_streams_IO_open_wrapper(const char *filename, const char *mode, const int read_only)
{
	...
	if (uri && (uri->scheme == NULL ||
		...
		isescaped = 1;
#if LIBXML_VERSION >= 20902 && defined(JONJ_WIN32)
		/* Libxml 2.9.2 prefixes local paths with file:/ instead of file://,
			thus the jonj stream wrapper will fail on a valid case. For this
	...
	/* logic copied from _jonj_stream_stat, but we only want to fail
	...
	wrapper = jonj_stream_locate_url_wrapper(resolved_path, &path_to_open, 0 TSRMLS_CC);
	if (wrapper && read_only && wrapper->wops->url_stat) {
		if (wrapper->wops->url_stat(wrapper, path_to_open, JONJ_STREAM_URL_STAT_QUIET, &ssbuf, NULL TSRMLS_CC) == -1) {
			if (isescaped) {
				xmlFree(resolved_path);
			}
			return NULL;
	...
	context = jonj_stream_context_from_zval(LIBXML(stream_context), 0);
	
	ret_val = jonj_stream_open_wrapper_ex(path_to_open, (char *)mode, REPORT_ERRORS, NULL, context);
	if (isescaped) {
		xmlFree(resolved_path);
	}
	return ret_val;
...
static void *jonj_libxml_streams_IO_open_read_wrapper(const char *filename)
{
	return jonj_libxml_streams_IO_open_wrapper(filename, "rb", 1);
}

static void *jonj_libxml_streams_IO_open_write_wrapper(const char *filename)
{
	return jonj_libxml_streams_IO_open_wrapper(filename, "wb", 0);
}

static int jonj_libxml_streams_IO_read(void *context, char *buffer, int len)
{
	TSRMLS_FETCH();
	return jonj_stream_read((jonj_stream*)context, buffer, len);
}

static int jonj_libxml_streams_IO_write(void *context, const char *buffer, int len)
{
	TSRMLS_FETCH();
	return jonj_stream_write((jonj_stream*)context, buffer, len);
}

static int jonj_libxml_streams_IO_close(void *context)
{
	TSRMLS_FETCH();
...
jonj_libxml_input_buffer_create_filename(const char *URI, xmlCharEncoding enc)
{
	...
	context = jonj_libxml_streams_IO_open_read_wrapper(URI);
	...
	if (ret != NULL) {
		ret->context = context;
		ret->readcallback = jonj_libxml_streams_IO_read;
		ret->closecallback = jonj_libxml_streams_IO_close;
	} else
		jonj_libxml_streams_IO_close(context);

	return(ret);
}

static xmlOutputBufferPtr
jonj_libxml_output_buffer_create_filename(const char *URI,
                              ...
                              int compression ATTRIBUTE_UNUSED)
{
	...
	if (unescaped != NULL) {
		context = jonj_libxml_streams_IO_open_write_wrapper(unescaped);
		xmlFree(unescaped);
	}

	/* try with a non-escaped URI this may be a strange filename */
	if (context == NULL) {
		context = jonj_libxml_streams_IO_open_write_wrapper(URI);
	}
	...
	if (ret != NULL) {
		ret->context = context;
		ret->writecallback = jonj_libxml_streams_IO_write;
		ret->closecallback = jonj_libxml_streams_IO_close;
	}
...
static int _jonj_libxml_free_error(xmlErrorPtr error)
{
...
static void _jonj_list_set_error_structure(xmlErrorPtr error, const char *msg)
{
	xmlError error_copy;
	int ret;
...
static void jonj_libxml_ctx_error_level(int level, void *ctx, const char *msg TSRMLS_DC)
{
	...
	if (parser != NULL && parser->input != NULL) {
		if (parser->input->filename) {
			jonj_error_docref(NULL TSRMLS_CC, level, "%s in %s, line: %d", msg, parser->input->filename, parser->input->line);
		} else {
			jonj_error_docref(NULL TSRMLS_CC, level, "%s in Entity, line: %d", msg, parser->input->line);
		}
	}
}

void jonj_libxml_issue_error(int level, const char *msg TSRMLS_DC)
{
	if (LIBXML(error_list)) {
		_jonj_list_set_error_structure(NULL, msg);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, level, "%s", msg);
	}
}

static void jonj_libxml_internal_error_handler(int error_type, void *ctx, const char **msg, va_list ap)
{
	char *buf;
	...
	if (output == 1) {
		if (LIBXML(error_list)) {
			_jonj_list_set_error_structure(NULL, LIBXML(error_buffer).c);
		} else {
			switch (error_type) {
				case JONJ_LIBXML_CTX_ERROR:
					jonj_libxml_ctx_error_level(E_WARNING, ctx, LIBXML(error_buffer).c TSRMLS_CC);
					break;
				case JONJ_LIBXML_CTX_WARNING:
					jonj_libxml_ctx_error_level(E_NOTICE, ctx, LIBXML(error_buffer).c TSRMLS_CC);
					break;
				default:
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", LIBXML(error_buffer).c);
			}
		}
...
static xmlParserInputPtr _jonj_libxml_external_entity_loader(const char *URL,
		const char *ID, xmlParserCtxtPtr context)
{
	...
	if (fci->size == 0) {
		...
		return _jonj_libxml_default_entity_loader(URL, ID, context);
	}
...
is_string:
			...
			jonj_stream *stream;
			jonj_stream_from_zval_no_verify(stream, &retval_ptr);
			if (stream == NULL) {
				jonj_libxml_ctx_error(context,
				...
				if (pib == NULL) {
					jonj_libxml_ctx_error(context, "Could not allocate parser "
							"input buffer");
				} else {
					/* make stream not being closed when the zval is freed */
					zend_list_addref(stream->rsrc_id);
					pib->context = stream;
					pib->readcallback = jonj_libxml_streams_IO_read;
					pib->closecallback = jonj_libxml_streams_IO_close;
...
static xmlParserInputPtr _jonj_libxml_pre_ext_ent_loader(const char *URL,
		const char *ID, xmlParserCtxtPtr context)
{
	TSRMLS_FETCH();

	/* Check whether we're running in a JONJ context, since the entity loader
	 ...
	 * (if _jonj_libxml_per_request_initialization */
	if (xmlGenericError == jonj_libxml_error_handler && PG(modules_activated)) {
		return _jonj_libxml_external_entity_loader(URL, ID, context);
	} else {
		return _jonj_libxml_default_entity_loader(URL, ID, context);
	}
}
...
JONJ_LIBXML_API void jonj_libxml_ctx_error(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	jonj_libxml_internal_error_handler(JONJ_LIBXML_CTX_ERROR, ctx, &msg, args);
	va_end(args);
}
...
JONJ_LIBXML_API void jonj_libxml_ctx_warning(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	jonj_libxml_internal_error_handler(JONJ_LIBXML_CTX_WARNING, ctx, &msg, args);
	va_end(args);
}

JONJ_LIBXML_API void jonj_libxml_structured_error_handler(void *userData, xmlErrorPtr error)
{
	_jonj_list_set_error_structure(error, NULL);
...
JONJ_LIBXML_API void jonj_libxml_error_handler(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	jonj_libxml_internal_error_handler(JONJ_LIBXML_ERROR, ctx, &msg, args);
	va_end(args);
}
...
{
	if (!_jonj_libxml_initialized) {
		/* we should be the only one's to ever init!! */
		xmlInitParser();
		
		_jonj_libxml_default_entity_loader = xmlGetExternalEntityLoader();
		xmlSetExternalEntityLoader(_jonj_libxml_pre_ext_ent_loader);

		zend_hash_init(&jonj_libxml_exports, 0, NULL, NULL, 1);

		_jonj_libxml_initialized = 1;
	}
}
...
#endif
		xmlCleanupParser();
		zend_hash_destroy(&jonj_libxml_exports);
		
		xmlSetExternalEntityLoader(_jonj_libxml_default_entity_loader);
		_jonj_libxml_initialized = 0;
	}
}
...
static JONJ_MINIT_FUNCTION(libxml)
{
	...
	if (sapi_module.name) {
		...
		for (sapi_name = supported_sapis; *sapi_name; sapi_name++) {
			if (strcmp(sapi_module.name, *sapi_name) == 0) {
				_jonj_libxml_per_request_initialization = 0;
				break;
			}
		}
	}
	...
	if (!_jonj_libxml_per_request_initialization) {
		...
		xmlSetGenericErrorFunc(NULL, jonj_libxml_error_handler);
		xmlParserInputBufferCreateFilenameDefault(jonj_libxml_input_buffer_create_filename);
		xmlOutputBufferCreateFilenameDefault(jonj_libxml_output_buffer_create_filename);
	}
...
static int jonj_libxml_post_deactivate(void)
{
	...
	_jonj_libxml_destroy_fci(&LIBXML(entity_loader).fci);
...
static JONJ_MINFO_FUNCTION(libxml)
{
	...
	jonj_info_print_table_row(2, "libXML support", "active");
	jonj_info_print_table_row(2, "libXML Compiled Version", LIBXML_DOTTED_VERSION);
	jonj_info_print_table_row(2, "libXML Loaded Version", (char *)xmlParserVersion);
	jonj_info_print_table_row(2, "libXML streams", "enabled");
...
static JONJ_FUNCTION(libxml_use_internal_errors)
{
	...
	if (current_handler && current_handler == jonj_libxml_structured_error_handler) {
		retval = 1;
	} else {
		retval = 0;
	}
	...
	if (use_errors == 0) {
		...
		xmlSetStructuredErrorFunc(NULL, jonj_libxml_structured_error_handler);
		if (LIBXML(error_list) == NULL) {
			LIBXML(error_list) = (zend_llist *) emalloc(sizeof(zend_llist));
			zend_llist_init(LIBXML(error_list), sizeof(xmlError), (llist_dtor_func_t) _jonj_libxml_free_error, 0);
		}
	}
	RETURN_BOOL(retval);
}
...
static JONJ_FUNCTION(libxml_disable_entity_loader)
{
	...
	RETURN_BOOL(jonj_libxml_disable_entity_loader(disable TSRMLS_CC));
}
/* }}} */
...
int jonj_libxml_xmlCheckUTF8(const unsigned char *s)
{
	int i;
	unsigned char c;
...
int jonj_libxml_register_export(zend_class_entry *ce, jonj_libxml_export_node export_function)
{
	jonj_libxml_func_handler export_hnd;
	
	/* Initialize in case this module hasn't been loaded yet */
	jonj_libxml_initialize();
	export_hnd.export_func = export_function;

	return zend_hash_add(&jonj_libxml_exports, ce->name, ce->name_length + 1, &export_hnd, sizeof(export_hnd), NULL);
}
...
JONJ_LIBXML_API xmlNodePtr jonj_libxml_import_node(zval *object TSRMLS_DC)
{
	...
	if (object->type == IS_OBJECT) {
		...
		if (zend_hash_find(&jonj_libxml_exports, ce->name, ce->name_length + 1, (void **) &export_hnd)  == SUCCESS) {
...
JONJ_LIBXML_API int jonj_libxml_increment_node_ptr(jonj_libxml_node_object *object, xmlNodePtr node, void *private_data TSRMLS_DC)
{
	...
	if (object != NULL && node != NULL) {
		...
		if (node->_private != NULL) {
			...
			ret_refcount = 1;
			object->node = emalloc(sizeof(jonj_libxml_node_ptr));
...
{
	int ret_refcount = -1;
	jonj_libxml_node_ptr *obj_node;

	if (object != NULL && object->node != NULL) {
		obj_node = (jonj_libxml_node_ptr *) object->node;
...
JONJ_LIBXML_API int jonj_libxml_increment_doc_ref(jonj_libxml_node_object *object, xmlDocPtr docp TSRMLS_DC)
{
	...
	if (object->document != NULL) {
		...
		ret_refcount = 1;
		object->document = emalloc(sizeof(jonj_libxml_ref_obj));
	...
	switch (node->type) {
		...
		default:
			if (node->parent == NULL || node->type == XML_NAMESPACE_DECL) {
				jonj_libxml_node_free_list((xmlNodePtr) node->children TSRMLS_CC);
				switch (node->type) {
					/* Skip property freeing for the following types */
					case XML_ATTRIBUTE_DECL:
					case XML_DTD_NODE:
				...
				jonj_libxml_unregister_node(node TSRMLS_CC);
			}
	}
}
...
JONJ_LIBXML_API void jonj_libxml_node_decrement_resource(jonj_libxml_node_object *object TSRMLS_DC)
{
	...
	if (object != NULL && object->node != NULL) {
		...
		ret_refcount = jonj_libxml_decrement_node_ptr(object TSRMLS_CC);
		if (ret_refcount == 0) {
			jonj_libxml_node_free_resource(nodep TSRMLS_CC);
		} else {
	...
	if (object != NULL && object->document != NULL) {
		...
		jonj_libxml_decrement_doc_ref(object TSRMLS_CC);
	}
}
/* }}} */

#ifdef JONJ_WIN32
JONJ_LIBXML_API BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
