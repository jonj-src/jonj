...
static int odbc_stmt_execute(pdo_stmt_t *stmt TSRMLS_DC)
{
	...
	while (rc == SQL_NEED_DATA) {
		...
		if (rc == SQL_NEED_DATA) {
			...
			do {
				len = jonj_stream_read(stm, buf, 8192);
				if (len == 0) {
					break;
				}
						...
						if (0 == jonj_stream_stat(stm, &sb)) {
							if (P->outbuf) {
								...
								do {
									...
									if (amount > 8192)
										amount = 8192;
									len = jonj_stream_read(stm, ptr, amount);
									if (len == 0) {
										break;
									}
									ptr += len;
...
static int odbc_stmt_get_col(pdo_stmt_t *stmt, int colno, char **ptr, unsigned long *len, int *caller_frees TSRMLS_DC)
{
	...
	if (C->is_long) {
		...
		if (rc == SQL_SUCCESS_WITH_INFO) {
			...
			/* NULL terminate the buffer once, when finished, for use with the rest of JONJ */
			buf[used] = '\0';
