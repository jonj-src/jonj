...
JONJ_ARG_WITH(pdo-odbc, for ODBC v3 support for PDO,
...
AC_DEFUN([PDO_ODBC_CHECK_HEADER],[
  ...
  if test -f "$PDO_ODBC_INCDIR/$1"; then
    jonj_pdo_have_header=yes
    JONJ_DEF_HAVE(translit($1,.,_))
    AC_MSG_RESULT(yes)
  else
    AC_MSG_RESULT(no)
  fi
...
if test "$JONJ_PDO_ODBC" != "no"; then
  ...
  pdo_odbc_flavour="`echo $JONJ_PDO_ODBC | cut -d, -f1`"
  pdo_odbc_dir="`echo $JONJ_PDO_ODBC | cut -d, -f2`"

  if test "$pdo_odbc_dir" = "$JONJ_PDO_ODBC" ; then
    pdo_odbc_dir=
  fi
  ...
  case $pdo_odbc_flavour in
    ...
    iODBC|iodbc)
        pdo_odbc_def_libdir=/usr/local/$JONJ_LIBDIR
    ...
    ODBCRouter|odbcrouter)
        pdo_odbc_def_libdir=/usr/$JONJ_LIBDIR
        pdo_odbc_def_incdir=/usr/include
        pdo_odbc_def_lib=odbcsdk
        ;;

    generic)
        pdo_odbc_def_lib="`echo $JONJ_PDO_ODBC | cut -d, -f3`"
        pdo_odbc_def_ldflags="`echo $JONJ_PDO_ODBC | cut -d, -f4`"
        pdo_odbc_def_cflags="`echo $JONJ_PDO_ODBC | cut -d, -f5`"
  ...
  if test -n "$pdo_odbc_dir"; then
    ...
    PDO_ODBC_LIBDIR="$pdo_odbc_dir/$JONJ_LIBDIR"
  else
  ...
  if test "$jonj_pdo_have_header" != "yes"; then
  ...
  JONJ_EVAL_LIBLINE([$PDO_ODBC_LDFLAGS], [PDO_ODBC_SHARED_LIBADD])

  dnl Check first for an ODBC 1.0 function to assert that the libraries work
  JONJ_CHECK_LIBRARY($pdo_odbc_def_lib, SQLBindCol,
  [
    dnl And now check for an ODBC 3.0 function to assert that they're
    dnl *good* libraries.
    JONJ_CHECK_LIBRARY($pdo_odbc_def_lib, SQLAllocHandle,
    [], [
      AC_MSG_ERROR([
...
functions required for PDO support.
  ...
  JONJ_NEW_EXTENSION(pdo_odbc, pdo_odbc.c odbc_driver.c odbc_stmt.c, $ext_shared,,-I$pdo_cv_inc_path $PDO_ODBC_INCLUDE)
  JONJ_SUBST(PDO_ODBC_SHARED_LIBADD)
  ifdef([JONJ_ADD_EXTENSION_DEP],
  [
    JONJ_ADD_EXTENSION_DEP(pdo_odbc, pdo)
  ])
fi
