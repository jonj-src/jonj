...
#include "jonj_pdo_odbc.h"
#include "jonj_pdo_odbc_int.h"
...
#if defined(DB2CLI_VER) && !defined(JONJ_WIN32)
JONJ_INI_BEGIN()
	JONJ_INI_ENTRY("pdo_odbc.db2_instance_name", NULL, JONJ_INI_SYSTEM, NULL)
JONJ_INI_END()
...
JONJ_MINIT_FUNCTION(pdo_odbc)
{
	...
	if (FAILURE == jonj_pdo_register_driver(&pdo_odbc_driver)) {
		return FAILURE;
	}
	...
	if (strcasecmp(pooling_val, "strict") == 0 || strcmp(pooling_val, "1") == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Error in pdo_odbc.connection_pooling configuration.  Value MUST be one of 'strict', 'relaxed' or 'off'");
		return FAILURE;
	}
...
#endif
	jonj_pdo_unregister_driver(&pdo_odbc_driver);
	return SUCCESS;
}
/* }}} */
...
JONJ_MINFO_FUNCTION(pdo_odbc)
{
	...
	jonj_info_print_table_header(2, "PDO Driver for ODBC (" PDO_ODBC_TYPE ")" , "enabled");
#ifdef SQL_ATTR_CONNECTION_POOLING
	jonj_info_print_table_row(2, "ODBC Connection Pooling",	pdo_odbc_pool_on == SQL_CP_OFF ?
			"Disabled" : (pdo_odbc_pool_mode == SQL_CP_STRICT_MATCH ? "Enabled, strict matching" : "Enabled, relaxed matching"));
#else
	jonj_info_print_table_row(2, "ODBC Connection Pooling", "Not supported in this build");
#endif
