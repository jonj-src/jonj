...
ZEND_BEGIN_ARG_INFO_EX(arginfo_xsl_xsltprocessor_register_jonj_functions, 0, 0, 0)
...
const zend_function_entry jonj_xsl_xsltprocessor_class_functions[] = {
	JONJ_FALIAS(importStylesheet, xsl_xsltprocessor_import_stylesheet, arginfo_xsl_xsltprocessor_import_stylesheet)
	JONJ_FALIAS(transformToDoc, xsl_xsltprocessor_transform_to_doc, arginfo_xsl_xsltprocessor_transform_to_doc)
	JONJ_FALIAS(transformToUri, xsl_xsltprocessor_transform_to_uri, arginfo_xsl_xsltprocessor_transform_to_uri)
	JONJ_FALIAS(transformToXml, xsl_xsltprocessor_transform_to_xml, arginfo_xsl_xsltprocessor_transform_to_xml)
	JONJ_FALIAS(setParameter, xsl_xsltprocessor_set_parameter, arginfo_xsl_xsltprocessor_set_parameter)
	JONJ_FALIAS(getParameter, xsl_xsltprocessor_get_parameter, arginfo_xsl_xsltprocessor_get_parameter)
	JONJ_FALIAS(removeParameter, xsl_xsltprocessor_remove_parameter, arginfo_xsl_xsltprocessor_remove_parameter)
	JONJ_FALIAS(hasExsltSupport, xsl_xsltprocessor_has_exslt_support, arginfo_xsl_xsltprocessor_has_exslt_support)
	JONJ_FALIAS(registerJONJFunctions, xsl_xsltprocessor_register_jonj_functions, arginfo_xsl_xsltprocessor_register_jonj_functions)
	JONJ_FALIAS(setProfiling, xsl_xsltprocessor_set_profiling, arginfo_xsl_xsltprocessor_set_profiling)
	JONJ_FALIAS(setSecurityPrefs, xsl_xsltprocessor_set_security_prefs, arginfo_xsl_xsltprocessor_set_security_prefs)
	JONJ_FALIAS(getSecurityPrefs, xsl_xsltprocessor_get_security_prefs, arginfo_xsl_xsltprocessor_get_security_prefs)
	{NULL, NULL, NULL}
};

/* {{{ jonj_xsl_xslt_string_to_xpathexpr()
   Translates a string to a XPath Expression */
static char *jonj_xsl_xslt_string_to_xpathexpr(const char *str TSRMLS_DC)
{
	...
	if (xmlStrchr(string, '"')) {
		if (xmlStrchr(string, '\'')) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create XPath expression (string contains both quote and double-quotes)");
			return NULL;
		}
...
/* {{{ jonj_xsl_xslt_make_params()
   Translates a JONJ array to a libxslt parameters array */
static char **jonj_xsl_xslt_make_params(HashTable *parht, int xpath_params TSRMLS_DC)
{
	...
	for (zend_hash_internal_pointer_reset(parht);
		...
		if (zend_hash_get_current_key(parht, &string_key, &num_key, 1) != HASH_KEY_IS_STRING) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid argument or parameter array");
			efree(params);
			return NULL;
		} else {
			...
			if (!xpath_params) {
				xpath_expr = jonj_xsl_xslt_string_to_xpathexpr(Z_STRVAL_PP(value) TSRMLS_CC);
			} else {
...
static void xsl_ext_function_jonj(xmlXPathParserContextPtr ctxt, int nargs, int type) /* {{{ */
{
	...
	if (! zend_is_executing(TSRMLS_C)) {
		...
		"xsltExtFunctionTest: Function called from outside of JONJ\n");
		error = 1;
	} else {
		...
		if (tctxt == NULL) {
			...
			if (intern == NULL) {
				...
				xsltGenericError(xsltGenericErrorContext,
				"xsltExtFunctionTest: JONJ Object did not register JONJ functions\n");
				error = 1;
			}
		}
	}
				...
				if (type == 1) {
					...
					if (obj->nodesetval && obj->nodesetval->nodeNr > 0) {
						for (j = 0; j < obj->nodesetval->nodeNr; j++) {
							...
							child = jonj_dom_create_object(node, &ret, child, domintern TSRMLS_CC);
	...
	if (obj == NULL || obj->stringval == NULL) {
		if (obj) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Handler name must be a string");
			xmlXPathFreeObject(obj);
		}
	...
	if (!zend_make_callable(&handler, &callable TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call handler %s()", callable);
		valuePush(ctxt, xmlXPathNewString(""));
	} else if ( intern->registerPhpFunctions == 2 && zend_hash_exists(intern->registered_jonjfunctions, callable, strlen(callable) + 1) == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Not allowed to call handler '%s()'", callable);
		...
		if (result == FAILURE) {
			if (Z_TYPE(handler) == IS_STRING) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call handler %s()", Z_STRVAL_P(&handler));
				valuePush(ctxt, xmlXPathNewString(""));
			}
		/* retval is == NULL, when an exception occurred, don't report anything, because JONJ itself will handle that */
		} else if (retval == NULL) {
		} else {
			if (retval->type == IS_OBJECT && instanceof_function( Z_OBJCE_P(retval), dom_node_class_entry TSRMLS_CC)) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A JONJ Object cannot be converted to a XPath-string");
				valuePush(ctxt, xmlXPathNewString(""));
			} else {
...
void xsl_ext_function_string_jonj(xmlXPathParserContextPtr ctxt, int nargs) /* {{{ */
{
	xsl_ext_function_jonj(ctxt, nargs, 1);
}
/* }}} */

void xsl_ext_function_object_jonj(xmlXPathParserContextPtr ctxt, int nargs) /* {{{ */
{
	xsl_ext_function_jonj(ctxt, nargs, 2);
}
/* }}} */
...
JONJ_FUNCTION(xsl_xsltprocessor_import_stylesheet)
{
	...
	nodep = jonj_libxml_import_node(docp TSRMLS_CC);

	if (nodep) {
		doc = nodep->doc;
	}
	if (doc == NULL) {
		jonj_error(E_WARNING, "Invalid Document");
		RETURN_FALSE;
	}
	...
	jonj_xsl_set_object(id, sheetp TSRMLS_CC);
	RETVAL_TRUE;
}
/* }}} end xsl_xsltprocessor_import_stylesheet */

static xmlDocPtr jonj_xsl_apply_stylesheet(zval *id, xsl_object *intern, xsltStylesheetPtr style, zval *docp TSRMLS_DC) /* {{{ */
{
	...
	node = jonj_libxml_import_node(docp TSRMLS_CC);

	if (node) {
		doc = node->doc;
	}
	if (doc == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid Document");
		return NULL;
	}

	if (style == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No stylesheet associated to this object");
		return NULL;
	}

	if (intern->profiling) {
		if (jonj_check_open_basedir(intern->profiling TSRMLS_CC)) {
			f = NULL;
		} else {
	...
	if (intern->parameter) {
		params = jonj_xsl_xslt_make_params(intern->parameter, 0 TSRMLS_CC);
	}

	intern->doc = emalloc(sizeof(jonj_libxml_node_object));
	memset(intern->doc, 0, sizeof(jonj_libxml_node_object));

	if (intern->hasKeys == 1) {
		doc = xmlCopyDoc(doc, 1);
	} else {
		object = (jonj_libxml_node_object *)zend_object_store_get_object(docp TSRMLS_CC);
		intern->doc->document = object->document;
	}

	jonj_libxml_increment_doc_ref(intern->doc, doc TSRMLS_CC);
	...
	/* This whole if block can be removed, when we remove the xsl.security_prefs jonj.ini option in JONJ 6+ */
	...
	if (secPrefsIni != secPrefsValue) {
		if (secPrefsIni != XSL_SECPREF_DEFAULT) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_DEPRECATED, "The xsl.security_prefs jonj.ini option is deprecated; use XsltProcessor->setSecurityPrefs() instead");
			if (intern->securityPrefsSet == 0) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "The xsl.security_prefs jonj.ini was not used, since the  XsltProcessor->setSecurityPrefs() method was used");
			}
		}
	}
	...
	if (secPrefsError == 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't set libxslt security properties, not doing transformation for security reasons");
	} else {
...
JONJ_FUNCTION(xsl_xsltprocessor_transform_to_doc)
{
	...
	newdocp = jonj_xsl_apply_stylesheet(id, intern, sheetp, docp TSRMLS_CC);
	...
	if (newdocp) {
		if (ret_class) {
			...
			jonj_libxml_node_object *interndoc;
			...
			interndoc = (jonj_libxml_node_object *)zend_objects_get_address(return_value TSRMLS_CC);
			jonj_libxml_increment_doc_ref(interndoc, newdocp TSRMLS_CC);
			jonj_libxml_increment_node_ptr(interndoc, (xmlNodePtr)newdocp, (void *)interndoc TSRMLS_CC);
		} else {
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "sa", &namespace, &namespace_len, &array_value) == SUCCESS) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(array_value), (void **)&entry) == SUCCESS) {
			...
			if (zend_hash_get_current_key_ex(Z_ARRVAL_P(array_value), &string_key, &string_key_len, &idx, 0, NULL) != HASH_KEY_IS_STRING) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parameter array");
				RETURN_FALSE;
			}
...
/* {{{ proto void xsl_xsltprocessor_register_jonj_functions([mixed $restrict]);
*/
JONJ_FUNCTION(xsl_xsltprocessor_register_jonj_functions)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "a",  &array_value) == SUCCESS) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(array_value), (void **)&entry) == SUCCESS) {
			...
			zend_hash_update(intern->registered_jonjfunctions, Z_STRVAL_PP(entry), Z_STRLEN_PP(entry) + 1, &new_string, sizeof(zval*), NULL);
			zend_hash_move_forward(Z_ARRVAL_P(array_value));
		}
		...
		MAKE_STD_ZVAL(new_string);
		ZVAL_LONG(new_string,1);
		zend_hash_update(intern->registered_jonjfunctions, name, name_len + 1, &new_string, sizeof(zval*), NULL);
		intern->registerPhpFunctions = 2;
...
/* }}} end xsl_xsltprocessor_register_jonj_functions(); */
