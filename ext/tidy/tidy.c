...
#define JONJ_TIDY_MODULE_VERSION	"2.0"
...
#define TIDY_FETCH_OBJECT	\
	...
	obj = (JONJTidyObj *) zend_object_store_get_object(object TSRMLS_CC);	\
...
#define TIDY_APPLY_CONFIG_ZVAL(_doc, _val) \
    if(_val) { \
        if(Z_TYPE_PP(_val) == IS_ARRAY) { \
            _jonj_tidy_apply_config_array(_doc, HASH_OF(*_val) TSRMLS_CC); \
        } else { \
            convert_to_string_ex(_val); \
            TIDY_OPEN_BASE_DIR_CHECK(Z_STRVAL_PP(_val)); \
            switch (tidyLoadConfig(_doc, Z_STRVAL_PP(_val))) { \
              case -1: \
                jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not load configuration file '%s'", Z_STRVAL_PP(_val)); \
                break; \
              case 1: \
                jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "There were errors while parsing the configuration file '%s'", Z_STRVAL_PP(_val)); \
                break; \
            } \
        } \
    }
...
#define TIDY_SET_DEFAULT_CONFIG(_doc) \
	if (TG(default_config) && TG(default_config)[0]) { \
		if (tidyLoadConfig(_doc, TG(default_config)) < 0) { \
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to load Tidy configuration file at '%s'.", TG(default_config)); \
		} \
	}
/* }}} */
...
/* {{{ ext/tidy structs 
*/
typedef struct _JONJTidyDoc JONJTidyDoc;
typedef struct _JONJTidyObj JONJTidyObj;
...
/* {{{ ext/tidy prototypes
*/
static char *jonj_tidy_file_to_mem(char *, zend_bool, int * TSRMLS_DC);
...
static void tidy_doc_update_properties(JONJTidyObj * TSRMLS_DC);
static void tidy_add_default_properties(JONJTidyObj *, tidy_obj_type TSRMLS_DC);
static void *jonj_tidy_get_opt_val(JONJTidyDoc *, TidyOption, TidyOptionType * TSRMLS_DC);
static void jonj_tidy_create_node(INTERNAL_FUNCTION_PARAMETERS, tidy_base_nodetypes);
static int _jonj_tidy_set_tidy_opt(TidyDoc, char *, zval * TSRMLS_DC);
static int _jonj_tidy_apply_config_array(TidyDoc doc, HashTable *ht_options TSRMLS_DC);
static void _jonj_tidy_register_nodetypes(INIT_FUNC_ARGS);
static void _jonj_tidy_register_tags(INIT_FUNC_ARGS);
static JONJ_INI_MH(jonj_tidy_set_clean_output);
static void jonj_tidy_clean_output_start(const char *name, size_t name_len TSRMLS_DC);
static jonj_output_handler *jonj_tidy_output_handler_init(const char *handler_name, size_t handler_name_len, size_t chunk_size, int flags TSRMLS_DC);
static int jonj_tidy_output_handler(void **nothing, jonj_output_context *output_context);

static JONJ_MINIT_FUNCTION(tidy);
static JONJ_MSHUTDOWN_FUNCTION(tidy);
static JONJ_RINIT_FUNCTION(tidy);
static JONJ_MINFO_FUNCTION(tidy);

static JONJ_FUNCTION(tidy_getopt);
static JONJ_FUNCTION(tidy_parse_string);
static JONJ_FUNCTION(tidy_parse_file);
static JONJ_FUNCTION(tidy_clean_repair);
static JONJ_FUNCTION(tidy_repair_string);
static JONJ_FUNCTION(tidy_repair_file);
static JONJ_FUNCTION(tidy_diagnose);
static JONJ_FUNCTION(tidy_get_output);
static JONJ_FUNCTION(tidy_get_error_buffer);
static JONJ_FUNCTION(tidy_get_release);
static JONJ_FUNCTION(tidy_get_config);
static JONJ_FUNCTION(tidy_get_status);
static JONJ_FUNCTION(tidy_get_html_ver);
#if HAVE_TIDYOPTGETDOC
static JONJ_FUNCTION(tidy_get_opt_doc);
#endif
static JONJ_FUNCTION(tidy_is_xhtml);
static JONJ_FUNCTION(tidy_is_xml);
static JONJ_FUNCTION(tidy_error_count);
static JONJ_FUNCTION(tidy_warning_count);
static JONJ_FUNCTION(tidy_access_count);
static JONJ_FUNCTION(tidy_config_count);

static JONJ_FUNCTION(tidy_get_root);
static JONJ_FUNCTION(tidy_get_html);
static JONJ_FUNCTION(tidy_get_head);
static JONJ_FUNCTION(tidy_get_body);
...
JONJ_INI_BEGIN()
STD_JONJ_INI_ENTRY("tidy.default_config",	"",		JONJ_INI_SYSTEM,		OnUpdateString,				default_config,		zend_tidy_globals,	tidy_globals)
STD_JONJ_INI_ENTRY("tidy.clean_output",		"0",	JONJ_INI_USER,		jonj_tidy_set_clean_output,	clean_output,		zend_tidy_globals,	tidy_globals)
JONJ_INI_END()
...
static const zend_function_entry tidy_functions[] = {
	JONJ_FE(tidy_getopt,             arginfo_tidy_getopt)
	JONJ_FE(tidy_parse_string,       arginfo_tidy_parse_string)
	JONJ_FE(tidy_parse_file,         arginfo_tidy_parse_file)
	JONJ_FE(tidy_get_output,         arginfo_tidy_get_output)
	JONJ_FE(tidy_get_error_buffer,   arginfo_tidy_get_error_buffer) 
	JONJ_FE(tidy_clean_repair,       arginfo_tidy_clean_repair)
	JONJ_FE(tidy_repair_string,	arginfo_tidy_repair_string)
	JONJ_FE(tidy_repair_file,	arginfo_tidy_repair_file)
	JONJ_FE(tidy_diagnose,           arginfo_tidy_diagnose)
	JONJ_FE(tidy_get_release,	arginfo_tidy_get_release)
	JONJ_FE(tidy_get_config,		arginfo_tidy_get_config)
	JONJ_FE(tidy_get_status,		arginfo_tidy_get_status)
	JONJ_FE(tidy_get_html_ver,	arginfo_tidy_get_html_ver)
	JONJ_FE(tidy_is_xhtml,		arginfo_tidy_is_xhtml)
	JONJ_FE(tidy_is_xml,		arginfo_tidy_is_xml)
	JONJ_FE(tidy_error_count,	arginfo_tidy_error_count)
	JONJ_FE(tidy_warning_count,	arginfo_tidy_warning_count)
	JONJ_FE(tidy_access_count,	arginfo_tidy_access_count)
	JONJ_FE(tidy_config_count,	arginfo_tidy_config_count) 
#if HAVE_TIDYOPTGETDOC
	JONJ_FE(tidy_get_opt_doc,	arginfo_tidy_get_opt_doc)
#endif
	JONJ_FE(tidy_get_root,		arginfo_tidy_get_root)
	JONJ_FE(tidy_get_head,		arginfo_tidy_get_head)
	JONJ_FE(tidy_get_html,		arginfo_tidy_get_html)
	JONJ_FE(tidy_get_body,		arginfo_tidy_get_body)
	JONJ_FE_END
};
...
zend_module_entry tidy_module_entry = {
	...
	NULL,
	JONJ_MINFO(tidy),
	JONJ_TIDY_MODULE_VERSION,
	JONJ_MODULE_GLOBALS(tidy),
	NULL,
	NULL,
	NULL,
...
static void* TIDY_CALL jonj_tidy_malloc(size_t len)
{
	return emalloc(len);
}

static void* TIDY_CALL jonj_tidy_realloc(void *buf, size_t len)
{
	return erealloc(buf, len);
}

static void TIDY_CALL jonj_tidy_free(void *buf)
{
	efree(buf);
}

static void TIDY_CALL jonj_tidy_panic(ctmbstr msg)
{
	TSRMLS_FETCH();
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Could not allocate memory for tidy! (Reason: %s)", (char *)msg);
}

static int _jonj_tidy_set_tidy_opt(TidyDoc doc, char *optname, zval *value TSRMLS_DC)
{
	TidyOption opt = tidyGetOptionByName(doc, optname);
	zval conv = *value;

	if (!opt) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Unknown Tidy Configuration Option '%s'", optname);
		return FAILURE;
	}
	
	if (tidyOptIsReadOnly(opt)) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Attempting to set read-only option '%s'", optname);
		return FAILURE;
	}
	...
	switch(tidyOptGetType(opt)) {
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to determine type of configuration option");
			break;
	}	
...
static void jonj_tidy_quick_repair(INTERNAL_FUNCTION_PARAMETERS, zend_bool is_file)
{
	...
	if (is_file) {
		...
		if (!(data = jonj_tidy_file_to_mem(arg1, use_include_path, &data_len TSRMLS_CC))) {
			RETURN_FALSE;
		}
	} else {
	...
	if (tidySetErrorBuffer(doc, errbuf) != 0) {
		...
		efree(errbuf);
		tidyRelease(doc);
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Could not set Tidy error buffer");
	}
	...
	if(enc_len) {
		if (tidySetCharEncoding(doc, enc) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not set encoding '%s'", enc);
			RETVAL_FALSE;
		}
	}
	...
	if (data) {
		...
		if (tidyParseBuffer(doc, &buf) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", errbuf->bp);
			RETVAL_FALSE;
		} else {
...
static char *jonj_tidy_file_to_mem(char *filename, zend_bool use_include_path, int *len TSRMLS_DC)
{
	jonj_stream *stream;
	char *data = NULL;

	if (!(stream = jonj_stream_open_wrapper(filename, "rb", (use_include_path ? USE_PATH : 0), NULL))) {
		return NULL;
	}
	if ((*len = (int) jonj_stream_copy_to_mem(stream, (void*) &data, JONJ_STREAM_COPY_ALL, 0)) == 0) {
		data = estrdup("");
		*len = 0;
	}
...
{
	JONJTidyObj *intern = (JONJTidyObj *)object;
							...
							zend_object_value *retval, tidy_obj_type objtype TSRMLS_DC)
{
	...
	intern = emalloc(sizeof(JONJTidyObj));
	memset(intern, 0, sizeof(JONJTidyObj));
	...
	switch(objtype) {
		...
		case is_doc:
			intern->ptdoc = emalloc(sizeof(JONJTidyDoc));
...
static int tidy_doc_cast_handler(zval *in, zval *out, int type TSRMLS_DC)
{
	...
	switch(type) {
		...
		case IS_STRING:
			obj = (JONJTidyObj *)zend_object_store_get_object(in TSRMLS_CC);
			tidyBufInit(&output);
...
static void tidy_doc_update_properties(JONJTidyObj *obj TSRMLS_DC)
{
...
static void tidy_add_default_properties(JONJTidyObj *obj, tidy_obj_type type TSRMLS_DC)
{
		...
		case is_node:
			...
			if (tempnode) {
				...
				do {
					...
					newobj = (JONJTidyObj *) zend_object_store_get_object(temp TSRMLS_CC);
...
static void *jonj_tidy_get_opt_val(JONJTidyDoc *ptdoc, TidyOption opt, TidyOptionType *type TSRMLS_DC)
{
...
static void jonj_tidy_create_node(INTERNAL_FUNCTION_PARAMETERS, tidy_base_nodetypes node_type)
{
	JONJTidyObj *newobj;
	TidyNode node;
	TIDY_FETCH_OBJECT;
	...
	newobj = (JONJTidyObj *) zend_object_store_get_object(return_value TSRMLS_CC);
	newobj->type  = is_node;
	newobj->ptdoc = obj->ptdoc;
	newobj->node  = node;
	newobj->ptdoc->ref_count++;
...
static int _jonj_tidy_apply_config_array(TidyDoc doc, HashTable *ht_options TSRMLS_DC)
{
	...
	for (zend_hash_internal_pointer_reset(ht_options);
		...
		switch (zend_hash_get_current_key_ex(ht_options, &opt_name, &opt_name_len, &opt_indx, FALSE, NULL)) {
			...
			default:
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Could not retrieve key from option array");
			return FAILURE;
		}

		_jonj_tidy_set_tidy_opt(doc, opt_name, *opt_val TSRMLS_CC);
		if (clear_str) {
			efree(opt_name);
		}
	}
...
static int jonj_tidy_parse_string(JONJTidyObj *obj, char *string, int len, char *enc TSRMLS_DC)
{
	TidyBuffer buf;
	...
	if (tidyParseBuffer(obj->ptdoc->doc, &buf) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", obj->ptdoc->errbuf->bp);
		return FAILURE;
	}
...
static JONJ_MINIT_FUNCTION(tidy)
{
	tidySetMallocCall(jonj_tidy_malloc);
	tidySetReallocCall(jonj_tidy_realloc);
	tidySetFreeCall(jonj_tidy_free);
	tidySetPanicCall(jonj_tidy_panic);
	...
	_jonj_tidy_register_tags(INIT_FUNC_ARGS_PASSTHRU);
	_jonj_tidy_register_nodetypes(INIT_FUNC_ARGS_PASSTHRU);

	jonj_output_handler_alias_register(ZEND_STRL("ob_tidyhandler"), jonj_tidy_output_handler_init TSRMLS_CC);
...
static JONJ_RINIT_FUNCTION(tidy)
{
	jonj_tidy_clean_output_start(ZEND_STRL("ob_tidyhandler") TSRMLS_CC);
...
static JONJ_MINFO_FUNCTION(tidy)
{
	...
	jonj_info_print_table_header(2, "Tidy support", "enabled");
	jonj_info_print_table_row(2, "libTidy Release", (char *)tidyReleaseDate());
	jonj_info_print_table_row(2, "Extension Version", JONJ_TIDY_MODULE_VERSION " ($Id$)");
...
static JONJ_INI_MH(jonj_tidy_set_clean_output)
{
	...
	if (stage == JONJ_INI_STAGE_RUNTIME) {
		...
		if (value && (status & JONJ_OUTPUT_WRITTEN)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot enable tidy.clean_output - there has already been output");
			return FAILURE;
		}
		if (status & JONJ_OUTPUT_SENT) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot change tidy.clean_output - headers already sent");
			return FAILURE;
		}
	}
	...
	if (stage == JONJ_INI_STAGE_RUNTIME && value) {
		if (!jonj_output_handler_started(ZEND_STRL("ob_tidyhandler") TSRMLS_CC)) {
			jonj_tidy_clean_output_start(ZEND_STRL("ob_tidyhandler") TSRMLS_CC);
		}
	}
...
static void jonj_tidy_clean_output_start(const char *name, size_t name_len TSRMLS_DC)
{
	...
	if (TG(clean_output) && (h = jonj_tidy_output_handler_init(name, name_len, 0, JONJ_OUTPUT_HANDLER_STDFLAGS TSRMLS_CC))) {
...
{
	if (chunk_size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot use a chunk size for ob_tidyhandler");
		return NULL;
	}
	if (!TG(clean_output)) {
		TG(clean_output) = 1;
	}
	return jonj_output_handler_create_internal(handler_name, handler_name_len, jonj_tidy_output_handler, chunk_size, flags TSRMLS_CC);
}
...
static int jonj_tidy_output_handler(void **nothing, jonj_output_context *output_context)
{
	...
	if (TG(clean_output) && (output_context->op & JONJ_OUTPUT_HANDLER_START) && (output_context->op & JONJ_OUTPUT_HANDLER_FINAL)) {
		doc = tidyCreate();
		tidyBufInit(&errbuf);
...
static JONJ_FUNCTION(tidy_parse_string)
{
	...
	obj = (JONJTidyObj *) zend_object_store_get_object(return_value TSRMLS_CC);
		
	TIDY_APPLY_CONFIG_ZVAL(obj->ptdoc->doc, options);
	
	if(jonj_tidy_parse_string(obj, input, input_len, enc TSRMLS_CC) == FAILURE) {
...
static JONJ_FUNCTION(tidy_parse_file)
{
	...
	if (!(contents = jonj_tidy_file_to_mem(inputfile, use_include_path, &contents_len TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot Load '%s' into memory%s", inputfile, (use_include_path) ? " (Using include path)" : "");
		RETURN_FALSE;
	}

	TIDY_APPLY_CONFIG_ZVAL(obj->ptdoc->doc, options);

	if(jonj_tidy_parse_string(obj, contents, contents_len, enc TSRMLS_CC) == FAILURE) {
		zval_dtor(return_value);
		INIT_ZVAL(*return_value);
		RETVAL_FALSE;
	}
...
static JONJ_FUNCTION(tidy_repair_string)
{
	jonj_tidy_quick_repair(INTERNAL_FUNCTION_PARAM_PASSTHRU, FALSE);
}
/* }}} */
...
static JONJ_FUNCTION(tidy_repair_file)
{
	jonj_tidy_quick_repair(INTERNAL_FUNCTION_PARAM_PASSTHRU, TRUE);
}
/* }}} */
...
static JONJ_FUNCTION(tidy_get_opt_doc)
{
	...
	if (!opt) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown Tidy Configuration Option '%s'", optname);
		RETURN_FALSE;
	}
...
static JONJ_FUNCTION(tidy_get_config)
{
	...
	while (itOpt) {
		...
		opt_value = jonj_tidy_get_opt_val(obj->ptdoc, opt, &optt TSRMLS_CC);
		switch (optt) {
			case TidyString:
...
{  
	...
	optval = jonj_tidy_get_opt_val(obj->ptdoc, opt, &optt TSRMLS_CC);
	switch (optt) {
		case TidyString:
...
static TIDY_DOC_METHOD(__construct)
{
	...
	if (inputfile) {
		...
		jonj_tidy_parse_string(obj, contents, contents_len, enc TSRMLS_CC);
...
static TIDY_DOC_METHOD(parseString)
{
	...
	if(jonj_tidy_parse_string(obj, input, input_len, enc TSRMLS_CC) == SUCCESS) {
		RETURN_TRUE;
	}
...
static JONJ_FUNCTION(tidy_get_root)
{
	jonj_tidy_create_node(INTERNAL_FUNCTION_PARAM_PASSTHRU, is_root_node);
}
/* }}} */
...
static JONJ_FUNCTION(tidy_get_html)
{
	jonj_tidy_create_node(INTERNAL_FUNCTION_PARAM_PASSTHRU, is_html_node);
}
/* }}} */
...
static JONJ_FUNCTION(tidy_get_head)
{
	jonj_tidy_create_node(INTERNAL_FUNCTION_PARAM_PASSTHRU, is_head_node);
}
/* }}} */
...
static JONJ_FUNCTION(tidy_get_body)
{
	jonj_tidy_create_node(INTERNAL_FUNCTION_PARAM_PASSTHRU, is_body_node);
}
/* }}} */
...
/* {{{ proto boolean tidyNode::isPhp()
   Returns true if this node is JONJ */
static TIDY_NODE_METHOD(isPhp)
{
	TIDY_FETCH_ONLY_OBJECT;
...
static TIDY_NODE_METHOD(__construct)
{
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "You should not create a tidyNode manually");
}   
/* }}} */
