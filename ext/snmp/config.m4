...
JONJ_ARG_WITH(snmp,for SNMP support,
[  --with-snmp[=DIR]         Include SNMP support])

JONJ_ARG_WITH(openssl-dir,OpenSSL dir for SNMP,
[  --with-openssl-dir[=DIR]  SNMP: openssl install prefix], no, no)

if test "$JONJ_SNMP" != "no"; then

  if test "$JONJ_SNMP" = "yes"; then
    AC_PATH_PROG(SNMP_CONFIG,net-snmp-config,,[/usr/local/bin:$PATH])
  else
    SNMP_CONFIG="$JONJ_SNMP/bin/net-snmp-config"
  fi
  ...
  if test -x "$SNMP_CONFIG"; then
    ...
    if test "$SNMP_VERSION" -ge "5003"; then
      if test -n "$SNMP_LIBS" && test -n "$SNMP_PREFIX"; then
        JONJ_ADD_INCLUDE(${SNMP_PREFIX}/include)
        JONJ_EVAL_LIBLINE($SNMP_LIBS, SNMP_SHARED_LIBADD)
  ...
  dnl Test build.
  JONJ_CHECK_LIBRARY($SNMP_LIBNAME, init_snmp,
  [
  ...
  JONJ_CHECK_LIBRARY($SNMP_LIBNAME, shutdown_snmp_logging,
  [
  ...
  JONJ_NEW_EXTENSION(snmp, snmp.c, $ext_shared)
  JONJ_SUBST(SNMP_SHARED_LIBADD)
fi
