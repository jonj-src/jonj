...
typedef struct snmp_session jonj_snmp_session;
#define JONJ_SNMP_SESSION_RES_NAME "SNMP session"

#define JONJ_SNMP_ADD_PROPERTIES(a, b) \
{ \
	int i = 0; \
	while (b[i].name != NULL) { \
		jonj_snmp_add_property((a), (b)[i].name, (b)[i].name_length, \
							(jonj_snmp_read_t)(b)[i].read_func, (jonj_snmp_write_t)(b)[i].write_func TSRMLS_CC); \
		i++; \
	} \
}

#define JONJ_SNMP_ERRNO_NOERROR			0
#define JONJ_SNMP_ERRNO_GENERIC			(1 << 1)
#define JONJ_SNMP_ERRNO_TIMEOUT			(1 << 2)
#define JONJ_SNMP_ERRNO_ERROR_IN_REPLY		(1 << 3)
#define JONJ_SNMP_ERRNO_OID_NOT_INCREASING	(1 << 4)
#define JONJ_SNMP_ERRNO_OID_PARSING_ERROR	(1 << 5)
#define JONJ_SNMP_ERRNO_MULTIPLE_SET_QUERIES	(1 << 6)
#define JONJ_SNMP_ERRNO_ANY	( \
		JONJ_SNMP_ERRNO_GENERIC | \
		JONJ_SNMP_ERRNO_TIMEOUT | \
		JONJ_SNMP_ERRNO_ERROR_IN_REPLY | \
		JONJ_SNMP_ERRNO_OID_NOT_INCREASING | \
		JONJ_SNMP_ERRNO_OID_PARSING_ERROR | \
		JONJ_SNMP_ERRNO_MULTIPLE_SET_QUERIES | \
		JONJ_SNMP_ERRNO_NOERROR \
	)

ZEND_DECLARE_MODULE_GLOBALS(snmp)
static JONJ_GINIT_FUNCTION(snmp);
...
/* Handlers */
static zend_object_handlers jonj_snmp_object_handlers;

/* Class entries */
zend_class_entry *jonj_snmp_ce;
zend_class_entry *jonj_snmp_exception_ce;

/* Class object properties */
static HashTable jonj_snmp_properties;
...
const zend_function_entry snmp_functions[] = {
	JONJ_FE(snmpget,					arginfo_snmpget)
	JONJ_FE(snmpgetnext, 				arginfo_snmpgetnext)
	JONJ_FE(snmpwalk, 				arginfo_snmpwalk)
	JONJ_FE(snmprealwalk, 				arginfo_snmprealwalk)
	JONJ_FALIAS(snmpwalkoid, snmprealwalk, 		arginfo_snmprealwalk)
	JONJ_FE(snmpset, 				arginfo_snmpset)
	JONJ_FE(snmp_get_quick_print, 			arginfo_snmp_get_quick_print)
	JONJ_FE(snmp_set_quick_print, 			arginfo_snmp_set_quick_print)
	JONJ_FE(snmp_set_enum_print, 			arginfo_snmp_set_enum_print)
	JONJ_FE(snmp_set_oid_output_format, 		arginfo_snmp_set_oid_output_format)
	JONJ_FALIAS(snmp_set_oid_numeric_print, snmp_set_oid_output_format, arginfo_snmp_set_oid_output_format)

	JONJ_FE(snmp2_get, 				arginfo_snmp2_get)
	JONJ_FE(snmp2_getnext, 				arginfo_snmp2_getnext)
	JONJ_FE(snmp2_walk, 				arginfo_snmp2_walk)
	JONJ_FE(snmp2_real_walk, 			arginfo_snmp2_real_walk)
	JONJ_FE(snmp2_set, 				arginfo_snmp2_set)

	JONJ_FE(snmp3_get, 				arginfo_snmp3_get)
	JONJ_FE(snmp3_getnext, 				arginfo_snmp3_getnext)
	JONJ_FE(snmp3_walk, 				arginfo_snmp3_walk)
	JONJ_FE(snmp3_real_walk, 			arginfo_snmp3_real_walk)
	JONJ_FE(snmp3_set, 				arginfo_snmp3_set)
	JONJ_FE(snmp_set_valueretrieval,			arginfo_snmp_set_valueretrieval)
	JONJ_FE(snmp_get_valueretrieval,			arginfo_snmp_get_valueretrieval)

	JONJ_FE(snmp_read_mib, 				arginfo_snmp_read_mib)
	JONJ_FE_END
};
/* }}} */
...
/* THREAD_LS snmp_module jonj_snmp_module; - may need one of these at some point */
...
#define JONJ_SNMP_SESSION_FREE(a) { \
	if ((*session)->a) { \
		efree((*session)->a); \
		(*session)->a = NULL; \
	} \
...
static void netsnmp_session_free(jonj_snmp_session **session)
{
	if (*session) {
		JONJ_SNMP_SESSION_FREE(peername);
		JONJ_SNMP_SESSION_FREE(community);
		JONJ_SNMP_SESSION_FREE(securityName);
		JONJ_SNMP_SESSION_FREE(contextEngineID);
		efree(*session);
		*session = NULL;
	}
}

static void jonj_snmp_session_destructor(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	jonj_snmp_session *session = (jonj_snmp_session *)rsrc->ptr;
	netsnmp_session_free(&session);
}

static void jonj_snmp_object_free_storage(void *object TSRMLS_DC)
{
	jonj_snmp_object *intern = (jonj_snmp_object *)object;
...
static zend_object_value jonj_snmp_object_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	zend_object_value retval;
	jonj_snmp_object *intern;

	/* Allocate memory for it */
	intern = emalloc(sizeof(jonj_snmp_object));
	memset(&intern->zo, 0, sizeof(jonj_snmp_object));

	zend_object_std_init(&intern->zo, class_type TSRMLS_CC);
	object_properties_init(&intern->zo, class_type);

	retval.handle = zend_objects_store_put(intern, (zend_objects_store_dtor_t)zend_objects_destroy_object, (zend_objects_free_object_storage_t) jonj_snmp_object_free_storage, NULL TSRMLS_CC);
	retval.handlers = (zend_object_handlers *) &jonj_snmp_object_handlers;
 ...
 *
 */
static void jonj_snmp_error(zval *object, const char *docref TSRMLS_DC, int type, const char *format, ...)
{
	va_list args;
	jonj_snmp_object *snmp_object = NULL;

	if (object) {
		snmp_object = (jonj_snmp_object *)zend_object_store_get_object(object TSRMLS_CC);
		if (type == JONJ_SNMP_ERRNO_NOERROR) {
	...
	if (object && (snmp_object->exceptions_enabled & type)) {
		zend_throw_exception_ex(jonj_snmp_exception_ce, type TSRMLS_CC, "%s", snmp_object->snmp_errstr);
	} else {
		va_start(args, format);
		jonj_verror(docref, "", E_WARNING, format, args TSRMLS_CC);
		va_end(args);
	}
}
...
*
*/
static void jonj_snmp_getvalue(struct variable_list *vars, zval *snmpval TSRMLS_DC, int valueretrieval)
{
	zval *val;
	char sbuf[512];
	char *buf = &(sbuf[0]);
	...
	while ((valueretrieval & SNMP_VALUE_PLAIN) == 0) {
		*buf = '\0';
		if (snprint_value(buf, buflen, vars->name, vars->name_length, vars) == -1) {
			if (val_len > 512*1024) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "snprint_value() asks for a buffer more than 512k, Net-SNMP bug?");
				break;
			}
		...
		if (!dbuf) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "emalloc() failed: %s, fallback to static buffer", strerror(errno));
			buf = &(sbuf[0]);
	...
	if (valueretrieval & SNMP_VALUE_PLAIN) {
		*buf = 0;
		...
		default:
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown value type: %u", vars->type);
			break;
		}
...
*
*/
static void jonj_snmp_internal(INTERNAL_FUNCTION_PARAMETERS, int st,
							...
							struct objid_query *objid_query)
{
	...
	jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_NOERROR, "");
	...
	if ((ss = snmp_open(session)) == NULL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not open snmp connection: %s", err);
		free(err);
		RETVAL_FALSE;
		return;
	}
	...
	if ((st & SNMP_CMD_SET) && objid_query->count > objid_query->step) {
		jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_MULTIPLE_SET_QUERIES, "Can not fit all OIDs for SET query into one packet, using multiple queries");
	}
	...
	while (keepwalking) {
		...
		if (st & SNMP_CMD_WALK) {
			...
			if (st & SNMP_CMD_GET) {
				...
				snmp_close(ss);
				jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Unknown SNMP command (internals)");
				RETVAL_FALSE;
				return;
			}
			for (count = 0; objid_query->offset < objid_query->count && count < objid_query->step; objid_query->offset++, count++){
				if (st & SNMP_CMD_SET) {
					if ((snmp_errno = snmp_add_var(pdu, objid_query->vars[objid_query->offset].name, objid_query->vars[objid_query->offset].name_length, objid_query->vars[objid_query->offset].type, objid_query->vars[objid_query->offset].value))) {
						...
						jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_OID_PARSING_ERROR, "Could not add variable: OID='%s' type='%c' value='%s': %s", buf, objid_query->vars[objid_query->offset].type, objid_query->vars[objid_query->offset].value, snmp_api_errstring(snmp_errno));
						snmp_free_pdu(pdu);
						snmp_close(ss);
						RETVAL_FALSE;
						return;
...
retry:
		...
		if (status == STAT_SUCCESS) {
			if (response->errstat == SNMP_ERR_NOERROR) {
				...
				for (vars = response->variables; vars; vars = vars->next_variable) {
					...
					if ( 	vars->type == SNMP_ENDOFMIBVIEW ||
						...
						jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at '%s': %s", buf, buf2);
						continue;
					}
					...
					jonj_snmp_getvalue(vars, snmpval TSRMLS_CC, objid_query->valueretrieval);
					...
					if (objid_query->array_output) {
						...
						if (st & SNMP_NUMERIC_KEYS) {
							...
							if (found) {
								...
								jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not find original OID name for '%s'", buf2);
							}
					...
					if (st & SNMP_CMD_WALK) {
						if (objid_query->oid_increasing_check == TRUE && snmp_oid_compare(objid_query->vars[0].name, objid_query->vars[0].name_length, vars->name, vars->name_length) >= 0) {
							snprint_objid(buf2, sizeof(buf2), vars->name, vars->name_length);
							jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_OID_NOT_INCREASING, "Error: OID not increasing: %s", buf2);
							keepwalking = 0;
						} else {
				...
				if (!(st & SNMP_CMD_WALK) || response->errstat != SNMP_ERR_NOSUCHNAME || Z_TYPE_P(return_value) == IS_BOOL) {
					...
					if (vars) {
						snprint_objid(buf, sizeof(buf), vars->name, vars->name_length);
						jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at '%s': %s", buf, snmp_errstring(response->errstat));
					} else {
						jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at %u object_id: %s", response->errindex, snmp_errstring(response->errstat));
					}
			...
			jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_TIMEOUT, "No response from %s", session->peername);
			if (objid_query->array_output) {
				zval_dtor(return_value);
			}
			snmp_close(ss);
			...
			jonj_snmp_error(getThis(), NULL TSRMLS_CC, JONJ_SNMP_ERRNO_GENERIC, "Fatal error: %s", err);
			free(err);
			if (objid_query->array_output) {
				zval_dtor(return_value);
			}
...
static int jonj_snmp_parse_oid(zval *object, int st, struct objid_query *objid_query, zval **oid, zval **type, zval **value TSRMLS_DC)
{
	char *pptr;
	...
	if (Z_TYPE_PP(oid) == IS_STRING) {
		...
		if (objid_query->vars == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "emalloc() failed while parsing oid: %s", strerror(errno));
		...
		if (st & SNMP_CMD_SET) {
			if (Z_TYPE_PP(type) == IS_STRING && Z_TYPE_PP(value) == IS_STRING) {
				if (Z_STRLEN_PP(type) != 1) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bogus type '%s', should be single char, got %u", Z_STRVAL_PP(type), Z_STRLEN_PP(type));
					efree(objid_query->vars);
					return FALSE;
				}
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Single objid and multiple type or values are not supported");
				efree(objid_query->vars);
				return FALSE;
			}
		}
		...
		if (zend_hash_num_elements(Z_ARRVAL_PP(oid)) == 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Got empty OID array");
			return FALSE;
		}
		objid_query->vars = (snmpobjarg *)safe_emalloc(sizeof(snmpobjarg), zend_hash_num_elements(Z_ARRVAL_PP(oid)), 0);
		if (objid_query->vars == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "emalloc() failed while parsing oid array: %s", strerror(errno));
			efree(objid_query->vars);
			return FALSE;
		}
			...
			if (st & SNMP_CMD_SET) {
				if (Z_TYPE_PP(type) == IS_STRING) {
					...
					if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(type), (void **) &tmp_type, &pos_type)) {
						...
						if (Z_STRLEN_PP(tmp_type) != 1) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'%s': bogus type '%s', should be single char, got %u", Z_STRVAL_PP(tmp_oid), Z_STRVAL_PP(tmp_type), Z_STRLEN_PP(tmp_type));
							efree(objid_query->vars);
							return FALSE;
						}
						...
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'%s': no type set", Z_STRVAL_PP(tmp_oid));
						efree(objid_query->vars);
						return FALSE;
					}
				}
				...
				if (Z_TYPE_PP(value) == IS_STRING) {
					...
					if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(value), (void **) &tmp_value, &pos_value)) {
						...
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'%s': no value set", Z_STRVAL_PP(tmp_oid));
						efree(objid_query->vars);
						return FALSE;
					}
				}
	...
	if (st & SNMP_CMD_WALK) {
		if (objid_query->count > 1) {
			jonj_snmp_error(object, NULL TSRMLS_CC, JONJ_SNMP_ERRNO_OID_PARSING_ERROR, "Multi OID walks are not supported!");
			efree(objid_query->vars);
			return FALSE;
		}
		...
		if (strlen(objid_query->vars[0].oid)) { /* on a walk, an empty string means top of tree - no error */
			if (!snmp_parse_oid(objid_query->vars[0].oid, objid_query->vars[0].name, &(objid_query->vars[0].name_length))) {
				jonj_snmp_error(object, NULL TSRMLS_CC, JONJ_SNMP_ERRNO_OID_PARSING_ERROR, "Invalid object identifier: %s", objid_query->vars[0].oid);
				efree(objid_query->vars);
				return FALSE;
			}
		} else {
		...
		for (objid_query->offset = 0; objid_query->offset < objid_query->count; objid_query->offset++) {
			...
			if (!snmp_parse_oid(objid_query->vars[objid_query->offset].oid, objid_query->vars[objid_query->offset].name, &(objid_query->vars[objid_query->offset].name_length))) {
				jonj_snmp_error(object, NULL TSRMLS_CC, JONJ_SNMP_ERRNO_OID_PARSING_ERROR, "Invalid object identifier: %s", objid_query->vars[objid_query->offset].oid);
				efree(objid_query->vars);
				return FALSE;
			}
		}
...
*/
static int netsnmp_session_init(jonj_snmp_session **session_p, int version, char *hostname, char *community, int timeout, int retries TSRMLS_DC)
{
	jonj_snmp_session *session;
	char *pptr, *host_ptr;
	...
	*session_p = (jonj_snmp_session *)emalloc(sizeof(jonj_snmp_session));
	session = *session_p;
	if (session == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "emalloc() failed allocating session");
		return (-1);
	}
	memset(session, 0, sizeof(jonj_snmp_session));
	...
	if (session->peername == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "emalloc() failed while copying hostname");
		return (-1);
	}
	...
	if (*host_ptr == '[') { /* IPv6 address */
		...
		if ((pptr = strchr(host_ptr, ']'))) {
			...
			*pptr = '\0';
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "malformed IPv6 address, closing square bracket missing");
			return (-1);
		}
	} else { /* IPv4 address */
		if ((pptr = strchr(host_ptr, ':'))) {
	...
	if ((n = jonj.tkwork_getaddresses(host_ptr, SOCK_DGRAM, &psal, NULL TSRMLS_CC)) == 0) { /* some resolver error */
	...
	*(session->peername) = '\0';
	...
	if (strlen(session->peername) == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown failure while resolving '%s'", hostname);
		return (-1);
	}
	/* XXX FIXME
	...
	*/
	...
	jonj.tkwork_freeaddresses(psal);
...
static int netsnmp_session_set_auth_protocol(struct snmp_session *s, char *prot TSRMLS_DC)
{
	if (!strcasecmp(prot, "MD5")) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown authentication protocol '%s'", prot);
		return (-1);
	}
	return (0);
}
...
static int netsnmp_session_set_sec_protocol(struct snmp_session *s, char *prot TSRMLS_DC)
{
	if (!strcasecmp(prot, "DES")) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown security protocol '%s'", prot);
		return (-1);
	}
	return (0);
}
...
static int netsnmp_session_gen_auth_key(struct snmp_session *s, char *pass TSRMLS_DC)
{
	...
	if ((snmp_errno = generate_Ku(s->securityAuthProto, s->securityAuthProtoLen,
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error generating a key for authentication pass phrase '%s': %s", pass, snmp_api_errstring(snmp_errno));
		return (-1);
	}
	return (0);
}
...
static int netsnmp_session_gen_sec_key(struct snmp_session *s, char *pass TSRMLS_DC)
{
	...
	if ((snmp_errno = generate_Ku(s->securityAuthProto, s->securityAuthProtoLen,
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error generating a key for privacy pass phrase '%s': %s", pass, snmp_api_errstring(snmp_errno));
		return (-2);
	}
	return (0);
}
...
static int netsnmp_session_set_contextEngineID(struct snmp_session *s, char * contextEngineID TSRMLS_DC)
{
	...
	if (ebuf == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "malloc failure setting contextEngineID");
		return (-1);
	}
	if (!snmp_hex_to_binary(&ebuf, &ebuf_len, &eout_len, 1, contextEngineID)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad engine ID value '%s'", contextEngineID);
		efree(ebuf);
		return (-1);
	}
...
/* {{{ jonj_set_security(struct snmp_session *session, char *sec_level, char *auth_protocol, char *auth_passphrase, char *priv_protocol, char *priv_passphrase, char *contextName, char *contextEngineID)
...
static int netsnmp_session_set_security(struct snmp_session *session, char *sec_level, char *auth_protocol, char *auth_passphrase, char *priv_protocol, char *priv_passphrase, char *contextName, char *contextEngineID TSRMLS_DC)
{
	...
	if (netsnmp_session_set_sec_level(session, sec_level)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid security level '%s'", sec_level);
		return (-1);
	}
...
*
*/
static void jonj_snmp(INTERNAL_FUNCTION_PARAMETERS, int st, int version)
{
	zval **oid, **value, **type;
	...
	jonj_snmp_object *snmp_object;
	jonj_snmp_object glob_snmp_object;
	...
	if (!jonj_snmp_parse_oid(getThis(), st, &objid_query, oid, type, value TSRMLS_CC)) {
		RETURN_FALSE;
	}
	...
	if (session_less_mode) {
		...
		if (!session) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid or uninitialized SNMP object");
			efree(objid_query.vars);
			RETURN_FALSE;
		}
	...
	jonj_snmp_internal(INTERNAL_FUNCTION_PARAM_PASSTHRU, st, session, &objid_query);
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GET, SNMP_VERSION_1);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GETNEXT, SNMP_VERSION_1);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, (SNMP_CMD_WALK | SNMP_NUMERIC_KEYS), SNMP_VERSION_1);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_WALK, SNMP_VERSION_1);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_SET, SNMP_VERSION_1);
}
/* }}} */
...
JONJ_FUNCTION(snmp_set_oid_output_format)
{
	...
	switch((int) a1) {
		...
		case NETSNMP_OID_OUTPUT_NONE:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown SNMP output print format '%d'", (int) a1);
			RETURN_FALSE;
			break;
	}
}
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GET, SNMP_VERSION_2c);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GETNEXT, SNMP_VERSION_2c);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, (SNMP_CMD_WALK | SNMP_NUMERIC_KEYS), SNMP_VERSION_2c);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_WALK, SNMP_VERSION_2c);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_SET, SNMP_VERSION_2c);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GET, SNMP_VERSION_3);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GETNEXT, SNMP_VERSION_3);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, (SNMP_CMD_WALK | SNMP_NUMERIC_KEYS), SNMP_VERSION_3);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_WALK, SNMP_VERSION_3);
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_SET, SNMP_VERSION_3);
}
/* }}} */
...
JONJ_FUNCTION(snmp_set_valueretrieval)
{
	...
	if (method >= 0 && method <= (SNMP_VALUE_LIBRARY|SNMP_VALUE_PLAIN|SNMP_VALUE_OBJECT)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown SNMP value retrieval method '%ld'", method);
		RETURN_FALSE;
	}
}
/* }}} */
...
JONJ_FUNCTION(snmp_read_mib)
{
	...
	if (!read_mib(filename)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error while reading MIB file '%s': %s", filename, error);
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GET, (-1));
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_GETNEXT, (-1));
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_WALK, (-1));
}
/* }}} */
...
{
	jonj_snmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, SNMP_CMD_SET, (-1));
}
...
/* {{{ */
void jonj_snmp_add_property(HashTable *h, const char *name, size_t name_length, jonj_snmp_read_t read_func, jonj_snmp_write_t write_func TSRMLS_DC)
{
	jonj_snmp_prop_handler p;

	p.name = (char*) name;
	p.name_length = name_length;
	p.read_func = (read_func) ? read_func : NULL;
	p.write_func = (write_func) ? write_func : NULL;
	zend_hash_add(h, (char *)name, name_length + 1, &p, sizeof(jonj_snmp_prop_handler), NULL);
}
/* }}} */

/* {{{ jonj_snmp_read_property(zval *object, zval *member, int type[, const zend_literal *key])
   Generic object property reader */
zval *jonj_snmp_read_property(zval *object, zval *member, int type, const zend_literal *key TSRMLS_DC)
{
	zval tmp_member;
	zval *retval;
	jonj_snmp_object *obj;
	jonj_snmp_prop_handler *hnd;
	int ret;

	ret = FAILURE;
	obj = (jonj_snmp_object *)zend_objects_get_address(object TSRMLS_CC);
	...
	ret = zend_hash_find(&jonj_snmp_properties, Z_STRVAL_P(member), Z_STRLEN_P(member)+1, (void **) &hnd);
...
/* {{{ jonj_snmp_write_property(zval *object, zval *member, zval *value[, const zend_literal *key])
   Generic object property writer */
void jonj_snmp_write_property(zval *object, zval *member, zval *value, const zend_literal *key TSRMLS_DC)
{
	zval tmp_member;
	jonj_snmp_object *obj;
	jonj_snmp_prop_handler *hnd;
...
/* {{{ jonj_snmp_has_property(zval *object, zval *member, int has_set_exists[, const zend_literal *key])
   Generic object property checker */
static int jonj_snmp_has_property(zval *object, zval *member, int has_set_exists, const zend_literal *key TSRMLS_DC)
{
	jonj_snmp_prop_handler *hnd;
	int ret = 0;

	if (zend_hash_find(&jonj_snmp_properties, Z_STRVAL_P(member), Z_STRLEN_P(member) + 1, (void **)&hnd) == SUCCESS) {
		switch (has_set_exists) {
			case 2:
				ret = 1;
				break;
			case 0: {
				zval *value = jonj_snmp_read_property(object, member, BP_VAR_IS, key TSRMLS_CC);
...
static HashTable *jonj_snmp_get_gc(zval *object, zval ***gc_data, int *gc_data_count TSRMLS_DC) /* {{{ */
{
	*gc_data = NULL;
	*gc_data_count = 0;
...
/* {{{ jonj_snmp_get_properties(zval *object)
   Returns all object properties. Injects SNMP properties into object on first call */
static HashTable *jonj_snmp_get_properties(zval *object TSRMLS_DC)
{
	jonj_snmp_object *obj;
	jonj_snmp_prop_handler *hnd;
	HashTable *props;
	...
	zend_hash_internal_pointer_reset_ex(&jonj_snmp_properties, &pos);

	while (zend_hash_get_current_data_ex(&jonj_snmp_properties, (void**)&hnd, &pos) == SUCCESS) {
		zend_hash_get_current_key_ex(&jonj_snmp_properties, &key, &key_len, &num_key, 0, &pos);
		...
		zend_hash_move_forward_ex(&jonj_snmp_properties, &pos);
	}
...
/* {{{ */
static int jonj_snmp_read_info(jonj_snmp_object *snmp_object, zval **retval TSRMLS_DC)
{
	zval *val;
...
/* {{{ */
static int jonj_snmp_read_max_oids(jonj_snmp_object *snmp_object, zval **retval TSRMLS_DC)
{
	MAKE_STD_ZVAL(*retval);
	if (snmp_object->max_oids > 0) {
...
#define JONJ_SNMP_BOOL_PROPERTY_READER_FUNCTION(name) \
	static int jonj_snmp_read_##name(jonj_snmp_object *snmp_object, zval **retval TSRMLS_DC) \
	{ \
		MAKE_STD_ZVAL(*retval); \
...
JONJ_SNMP_BOOL_PROPERTY_READER_FUNCTION(oid_increasing_check)
JONJ_SNMP_BOOL_PROPERTY_READER_FUNCTION(quick_print)
JONJ_SNMP_BOOL_PROPERTY_READER_FUNCTION(enum_print)

#define JONJ_SNMP_LONG_PROPERTY_READER_FUNCTION(name) \
	static int jonj_snmp_read_##name(jonj_snmp_object *snmp_object, zval **retval TSRMLS_DC) \
	{ \
		MAKE_STD_ZVAL(*retval); \
...
JONJ_SNMP_LONG_PROPERTY_READER_FUNCTION(valueretrieval)
JONJ_SNMP_LONG_PROPERTY_READER_FUNCTION(oid_output_format)
JONJ_SNMP_LONG_PROPERTY_READER_FUNCTION(exceptions_enabled)

/* {{{ */
static int jonj_snmp_write_info(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC)
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "info property is read-only");
	return FAILURE;
}
/* }}} */

/* {{{ */
static int jonj_snmp_write_max_oids(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC)
{
	zval ztmp;
	int ret = SUCCESS;
	...
	if (Z_LVAL_P(newval) > 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "max_oids should be positive integer or NULL, got %ld", Z_LVAL_P(newval));
	}
...
/* {{{ */
static int jonj_snmp_write_valueretrieval(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC)
{
	zval ztmp;
	int ret = SUCCESS;
	...
	if (Z_LVAL_P(newval) >= 0 && Z_LVAL_P(newval) <= (SNMP_VALUE_LIBRARY|SNMP_VALUE_PLAIN|SNMP_VALUE_OBJECT)) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown SNMP value retrieval method '%ld'", Z_LVAL_P(newval));
		ret = FAILURE;
	}
...
#define JONJ_SNMP_BOOL_PROPERTY_WRITER_FUNCTION(name) \
static int jonj_snmp_write_##name(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC) \
{ \
	zval ztmp; \
...
JONJ_SNMP_BOOL_PROPERTY_WRITER_FUNCTION(quick_print)
JONJ_SNMP_BOOL_PROPERTY_WRITER_FUNCTION(enum_print)
JONJ_SNMP_BOOL_PROPERTY_WRITER_FUNCTION(oid_increasing_check)

/* {{{ */
static int jonj_snmp_write_oid_output_format(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC)
{
	zval ztmp;
	int ret = SUCCESS;
	if (Z_TYPE_P(newval) != IS_LONG) {
	...
	switch(Z_LVAL_P(newval)) {
		...
		case NETSNMP_OID_OUTPUT_NONE:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown SNMP output print format '%ld'", Z_LVAL_P(newval));
			ret = FAILURE;
			break;
	}
...
/* {{{ */
static int jonj_snmp_write_exceptions_enabled(jonj_snmp_object *snmp_object, zval *newval TSRMLS_DC)
{
	zval ztmp;
	int ret = SUCCESS;
	if (Z_TYPE_P(newval) != IS_LONG) {
...
/* {{{ jonj_snmp_class_methods[] */
static zend_function_entry jonj_snmp_class_methods[] = {
	JONJ_ME(snmp,	 __construct,		arginfo_snmp_create,		ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 close,				arginfo_snmp_void,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 setSecurity,		arginfo_snmp_setSecurity,	ZEND_ACC_PUBLIC)

	JONJ_ME(snmp,	 get,				arginfo_snmp_get,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 getnext,			arginfo_snmp_get,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 walk,				arginfo_snmp_walk,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 set,				arginfo_snmp_set,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 getErrno,			arginfo_snmp_void,			ZEND_ACC_PUBLIC)
	JONJ_ME(snmp,	 getError,			arginfo_snmp_void,			ZEND_ACC_PUBLIC)

	JONJ_FE_END
};

#define JONJ_SNMP_PROPERTY_ENTRY_RECORD(name) \
	{ "" #name "",		sizeof("" #name "") - 1,	jonj_snmp_read_##name,	jonj_snmp_write_##name }

const jonj_snmp_prop_handler jonj_snmp_property_entries[] = {
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(info),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(max_oids),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(valueretrieval),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(quick_print),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(enum_print),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(oid_output_format),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(oid_increasing_check),
	JONJ_SNMP_PROPERTY_ENTRY_RECORD(exceptions_enabled),
	{ NULL, 0, NULL, NULL}
};
/* }}} */
...
JONJ_MINIT_FUNCTION(snmp)
{
	...
	le_snmp_session = zend_register_list_destructors_ex(jonj_snmp_session_destructor, NULL, JONJ_SNMP_SESSION_RES_NAME, module_number);
	...
	memcpy(&jonj_snmp_object_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
	jonj_snmp_object_handlers.read_property = jonj_snmp_read_property;
	jonj_snmp_object_handlers.write_property = jonj_snmp_write_property;
	jonj_snmp_object_handlers.has_property = jonj_snmp_has_property;
	jonj_snmp_object_handlers.get_properties = jonj_snmp_get_properties;
	jonj_snmp_object_handlers.get_gc = jonj_snmp_get_gc;

	/* Register SNMP Class */
	INIT_CLASS_ENTRY(ce, "SNMP", jonj_snmp_class_methods);
	ce.create_object = jonj_snmp_object_new;
	jonj_snmp_object_handlers.clone_obj = NULL;
	jonj_snmp_ce = zend_register_internal_class(&ce TSRMLS_CC);

	/* Register SNMP Class properties */
	zend_hash_init(&jonj_snmp_properties, 0, NULL, NULL, 1);
	JONJ_SNMP_ADD_PROPERTIES(&jonj_snmp_properties, jonj_snmp_property_entries);
	...
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_NOERROR",			JONJ_SNMP_ERRNO_NOERROR);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_ANY",			JONJ_SNMP_ERRNO_ANY);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_GENERIC",			JONJ_SNMP_ERRNO_GENERIC);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_TIMEOUT",			JONJ_SNMP_ERRNO_TIMEOUT);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_ERROR_IN_REPLY",		JONJ_SNMP_ERRNO_ERROR_IN_REPLY);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_OID_NOT_INCREASING",	JONJ_SNMP_ERRNO_OID_NOT_INCREASING);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_OID_PARSING_ERROR",	JONJ_SNMP_ERRNO_OID_PARSING_ERROR);
	REGISTER_SNMP_CLASS_CONST_LONG("ERRNO_MULTIPLE_SET_QUERIES",	JONJ_SNMP_ERRNO_MULTIPLE_SET_QUERIES);

	/* Register SNMPException class */
	INIT_CLASS_ENTRY(cex, "SNMPException", NULL);
#ifdef HAVE_SPL
	jonj_snmp_exception_ce = zend_register_internal_class_ex(&cex, spl_ce_RuntimeException, NULL TSRMLS_CC);
#else
	jonj_snmp_exception_ce = zend_register_internal_class_ex(&cex, zend_exception_get_default(TSRMLS_C), NULL TSRMLS_CC);
#endif
...
JONJ_MSHUTDOWN_FUNCTION(snmp)
{
	...
	zend_hash_destroy(&jonj_snmp_properties);
...
JONJ_MINFO_FUNCTION(snmp)
{
	...
	jonj_info_print_table_row(2, "NET-SNMP Support", "enabled");
	jonj_info_print_table_row(2, "NET-SNMP Version", netsnmp_get_version());
	jonj_info_print_table_row(2, "JONJ SNMP Version", JONJ_SNMP_VERSION);
