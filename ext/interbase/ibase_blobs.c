...
static void _jonj_ibase_free_blob(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_blob *ib_blob = (ibase_blob *)rsrc->ptr;

	if (ib_blob->bl_handle != NULL) { /* blob open*/
		if (isc_cancel_blob(IB_STATUS, &ib_blob->bl_handle)) {
			_jonj_ibase_module_error("You can lose data. Close any blob after reading from or "
...
void jonj_ibase_blobs_minit(INIT_FUNC_ARGS) /* {{{ */
{
	le_blob = zend_register_list_destructors_ex(_jonj_ibase_free_blob, NULL, 
	    "interbase blob", module_number);
}
/* }}} */

int _jonj_ibase_string_to_quad(char const *id, ISC_QUAD *qd) /* {{{ */
{
...
char *_jonj_ibase_quad_to_string(ISC_QUAD const qd) /* {{{ */
{
	char *result;
...
int _jonj_ibase_blob_get(zval *return_value, ibase_blob *ib_blob, unsigned long max_len TSRMLS_DC) /* {{{ */
{
...
int _jonj_ibase_blob_add(zval **string_arg, ibase_blob *ib_blob TSRMLS_DC) /* {{{ */
{
...
static int _jonj_ibase_blob_info(isc_blob_handle bl_handle, IBASE_BLOBINFO *bl_info TSRMLS_DC) /* {{{ */
{
	...
	for (p = bl_inf; *p != isc_info_end && p < bl_inf + sizeof(bl_inf);) {
		...
		switch (item) {
			...
			case isc_info_error:  /* hmm. don't think so...*/
				_jonj_ibase_module_error("JONJ module internal error" TSRMLS_CC);
				return FAILURE;
		} /* switch */
		p += item_len;
	} /* for */
...
JONJ_FUNCTION(ibase_blob_open)
{
	...
	do {
		if (! _jonj_ibase_string_to_quad(blob_id, &ib_blob->bl_qd)) {
			_jonj_ibase_module_error("String is not a BLOB ID" TSRMLS_CC);
			break;
		}
...
JONJ_FUNCTION(ibase_blob_add)
{
	...
	if (ib_blob->type != BLOB_INPUT) {
		_jonj_ibase_module_error("BLOB is not open for input" TSRMLS_CC);
		RETURN_FALSE;
	}

	if (_jonj_ibase_blob_add(string_arg, ib_blob TSRMLS_CC) != SUCCESS) {
		RETURN_FALSE;
	}
}
/* }}} */
...
JONJ_FUNCTION(ibase_blob_get)
{
	...
	if (ib_blob->type != BLOB_OUTPUT) {
		_jonj_ibase_module_error("BLOB is not open for output" TSRMLS_CC);
		RETURN_FALSE;
	}

	convert_to_long_ex(len_arg);

	if (_jonj_ibase_blob_get(return_value, ib_blob, Z_LVAL_PP(len_arg) TSRMLS_CC) != SUCCESS) {
		RETURN_FALSE;
	}
}
/* }}} */

static void _jonj_ibase_blob_end(INTERNAL_FUNCTION_PARAMETERS, int bl_end) /* {{{ */
{
	zval **blob_arg;
	ibase_blob *ib_blob;
	...
	if (bl_end == BLOB_CLOSE) { /* return id here */
		...
		RETVAL_STRINGL(_jonj_ibase_quad_to_string(ib_blob->bl_qd), BLOB_ID_LEN, 0);
...
{
	_jonj_ibase_blob_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, BLOB_CLOSE);
}
/* }}} */
...
{
	_jonj_ibase_blob_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, BLOB_CANCEL);
}
/* }}} */
...
JONJ_FUNCTION(ibase_blob_info)
{
	...
	if (! _jonj_ibase_string_to_quad(blob_id, &ib_blob.bl_qd)) {
		_jonj_ibase_module_error("Unrecognized BLOB ID" TSRMLS_CC);
		RETURN_FALSE;
	}
	...
	if (ib_blob.bl_qd.gds_quad_high || ib_blob.bl_qd.gds_quad_low) { /* not null ? */
		...
		if (_jonj_ibase_blob_info(ib_blob.bl_handle, &bl_info TSRMLS_CC)) {
			RETURN_FALSE;
		}
		if (isc_close_blob(IB_STATUS, &ib_blob.bl_handle)) {
			_jonj_ibase_error(TSRMLS_C);
...
JONJ_FUNCTION(ibase_blob_echo)
{
	...
	if (! _jonj_ibase_string_to_quad(blob_id, &ib_blob_id.bl_qd)) {
		_jonj_ibase_module_error("Unrecognized BLOB ID" TSRMLS_CC);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ibase_blob_import)
{
	...
	do {
		...
		for (size = 0; (b = jonj_stream_read(stream, bl_data, sizeof(bl_data))); size += b) {
		...
		RETURN_STRINGL( _jonj_ibase_quad_to_string(ib_blob.bl_qd), BLOB_ID_LEN, 0);
	} while (0);
