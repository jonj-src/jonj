...
static void _jonj_ibase_event_free(char *event_buf, char *result_buf) /* {{{ */
{
	isc_free(event_buf);
	isc_free(result_buf);
}
...
void _jonj_ibase_free_event(ibase_event *event TSRMLS_DC) /* {{{ */
{
	...
	if (event->callback) {
		...
		_jonj_ibase_event_free(event->event_buffer,event->result_buffer);
...
static void _jonj_ibase_free_event_rsrc(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
...
{
	le_event = zend_register_list_destructors_ex(_jonj_ibase_free_event_rsrc, NULL, 
	    "interbase event", module_number);
}
/* }}} */

static void _jonj_ibase_event_block(ibase_db_link *ib_link, unsigned short count, /* {{{ */
...
JONJ_FUNCTION(ibase_wait_event)
{
	...
	_jonj_ibase_event_block(ib_link, event_count, events, &buffer_size, &event_buffer, &result_buffer);

	/* now block until an event occurs */
	if (isc_wait_for_event(IB_STATUS, &ib_link->handle, buffer_size, event_buffer, result_buffer)) {
		_jonj_ibase_error(TSRMLS_C);
		_jonj_ibase_event_free(event_buffer,result_buffer);
		efree(args);
		RETURN_FALSE;
	}
...
static isc_callback _jonj_ibase_callback(ibase_event *event, /* {{{ */
	...
	switch (event->state) {
		...
		case ACTIVE:
			...
			if (SUCCESS != call_user_function(EG(function_table), NULL,
				...
				_jonj_ibase_module_error("Error calling callback %s" TSRMLS_CC, Z_STRVAL_P(event->callback));
				break;
			}
		...
		case NEW:
			...
			if (isc_que_events(IB_STATUS, &event->link->handle, &event->event_id, buffer_size,
				event->event_buffer,(isc_callback)_jonj_ibase_callback, (void *)event)) {
...
JONJ_FUNCTION(ibase_set_event_handler)
{
	...
	if (!zend_is_callable(*cb_arg, 0, &cb_name TSRMLS_CC)) {
		_jonj_ibase_module_error("Callback argument %s is not a callable function" TSRMLS_CC, cb_name);
		efree(cb_name);
		efree(args);
		RETURN_FALSE;
	}
	...
	*event->callback = **cb_arg;
	...
	/* fills the required data structure with information about the events */
	_jonj_ibase_event_block(ib_link, event->event_count, event->events,
