...
#include "jonj_interbase.h"
#include "jonj_ibase_includes.h"
#include "SAPI.h"
...
static JONJ_GINIT_FUNCTION(ibase);
...
const zend_function_entry ibase_functions[] = {
	JONJ_FE(ibase_connect, 		arginfo_ibase_connect)
	JONJ_FE(ibase_pconnect, 		arginfo_ibase_pconnect)
	JONJ_FE(ibase_close, 		arginfo_ibase_close)
	JONJ_FE(ibase_drop_db, 		arginfo_ibase_drop_db)
	JONJ_FE(ibase_query, 		arginfo_ibase_query)
	JONJ_FE(ibase_fetch_row, 	arginfo_ibase_fetch_row)
	JONJ_FE(ibase_fetch_assoc, 	arginfo_ibase_fetch_assoc)
	JONJ_FE(ibase_fetch_object, 	arginfo_ibase_fetch_object)
	JONJ_FE(ibase_free_result, 	arginfo_ibase_free_result)
	JONJ_FE(ibase_name_result, 	arginfo_ibase_name_result)
	JONJ_FE(ibase_prepare, 		arginfo_ibase_prepare)
	JONJ_FE(ibase_execute, 		arginfo_ibase_execute)
	JONJ_FE(ibase_free_query, 	arginfo_ibase_free_query)
	JONJ_FE(ibase_gen_id, 		arginfo_ibase_gen_id)
	JONJ_FE(ibase_num_fields, 	arginfo_ibase_num_fields)
	JONJ_FE(ibase_num_params, 	arginfo_ibase_num_params)
#if abies_0
	JONJ_FE(ibase_num_rows, 		arginfo_ibase_num_rows)
#endif
	JONJ_FE(ibase_affected_rows, arginfo_ibase_affected_rows)
	JONJ_FE(ibase_field_info, 	arginfo_ibase_field_info)
	JONJ_FE(ibase_param_info, 	arginfo_ibase_param_info)

	JONJ_FE(ibase_trans, 		arginfo_ibase_trans)
	JONJ_FE(ibase_commit, 		arginfo_ibase_commit)
	JONJ_FE(ibase_rollback, 		arginfo_ibase_rollback)
	JONJ_FE(ibase_commit_ret, 	arginfo_ibase_commit_ret)
	JONJ_FE(ibase_rollback_ret, 	arginfo_ibase_rollback_ret)

	JONJ_FE(ibase_blob_info, 	arginfo_ibase_blob_info)
	JONJ_FE(ibase_blob_create, 	arginfo_ibase_blob_create)
	JONJ_FE(ibase_blob_add, 		arginfo_ibase_blob_add)
	JONJ_FE(ibase_blob_cancel, 	arginfo_ibase_blob_cancel)
	JONJ_FE(ibase_blob_close, 	arginfo_ibase_blob_close)
	JONJ_FE(ibase_blob_open, 	arginfo_ibase_blob_open)
	JONJ_FE(ibase_blob_get, 		arginfo_ibase_blob_get)
	JONJ_FE(ibase_blob_echo, 	arginfo_ibase_blob_echo)
	JONJ_FE(ibase_blob_import, 	arginfo_ibase_blob_import)
	JONJ_FE(ibase_errmsg, 		arginfo_ibase_errmsg)
	JONJ_FE(ibase_errcode, 		arginfo_ibase_errcode)

	JONJ_FE(ibase_add_user, 		arginfo_ibase_add_user)
	JONJ_FE(ibase_modify_user, 	arginfo_ibase_modify_user)
	JONJ_FE(ibase_delete_user, 	arginfo_ibase_delete_user)

	JONJ_FE(ibase_service_attach, arginfo_ibase_service_attach)
	JONJ_FE(ibase_service_detach, arginfo_ibase_service_detach)
	JONJ_FE(ibase_backup, 		arginfo_ibase_backup)
	JONJ_FE(ibase_restore, 		arginfo_ibase_restore)
	JONJ_FE(ibase_maintain_db, 	arginfo_ibase_maintain_db)
	JONJ_FE(ibase_db_info, 		arginfo_ibase_db_info)
	JONJ_FE(ibase_server_info, 	arginfo_ibase_server_info)

	JONJ_FE(ibase_wait_event, 			arginfo_ibase_wait_event)
	JONJ_FE(ibase_set_event_handler, 	arginfo_ibase_set_event_handler)
	JONJ_FE(ibase_free_event_handler, 	arginfo_ibase_free_event_handler)
	...
	*/
	JONJ_FALIAS(fbird_connect,		ibase_connect, 		arginfo_ibase_connect)
	JONJ_FALIAS(fbird_pconnect,		ibase_pconnect, 	arginfo_ibase_pconnect)
	JONJ_FALIAS(fbird_close,			ibase_close, 		arginfo_ibase_close)
	JONJ_FALIAS(fbird_drop_db,		ibase_drop_db, 		arginfo_ibase_drop_db)
	JONJ_FALIAS(fbird_query,			ibase_query, 		arginfo_ibase_query)
	JONJ_FALIAS(fbird_fetch_row,		ibase_fetch_row, 	arginfo_ibase_fetch_row)
	JONJ_FALIAS(fbird_fetch_assoc,	ibase_fetch_assoc, 	arginfo_ibase_fetch_assoc)
	JONJ_FALIAS(fbird_fetch_object,	ibase_fetch_object, arginfo_ibase_fetch_object)
	JONJ_FALIAS(fbird_free_result,	ibase_free_result, 	arginfo_ibase_free_result)
	JONJ_FALIAS(fbird_name_result,	ibase_name_result, 	arginfo_ibase_name_result)
	JONJ_FALIAS(fbird_prepare,		ibase_prepare, 		arginfo_ibase_prepare)
	JONJ_FALIAS(fbird_execute,		ibase_execute, 		arginfo_ibase_execute)
	JONJ_FALIAS(fbird_free_query,	ibase_free_query, 	arginfo_ibase_free_query)
	JONJ_FALIAS(fbird_gen_id,		ibase_gen_id, 		arginfo_ibase_gen_id)
	JONJ_FALIAS(fbird_num_fields,	ibase_num_fields, 	arginfo_ibase_num_fields)
	JONJ_FALIAS(fbird_num_params,	ibase_num_params, 	arginfo_ibase_num_params)
#if abies_0
	JONJ_FALIAS(fbird_num_rows,		ibase_num_rows, 	arginfo_ibase_num_rows)
#endif
	JONJ_FALIAS(fbird_affected_rows,	ibase_affected_rows, arginfo_ibase_affected_rows)
	JONJ_FALIAS(fbird_field_info,	ibase_field_info, 	arginfo_ibase_field_info)
	JONJ_FALIAS(fbird_param_info,	ibase_param_info, 	arginfo_ibase_param_info)

	JONJ_FALIAS(fbird_trans,			ibase_trans, 		arginfo_ibase_trans)
	JONJ_FALIAS(fbird_commit,		ibase_commit, 		arginfo_ibase_commit)
	JONJ_FALIAS(fbird_rollback,		ibase_rollback, 	arginfo_ibase_rollback)
	JONJ_FALIAS(fbird_commit_ret,	ibase_commit_ret, 	arginfo_ibase_commit_ret)
	JONJ_FALIAS(fbird_rollback_ret,	ibase_rollback_ret, arginfo_ibase_rollback_ret)

	JONJ_FALIAS(fbird_blob_info,		ibase_blob_info, 	arginfo_ibase_blob_info)
	JONJ_FALIAS(fbird_blob_create,	ibase_blob_create, 	arginfo_ibase_blob_create)
	JONJ_FALIAS(fbird_blob_add,		ibase_blob_add, 	arginfo_ibase_blob_add)
	JONJ_FALIAS(fbird_blob_cancel,	ibase_blob_cancel, 	arginfo_ibase_blob_cancel)
	JONJ_FALIAS(fbird_blob_close,	ibase_blob_close, 	arginfo_ibase_blob_close)
	JONJ_FALIAS(fbird_blob_open,		ibase_blob_open, 	arginfo_ibase_blob_open)
	JONJ_FALIAS(fbird_blob_get,		ibase_blob_get, 	arginfo_ibase_blob_get)
	JONJ_FALIAS(fbird_blob_echo,		ibase_blob_echo, 	arginfo_ibase_blob_echo)
	JONJ_FALIAS(fbird_blob_import,	ibase_blob_import, 	arginfo_ibase_blob_import)
	JONJ_FALIAS(fbird_errmsg,		ibase_errmsg, 		arginfo_ibase_errmsg)
	JONJ_FALIAS(fbird_errcode,		ibase_errcode, 		arginfo_ibase_errcode)

	JONJ_FALIAS(fbird_add_user,		ibase_add_user, 	arginfo_ibase_add_user)
	JONJ_FALIAS(fbird_modify_user,	ibase_modify_user, 	arginfo_ibase_modify_user)
	JONJ_FALIAS(fbird_delete_user,	ibase_delete_user, 	arginfo_ibase_delete_user)

	JONJ_FALIAS(fbird_service_attach,	ibase_service_attach, arginfo_ibase_service_attach)
	JONJ_FALIAS(fbird_service_detach,	ibase_service_detach, arginfo_ibase_service_detach)
	JONJ_FALIAS(fbird_backup,		ibase_backup, 		arginfo_ibase_backup)
	JONJ_FALIAS(fbird_restore,		ibase_restore, 		arginfo_ibase_restore)
	JONJ_FALIAS(fbird_maintain_db,	ibase_maintain_db, 	arginfo_ibase_maintain_db)
	JONJ_FALIAS(fbird_db_info,		ibase_db_info, 		arginfo_ibase_db_info)
	JONJ_FALIAS(fbird_server_info,	ibase_server_info, 	arginfo_ibase_server_info)

	JONJ_FALIAS(fbird_wait_event,	ibase_wait_event, 	arginfo_ibase_wait_event)
	JONJ_FALIAS(fbird_set_event_handler,	ibase_set_event_handler, 	arginfo_ibase_set_event_handler)
	JONJ_FALIAS(fbird_free_event_handler,	ibase_free_event_handler, arginfo_ibase_free_event_handler)
	JONJ_FE_END
};
...
zend_module_entry ibase_module_entry = {
	...
	ibase_functions,
	JONJ_MINIT(ibase),
	JONJ_MSHUTDOWN(ibase),
	NULL,
	JONJ_RSHUTDOWN(ibase),
	JONJ_MINFO(ibase),
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(ibase),
	JONJ_GINIT(ibase),
	NULL,
	NULL,
...
void _jonj_ibase_error(TSRMLS_D) /* {{{ */
{
	char *s = IBG(errmsg);
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", IBG(errmsg));
}
/* }}} */

/* print jonj interbase module error and save it for ibase_errmsg() */
void _jonj_ibase_module_error(char *msg TSRMLS_DC, ...) /* {{{ */
{
	va_list ap;
...
void _jonj_ibase_get_link_trans(INTERNAL_FUNCTION_PARAMETERS, /* {{{ */
	zval **link_id, ibase_db_link **ib_link, ibase_trans **trans)
{
	...
	if (zend_list_find(Z_LVAL_PP(link_id), &type)) {
	 	if (type == le_trans) {
			...
			if ((*trans)->link_cnt > 1) {
				_jonj_ibase_module_error("Link id is ambiguous: transaction spans multiple connections."
					TSRMLS_CC);
				return;
			}				
...
static void _jonj_ibase_commit_link(ibase_db_link *link TSRMLS_DC) /* {{{ */
{
	...
	for (e = link->event_head; e; e = e->event_next) {
		_jonj_ibase_free_event(e TSRMLS_CC);
		e->link = NULL;
	}
}
...
static void jonj_ibase_commit_link_rsrc(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_db_link *link = (ibase_db_link *) rsrc->ptr;

	_jonj_ibase_commit_link(link TSRMLS_CC);
}
/* }}} */

static void _jonj_ibase_close_link(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_db_link *link = (ibase_db_link *) rsrc->ptr;
...
static void _jonj_ibase_close_plink(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_db_link *link = (ibase_db_link *) rsrc->ptr;
...
static void _jonj_ibase_free_trans(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
...
/* TODO this function should be part of either Zend or JONJ API */
static JONJ_INI_DISP(jonj_ibase_password_displayer_cb)
{
	TSRMLS_FETCH();

	if ((type == JONJ_INI_DISPLAY_ORIG && ini_entry->orig_value) 
			|| (type == JONJ_INI_DISPLAY_ACTIVE && ini_entry->value)) {
		PUTS("********");
	} else if (!sapi_module.jonjinfo_as_text) {
		PUTS("<i>no value</i>");
	} else {
		PUTS("no value");
	}
...
JONJ_INI_BEGIN()
	JONJ_INI_ENTRY_EX("ibase.allow_persistent", "1", JONJ_INI_SYSTEM, NULL, zend_ini_boolean_displayer_cb)
	JONJ_INI_ENTRY_EX("ibase.max_persistent", "-1", JONJ_INI_SYSTEM, NULL, display_link_numbers)
	JONJ_INI_ENTRY_EX("ibase.max_links", "-1", JONJ_INI_SYSTEM, NULL, display_link_numbers)
	JONJ_INI_ENTRY("ibase.default_db", NULL, JONJ_INI_SYSTEM, NULL)
	JONJ_INI_ENTRY("ibase.default_user", NULL, JONJ_INI_ALL, NULL)
	JONJ_INI_ENTRY_EX("ibase.default_password", NULL, JONJ_INI_ALL, NULL, jonj_ibase_password_displayer_cb)
	JONJ_INI_ENTRY("ibase.default_charset", NULL, JONJ_INI_ALL, NULL)
	JONJ_INI_ENTRY("ibase.timestampformat", IB_DEF_DATE_FMT " " IB_DEF_TIME_FMT, JONJ_INI_ALL, NULL)
	JONJ_INI_ENTRY("ibase.dateformat", IB_DEF_DATE_FMT, JONJ_INI_ALL, NULL)
	JONJ_INI_ENTRY("ibase.timeformat", IB_DEF_TIME_FMT, JONJ_INI_ALL, NULL)
JONJ_INI_END()
...
JONJ_MINIT_FUNCTION(ibase)
{
	...
	le_link = zend_register_list_destructors_ex(_jonj_ibase_close_link, NULL, LE_LINK, module_number);
	le_plink = zend_register_list_destructors_ex(jonj_ibase_commit_link_rsrc, _jonj_ibase_close_plink, LE_PLINK, module_number);
	le_trans = zend_register_list_destructors_ex(_jonj_ibase_free_trans, NULL, LE_TRANS, module_number);

	REGISTER_LONG_CONSTANT("IBASE_DEFAULT", JONJ_IBASE_DEFAULT, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_CREATE", JONJ_IBASE_CREATE, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_TEXT", JONJ_IBASE_FETCH_BLOBS, CONST_PERSISTENT); /* deprecated, for BC only */
	REGISTER_LONG_CONSTANT("IBASE_FETCH_BLOBS", JONJ_IBASE_FETCH_BLOBS, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_FETCH_ARRAYS", JONJ_IBASE_FETCH_ARRAYS, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_UNIXTIME", JONJ_IBASE_UNIXTIME, CONST_PERSISTENT);

	/* transactions */
	REGISTER_LONG_CONSTANT("IBASE_WRITE", JONJ_IBASE_WRITE, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_READ", JONJ_IBASE_READ, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_COMMITTED", JONJ_IBASE_COMMITTED, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_CONSISTENCY", JONJ_IBASE_CONSISTENCY, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_CONCURRENCY", JONJ_IBASE_CONCURRENCY, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_REC_VERSION", JONJ_IBASE_REC_VERSION, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_REC_NO_VERSION", JONJ_IBASE_REC_NO_VERSION, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_NOWAIT", JONJ_IBASE_NOWAIT, CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("IBASE_WAIT", JONJ_IBASE_WAIT, CONST_PERSISTENT);

	jonj_ibase_query_minit(INIT_FUNC_ARGS_PASSTHRU);
	jonj_ibase_blobs_minit(INIT_FUNC_ARGS_PASSTHRU);
	jonj_ibase_events_minit(INIT_FUNC_ARGS_PASSTHRU);
	jonj_ibase_service_minit(INIT_FUNC_ARGS_PASSTHRU);
...
JONJ_MSHUTDOWN_FUNCTION(ibase)
{
	 ...
	 * JONJ tries to unload modules after every request [dl()'ed modules], and right before the 
	 * process shuts down [modules loaded from jonj.ini]. This results in a segfault for this module.
...
JONJ_MINFO_FUNCTION(ibase)
{
	...
	jonj_info_print_table_row(2, "Firebird/InterBase Support", 
#ifdef COMPILE_DL_INTERBASE
		"dynamic");
#else
		"static");
	...
	s = "Firebird 1.0";
#endif
	jonj_info_print_table_row(2, "Compile-time Client Library Version", s);

#if defined(__GNUC__) || defined(JONJ_WIN32)
	do {
		...
		jonj_info_print_table_row(2, "Run-time Client Library Version", s);
	} while (0);
#endif			
...
int _jonj_ibase_attach_db(char **args, int *len, long *largs, isc_db_handle *db TSRMLS_DC)
{
...
static void _jonj_ibase_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent) /* {{{ */
{
	char *c, hash[16], *args[] = { NULL, NULL, NULL, NULL, NULL };
	int i, len[] = { 0, 0, 0, 0, 0 };
	long largs[] = { 0, 0, 0 };
	JONJ_MD5_CTX hash_context;
	...
	JONJ_MD5Init(&hash_context);
	for (i = 0; i < sizeof(args)/sizeof(char*); ++i) {
		JONJ_MD5Update(&hash_context,args[i],len[i]);
	}
	for (i = 0; i < sizeof(largs)/sizeof(long); ++i) {
		JONJ_MD5Update(&hash_context,(char*)&largs[i],sizeof(long));
	}
	JONJ_MD5Final(hash, &hash_context);
	...
	default:
		...
		if ((l = INI_INT("ibase.max_links")) != -1 && IBG(num_links) >= l) {
			_jonj_ibase_module_error("Too many open links (%ld)" TSRMLS_CC, IBG(num_links));
			RETURN_FALSE;
		}
	
		/* create the ib_link */
		if (FAILURE == _jonj_ibase_attach_db(args, len, largs, &db_handle TSRMLS_CC)) {
			RETURN_FALSE;
		}
...
{
	_jonj_ibase_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	_jonj_ibase_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INI_INT("ibase.allow_persistent"));
}
/* }}} */
	...
	if (argn > 0) {
		...
		for (i = 0; i < argn; ++i) {
			...
			if (Z_TYPE_PP(args[i]) == IS_RESOURCE) {
				...
				if (trans_argl != JONJ_IBASE_DEFAULT) {
					last_tpb[tpb_len++] = isc_tpb_version3;

					/* access mode */
					if (JONJ_IBASE_READ == (trans_argl & JONJ_IBASE_READ)) {
						last_tpb[tpb_len++] = isc_tpb_read;
					} else if (JONJ_IBASE_WRITE == (trans_argl & JONJ_IBASE_WRITE)) {
						last_tpb[tpb_len++] = isc_tpb_write;
					}

					/* isolation level */
					if (JONJ_IBASE_COMMITTED == (trans_argl & JONJ_IBASE_COMMITTED)) {
						last_tpb[tpb_len++] = isc_tpb_read_committed;
						if (JONJ_IBASE_REC_VERSION == (trans_argl & JONJ_IBASE_REC_VERSION)) {
							last_tpb[tpb_len++] = isc_tpb_rec_version;
						} else if (JONJ_IBASE_REC_NO_VERSION == (trans_argl & JONJ_IBASE_REC_NO_VERSION)) {
							last_tpb[tpb_len++] = isc_tpb_no_rec_version; 
						}	
					} else if (JONJ_IBASE_CONSISTENCY == (trans_argl & JONJ_IBASE_CONSISTENCY)) {
						last_tpb[tpb_len++] = isc_tpb_consistency;
					} else if (JONJ_IBASE_CONCURRENCY == (trans_argl & JONJ_IBASE_CONCURRENCY)) {
						last_tpb[tpb_len++] = isc_tpb_concurrency;
					}
					
					/* lock resolution */
					if (JONJ_IBASE_NOWAIT == (trans_argl & JONJ_IBASE_NOWAIT)) {
						last_tpb[tpb_len++] = isc_tpb_nowait;
					} else if (JONJ_IBASE_WAIT == (trans_argl & JONJ_IBASE_WAIT)) {
...
int _jonj_ibase_def_trans(ibase_db_link *ib_link, ibase_trans **trans TSRMLS_DC) /* {{{ */
{
	if (ib_link == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid database link");
		return FAILURE;
	}
...
static void _jonj_ibase_trans_end(INTERNAL_FUNCTION_PARAMETERS, int commit) /* {{{ */
{
	...
	if (ZEND_NUM_ARGS() == 0) {
		...
		if (ib_link->tr_list == NULL || ib_link->tr_list->trans == NULL) {
			...
			_jonj_ibase_module_error("Default link has no default transaction" TSRMLS_CC);
			RETURN_FALSE;
		}
		...
		if (zend_list_find(Z_RESVAL_P(arg), &type) && type == le_trans) {
			...
			if (ib_link->tr_list == NULL || ib_link->tr_list->trans == NULL) {
				/* this link doesn't have a default transaction */
				_jonj_ibase_module_error("Link has no default transaction" TSRMLS_CC);
				RETURN_FALSE;
			}
			trans = ib_link->tr_list->trans;
		}
...
{
	_jonj_ibase_trans_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, COMMIT);
}
/* }}} */
...
{
	_jonj_ibase_trans_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, ROLLBACK);
}
/* }}} */
...
{
	_jonj_ibase_trans_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, COMMIT | RETAIN);
}
/* }}} */
...
{
	_jonj_ibase_trans_end(INTERNAL_FUNCTION_PARAM_PASSTHRU, ROLLBACK | RETAIN);
}
/* }}} */
...
JONJ_FUNCTION(ibase_gen_id)
{
	...
	if (gen_len > 31) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid generator name");
		RETURN_FALSE;
	}

	JONJ_IBASE_LINK_TRANS(link, ib_link, trans);
