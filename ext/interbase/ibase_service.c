...
static void _jonj_ibase_free_service(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
...
#define IBASE_SVC_ERROR(svm) \
	do { zend_list_delete(svm->res_id); _jonj_ibase_error(TSRMLS_C); } while (0)
	

void jonj_ibase_service_minit(INIT_FUNC_ARGS) /* {{{ */
{
	le_service = zend_register_list_destructors_ex(_jonj_ibase_free_service, NULL, 
...
static void _jonj_ibase_user(INTERNAL_FUNCTION_PARAMETERS, char operation) /* {{{ */
{
	...
	for (i = 0; i < sizeof(user_flags); ++i) {
		if (args[i] != NULL) {
			...
			if ((spb_len + chunk) > sizeof(buf) || chunk <= 0) {
				_jonj_ibase_module_error("Internal error: insufficient buffer space for SPB (%d)"
					TSRMLS_CC, spb_len);
				RETURN_FALSE;
			}
			spb_len += chunk;
...
{
	_jonj_ibase_user(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_add_user);
}
/* }}} */
...
{
	_jonj_ibase_user(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_modify_user);
}
/* }}} */
...
{
	_jonj_ibase_user(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_delete_user);
}
/* }}} */
...
JONJ_FUNCTION(ibase_service_attach)
{
	...
	if (spb_len > sizeof(buf) || spb_len == -1) {
		_jonj_ibase_module_error("Internal error: insufficient buffer space for SPB (%d)" TSRMLS_CC, spb_len);
		RETURN_FALSE;
	}
...
static void _jonj_ibase_service_query(INTERNAL_FUNCTION_PARAMETERS, /* {{{ */
...
static void _jonj_ibase_backup_restore(INTERNAL_FUNCTION_PARAMETERS, char operation) /* {{{ */
{
	/**
	...
	if (!verbose) {
		...
		_jonj_ibase_service_query(INTERNAL_FUNCTION_PARAM_PASSTHRU, svm, isc_info_svc_line);
	}
}
/* }}} */
...
{
	_jonj_ibase_backup_restore(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_backup);
}
/* }}} */
...
{
	_jonj_ibase_backup_restore(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_restore);
}
/* }}} */

static void _jonj_ibase_service_action(INTERNAL_FUNCTION_PARAMETERS, char svc_action) /* {{{ */
{
	zval *res;
	char buf[128], *db;
	int dblen, spb_len;
	...
	if (svc_action == isc_action_svc_db_stats) {
		...
		switch (action) {
			default:
unknown_option:
				_jonj_ibase_module_error("Unrecognised option (%ld)" TSRMLS_CC, action);
				RETURN_FALSE;
...
{
	_jonj_ibase_service_action(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_properties);
}
/* }}} */
...
{
	_jonj_ibase_service_action(INTERNAL_FUNCTION_PARAM_PASSTHRU, isc_action_svc_db_stats);
}
/* }}} */
...
JONJ_FUNCTION(ibase_server_info)
{
	...
	_jonj_ibase_service_query(INTERNAL_FUNCTION_PARAM_PASSTHRU, svm, (char)action);
}
/* }}} */

#else

void jonj_ibase_register_service_constants(INIT_FUNC_ARGS) { /* nop */ }
