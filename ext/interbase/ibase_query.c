...
static void _jonj_ibase_free_xsqlda(XSQLDA *sqlda) /* {{{ */
{
	int i;
	XSQLVAR *var;
...
static void _jonj_ibase_free_stmt_handle(ibase_db_link *link, isc_stmt_handle stmt TSRMLS_DC) /* {{{ */
{
...
static void _jonj_ibase_free_result(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_result *ib_result = (ibase_result *) rsrc->ptr;

	IBDEBUG("Freeing result by dtor...");
	if (ib_result) {
		_jonj_ibase_free_xsqlda(ib_result->out_sqlda);
		if (ib_result->query != NULL) {
			...
			_jonj_ibase_free_stmt_handle(ib_result->link, ib_result->stmt TSRMLS_CC);
		}
		efree(ib_result);
	}
}
...
static void _jonj_ibase_free_query(ibase_query *ib_query TSRMLS_DC) /* {{{ */
{
	...
	if (ib_query->result != NULL) {
		...
		_jonj_ibase_free_stmt_handle(ib_query->link, ib_query->stmt TSRMLS_CC);
	}
...
static void jonj_ibase_free_query_rsrc(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	ibase_query *ib_query = (ibase_query *)rsrc->ptr;

	if (ib_query != NULL) {
		IBDEBUG("Preparing to free query by dtor...");
		_jonj_ibase_free_query(ib_query TSRMLS_CC);
		efree(ib_query);
	}
}
/* }}} */

void jonj_ibase_query_minit(INIT_FUNC_ARGS) /* {{{ */
{
	le_result = zend_register_list_destructors_ex(_jonj_ibase_free_result, NULL, 
	    "interbase result", module_number);
	le_query = zend_register_list_destructors_ex(jonj_ibase_free_query_rsrc, NULL, 
	    "interbase query", module_number);
}
/* }}} */

static int _jonj_ibase_alloc_array(ibase_array **ib_arrayp, XSQLDA *sqlda, /* {{{ */
		...
		if ((var->sqltype & ~1) == SQL_ARRAY) {
			...
			switch (ar_desc->array_desc_dtype) {
				...
				case blr_cstring2:
					 ...
					 */
				default:
					_jonj_ibase_module_error("Unsupported array type %d in relation '%s' column '%s'"
...
static int _jonj_ibase_alloc_query(ibase_query *ib_query, ibase_db_link *link, /* {{{ */
	ibase_trans *trans, char *query, unsigned short dialect, int trans_res_id TSRMLS_DC)
{
	...
	if (*query == '\0') {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Querystring empty.");
		return FAILURE;
	}
	...
	if (isc_dsql_allocate_statement(IB_STATUS, &link->handle, &ib_query->stmt)) {
		_jonj_ibase_error(TSRMLS_C);
		goto _jonj_ibase_alloc_query_error;
	}
	...
	if (ib_query->in_sqlda->sqld == 0) {
		efree(ib_query->in_sqlda);
		ib_query->in_sqlda = NULL;
	} else if (FAILURE == _jonj_ibase_alloc_array(&ib_query->in_array, ib_query->in_sqlda,
	...
	if (ib_query->out_sqlda->sqld == 0) {
		efree(ib_query->out_sqlda);
		ib_query->out_sqlda = NULL;
	} else 	if (FAILURE == _jonj_ibase_alloc_array(&ib_query->out_array, ib_query->out_sqlda,
...
static int _jonj_ibase_bind_array(zval *val, char *buf, unsigned long buf_size, /* {{{ */
	ibase_array *array, int dim TSRMLS_DC)
{
	...
	if (dim < array->ar_desc.array_desc_dimensions) {
		...
		for (i = 0; i < dim_len; ++i) { 
			...
			if (_jonj_ibase_bind_array(*subval, buf, slice_size, array, dim+1 TSRMLS_CC) == FAILURE)
			{
				return FAILURE;
			}
			buf += slice_size;
		...
		if (Z_TYPE_P(val) == IS_NULL) {
			...
			switch (array->el_type) {
				case SQL_SHORT:
					if (l > SHRT_MAX || l < SHRT_MIN) {
						_jonj_ibase_module_error("Array parameter exceeds field width" TSRMLS_CC);
						return FAILURE;
					}
					*(short*) buf = (short) l;
					break;
				...
				case SQL_INT64:
					{
						...
						if (!sscanf(Z_STRVAL_P(val), "%Lf", &l)) {
							_jonj_ibase_module_error("Cannot convert '%s' to long double"
			...
			switch (array->el_type) {
				...
				case SQL_INT64:
					...
					*(long *) buf = Z_LVAL_P(val);
					...
					if (!sscanf(Z_STRVAL_P(val), "%" LL_MASK "d", &l)) {
						_jonj_ibase_module_error("Cannot convert '%s' to long integer"
							TSRMLS_CC, Z_STRVAL_P(val));
						return FAILURE;
					} else {
						*(ISC_INT64 *) buf = l;
				...
				case SQL_TIMESTAMP:
					...
					if (n != 3 && n != 6) {
						_jonj_ibase_module_error("Invalid date/time format (expected 3 or 6 fields, got %d."
				...
				case SQL_TYPE_DATE:
					...
					if (n != 3) {
						_jonj_ibase_module_error("Invalid date format (expected 3 fields, got %d. "
				...
				case SQL_TYPE_TIME:
					...
					if (n != 3) {
						_jonj_ibase_module_error("Invalid time format (expected 3 fields, got %d. "
...
static int _jonj_ibase_bind(XSQLDA *sqlda, zval ***b_vars, BIND_BUF *buf, /* {{{ */
		...
		switch (var->sqltype & ~1) {
			...
			case SQL_TYPE_TIME:
				if (Z_TYPE_P(b_var) == IS_LONG) {
					struct tm *res;
					res = jonj_gmtime_r(&Z_LVAL_P(b_var), &t);
					if (!res) {
						return FAILURE;
					}
				} else {
			...
			case SQL_BLOB:
				...
				if (Z_STRLEN_P(b_var) != BLOB_ID_LEN ||
					!_jonj_ibase_string_to_quad(Z_STRVAL_P(b_var), &buf[i].val.qval)) {
					...
					if (_jonj_ibase_blob_add(&b_var, &ib_blob TSRMLS_CC) != SUCCESS) {
						return FAILURE;
					}
			...
			case SQL_ARRAY:
				...
				if (Z_TYPE_P(b_var) != IS_ARRAY) {
					...
					if (Z_STRLEN_P(b_var) != BLOB_ID_LEN ||
						...
						_jonj_ibase_module_error("Parameter %d: invalid array ID" TSRMLS_CC,i+1);
						rv = FAILURE;
					}
				} else {
					...
					if (FAILURE == _jonj_ibase_bind_array(b_var, array_data, ar->ar_size, 
							ar, 0 TSRMLS_CC)) {
						_jonj_ibase_module_error("Parameter %d: failed to bind array argument"
							TSRMLS_CC,i+1);
						efree(array_data);
						rv = FAILURE;
						continue;
...
static void _jonj_ibase_alloc_xsqlda(XSQLDA *sqlda) /* {{{ */
{
	int i;
...
static int _jonj_ibase_exec(INTERNAL_FUNCTION_PARAMETERS, ibase_result **ib_resultp, /* {{{ */
	ibase_query *ib_query, zval ***args)
{
	...
	if (ib_query->out_sqlda) { /* output variables in select, select for update */
		...
		_jonj_ibase_alloc_xsqlda(out_sqlda);
	...
	if (ib_query->in_sqlda) { /* has placeholders */
		...
		if (_jonj_ibase_bind(in_sqlda, args, bind_buf, ib_query TSRMLS_CC) == FAILURE) {
...
_jonj_ibase_exec_error:
	...
	if (rv == FAILURE) {
		...
		if (out_sqlda) {
			_jonj_ibase_free_xsqlda(out_sqlda);
		}
	}
...
JONJ_FUNCTION(ibase_query)
{
	...
	switch (ZEND_NUM_ARGS()) {
		...
		case 2:
			if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, 2 TSRMLS_CC, "rs",
				...
				_jonj_ibase_get_link_trans(INTERNAL_FUNCTION_PARAM_PASSTHRU, &zlink, &ib_link, &trans);
			...
			if (SUCCESS == zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS()
					TSRMLS_CC, "ls", &l, &query, &query_len) && l == JONJ_IBASE_CREATE) {
				isc_db_handle db = NULL;
				isc_tr_handle trans = NULL;

				if (PG(sql_safe_mode)) {
					_jonj_ibase_module_error("CREATE DATABASE is not allowed in SQL safe mode"
						TSRMLS_CC);

				} else if (((l = INI_INT("ibase.max_links")) != -1) && (IBG(num_links) >= l)) {
					_jonj_ibase_module_error("CREATE DATABASE is not allowed: maximum link count "
					...
					_jonj_ibase_module_error("Connection to created database could not be "
	...
	if (ib_link == NULL || FAILURE == _jonj_ibase_def_trans(ib_link, &trans TSRMLS_CC) 
			|| FAILURE == _jonj_ibase_alloc_query(&ib_query, ib_link, trans, query, ib_link->dialect,
	...
	do {
		...
		if (bind_n != expected_n) {
			jonj_error_docref(NULL TSRMLS_CC, (bind_n < expected_n) ? E_WARNING : E_NOTICE,
		...
		if (FAILURE == _jonj_ibase_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, &result, &ib_query, 
				&bind_args[bind_i])) {
			break;
		}
...
JONJ_FUNCTION(ibase_affected_rows)
{
	...
	if (!arg) {
		...
		_jonj_ibase_get_link_trans(INTERNAL_FUNCTION_PARAM_PASSTHRU, &arg, &ib_link, &trans);
		if (trans == NULL) {
...
static int _jonj_ibase_var_zval(zval *val, void *data, int type, int len, /* {{{ */
...
static int _jonj_ibase_arr_zval(zval *ar_zval, char *data, unsigned long data_size, /* {{{ */
	ibase_array *ib_array, int dim, int flag TSRMLS_DC)
{
	...
	if (dim < ib_array->ar_desc.array_desc_dimensions) { /* array again */
		...
		for (i = 0; i < dim_len; ++i) {
			...
			/* recursion here */
			if (FAILURE == _jonj_ibase_arr_zval(slice_zval, data, slice_size, ib_array, dim + 1,
					flag TSRMLS_CC)) {
				return FAILURE;
			}
			data += slice_size;
		...
		if (FAILURE == _jonj_ibase_var_zval(ar_zval, data, ib_array->el_type,
...
static void _jonj_ibase_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, int fetch_type) /* {{{ */
{
	zval *result_arg;
	...
	for (i = 0; i < ib_result->out_sqlda->sqld; ++i) {
		...
		if (((var->sqltype & 1) == 0) || *var->sqlind != -1) {
			...
			switch (var->sqltype & ~1) {
				...
				default:
					_jonj_ibase_var_zval(result, var->sqldata, var->sqltype, var->sqllen,
						var->sqlscale, flag TSRMLS_CC);
					break;
				case SQL_BLOB:
					if (flag & JONJ_IBASE_FETCH_BLOBS) { /* fetch blob contents into hash */
						...
						for (i = 0; i < sizeof(bl_info); ) {
							...
							if (item == isc_info_end || item == isc_info_truncated || 
								...
								_jonj_ibase_module_error("Could not determine BLOB size (internal error)"
									TSRMLS_CC);
								goto _jonj_ibase_fetch_error;
							}								
						...
						} else if (SUCCESS != _jonj_ibase_blob_get(result, &blob_handle, 
								max_len TSRMLS_CC)) {
							goto _jonj_ibase_fetch_error;
						}
						...
						ZVAL_STRINGL(result,_jonj_ibase_quad_to_string(bl_qd), BLOB_ID_LEN, 0);
					}
					break;
				case SQL_ARRAY:
					if (flag & JONJ_IBASE_FETCH_ARRAYS) { /* array can be *huge* so only fetch if asked */
						...
						if (FAILURE == _jonj_ibase_arr_zval(result, ar_data, ib_array->ar_size, ib_array,
								0, flag TSRMLS_CC)) {
							efree(ar_data);
							goto _jonj_ibase_fetch_error;
						}
						...
						ISC_QUAD ar_qd = *(ISC_QUAD *) var->sqldata;
						ZVAL_STRINGL(result,_jonj_ibase_quad_to_string(ar_qd), BLOB_ID_LEN, 0);
					}
					break;
				_jonj_ibase_fetch_error:
					zval_dtor(result);
					FREE_ZVAL(result);
					RETURN_FALSE;
			} /* switch */
...
{
	_jonj_ibase_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, FETCH_ROW);
}
/* }}} */
...
{
	_jonj_ibase_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, FETCH_ARRAY);
}
/* }}} */
...
JONJ_FUNCTION(ibase_prepare)
{
	...
	if (ZEND_NUM_ARGS() == 1) {
		...
		_jonj_ibase_get_link_trans(INTERNAL_FUNCTION_PARAM_PASSTHRU, &link_arg, &ib_link, &trans);
	...
	if (FAILURE == _jonj_ibase_def_trans(ib_link, &trans TSRMLS_CC)) {
		RETURN_FALSE;
	}

	ib_query = (ibase_query *) emalloc(sizeof(ibase_query));

	if (FAILURE == _jonj_ibase_alloc_query(ib_query, ib_link, trans, query, ib_link->dialect, trans_res_id TSRMLS_CC)) {
		efree(ib_query);
		RETURN_FALSE;
	}
...
static void _jonj_ibase_field_info(zval *return_value, XSQLVAR *var) /* {{{ */
{
	unsigned short len;
	char buf[16], *s = buf;
...
JONJ_FUNCTION(ibase_field_info)
{
	...
	if (sqlda == NULL) {
		_jonj_ibase_module_error("Trying to get field info from a non-select query" TSRMLS_CC);
		RETURN_FALSE;
	}
	...
	_jonj_ibase_field_info(return_value, sqlda->sqlvar + field_arg);
}
/* }}} */
...
JONJ_FUNCTION(ibase_param_info)
{
	...
	_jonj_ibase_field_info(return_value,ib_query->in_sqlda->sqlvar + field_arg);
}
/* }}} */
