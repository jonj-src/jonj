...
jonj_oci_descriptor *jonj_oci_lob_create (jonj_oci_connection *connection, long type TSRMLS_DC)
{
	...
	switch (type) {
		...
		case OCI_DTYPE_ROWID:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown descriptor type %ld", type);
			return NULL;
			break;
	}

	descriptor = ecalloc(1, sizeof(jonj_oci_descriptor));
	descriptor->type = type;
	descriptor->connection = connection;
	zend_list_addref(descriptor->connection->id);

	JONJ_OCI_CALL_RETURN(errstatus, OCIDescriptorAlloc, (connection->env, (dvoid*)&(descriptor->descriptor), descriptor->type, (size_t) 0, (dvoid **) 0));

	if (errstatus != OCI_SUCCESS) {
		OCI_G(errcode) = jonj_oci_error(OCI_G(err), errstatus TSRMLS_CC);
		JONJ_OCI_HANDLE_ERROR(connection, OCI_G(errcode));
		efree(descriptor);
		return NULL;
	} else {
	...
	JONJ_OCI_REGISTER_RESOURCE(descriptor, le_descriptor);
	
	descriptor->lob_current_position = 0;
	descriptor->lob_size = -1;				/* we should set it to -1 to know, that it's just not initialized */
	descriptor->buffering = JONJ_OCI_LOB_BUFFER_DISABLED;				/* buffering is off by default */
	...
	if (descriptor->type == OCI_DTYPE_LOB || descriptor->type == OCI_DTYPE_FILE) {
		...
		if (!connection->descriptors) {
			...
			zend_hash_init(connection->descriptors, 0, NULL, jonj_oci_descriptor_flush_hash_dtor, 0);
			connection->descriptor_count = 0;
		}
		
		descriptor->index = (connection->descriptor_count)++;
		if (connection->descriptor_count == LONG_MAX) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Internal descriptor counter has reached limit");
			jonj_oci_connection_descriptors_free(connection TSRMLS_CC);
			return NULL;
		}

		zend_hash_index_update(connection->descriptors,descriptor->index,&descriptor,sizeof(jonj_oci_descriptor *),NULL);
	}
	return descriptor;
...
{
	jonj_oci_connection *connection = descriptor->connection;
	sword errstatus;
	...
	if (descriptor->lob_size >= 0) {
		*length = descriptor->lob_size;
		...
		if (descriptor->type == OCI_DTYPE_FILE) {
			JONJ_OCI_CALL_RETURN(errstatus, OCILobFileOpen, (connection->svc, connection->err, descriptor->descriptor, OCI_FILE_READONLY));
			if (errstatus != OCI_SUCCESS) {
		...
		JONJ_OCI_CALL_RETURN(errstatus, OCILobGetLength, (connection->svc, connection->err, descriptor->descriptor, (ub4 *)length));
		...
		if (descriptor->type == OCI_DTYPE_FILE) {
			JONJ_OCI_CALL_RETURN(errstatus, OCILobFileClose, (connection->svc, connection->err, descriptor->descriptor));
...
{
	ub4 lenp = (ub4) len;
	jonj_oci_lob_ctx *ctx = (jonj_oci_lob_ctx *)ctxp;
	...
	switch (piece)
	{
		...
		default: {
			TSRMLS_FETCH();
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unexpected LOB piece id received (value:%d)", piece);
			*(ctx->lob_data) = NULL;
			*(ctx->lob_len) = 0;
			return OCI_ERROR;
		}
...
/* {{{ jonj_oci_lob_calculate_buffer() 
   Work out the size for LOB buffering */
static inline int jonj_oci_lob_calculate_buffer(jonj_oci_descriptor *descriptor, long read_length TSRMLS_DC)
{
	jonj_oci_connection *connection = descriptor->connection;
	ub4 chunk_size;
	sword errstatus;
	...
	if (!descriptor->chunk_size) {
		JONJ_OCI_CALL_RETURN(errstatus, OCILobGetChunkSize, (connection->svc, connection->err, descriptor->descriptor, &chunk_size));
...
int jonj_oci_lob_read (jonj_oci_descriptor *descriptor, long read_length, long initial_offset, char **data, ub4 *data_len TSRMLS_DC)
{
	jonj_oci_connection *connection = descriptor->connection;
	ub4 length = 0;
	int buffer_size = JONJ_OCI_LOB_BUFFER_SIZE;
	jonj_oci_lob_ctx ctx;
	ub1 *bufp;
	...
	*data = NULL;
	...
	if (jonj_oci_lob_get_length(descriptor, &length TSRMLS_CC)) {
		return 1;
	}
	...
	if (initial_offset > length) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset must be less than size of the LOB");
		return 1;
	}
	...
	if (descriptor->type == OCI_DTYPE_FILE) {
		...
		JONJ_OCI_CALL_RETURN(errstatus, OCILobCharSetId, (connection->env, connection->err, descriptor->descriptor, &charset_id));
	...
	if (is_clob) {
		JONJ_OCI_CALL_RETURN(errstatus, OCINlsNumericInfoGet, (connection->env, connection->err, &bytes_per_char, OCI_NLS_CHARSET_MAXBYTESZ));
	...
	*data = ecalloc(bytes_per_char, requested_len + 1);
	...
	buffer_size = jonj_oci_lob_calculate_buffer(descriptor, buffer_size TSRMLS_CC);	/* use chunk size */

	bufp = (ub1 *) ecalloc(1, buffer_size);
	JONJ_OCI_CALL_RETURN(errstatus, OCILobRead2,
		(
			connection->svc,
			connection->err,
			...
			OCI_FIRST_PIECE,
			(dvoid *)&ctx,
			(OCICallbackLobRead2) jonj_oci_lob_callback,				/* callback... */
...
int jonj_oci_lob_write (jonj_oci_descriptor *descriptor, ub4 offset, char *data, int data_len, ub4 *bytes_written TSRMLS_DC)
{
	...
	jonj_oci_connection *connection = (jonj_oci_connection *) descriptor->connection;
	ub4 lob_length;
	sword errstatus;
	...
	*bytes_written = 0;
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobWrite,
			(
				connection->svc,
				connection->err,
				lob,
	...
	*bytes_written = data_len;
	...
	if (descriptor->buffering == JONJ_OCI_LOB_BUFFER_ENABLED) {
		descriptor->buffering = JONJ_OCI_LOB_BUFFER_USED;
	}
...
int jonj_oci_lob_set_buffering (jonj_oci_descriptor *descriptor, int on_off TSRMLS_DC)
{
	...
	if (!on_off && descriptor->buffering == JONJ_OCI_LOB_BUFFER_DISABLED) {
	...
	if (on_off) {
		JONJ_OCI_CALL_RETURN(errstatus, OCILobEnableBuffering, (connection->svc, connection->err, descriptor->descriptor));
	} else {
		JONJ_OCI_CALL_RETURN(errstatus, OCILobDisableBuffering, (connection->svc, connection->err, descriptor->descriptor));
	}
	...
	descriptor->buffering = on_off ? JONJ_OCI_LOB_BUFFER_ENABLED : JONJ_OCI_LOB_BUFFER_DISABLED;
	connection->errcode = 0; /* retain backwards compat with OCI8 1.4 */
	return 0;
}
/* }}} */

/* {{{ jonj_oci_lob_get_buffering()
...
int jonj_oci_lob_copy (jonj_oci_descriptor *descriptor_dest, jonj_oci_descriptor *descriptor_from, long length TSRMLS_DC)
{
	jonj_oci_connection *connection = descriptor_dest->connection;
	ub4 length_dest, length_from, copy_len;
	sword errstatus;
	
	if (jonj_oci_lob_get_length(descriptor_dest, &length_dest TSRMLS_CC)) {
		return 1;
	}
	
	if (jonj_oci_lob_get_length(descriptor_from, &length_from TSRMLS_CC)) {
		return 1;
	}
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobCopy,
			(
			 connection->svc,
			 connection->err,
...
int jonj_oci_lob_close (jonj_oci_descriptor *descriptor TSRMLS_DC)
{
	...
	if (descriptor->is_open) {
		JONJ_OCI_CALL_RETURN(errstatus, OCILobClose, (connection->svc, connection->err, descriptor->descriptor));
	...
	if (jonj_oci_temp_lob_close(descriptor TSRMLS_CC)) {
		return 1;
	}
...
int jonj_oci_temp_lob_close (jonj_oci_descriptor *descriptor TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobIsTemporary, (connection->env,connection->err, descriptor->descriptor, &is_temporary));
	...
	if (is_temporary) {
		JONJ_OCI_CALL_RETURN(errstatus, OCILobFreeTemporary, (connection->svc, connection->err, descriptor->descriptor));
...
int jonj_oci_lob_flush(jonj_oci_descriptor *descriptor, long flush_flag TSRMLS_DC)
{
	...
	switch (flush_flag) {
		...
		case OCI_LOB_BUFFER_FREE:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid flag value: %ld", flush_flag);
			return 1;
			break;
	}
	 ...
	 * */
	if (descriptor->buffering != JONJ_OCI_LOB_BUFFER_USED) {
		return 0;
	}

	JONJ_OCI_CALL_RETURN(errstatus, OCILobFlushBuffer, (connection->svc, connection->err, lob, flush_flag));
...
void jonj_oci_lob_free (jonj_oci_descriptor *descriptor TSRMLS_DC)
{
	...
	if ((descriptor->type == OCI_DTYPE_FILE || descriptor->type == OCI_DTYPE_LOB) && descriptor->buffering == JONJ_OCI_LOB_BUFFER_USED) {
	...
	JONJ_OCI_CALL(OCIDescriptorFree, (descriptor->descriptor, descriptor->type));
...
#else
	if ((PG(safe_mode) && (!jonj_checkuid(filename, NULL, CHECKUID_CHECK_FILE_AND_DIR))) || jonj_check_open_basedir(filename TSRMLS_CC)) {
#endif
		return 1;
	}
...
int jonj_oci_lob_append (jonj_oci_descriptor *descriptor_dest, jonj_oci_descriptor *descriptor_from TSRMLS_DC)
{
	...
	if (jonj_oci_lob_get_length(descriptor_dest, &dest_len TSRMLS_CC)) {
		return 1;
	}
	
	if (jonj_oci_lob_get_length(descriptor_from, &from_len TSRMLS_CC)) {
		return 1;
	}
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobAppend, (connection->svc, connection->err, lob_dest, lob_from));
...
int jonj_oci_lob_truncate (jonj_oci_descriptor *descriptor, long new_lob_length TSRMLS_DC)
{
	...
	if (new_lob_length < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Size must be greater than or equal to 0");
		return 1;
	}

	if (new_lob_length > lob_length) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Size must be less than or equal to the current LOB size");
		return 1;
	}
	
	JONJ_OCI_CALL_RETURN(errstatus, OCILobTrim, (connection->svc, connection->err, lob, new_lob_length));
...
int jonj_oci_lob_erase (jonj_oci_descriptor *descriptor, long offset, ub4 length, ub4 *bytes_erased TSRMLS_DC)
{
	...
	*bytes_erased = 0;
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobErase, (connection->svc, connection->err, lob, (ub4 *)&length, offset+1));
...
{
	jonj_oci_connection *connection = descriptor_first->connection;
	OCILobLocator *first_lob   = descriptor_first->descriptor;
	OCILobLocator *second_lob  = descriptor_second->descriptor;
	sword errstatus;

	JONJ_OCI_CALL_RETURN(errstatus, OCILobIsEqual, (connection->env, first_lob, second_lob, result));
...
int jonj_oci_lob_write_tmp (jonj_oci_descriptor *descriptor, long type, char *data, int data_len TSRMLS_DC)
{
	...
	switch (type) {
		...
		case OCI_TEMP_CLOB:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid temporary lob type: %ld", type);
			return 1;
			break;
	}
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobCreateTemporary,
			(
			 connection->svc,
			 connection->err,
			 lob,
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCILobOpen, (connection->svc, connection->err, lob, OCI_LOB_READWRITE));
	...
	return jonj_oci_lob_write(descriptor, 0, data, data_len, &bytes_written TSRMLS_CC);
}
/* }}} */
