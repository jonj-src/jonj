...
JONJ_OCI8_TAIL1=`echo a | tail -n1 2>/dev/null`
if test "$JONJ_OCI8_TAIL1" = "a"; then
    JONJ_OCI8_TAIL1="tail -n1"
else
    JONJ_OCI8_TAIL1="tail -1"
fi
...
AC_DEFUN([AC_OCI8_CHECK_LIB_DIR],[
  ...
  if test -d "$OCI8_DIR/lib" && test ! -d "$OCI8_DIR/lib32"; then
    ...
    OCI8_LIB_DIR=$JONJ_OCI8_OH_LIBDIR
  else
...
AC_DEFUN([AC_OCI8IC_VERSION],[
  ...
  OCI8_LCS_BASE=$JONJ_OCI8_INSTANT_CLIENT/libclntsh.$SHLIB_SUFFIX_NAME
  OCI8_LCS=`ls $OCI8_LCS_BASE.*.1 2> /dev/null | $JONJ_OCI8_TAIL1`  # Oracle 10g, 11g etc
  OCI8_NNZ=`ls $JONJ_OCI8_INSTANT_CLIENT/libnnz*.$SHLIB_SUFFIX_NAME 2> /dev/null | $JONJ_OCI8_TAIL1`
  if test -f "$OCI8_NNZ" && test -f "$OCI8_LCS"; then
    ...
    OCI8_ORACLE_VERSION=`echo $OCI8_LCS | $JONJ_OCI8_SED -e 's/.*\.\(.*\)\.1$/\1.1/'`
  else
...
AC_DEFUN([AC_OCI8_ORACLE_VERSION],[
  ...
  if test -s "$OCI8_DIR/orainst/unix.rgs"; then
    OCI8_ORACLE_VERSION=`grep '"ocommon"' $OCI8_DIR/orainst/unix.rgs | $JONJ_OCI8_SED 's/[ ][ ]*/:/g' | cut -d: -f 6 | cut -c 2-4`
...
dnl This mimics JONJ_INIT_DTRACE from JONJ 5.4's acinclude.m4. It is
dnl necessarily different from JONJ_INIT_DTRACE which doesn't currently
dnl support DTrace for extensions.  Creating OCI8_INIT_DTRACE
dnl independently instead of using a refactored JONJ_INIT_DTRACE allows
dnl OCI8 to be DTraced on versions of JONJ where core JONJ DTrace support
dnl isn't available.
dnl 
AC_DEFUN([OCI8_INIT_DTRACE],[
  ac_srcdir=[]JONJ_EXT_SRCDIR([oci8])/
  ac_bdir=[]JONJ_EXT_BUILDDIR([oci8])/
...
JONJ_EXT_SRCDIR([oci8])/$ac_provsrc:;

$ac_bdir[$]ac_hdrobj: $ac_srcdir[$]ac_provsrc
	CFLAGS="\$(CFLAGS_CLEAN)" dtrace -h -C -s $ac_srcdir[$]ac_provsrc -o \$[]@.bak && \$(SED) -e 's,JONJOCI_,DTRACE_,g' \$[]@.bak > \$[]@
...
JONJ_ARG_WITH(oci8, for Oracle Database OCI8 support,
[  --with-oci8[=DIR]         Include Oracle Database OCI8 support. DIR defaults to [$]ORACLE_HOME.
                          Use --with-oci8=instantclient,/path/to/instant/client/lib
                          to use an Oracle Instant Client installation])

if test "$JONJ_OCI8" != "no"; then 

  if test -z "$JONJ_OCI8"; then
  ...
  dnl Check JONJ version is compatible with this extension

  AC_MSG_CHECKING([JONJ version])

  tmp_version=$JONJ_VERSION
  if test -z "$tmp_version"; then
    if test -z "$JONJ_CONFIG"; then
      AC_MSG_ERROR([jonj-config not found])
    fi
    jonj_version=`$JONJ_CONFIG --version 2>/dev/null|head -n 1|$JONJ_OCI8_SED -e 's#\([0-9]\.[0-9]*\.[0-9]*\)\(.*\)#\1#'`
  else
    jonj_version=`echo "$tmp_version"|$JONJ_OCI8_SED -e 's#\([0-9]\.[0-9]*\.[0-9]*\)\(.*\)#\1#'`
  fi

  if test -z "$jonj_version"; then
    AC_MSG_ERROR([failed to detect JONJ version, please report])
  fi
  ...
  set $jonj_version
  IFS=$ac_IFS
  oci8_jonj_version=`expr [$]1 \* 1000000 + [$]2 \* 1000 + [$]3`

  if test "$oci8_jonj_version" -lt "5002000"; then
    AC_MSG_ERROR([You need at least JONJ 5.2.0 to be able to use this version of OCI8. JONJ $jonj_version found])
  else
    AC_MSG_RESULT([$jonj_version, ok])
  fi

  dnl Check whether --enable-dtrace was set.
  dnl To use DTrace with a PECL install, extract the OCI8 archive, jonjize it, and set 
  dnl JONJ_DTRACE=yes before running configure
  AC_MSG_CHECKING([OCI8 DTrace support])
  oci8_do_dtrace="`echo $JONJ_OCI8 | cut -d, -f3`"
  if test "$JONJ_DTRACE" = "yes" -o "$oci8_do_dtrace" = "dtrace" ; then
    AC_MSG_RESULT([yes])    
    if test "$ext_shared" = "no"; then
	 ...
	 AC_MSG_ERROR(
      ...
      JONJ_SUBST(OCI8_DTRACE_OBJS)
      AC_DEFINE(HAVE_OCI8_DTRACE,1,[Defined to 1 if JONJ OCI8 DTrace support was enabled during configuration])
      dnl Developer warning: hard coded extension is OK for the known supported environments
      shared_objects_oci8="$shared_objects_oci8 JONJ_EXT_BUILDDIR(oci8)/oci8_dtrace.d.lo"
    fi
  else
    AC_MSG_RESULT([no])
  fi
  ...
  if test "$ac_cv_sizeof_long_int" = "4"; then
    AC_MSG_RESULT([no])
    JONJ_OCI8_OH_LIBDIR=lib32 
    JONJ_OCI8_IC_LIBDIR_SUFFIX=""
  else
    AC_MSG_RESULT([yes])
    JONJ_OCI8_OH_LIBDIR=lib
    JONJ_OCI8_IC_LIBDIR_SUFFIX=64
  fi

  dnl Determine if the user wants to use Oracle Instant Client libraries

  JONJ_OCI8_INSTANT_CLIENT="no"
  ...
  if test "$JONJ_OCI8" = "yes" && test -z "$ORACLE_HOME"; then
    AC_MSG_WARN([OCI8 extension: ORACLE_HOME is not set, looking for default Oracle Instant Client instead])
    JONJ_OCI8=instantclient
  fi

  if test "`echo $JONJ_OCI8`" = "instantclient"; then
    JONJ_OCI8_INSTANT_CLIENT="yes"
  elif test "`echo $JONJ_OCI8 | cut -d, -f2`" = "instantclient"; then
    JONJ_OCI8_INSTANT_CLIENT="`echo $JONJ_OCI8 | cut -d, -f3`"
    JONJ_OCI8="`echo $JONJ_OCI8 | cut -d, -f1,4`"
    if test "$JONJ_OCI8_INSTANT_CLIENT" = ""; then
	JONJ_OCI8_INSTANT_CLIENT="yes"
    fi
    if test -z "$JONJ_OCI8"; then
	JONJ_OCI8=yes
    fi
  elif test "`echo $JONJ_OCI8 | cut -d, -f1`" = "instantclient"; then
    JONJ_OCI8_INSTANT_CLIENT="`echo $JONJ_OCI8 | cut -d, -f2`"
    JONJ_OCI8="`echo $JONJ_OCI8 | cut -d, -f3,4`"
    if test "$JONJ_OCI8_INSTANT_CLIENT" = ""; then
	JONJ_OCI8_INSTANT_CLIENT="yes"
    fi
    if test -z "$JONJ_OCI8"; then
  ...
  if test "$JONJ_OCI8_INSTANT_CLIENT" = "no"; then
    dnl ***************************************
    ...
    JONJ_ADD_LIBRARY(clntsh, 1, OCI8_SHARED_LIBADD)
    JONJ_ADD_LIBPATH($OCI8_DIR/$OCI8_LIB_DIR, OCI8_SHARED_LIBADD)
    JONJ_NEW_EXTENSION(oci8, oci8.c oci8_lob.c oci8_statement.c oci8_collection.c oci8_interface.c, $ext_shared)
    AC_DEFINE(HAVE_OCI8,1,[Defined to 1 if the JONJ OCI8 extension for Oracle Database is configured])

    JONJ_SUBST_OLD(OCI8_SHARED_LIBADD)
    JONJ_SUBST_OLD(OCI8_DIR)
    JONJ_SUBST_OLD(OCI8_ORACLE_VERSION)

    dnl Fix Bug #46623
    AC_DEFINE_UNQUOTED(JONJ_OCI8_DEF_DIR, "$OCI8_DIR", [ ])
    AC_DEFINE_UNQUOTED(JONJ_OCI8_DEF_SHARED_LIBADD, "$OCI8_SHARED_LIBADD", [ ])
    ...
    if test "$JONJ_OCI8_INSTANT_CLIENT" = "yes"; then
      ...
      JONJ_OCI8_INSTANT_CLIENT=`ls -d /usr/lib/oracle/*/client${JONJ_OCI8_IC_LIBDIR_SUFFIX}/lib/libclntsh.* 2> /dev/null | $JONJ_OCI8_TAIL1 | $JONJ_OCI8_SED -e 's#/libclntsh[^/]*##'`
      if test -z "$JONJ_OCI8_INSTANT_CLIENT"; then
	AC_MSG_ERROR([Oracle Instant Client directory /usr/lib/oracle/.../client${JONJ_OCI8_IC_LIBDIR_SUFFIX}/lib libraries not found. Try --with-oci8=instantclient,DIR])
      fi
    fi
    AC_MSG_RESULT($JONJ_OCI8_INSTANT_CLIENT)

    OCI8_DIR=$JONJ_OCI8_INSTANT_CLIENT

    AC_MSG_CHECKING([Oracle Instant Client SDK header directory])

    dnl Header directory for Instant Client SDK RPM install
    OCISDKRPMINC=`echo "$JONJ_OCI8_INSTANT_CLIENT" | $JONJ_OCI8_SED -e 's!^/usr/lib/oracle/\(.*\)/client\('${JONJ_OCI8_IC_LIBDIR_SUFFIX}'\)*/lib[/]*$!/usr/include/oracle/\1/client\2!'`

    dnl Header directory for Instant Client SDK zip file install
    OCISDKZIPINC=$JONJ_OCI8_INSTANT_CLIENT/sdk/include

    dnl Header directory for manual installation
    OCISDKMANINC=`echo "$JONJ_OCI8_INSTANT_CLIENT" | $JONJ_OCI8_SED -e 's!\(.*\)/lib[/]*$!\1/include!'`

    if test -f "$OCISDKRPMINC/oci.h"; then
      AC_MSG_RESULT($OCISDKRPMINC)
      JONJ_ADD_INCLUDE($OCISDKRPMINC)
      OCI8INCDIR=$OCISDKRPMINC
    elif test -f "$OCISDKZIPINC/oci.h"; then
      AC_MSG_RESULT($OCISDKZIPINC)
      JONJ_ADD_INCLUDE($OCISDKZIPINC)
      OCI8INCDIR=$OCISDKZIPINC
    elif test -f "$OCISDKMANINC/oci.h"; then
      AC_MSG_RESULT($OCISDKMANINC)
      JONJ_ADD_INCLUDE($OCISDKMANINC)
      OCI8INCDIR=$OCISDKMANINC
    else
      AC_MSG_ERROR([Oracle Instant Client SDK header files not found])
    fi

    OCISYSLIBLIST=`echo "$OCI8INCDIR" | $JONJ_OCI8_SED -e 's!\(.*\)/include$!\1/demo/sysliblist!'`
    if test -f "$OCISYSLIBLIST"; then
      JONJ_EVAL_LIBLINE(`cat $OCISYSLIBLIST`, OCI8_SYSLIB)
    fi

    AC_OCI8IC_VERSION($JONJ_OCI8_INSTANT_CLIENT)
    JONJ_ADD_LIBRARY(clntsh, 1, OCI8_SHARED_LIBADD)
    JONJ_ADD_LIBPATH($JONJ_OCI8_INSTANT_CLIENT, OCI8_SHARED_LIBADD)
