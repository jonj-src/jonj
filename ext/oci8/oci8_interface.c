...
/* {{{ proto bool oci_define_by_name(resource stmt, string name, mixed &var [, int type])
   Define a JONJ variable to an Oracle column by name */
...
JONJ_FUNCTION(oci_define_by_name)
{
	...
	int name_len;
	long type = 0;
	jonj_oci_statement *statement;
	jonj_oci_define *define, *tmp_define;
	...
	if (!name_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Column name cannot be empty");
		RETURN_FALSE;
	}

	JONJ_OCI_ZVAL_TO_STATEMENT(stmt, statement);

	if (statement->defines == NULL) {
		ALLOC_HASHTABLE(statement->defines);
		zend_hash_init(statement->defines, 13, NULL, jonj_oci_define_hash_dtor, 0);
	}

	define = ecalloc(1,sizeof(jonj_oci_define));

	if (zend_hash_add(statement->defines, name, name_len, define, sizeof(jonj_oci_define), (void **)&tmp_define) == SUCCESS) {
		efree(define);
		define = tmp_define;
	} else {
		efree(define);
...
/* {{{ proto bool oci_bind_by_name(resource stmt, string name, mixed &var, [, int maxlength [, int type]])
   Bind a JONJ variable to an Oracle placeholder by name */
...
JONJ_FUNCTION(oci_bind_by_name)
{
	...
	if (jonj_oci_bind_by_name(statement, name, name_len, bind_var, maxlen, bind_type TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
/* {{{ proto bool oci_bind_array_by_name(resource stmt, string name, array &var, int max_table_length [, int max_item_length [, int type ]])
   Bind a JONJ array to an Oracle PL/SQL type by name */
JONJ_FUNCTION(oci_bind_array_by_name)
{
	...
	if (max_array_len <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Maximum array length must be greater than zero");
		RETURN_FALSE;
	}
	
	if (jonj_oci_bind_array_by_name(statement, name, name_len, bind_var, max_array_len, max_item_len, type TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_free_descriptor)
{
	...
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor), "descriptor", sizeof("descriptor"), (void **)&tmp) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find descriptor property");
		RETURN_FALSE;
	}

	JONJ_OCI_ZVAL_TO_DESCRIPTOR(*tmp, descriptor);
...
JONJ_FUNCTION(oci_lob_save)
{
	...
	if (offset < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset parameter must be greater than or equal to 0");
		RETURN_FALSE;
	}
	
	if (jonj_oci_lob_write(descriptor, offset, data, data_len, &bytes_written TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_import)
{
	...
	if (getThis()) {
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION > 3) || (JONJ_MAJOR_VERSION > 5)
	...
	/* The "p" parsing parameter handles this case in JONJ 5.4+ */
	if (strlen(filename) != filename_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filename cannot contain null bytes");
		RETURN_FALSE;  
	}
#endif
	...
	if (jonj_oci_lob_import(descriptor, filename TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_load)
{
	...
	if (jonj_oci_lob_read(descriptor, -1, 0, &buffer, &buffer_len TSRMLS_CC)) {
		RETURN_FALSE;
	}
	if (buffer_len > 0) {
...
JONJ_FUNCTION(oci_lob_read)
{
	...
	if (jonj_oci_lob_read(descriptor, length, descriptor->lob_current_position, &buffer, &buffer_len TSRMLS_CC)) {
		RETURN_FALSE;
	}	
	if (buffer_len > 0) {
		RETURN_STRINGL(buffer, buffer_len, 0);
...
JONJ_FUNCTION(oci_lob_eof)
{
	...
	if (!jonj_oci_lob_get_length(descriptor, &lob_length TSRMLS_CC)) {
...
JONJ_FUNCTION(oci_lob_seek)
{
	zval **tmp, *z_descriptor = getThis();
	jonj_oci_descriptor *descriptor;
	long offset, whence = JONJ_OCI_SEEK_SET;
	ub4 lob_length;
	...
	switch(whence) {
		case JONJ_OCI_SEEK_CUR:
			descriptor->lob_current_position += offset;
			break;
		case JONJ_OCI_SEEK_END:
...
JONJ_FUNCTION(oci_lob_write)
{
	...
	if (jonj_oci_lob_write(descriptor, descriptor->lob_current_position, data, data_len, &bytes_written TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_lob_append)
{
	...
	jonj_oci_descriptor *descriptor_dest, *descriptor_from;
	...
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor_dest), "descriptor", sizeof("descriptor"), (void **)&tmp_dest) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find descriptor property. The first argument should be valid descriptor object");
		RETURN_FALSE;
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor_from), "descriptor", sizeof("descriptor"), (void **)&tmp_from) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find descriptor property. The second argument should be valid descriptor object");
		RETURN_FALSE;
	}
	
	JONJ_OCI_ZVAL_TO_DESCRIPTOR(*tmp_dest, descriptor_dest);
	JONJ_OCI_ZVAL_TO_DESCRIPTOR(*tmp_from, descriptor_from);
	
	if (jonj_oci_lob_append(descriptor_dest, descriptor_from TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_lob_truncate)
{
	...
	if (jonj_oci_lob_truncate(descriptor, ub_trim_length TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_erase)
{
	...
	if (getThis()) {
		...
		if (ZEND_NUM_ARGS() > 0 && offset < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset must be greater than or equal to 0");
			RETURN_FALSE;
		}

		if (ZEND_NUM_ARGS() > 1 && length < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length must be greater than or equal to 0");
			RETURN_FALSE;
		}
	}
	else {
	...
	if (jonj_oci_lob_erase(descriptor, offset, length, &bytes_erased TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_lob_flush)
{
	...
	if (descriptor->buffering == JONJ_OCI_LOB_BUFFER_DISABLED) {
		/* buffering wasn't enabled, there is nothing to flush */
		RETURN_FALSE;
	}

	if (jonj_oci_lob_flush(descriptor, flush_flag TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(ocisetbufferinglob)
{
	...
	if (jonj_oci_lob_set_buffering(descriptor, flag TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_copy)
{
	...
	if (jonj_oci_lob_copy(descriptor_dest, descriptor_from, length TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_is_equal)
{
	...
	jonj_oci_descriptor *descriptor_first, *descriptor_second;
	boolean is_equal;
	...
	JONJ_OCI_ZVAL_TO_DESCRIPTOR(*tmp_first, descriptor_first);
	JONJ_OCI_ZVAL_TO_DESCRIPTOR(*tmp_second, descriptor_second);

	if (jonj_oci_lob_is_equal(descriptor_first, descriptor_second, &is_equal TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
{	
	...
	if (getThis()) {
		...
		if (ZEND_NUM_ARGS() > 1 && start < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Start parameter must be greater than or equal to 0");
			RETURN_FALSE;
		}
		if (ZEND_NUM_ARGS() > 2 && length < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter must be greater than or equal to 0");
			RETURN_FALSE;
		}
	}
	else {
	...
	if (PG(safe_mode) && (!jonj_checkuid(filename, NULL, CHECKUID_CHECK_FILE_AND_DIR))) {
		RETURN_FALSE;
	}
#endif
...
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION > 3) || (JONJ_MAJOR_VERSION > 5)
	stream = jonj_stream_open_wrapper_ex(filename, "w", REPORT_ERRORS, NULL, NULL);
#else
	stream = jonj_stream_open_wrapper_ex(filename, "w", ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, NULL);
#endif

	block_length = JONJ_OCI_LOB_BUFFER_SIZE;
	...
	while(length > 0) {
		...
		if (jonj_oci_lob_read(descriptor, block_length, start, &buffer, &tmp_bytes_read TSRMLS_CC)) {
			jonj_stream_close(stream);
			RETURN_FALSE;
		}
		if (tmp_bytes_read && !jonj_stream_write(stream, buffer, tmp_bytes_read)) {
			jonj_stream_close(stream);
			efree(buffer);
			RETURN_FALSE;
		}
...
JONJ_FUNCTION(oci_lob_write_temporary)
{
	...
	if (jonj_oci_lob_write_tmp(descriptor, type, data, data_len TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_lob_close)
{
	...
	if (jonj_oci_lob_close(descriptor TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_new_descriptor)
{
	...
	JONJ_OCI_ZVAL_TO_CONNECTION(z_connection, connection);

	/* jonj_oci_lob_create() checks type */
	descriptor = jonj_oci_lob_create(connection, type TSRMLS_CC);	
...
JONJ_FUNCTION(oci_commit)
{
	...
	if (jonj_oci_connection_commit(connection TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_field_name)
{
	...
	if ( ( column = jonj_oci_statement_get_column_helper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0) ) ) {
...
JONJ_FUNCTION(oci_field_type)
{
	...
	column = jonj_oci_statement_get_column_helper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
...
JONJ_FUNCTION(oci_execute)
{
	...
	if (jonj_oci_statement_execute(statement, mode TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_cancel)
{
	...
	if (jonj_oci_statement_cancel(statement TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
{
	jonj_oci_fetch_row(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_OCI_NUM, 3);
}
/* }}} */
...
JONJ_FUNCTION(oci_fetch_all)
{
	...
	if (flags & JONJ_OCI_FETCHSTATEMENT_BY_ROW) {
		columns = safe_emalloc(statement->ncolumns, sizeof(jonj_oci_out_column *), 0);

		for (i = 0; i < statement->ncolumns; i++) {
			columns[ i ] = jonj_oci_statement_get_column(statement, i + 1, NULL, 0 TSRMLS_CC);
		}

		while (!jonj_oci_statement_fetch(statement, nrows TSRMLS_CC)) {
			zval *row;
			...
			for (i = 0; i < statement->ncolumns; i++) {
				...
				jonj_oci_column_to_zval(columns[ i ], element, JONJ_OCI_RETURN_LOBS TSRMLS_CC);

				if (flags & JONJ_OCI_NUM) {
			...
			if (maxrows != -1 && rows == maxrows) {
				jonj_oci_statement_cancel(statement TSRMLS_CC);
				break;
			}
		}
		efree(columns);
...
{
	jonj_oci_fetch_row(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_OCI_ASSOC | JONJ_OCI_RETURN_NULLS, 2);
...
{
	jonj_oci_fetch_row(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_OCI_NUM | JONJ_OCI_RETURN_NULLS, 1);
}
/* }}} */
...
{
	jonj_oci_fetch_row(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_OCI_ASSOC | JONJ_OCI_RETURN_NULLS, 1);
}
/* }}} */
...
{
	jonj_oci_fetch_row(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_OCI_BOTH | JONJ_OCI_RETURN_NULLS, 2);
}
/* }}} */
...
{
	jonj_oci_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 1);
}
/* }}} */
...
{
	jonj_oci_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */
...
{
	jonj_oci_do_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1, 0);
}
/* }}} */
...
JONJ_FUNCTION(oci_error)
{
	...
	if (ZEND_NUM_ARGS() > 0) {
		statement = (jonj_oci_statement *) zend_fetch_resource(&arg TSRMLS_CC, -1, NULL, NULL, 1, le_statement);
		if (statement) {
			errh = statement->err;
			errcode = statement->errcode;

			if (jonj_oci_fetch_sqltext_offset(statement, &sqltext, &error_offset TSRMLS_CC)) {
				RETURN_FALSE;
			}
			goto go_out;
		}

		connection = (jonj_oci_connection *) zend_fetch_resource(&arg TSRMLS_CC, -1, NULL, NULL, 1, le_connection);
		if (connection) {
			errh = connection->err;
		...
		connection = (jonj_oci_connection *) zend_fetch_resource(&arg TSRMLS_CC, -1, NULL, NULL, 1, le_pconnection);
		if (connection) {
			errh = connection->err;
			errcode = connection->errcode;
			goto go_out;
...
go_out:
	...
	if (!errh) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Oci_error: unable to find error handle");
		RETURN_FALSE;
	}

	errcode = jonj_oci_fetch_errmsg(errh, &errbuf TSRMLS_CC);
...
JONJ_FUNCTION(oci_parse)
{
	...
	statement = jonj_oci_statement_create(connection, query, query_len TSRMLS_CC);
...
JONJ_FUNCTION(oci_set_prefetch)
{
	...
	if (size < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of rows to be prefetched has to be greater than or equal to 0");
		return;
	}

	if (jonj_oci_statement_set_prefetch(statement, (ub4)size TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_set_client_identifier)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) client_id, (ub4) client_id_len, (ub4) OCI_ATTR_CLIENT_IDENTIFIER, connection->err));
	...
	*/
	...
	if (client_id) {
		/* this long winded copy allows compatibility with older JONJ versions */
...
JONJ_FUNCTION(oci_set_edition)
{
	...
	RETURN_TRUE;
#else
	jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Unsupported attribute type");
	RETURN_FALSE;
#endif
}
/* }}} */
...
JONJ_FUNCTION(oci_set_module_name)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) module, (ub4) module_len, (ub4) OCI_ATTR_MODULE, connection->err));
...
JONJ_FUNCTION(oci_set_action)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) action, (ub4) action_len, (ub4) OCI_ATTR_ACTION, connection->err));
...
JONJ_FUNCTION(oci_set_client_info)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) client_info, (ub4) client_info_len, (ub4) OCI_ATTR_CLIENT_INFO, connection->err));
...
JONJ_FUNCTION(oci_set_db_operation)
{
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) dbop_name, (ub4) dbop_name_len, (ub4) OCI_ATTR_DBOP, OCI_G(err)));
...
JONJ_FUNCTION(oci_password_change)
{
	...
	/* Safe mode has been removed in JONJ 5.4 */
	if (PG(safe_mode)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "is disabled in Safe Mode");
		RETURN_FALSE;
	}
#endif
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "rsss", &z_connection, &user, &user_len, &pass_old, &pass_old_len, &pass_new, &pass_new_len) == SUCCESS) {
		...
		if (!user_len) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "username cannot be empty");
			RETURN_FALSE;
		}
		if (!pass_old_len) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "old password cannot be empty");
			RETURN_FALSE;
		}
		if (!pass_new_len) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "new password cannot be empty");
			RETURN_FALSE;
		}

		if (jonj_oci_password_change(connection, user, user_len, pass_old, pass_old_len, pass_new, pass_new_len TSRMLS_CC)) {
			RETURN_FALSE;
		}
		RETURN_TRUE;
		...
		connection = jonj_oci_do_connect_ex(user, user_len, pass_old, pass_old_len, pass_new, pass_new_len, dbname, dbname_len, NULL, OCI_DEFAULT, 0, 0 TSRMLS_CC);
		if (!connection) {
			RETURN_FALSE;
		}
...
JONJ_FUNCTION(oci_new_cursor)
{
	...
	statement = jonj_oci_statement_create(connection, NULL, 0 TSRMLS_CC);
...
JONJ_FUNCTION(oci_result)
{
	...
	column = jonj_oci_statement_get_column_helper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
	if(column) {
		jonj_oci_column_to_zval(column, return_value, 0 TSRMLS_CC);
	}
	else {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_client_version)
{
	...
	jonj_oci_client_get_version(&version TSRMLS_CC);
...
JONJ_FUNCTION(oci_server_version)
{
	...
	if (jonj_oci_server_get_version(connection, &version TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_statement_type)
{
	...
	if (jonj_oci_statement_get_type(statement, &type TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_num_rows)
{
	...
	if (jonj_oci_statement_get_numrows(statement, &rowcount TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_free_collection)
{
	...
	if (zend_hash_find(Z_OBJPROP_P(z_collection), "collection", sizeof("collection"), (void **)&tmp) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find collection property");
		RETURN_FALSE;
	}
	
	JONJ_OCI_ZVAL_TO_COLLECTION(*tmp, collection);
...
JONJ_FUNCTION(oci_collection_append)
{
	...
	if (jonj_oci_collection_append(collection, value, value_len TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_collection_element_get)
{
	...
	if (jonj_oci_collection_element_get(collection, element_index, &value TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(oci_collection_assign)
{
	...
	jonj_oci_collection *collection_dest, *collection_from;
	...
	if (zend_hash_find(Z_OBJPROP_P(z_collection_dest), "collection", sizeof("collection"), (void **)&tmp_dest) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find collection property. The first argument should be valid collection object");
		RETURN_FALSE;
	}

	if (zend_hash_find(Z_OBJPROP_P(z_collection_from), "collection", sizeof("collection"), (void **)&tmp_from) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find collection property. The second argument should be valid collection object");
		RETURN_FALSE;
	}

	JONJ_OCI_ZVAL_TO_COLLECTION(*tmp_dest, collection_dest);
	JONJ_OCI_ZVAL_TO_COLLECTION(*tmp_from, collection_from);

	if (jonj_oci_collection_assign(collection_dest, collection_from TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_collection_element_assign)
{
	...
	if (jonj_oci_collection_element_set(collection, element_index, value, value_len TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(oci_collection_size)
{
	...
	if (jonj_oci_collection_size(collection, &size TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_LONG(size);
}
...
JONJ_FUNCTION(oci_collection_max)
{
	...
	if (jonj_oci_collection_max(collection, &max TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_LONG(max);
}
...
JONJ_FUNCTION(oci_collection_trim)
{
	...
	if (jonj_oci_collection_trim(collection, trim_size TSRMLS_CC)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;	
}
...
JONJ_FUNCTION(oci_new_collection)
{
	...
	if ( (collection = jonj_oci_collection_create(connection, tdo, tdo_len, schema, schema_len TSRMLS_CC)) ) {
...
JONJ_FUNCTION(oci_get_implicit_resultset)
{
	zval *z_statement;
	jonj_oci_statement *statement;
	jonj_oci_statement *imp_statement;
	...
	imp_statement = jonj_oci_get_implicit_resultset(statement TSRMLS_CC);

	if (imp_statement) {
		if (jonj_oci_statement_execute(imp_statement, (ub4)OCI_DEFAULT TSRMLS_CC))
			RETURN_FALSE;
