...
jonj_oci_statement *jonj_oci_statement_create(jonj_oci_connection *connection, char *query, int query_len TSRMLS_DC)
{
	...
	statement = ecalloc(1,sizeof(jonj_oci_statement));

	if (!query_len) {
		/* do not allocate stmt handle for refcursors, we'll get it from OCIStmtPrepare2() */
		JONJ_OCI_CALL(OCIHandleAlloc, (connection->env, (dvoid **)&(statement->stmt), OCI_HTYPE_STMT, 0, NULL));
	}
		
	JONJ_OCI_CALL(OCIHandleAlloc, (connection->env, (dvoid **)&(statement->err), OCI_HTYPE_ERROR, 0, NULL));
	
	if (query_len > 0) {
		JONJ_OCI_CALL_RETURN(errstatus, OCIStmtPrepare2,
				(
				 connection->svc,
				 &(statement->stmt),
				 connection->err,
		...
		if (errstatus != OCI_SUCCESS) {
			...
			JONJ_OCI_CALL(OCIStmtRelease, (statement->stmt, statement->err, NULL, 0, OCI_STRLS_CACHE_DELETE));
			JONJ_OCI_CALL(OCIHandleFree,(statement->err, OCI_HTYPE_ERROR));
	...
	statement->impres_flag = JONJ_OCI_IMPRES_UNKNOWN;  /* may or may not have Implicit Result Set children */
	zend_list_addref(statement->connection->id);

	if (OCI_G(default_prefetch) >= 0) {
		jonj_oci_statement_set_prefetch(statement, (ub4)OCI_G(default_prefetch) TSRMLS_CC);
	} else {
		jonj_oci_statement_set_prefetch(statement, (ub4)100 TSRMLS_CC); /* semi-arbitrary, "sensible default" */
	}
	
	JONJ_OCI_REGISTER_RESOURCE(statement, le_statement);
...
/* {{{ jonj_oci_get_implicit_resultset()
...
jonj_oci_statement *jonj_oci_get_implicit_resultset(jonj_oci_statement *statement TSRMLS_DC)
{
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Implicit results are available in Oracle Database 12c onwards");
	return NULL;
#else
	void *result;
	ub4   rtype;
	jonj_oci_statement *statement2;  /* implicit result set statement handle */
	sword errstatus;

	JONJ_OCI_CALL_RETURN(errstatus, OCIStmtGetNextResult, (statement->stmt, statement->err, &result, &rtype, OCI_DEFAULT));
	...
	if (rtype != OCI_RESULT_TYPE_SELECT) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unexpected implicit result type returned from Oracle Database");
		return NULL;
	} else {
		statement2 = ecalloc(1,sizeof(jonj_oci_statement));

		JONJ_OCI_CALL(OCIHandleAlloc, (statement->connection->env, (dvoid **)&(statement2->err), OCI_HTYPE_ERROR, 0, NULL));
		...
		statement2->impres_flag = JONJ_OCI_IMPRES_IS_CHILD;
		...
		jonj_oci_statement_set_prefetch(statement2, statement->prefetch_count TSRMLS_CC);
		
		JONJ_OCI_REGISTER_RESOURCE(statement2, le_statement);
...
/* {{{ jonj_oci_statement_set_prefetch()
...
int jonj_oci_statement_set_prefetch(jonj_oci_statement *statement, ub4 prefetch  TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, (statement->stmt, OCI_HTYPE_STMT, &prefetch, 0, OCI_ATTR_PREFETCH_ROWS, statement->err));
...
{
	jonj_oci_out_column *outcol = data;
...
int jonj_oci_statement_fetch(jonj_oci_statement *statement, ub4 nrows TSRMLS_DC)
{
	...
	if (statement->has_descr && statement->columns) {
		zend_hash_apply(statement->columns, (apply_func_t) jonj_oci_cleanup_pre_fetch TSRMLS_CC);
    }

	JONJ_OCI_CALL_RETURN(errstatus, OCIStmtFetch, (statement->stmt, statement->err, nrows, OCI_FETCH_NEXT, OCI_DEFAULT));
	...
	while (errstatus == OCI_NEED_DATA) {
		if (piecewisecols) {
			JONJ_OCI_CALL_RETURN(errstatus,
				OCIStmtGetPieceInfo,
				   (
					statement->stmt,
					statement->err,
			...
			for (i = 0; i < statement->ncolumns; i++) {
				...
				if (column && column->piecewise && handlepp == column->oci_define)   {
					if (!column->data) {
						column->data = (text *) ecalloc(1, JONJ_OCI_PIECE_SIZE + 1);
					} else {
						column->data = erealloc(column->data, column->retlen4 + JONJ_OCI_PIECE_SIZE + 1);
					}
					column->cb_retlen = JONJ_OCI_PIECE_SIZE;

					/* and instruct fetch to fetch waiting piece into our buffer */
					JONJ_OCI_CALL(OCIStmtSetPieceInfo,
						   (
	...
	if (errstatus == OCI_SUCCESS_WITH_INFO || errstatus == OCI_SUCCESS) {
		...
		for (i = 0; i < statement->ncolumns; i++) {
			...
			jonj_oci_column_to_zval(column, column->define->zval, 0 TSRMLS_CC);
		}
...
{
	jonj_oci_out_column *column = NULL;
	int i;
...
/* {{{ jonj_oci_define_callback() */
sb4 jonj_oci_define_callback(dvoid *ctx, OCIDefine *define, ub4 iter, dvoid **bufpp, ub4 **alenpp, ub1 *piecep, dvoid **indpp, ub2 **rcpp)
{
	jonj_oci_out_column *outcol = (jonj_oci_out_column *)ctx;
	TSRMLS_FETCH();

	if (!outcol) {
		
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid context pointer value");
		return OCI_ERROR;
	}
	
	switch(outcol->data_type) {
		case SQLT_RSET: {
				jonj_oci_statement *nested_stmt;

				nested_stmt = jonj_oci_statement_create(outcol->statement->connection, NULL, 0 TSRMLS_CC);
				if (!nested_stmt) {
					return OCI_ERROR;
				}
		...
		case SQLT_BFILE: {
				...
				descr = jonj_oci_lob_create(outcol->statement->connection, dtype TSRMLS_CC);
				if (!descr) {
					return OCI_ERROR;
				}
...
int jonj_oci_statement_execute(jonj_oci_statement *statement, ub4 mode TSRMLS_DC)
{
	...
	switch (mode) {
		...
		case OCI_DEFAULT:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid execute mode given: %d", mode);
			return 1;
			break;
	}
	...
	if (!statement->stmttype) {
		/* get statement type */
		JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (ub2 *)&statement->stmttype, (ub4 *)0, OCI_ATTR_STMT_TYPE, statement->err));
	...
	if (statement->last_query) { /* Don't execute REFCURSORS or Implicit Result Set handles */
		...
		if (statement->binds) {
			int result = 0;
			zend_hash_apply_with_argument(statement->binds, (apply_func_arg_t) jonj_oci_bind_pre_exec, (void *)&result TSRMLS_CC);
			if (result) {
				return 1;
			}
		}
		...
		/* execute statement */
		JONJ_OCI_CALL_RETURN(errstatus, OCIStmtExecute, (statement->connection->svc, statement->stmt, statement->err, iters, 0, NULL, NULL, mode));
		...
		if (statement->binds) {
			zend_hash_apply(statement->binds, (apply_func_t) jonj_oci_bind_post_exec TSRMLS_CC);
		}
	...
	if (statement->stmttype == OCI_STMT_SELECT && statement->executed == 0) {
		...
		zend_hash_init(statement->columns, 13, NULL, jonj_oci_column_hash_dtor, 0);
		
		counter = 1;

		/* get number of columns */
		JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (dvoid *)&colcount, (ub4 *)0, OCI_ATTR_PARAM_COUNT, statement->err));
		...
		for (counter = 1; counter <= colcount; counter++) {
			memset(&column,0,sizeof(jonj_oci_out_column));
			
			if (zend_hash_index_update(statement->columns, counter, &column, sizeof(jonj_oci_out_column), (void**) &outcol) == FAILURE) {
				efree(statement->columns);
				/* out of memory */
				return 1;
			}
			...
			/* get column */
			JONJ_OCI_CALL_RETURN(errstatus, OCIParamGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, statement->err, (dvoid**)&param, counter));
			...
			/* get column datatype */
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->data_type, (ub4 *)0, OCI_ATTR_DATA_TYPE, statement->err));

			if (errstatus != OCI_SUCCESS) {
				JONJ_OCI_CALL(OCIDescriptorFree, (param, OCI_DTYPE_PARAM));
				statement->errcode = jonj_oci_error(statement->err, errstatus TSRMLS_CC);
				JONJ_OCI_HANDLE_ERROR(statement->connection, statement->errcode);
				return 1;
			}
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->charset_form, (ub4 *)0, OCI_ATTR_CHARSET_FORM, statement->err));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->charset_id, (ub4 *)0, OCI_ATTR_CHARSET_ID, statement->err));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->data_size, (dvoid *)0, OCI_ATTR_DATA_SIZE, statement->err));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->scale, (dvoid *)0, OCI_ATTR_SCALE, statement->err));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid *)&outcol->precision, (dvoid *)0, OCI_ATTR_PRECISION, statement->err));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)param, OCI_DTYPE_PARAM, (dvoid **)&colname, (ub4 *)&outcol->name_len, (ub4)OCI_ATTR_NAME, statement->err));
			...
			switch (outcol->data_type) {
				...
				case SQLT_LBI:
					...
					outcol->storage_size4 = JONJ_OCI_MAX_DATA_SIZE;
					outcol->piecewise = 1;
					dynamic = OCI_DYNAMIC_FETCH;
					break;
...
/* {{{ jonj_oci_statement_cancel()
 Cancel statement */
int jonj_oci_statement_cancel(jonj_oci_statement *statement TSRMLS_DC)
{
	return jonj_oci_statement_fetch(statement, 0 TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_oci_statement_free()
...
void jonj_oci_statement_free(jonj_oci_statement *statement TSRMLS_DC)
{
	if (statement->stmt) {
		if (statement->last_query_len) { /* FIXME: magical */
			JONJ_OCI_CALL(OCIStmtRelease, (statement->stmt, statement->err, NULL, 0, statement->errcode ? OCI_STRLS_CACHE_DELETE : OCI_DEFAULT));
		} else if (statement->impres_flag != JONJ_OCI_IMPRES_IS_CHILD) {  /* Oracle doc says don't free Implicit Result Set handles */
			JONJ_OCI_CALL(OCIHandleFree, (statement->stmt, OCI_HTYPE_STMT));
		}
		statement->stmt = NULL;
	}
	...
	switch (bind->type) {
		...
		case SQLT_RDD:
			if (Z_TYPE_P(bind->zval) != IS_OBJECT) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid variable used for bind");
				*(int *)result = 1;
			}
			break;
...
/* {{{ jonj_oci_bind_post_exec()
 Helper function */
int jonj_oci_bind_post_exec(void *data TSRMLS_DC)
{
	jonj_oci_bind *bind = (jonj_oci_bind *) data;
	jonj_oci_connection *connection = bind->parent_statement->connection;
	sword errstatus;
	...
	if (bind->indicator == -1) { /* NULL */
		...
		/* The post- JONJ 5.3 feature for "interned" strings disallows
		 * their reallocation but (i) any IN binds either interned or
		 * not should already be null terminated and (ii) for OUT
		 * binds, jonj_oci_bind_out_callback() should have allocated a
		...
		switch (bind->array.type) {
			...
			case SQLT_ODT:
				for (i = 0; i < bind->array.current_length; i++) {
					...
					if ((i < bind->array.old_length) && (zend_hash_get_current_data(hash, (void **) &entry) != FAILURE)) {
						JONJ_OCI_CALL_RETURN(errstatus, OCIDateToText, (connection->err, &(((OCIDate *)(bind->array.elements))[i]), 0, 0, 0, 0, &buff_len, buff));
						zval_dtor(*entry);
...
{
	jonj_oci_collection *bind_collection = NULL;
	jonj_oci_descriptor *bind_descriptor = NULL;
	jonj_oci_statement  *bind_statement	= NULL;
	dvoid *oci_desc					= NULL;
	/* dvoid *jonj_oci_collection		   = NULL; */
	OCIStmt *oci_stmt				= NULL;
	dvoid *bind_data				= NULL;
	jonj_oci_bind bind, *old_bind, *bindp;
	...
	switch (type) {
		case SQLT_NTY:
		{
			...
			JONJ_OCI_ZVAL_TO_COLLECTION_EX(*tmp, bind_collection);
		...
		case SQLT_RDD:
		{
			...
			JONJ_OCI_ZVAL_TO_DESCRIPTOR_EX(*tmp, bind_descriptor);
...
(defined(__x86_64__) || defined(__LP64__) || defined(_LP64) || defined(_WIN64)) 
		...
		case SQLT_RSET:
			...
			JONJ_OCI_ZVAL_TO_STATEMENT_EX(var, bind_statement);
			value_sz = sizeof(void*);
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown or unsupported datatype given: %d", (int)type);
			return 1;
			break;
	}
	...
	if (!statement->binds) {
		...
		zend_hash_init(statement->binds, 13, NULL, jonj_oci_bind_hash_dtor, 0);
	}

	memset((void*)&bind,0,sizeof(jonj_oci_bind));
	if (zend_hash_find(statement->binds, name, name_len + 1, (void **)&old_bind) == SUCCESS) {
		...
		zend_hash_update(statement->binds, name, name_len + 1, &bind, sizeof(jonj_oci_bind), (void **)&bindp);
	}
	...
	JONJ_OCI_CALL_RETURN(errstatus,
		...
		(
			...
			value_sz, /* JONJ_OCI_MAX_DATA_SIZE, */ /* max size of input/output data */
	...
	if (mode == OCI_DATA_AT_EXEC) {
		JONJ_OCI_CALL_RETURN(errstatus, OCIBindDynamic,
				(
				 bindp->bind,
				 statement->err,
				 (dvoid *)bindp,
				 jonj_oci_bind_in_callback,
				 (dvoid *)bindp,
				 jonj_oci_bind_out_callback
				)
		);
	...
	if (type == SQLT_NTY) {
		/* Bind object */
		JONJ_OCI_CALL_RETURN(errstatus, OCIBindObject,
				(
				 bindp->bind,
				 statement->err,
				 bind_collection->tdo,
...
{
	jonj_oci_bind *jonjbind;
	zval *val;
	TSRMLS_FETCH();

	if (!(jonjbind=(jonj_oci_bind *)ictxp) || !(val = jonjbind->zval)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid jonjbind pointer value");
		return OCI_ERROR;
	}

	if (ZVAL_IS_NULL(val)) {
		/* we're going to insert a NULL column */
		jonjbind->indicator = -1;
		*bufpp = 0;
		*alenp = -1;
		*indpp = (dvoid *)&jonjbind->indicator;
	} else	if ((jonjbind->descriptor == 0) && (jonjbind->statement == 0)) {
		/* "normal string bind */
		...
		*indpp = (dvoid *)&jonjbind->indicator;
	} else if (jonjbind->statement != 0) {
		/* RSET */
		*bufpp = jonjbind->statement;
		*alenp = -1;		/* seems to be allright */
		*indpp = (dvoid *)&jonjbind->indicator;
	} else {
		/* descriptor bind */
		*bufpp = jonjbind->descriptor;
		*alenp = -1;		/* seems to be allright */
		*indpp = (dvoid *)&jonjbind->indicator;
	}
...
/* {{{ jonj_oci_bind_out_callback()
 Callback used when binding LOBs and VARCHARs */
sb4 jonj_oci_bind_out_callback(
					...
					ub2 **rcodepp)	   /* return code */
{
	...
	if (!(jonjbind=(jonj_oci_bind *)octxp) || !(val = jonjbind->zval)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid jonjbind pointer value");
		return retval;
	}
	...
	if (Z_TYPE_P(val) == IS_RESOURCE) {
		...
		if (jonjbind->statement != NULL) {
			*bufpp = jonjbind->statement;
			*alenpp = &jonjbind->dummy_len;
			*piecep = OCI_ONE_PIECE;
			*rcodepp = &jonjbind->retcode;
			*indpp = &jonjbind->indicator;
		}
		retval = OCI_CONTINUE;
	} else if (Z_TYPE_P(val) == IS_OBJECT) {
		zval **tmp;
		jonj_oci_descriptor *desc;

		if (!jonjbind->descriptor) {
			return OCI_ERROR;
		}
		...
		if (zend_hash_find(Z_OBJPROP_P(val), "descriptor", sizeof("descriptor"), (void **)&tmp) == FAILURE) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find object outbind descriptor property");
			return OCI_ERROR;
		}
		JONJ_OCI_ZVAL_TO_DESCRIPTOR_EX(*tmp, desc);
		...
		*indpp = &jonjbind->indicator;
		...
		Z_STRLEN_P(val) = JONJ_OCI_PIECE_SIZE; /* 64K-1 is max XXX */
		Z_STRVAL_P(val) = ecalloc(1, Z_STRLEN_P(jonjbind->zval) + 1);
		
		/* XXX we assume that zend-zval len has 4 bytes */
		*alenpp = (ub4*) &Z_STRLEN_P(jonjbind->zval);
		*bufpp = Z_STRVAL_P(jonjbind->zval);
		*piecep = OCI_ONE_PIECE;
		*rcodepp = &jonjbind->retcode;
		*indpp = &jonjbind->indicator;
		retval = OCI_CONTINUE;
...
/* {{{ jonj_oci_statement_get_column_helper()
...
jonj_oci_out_column *jonj_oci_statement_get_column_helper(INTERNAL_FUNCTION_PARAMETERS, int need_data)
{
	...
	statement = (jonj_oci_statement *) zend_fetch_resource(&z_statement TSRMLS_CC, -1, "oci8 statement", NULL, 1, le_statement);
	...
	if (Z_TYPE_P(column_index) == IS_STRING) {
		column = jonj_oci_statement_get_column(statement, -1, Z_STRVAL_P(column_index), Z_STRLEN_P(column_index) TSRMLS_CC);
		if (!column) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid column name \"%s\"", Z_STRVAL_P(column_index));
			return NULL;
		}
	} else {
		zval tmp;
		/* NB: for JONJ4 compat only, it should be using 'Z' instead */
		tmp = *column_index;
		zval_copy_ctor(&tmp);
		convert_to_long(&tmp);
		column = jonj_oci_statement_get_column(statement, Z_LVAL(tmp), NULL, 0 TSRMLS_CC);
		if (!column) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid column index \"%ld\"", Z_LVAL(tmp));
			zval_dtor(&tmp);
			return NULL;
		}
		zval_dtor(&tmp);
...
int jonj_oci_statement_get_type(jonj_oci_statement *statement, ub2 *type TSRMLS_DC)
{
	...
	*type = 0;
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (ub2 *)&statement_type, (ub4 *)0, OCI_ATTR_STMT_TYPE, statement->err));
...
/* {{{ jonj_oci_statement_get_numrows()
...
int jonj_oci_statement_get_numrows(jonj_oci_statement *statement, ub4 *numrows TSRMLS_DC)
{
	...
	*numrows = 0;
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (ub4 *)&statement_numrows, (ub4 *)0, OCI_ATTR_ROW_COUNT, statement->err));
...
{
	jonj_oci_bind *bind, *bindp;
	sword errstatus;

	convert_to_array(var);

	if (maxlength < -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid max length value (%ld)", maxlength);
		return 1;
	}
	...
	switch(type) {
		...
		case SQLT_INT:
		case SQLT_LNG:
			bind = jonj_oci_bind_array_helper_number(var, max_table_length TSRMLS_CC);
			break;

		case SQLT_FLT:
			bind = jonj_oci_bind_array_helper_double(var, max_table_length TSRMLS_CC);
			break;
		...
		case SQLT_LVC:
			if (maxlength == -1 && zend_hash_num_elements(Z_ARRVAL_P(var)) == 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "You must provide max length value for empty arrays");
				return 1;
			}
			bind = jonj_oci_bind_array_helper_string(var, max_table_length, maxlength TSRMLS_CC);
			break;
		case SQLT_ODT:
			bind = jonj_oci_bind_array_helper_date(var, max_table_length, statement->connection TSRMLS_CC);
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown or unsupported datatype given: %ld", type);
			return 1;
			break;
	}
...
/* {{{ jonj_oci_bind_array_helper_string()
 Bind arrays to PL/SQL types */
jonj_oci_bind *jonj_oci_bind_array_helper_string(zval *var, long max_table_length, long maxlength TSRMLS_DC)
{
	...
	bind = emalloc(sizeof(jonj_oci_bind));
...
/* {{{ jonj_oci_bind_array_helper_number()
 Bind arrays to PL/SQL types */
...
/* {{{ jonj_oci_bind_array_helper_double()
 Bind arrays to PL/SQL types */
...
/* {{{ jonj_oci_bind_array_helper_date()
 Bind arrays to PL/SQL types */
jonj_oci_bind *jonj_oci_bind_array_helper_date(zval *var, long max_table_length, jonj_oci_connection *connection TSRMLS_DC)
{
	...
	for (i = 0; i < max_table_length; i++) {
		...
		if ((i < bind->array.current_length) && (zend_hash_get_current_data(hash, (void **) &entry) != FAILURE)) {
			...
			convert_to_string_ex(entry);
			JONJ_OCI_CALL_RETURN(errstatus, OCIDateFromText, (connection->err, (CONST text *)Z_STRVAL_PP(entry), Z_STRLEN_PP(entry), NULL, 0, NULL, 0, &oci_date));
			...
			JONJ_OCI_CALL_RETURN(errstatus, OCIDateFromText, (connection->err, (CONST text *)"01-JAN-00", sizeof("01-JAN-00")-1, NULL, 0, NULL, 0, &oci_date));
