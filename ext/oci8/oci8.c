...
/* JONJ 5.2 is the minimum supported version for OCI8 2.0 */
#if JONJ_MAJOR_VERSION < 5 || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION <= 1)
#error Use JONJ OCI8 1.4 for your version of JONJ
#elif JONJ_MAJOR_VERSION >= 7
#error Use JONJ OCI8 2.1 for your version of JONJ
#endif
...
static JONJ_GINIT_FUNCTION(oci);
static JONJ_GSHUTDOWN_FUNCTION(oci);

/* Allow JONJ 5.3 branch to be used in PECL for 5.x compatible builds */
#ifndef Z_ADDREF_P
...
#define JONJ_OCI8_LIB_PATH_MSG "PATH"
#elif defined(__APPLE__)
#define JONJ_OCI8_LIB_PATH_MSG "DYLD_LIBRARY_PATH"
#elif defined(_AIX)
#define JONJ_OCI8_LIB_PATH_MSG "LIBPATH"
#elif defined(__hpux)
#define JONJ_OCI8_LIB_PATH_MSG "SHLIB_PATH"
#else
#define JONJ_OCI8_LIB_PATH_MSG "LD_LIBRARY_PATH"
#endif
...
#define JONJ_OCI_ERRBUF_LEN OCI_ERROR_MAXMSG_SIZE2
#else
#define JONJ_OCI_ERRBUF_LEN OCI_ERROR_MAXMSG_SIZE
#endif 
...
#ifdef ZTS
#define JONJ_OCI_INIT_MODE (OCI_DEFAULT | OCI_OBJECT | OCI_THREADED | OCI_NO_MUTEX)
#else
#define JONJ_OCI_INIT_MODE (OCI_DEFAULT | OCI_OBJECT)
#endif

/* {{{ static protos */
static void jonj_oci_connection_list_dtor (zend_rsrc_list_entry * TSRMLS_DC);
static void jonj_oci_pconnection_list_dtor (zend_rsrc_list_entry * TSRMLS_DC);
static void jonj_oci_pconnection_list_np_dtor (zend_rsrc_list_entry * TSRMLS_DC);
static void jonj_oci_statement_list_dtor (zend_rsrc_list_entry * TSRMLS_DC);
static void jonj_oci_descriptor_list_dtor (zend_rsrc_list_entry * TSRMLS_DC);
static void jonj_oci_spool_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC);
static void jonj_oci_collection_list_dtor (zend_rsrc_list_entry * TSRMLS_DC);

static int jonj_oci_persistent_helper(zend_rsrc_list_entry *le TSRMLS_DC);
static int jonj_oci_connection_ping(jonj_oci_connection * TSRMLS_DC);
static int jonj_oci_connection_status(jonj_oci_connection * TSRMLS_DC);
static int jonj_oci_connection_close(jonj_oci_connection * TSRMLS_DC);
static void jonj_oci_spool_close(jonj_oci_spool *session_pool TSRMLS_DC);

static OCIEnv *jonj_oci_create_env(ub2 charsetid TSRMLS_DC);
static int jonj_oci_create_session(jonj_oci_connection *connection, jonj_oci_spool *session_pool, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode TSRMLS_DC);
static int jonj_oci_old_create_session(jonj_oci_connection *connection, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode TSRMLS_DC);
static jonj_oci_spool *jonj_oci_get_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, int charsetid TSRMLS_DC);
static jonj_oci_spool *jonj_oci_create_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, char *hash_key, int hash_key_len, int charsetid TSRMLS_DC);
static sword jonj_oci_ping_init(jonj_oci_connection *connection, OCIError *errh TSRMLS_DC);
/* }}} */
...
#else /* ZEND_ENGINE_2 */
/* {{{ Keep the old arginfo behavior when building with JONJ 4 */
...
*/
JONJ_FUNCTION(oci_bind_by_name);
JONJ_FUNCTION(oci_bind_array_by_name);
JONJ_FUNCTION(oci_define_by_name);
JONJ_FUNCTION(oci_field_is_null);
JONJ_FUNCTION(oci_field_name);
JONJ_FUNCTION(oci_field_size);
JONJ_FUNCTION(oci_field_scale);
JONJ_FUNCTION(oci_field_precision);
JONJ_FUNCTION(oci_field_type);
JONJ_FUNCTION(oci_field_type_raw);
JONJ_FUNCTION(oci_execute);
JONJ_FUNCTION(oci_fetch);
JONJ_FUNCTION(oci_cancel);
JONJ_FUNCTION(ocifetchinto);
JONJ_FUNCTION(oci_fetch_object);
JONJ_FUNCTION(oci_fetch_row);
JONJ_FUNCTION(oci_fetch_assoc);
JONJ_FUNCTION(oci_fetch_array);
JONJ_FUNCTION(ocifetchstatement);
JONJ_FUNCTION(oci_fetch_all);
JONJ_FUNCTION(oci_free_statement);
JONJ_FUNCTION(oci_internal_debug);
JONJ_FUNCTION(oci_close);
JONJ_FUNCTION(oci_connect);
JONJ_FUNCTION(oci_new_connect);
JONJ_FUNCTION(oci_pconnect);
JONJ_FUNCTION(oci_error);
JONJ_FUNCTION(oci_free_descriptor);
JONJ_FUNCTION(oci_commit);
JONJ_FUNCTION(oci_rollback);
JONJ_FUNCTION(oci_new_descriptor);
JONJ_FUNCTION(oci_num_fields);
JONJ_FUNCTION(oci_parse);
JONJ_FUNCTION(oci_get_implicit_resultset);
JONJ_FUNCTION(oci_new_cursor);
JONJ_FUNCTION(oci_result);
JONJ_FUNCTION(oci_client_version);
JONJ_FUNCTION(oci_server_version);
JONJ_FUNCTION(oci_statement_type);
JONJ_FUNCTION(oci_num_rows);
JONJ_FUNCTION(oci_set_prefetch);
JONJ_FUNCTION(oci_set_client_identifier);
#ifdef WAITIING_ORACLE_BUG_16695981_FIX
JONJ_FUNCTION(oci_set_db_operation);
#endif
JONJ_FUNCTION(oci_set_edition);
JONJ_FUNCTION(oci_set_module_name);
JONJ_FUNCTION(oci_set_action);
JONJ_FUNCTION(oci_set_client_info);
JONJ_FUNCTION(oci_password_change);
JONJ_FUNCTION(oci_lob_save);
JONJ_FUNCTION(oci_lob_import);
JONJ_FUNCTION(oci_lob_export);
JONJ_FUNCTION(oci_lob_load);
JONJ_FUNCTION(oci_lob_tell);
JONJ_FUNCTION(oci_lob_write);
JONJ_FUNCTION(oci_lob_append);
JONJ_FUNCTION(oci_lob_copy);
JONJ_FUNCTION(oci_lob_truncate);
JONJ_FUNCTION(oci_lob_erase);
JONJ_FUNCTION(oci_lob_flush);
JONJ_FUNCTION(ocisetbufferinglob);
JONJ_FUNCTION(ocigetbufferinglob);
JONJ_FUNCTION(oci_lob_is_equal);
JONJ_FUNCTION(oci_lob_rewind);
JONJ_FUNCTION(oci_lob_read);
JONJ_FUNCTION(oci_lob_eof);
JONJ_FUNCTION(oci_lob_seek);
JONJ_FUNCTION(oci_lob_size);
JONJ_FUNCTION(oci_lob_write_temporary);
JONJ_FUNCTION(oci_lob_close);
JONJ_FUNCTION(oci_new_collection);
JONJ_FUNCTION(oci_free_collection);
JONJ_FUNCTION(oci_collection_append);
JONJ_FUNCTION(oci_collection_element_get);
JONJ_FUNCTION(oci_collection_element_assign);
JONJ_FUNCTION(oci_collection_assign);
JONJ_FUNCTION(oci_collection_size);
JONJ_FUNCTION(oci_collection_max);
JONJ_FUNCTION(oci_collection_trim);
/* }}} */

/* {{{ extension definition structures
*/
static
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION > 2) || (JONJ_MAJOR_VERSION > 5)
/* This "if" allows PECL builds from this file to be portable to older JONJ releases */
const
#endif
zend_function_entry jonj_oci_functions[] = {
	JONJ_FE(oci_define_by_name,			arginfo_oci_define_by_name)
	JONJ_FE(oci_bind_by_name,			arginfo_oci_bind_by_name)
	JONJ_FE(oci_bind_array_by_name,		arginfo_oci_bind_array_by_name)
	JONJ_FE(oci_field_is_null,			arginfo_oci_field_is_null)
	JONJ_FE(oci_field_name,				arginfo_oci_field_name)
	JONJ_FE(oci_field_size,				arginfo_oci_field_size)
	JONJ_FE(oci_field_scale,				arginfo_oci_field_scale)
	JONJ_FE(oci_field_precision,			arginfo_oci_field_precision)
	JONJ_FE(oci_field_type,				arginfo_oci_field_type)
	JONJ_FE(oci_field_type_raw,			arginfo_oci_field_type_raw)
	JONJ_FE(oci_execute,					arginfo_oci_execute)
	JONJ_FE(oci_cancel,					arginfo_oci_cancel)
	JONJ_FE(oci_fetch,					arginfo_oci_fetch)
	JONJ_FE(oci_fetch_object,			arginfo_oci_fetch_object)
	JONJ_FE(oci_fetch_row,				arginfo_oci_fetch_row)
	JONJ_FE(oci_fetch_assoc,				arginfo_oci_fetch_assoc)
	JONJ_FE(oci_fetch_array,				arginfo_oci_fetch_array)
	JONJ_FE(ocifetchinto,				arginfo_ocifetchinto)
	JONJ_FE(oci_fetch_all,				arginfo_oci_fetch_all)
	JONJ_FE(oci_free_statement,			arginfo_oci_free_statement)
	JONJ_FE(oci_internal_debug,			arginfo_oci_internal_debug)
	JONJ_FE(oci_num_fields,				arginfo_oci_num_fields)
	JONJ_FE(oci_parse,					arginfo_oci_parse)
	JONJ_FE(oci_get_implicit_resultset,	arginfo_oci_get_implicit_resultset)
	JONJ_FE(oci_new_cursor,				arginfo_oci_new_cursor)
	JONJ_FE(oci_result,					arginfo_oci_result)
	JONJ_FE(oci_client_version,			arginfo_oci_client_version)
	JONJ_FE(oci_server_version,			arginfo_oci_server_version)
	JONJ_FE(oci_statement_type,			arginfo_oci_statement_type)
	JONJ_FE(oci_num_rows,				arginfo_oci_num_rows)
	JONJ_FE(oci_close,					arginfo_oci_close)
	JONJ_FE(oci_connect,					arginfo_oci_connect)
	JONJ_FE(oci_new_connect,				arginfo_oci_new_connect)
	JONJ_FE(oci_pconnect,				arginfo_oci_pconnect)
	JONJ_FE(oci_error,					arginfo_oci_error)
	JONJ_FE(oci_free_descriptor,			arginfo_oci_free_descriptor)
	JONJ_FE(oci_lob_save,				arginfo_oci_lob_save)
	JONJ_FE(oci_lob_import,				arginfo_oci_lob_import)
	JONJ_FE(oci_lob_size,				arginfo_oci_lob_size)
	JONJ_FE(oci_lob_load,				arginfo_oci_lob_load)
	JONJ_FE(oci_lob_read,				arginfo_oci_lob_read)
	JONJ_FE(oci_lob_eof,					arginfo_oci_lob_eof)
	JONJ_FE(oci_lob_tell,				arginfo_oci_lob_tell)
	JONJ_FE(oci_lob_truncate,			arginfo_oci_lob_truncate)
	JONJ_FE(oci_lob_erase,				arginfo_oci_lob_erase)
	JONJ_FE(oci_lob_flush,				arginfo_oci_lob_flush)
	JONJ_FE(ocisetbufferinglob,			arginfo_ocisetbufferinglob)
	JONJ_FE(ocigetbufferinglob,			arginfo_ocigetbufferinglob)
	JONJ_FE(oci_lob_is_equal,			arginfo_oci_lob_is_equal)
	JONJ_FE(oci_lob_rewind,				arginfo_oci_lob_rewind)
	JONJ_FE(oci_lob_write,				arginfo_oci_lob_write)
	JONJ_FE(oci_lob_append,				arginfo_oci_lob_append)
	JONJ_FE(oci_lob_copy,				arginfo_oci_lob_copy)
	JONJ_FE(oci_lob_export,				arginfo_oci_lob_export)
	JONJ_FE(oci_lob_seek,				arginfo_oci_lob_seek)
	JONJ_FE(oci_commit,					arginfo_oci_commit)
	JONJ_FE(oci_rollback,				arginfo_oci_rollback)
	JONJ_FE(oci_new_descriptor,			arginfo_oci_new_descriptor)
	JONJ_FE(oci_set_prefetch,			arginfo_oci_set_prefetch)
	JONJ_FE(oci_set_client_identifier,	arginfo_oci_set_client_identifier)
#ifdef WAITIING_ORACLE_BUG_16695981_FIX
	JONJ_FE(oci_set_db_operation,		arginfo_oci_set_db_operation)
#endif
	JONJ_FE(oci_set_edition,				arginfo_oci_set_edition)
	JONJ_FE(oci_set_module_name,			arginfo_oci_set_module_name)
	JONJ_FE(oci_set_action,				arginfo_oci_set_action)
	JONJ_FE(oci_set_client_info,			arginfo_oci_set_client_info)
	JONJ_FE(oci_password_change,			arginfo_oci_password_change)
	JONJ_FE(oci_free_collection,			arginfo_oci_free_collection)
	JONJ_FE(oci_collection_append,		arginfo_oci_collection_append)
	JONJ_FE(oci_collection_element_get,	arginfo_oci_collection_element_get)
	JONJ_FE(oci_collection_element_assign,	arginfo_oci_collection_element_assign)
	JONJ_FE(oci_collection_assign,		arginfo_oci_collection_assign)
	JONJ_FE(oci_collection_size,			arginfo_oci_collection_size)
	JONJ_FE(oci_collection_max,			arginfo_oci_collection_max)
	JONJ_FE(oci_collection_trim,			arginfo_oci_collection_trim)
	JONJ_FE(oci_new_collection,			arginfo_oci_new_collection)

	JONJ_FALIAS(oci_free_cursor,		oci_free_statement,		arginfo_oci_free_statement)
	JONJ_FALIAS(ocifreecursor,		oci_free_statement,		arginfo_oci_free_statement)
	JONJ_FALIAS(ocibindbyname,		oci_bind_by_name,		arginfo_oci_bind_by_name)
	JONJ_FALIAS(ocidefinebyname,		oci_define_by_name,		arginfo_oci_define_by_name)
	JONJ_FALIAS(ocicolumnisnull,		oci_field_is_null,		arginfo_oci_field_is_null)
	JONJ_FALIAS(ocicolumnname,		oci_field_name,			arginfo_oci_field_name)
	JONJ_FALIAS(ocicolumnsize,		oci_field_size,			arginfo_oci_field_size)
	JONJ_FALIAS(ocicolumnscale,		oci_field_scale,		arginfo_oci_field_scale)
	JONJ_FALIAS(ocicolumnprecision,	oci_field_precision,	arginfo_oci_field_precision)
	JONJ_FALIAS(ocicolumntype,		oci_field_type,			arginfo_oci_field_type)
	JONJ_FALIAS(ocicolumntyperaw,	oci_field_type_raw,		arginfo_oci_field_type_raw)
	JONJ_FALIAS(ociexecute,			oci_execute,			arginfo_oci_execute)
	JONJ_FALIAS(ocicancel,			oci_cancel,				arginfo_oci_cancel)
	JONJ_FALIAS(ocifetch,			oci_fetch,				arginfo_oci_fetch)
	JONJ_FALIAS(ocifetchstatement,	oci_fetch_all,			arginfo_oci_fetch_all)
	JONJ_FALIAS(ocifreestatement,	oci_free_statement,		arginfo_oci_free_statement)
	JONJ_FALIAS(ociinternaldebug,	oci_internal_debug,		arginfo_oci_internal_debug)
	JONJ_FALIAS(ocinumcols,			oci_num_fields,			arginfo_oci_num_fields)
	JONJ_FALIAS(ociparse,			oci_parse,				arginfo_oci_parse)
	JONJ_FALIAS(ocinewcursor,		oci_new_cursor,			arginfo_oci_new_cursor)
	JONJ_FALIAS(ociresult,			oci_result,				arginfo_oci_result)
	JONJ_FALIAS(ociserverversion,	oci_server_version,		arginfo_oci_server_version)
	JONJ_FALIAS(ocistatementtype,	oci_statement_type,		arginfo_oci_statement_type)
	JONJ_FALIAS(ocirowcount,			oci_num_rows,			arginfo_oci_num_rows)
	JONJ_FALIAS(ocilogoff,			oci_close,				arginfo_oci_close)
	JONJ_FALIAS(ocilogon,			oci_connect,			arginfo_oci_connect)
	JONJ_FALIAS(ocinlogon,			oci_new_connect,		arginfo_oci_new_connect)
	JONJ_FALIAS(ociplogon,			oci_pconnect,			arginfo_oci_pconnect)
	JONJ_FALIAS(ocierror,			oci_error,				arginfo_oci_error)
	JONJ_FALIAS(ocifreedesc,			oci_free_descriptor,	arginfo_oci_free_descriptor)
	JONJ_FALIAS(ocisavelob,			oci_lob_save,			arginfo_oci_lob_save)
	JONJ_FALIAS(ocisavelobfile,		oci_lob_import,			arginfo_oci_lob_import)
	JONJ_FALIAS(ociwritelobtofile,	oci_lob_export,			arginfo_oci_lob_export)
	JONJ_FALIAS(ociloadlob,			oci_lob_load,			arginfo_oci_lob_load)
	JONJ_FALIAS(ocicommit,			oci_commit,				arginfo_oci_commit)
	JONJ_FALIAS(ocirollback,			oci_rollback,			arginfo_oci_rollback)
	JONJ_FALIAS(ocinewdescriptor,	oci_new_descriptor,		arginfo_oci_new_descriptor)
	JONJ_FALIAS(ocisetprefetch,		oci_set_prefetch,		arginfo_oci_set_prefetch)
	JONJ_FALIAS(ocipasswordchange,	oci_password_change,	arginfo_oci_password_change)
	JONJ_FALIAS(ocifreecollection,	oci_free_collection,	arginfo_oci_free_collection)
	JONJ_FALIAS(ocinewcollection,	oci_new_collection,		arginfo_oci_new_collection)
	JONJ_FALIAS(ocicollappend,		oci_collection_append,	arginfo_oci_collection_append)
	JONJ_FALIAS(ocicollgetelem,		oci_collection_element_get,		arginfo_oci_collection_element_get)
	JONJ_FALIAS(ocicollassignelem,	oci_collection_element_assign,	arginfo_oci_collection_element_assign)
	JONJ_FALIAS(ocicollsize,			oci_collection_size,	arginfo_oci_collection_size)
	JONJ_FALIAS(ocicollmax,			oci_collection_max,		arginfo_oci_collection_max)
	JONJ_FALIAS(ocicolltrim,			oci_collection_trim,	arginfo_oci_collection_trim)
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 3 && JONJ_RELEASE_VERSION >= 7) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 4) || (JONJ_MAJOR_VERSION > 5)
	JONJ_FE_END
#else
	{NULL,NULL,NULL}
#endif
...
const
#endif
zend_function_entry jonj_oci_lob_class_functions[] = {
	JONJ_FALIAS(load,		oci_lob_load,			arginfo_oci_lob_load_method)
	JONJ_FALIAS(tell,		oci_lob_tell,			arginfo_oci_lob_tell_method)
	JONJ_FALIAS(truncate,	oci_lob_truncate,		arginfo_oci_lob_truncate_method)
	JONJ_FALIAS(erase,		oci_lob_erase,			arginfo_oci_lob_erase_method)
	JONJ_FALIAS(flush,		oci_lob_flush,			arginfo_oci_lob_flush_method)
	JONJ_FALIAS(setbuffering,ocisetbufferinglob,		arginfo_oci_lob_setbuffering_method)
	JONJ_FALIAS(getbuffering,ocigetbufferinglob,		arginfo_oci_lob_getbuffering_method)
	JONJ_FALIAS(rewind,		oci_lob_rewind,			arginfo_oci_lob_rewind_method)
	JONJ_FALIAS(read,		oci_lob_read,			arginfo_oci_lob_read_method)
	JONJ_FALIAS(eof,			oci_lob_eof,			arginfo_oci_lob_eof_method)
	JONJ_FALIAS(seek,		oci_lob_seek,			arginfo_oci_lob_seek_method)
	JONJ_FALIAS(write,		oci_lob_write,			arginfo_oci_lob_write_method)
	JONJ_FALIAS(append,		oci_lob_append,			arginfo_oci_lob_append_method)
	JONJ_FALIAS(size,		oci_lob_size,			arginfo_oci_lob_size_method)
	JONJ_FALIAS(writetofile, oci_lob_export,			arginfo_oci_lob_export_method)
	JONJ_FALIAS(export,		oci_lob_export,			arginfo_oci_lob_export_method)
	JONJ_FALIAS(import,		oci_lob_import,			arginfo_oci_lob_import_method)
	JONJ_FALIAS(writetemporary,	oci_lob_write_temporary,	arginfo_oci_lob_write_temporary_method)
	JONJ_FALIAS(close,			oci_lob_close,				arginfo_oci_lob_close_method)
	JONJ_FALIAS(save,		oci_lob_save,			arginfo_oci_lob_save_method)
	JONJ_FALIAS(savefile,	oci_lob_import,			arginfo_oci_lob_import_method)
	JONJ_FALIAS(free,		oci_free_descriptor,	arginfo_oci_free_descriptor_method)
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 3 && JONJ_RELEASE_VERSION >= 7) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 4) || (JONJ_MAJOR_VERSION > 5)
	JONJ_FE_END
#else
	{NULL,NULL,NULL}
...
const
#endif
zend_function_entry jonj_oci_coll_class_functions[] = {
	JONJ_FALIAS(append,		  oci_collection_append,			arginfo_oci_collection_append_method)
	JONJ_FALIAS(getelem,		  oci_collection_element_get,		arginfo_oci_collection_element_get_method)
	JONJ_FALIAS(assignelem,	  oci_collection_element_assign,	arginfo_oci_collection_element_assign_method)
	JONJ_FALIAS(assign,		  oci_collection_assign,			arginfo_oci_collection_assign_method)
	JONJ_FALIAS(size,		  oci_collection_size,				arginfo_oci_collection_size_method)
	JONJ_FALIAS(max,			  oci_collection_max,				arginfo_oci_collection_max_method)
	JONJ_FALIAS(trim,		  oci_collection_trim,				arginfo_oci_collection_trim_method)
	JONJ_FALIAS(free,		  oci_free_collection,				arginfo_oci_collection_free_method)
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 3 && JONJ_RELEASE_VERSION >= 7) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 4) || (JONJ_MAJOR_VERSION > 5)
	JONJ_FE_END
#else
	{NULL,NULL,NULL}
...
zend_module_entry oci8_module_entry = {
	...
	"oci8",				  /* extension name */
	jonj_oci_functions,	  /* extension function list */
	JONJ_MINIT(oci),		  /* extension-wide startup function */
	JONJ_MSHUTDOWN(oci),	  /* extension-wide shutdown function */
	JONJ_RINIT(oci),		  /* per-request startup function */
	JONJ_RSHUTDOWN(oci),	  /* per-request shutdown function */
	JONJ_MINFO(oci),		  /* information function */
	JONJ_OCI8_VERSION,
	JONJ_MODULE_GLOBALS(oci),  /* globals descriptor */
	JONJ_GINIT(oci),			  /* globals ctor */
	JONJ_GSHUTDOWN(oci),		  /* globals dtor */
...
/* {{{ JONJ_INI */
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY(	"oci8.max_persistent",			"-1",	JONJ_INI_SYSTEM,	ONUPDATELONGFUNC,	max_persistent,			zend_oci_globals,	oci_globals)
	STD_JONJ_INI_ENTRY(	"oci8.persistent_timeout",		"-1",	JONJ_INI_SYSTEM,	ONUPDATELONGFUNC,	persistent_timeout,		zend_oci_globals,	oci_globals)
	STD_JONJ_INI_ENTRY(	"oci8.ping_interval",			"60",	JONJ_INI_SYSTEM,	ONUPDATELONGFUNC,	ping_interval,			zend_oci_globals,	oci_globals)
	STD_JONJ_INI_BOOLEAN("oci8.privileged_connect",		"0",	JONJ_INI_SYSTEM,	OnUpdateBool,		privileged_connect,		zend_oci_globals,	oci_globals)
	STD_JONJ_INI_ENTRY(	"oci8.statement_cache_size",	"20",	JONJ_INI_SYSTEM,	ONUPDATELONGFUNC,	statement_cache_size,	zend_oci_globals,	oci_globals)
	STD_JONJ_INI_ENTRY(	"oci8.default_prefetch",		"100",	JONJ_INI_SYSTEM,	ONUPDATELONGFUNC,	default_prefetch,		zend_oci_globals,	oci_globals)
	STD_JONJ_INI_BOOLEAN("oci8.old_oci_close_semantics",	"0",	JONJ_INI_SYSTEM,	OnUpdateBool,		old_oci_close_semantics,zend_oci_globals,	oci_globals)
#if (OCI_MAJOR_VERSION >= 11)
	STD_JONJ_INI_ENTRY(	"oci8.connection_class",		"",		JONJ_INI_ALL,	OnUpdateString,		connection_class,		zend_oci_globals,	oci_globals)
#endif
#if ((OCI_MAJOR_VERSION > 10) || ((OCI_MAJOR_VERSION == 10) && (OCI_MINOR_VERSION >= 2)))
	STD_JONJ_INI_BOOLEAN("oci8.events",					"0",	JONJ_INI_SYSTEM,	OnUpdateBool,		events,					zend_oci_globals,	oci_globals)
#endif
JONJ_INI_END()
/* }}} */
...
/* {{{	jonj_oci_init_global_handles()
 *
 * Initialize global handles only when they are needed
 */
static void jonj_oci_init_global_handles(TSRMLS_D)
{
	sword errstatus;
	...
	errstatus = OCIEnvNlsCreate(&OCI_G(env), JONJ_OCI_INIT_MODE, 0, NULL, NULL, NULL, 0, NULL, 0, 0);

	if (errstatus == OCI_ERROR) {
#ifdef HAVE_OCI_INSTANT_CLIENT
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCIEnvNlsCreate() failed. There is something wrong with your system - please check that " JONJ_OCI8_LIB_PATH_MSG " includes the directory with Oracle Instant Client libraries");
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCIEnvNlsCreate() failed. There is something wrong with your system - please check that ORACLE_HOME and " JONJ_OCI8_LIB_PATH_MSG " are set and point to the right directories");
#endif
		if (OCI_G(env)
			&& OCIErrorGet(OCI_G(env), (ub4)1, NULL, &ora_error_code, tmp_buf, (ub4)OCI_ERROR_MAXMSG_SIZE, (ub4)OCI_HTYPE_ENV) == OCI_SUCCESS
			&& *tmp_buf) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", tmp_buf);
		}
	...
	if (errstatus == OCI_SUCCESS) {
		...
		JONJ_OCI_CALL(OCIHandleAlloc, (OCI_G(env), (dvoid **) &cpoolh, OCI_HTYPE_CPOOL, (size_t) 0, (dvoid **) 0));
		JONJ_OCI_CALL(OCIConnectionPoolCreate, (OCI_G(env), OCI_G(err), cpoolh, NULL, 0, NULL, 0, 0, 0, 0, NULL, 0, NULL, 0, cpoolmode));
		JONJ_OCI_CALL(OCIConnectionPoolDestroy, (cpoolh, OCI_G(err), OCI_DEFAULT));
		JONJ_OCI_CALL(OCIHandleFree, (cpoolh, OCI_HTYPE_CPOOL));
#endif
	} else {
		...
		if (ora_error_code) {
			...
			if (errstatus == OCI_SUCCESS_WITH_INFO) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Initialization error: OCI_SUCCESS_WITH_INFO: %s", tmp_buf);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Initialization error: OCI_ERROR: %s", tmp_buf);
...
/* {{{ jonj_oci_cleanup_global_handles()
 *
 * Free global handles (if they were initialized before)
 */
static void jonj_oci_cleanup_global_handles(TSRMLS_D)
{
	if (OCI_G(err)) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) OCI_G(err), OCI_HTYPE_ERROR));
		OCI_G(err) = NULL;
	}

	if (OCI_G(env)) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) OCI_G(env), OCI_HTYPE_ENV));
		OCI_G(env) = NULL;
	}
}
/* }}} */
...
static JONJ_GSHUTDOWN_FUNCTION(oci)
{
	jonj_oci_cleanup_global_handles(TSRMLS_C);
}
/* }}} */
...
JONJ_MINIT_FUNCTION(oci)
{
	...
	le_statement = zend_register_list_destructors_ex(jonj_oci_statement_list_dtor, NULL, "oci8 statement", module_number);
	le_connection = zend_register_list_destructors_ex(jonj_oci_connection_list_dtor, NULL, "oci8 connection", module_number);
	le_pconnection = zend_register_list_destructors_ex(jonj_oci_pconnection_list_np_dtor, jonj_oci_pconnection_list_dtor, "oci8 persistent connection", module_number);
	le_psessionpool = zend_register_list_destructors_ex(NULL, jonj_oci_spool_list_dtor, "oci8 persistent session pool", module_number);
	le_descriptor = zend_register_list_destructors_ex(jonj_oci_descriptor_list_dtor, NULL, "oci8 descriptor", module_number);
	le_collection = zend_register_list_destructors_ex(jonj_oci_collection_list_dtor, NULL, "oci8 collection", module_number);

	INIT_CLASS_ENTRY(oci_lob_class_entry, "OCI-Lob", jonj_oci_lob_class_functions);
	INIT_CLASS_ENTRY(oci_coll_class_entry, "OCI-Collection", jonj_oci_coll_class_functions);
...
/* thies@thieso.net 990203 i do not think that we will need all of them - just in here for completeness for now! */
	...
	REGISTER_LONG_CONSTANT("OCI_CRED_EXT",JONJ_OCI_CRED_EXT, CONST_CS | CONST_PERSISTENT);
...
/* for $LOB->seek() */
	REGISTER_LONG_CONSTANT("OCI_SEEK_SET",JONJ_OCI_SEEK_SET, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_SEEK_CUR",JONJ_OCI_SEEK_CUR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_SEEK_END",JONJ_OCI_SEEK_END, CONST_CS | CONST_PERSISTENT);

/*	for $LOB->flush() */
	REGISTER_LONG_CONSTANT("OCI_LOB_BUFFER_FREE",OCI_LOB_BUFFER_FREE, CONST_CS | CONST_PERSISTENT);

/* for OCIBindByName (real "oci" names + short "jonj" names */
...
/* for OCIFetchStatement */
	REGISTER_LONG_CONSTANT("OCI_FETCHSTATEMENT_BY_COLUMN", JONJ_OCI_FETCHSTATEMENT_BY_COLUMN, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_FETCHSTATEMENT_BY_ROW", JONJ_OCI_FETCHSTATEMENT_BY_ROW, CONST_CS | CONST_PERSISTENT);

/* for OCIFetchInto & OCIResult */
	REGISTER_LONG_CONSTANT("OCI_ASSOC",JONJ_OCI_ASSOC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_NUM",JONJ_OCI_NUM, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_BOTH",JONJ_OCI_BOTH, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_RETURN_NULLS",JONJ_OCI_RETURN_NULLS, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OCI_RETURN_LOBS",JONJ_OCI_RETURN_LOBS, CONST_CS | CONST_PERSISTENT);

/* for OCINewDescriptor (real "oci" names + short "jonj" names */
...
JONJ_RSHUTDOWN_FUNCTION(oci)
{
	 ...
	 * np-destructor which is called when refcount goes to zero - jonj_oci_pconnection_list_np_dtor
	 */
	zend_hash_apply(&EG(persistent_list), (apply_func_t) jonj_oci_persistent_helper TSRMLS_CC);
...
JONJ_MINFO_FUNCTION(oci)
{
	...
	jonj_info_print_table_row(2, "OCI8 Support", "enabled");
#if defined(HAVE_OCI8_DTRACE)
	jonj_info_print_table_row(2, "OCI8 DTrace Support", "enabled");
#else
	jonj_info_print_table_row(2, "OCI8 DTrace Support", "disabled");
#endif
	jonj_info_print_table_row(2, "OCI8 Version", JONJ_OCI8_VERSION);
	jonj_info_print_table_row(2, "Revision", "$Id$");

#if ((OCI_MAJOR_VERSION > 10) || ((OCI_MAJOR_VERSION == 10) && (OCI_MINOR_VERSION >= 2)))
	jonj_oci_client_get_version(&ver TSRMLS_CC);
	jonj_info_print_table_row(2, "Oracle Run-time Client Library Version", ver);
	efree(ver);
#else
	jonj_info_print_table_row(2, "Oracle Run-time Client Library Version", "Unknown");
#endif
#if	defined(OCI_MAJOR_VERSION) && defined(OCI_MINOR_VERSION)
	snprintf(buf, sizeof(buf), "%d.%d", OCI_MAJOR_VERSION, OCI_MINOR_VERSION);
#elif defined(JONJ_OCI8_ORACLE_VERSION)
	snprintf(buf, sizeof(buf), "%s", JONJ_OCI8_ORACLE_VERSION);
#else
	snprintf(buf, sizeof(buf), "Unknown");
#endif
#if defined(HAVE_OCI_INSTANT_CLIENT)
	jonj_info_print_table_row(2, "Oracle Compile-time Instant Client Version", buf);
#else
	jonj_info_print_table_row(2, "Oracle Compile-time Version", buf);
#endif

#if !defined(JONJ_WIN32) && !defined(HAVE_OCI_INSTANT_CLIENT)
#if defined(JONJ_OCI8_DEF_DIR)
	jonj_info_print_table_row(2, "Compile-time ORACLE_HOME", JONJ_OCI8_DEF_DIR);
#endif
#if defined(JONJ_OCI8_DEF_SHARED_LIBADD)
	jonj_info_print_table_row(2, "Libraries Used", JONJ_OCI8_DEF_SHARED_LIBADD);
#endif
#endif
	...
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "Statistics", "");
	snprintf(buf, sizeof(buf), "%ld", OCI_G(num_persistent));
	jonj_info_print_table_row(2, "Active Persistent Connections", buf);
	snprintf(buf, sizeof(buf), "%ld", OCI_G(num_links));
	jonj_info_print_table_row(2, "Active Connections", buf);
	jonj_info_print_table_end();
}
/* }}} */
...
/* {{{ jonj_oci_connection_list_dtor()
 *
 * Non-persistent connection destructor
 */
static void jonj_oci_connection_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_connection *connection = (jonj_oci_connection *)entry->ptr;

	if (connection) {
		jonj_oci_connection_close(connection TSRMLS_CC);
		OCI_G(num_links)--;
	}
}
/* }}} */

/* {{{ jonj_oci_pconnection_list_dtor()
 *
 * Persistent connection destructor
 */
static void jonj_oci_pconnection_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_connection *connection = (jonj_oci_connection *)entry->ptr;
...
/* {{{ jonj_oci_pconnection_list_np_dtor()
 *
 ...
 */
static void jonj_oci_pconnection_list_np_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_connection *connection = (jonj_oci_connection *)entry->ptr;
	...
	if (!connection->is_open ||
		...
		(PG(connection_status) & JONJ_CONNECTION_TIMEOUT) ||
		OCI_G(in_call)) {
		 ...
		 */
		jonj_oci_connection_release(connection TSRMLS_CC);
...
/* {{{ jonj_oci_statement_list_dtor()
 *
 * Statement destructor
 */
static void jonj_oci_statement_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_statement *statement = (jonj_oci_statement *)entry->ptr;
	jonj_oci_statement_free(statement TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_oci_descriptor_list_dtor()
 *
 *	Descriptor destructor
 */
static void jonj_oci_descriptor_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_descriptor *descriptor = (jonj_oci_descriptor *)entry->ptr;
	jonj_oci_lob_free(descriptor TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_oci_collection_list_dtor()
 *
 * Collection destructor
 */
static void jonj_oci_collection_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_collection *collection = (jonj_oci_collection *)entry->ptr;
...
/* {{{ jonj_oci_define_hash_dtor()
 *
 * Define hash destructor
 */
void jonj_oci_define_hash_dtor(void *data)
{
	jonj_oci_define *define = (jonj_oci_define *) data;
...
/* {{{ jonj_oci_bind_hash_dtor()
 *
 * Bind hash destructor
 */
void jonj_oci_bind_hash_dtor(void *data)
{
	jonj_oci_bind *bind = (jonj_oci_bind *) data;
...
/* {{{ jonj_oci_column_hash_dtor()
 *
 * Column hash destructor
 */
void jonj_oci_column_hash_dtor(void *data)
{
	jonj_oci_out_column *column = (jonj_oci_out_column *) data;
	TSRMLS_FETCH();
...
/* {{{ jonj_oci_descriptor_flush_hash_dtor()
 *
 * Flush descriptors on commit
 */
void jonj_oci_descriptor_flush_hash_dtor(void *data)
{
	jonj_oci_descriptor *descriptor = *(jonj_oci_descriptor **)data;
	TSRMLS_FETCH();

	if (descriptor && descriptor->buffering == JONJ_OCI_LOB_BUFFER_USED && (descriptor->type == OCI_DTYPE_LOB || descriptor->type == OCI_DTYPE_FILE)) {
		jonj_oci_lob_flush(descriptor, OCI_LOB_BUFFER_FREE TSRMLS_CC);
		descriptor->buffering = JONJ_OCI_LOB_BUFFER_ENABLED;
	}
	data = NULL;
}
/* }}} */
...
/* {{{ jonj_oci_connection_descriptors_free()
 *
 ...
 */
sb4 jonj_oci_error(OCIError *err_p, sword errstatus TSRMLS_DC)
{
	...
	switch (errstatus) {
		...
		case OCI_SUCCESS_WITH_INFO:
			errcode = jonj_oci_fetch_errmsg(err_p, &errbuf TSRMLS_CC);
			if (errbuf) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_SUCCESS_WITH_INFO: %s", errbuf);
				efree(errbuf);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_SUCCESS_WITH_INFO: failed to fetch error message");
			}
			break;
		case OCI_NEED_DATA:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_NEED_DATA");
			break;
		case OCI_NO_DATA:
			errcode = jonj_oci_fetch_errmsg(err_p, &errbuf TSRMLS_CC);
			if (errbuf) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", errbuf);
				efree(errbuf);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_NO_DATA: failed to fetch error message");
			}
			break;
		case OCI_ERROR:
			errcode = jonj_oci_fetch_errmsg(err_p, &errbuf TSRMLS_CC);
			if (errbuf) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to fetch error message");
			}
			break;
		case OCI_INVALID_HANDLE:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_INVALID_HANDLE");
			break;
		case OCI_STILL_EXECUTING:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_STILL_EXECUTING");
			break;
		case OCI_CONTINUE:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_CONTINUE");
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown OCI error code: %d", errstatus);
			break;
	}
...
{
	sb4 error_code = 0;
	text err_buf[JONJ_OCI_ERRBUF_LEN];

	memset(err_buf, 0, sizeof(err_buf));
	JONJ_OCI_CALL(OCIErrorGet, (error_handle, (ub4)1, NULL, &error_code, err_buf, (ub4)JONJ_OCI_ERRBUF_LEN, (ub4)OCI_HTYPE_ERROR));
...
int jonj_oci_fetch_sqltext_offset(jonj_oci_statement *statement, text **sqltext, ub2 *error_offset TSRMLS_DC)
{
	...
	*sqltext = NULL;
	*error_offset = 0;
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (dvoid *) sqltext, (ub4 *)0, OCI_ATTR_STATEMENT, statement->err));

	if (errstatus != OCI_SUCCESS) {
		statement->errcode = jonj_oci_error(statement->err, errstatus TSRMLS_CC);
		JONJ_OCI_HANDLE_ERROR(statement->connection, statement->errcode);
		return 1;
	}

	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)statement->stmt, OCI_HTYPE_STMT, (ub2 *)error_offset, (ub4 *)0, OCI_ATTR_PARSE_ERROR_OFFSET, statement->err));
...
{
	jonj_oci_connection *connection;
	...
	connection = jonj_oci_do_connect_ex(username, username_len, password, password_len, NULL, 0, dbname, dbname_len, charset, session_mode, persistent, exclusive TSRMLS_CC);
...
jonj_oci_connection *jonj_oci_do_connect_ex(char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, char *dbname, int dbname_len, char *charset, long session_mode, int persistent, int exclusive TSRMLS_DC)
{
	...
	jonj_oci_connection *connection = NULL;
	smart_str hashed_details = {0};
	time_t timestamp;
	jonj_oci_spool *session_pool = NULL;
	...
	if (session_mode & ~(OCI_SYSOPER | OCI_SYSDBA | JONJ_OCI_CRED_EXT)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid session mode specified (%ld)", session_mode);
		return NULL;
	}
	if (session_mode & (OCI_SYSOPER | OCI_SYSDBA | JONJ_OCI_CRED_EXT)) {
		if ((session_mode & OCI_SYSOPER) && (session_mode & OCI_SYSDBA)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_SYSDBA and OCI_SYSOPER cannot be used together");
			return NULL;
		}
		if (session_mode & JONJ_OCI_CRED_EXT) {
#ifdef JONJ_WIN32
			 ...
			 */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "External Authentication is not supported on Windows");
			return NULL;
#endif
			if (username_len != 1 || username[0] != '/' || password_len != 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "OCI_CRED_EXT can only be used with a username of \"/\" and a NULL password");
				return NULL;
			}
		}
		if (session_mode & (OCI_SYSOPER | OCI_SYSDBA)) {
			...
			if (!OCI_G(privileged_connect)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Privileged connect is disabled. Enable oci8.privileged_connect to be able to connect as SYSOPER or SYSDBA");
				return NULL;
			}
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION < 4) || (JONJ_MAJOR_VERSION < 5)
			/* Safe mode has been removed in JONJ 5.4 */
			if (PG(safe_mode)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Privileged connect is disabled in Safe Mode");
				return NULL;
			}
#endif
		}
	...
	if (OCI_G(env) == NULL) {
		jonj_oci_init_global_handles(TSRMLS_C);
		if (OCI_G(env) == NULL) {
			return NULL;
		}
	}
	 ...
	 */
	if ((session_mode & (OCI_SYSOPER | OCI_SYSDBA | JONJ_OCI_CRED_EXT)) || (new_password_len)) {
		use_spool = 0;
	}
	...
	if (charset && *charset) {
		JONJ_OCI_CALL_RETURN(charsetid, OCINlsCharSetNameToId, (OCI_G(env), (CONST oratext *)charset));
		if (!charsetid) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid character set name: %s", charset);
		} else {
	...
	if (!charsetid) {
		...
		JONJ_OCI_CALL_RETURN(result, OCINlsEnvironmentVariableGet, (&charsetid_nls_lang, 0, OCI_NLS_CHARSET_ID, 0, &rsize));
	...
	/* make it lowercase */
	jonj_strtolower(hashed_details.c, hashed_details.len);
	...
	if (!exclusive && !new_password) {
		...
		if (persistent && zend_hash_find(&EG(persistent_list), hashed_details.c, hashed_details.len+1, (void **) &le) == SUCCESS) {
			...
			if (le->type == le_pconnection) {
				connection = (jonj_oci_connection *)le->ptr;
			}
			...
			if (le->type == le_index_ptr) {
				...
				if (ptr && (type == le_connection)) {
					connection = (jonj_oci_connection *)ptr;
				}
			}
		}
		...
		if (connection && connection->is_persistent && connection->is_stub) {
			if (jonj_oci_create_session(connection, NULL, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode TSRMLS_CC)) {
				smart_str_free_ex(&hashed_details, 0);
				zend_hash_del(&EG(persistent_list), connection->hash_key, strlen(connection->hash_key)+1);

				return NULL;
			}
			/* We do the ping in jonj_oci_create_session, no need to ping again below */
			ping_done = 1;
		}
		...
		if (connection) {
			if (connection->is_open) {
				...
				if (connection->is_persistent) {
					 ...
					 * 3) ping it if needed
					 */
					if (jonj_oci_connection_status(connection TSRMLS_CC)) {
						/* Only ping if:
						 *
						 ...
						 * still alive"
						 */
						if (!ping_done && (*(connection->next_pingp) > 0) && (timestamp >= *(connection->next_pingp)) && !jonj_oci_connection_ping(connection TSRMLS_CC)) {
							/* server died */
						} else {
							jonj_oci_connection *tmp;

							/* okay, the connection is open and the server is still alive */
							connection->used_this_request = 1;
							tmp = (jonj_oci_connection *)zend_list_find(connection->id, &rsrc_type);
							...
							if (tmp != NULL && rsrc_type == le_pconnection && strlen(tmp->hash_key) == hashed_details.len &&
								...
								JONJ_OCI_REGISTER_RESOURCE(connection, le_pconnection);
	...
	if (persistent) {
		...
		if (OCI_G(max_persistent) != -1 && OCI_G(num_persistent) >= OCI_G(max_persistent)) {
			...
			if (OCI_G(max_persistent) != -1 && OCI_G(num_persistent) >= OCI_G(max_persistent)) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Too many open persistent connections (%ld)", OCI_G(num_persistent));
				alloc_non_persistent = 1;
			}
		}

		if (alloc_non_persistent) {
			connection = (jonj_oci_connection *) ecalloc(1, sizeof(jonj_oci_connection));
			...
			connection = (jonj_oci_connection *) calloc(1, sizeof(jonj_oci_connection));
			if (connection == NULL) {
				return NULL;
			}
	 ...
	 * caching the physical connection (for DRCP) even when the non-persistent jonj connection is
	 * destroyed.
	 *
	 * TODO: Unconditionally do this once OCI provides extended OCISessionGet capability
	 */
	if (use_spool && !connection->is_persistent) {
		if ((session_pool = jonj_oci_get_spool(username, username_len, password, password_len, dbname, dbname_len, charsetid ? charsetid:charsetid_nls_lang TSRMLS_CC))==NULL)
		{
			jonj_oci_connection_close(connection TSRMLS_CC);
			smart_str_free_ex(&hashed_details, 0);
			return NULL;
	...
	/* Mark password as unchanged by JONJ during the duration of the database session */
	 ...
	 */
	if (!use_spool) {
		if (jonj_oci_old_create_session(connection, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode TSRMLS_CC)) {
			jonj_oci_connection_close(connection TSRMLS_CC);
			return NULL;
		}
	} else {
		/* create using the client-side session pool */
		if (jonj_oci_create_session(connection, session_pool, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode TSRMLS_CC)) {
			jonj_oci_connection_close(connection TSRMLS_CC);
			return NULL;
		}
	}
	...
	if (connection->is_persistent) {
		...
		JONJ_OCI_REGISTER_RESOURCE(connection, le_connection);
...
/* {{{ jonj_oci_connection_ping()
 *
 * Ping connection. Uses OCIPing() or OCIServerVersion() depending on the Oracle Client version
 */
static int jonj_oci_connection_ping(jonj_oci_connection *connection TSRMLS_DC)
{
	sword errstatus;
#if (!((OCI_MAJOR_VERSION > 10) || ((OCI_MAJOR_VERSION == 10) && (OCI_MINOR_VERSION >= 2))))
	char version[256];
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIPing, (connection->svc, OCI_G(err), OCI_DEFAULT));
#else
	/* use good old OCIServerVersion() */
	JONJ_OCI_CALL_RETURN(errstatus, OCIServerVersion, (connection->svc, OCI_G(err), (text *)version, sizeof(version), OCI_HTYPE_SVCCTX));
#endif
...
/* {{{ jonj_oci_connection_status()
 *
 * Check connection status (pre-ping check)
 */
static int jonj_oci_connection_status(jonj_oci_connection *connection TSRMLS_DC)
{
	ub4 ss = OCI_SERVER_NOT_CONNECTED;
	sword errstatus;

	/* get OCI_ATTR_SERVER_STATUS */
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)connection->server, OCI_HTYPE_SERVER, (dvoid *)&ss, (ub4 *)0, OCI_ATTR_SERVER_STATUS, OCI_G(err)));
...
int jonj_oci_connection_rollback(jonj_oci_connection *connection TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCITransRollback, (connection->svc, connection->err, (ub4) 0));
...
int jonj_oci_connection_commit(jonj_oci_connection *connection TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCITransCommit, (connection->svc, connection->err, (ub4) 0));
	connection->rb_on_disconnect = 0;
...
/* {{{ jonj_oci_connection_close()
 *
 * Close the connection and free all its resources
 */
static int jonj_oci_connection_close(jonj_oci_connection *connection TSRMLS_DC)
{
	int result = 0;
	...
	if (!connection->using_spool && connection->svc) {
		JONJ_OCI_CALL(OCISessionEnd, (connection->svc, connection->err, connection->session, (ub4) 0));
	}

	if (connection->err) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->err, (ub4) OCI_HTYPE_ERROR));
	}
	if (connection->authinfo) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->authinfo, (ub4) OCI_HTYPE_AUTHINFO));
	}

	/* No Handlefrees for session pool connections */
	if (!connection->using_spool) {
		if (connection->session) {
			JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->session, OCI_HTYPE_SESSION));
		}

		if (connection->is_attached) {
			JONJ_OCI_CALL(OCIServerDetach, (connection->server, OCI_G(err), OCI_DEFAULT));
		}

		if (connection->svc) {
			JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->svc, (ub4) OCI_HTYPE_SVCCTX));
		}

		if (connection->server) {
			JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->server, (ub4) OCI_HTYPE_SERVER));
		}

		if (connection->env) {
			JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) connection->env, OCI_HTYPE_ENV));
		}
	 ...
	 */
		jonj_oci_spool_close(connection->private_spool TSRMLS_CC);
	}
...
int jonj_oci_connection_release(jonj_oci_connection *connection TSRMLS_DC)
{
	...
	if (connection->descriptors) {
		jonj_oci_connection_descriptors_free(connection TSRMLS_CC);
	}

	if (connection->svc) {
		/* rollback outstanding transactions */
		if (connection->rb_on_disconnect) {
			if (jonj_oci_connection_rollback(connection TSRMLS_CC)) {
				/* rollback failed */
				result = 1;
			}
		}
	...
	if (connection->using_spool) {
		...
		if (connection->svc) {
			JONJ_OCI_CALL(OCISessionRelease, (connection->svc, connection->err, NULL,
										 0, rlsMode));
		}
...
int jonj_oci_password_change(jonj_oci_connection *connection, char *user, int user_len, char *pass_old, int pass_old_len, char *pass_new, int pass_new_len TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIPasswordChange, (connection->svc, connection->err, (text *)user, user_len, (text *)pass_old, pass_old_len, (text *)pass_new, pass_new_len, OCI_DEFAULT));
...
void jonj_oci_client_get_version(char **version TSRMLS_DC)
{
	...
	JONJ_OCI_CALL(OCIClientVersion, (&major_version, &minor_version, &update_num, &patch_num, &port_update_num));
...
int jonj_oci_server_get_version(jonj_oci_connection *connection, char **version TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIServerVersion, (connection->svc, connection->err, (text *)version_buff, sizeof(version_buff), OCI_HTYPE_SVCCTX));
...
/* {{{ jonj_oci_column_to_zval()
 *
 * Convert jonj_oci_out_column struct into zval
 */
int jonj_oci_column_to_zval(jonj_oci_out_column *column, zval *value, int mode TSRMLS_DC)
{
	jonj_oci_descriptor *descriptor;
	ub4 lob_length;
	int column_size;
	char *lob_buffer;
	...
	if (column->is_cursor) { /* REFCURSOR -> simply return the statement id */
		...
		if (column->data_type != SQLT_RDD) {
			...
			descriptor = (jonj_oci_descriptor *) zend_list_find(column->descid, &rsrc_type);

			if (!descriptor || rsrc_type != le_descriptor) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find LOB descriptor #%d", column->descid);
				return 1;
			}
		...
		if (column->data_type != SQLT_RDD && (mode & JONJ_OCI_RETURN_LOBS)) {
			/* JONJ_OCI_RETURN_LOBS means that we want the content of the LOB back instead of the locator */

			if (column->chunk_size)
				descriptor->chunk_size = column->chunk_size;			
			lob_fetch_status = jonj_oci_lob_read(descriptor, -1, 0, &lob_buffer, &lob_length TSRMLS_CC);
			...
			jonj_oci_temp_lob_close(descriptor TSRMLS_CC);
			if (lob_fetch_status) {
				ZVAL_FALSE(value);
				return 1;
			} else {
...
void jonj_oci_fetch_row (INTERNAL_FUNCTION_PARAMETERS, int mode, int expected_args)
{
	...
	jonj_oci_statement *statement;		  /* statement that will be fetched from */
#if (OCI_MAJOR_VERSION >= 12)
	jonj_oci_statement *invokedstatement;  /* statement this function was invoked with */
#endif /* OCI_MAJOR_VERSION */
	jonj_oci_out_column *column;
	ub4 nrows = 1;
	int i;
	long fetch_mode = 0;
	...
	if (!(fetch_mode & JONJ_OCI_NUM) && !(fetch_mode & JONJ_OCI_ASSOC)) {
		/* none of the modes present, use the default one */
		if (mode & JONJ_OCI_ASSOC) {
			fetch_mode |= JONJ_OCI_ASSOC;
		}
		if (mode & JONJ_OCI_NUM) {
			fetch_mode |= JONJ_OCI_NUM;
		}
	}

#if (OCI_MAJOR_VERSION < 12)
	JONJ_OCI_ZVAL_TO_STATEMENT(z_statement, statement);

	if (jonj_oci_statement_fetch(statement, nrows TSRMLS_CC)) {
		RETURN_FALSE;			/* end of fetch */
	}
#else /* OCI_MAJOR_VERSION */
	JONJ_OCI_ZVAL_TO_STATEMENT(z_statement, invokedstatement);

	if (invokedstatement->impres_flag == JONJ_OCI_IMPRES_NO_CHILDREN ||
        invokedstatement->impres_flag == JONJ_OCI_IMPRES_IS_CHILD) {
		/* Already know there are no Implicit Result Sets */
	    statement = invokedstatement; 
	} else if (invokedstatement->impres_flag == JONJ_OCI_IMPRES_HAS_CHILDREN) {
		/* Previously saw an Implicit Result Set in an earlier invocation of jonj_oci_fetch_row */
		statement = (jonj_oci_statement *)invokedstatement->impres_child_stmt;
	} else {
		sword errstatus;
		
		/* Check for an Implicit Result Set on this statement handle */
		JONJ_OCI_CALL_RETURN(errstatus, OCIAttrGet, ((dvoid *)invokedstatement->stmt, OCI_HTYPE_STMT, 
		...
		if (invokedstatement->impres_count > 0) {
			...
			statement = jonj_oci_get_implicit_resultset(invokedstatement TSRMLS_CC);
			if (!statement || jonj_oci_statement_execute(statement, (ub4)OCI_DEFAULT TSRMLS_CC))
				RETURN_FALSE;
			invokedstatement->impres_count--;
			invokedstatement->impres_child_stmt = (struct jonj_oci_statement *)statement;
			invokedstatement->impres_flag = JONJ_OCI_IMPRES_HAS_CHILDREN;
		} else {
			statement = invokedstatement; /* didn't find Implicit Result Sets */
			invokedstatement->impres_flag = JONJ_OCI_IMPRES_NO_CHILDREN;  /* Don't bother checking again */
		}
	}
	...
	for (i = 0; i < statement->ncolumns; i++) {
		...
		column = jonj_oci_statement_get_column(statement, i + 1, NULL, 0 TSRMLS_CC);

		if (column == NULL) {
			continue;
		}
		if ((column->indicator == -1) && ((fetch_mode & JONJ_OCI_RETURN_NULLS) == 0)) {
			continue;
		}
		...
		if (!(column->indicator == -1)) {
			...
			jonj_oci_column_to_zval(column, element, fetch_mode TSRMLS_CC);

			if (fetch_mode & JONJ_OCI_NUM || !(fetch_mode & JONJ_OCI_ASSOC)) {
				add_index_zval(return_value, i, element);
			}
			if (fetch_mode & JONJ_OCI_ASSOC) {
				if (fetch_mode & JONJ_OCI_NUM) {
					Z_ADDREF_P(element);
				}
...
static jonj_oci_spool *jonj_oci_create_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, char *hash_key, int hash_key_len, int charsetid TSRMLS_DC)
{
	...
	session_pool = (jonj_oci_spool *) calloc(1, sizeof(jonj_oci_spool));
	...
	if (!(session_pool->env = jonj_oci_create_env(charsetid TSRMLS_CC))) {
		iserror = 1;
		goto exit_create_spool;
	}

	/* Allocate the pool handle */
	JONJ_OCI_CALL_RETURN(errstatus, OCIHandleAlloc, (session_pool->env, (dvoid **) &session_pool->poolh, OCI_HTYPE_SPOOL, (size_t) 0, (dvoid **) 0));

	if (errstatus != OCI_SUCCESS) {
		OCI_G(errcode) = jonj_oci_error(OCI_G(err), errstatus TSRMLS_CC);
		iserror = 1;
		goto exit_create_spool;
	}
	 ...
	 */
	JONJ_OCI_CALL_RETURN(errstatus, OCIHandleAlloc, ((dvoid *) session_pool->env, (dvoid **)&(session_pool->err), (ub4) OCI_HTYPE_ERROR,(size_t) 0, (dvoid **) 0));
...
/* Disable RLB as we mostly have single-connection pools */
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIHandleAlloc, (session_pool->env, (dvoid **)&(spoolAuth), OCI_HTYPE_AUTHINFO, 0, NULL));
	...
	if (OCI_G(edition)) {
		JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) spoolAuth, (ub4) OCI_HTYPE_AUTHINFO, (dvoid *) OCI_G(edition), (ub4)(strlen(OCI_G(edition))), (ub4)OCI_ATTR_EDITION, OCI_G(err)));
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) spoolAuth, (ub4) OCI_HTYPE_AUTHINFO, (dvoid *) JONJ_OCI8_DRIVER_NAME, (ub4) sizeof(JONJ_OCI8_DRIVER_NAME)-1, (ub4) OCI_ATTR_DRIVER_NAME, OCI_G(err)));
	...
	JONJ_OCI_CALL_RETURN(errstatus, OCIAttrSet, ((dvoid *) (session_pool->poolh),(ub4) OCI_HTYPE_SPOOL, (dvoid *) spoolAuth, (ub4)0, (ub4)OCI_ATTR_SPOOL_AUTH, OCI_G(err)));
	 ...
	 */
	JONJ_OCI_CALL_RETURN(errstatus, OCISessionPoolCreate,(session_pool->env, OCI_G(err), session_pool->poolh, (OraText **)&session_pool->poolname, &session_pool->poolname_len, (OraText *)dbname, (ub4)dbname_len, 0, UB4MAXVAL, 1,(OraText *)username, (ub4)username_len, (OraText *)password,(ub4)password_len, poolmode));
...
exit_create_spool:
	if (iserror && session_pool) {
		jonj_oci_spool_close(session_pool TSRMLS_CC);
		session_pool = NULL;
	}

	if (spoolAuth) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) spoolAuth, (ub4) OCI_HTYPE_AUTHINFO));
	}
...
static jonj_oci_spool *jonj_oci_get_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, int charsetid TSRMLS_DC)
{
	...
	jonj_strtolower(spool_hashed_details.c, spool_hashed_details.len);
	/* }}} */

	if (zend_hash_find(&EG(persistent_list),spool_hashed_details.c, spool_hashed_details.len+1, (void **)&spool_out_le) == FAILURE) {

		session_pool = jonj_oci_create_spool(username, username_len, password, password_len, dbname, dbname_len, spool_hashed_details.c, spool_hashed_details.len, charsetid TSRMLS_CC);
		...
		JONJ_OCI_REGISTER_RESOURCE(session_pool, le_psessionpool);
		...
		strlen(((jonj_oci_spool *)(spool_out_le->ptr))->spool_hash_key) == spool_hashed_details.len &&
		memcmp(((jonj_oci_spool *)(spool_out_le->ptr))->spool_hash_key, spool_hashed_details.c, spool_hashed_details.len) == 0) {
		/* retrieve the cached session pool */
		session_pool = (jonj_oci_spool *)(spool_out_le->ptr);
	}
...
static OCIEnv *jonj_oci_create_env(ub2 charsetid TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIEnvNlsCreate, (&retenv, OCI_G(events) ? JONJ_OCI_INIT_MODE | OCI_EVENTS : JONJ_OCI_INIT_MODE, 0, NULL, NULL, NULL, 0, NULL, charsetid, charsetid));
	...
	if (OCI_G(errcode) != OCI_SUCCESS) {
		...
		if (retenv
			...
			&& *ora_msg_buf) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ora_msg_buf);
		}
...
static int jonj_oci_old_create_session(jonj_oci_connection *connection, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode TSRMLS_DC)
{
	...
	if (!(connection->env = jonj_oci_create_env(connection->charset TSRMLS_CC))) {
		return 1;
	}

	/* {{{ Allocate our server handle */
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIHandleAlloc, (connection->env, (dvoid **)&(connection->server), OCI_HTYPE_SERVER, 0, NULL));

	if (OCI_G(errcode) != OCI_SUCCESS) {
		jonj_oci_error(OCI_G(err), OCI_G(errcode) TSRMLS_CC);
		return 1;
	}
	/* }}} */

	/* {{{ Attach to the server */
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIServerAttach, (connection->server, OCI_G(err), (text *)dbname, dbname_len, (ub4) OCI_DEFAULT));
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIHandleAlloc, (connection->env, (dvoid **)&(connection->session), OCI_HTYPE_SESSION, 0, NULL));
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIHandleAlloc, (connection->env, (dvoid **)&(connection->err), OCI_HTYPE_ERROR, 0, NULL));
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIHandleAlloc, (connection->env, (dvoid **)&(connection->svc), OCI_HTYPE_SVCCTX, 0, NULL));
	...
	/* {{{ Set the username */
	if (username) {
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) username, (ub4) username_len, (ub4) OCI_ATTR_USERNAME, OCI_G(err)));
	...
	/* {{{ Set the password */
	if (password) {
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) password, (ub4) password_len, (ub4) OCI_ATTR_PASSWORD, OCI_G(err)));
...
#if ((OCI_MAJOR_VERSION > 11) || ((OCI_MAJOR_VERSION == 11) && (OCI_MINOR_VERSION >= 2)))
	if (OCI_G(edition)) {
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) OCI_G(edition), (ub4) (strlen(OCI_G(edition))), (ub4) OCI_ATTR_EDITION, OCI_G(err)));
...
#if (OCI_MAJOR_VERSION >= 11)
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->session, (ub4) OCI_HTYPE_SESSION, (dvoid *) JONJ_OCI8_DRIVER_NAME, (ub4) sizeof(JONJ_OCI8_DRIVER_NAME)-1, (ub4) OCI_ATTR_DRIVER_NAME, OCI_G(err)));
...
/* }}} */
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, (connection->svc, OCI_HTYPE_SVCCTX, connection->server, 0, OCI_ATTR_SERVER, OCI_G(err)));
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, (connection->svc, OCI_HTYPE_SVCCTX, connection->session, 0, OCI_ATTR_SESSION, OCI_G(err)));
	...
	if (new_password) {
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIPasswordChange, (connection->svc, OCI_G(err), (text *)username, username_len, (text *)password, password_len, (text *)new_password, new_password_len, OCI_AUTH));
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrGet, ((dvoid *)connection->svc, OCI_HTYPE_SVCCTX, (dvoid *)&(connection->session), (ub4 *)0, OCI_ATTR_SESSION, OCI_G(err)));
		...
		if (session_mode & JONJ_OCI_CRED_EXT) {
			...
			session_mode ^= JONJ_OCI_CRED_EXT;
		}

		session_mode |= OCI_STMT_CACHE;

		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCISessionBegin, (connection->svc, OCI_G(err), connection->session, (ub4) cred_type, (ub4) session_mode));
	...
	if (jonj_oci_ping_init(connection, OCI_G(err) TSRMLS_CC) != OCI_SUCCESS) {
		jonj_oci_error(OCI_G(err), OCI_G(errcode) TSRMLS_CC);
		return 1;
	}

	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->svc, (ub4) OCI_HTYPE_SVCCTX, (ub4 *) &statement_cache_size, 0, (ub4) OCI_ATTR_STMTCACHESIZE, OCI_G(err)));
...
/* {{{ jonj_oci_create_session()
 *
 * Create session using client-side session pool - new norm
 */
static int jonj_oci_create_session(jonj_oci_connection *connection, jonj_oci_spool *session_pool, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode TSRMLS_DC)
{
	jonj_oci_spool *actual_spool = NULL;
#if (OCI_MAJOR_VERSION > 10)
	...
	if (connection->is_persistent && !connection->private_spool &&
		!(connection->private_spool = jonj_oci_create_spool(username, username_len, password, password_len, dbname, dbname_len, NULL, 0, connection->charset TSRMLS_CC))) {
			return 1;
	}
	...
	if (!connection->authinfo) {
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIHandleAlloc, (connection->env, (dvoid **)&(connection->authinfo), OCI_HTYPE_AUTHINFO, 0, NULL));
...
#if (OCI_MAJOR_VERSION > 10)
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrSet, ((dvoid *) connection->authinfo,(ub4) OCI_HTYPE_SESSION, (dvoid *) OCI_G(connection_class), (ub4)(strlen(OCI_G(connection_class))), (ub4)OCI_ATTR_CONNECTION_CLASS, OCI_G(err)));
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode),OCIAttrSet, ((dvoid *) connection->authinfo,(ub4) OCI_HTYPE_AUTHINFO, (dvoid *) &purity, (ub4)0, (ub4)OCI_ATTR_PURITY, OCI_G(err)));
	...
	if (DTRACE_OCI8_SESSPOOL_STATS_ENABLED()) {
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrGet, ((dvoid *)actual_spool->poolh, OCI_HTYPE_SPOOL, (dvoid *)&numopen, (ub4 *)0, OCI_ATTR_SPOOL_OPEN_COUNT, OCI_G(err)));
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrGet, ((dvoid *)actual_spool->poolh, OCI_HTYPE_SPOOL, (dvoid *)&numbusy, (ub4 *)0, OCI_ATTR_SPOOL_BUSY_COUNT, OCI_G(err)));
	...
	do {
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode),OCISessionGet, (connection->env, OCI_G(err), &(connection->svc), (OCIAuthInfo *)connection->authinfo, (OraText *)actual_spool->poolname, (ub4)actual_spool->poolname_len, NULL, 0, NULL, NULL, NULL, OCI_SESSGET_SPOOL));
		...
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrGet, ((dvoid *)connection->svc, OCI_HTYPE_SVCCTX, (dvoid *)&(connection->server), (ub4 *)0, OCI_ATTR_SERVER, OCI_G(err)));

		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIAttrGet, ((dvoid *)connection->svc, OCI_HTYPE_SVCCTX, (dvoid *)&(connection->session), (ub4 *)0, OCI_ATTR_SESSION, OCI_G(err))); 
		/* }}} */

		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIContextGetValue, (connection->session, OCI_G(err), (ub1 *)"NEXT_PING", (ub1)sizeof("NEXT_PING"), (void **)&(connection->next_pingp)));
		if (OCI_G(errcode) != OCI_SUCCESS) {
			jonj_oci_error(OCI_G(err), OCI_G(errcode) TSRMLS_CC);
			return 1;
		}
		...
		if (!(connection->next_pingp)){
			...
			if (jonj_oci_connection_ping(connection TSRMLS_CC)) {
				/* Got a good connection - update next_ping and get out of ping loop */
				*(connection->next_pingp) = timestamp + OCI_G(ping_interval);
			} else {
				/* Bad connection - remove from pool */
				JONJ_OCI_CALL(OCISessionRelease, (connection->svc, connection->err, NULL,0, (ub4) OCI_SESSRLS_DROPSESS));
				connection->svc = NULL;
				connection->server = NULL;
				connection->session = NULL;
			}
...
/* {{{ jonj_oci_spool_list_dtor()
 *
 * Session pool destructor function
 */
static void jonj_oci_spool_list_dtor(zend_rsrc_list_entry *entry TSRMLS_DC)
{
	jonj_oci_spool *session_pool = (jonj_oci_spool *)entry->ptr;
...
static void jonj_oci_spool_close(jonj_oci_spool *session_pool TSRMLS_DC)
{
	if (session_pool->poolname_len) {
		JONJ_OCI_CALL(OCISessionPoolDestroy, ((dvoid *) session_pool->poolh,
			(dvoid *) session_pool->err, OCI_SPD_FORCE));
	}

	if (session_pool->poolh) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) session_pool->poolh, OCI_HTYPE_SPOOL));
	}

	if (session_pool->err) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) session_pool->err, OCI_HTYPE_ERROR));
	}

	if (session_pool->env) {
		JONJ_OCI_CALL(OCIHandleFree, ((dvoid *) session_pool->env, OCI_HTYPE_ENV));
	}
...
static sword jonj_oci_ping_init(jonj_oci_connection *connection, OCIError *errh TSRMLS_DC)
{
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIContextGetValue, (connection->session, errh, (ub1 *)"NEXT_PING", (ub1)sizeof("NEXT_PING"), (void **)&next_pingp));
	if (OCI_G(errcode) != OCI_SUCCESS) {
	...
	if (!next_pingp) {
		JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIMemoryAlloc, (connection->session, errh, (void **)&next_pingp, OCI_DURATION_SESSION, sizeof(time_t), OCI_MEMORY_CLEARED));
		if (OCI_G(errcode) != OCI_SUCCESS) {
			return OCI_G(errcode);
		}
	}
	...
	JONJ_OCI_CALL_RETURN(OCI_G(errcode), OCIContextSetValue, (connection->session, errh, OCI_DURATION_SESSION, (ub1 *)"NEXT_PING", (ub1)sizeof("NEXT_PING"), next_pingp));
	if (OCI_G(errcode) != OCI_SUCCESS) {
...
/* {{{ jonj_oci_dtrace_check_connection()
 *
