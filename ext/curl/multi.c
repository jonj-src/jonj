...
JONJ_FUNCTION(curl_multi_init)
{
	...
	mh = ecalloc(1, sizeof(jonj_curlm));
	mh->multi = curl_multi_init();

	zend_llist_init(&mh->easyh, sizeof(zval), _jonj_curl_multi_cleanup_list, 0);
...
JONJ_FUNCTION(curl_multi_add_handle)
{
	...
	ZEND_FETCH_RESOURCE(mh, jonj_curlm *, &z_mh, -1, le_curl_multi_handle_name, le_curl_multi_handle);
	ZEND_FETCH_RESOURCE(ch, jonj_curl *, &z_ch, -1, le_curl_name, le_curl);

	_jonj_curl_cleanup_handle(ch);
...
void _jonj_curl_multi_cleanup_list(void *data) /* {{{ */
{
	...
	ch = (jonj_curl *) zend_fetch_resource(&z_ch TSRMLS_CC, -1, le_curl_name, NULL, 1, le_curl);
	if (!ch) {
		return;
	}
...
JONJ_FUNCTION(curl_multi_exec)
{
...

	{
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			ZEND_FETCH_RESOURCE(ch, jonj_curl *, &pz_ch, -1, le_curl_name, le_curl);
			_jonj_curl_verify_handlers(ch, 1 TSRMLS_CC);
		}
	}
...
JONJ_FUNCTION(curl_multi_getcontent)
{
	...
	if (ch->handlers->write->method == JONJ_CURL_RETURN) {
	...
	/* find the original easy curl handle */
	{
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			if (ch->cp == tmp_msg->easy_handle) {
				...
				/* we are adding a reference to the underlying jonj_curl
...
void _jonj_curl_multi_close(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	jonj_curlm *mh = (jonj_curlm *) rsrc->ptr;
	if (mh) {
		zend_llist_position pos;
		jonj_curl *ch;
		zval	*pz_ch;
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			ch = (jonj_curl *) zend_fetch_resource(&pz_ch TSRMLS_CC, -1, le_curl_name, NULL, 1, le_curl);
			_jonj_curl_verify_handlers(ch, 0 TSRMLS_CC);
		}
...
static int _jonj_curl_multi_setopt(jonj_curlm *mh, long option, zval **zvalue, zval *return_value TSRMLS_DC) /* {{{ */
{ 
	...
	switch (option) {
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid curl multi configuration option");
...
JONJ_FUNCTION(curl_multi_setopt)
{
	...
	if (!_jonj_curl_multi_setopt(mh, options, zvalue, return_value TSRMLS_CC)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
