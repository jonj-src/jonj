...
# ifdef JONJ_WIN32
#  define JONJ_CURL_NEED_OPENSSL_TSL
...
#    define JONJ_CURL_NEED_GNUTLS_TSL
#    include <gcrypt.h>
#   else
#    warning \
...
#ifdef JONJ_CURL_NEED_OPENSSL_TSL /* {{{ */
static MUTEX_T *jonj_curl_openssl_tsl = NULL;

static void jonj_curl_ssl_lock(int mode, int n, const char * file, int line)
{
	if (mode & CRYPTO_LOCK) {
		tsrm_mutex_lock(jonj_curl_openssl_tsl[n]);
	} else {
		tsrm_mutex_unlock(jonj_curl_openssl_tsl[n]);
	}
}

static unsigned long jonj_curl_ssl_id(void)
{
...
#ifdef JONJ_CURL_NEED_GNUTLS_TSL /* {{{ */
static int jonj_curl_ssl_mutex_create(void **m)
{
...
static int jonj_curl_ssl_mutex_destroy(void **m)
{
	tsrm_mutex_free(*((MUTEX_T *) m));
	return SUCCESS;
}

static int jonj_curl_ssl_mutex_lock(void **m)
{
	return tsrm_mutex_lock(*((MUTEX_T *) m));
}

static int jonj_curl_ssl_mutex_unlock(void **m)
{
	return tsrm_mutex_unlock(*((MUTEX_T *) m));
}

static struct gcry_thread_cbs jonj_curl_gnutls_tsl = {
	GCRY_THREAD_OPTION_USER,
	NULL,
	jonj_curl_ssl_mutex_create,
	jonj_curl_ssl_mutex_destroy,
	jonj_curl_ssl_mutex_lock,
	jonj_curl_ssl_mutex_unlock
};
#endif
/* }}} */

static void _jonj_curl_close_ex(jonj_curl *ch TSRMLS_DC);
static void _jonj_curl_close(zend_rsrc_list_entry *rsrc TSRMLS_DC);
...
#if defined(JONJ_WIN32) || defined(__GNUC__)
# define jonj_curl_ret(__ret) RETVAL_FALSE; return __ret;
#else
# define jonj_curl_ret(__ret) RETVAL_FALSE; return;
#endif

static int jonj_curl_option_str(jonj_curl *ch, long option, const char *str, const int len, zend_bool make_copy TSRMLS_DC)
{
	CURLcode error = CURLE_OK;

	if (strlen(str) != len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Curl option contains invalid characters (\\0)");
		return FAILURE;
	}
...
static int jonj_curl_option_url(jonj_curl *ch, const char *url, const int len TSRMLS_DC) /* {{{ */
{
	...
	if (PG(open_basedir) && *PG(open_basedir)) {
		...
		if (!(uri = jonj_url_parse_ex(url, len))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid URL '%s'", url);
			return FAILURE;
		}

		if (uri->scheme && !strncasecmp("file", uri->scheme, sizeof("file"))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Protocol 'file' disabled in cURL");
			jonj_url_free(uri);
			return FAILURE;
		}
		jonj_url_free(uri);
	...
	return jonj_curl_option_str(ch, CURLOPT_URL, url, len, 0 TSRMLS_CC);
}
/* }}} */
...
void _jonj_curl_verify_handlers(jonj_curl *ch, int reporterror TSRMLS_DC) /* {{{ */
{
	...
	if (ch->handlers->std_err) {
		stream = (jonj_stream *) zend_fetch_resource(&ch->handlers->std_err TSRMLS_CC, -1, NULL, NULL, 2, jonj_file_le_stream(), jonj_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_STDERR resource has gone away, resetting to stderr");
			}
	...
	if (ch->handlers->read && ch->handlers->read->stream) {
		stream = (jonj_stream *) zend_fetch_resource(&ch->handlers->read->stream TSRMLS_CC, -1, NULL, NULL, 2, jonj_file_le_stream(), jonj_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_INFILE resource has gone away, resetting to default");
			}
	...
	if (ch->handlers->write_header && ch->handlers->write_header->stream) {
		stream = (jonj_stream *) zend_fetch_resource(&ch->handlers->write_header->stream TSRMLS_CC, -1, NULL, NULL, 2, jonj_file_le_stream(), jonj_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_WRITEHEADER resource has gone away, resetting to default");
			}
			zval_ptr_dtor(&ch->handlers->write_header->stream);
			ch->handlers->write_header->fp = 0;
			ch->handlers->write_header->stream = NULL;

			ch->handlers->write_header->method = JONJ_CURL_IGNORE;
			curl_easy_setopt(ch->cp, CURLOPT_WRITEHEADER, (void *) ch);
		}
	}
	if (ch->handlers->write && ch->handlers->write->stream) {
		stream = (jonj_stream *) zend_fetch_resource(&ch->handlers->write->stream TSRMLS_CC, -1, NULL, NULL, 2, jonj_file_le_stream(), jonj_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_FILE resource has gone away, resetting to default");
			}
			zval_ptr_dtor(&ch->handlers->write->stream);
			ch->handlers->write->fp = 0;
			ch->handlers->write->stream = NULL;

			ch->handlers->write->method = JONJ_CURL_STDOUT;
...
const zend_function_entry curl_functions[] = {
	JONJ_FE(curl_init,                arginfo_curl_init)
	JONJ_FE(curl_copy_handle,         arginfo_curl_copy_handle)
	JONJ_FE(curl_version,             arginfo_curl_version)
	JONJ_FE(curl_setopt,              arginfo_curl_setopt)
	JONJ_FE(curl_setopt_array,        arginfo_curl_setopt_array)
	JONJ_FE(curl_exec,                arginfo_curl_exec)
	JONJ_FE(curl_getinfo,             arginfo_curl_getinfo)
	JONJ_FE(curl_error,               arginfo_curl_error)
	JONJ_FE(curl_errno,               arginfo_curl_errno)
	JONJ_FE(curl_close,               arginfo_curl_close)
#if LIBCURL_VERSION_NUM >= 0x070c00 /* 7.12.0 */
	JONJ_FE(curl_strerror,            arginfo_curl_strerror)
	JONJ_FE(curl_multi_strerror,      arginfo_curl_multi_strerror)
#endif
#if LIBCURL_VERSION_NUM >= 0x070c01 /* 7.12.1 */
	JONJ_FE(curl_reset,               arginfo_curl_reset)
#endif
#if LIBCURL_VERSION_NUM >= 0x070f04 /* 7.15.4 */
	JONJ_FE(curl_escape,              arginfo_curl_escape)
	JONJ_FE(curl_unescape,            arginfo_curl_unescape)
#endif
#if LIBCURL_VERSION_NUM >= 0x071200 /* 7.18.0 */
	JONJ_FE(curl_pause,               arginfo_curl_pause)
#endif
	JONJ_FE(curl_multi_init,          arginfo_curl_multi_init)
	JONJ_FE(curl_multi_add_handle,    arginfo_curl_multi_add_handle)
	JONJ_FE(curl_multi_remove_handle, arginfo_curl_multi_remove_handle)
	JONJ_FE(curl_multi_select,        arginfo_curl_multi_select)
	JONJ_FE(curl_multi_exec,          arginfo_curl_multi_exec)
	JONJ_FE(curl_multi_getcontent,    arginfo_curl_multi_getcontent)
	JONJ_FE(curl_multi_info_read,     arginfo_curl_multi_info_read)
	JONJ_FE(curl_multi_close,         arginfo_curl_multi_close)
#if LIBCURL_VERSION_NUM >= 0x070f04 /* 7.15.4 */
	JONJ_FE(curl_multi_setopt,        arginfo_curl_multi_setopt)
#endif
	JONJ_FE(curl_share_init,          arginfo_curl_share_init)
	JONJ_FE(curl_share_close,         arginfo_curl_share_close)
	JONJ_FE(curl_share_setopt,        arginfo_curl_share_setopt)
	JONJ_FE(curl_file_create,         arginfo_curlfile_create)
	JONJ_FE_END
};
/* }}} */
...
/* {{{ JONJ_INI_BEGIN */
JONJ_INI_BEGIN()
	JONJ_INI_ENTRY("curl.cainfo", "", JONJ_INI_SYSTEM, NULL)
JONJ_INI_END()
/* }}} */
...
JONJ_MINFO_FUNCTION(curl)
{
	...
	jonj_info_print_table_row(2, "cURL support",    "enabled");
	jonj_info_print_table_row(2, "cURL Information", d->version);
	sprintf(str, "%d", d->age);
	jonj_info_print_table_row(2, "Age", str);
	...
	if (d->features) {
		...
		jonj_info_print_table_row(1, "Features");
		for(i=0; i<sizeof(feats)/sizeof(feats[0]); i++) {
			if (feats[i].name) {
				jonj_info_print_table_row(2, feats[i].name, d->features & feats[i].bitmask ? "Yes" : "No");
			}
		}
	}
	...
	jonj_info_print_table_row(2, "Protocols", str);

	jonj_info_print_table_row(2, "Host", d->host);

	if (d->ssl_version) {
		jonj_info_print_table_row(2, "SSL Version", d->ssl_version);
	}

	if (d->libz_version) {
		jonj_info_print_table_row(2, "ZLib Version", d->libz_version);
	}

#if defined(CURLVERSION_SECOND) && CURLVERSION_NOW >= CURLVERSION_SECOND
	if (d->ares) {
		jonj_info_print_table_row(2, "ZLib Version", d->ares);
	}
#endif

#if defined(CURLVERSION_THIRD) && CURLVERSION_NOW >= CURLVERSION_THIRD
	if (d->libidn) {
		jonj_info_print_table_row(2, "libIDN Version", d->libidn);
	}
#endif
	...
	if (d->iconv_ver_num) {
		jonj_info_print_table_row(2, "IconV Version", d->iconv_ver_num);
	}

	if (d->libssh_version) {
		jonj_info_print_table_row(2, "libSSH Version", d->libssh_version);
	}
#endif
...
{
	le_curl = zend_register_list_destructors_ex(_jonj_curl_close, NULL, "curl", module_number);
	le_curl_multi_handle = zend_register_list_destructors_ex(_jonj_curl_multi_close, NULL, "curl_multi", module_number);
	le_curl_share_handle = zend_register_list_destructors_ex(_jonj_curl_share_close, NULL, "curl_share", module_number);
	...
	if (!CRYPTO_get_id_callback()) {
		...
		jonj_curl_openssl_tsl = malloc(c * sizeof(MUTEX_T));
		if (!jonj_curl_openssl_tsl) {
			return FAILURE;
		}

		for (i = 0; i < c; ++i) {
			jonj_curl_openssl_tsl[i] = tsrm_mutex_alloc();
		}

		CRYPTO_set_id_callback(jonj_curl_ssl_id);
		CRYPTO_set_locking_callback(jonj_curl_ssl_lock);
	}
#endif
#ifdef JONJ_CURL_NEED_GNUTLS_TSL
	gcry_control(GCRYCTL_SET_THREAD_CBS, &jonj_curl_gnutls_tsl);
#endif
...
JONJ_MSHUTDOWN_FUNCTION(curl)
{
	...
	if (jonj_curl_openssl_tsl) {
		...
		for (i = 0; i < c; ++i) {
			tsrm_mutex_free(jonj_curl_openssl_tsl[i]);
		}

		free(jonj_curl_openssl_tsl);
		jonj_curl_openssl_tsl = NULL;
	}
#endif
...
/* {{{ curl_write_nothing
 * Used as a work around. See _jonj_curl_close_ex
 */
...
{
	jonj_curl       *ch     = (jonj_curl *) ctx;
	jonj_curl_write *t      = ch->handlers->write;
	...
	switch (t->method) {
		case JONJ_CURL_STDOUT:
			JONJWRITE(data, length);
			break;
		case JONJ_CURL_FILE:
			return fwrite(data, size, nmemb, t->fp);
		case JONJ_CURL_RETURN:
			if (length > 0) {
		...
		case JONJ_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not call the CURLOPT_WRITEFUNCTION");
				length = -1;
			} else if (retval_ptr) {
...
{
	jonj_curl       *ch = (jonj_curl *) ctx;
	jonj_curl_fnmatch *t = ch->handlers->fnmatch;
	...
	switch (t->method) {
		case JONJ_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot call the CURLOPT_FNMATCH_FUNCTION");
			} else if (retval_ptr) {
				_jonj_curl_verify_handlers(ch, 1 TSRMLS_CC);
				if (Z_TYPE_P(retval_ptr) != IS_LONG) {
					convert_to_long_ex(&retval_ptr);
...
{
	jonj_curl       *ch = (jonj_curl *) clientp;
	jonj_curl_progress  *t  = ch->handlers->progress;
	size_t	rval = 0;
	...
	switch (t->method) {
		case JONJ_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot call the CURLOPT_PROGRESSFUNCTION");
			} else if (retval_ptr) {
				_jonj_curl_verify_handlers(ch, 1 TSRMLS_CC);
				if (Z_TYPE_P(retval_ptr) != IS_LONG) {
					convert_to_long_ex(&retval_ptr);
...
{
	jonj_curl       *ch = (jonj_curl *) ctx;
	jonj_curl_read  *t  = ch->handlers->read;
	int             length = 0;

	switch (t->method) {
		case JONJ_CURL_DIRECT:
			if (t->fp) {
		...
		case JONJ_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot call the CURLOPT_READFUNCTION");
#if LIBCURL_VERSION_NUM >= 0x070c01 /* 7.12.1 */
...
{
	jonj_curl       *ch  = (jonj_curl *) ctx;
	jonj_curl_write *t   = ch->handlers->write_header;
	size_t          length = size * nmemb;
	TSRMLS_FETCH_FROM_CTX(ch->thread_ctx);
	...
	switch (t->method) {
		case JONJ_CURL_STDOUT:
			 ...
			 */
			if (ch->handlers->write->method == JONJ_CURL_RETURN && length > 0) {
		...
		case JONJ_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not call the CURLOPT_HEADERFUNCTION");
				length = -1;
			} else if (retval_ptr) {
				_jonj_curl_verify_handlers(ch, 1 TSRMLS_CC);
				if (Z_TYPE_P(retval_ptr) != IS_LONG) {
...
static size_t curl_passwd(void *ctx, char *prompt, char *buf, int buflen)
{
	...
	if (error == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not call the CURLOPT_PASSWDFUNCTION");
	} else if (Z_TYPE_P(retval) == IS_STRING) {
		if (Z_STRLEN_P(retval) > buflen) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Returned password is too long for libcurl to handle");
		} else {
			strlcpy(buf, Z_STRVAL_P(retval), Z_STRLEN_P(retval));
		}
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "User handler '%s' did not return a string", Z_STRVAL_P(func));
	}
...
/* {{{ alloc_curl_handle
 */
static void alloc_curl_handle(jonj_curl **ch)
{
	*ch                           = emalloc(sizeof(jonj_curl));
	(*ch)->to_free                = ecalloc(1, sizeof(struct _jonj_curl_free));
	(*ch)->handlers               = ecalloc(1, sizeof(jonj_curl_handlers));
	(*ch)->handlers->write        = ecalloc(1, sizeof(jonj_curl_write));
	(*ch)->handlers->write_header = ecalloc(1, sizeof(jonj_curl_write));
	(*ch)->handlers->read         = ecalloc(1, sizeof(jonj_curl_read));
			...
			for(slist = ci->certinfo[i]; slist; slist = slist->next) {
				...
				if(tmp) {
					*tmp = '\0';
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not extract hash key from certificate info");
				}
			}
...
/* {{{ _jonj_curl_set_default_options()
   Set default options for a handle */
static void _jonj_curl_set_default_options(jonj_curl *ch)
{
	char *cainfo;
...
JONJ_FUNCTION(curl_init)
{
	...
	cp = curl_easy_init();
	if (!cp) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not initialize a new cURL handle");
		RETURN_FALSE;
	}
	...
	ch->handlers->write->method = JONJ_CURL_STDOUT;
	ch->handlers->read->method  = JONJ_CURL_DIRECT;
	ch->handlers->write_header->method = JONJ_CURL_IGNORE;

	MAKE_STD_ZVAL(clone);
	ch->clone = clone;

	_jonj_curl_set_default_options(ch);

	if (url) {
		if (jonj_curl_option_url(ch, url, url_len TSRMLS_CC) == FAILURE) {
			_jonj_curl_close_ex(ch TSRMLS_CC);
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(curl_copy_handle)
{
	...
	ZEND_FETCH_RESOURCE(ch, jonj_curl *, &zid, -1, le_curl_name, le_curl);

	cp = curl_easy_duphandle(ch->cp);
	if (!cp) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot duplicate cURL handle");
		RETURN_FALSE;
	}
	...
	if (ch->handlers->progress) {
		dupch->handlers->progress = ecalloc(1, sizeof(jonj_curl_progress));
...
/* Available since 7.21.0 */
	...
	if (ch->handlers->fnmatch) {
		dupch->handlers->fnmatch = ecalloc(1, sizeof(jonj_curl_fnmatch));
...
static int _jonj_curl_setopt(jonj_curl *ch, long option, zval **zvalue TSRMLS_DC) /* {{{ */
{
	CURLcode     error=CURLE_OK;
	...
	switch (option) {
		...
		case CURLOPT_SSL_VERIFYHOST:
			if(Z_BVAL_PP(zvalue) == 1) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "CURLOPT_SSL_VERIFYHOST with value 1 is deprecated and will be removed as of libcurl 7.28.1. It is recommended to use value 2 instead");
#else
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "CURLOPT_SSL_VERIFYHOST no longer accepts the value 1, value 2 will be used instead");
		...
		case CURLOPT_MUTE:
			...
			if ((option == CURLOPT_PROTOCOLS || option == CURLOPT_REDIR_PROTOCOLS) &&
				(PG(open_basedir) && *PG(open_basedir)) && (Z_LVAL_PP(zvalue) & CURLPROTO_FILE)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLPROTO_FILE cannot be activated when an open_basedir is set");
					return 1;
			}
#endif
# if defined(ZTS)
			if (option == CURLOPT_DNS_USE_GLOBAL_CACHE) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_DNS_USE_GLOBAL_CACHE cannot be activated when thread safety is enabled");
				return 1;
			}
# endif
...
#endif
		{
			...
			return jonj_curl_option_str(ch, option, Z_STRVAL_PP(zvalue), Z_STRLEN_PP(zvalue), 0 TSRMLS_CC);
		}
		...
		case CURLOPT_PRIVATE:
			convert_to_string_ex(zvalue);
			return jonj_curl_option_str(ch, option, Z_STRVAL_PP(zvalue), Z_STRLEN_PP(zvalue), 1 TSRMLS_CC);

		/* Curl url option */
		case CURLOPT_URL:
			convert_to_string_ex(zvalue);
			return jonj_curl_option_url(ch, Z_STRVAL_PP(zvalue), Z_STRLEN_PP(zvalue) TSRMLS_CC);
		...
		case CURLOPT_WRITEHEADER: {
			...
			if (Z_TYPE_PP(zvalue) != IS_NULL) {
				what = zend_fetch_resource(zvalue TSRMLS_CC, -1, "File-Handle", &type, 1, jonj_file_le_stream(), jonj_file_le_pstream());
				if (!what) {
					return FAILURE;
				}

				if (FAILURE == jonj_stream_cast((jonj_stream *) what, JONJ_STREAM_AS_STDIO, (void *) &fp, REPORT_ERRORS)) {
					return FAILURE;
				}
			...
			switch (option) {
				case CURLOPT_FILE:
					if (!what) {
						...
						ch->handlers->write->method = JONJ_CURL_STDOUT;
					} else if (((jonj_stream *) what)->mode[0] != 'r' || ((jonj_stream *) what)->mode[1] == '+') {
						if (ch->handlers->write->stream) {
							Z_DELREF_P(ch->handlers->write->stream);
						}
						Z_ADDREF_PP(zvalue);
						ch->handlers->write->fp = fp;
						ch->handlers->write->method = JONJ_CURL_FILE;
						ch->handlers->write->stream = *zvalue;
					} else {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "the provided file handle is not writable");
						return FAILURE;
					}
					break;
				case CURLOPT_WRITEHEADER:
					if (!what) {
						...
						ch->handlers->write_header->method = JONJ_CURL_FILE;
...
#endif
		{
			...
			if (!ph) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "You must pass either an object or an array with the %s argument", name);
				return FAILURE;
			}
			...
			for (zend_hash_internal_pointer_reset(ph);
				...
				if (!slist) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not build curl_slist");
					return 1;
				}
			}
		...
		case CURLOPT_FOLLOWLOCATION:
			...
			if (PG(open_basedir) && *PG(open_basedir)) {
				if (Z_LVAL_PP(zvalue) != 0) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CURLOPT_FOLLOWLOCATION cannot be activated when an open_basedir is set");
					return FAILURE;
				}
			}
#endif
		...
		case CURLOPT_HEADERFUNCTION:
			...
			ch->handlers->write_header->method = JONJ_CURL_USER;
			break;
		...
		case CURLOPT_POSTFIELDS:
			if (Z_TYPE_PP(zvalue) == IS_ARRAY || Z_TYPE_PP(zvalue) == IS_OBJECT) {
				...
				if (!postfields) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Couldn't get HashTable in CURLOPT_POSTFIELDS");
					return FAILURE;
				}
				...
				for (zend_hash_internal_pointer_reset(postfields);
					...
					if(Z_TYPE_PP(current) == IS_OBJECT && instanceof_function(Z_OBJCE_PP(current), curl_CURLFile_class TSRMLS_CC)) {
						...
						if(Z_TYPE_P(prop) != IS_STRING) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid filename for key %s", string_key);
						} else {
							postval = Z_STRVAL_P(prop);

							if (jonj_check_open_basedir(postval TSRMLS_CC)) {
								return 1;
							}
					...
					if (!ch->safe_upload && *postval == '@') {
						...
						jonj_error_docref("curl.curlfile" TSRMLS_CC, E_DEPRECATED, "The usage of the @filename API for file uploading is deprecated. Please use the CURLFile class instead");

						if ((type = jonj_memnstr(postval, ";type=", sizeof(";type=") - 1, postval + Z_STRLEN_PP(current)))) {
							*type = '\0';
						}
						if ((filename = jonj_memnstr(postval, ";filename=", sizeof(";filename=") - 1, postval + Z_STRLEN_PP(current)))) {
							*filename = '\0';
						}
						/* open_basedir check */
						if (jonj_check_open_basedir(postval TSRMLS_CC)) {
		...
		case CURLOPT_PROGRESSFUNCTION:
			...
			if (ch->handlers->progress == NULL) {
				ch->handlers->progress = ecalloc(1, sizeof(jonj_curl_progress));
			...
			ch->handlers->progress->method = JONJ_CURL_USER;
			break;
		...
		case CURLOPT_READFUNCTION:
			...
			ch->handlers->read->method = JONJ_CURL_USER;
			break;

		case CURLOPT_RETURNTRANSFER:
			convert_to_long_ex(zvalue);
			if (Z_LVAL_PP(zvalue)) {
				ch->handlers->write->method = JONJ_CURL_RETURN;
			} else {
				ch->handlers->write->method = JONJ_CURL_STDOUT;
			}
			break;
		...
		case CURLOPT_WRITEFUNCTION:
			...
			ch->handlers->write->method = JONJ_CURL_USER;
			break;
...
#endif
		{
			...
			if (Z_STRLEN_PP(zvalue) && jonj_check_open_basedir(Z_STRVAL_PP(zvalue) TSRMLS_CC)) {
				return FAILURE;
			}
		...
		case CURLOPT_SHARE:
			{
				jonj_curlsh *sh = NULL;
				ZEND_FETCH_RESOURCE_NO_RETURN(sh, jonj_curlsh *, zvalue, -1, le_curl_share_handle_name, le_curl_share_handle);
				if (sh) {
		...
		case CURLOPT_FNMATCH_FUNCTION:
			...
			if (ch->handlers->fnmatch == NULL) {
				ch->handlers->fnmatch = ecalloc(1, sizeof(jonj_curl_fnmatch));
			...
			ch->handlers->fnmatch->method = JONJ_CURL_USER;
			break;
#endif
...
JONJ_FUNCTION(curl_setopt)
{
	...
	if (options <= 0 && options != CURLOPT_SAFE_UPLOAD) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid curl configuration option");
		RETURN_FALSE;
	}

	if (_jonj_curl_setopt(ch, options, zvalue TSRMLS_CC) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(curl_setopt_array)
{
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(arr), (void **)&entry, &pos) == SUCCESS) {
		if (zend_hash_get_current_key_ex(Z_ARRVAL_P(arr), &string_key, &str_key_len, &option, 0, &pos) != HASH_KEY_IS_LONG) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array keys must be CURLOPT constants or equivalent integer values");
			RETURN_FALSE;
		}
		if (_jonj_curl_setopt(ch, (long) option, entry TSRMLS_CC) == FAILURE) {
			RETURN_FALSE;
		}
...
void _jonj_curl_cleanup_handle(jonj_curl *ch)
{
...
JONJ_FUNCTION(curl_exec)
{
	...
	if (ch->handlers->write->method == JONJ_CURL_RETURN && ch->handlers->write->buf.len > 0) {
		smart_str_0(&ch->handlers->write->buf);
		RETURN_STRINGL(ch->handlers->write->buf.c, ch->handlers->write->buf.len, 1);
	}

	/* flush the file handle, so any remaining data is synched to disk */
	if (ch->handlers->write->method == JONJ_CURL_FILE && ch->handlers->write->fp) {
		fflush(ch->handlers->write->fp);
	}
	if (ch->handlers->write_header->method == JONJ_CURL_FILE && ch->handlers->write_header->fp) {
...
JONJ_FUNCTION(curl_close)
{
	...
	if (ch->in_callback) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attempt to close cURL handle from a callback");
		return;
	}
...
static void _jonj_curl_close(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	jonj_curl *ch = (jonj_curl *) rsrc->ptr;
	_jonj_curl_close_ex(ch TSRMLS_CC);
}
/* }}} */
...
#if LIBCURL_VERSION_NUM >= 0x070c01 /* 7.12.1 */
/* {{{ _jonj_curl_reset_handlers()
   Reset all handlers of a given jonj_curl */
static void _jonj_curl_reset_handlers(jonj_curl *ch)
{
	if (ch->handlers->write->stream) {
		Z_DELREF_P(ch->handlers->write->stream);
		ch->handlers->write->stream = NULL;
...
JONJ_FUNCTION(curl_reset)
{
	...
	if (ch->in_callback) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attempt to reset cURL handle from a callback");
		return;
	}

	curl_easy_reset(ch->cp);
	_jonj_curl_reset_handlers(ch);
	_jonj_curl_set_default_options(ch);
}
/* }}} */
#endif
...
JONJ_FUNCTION(curl_escape)
{
	...
	if ((res = curl_easy_escape(ch->cp, str, str_len))) {
		if (strlen(res) > INT_MAX) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Escaped string is too long, maximum is %d", INT_MAX);
			RETURN_FALSE;
		}
		RETVAL_STRING(res, 1);
		curl_free(res);
