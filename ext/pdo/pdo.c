...
static JONJ_GINIT_FUNCTION(pdo);
...
PDO_API zend_class_entry *jonj_pdo_get_dbh_ce(void)
{
	return pdo_dbh_ce;
}

PDO_API zend_class_entry *jonj_pdo_get_exception(void)
{
	return pdo_exception_ce;
}

PDO_API char *jonj_pdo_str_tolower_dup(const char *src, int len)
{
...
PDO_API zend_class_entry *jonj_pdo_get_exception_base(int root TSRMLS_DC)
{
#if can_handle_soft_dependency_on_SPL && defined(HAVE_SPL) && ((JONJ_MAJOR_VERSION > 5) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION >= 1))
	if (!root) {
...
#endif
#if (JONJ_MAJOR_VERSION == 5) && (JONJ_MINOR_VERSION < 2)
...
const zend_function_entry pdo_functions[] = {
	JONJ_FE(pdo_drivers,             arginfo_pdo_drivers)
	JONJ_FE_END
};
/* }}} */
...
PDO_API int jonj_pdo_register_driver(pdo_driver_t *driver)
{
...
PDO_API void jonj_pdo_unregister_driver(pdo_driver_t *driver)
{
	if (!zend_hash_exists(&module_registry, "pdo", sizeof("pdo"))) {
		return;
	}
...
PDO_API int jonj_pdo_parse_data_source(const char *data_source,
...
PDO_API char *jonj_pdo_int64_to_str(pdo_int64_t i64 TSRMLS_DC)
{
	char buffer[65];
	char outbuf[65] = "";
	register char *p;
...
JONJ_MINIT_FUNCTION(pdo)
{
	...
	le_ppdo = zend_register_list_destructors_ex(NULL, jonj_pdo_pdbh_dtor,
		"PDO persistent database", module_number);

	INIT_CLASS_ENTRY(ce, "PDOException", NULL);

 	pdo_exception_ce = zend_register_internal_class_ex(&ce, jonj_pdo_get_exception_base(0 TSRMLS_CC), NULL TSRMLS_CC);
...
JONJ_MINFO_FUNCTION(pdo)
{
	...
	jonj_info_print_table_header(2, "PDO support", "enabled");
	...
	jonj_info_print_table_row(2, "PDO drivers", drivers ? drivers+2 : "");
