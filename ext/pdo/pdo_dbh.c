...
void pdo_raise_impl_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, const char *sqlstate, const char *supp TSRMLS_DC) /* {{{ */
{
	...
	if (dbh && dbh->error_mode != PDO_ERRMODE_EXCEPTION) {
		...
		zval *ex, *info;
		zend_class_entry *def_ex = jonj_pdo_get_exception_base(1 TSRMLS_CC), *pdo_ex = jonj_pdo_get_exception();
...
static char *dsn_from_uri(char *uri, char *buf, size_t buflen TSRMLS_DC) /* {{{ */
{
	...
	stream = jonj_stream_open_wrapper(uri, "rb", REPORT_ERRORS, NULL);
	if (stream) {
		dsn = jonj_stream_get_line(stream, buf, buflen, NULL);
...
/* {{{ proto void PDO::__construct(string dsn[, string username[, string passwd [, array options]]])
   */
static JONJ_METHOD(PDO, dbh_constructor)
{
	zval *object = getThis();
	pdo_dbh_t *dbh = NULL;
	...
	if (!colon) {
		/* let's see if this string has a matching dsn in the jonj.ini */
		char *ini_dsn = NULL;

		snprintf(alt_dsn, sizeof(alt_dsn), "pdo.dsn.%s", data_source);
		if (FAILURE == cfg_get_string(alt_dsn, &ini_dsn)) {
			zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "invalid data source name");
			zval_dtor(object);
			ZVAL_NULL(object);
			return;
		}
		...
		if (!colon) {
			zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "invalid data source name (via INI: %s)", alt_dsn);
			ZVAL_NULL(object);
			return;
		}
	}
	...
	if (!strncmp(data_source, "uri:", sizeof("uri:")-1)) {
		...
		if (!data_source) {
			zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "invalid data source URI");
			ZVAL_NULL(object);
			return;
		}
		colon = strchr(data_source, ':');
		if (!colon) {
			zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "invalid data source name (via URI)");
			ZVAL_NULL(object);
			return;
		}
	}
	...
	if (!driver) {
		...
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "could not find driver");
		ZVAL_NULL(object);
		return;
	}
	...
	if (options) {
		...
		if (is_persistent) {
			...
			if (SUCCESS == zend_hash_find(&EG(persistent_list), hashkey, plen+1, (void*)&le)) {
				if (Z_TYPE_P(le) == jonj_pdo_list_entry()) {
			...
			if (pdbh) {
				...
				if (!pdbh) {
					jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "out of memory while allocating PDO handle");
					/* NOTREACHED */
				}
	...
	if (!dbh->data_source || (username && !dbh->username) || (password && !dbh->password)) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "out of memory");
	}
	...
	if (driver->db_handle_factory(dbh, options TSRMLS_CC)) {
		...
		if (is_persistent) {
			...
			le.type = jonj_pdo_list_entry();
			le.ptr = dbh;

			if (FAILURE == zend_hash_update(&EG(persistent_list),
					(char*)dbh->persistent_id, dbh->persistent_id_len, (void*)&le,
					sizeof(le), NULL)) {
				jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Failed to register persistent entry");
			}
		}
...
static JONJ_METHOD(PDO, prepare)
{
	...
	jonj_pdo_dbh_addref(dbh TSRMLS_CC);
...
static JONJ_METHOD(PDO, beginTransaction)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
	...
	if (dbh->in_txn) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "There is already an active transaction");
		RETURN_FALSE;
	}
	...
	if (!dbh->methods->begin) {
		 ...
		 * comments below */
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "This driver doesn't support transactions");
		RETURN_FALSE;
	}
...
static JONJ_METHOD(PDO, commit)
{
	...
	if (!dbh->in_txn) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "There is no active transaction");
		RETURN_FALSE;
	}
...
static JONJ_METHOD(PDO, rollBack)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
...
static JONJ_METHOD(PDO, inTransaction)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
...
fail:
	if (attr == PDO_ATTR_AUTOCOMMIT) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "The auto-commit mode cannot be changed for this driver");
...
/* {{{ proto bool PDO::setAttribute(long attribute, mixed value)
   Set an attribute */
static JONJ_METHOD(PDO, setAttribute)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
	long attr;
	zval *value;
...
/* {{{ proto mixed PDO::getAttribute(long attribute)
   Get an attribute */
static JONJ_METHOD(PDO, getAttribute)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
	long attr;
...
static JONJ_METHOD(PDO, lastInsertId)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
	char *name = NULL;
	int namelen;
...
static JONJ_METHOD(PDO, errorCode)
{
	pdo_dbh_t *dbh = zend_object_store_get_object(getThis() TSRMLS_CC);
...
static JONJ_METHOD(PDO, errorInfo)
{
	int error_count;
...
static JONJ_METHOD(PDO, __wakeup)
{
	zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "You cannot serialize or unserialize PDO instances");
}
/* }}} */

/* {{{ proto int PDO::__sleep()
   Prevents serialization of a PDO instance */
static JONJ_METHOD(PDO, __sleep)
{
	zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "You cannot serialize or unserialize PDO instances");
}
/* }}} */
...
static JONJ_METHOD(PDO, getAvailableDrivers)
{
	HashPosition pos;
...
const zend_function_entry pdo_dbh_functions[] = {
	...
	JONJ_ME(PDO, prepare, 				arginfo_pdo_prepare,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, beginTransaction,       arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, commit,                 arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, rollBack,               arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, inTransaction,          arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, setAttribute,	arginfo_pdo_setattribute,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, exec,			arginfo_pdo_exec,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, query,			NULL,					ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, lastInsertId,	arginfo_pdo_lastinsertid,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, errorCode,              arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, errorInfo,              arginfo_pdo__void,         ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, getAttribute,	arginfo_pdo_getattribute,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, quote,			arginfo_pdo_quote,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDO, __wakeup,               arginfo_pdo__void,         ZEND_ACC_PUBLIC|ZEND_ACC_FINAL)
	JONJ_ME(PDO, __sleep,                arginfo_pdo__void,         ZEND_ACC_PUBLIC|ZEND_ACC_FINAL)
	JONJ_ME(PDO, getAvailableDrivers,    arginfo_pdo__void,         ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	{NULL, NULL, NULL}
};
...
int pdo_hash_methods(pdo_dbh_t *dbh, int kind TSRMLS_DC)
{
	...
	if (!(dbh->cls_methods[kind] = pemalloc(sizeof(HashTable), dbh->is_persistent))) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "out of memory while allocating PDO methods.");
	}
...
#if JONJ_MAJOR_VERSION > 5 || JONJ_MINOR_VERSION >= 1
...
PDO_API void jonj_pdo_dbh_addref(pdo_dbh_t *dbh TSRMLS_DC)
{
	dbh->refcount++;
}

PDO_API void jonj_pdo_dbh_delref(pdo_dbh_t *dbh TSRMLS_DC)
{
...
ZEND_RSRC_DTOR_FUNC(jonj_pdo_pdbh_dtor)
{
	if (rsrc->ptr) {
