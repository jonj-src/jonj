...
#include "jonj_pdo.h"
#include "jonj_pdo_driver.h"
#include "jonj_pdo_int.h"
...
#define JONJ_STMT_GET_OBJ	\
  pdo_stmt_t *stmt = (pdo_stmt_t*)zend_object_store_get_object(getThis() TSRMLS_CC);	\
  if (!stmt->dbh) {	\
    RETURN_FALSE;	\
  }	\

static JONJ_FUNCTION(dbstmt_constructor) /* {{{ */
{
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "You should not create a PDOStatement manually");
}
/* }}} */
...
static JONJ_METHOD(PDOStatement, execute)
{
		...
		case PDO_PARAM_LOB:
			if (value == NULL) {
				...
				if (stmt->dbh->stringify || new_type == PDO_PARAM_STR) {
					char *buf = NULL;
					size_t len;
					len = jonj_stream_copy_to_mem((jonj_stream*)value, &buf, JONJ_STREAM_COPY_ALL, 0);
					if(buf == NULL) {
						ZVAL_EMPTY_STRING(dest);
					} else {
						ZVAL_STRINGL(dest, buf, len, 0);
					}
					jonj_stream_close((jonj_stream*)value);
				} else {
					jonj_stream_to_zval((jonj_stream*)value, dest);
				}
				...
				if (caller_frees) {
					stm = jonj_stream_memory_open(TEMP_STREAM_TAKE_BUFFER, value, value_len);
					if (stm) {
						caller_frees = 0;
					}
				} else
#endif
				{
					stm = jonj_stream_memory_open(TEMP_STREAM_READONLY, value, value_len);
				}
				if (stm) {
					jonj_stream_to_zval(stm, dest);
				} else {
					ZVAL_NULL(dest);
				}
			} else {
			...
			switch (how) {
				...
				case PDO_FETCH_CLASS:
					if ((flags & PDO_FETCH_SERIALIZE) == 0 || idx) {
						...
						if (jonj_var_unserialize(&return_value, (const unsigned char**)&Z_STRVAL_P(val), Z_STRVAL_P(val)+Z_STRLEN_P(val), NULL TSRMLS_CC) == FAILURE) {
...
static JONJ_METHOD(PDOStatement, fetch)
{
...
static JONJ_METHOD(PDOStatement, fetchObject)
{
...
static JONJ_METHOD(PDOStatement, fetchColumn)
{
	long col_n = 0;
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, fetchAll)
{
	long how = PDO_FETCH_USE_DEFAULT;
...
/* {{{ proto bool PDOStatement::bindValue(mixed $paramno, mixed $param [, int $type ])
   bind an input parameter to the value of a JONJ variable.  $paramno is the 1-based position of the placeholder in the SQL statement (but can be the parameter name for drivers that support named placeholders).  It should be called prior to execute(). */
static JONJ_METHOD(PDOStatement, bindValue)
{
	struct pdo_bound_param_data param = {0};
	long param_type = PDO_PARAM_STR;
	JONJ_STMT_GET_OBJ;
...
/* {{{ proto bool PDOStatement::bindParam(mixed $paramno, mixed &$param [, int $type [, int $maxlen [, mixed $driverdata]]])
   bind a parameter to a JONJ variable.  $paramno is the 1-based position of the placeholder in the SQL statement (but can be the parameter name for drivers that support named placeholders).  This isn't supported by all drivers.  It should be called prior to execute(). */
static JONJ_METHOD(PDOStatement, bindParam)
{
	JONJ_STMT_GET_OBJ;
...
/* {{{ proto bool PDOStatement::bindColumn(mixed $column, mixed &$param [, int $type [, int $maxlen [, mixed $driverdata]]])
   bind a column to a JONJ variable.  On each row fetch $param will contain the value of the corresponding column.  $column is the 1-based offset of the column, or the column name.  For portability, don't call this before execute(). */
static JONJ_METHOD(PDOStatement, bindColumn)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, rowCount)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, errorCode)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, errorInfo)
{
	int error_count;
...
/* {{{ proto bool PDOStatement::setAttribute(long attribute, mixed value)
   Set an attribute */
static JONJ_METHOD(PDOStatement, setAttribute)
{
	long attr;
	zval *value = NULL;
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, getAttribute)
{
	long attr;
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, columnCount)
{
	JONJ_STMT_GET_OBJ;
	if (zend_parse_parameters_none() == FAILURE) {
		return;
...
static JONJ_METHOD(PDOStatement, getColumnMeta)
{
	long colno;
	struct pdo_column_data *col;
	JONJ_STMT_GET_OBJ;
		...
		case PDO_FETCH_CLASS:
			...
			if (SUCCESS == retval) {
				...
				if (stmt->dbh->is_persistent) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "JONJ might crash if you don't call $stmt->setFetchMode() to reset to defaults on this persistent statement.  This will be fixed in a later release");
				}
#endif
				if (argc == 3) {
...
static JONJ_METHOD(PDOStatement, setFetchMode)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, nextRowset)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, closeCursor)
{
	JONJ_STMT_GET_OBJ;
...
static JONJ_METHOD(PDOStatement, debugDumpParams)
{
	jonj_stream *out = jonj_stream_open_wrapper("jonj://output", "w", 0, NULL);
	HashPosition pos;
	struct pdo_bound_param_data *param;
	JONJ_STMT_GET_OBJ;
	...
	jonj_stream_printf(out TSRMLS_CC, "SQL: [%d] %.*s\n",
		stmt->query_stringlen,
		stmt->query_stringlen, stmt->query_string);

	jonj_stream_printf(out TSRMLS_CC, "Params:  %d\n",
	...
	if (stmt->bound_params) {
		...
		while (SUCCESS == zend_hash_get_current_data_ex(stmt->bound_params,
			...
			if (res == HASH_KEY_IS_LONG) {
				jonj_stream_printf(out TSRMLS_CC, "Key: Position #%ld:\n", num);
			} else if (res == HASH_KEY_IS_STRING) {
				jonj_stream_printf(out TSRMLS_CC, "Key: Name: [%d] %.*s\n", len, len, str);
			}

			jonj_stream_printf(out TSRMLS_CC, "paramno=%ld\nname=[%d] \"%.*s\"\nis_param=%d\nparam_type=%d\n",
	...
	jonj_stream_close(out);
}
/* }}} */

/* {{{ proto int PDOStatement::__wakeup()
   Prevents use of a PDOStatement instance that has been unserialized */
static JONJ_METHOD(PDOStatement, __wakeup)
{
	zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "You cannot serialize or unserialize PDOStatement instances");
}
/* }}} */

/* {{{ proto int PDOStatement::__sleep()
   Prevents serialization of a PDOStatement instance */
static JONJ_METHOD(PDOStatement, __sleep)
{
	zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC, "You cannot serialize or unserialize PDOStatement instances");
}
/* }}} */
...
const zend_function_entry pdo_dbstmt_functions[] = {
	JONJ_ME(PDOStatement, execute,		arginfo_pdostatement_execute,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, fetch,			arginfo_pdostatement_fetch,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, bindParam,		arginfo_pdostatement_bindparam,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, bindColumn,	arginfo_pdostatement_bindcolumn,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, bindValue,		arginfo_pdostatement_bindvalue,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, rowCount,		arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, fetchColumn,	arginfo_pdostatement_fetchcolumn,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, fetchAll,		arginfo_pdostatement_fetchall,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, fetchObject,	arginfo_pdostatement_fetchobject,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, errorCode,		arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, errorInfo,		arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, setAttribute,	arginfo_pdostatement_setattribute,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, getAttribute,	arginfo_pdostatement_getattribute,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, columnCount,	arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, getColumnMeta,	arginfo_pdostatement_getcolumnmeta,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, setFetchMode,	arginfo_pdostatement_setfetchmode,	ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, nextRowset,	arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, closeCursor,	arginfo_pdostatement__void,			ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, debugDumpParams, arginfo_pdostatement__void,		ZEND_ACC_PUBLIC)
	JONJ_ME(PDOStatement, __wakeup,		arginfo_pdostatement__void,			ZEND_ACC_PUBLIC|ZEND_ACC_FINAL)
	JONJ_ME(PDOStatement, __sleep,		arginfo_pdostatement__void,			ZEND_ACC_PUBLIC|ZEND_ACC_FINAL)
	{NULL, NULL, NULL}
};
...
static void free_statement(pdo_stmt_t *stmt TSRMLS_DC)
{
	...
	if (stmt->dbh) {
		jonj_pdo_dbh_delref(stmt->dbh TSRMLS_CC);
	}
	zend_object_std_dtor(&stmt->std TSRMLS_CC);
	efree(stmt);
}

PDO_API void jonj_pdo_stmt_addref(pdo_stmt_t *stmt TSRMLS_DC)
{
	stmt->refcount++;
}

PDO_API void jonj_pdo_stmt_delref(pdo_stmt_t *stmt TSRMLS_DC)
{
	if (--stmt->refcount == 0) {
...
{
	jonj_pdo_stmt_delref(stmt TSRMLS_CC);
}
...
{
	struct jonj_pdo_iterator *I = (struct jonj_pdo_iterator*)iter->data;
...
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "This PDORow is not from a writable result set");
}
...
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot delete properties from a PDORow");
}
...
static int pdo_row_serialize(zval *object, unsigned char **buffer, zend_uint *buf_len, zend_serialize_data *data TSRMLS_DC)
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "PDORow instances may not be serialized");
	return FAILURE;
}
/* }}} */
