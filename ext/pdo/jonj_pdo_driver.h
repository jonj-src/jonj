...
#ifndef JONJ_PDO_DRIVER_H
#define JONJ_PDO_DRIVER_H
...
enum pdo_param_type {
	...
	/* int as in long (the jonj native int type).
	...
	/* get_col: when len is 0 ptr should point to a jonj_stream *,
...
#define PDO_DRIVER_HEADER(name)	\
	 ...
	 * data in the db, otherwise you will crash JONJ when persistent connections
	 * are used.
	 */
 ...
 *   type => a string representation of the type, mapped to the JONJ equivalent type name
...
struct _pdo_stmt_t {
    ...
    */
	...
	/* keep track of JONJ variables bound to named (or positional) columns
	 * in the result set */
