...
#include "jonj_hash_whirlpool.h"
#include "jonj_hash_whirlpool_tables.h"
...
static void WhirlpoolTransform(JONJ_WHIRLPOOL_CTX *context)
{
    int i, r;
    jonj_hash_uint64 K[8];        /* the round key */
    jonj_hash_uint64 block[8];    /* mu(buffer) */
    jonj_hash_uint64 state[8];    /* the cipher state */
    jonj_hash_uint64 L[8];
    ...
    for (i = 0; i < 8; i++, buffer += 8) {
        block[i] =
            (((jonj_hash_uint64)buffer[0]        ) << 56) ^
            (((jonj_hash_uint64)buffer[1] & 0xffL) << 48) ^
            (((jonj_hash_uint64)buffer[2] & 0xffL) << 40) ^
            (((jonj_hash_uint64)buffer[3] & 0xffL) << 32) ^
            (((jonj_hash_uint64)buffer[4] & 0xffL) << 24) ^
            (((jonj_hash_uint64)buffer[5] & 0xffL) << 16) ^
            (((jonj_hash_uint64)buffer[6] & 0xffL) <<  8) ^
            (((jonj_hash_uint64)buffer[7] & 0xffL)      );
    }
    /*
...
JONJ_HASH_API void JONJ_WHIRLPOOLInit(JONJ_WHIRLPOOL_CTX *context)
{
	memset(context, 0, sizeof(*context));
}

JONJ_HASH_API void JONJ_WHIRLPOOLUpdate(JONJ_WHIRLPOOL_CTX *context, const unsigned char *input, size_t len)
{
    jonj_hash_uint64 sourceBits = len * 8;
    ...
    jonj_hash_uint32 b, carry;
    int i;

    /*
     * tally the length of the added data:
     */
    jonj_hash_uint64 value = sourceBits;
    for (i = 31, carry = 0; i >= 0 && (carry != 0 || value != L64(0)); i--) {
        carry += bitLength[i] + ((jonj_hash_uint32)value & 0xff);
...
JONJ_HASH_API void JONJ_WHIRLPOOLFinal(unsigned char digest[64], JONJ_WHIRLPOOL_CTX *context)
{
    int i;
    unsigned char *buffer      = context->buffer.data;
    unsigned char *bitLength   = context->bitlength;
...
const jonj_hash_ops jonj_hash_whirlpool_ops = {
	(jonj_hash_init_func_t) JONJ_WHIRLPOOLInit,
	(jonj_hash_update_func_t) JONJ_WHIRLPOOLUpdate,
	(jonj_hash_final_func_t) JONJ_WHIRLPOOLFinal,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	64,
	64,
	sizeof(JONJ_WHIRLPOOL_CTX)
};
