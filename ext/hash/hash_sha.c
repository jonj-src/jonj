/*
  ...
  +----------------------------------------------------------------------+
  | Authors: Steffan Esser <sesser@jonj.tk>                              |
...
/* {{{ SHAEncode32
   Encodes input (jonj_hash_uint32) into output (unsigned char). Assumes len is
   a multiple of 4.
 */
static void SHAEncode32(unsigned char *output, jonj_hash_uint32 *input, unsigned int len)
{
	unsigned int i, j;
...
/* {{{ SHADecode32
   Decodes input (unsigned char) into output (jonj_hash_uint32). Assumes len is
   a multiple of 4.
 */
static void SHADecode32(jonj_hash_uint32 *output, const unsigned char *input, unsigned int len)
{
	unsigned int i, j;

	for (i = 0, j = 0; j < len; i++, j += 4)
		output[i] = ((jonj_hash_uint32) input[j + 3]) | (((jonj_hash_uint32) input[j + 2]) << 8) |
			(((jonj_hash_uint32) input[j + 1]) << 16) | (((jonj_hash_uint32) input[j]) << 24);
}
/* }}} */

const jonj_hash_ops jonj_hash_sha1_ops = {
	(jonj_hash_init_func_t) JONJ_SHA1Init,
	(jonj_hash_update_func_t) JONJ_SHA1Update,
	(jonj_hash_final_func_t) JONJ_SHA1Final,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	20,
	64,
	sizeof(JONJ_SHA1_CTX)
};

#ifdef JONJ_HASH_SHA1_NOT_IN_CORE

JONJ_HASH_API void make_sha1_digest(char *sha1str, unsigned char *digest)
{
	jonj_hash_bin2hex(sha1str, digest, 20);
	sha1str[40] = '\0';
}
...
JONJ_FUNCTION(sha1)
{
	...
	sha1str[0] = '\0';
	JONJ_SHA1Init(&context);
	JONJ_SHA1Update(&context, arg, arg_len);
	JONJ_SHA1Final(digest, &context);
	if (raw_output) {
...
JONJ_FUNCTION(sha1_file)
{
	...
	stream = jonj_stream_open_wrapper(arg, "rb", REPORT_ERRORS, NULL);
	if (!stream) {
		RETURN_FALSE;
	}
...
#define FF(a, b, c, d, e, w) { \
 (e) += F ((b), (c), (d)) + (w) + (jonj_hash_uint32)(0x5A827999); \
 (e) += ROTATE_LEFT ((a), 5); \
 (b) = ROTATE_LEFT((b), 30); \
  }
#define GG(a, b, c, d, e, w) { \
 (e) += G ((b), (c), (d)) + (w) + (jonj_hash_uint32)(0x6ED9EBA1); \
 (e) += ROTATE_LEFT ((a), 5); \
 (b) = ROTATE_LEFT((b), 30); \
  }
#define HH(a, b, c, d, e, w) { \
 (e) += H ((b), (c), (d)) + (w) + (jonj_hash_uint32)(0x8F1BBCDC); \
 (e) += ROTATE_LEFT ((a), 5); \
 (b) = ROTATE_LEFT((b), 30); \
  }
#define II(a, b, c, d, e, w) { \
 (e) += I ((b), (c), (d)) + (w) + (jonj_hash_uint32)(0xCA62C1D6); \
 (e) += ROTATE_LEFT ((a), 5); \
 (b) = ROTATE_LEFT((b), 30); \
  }
 ...
 */
JONJ_HASH_API void JONJ_SHA1Init(JONJ_SHA1_CTX * context)
{
 ...
 */
static void SHA1Transform(jonj_hash_uint32 state[5], const unsigned char block[64])
{
	jonj_hash_uint32 a = state[0], b = state[1], c = state[2];
	jonj_hash_uint32 d = state[3], e = state[4], x[16], tmp;
...
/* {{{ JONJ_SHA1Update
   ...
   context.
 */
JONJ_HASH_API void JONJ_SHA1Update(JONJ_SHA1_CTX * context, const unsigned char *input,
			   unsigned int inputLen)
{
	...
	if ((context->count[0] += ((jonj_hash_uint32) inputLen << 3))
		< ((jonj_hash_uint32) inputLen << 3))
		context->count[1]++;
	context->count[1] += ((jonj_hash_uint32) inputLen >> 29);
 ...
 */
JONJ_HASH_API void JONJ_SHA1Final(unsigned char digest[20], JONJ_SHA1_CTX * context)
{
	...
	JONJ_SHA1Update(context, PADDING, padLen);

	/* Append length (before padding) */
	JONJ_SHA1Update(context, bits, 8);
...
const jonj_hash_ops jonj_hash_sha256_ops = {
	(jonj_hash_init_func_t) JONJ_SHA256Init,
	(jonj_hash_update_func_t) JONJ_SHA256Update,
	(jonj_hash_final_func_t) JONJ_SHA256Final,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	32,
	64,
	sizeof(JONJ_SHA256_CTX)
};

const jonj_hash_ops jonj_hash_sha224_ops = {
	(jonj_hash_init_func_t) JONJ_SHA224Init,
	(jonj_hash_update_func_t) JONJ_SHA224Update,
	(jonj_hash_final_func_t) JONJ_SHA224Final,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	28,
	64,
	sizeof(JONJ_SHA224_CTX)
};
...
static const jonj_hash_uint32 SHA256_K[64] = {
 ...
 */
JONJ_HASH_API void JONJ_SHA256Init(JONJ_SHA256_CTX * context)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
 ...
 */
static void SHA256Transform(jonj_hash_uint32 state[8], const unsigned char block[64])
{
	jonj_hash_uint32 a = state[0], b = state[1], c = state[2], d = state[3];
	jonj_hash_uint32 e = state[4], f = state[5], g = state[6], h = state[7];
	jonj_hash_uint32 x[16], T1, T2, W[64];
	int i;
 ...
 */
JONJ_HASH_API void JONJ_SHA224Init(JONJ_SHA224_CTX * context)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
...
/* {{{ JONJ_SHA224Update
   ...
   context.
 */
JONJ_HASH_API void JONJ_SHA224Update(JONJ_SHA224_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
	...
	/* Update number of bits */
	if ((context->count[0] += ((jonj_hash_uint32) inputLen << 3)) < ((jonj_hash_uint32) inputLen << 3)) {
		context->count[1]++;
	}
	context->count[1] += ((jonj_hash_uint32) inputLen >> 29);
...
/* {{{ JONJ_SHA224Final
   ...
   the message digest and zeroizing the context.
 */
JONJ_HASH_API void JONJ_SHA224Final(unsigned char digest[28], JONJ_SHA224_CTX * context)
{
	unsigned char bits[8];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
	padLen = (index < 56) ? (56 - index) : (120 - index);
	JONJ_SHA224Update(context, PADDING, padLen);

	/* Append length (before padding) */
	JONJ_SHA224Update(context, bits, 8);
...
/* {{{ JONJ_SHA256Update
   ...
   context.
 */
JONJ_HASH_API void JONJ_SHA256Update(JONJ_SHA256_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ JONJ_SHA256Final
   ...
   the message digest and zeroizing the context.
 */
JONJ_HASH_API void JONJ_SHA256Final(unsigned char digest[32], JONJ_SHA256_CTX * context)
{
	unsigned char bits[8];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
	padLen = (index < 56) ? (56 - index) : (120 - index);
	JONJ_SHA256Update(context, PADDING, padLen);

	/* Append length (before padding) */
	JONJ_SHA256Update(context, bits, 8);
...
static const jonj_hash_uint64 SHA512_K[128] = {
...
/* {{{ SHAEncode64
   Encodes input (jonj_hash_uint64) into output (unsigned char). Assumes len is
   a multiple of 8.
 */
static void SHAEncode64(unsigned char *output, jonj_hash_uint64 *input, unsigned int len)
{
	unsigned int i, j;
...
/* {{{ SHADecode64
   Decodes input (unsigned char) into output (jonj_hash_uint64). Assumes len is
   a multiple of 8.
 */
static void SHADecode64(jonj_hash_uint64 *output, const unsigned char *input, unsigned int len)
{
	unsigned int i, j;

	for (i = 0, j = 0; j < len; i++, j += 8)
		output[i] = 
			((jonj_hash_uint64) input[j + 7]) | (((jonj_hash_uint64) input[j + 6]) << 8) |
			(((jonj_hash_uint64) input[j + 5]) << 16) | (((jonj_hash_uint64) input[j + 4]) << 24) |
			(((jonj_hash_uint64) input[j + 3]) << 32) | (((jonj_hash_uint64) input[j + 2]) << 40) |
			(((jonj_hash_uint64) input[j + 1]) << 48) | (((jonj_hash_uint64) input[j]) << 56);
}
/* }}} */
 ...
 */
JONJ_HASH_API void JONJ_SHA384Init(JONJ_SHA384_CTX * context)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
 ...
 */
static void SHA512Transform(jonj_hash_uint64 state[8], const unsigned char block[128])
{
	jonj_hash_uint64 a = state[0], b = state[1], c = state[2], d = state[3];
	jonj_hash_uint64 e = state[4], f = state[5], g = state[6], h = state[7];
	jonj_hash_uint64 x[16], T1, T2, W[80];
	int i;
...
/* {{{ JONJ_SHA384Update
   ...
   context.
 */
JONJ_HASH_API void JONJ_SHA384Update(JONJ_SHA384_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
	...
	/* Update number of bits */
	if ((context->count[0] += ((jonj_hash_uint64) inputLen << 3)) < ((jonj_hash_uint64) inputLen << 3)) {
		context->count[1]++;
	}
	context->count[1] += ((jonj_hash_uint64) inputLen >> 61);
...
/* {{{ JONJ_SHA384Final
   ...
   the message digest and zeroizing the context.
 */
JONJ_HASH_API void JONJ_SHA384Final(unsigned char digest[48], JONJ_SHA384_CTX * context)
{
	...
	JONJ_SHA384Update(context, PADDING, padLen);

	/* Append length (before padding) */
	JONJ_SHA384Update(context, bits, 16);
...
const jonj_hash_ops jonj_hash_sha384_ops = {
	(jonj_hash_init_func_t) JONJ_SHA384Init,
	(jonj_hash_update_func_t) JONJ_SHA384Update,
	(jonj_hash_final_func_t) JONJ_SHA384Final,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	48,
	128,
	sizeof(JONJ_SHA384_CTX)
};

/* {{{ JONJ_SHA512Init
 * SHA512 initialization. Begins an SHA512 operation, writing a new context.
 */
JONJ_HASH_API void JONJ_SHA512Init(JONJ_SHA512_CTX * context)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
...
/* {{{ JONJ_SHA512Update
   ...
   context.
 */
JONJ_HASH_API void JONJ_SHA512Update(JONJ_SHA512_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ JONJ_SHA512Final
   ...
   the message digest and zeroizing the context.
 */
JONJ_HASH_API void JONJ_SHA512Final(unsigned char digest[64], JONJ_SHA512_CTX * context)
{
	unsigned char bits[16];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x7f);
	padLen = (index < 112) ? (112 - index) : (240 - index);
	JONJ_SHA512Update(context, PADDING, padLen);

	/* Append length (before padding) */
	JONJ_SHA512Update(context, bits, 16);
...
const jonj_hash_ops jonj_hash_sha512_ops = {
	(jonj_hash_init_func_t) JONJ_SHA512Init,
	(jonj_hash_update_func_t) JONJ_SHA512Update,
	(jonj_hash_final_func_t) JONJ_SHA512Final,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	64,
	128,
	sizeof(JONJ_SHA512_CTX)
};
