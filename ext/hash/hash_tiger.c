...
#include "jonj_hash.h"
#include "jonj_hash_tiger.h"
#include "jonj_hash_tiger_tables.h"
...
#define round(a,b,c,x,mul) \
	...
	a -= t1[(unsigned char)(c)] ^ \
		t2[(unsigned char)(((jonj_hash_uint32)(c))>>(2*8))] ^ \
		t3[(unsigned char)((c)>>(4*8))] ^ \
		t4[(unsigned char)(((jonj_hash_uint32)((c)>>(4*8)))>>(2*8))] ; \
	b += t4[(unsigned char)(((jonj_hash_uint32)(c))>>(1*8))] ^ \
		t3[(unsigned char)(((jonj_hash_uint32)(c))>>(3*8))] ^ \
		t2[(unsigned char)(((jonj_hash_uint32)((c)>>(4*8)))>>(1*8))] ^ \
		t1[(unsigned char)(((jonj_hash_uint32)((c)>>(4*8)))>>(3*8))]; \
	b *= mul;
...
{ \
	register jonj_hash_uint64 a, b, c, tmpa, x0, x1, x2, x3, x4, x5, x6, x7; \
	jonj_hash_uint64 aa, bb, cc; \
...
static inline void TigerFinalize(JONJ_TIGER_CTX *context)
{
	context->passed += (jonj_hash_uint64) context->length << 3;
	...
	if (context->length > 56) {
		...
		tiger_compress(context->passes, ((jonj_hash_uint64 *) context->buffer), context->state);
	...
	memcpy(&context->buffer[56], &context->passed, sizeof(jonj_hash_uint64));
#else
...
static inline void TigerDigest(unsigned char *digest_str, unsigned int digest_len, JONJ_TIGER_CTX *context)
{
	unsigned int i;
...
JONJ_HASH_API void JONJ_TIGERUpdate(JONJ_TIGER_CTX *context, const unsigned char *input, size_t len)
{
	if (context->length + len < 64) {
		...
		if (context->length) {
			...
			tiger_compress(context->passes, ((const jonj_hash_uint64 *) context->buffer), context->state);
...
#define JONJ_HASH_TIGER_OPS(p, b) \
	const jonj_hash_ops jonj_hash_##p##tiger##b##_ops = { \
		(jonj_hash_init_func_t) JONJ_##p##TIGERInit, \
		(jonj_hash_update_func_t) JONJ_TIGERUpdate, \
		(jonj_hash_final_func_t) JONJ_TIGER##b##Final, \
		(jonj_hash_copy_func_t) jonj_hash_copy, \
		b/8, \
		64, \
		sizeof(JONJ_TIGER_CTX) \
	}

JONJ_HASH_TIGER_OPS(3, 128);
JONJ_HASH_TIGER_OPS(3, 160);
JONJ_HASH_TIGER_OPS(3, 192);
JONJ_HASH_TIGER_OPS(4, 128);
JONJ_HASH_TIGER_OPS(4, 160);
JONJ_HASH_TIGER_OPS(4, 192);
