...
#include "jonj_hash.h"
#include "jonj_hash_gost.h"
#include "jonj_hash_gost_tables.h"
...
static inline void Gost(JONJ_GOST_CTX *context, jonj_hash_uint32 data[8])
{
	int i;
	jonj_hash_uint32 l, r, t, key[8], u[8], v[8], w[8], s[8], *h = context->state, *m = data;
...
static inline void GostTransform(JONJ_GOST_CTX *context, const unsigned char input[32])
{
	int i, j;
	jonj_hash_uint32 data[8], temp = 0, save = 0;
	
	for (i = 0, j = 0; i < 8; ++i, j += 4) {
		data[i] =	((jonj_hash_uint32) input[j]) | (((jonj_hash_uint32) input[j + 1]) << 8) | 
...
JONJ_HASH_API void JONJ_GOSTInit(JONJ_GOST_CTX *context)
{
	memset(context, 0, sizeof(*context));
	context->tables = &tables_test;
}

JONJ_HASH_API void JONJ_GOSTInitCrypto(JONJ_GOST_CTX *context)
{
	JONJ_GOSTInit(context);
	context->tables = &tables_crypto;
}

static const jonj_hash_uint32 MAX32 = 0xffffffffLU;

JONJ_HASH_API void JONJ_GOSTUpdate(JONJ_GOST_CTX *context, const unsigned char *input, size_t len)
{
...
JONJ_HASH_API void JONJ_GOSTFinal(unsigned char digest[32], JONJ_GOST_CTX *context)
{
	jonj_hash_uint32 i, j, l[8] = {0};
...
const jonj_hash_ops jonj_hash_gost_ops = {
	(jonj_hash_init_func_t) JONJ_GOSTInit,
	(jonj_hash_update_func_t) JONJ_GOSTUpdate,
	(jonj_hash_final_func_t) JONJ_GOSTFinal,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	32,
	32,
	sizeof(JONJ_GOST_CTX)
};

const jonj_hash_ops jonj_hash_gost_crypto_ops = {
	(jonj_hash_init_func_t) JONJ_GOSTInitCrypto,
	(jonj_hash_update_func_t) JONJ_GOSTUpdate,
	(jonj_hash_final_func_t) JONJ_GOSTFinal,
	(jonj_hash_copy_func_t) jonj_hash_copy,
	32,
