...
#define JONJ_LDAP_ESCAPE_FILTER 0x01
#define JONJ_LDAP_ESCAPE_DN     0x02
...
static JONJ_GINIT_FUNCTION(ldap);
 ...
 */
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY_EX("ldap.max_links", "-1", JONJ_INI_SYSTEM, OnUpdateLong, max_links, zend_ldap_globals, ldap_globals, display_link_numbers)
JONJ_INI_END()
/* }}} */
...
JONJ_MINIT_FUNCTION(ldap)
{
	...
	/* Constants to be used with deref-parameter in jonj_ldap_do_search() */
	...
	REGISTER_LONG_CONSTANT("LDAP_ESCAPE_FILTER", JONJ_LDAP_ESCAPE_FILTER, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("LDAP_ESCAPE_DN", JONJ_LDAP_ESCAPE_DN, CONST_PERSISTENT | CONST_CS);
...
JONJ_MINFO_FUNCTION(ldap)
{
	...
	jonj_info_print_table_row(2, "LDAP Support", "enabled");
	jonj_info_print_table_row(2, "RCS Version", "$Id$");
	...
	jonj_info_print_table_row(2, "Total Links", tmp);

#ifdef LDAP_API_VERSION
	snprintf(tmp, 31, "%d", LDAP_API_VERSION);
	jonj_info_print_table_row(2, "API Version", tmp);
#endif

#ifdef LDAP_VENDOR_NAME
	jonj_info_print_table_row(2, "Vendor Name", LDAP_VENDOR_NAME);
#endif

#ifdef LDAP_VENDOR_VERSION
	snprintf(tmp, 31, "%d", LDAP_VENDOR_VERSION);
	jonj_info_print_table_row(2, "Vendor Version", tmp);
#endif

#if HAVE_NSLDAP
	SDKVersion = ldap_version(&ver);
	snprintf(tmp, 31, "%F", SDKVersion/100.0);
	jonj_info_print_table_row(2, "SDK Version", tmp);

	snprintf(tmp, 31, "%F", ver.protocol_version/100.0);
	jonj_info_print_table_row(2, "Highest LDAP Protocol Supported", tmp);

	snprintf(tmp, 31, "%F", ver.SSL_version/100.0);
	jonj_info_print_table_row(2, "SSL Level Supported", tmp);
	...
	jonj_info_print_table_row(2, "Level of Encryption", tmp);
#endif

#ifdef HAVE_LDAP_SASL
	jonj_info_print_table_row(2, "SASL Support", "Enabled");
#endif
...
JONJ_FUNCTION(ldap_connect)
{
	...
	if (LDAPG(max_links) != -1 && LDAPG(num_links) >= LDAPG(max_links)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too many open links (%ld)", LDAPG(num_links));
		RETURN_FALSE;
	}
...

	{
		...
		if (!ldap_is_ldap_url(url)) {
			...
			if (port <= 0 || port > 65535) {
				efree(ld);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "invalid port number: %ld", port);
				RETURN_FALSE;
			}
		...
		if (ldap == NULL) {
			efree(ld);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not create session handle");
			RETURN_FALSE;
		}
		...
		if (rc != LDAP_SUCCESS) {
			efree(ld);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not create session handle: %s", ldap_err2string(rc));
			RETURN_FALSE;
		}
	}
	...
	if (ldap == NULL) {
		...
		if (ssl) {
			if (ldap_init_SSL(&ldap->ld_sb, wallet, walletpasswd, authmode)) {
				efree(ld);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL init failed");
				RETURN_FALSE;
			}
		}
#endif
...
JONJ_FUNCTION(ldap_bind)
{
	...
	if (ldap_bind_dn != NULL && memchr(ldap_bind_dn, '\0', ldap_bind_dnlen) != NULL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "DN contains a null byte");
		RETURN_FALSE;
	}

	if (ldap_bind_pw != NULL && memchr(ldap_bind_pw, '\0', ldap_bind_pwlen) != NULL) {
		_set_lderrno(ld->link, LDAP_INVALID_CREDENTIALS);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Password contains a null byte");
		RETURN_FALSE;
	}
	...
	if ( rc != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to bind to server: %s", ldap_err2string(rc));
		RETURN_FALSE;
	} else {
		RETURN_TRUE;
	}
...
/* {{{ _jonj_sasl_setdefs
 */
static jonj_ldap_bictx *_jonj_sasl_setdefs(LDAP *ld, char *sasl_mech, char *sasl_realm, char *sasl_authc_id, char *passwd, char *sasl_authz_id)
{
	jonj_ldap_bictx *ctx;

	ctx = ber_memalloc(sizeof(jonj_ldap_bictx));
...
/* {{{ _jonj_sasl_freedefs
 */
static void _jonj_sasl_freedefs(jonj_ldap_bictx *ctx)
{
...
static int _jonj_sasl_interact(LDAP *ld, unsigned flags, void *defaults, void *in)
{
	sasl_interact_t *interact = in;
	const char *p;
	jonj_ldap_bictx *ctx = defaults;
...
JONJ_FUNCTION(ldap_sasl_bind)
{
	...
	ctx = _jonj_sasl_setdefs(ld->link, sasl_mech, sasl_realm, sasl_authc_id, passwd, sasl_authz_id);

	if (props) {
		ldap_set_option(ld->link, LDAP_OPT_X_SASL_SECPROPS, props);
	}

	rc = ldap_sasl_interactive_bind_s(ld->link, binddn, ctx->mech, NULL, NULL, LDAP_SASL_QUIET, _jonj_sasl_interact, ctx);
	if (rc != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to bind to server: %s", ldap_err2string(rc));
		RETVAL_FALSE;
	} else {
...
/* {{{ jonj_set_opts
 */
static void jonj_set_opts(LDAP *ldap, int sizelimit, int timelimit, int deref, int *old_sizelimit, int *old_timelimit, int *old_deref)
{
	/* sizelimit */
	if (sizelimit > -1) {
...
/* {{{ jonj_ldap_do_search
 */
static void jonj_ldap_do_search(INTERNAL_FUNCTION_PARAMETERS, int scope)
{
		...
		case 4:
			...
			for (i = 0; i<num_attribs; i++) {
				if (zend_hash_index_find(Z_ARRVAL_P(attrs), i, (void **) &attr) != SUCCESS) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array initialization wrong");
					ret = 0;
					goto cleanup;
				}
	...
	if (Z_TYPE_P(link) == IS_ARRAY) {
		...
		if (nlinks == 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No links in link array");
			ret = 0;
			goto cleanup;
		}
		...
		if (Z_TYPE_P(base_dn) == IS_ARRAY) {
			...
			if (nbases != nlinks) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Base must either be a string, or an array with the same number of elements as the links array");
				ret = 0;
				goto cleanup;
			}
		...
		if (Z_TYPE_PP(filter) == IS_ARRAY) {
			...
			if (nfilters != nlinks) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter must either be a string, or an array with the same number of elements as the links array");
				ret = 0;
				goto cleanup;
			}
		...
		for (i=0; i<nlinks; i++) {
			...
			jonj_set_opts(ld->link, ldap_sizelimit, ldap_timelimit, ldap_deref, &old_ldap_sizelimit, &old_ldap_timelimit, &old_ldap_deref);
...
cleanup_parallel:
		...
		if (errno != LDAP_SUCCESS
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Search: %s", ldap_err2string(errno));
			ret = 0;
		} else {
			if (errno == LDAP_SIZELIMIT_EXCEEDED) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Partial search results returned: Sizelimit exceeded");
			}
#ifdef LDAP_ADMINLIMIT_EXCEEDED
			else if (errno == LDAP_ADMINLIMIT_EXCEEDED) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Partial search results returned: Adminlimit exceeded");
			}
#endif
...
cleanup:
	if (ld) {
		...
		jonj_set_opts(ld->link, old_ldap_sizelimit, old_ldap_timelimit, old_ldap_deref, &ldap_sizelimit, &ldap_timelimit, &ldap_deref);
	}
	if (ldap_attrs != NULL) {
		efree(ldap_attrs);
	}
...
/* {{{ proto resource ldap_read(resource|array link, string base_dn, string filter [, array attrs [, int attrsonly [, int sizelimit [, int timelimit [, int deref]]]]])
   Read an entry */
JONJ_FUNCTION(ldap_read)
{
	jonj_ldap_do_search(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_SCOPE_BASE);
}
/* }}} */
...
/* {{{ proto resource ldap_list(resource|array link, string base_dn, string filter [, array attrs [, int attrsonly [, int sizelimit [, int timelimit [, int deref]]]]])
   Single-level search */
JONJ_FUNCTION(ldap_list)
{
	jonj_ldap_do_search(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_SCOPE_ONELEVEL);
}
/* }}} */
...
JONJ_FUNCTION(ldap_search)
{
	jonj_ldap_do_search(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_SCOPE_SUBTREE);
}
/* }}} */

/* {{{ proto bool ldap_free_result(resource result)
   Free result memory */
JONJ_FUNCTION(ldap_free_result)
{
	zval *result;
...
JONJ_FUNCTION(ldap_count_entries)
{
	zval *link, *result;
	ldap_linkdata *ld;
	LDAPMessage *ldap_result;
...
JONJ_FUNCTION(ldap_first_entry)
{
	zval *link, *result;
	ldap_linkdata *ld;
...
JONJ_FUNCTION(ldap_get_entries)
{
	zval *link, *result;
	...
	while (ldap_result_entry != NULL) {
		...
		while (attribute != NULL) {
			...
			zend_hash_update(Z_ARRVAL_P(tmp1), jonj_strtolower(attribute, attr_len), attr_len+1, (void *) &tmp2, sizeof(zval *), NULL);
...
JONJ_FUNCTION(ldap_first_attribute)
{
	zval *link, *result_entry;
	ldap_linkdata *ld;
	ldap_resultentry *resultentry;
...
JONJ_FUNCTION(ldap_next_attribute)
{
	zval *link, *result_entry;
	ldap_linkdata *ld;
	ldap_resultentry *resultentry;
	...
	if (resultentry->ber == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "called before calling ldap_first_attribute() or no attributes found in result entry");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ldap_get_attributes)
{
	zval *link, *result_entry;
	zval *tmp;
	ldap_linkdata *ld;
...
JONJ_FUNCTION(ldap_get_values_len)
{
	zval *link, *result_entry;
	ldap_linkdata *ld;
	ldap_resultentry *resultentry;
	...
	if ((ldap_value_len = ldap_get_values_len(ld->link, resultentry->data, attr)) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot get the value(s) of attribute %s", ldap_err2string(_get_lderrno(ld->link)));
		RETURN_FALSE;
	}
...
#define JONJ_LD_FULL_ADD 0xff
/* {{{ jonj_ldap_do_modify
 */
static void jonj_ldap_do_modify(INTERNAL_FUNCTION_PARAMETERS, int oper)
{
	...
	for (i = 0; i < num_attribs; i++) {
		...
		if (zend_hash_get_current_key(Z_ARRVAL_P(entry), &attribute, &index, 0) == HASH_KEY_IS_STRING) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown attribute in the data");
		...
		if ((num_values == 1) && (Z_TYPE_PP(value) != IS_ARRAY)) {
			...
			for (j = 0; j < num_values; j++) {
				if (zend_hash_index_find(Z_ARRVAL_PP(value), j, (void **) &ivalue) != SUCCESS) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Value array must have consecutive indices 0, 1, ...");
					num_berval[i] = j;
					num_attribs = i + 1;
					RETVAL_FALSE;
					goto errexit;
...
/* check flag to see if do_mod was called to perform full add , gerrit thomson */
	if (is_full_add == 1) {
		if ((i = ldap_add_ext_s(ld->link, dn, ldap_mods, NULL, NULL)) != LDAP_SUCCESS) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Add: %s", ldap_err2string(i));
			RETVAL_FALSE;
		} else RETVAL_TRUE;
	} else {
		if ((i = ldap_modify_ext_s(ld->link, dn, ldap_mods, NULL, NULL)) != LDAP_SUCCESS) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Modify: %s", ldap_err2string(i));
			RETVAL_FALSE;
		} else RETVAL_TRUE;
	}
...
JONJ_FUNCTION(ldap_add)
{
	/* use a newly define parameter into the do_modify so ldap_mod_add can be used the way it is supposed to be used , Gerrit THomson */
	jonj_ldap_do_modify(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_LD_FULL_ADD);
}
/* }}} */
...
JONJ_FUNCTION(ldap_mod_replace)
{
	jonj_ldap_do_modify(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_MOD_REPLACE);
}
/* }}} */

/* {{{ proto bool ldap_mod_add(resource link, string dn, array entry)
   Add attribute values to current */
JONJ_FUNCTION(ldap_mod_add)
{
	jonj_ldap_do_modify(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_MOD_ADD);
}
/* }}} */

/* {{{ proto bool ldap_mod_del(resource link, string dn, array entry)
   Delete attribute values */
JONJ_FUNCTION(ldap_mod_del)
{
	jonj_ldap_do_modify(INTERNAL_FUNCTION_PARAM_PASSTHRU, LDAP_MOD_DELETE);
}
/* }}} */

/* {{{ proto bool ldap_delete(resource link, string dn)
   Delete an entry from a directory */
JONJ_FUNCTION(ldap_delete)
{
	zval *link;
	ldap_linkdata *ld;
	char *dn;
	...
	if ((rc = ldap_delete_ext_s(ld->link, dn, NULL, NULL)) != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Delete: %s", ldap_err2string(rc));
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ldap_modify_batch)
{
	ldap_linkdata *ld;
	...
	*/
	...
	/* perform validation */
	{
		...
		if (_ldap_strlen_max(dn, dn_len) != dn_len) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "DN must not contain NUL bytes");
			RETURN_FALSE;
		}
		...
		if (zend_hash_get_current_key_type(Z_ARRVAL_P(mods)) != HASH_KEY_IS_LONG) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Modifications array must not be string-indexed");
			RETURN_FALSE;
		}
		...
		for (i = 0; i < num_mods; i++) {
			...
			if (zend_hash_index_find(Z_ARRVAL_P(mods), i, (void **) &fetched) != SUCCESS) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Modifications array must have consecutive indices 0, 1, ...");
				RETURN_FALSE;
			}
			mod = *fetched;
			...
			if (Z_TYPE_P(mod) != IS_ARRAY) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Each entry of modifications array must be an array itself");
				RETURN_FALSE;
			}
			...
			for (j = 0; j < num_modprops; j++) {
				...
				if (zend_hash_get_current_key_ex(Z_ARRVAL_P(mod), &modkey, &modkeylen, &tmpUlong, 0, NULL) != HASH_KEY_IS_STRING) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Each entry of modifications array must be string-indexed");
					RETURN_FALSE;
				}
				...
				if (
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The only allowed keys in entries of the modifications array are '" LDAP_MODIFY_BATCH_ATTRIB "', '" LDAP_MODIFY_BATCH_MODTYPE "' and '" LDAP_MODIFY_BATCH_VALUES "'");
					RETURN_FALSE;
				}
				...
				if (_ldap_str_equal_to_const(modkey, modkeylen, LDAP_MODIFY_BATCH_ATTRIB)) {
					if (Z_TYPE_P(modinfo) != IS_STRING) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_ATTRIB "' value must be a string");
						RETURN_FALSE;
					}

					if (Z_STRLEN_P(modinfo) != _ldap_strlen_max(Z_STRVAL_P(modinfo), Z_STRLEN_P(modinfo))) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_ATTRIB "' value must not contain NUL bytes");
						RETURN_FALSE;
					}
				}
				else if (_ldap_str_equal_to_const(modkey, modkeylen, LDAP_MODIFY_BATCH_MODTYPE)) {
					if (Z_TYPE_P(modinfo) != IS_LONG) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_MODTYPE "' value must be a long");
						RETURN_FALSE;
					}
					...
					if (
						...
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The '" LDAP_MODIFY_BATCH_MODTYPE "' value must match one of the LDAP_MODIFY_BATCH_* constants");
						RETURN_FALSE;
					}
					...
					if (modtype == LDAP_MODIFY_BATCH_REMOVE_ALL) {
						if (zend_hash_exists(Z_ARRVAL_P(mod), LDAP_MODIFY_BATCH_VALUES, strlen(LDAP_MODIFY_BATCH_VALUES) + 1)) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "If '" LDAP_MODIFY_BATCH_MODTYPE "' is LDAP_MODIFY_BATCH_REMOVE_ALL, a '" LDAP_MODIFY_BATCH_VALUES "' array must not be provided");
							RETURN_FALSE;
						}
					}
					else {
						if (!zend_hash_exists(Z_ARRVAL_P(mod), LDAP_MODIFY_BATCH_VALUES, strlen(LDAP_MODIFY_BATCH_VALUES) + 1)) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "If '" LDAP_MODIFY_BATCH_MODTYPE "' is not LDAP_MODIFY_BATCH_REMOVE_ALL, a '" LDAP_MODIFY_BATCH_VALUES "' array must be provided");
							RETURN_FALSE;
						}
					}
				}
					...
					if (Z_TYPE_P(modinfo) != IS_ARRAY) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_VALUES "' value must be an array");
						RETURN_FALSE;
					}
					...
					if (num_modvals == 0) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_VALUES "' array must have at least one element");
						RETURN_FALSE;
					}

					/* are its keys integers? */
					if (zend_hash_get_current_key_type(Z_ARRVAL_P(modinfo)) != HASH_KEY_IS_LONG) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_VALUES "' array must not be string-indexed");
						RETURN_FALSE;
					}
					...
					for (k = 0; k < num_modvals; k++) {
						if (zend_hash_index_find(Z_ARRVAL_P(modinfo), k, (void **) &fetched) != SUCCESS) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A '" LDAP_MODIFY_BATCH_VALUES "' array must have consecutive indices 0, 1, ...");
							RETURN_FALSE;
						}
						modval = *fetched;
						...
						if (Z_TYPE_P(modval) != IS_STRING) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Each element of a '" LDAP_MODIFY_BATCH_VALUES "' array must be a string");
							RETURN_FALSE;
						}
					}
				}
	...
	for (i = 0; i < num_mods; i++) {
		...
		switch (Z_LVAL_P(modtype)) {
			...
			case LDAP_MODIFY_BATCH_REPLACE:
				...
				break;
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Unknown and uncaught modification type.");
				RETURN_FALSE;
		}
	...
	/* perform (finally) */
	if ((i = ldap_modify_ext_s(ld->link, dn, ldap_mods, NULL, NULL)) != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Batch Modify: %s", ldap_err2string(i));
		RETVAL_FALSE;
	} else RETVAL_TRUE;
...
JONJ_FUNCTION(ldap_compare)
{
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Compare: %s", ldap_err2string(errno));
	RETURN_LONG(-1);
}
/* }}} */
...
JONJ_FUNCTION(ldap_sort)
{
	...
	if (zend_hash_index_find(&EG(regular_list), Z_LVAL_P(result), (void **) &le) != SUCCESS || le->type != le_result) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Supplied resource is not a valid ldap result resource");
		RETURN_FALSE;
	}

	if (ldap_sort_entries(ld->link, (LDAPMessage **) &le->ptr, sflen ? sortfilter : NULL, strcmp) != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ldap_err2string(errno));
		RETURN_FALSE;
	}
	...
	case LDAP_OPT_CLIENT_CONTROLS:
		{
			...
			if ((Z_TYPE_PP(newval) != IS_ARRAY) || !(ncontrols = zend_hash_num_elements(Z_ARRVAL_PP(newval)))) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Expected non-empty array value for this option");
				RETURN_FALSE;
			}
			...
			*ctrls = NULL;
			...
			while (zend_hash_get_current_data(Z_ARRVAL_PP(newval), (void**)&ctrlval) == SUCCESS) {
				if (Z_TYPE_PP(ctrlval) != IS_ARRAY) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The array value must contain only arrays, where each array is a control");
					error = 1;
					break;
				}
				if (zend_hash_find(Z_ARRVAL_PP(ctrlval), "oid", sizeof("oid"), (void **) &val) != SUCCESS) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Control must have an oid key");
					error = 1;
					break;
				}
...
JONJ_FUNCTION(ldap_parse_result)
{
	...
	rc = ldap_parse_result(ld->link, ldap_result, &lerrcode,
				...
				0);
	if (rc != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to parse result: %s", ldap_err2string(rc));
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ldap_first_reference)
{
	zval *link, *result;
	ldap_linkdata *ld;
	ldap_resultentry *resultentry;
...
/* {{{ proto resource ldap_next_reference(resource link, resource reference_entry)
   Get next reference */
JONJ_FUNCTION(ldap_next_reference)
{
	zval *link, *result_entry;
	ldap_linkdata *ld;
	ldap_resultentry *resultentry, *resultentry_next;
...
JONJ_FUNCTION(ldap_parse_reference)
{
...
#else
	if (newparent_len != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "You are using old LDAP API, newparent must be the empty string, can only modify RDN");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ldap_start_tls)
{
	...
	if (((rc = ldap_set_option(ld->link, LDAP_OPT_PROTOCOL_VERSION, &protocol)) != LDAP_SUCCESS) ||
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,"Unable to start TLS: %s", ldap_err2string(rc));
		RETURN_FALSE;
	} else {
		RETURN_TRUE;
	}
...
int _ldap_rebind_proc(LDAP *ldap, const char *url, ber_tag_t req, ber_int_t msgid, void *params)
{
	...
	if (ld == NULL || ld->rebindproc == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Link not found or no callback set");
		return LDAP_OTHER;
	}
	...
	if (call_user_function_ex(EG(function_table), NULL, ld->rebindproc, &cb_retval, 2, cb_args, 0, NULL TSRMLS_CC) == SUCCESS && cb_retval) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "rebind_proc JONJ callback failed");
		retval = LDAP_OTHER;
	}
	zval_dtor(cb_url);
	FREE_ZVAL(cb_url);
...
JONJ_FUNCTION(ldap_set_rebind_proc)
{
	zval *link, *callback;
	ldap_linkdata *ld;
	char *callback_name;
	...
	if (!zend_is_callable(callback, 0, &callback_name TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Two arguments expected for '%s' to be a valid callback", callback_name);
		efree(callback_name);
		RETURN_FALSE;
	}
	efree(callback_name);
...
static void jonj_ldap_do_escape(const zend_bool *map, const char *value, size_t valuelen, char **result, size_t *resultlen)
{
...
static void jonj_ldap_escape_map_set_chars(zend_bool *map, const char *chars, const int charslen, char escape)
{
	int i = 0;
	while (i < charslen) {
...
JONJ_FUNCTION(ldap_escape)
{
	...
	if (flags & JONJ_LDAP_ESCAPE_FILTER) {
		havecharlist = 1;
		jonj_ldap_escape_map_set_chars(map, "\\*()\0", sizeof("\\*()\0") - 1, 1);
	}

	if (flags & JONJ_LDAP_ESCAPE_DN) {
		havecharlist = 1;
		jonj_ldap_escape_map_set_chars(map, "\\,=+<>;\"#", sizeof("\\,=+<>;\"#") - 1, 1);
	}
	...
	if (ignoreslen) {
		jonj_ldap_escape_map_set_chars(map, ignores, ignoreslen, 0);
	}

	jonj_ldap_do_escape(map, value, valuelen, &result, &resultlen);

	RETURN_STRINGL(result, resultlen, 0);
}

#ifdef STR_TRANSLATION
/* {{{ jonj_ldap_do_translate
 */
static void jonj_ldap_do_translate(INTERNAL_FUNCTION_PARAMETERS, int way)
{
	char *value;
	int result, ldap_len;
	...
	if (result == LDAP_SUCCESS) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Conversion from iso-8859-1 to t61 failed: %s", ldap_err2string(result));
		RETVAL_FALSE;
	}
}
/* }}} */
...
JONJ_FUNCTION(ldap_t61_to_8859)
{
	jonj_ldap_do_translate(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ proto string ldap_8859_to_t61(string value)
   Translate 8859 characters to t61 characters */
JONJ_FUNCTION(ldap_8859_to_t61)
{
	jonj_ldap_do_translate(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
#endif
...
JONJ_FUNCTION(ldap_control_paged_result)
{
	long pagesize;
	...
	if (ber == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to alloc BER encoding resources for paged results control");
		RETURN_FALSE;
	}
	...
	if (ber_printf(ber, "{iO}", (int)pagesize, &lcookie) == LBER_ERROR) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to BER printf paged results control");
		RETVAL_FALSE;
		goto lcpr_error_out;
	}
	rc = ber_flatten2(ber, &ctrl.ldctl_value, 0);
	if (rc == LBER_ERROR) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to BER encode paged results control");
		RETVAL_FALSE;
		goto lcpr_error_out;
	}
	...
	if (ldap) {
		...
		if (rc != LDAP_SUCCESS) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set paged results control: %s (%d)", ldap_err2string(rc), rc);
			RETVAL_FALSE;
			goto lcpr_error_out;
		}
		RETVAL_TRUE;
	} else {
		/* return a JONJ control object */
		array_init(return_value);
...
JONJ_FUNCTION(ldap_control_paged_result_response)
{
	...
	if (rc != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to parse result: %s (%d)", ldap_err2string(rc), rc);
		RETURN_FALSE;
	}

	if (lerrcode != LDAP_SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Result is: %s (%d)", ldap_err2string(lerrcode), lerrcode);
		RETURN_FALSE;
	}

	if (lserverctrls == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No server controls in result");
		RETURN_FALSE;
	}
	...
	if (lctrl == NULL) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No paged results control response in result");
		RETURN_FALSE;
	}
	...
	if (ber == NULL) {
		ldap_controls_free(lserverctrls);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to alloc BER decoding resources for paged results control response");
		RETURN_FALSE;
	}
	...
	if (tag == LBER_ERROR) {
		ldap_controls_free(lserverctrls);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to decode paged results control response");
		RETURN_FALSE;
	}

	if (lestimated < 0) {
		ldap_controls_free(lserverctrls);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid paged results control response value");
		RETURN_FALSE;
	}
...
const zend_function_entry ldap_functions[] = {
	JONJ_FE(ldap_connect,								arginfo_ldap_connect)
	JONJ_FALIAS(ldap_close,		ldap_unbind,			arginfo_ldap_resource)
	JONJ_FE(ldap_bind,									arginfo_ldap_bind)
#ifdef HAVE_LDAP_SASL
	JONJ_FE(ldap_sasl_bind,								arginfo_ldap_sasl_bind)
#endif
	JONJ_FE(ldap_unbind,									arginfo_ldap_resource)
	JONJ_FE(ldap_read,									arginfo_ldap_read)
	JONJ_FE(ldap_list,									arginfo_ldap_list)
	JONJ_FE(ldap_search,									arginfo_ldap_search)
	JONJ_FE(ldap_free_result,							arginfo_ldap_resource)
	JONJ_FE(ldap_count_entries,							arginfo_ldap_count_entries)
	JONJ_FE(ldap_first_entry,							arginfo_ldap_first_entry)
	JONJ_FE(ldap_next_entry,								arginfo_ldap_next_entry)
	JONJ_FE(ldap_get_entries,							arginfo_ldap_get_entries)
	JONJ_FE(ldap_first_attribute,						arginfo_ldap_first_attribute)
	JONJ_FE(ldap_next_attribute,							arginfo_ldap_next_attribute)
	JONJ_FE(ldap_get_attributes,							arginfo_ldap_get_attributes)
	JONJ_FALIAS(ldap_get_values,	ldap_get_values_len,	arginfo_ldap_get_values)
	JONJ_FE(ldap_get_values_len,							arginfo_ldap_get_values_len)
	JONJ_FE(ldap_get_dn,									arginfo_ldap_get_dn)
	JONJ_FE(ldap_explode_dn,								arginfo_ldap_explode_dn)
	JONJ_FE(ldap_dn2ufn,									arginfo_ldap_dn2ufn)
	JONJ_FE(ldap_add,									arginfo_ldap_add)
	JONJ_FE(ldap_delete,									arginfo_ldap_delete)
	JONJ_FE(ldap_modify_batch,							arginfo_ldap_modify_batch)
	JONJ_FALIAS(ldap_modify,		ldap_mod_replace,		arginfo_ldap_modify)

/* additional functions for attribute based modifications, Gerrit Thomson */
	JONJ_FE(ldap_mod_add,								arginfo_ldap_mod_add)
	JONJ_FE(ldap_mod_replace,							arginfo_ldap_mod_replace)
	JONJ_FE(ldap_mod_del,								arginfo_ldap_mod_del)
/* end gjt mod */

	JONJ_FE(ldap_errno,									arginfo_ldap_resource)
	JONJ_FE(ldap_err2str,								arginfo_ldap_err2str)
	JONJ_FE(ldap_error,									arginfo_ldap_resource)
	JONJ_FE(ldap_compare,								arginfo_ldap_compare)
	JONJ_FE(ldap_sort,									arginfo_ldap_sort)

#if (LDAP_API_VERSION > 2000) || HAVE_NSLDAP || HAVE_ORALDAP
	JONJ_FE(ldap_rename,									arginfo_ldap_rename)
	JONJ_FE(ldap_get_option,								arginfo_ldap_get_option)
	JONJ_FE(ldap_set_option,								arginfo_ldap_set_option)
	JONJ_FE(ldap_first_reference,						arginfo_ldap_first_reference)
	JONJ_FE(ldap_next_reference,							arginfo_ldap_next_reference)
#ifdef HAVE_LDAP_PARSE_REFERENCE
	JONJ_FE(ldap_parse_reference,						arginfo_ldap_parse_reference)
#endif
#ifdef HAVE_LDAP_PARSE_RESULT
	JONJ_FE(ldap_parse_result,							arginfo_ldap_parse_result)
#endif
#ifdef HAVE_LDAP_START_TLS_S
	JONJ_FE(ldap_start_tls,								arginfo_ldap_resource)
#endif
#endif

#if defined(LDAP_API_FEATURE_X_OPENLDAP) && defined(HAVE_3ARG_SETREBINDPROC)
	JONJ_FE(ldap_set_rebind_proc,						arginfo_ldap_set_rebind_proc)
#endif

	JONJ_FE(ldap_escape,									arginfo_ldap_escape)

#ifdef STR_TRANSLATION
	JONJ_FE(ldap_t61_to_8859,							arginfo_ldap_t61_to_8859)
	JONJ_FE(ldap_8859_to_t61,							arginfo_ldap_8859_to_t61)
#endif

#ifdef LDAP_CONTROL_PAGEDRESULTS
	JONJ_FE(ldap_control_paged_result,							arginfo_ldap_control_paged_result)
	JONJ_FE(ldap_control_paged_result_response,		arginfo_ldap_control_paged_result_response)
#endif
	JONJ_FE_END
};
/* }}} */
