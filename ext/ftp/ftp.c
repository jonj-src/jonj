...
static int		my_send(ftpbuf_t *ftp, jonj_socket_t s, void *buf, size_t len);
static int		my_recv(ftpbuf_t *ftp, jonj_socket_t s, void *buf, size_t len);
static int		my_accept(ftpbuf_t *ftp, jonj_socket_t s, struct sockaddr *addr, socklen_t *addrlen);
...
ftp_open(const char *host, short port, long timeout_sec TSRMLS_DC)
{
	...
	ftp->fd = jonj.tkwork_connect_socket_to_host(host,
	...
	if (getsockname(ftp->fd, (struct sockaddr*) &ftp->localaddr, &size) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "getsockname failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
...
ftp_login(ftpbuf_t *ftp, const char *user, const char *pass TSRMLS_DC)
{
	...
	if (ftp->use_ssl && !ftp->ssl_active) {
		...
		if (ctx == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to create the SSL context");
			return 0;
		}
		...
		if (ftp->ssl_handle == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to create the SSL handle");
			SSL_CTX_free(ctx);
			return 0;
		}
		...
		if (SSL_connect(ftp->ssl_handle) <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL/TLS handshake failed");
			SSL_shutdown(ftp->ssl_handle);
			SSL_free(ftp->ssl_handle);
			return 0;
		}
 ...
 */
int
ftp_get(ftpbuf_t *ftp, jonj_stream *outstream, const char *path, ftptype_t type, long resumepos TSRMLS_DC)
{
	databuf_t		*data = NULL;
	size_t			rcvd;
	char			arg[11];
	...
	while ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		if (type == FTPTYPE_ASCII) {
			 ...
			 */
#ifdef JONJ_WIN32
			jonj_stream_write(outstream, ptr, (e - ptr));
			ptr = e;
#else
			while (e > ptr && (s = memchr(ptr, '\r', (e - ptr)))) {
				jonj_stream_write(outstream, ptr, (s - ptr));
				if (*(s + 1) == '\n') {
					s++;
					jonj_stream_putc(outstream, '\n');
				}
				ptr = s + 1;
			}
#endif
		...
		} else if (rcvd != jonj_stream_write(outstream, data->buf, rcvd)) {
			goto bail;
		}
	}
 ...
 */
int
ftp_put(ftpbuf_t *ftp, const char *path, jonj_stream *instream, ftptype_t type, long startpos TSRMLS_DC)
{
	databuf_t		*data = NULL;
	long			size;
	char			*ptr;
	...
	size = 0;
	ptr = data->buf;
	while (!jonj_stream_eof(instream) && (ch = jonj_stream_getc(instream))!=EOF) {
		/* flush if necessary */
 ...
 */
int
my_send(ftpbuf_t *ftp, jonj_socket_t s, void *buf, size_t len)
{
	long		size, sent;
    int         n;
	...
	size = len;
	while (size) {
		n = jonj_pollfd_for_ms(s, POLLOUT, ftp->timeout_sec * 1000);
 ...
 */
int
my_recv(ftpbuf_t *ftp, jonj_socket_t s, void *buf, size_t len)
{
	int		n, nr_bytes;

	n = jonj_pollfd_for_ms(s, JONJ_POLLREADABLE, ftp->timeout_sec * 1000);
	if (n < 1) {
#if !defined(JONJ_WIN32) && !(defined(NETWARE) && defined(USE_WINSOCK))
		if (n == 0) {
			errno = ETIMEDOUT;
 ...
 */
int
data_available(ftpbuf_t *ftp, jonj_socket_t s)
{
	int		n;

	n = jonj_pollfd_for_ms(s, JONJ_POLLREADABLE, 1000);
	if (n < 1) {
#if !defined(JONJ_WIN32) && !(defined(NETWARE) && defined(USE_WINSOCK))
		if (n == 0) {
			errno = ETIMEDOUT;
 ...
 */
int
data_writeable(ftpbuf_t *ftp, jonj_socket_t s)
{
	int		n;

	n = jonj_pollfd_for_ms(s, POLLOUT, 1000);
	if (n < 1) {
#ifndef JONJ_WIN32
		if (n == 0) {
			errno = ETIMEDOUT;
 ...
 */
int
my_accept(ftpbuf_t *ftp, jonj_socket_t s, struct sockaddr *addr, socklen_t *addrlen)
{
	int		n;
...
ftp_getdata(ftpbuf_t *ftp TSRMLS_DC)
{
	...
	if ((fd = socket(sa->sa_family, SOCK_STREAM, 0)) == SOCK_ERR) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "socket() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
	...
	if (ftp->pasv) {
		...
		size = jonj_sockaddr_size(&ftp->pasvaddr);
		tv.tv_sec = ftp->timeout_sec;
		tv.tv_usec = 0;
		if (jonj_connect_nonb(fd, (struct sockaddr*) &ftp->pasvaddr, size, &tv) == -1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "jonj_connect_nonb() failed: %s (%d)", strerror(errno), errno);
			goto bail;
		}
	...
	jonj_any_addr(sa->sa_family, &addr, 0);
	size = jonj_sockaddr_size(&addr);

	if (bind(fd, (struct sockaddr*) &addr, size) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "bind() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
	...
	if (listen(fd, 5) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "listen() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
...
data_accepted:
	...
	if (ftp->use_ssl && ftp->use_ssl_for_data) {
		...
		if (ctx == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_accept: failed to retreive the existing SSL context");
			return 0;
		}

		data->ssl_handle = SSL_new(ctx);
		if (data->ssl_handle == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_accept: failed to create the SSL handle");
			return 0;
		}
		...
		if (session == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_accept: failed to retreive the existing SSL session");
		...
		if (result == 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_accept: failed to set the existing SSL session");
			SSL_free(data->ssl_handle);
			return 0;
		}

		if (SSL_connect(data->ssl_handle) <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_accept: SSL/TLS handshake failed");
...
ftp_genlist(ftpbuf_t *ftp, const char *cmd, const char *path TSRMLS_DC)
{
	...
	if ((tmpstream = jonj_stream_fopen_tmpfile()) == NULL) {
	...
	while ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		jonj_stream_write(tmpstream, data->buf, rcvd);
	...
	*entry = text;
	lastch = 0;
	while ((ch = jonj_stream_getc(tmpstream)) != EOF) {
		if (ch == '\n' && lastch == '\r') {
			*(text - 1) = 0;
			*++entry = text;
		} else {
 ...
 */
int
ftp_nb_get(ftpbuf_t *ftp, jonj_stream *outstream, const char *path, ftptype_t type, long resumepos TSRMLS_DC)
{
	databuf_t		*data = NULL;
	char			arg[11];

	if (ftp == NULL) {
		return JONJ_FTP_FAILED;
	}
...
ftp_nb_continue_read(ftpbuf_t *ftp TSRMLS_DC)
{
	...
	if ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		if (type == FTPTYPE_ASCII) {
			for (ptr = data->buf; rcvd; rcvd--, ptr++) {
				if (lastch == '\r' && *ptr != '\n') {
					jonj_stream_putc(ftp->stream, '\r');
				}
				if (*ptr != '\r') {
					jonj_stream_putc(ftp->stream, *ptr);
				}
				lastch = *ptr;
			}
		} else if (rcvd != jonj_stream_write(ftp->stream, data->buf, rcvd)) {
			goto bail;
		}
 ...
 */
int
ftp_nb_put(ftpbuf_t *ftp, const char *path, jonj_stream *instream, ftptype_t type, long startpos TSRMLS_DC)
{
	databuf_t		*data = NULL;
	char			arg[11];
...
ftp_nb_continue_write(ftpbuf_t *ftp TSRMLS_DC)
{
	...
	size = 0;
	ptr = ftp->data->buf;
	while (!jonj_stream_eof(ftp->stream) && (ch = jonj_stream_getc(ftp->stream)) != EOF) {
