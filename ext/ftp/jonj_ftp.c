...
const zend_function_entry jonj_ftp_functions[] = {
	JONJ_FE(ftp_connect,			arginfo_ftp_connect)
#if HAVE_OPENSSL_EXT
	JONJ_FE(ftp_ssl_connect,		arginfo_ftp_ssl_connect)
#endif	
	JONJ_FE(ftp_login,			arginfo_ftp_login)
	JONJ_FE(ftp_pwd,				arginfo_ftp_pwd)
	JONJ_FE(ftp_cdup,			arginfo_ftp_cdup)
	JONJ_FE(ftp_chdir,			arginfo_ftp_chdir)
	JONJ_FE(ftp_exec,			arginfo_ftp_exec)
	JONJ_FE(ftp_raw,				arginfo_ftp_raw)
	JONJ_FE(ftp_mkdir,			arginfo_ftp_mkdir)
	JONJ_FE(ftp_rmdir,			arginfo_ftp_rmdir)
	JONJ_FE(ftp_chmod,			arginfo_ftp_chmod)
	JONJ_FE(ftp_alloc,			arginfo_ftp_alloc)
	JONJ_FE(ftp_nlist,			arginfo_ftp_nlist)
	JONJ_FE(ftp_rawlist,			arginfo_ftp_rawlist)
	JONJ_FE(ftp_systype,			arginfo_ftp_systype)
	JONJ_FE(ftp_pasv,			arginfo_ftp_pasv)
	JONJ_FE(ftp_get,				arginfo_ftp_get)
	JONJ_FE(ftp_fget,			arginfo_ftp_fget)
	JONJ_FE(ftp_put,				arginfo_ftp_put)
	JONJ_FE(ftp_fput,			arginfo_ftp_fput)
	JONJ_FE(ftp_size,			arginfo_ftp_size)
	JONJ_FE(ftp_mdtm,			arginfo_ftp_mdtm)
	JONJ_FE(ftp_rename,			arginfo_ftp_rename)
	JONJ_FE(ftp_delete,			arginfo_ftp_delete)
	JONJ_FE(ftp_site,			arginfo_ftp_site)
	JONJ_FE(ftp_close,			arginfo_ftp_close)
	JONJ_FE(ftp_set_option,		arginfo_ftp_set_option)
	JONJ_FE(ftp_get_option,		arginfo_ftp_get_option)
	JONJ_FE(ftp_nb_fget,			arginfo_ftp_nb_fget)
	JONJ_FE(ftp_nb_get,			arginfo_ftp_nb_get)
	JONJ_FE(ftp_nb_continue,		arginfo_ftp_nb_continue)
	JONJ_FE(ftp_nb_put,			arginfo_ftp_nb_put)
	JONJ_FE(ftp_nb_fput,			arginfo_ftp_nb_fput)
	JONJ_FALIAS(ftp_quit, ftp_close, arginfo_ftp_close)
	JONJ_FE_END
};

zend_module_entry jonj_ftp_module_entry = {
...
JONJ_MINIT_FUNCTION(ftp)
{
	...
	REGISTER_LONG_CONSTANT("FTP_AUTORESUME", JONJ_FTP_AUTORESUME, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_TIMEOUT_SEC", JONJ_FTP_OPT_TIMEOUT_SEC, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_AUTOSEEK", JONJ_FTP_OPT_AUTOSEEK, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_USEPASVADDRESS", JONJ_FTP_OPT_USEPASVADDRESS, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_FAILED", JONJ_FTP_FAILED, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_FINISHED", JONJ_FTP_FINISHED, CONST_PERSISTENT | CONST_CS);
	REGISTER_LONG_CONSTANT("FTP_MOREDATA", JONJ_FTP_MOREDATA, CONST_PERSISTENT | CONST_CS);
	return SUCCESS;
}
...
JONJ_MINFO_FUNCTION(ftp)
{
	...
	jonj_info_print_table_row(2, "FTP support", "enabled");
	jonj_info_print_table_end();
}

#define	XTYPE(xtype, mode)	{ \
								if (mode != FTPTYPE_ASCII && mode != FTPTYPE_IMAGE) { \
									jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Mode must be FTP_ASCII or FTP_BINARY"); \
									RETURN_FALSE; \
								} \
								xtype = mode; \
							}
...
JONJ_FUNCTION(ftp_connect)
{
	...
	if (timeout_sec <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Timeout has to be greater than 0");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ftp_login)
{
	...
	if (!ftp_login(ftp, user, pass TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ftp_fget)
{
	...
	ZEND_FETCH_RESOURCE(ftp, ftpbuf_t*, &z_ftp, -1, le_ftpbuf_name, le_ftpbuf);
	jonj_stream_from_zval(stream, &z_file);
	XTYPE(xtype, mode);

	/* ignore autoresume if autoseek is switched off */
	if (!ftp->autoseek && resumepos == JONJ_FTP_AUTORESUME) {
		resumepos = 0;
	}

	if (ftp->autoseek && resumepos) {
		/* if autoresume is wanted seek to end */
		if (resumepos == JONJ_FTP_AUTORESUME) {
			jonj_stream_seek(stream, 0, SEEK_END);
			resumepos = jonj_stream_tell(stream);
		} else {
			jonj_stream_seek(stream, resumepos, SEEK_SET);
		}
	}
...
JONJ_FUNCTION(ftp_nb_fget)
{
	...
	if ((ret = ftp_nb_get(ftp, stream, file, xtype, resumepos TSRMLS_CC)) == JONJ_FTP_FAILED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_LONG(ret);
	}
...
JONJ_FUNCTION(ftp_get)
{
	...
	if (ftp->autoseek && resumepos) {
		outstream = jonj_stream_open_wrapper(local, mode == FTPTYPE_ASCII ? "rt+" : "rb+", REPORT_ERRORS, NULL);
		if (outstream == NULL) {
			outstream = jonj_stream_open_wrapper(local, mode == FTPTYPE_ASCII ? "wt" : "wb", REPORT_ERRORS, NULL);
		}
		if (outstream != NULL) {
			/* if autoresume is wanted seek to end */
			if (resumepos == JONJ_FTP_AUTORESUME) {
				jonj_stream_seek(outstream, 0, SEEK_END);
				resumepos = jonj_stream_tell(outstream);
			} else {
				jonj_stream_seek(outstream, resumepos, SEEK_SET);
			}
		}
	} else {
		outstream = jonj_stream_open_wrapper(local, mode == FTPTYPE_ASCII ? "wt" : "wb", REPORT_ERRORS, NULL);
	...
	if (outstream == NULL)	{
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error opening %s", local);
		RETURN_FALSE;
	}

	if (!ftp_get(ftp, outstream, remote, xtype, resumepos TSRMLS_CC)) {
		jonj_stream_close(outstream);
		VCWD_UNLINK(local);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(ftp_nb_get)
{
	...
	if ((ret = ftp_nb_get(ftp, outstream, remote, xtype, resumepos TSRMLS_CC)) == JONJ_FTP_FAILED) {
		jonj_stream_close(outstream);
		ftp->stream = NULL;
		VCWD_UNLINK(local);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_LONG(JONJ_FTP_FAILED);
	}

	if (ret == JONJ_FTP_FINISHED){
		jonj_stream_close(outstream);
		ftp->stream = NULL;
	}
...
JONJ_FUNCTION(ftp_nb_continue)
{
	...
	if (!ftp->nb) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no nbronous transfer to continue.");
		RETURN_LONG(JONJ_FTP_FAILED);
	}
	...
	if (ret != JONJ_FTP_MOREDATA && ftp->closestream) {
		jonj_stream_close(ftp->stream);
		ftp->stream = NULL;
	}

	if (ret == JONJ_FTP_FAILED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
	}
...
JONJ_FUNCTION(ftp_fput)
{
	...
	/* ignore autoresume if autoseek is switched off */
	if (!ftp->autoseek && startpos == JONJ_FTP_AUTORESUME) {
		startpos = 0;
	}

	if (ftp->autoseek && startpos) {
		/* if autoresume is wanted ask for remote size */
		if (startpos == JONJ_FTP_AUTORESUME) {
		...
		if (startpos) {
			jonj_stream_seek(stream, startpos, SEEK_SET);
		}
	}
...
JONJ_FUNCTION(ftp_nb_fput)
{
	...
	if (((ret = ftp_nb_put(ftp, remote, stream, xtype, startpos TSRMLS_CC)) == JONJ_FTP_FAILED)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_LONG(ret);
	}
...
JONJ_FUNCTION(ftp_put)
{
	...
	if (!(instream = jonj_stream_open_wrapper(local, mode == FTPTYPE_ASCII ? "rt" : "rb", REPORT_ERRORS, NULL))) {
		RETURN_FALSE;
	}
	...
	if (ftp->autoseek && startpos) {
		...
		if (startpos) {
			jonj_stream_seek(instream, startpos, SEEK_SET);
		}
	}

	if (!ftp_put(ftp, remote, instream, xtype, startpos TSRMLS_CC)) {
		jonj_stream_close(instream);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", ftp->inbuf);
		RETURN_FALSE;
	}
	jonj_stream_close(instream);
...
JONJ_FUNCTION(ftp_set_option)
{
	...
	switch (option) {
		case JONJ_FTP_OPT_TIMEOUT_SEC:
			if (Z_TYPE_P(z_value) != IS_LONG) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Option TIMEOUT_SEC expects value of type long, %s given",
			...
			RETURN_TRUE;
			break;
		case JONJ_FTP_OPT_AUTOSEEK:
			if (Z_TYPE_P(z_value) != IS_BOOL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Option AUTOSEEK expects value of type boolean, %s given",
					zend_zval_type_name(z_value));
				RETURN_FALSE;
			}
			...
			RETURN_TRUE;
			break;
		case JONJ_FTP_OPT_USEPASVADDRESS:
			if (Z_TYPE_P(z_value) != IS_BOOL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Option USEPASVADDRESS expects value of type boolean, %s given",
					zend_zval_type_name(z_value));
				RETURN_FALSE;
			}
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown option '%ld'", option);
			RETURN_FALSE;
			break;
	}
}
