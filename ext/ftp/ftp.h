...
#define JONJ_FTP_FAILED			0
#define JONJ_FTP_FINISHED		1
#define JONJ_FTP_MOREDATA		2
...
typedef struct databuf
{
	...
	jonj_socket_t		fd;			/* data connection */
...
{
	jonj_socket_t		fd;			/* control connection */
	jonj_sockaddr_storage	localaddr;	/* local address */
	...
	jonj_sockaddr_storage	pasvaddr;	/* passive mode address */
	...
	jonj_stream		*stream; /* output stream for "nonblocking" transfers */
 ...
 * returns true on success, false on error
 */
int		ftp_get(ftpbuf_t *ftp, jonj_stream *outstream, const char *path, ftptype_t type, long resumepos TSRMLS_DC);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_put(ftpbuf_t *ftp, const char *path, jonj_stream *instream, ftptype_t type, long startpos TSRMLS_DC);
 ...
 * returns true on success, false on error
 */
int		ftp_nb_get(ftpbuf_t *ftp, jonj_stream *outstream, const char *path, ftptype_t type, long resumepos TSRMLS_DC);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_nb_put(ftpbuf_t *ftp, const char *path, jonj_stream *instream, ftptype_t type, long startpos TSRMLS_DC);
