...
static void gdPngErrorHandler (png_structp png_ptr, png_const_charp msg)
{
	...
	jonj_gd_error_ex(E_WARNING, "gd-png:  fatal libpng error: %s", msg);

	jmpbuf_ptr = png_get_error_ptr (png_ptr);
	if (jmpbuf_ptr == NULL) { /* we are completely hosed now */
		jonj_gd_error_ex(E_ERROR, "gd-png:  EXTREMELY fatal error: jmpbuf unrecoverable; terminating.");
	}
...
gdImagePtr gdImageCreateFromPngCtx (gdIOCtx * infile)
{
...
#endif
	if (png_ptr == NULL) {
		jonj_gd_error("gd-png error: cannot allocate libpng main struct");
		return NULL;
	}

	info_ptr = png_create_info_struct(png_ptr);
	if (info_ptr == NULL) {
		jonj_gd_error("gd-png error: cannot allocate libpng info struct");
	...
	if (setjmp(jbw.jmpbuf)) {
		jonj_gd_error("gd-png error: setjmp returns error condition");
	...
	if (im == NULL) {
		jonj_gd_error("gd-png error: cannot allocate gdImage struct");
		png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
	...
	switch (color_type) {
...
#ifdef DEBUG
			jonj_gd_error("gd-png color_type is palette, colors: %d", num_palette);
#endif /* DEBUG */
		...
		case PNG_COLOR_TYPE_GRAY:
			...
			if ((palette = (png_colorp) gdMalloc (256 * sizeof (png_color))) == NULL) {
				jonj_gd_error("gd-png error: cannot allocate gray palette");
				png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
	...
	if (!im->trueColor) {
		for (i = num_palette; i < gdMaxColors; ++i) {
			if (!open[i]) {
				jonj_gd_error("gd-png warning: image data references out-of-range color index (%d)", i);
			}
		}
	}
#endif
...
void gdImagePngCtxEx (gdImagePtr im, gdIOCtx * outfile, int level, int basefilter)
{
	...
	if (level != -1 && (level < 0 || level > 9)) {
		jonj_gd_error("gd-png error: compression level must be 0 through 9");
		return;
	}
	...
	if (!im->trueColor) {
		...
		if (colors == 0) {
			jonj_gd_error("gd-png error: no colors in palette");
			goto bail;
		}
