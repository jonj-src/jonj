...
{
	IO_DBG (jonj_gd_error("Putting word..."));
	(ctx->putC) (ctx, (unsigned char) (w >> 8));
	(ctx->putC) (ctx, (unsigned char) (w & 0xFF));
	IO_DBG (jonj_gd_error("put."));
}

void gdPutInt (int w, gdIOCtx * ctx)
{
	IO_DBG (jonj_gd_error("Putting int..."));
...
{
	IO_DBG (jonj_gd_error("Putting buf..."));
...
{
	IO_DBG (jonj_gd_error("Seeking..."));
	return ((ctx->seek) (ctx, pos));
	IO_DBG (jonj_gd_error("Done."));
}

long gdTell (gdIOCtx * ctx)
{
	IO_DBG (jonj_gd_error("Telling..."));
	return ((ctx->tell) (ctx));
	IO_DBG (jonj_gd_error ("told."));
}
