...
static long jonj_jpeg_emit_message(j_common_ptr jpeg_info, int level)
{
	...
	if (level < 0) {
		...
		if ((jpeg_info->err->num_warnings == 0) || (jpeg_info->err->trace_level >= 3)) {
			jonj_gd_error_ex(ignore_warning ? E_NOTICE : E_WARNING, "gd-jpeg, libjpeg: recoverable error: %s\n", message);
		}
		...
		if (jpeg_info->err->trace_level >= level) {
			jonj_gd_error_ex(E_NOTICE, "gd-jpeg, libjpeg: strace message: %s\n", message);
		}
	}
	return 1;
}
...
static void fatal_jpeg_error (j_common_ptr cinfo)
{
	...
	jonj_gd_error("gd-jpeg: JPEG library reports unrecoverable error: ");
	...
	if (jmpbufw != 0) {
		...
		jonj_gd_error_ex(E_ERROR, "gd-jpeg: EXTREMELY fatal error: longjmp returned control; terminating");
	} else {
		jonj_gd_error_ex(E_ERROR, "gd-jpeg: EXTREMELY fatal error: jmpbuf unrecoverable; terminating");
	}
...
void gdImageJpegCtx (gdImagePtr im, gdIOCtx * outfile, int quality)
{
	...
	if (im->trueColor) {
		...
		for (i = 0; i < im->sy; i++) {
			...
			if (nlines != 1) {
				jonj_gd_error_ex(E_WARNING, "gd_jpeg: warning: jpeg_write_scanlines returns %u -- expected 1", nlines);
			}
		}
	} else {
		for (i = 0; i < im->sy; i++) {
...
gdImagePtr gdImageCreateFromJpegCtxEx (gdIOCtx * infile, int ignore_warning)
{
	...
	cinfo.err->emit_message = (void (*)(j_common_ptr,int)) jonj_jpeg_emit_message;
	...
	if (retval != JPEG_HEADER_OK) {
		jonj_gd_error_ex(E_WARNING, "gd-jpeg: warning: jpeg_read_header returned %d, expected %d", retval, JPEG_HEADER_OK);
	}

	if (cinfo.image_height > INT_MAX) {
		jonj_gd_error_ex(E_WARNING, "gd-jpeg: warning: JPEG image height (%u) is greater than INT_MAX (%d) (and thus greater than gd can handle)", cinfo.image_height, INT_MAX);
	}

	if (cinfo.image_width > INT_MAX) {
		jonj_gd_error_ex(E_WARNING, "gd-jpeg: warning: JPEG image width (%u) is greater than INT_MAX (%d) (and thus greater than gd can handle)", cinfo.image_width, INT_MAX);
	}

	im = gdImageCreateTrueColor ((int) cinfo.image_width, (int) cinfo.image_height);
	if (im == 0) {
		jonj_gd_error("gd-jpeg error: cannot allocate gdImage struct");
		goto error;
	}
	...
	if (jpeg_start_decompress (&cinfo) != TRUE) {
		jonj_gd_error("gd-jpeg: warning: jpeg_start_decompress reports suspended data source");
	}
  ...
  gdImageInterlace (im, cinfo.progressive_mode != 0);
	...
	if (cinfo.out_color_space == JCS_RGB) {
		if (cinfo.output_components != 3) {
			jonj_gd_error_ex(E_WARNING, "gd-jpeg: error: JPEG color quantization request resulted in output_components == %d (expected 3 for RGB)", cinfo.output_components);
			goto error;
		}
		channels = 3;
		...
		if (cinfo.output_components != 4)  {
			jonj_gd_error_ex(E_WARNING, "gd-jpeg: error: JPEG color quantization request resulted in output_components == %d (expected 4 for CMYK)", cinfo.output_components);
			goto error;
		}
		channels = 4;
		...
		jonj_gd_error_ex(E_WARNING, "gd-jpeg: error: unexpected colorspace.");
		goto error;
	}
	...
	if (cinfo.out_color_space == JCS_CMYK) {
		for (i = 0; i < cinfo.output_height; i++) {
			...
			if (nrows != 1) {
				jonj_gd_error_ex(E_WARNING, "gd-jpeg: error: jpeg_read_scanlines returns %u, expected 1", nrows);
				goto error;
			}
	...
	if (jpeg_finish_decompress (&cinfo) != TRUE) {
		jonj_gd_error("gd-jpeg: warning: jpeg_finish_decompress reports suspended data source");
	}
	if (!ignore_warning) {
