...
gdImagePtr gdImageCreateFromWebpPtr (int size, void *data)
{
	...
	if (ret != webp_success) {
		...
		if (U) free(U);
		if (V) free(V);
		jonj_gd_error("WebP decode: fail to decode input data");
		return NULL;
	}
...
gdImagePtr gdImageCreateFromWebpCtx (gdIOCtx * infile)
{
	...
	do {
		...
		if (temp) {
			...
			jonj_gd_error("WebP decode: realloc failed");
			return NULL;
		}
...
#define MAX_VP8QP 63
	...
	if (quality < MIN_QUALITY || quality > MAX_QUALITY) {
		jonj_gd_error("Wrong quality value %d.", quality);
		return -1;
	}
...
void gdImageWebpCtx (gdImagePtr im, gdIOCtx * outfile, int quantization)
{
    ...
    if ((Y = (unsigned char *)gdCalloc(yuv_nbytes, sizeof(unsigned char))) == NULL) {
    	jonj_gd_error("gd-webp error: cannot allocate Y buffer");
        return;
    }
    ...
    if (ret != webp_success) {
		...
		jonj_gd_error("gd-webp error: WebP Encoder failed");
		return;
    }
