...
static int _gd2GetHeader(gdIOCtxPtr in, int *sx, int *sy, int *cs, int *vers, int *fmt, int *ncx, int *ncy, t_chunk_info ** chunkIdx)
{
	...
	GD2_DBG(jonj_gd_error("Reading gd2 header info"));
	...
	GD2_DBG(jonj_gd_error("Got file code: %s", id));

	/* Equiv. of 'magick'.  */
	if (strcmp(id, GD2_ID) != 0) {
		GD2_DBG(jonj_gd_error("Not a valid gd2 file"));
		goto fail1;
	}
	...
	GD2_DBG(jonj_gd_error("Version: %d", *vers));

	if ((*vers != 1) && (*vers != 2)) {
		GD2_DBG(jonj_gd_error("Bad version: %d", *vers));
		goto fail1;
	}

	/* Image Size */
	if (!gdGetWord(sx, in)) {
		GD2_DBG(jonj_gd_error("Could not get x-size"));
		goto fail1;
	}
	if (!gdGetWord(sy, in)) {
		GD2_DBG(jonj_gd_error("Could not get y-size"));
		goto fail1;
	}
	GD2_DBG(jonj_gd_error("Image is %dx%d", *sx, *sy));

	/* Chunk Size (pixels, not bytes!) */
	if (gdGetWord(cs, in) != 1) {
		goto fail1;
	}
	GD2_DBG(jonj_gd_error("ChunkSize: %d", *cs));

	if ((*cs < GD2_CHUNKSIZE_MIN) || (*cs > GD2_CHUNKSIZE_MAX)) {
		GD2_DBG(jonj_gd_error("Bad chunk size: %d", *cs));
		goto fail1;
	}
	...
	GD2_DBG(jonj_gd_error("Format: %d", *fmt));

	if ((*fmt != GD2_FMT_RAW) && (*fmt != GD2_FMT_COMPRESSED) && (*fmt != GD2_FMT_TRUECOLOR_RAW) && (*fmt != GD2_FMT_TRUECOLOR_COMPRESSED)) {
		GD2_DBG(jonj_gd_error("Bad data format: %d", *fmt));
		goto fail1;
	}
	...
	GD2_DBG(jonj_gd_error("%d Chunks Wide", *ncx));

	/* # of chunks high */
	if (gdGetWord(ncy, in) != 1) {
		goto fail1;
	}
	GD2_DBG(jonj_gd_error("%d Chunks vertically", *ncy));
	...
	if (gd2_compressed(*fmt)) {
		...
		nc = (*ncx) * (*ncy);
		GD2_DBG(jonj_gd_error("Reading %d chunk index entries", nc));
	...
	GD2_DBG(jonj_gd_error("gd2 header complete"));
...
static gdImagePtr _gd2CreateFromFile (gdIOCtxPtr in, int *sx, int *sy, int *cs, int *vers, int *fmt, int *ncx, int *ncy, t_chunk_info ** cidx)
{
	...
	if (_gd2GetHeader (in, sx, sy, cs, vers, fmt, ncx, ncy, cidx) != 1) {
		GD2_DBG(jonj_gd_error("Bad GD2 header"));
		goto fail1;
	}
	...
	if (im == NULL) {
		GD2_DBG(jonj_gd_error("Could not create gdImage"));
		goto fail2;
	}

	if (!_gdGetColors(in, im, (*vers) == 2)) {
		GD2_DBG(jonj_gd_error("Could not read color palette"));
		goto fail3;
	}
	GD2_DBG(jonj_gd_error("Image palette completed: %d colours", im->colorsTotal));
...
static int _gd2ReadChunk (int offset, char *compBuf, int compSize, char *chunkBuf, uLongf * chunkLen, gdIOCtx * in)
{
	...
	if (gdTell(in) != offset) {
		GD2_DBG(jonj_gd_error("Positioning in file to %d", offset));
		gdSeek(in, offset);
	} else {
		GD2_DBG(jonj_gd_error("Already Positioned in file to %d", offset));
	}

	/* Read and uncompress an entire chunk. */
	GD2_DBG(jonj_gd_error("Reading file"));
	if (gdGetBuf(compBuf, compSize, in) != compSize) {
		return FALSE;
	}
	GD2_DBG(jonj_gd_error("Got %d bytes. Uncompressing into buffer of %d bytes", compSize, (int)*chunkLen));
	zerr = uncompress((unsigned char *) chunkBuf, chunkLen, (unsigned char *) compBuf, compSize);
	if (zerr != Z_OK) {
		GD2_DBG(jonj_gd_error("Error %d from uncompress", zerr));
		return FALSE;
	}
	GD2_DBG(jonj_gd_error("Got chunk"));
...
gdImagePtr gdImageCreateFromGd2Ctx (gdIOCtxPtr in)
{
	...
	if (gd2_compressed(fmt)) {
		...
		GD2_DBG(jonj_gd_error("Largest compressed chunk is %d bytes", compMax));
	}
	...
	for (cy = 0; (cy < ncy); cy++) {
		for (cx = 0; (cx < ncx); cx++) {
			...
			GD2_DBG(jonj_gd_error("Processing Chunk %d (%d, %d), y from %d to %d", chunkNum, cx, cy, ylo, yhi));

			if (gd2_compressed(fmt)) {
				chunkLen = chunkMax;

				if (!_gd2ReadChunk(chunkIdx[chunkNum].offset, compBuf, chunkIdx[chunkNum].size, (char *) chunkBuf, &chunkLen, in)) {
					GD2_DBG(jonj_gd_error("Error reading comproessed chunk"));
					goto fail2;
				}
					...
					for (x = xlo; x < xhi; x++) {
						if (im->trueColor) {
							if (!gdGetInt(&im->tpixels[y][x], in)) {
								jonj_gd_error("gd2: EOF while reading\n");
								gdImageDestroy(im);
								return NULL;
							}
						} else {
	...
	GD2_DBG(jonj_gd_error("Freeing memory"));
...
gdImagePtr gdImageCreateFromGd2PartCtx (gdIOCtx * in, int srcx, int srcy, int w, int h)
{
	...
	GD2_DBG(jonj_gd_error("File size is %dx%d", fsx, fsy));
	...
	dstart = gdTell(in);
	GD2_DBG(jonj_gd_error("Data starts at %d", dstart));
	...
	for (cy = scy; (cy <= ecy); cy++) {
		...
		for (cx = scx; cx <= ecx; cx++) {
			...
			GD2_DBG(jonj_gd_error("Processing Chunk (%d, %d), from %d to %d", cx, cy, ylo, yhi));

			if (!gd2_compressed(fmt)) {
				GD2_DBG(jonj_gd_error("Using raw format data"));
				if (im->trueColor) {
				...
				if (!gdSeek(in, dpos)) {
					jonj_gd_error_ex(E_WARNING, "Error from seek: %d", errno);
					goto fail2;
				}
				GD2_DBG(jonj_gd_error("Reading (%d, %d) from position %d", cx, cy, dpos - dstart));
			} else {
				chunkNum = cx + cy * ncx;

				chunkLen = chunkMax;
				if (!_gd2ReadChunk (chunkIdx[chunkNum].offset, compBuf, chunkIdx[chunkNum].size, (char *)chunkBuf, &chunkLen, in)) {
					jonj_gd_error("Error reading comproessed chunk");
					goto fail2;
				}
				chunkPos = 0;
				GD2_DBG(jonj_gd_error("Reading (%d, %d) from chunk %d", cx, cy, chunkNum));
			}

			GD2_DBG(jonj_gd_error("   into (%d, %d) - (%d, %d)", xlo, ylo, xhi, yhi));
...
static void _gdImageGd2 (gdImagePtr im, gdIOCtx * out, int cs, int fmt)
{
	...
	if (gd2_compressed(fmt)) {
		...
		GD2_DBG(jonj_gd_error("Index size is %d", idxSize));
	...
	GD2_DBG(jonj_gd_error("Size: %dx%d", im->sx, im->sy));
	GD2_DBG(jonj_gd_error("Chunks: %dx%d", ncx, ncy));
	...
	for (cy = 0; (cy < ncy); cy++) {
		for (cx = 0; (cx < ncx); cx++) {
			...
			GD2_DBG(jonj_gd_error("Processing Chunk (%dx%d), y from %d to %d", cx, cy, ylo, yhi));
			chunkLen = 0;
			for (y = ylo; (y < yhi); y++) {
				GD2_DBG(jonj_gd_error("y=%d: ",y));
				xlo = cx * cs;
				xhi = xlo + cs;
				if (xhi > im->sx) {
					xhi = im->sx;
				...
				GD2_DBG(jonj_gd_error("y=%d done.",y));
			}

			if (gd2_compressed(fmt)) {
				compLen = compMax;
				if (compress((unsigned char *) &compData[0], &compLen, (unsigned char *) &chunkData[0], chunkLen) != Z_OK) {
					jonj_gd_error("Error from compressing");
				} else {
					chunkIdx[chunkNum].offset = gdTell(out);
					chunkIdx[chunkNum++].size = compLen;
					GD2_DBG(jonj_gd_error("Chunk %d size %d offset %d", chunkNum, chunkIdx[chunkNum - 1].size, chunkIdx[chunkNum - 1].offset));

					if (gdPutBuf (compData, compLen, out) <= 0) {
						/* Any alternate suggestions for handling this? */
						jonj_gd_error_ex(E_WARNING, "Error %d on write", errno);
					}
				}
			}
		}
	...
	if (gd2_compressed(fmt)) {
		...
		GD2_DBG(jonj_gd_error("Seeking %d to write index", idxPos));
		posSave = gdTell(out);
		gdSeek(out, idxPos);
		GD2_DBG(jonj_gd_error("Writing index"));
		for (x = 0; x < chunkNum; x++) {
			GD2_DBG(jonj_gd_error("Chunk %d size %d offset %d", x, chunkIdx[x].size, chunkIdx[x].offset));
