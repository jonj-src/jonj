...
#define SPL_DEFAULT_FILE_EXTENSIONS ".inc,.jonj"

/* {{{ JONJ_GINIT_FUNCTION
 */
static JONJ_GINIT_FUNCTION(spl)
{
...
static zend_class_entry * spl_find_ce_by_name(char *name, int len, zend_bool autoload TSRMLS_DC)
{
	...
	if (!autoload) {
 	...
 	if (found != SUCCESS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Class %s does not exist%s", name, autoload ? " and could not be loaded" : "");
		return NULL;
	}
...
JONJ_FUNCTION(class_parents)
{
	...
	if (Z_TYPE_P(obj) != IS_OBJECT && Z_TYPE_P(obj) != IS_STRING) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "object or string expected");
		RETURN_FALSE;
	}
...
static int spl_autoload(const char *class_name, const char * lc_name, int class_name_len, const char * file_extension TSRMLS_DC) /* {{{ */
{
	...
	ret = jonj_stream_open_for_zend_ex(class_file, &file_handle, USE_PATH|STREAM_OPEN_FOR_INCLUDE TSRMLS_CC);
...
JONJ_FUNCTION(spl_autoload)
{
	...
	if (!found && !SPL_G(autoload_running)) {
		...
		if (EG(opline_ptr) && active_opline->opcode != ZEND_FETCH_CLASS) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Class %s could not be loaded", class_name);
		}
	}
} /* }}} */
...
JONJ_FUNCTION(spl_autoload_extensions)
{
	char *file_exts = NULL;
	int file_exts_len;
...
JONJ_FUNCTION(spl_autoload_call)
{
...
JONJ_FUNCTION(spl_autoload_register)
{
...
JONJ_FUNCTION(spl_autoload_unregister)
{
	char *func_name, *error = NULL;
	int func_name_len;
	char *lc_name = NULL;
...
JONJ_FUNCTION(spl_autoload_functions)
{
	zend_function *fptr;
	HashPosition function_pos;
	autoload_func_info *alfi;
...
JONJ_FUNCTION(spl_object_hash)
{
	...
	hash = emalloc(33);
	jonj_spl_object_hash(obj, hash TSRMLS_CC);
	
	RETVAL_STRING(hash, 0);
}
/* }}} */

JONJAPI void jonj_spl_object_hash(zval *obj, char *result TSRMLS_DC) /* {{{*/
{
	...
	if (!SPL_G(hash_mask_init)) {
		if (!BG(mt_rand_is_seeded)) {
			jonj_mt_srand(GENERATE_SEED() TSRMLS_CC);
		}

		SPL_G(hash_mask_handle)   = (intptr_t)(jonj_mt_rand(TSRMLS_C) >> 1);
		SPL_G(hash_mask_handlers) = (intptr_t)(jonj_mt_rand(TSRMLS_C) >> 1);
...
JONJ_MINFO_FUNCTION(spl)
{
	...
	jonj_info_print_table_header(2, "SPL support",        "enabled");
	...
	zval_dtor(&list);
	jonj_info_print_table_row(2, "Interfaces", strg + 2);
	efree(strg);
	...
	zend_hash_apply_with_argument(Z_ARRVAL_P(&list), (apply_func_arg_t)spl_build_class_list_string, &strg TSRMLS_CC);
	zval_dtor(&list);
	jonj_info_print_table_row(2, "Classes", strg + 2);
	efree(strg);
...
const zend_function_entry spl_functions[] = {
	JONJ_FE(spl_classes,             arginfo_spl_classes)
	JONJ_FE(spl_autoload,            arginfo_spl_autoload)
	JONJ_FE(spl_autoload_extensions, arginfo_spl_autoload_extensions)
	JONJ_FE(spl_autoload_register,   arginfo_spl_autoload_register)
	JONJ_FE(spl_autoload_unregister, arginfo_spl_autoload_unregister)
	JONJ_FE(spl_autoload_functions,  arginfo_spl_autoload_functions)
	JONJ_FE(spl_autoload_call,       arginfo_spl_autoload_call)
	JONJ_FE(class_parents,           arginfo_class_parents)
	JONJ_FE(class_implements,        arginfo_class_implements)
	JONJ_FE(class_uses,              arginfo_class_uses)
	JONJ_FE(spl_object_hash,         arginfo_spl_object_hash)
#ifdef SPL_ITERATORS_H
	JONJ_FE(iterator_to_array,       arginfo_iterator_to_array)
	JONJ_FE(iterator_count,          arginfo_iterator)
	JONJ_FE(iterator_apply,          arginfo_iterator_apply)
...
JONJ_MINIT_FUNCTION(spl)
{
	JONJ_MINIT(spl_exceptions)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_iterators)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_array)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_directory)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_dllist)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_heap)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_fixedarray)(INIT_FUNC_ARGS_PASSTHRU);
	JONJ_MINIT(spl_observer)(INIT_FUNC_ARGS_PASSTHRU);
