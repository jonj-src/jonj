...
JONJAPI zend_class_entry *spl_ce_SplFileInfo;
JONJAPI zend_class_entry *spl_ce_DirectoryIterator;
JONJAPI zend_class_entry *spl_ce_FilesystemIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveDirectoryIterator;
JONJAPI zend_class_entry *spl_ce_GlobIterator;
JONJAPI zend_class_entry *spl_ce_SplFileObject;
JONJAPI zend_class_entry *spl_ce_SplTempFileObject;
...
static void spl_filesystem_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	...
	case SPL_FS_DIR:
		if (intern->u.dir.dirp) {
			jonj_stream_close(intern->u.dir.dirp);
...
/*				zend_list_delref(Z_RESVAL_P(intern->zcontext));*/
			...
			if (!intern->u.file.stream->is_persistent) {
				jonj_stream_free(intern->u.file.stream, JONJ_STREAM_FREE_CLOSE);
			} else {
				jonj_stream_free(intern->u.file.stream, JONJ_STREAM_FREE_CLOSE_PERSISTENT);
			}
...
JONJAPI char* spl_filesystem_object_get_path(spl_filesystem_object *intern, int *len TSRMLS_DC) /* {{{ */
{
#ifdef HAVE_GLOB
	if (intern->type == SPL_FS_DIR) {
		if (jonj_stream_is(intern->u.dir.dirp ,&jonj_glob_stream_ops)) {
			return jonj_glob_stream_get_path(intern->u.dir.dirp, 0, len);
		}
	}
#endif
	if (len) {
...
static inline void spl_filesystem_object_get_file_name(spl_filesystem_object *intern TSRMLS_DC) /* {{{ */
{
	...
	switch (intern->type) {
		...
		case SPL_FS_FILE:
			if (!intern->file_name) {
				jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Object not initialized");
			}
			break;
		case SPL_FS_DIR:
			if (intern->file_name) {
...
{
	if (!intern->u.dir.dirp || !jonj_stream_readdir(intern->u.dir.dirp, &intern->u.dir.entry)) {
...
static void spl_filesystem_dir_open(spl_filesystem_object* intern, char *path TSRMLS_DC)
{
	...
	intern->u.dir.dirp = jonj_stream_opendir(path, REPORT_ERRORS, FG(default_context));
...
static int spl_filesystem_file_open(spl_filesystem_object *intern, int use_include_path, int silent TSRMLS_DC) /* {{{ */
{
	...
	jonj_stat(intern->file_name, intern->file_name_len, FS_IS_DIR, &tmp TSRMLS_CC);
	if (Z_LVAL(tmp)) {
	...
	intern->u.file.context = jonj_stream_context_from_zval(intern->u.file.zcontext, 0);
	intern->u.file.stream = jonj_stream_open_wrapper_ex(intern->file_name, intern->u.file.open_mode, (use_include_path ? USE_PATH : 0) | REPORT_ERRORS, NULL, intern->u.file.context);
	...
	ZVAL_RESOURCE(&intern->u.file.zresource, jonj_stream_get_resource_id(intern->u.file.stream));
...
static zend_object_value spl_filesystem_object_clone(zval *zobject TSRMLS_DC)
{
	...
	case SPL_FS_DIR:
		...
		break;
	case SPL_FS_FILE:
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "An object of class %s cannot be cloned", old_object->ce->name);
		break;
	}
...
void spl_filesystem_object_construct(INTERNAL_FUNCTION_PARAMETERS, long ctor_flags) /* {{{ */
{
	...
	if (intern->_path) {
		...
		zend_restore_error_handling(&error_handling TSRMLS_CC);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Directory object is already initialized");
		return;
	}
	intern->flags = flags;
#ifdef HAVE_GLOB
...
SPL_METHOD(DirectoryIterator, rewind)
{
	...
	intern->u.dir.index = 0;
	if (intern->u.dir.dirp) {
		jonj_stream_rewinddir(intern->u.dir.dirp);
	}
	spl_filesystem_dir_read(intern TSRMLS_CC);
}
/* }}} */
...
SPL_METHOD(SplFileInfo, getExtension)
{
	...
	jonj_basename(fname, flen, NULL, 0, &fname, &flen TSRMLS_CC);
...
SPL_METHOD(DirectoryIterator, getExtension)
{
	...
	jonj_basename(intern->u.dir.entry.d_name, strlen(intern->u.dir.entry.d_name), NULL, 0, &fname, &flen TSRMLS_CC);
...
SPL_METHOD(SplFileInfo, getBasename)
{
	...
	jonj_basename(fname, flen, suffix, slen, &fname, &flen TSRMLS_CC);
...
SPL_METHOD(DirectoryIterator, getBasename)
{
	...
	jonj_basename(intern->u.dir.entry.d_name, strlen(intern->u.dir.entry.d_name), suffix, slen, &fname, &flen TSRMLS_CC);
...
{ \
 ...
 \
	zend_replace_error_handling(EH_THROW, spl_ce_RuntimeException, &error_handling TSRMLS_CC);\
	spl_filesystem_object_get_file_name(intern TSRMLS_CC); \
	jonj_stat(intern->file_name, intern->file_name_len, func_num, return_value TSRMLS_CC); \
	zend_restore_error_handling(&error_handling TSRMLS_CC); \
}
/* }}} */
...
#if defined(JONJ_WIN32) || HAVE_SYMLINK
	if (intern->file_name == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty filename");
		RETURN_FALSE;
		...
		if (!expand_filepath_with_mode(intern->file_name, expanded_path, NULL, 0, CWD_EXPAND  TSRMLS_CC)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No such file or directory");
			RETURN_FALSE;
		}
		ret = jonj_sys_readlink(expanded_path, buff, MAXPATHLEN - 1);
	} else {
		ret = jonj_sys_readlink(intern->file_name, buff,  MAXPATHLEN-1);
	}
#else
...
SPL_METHOD(SplFileInfo, getPathInfo)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|C", &ce) == SUCCESS) {
		...
		if (path) {
			...
			path_len = jonj_dirname(dpath, path_len);
...
SPL_METHOD(RecursiveDirectoryIterator, hasChildren)
{
	...
	if (spl_filesystem_is_invalid_or_dot(intern->u.dir.entry.d_name)) {
		...
		if (!allow_links && !(intern->flags & SPL_FILE_DIR_FOLLOW_SYMLINKS)) {
			jonj_stat(intern->file_name, intern->file_name_len, FS_IS_LINK, return_value TSRMLS_CC);
			if (zend_is_true(return_value)) {
				RETURN_FALSE;
			}
		}
		jonj_stat(intern->file_name, intern->file_name_len, FS_IS_DIR, return_value TSRMLS_CC);
    }
}
/* }}} */
...
SPL_METHOD(GlobIterator, count)
{
	...
	if (intern->u.dir.dirp && jonj_stream_is(intern->u.dir.dirp ,&jonj_glob_stream_ops)) {
		RETURN_LONG(jonj_glob_stream_get_count(intern->u.dir.dirp, NULL));
	} else {
		/* should not happen */
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "GlobIterator lost glob state");
	}
}
/* }}} */
#endif /* HAVE_GLOB */
...
static void spl_filesystem_dir_it_rewind(zend_object_iterator *iter TSRMLS_DC)
{
	...
	if (object->u.dir.dirp) {
		jonj_stream_rewinddir(object->u.dir.dirp);
	}
	spl_filesystem_dir_read(object TSRMLS_CC);
}
/* }}} */
...
static int spl_filesystem_file_read(spl_filesystem_object *intern, int silent TSRMLS_DC) /* {{{ */
{
	...
	if (jonj_stream_eof(intern->u.file.stream)) {
		if (!silent) {
	...
	if (intern->u.file.max_line_len > 0) {
		...
		if (jonj_stream_get_line(intern->u.file.stream, buf, intern->u.file.max_line_len + 1, &line_len) == NULL) {
			efree(buf);
			buf = NULL;
		} else {
			buf[line_len] = '\0';
		...
		buf = jonj_stream_get_line(intern->u.file.stream, NULL, 0, &line_len);
	}
...
static int spl_filesystem_file_read_csv(spl_filesystem_object *intern, char delimiter, char enclosure, char escape, zval *return_value TSRMLS_DC) /* {{{ */
{
	...
	if (ret == SUCCESS) {
		...
		jonj_fgetcsv(intern->u.file.stream, delimiter, enclosure, escape, buf_len, buf, intern->u.file.current_zval TSRMLS_CC);
		if (return_value) {
...
static void spl_filesystem_file_rewind(zval * this_ptr, spl_filesystem_object *intern TSRMLS_DC) /* {{{ */
{
	...
	if (-1 == jonj_stream_rewind(intern->u.file.stream)) {
...
{
	long max_memory = JONJ_STREAM_MAX_MEM;
	char tmp_fname[48];
	spl_filesystem_object *intern = (spl_filesystem_object*)zend_object_store_get_object(getThis() TSRMLS_CC);
	zend_error_handling error_handling;
	...
	if (max_memory < 0) {
		intern->file_name = "jonj://memory";
		intern->file_name_len = 12;
	} else if (ZEND_NUM_ARGS()) {
		intern->file_name_len = slprintf(tmp_fname, sizeof(tmp_fname), "jonj://temp/maxmemory:%ld", max_memory);
		intern->file_name = tmp_fname;
	} else {
		intern->file_name = "jonj://temp";
...
SPL_METHOD(SplFileObject, eof)
{
	...
	RETURN_BOOL(jonj_stream_eof(intern->u.file.stream));
} /* }}} */
...
SPL_METHOD(SplFileObject, valid)
{
	...
	if (SPL_HAS_FLAG(intern->flags, SPL_FILE_OBJECT_READ_AHEAD)) {
		...
		RETVAL_BOOL(!jonj_stream_eof(intern->u.file.stream));
	}
} /* }}} */
...
SPL_METHOD(SplFileObject, fputcsv)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a|sss", &fields, &delim, &d_len, &enclo, &e_len, &esc, &esc_len) == SUCCESS) {
		...
		ret = jonj_fputcsv(intern->u.file.stream, fields, delimiter, enclosure, escape TSRMLS_CC);
		RETURN_LONG(ret);
	}
}
/* }}} */
...
SPL_METHOD(SplFileObject, fflush)
{
	...
	RETURN_BOOL(!jonj_stream_flush(intern->u.file.stream));
} /* }}} */
...
SPL_METHOD(SplFileObject, ftell)
{
	...
	ret = jonj_stream_tell(intern->u.file.stream);
...
SPL_METHOD(SplFileObject, fseek)
{
	...
	spl_filesystem_file_free_line(intern TSRMLS_CC);
	RETURN_LONG(jonj_stream_seek(intern->u.file.stream, pos, whence));
} /* }}} */
...
SPL_METHOD(SplFileObject, fgetc)
{
	...
	result = jonj_stream_getc(intern->u.file.stream);
...
SPL_METHOD(SplFileObject, fpassthru)
{
	...
	RETURN_LONG(jonj_stream_passthru(intern->u.file.stream));
} /* }}} */
...
SPL_METHOD(SplFileObject, fwrite)
{
	...
	RETURN_LONG(jonj_stream_write(intern->u.file.stream, str, str_len));
} /* }}} */
...
SPL_METHOD(SplFileObject, fread)
{
	...
	Z_STRLEN_P(return_value) = jonj_stream_read(intern->u.file.stream, Z_STRVAL_P(return_value), length);
...
SPL_METHOD(SplFileObject, ftruncate)
{
	...
	if (!jonj_stream_truncate_supported(intern->u.file.stream)) {
		zend_throw_exception_ex(spl_ce_LogicException, 0 TSRMLS_CC, "Can't truncate file %s", intern->file_name);
		RETURN_FALSE;
	}

	RETURN_BOOL(0 == jonj_stream_truncate_set_size(intern->u.file.stream, size));
} /* }}} */
...
/* {{{ JONJ_MINIT_FUNCTION(spl_directory)
 */
JONJ_MINIT_FUNCTION(spl_directory)
{
