...
JONJAPI zend_class_entry *spl_ce_RecursiveIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveIteratorIterator;
JONJAPI zend_class_entry *spl_ce_FilterIterator;
JONJAPI zend_class_entry *spl_ce_CallbackFilterIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveFilterIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveCallbackFilterIterator;
JONJAPI zend_class_entry *spl_ce_ParentIterator;
JONJAPI zend_class_entry *spl_ce_SeekableIterator;
JONJAPI zend_class_entry *spl_ce_LimitIterator;
JONJAPI zend_class_entry *spl_ce_CachingIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveCachingIterator;
JONJAPI zend_class_entry *spl_ce_OuterIterator;
JONJAPI zend_class_entry *spl_ce_IteratorIterator;
JONJAPI zend_class_entry *spl_ce_NoRewindIterator;
JONJAPI zend_class_entry *spl_ce_InfiniteIterator;
JONJAPI zend_class_entry *spl_ce_EmptyIterator;
JONJAPI zend_class_entry *spl_ce_AppendIterator;
JONJAPI zend_class_entry *spl_ce_RegexIterator;
JONJAPI zend_class_entry *spl_ce_RecursiveRegexIterator;
JONJAPI zend_class_entry *spl_ce_Countable;
JONJAPI zend_class_entry *spl_ce_RecursiveTreeIterator;
...
static union _zend_function *spl_recursive_it_get_method(zval **object_ptr, char *method, int method_len, const zend_literal *key TSRMLS_DC)
{
	...
	if (!object->iterators) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "The %s instance wasn't initialized properly", Z_OBJCE_PP(object_ptr)->name);
	}
...
int spl_dual_it_call_method(char *method, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (!zend_is_callable(&func, 0, &method TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Method %s::%s() does not exist", intern->inner.ce->name, method);
		return FAILURE;
	}
	...
	if (call_user_function_ex(EG(function_table), NULL, &func, &retval_ptr, arg_count, func_params, 0, NULL TSRMLS_CC) == SUCCESS && retval_ptr) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Unable to call %s::%s()", intern->inner.ce->name, method);
		success = FAILURE;
	}
...
static inline void spl_dual_it_require(spl_dual_it_object *intern TSRMLS_DC)
{
	if (!intern->inner.iterator) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "The inner constructor wasn't initialized with an iterator instance");
	}
}
...
SPL_METHOD(RegexIterator, accept)
{
	...
	case REGIT_MODE_GET_MATCH:
		...
		jonj_pcre_match_impl(intern->u.regex.pce, subject, subject_len, &zcount,
	...
	case REGIT_MODE_SPLIT:
		...
		zval_ptr_dtor(&intern->current.data);
		ALLOC_INIT_ZVAL(intern->current.data);
		jonj_pcre_split_impl(intern->u.regex.pce, subject, subject_len, intern->current.data, -1, intern->u.regex.preg_flags TSRMLS_CC);
	...
	case REGIT_MODE_REPLACE:
		...
		result = jonj_pcre_replace_impl(intern->u.regex.pce, subject, subject_len, replacement, 0, &result_len, -1, &count TSRMLS_CC);
...
JONJAPI int spl_iterator_apply(zval *obj, spl_iterator_apply_func_t apply_func, void *puser TSRMLS_DC)
{
...
JONJ_FUNCTION(iterator_to_array)
{
	zval  *obj;
...
/* {{{ JONJ_MINIT_FUNCTION(spl_iterators)
 */
JONJ_MINIT_FUNCTION(spl_iterators)
{
