...
JONJAPI zend_class_entry     *spl_ce_SplObserver;
JONJAPI zend_class_entry     *spl_ce_SplSubject;
JONJAPI zend_class_entry     *spl_ce_SplObjectStorage;
JONJAPI zend_class_entry     *spl_ce_MultipleIterator;

JONJAPI zend_object_handlers spl_handler_SplObjectStorage;
	...
	*is_temp = 0;
	...
	if (intern->debug_info->nApplyCount == 0) {
		...
		while (zend_hash_get_current_data_ex(&intern->storage, (void **)&element, &pos) == SUCCESS) {
				jonj_spl_object_hash(element->obj, md5str TSRMLS_CC);
				MAKE_STD_ZVAL(tmp);
				array_init(tmp);
...
SPL_METHOD(SplObjectStorage, count)
{
	...
	if (mode == COUNT_RECURSIVE) {
		...
		for (zend_hash_internal_pointer_reset_ex(&intern->storage, &position);
			...
			ret += jonj_count_recursive(element, mode TSRMLS_CC);
		}
...
SPL_METHOD(SplObjectStorage, serialize)
{
	...
	while(zend_hash_has_more_elements_ex(&intern->storage, &pos) == SUCCESS) {
		...
		jonj_var_serialize(&buf, &element->obj, &var_hash TSRMLS_CC);
		smart_str_appendc(&buf, ',');
		jonj_var_serialize(&buf, &element->inf, &var_hash TSRMLS_CC);
		smart_str_appendc(&buf, ';');
		zend_hash_move_forward_ex(&intern->storage, &pos);
	}
...
SPL_METHOD(SplObjectStorage, unserialize)
{
	...
	if (!jonj_var_unserialize(&pcount, &p, s + buf_len, &var_hash TSRMLS_CC) || Z_TYPE_P(pcount) != IS_LONG) {
		goto outexcept;
	}
	...
	while(count-- > 0) {
		...
		if (!jonj_var_unserialize(&pentry, &p, s + buf_len, &var_hash TSRMLS_CC)) {
		...
		if (*p == ',') { /* new version has inf */
			++p;
			if (!jonj_var_unserialize(&pinf, &p, s + buf_len, &var_hash TSRMLS_CC)) {
				zval_ptr_dtor(&pinf);
				goto outexcept;
			}
