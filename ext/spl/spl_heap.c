...
JONJAPI zend_class_entry  *spl_ce_SplHeap;
JONJAPI zend_class_entry  *spl_ce_SplMaxHeap;
JONJAPI zend_class_entry  *spl_ce_SplMinHeap;
JONJAPI zend_class_entry  *spl_ce_SplPriorityQueue;
...
static zend_object_value spl_heap_object_new_ex(zend_class_entry *class_type, spl_heap_object **obj, zval *orig, int clone_orig TSRMLS_DC) /* {{{ */
{
	...
	*obj = intern;
	...
	if (!parent) { /* this must never happen */
		jonj_error_docref(NULL TSRMLS_CC, E_COMPILE_ERROR, "Internal compiler error, Class is not child of SplHeap");
	}
