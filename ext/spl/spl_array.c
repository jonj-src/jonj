...
JONJAPI zend_class_entry  *spl_ce_ArrayObject;

zend_object_handlers spl_handler_ArrayIterator;
JONJAPI zend_class_entry  *spl_ce_ArrayIterator;
JONJAPI zend_class_entry  *spl_ce_RecursiveArrayIterator;
...
static zend_object_value spl_array_object_new_ex(zend_class_entry *class_type, spl_array_object **obj, zval *orig, int clone_orig TSRMLS_DC)
{
	...
	*obj = intern;
	...
	if (!parent) { /* this must never happen */
		jonj_error_docref(NULL TSRMLS_CC, E_COMPILE_ERROR, "Internal compiler error, Class is not child of ArrayObject or ArrayIterator");
	}
	if (inherited) {
...
{
	if (!ht) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "%sArray was modified outside object and is no longer an array", msg_prefix);
		return FAILURE;
	}

	if (object->pos && (object->ar_flags & SPL_ARRAY_IS_REF) && spl_hash_verify_pos_ex(object, ht TSRMLS_CC) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "%sArray was modified outside object and internal position is no longer valid", msg_prefix);
		return FAILURE;
	}
...
void spl_array_iterator_append(zval *object, zval *append_value TSRMLS_DC) /* {{{ */
{
	...
	if (!aht) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Array was modified outside object and is no longer an array");
		return;
	}

	if (Z_TYPE_P(intern->array) == IS_OBJECT) {
		jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Cannot append properties to objects, use %s::offsetSet() instead", Z_OBJCE_P(object)->name);
		return;
	}
...
static HashTable *spl_array_get_properties(zval *object TSRMLS_DC) /* {{{ */
{
	...
	if (intern->nApplyCount > 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Nesting level too deep - recursive dependency?");
	}
...
static int spl_array_next_ex(spl_array_object *intern, HashTable *aht TSRMLS_DC) /* {{{ */
{
	if ((intern->ar_flags & SPL_ARRAY_IS_REF) && spl_hash_verify_pos_ex(intern, aht TSRMLS_CC) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Array was modified outside object and internal position is no longer valid");
		return FAILURE;
	}
...
static void spl_array_it_move_forward(zend_object_iterator *iter TSRMLS_DC) /* {{{ */
{
	...
	if (object->ar_flags & SPL_ARRAY_OVERLOADED_NEXT) {
		...
		zend_user_it_invalidate_current(iter TSRMLS_CC);
		if (!aht) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "ArrayIterator::current(): Array was modified outside object and is no longer an array");
			return;
		}

		if ((object->ar_flags & SPL_ARRAY_IS_REF) && spl_hash_verify_pos_ex(object, aht TSRMLS_CC) == FAILURE) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "ArrayIterator::next(): Array was modified outside object and internal position is no longer valid");
		} else {
...
static void spl_array_rewind(spl_array_object *intern TSRMLS_DC) /* {{{ */
{
	...
	if (!aht) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "ArrayIterator::rewind(): Array was modified outside object and is no longer an array");
		return;
	}
...
SPL_METHOD(Array, serialize)
{
	...
	jonj_var_serialize(&buf, &flags, &var_hash TSRMLS_CC);
	zval_ptr_dtor(&flags);

	if (!(intern->ar_flags & SPL_ARRAY_IS_SELF)) {
		jonj_var_serialize(&buf, &intern->array, &var_hash TSRMLS_CC);
	...
	pmembers = &members;
	jonj_var_serialize(&buf, &pmembers, &var_hash TSRMLS_CC); /* finishes the string */
...
SPL_METHOD(Array, unserialize)
{
	...
	if (!jonj_var_unserialize(&pflags, &p, s + buf_len, &var_hash TSRMLS_CC) || Z_TYPE_P(pflags) != IS_LONG) {
		goto outexcept;
	}
	...
	if (*p!='m') {
		...
		if (!jonj_var_unserialize(&intern->array, &p, s + buf_len, &var_hash TSRMLS_CC)
	...
	if (!jonj_var_unserialize(&pmembers, &p, s + buf_len, &var_hash TSRMLS_CC) || Z_TYPE_P(pmembers) != IS_ARRAY) {
