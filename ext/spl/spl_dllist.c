...
JONJAPI zend_class_entry  *spl_ce_SplDoublyLinkedList;
JONJAPI zend_class_entry  *spl_ce_SplQueue;
JONJAPI zend_class_entry  *spl_ce_SplStack;
...
static zend_object_value spl_dllist_object_new_ex(zend_class_entry *class_type, spl_dllist_object **obj, zval *orig, int clone_orig TSRMLS_DC) /* {{{ */
{
	...
	*obj = intern;
	...
	if (!parent) { /* this must never happen */
		jonj_error_docref(NULL TSRMLS_CC, E_COMPILE_ERROR, "Internal compiler error, Class is not child of SplDoublyLinkedList");
	}
	if (inherited) {
...
SPL_METHOD(SplDoublyLinkedList, serialize)
{
	...
	while (current) {
		...
		jonj_var_serialize(&buf, (zval **)&current->data, &var_hash TSRMLS_CC);
...
SPL_METHOD(SplDoublyLinkedList, unserialize)
{
	...
	if (!jonj_var_unserialize(&flags, &p, s + buf_len, &var_hash TSRMLS_CC) || Z_TYPE_P(flags) != IS_LONG) {
		zval_ptr_dtor(&flags);
		goto error;
	}
	...
	while(*p == ':') {
		...
		if (!jonj_var_unserialize(&elem, &p, s + buf_len, &var_hash TSRMLS_CC)) {
			zval_ptr_dtor(&elem);
			goto error;
		}
...
JONJ_MINIT_FUNCTION(spl_dllist) /* {{{ */
{
