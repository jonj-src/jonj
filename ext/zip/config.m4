...
JONJ_ARG_ENABLE(zip, for zip archive read/writesupport,
...
JONJ_ARG_WITH(libzip, libzip,
[  --with-libzip[=DIR]       ZIP: use libzip], no, no)

if test "$JONJ_ZIP" != "no"; then
  ...
  if test "$JONJ_ZLIB_DIR" != "no" && test "$JONJ_ZLIB_DIR" != "yes"; then
    if test -f "$JONJ_ZLIB_DIR/include/zlib/zlib.h"; then
      ...
      AC_MSG_ERROR([Can not find zlib headers under "$JONJ_ZLIB_DIR"])
    fi
  else
  ...
  if test "$JONJ_ZLIB_DIR" = "no"; then
    ...
    AC_MSG_RESULT([$JONJ_ZLIB_DIR])
    JONJ_ADD_LIBRARY_WITH_PATH(z, $JONJ_ZLIB_DIR/$JONJ_LIBDIR, ZIP_SHARED_LIBADD)
    JONJ_ADD_INCLUDE($JONJ_ZLIB_INCDIR)
  fi

  if test "$JONJ_LIBZIP" != "no"; then

    AC_PATH_PROG(PKG_CONFIG, pkg-config, no)

    dnl system libzip, depends on libzip
    AC_MSG_CHECKING(for libzip)
    if test -r $JONJ_LIBZIP/include/zip.h; then
      LIBZIP_CFLAGS="-I$JONJ_LIBZIP/include"
      LIBZIP_LIBDIR="$JONJ_LIBZIP/$JONJ_LIBDIR"
      AC_MSG_RESULT(from option: found in $JONJ_LIBZIP)
    ...
    JONJ_CHECK_LIBRARY(zip, zip_open,
    [
      JONJ_ADD_LIBRARY_WITH_PATH(zip, $LIBZIP_LIBDIR, ZIP_SHARED_LIBADD)
    ...
    JONJ_NEW_EXTENSION(zip, jonj_zip.c zip_stream.c, $ext_shared,, $LIBZIP_CFLAGS)
    JONJ_SUBST(ZIP_SHARED_LIBADD)
  else


  JONJ_ZIP_SOURCES="$JONJ_ZIP_SOURCES lib/zip_add.c lib/zip_add_dir.c lib/zip_add_entry.c\
  ...
  AC_DEFINE(HAVE_ZIP,1,[ ])
  JONJ_NEW_EXTENSION(zip, jonj_zip.c zip_stream.c $JONJ_ZIP_SOURCES, $ext_shared)
