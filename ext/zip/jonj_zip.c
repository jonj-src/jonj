...
/* zip_open is a macro for renaming libzip zipopen, so we need to use JONJ_NAMED_FUNCTION */
static JONJ_NAMED_FUNCTION(zif_zip_open);
static JONJ_NAMED_FUNCTION(zif_zip_read);
static JONJ_NAMED_FUNCTION(zif_zip_close);
static JONJ_NAMED_FUNCTION(zif_zip_entry_read);
static JONJ_NAMED_FUNCTION(zif_zip_entry_filesize);
static JONJ_NAMED_FUNCTION(zif_zip_entry_name);
static JONJ_NAMED_FUNCTION(zif_zip_entry_compressedsize);
static JONJ_NAMED_FUNCTION(zif_zip_entry_compressionmethod);
static JONJ_NAMED_FUNCTION(zif_zip_entry_open);
static JONJ_NAMED_FUNCTION(zif_zip_entry_close);
...
/* {{{ JONJ_ZIP_STAT_INDEX(za, index, flags, sb) */
#define JONJ_ZIP_STAT_INDEX(za, index, flags, sb) \
	if (zip_stat_index(za, index, flags, &sb) != 0) { \
		RETURN_FALSE; \
	}
/* }}} */

/* {{{  JONJ_ZIP_STAT_PATH(za, path, path_len, flags, sb) */
#define JONJ_ZIP_STAT_PATH(za, path, path_len, flags, sb) \
	if (path_len < 1) { \
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string as entry name"); \
		RETURN_FALSE; \
	} \
...
/* {{{ JONJ_ZIP_SET_FILE_COMMENT(za, index, comment, comment_len) */
#define JONJ_ZIP_SET_FILE_COMMENT(za, index, comment, comment_len) \
	if (comment_len == 0) { \
...
static char * jonj_zip_make_relative_path(char *path, int path_len) /* {{{ */
{
...
/* {{{ jonj_zip_extract_file */
static int jonj_zip_extract_file(struct zip * za, char *dest, char *file, int file_len TSRMLS_DC)
{
	...
	path_cleaned =  jonj_zip_make_relative_path(new_state.cwd, new_state.cwd_length);
	if(!path_cleaned) {
		return 0;
	}
	...
	if (path_cleaned_len > 1 && IS_SLASH(path_cleaned[path_cleaned_len - 1])) {
		...
		dir_len = jonj_dirname(file_dirname, path_cleaned_len);
		...
		jonj_basename(path_cleaned, path_cleaned_len, NULL, 0, &file_basename, (size_t *)&file_basename_len TSRMLS_CC);
	...
	if (jonj_stream_stat_path_ex(file_dirname_fullpath, JONJ_STREAM_URL_STAT_QUIET, &ssb, NULL) < 0) {

#if defined(JONJ_WIN32) && (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 1)
		char *e;
		...
		ret = jonj_stream_mkdir(file_dirname_fullpath, 0777,  JONJ_STREAM_MKDIR_RECURSIVE|REPORT_ERRORS, NULL);
		if (!ret) {
			efree(file_dirname_fullpath);
			if (!is_dir_only) {
				efree(file_basename);
	...
	if (!len) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Full extraction path exceed MAXPATHLEN (%i)", MAXPATHLEN);
		efree(file_dirname_fullpath);
		efree(file_basename);
		CWD_STATE_FREE(new_state.cwd);
		return 0;
	...
	stream = jonj_stream_open_wrapper(fullpath, "w+b", REPORT_ERRORS|ENFORCE_SAFE_MODE, NULL);
#else
	stream = jonj_stream_open_wrapper(fullpath, "w+b", REPORT_ERRORS, NULL);
#endif
...
static int jonj_zip_add_file(struct zip *za, const char *filename, size_t filename_len,
	char *entry_name, size_t entry_name_len, long offset_start, long offset_len TSRMLS_DC) /* {{{ */
{
	...
	jonj_stat(resolved_path, strlen(resolved_path), FS_EXISTS, &exists_flag TSRMLS_CC);
...
static int jonj_zip_parse_options(zval *options, long *remove_all_path,
	char **remove_path, int *remove_path_len, char **add_path, int *add_path_len TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_find(HASH_OF(options), "remove_path", sizeof("remove_path"), (void **)&option) == SUCCESS) {
		if (Z_TYPE_PP(option) != IS_STRING) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "remove_path option expected to be a string");
			return -1;
		}

		if (Z_STRLEN_PP(option) < 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string given as remove_path option");
			return -1;
		}

		if (Z_STRLEN_PP(option) >= MAXPATHLEN) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "remove_path string is too long (max: %i, %i given)",
	...
	if (zend_hash_find(HASH_OF(options), "add_path", sizeof("add_path"), (void **)&option) == SUCCESS) {
		if (Z_TYPE_PP(option) != IS_STRING) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "add_path option expected to be a string");
			return -1;
		}

		if (Z_STRLEN_PP(option) < 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string given as the add_path option");
			return -1;
		}

		if (Z_STRLEN_PP(option) >= MAXPATHLEN) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "add_path string too long (max: %i, %i given)",
						MAXPATHLEN - 1, Z_STRLEN_PP(option));
			return -1;
		}
...
#define ZIP_FROM_OBJECT(intern, object) \
	{ \
		...
		intern = obj->za; \
		if (!intern) { \
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid or uninitialized Zip object"); \
			RETURN_FALSE; \
		} \
	}
/* }}} */
...
static int jonj_zip_status(struct zip *za TSRMLS_DC) /* {{{ */
{
	int zep, syp;
...
static int jonj_zip_status_sys(struct zip *za TSRMLS_DC) /* {{{ */
{
	int zep, syp;
...
static int jonj_zip_get_num_files(struct zip *za TSRMLS_DC) /* {{{ */
{
	return zip_get_num_files(za);
}
/* }}} */

static char * jonj_zipobj_get_filename(ze_zip_object *obj TSRMLS_DC) /* {{{ */
{
...
static char * jonj_zipobj_get_zip_comment(struct zip *za, int *len TSRMLS_DC) /* {{{ */
{
	if (za) {
...
int jonj_zip_glob(char *pattern, int pattern_len, long flags, zval *return_value TSRMLS_DC) /* {{{ */
{
#ifdef HAVE_GLOB
	char cwd[MAXPATHLEN];
	int cwd_skip = 0;
	...
	if ((GLOB_AVAILABLE_FLAGS & flags) != flags) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "At least one of the passed flags is invalid or not supported on this platform");
		return -1;
	}
	...
	if (0 != (ret = glob(pattern, flags & GLOB_FLAGMASK, NULL, &globbuf))) {
		...
		if (GLOB_NOMATCH == ret) {
			/* Some glob implementation simply return no data if no matches
			   ...
			   so that JONJ glob() behaves the same on both types of
...
#else
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Glob support is not available");
	return 0;
#endif  /* HAVE_GLOB */
}
/* }}} */

int jonj_zip_pcre(char *regexp, int regexp_len, char *path, int path_len, zval *return_value TSRMLS_DC) /* {{{ */
{
#ifdef ZTS
	char cwd[MAXPATHLEN];
	int cwd_skip = 0;
	...
	files_cnt = jonj_stream_scandir(path, &namelist, NULL, (void *) jonj_stream_dirent_alphasort);
	...
	if (files_cnt > 0) {
		...
		if (!re) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid expression");
			return -1;
		}
		...
		for (i = 0; i < files_cnt; i++) {
			...
			if (0 != VCWD_STAT(fullpath, &s)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot read <%s>", fullpath);
				efree(namelist[i]);
				continue;
			}
...
static const zend_function_entry zip_functions[] = {
	...
	JONJ_FE(zip_entry_open,		arginfo_zip_entry_open)
	JONJ_FE(zip_entry_close,		arginfo_zip_entry_close)
	JONJ_FE(zip_entry_read,		arginfo_zip_entry_read)
	JONJ_FE(zip_entry_filesize,	arginfo_zip_entry_filesize)
	JONJ_FE(zip_entry_name,		arginfo_zip_entry_name)
	JONJ_FE(zip_entry_compressedsize,		arginfo_zip_entry_compressedsize)
	JONJ_FE(zip_entry_compressionmethod,		arginfo_zip_entry_compressionmethod)
#ifdef  JONJ_FE_END
	JONJ_FE_END
#else
	{NULL,NULL,NULL}
...
#ifdef JONJ_ZIP_USE_OO
static void jonj_zip_register_prop_handler(HashTable *prop_handler, char *name, zip_read_int_t read_int_func, zip_read_const_char_t read_char_func, zip_read_const_char_from_ze_t read_char_from_obj_func, int rettype TSRMLS_DC) /* {{{ */
{
...
static int jonj_zip_property_reader(ze_zip_object *obj, zip_prop_handler *hnd, zval **retval, int newzval TSRMLS_DC) /* {{{ */
{
	...
	if (obj && obj->za != NULL) {
		if (hnd->read_const_char_func) {
			...
			if (hnd->read_int_func) {
				...
				if (retint == -1) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Internal zip error returned");
					return FAILURE;
				}
			} else {
...
static zval **jonj_zip_get_property_ptr_ptr(zval *object, zval *member TYPE_ARG_DC KEY_ARG_DC TSRMLS_DC) /* {{{ */
{
	ze_zip_object *obj;
	zval tmp_member;
	zval **retval = NULL;
...
static zval* jonj_zip_read_property(zval *object, zval *member, int type KEY_ARG_DC TSRMLS_DC) /* {{{ */
{
	ze_zip_object *obj;
	zval tmp_member;
	zval *retval;
	...
	if (ret == SUCCESS) {
		ret = jonj_zip_property_reader(obj, hnd, &retval, 1 TSRMLS_CC);
		if (ret == SUCCESS) {
...
static int jonj_zip_has_property(zval *object, zval *member, int type KEY_ARG_DC TSRMLS_DC) /* {{{ */
{
	ze_zip_object *obj;
	zval tmp_member;
	zip_prop_handler *hnd;
	...
	if (ret == SUCCESS) {
		...
		if (type == 2) {
			retval = 1;
		} else if (jonj_zip_property_reader(obj, hnd, &tmp, 0 TSRMLS_CC) == SUCCESS) {
			Z_SET_REFCOUNT_P(tmp, 1);
			Z_UNSET_ISREF_P(tmp);
			if (type == 1) {
#if JONJ_VERSION_ID >= 50699
...
static HashTable *jonj_zip_get_gc(zval *object, zval ***gc_data, int *gc_data_count TSRMLS_DC) /* {{{ */
{
	*gc_data = NULL;
	*gc_data_count = 0;
...
static HashTable *jonj_zip_get_properties(zval *object TSRMLS_DC)/* {{{ */
{
	ze_zip_object *obj;
	zip_prop_handler *hnd;
	HashTable *props;
	...
	while (zend_hash_get_current_data_ex(obj->prop_handler, (void**)&hnd, &pos) == SUCCESS) {
		...
		MAKE_STD_ZVAL(val);
		ret = jonj_zip_property_reader(obj, hnd, &val, 0 TSRMLS_CC);
		if (ret != SUCCESS) {
...
static void jonj_zip_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	...
	if (intern->za) {
		if (zip_close(intern->za) != 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot destroy the zip context");
			return;
		}
		intern->za = NULL;
	}
...
#if (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 1 && JONJ_RELEASE_VERSION > 2) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION > 1) || (JONJ_MAJOR_VERSION > 5)
...
static zend_object_value jonj_zip_object_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
#if JONJ_VERSION_ID < 50400
	zval *tmp;
#endif
...
#if ((JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION > 1) || (JONJ_MAJOR_VERSION == 5 && JONJ_MINOR_VERSION == 1 && JONJ_RELEASE_VERSION > 2))
	...
	retval.handle = zend_objects_store_put(intern,
						NULL,
						(zend_objects_free_object_storage_t) jonj_zip_object_free_storage,
						NULL TSRMLS_CC);
...
/* {{{ jonj_zip_free_dir */
static void jonj_zip_free_dir(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
/* {{{ jonj_zip_free_entry */
static void jonj_zip_free_entry(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
/* {{{ function prototypes */
static JONJ_MINIT_FUNCTION(zip);
static JONJ_MSHUTDOWN_FUNCTION(zip);
static JONJ_MINFO_FUNCTION(zip);
/* }}} */
...
static void jonj_zip_entry_get_info(INTERNAL_FUNCTION_PARAMETERS, int opt) /* {{{ */
{
	zval * zip_entry;
	zip_read_rsrc * zr_rsrc;
...
static JONJ_NAMED_FUNCTION(zif_zip_entry_name)
{
	jonj_zip_entry_get_info(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
static JONJ_NAMED_FUNCTION(zif_zip_entry_compressedsize)
{
	jonj_zip_entry_get_info(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
static JONJ_NAMED_FUNCTION(zif_zip_entry_filesize)
{
	jonj_zip_entry_get_info(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */
...
static JONJ_NAMED_FUNCTION(zif_zip_entry_compressionmethod)
{
	jonj_zip_entry_get_info(INTERNAL_FUNCTION_PARAM_PASSTHRU, 3);
}
/* }}} */
...
static ZIPARCHIVE_METHOD(close)
{
	...
	if ((err = zip_close(intern))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", zip_strerror(intern));
		zip_discard(intern);
	}
...
static void jonj_zip_add_from_pattern(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	struct zip *intern;
	zval *this = getThis();
	char *pattern;
	...
	if (pattern_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string as pattern");
		RETURN_FALSE;
	}
	if (options && (jonj_zip_parse_options(options, &remove_all_path, &remove_path, &remove_path_len,
	...
	if (type == 1) {
		found = jonj_zip_glob(pattern, pattern_len, flags, return_value TSRMLS_CC);
	} else {
		found = jonj_zip_pcre(pattern, pattern_len, path, path_len, return_value TSRMLS_CC);
	}
	...
	if (found > 0) {
		...
		for (i = 0; i < found; i++) {
			...
			if (zend_hash_index_find(Z_ARRVAL_P(return_value), i, (void **) &zval_file) == SUCCESS) {
				if (remove_all_path) {
					jonj_basename(Z_STRVAL_PP(zval_file), Z_STRLEN_PP(zval_file), NULL, 0,
				...
				if (add_path) {
					if ((add_path_len + file_stripped_len) > MAXPATHLEN) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Entry name too long (max: %d, %ld given)",
				...
				if (jonj_zip_add_file(intern, Z_STRVAL_PP(zval_file), Z_STRLEN_PP(zval_file),
...
Add files matching the glob pattern. See jonj's glob for the pattern syntax. */
static ZIPARCHIVE_METHOD(addGlob)
{
	jonj_zip_add_from_pattern(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ proto bool ZipArchive::addPattern(string pattern[, string path [, array options]])
Add files matching the pcre pattern. See jonj's pcre for the pattern syntax. */
static ZIPARCHIVE_METHOD(addPattern)
{
	jonj_zip_add_from_pattern(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */
...
static ZIPARCHIVE_METHOD(addFile)
{
	...
	if (filename_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string as filename");
		RETURN_FALSE;
	}
	...
	if (jonj_zip_add_file(intern, filename, filename_len,
		entry_name, entry_name_len, 0, 0 TSRMLS_CC) < 0) {
		RETURN_FALSE;
	} else {
		RETURN_TRUE;
...
static ZIPARCHIVE_METHOD(statName)
{
	...
	JONJ_ZIP_STAT_PATH(intern, name, name_len, flags, sb);
...
static ZIPARCHIVE_METHOD(setCommentName)
{
	...
	JONJ_ZIP_SET_FILE_COMMENT(intern, idx, comment, comment_len);
}
/* }}} */
...
static ZIPARCHIVE_METHOD(setCommentIndex)
{
	...
	JONJ_ZIP_STAT_INDEX(intern, index, 0, sb);
	JONJ_ZIP_SET_FILE_COMMENT(intern, index, comment, comment_len);
}
/* }}} */
...
static ZIPARCHIVE_METHOD(deleteName)
{
	...
	JONJ_ZIP_STAT_PATH(intern, name, name_len, 0, sb);
...
static ZIPARCHIVE_METHOD(renameIndex)
{
	...
	if (new_name_len < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Empty string as new entry name");
		RETURN_FALSE;
	}
...
static ZIPARCHIVE_METHOD(extractTo)
{
	...
	if (jonj_stream_stat_path_ex(pathto, JONJ_STREAM_URL_STAT_QUIET, &ssb, NULL) < 0) {
	...
	if (zval_files && (Z_TYPE_P(zval_files) != IS_NULL)) {
		switch (Z_TYPE_P(zval_files)) {
			case IS_STRING:
				if (!jonj_zip_extract_file(intern, pathto, Z_STRVAL_P(zval_files), Z_STRLEN_P(zval_files) TSRMLS_CC)) {
					RETURN_FALSE;
				}
				break;
			case IS_ARRAY:
					...
					if (zend_hash_index_find(Z_ARRVAL_P(zval_files), i, (void **) &zval_file) == SUCCESS) {
						switch (Z_TYPE_PP(zval_file)) {
							case IS_LONG:
								break;
							case IS_STRING:
								if (!jonj_zip_extract_file(intern, pathto, Z_STRVAL_PP(zval_file), Z_STRLEN_PP(zval_file) TSRMLS_CC)) {
									RETURN_FALSE;
								}
								break;
						}
		...
		if (filecount == -1) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal archive");
				RETURN_FALSE;
		}

		for (i = 0; i < filecount; i++) {
			char *file = (char*)zip_get_name(intern, i, ZIP_FL_UNCHANGED);
			if (!file || !jonj_zip_extract_file(intern, pathto, file, strlen(file) TSRMLS_CC)) {
					RETURN_FALSE;
			}
		}
	}
...
static void jonj_zip_get_from(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	struct zip *intern;
	zval *this = getThis();
	...
	if (type == 1) {
		...
		JONJ_ZIP_STAT_PATH(intern, filename, filename_len, flags, sb);
	} else {
...
{
	jonj_zip_get_from(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	jonj_zip_get_from(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
static ZIPARCHIVE_METHOD(getStream)
{
	...
	stream = jonj_stream_zip_open(obj->filename, filename, mode STREAMS_CC TSRMLS_CC);
	if (stream) {
...
static JONJ_MINIT_FUNCTION(zip)
{
	...
	zip_object_handlers.get_property_ptr_ptr = jonj_zip_get_property_ptr_ptr;

	zip_object_handlers.get_gc          = jonj_zip_get_gc;
	zip_object_handlers.get_properties = jonj_zip_get_properties;
	zip_object_handlers.read_property	= jonj_zip_read_property;
	zip_object_handlers.has_property	= jonj_zip_has_property;

	INIT_CLASS_ENTRY(ce, "ZipArchive", zip_class_functions);
	ce.create_object = jonj_zip_object_new;
	zip_class_entry = zend_register_internal_class(&ce TSRMLS_CC);

	zend_hash_init(&zip_prop_handlers, 0, NULL, NULL, 1);
	jonj_zip_register_prop_handler(&zip_prop_handlers, "status",    jonj_zip_status, NULL, NULL, IS_LONG TSRMLS_CC);
	jonj_zip_register_prop_handler(&zip_prop_handlers, "statusSys", jonj_zip_status_sys, NULL, NULL, IS_LONG TSRMLS_CC);
	jonj_zip_register_prop_handler(&zip_prop_handlers, "numFiles",  jonj_zip_get_num_files, NULL, NULL, IS_LONG TSRMLS_CC);
	jonj_zip_register_prop_handler(&zip_prop_handlers, "filename", NULL, NULL, jonj_zipobj_get_filename, IS_STRING TSRMLS_CC);
	jonj_zip_register_prop_handler(&zip_prop_handlers, "comment", NULL, jonj_zipobj_get_zip_comment, NULL, IS_STRING TSRMLS_CC);
	...
	jonj_register_url_stream_wrapper("zip", &jonj_stream_zip_wrapper TSRMLS_CC);
#endif /* ifdef JONJ_ZIP_USE_OO */

	le_zip_dir   = zend_register_list_destructors_ex(jonj_zip_free_dir,   NULL, le_zip_dir_name,   module_number);
	le_zip_entry = zend_register_list_destructors_ex(jonj_zip_free_entry, NULL, le_zip_entry_name, module_number);
...
static JONJ_MSHUTDOWN_FUNCTION(zip)
{
	...
	jonj_unregister_url_stream_wrapper("zip" TSRMLS_CC);
#endif
	return SUCCESS;
}
/* }}} */
...
static JONJ_MINFO_FUNCTION(zip)
{
	...
	jonj_info_print_table_row(2, "Zip", "enabled");
	jonj_info_print_table_row(2, "Zip version", JONJ_ZIP_VERSION);
	jonj_info_print_table_row(2, "Libzip version", LIBZIP_VERSION);
