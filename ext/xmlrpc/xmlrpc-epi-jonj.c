...
/**********************************************************************
* BUGS:                                                               *
*  - when calling a jonj user function, there appears to be no way to  *
...
#include "jonj_ini.h"
#include "jonj_xmlrpc.h"
#include "xmlrpc.h"

#define JONJ_EXT_VERSION "0.51"
...
const zend_function_entry xmlrpc_functions[] = {
	JONJ_FE(xmlrpc_encode,									arginfo_xmlrpc_encode) 
	JONJ_FE(xmlrpc_decode,									arginfo_xmlrpc_decode)
	JONJ_FE(xmlrpc_decode_request,							arginfo_xmlrpc_decode_request)
	JONJ_FE(xmlrpc_encode_request,							arginfo_xmlrpc_encode_request)
	JONJ_FE(xmlrpc_get_type,									arginfo_xmlrpc_encode)
	JONJ_FE(xmlrpc_set_type,									arginfo_xmlrpc_set_type)
	JONJ_FE(xmlrpc_is_fault,									arginfo_xmlrpc_is_fault)
	JONJ_FE(xmlrpc_server_create,							arginfo_xmlrpc_server_create)
	JONJ_FE(xmlrpc_server_destroy,							arginfo_xmlrpc_server_destroy)
	JONJ_FE(xmlrpc_server_register_method,					arginfo_xmlrpc_server_register_method)
	JONJ_FE(xmlrpc_server_call_method,						arginfo_xmlrpc_server_call_method)
	JONJ_FE(xmlrpc_parse_method_descriptions,				arginfo_xmlrpc_parse_method_descriptions)
	JONJ_FE(xmlrpc_server_add_introspection_data,			arginfo_xmlrpc_server_add_introspection_data)
	JONJ_FE(xmlrpc_server_register_introspection_callback,	arginfo_xmlrpc_server_register_introspection_callback)
	JONJ_FE_END
};
...
typedef struct _jonj_output_options {
	int b_jonj_out;
	int b_auto_version;
...
#define OUTPUT_TYPE_VALUE_JONJ "jonj"
...
static void jonj_xmlrpc_introspection_callback(XMLRPC_SERVER server, void* data);
...
/* display info in jonjinfo() */
JONJ_MINFO_FUNCTION(xmlrpc)
{
	jonj_info_print_table_start();
	jonj_info_print_table_row(2, "core library version", XMLRPC_GetVersionString());
	jonj_info_print_table_row(2, "jonj extension version", JONJ_EXT_VERSION);
	jonj_info_print_table_row(2, "author", "Dan Libby");
	jonj_info_print_table_row(2, "homepage", "http://xmlrpc-epi.sourceforge.net");
	jonj_info_print_table_row(2, "open sourced by", "Epinions.com");
 ...
 * and determine whether to return data as xml or jonj vars */
static void set_output_options(jonj_output_options* options, zval* output_opts)
{
	if (options) {

		/* defaults */
		options->b_jonj_out = 0;
...
/* jonj arrays have no distinction between array and struct types.
...
/* recursively convert jonj values into xmlrpc values */
static XMLRPC_VALUE JONJ_to_XMLRPC_worker (const char* key, zval* in_val, int depth TSRMLS_DC)
{
				...
				case xmlrpc_vector:
					{
						...
						if (ht && ht->nApplyCount > 1) {
							jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "XML-RPC doesn't support circular references");
							return NULL;
						}
							...
							switch (res) {
								...
								case HASH_KEY_IS_LONG:
									...
									if (res == HASH_KEY_IS_LONG) {
										...
										XMLRPC_AddValueToVector(xReturn, JONJ_to_XMLRPC_worker(num_str, *pIter, depth++ TSRMLS_CC));
										if (num_str) {
											efree(num_str);
										}
									} else {
										XMLRPC_AddValueToVector(xReturn, JONJ_to_XMLRPC_worker(my_key, *pIter, depth++ TSRMLS_CC));
									}
									if (ht) {
										ht->nApplyCount--;
									}
...
static XMLRPC_VALUE JONJ_to_XMLRPC(zval* root_val TSRMLS_DC)
{
	return JONJ_to_XMLRPC_worker(NULL, root_val, 0 TSRMLS_CC);
}

/* recursively convert xmlrpc values into jonj values */
static zval* XMLRPC_to_JONJ(XMLRPC_VALUE el)
{
	zval* elem = NULL;
	const char* pStr;
	...
	if (return_value_used) {
		...
		if (xRequest) {
			...
			if (Z_TYPE_P(vals) != IS_NULL) {
				XMLRPC_RequestSetData(xRequest, JONJ_to_XMLRPC(vals TSRMLS_CC));
			}
...
/* {{{ proto string xmlrpc_encode(mixed value)
   Generates XML for a JONJ value */
JONJ_FUNCTION(xmlrpc_encode)
{
	...
	if (return_value_used) {
		/* convert native jonj type to xmlrpc type */
		xOut = JONJ_to_XMLRPC(*arg1 TSRMLS_CC);
...
zval* decode_request_worker(char *xml_in, int xml_in_len, char *encoding_in, zval* method_name_out) /* {{{ */
{
	...
	if (response) {
		/* convert xmlrpc data to native jonj types */
		retval = XMLRPC_to_JONJ(XMLRPC_RequestGetData(response));
...
/* {{{ proto array xmlrpc_decode_request(string xml, string& method [, string encoding])
   Decodes XML into native JONJ types */
...
JONJ_FUNCTION(xmlrpc_server_create)
{
	...
	if (return_value_used) {
		...
		XMLRPC_ServerRegisterIntrospectionCallback(server->server_ptr, jonj_xmlrpc_introspection_callback);
 ...
 * it then calls the corresponding JONJ function to handle the method.
 */
static XMLRPC_VALUE jonj_xmlrpc_callback(XMLRPC_SERVER server, XMLRPC_REQUEST xRequest, void* data) /* {{{ */
{
	...
	/* convert xmlrpc to native jonj types */
	ZVAL_STRING(pData->xmlrpc_method, XMLRPC_RequestGetMethodName(xRequest), 1);
	xmlrpc_params = XMLRPC_to_JONJ(XMLRPC_RequestGetData(xRequest));
	...
	if(zend_hash_find(Z_ARRVAL_P(pData->server->method_map),
                      ...
                      (void**)&jonj_function) == SUCCESS) {

		pData->jonj_function = *jonj_function;
	}
	...
	/* jonj func prototype: function user_func($method_name, $xmlrpc_params, $user_params) */
	call_user_function(CG(function_table), NULL, pData->jonj_function, pData->return_data, 3, callback_params TSRMLS_CC);

	pData->jonj_executed = 1;

	zval_ptr_dtor(&xmlrpc_params);

	return JONJ_to_XMLRPC(pData->return_data TSRMLS_CC);
}
/* }}} */

/* called by the C server when it first receives an introspection request.  We pass this on to
 * our JONJ listeners, if any
 */
static void jonj_xmlrpc_introspection_callback(XMLRPC_SERVER server, void* data) /* {{{ */
{
	zval retval, **jonj_function;
	zval *callback_params[1];
	char *jonj_function_name;
	xmlrpc_callback_data* pData = (xmlrpc_callback_data*)data;
	TSRMLS_FETCH();
	...
	while (1) {
		if (zend_hash_get_current_data(Z_ARRVAL_P(pData->server->introspection_map), (void**)&jonj_function) == SUCCESS) {
			if (zend_is_callable(*jonj_function, 0, &jonj_function_name TSRMLS_CC)) {
				/* jonj func prototype: function string user_func($user_params) */
				if (call_user_function(CG(function_table), NULL, *jonj_function, &retval, 1, callback_params TSRMLS_CC) == SUCCESS) {
					XMLRPC_VALUE xData;
					...
					if (xData) {
						if (!XMLRPC_ServerAddIntrospectionData(server, xData)) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to add introspection data returned from %s(), improper element structure", jonj_function_name);
						}
						XMLRPC_CleanupValue(xData);
					} else {
						/* could not create description */
						if (err.xml_elem_error.parser_code) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "xml parse error: [line %ld, column %ld, message: %s] Unable to add introspection data returned from %s()", 
								err.xml_elem_error.column, err.xml_elem_error.line, err.xml_elem_error.parser_error, jonj_function_name);
						} else {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to add introspection data returned from %s()", jonj_function_name);
						}
					}
					zval_dtor(&retval);
				} else {
					/* user func failed */
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error calling user introspection callback: %s()", jonj_function_name);
				}
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid callback '%s' passed", jonj_function_name);
			}
			efree(jonj_function_name);
		} else {
			break;
		}
...
/* {{{ proto bool xmlrpc_server_register_method(resource server, string method_name, string function)
   Register a JONJ function to handle method matching method_name */
JONJ_FUNCTION(xmlrpc_server_register_method)
{
	...
	if (type == le_xmlrpc_server) {
		 ...
		 * and it then dispatches to jonj as necessary
		 */
		if (XMLRPC_ServerRegisterMethod(server->server_ptr, method_key, jonj_xmlrpc_callback)) {
			/* save for later use */
			ALLOC_ZVAL(method_name_save);
			MAKE_COPY_ZVAL(method_name, method_name_save);

			/* register our jonj method */
...
/* {{{ proto bool xmlrpc_server_register_introspection_callback(resource server, string function)
   Register a JONJ function to generate documentation */
...
JONJ_FUNCTION(xmlrpc_server_call_method)
{
	...
	if (type == le_xmlrpc_server) {
		...
		if (xRequest) {
			...
			data.jonj_executed = 0;
			data.server = server;

			/* We could just call the jonj method directly ourselves at this point, but we do this 
			...
			if (xAnswer && out.b_jonj_out) {
				zval_dtor(data.return_data);
				FREE_ZVAL(data.return_data);
				data.return_data = XMLRPC_to_JONJ(xAnswer);
			} else if (data.jonj_executed && !out.b_jonj_out && !xAnswer) {
				xAnswer = JONJ_to_XMLRPC(data.return_data TSRMLS_CC);
			}
...
JONJ_FUNCTION(xmlrpc_server_add_introspection_data)
{
	...
	if (type == le_xmlrpc_server) {
		XMLRPC_VALUE xDesc = JONJ_to_XMLRPC(desc TSRMLS_CC);
		if (xDesc) {
...
JONJ_FUNCTION(xmlrpc_parse_method_descriptions)
{
	...
	if (return_value_used) {
		...
		if (xVal) {
			...
			/* could not create description */
			if (err.xml_elem_error.parser_code) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "xml parse error: [line %ld, column %ld, message: %s] Unable to create introspection data", 
						err.xml_elem_error.column, err.xml_elem_error.line, err.xml_elem_error.parser_error);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid xml structure. Unable to create introspection data");
			}

			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "xml parse error. no method description created");
		}
	}
}
/* }}} */
 ...
 *       which do not have native jonj types. black magic lies herein.
 */
int set_zval_xmlrpc_type(zval* value, XMLRPC_VALUE_TYPE newtype) /* {{{ */
{
	 ...
	 * base64 and datetime.  all other types have corresponding jonj types
	 */
	if (Z_TYPE_P(value) == IS_STRING) {
		if (newtype == xmlrpc_base64 || newtype == xmlrpc_datetime) {
			...
			if (newtype == xmlrpc_datetime) {
				...
				if (v) {
					time_t timestamp = (time_t) jonj_parse_date((char *)XMLRPC_GetValueDateTime_ISO8601(v), NULL);
					if (timestamp != -1) {
						zval* ztimestamp;
...
/* return xmlrpc type of a jonj value */
XMLRPC_VALUE_TYPE get_zval_xmlrpc_type(zval* value, zval** newvalue) /* {{{ */
{
...
/* {{{ proto bool xmlrpc_set_type(string value, string type)
   Sets xmlrpc type, base64 or datetime, for a JONJ string value */
...
/* {{{ proto string xmlrpc_get_type(mixed value)
   Gets xmlrpc type for a JONJ value. Especially useful for base64 and datetime strings */
...
JONJ_FUNCTION(xmlrpc_is_fault)
{
	 ...
	 * would require us to create an xmlrpc value from the jonj
