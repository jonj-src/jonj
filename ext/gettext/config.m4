...
JONJ_ARG_WITH(gettext,for GNU gettext support,
[  --with-gettext[=DIR]      Include GNU gettext support])

if test "$JONJ_GETTEXT" != "no"; then
  for i in $JONJ_GETTEXT /usr/local /usr; do
  ...
  GETTEXT_LIBDIR=$GETTEXT_DIR/$JONJ_LIBDIR
  ...
  JONJ_NEW_EXTENSION(gettext, gettext.c, $ext_shared)
  JONJ_SUBST(GETTEXT_SHARED_LIBADD)

  if test -n "$GETTEXT_LIBS"; then
    JONJ_ADD_LIBRARY_WITH_PATH($GETTEXT_LIBS, $GETTEXT_LIBDIR, GETTEXT_SHARED_LIBADD)
  fi

  JONJ_ADD_INCLUDE($GETTEXT_INCDIR)
