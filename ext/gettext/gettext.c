...
/* {{{ jonj_gettext_functions[]
 */
const zend_function_entry jonj_gettext_functions[] = {
	JONJ_NAMED_FE(textdomain,		zif_textdomain,		arginfo_textdomain)
	JONJ_NAMED_FE(gettext,			zif_gettext,		arginfo_gettext)
	/* Alias for gettext() */
	JONJ_NAMED_FE(_,					zif_gettext,		arginfo_gettext)
	JONJ_NAMED_FE(dgettext,			zif_dgettext,		arginfo_dgettext)
	JONJ_NAMED_FE(dcgettext,			zif_dcgettext,		arginfo_dcgettext)
	JONJ_NAMED_FE(bindtextdomain,	zif_bindtextdomain,	arginfo_bindtextdomain)
#if HAVE_NGETTEXT
	JONJ_NAMED_FE(ngettext,			zif_ngettext,		arginfo_ngettext)
#endif
#if HAVE_DNGETTEXT
	JONJ_NAMED_FE(dngettext,			zif_dngettext,		arginfo_dngettext)
#endif
#if HAVE_DCNGETTEXT
	JONJ_NAMED_FE(dcngettext,		zif_dcngettext,		arginfo_dcngettext)
#endif
#if HAVE_BIND_TEXTDOMAIN_CODESET
	JONJ_NAMED_FE(bind_textdomain_codeset,	zif_bind_textdomain_codeset,	arginfo_bind_textdomain_codeset)
#endif
    JONJ_FE_END
};
/* }}} */
...
zend_module_entry jonj_gettext_module_entry = {
	...
	NULL,
	NULL,
	JONJ_MINFO(jonj_gettext),
	NO_VERSION_YET,
	STANDARD_MODULE_PROPERTIES
};

#ifdef COMPILE_DL_GETTEXT
ZEND_GET_MODULE(jonj_gettext)
#endif

#define JONJ_GETTEXT_MAX_DOMAIN_LENGTH 1024
#define JONJ_GETTEXT_MAX_MSGID_LENGTH 4096

#define JONJ_GETTEXT_DOMAIN_LENGTH_CHECK \
	if (domain_len > JONJ_GETTEXT_MAX_DOMAIN_LENGTH) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "domain passed too long"); \
		RETURN_FALSE; \
	}

#define JONJ_GETTEXT_LENGTH_CHECK(check_name, check_len) \
	if (check_len > JONJ_GETTEXT_MAX_MSGID_LENGTH) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s passed too long", check_name); \
		RETURN_FALSE; \
	}
...
JONJ_MINFO_FUNCTION(jonj_gettext)
{
	...
	jonj_info_print_table_row(2, "GetText Support", "enabled");
...
JONJ_NAMED_FUNCTION(zif_textdomain)
{
	...
	JONJ_GETTEXT_DOMAIN_LENGTH_CHECK
...
JONJ_NAMED_FUNCTION(zif_gettext)
{
	...
	JONJ_GETTEXT_LENGTH_CHECK("msgid", msgid_len)
...
JONJ_NAMED_FUNCTION(zif_bindtextdomain)
{
	...
	if (domain[0] == '\0') {
		jonj_error(E_WARNING, "The first parameter of bindtextdomain must not be empty");
		RETURN_FALSE;
	}
...
JONJ_NAMED_FUNCTION(zif_ngettext)
{
	...
	JONJ_GETTEXT_LENGTH_CHECK("msgid1", msgid1_len)
	JONJ_GETTEXT_LENGTH_CHECK("msgid2", msgid2_len)
