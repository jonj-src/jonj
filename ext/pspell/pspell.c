...
static JONJ_MINIT_FUNCTION(pspell);
static JONJ_MINFO_FUNCTION(pspell);
static JONJ_FUNCTION(pspell_new);
static JONJ_FUNCTION(pspell_new_personal);
static JONJ_FUNCTION(pspell_new_config);
static JONJ_FUNCTION(pspell_check);
static JONJ_FUNCTION(pspell_suggest);
static JONJ_FUNCTION(pspell_store_replacement);
static JONJ_FUNCTION(pspell_add_to_personal);
static JONJ_FUNCTION(pspell_add_to_session);
static JONJ_FUNCTION(pspell_clear_session);
static JONJ_FUNCTION(pspell_save_wordlist);
static JONJ_FUNCTION(pspell_config_create);
static JONJ_FUNCTION(pspell_config_runtogether);
static JONJ_FUNCTION(pspell_config_mode);
static JONJ_FUNCTION(pspell_config_ignore);
static JONJ_FUNCTION(pspell_config_personal);
static JONJ_FUNCTION(pspell_config_dict_dir);
static JONJ_FUNCTION(pspell_config_data_dir);
static JONJ_FUNCTION(pspell_config_repl);
static JONJ_FUNCTION(pspell_config_save_repl);
...
static const zend_function_entry pspell_functions[] = {
	JONJ_FE(pspell_new,					arginfo_pspell_new)
	JONJ_FE(pspell_new_personal,			arginfo_pspell_new_personal)
	JONJ_FE(pspell_new_config,			arginfo_pspell_new_config)
	JONJ_FE(pspell_check,				arginfo_pspell_check)
	JONJ_FE(pspell_suggest,				arginfo_pspell_suggest)
	JONJ_FE(pspell_store_replacement,	arginfo_pspell_store_replacement)
	JONJ_FE(pspell_add_to_personal,		arginfo_pspell_add_to_personal)
	JONJ_FE(pspell_add_to_session,		arginfo_pspell_add_to_session)
	JONJ_FE(pspell_clear_session,		arginfo_pspell_clear_session)
	JONJ_FE(pspell_save_wordlist,		arginfo_pspell_save_wordlist)
	JONJ_FE(pspell_config_create,		arginfo_pspell_config_create)
	JONJ_FE(pspell_config_runtogether,	arginfo_pspell_config_runtogether)
	JONJ_FE(pspell_config_mode,			arginfo_pspell_config_mode)
	JONJ_FE(pspell_config_ignore,		arginfo_pspell_config_ignore)
	JONJ_FE(pspell_config_personal,		arginfo_pspell_config_personal)
	JONJ_FE(pspell_config_dict_dir,		arginfo_pspell_config_dict_dir)
	JONJ_FE(pspell_config_data_dir,		arginfo_pspell_config_data_dir)
	JONJ_FE(pspell_config_repl,			arginfo_pspell_config_repl)
	JONJ_FE(pspell_config_save_repl,		arginfo_pspell_config_save_repl)
	JONJ_FE_END
};
/* }}} */
...
zend_module_entry pspell_module_entry = {
	...
	"pspell", pspell_functions, JONJ_MINIT(pspell), NULL, NULL, NULL, JONJ_MINFO(pspell), NO_VERSION_YET, STANDARD_MODULE_PROPERTIES
};
...
static void jonj_pspell_close(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
static void jonj_pspell_close_config(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
#define PSPELL_FETCH_CONFIG \
	...
	if (config == NULL || type != le_pspell_config) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%ld is not a PSPELL config index", conf);	\
		RETURN_FALSE;	\
	}	\
...
#define PSPELL_FETCH_MANAGER \
	...
	if (!manager || type != le_pspell) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%ld is not a PSPELL result index", scin);	\
		RETURN_FALSE;	\
	}	\
...
static JONJ_MINIT_FUNCTION(pspell)
{
	...
	le_pspell = zend_register_list_destructors_ex(jonj_pspell_close, NULL, "pspell", module_number);
	le_pspell_config = zend_register_list_destructors_ex(jonj_pspell_close_config, NULL, "pspell config", module_number);
	return SUCCESS;
}
/* }}} */
...
static JONJ_FUNCTION(pspell_new)
{
	...
	if (pspell_error_number(ret) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "PSPELL couldn't open the dictionary. reason: %s", pspell_error_message(ret));
...
static JONJ_FUNCTION(pspell_new_personal)
{
	...
	if (jonj_check_open_basedir(personal TSRMLS_CC)) {
		delete_pspell_config(config);
		RETURN_FALSE;
	}
...
static JONJ_FUNCTION(pspell_suggest)
{
	...
	if (wl) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "PSPELL had a problem. details: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static JONJ_FUNCTION(pspell_store_replacement)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pspell_store_replacement() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static JONJ_FUNCTION(pspell_add_to_personal)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pspell_add_to_personal() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static JONJ_FUNCTION(pspell_add_to_session)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pspell_add_to_session() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static JONJ_FUNCTION(pspell_clear_session)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pspell_clear_session() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static JONJ_FUNCTION(pspell_save_wordlist)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pspell_save_wordlist() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
...
static void pspell_config_path(INTERNAL_FUNCTION_PARAMETERS, char *option)
{
	...
	if (jonj_check_open_basedir(value TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
static JONJ_FUNCTION(pspell_config_repl)
{
	...
	if (jonj_check_open_basedir(repl TSRMLS_CC)) {
		RETURN_FALSE;
	}
...
static JONJ_MINFO_FUNCTION(pspell)
{
	...
	jonj_info_print_table_row(2, "PSpell Support", "enabled");
