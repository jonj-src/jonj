...
static int get_http_body(jonj_stream *socketd, int close, char *headers,  char **response, int *out_size TSRMLS_DC);
static int get_http_headers(jonj_stream *socketd,char **response, int *out_size TSRMLS_DC);
...
int proxy_authentication(zval* this_ptr, smart_str* soap_headers TSRMLS_DC)
{
	...
	if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_proxy_login", sizeof("_proxy_login"), (void **)&login) == SUCCESS &&
		...
		smart_str_0(&auth);
		buf = jonj_base64_encode((unsigned char*)auth.c, auth.len, &len);
                          ...
                          smart_str* soap_headers TSRMLS_DC)
{
	...
	if (context &&
		jonj_stream_context_get_option(context, "http", "header", &tmp) == SUCCESS &&
...
static jonj_stream* http_connect(zval* this_ptr, jonj_url *jonjurl, int use_ssl, jonj_stream_context *context, int *use_proxy TSRMLS_DC)
{
	jonj_stream *stream;
	...
	stream = jonj_stream_xport_create(name, namelen,
		REPORT_ERRORS,
	...
	if (stream && *use_proxy && use_ssl) {
		...
		smart_str_appends(&soap_headers, jonjurl->host);
		smart_str_appendc(&soap_headers, ':');
		smart_str_append_unsigned(&soap_headers, jonjurl->port);
		...
		smart_str_append_const(&soap_headers, "\r\n");
		if (jonj_stream_write(stream, soap_headers.c, soap_headers.len) != soap_headers.len) {
		...
		if (stream) {
			...
			if (jonj_stream_xport_crypto_setup(stream, crypto_method, NULL TSRMLS_CC) < 0 ||
                           ...
                           int   *buffer_len TSRMLS_DC)
{
  ...
  request_size = buf_size;
	...
	if (zend_hash_find(Z_OBJPROP_P(this_ptr), "httpsocket", sizeof("httpsocket"), (void **)&tmp) == SUCCESS) {
		jonj_stream_from_zval_no_verify(stream,tmp);
	...
	if (location != NULL && location[0] != '\000') {
		jonjurl = jonj_url_parse(location);
	}

	if (SUCCESS == zend_hash_find(Z_OBJPROP_P(this_ptr),
			"_stream_context", sizeof("_stream_context"), (void**)&tmp)) {
		context = jonj_stream_context_from_zval(*tmp, 0);
	}

	if (context &&
		jonj_stream_context_get_option(context, "http", "max_redirects", &tmp) == SUCCESS) {
...
try_again:
	if (jonjurl == NULL || jonjurl->host == NULL) {
	  if (jonjurl != NULL) {jonj_url_free(jonjurl);}
		if (request != buf) {efree(request);}
	...
	use_ssl = 0;
	if (jonjurl->scheme != NULL && strcmp(jonjurl->scheme, "https") == 0) {
		use_ssl = 1;
	} else if (jonjurl->scheme == NULL || strcmp(jonjurl->scheme, "http") != 0) {
		jonj_url_free(jonjurl);
		if (request != buf) {efree(request);}
	...
	if (use_ssl && jonj_stream_locate_url_wrapper("https://", NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC) == NULL) {
		jonj_url_free(jonjurl);
		if (request != buf) {efree(request);}
	...
	if (stream != NULL) {
	  jonj_url *orig;
		if (zend_hash_find(Z_OBJPROP_P(this_ptr), "httpurl", sizeof("httpurl"), (void **)&tmp) == SUCCESS &&
		    (orig = (jonj_url *) zend_fetch_resource(tmp TSRMLS_CC, -1, "httpurl", NULL, 1, le_url)) != NULL &&
		    ((use_proxy && !use_ssl) ||
		     (((use_ssl && orig->scheme != NULL && strcmp(orig->scheme, "https") == 0) ||
		      (!use_ssl && orig->scheme == NULL) ||
		      (!use_ssl && strcmp(orig->scheme, "https") != 0)) &&
		     strcmp(orig->host, jonjurl->host) == 0 &&
		     orig->port == jonjurl->port))) {
    } else {
			jonj_stream_close(stream);
	...
	if (stream != NULL && jonj_stream_eof(stream)) {
		jonj_stream_close(stream);
		zend_hash_del(Z_OBJPROP_P(this_ptr), "httpurl", sizeof("httpurl"));
		zend_hash_del(Z_OBJPROP_P(this_ptr), "httpsocket", sizeof("httpsocket"));
		zend_hash_del(Z_OBJPROP_P(this_ptr), "_use_proxy", sizeof("_use_proxy"));
	...
	if (!stream) {
		stream = http_connect(this_ptr, jonjurl, use_ssl, context, &use_proxy TSRMLS_CC);
		if (stream) {
			jonj_stream_auto_cleanup(stream);
			add_property_resource(this_ptr, "httpsocket", jonj_stream_get_resource_id(stream));
	...
	if (stream) {
	  ...
	  int ret = zend_list_insert(jonjurl, le_url TSRMLS_CC);

		add_property_resource(this_ptr, "httpurl", ret);
		/*zend_list_addref(ret);*/

		if (context &&
		    jonj_stream_context_get_option(context, "http", "protocol_version", &tmp) == SUCCESS &&
		...
		if (use_proxy && !use_ssl) {
			smart_str_appends(&soap_headers, jonjurl->scheme);
			smart_str_append_const(&soap_headers, "://");
			smart_str_appends(&soap_headers, jonjurl->host);
			smart_str_appendc(&soap_headers, ':');
			smart_str_append_unsigned(&soap_headers, jonjurl->port);
		...
		if (jonjurl->path) {
			smart_str_appends(&soap_headers, jonjurl->path);
		} else {
			smart_str_appendc(&soap_headers, '/');
		}
		if (jonjurl->query) {
			smart_str_appendc(&soap_headers, '?');
			smart_str_appends(&soap_headers, jonjurl->query);
		}
		if (jonjurl->fragment) {
			smart_str_appendc(&soap_headers, '#');
			smart_str_appends(&soap_headers, jonjurl->fragment);
		}
		if (http_1_1) {
			smart_str_append_const(&soap_headers, " HTTP/1.1\r\n");
		} else {
		...
		smart_str_append_const(&soap_headers, "Host: ");
		smart_str_appends(&soap_headers, jonjurl->host);
		if (jonjurl->port != (use_ssl?443:80)) {
			smart_str_appendc(&soap_headers, ':');
			smart_str_append_unsigned(&soap_headers, jonjurl->port);
		}
		if (!http_1_1 ||
		...
		if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_user_agent", sizeof("_user_agent"), (void **)&tmp) == SUCCESS &&
			...
			if (Z_STRLEN_PP(tmp) > 0) {
				...
				smart_str_append_const(&soap_headers, "\r\n");
		           ...
		           jonj_stream_context_get_option(context, "http", "user_agent", &tmp) == SUCCESS &&
		           Z_TYPE_PP(tmp) == IS_STRING) {
			if (Z_STRLEN_PP(tmp) > 0) {
				smart_str_append_const(&soap_headers, "User-Agent: ");
				smart_str_appendl(&soap_headers, Z_STRVAL_PP(tmp), Z_STRLEN_PP(tmp));
			...
			smart_str_append_const(&soap_headers, "User-Agent: JONJ-SOAP/"JONJ_VERSION"\r\n");
		}
		...
		if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_login", sizeof("_login"), (void **)&login) == SUCCESS &&
			...
			if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_digest", sizeof("_digest"), (void **)&digest) == SUCCESS) {
				if (Z_TYPE_PP(digest) == IS_ARRAY) {
					...
					JONJ_MD5Init(&md5ctx);
					snprintf(cnonce, sizeof(cnonce), "%ld", jonj_rand(TSRMLS_C));
					JONJ_MD5Update(&md5ctx, (unsigned char*)cnonce, strlen(cnonce));
					JONJ_MD5Final(hash, &md5ctx);
					...
					JONJ_MD5Init(&md5ctx);
					JONJ_MD5Update(&md5ctx, (unsigned char*)Z_STRVAL_PP(login), Z_STRLEN_PP(login));
					JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
					if (zend_hash_find(Z_ARRVAL_PP(digest), "realm", sizeof("realm"), (void **)&tmp) == SUCCESS &&
					    Z_TYPE_PP(tmp) == IS_STRING) {
						JONJ_MD5Update(&md5ctx, (unsigned char*)Z_STRVAL_PP(tmp), Z_STRLEN_PP(tmp));
					}
					JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
					if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_password", sizeof("_password"), (void **)&password) == SUCCESS &&
					    Z_TYPE_PP(password) == IS_STRING) {
						JONJ_MD5Update(&md5ctx, (unsigned char*)Z_STRVAL_PP(password), Z_STRLEN_PP(password));
					}
					JONJ_MD5Final(hash, &md5ctx);
					...
					if (zend_hash_find(Z_ARRVAL_PP(digest), "algorithm", sizeof("algorithm"), (void **)&tmp) == SUCCESS &&
						...
						JONJ_MD5Init(&md5ctx);
						JONJ_MD5Update(&md5ctx, (unsigned char*)HA1, 32);
						JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
						...
						JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
						JONJ_MD5Update(&md5ctx, (unsigned char*)cnonce, 8);
						JONJ_MD5Final(hash, &md5ctx);
						make_digest(HA1, hash);
					}

					JONJ_MD5Init(&md5ctx);
					JONJ_MD5Update(&md5ctx, (unsigned char*)"POST:", sizeof("POST:")-1);
					if (jonjurl->path) {
						JONJ_MD5Update(&md5ctx, (unsigned char*)jonjurl->path, strlen(jonjurl->path));
					} else {
						JONJ_MD5Update(&md5ctx, (unsigned char*)"/", 1);
					}
					if (jonjurl->query) {
						JONJ_MD5Update(&md5ctx, (unsigned char*)"?", 1);
						JONJ_MD5Update(&md5ctx, (unsigned char*)jonjurl->query, strlen(jonjurl->query));
					}
...
/*
					if (zend_hash_find(Z_ARRVAL_PP(digest), "qop", sizeof("qop"), (void **)&tmp) == SUCCESS &&
						...
						JONJ_MD5Update(&md5ctx, ":", 1);
						JONJ_MD5Update(&md5ctx, HEntity, HASHHEXLEN);
					}
*/
					JONJ_MD5Final(hash, &md5ctx);
					...
					if (zend_hash_find(Z_ARRVAL_PP(digest), "qop", sizeof("qop"), (void **)&tmp) == SUCCESS &&
					    Z_TYPE_PP(tmp) == IS_STRING) {
						JONJ_MD5Update(&md5ctx, (unsigned char*)nc, 8);
						JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
						JONJ_MD5Update(&md5ctx, (unsigned char*)cnonce, 8);
						JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
						/* TODO: Support for qop="auth-int" */
						JONJ_MD5Update(&md5ctx, (unsigned char*)"auth", sizeof("auth")-1);
						JONJ_MD5Update(&md5ctx, (unsigned char*)":", 1);
					}
					JONJ_MD5Update(&md5ctx, (unsigned char*)HA2, 32);
					JONJ_MD5Final(hash, &md5ctx);
					  ...
					  zval** value;
						...
						if (zend_hash_index_find(Z_ARRVAL_PP(data), 0, (void**)&value) == SUCCESS &&
						  ...
						  if ((zend_hash_index_find(Z_ARRVAL_PP(data), 1, (void**)&tmp) == FAILURE ||
					           Z_TYPE_PP(tmp) != IS_STRING ||
						       strncmp(jonjurl->path?jonjurl->path:"/",Z_STRVAL_PP(tmp),Z_STRLEN_PP(tmp)) == 0) &&
						      (zend_hash_index_find(Z_ARRVAL_PP(data), 2, (void**)&tmp) == FAILURE ||
						       Z_TYPE_PP(tmp) != IS_STRING ||
						       in_domain(jonjurl->host,Z_STRVAL_PP(tmp))) &&
		...
		err = jonj_stream_write(stream, soap_headers.c, soap_headers.len);
	...
	*/
	...
	while (cookie_itt) {
		...
		if (eqpos != NULL && (sempos == NULL || sempos > eqpos)) {
			...
			if (!zend_hash_index_exists(Z_ARRVAL_P(zcookie), 1)) {
				char *t = jonjurl->path?jonjurl->path:"/";
				char *c = strrchr(t, '/');
				if (c) {
			...
			if (!zend_hash_index_exists(Z_ARRVAL_P(zcookie), 2)) {
				add_index_string(zcookie, 2, jonjurl->host, 1);
			}
	...
	if (http_status >= 300 && http_status < 400) {
		...
		if ((loc = get_http_header_value(http_headers,"Location: ")) != NULL) {
			jonj_url *new_url  = jonj_url_parse(loc);
			...
			if (new_url != NULL) {
				...
				if (new_url->scheme == NULL && new_url->path != NULL) {
					new_url->scheme = jonjurl->scheme ? estrdup(jonjurl->scheme) : NULL;
					new_url->host = jonjurl->host ? estrdup(jonjurl->host) : NULL;
					new_url->port = jonjurl->port;
		...
		if (auth &&
			...
			if (digest != NULL) {
				jonj_url *new_url  = emalloc(sizeof(jonj_url));

				Z_DELREF_P(digest);
				add_property_zval_ex(this_ptr, "_digest", sizeof("_digest"), digest TSRMLS_CC);

				*new_url = *jonjurl;
				if (jonjurl->scheme) jonjurl->scheme = estrdup(jonjurl->scheme);
				if (jonjurl->user) jonjurl->user = estrdup(jonjurl->user);
				if (jonjurl->pass) jonjurl->pass = estrdup(jonjurl->pass);
				if (jonjurl->host) jonjurl->host = estrdup(jonjurl->host);
				if (jonjurl->path) jonjurl->path = estrdup(jonjurl->path);
				if (jonjurl->query) jonjurl->query = estrdup(jonjurl->query);
				if (jonjurl->fragment) jonjurl->fragment = estrdup(jonjurl->fragment);
				jonjurl = new_url;
...
static int get_http_body(jonj_stream *stream, int close, char *headers,  char **response, int *out_size TSRMLS_DC)
{
	...
	if (header_chunked) {
		...
		while (!done) {
			...
			jonj_stream_gets(stream, headerbuf, sizeof(headerbuf));
			if (sscanf(headerbuf, "%x", &buf_size) > 0 ) {
				if (buf_size > 0) {
					...
					while (len_size < buf_size) {
						int len_read = jonj_stream_read(stream, http_buf + http_buf_size, buf_size - len_size);
						if (len_read <= 0) {
							/* Error or EOF */
							done = TRUE;
						  break;
		...
		while (1) {
			if (!jonj_stream_gets(stream, headerbuf, sizeof(headerbuf))) {
				break;
			}
		...
		while (http_buf_size < header_length) {
			int len_read = jonj_stream_read(stream, http_buf + http_buf_size, header_length - http_buf_size);
			if (len_read <= 0) {
				break;
			}
			http_buf_size += len_read;
		...
		do {
			...
			len_read = jonj_stream_read(stream, http_buf + http_buf_size, 4096);
			if (len_read > 0) {
				http_buf_size += len_read;
			}
		} while(!jonj_stream_eof(stream));
	} else {
		return FALSE;
	}
...
static int get_http_headers(jonj_stream *stream, char **response, int *out_size TSRMLS_DC)
{
	int done = FALSE;
