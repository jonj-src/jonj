...
void sdl_set_uri_credentials(sdlCtx *ctx, char *uri TSRMLS_DC)
{
	...
	if (l1 != l2 || memcmp(ctx->sdl->source, uri, l1) != 0) {
		...
		context = jonj_libxml_switch_context(NULL TSRMLS_CC);
		jonj_libxml_switch_context(context TSRMLS_CC);
		if (context) {
			ctx->context = jonj_stream_context_from_zval(context, 1);

			if (ctx->context &&
			    jonj_stream_context_get_option(ctx->context, "http", "header", &header) == SUCCESS) {
				...
				if (s && (s == Z_STRVAL_PP(header) || *(s-1) == '\n' || *(s-1) == '\r')) {
					...
					if (rest) {
						...
						jonj_stream_context_set_option(ctx->context, "http", "header", &new_header);
...
{
	if (ctx->old_header) {
	    jonj_stream_context_set_option(ctx->context, "http", "header", ctx->old_header);
...
*/
...

					{
						...
						if (zend_hash_add(&ctx.sdl->functions, jonj_strtolower(tmp, len), len+1, &function, sizeof(sdlFunctionPtr), NULL) != SUCCESS) {
						...
						if (function->requestName != NULL && strcmp(function->requestName,function->functionName) != 0) {
							...
							len = strlen(tmp);
							zend_hash_add(ctx.sdl->requests, jonj_strtolower(tmp, len), len+1, &function, sizeof(sdlFunctionPtr), NULL);
							efree(tmp);
						}
					}
					trav2 = trav2->next;
...
static void add_sdl_to_cache(const char *fn, const char *uri, time_t t, sdlPtr sdl TSRMLS_DC)
{
	...
	jonj_ignore_value(write(f, buf.c, buf.len));
	close(f);
	smart_str_free(&buf);
...
sdlPtr get_sdl(zval *this_ptr, char *uri, long cache_wsdl TSRMLS_DC)
{
	...
	if ((cache_wsdl & WSDL_CACHE_DISK) && (uri_len < MAXPATHLEN)) {
		...
		time_t cached;
		char *user = jonj_get_current_user(TSRMLS_C);
		int user_len = user ? strlen(user) + 1 : 0;

		md5str[0] = '\0';
		JONJ_MD5Init(&context);
		JONJ_MD5Update(&context, (unsigned char*)uri, uri_len);
	...
	if (zend_hash_find(Z_OBJPROP_P(this_ptr), "_proxy_host", sizeof("_proxy_host"), (void **) &proxy_host) == SUCCESS &&
		...
		jonj_stream_context_set_option(context, "http", "proxy", str_proxy);
		...
		if (uri_len < sizeof("https://")-1 ||
			...
			jonj_stream_context_set_option(context, "http", "request_fulluri", str_proxy);
			zval_ptr_dtor(&str_proxy);
		}
	...
	if (jonj_stream_context_get_option(context, "http", "protocol_version", &tmp) == FAILURE) {
    	zval *http_version;
		MAKE_STD_ZVAL(http_version);
		ZVAL_DOUBLE(http_version, 1.1);
		jonj_stream_context_set_option(context, "http", "protocol_version", http_version);
	...
	if (headers.len > 0) {
		...
		jonj_stream_context_set_option(context, "http", "header", str_headers);
	...
	if (context) {
		...
		jonj_stream_context_to_zval(context, new_context);
		orig_context = jonj_libxml_switch_context(new_context TSRMLS_CC);
	}
	...
	if (context) {
		jonj_libxml_switch_context(orig_context TSRMLS_CC);
	...
	if (cache_wsdl & WSDL_CACHE_MEMORY) {
		if (sdl) {
			...
			if (SUCCESS == zend_hash_update(SOAP_GLOBAL(mem_cache), uri,
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to register persistent entry");
