...
{ \
	...
	finfo = obj->ptr; \
	if (!finfo) { \
        	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The invalid fileinfo object."); \
		RETURN_FALSE; \
	} \
}
...
/* {{{ finfo_objects_new
 */
JONJ_FILEINFO_API zend_object_value finfo_objects_new(zend_class_entry *class_type TSRMLS_DC)
{
...
#define FINFO_SET_OPTION(magic, options) \
	if (magic_setflags(magic, options) == -1) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to set option '%ld' %d:%s", \
...
{
	if (rsrc->ptr) {
		struct jonj_fileinfo *finfo = (struct jonj_fileinfo *) rsrc->ptr;
...
zend_function_entry fileinfo_functions[] = {
	JONJ_FE(finfo_open,		arginfo_finfo_open)
	JONJ_FE(finfo_close,		arginfo_finfo_close)
	JONJ_FE(finfo_set_flags,	arginfo_finfo_set_flags)
	JONJ_FE(finfo_file,		arginfo_finfo_file)
	JONJ_FE(finfo_buffer,	arginfo_finfo_buffer)
	JONJ_FE(mime_content_type, arginfo_mime_content_type)
	{NULL, NULL, NULL}
};
/* }}} */
...
JONJ_MINFO_FUNCTION(fileinfo)
{
	...
	jonj_info_print_table_row(2, "fileinfo support", "enabled");
	jonj_info_print_table_row(2, "version", JONJ_FILEINFO_VERSION);
	jonj_info_print_table_row(2, "libmagic", magic_ver);
...
JONJ_FUNCTION(finfo_open)
{
	...
	if (file_len == 0) {
		...
		if (jonj_check_open_basedir(file TSRMLS_CC)) {
			FILEINFO_DESTROY_OBJECT(object);
			RETURN_FALSE;
		}
	...
	finfo = emalloc(sizeof(struct jonj_fileinfo));
	...
	if (finfo->magic == NULL) {
		efree(finfo);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid mode '%ld'.", options);
		FILEINFO_DESTROY_OBJECT(object);
		RETURN_FALSE;
	}

	if (magic_load(finfo->magic, file) == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to load magic database at '%s'.", file);
		magic_close(finfo->magic);
		efree(finfo);
		FILEINFO_DESTROY_OBJECT(object);
		RETURN_FALSE;
...
JONJ_FUNCTION(finfo_close)
{
	...
	ZEND_FETCH_RESOURCE(finfo, struct jonj_fileinfo *, &zfinfo, -1, "file_info", le_fileinfo);
...
static void _jonj_finfo_get_type(INTERNAL_FUNCTION_PARAMETERS, int mode, int mimetype_emu) /* {{{ */
{
	long options = 0;
	char *ret_val = NULL, *buffer = NULL;
	int buffer_len;
	struct jonj_fileinfo *finfo = NULL;
	...
	if (mimetype_emu) {
		...
		switch (Z_TYPE_P(what)) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can only process string or stream arguments");
				RETURN_FALSE;
		}

		magic = magic_open(MAGIC_MIME_TYPE);
		if (magic_load(magic, NULL) == -1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to load magic database.");
			goto common;
		}
	} else if (object) {
	...
	switch (mode) {
		...
		case FILEINFO_MODE_STREAM:
		{
				...
				jonj_stream_from_zval_no_verify(stream, &what);
				if (!stream) {
					goto common;
				}

				streampos = jonj_stream_tell(stream); /* remember stream position for restoration */
				jonj_stream_seek(stream, 0, SEEK_SET);

				ret_val = (char *) magic_stream(magic, stream);

				jonj_stream_seek(stream, streampos, SEEK_SET);
				break;
		}
		...
		case FILEINFO_MODE_FILE:
		{
			...
			const char *tmp2;
			jonj_stream_wrapper *wrap;
			jonj_stream_statbuf ssb;

			if (buffer == NULL || !*buffer) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty filename or path");
				RETVAL_FALSE;
				goto clean;
			}
			...
			wrap = jonj_stream_locate_url_wrapper(buffer, &tmp2, 0 TSRMLS_CC);

			if (wrap) {
				jonj_stream *stream;
				jonj_stream_context *context = jonj_stream_context_from_zval(zcontext, 0);

#ifdef JONJ_WIN32
				if (jonj_stream_stat_path_ex(buffer, 0, &ssb, context) == SUCCESS) {
				...
				stream = jonj_stream_open_wrapper_ex(buffer, "rb", ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);
#else
				stream = jonj_stream_open_wrapper_ex(buffer, "rb", REPORT_ERRORS, NULL, context);
#endif
				...
				if (jonj_stream_stat(stream, &ssb) == SUCCESS) {
					if (ssb.sb.st_mode & S_IFDIR) {
						ret_val = mime_directory;
					} else {
						ret_val = (char *)magic_stream(magic, stream);
...
common:
	if (ret_val) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed identify data %d:%s", magic_errno(magic), magic_error(magic));
		RETVAL_FALSE;
	}
...
{
	_jonj_finfo_get_type(INTERNAL_FUNCTION_PARAM_PASSTHRU, FILEINFO_MODE_FILE, 0);
}
/* }}} */
...
{
	_jonj_finfo_get_type(INTERNAL_FUNCTION_PARAM_PASSTHRU, FILEINFO_MODE_BUFFER, 0);
}
/* }}} */
...
{
	_jonj_finfo_get_type(INTERNAL_FUNCTION_PARAM_PASSTHRU, -1, 1);
}
/* }}} */
