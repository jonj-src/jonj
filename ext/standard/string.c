/*
   ...
   |          Stig S�ther Bakken <ssb@jonj.tk>                            |
 ...
 */
#define jonj_my_setlocale setlocale
#ifdef HAVE_LIBINTL
...
#define JONJ_PATHINFO_DIRNAME 	1
#define JONJ_PATHINFO_BASENAME 	2
#define JONJ_PATHINFO_EXTENSION 	4
#define JONJ_PATHINFO_FILENAME 	8
#define JONJ_PATHINFO_ALL	(JONJ_PATHINFO_DIRNAME | JONJ_PATHINFO_BASENAME | JONJ_PATHINFO_EXTENSION | JONJ_PATHINFO_FILENAME)
...
void register_string_constants(INIT_FUNC_ARGS)
{
	...
	REGISTER_LONG_CONSTANT("PATHINFO_DIRNAME", JONJ_PATHINFO_DIRNAME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_BASENAME", JONJ_PATHINFO_BASENAME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_EXTENSION", JONJ_PATHINFO_EXTENSION, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_FILENAME", JONJ_PATHINFO_FILENAME, CONST_CS | CONST_PERSISTENT);
...
int jonj_tag_find(char *tag, int len, char *set);
...
/* {{{ jonj_bin2hex
 */
static char *jonj_bin2hex(const unsigned char *old, const size_t oldlen, size_t *newlen)
{
...
/* {{{ jonj_hex2bin
 */
static char *jonj_hex2bin(const unsigned char *old, const size_t oldlen, size_t *newlen)
{
...
#if defined(JONJ_WIN32) && defined(ZTS)
	{
...
JONJ_FUNCTION(bin2hex)
{
	...
	result = jonj_bin2hex((unsigned char *)data, datalen, &newlen);
...
JONJ_FUNCTION(hex2bin)
{
	...
	if (datalen % 2 != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Hexadecimal input string must have an even length");
		RETURN_FALSE;
	}

	result = jonj_hex2bin((unsigned char *)data, datalen, &newlen);

	if (!result) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Input string must be hexadecimal string");
		RETURN_FALSE;
	}
...
static void jonj_spn_common_handler(INTERNAL_FUNCTION_PARAMETERS, int behavior) /* {{{ */
{
	char *s11, *s22;
	int len1, len2;
	long start = 0, len = 0;
	...
	if (behavior == STR_STRSPN) {
		RETURN_LONG(jonj_strspn(s11 + start /*str1_start*/,
						s22 /*str2_start*/,
						s11 + start + len /*str1_end*/,
						s22 + len2 /*str2_end*/));
	} else if (behavior == STR_STRCSPN) {
		RETURN_LONG(jonj_strcspn(s11 + start /*str1_start*/,
						s22 /*str2_start*/,
						s11 + start + len /*str1_end*/,
						s22 + len2 /*str2_end*/));
	}
...
{
	jonj_spn_common_handler(INTERNAL_FUNCTION_PARAM_PASSTHRU, STR_STRSPN);
}
/* }}} */
...
{
	jonj_spn_common_handler(INTERNAL_FUNCTION_PARAM_PASSTHRU, STR_STRCSPN);
}
/* }}} */
...
JONJ_FUNCTION(nl_langinfo)
{
	...
	switch(item) { /* {{{ */
		...
		case CODESET:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Item '%ld' is not valid", item);
			RETURN_FALSE;
	}
	/* }}} */
 ...
 */
static inline int jonj_charmask(unsigned char *input, int len, char *mask TSRMLS_DC)
{
	unsigned char *end;
	unsigned char c;
	int result = SUCCESS;
	...
	for (end = input+len; input < end; input++) {
		...
		if ((input+3 < end) && input[1] == '.' && input[2] == '.'
			...
			if (end-len >= input) { /* there was no 'left' char */
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, no character to the left of '..'");
				result = FAILURE;
				continue;
			}
			if (input+2 >= end) { /* there is no 'right' char */
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, no character to the right of '..'");
				result = FAILURE;
				continue;
			}
			if (input[-1] > input[2]) { /* wrong order */
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, '..'-range needs to be incrementing");
				result = FAILURE;
				continue;
			}
			/* FIXME: better error (a..b..c is the only left possibility?) */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range");
			result = FAILURE;
			continue;
		} else {
			mask[c]=1;
...
JONJAPI char *jonj_trim(char *c, int len, char *what, int what_len, zval *return_value, int mode TSRMLS_DC)
{
	...
	if (what) {
		jonj_charmask((unsigned char*)what, what_len, mask TSRMLS_CC);
	} else {
		jonj_charmask((unsigned char*)" \n\r\t\v\0", 6, mask TSRMLS_CC);
	}
 ...
 */
static void jonj_do_trim(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	char *str;
	char *what = NULL;
	...
	jonj_trim(str, str_len, what, what_len, return_value, mode TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(trim)
{
	jonj_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 3);
}
/* }}} */
...
JONJ_FUNCTION(rtrim)
{
	jonj_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */
...
JONJ_FUNCTION(ltrim)
{
	jonj_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(wordwrap)
{
	...
	if (breakcharlen == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Break string cannot be empty");
		RETURN_FALSE;
	}

	if (linelength == 0 && docut) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't force cut when width is zero");
		RETURN_FALSE;
	}
	...
	if (linelength > INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length should be between 0 and %d", INT_MAX);
		RETURN_FALSE;
	}
...
JONJAPI void jonj_explode(zval *delim, zval *str, zval *return_value, long limit)
{
	...
	p1 = Z_STRVAL_P(str);
	p2 = jonj_memnstr(Z_STRVAL_P(str), Z_STRVAL_P(delim), Z_STRLEN_P(delim), endp);
	...
	if (p2 == NULL) {
		...
		} while ((p2 = jonj_memnstr(p1, Z_STRVAL_P(delim), Z_STRLEN_P(delim), endp)) != NULL &&
				 --limit > 1);
...
/* {{{ jonj_explode_negative_limit
 */
JONJAPI void jonj_explode_negative_limit(zval *delim, zval *str, zval *return_value, long limit)
{
...
JONJ_FUNCTION(explode)
{
	...
	if (limit > 1) {
		jonj_explode(&zdelim, &zstr, return_value, limit);
	} else if (limit < 0) {
		jonj_explode_negative_limit(&zdelim, &zstr, return_value, limit);
	} else {
...
JONJ_FUNCTION(implode)
{
	...
	if (arg2 == NULL) {
		if (Z_TYPE_PP(arg1) != IS_ARRAY) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument must be an array");
			return;
		}
...
#define _IMPL_EMPTY ""
		...
		if (Z_TYPE_PP(arg1) == IS_ARRAY) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid arguments passed");
			return;
		}
	}

	jonj_implode(delim, arr, return_value TSRMLS_CC);
...
JONJAPI void jonj_basename(const char *s, size_t len, char *suffix, size_t sufflen, char **p_ret, size_t *p_len TSRMLS_DC)
{
	...
	state = 0;
	while (cnt > 0) {
		inc_len = (*c == '\0' ? 1: jonj_mblen(c, cnt));

		switch (inc_len) {
			case -2:
			case -1:
				inc_len = 1;
				jonj_ignore_value(jonj_mblen(NULL, 0));
				break;
			case 0:
				goto quit_loop;
			case 1:
...
JONJ_FUNCTION(basename)
{
	...
	jonj_basename(string, string_len, suffix, suffix_len, &ret, &ret_len TSRMLS_CC);
...
JONJAPI size_t jonj_dirname(char *path, size_t len)
{
...
JONJ_FUNCTION(dirname)
{
	...
	ret_len = jonj_dirname(ret, str_len);
...
JONJ_FUNCTION(pathinfo)
{
	...
	size_t ret_len;
	long opt = JONJ_PATHINFO_ALL;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l", &path, &path_len, &opt) == FAILURE) {
		return;
	}

	have_basename = ((opt & JONJ_PATHINFO_BASENAME) == JONJ_PATHINFO_BASENAME);

	MAKE_STD_ZVAL(tmp);
	array_init(tmp);

	if ((opt & JONJ_PATHINFO_DIRNAME) == JONJ_PATHINFO_DIRNAME) {
		ret = estrndup(path, path_len);
		jonj_dirname(ret, path_len);
		if (*ret) {
	...
	if (have_basename) {
		jonj_basename(path, path_len, NULL, 0, &ret, &ret_len TSRMLS_CC);
		add_assoc_stringl(tmp, "basename", ret, ret_len, 0);
	}

	if ((opt & JONJ_PATHINFO_EXTENSION) == JONJ_PATHINFO_EXTENSION) {
		const char *p;
		int idx;
	...
	if ((opt & JONJ_PATHINFO_FILENAME) == JONJ_PATHINFO_FILENAME) {
		const char *p;
		int idx;
...
{
	jonj_strtolower(s, s_len);
	jonj_strtolower(t, t_len);
	return jonj_memnstr(s, t, t_len, s + s_len);
}
/* }}} */
...
/* {{{ jonj_needle_char
 */
static int jonj_needle_char(zval *needle, char *target TSRMLS_DC)
{
	switch (Z_TYPE_P(needle)) {
		...
		default: {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "needle is not a string or an integer");
			return FAILURE;
		 }
	}
}
...
JONJ_FUNCTION(stristr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		if (!Z_STRLEN_P(needle)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty needle");
			efree(haystack_dup);
			RETURN_FALSE;
		}
		orig_needle = estrndup(Z_STRVAL_P(needle), Z_STRLEN_P(needle));
		found = jonj_stristr(haystack_dup, orig_needle,	haystack_len, Z_STRLEN_P(needle));
		efree(orig_needle);
	} else {
		if (jonj_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
		}
		needle_char[1] = 0;

		found = jonj_stristr(haystack_dup, needle_char,	haystack_len, 1);
	}
...
JONJ_FUNCTION(strstr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = jonj_memnstr(haystack, Z_STRVAL_P(needle), Z_STRLEN_P(needle), haystack + haystack_len);
	} else {
		if (jonj_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
		...
		found = jonj_memnstr(haystack, needle_char,	1, haystack + haystack_len);
	}
...
JONJ_FUNCTION(strpos)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = jonj_memnstr(haystack + offset,
			                Z_STRVAL_P(needle),
			                Z_STRLEN_P(needle),
...
JONJ_FUNCTION(stripos)
{
	...
	haystack_dup = estrndup(haystack, haystack_len);
	jonj_strtolower(haystack_dup, haystack_len);
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		jonj_strtolower(needle_dup, Z_STRLEN_P(needle));
		found = jonj_memnstr(haystack_dup + offset, needle_dup, Z_STRLEN_P(needle), haystack_dup + haystack_len);
	} else {
		if (jonj_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
		...
		needle_char[1] = '\0';
		found = jonj_memnstr(haystack_dup + offset,
							needle_char,
...
JONJ_FUNCTION(strrpos)
{
	...
	if (Z_TYPE_P(zneedle) == IS_STRING) {
		...
		if (jonj_needle_char(zneedle, ord_needle TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
		ord_needle[1] = '\0';
		needle = ord_needle;
...
JONJ_FUNCTION(strripos)
{
	...
	jonj_strtolower(needle_dup, needle_len);
	haystack_dup = estrndup(haystack, haystack_len);
	jonj_strtolower(haystack_dup, haystack_len);
...
JONJ_FUNCTION(strrchr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		char needle_chr;
		if (jonj_needle_char(needle, &needle_chr TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
...
/* {{{ jonj_chunk_split
 */
static char *jonj_chunk_split(char *src, int srclen, char *end, int endlen, int chunklen, int *destlen)
{
	char *dest;
	char *p, *q;
...
JONJ_FUNCTION(chunk_split)
{
	...
	if (chunklen <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Chunk length should be greater than zero");
		RETURN_FALSE;
	}
	...
	result = jonj_chunk_split(str, str_len, end, endlen, chunklen, &result_len);
...
JONJ_FUNCTION(substr_replace)
{
	...
	if (Z_TYPE_PP(str) == IS_STRING) {
		if (
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'from' and 'len' should be of same type - numerical or array ");
		...
		if (argc == 4 && Z_TYPE_PP(from) == IS_ARRAY) {
			if (zend_hash_num_elements(Z_ARRVAL_PP(from)) != zend_hash_num_elements(Z_ARRVAL_PP(len))) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'from' and 'len' should have the same number of elements");
				RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);
			}
		}
	}
	...
	if (Z_TYPE_PP(str) != IS_ARRAY) {
		if (Z_TYPE_PP(from) != IS_ARRAY) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Functionality of 'from' and 'len' as arrays is not implemented");
			RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);
		}
			...
			if (Z_TYPE_PP(repl) == IS_ARRAY) {
				if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(repl), (void **) &tmp_repl, &pos_repl)) {
					...
					if(Z_REFCOUNT_P(orig_str) != refcount) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument was modified while replacing");
						if(Z_TYPE_PP(tmp_repl) != IS_STRING) {
							zval_dtor(repl_str);
						}
						break;
...
static void jonj_ucfirst(char *str)
{
	register char *r;
	r = str;
...
JONJ_FUNCTION(ucfirst)
{
	...
	jonj_ucfirst(Z_STRVAL_P(return_value));
}
/* }}} */

/* {{{
   Lowercase the first character of the word in a native string */
static void jonj_lcfirst(char *str)
{
	register char *r;
	r = str;
...
JONJ_FUNCTION(lcfirst)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	jonj_lcfirst(Z_STRVAL_P(return_value));
}
/* }}} */
...
JONJ_FUNCTION(ucwords)
{
	...
	jonj_charmask((unsigned char *)delims, delims_len, mask TSRMLS_CC);
...
/* {{{ Definitions for jonj_strtr_array */
...
/* {{{ jonj_strtr_hash */
static inline HASH jonj_strtr_hash(const char *str, int len)
{
	HASH	res = 0;
	int		i;
	for (i = 0; i < len; i++) {
...
/* }}} */
/* {{{ jonj_strtr_populate_shift */
static inline void jonj_strtr_populate_shift(PATNREPL *patterns, int patnum, int B, STRLEN m, SHIFT_TAB *shift)
{
	int		i;
	STRLEN	j,
			max_shift;
	...
	for (i = 0; i < patnum; i++) {
		for (j = 0; j < m - B + 1; j++) {
			HASH h = jonj_strtr_hash(&S(&patterns[i].pat)[j], B) & shift->table_mask;
...
/* }}} */
/* {{{ jonj_strtr_compare_hash_suffix */
static int jonj_strtr_compare_hash_suffix(const void *a, const void *b TSRMLS_DC, void *ctx_g)
{
	const PPRES		*res = ctx_g;
	const PATNREPL	*pnr_a = a,
					*pnr_b = b;
	HASH			hash_a = jonj_strtr_hash(&S(&pnr_a->pat)[res->m - res->B], res->B)
								& res->hash->table_mask,
					hash_b = jonj_strtr_hash(&S(&pnr_b->pat)[res->m - res->B], res->B)
								& res->hash->table_mask;
...
/* }}} */
/* {{{ jonj_strtr_free_strp */
static void jonj_strtr_free_strp(void *strp)
{
	STR_FREE(*(char**)strp);
}
/* }}} */
/* {{{ jonj_strtr_array_prepare_repls */
static PATNREPL *jonj_strtr_array_prepare_repls(int slen, HashTable *pats, zend_llist **allocs, int *outsize)
{
	PATNREPL		*patterns;
	HashPosition	hpos;
	zval			**entry;
	...
	*allocs = emalloc(sizeof **allocs);
	zend_llist_init(*allocs, sizeof(void*), &jonj_strtr_free_strp, 0);
...
/* {{{ PPRES *jonj_strtr_array_prepare(STR *text, PATNREPL *patterns, int patnum, int B, int Bp) */
static PPRES *jonj_strtr_array_prepare(STR *text, PATNREPL *patterns, int patnum, int B, int Bp)
{
	int		i;
	...
	jonj_strtr_populate_shift(patterns, patnum, B, res->m, res->shift);
	...
	zend_qsort_r(res->patterns, patnum, sizeof(*res->patterns),
			jonj_strtr_compare_hash_suffix, res, NULL); /* tsrmls not needed */
#else
	zend_qsort_r(res->patterns, patnum, sizeof(*res->patterns),
			jonj_strtr_compare_hash_suffix, res);
#endif

	res->prefix = safe_emalloc(patnum, sizeof(*res->prefix), 0);
	for (i = 0; i < patnum; i++) {
		res->prefix[i] = jonj_strtr_hash(S(&res->patterns[i].pat), Bp);
	}
	...
	}
	{
		...
		for (i = 0; i < patnum; i++) {
			HASH h = jonj_strtr_hash(&S(&res->patterns[i].pat)[res->m - res->B], res->B)
						& res->hash->table_mask;
			if (h != last_h) {
...
/* }}} */
/* {{{ jonj_strtr_array_destroy_ppres(PPRES *d) */
static void jonj_strtr_array_destroy_ppres(PPRES *d)
{
	efree(d->shift);
	efree(d->hash);
	efree(d->prefix);
...
/* {{{ jonj_strtr_array_do_repl(STR *text, PPRES *d, zval *return_value) */
static void jonj_strtr_array_do_repl(STR *text, PPRES *d, zval *return_value)
{
	STRLEN		pos = 0,
				nextwpos = 0,
				lastpos	= L(text) - d->m;
	...
	while (pos <= lastpos) {
		HASH	h		= jonj_strtr_hash(&S(text)[pos + d->m - d->B], d->B) & d->shift->table_mask;
		...
		if (shift > 0) {
			...
			HASH	h2				= h & d->hash->table_mask,
					prefix_h		= jonj_strtr_hash(&S(text)[pos], d->Bp);
...
/* {{{ jonj_strtr_array */
static void jonj_strtr_array(zval *return_value, char *str, int slen, HashTable *pats)
{
	PPRES		*data;
	STR			text;
	PATNREPL	*patterns;
	...
	patterns = jonj_strtr_array_prepare_repls(slen, pats, &allocs, &patterns_len);
	if (patterns == NULL) {
		RETURN_FALSE;
	}
	if (patterns_len == 0) {
	...
	data = jonj_strtr_array_prepare(&text, patterns, patterns_len, 2, 2);
	efree(patterns);
	jonj_strtr_array_do_repl(&text, data, return_value);
	jonj_strtr_array_destroy_ppres(data);
	zend_llist_destroy(allocs);
	efree(allocs);
}
/* }}} */
...
JONJ_FUNCTION(strtr)
{
	...
	if (ac == 2 && Z_TYPE_PP(from) != IS_ARRAY) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The second argument is not an array");
		RETURN_FALSE;
	}
	...
	if (ac == 2) {
		jonj_strtr_array(return_value, str, str_len, HASH_OF(*from));
	} else {
		convert_to_string_ex(from);

		ZVAL_STRINGL(return_value, str, str_len, 1);

		jonj_strtr(Z_STRVAL_P(return_value),
...
/* {{{ jonj_similar_str
 */
static void jonj_similar_str(const char *txt1, int len1, const char *txt2, int len2, int *pos1, int *pos2, int *max)
{
	char *p, *q;
...
/* {{{ jonj_similar_char
 */
static int jonj_similar_char(const char *txt1, int len1, const char *txt2, int len2)
{
	int sum;
	int pos1 = 0, pos2 = 0, max;

	jonj_similar_str(txt1, len1, txt2, len2, &pos1, &pos2, &max);
	if ((sum = max)) {
		if (pos1 && pos2) {
			sum += jonj_similar_char(txt1, pos1,
									txt2, pos2);
		}
		if ((pos1 + max < len1) && (pos2 + max < len2)) {
			sum += jonj_similar_char(txt1 + pos1 + max, len1 - pos1 - max,
...
JONJ_FUNCTION(similar_text)
{
	...
	sim = jonj_similar_char(t1, t1_len, t2, t2_len);
...
JONJ_FUNCTION(addcslashes)
{
	...
	Z_STRVAL_P(return_value) = jonj_addcslashes(str, str_len, &Z_STRLEN_P(return_value), 0, what, what_len TSRMLS_CC);
...
JONJ_FUNCTION(addslashes)
{
	...
	RETURN_STRING(jonj_addslashes(str,
	                             str_len,
...
JONJ_FUNCTION(stripcslashes)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	jonj_stripcslashes(Z_STRVAL_P(return_value), &Z_STRLEN_P(return_value));
}
/* }}} */
...
JONJ_FUNCTION(stripslashes)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	jonj_stripslashes(Z_STRVAL_P(return_value), &Z_STRLEN_P(return_value) TSRMLS_CC);
}
/* }}} */
...
char *jonj_strerror(int errnum)
{
	...
	(void) snprintf(BG(str_ebuf), sizeof(jonj_basic_globals.str_ebuf), "Unknown error: %d", errnum);
	return(BG(str_ebuf));
}
/* }}} */
#endif
...
/* {{{ jonj_addcslashes
 */
JONJAPI char *jonj_addcslashes(const char *str, int length, int *new_length, int should_free, char *what, int wlength TSRMLS_DC)
{
	char flags[256];
	...
	jonj_charmask((unsigned char *)what, wlength, flags TSRMLS_CC);
...
/* {{{ jonj_addslashes
 */
JONJAPI char *jonj_addslashes(char *str, int length, int *new_length, int should_free TSRMLS_DC)
{
...
{
	return jonj_char_to_str_ex(str, len, from, to, to_len, result, 1, NULL);
}
/* }}} */

/* {{{ jonj_str_to_str_ex
 */
JONJAPI char *jonj_str_to_str_ex(char *haystack, int length,
		...
		if (needle_len == str_len) {
			...
			*_new_length = length;
			...
			if (case_sensitivity) {
				...
				for (p = new_str; (r = jonj_memnstr(p, needle, needle_len, end)); p = r + needle_len) {
				...
				needle_dup = estrndup(needle, needle_len);
				jonj_strtolower(haystack_dup, length);
				jonj_strtolower(needle_dup, needle_len);
				end = haystack_dup + length;
				for (p = haystack_dup; (r = jonj_memnstr(p, needle_dup, needle_len, end)); p = r + needle_len) {
			...
			if (str_len < needle_len) {
				...
				while ((o = jonj_memnstr(o, n, needle_len, endp))) {
					o += needle_len;
					count++;
				}
				if (count == 0) {
			...
			if (case_sensitivity) {
				...
				for (p = haystack; (r = jonj_memnstr(p, needle, needle_len, end)); p = r + needle_len) {
					memcpy(e, p, r - p);
					e += r - p;
...
nothing_todo:
		*_new_length = length;
		...
		if (case_sensitivity && memcmp(haystack, needle, length)) {
			...
			jonj_strtolower(l_haystack, length);
			jonj_strtolower(l_needle, length);
...
/* {{{ jonj_str_to_str
 */
JONJAPI char *jonj_str_to_str(char *haystack, int length,
	char *needle, int needle_len, char *str, int str_len, int *_new_length)
{
	return jonj_str_to_str_ex(haystack, length, needle, needle_len, str, str_len, _new_length, 1, NULL);
}
/* }}} */

/* {{{ jonj_str_replace_in_subject
 */
static void jonj_str_replace_in_subject(zval *search, zval *replace, zval **subject, zval *result, int case_sensitivity, int *replace_count)
{
	zval		**search_entry,
				**replace_entry = NULL,
				  temp_result;
	...
	if (Z_TYPE_P(search) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(search), (void **) &search_entry) == SUCCESS) {
			...
			if (Z_STRLEN_PP(search_entry) == 1) {
				jonj_char_to_str_ex(Z_STRVAL_P(result),
								Z_STRLEN_P(result),
				...
				Z_STRVAL(temp_result) = jonj_str_to_str_ex(Z_STRVAL_P(result), Z_STRLEN_P(result),
		...
		if (Z_STRLEN_P(search) == 1) {
			jonj_char_to_str_ex(Z_STRVAL_PP(subject),
							Z_STRLEN_PP(subject),
							Z_STRVAL_P(search)[0],
							Z_STRVAL_P(replace),
							Z_STRLEN_P(replace),
			...
			Z_STRVAL_P(result) = jonj_str_to_str_ex(Z_STRVAL_PP(subject), Z_STRLEN_PP(subject),
...
/* {{{ jonj_str_replace_common
 */
static void jonj_str_replace_common(INTERNAL_FUNCTION_PARAMETERS, int case_sensitivity)
{
	...
	if (Z_TYPE_PP(subject) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_PP(subject), (void **)&subject_entry) == SUCCESS) {
			if (Z_TYPE_PP(subject_entry) != IS_ARRAY && Z_TYPE_PP(subject_entry) != IS_OBJECT) {
				...
				jonj_str_replace_in_subject(*search, *replace, subject_entry, result, case_sensitivity, (argc > 3) ? &count : NULL);
			} else {
				ALLOC_ZVAL(result);
		...
		jonj_str_replace_in_subject(*search, *replace, subject, return_value, case_sensitivity, (argc > 3) ? &count : NULL);
	}
	if (argc > 3) {
		zval_dtor(*zcount);
...
{
	jonj_str_replace_common(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	jonj_str_replace_common(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
 ...
 * Cheers/complaints/flames - Zeev Suraski <zeev@jonj.tk>
 */
static void jonj_hebrev(INTERNAL_FUNCTION_PARAMETERS, int convert_newlines)
{
	char *str;
	...
	*target = 0;
	...
	if (convert_newlines) {
		jonj_char_to_str(broken_str, str_len,'\n', "<br />\n", 7, return_value);
		efree(broken_str);
	} else {
...
{
	jonj_hebrev(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_hebrev(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
/* {{{ proto string strip_tags(string str [, string allowable_tags])
   Strips HTML and JONJ tags from a string */
JONJ_FUNCTION(strip_tags)
{
	...
	retval_len = jonj_strip_tags_ex(buf, str_len, NULL, allowed_tags, allowed_tags_len, 0);
...
JONJ_FUNCTION(setlocale)
{
	...
	if (Z_TYPE_PP(pcategory) == IS_LONG) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_DEPRECATED, "Passing locale category name as string is deprecated. Use the LC_* -constants instead");
		...
		if (!strcasecmp("LC_ALL", category)) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid locale category name %s, must be one of LC_ALL, LC_COLLATE, LC_CTYPE, LC_MONETARY, LC_NUMERIC, or LC_TIME", category);
	...
	while (1) {
		...
		if (!strcmp ("0", Z_STRVAL_PP(plocale))) {
			...
			if (Z_STRLEN_PP(plocale) >= 255) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Specified locale name is too long");
				break;
			}
		}

		retval = jonj_my_setlocale(cat, loc);
...
JONJAPI size_t jonj_strip_tags(char *rbuf, int len, int *stateptr, char *allow, int allow_len) /* {{{ */
{
	return jonj_strip_tags_ex(rbuf, len, stateptr, allow, allow_len, 0);
}
/* }}} */

/* {{{ jonj_strip_tags

	A simple little state-machine to strip out html and jonj tags

	State 0 is the output state, State 1 means we are inside a
	normal html tag and state 2 means we are inside a jonj tag.
	...
	swm: Added ability to strip <?xml tags without assuming it JONJ
	code.
*/
JONJAPI size_t jonj_strip_tags_ex(char *rbuf, int len, int *stateptr, char *allow, int allow_len, zend_bool allow_tag_spaces)
{
	...
	if (allow) {
		if (IS_INTERNED(allow)) {
			...
			allow_free = NULL;
			jonj_strtolower(allow, allow_len);
		}
		tbuf = emalloc(JONJ_TAG_BUF_SIZE + 1);
		tp = tbuf;
	} else {
		tbuf = tp = NULL;
	}
		...
		switch (c) {
			...
			case '<':
				...
				if (state == 0) {
					...
					state = 1;
					if (allow) {
						if (tp - tbuf >= JONJ_TAG_BUF_SIZE) {
							pos = tp - tbuf;
							tbuf = erealloc(tbuf, (tp - tbuf) + JONJ_TAG_BUF_SIZE + 1);
							tp = tbuf + pos;
						}
						*(tp++) = '<';
				 	}
				...
				switch (state) {
					case 1: /* HTML/XML */
						...
						if (allow) {
							...
							*(tp++) = '>';
							*tp='\0';
							if (jonj_tag_find(tbuf, tp-tbuf, allow)) {
			...
			case 'L':
				 ...
				 * state == 2 (JONJ). Switch back to HTML.
				 */
...
JONJ_FUNCTION(str_getcsv)
{
	...
	jonj_fgetcsv(NULL, delim, enc, esc, str_len, str, return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(str_repeat)
{
	...
	if (mult < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Second argument has to be greater than or equal to 0");
		return;
	}
	...
	if(result_len > INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Result is too big, maximum %d allowed", INT_MAX);
		RETURN_EMPTY_STRING();
	}
...
JONJ_FUNCTION(count_chars)
{
	...
	if (mymode < 0 || mymode > 4) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown mode");
		RETURN_FALSE;
	}
...
/* {{{ jonj_strnatcmp
 */
static void jonj_strnatcmp(INTERNAL_FUNCTION_PARAMETERS, int fold_case)
{
	char *s1, *s2;
	int s1_len, s2_len;
...
{
	jonj_strnatcmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_strnatcmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(substr_count)
{
	...
	if (offset < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset should be greater than or equal to 0");
		RETURN_FALSE;
	}

	if (offset > haystack_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset value %ld exceeds string length", offset);
		RETURN_FALSE;
	}
	p += offset;
	...
	if (ac == 4) {
		...
		if (length <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length should be greater than 0");
			RETURN_FALSE;
		}
		if (length > (haystack_len - offset)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length value %ld exceeds string length", length);
			RETURN_FALSE;
		}
		endp = p + length;
	}
	...
	if (needle_len == 1) {
		...
		while ((p = jonj_memnstr(p, needle, needle_len, endp))) {
			p += needle_len;
			count++;
		}
	}
...
JONJ_FUNCTION(str_pad)
{
	...
	if (pad_str_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding string cannot be empty");
		return;
	}

	if (pad_type_val < STR_PAD_LEFT || pad_type_val > STR_PAD_BOTH) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding type has to be STR_PAD_LEFT, STR_PAD_RIGHT, or STR_PAD_BOTH");
		return;
	}

	num_pad_chars = pad_length - input_len;
	if (num_pad_chars >= INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding length is too long");
		return;
	}
...
JONJ_FUNCTION(sscanf)
{
	...
	result = jonj_sscanf_internal(str, format, num_args, args, 0, &return_value TSRMLS_CC);
...
JONJ_FUNCTION(str_rot13)
{
	...
	jonj_strtr(Z_STRVAL_P(return_value), Z_STRLEN_P(return_value), rot13_from, rot13_to, 52);
}
/* }}} */

static void jonj_string_shuffle(char *str, long len TSRMLS_DC) /* {{{ */
{
	...
	while (--n_left) {
		rnd_idx = jonj_rand(TSRMLS_C);
		RAND_RANGE(rnd_idx, 0, n_left, JONJ_RAND_MAX);
		if (rnd_idx != n_left) {
			temp = str[n_left];
...
JONJ_FUNCTION(str_shuffle)
{
	...
	if (Z_STRLEN_P(return_value) > 1) {
		jonj_string_shuffle(Z_STRVAL_P(return_value), (long) Z_STRLEN_P(return_value) TSRMLS_CC);
	}
}
/* }}} */
...
JONJ_FUNCTION(str_word_count)
{
	...
	switch(type) {
		...
		case 0:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid format value %ld", type);
			RETURN_FALSE;
	}

	if (char_list) {
		jonj_charmask((unsigned char *)char_list, char_list_len, ch TSRMLS_CC);
	}
...
JONJ_FUNCTION(money_format)
{
	...
	while ((p = memchr(p, '%', (e - p)))) {
		if (*(p + 1) == '%') {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Only a single %%i or %%n token can be used");
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(str_split)
{
	...
	if (split_length <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The length of each segment must be greater than zero");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(strpbrk)
{
	...
	if (!char_list_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The character list cannot be empty");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(substr_compare)
{
	...
	if (ZEND_NUM_ARGS() >= 4 && len <= 0) {
		if (len == 0) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The length must be greater than or equal to zero");
			RETURN_FALSE;
		}
	}
	...
	if (offset >= s1_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The start position cannot exceed initial string length");
		RETURN_FALSE;
	}
