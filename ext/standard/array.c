...
/* {{{ jonj_array_init_globals
*/
static void jonj_array_init_globals(zend_array_globals *array_globals)
{
...
{
	ZEND_INIT_MODULE_GLOBALS(array, jonj_array_init_globals, NULL);
	...
	REGISTER_LONG_CONSTANT("SORT_ASC", JONJ_SORT_ASC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_DESC", JONJ_SORT_DESC, CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("SORT_REGULAR", JONJ_SORT_REGULAR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_NUMERIC", JONJ_SORT_NUMERIC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_STRING", JONJ_SORT_STRING, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_LOCALE_STRING", JONJ_SORT_LOCALE_STRING, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_NATURAL", JONJ_SORT_NATURAL, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_FLAG_CASE", JONJ_SORT_FLAG_CASE, CONST_CS | CONST_PERSISTENT);
...
static void jonj_set_compare_func(int sort_type TSRMLS_DC) /* {{{ */
{
	switch (sort_type & ~JONJ_SORT_FLAG_CASE) {
		case JONJ_SORT_NUMERIC:
			ARRAYG(compare_func) = numeric_compare_function;
			break;

		case JONJ_SORT_STRING:
			ARRAYG(compare_func) = sort_type & JONJ_SORT_FLAG_CASE ? string_case_compare_function : string_compare_function;
			break;

		case JONJ_SORT_NATURAL:
			ARRAYG(compare_func) = sort_type & JONJ_SORT_FLAG_CASE ? string_natural_case_compare_function : string_natural_compare_function;
			break;

#if HAVE_STRCOLL
		case JONJ_SORT_LOCALE_STRING:
			ARRAYG(compare_func) = string_locale_compare_function;
			break;
#endif

		case JONJ_SORT_REGULAR:
		default:
...
static int jonj_array_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval result;
...
static int jonj_array_reverse_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return jonj_array_key_compare(a, b TSRMLS_CC) * -1;
}
/* }}} */
...
{
	zval *array;
	long sort_type = JONJ_SORT_REGULAR;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a|l", &array, &sort_type) == FAILURE) {
		RETURN_FALSE;
	}

	jonj_set_compare_func(sort_type TSRMLS_CC);

	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_reverse_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(ksort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJAPI int jonj_count_recursive(zval *array, long mode TSRMLS_DC) /* {{{ */
{
	...
	if (Z_TYPE_P(array) == IS_ARRAY) {
		if (Z_ARRVAL_P(array)->nApplyCount > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "recursion detected");
			return 0;
		}
		...
		if (mode == COUNT_RECURSIVE) {
			...
			for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(array), &pos);
				...
				cnt += jonj_count_recursive(*element, COUNT_RECURSIVE TSRMLS_CC);
				Z_ARRVAL_P(array)->nApplyCount--;
			}
		}
	}
...
JONJ_FUNCTION(count)
{
	...
	switch (Z_TYPE_P(array)) {
		case IS_NULL:
			...
			break;
		case IS_ARRAY:
			RETURN_LONG (jonj_count_recursive (array, mode TSRMLS_CC));
			break;
		case IS_OBJECT: {
#ifdef HAVE_SPL
			zval *retval;
 ...
 */
static int jonj_array_data_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval result;
...
static int jonj_array_reverse_data_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return jonj_array_data_compare(a, b TSRMLS_CC) * -1;
}
/* }}} */

static int jonj_array_natural_general_compare(const void *a, const void *b, int fold_case) /* {{{ */
{
	Bucket *f, *s;
	zval *fval, *sval;
	zval first, second;
...
static int jonj_array_natural_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return jonj_array_natural_general_compare(a, b, 0);
}
/* }}} */

static int jonj_array_natural_case_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return jonj_array_natural_general_compare(a, b, 1);
}
/* }}} */

static void jonj_natsort(INTERNAL_FUNCTION_PARAMETERS, int fold_case) /* {{{ */
{
	zval *array;
	...
	if (fold_case) {
		if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_natural_case_compare, 0 TSRMLS_CC) == FAILURE) {
			return;
		}
	} else {
		if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_natural_compare, 0 TSRMLS_CC) == FAILURE) {
			return;
		}
	}
...
{
	jonj_natsort(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_natsort(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(asort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_data_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(arsort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_reverse_data_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(sort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_data_compare, 1 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJ_FUNCTION(rsort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_reverse_data_compare, 1 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
static int jonj_array_user_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval **args[2];
...
#define JONJ_ARRAY_CMP_FUNC_CHECK(func_name)	\
	if (!zend_is_callable(*func_name, 0, NULL TSRMLS_CC)) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid comparison function");	\
...
#define JONJ_ARRAY_CMP_FUNC_VARS \
	zend_fcall_info old_user_compare_fci; \
	zend_fcall_info_cache old_user_compare_fci_cache \

#define JONJ_ARRAY_CMP_FUNC_BACKUP() \
	old_user_compare_fci = BG(user_compare_fci); \
	old_user_compare_fci_cache = BG(user_compare_fci_cache); \
	BG(user_compare_fci_cache) = empty_fcall_info_cache; \

#define JONJ_ARRAY_CMP_FUNC_RESTORE() \
	BG(user_compare_fci) = old_user_compare_fci; \
	BG(user_compare_fci_cache) = old_user_compare_fci_cache; \
...
JONJ_FUNCTION(usort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_user_compare, 1 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
			RETVAL_FALSE;
		} else {
			RETVAL_TRUE;
		}
...
JONJ_FUNCTION(uasort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_user_compare, 0 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
...
static int jonj_array_user_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval *key1, *key2;
...
JONJ_FUNCTION(uksort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, jonj_array_user_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
...
JONJ_FUNCTION(min)
{
	...
	jonj_set_compare_func(JONJ_SORT_REGULAR TSRMLS_CC);
	...
	if (argc == 1) {
		...
		if (Z_TYPE_PP(args[0]) != IS_ARRAY) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "When only one parameter is given, it must be an array");
			RETVAL_NULL();
		} else {
			if (zend_hash_minmax(Z_ARRVAL_PP(args[0]), jonj_array_data_compare, 0, (void **) &result TSRMLS_CC) == SUCCESS) {
				RETVAL_ZVAL_FAST(*result);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array must contain at least one element");
				RETVAL_FALSE;
			}
		}
	} else {
...
JONJ_FUNCTION(max)
{
	...
	if (argc == 1) {
		...
		if (Z_TYPE_PP(args[0]) != IS_ARRAY) {
			...
			if (zend_hash_minmax(Z_ARRVAL_PP(args[0]), jonj_array_data_compare, 1, (void **) &result TSRMLS_CC) == SUCCESS) {
				RETVAL_ZVAL_FAST(*result);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array must contain at least one element");
				RETVAL_FALSE;
...
static int jonj_array_walk(HashTable *target_hash, zval *userdata, int recursive TSRMLS_DC) /* {{{ */
{
	...
	while (!EG(exception) && zend_hash_get_current_data(target_hash, (void **)&args[0]) == SUCCESS) {
		if (recursive && Z_TYPE_PP(args[0]) == IS_ARRAY) {
			...
			thash->nApplyCount++;
			jonj_array_walk(thash, userdata, recursive TSRMLS_CC);
			thash->nApplyCount--;
...
JONJ_FUNCTION(array_walk)
{
	...
	jonj_array_walk(array, userdata, 0 TSRMLS_CC);
	BG(array_walk_fci) = orig_array_walk_fci;
	BG(array_walk_fci_cache) = orig_array_walk_fci_cache;
	RETURN_TRUE;
}
...
JONJ_FUNCTION(array_walk_recursive)
{
	...
	jonj_array_walk(array, userdata, 1 TSRMLS_CC);
	BG(array_walk_fci) = orig_array_walk_fci;
	BG(array_walk_fci_cache) = orig_array_walk_fci_cache;
	RETURN_TRUE;
}
...
/* void jonj_search_array(INTERNAL_FUNCTION_PARAMETERS, int behavior)
 * 0 = return boolean
 * 1 = return key
 */
static void jonj_search_array(INTERNAL_FUNCTION_PARAMETERS, int behavior) /* {{{ */
{
...
{
	jonj_search_array(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	jonj_search_array(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

static int jonj_valid_var_name(char *var_name, int var_name_len) /* {{{ */
{
	int i, ch;
...
JONJ_FUNCTION(extract)
{
	...
	if (extract_type < EXTR_OVERWRITE || extract_type > EXTR_IF_EXISTS) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid extract type");
		return;
	}

	if (extract_type > EXTR_SKIP && extract_type <= EXTR_PREFIX_IF_EXISTS && ZEND_NUM_ARGS() < 3) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "specified extract type requires the prefix parameter");
		return;
	}

	if (prefix) {
		convert_to_string(prefix);
		if (Z_STRLEN_P(prefix) && !jonj_valid_var_name(Z_STRVAL_P(prefix), Z_STRLEN_P(prefix))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "prefix is not a valid identifier");
			return;
		}
	}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(var_array), (void **)&entry, &pos) == SUCCESS) {
		...
		if (key_type == HASH_KEY_IS_STRING) {
			...
			jonj_prefix_varname(&final_name, prefix, Z_STRVAL(num), Z_STRLEN(num), 1 TSRMLS_CC);
			zval_dtor(&num);
		} else {
		...
		switch (extract_type) {
			...
			case EXTR_PREFIX_IF_EXISTS:
				if (var_exists) {
					jonj_prefix_varname(&final_name, prefix, var_name, var_name_len, 1 TSRMLS_CC);
				}
				break;
			...
			case EXTR_PREFIX_INVALID:
				if (Z_TYPE(final_name) == IS_NULL) {
					if (!jonj_valid_var_name(var_name, var_name_len)) {
						jonj_prefix_varname(&final_name, prefix, var_name, var_name_len, 1 TSRMLS_CC);
					} else {
						ZVAL_STRINGL(&final_name, var_name, var_name_len, 1);
					}
		...
		if (Z_TYPE(final_name) != IS_NULL && jonj_valid_var_name(Z_STRVAL(final_name), Z_STRLEN(final_name))) {
			if (extract_refs) {
				zval **orig_var;
...
static void jonj_compact_var(HashTable *eg_active_symbol_table, zval *return_value, zval *entry TSRMLS_DC) /* {{{ */
{
	...
	if (Z_TYPE_P(entry) == IS_STRING) {
		...
		while (zend_hash_get_current_data_ex(Z_ARRVAL_P(entry), (void**)&value_ptr, &pos) == SUCCESS) {
			...
			jonj_compact_var(eg_active_symbol_table, return_value, value TSRMLS_CC);
...
JONJ_FUNCTION(compact)
{
	...
	for (i=0; i<ZEND_NUM_ARGS(); i++) {
		jonj_compact_var(EG(active_symbol_table), return_value, *args[i] TSRMLS_CC);
	}
...
JONJ_FUNCTION(array_fill)
{
	...
	if (num < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of elements can't be negative");
		RETURN_FALSE;
	}
	...
	while (num--) {
		if (zend_hash_next_index_insert(Z_ARRVAL_P(return_value), &val, sizeof(zval *), NULL) == SUCCESS) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add element to the array as the next element is already occupied");
			RETURN_FALSE;
		}
	}
}
...
err:
	if (err) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "step exceeds the specified range");
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
static void jonj_array_data_shuffle(zval *array TSRMLS_DC) /* {{{ */
{
	Bucket **elems, *temp;
	HashTable *hash;
...
JONJ_FUNCTION(shuffle)
{
	...
	jonj_array_data_shuffle(array TSRMLS_CC);
...
/* {{{ void _jonji_pop(INTERNAL_FUNCTION_PARAMETERS, int off_the_end) */
static void _jonji_pop(INTERNAL_FUNCTION_PARAMETERS, int off_the_end)
{
...
{
	_jonji_pop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	_jonji_pop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
JONJ_FUNCTION(array_unshift)
{
	...
	jonj_splice(Z_ARRVAL_P(stack), 0, 0, args, argc, NULL TSRMLS_CC);
...
JONJ_FUNCTION(array_splice)
{
	...
	/* Perform splice */
	jonj_splice(Z_ARRVAL_P(array), offset, length, repl, repl_num, rem_hash TSRMLS_CC);
			...
			case HASH_KEY_IS_STRING:
				if (recursive && zend_hash_find(dest, string_key, string_key_len, (void **)&dest_entry) == SUCCESS) {
					...
					if (Z_TYPE_P(src_zval) == IS_ARRAY) {
						...
						if (!jonj_array_merge(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_P(src_zval), recursive TSRMLS_CC)) {
							if (thash) {
								thash->nApplyCount--;
							}
							return 0;
...
JONJAPI int jonj_array_replace_recursive(HashTable *dest, HashTable *src TSRMLS_DC) /* {{{ */
{
	...
	for (zend_hash_internal_pointer_reset_ex(src, &pos);
		...
		if (!jonj_array_replace_recursive(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_PP(src_entry) TSRMLS_CC)) {
			Z_ARRVAL_PP(dest_entry)->nApplyCount--;
			Z_ARRVAL_PP(src_entry)->nApplyCount--;
			return 0;
		}
...
static void jonj_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAMETERS, int recursive, int replace) /* {{{ */
{
	zval ***args = NULL;
	...
	for (i = 0; i < argc; i++) {
		if (Z_TYPE_PP(args[i]) != IS_ARRAY) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is not an array", i + 1);
			efree(args);
			RETURN_NULL();
		} else {
	...
	for (i = 0; i < argc; i++) {
		if (!replace) {
			jonj_array_merge(Z_ARRVAL_P(return_value), Z_ARRVAL_PP(args[i]), recursive TSRMLS_CC);
		} else if (recursive && i > 0) { /* First array will be copied directly instead */
			jonj_array_replace_recursive(Z_ARRVAL_P(return_value), Z_ARRVAL_PP(args[i]) TSRMLS_CC);
		} else {
...
{
	jonj_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */
...
{
	jonj_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1, 0);
}
/* }}} */
...
{
	jonj_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 1);
}
/* }}} */
...
{
	jonj_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1, 1);
}
/* }}} */
...
JONJ_FUNCTION(array_count_values)
{
	...
	while (zend_hash_get_current_data_ex(myht, (void **)&entry, &pos) == SUCCESS) {
		if (Z_TYPE_PP(entry) == IS_LONG) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can only count STRING and INTEGER values!");
		}
...
zend_bool array_column_param_helper(zval **param,
	...
	switch (Z_TYPE_PP(param)) {
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The %s key should be either a string or an integer", name);
			return 0;
	}
}
...
JONJ_FUNCTION(array_pad)
{
	...
	if (pad_size_abs < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "You may only pad up to 1048576 elements at a time");
		zval_dtor(return_value);
		RETURN_FALSE;
	}
	...
	if (pad_size > 0) {
		jonj_splice(Z_ARRVAL_P(return_value), input_size, 0, pads, num_pads, NULL TSRMLS_CC);
	} else {
		jonj_splice(Z_ARRVAL_P(return_value), 0, 0, pads, num_pads, NULL TSRMLS_CC);
	}
...
JONJ_FUNCTION(array_flip)
{
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(array), (void **)&entry, &pos) == SUCCESS) {
		...
		if (Z_TYPE_PP(entry) == IS_LONG) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can only flip STRING and INTEGER values!");
		}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(array), (void **)&entry, &pos) == SUCCESS) {
		...
		switch (zend_hash_get_current_key_ex(Z_ARRVAL_P(array), &string_key, &str_key_len, &num_key, 0, &pos)) {
			...
			case HASH_KEY_IS_STRING:
				...
				if (change_to_upper) {
					jonj_strtoupper(new_key, str_key_len - 1);
				} else {
					jonj_strtolower(new_key, str_key_len - 1);
				}
...
JONJ_FUNCTION(array_unique)
{
	...
	unsigned int i;
	long sort_type = JONJ_SORT_STRING;
	...
	arTmp[i].b = NULL;
	zend_qsort((void *) arTmp, i, sizeof(struct bucketindex), jonj_array_data_compare TSRMLS_CC);

	/* go through the sorted array and delete duplicates from the copy */
	lastkept = arTmp;
	for (cmpdata = arTmp + 1; cmpdata->b; cmpdata++) {
		if (jonj_array_data_compare(lastkept, cmpdata TSRMLS_CC)) {
			lastkept = cmpdata;
		} else {
...
static void jonj_array_intersect_key(INTERNAL_FUNCTION_PARAMETERS, int data_compare_type) /* {{{ */
{
	Bucket *p;
	int argc, i;
	zval ***args;
	...
	if (argc < req_args) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "at least %d parameters are required, %d given", req_args, argc);
		return;
	}
...
static void jonj_array_intersect(INTERNAL_FUNCTION_PARAMETERS, int behavior, int data_compare_type, int key_compare_type) /* {{{ */
{
	zval ***args = NULL;
	HashTable *hash;
	int arr_argc, i, c = 0;
	...
	if (behavior == INTERSECT_NORMAL) {
		intersect_key_compare_func = jonj_array_key_compare;

		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL) {
			/* array_intersect() */
			req_args = 2;
			param_spec = "+";
			intersect_data_compare_func = jonj_array_data_compare;
		} else if (data_compare_type == INTERSECT_COMP_DATA_USER) {
			/* array_uintersect() */
			req_args = 3;
			param_spec = "+f";
			intersect_data_compare_func = jonj_array_user_compare;
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. This should never happen. Please report as a bug", data_compare_type);
			return;
		}

		if (ZEND_NUM_ARGS() < req_args) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "at least %d parameters are required, %d given", req_args, ZEND_NUM_ARGS());
			return;
		}
		...
		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL && key_compare_type == INTERSECT_COMP_KEY_INTERNAL) {
			...
			req_args = 3;
			param_spec = "+f";
			intersect_key_compare_func = jonj_array_user_key_compare;
			intersect_data_compare_func = jonj_array_data_compare;
			fci_key = &fci1;
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. key_compare_type is %d. This should never happen. Please report as a bug", data_compare_type, key_compare_type);
			return;
		}
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "behavior is %d. This should never happen. Please report as a bug", behavior);
		return;
	}
	...
	jonj_set_compare_func(JONJ_SORT_STRING TSRMLS_CC);
...
{
	jonj_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_NONE);
}
/* }}} */
...
{
	jonj_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_KEY, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_USER);
}
/* }}} */
...
{
	jonj_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_NORMAL, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	jonj_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_NORMAL, INTERSECT_COMP_DATA_USER, INTERSECT_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	jonj_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_INTERNAL);
}
/* }}} */
...
{
	jonj_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_ASSOC, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_USER);
}
/* }}} */
...
{
	jonj_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_USER);
}
/* }}} */
...
{
	jonj_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_ASSOC, INTERSECT_COMP_DATA_USER, INTERSECT_COMP_KEY_USER);
}
/* }}} */

static void jonj_array_diff_key(INTERNAL_FUNCTION_PARAMETERS, int data_compare_type) /* {{{ */
{
	Bucket *p;
	int argc, i;
	zval ***args;
	...
	if (data_compare_type == DIFF_COMP_DATA_USER) {
		if (argc < 3) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "at least 3 parameters are required, %d given", ZEND_NUM_ARGS());
			return;
		}
		...
		if (argc < 2) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "at least 2 parameters are required, %d given", ZEND_NUM_ARGS());
			return;
		}
		if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "+", &args, &argc) == FAILURE) {
			return;
...
static void jonj_array_diff(INTERNAL_FUNCTION_PARAMETERS, int behavior, int data_compare_type, int key_compare_type) /* {{{ */
{
	zval ***args = NULL;
	HashTable *hash;
	int arr_argc, i, c;
	...
	if (behavior == DIFF_NORMAL) {
		diff_key_compare_func = jonj_array_key_compare;

		if (data_compare_type == DIFF_COMP_DATA_INTERNAL) {
			/* array_diff */
			req_args = 2;
			param_spec = "+";
			diff_data_compare_func = jonj_array_data_compare;
		} else if (data_compare_type == DIFF_COMP_DATA_USER) {
			/* array_udiff */
			req_args = 3;
			param_spec = "+f";
			diff_data_compare_func = jonj_array_user_compare;
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. This should never happen. Please report as a bug", data_compare_type);
			return;
		}
		...
		if (data_compare_type == DIFF_COMP_DATA_INTERNAL && key_compare_type == DIFF_COMP_KEY_INTERNAL) {
			...
			req_args = 3;
			param_spec = "+f";
			diff_key_compare_func = jonj_array_user_key_compare;
			diff_data_compare_func = jonj_array_data_compare;
			fci_key = &fci1;
			fci_key_cache = &fci1_cache;
		...
		for (i = 1; i < arr_argc; i++) {
			...
			if (!c) {
				if (behavior == DIFF_NORMAL) {
					...
					/* the behavior here differs from INTERSECT_KEY in jonj_intersect
...
{
	jonj_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_NONE);
}
/* }}} */
...
{
	jonj_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_KEY, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_USER);
}
/* }}} */
...
{
	jonj_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_NORMAL, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	jonj_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_NORMAL, DIFF_COMP_DATA_USER, DIFF_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	jonj_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_INTERNAL);
}
/* }}} */
...
{
	jonj_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_ASSOC, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_USER);
}
/* }}} */
...
{
	jonj_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_USER);
}
/* }}} */
...
{
	jonj_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_ASSOC, DIFF_COMP_DATA_USER, DIFF_COMP_KEY_USER);
}
/* }}} */
...
JONJAPI int jonj_multisort_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	...
	r = 0;
	do {
		jonj_set_compare_func(ARRAYG(multisort_flags)[MULTISORT_TYPE][r] TSRMLS_CC);
...
JONJ_FUNCTION(array_multisort)
{
	...
	int				sort_order = JONJ_SORT_ASC;
	int				sort_type  = JONJ_SORT_REGULAR;
	int				i, k;
	 ...
	 * specified, the sort flags defaults to JONJ_SORT_ASC and JONJ_SORT_REGULAR
	...
	for (i = 0; i < argc; i++) {
		if (Z_TYPE_PP(args[i]) == IS_ARRAY) {
			...
			if (i > 0) {
				...
				sort_order = JONJ_SORT_ASC;
				sort_type = JONJ_SORT_REGULAR;
			}
			...
			switch (Z_LVAL_PP(args[i]) & ~JONJ_SORT_FLAG_CASE) {
				case JONJ_SORT_ASC:
				case JONJ_SORT_DESC:
					/* flag allowed here */
					if (parse_state[MULTISORT_ORDER] == 1) {
						/* Save the flag and make sure then next arg is not the current flag. */
						sort_order = Z_LVAL_PP(args[i]) == JONJ_SORT_DESC ? -1 : 1;
						parse_state[MULTISORT_ORDER] = 0;
					} else {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is expected to be an array or sorting flag that has not already been specified", i + 1);
						MULTISORT_ABORT;
					}
					break;
				...
				default:
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is an unknown sort flag", i + 1);
					MULTISORT_ABORT;
					break;

			}
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is expected to be an array or a sort flag", i + 1);
			MULTISORT_ABORT;
		}
	}
	...
	for (i = 0; i < num_arrays; i++) {
		if (zend_hash_num_elements(Z_ARRVAL_PP(arrays[i])) != array_size) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Array sizes are inconsistent");
			MULTISORT_ABORT;
		}
	}
	...
	zend_qsort(indirect, array_size, sizeof(Bucket **), jonj_multisort_compare TSRMLS_CC);
...
JONJ_FUNCTION(array_rand)
{
	...
	if (ZEND_NUM_ARGS() > 1) {
		if (num_req <= 0 || num_req > num_avail) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Second argument has to be between 1 and the number of elements in the array");
			return;
		}
	}
	...
	while (num_req && (key_type = zend_hash_get_current_key_ex(Z_ARRVAL_P(input), &string_key, &string_key_len, &num_key, 0, &pos)) != HASH_KEY_NON_EXISTENT) {
		...
		randval = jonj_rand(TSRMLS_C);

		if ((double) (randval / (JONJ_RAND_MAX + 1.0)) < (double) num_req / (double) num_avail) {
	...
	while (zend_hash_get_current_data_ex(htbl, (void **)&operand, &pos) == SUCCESS) {
		...
		if (result) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the reduction callback");
				return;
			}
		} else {
			result = *operand;
	...
	for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(array), &pos);
		...
		if (have_callback) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the filter callback");
				return;
			}
...
JONJ_FUNCTION(array_map)
{
	...
	for (i = 0; i < n_arrays; i++) {
		if (Z_TYPE_PP(arrays[i]) != IS_ARRAY) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d should be an array", i + 2);
			efree(arrays);
			efree(args);
			efree(array_len);
			efree(array_pos);
	...
	for (k = 0; k < maxlen; k++) {
		...
		if (ZEND_FCI_INITIALIZED(fci)) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) != SUCCESS || !result) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the map callback");
				efree(array_len);
				efree(args);
				efree(array_pos);
				zval_dtor(return_value);
...
JONJ_FUNCTION(array_key_exists)
{
	...
	switch (Z_TYPE_P(key)) {
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The first argument should be either a string or an integer");
			RETURN_FALSE;
	}
}
/* }}} */
...
JONJ_FUNCTION(array_chunk)
{
	...
	if (size < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Size parameter expected to be greater than 0");
		return;
	}
...
JONJ_FUNCTION(array_combine)
{
	...
	if (num_keys != num_values) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Both parameters should have an equal number of elements");
		RETURN_FALSE;
	}
