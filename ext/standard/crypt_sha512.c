...
/* Windows VC++ port by Pierre Joye <pierre@jonj.tk> */
...
extern void * __jonj_mempcpy(void * dst, const void * src, size_t len);
extern char * __jonj_stpncpy(char *dst, const char *src, size_t len);
...
#if JONJ_WIN32 || (!defined(WORDS_BIGENDIAN))
# define SWAP(n) \
  (((n) << 56)					\
   | (((n) & 0xff00) << 40)			\
   | (((n) & 0xff0000) << 24)			\
...
char *
jonj_sha512_crypt_r(const char *key, const char *salt, char *buffer, int buflen) {
#ifdef JONJ_WIN32
# if _MSC <= 1300
#  pragma pack(push, 16)
...
# endif
	...
	for (cnt = key_len; cnt >= 64; cnt -= 64) {
		cp = __jonj_mempcpy((void *) cp, (const void *)temp_result, 64);
	}
	...
	for (cnt = salt_len; cnt >= 64; cnt -= 64) {
		cp = __jonj_mempcpy(cp, temp_result, 64);
	}
	memcpy(cp, temp_result, cnt);
	...
	/* Now we can construct the result string.  It consists of three
	 parts.  */
	cp = __jonj_stpncpy(buffer, sha512_salt_prefix, MAX(0, buflen));
	...
	cp = __jonj_stpncpy(cp, salt, MIN((size_t) MAX(0, buflen), salt_len));
...
/* This entry point is equivalent to the `crypt' function in Unix
   libcs.  */
char *
jonj_sha512_crypt(const char *key, const char *salt) {
	...
	return jonj_sha512_crypt_r (key, salt, buffer, buflen);
}
...
int main (void) {
	...
	for (cnt = 0; cnt < ntests2; ++cnt) {
		char *cp = jonj_sha512_crypt(tests2[cnt].input, tests2[cnt].salt);
