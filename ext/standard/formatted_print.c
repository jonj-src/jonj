...
# define PRINTF_DEBUG(arg) jonj_printf arg
#else
...
/* jonj_spintf_appendchar() {{{ */
inline static void
jonj_sprintf_appendchar(char **buffer, int *pos, int *size, char add TSRMLS_DC)
{
	if ((*pos + 1) >= *size) {
		*size <<= 1;
...
/* jonj_spintf_appendstring() {{{ */
inline static void
jonj_sprintf_appendstring(char **buffer, int *pos, int *size, char *add,
...
/* jonj_spintf_appendint() {{{ */
inline static void
jonj_sprintf_appendint(char **buffer, int *pos, int *size, long number,
						...
						int always_sign)
{
	...
	PRINTF_DEBUG(("sprintf: appending %d as \"%s\", i=%d\n",
				  number, &numbuf[i], i));
	jonj_sprintf_appendstring(buffer, pos, size, &numbuf[i], width, 0,
							 padding, alignment, (NUM_BUF_SIZE - 1) - i,
							 neg, 0, always_sign);
}
/* }}} */

/* jonj_spintf_appenduint() {{{ */
inline static void
jonj_sprintf_appenduint(char **buffer, int *pos, int *size,
...
/* jonj_spintf_appenddouble() {{{ */
inline static void
jonj_sprintf_appenddouble(char **buffer, int *pos,
						 ...
						 TSRMLS_DC)
{
	...
	if ((adjust & ADJ_PRECISION) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Requested precision of %d digits was truncated to JONJ maximum of %d digits", precision, MAX_FLOAT_PRECISION);
		precision = MAX_FLOAT_PRECISION;
	}
	
	if (zend_isnan(number)) {
		is_negative = (number<0);
		jonj_sprintf_appendstring(buffer, pos, size, "NaN", 3, 0, padding,
	...
	if (zend_isinf(number)) {
		is_negative = (number<0);
		jonj_sprintf_appendstring(buffer, pos, size, "INF", 3, 0, padding,
								 alignment, 3, is_negative, 0, always_sign);
		return;
	}
...
#endif
#endif
			s = jonj_conv_fp((fmt == 'f')?'F':fmt, number, 0, precision,
...
#endif
#endif
			s = jonj_gcvt(number, precision, LCONV_DECIMAL_POINT, (fmt == 'G')?'E':'e', &num_buf[1]);
			is_negative = 0;
			if (*s == '-') {
				is_negative = 1;
				s = &num_buf[1];
	...
	jonj_sprintf_appendstring(buffer, pos, size, s, width, 0, padding,
							 alignment, s_len, is_negative, 0, always_sign);
}
/* }}} */

/* jonj_spintf_appendd2n() {{{ */
inline static void
jonj_sprintf_append2n(char **buffer, int *pos, int *size, long number,
...
/* jonj_spintf_getnumber() {{{ */
inline static int
jonj_sprintf_getnumber(char *buffer, int *pos)
{
	char *endptr;
...
/* jonj_formatted_print() {{{
 * New sprintf implementation for JONJ.
 *
 * Modifiers:
 *
 ...
 */
static char *
jonj_formatted_print(int ht, int *len, int use_array, int format_offset TSRMLS_DC)
{
	zval ***args, **z_format;
	...
	while (inpos<format_len) {
		...
		if (format[inpos] != '%') {
			jonj_sprintf_appendchar(&result, &outpos, &size, format[inpos++] TSRMLS_CC);
		} else if (format[inpos + 1] == '%') {
			jonj_sprintf_appendchar(&result, &outpos, &size, '%' TSRMLS_CC);
			inpos += 2;
		} else {
			...
			if (isascii((int)format[inpos]) && !isalpha((int)format[inpos])) {
				...
				if (format[temppos] == '$') {
					argnum = jonj_sprintf_getnumber(format, &inpos);

					if (argnum <= 0) {
						efree(result);
						efree(args);
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument number must be greater than zero");
						return NULL;
					}
				...
				if (isdigit((int)format[inpos])) {
					...
					if ((width = jonj_sprintf_getnumber(format, &inpos)) < 0) {
						efree(result);
						efree(args);
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Width must be greater than zero and less than %d", INT_MAX);
						return NULL;
					}
					adjusting |= ADJ_WIDTH;
				} else {
				...
				if (format[inpos] == '.') {
					...
					if (isdigit((int)format[inpos])) {
						if ((precision = jonj_sprintf_getnumber(format, &inpos)) < 0) {
							efree(result);
							efree(args);
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Precision must be greater than zero and less than %d", INT_MAX);
							return NULL;
						}
			...
			if (argnum >= argc) {
				efree(result);
				efree(args);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Too few arguments");
				return NULL;
			}
			...
			switch (format[inpos]) {
				case 's': {
					...
					jonj_sprintf_appendstring(&result, &outpos, &size,
											 Z_STRVAL_P(var),
				...
				case 'd':
					convert_to_long(tmp);
					jonj_sprintf_appendint(&result, &outpos, &size,
										  Z_LVAL_P(tmp),
				...
				case 'u':
					convert_to_long(tmp);
					jonj_sprintf_appenduint(&result, &outpos, &size,
										  Z_LVAL_P(tmp),
										  width, padding, alignment);
					break;
				...
				case 'F':
					...
					jonj_sprintf_appenddouble(&result, &outpos, &size,
											 Z_DVAL_P(tmp),
											 width, padding, alignment,
											 precision, adjusting,
				...
				case 'c':
					convert_to_long(tmp);
					jonj_sprintf_appendchar(&result, &outpos, &size,
										(char) Z_LVAL_P(tmp) TSRMLS_CC);
					break;

				case 'o':
					convert_to_long(tmp);
					jonj_sprintf_append2n(&result, &outpos, &size,
										 Z_LVAL_P(tmp),
...
JONJ_FUNCTION(user_sprintf)
{
	...
	if ((result=jonj_formatted_print(ht, &len, 0, 0 TSRMLS_CC))==NULL) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(vsprintf)
{
	...
	if ((result=jonj_formatted_print(ht, &len, 1, 0 TSRMLS_CC))==NULL) {
		RETURN_FALSE;
	}
	RETVAL_STRINGL(result, len, 0);
}
...
JONJ_FUNCTION(fprintf)
{
	...
	if ((result=jonj_formatted_print(ht, &len, 0, 1 TSRMLS_CC))==NULL) {
		RETURN_FALSE;
	}

	jonj_stream_write(stream, result, len);
...
JONJ_FUNCTION(vfprintf)
{
	...
	if ((result=jonj_formatted_print(ht, &len, 1, 1 TSRMLS_CC))==NULL) {
		RETURN_FALSE;
	}
