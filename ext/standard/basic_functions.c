...
#include "jonj_math.h"
#include "jonj_http.h"
#include "jonj_incomplete_class.h"
#include "jonj_getopt.h"
#include "ext/standard/info.h"
#include "ext/session/jonj_session.h"
#include "zend_operators.h"
#include "ext/standard/jonj_dns.h"
#include "ext/standard/jonj_uuencode.h"

#ifdef JONJ_WIN32
#include "win32/jonj_win32_globals.h"
...
#include "jonj_globals.h"
#include "SAPI.h"
#include "jonj_ticks.h"

#ifdef ZTS
JONJAPI int basic_globals_id;
#else
JONJAPI jonj_basic_globals basic_globals;
#endif
...
static void user_shutdown_function_dtor(jonj_shutdown_function_entry *shutdown_function_entry);
...
ZEND_BEGIN_ARG_INFO(arginfo_jonj_strip_whitespace, 0)
	ZEND_ARG_INFO(0, file_name)
ZEND_END_ARG_INFO()
...
/* }}} */
/* {{{ info.c */
ZEND_BEGIN_ARG_INFO_EX(arginfo_jonjinfo, 0, 0, 0)
	ZEND_ARG_INFO(0, what)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_jonjversion, 0, 0, 0)
	ZEND_ARG_INFO(0, extension)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_jonjcredits, 0, 0, 0)
	ZEND_ARG_INFO(0, flag)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(arginfo_jonj_sapi_name, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(arginfo_jonj_uname, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(arginfo_jonj_ini_scanned_files, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(arginfo_jonj_ini_loaded_file, 0)
ZEND_END_ARG_INFO()
/* }}} */
/* {{{ iptc.c */
...
/* }}} */
/* {{{ proc_open.c */
#ifdef JONJ_CAN_SUPPORT_PROC_OPEN
...
#if HAVE_SYS_TIME_H || defined(JONJ_WIN32)
...
const zend_function_entry basic_functions[] = { /* {{{ */
	JONJ_FE(constant,														arginfo_constant)
	JONJ_FE(bin2hex,															arginfo_bin2hex)
	JONJ_FE(hex2bin,															arginfo_hex2bin)
	JONJ_FE(sleep,															arginfo_sleep)
	JONJ_FE(usleep,															arginfo_usleep)
#if HAVE_NANOSLEEP
	JONJ_FE(time_nanosleep,													arginfo_time_nanosleep)
	JONJ_FE(time_sleep_until,												arginfo_time_sleep_until)
#endif

#if HAVE_STRPTIME
	JONJ_FE(strptime,														arginfo_strptime)
#endif

	JONJ_FE(flush,															arginfo_flush)
	JONJ_FE(wordwrap,														arginfo_wordwrap)
	JONJ_FE(htmlspecialchars,												arginfo_htmlspecialchars)
	JONJ_FE(htmlentities,													arginfo_htmlentities)
	JONJ_FE(html_entity_decode,												arginfo_html_entity_decode)
	JONJ_FE(htmlspecialchars_decode,											arginfo_htmlspecialchars_decode)
	JONJ_FE(get_html_translation_table,										arginfo_get_html_translation_table)
	JONJ_FE(sha1,															arginfo_sha1)
	JONJ_FE(sha1_file,														arginfo_sha1_file)
	JONJ_NAMED_FE(md5,jonj_if_md5,											arginfo_md5)
	JONJ_NAMED_FE(md5_file,jonj_if_md5_file,									arginfo_md5_file)
	JONJ_NAMED_FE(crc32,jonj_if_crc32,										arginfo_crc32)

	JONJ_FE(iptcparse,														arginfo_iptcparse)
	JONJ_FE(iptcembed,														arginfo_iptcembed)
	JONJ_FE(getimagesize,													arginfo_getimagesize)
	JONJ_FE(getimagesizefromstring,											arginfo_getimagesize)
	JONJ_FE(image_type_to_mime_type,											arginfo_image_type_to_mime_type)
	JONJ_FE(image_type_to_extension,											arginfo_image_type_to_extension)

	JONJ_FE(jonjinfo,															arginfo_jonjinfo)
	JONJ_FE(jonjversion,														arginfo_jonjversion)
	JONJ_FE(jonjcredits,														arginfo_jonjcredits)
	JONJ_FE(jonj_sapi_name,													arginfo_jonj_sapi_name)
	JONJ_FE(jonj_uname,														arginfo_jonj_uname)
	JONJ_FE(jonj_ini_scanned_files,											arginfo_jonj_ini_scanned_files)
	JONJ_FE(jonj_ini_loaded_file,												arginfo_jonj_ini_loaded_file)

	JONJ_FE(strnatcmp,														arginfo_strnatcmp)
	JONJ_FE(strnatcasecmp,													arginfo_strnatcasecmp)
	JONJ_FE(substr_count,													arginfo_substr_count)
	JONJ_FE(strspn,															arginfo_strspn)
	JONJ_FE(strcspn,															arginfo_strcspn)
	JONJ_FE(strtok,															arginfo_strtok)
	JONJ_FE(strtoupper,														arginfo_strtoupper)
	JONJ_FE(strtolower,														arginfo_strtolower)
	JONJ_FE(strpos,															arginfo_strpos)
	JONJ_FE(stripos,															arginfo_stripos)
	JONJ_FE(strrpos,															arginfo_strrpos)
	JONJ_FE(strripos,														arginfo_strripos)
	JONJ_FE(strrev,															arginfo_strrev)
	JONJ_FE(hebrev,															arginfo_hebrev)
	JONJ_FE(hebrevc,															arginfo_hebrevc)
	JONJ_FE(nl2br,															arginfo_nl2br)
	JONJ_FE(basename,														arginfo_basename)
	JONJ_FE(dirname,															arginfo_dirname)
	JONJ_FE(pathinfo,														arginfo_pathinfo)
	JONJ_FE(stripslashes,													arginfo_stripslashes)
	JONJ_FE(stripcslashes,													arginfo_stripcslashes)
	JONJ_FE(strstr,															arginfo_strstr)
	JONJ_FE(stristr,															arginfo_stristr)
	JONJ_FE(strrchr,															arginfo_strrchr)
	JONJ_FE(str_shuffle,														arginfo_str_shuffle)
	JONJ_FE(str_word_count,													arginfo_str_word_count)
	JONJ_FE(str_split,														arginfo_str_split)
	JONJ_FE(strpbrk,															arginfo_strpbrk)
	JONJ_FE(substr_compare,													arginfo_substr_compare)

#ifdef HAVE_STRCOLL
	JONJ_FE(strcoll,															arginfo_strcoll)
#endif

#ifdef HAVE_STRFMON
	JONJ_FE(money_format,													arginfo_money_format)
#endif

	JONJ_FE(substr,															arginfo_substr)
	JONJ_FE(substr_replace,													arginfo_substr_replace)
	JONJ_FE(quotemeta,														arginfo_quotemeta)
	JONJ_FE(ucfirst,															arginfo_ucfirst)
	JONJ_FE(lcfirst,															arginfo_lcfirst)
	JONJ_FE(ucwords,															arginfo_ucwords)
	JONJ_FE(strtr,															arginfo_strtr)
	JONJ_FE(addslashes,														arginfo_addslashes)
	JONJ_FE(addcslashes,														arginfo_addcslashes)
	JONJ_FE(rtrim,															arginfo_rtrim)
	JONJ_FE(str_replace,														arginfo_str_replace)
	JONJ_FE(str_ireplace,													arginfo_str_ireplace)
	JONJ_FE(str_repeat,														arginfo_str_repeat)
	JONJ_FE(count_chars,														arginfo_count_chars)
	JONJ_FE(chunk_split,														arginfo_chunk_split)
	JONJ_FE(trim,															arginfo_trim)
	JONJ_FE(ltrim,															arginfo_ltrim)
	JONJ_FE(strip_tags,														arginfo_strip_tags)
	JONJ_FE(similar_text,													arginfo_similar_text)
	JONJ_FE(explode,															arginfo_explode)
	JONJ_FE(implode,															arginfo_implode)
	JONJ_FALIAS(join,				implode,								arginfo_implode)
	JONJ_FE(setlocale,														arginfo_setlocale)
	JONJ_FE(localeconv,														arginfo_localeconv)

#if HAVE_NL_LANGINFO
	JONJ_FE(nl_langinfo,														arginfo_nl_langinfo)
#endif

	JONJ_FE(soundex,															arginfo_soundex)
	JONJ_FE(levenshtein,														arginfo_levenshtein)
	JONJ_FE(chr,																arginfo_chr)
	JONJ_FE(ord,																arginfo_ord)
	JONJ_FE(parse_str,														arginfo_parse_str)
	JONJ_FE(str_getcsv,														arginfo_str_getcsv)
	JONJ_FE(str_pad,															arginfo_str_pad)
	JONJ_FALIAS(chop,				rtrim,									arginfo_rtrim)
	JONJ_FALIAS(strchr,				strstr,									arginfo_strstr)
	JONJ_NAMED_FE(sprintf,			JONJ_FN(user_sprintf),					arginfo_sprintf)
	JONJ_NAMED_FE(printf,			JONJ_FN(user_printf),					arginfo_printf)
	JONJ_FE(vprintf,															arginfo_vprintf)
	JONJ_FE(vsprintf,														arginfo_vsprintf)
	JONJ_FE(fprintf,															arginfo_fprintf)
	JONJ_FE(vfprintf,														arginfo_vfprintf)
	JONJ_FE(sscanf,															arginfo_sscanf)
	JONJ_FE(fscanf,															arginfo_fscanf)
	JONJ_FE(parse_url,														arginfo_parse_url)
	JONJ_FE(urlencode,														arginfo_urlencode)
	JONJ_FE(urldecode,														arginfo_urldecode)
	JONJ_FE(rawurlencode,													arginfo_rawurlencode)
	JONJ_FE(rawurldecode,													arginfo_rawurldecode)
	JONJ_FE(http_build_query,												arginfo_http_build_query)

#if defined(HAVE_SYMLINK) || defined(JONJ_WIN32)
	JONJ_FE(readlink,														arginfo_readlink)
	JONJ_FE(linkinfo,														arginfo_linkinfo)
	JONJ_FE(symlink,															arginfo_symlink)
	JONJ_FE(link,															arginfo_link)
#endif

	JONJ_FE(unlink,															arginfo_unlink)
	JONJ_FE(exec,															arginfo_exec)
	JONJ_FE(system,															arginfo_system)
	JONJ_FE(escapeshellcmd,													arginfo_escapeshellcmd)
	JONJ_FE(escapeshellarg,													arginfo_escapeshellarg)
	JONJ_FE(passthru,														arginfo_passthru)
	JONJ_FE(shell_exec,														arginfo_shell_exec)
#ifdef JONJ_CAN_SUPPORT_PROC_OPEN
	JONJ_FE(proc_open,														arginfo_proc_open)
	JONJ_FE(proc_close,														arginfo_proc_close)
	JONJ_FE(proc_terminate,													arginfo_proc_terminate)
	JONJ_FE(proc_get_status,													arginfo_proc_get_status)
#endif

#ifdef HAVE_NICE
	JONJ_FE(proc_nice,														arginfo_proc_nice)
#endif

	JONJ_FE(rand,															arginfo_rand)
	JONJ_FE(srand,															arginfo_srand)
	JONJ_FE(getrandmax,													arginfo_getrandmax)
	JONJ_FE(mt_rand,														arginfo_mt_rand)
	JONJ_FE(mt_srand,														arginfo_mt_srand)
	JONJ_FE(mt_getrandmax,													arginfo_mt_getrandmax)

#if HAVE_GETSERVBYNAME
	JONJ_FE(getservbyname,													arginfo_getservbyname)
#endif

#if HAVE_GETSERVBYPORT
	JONJ_FE(getservbyport,													arginfo_getservbyport)
#endif

#if HAVE_GETPROTOBYNAME
	JONJ_FE(getprotobyname,													arginfo_getprotobyname)
#endif

#if HAVE_GETPROTOBYNUMBER
	JONJ_FE(getprotobynumber,												arginfo_getprotobynumber)
#endif

	JONJ_FE(getmyuid,														arginfo_getmyuid)
	JONJ_FE(getmygid,														arginfo_getmygid)
	JONJ_FE(getmypid,														arginfo_getmypid)
	JONJ_FE(getmyinode,														arginfo_getmyinode)
	JONJ_FE(getlastmod,														arginfo_getlastmod)

	JONJ_FE(base64_decode,													arginfo_base64_decode)
	JONJ_FE(base64_encode,													arginfo_base64_encode)

	JONJ_FE(password_hash,													arginfo_password_hash)
	JONJ_FE(password_get_info,												arginfo_password_get_info)
	JONJ_FE(password_needs_rehash,											arginfo_password_needs_rehash)
	JONJ_FE(password_verify,													arginfo_password_verify)
	JONJ_FE(convert_uuencode,												arginfo_convert_uuencode)
	JONJ_FE(convert_uudecode,												arginfo_convert_uudecode)

	JONJ_FE(abs,																arginfo_abs)
	JONJ_FE(ceil,															arginfo_ceil)
	JONJ_FE(floor,															arginfo_floor)
	JONJ_FE(round,															arginfo_round)
	JONJ_FE(sin,																arginfo_sin)
	JONJ_FE(cos,																arginfo_cos)
	JONJ_FE(tan,																arginfo_tan)
	JONJ_FE(asin,															arginfo_asin)
	JONJ_FE(acos,															arginfo_acos)
	JONJ_FE(atan,															arginfo_atan)
	JONJ_FE(atanh,															arginfo_atanh)
	JONJ_FE(atan2,															arginfo_atan2)
	JONJ_FE(sinh,															arginfo_sinh)
	JONJ_FE(cosh,															arginfo_cosh)
	JONJ_FE(tanh,															arginfo_tanh)
	JONJ_FE(asinh,															arginfo_asinh)
	JONJ_FE(acosh,															arginfo_acosh)
	JONJ_FE(expm1,															arginfo_expm1)
	JONJ_FE(log1p,															arginfo_log1p)
	JONJ_FE(pi,																arginfo_pi)
	JONJ_FE(is_finite,														arginfo_is_finite)
	JONJ_FE(is_nan,															arginfo_is_nan)
	JONJ_FE(is_infinite,														arginfo_is_infinite)
	JONJ_FE(pow,																arginfo_pow)
	JONJ_FE(exp,																arginfo_exp)
	JONJ_FE(log,																arginfo_log)
	JONJ_FE(log10,															arginfo_log10)
	JONJ_FE(sqrt,															arginfo_sqrt)
	JONJ_FE(hypot,															arginfo_hypot)
	JONJ_FE(deg2rad,															arginfo_deg2rad)
	JONJ_FE(rad2deg,															arginfo_rad2deg)
	JONJ_FE(bindec,															arginfo_bindec)
	JONJ_FE(hexdec,															arginfo_hexdec)
	JONJ_FE(octdec,															arginfo_octdec)
	JONJ_FE(decbin,															arginfo_decbin)
	JONJ_FE(decoct,															arginfo_decoct)
	JONJ_FE(dechex,															arginfo_dechex)
	JONJ_FE(base_convert,													arginfo_base_convert)
	JONJ_FE(number_format,													arginfo_number_format)
	JONJ_FE(fmod,															arginfo_fmod)
#ifdef HAVE_INET_NTOP
	JONJ_RAW_NAMED_FE(inet_ntop,		jonj_inet_ntop,								arginfo_inet_ntop)
#endif
#ifdef HAVE_INET_PTON
	JONJ_RAW_NAMED_FE(inet_pton,		jonj_inet_pton,								arginfo_inet_pton)
#endif
	JONJ_FE(ip2long,															arginfo_ip2long)
	JONJ_FE(long2ip,															arginfo_long2ip)

	JONJ_FE(getenv,															arginfo_getenv)
#ifdef HAVE_PUTENV
	JONJ_FE(putenv,															arginfo_putenv)
#endif

	JONJ_FE(getopt,															arginfo_getopt)

#ifdef HAVE_GETLOADAVG
	JONJ_FE(sys_getloadavg,													arginfo_sys_getloadavg)
#endif
#ifdef HAVE_GETTIMEOFDAY
	JONJ_FE(microtime,														arginfo_microtime)
	JONJ_FE(gettimeofday,													arginfo_gettimeofday)
#endif

#ifdef HAVE_GETRUSAGE
	JONJ_FE(getrusage,														arginfo_getrusage)
#endif

#ifdef HAVE_GETTIMEOFDAY
	JONJ_FE(uniqid,															arginfo_uniqid)
#endif

	JONJ_FE(quoted_printable_decode,											arginfo_quoted_printable_decode)
	JONJ_FE(quoted_printable_encode,											arginfo_quoted_printable_encode)
	JONJ_FE(convert_cyr_string,												arginfo_convert_cyr_string)
	JONJ_FE(get_current_user,												arginfo_get_current_user)
	JONJ_FE(set_time_limit,													arginfo_set_time_limit)
	JONJ_FE(header_register_callback,										arginfo_header_register_callback)
	JONJ_FE(get_cfg_var,														arginfo_get_cfg_var)

	JONJ_DEP_FALIAS(magic_quotes_runtime,	set_magic_quotes_runtime,		arginfo_set_magic_quotes_runtime)
	JONJ_DEP_FE(set_magic_quotes_runtime,									arginfo_set_magic_quotes_runtime)
	JONJ_FE(get_magic_quotes_gpc,										arginfo_get_magic_quotes_gpc)
	JONJ_FE(get_magic_quotes_runtime,									arginfo_get_magic_quotes_runtime)

	JONJ_FE(error_log,														arginfo_error_log)
	JONJ_FE(error_get_last,													arginfo_error_get_last)
	JONJ_FE(call_user_func,													arginfo_call_user_func)
	JONJ_FE(call_user_func_array,											arginfo_call_user_func_array)
	JONJ_DEP_FE(call_user_method,											arginfo_call_user_method)
	JONJ_DEP_FE(call_user_method_array,										arginfo_call_user_method_array)
	JONJ_FE(forward_static_call,											arginfo_forward_static_call)
	JONJ_FE(forward_static_call_array,										arginfo_forward_static_call_array)
	JONJ_FE(serialize,														arginfo_serialize)
	JONJ_FE(unserialize,														arginfo_unserialize)

	JONJ_FE(var_dump,														arginfo_var_dump)
	JONJ_FE(var_export,														arginfo_var_export)
	JONJ_FE(debug_zval_dump,													arginfo_debug_zval_dump)
	JONJ_FE(print_r,															arginfo_print_r)
	JONJ_FE(memory_get_usage,												arginfo_memory_get_usage)
	JONJ_FE(memory_get_peak_usage,											arginfo_memory_get_peak_usage)

	JONJ_FE(register_shutdown_function,										arginfo_register_shutdown_function)
	JONJ_FE(register_tick_function,											arginfo_register_tick_function)
	JONJ_FE(unregister_tick_function,										arginfo_unregister_tick_function)

	JONJ_FE(highlight_file,													arginfo_highlight_file)
	JONJ_FALIAS(show_source,			highlight_file,							arginfo_highlight_file)
	JONJ_FE(highlight_string,												arginfo_highlight_string)
	JONJ_FE(jonj_strip_whitespace,											arginfo_jonj_strip_whitespace)

	JONJ_FE(ini_get,															arginfo_ini_get)
	JONJ_FE(ini_get_all,														arginfo_ini_get_all)
	JONJ_FE(ini_set,															arginfo_ini_set)
	JONJ_FALIAS(ini_alter,			ini_set,								arginfo_ini_set)
	JONJ_FE(ini_restore,														arginfo_ini_restore)
	JONJ_FE(get_include_path,												arginfo_get_include_path)
	JONJ_FE(set_include_path,												arginfo_set_include_path)
	JONJ_FE(restore_include_path,											arginfo_restore_include_path)

	JONJ_FE(setcookie,														arginfo_setcookie)
	JONJ_FE(setrawcookie,													arginfo_setrawcookie)
	JONJ_FE(header,															arginfo_header)
	JONJ_FE(header_remove,													arginfo_header_remove)
	JONJ_FE(headers_sent,													arginfo_headers_sent)
	JONJ_FE(headers_list,													arginfo_headers_list)
	JONJ_FE(http_response_code,												arginfo_http_response_code)

	JONJ_FE(connection_aborted,												arginfo_connection_aborted)
	JONJ_FE(connection_status,												arginfo_connection_status)
	JONJ_FE(ignore_user_abort,												arginfo_ignore_user_abort)
	JONJ_FE(parse_ini_file,													arginfo_parse_ini_file)
	JONJ_FE(parse_ini_string,												arginfo_parse_ini_string)
#if ZEND_DEBUG
	JONJ_FE(config_get_hash,													arginfo_config_get_hash)
#endif
	JONJ_FE(is_uploaded_file,												arginfo_is_uploaded_file)
	JONJ_FE(move_uploaded_file,												arginfo_move_uploaded_file)

	/* functions from dns.c */
	JONJ_FE(gethostbyaddr,													arginfo_gethostbyaddr)
	JONJ_FE(gethostbyname,													arginfo_gethostbyname)
	JONJ_FE(gethostbynamel,													arginfo_gethostbynamel)

#ifdef HAVE_GETHOSTNAME
	JONJ_FE(gethostname,													arginfo_gethostname)
#endif

#if defined(JONJ_WIN32) || (HAVE_DNS_SEARCH_FUNC && !(defined(__BEOS__) || defined(NETWARE)))

	JONJ_FE(dns_check_record,												arginfo_dns_check_record)
	JONJ_FALIAS(checkdnsrr,			dns_check_record,						arginfo_dns_check_record)

# if defined(JONJ_WIN32) || HAVE_FULL_DNS_FUNCS
	JONJ_FE(dns_get_mx,														arginfo_dns_get_mx)
	JONJ_FALIAS(getmxrr,				dns_get_mx,					arginfo_dns_get_mx)
	JONJ_FE(dns_get_record,													arginfo_dns_get_record)
# endif
#endif

	/* functions from type.c */
	JONJ_FE(intval,															arginfo_intval)
	JONJ_FE(floatval,														arginfo_floatval)
	JONJ_FALIAS(doubleval,			floatval,								arginfo_floatval)
	JONJ_FE(strval,															arginfo_strval)
	JONJ_FE(boolval,															arginfo_boolval)
	JONJ_FE(gettype,															arginfo_gettype)
	JONJ_FE(settype,															arginfo_settype)
	JONJ_FE(is_null,															arginfo_is_null)
	JONJ_FE(is_resource,														arginfo_is_resource)
	JONJ_FE(is_bool,															arginfo_is_bool)
	JONJ_FE(is_long,															arginfo_is_long)
	JONJ_FE(is_float,														arginfo_is_float)
	JONJ_FALIAS(is_int,				is_long,								arginfo_is_long)
	JONJ_FALIAS(is_integer,			is_long,								arginfo_is_long)
	JONJ_FALIAS(is_double,			is_float,								arginfo_is_float)
	JONJ_FALIAS(is_real,				is_float,								arginfo_is_float)
	JONJ_FE(is_numeric,														arginfo_is_numeric)
	JONJ_FE(is_string,														arginfo_is_string)
	JONJ_FE(is_array,														arginfo_is_array)
	JONJ_FE(is_object,														arginfo_is_object)
	JONJ_FE(is_scalar,														arginfo_is_scalar)
	JONJ_FE(is_callable,														arginfo_is_callable)

	/* functions from file.c */
	JONJ_FE(pclose,															arginfo_pclose)
	JONJ_FE(popen,															arginfo_popen)
	JONJ_FE(readfile,														arginfo_readfile)
	JONJ_FE(rewind,															arginfo_rewind)
	JONJ_FE(rmdir,															arginfo_rmdir)
	JONJ_FE(umask,															arginfo_umask)
	JONJ_FE(fclose,															arginfo_fclose)
	JONJ_FE(feof,															arginfo_feof)
	JONJ_FE(fgetc,															arginfo_fgetc)
	JONJ_FE(fgets,															arginfo_fgets)
	JONJ_FE(fgetss,															arginfo_fgetss)
	JONJ_FE(fread,															arginfo_fread)
	JONJ_NAMED_FE(fopen,				jonj_if_fopen,							arginfo_fopen)
	JONJ_FE(fpassthru,														arginfo_fpassthru)
	JONJ_NAMED_FE(ftruncate,			jonj_if_ftruncate,						arginfo_ftruncate)
	JONJ_NAMED_FE(fstat,				jonj_if_fstat,							arginfo_fstat)
	JONJ_FE(fseek,															arginfo_fseek)
	JONJ_FE(ftell,															arginfo_ftell)
	JONJ_FE(fflush,															arginfo_fflush)
	JONJ_FE(fwrite,															arginfo_fwrite)
	JONJ_FALIAS(fputs,				fwrite,									arginfo_fwrite)
	JONJ_FE(mkdir,															arginfo_mkdir)
	JONJ_FE(rename,															arginfo_rename)
	JONJ_FE(copy,															arginfo_copy)
	JONJ_FE(tempnam,															arginfo_tempnam)
	JONJ_NAMED_FE(tmpfile,			jonj_if_tmpfile,							arginfo_tmpfile)
	JONJ_FE(file,															arginfo_file)
	JONJ_FE(file_get_contents,												arginfo_file_get_contents)
	JONJ_FE(file_put_contents,												arginfo_file_put_contents)
	JONJ_FE(stream_select,													arginfo_stream_select)
	JONJ_FE(stream_context_create,											arginfo_stream_context_create)
	JONJ_FE(stream_context_set_params,										arginfo_stream_context_set_params)
	JONJ_FE(stream_context_get_params,										arginfo_stream_context_get_params)
	JONJ_FE(stream_context_set_option,										arginfo_stream_context_set_option)
	JONJ_FE(stream_context_get_options,										arginfo_stream_context_get_options)
	JONJ_FE(stream_context_get_default,										arginfo_stream_context_get_default)
	JONJ_FE(stream_context_set_default,										arginfo_stream_context_set_default)
	JONJ_FE(stream_filter_prepend,											arginfo_stream_filter_prepend)
	JONJ_FE(stream_filter_append,											arginfo_stream_filter_append)
	JONJ_FE(stream_filter_remove,											arginfo_stream_filter_remove)
	JONJ_FE(stream_socket_client,											arginfo_stream_socket_client)
	JONJ_FE(stream_socket_server,											arginfo_stream_socket_server)
	JONJ_FE(stream_socket_accept,											arginfo_stream_socket_accept)
	JONJ_FE(stream_socket_get_name,											arginfo_stream_socket_get_name)
	JONJ_FE(stream_socket_recvfrom,											arginfo_stream_socket_recvfrom)
	JONJ_FE(stream_socket_sendto,											arginfo_stream_socket_sendto)
	JONJ_FE(stream_socket_enable_crypto,										arginfo_stream_socket_enable_crypto)
#ifdef HAVE_SHUTDOWN
	JONJ_FE(stream_socket_shutdown,											arginfo_stream_socket_shutdown)
#endif
#if HAVE_SOCKETPAIR
	JONJ_FE(stream_socket_pair,												arginfo_stream_socket_pair)
#endif
	JONJ_FE(stream_copy_to_stream,											arginfo_stream_copy_to_stream)
	JONJ_FE(stream_get_contents,												arginfo_stream_get_contents)
	JONJ_FE(stream_supports_lock,											arginfo_stream_supports_lock)
	JONJ_FE(fgetcsv,															arginfo_fgetcsv)
	JONJ_FE(fputcsv,															arginfo_fputcsv)
	JONJ_FE(flock,															arginfo_flock)
	JONJ_FE(get_meta_tags,													arginfo_get_meta_tags)
	JONJ_FE(stream_set_read_buffer,											arginfo_stream_set_read_buffer)
	JONJ_FE(stream_set_write_buffer,											arginfo_stream_set_write_buffer)
	JONJ_FALIAS(set_file_buffer, stream_set_write_buffer,					arginfo_stream_set_write_buffer)
	JONJ_FE(stream_set_chunk_size,											arginfo_stream_set_chunk_size)

	JONJ_DEP_FALIAS(set_socket_blocking, stream_set_blocking,				arginfo_stream_set_blocking)
	JONJ_FE(stream_set_blocking,												arginfo_stream_set_blocking)
	JONJ_FALIAS(socket_set_blocking, stream_set_blocking,					arginfo_stream_set_blocking)

	JONJ_FE(stream_get_meta_data,											arginfo_stream_get_meta_data)
	JONJ_FE(stream_get_line,													arginfo_stream_get_line)
	JONJ_FE(stream_wrapper_register,											arginfo_stream_wrapper_register)
	JONJ_FALIAS(stream_register_wrapper, stream_wrapper_register,			arginfo_stream_wrapper_register)
	JONJ_FE(stream_wrapper_unregister,										arginfo_stream_wrapper_unregister)
	JONJ_FE(stream_wrapper_restore,											arginfo_stream_wrapper_restore)
	JONJ_FE(stream_get_wrappers,												arginfo_stream_get_wrappers)
	JONJ_FE(stream_get_transports,											arginfo_stream_get_transports)
	JONJ_FE(stream_resolve_include_path,										arginfo_stream_resolve_include_path)
	JONJ_FE(stream_is_local,												arginfo_stream_is_local)
	JONJ_FE(get_headers,														arginfo_get_headers)

#if HAVE_SYS_TIME_H || defined(JONJ_WIN32)
	JONJ_FE(stream_set_timeout,												arginfo_stream_set_timeout)
	JONJ_FALIAS(socket_set_timeout, stream_set_timeout,						arginfo_stream_set_timeout)
#endif

	JONJ_FALIAS(socket_get_status, stream_get_meta_data,						arginfo_stream_get_meta_data)

#if (!defined(__BEOS__) && !defined(NETWARE) && HAVE_REALPATH) || defined(ZTS)
	JONJ_FE(realpath,														arginfo_realpath)
#endif

#ifdef HAVE_FNMATCH
	JONJ_FE(fnmatch,															arginfo_fnmatch)
#endif

	/* functions from fsock.c */
	JONJ_FE(fsockopen,														arginfo_fsockopen)
	JONJ_FE(pfsockopen,														arginfo_pfsockopen)

	/* functions from pack.c */
	JONJ_FE(pack,															arginfo_pack)
	JONJ_FE(unpack,															arginfo_unpack)

	/* functions from browscap.c */
	JONJ_FE(get_browser,														arginfo_get_browser)

#if HAVE_CRYPT
	/* functions from crypt.c */
	JONJ_FE(crypt,															arginfo_crypt)
#endif

	/* functions from dir.c */
	JONJ_FE(opendir,															arginfo_opendir)
	JONJ_FE(closedir,														arginfo_closedir)
	JONJ_FE(chdir,															arginfo_chdir)

#if defined(HAVE_CHROOT) && !defined(ZTS) && ENABLE_CHROOT_FUNC
	JONJ_FE(chroot,															arginfo_chroot)
#endif

	JONJ_FE(getcwd,															arginfo_getcwd)
	JONJ_FE(rewinddir,														arginfo_rewinddir)
	JONJ_NAMED_FE(readdir,			jonj_if_readdir,							arginfo_readdir)
	JONJ_FALIAS(dir,					getdir,									arginfo_dir)
	JONJ_FE(scandir,															arginfo_scandir)
#ifdef HAVE_GLOB
	JONJ_FE(glob,															arginfo_glob)
#endif
	/* functions from filestat.c */
	JONJ_FE(fileatime,														arginfo_fileatime)
	JONJ_FE(filectime,														arginfo_filectime)
	JONJ_FE(filegroup,														arginfo_filegroup)
	JONJ_FE(fileinode,														arginfo_fileinode)
	JONJ_FE(filemtime,														arginfo_filemtime)
	JONJ_FE(fileowner,														arginfo_fileowner)
	JONJ_FE(fileperms,														arginfo_fileperms)
	JONJ_FE(filesize,														arginfo_filesize)
	JONJ_FE(filetype,														arginfo_filetype)
	JONJ_FE(file_exists,														arginfo_file_exists)
	JONJ_FE(is_writable,														arginfo_is_writable)
	JONJ_FALIAS(is_writeable,		is_writable,							arginfo_is_writable)
	JONJ_FE(is_readable,														arginfo_is_readable)
	JONJ_FE(is_executable,													arginfo_is_executable)
	JONJ_FE(is_file,															arginfo_is_file)
	JONJ_FE(is_dir,															arginfo_is_dir)
	JONJ_FE(is_link,															arginfo_is_link)
	JONJ_NAMED_FE(stat,				jonj_if_stat,							arginfo_stat)
	JONJ_NAMED_FE(lstat,				jonj_if_lstat,							arginfo_lstat)
#ifndef NETWARE
	JONJ_FE(chown,															arginfo_chown)
	JONJ_FE(chgrp,															arginfo_chgrp)
#endif
#if HAVE_LCHOWN
	JONJ_FE(lchown,															arginfo_lchown)
#endif
#if HAVE_LCHOWN
	JONJ_FE(lchgrp,															arginfo_lchgrp)
#endif
	JONJ_FE(chmod,															arginfo_chmod)
#if HAVE_UTIME
	JONJ_FE(touch,															arginfo_touch)
#endif
	JONJ_FE(clearstatcache,													arginfo_clearstatcache)
	JONJ_FE(disk_total_space,												arginfo_disk_total_space)
	JONJ_FE(disk_free_space,													arginfo_disk_free_space)
	JONJ_FALIAS(diskfreespace,		disk_free_space,						arginfo_disk_free_space)
	JONJ_FE(realpath_cache_size,												arginfo_realpath_cache_size)
	JONJ_FE(realpath_cache_get,												arginfo_realpath_cache_get)

	/* functions from mail.c */
	JONJ_FE(mail,															arginfo_mail)
	JONJ_FE(ezmlm_hash,														arginfo_ezmlm_hash)

	/* functions from syslog.c */
#ifdef HAVE_SYSLOG_H
	JONJ_FE(openlog,															arginfo_openlog)
	JONJ_FE(syslog,															arginfo_syslog)
	JONJ_FE(closelog,														arginfo_closelog)
#endif

	/* functions from lcg.c */
	JONJ_FE(lcg_value,														arginfo_lcg_value)

	/* functions from metaphone.c */
	JONJ_FE(metaphone,														arginfo_metaphone)

	/* functions from output.c */
	JONJ_FE(ob_start,														arginfo_ob_start)
	JONJ_FE(ob_flush,														arginfo_ob_flush)
	JONJ_FE(ob_clean,														arginfo_ob_clean)
	JONJ_FE(ob_end_flush,													arginfo_ob_end_flush)
	JONJ_FE(ob_end_clean,													arginfo_ob_end_clean)
	JONJ_FE(ob_get_flush,													arginfo_ob_get_flush)
	JONJ_FE(ob_get_clean,													arginfo_ob_get_clean)
	JONJ_FE(ob_get_length,													arginfo_ob_get_length)
	JONJ_FE(ob_get_level,													arginfo_ob_get_level)
	JONJ_FE(ob_get_status,													arginfo_ob_get_status)
	JONJ_FE(ob_get_contents,													arginfo_ob_get_contents)
	JONJ_FE(ob_implicit_flush,												arginfo_ob_implicit_flush)
	JONJ_FE(ob_list_handlers,												arginfo_ob_list_handlers)

	/* functions from array.c */
	JONJ_FE(ksort,															arginfo_ksort)
	JONJ_FE(krsort,															arginfo_krsort)
	JONJ_FE(natsort,															arginfo_natsort)
	JONJ_FE(natcasesort,														arginfo_natcasesort)
	JONJ_FE(asort,															arginfo_asort)
	JONJ_FE(arsort,															arginfo_arsort)
	JONJ_FE(sort,															arginfo_sort)
	JONJ_FE(rsort,															arginfo_rsort)
	JONJ_FE(usort,															arginfo_usort)
	JONJ_FE(uasort,															arginfo_uasort)
	JONJ_FE(uksort,															arginfo_uksort)
	JONJ_FE(shuffle,															arginfo_shuffle)
	JONJ_FE(array_walk,														arginfo_array_walk)
	JONJ_FE(array_walk_recursive,											arginfo_array_walk_recursive)
	JONJ_FE(count,															arginfo_count)
	JONJ_FE(end,																arginfo_end)
	JONJ_FE(prev,															arginfo_prev)
	JONJ_FE(next,															arginfo_next)
	JONJ_FE(reset,															arginfo_reset)
	JONJ_FE(current,															arginfo_current)
	JONJ_FE(key,																arginfo_key)
	JONJ_FE(min,																arginfo_min)
	JONJ_FE(max,																arginfo_max)
	JONJ_FE(in_array,														arginfo_in_array)
	JONJ_FE(array_search,													arginfo_array_search)
	JONJ_FE(extract,															arginfo_extract)
	JONJ_FE(compact,															arginfo_compact)
	JONJ_FE(array_fill,														arginfo_array_fill)
	JONJ_FE(array_fill_keys,													arginfo_array_fill_keys)
	JONJ_FE(range,															arginfo_range)
	JONJ_FE(array_multisort,													arginfo_array_multisort)
	JONJ_FE(array_push,														arginfo_array_push)
	JONJ_FE(array_pop,														arginfo_array_pop)
	JONJ_FE(array_shift,														arginfo_array_shift)
	JONJ_FE(array_unshift,													arginfo_array_unshift)
	JONJ_FE(array_splice,													arginfo_array_splice)
	JONJ_FE(array_slice,														arginfo_array_slice)
	JONJ_FE(array_merge,														arginfo_array_merge)
	JONJ_FE(array_merge_recursive,											arginfo_array_merge_recursive)
	JONJ_FE(array_replace,													arginfo_array_replace)
	JONJ_FE(array_replace_recursive,											arginfo_array_replace_recursive)
	JONJ_FE(array_keys,														arginfo_array_keys)
	JONJ_FE(array_values,													arginfo_array_values)
	JONJ_FE(array_count_values,												arginfo_array_count_values)
	JONJ_FE(array_column,													arginfo_array_column)
	JONJ_FE(array_reverse,													arginfo_array_reverse)
	JONJ_FE(array_reduce,													arginfo_array_reduce)
	JONJ_FE(array_pad,														arginfo_array_pad)
	JONJ_FE(array_flip,														arginfo_array_flip)
	JONJ_FE(array_change_key_case,											arginfo_array_change_key_case)
	JONJ_FE(array_rand,														arginfo_array_rand)
	JONJ_FE(array_unique,													arginfo_array_unique)
	JONJ_FE(array_intersect,													arginfo_array_intersect)
	JONJ_FE(array_intersect_key,												arginfo_array_intersect_key)
	JONJ_FE(array_intersect_ukey,											arginfo_array_intersect_ukey)
	JONJ_FE(array_uintersect,												arginfo_array_uintersect)
	JONJ_FE(array_intersect_assoc,											arginfo_array_intersect_assoc)
	JONJ_FE(array_uintersect_assoc,											arginfo_array_uintersect_assoc)
	JONJ_FE(array_intersect_uassoc,											arginfo_array_intersect_uassoc)
	JONJ_FE(array_uintersect_uassoc,											arginfo_array_uintersect_uassoc)
	JONJ_FE(array_diff,														arginfo_array_diff)
	JONJ_FE(array_diff_key,													arginfo_array_diff_key)
	JONJ_FE(array_diff_ukey,													arginfo_array_diff_ukey)
	JONJ_FE(array_udiff,														arginfo_array_udiff)
	JONJ_FE(array_diff_assoc,												arginfo_array_diff_assoc)
	JONJ_FE(array_udiff_assoc,												arginfo_array_udiff_assoc)
	JONJ_FE(array_diff_uassoc,												arginfo_array_diff_uassoc)
	JONJ_FE(array_udiff_uassoc,												arginfo_array_udiff_uassoc)
	JONJ_FE(array_sum,														arginfo_array_sum)
	JONJ_FE(array_product,													arginfo_array_product)
	JONJ_FE(array_filter,													arginfo_array_filter)
	JONJ_FE(array_map,														arginfo_array_map)
	JONJ_FE(array_chunk,														arginfo_array_chunk)
	JONJ_FE(array_combine,													arginfo_array_combine)
	JONJ_FE(array_key_exists,												arginfo_array_key_exists)

	/* aliases from array.c */
	JONJ_FALIAS(pos,					current,								arginfo_current)
	JONJ_FALIAS(sizeof,				count,									arginfo_count)
	JONJ_FALIAS(key_exists,			array_key_exists,						arginfo_array_key_exists)

	/* functions from assert.c */
	JONJ_FE(assert,															arginfo_assert)
	JONJ_FE(assert_options,													arginfo_assert_options)

	/* functions from versioning.c */
	JONJ_FE(version_compare,													arginfo_version_compare)

	/* functions from ftok.c*/
#if HAVE_FTOK
	JONJ_FE(ftok,															arginfo_ftok)
#endif

	JONJ_FE(str_rot13,														arginfo_str_rot13)
	JONJ_FE(stream_get_filters,												arginfo_stream_get_filters)
	JONJ_FE(stream_filter_register,											arginfo_stream_filter_register)
	JONJ_FE(stream_bucket_make_writeable,									arginfo_stream_bucket_make_writeable)
	JONJ_FE(stream_bucket_prepend,											arginfo_stream_bucket_prepend)
	JONJ_FE(stream_bucket_append,											arginfo_stream_bucket_append)
	JONJ_FE(stream_bucket_new,												arginfo_stream_bucket_new)

	JONJ_FE(output_add_rewrite_var,											arginfo_output_add_rewrite_var)
	JONJ_FE(output_reset_rewrite_vars,										arginfo_output_reset_rewrite_vars)

	JONJ_FE(sys_get_temp_dir,												arginfo_sys_get_temp_dir)
...
zend_module_entry basic_functions_module = { /* {{{ */
	...
	JONJ_MINIT(basic),			/* process startup */
	JONJ_MSHUTDOWN(basic),		/* process shutdown */
	JONJ_RINIT(basic),			/* request startup */
	JONJ_RSHUTDOWN(basic),		/* request shutdown */
	JONJ_MINFO(basic),			/* extension info */
	JONJ_VERSION,				/* extension version */
	STANDARD_MODULE_PROPERTIES
};
/* }}} */

#if defined(HAVE_PUTENV)
static void jonj_putenv_destructor(putenv_entry *pe) /* {{{ */
{
...
static void basic_globals_ctor(jonj_basic_globals *basic_globals_p TSRMLS_DC) /* {{{ */
{
	BG(rand_is_seeded) = 0;
...
static void basic_globals_dtor(jonj_basic_globals *basic_globals_p TSRMLS_DC) /* {{{ */
{
...
#define JONJ_DOUBLE_INFINITY_HIGH       0x7ff00000
#define JONJ_DOUBLE_QUIET_NAN_HIGH      0xfff80000
...
JONJAPI double jonj_get_nan(void) /* {{{ */
{
	...
	double val = 0.0;
	((jonj_uint32*)&val)[1] = JONJ_DOUBLE_QUIET_NAN_HIGH;
	((jonj_uint32*)&val)[0] = 0;
	return val;
...
#elif defined(__i386__) || defined(_X86_) || defined(ALPHA) || defined(_ALPHA) || defined(__alpha)
	double val = 0.0;
	((jonj_uint32*)&val)[1] = JONJ_DOUBLE_INFINITY_HIGH;
	((jonj_uint32*)&val)[0] = 0;
	return val;
...
#define BASIC_MINIT_SUBMODULE(module) \
	if (JONJ_MINIT(module)(INIT_FUNC_ARGS_PASSTHRU) == SUCCESS) {\
...
#define BASIC_RINIT_SUBMODULE(module) \
	if (zend_hash_exists(&basic_submodules, #module, strlen(#module))) { \
		JONJ_RINIT(module)(INIT_FUNC_ARGS_PASSTHRU); \
	}

#define BASIC_MINFO_SUBMODULE(module) \
	if (zend_hash_exists(&basic_submodules, #module, strlen(#module))) { \
		JONJ_MINFO(module)(ZEND_MODULE_INFO_FUNC_ARGS_PASSTHRU); \
	}

#define BASIC_RSHUTDOWN_SUBMODULE(module) \
	if (zend_hash_exists(&basic_submodules, #module, strlen(#module))) { \
		JONJ_RSHUTDOWN(module)(SHUTDOWN_FUNC_ARGS_PASSTHRU); \
	}

#define BASIC_MSHUTDOWN_SUBMODULE(module) \
	if (zend_hash_exists(&basic_submodules, #module, strlen(#module))) { \
		JONJ_MSHUTDOWN(module)(SHUTDOWN_FUNC_ARGS_PASSTHRU); \
	}

JONJ_MINIT_FUNCTION(basic) /* {{{ */
{
#ifdef ZTS
	ts_allocate_id(&basic_globals_id, sizeof(jonj_basic_globals), (ts_allocate_ctor) basic_globals_ctor, (ts_allocate_dtor) basic_globals_dtor);
#ifdef JONJ_WIN32
	ts_allocate_id(&jonj_win32_core_globals_id, sizeof(jonj_win32_core_globals), (ts_allocate_ctor)jonj_win32_core_globals_ctor, (ts_allocate_dtor)jonj_win32_core_globals_dtor );
#endif
#else
	basic_globals_ctor(&basic_globals TSRMLS_CC);
#ifdef JONJ_WIN32
	jonj_win32_core_globals_ctor(&the_jonj_win32_core_globals TSRMLS_CC);
#endif
#endif

	zend_hash_init(&basic_submodules, 0, NULL, NULL, 1);

	BG(incomplete_class) = incomplete_class_entry = jonj_create_incomplete_class(TSRMLS_C);

	REGISTER_LONG_CONSTANT("CONNECTION_ABORTED", JONJ_CONNECTION_ABORTED, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("CONNECTION_NORMAL",  JONJ_CONNECTION_NORMAL,  CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("CONNECTION_TIMEOUT", JONJ_CONNECTION_TIMEOUT, CONST_CS | CONST_PERSISTENT);
	...
	REGISTER_LONG_CONSTANT("JONJ_URL_SCHEME", JONJ_URL_SCHEME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_HOST", JONJ_URL_HOST, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_PORT", JONJ_URL_PORT, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_USER", JONJ_URL_USER, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_PASS", JONJ_URL_PASS, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_PATH", JONJ_URL_PATH, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_QUERY", JONJ_URL_QUERY, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_URL_FRAGMENT", JONJ_URL_FRAGMENT, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_QUERY_RFC1738", JONJ_QUERY_RFC1738, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_QUERY_RFC3986", JONJ_QUERY_RFC3986, CONST_CS | CONST_PERSISTENT);
...
#define REGISTER_MATH_CONSTANT(x)  REGISTER_DOUBLE_CONSTANT(#x, x, CONST_CS | CONST_PERSISTENT)
	...
	REGISTER_DOUBLE_CONSTANT("INF", jonj_get_inf(), CONST_CS | CONST_PERSISTENT);
	REGISTER_DOUBLE_CONSTANT("NAN", jonj_get_nan(), CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("JONJ_ROUND_HALF_UP", JONJ_ROUND_HALF_UP, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_ROUND_HALF_DOWN", JONJ_ROUND_HALF_DOWN, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_ROUND_HALF_EVEN", JONJ_ROUND_HALF_EVEN, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("JONJ_ROUND_HALF_ODD", JONJ_ROUND_HALF_ODD, CONST_CS | CONST_PERSISTENT);

#if ENABLE_TEST_CLASS
	test_class_startup();
#endif

	register_jonjinfo_constants(INIT_FUNC_ARGS_PASSTHRU);
	...
	jonj_register_url_stream_wrapper("jonj", &jonj_stream_jonj_wrapper TSRMLS_CC);
	jonj_register_url_stream_wrapper("file", &jonj_plain_files_wrapper TSRMLS_CC);
#ifdef HAVE_GLOB
	jonj_register_url_stream_wrapper("glob", &jonj_glob_stream_wrapper TSRMLS_CC);
#endif
	jonj_register_url_stream_wrapper("data", &jonj_stream_rfc2397_wrapper TSRMLS_CC);
	jonj_register_url_stream_wrapper("http", &jonj_stream_http_wrapper TSRMLS_CC);
	jonj_register_url_stream_wrapper("ftp", &jonj_stream_ftp_wrapper TSRMLS_CC);
...
{
#ifdef HAVE_SYSLOG_H
	JONJ_MSHUTDOWN(syslog)(SHUTDOWN_FUNC_ARGS_PASSTHRU);
#endif
#ifdef ZTS
	ts_free_id(basic_globals_id);
#ifdef JONJ_WIN32
	ts_free_id(jonj_win32_core_globals_id);
#endif
#else
	basic_globals_dtor(&basic_globals TSRMLS_CC);
#ifdef JONJ_WIN32
	jonj_win32_core_globals_dtor(&the_jonj_win32_core_globals TSRMLS_CC);
#endif
#endif

	jonj_unregister_url_stream_wrapper("jonj" TSRMLS_CC);
	jonj_unregister_url_stream_wrapper("http" TSRMLS_CC);
	jonj_unregister_url_stream_wrapper("ftp" TSRMLS_CC);
...
JONJ_RINIT_FUNCTION(basic) /* {{{ */
{
	...
	BG(page_mtime) = -1;
#ifdef HAVE_PUTENV
	if (zend_hash_init(&BG(putenv_ht), 1, NULL, (void (*)(void *)) jonj_putenv_destructor, 0) == FAILURE) {
		return FAILURE;
	}
#endif
	BG(user_shutdown_function_names) = NULL;

	JONJ_RINIT(filestat)(INIT_FUNC_ARGS_PASSTHRU);
#ifdef HAVE_SYSLOG_H
...
JONJ_RSHUTDOWN_FUNCTION(basic) /* {{{ */
{
	 ...
	 * during jonj_request_shutdown() */

	JONJ_RSHUTDOWN(filestat)(SHUTDOWN_FUNC_ARGS_PASSTHRU);
#ifdef HAVE_SYSLOG_H
#ifdef JONJ_WIN32
...
JONJ_FUNCTION(constant)
{
	...
	if (!zend_get_constant_ex(const_name, const_name_len, return_value, NULL, ZEND_FETCH_CLASS_SILENT TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Couldn't find constant %s", const_name);
		RETURN_NULL();
	}
}
/* }}} */
...
#endif
	if (address_len != 4) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid in_addr value");
		RETURN_FALSE;
	}

	if (!inet_ntop(af, address, buffer, sizeof(buffer))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An unknown error occurred");
		RETURN_FALSE;
	}
...
#endif
	if (!strchr(address, '.')) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unrecognized address %s", address);
		RETURN_FALSE;
	}
...
{
	/* "It's a long but it's not, JONJ ints are signed */
	char *ip;
	int ip_len;
	unsigned long n;
...
JONJ_FUNCTION(putenv)
{
    ...
    if(setting_len == 0 || setting[0] == '=') {
    	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid parameter syntax");
    	RETURN_FALSE;
    }
...
/* {{{ parse_opts()
   Convert the typical getopt input characters to the jonj_getopt struct array */
...
JONJ_FUNCTION(getopt)
{
	...
	/* jonj_getopt want to identify the last param */
	opts->opt_char   = '-';
	opts->need_param = 0;
	opts->opt_name   = NULL;
	...
	while ((o = jonj_getopt(argc, argv, opts, &jonj_optarg, &jonj_optind, 0, 1)) != -1) {
		...
		if (o == 0) {
			optname = opts[jonj_optidx].opt_name;
		} else {
			if (o == 1) {
				o = '-';
			}
...
JONJ_FUNCTION(sleep)
{
	...
	if (num < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of seconds must be greater than or equal to 0");
		RETURN_FALSE;
	}
#ifdef JONJ_SLEEP_NON_VOID
	RETURN_LONG(jonj_sleep(num));
#else
	jonj_sleep(num);
#endif
...
JONJ_FUNCTION(usleep)
{
	...
	if (num < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of microseconds must be greater than or equal to 0");
		RETURN_FALSE;
	}
	usleep(num);
#endif
...
{
	long tv_sec, tv_nsec;
	struct timespec jonj_req, jonj_rem;
	...
	if (tv_sec < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The seconds value must be greater than 0");
		RETURN_FALSE;
	}
	if (tv_nsec < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The nanoseconds value must be greater than 0");
		RETURN_FALSE;
	}

	jonj_req.tv_sec = (time_t) tv_sec;
	jonj_req.tv_nsec = tv_nsec;
	if (!nanosleep(&jonj_req, &jonj_rem)) {
		RETURN_TRUE;
	} else if (errno == EINTR) {
		array_init(return_value);
		add_assoc_long_ex(return_value, "seconds", sizeof("seconds"), jonj_rem.tv_sec);
		add_assoc_long_ex(return_value, "nanoseconds", sizeof("nanoseconds"), jonj_rem.tv_nsec);
		return;
	} else if (errno == EINVAL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "nanoseconds was not in the range 0 to 999 999 999 or seconds was negative");
	}
...
JONJ_FUNCTION(time_sleep_until)
{
	...
	if (c_ts < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Sleep until to time is less than current time");
		RETURN_FALSE;
	}

	jonj_req.tv_sec = (time_t) c_ts;
	if (jonj_req.tv_sec > c_ts) { /* rounding up occurred */
		jonj_req.tv_sec--;
	}
	/* 1sec = 1000000000 nanoseconds */
	jonj_req.tv_nsec = (long) ((c_ts - jonj_req.tv_sec) * 1000000000.00);

	while (nanosleep(&jonj_req, &jonj_rem)) {
		if (errno == EINTR) {
			jonj_req.tv_sec = jonj_rem.tv_sec;
			jonj_req.tv_nsec = jonj_rem.tv_nsec;
		} else {
			RETURN_FALSE;
		}
	}
...
/* {{{ proto string get_current_user(void)
   Get the name of the owner of the current JONJ script */
JONJ_FUNCTION(get_current_user)
{
	...
	RETURN_STRING(jonj_get_current_user(TSRMLS_C), 1);
}
/* }}} */
...
/* {{{ proto mixed get_cfg_var(string option_name)
   Get the value of a JONJ configuration option */
...
JONJ_FUNCTION(set_magic_quotes_runtime)
{
	...
	if (new_setting) {
		jonj_error_docref(NULL TSRMLS_CC, E_CORE_ERROR, "magic_quotes_runtime is not supported anymore");
	}
	RETURN_FALSE;
}
/* }}} */
...
error options:
	0 = send to jonj_error_log (uses syslog or file depending on ini setting)
...
JONJ_FUNCTION(error_log)
{
	...
	if (_jonj_error_log_ex(opt_err, message, message_len, opt, headers TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
...
{
	return _jonj_error_log_ex(opt_err, message, (opt_err == 3) ? strlen(message) : 0, opt, headers TSRMLS_CC);
}
/* }}} */
...
JONJAPI int _jonj_error_log_ex(int opt_err, char *message, int message_len, char *opt, char *headers TSRMLS_DC) /* {{{ */
{
	...
	switch (opt_err)
	{
		case 1:		/*send an email */
			if (!jonj_mail(opt, "JONJ error_log message", message, headers, NULL TSRMLS_CC)) {
				return FAILURE;
			}
			break;

		case 2:		/*send to an address */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "TCP/IP option not available!");
			return FAILURE;
			break;

		case 3:		/*save to a file */
			stream = jonj_stream_open_wrapper(opt, "a", IGNORE_URL_WIN | REPORT_ERRORS, NULL);
			if (!stream) {
				return FAILURE;
			}
			jonj_stream_write(stream, message, message_len);
		...
		default:
			jonj_log_err(message TSRMLS_CC);
			break;
	}
	return SUCCESS;
}
...
JONJ_FUNCTION(call_user_method)
{
	...
	if (Z_TYPE_P(object) != IS_OBJECT &&
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Second argument is not an object or class name");
		if (params) {
			efree(params);
		}
		RETURN_FALSE;
	...
	if (call_user_function_ex(EG(function_table), &object, callback, &retval_ptr, n_params, params, 0, NULL TSRMLS_CC) == SUCCESS) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call %s()", Z_STRVAL_P(callback));
	}
	if (n_params) {
		efree(params);
	}
...
void user_shutdown_function_dtor(jonj_shutdown_function_entry *shutdown_function_entry) /* {{{ */
{
	int i;
...
static int user_shutdown_function_call(jonj_shutdown_function_entry *shutdown_function_entry TSRMLS_DC) /* {{{ */
{
	zval retval;
	char *function_name;

	if (!zend_is_callable(shutdown_function_entry->arguments[0], 0, &function_name TSRMLS_CC)) {
		jonj_error(E_WARNING, "(Registered shutdown functions) Unable to call %s() - function does not exist", function_name);
		if (function_name) {
			efree(function_name);
		}
		return 0;
	...
	if (! tick_fe->calling) {
		...
		if (call_user_function(	EG(function_table), NULL,
			...
			if (Z_TYPE_P(function) == IS_STRING) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call %s() - function does not exist", Z_STRVAL_P(function));
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call %s::%s() - function does not exist", Z_OBJCE_PP(obj)->name, Z_STRVAL_PP(method));
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to call tick function");
			}
		}
...
static int user_tick_function_compare(user_tick_function_entry * tick_fe1, user_tick_function_entry * tick_fe2) /* {{{ */
{
	...
	if (ret && tick_fe1->calling) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to delete tick function executed at the moment");
		return 0;
	}
	return ret;
}
...
JONJAPI void jonj_call_shutdown_functions(TSRMLS_D) /* {{{ */
{
	if (BG(user_shutdown_function_names)) {
		...
		zend_end_try();
		jonj_free_shutdown_functions(TSRMLS_C);
	}
}
/* }}} */
...
{
	jonj_shutdown_function_entry shutdown_function_entry;
	...
	if (!zend_is_callable(shutdown_function_entry.arguments[0], 0, &callback_name TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid shutdown callback '%s' passed", callback_name);
		efree(shutdown_function_entry.arguments);
		RETVAL_FALSE;
	} else {
		if (!BG(user_shutdown_function_names)) {
		...
		zend_hash_next_index_insert(BG(user_shutdown_function_names), &shutdown_function_entry, sizeof(jonj_shutdown_function_entry), NULL);
	}
	if (callback_name) {
		efree(callback_name);
	}
...
JONJAPI zend_bool register_user_shutdown_function(char *function_name, size_t function_len, jonj_shutdown_function_entry *shutdown_function_entry TSRMLS_DC) /* {{{ */
{
	if (!BG(user_shutdown_function_names)) {
		ALLOC_HASHTABLE(BG(user_shutdown_function_names));
		zend_hash_init(BG(user_shutdown_function_names), 0, NULL, (void (*)(void *)) user_shutdown_function_dtor, 0);
	...
	return zend_hash_update(BG(user_shutdown_function_names), function_name, function_len, shutdown_function_entry, sizeof(jonj_shutdown_function_entry), NULL) != FAILURE;
}
/* }}} */

JONJAPI zend_bool remove_user_shutdown_function(char *function_name, size_t function_len TSRMLS_DC) /* {{{ */
{
	if (BG(user_shutdown_function_names)) {
...
JONJAPI zend_bool append_user_shutdown_function(jonj_shutdown_function_entry shutdown_function_entry TSRMLS_DC) /* {{{ */
{
	if (!BG(user_shutdown_function_names)) {
		ALLOC_HASHTABLE(BG(user_shutdown_function_names));
		zend_hash_init(BG(user_shutdown_function_names), 0, NULL, (void (*)(void *)) user_shutdown_function_dtor, 0);
	...
	return zend_hash_next_index_insert(BG(user_shutdown_function_names), &shutdown_function_entry, sizeof(jonj_shutdown_function_entry), NULL) != FAILURE;
}
/* }}} */
...
JONJ_FUNCTION(highlight_file)
{
	...
	if (jonj_check_open_basedir(filename TSRMLS_CC)) {
		RETURN_FALSE;
	}
	...
	if (i) {
		jonj_output_get_contents(return_value TSRMLS_CC);
...
/* {{{ proto string jonj_strip_whitespace(string file_name)
...
static int jonj_ini_get_option(zend_ini_entry *ini_entry TSRMLS_DC, int num_args, va_list args, zend_hash_key *hash_key) /* {{{ */
{
...
JONJ_FUNCTION(ini_get_all)
{
	...
	if (extname) {
		if (zend_hash_find(&module_registry, extname, extname_len+1, (void **) &module) == FAILURE) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find extension '%s'", extname);
			RETURN_FALSE;
		}
	...
	array_init(return_value);
	zend_hash_apply_with_arguments(EG(ini_directives) TSRMLS_CC, (apply_func_args_t) jonj_ini_get_option, 2, return_value, extnumber, details);
}
/* }}} */

static int jonj_ini_check_path(char *option_name, int option_len, char *new_option_name, int new_option_len) /* {{{ */
{
...
#define _CHECK_PATH(var, var_len, ini) jonj_ini_check_path(var, var_len, ini, sizeof(ini))
	/* open basedir check */
	if (PG(open_basedir)) {
		if (_CHECK_PATH(varname, varname_len, "error_log") ||
			...
			if (jonj_check_open_basedir(new_value TSRMLS_CC)) {
	...
	if (zend_alter_ini_entry_ex(varname, varname_len + 1, new_value, new_value_len, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC) == FAILURE) {
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
JONJ_FUNCTION(ini_restore)
{
	...
	zend_restore_ini_entry(varname, varname_len+1, JONJ_INI_STAGE_RUNTIME);
}
/* }}} */
...
JONJ_FUNCTION(set_include_path)
{
	...
	if (zend_alter_ini_entry_ex("include_path", sizeof("include_path"), new_value, new_value_len, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC) == FAILURE) {
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
JONJ_FUNCTION(restore_include_path)
{
	...
	zend_restore_ini_entry("include_path", sizeof("include_path"), JONJ_INI_STAGE_RUNTIME);
}
/* }}} */
...
{
	RETURN_LONG(PG(connection_status) & JONJ_CONNECTION_ABORTED);
}
/* }}} */
...
JONJ_FUNCTION(ignore_user_abort)
{
	...
	if (arg) {
		zend_alter_ini_entry_ex("ignore_user_abort", sizeof("ignore_user_abort"), arg, arg_len, JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
	}
...
JONJ_FUNCTION(register_tick_function)
{
	...
	if (!zend_is_callable(tick_fe.arguments[0], 0, &function_name TSRMLS_CC)) {
		efree(tick_fe.arguments);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid tick callback '%s' passed", function_name);
		efree(function_name);
		RETURN_FALSE;
	...
	if (!BG(user_tick_functions)) {
		...
		jonj_add_tick_function(run_user_tick_functions);
	}
...
JONJ_FUNCTION(move_uploaded_file)
{
	...
	if (jonj_check_open_basedir(new_path TSRMLS_CC)) {
		RETURN_FALSE;
	}
	...
	if (VCWD_RENAME(path, new_path) == 0) {
		...
		if (ret == -1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", strerror(errno));
		}
#endif
	} else if (jonj_copy_file_ex(path, new_path, STREAM_DISABLE_OPEN_BASEDIR TSRMLS_CC) == SUCCESS) {
		VCWD_UNLINK(path);
		successful = 1;
	}
	...
	if (successful) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to move '%s' to '%s'", path, new_path);
	}
...
/* {{{ jonj_simple_ini_parser_cb
 */
static void jonj_simple_ini_parser_cb(zval *arg1, zval *arg2, zval *arg3, int callback_type, zval *arr TSRMLS_DC)
{
	zval *element;
...
/* {{{ jonj_ini_parser_cb_with_sections
 */
static void jonj_ini_parser_cb_with_sections(zval *arg1, zval *arg2, zval *arg3, int callback_type, zval *arr TSRMLS_DC)
{
	if (callback_type == ZEND_INI_PARSER_SECTION) {
		...
		jonj_simple_ini_parser_cb(arg1, arg2, arg3, callback_type, active_arr TSRMLS_CC);
	}
}
/* }}} */
...
JONJ_FUNCTION(parse_ini_file)
{
	...
	if (filename_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filename cannot be empty!");
		RETURN_FALSE;
	}
	...
	if (process_sections) {
		...
		ini_parser_cb = (zend_ini_parser_cb_t) jonj_ini_parser_cb_with_sections;
	} else {
		ini_parser_cb = (zend_ini_parser_cb_t) jonj_simple_ini_parser_cb;
	}
 ...
 *  is not the same as ini_get_all() which returns only registered ini options. Only useful for devs to debug jonj.ini scanner/parser! */
JONJ_FUNCTION(config_get_hash) /* {{{ */
{
	HashTable *hash = jonj_ini_get_configuration_hash();
