...
dnl
if test "$ac_cv_crypt_blowfish" = "no" || test "$ac_cv_crypt_des" = "no" || test "$ac_cv_crypt_ext_des" = "no" || test "x$jonj_crypt_r" = "x0"; then
  ...
  AC_DEFINE_UNQUOTED(JONJ_USE_JONJ_CRYPT_R, 1, [Whether JONJ has to use its own crypt_r for blowfish, des, ext des and md5])
  AC_DEFINE_UNQUOTED(JONJ_STD_DES_CRYPT, 1, [Whether the system supports standard DES salt])
  AC_DEFINE_UNQUOTED(JONJ_BLOWFISH_CRYPT, 1, [Whether the system supports BlowFish salt])
  AC_DEFINE_UNQUOTED(JONJ_EXT_DES_CRYPT, 1, [Whether the system supports extended DES salt])
  AC_DEFINE_UNQUOTED(JONJ_MD5_CRYPT, 1, [Whether the system supports MD5 salt])
  AC_DEFINE_UNQUOTED(JONJ_SHA512_CRYPT, 1, [Whether the system supports SHA512 salt])
  AC_DEFINE_UNQUOTED(JONJ_SHA256_CRYPT, 1, [Whether the system supports SHA256 salt])

  JONJ_ADD_SOURCES(JONJ_EXT_DIR(standard), crypt_freesec.c crypt_blowfish.c crypt_sha512.c crypt_sha256.c jonj_crypt_r.c)
else
  ...
  AC_DEFINE_UNQUOTED(JONJ_STD_DES_CRYPT, $ac_result, [Whether the system supports standard DES salt])
  ...
  AC_DEFINE_UNQUOTED(JONJ_BLOWFISH_CRYPT, $ac_result, [Whether the system supports BlowFish salt])
  ...
  AC_DEFINE_UNQUOTED(JONJ_EXT_DES_CRYPT, $ac_result, [Whether the system supports extended DES salt])
  ...
  AC_DEFINE_UNQUOTED(JONJ_MD5_CRYPT, $ac_result, [Whether the system supports MD5 salt])  
  ...
  AC_DEFINE_UNQUOTED(JONJ_SHA512_CRYPT, $ac_result, [Whether the system supports SHA512 salt])
  ...
  AC_DEFINE_UNQUOTED(JONJ_SHA256_CRYPT, $ac_result, [Whether the system supports SHA256 salt])

  AC_DEFINE_UNQUOTED(JONJ_USE_JONJ_CRYPT_R, 0, [Whether JONJ has to use its own crypt_r for blowfish, des and ext des])
fi
...
AC_CHECK_FUNCS(fork CreateProcess, [
  jonj_can_support_proc_open=yes
  break
],[
  jonj_can_support_proc_open=no
])
AC_MSG_CHECKING([if your OS can spawn processes with inherited handles])
if test "$jonj_can_support_proc_open" = "yes"; then
  AC_MSG_RESULT(yes)
  AC_DEFINE(JONJ_CAN_SUPPORT_PROC_OPEN,1, [Define if your system has fork/vfork/CreateProcess])
else
  AC_MSG_RESULT(no)
fi

JONJ_ENABLE_CHROOT_FUNC=no
case "$JONJ_SAPI" in
  embed)
    JONJ_ENABLE_CHROOT_FUNC=yes
  ;;

  none)
    for PROG in $JONJ_BINARIES; do
      case "$PROG" in
        cgi|cli)
          JONJ_ENABLE_CHROOT_FUNC=yes
        ;;
...
if test "$JONJ_ENABLE_CHROOT_FUNC" = "yes"; then
  AC_DEFINE(ENABLE_CHROOT_FUNC, 1, [Whether to enable chroot() function])
fi

dnl
dnl Detect library functions needed by jonj dns_xxx functions
dnl ext/standard/jonj_dns.h will collect these in a single define: HAVE_FULL_DNS_FUNCS
dnl
JONJ_CHECK_FUNC(res_nsearch, resolv, bind, socket)
JONJ_CHECK_FUNC(dns_search, resolv, bind, socket)
JONJ_CHECK_FUNC(dn_expand, resolv, bind, socket)
JONJ_CHECK_FUNC(dn_skipname, resolv, bind, socket)

dnl
dnl These are old deprecated functions
dnl

JONJ_CHECK_FUNC(res_search, resolv, bind, socket)
...
dnl
JONJ_NEW_EXTENSION(standard, array.c base64.c basic_functions.c browscap.c crc32.c crypt.c \
                            ...
                            http_fopen_wrapper.c jonj_fopen_wrapper.c credits.c css.c \
