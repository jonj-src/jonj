...
#ifdef _OSD_POSIX
#ifndef APACHE
#error On this EBCDIC platform, JONJ is only supported as an Apache module.
#else /*APACHE*/
...
/* {{{ jonj_replace_controlchars
 */
JONJAPI char *jonj_replace_controlchars_ex(char *str, int len)
{
...
JONJAPI char *jonj_replace_controlchars(char *str)
{
	return jonj_replace_controlchars_ex(str, strlen(str));
}

JONJAPI jonj_url *jonj_url_parse(char const *str)
{
	return jonj_url_parse_ex(str, strlen(str));
}
...
{
	char port_buf[6];
	jonj_url *ret = ecalloc(1, sizeof(jonj_url));
	...
	if ((e = memchr(s, ':', length)) && e != s) {
		...
		if (e + 1 == ue) { /* only scheme is available */
			...
			jonj_replace_controlchars_ex(ret->scheme, (e - s));
			return ret;
		}
	...
	if ((p = zend_memrchr(s, '@', (e-s)))) {
		if ((pp = memchr(s, ':', (p-s)))) {
			...
			jonj_replace_controlchars_ex(ret->user, (pp - s));

			pp++;
			ret->pass = estrndup(pp, (p-pp));
			jonj_replace_controlchars_ex(ret->pass, (p-pp));
		} else {
			ret->user = estrndup(s, (p-s));
			jonj_replace_controlchars_ex(ret->user, (p-s));
		}
	...
	jonj_replace_controlchars_ex(ret->host, (p - s));
	...
	if (p) {
		...
		if (p < e) {
			...
			jonj_replace_controlchars_ex(ret->fragment, (e - p));
		}
		e = p-1;
	}
	...
	if (p) {
		...
		if (p < e) {
			...
			jonj_replace_controlchars_ex(ret->query, (e - p));
		}
		e = p-1;
	}
	...
	if (s < e || s == ue) {
		...
		jonj_replace_controlchars_ex(ret->path, (e - s));
	}
...
JONJ_FUNCTION(parse_url)
{
	...
	resource = jonj_url_parse_ex(str, str_len);
	if (resource == NULL) {
		/* @todo Find a method to determine why jonj_url_parse_ex() failed */
		RETURN_FALSE;
	}

	if (key > -1) {
		switch (key) {
			case JONJ_URL_SCHEME:
			...
			case JONJ_URL_QUERY:
				...
				break;
			case JONJ_URL_FRAGMENT:
				if (resource->fragment != NULL) RETVAL_STRING(resource->fragment, 1);
				break;
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid URL component identifier %ld", key);
				RETVAL_FALSE;
		}
		goto done;
	}
...
JONJAPI char *jonj_url_encode(char const *s, int len, int *new_length)
{
	...
	if ((to-start) > INT_MAX) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "String overflow, max length is %d", INT_MAX);
	}
...
JONJ_FUNCTION(urlencode)
{
	...
	out_str = jonj_url_encode(in_str, in_str_len, &out_str_len);
...
JONJ_FUNCTION(urldecode)
{
	...
	out_str_len = jonj_url_decode(out_str, in_str_len);
	...
	while (len--) {
		if (*data == '+') {
			*dest = ' ';
				 ...
				 && isxdigit((int) *(data + 2))) {
#ifndef CHARSET_EBCDIC
			*dest = (char) jonj_htoi(data + 1);
#else
			*dest = os_toebcdic[(char) jonj_htoi(data + 1)];
#endif
			data += 2;
			len -= 2;
		} else {
...
JONJ_FUNCTION(rawurlencode)
{
	...
	out_str = jonj_raw_url_encode(in_str, in_str_len, &out_str_len);
	RETURN_STRINGL(out_str, out_str_len, 0);
}
/* }}} */
...
JONJ_FUNCTION(rawurldecode)
{
	...
	out_str = estrndup(in_str, in_str_len);
	out_str_len = jonj_raw_url_decode(out_str, in_str_len);
...
/* {{{ jonj_raw_url_decode
 */
JONJAPI int jonj_raw_url_decode(char *str, int len)
{
	char *dest = str;
	char *data = str;
...
JONJ_FUNCTION(get_headers)
{
	...
	context = FG(default_context) ? FG(default_context) : (FG(default_context) = jonj_stream_context_alloc(TSRMLS_C));

	if (!(stream = jonj_stream_open_wrapper_ex(url, "r", REPORT_ERRORS | STREAM_USE_URL | STREAM_ONLY_GET_HEADERS, NULL, context))) {
		RETURN_FALSE;
	}
