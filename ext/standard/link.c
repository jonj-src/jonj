...
JONJ_FUNCTION(readlink)
{
	...
	if (jonj_check_open_basedir(link TSRMLS_CC)) {
		RETURN_FALSE;
	}

	ret = jonj_sys_readlink(link, buff, MAXPATHLEN-1);
...
JONJ_FUNCTION(linkinfo)
{
	...
	dir_len = jonj_dirname(dirname, link_len);

	if (jonj_check_open_basedir(dirname TSRMLS_CC)) {
		efree(dirname);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(symlink)
{
	...
	if (jonj_check_open_basedir(dest_p TSRMLS_CC)) {
		RETURN_FALSE;
	}

	if (jonj_check_open_basedir(source_p TSRMLS_CC)) {
		RETURN_FALSE;
	}
