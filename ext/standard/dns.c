...
static char *jonj_gethostbyaddr(char *ip);
static char *jonj_gethostbyname(char *name);
...
JONJ_FUNCTION(gethostname)
{
	...
	if (gethostname(buf, sizeof(buf) - 1)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to fetch host [%d]: %s", errno, strerror(errno));
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(gethostbyaddr)
{
	...
	hostname = jonj_gethostbyaddr(addr);

	if (hostname == NULL) {
#if HAVE_IPV6 && HAVE_INET_PTON
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Address is not a valid IPv4 or IPv6 address");
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Address is not in a.b.c.d form");
#endif
		RETVAL_FALSE;
	} else {
...
JONJ_FUNCTION(gethostbyname)
{
	...
	if(hostname_len > MAXFQDNLEN) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Host name is too long, the limit is %d characters", MAXFQDNLEN);
		RETURN_STRINGL(hostname, hostname_len, 1);
	}
	addr = jonj_gethostbyname(hostname);
...
#if HAVE_FULL_DNS_FUNCS || defined(JONJ_WIN32)
...
#endif /* HAVE_FULL_DNS_FUNCS || defined(JONJ_WIN32) */
...
#define jonj_dns_free_res(__res__) _jonj_dns_free_res(__res__)
static void _jonj_dns_free_res(struct __res_state res) { /* {{{ */
	int ns;
...
#else
#define jonj_dns_free_res(__res__)
#endif
...
#define MAXPACKET  8192 /* max packet size used internally by BIND */
	...
	RETVAL_TRUE;
	i = jonj_dns_search(handle, hostname, C_IN, type, ans, sizeof(ans));

	if (i < 0) {
		RETVAL_FALSE;
	}

	jonj_dns_free_handle(handle);
}
/* }}} */
...
/* {{{ jonj_parserr */
static u_char *jonj_parserr(u_char *cp, u_char *end, querybuf *answer, int type_to_fetch, int store, int raw, zval **subarray)
{
...
JONJ_FUNCTION(dns_get_record)
{
	 ...
	 * - In case of JONJ_DNS_ANY we use the directly fetch DNS_T_ANY. (step NUMTYPES+1 )
	...
	for ( ;
		...
		switch (type) {
			case -1: /* raw */
				...
				break;
			case 0:
				type_to_fetch = type_param&JONJ_DNS_A     ? DNS_T_A     : 0;
				break;
			case 1:
				type_to_fetch = type_param&JONJ_DNS_NS    ? DNS_T_NS    : 0;
				break;
			case 2:
				type_to_fetch = type_param&JONJ_DNS_CNAME ? DNS_T_CNAME : 0;
				break;
			case 3:
				type_to_fetch = type_param&JONJ_DNS_SOA   ? DNS_T_SOA   : 0;
				break;
			case 4:
				type_to_fetch = type_param&JONJ_DNS_PTR   ? DNS_T_PTR   : 0;
				break;
			case 5:
				type_to_fetch = type_param&JONJ_DNS_HINFO ? DNS_T_HINFO : 0;
				break;
			case 6:
				type_to_fetch = type_param&JONJ_DNS_MX    ? DNS_T_MX    : 0;
				break;
			case 7:
				type_to_fetch = type_param&JONJ_DNS_TXT   ? DNS_T_TXT   : 0;
				break;
			case 8:
				type_to_fetch = type_param&JONJ_DNS_AAAA	 ? DNS_T_AAAA  : 0;
				break;
			case 9:
				type_to_fetch = type_param&JONJ_DNS_SRV   ? DNS_T_SRV   : 0;
				break;
			case 10:
				type_to_fetch = type_param&JONJ_DNS_NAPTR ? DNS_T_NAPTR : 0;
				break;
			case 11:
				type_to_fetch = type_param&JONJ_DNS_A6	 ? DNS_T_A6 : 0;
				break;
		...
		if (type_to_fetch) {
			...
			n = jonj_dns_search(handle, hostname, C_IN, type_to_fetch, answer.qb2, sizeof answer);
			...
			if (n < 0) {
				...
				switch (h_errno) {
					...
					case NO_RECOVERY:
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An unexpected server failure occurred.");
						break;

					case TRY_AGAIN:
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "A temporary server error occurred.");
						break;
			...
			while (qd-- > 0) {
				...
				if (n < 0) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to parse DNS data received");
					zval_dtor(return_value);
					jonj_dns_free_handle(handle);
					RETURN_FALSE;
				}
			...
			while (an-- && cp && cp < end) {
				...
				cp = jonj_parserr(cp, end, &answer, type_to_fetch, store_results, raw, &retval);
			...
			if (authns || addtl) {
				...
				while (ns-- > 0 && cp && cp < end) {
					...
					cp = jonj_parserr(cp, end, &answer, DNS_T_ANY, authns != NULL, raw, &retval);
					if (retval != NULL) {
			...
			if (addtl) {
				...
				while (ar-- > 0 && cp && cp < end) {
					...
					cp = jonj_parserr(cp, end, &answer, DNS_T_ANY, 1, raw, &retval);
					if (retval != NULL) {
...
JONJ_FUNCTION(dns_get_mx)
{
	...
	i = jonj_dns_search(handle, hostname, C_IN, DNS_T_MX, (u_char *)&ans, sizeof(ans));
	if (i < 0) {
		RETURN_FALSE;
	}
	if (i > (int)sizeof(ans)) {
...
JONJ_MINIT_FUNCTION(dns) {
	REGISTER_LONG_CONSTANT("DNS_A",     JONJ_DNS_A,     CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_NS",    JONJ_DNS_NS,    CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_CNAME", JONJ_DNS_CNAME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_SOA",   JONJ_DNS_SOA,   CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_PTR",   JONJ_DNS_PTR,   CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_HINFO", JONJ_DNS_HINFO, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_MX",    JONJ_DNS_MX,    CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_TXT",   JONJ_DNS_TXT,   CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_SRV",   JONJ_DNS_SRV,   CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_NAPTR", JONJ_DNS_NAPTR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_AAAA",  JONJ_DNS_AAAA,  CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_A6",    JONJ_DNS_A6,    CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_ANY",   JONJ_DNS_ANY,   CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("DNS_ALL",   JONJ_DNS_ALL,   CONST_CS | CONST_PERSISTENT);
	return SUCCESS;
}
#endif /* HAVE_FULL_DNS_FUNCS */
