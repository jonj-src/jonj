...
#ifdef ZTS
MUTEX_T jonj_crypt_extended_init_lock;
#endif
...
{
#ifdef ZTS
	jonj_crypt_extended_init_lock = tsrm_mutex_alloc();
#endif
}
...
{
#ifdef ZTS
	tsrm_mutex_free(jonj_crypt_extended_init_lock);
#endif
}
...
#ifdef ZTS
	tsrm_mutex_lock(jonj_crypt_extended_init_lock);
#endif
...
#ifdef ZTS
	tsrm_mutex_unlock(jonj_crypt_extended_init_lock);
#endif
}
...
#if JONJ_WIN32
char * jonj_md5_crypt_r(const char *pw, const char *salt, char *out) {
...
char * jonj_md5_crypt_r(const char *pw, const char *salt, char *out)
{
	...
	/* The password first, since that is what is most unknown */
	JONJ_MD5Update(&ctx, (const unsigned char *)pw, pwl);

	/* Then our magic string */
	JONJ_MD5Update(&ctx, (const unsigned char *)MD5_MAGIC, MD5_MAGIC_LEN);

	/* Then the raw salt */
	JONJ_MD5Update(&ctx, (const unsigned char *)sp, sl);

	/* Then just as many characters of the MD5(pw,salt,pw) */
	JONJ_MD5Init(&ctx1);
	JONJ_MD5Update(&ctx1, (const unsigned char *)pw, pwl);
	JONJ_MD5Update(&ctx1, (const unsigned char *)sp, sl);
	JONJ_MD5Update(&ctx1, (const unsigned char *)pw, pwl);
	JONJ_MD5Final(final, &ctx1);

	for (pl = pwl; pl > 0; pl -= 16)
		JONJ_MD5Update(&ctx, final, (unsigned int)(pl > 16 ? 16 : pl));
	...
	for (i = pwl; i != 0; i >>= 1)
		if ((i & 1) != 0)
		    JONJ_MD5Update(&ctx, final, 1);
		else
		    JONJ_MD5Update(&ctx, (const unsigned char *)pw, 1);
