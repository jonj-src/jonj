 ...
 * Essentially, it is set if you HAVE_FORK || JONJ_WIN32
...
/* {{{ _jonj_array_to_envp */
static jonj_process_env_t _jonj_array_to_envp(zval *environment, int is_persistent TSRMLS_DC)
{
	zval **element;
...
/* {{{ _jonj_free_envp */
static void _jonj_free_envp(jonj_process_env_t env, int is_persistent)
{
#ifndef JONJ_WIN32
	if (env.envarray) {
...
{
	struct jonj_process_handle *proc = (struct jonj_process_handle*)rsrc->ptr;
	int i;
#ifdef JONJ_WIN32
	DWORD wstatus;
	...
	FG(pclose_ret) = -1;
#endif
	_jonj_free_envp(proc->env, proc->is_persistent);
...
{
	zval *zproc;
	struct jonj_process_handle *proc;
	long sig_no = SIGTERM;
	...
	ZEND_FETCH_RESOURCE(proc, struct jonj_process_handle *, &zproc, -1, "process", le_proc_open);
...
struct jonj_proc_open_descriptor_item {
	int index; 							/* desired fd number in child process */
	jonj_file_descriptor_t parentend, childend;	/* fds for pipes in parent/child */
...
JONJ_FUNCTION(proc_open)
{
	...
	zval **descitem = NULL;
	HashPosition pos;
	struct jonj_proc_open_descriptor_item descriptors[JONJ_PROC_OPEN_MAX_DESCRIPTORS];
#ifdef JONJ_WIN32
...
#endif
#if JONJ_CAN_DO_PTS
	jonj_file_descriptor_t dev_ptmx = -1;	/* master */
	jonj_file_descriptor_t slave_pty = -1;
#endif
	...
	if (environment) {
		env = _jonj_array_to_envp(environment, is_persistent TSRMLS_CC);
	} else {
		memset(&env, 0, sizeof(env));
	}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(descriptorspec), (void **)&descitem, &pos) == SUCCESS) {
		...
		if (str_index) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "descriptor spec must be an integer indexed array");
			goto exit_fail;
		}
		...
		if (Z_TYPE_PP(descitem) == IS_RESOURCE) {
			...
			jonj_stream_from_zval(stream, descitem);
			...
			if (descriptors[ndesc].childend == NULL) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to dup File-Handle for descriptor %d", nindex);
				goto exit_fail;
			}
#else
			descriptors[ndesc].childend = dup(fd);
			if (descriptors[ndesc].childend < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to dup File-Handle for descriptor %ld - %s", nindex, strerror(errno));
				goto exit_fail;
			}
#endif
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Descriptor item must be either an array or a File-Handle");
			goto exit_fail;
		} else {
			...
			if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 0, (void **)&ztype) == SUCCESS) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing handle qualifier in array");
				goto exit_fail;
			}

			if (strcmp(Z_STRVAL_PP(ztype), "pipe") == 0) {
				jonj_file_descriptor_t newpipe[2];
				zval **zmode;

				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zmode) == SUCCESS) {
					convert_to_string_ex(zmode);
				} else {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing mode parameter for 'pipe'");
					goto exit_fail;
				}
				...
				if (0 != pipe(newpipe)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to create pipe %s", strerror(errno));
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zfile) == SUCCESS) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing file name parameter for 'file'");
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 2, (void **)&zmode) == SUCCESS) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing mode parameter for 'file'");
					goto exit_fail;
				}

				/* try a wrapper */
				stream = jonj_stream_open_wrapper(Z_STRVAL_PP(zfile), Z_STRVAL_PP(zmode),
						REPORT_ERRORS|STREAM_WILL_CAST, NULL);

				/* force into an fd */
				if (stream == NULL || FAILURE == jonj_stream_cast(stream,
							JONJ_STREAM_CAST_RELEASE|JONJ_STREAM_AS_FD,
				...
				if (dev_ptmx == -1) {
					...
					if (dev_ptmx == -1) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to open /dev/ptmx, errno %d", errno);
						goto exit_fail;
					}
					grantpt(dev_ptmx);
					unlockpt(dev_ptmx);
					...
					if (slave_pty == -1) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to open slave pty, errno %d", errno);
						goto exit_fail;
					}
				}
				descriptors[ndesc].mode = DESC_PIPE;
...
#else
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "pty pseudo terminal not supported on this system");
				goto exit_fail;
#endif
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s is not a valid descriptor spec/mode", Z_STRVAL_PP(ztype));
				goto exit_fail;
			}
		}

		zend_hash_move_forward_ex(Z_ARRVAL_P(descriptorspec), &pos);
		if (++ndesc == JONJ_PROC_OPEN_MAX_DESCRIPTORS)
			break;
	}
	...
	if (cwd == NULL) {
		...
		if (!getcwd_result) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot get current directory");
			goto exit_fail;
		}
		cwd = cur_cwd;
	}
	...
	if (FALSE == newprocok) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "CreateProcess failed, error code - %u", dw);
		goto exit_fail;
	}
	...
	if (child < 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "procve failed - %s", strerror(errno));
		goto exit_fail;
	}
#elif HAVE_FORK
	/* the unix way */
	...
	if (child == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "fork failed - %s", strerror(errno));
...
# error You lose (configure should not have let you get here)
	...
	proc = (struct jonj_process_handle*)pemalloc(sizeof(struct jonj_process_handle), is_persistent);
...
#ifdef JONJ_WIN32
				stream = jonj_stream_fopen_from_fd(_open_osfhandle((zend_intptr_t)descriptors[i].parentend,
							descriptors[i].mode_flags), mode_string, NULL);
#else
				stream = jonj_stream_fopen_from_fd(descriptors[i].parentend, mode_string, NULL);
...
# endif
				...
				if (stream) {
					...
					MAKE_STD_ZVAL(retfp);
					jonj_stream_to_zval(stream, retfp);
...
exit_fail:
	_jonj_free_envp(env, is_persistent);
...
#endif /* JONJ_CAN_SUPPORT_PROC_OPEN */
