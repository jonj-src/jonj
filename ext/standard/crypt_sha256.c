...
char * __jonj_stpncpy(char *dst, const char *src, size_t len)
{
	size_t n = strlen(src);
	if (n > len) {
		n = len;
...
void * __jonj_mempcpy(void * dst, const void * src, size_t len)
{
...
char * jonj_sha256_crypt_r(const char *key, const char *salt, char *buffer, int buflen)
{
#ifdef JONJ_WIN32
# if _MSC <= 1300
#  pragma pack(push, 16)
...
# endif
	...
	for (cnt = key_len; cnt >= 32; cnt -= 32) {
		cp = __jonj_mempcpy((void *)cp, (const void *)temp_result, 32);
	}
	...
	for (cnt = salt_len; cnt >= 32; cnt -= 32) {
		cp = __jonj_mempcpy(cp, temp_result, 32);
	}
	memcpy(cp, temp_result, cnt);
	...
	parts.  */
	cp = __jonj_stpncpy(buffer, sha256_salt_prefix, MAX(0, buflen));
...
/* This entry point is equivalent to the `crypt' function in Unix
   libcs.  */
char * jonj_sha256_crypt(const char *key, const char *salt)
{
	...
	return jonj_sha256_crypt_r(key, salt, buffer, buflen);
}
...
int main(void) {
	...
	for (cnt = 0; cnt < ntests2; ++cnt) {
		char *cp = jonj_sha256_crypt(tests2[cnt].input, tests2[cnt].salt);
