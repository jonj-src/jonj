...
# define SET(n) \
	(ctx->block[(n)] = \
	(jonj_uint32)ptr[(n) * 4] | \
	((jonj_uint32)ptr[(n) * 4 + 1] << 8) | \
	((jonj_uint32)ptr[(n) * 4 + 2] << 16) | \
	((jonj_uint32)ptr[(n) * 4 + 3] << 24))
# define GET(n) \
	(ctx->block[(n)])
#endif
 ...
 */
static const void *body(JONJ_MD5_CTX *ctx, const void *data, size_t size)
{
	const unsigned char *ptr;
	jonj_uint32 a, b, c, d;
	jonj_uint32 saved_a, saved_b, saved_c, saved_d;
...
{
	jonj_uint32 saved_lo;
	jonj_uint32 used, free;
