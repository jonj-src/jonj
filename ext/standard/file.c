/*
   ...
   | JONJ streams by Wez Furlong (wez@thebrainroom.com)                    |
   +----------------------------------------------------------------------+
*/

/* $Id$ */

/* Synced with jonj 3.0 revision 1.218 1999-06-16 [ssb] */
...
int file_globals_id;
#else
jonj_file_globals file_globals;
#endif

#if defined(HAVE_FNMATCH) && !defined(JONJ_WIN32)
# ifndef _GNU_SOURCE
#  define _GNU_SOURCE
# endif
# include <fnmatch.h>
...
#define JONJ_STREAM_TO_ZVAL(stream, arg) \
	jonj_stream_from_zval_no_verify(stream, arg); \
	if (stream == NULL) {	\
		RETURN_FALSE;	\
	}
...
{
	jonj_stream_context *context = (jonj_stream_context*)rsrc->ptr;
	if (context->options) {
		zval_ptr_dtor(&context->options);
		context->options = NULL;
	}
	jonj_stream_context_free(context);
}

static void file_globals_ctor(jonj_file_globals *file_globals_p TSRMLS_DC)
{
	FG(pclose_ret) = 0;
	FG(pclose_wait) = 0;
	FG(user_stream_current_filename) = NULL;
	FG(def_chunk_size) = JONJ_SOCK_CHUNK_SIZE;
	FG(wrapper_errors) = NULL;
}

static void file_globals_dtor(jonj_file_globals *file_globals_p TSRMLS_DC)
{
}

JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("user_agent", NULL, JONJ_INI_ALL, OnUpdateString, user_agent, jonj_file_globals, file_globals)
	STD_JONJ_INI_ENTRY("from", NULL, JONJ_INI_ALL, OnUpdateString, from_address, jonj_file_globals, file_globals)
	STD_JONJ_INI_ENTRY("default_socket_timeout", "60", JONJ_INI_ALL, OnUpdateLong, default_socket_timeout, jonj_file_globals, file_globals)
	STD_JONJ_INI_ENTRY("auto_detect_line_endings", "0", JONJ_INI_ALL, OnUpdateLong, auto_detect_line_endings, jonj_file_globals, file_globals)
JONJ_INI_END()
...
JONJ_MINIT_FUNCTION(file)
{
...
#ifdef ZTS
	ts_allocate_id(&file_globals_id, sizeof(jonj_file_globals), (ts_allocate_ctor) file_globals_ctor, (ts_allocate_dtor) file_globals_dtor);
#else
	...
	REGISTER_LONG_CONSTANT("LOCK_SH", JONJ_LOCK_SH, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_EX", JONJ_LOCK_EX, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_UN", JONJ_LOCK_UN, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_NB", JONJ_LOCK_NB, CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_CONNECT",			JONJ_STREAM_NOTIFY_CONNECT,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_AUTH_REQUIRED",	JONJ_STREAM_NOTIFY_AUTH_REQUIRED,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_AUTH_RESULT",		JONJ_STREAM_NOTIFY_AUTH_RESULT,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_MIME_TYPE_IS",	JONJ_STREAM_NOTIFY_MIME_TYPE_IS,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_FILE_SIZE_IS",	JONJ_STREAM_NOTIFY_FILE_SIZE_IS,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_REDIRECTED",		JONJ_STREAM_NOTIFY_REDIRECTED,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_PROGRESS",		JONJ_STREAM_NOTIFY_PROGRESS,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_FAILURE",			JONJ_STREAM_NOTIFY_FAILURE,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_COMPLETED",		JONJ_STREAM_NOTIFY_COMPLETED,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_RESOLVE",			JONJ_STREAM_NOTIFY_RESOLVE,			CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_INFO",	JONJ_STREAM_NOTIFY_SEVERITY_INFO,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_WARN",	JONJ_STREAM_NOTIFY_SEVERITY_WARN,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_ERR",	JONJ_STREAM_NOTIFY_SEVERITY_ERR,		CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_FILTER_READ",			JONJ_STREAM_FILTER_READ,				CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_FILTER_WRITE",			JONJ_STREAM_FILTER_WRITE,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_FILTER_ALL",				JONJ_STREAM_FILTER_ALL,				CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_CLIENT_PERSISTENT",		JONJ_STREAM_CLIENT_PERSISTENT,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_CLIENT_ASYNC_CONNECT",	JONJ_STREAM_CLIENT_ASYNC_CONNECT,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_CLIENT_CONNECT",			JONJ_STREAM_CLIENT_CONNECT,	CONST_CS | CONST_PERSISTENT);
...
# endif
	...
	REGISTER_LONG_CONSTANT("FILE_USE_INCLUDE_PATH",			JONJ_FILE_USE_INCLUDE_PATH,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_IGNORE_NEW_LINES",			JONJ_FILE_IGNORE_NEW_LINES,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_SKIP_EMPTY_LINES",			JONJ_FILE_SKIP_EMPTY_LINES,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_APPEND",					JONJ_FILE_APPEND,					CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_NO_DEFAULT_CONTEXT",		JONJ_FILE_NO_DEFAULT_CONTEXT,		CONST_CS | CONST_PERSISTENT);
...
JONJ_FUNCTION(flock)
{
	...
	JONJ_STREAM_TO_ZVAL(stream, &res);

	act = operation & 3;
	if (act < 1 || act > 3) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal operation argument");
		RETURN_FALSE;
	}
	...
	act = flock_values[act - 1] | (operation & JONJ_LOCK_NB ? LOCK_NB : 0);
	if (jonj_stream_lock(stream, act)) {
...
JONJ_FUNCTION(get_meta_tags)
{
	...
	jonj_meta_tags_token tok, tok_last;
	jonj_meta_tags_data md;
	...
	md.stream = jonj_stream_open_wrapper(filename, "rb",
	...
	while (!done && (tok = jonj_next_meta_token(&md TSRMLS_CC)) != TOK_EOF) {
		if (tok == TOK_ID) {
			if (tok_last == TOK_OPENTAG) {
				...
				if (saw_name) {
					...
					while (temp && *temp) {
						if (strchr(JONJ_META_UNSAFE, *temp)) {
							*temp = '_';
						}
						temp++;
					}
			...
			if (have_name) {
				/* For BC */
				jonj_strtolower(name, strlen(name));
				if (have_content) {
...
JONJ_FUNCTION(file_get_contents)
{
	...
	long len;
	long offset = -1;
	long maxlen = JONJ_STREAM_COPY_ALL;
	zval *zcontext = NULL;
	...
	if (ZEND_NUM_ARGS() == 5 && maxlen < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "length must be greater than or equal to zero");
		RETURN_FALSE;
	}

	context = jonj_stream_context_from_zval(zcontext, 0);

	stream = jonj_stream_open_wrapper_ex(filename, "rb",
				(use_include_path ? USE_PATH : 0) | REPORT_ERRORS,
				NULL, context);
	if (!stream) {
		RETURN_FALSE;
	...
	if (offset > 0 && jonj_stream_seek(stream, offset, SEEK_SET) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to seek to position %ld in the stream", offset);
		jonj_stream_close(stream);
		RETURN_FALSE;
	}

	if ((len = jonj_stream_copy_to_mem(stream, &contents, maxlen, 0)) > 0) {
		if (len > INT_MAX) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "content truncated from %ld to %d bytes", len, INT_MAX);
			len = INT_MAX;
		}
...
JONJ_FUNCTION(file_put_contents)
{
	...
	if (Z_TYPE_P(data) == IS_RESOURCE) {
		jonj_stream_from_zval(srcstream, &data);
	}

	context = jonj_stream_context_from_zval(zcontext, flags & JONJ_FILE_NO_DEFAULT_CONTEXT);

	if (flags & JONJ_FILE_APPEND) {
		mode[0] = 'a';
	} else if (flags & LOCK_EX) {
		/* check to make sure we are dealing with a regular file */
		if (jonj_memnstr(filename, "://", sizeof("://") - 1, filename + filename_len)) {
			if (strncasecmp(filename, "file://", sizeof("file://") - 1)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Exclusive locks may only be set for regular files");
				RETURN_FALSE;
			}
		}
		mode[0] = 'c';
	...
	stream = jonj_stream_open_wrapper_ex(filename, mode, ((flags & JONJ_FILE_USE_INCLUDE_PATH) ? USE_PATH : 0) | REPORT_ERRORS, NULL, context);
	if (stream == NULL) {
		RETURN_FALSE;
	}

	if (flags & LOCK_EX && (!jonj_stream_supports_lock(stream) || jonj_stream_lock(stream, LOCK_EX))) {
		jonj_stream_close(stream);
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Exclusive locks are not supported for this stream");
		RETURN_FALSE;
	}

	if (mode[0] == 'c') {
		jonj_stream_truncate_set_size(stream, 0);
	}

	switch (Z_TYPE_P(data)) {
		case IS_RESOURCE: {
			size_t len;
			if (jonj_stream_copy_to_stream_ex(srcstream, stream, JONJ_STREAM_COPY_ALL, &len) != SUCCESS) {
				numbytes = -1;
			} else {
				if (len > LONG_MAX) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "content truncated from %lu to %ld bytes", (unsigned long) len, LONG_MAX);
					len = LONG_MAX;
				}
				numbytes = len;
			}
		...
		case IS_STRING:
			if (Z_STRLEN_P(data)) {
				numbytes = jonj_stream_write(stream, Z_STRVAL_P(data), Z_STRLEN_P(data));
				if (numbytes != Z_STRLEN_P(data)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %ld of %d bytes written, possibly out of free disk space", numbytes, Z_STRLEN_P(data));
					numbytes = -1;
				}
			}
			break;
			...
			if (zend_hash_num_elements(Z_ARRVAL_P(data))) {
				...
				while (zend_hash_get_current_data_ex(Z_ARRVAL_P(data), (void **) &tmp, &pos) == SUCCESS) {
					...
					if (Z_STRLEN_PP(tmp)) {
						...
						bytes_written = jonj_stream_write(stream, Z_STRVAL_PP(tmp), Z_STRLEN_PP(tmp));
						if (bytes_written < 0 || bytes_written != Z_STRLEN_PP(tmp)) {
							if (bytes_written < 0) {
								jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to write %d bytes to %s", Z_STRLEN_PP(tmp), filename);
							} else {
								jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %d of %d bytes written, possibly out of free disk space", bytes_written, Z_STRLEN_PP(tmp));
							}
							numbytes = -1;
							break;
						}
		...
		case IS_OBJECT:
			if (Z_OBJ_HT_P(data) != NULL) {
				...
				if (zend_std_cast_object_tostring(data, &out, IS_STRING TSRMLS_CC) == SUCCESS) {
					numbytes = jonj_stream_write(stream, Z_STRVAL(out), Z_STRLEN(out));
					if (numbytes != Z_STRLEN(out)) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %ld of %d bytes written, possibly out of free disk space", numbytes, Z_STRLEN(out));
						numbytes = -1;
					}
					zval_dtor(&out);
					break;
...
JONJ_FUNCTION(file)
{
	...
	if (flags < 0 || flags > (JONJ_FILE_USE_INCLUDE_PATH | JONJ_FILE_IGNORE_NEW_LINES | JONJ_FILE_SKIP_EMPTY_LINES | JONJ_FILE_NO_DEFAULT_CONTEXT)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'%ld' flag is not supported", flags);
		RETURN_FALSE;
	}

	use_include_path = flags & JONJ_FILE_USE_INCLUDE_PATH;
	include_new_line = !(flags & JONJ_FILE_IGNORE_NEW_LINES);
	skip_blank_lines = flags & JONJ_FILE_SKIP_EMPTY_LINES;

	context = jonj_stream_context_from_zval(zcontext, flags & JONJ_FILE_NO_DEFAULT_CONTEXT);

	stream = jonj_stream_open_wrapper_ex(filename, "rb", (use_include_path ? USE_PATH : 0) | REPORT_ERRORS, NULL, context);
	if (!stream) {
		RETURN_FALSE;
	}
	...
	if ((target_len = jonj_stream_copy_to_mem(stream, &target_buf, JONJ_STREAM_COPY_ALL, 0))) {
		s = target_buf;
		e = target_buf + target_len;

		if (!(p = jonj_stream_locate_eol(stream, target_buf, target_len TSRMLS_CC))) {
			p = e;
			goto parse_eol;
		}

		if (stream->flags & JONJ_STREAM_FLAG_EOL_MAC) {
			eol_marker = '\r';
		}
...
JONJ_FUNCTION(tempnam)
{
	...
	if (jonj_check_open_basedir(dir TSRMLS_CC)) {
		RETURN_FALSE;
	}

	jonj_basename(prefix, prefix_len, NULL, 0, &p, &p_len TSRMLS_CC);
	if (p_len > 64) {
		p[63] = '\0';
	}
	...
	if ((fd = jonj_open_temporary_fd_ex(dir, p, &opened_path, 1 TSRMLS_CC)) >= 0) {
		close(fd);
...
JONJ_NAMED_FUNCTION(jonj_if_tmpfile)
{
	...
	stream = jonj_stream_fopen_tmpfile();
...
JONJ_NAMED_FUNCTION(jonj_if_fopen)
{
	...
	stream = jonj_stream_open_wrapper_ex(filename, mode, (use_include_path ? USE_PATH : 0) | REPORT_ERRORS, NULL, context);
...
JONJAPI JONJ_FUNCTION(fclose)
{
	...
	if ((stream->flags & JONJ_STREAM_FLAG_NO_FCLOSE) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%d is not a valid stream resource", stream->rsrc_id);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(popen)
{
	...
	if (!fp) {
		jonj_error_docref2(NULL TSRMLS_CC, command, posix_mode, E_WARNING, "%s", strerror(errno));
		efree(posix_mode);
		RETURN_FALSE;
	}

	stream = jonj_stream_fopen_from_pipe(fp, mode);

	if (stream == NULL)	{
		jonj_error_docref2(NULL TSRMLS_CC, command, mode, E_WARNING, "%s", strerror(errno));
		RETVAL_FALSE;
	} else {
		jonj_stream_to_zval(stream, return_value);
	}
...
JONJAPI JONJ_FUNCTION(fgets)
{
	...
	if (argc == 1) {
		...
		buf = jonj_stream_get_line(stream, NULL, 0, &line_len);
		if (buf == NULL) {
			goto exit_failed;
		}
	} else if (argc > 1) {
		if (len <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter must be greater than 0");
			RETURN_FALSE;
		}

		buf = ecalloc(len + 1, sizeof(char));
		if (jonj_stream_get_line(stream, buf, len, &line_len) == NULL) {
			goto exit_failed;
		}
	}
...
JONJAPI JONJ_FUNCTION(fgetc)
{
	...
	result = jonj_stream_getc(stream);
...
JONJAPI JONJ_FUNCTION(fgetss)
{
	...
	if ((retval = jonj_stream_get_line(stream, buf, len, &actual_len)) == NULL)	{
		if (buf != NULL) {
			efree(buf);
		}
		RETURN_FALSE;
	...
	retval_len = jonj_strip_tags(retval, actual_len, &stream->fgetss_state, allowed_tags, allowed_tags_len);
...
JONJ_FUNCTION(fscanf)
{
	...
	what = zend_fetch_resource(&file_handle TSRMLS_CC, -1, "File-Handle", &type, 2, jonj_file_le_stream(), jonj_file_le_pstream());
	...
	buf = jonj_stream_get_line((jonj_stream *) what, NULL, 0, &len);
	if (buf == NULL) {
		if (args) {
			efree(args);
		}
	...
	result = jonj_sscanf_internal(buf, format, argc, args, 0, &return_value TSRMLS_CC);
...
JONJAPI JONJ_FUNCTION(fwrite)
{
	...
	ret = jonj_stream_write(stream, input, num_bytes);
...
JONJAPI JONJ_FUNCTION(rewind)
{
	...
	if (-1 == jonj_stream_rewind(stream)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
JONJAPI JONJ_FUNCTION(fseek)
{
	...
	RETURN_LONG(jonj_stream_seek(stream, offset, whence));
}
/* }}} */
...
/* DEPRECATED APIs: Use jonj_stream_mkdir() instead */
...
{
	return jonj_mkdir_ex(dir, mode, REPORT_ERRORS TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mkdir)
{
	...
	RETURN_BOOL(jonj_stream_mkdir(dir, mode, (recursive ? JONJ_STREAM_MKDIR_RECURSIVE : 0) | REPORT_ERRORS, context));
}
/* }}} */
...
JONJ_FUNCTION(rmdir)
{
	...
	RETURN_BOOL(jonj_stream_rmdir(dir, REPORT_ERRORS, context));
}
/* }}} */
...
JONJ_FUNCTION(rename)
{
	...
	wrapper = jonj_stream_locate_url_wrapper(old_name, NULL, 0 TSRMLS_CC);

	if (!wrapper || !wrapper->wops) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to locate stream wrapper");
		RETURN_FALSE;
	}

	if (!wrapper->wops->rename) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s wrapper does not support renaming", wrapper->wops->label ? wrapper->wops->label : "Source");
		RETURN_FALSE;
	}

	if (wrapper != jonj_stream_locate_url_wrapper(new_name, NULL, 0 TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot rename a file across wrapper types");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(unlink)
{
	...
	wrapper = jonj_stream_locate_url_wrapper(filename, NULL, 0 TSRMLS_CC);
	...
	if (!wrapper->wops->unlink) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s does not allow unlinking", wrapper->wops->label ? wrapper->wops->label : "Wrapper");
		RETURN_FALSE;
	}
...
JONJ_NAMED_FUNCTION(jonj_if_ftruncate)
{
	...
	if (size < 0) {
		/* jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Negative size is not supported"); */
		RETURN_FALSE;
	}

	JONJ_STREAM_TO_ZVAL(stream, &fp);

	if (!jonj_stream_truncate_supported(stream)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't truncate this stream!");
		RETURN_FALSE;
	}

	RETURN_BOOL(0 == jonj_stream_truncate_set_size(stream, size));
}
/* }}} */
...
JONJ_NAMED_FUNCTION(jonj_if_fstat)
{
	...
	if (jonj_stream_stat(stream, &stat_ssb)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(copy)
{
	...
	if (jonj_check_open_basedir(source TSRMLS_CC)) {
		RETURN_FALSE;
	}

	context = jonj_stream_context_from_zval(zcontext, 0);

	if (jonj_copy_file_ctx(source, target, 0, context TSRMLS_CC) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
{
	return jonj_copy_file_ctx(src, dest, 0, NULL TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_copy_file_ex
 */
JONJAPI int jonj_copy_file_ex(const char *src, const char *dest, int src_flg TSRMLS_DC)
{
	return jonj_copy_file_ctx(src, dest, 0, NULL TSRMLS_CC);
}
/* }}} */
...
/* {{{ jonj_copy_file_ctx
 */
JONJAPI int jonj_copy_file_ctx(const char *src, const char *dest, int src_flg, jonj_stream_context *ctx TSRMLS_DC)
{
	jonj_stream *srcstream = NULL, *deststream = NULL;
	int ret = FAILURE;
	jonj_stream_statbuf src_s, dest_s;

	switch (jonj_stream_stat_path_ex(src, 0, &src_s, ctx)) {
		case -1:
			/* non-statable stream */
			goto safe_to_copy;
			break;
	...
	if (S_ISDIR(src_s.sb.st_mode)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The first argument to copy() function cannot be a directory");
		return FAILURE;
	}

	switch (jonj_stream_stat_path_ex(dest, JONJ_STREAM_URL_STAT_QUIET | JONJ_STREAM_URL_STAT_NOCACHE, &dest_s, ctx)) {
		case -1:
			/* non-statable stream */
			goto safe_to_copy;
			break;
	...
	if (S_ISDIR(dest_s.sb.st_mode)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The second argument to copy() function cannot be a directory");
		return FAILURE;
	}
...
safe_to_copy:
	...
	srcstream = jonj_stream_open_wrapper_ex(src, "rb", src_flg | REPORT_ERRORS, NULL, ctx);

	if (!srcstream) {
		return ret;
	}

	deststream = jonj_stream_open_wrapper_ex(dest, "wb", REPORT_ERRORS, NULL, ctx);

	if (srcstream && deststream) {
		ret = jonj_stream_copy_to_stream_ex(srcstream, deststream, JONJ_STREAM_COPY_ALL, NULL);
	}
	if (srcstream) {
		jonj_stream_close(srcstream);
	}
	if (deststream) {
		jonj_stream_close(deststream);
	}
	return ret;
}
/* }}} */
...
JONJAPI JONJ_FUNCTION(fread)
{
	...
	if (len > INT_MAX) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter must be no more than %d", INT_MAX);
		RETURN_FALSE;
	}

	Z_STRVAL_P(return_value) = emalloc(len + 1);
	Z_STRLEN_P(return_value) = jonj_stream_read(stream, Z_STRVAL_P(return_value), len);
...
static const char *jonj_fgetcsv_lookup_trailing_spaces(const char *ptr, size_t len, const char delimiter TSRMLS_DC) /* {{{ */
{
	int inc_len;
	unsigned char last_chars[2] = { 0, 0 };

	while (len > 0) {
		inc_len = (*ptr == '\0' ? 1: jonj_mblen(ptr, len));
		switch (inc_len) {
			case -2:
			case -1:
				inc_len = 1;
...
JONJ_FUNCTION(fputcsv)
{
	...
	if (delimiter_str != NULL) {
		...
		if (delimiter_str_len < 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "delimiter must be a character");
			RETURN_FALSE;
		} else if (delimiter_str_len > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "delimiter must be a single character");
		}
	...
	if (enclosure_str != NULL) {
		if (enclosure_str_len < 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "enclosure must be a character");
			RETURN_FALSE;
		} else if (enclosure_str_len > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "enclosure must be a single character");
		}
		/* use first character from string */
	...
	if (escape_str != NULL) {
		if (escape_str_len < 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "escape must be a character");
			RETURN_FALSE;
		} else if (escape_str_len > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "escape must be a single character");
		}
		/* use first character from string */
	...
	ret = jonj_fputcsv(stream, fields, delimiter, enclosure, escape_char TSRMLS_CC);
	RETURN_LONG(ret);
}
/* }}} */
...
JONJAPI int jonj_fputcsv(jonj_stream *stream, zval *fields, char delimiter, char enclosure, char escape_char TSRMLS_DC)
{
	...
	ret = jonj_stream_write(stream, csvline.c, csvline.len);
...
JONJ_FUNCTION(fgetcsv)
{
	...
	/* first section exactly as jonj_fgetss */
...

	{
		...
		if (escape_str != NULL) {
			if (escape_str_len < 1) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "escape must be character");
				RETURN_FALSE;
			} else if (escape_str_len > 1) {
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "escape must be a single character");
			}
		...
		if (len_zv != NULL && Z_TYPE_PP(len_zv) != IS_NULL) {
			...
			if (len < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter may not be negative");
				RETURN_FALSE;
			} else if (len == 0) {
				len = -1;
			}
	...
	if (len < 0) {
		if ((buf = jonj_stream_get_line(stream, NULL, 0, &buf_len)) == NULL) {
			RETURN_FALSE;
		}
	} else {
		buf = emalloc(len + 1);
		if (jonj_stream_get_line(stream, buf, len + 1, &buf_len) == NULL) {
			efree(buf);
			RETURN_FALSE;
		}
	}

	jonj_fgetcsv(stream, delimiter, enclosure, escape, buf_len, buf, return_value TSRMLS_CC);
}
/* }}} */
...
JONJAPI void jonj_fgetcsv(jonj_stream *stream, char delimiter, char enclosure, char escape_char, size_t buf_len, char *buf, zval *return_value TSRMLS_DC) /* {{{ */
{
	...
	bptr = buf;
	tptr = (char *)jonj_fgetcsv_lookup_trailing_spaces(buf, buf_len, delimiter TSRMLS_CC);
	...
	do {
		...
		inc_len = (bptr < limit ? (*bptr == '\0' ? 1: jonj_mblen(bptr, limit - bptr)): 0);
		if (inc_len == 1) {
			char *tmp = bptr;
					...
					case 0:
						switch (state) {
							...
							case 0: {
								...
								if (stream == NULL) {
									goto quit_loop_2;
								} else if ((new_buf = jonj_stream_get_line(stream, NULL, 0, &new_len)) == NULL) {
								...
								line_end = limit = (char *)jonj_fgetcsv_lookup_trailing_spaces(buf, buf_len, delimiter TSRMLS_CC);
		...
		quit_loop_4:
			...
			comp_end = (char *)jonj_fgetcsv_lookup_trailing_spaces(temp, tptr - temp, delimiter TSRMLS_CC);
			if (*bptr == delimiter) {
				bptr++;
			}
		}

		/* 3. Now pass our field back to jonj */
		*comp_end = '\0';
...
JONJ_FUNCTION(realpath)
{
	...
	if (VCWD_REALPATH(filename, resolved_path_buff)) {
		if (jonj_check_open_basedir(resolved_path_buff TSRMLS_CC)) {
			RETURN_FALSE;
		}
...
#define JONJ_META_HTML401_CHARS "-_.:"

/* {{{ jonj_next_meta_token
   Tokenizes an HTML file for get_meta_tags */
jonj_meta_tags_token jonj_next_meta_token(jonj_meta_tags_data *md TSRMLS_DC)
{
	int ch = 0, compliment;
	...
	while (md->ulc || (!jonj_stream_eof(md->stream) && (ch = jonj_stream_getc(md->stream)))) {
		if (jonj_stream_eof(md->stream)) {
			break;
		}
		...
		switch (ch) {
			...
			case '"':
				compliment = ch;
				md->token_len = 0;
				while (!jonj_stream_eof(md->stream) && (ch = jonj_stream_getc(md->stream)) && ch != compliment && ch != '<' && ch != '>') {
			...
			default:
				if (isalnum(ch)) {
					md->token_len = 0;
					buff[(md->token_len)++] = ch;
					while (!jonj_stream_eof(md->stream) && (ch = jonj_stream_getc(md->stream)) && (isalnum(ch) || strchr(JONJ_META_HTML401_CHARS, ch))) {
						buff[(md->token_len)++] = ch;
...
JONJ_FUNCTION(fnmatch)
{
	...
	if (filename_len >= MAXPATHLEN) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filename exceeds the maximum allowed length of %d characters", MAXPATHLEN);
		RETURN_FALSE;
	}
	if (pattern_len >= MAXPATHLEN) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Pattern exceeds the maximum allowed length of %d characters", MAXPATHLEN);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(sys_get_temp_dir)
{
	...
	RETURN_STRING((char *)jonj_get_temporary_directory(TSRMLS_C), 1);
}
/* }}} */
