...
typedef unsigned long long jonj_timeout_ull;
#else
#include "win32/select.h"
#include "win32/sockets.h"
typedef unsigned __int64 jonj_timeout_ull;
#endif

#define GET_CTX_OPT(stream, wrapper, name, val) (stream->context && SUCCESS == jonj_stream_context_get_option(stream->context, wrapper, name, &val))

static jonj_stream_context *decode_context_param(zval *contextresource TSRMLS_DC);
...
JONJ_FUNCTION(stream_socket_pair)
{
	...
	if (0 != socketpair(domain, type, protocol, pair)) {
		char errbuf[256];
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to create sockets: [%d]: %s",
			jonj_socket_errno(), jonj_socket_strerror(jonj_socket_errno(), errbuf, sizeof(errbuf)));
		RETURN_FALSE;
	}

	array_init(return_value);

	s1 = jonj_stream_sock_open_from_socket(pair[0], 0);
	s2 = jonj_stream_sock_open_from_socket(pair[1], 0);

	/* set the __exposed flag.
	 * jonj_stream_to_zval() does, add_next_index_resource() does not */
	jonj_stream_auto_cleanup(s1);
	jonj_stream_auto_cleanup(s2);

	add_next_index_resource(return_value, jonj_stream_get_resource_id(s1));
	add_next_index_resource(return_value, jonj_stream_get_resource_id(s2));
}
/* }}} */
#endif
...
JONJ_FUNCTION(stream_socket_client)
{
	...
	jonj_timeout_ull conv;
	struct timeval tv;
	char *hashkey = NULL;
	jonj_stream *stream = NULL;
	int err;
	long flags = JONJ_STREAM_CLIENT_CONNECT;
	char *errstr = NULL;
	...
	if (flags & JONJ_STREAM_CLIENT_PERSISTENT) {
		spprintf(&hashkey, 0, "stream_socket_client__%s", host);
	}

	/* prepare the timeout value for use */
	conv = (jonj_timeout_ull) (timeout * 1000000.0);
#ifdef JONJ_WIN32
	...
	stream = jonj_stream_xport_create(host, host_len, REPORT_ERRORS,
			STREAM_XPORT_CLIENT | (flags & JONJ_STREAM_CLIENT_CONNECT ? STREAM_XPORT_CONNECT : 0) |
			(flags & JONJ_STREAM_CLIENT_ASYNC_CONNECT ? STREAM_XPORT_CONNECT_ASYNC : 0),
			hashkey, &tv, context, &errstr, &err);


	if (stream == NULL) {
		/* host might contain binary characters */
		char *quoted_host = jonj_addslashes(host, host_len, NULL, 0 TSRMLS_CC);

		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to connect to %s (%s)", quoted_host, errstr == NULL ? "Unknown error" : errstr);
		efree(quoted_host);
	}
...
JONJ_FUNCTION(stream_socket_server)
{
	...
	if (stream == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to connect to %s (%s)", host, errstr == NULL ? "Unknown error" : errstr);
	}
...
JONJ_FUNCTION(stream_socket_accept)
{
	...
	jonj_timeout_ull conv;
	struct timeval tv;
	jonj_stream *stream = NULL, *clistream = NULL;
	zval *zstream;
	...
	if (0 == jonj_stream_xport_accept(stream, &clistream,
		...
		jonj_stream_to_zval(clistream, return_value);
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "accept failed: %s", errstr ? errstr : "Unknown error");
		RETVAL_FALSE;
	}
...
JONJ_FUNCTION(stream_socket_get_name)
{
	...
	if (0 != jonj_stream_xport_get_name(stream, want_peer,
				&name,
				&name_len,
				NULL, NULL
				TSRMLS_CC)) {
...
JONJ_FUNCTION(stream_socket_sendto)
{
	...
	if (target_addr_len) {
		/* parse the address */
		if (FAILURE == jonj.tkwork_parse_network_address_with_port(target_addr, target_addr_len, (struct sockaddr*)&sa, &sl TSRMLS_CC)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to parse `%s' into a valid network address", target_addr);
			RETURN_FALSE;
		}
	}

	RETURN_LONG(jonj_stream_xport_sendto(stream, data, datalen, flags, target_addr ? &sa : NULL, sl TSRMLS_CC));
}
/* }}} */
...
JONJ_FUNCTION(stream_socket_recvfrom)
{
	...
	recvd = jonj_stream_xport_recvfrom(stream, read_buf, to_read, flags, NULL, NULL,
...
JONJ_FUNCTION(stream_get_contents)
{
	...
	jonj_stream_from_zval(stream, &zsrc);

	if (desiredpos >= 0) {
		int		seek_res = 0;
		off_t	position;

		position = jonj_stream_tell(stream);
		if (position >= 0 && desiredpos > position) {
			/* use SEEK_CUR to allow emulation in streams that don't support seeking */
			seek_res = jonj_stream_seek(stream, desiredpos - position, SEEK_CUR);
		} else if (desiredpos < position)  {
			/* desired position before position or error on tell */
			seek_res = jonj_stream_seek(stream, desiredpos, SEEK_SET);
		}
	...
	len = jonj_stream_copy_to_mem(stream, &contents, maxlen, 0);
...
{
	jonj_stream *src, *dest;
	zval *zsrc, *zdest;
	long maxlen = JONJ_STREAM_COPY_ALL, pos = 0;
	size_t len;
	int ret;
	...
	jonj_stream_from_zval(src, &zsrc);
	jonj_stream_from_zval(dest, &zdest);

	if (pos > 0 && jonj_stream_seek(src, pos, SEEK_SET) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to seek to position %ld in the stream", pos);
		RETURN_FALSE;
	}

	ret = jonj_stream_copy_to_stream_ex(src, dest, maxlen, &len);
...
JONJ_FUNCTION(stream_get_meta_data)
{
	...
	jonj_stream_from_zval(stream, &arg1);

	array_init(return_value);

	if (!jonj_stream_populate_meta_data(stream, return_value)) {
		add_assoc_bool(return_value, "timed_out", 0);
		add_assoc_bool(return_value, "blocked", 1);
		add_assoc_bool(return_value, "eof", jonj_stream_eof(stream));
	}
	...
	add_assoc_bool(return_value, "seekable", (stream->ops->seek) && (stream->flags & JONJ_STREAM_FLAG_NO_SEEK) == 0);
	if (stream->orig_path) {
...
JONJ_FUNCTION(stream_get_transports)
{
	...
	if ((stream_xport_hash = jonj_stream_xport_get_hash())) {
		HashPosition pos;
		array_init(return_value);
...
JONJ_FUNCTION(stream_get_wrappers)
{
	...
	if ((url_stream_wrappers_hash = jonj_stream_get_url_stream_wrappers_hash())) {
		HashPosition pos;
		array_init(return_value);
...
static int stream_array_to_fd_set(zval *stream_array, fd_set *fds, jonj_socket_t *max_fd TSRMLS_DC)
{
	zval **elem;
	jonj_stream *stream;
	int cnt = 0;
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(stream_array));
		...
		jonj_socket_t this_fd;

		jonj_stream_from_zval_no_verify(stream, elem);
		if (stream == NULL) {
			continue;
		}
		/* get the fd.
		 * NB: Most other code will NOT use the JONJ_STREAM_CAST_INTERNAL flag
		 * when casting.  It is only used here so that the buffered data warning
		 * is not displayed.
		 * */
		if (SUCCESS == jonj_stream_cast(stream, JONJ_STREAM_AS_FD_FOR_SELECT | JONJ_STREAM_CAST_INTERNAL, (void*)&this_fd, 1) && this_fd != -1) {

			JONJ_SAFE_FD_SET(this_fd, fds);
...
static int stream_array_from_fd_set(zval *stream_array, fd_set *fds TSRMLS_DC)
{
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(stream_array));
		 ...
		 * is not displayed.
		 */
		if (SUCCESS == jonj_stream_cast(stream, JONJ_STREAM_AS_FD_FOR_SELECT | JONJ_STREAM_CAST_INTERNAL, (void*)&this_fd, 1) && this_fd != SOCK_ERR) {
			if (JONJ_SAFE_FD_ISSET(this_fd, fds)) {
...
JONJ_FUNCTION(stream_select)
{
	...
	if (!sets) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No stream arrays were passed");
		RETURN_FALSE;
	}

	JONJ_SAFE_MAX_FD(max_fd, max_set_count);
	...
	if (sec != NULL) {
		...
		if (Z_LVAL_PP(sec) < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The seconds parameter must be greater than 0");
			RETURN_FALSE;
		} else if (usec < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The microseconds parameter must be greater than 0");
			RETURN_FALSE;
		}
	...
	retval = jonj_select(max_fd+1, &rfds, &wfds, &efds, tv_p);

	if (retval == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to select [%d]: %s (max_fd=%d)",
...
static void user_space_stream_notifier(jonj_stream_context *context, int notifycode, int severity,
		char *xmsg, int xcode, size_t bytes_sofar, size_t bytes_max, void * ptr TSRMLS_DC)
{
	...
	if (FAILURE == call_user_function_ex(EG(function_table), NULL, callback, &retval, 6, ptps, 0, NULL TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to call user notifier");
	}
	for (i = 0; i < 6; i++) {
		zval_ptr_dtor(&ps[i]);
	}
...
static void user_space_stream_notifier_dtor(jonj_stream_notifier *notifier)
{
...
static int parse_context_options(jonj_stream_context *context, zval *options TSRMLS_DC)
{
		...
		if (HASH_KEY_IS_STRING == zend_hash_get_current_key_ex(Z_ARRVAL_P(options), &wkey, &wkey_len, &num_key, 0, &pos)
			...
			while (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(wval), (void**)&oval, &opos)) {
				...
				if (HASH_KEY_IS_STRING == zend_hash_get_current_key_ex(Z_ARRVAL_PP(wval), &okey, &okey_len, &num_key, 0, &opos)) {
					jonj_stream_context_set_option(context, wkey, okey, *oval);
				}
				zend_hash_move_forward_ex(Z_ARRVAL_PP(wval), &opos);
			}

		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "options should have the form [\"wrappername\"][\"optionname\"] = $value");
		}
...
static int parse_context_params(jonj_stream_context *context, zval *params TSRMLS_DC)
{
	int ret = SUCCESS;
	zval **tmp;
	...
	if (SUCCESS == zend_hash_find(Z_ARRVAL_P(params), "notification", sizeof("notification"), (void**)&tmp)) {
		...
		context->notifier = jonj_stream_notification_alloc();
	...
	if (SUCCESS == zend_hash_find(Z_ARRVAL_P(params), "options", sizeof("options"), (void**)&tmp)) {
		if (Z_TYPE_PP(tmp) == IS_ARRAY) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid stream/context parameter");
		}
	}
...
static jonj_stream_context *decode_context_param(zval *contextresource TSRMLS_DC)
{
	...
	context = zend_fetch_resource(&contextresource TSRMLS_CC, -1, NULL, NULL, 1, jonj_le_stream_context(TSRMLS_C));
	if (context == NULL) {
		jonj_stream *stream;

		stream = zend_fetch_resource(&contextresource TSRMLS_CC, -1, NULL, NULL, 2, jonj_file_le_stream(), jonj_file_le_pstream);
			...
			if (context == NULL) {
				/* Only way this happens is if file is opened with NO_DEFAULT_CONTEXT
				   ...
				   Don't give them the default one though since they already said they
	 			   didn't want it. */
				context = stream->context = jonj_stream_context_alloc(TSRMLS_C);
			}
		}
	}
...
JONJ_FUNCTION(stream_context_set_option)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC,
		...
		if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC,
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "called with wrong number or type of parameters; please RTM");
			RETURN_FALSE;
		}
	}
	...
	if (options) {
		...
		jonj_stream_context_set_option(context, wrappername, optionname, zvalue);
		RETVAL_TRUE;
	}
}
/* }}} */
...
JONJ_FUNCTION(stream_context_get_default)
{
	...
	jonj_stream_context_to_zval(context, return_value);
}
/* }}} */
...
JONJ_FUNCTION(stream_context_create)
{
	...
	context = jonj_stream_context_alloc(TSRMLS_C);
...
static void apply_filter_to_stream(int append, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if ((read_write & JONJ_STREAM_FILTER_ALL) == 0) {
		/* Chain not specified.
		...
		if (strchr(stream->mode, 'r') || strchr(stream->mode, '+')) {
			read_write |= JONJ_STREAM_FILTER_READ;
		}
		if (strchr(stream->mode, 'w') || strchr(stream->mode, '+') || strchr(stream->mode, 'a')) {
			read_write |= JONJ_STREAM_FILTER_WRITE;
		}
	}

	if (read_write & JONJ_STREAM_FILTER_READ) {
		filter = jonj_stream_filter_create(filtername, filterparams, jonj_stream_is_persistent(stream) TSRMLS_CC);
		if (filter == NULL) {
			RETURN_FALSE;
		}

		if (append) {
			ret = jonj_stream_filter_append_ex(&stream->readfilters, filter TSRMLS_CC);
		} else {
			ret = jonj_stream_filter_prepend_ex(&stream->readfilters, filter TSRMLS_CC);
		}
		if (ret != SUCCESS) {
	...
	if (read_write & JONJ_STREAM_FILTER_WRITE) {
		filter = jonj_stream_filter_create(filtername, filterparams, jonj_stream_is_persistent(stream) TSRMLS_CC);
		if (filter == NULL) {
			RETURN_FALSE;
		}
		...
		if (append) {
			ret = jonj_stream_filter_append_ex(&stream->writefilters, filter TSRMLS_CC);
		} else {
			ret = jonj_stream_filter_prepend_ex(&stream->writefilters, filter TSRMLS_CC);
		}
		if (ret != SUCCESS) {
			jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
			RETURN_FALSE;
	...
	if (filter) {
		RETURN_RESOURCE(filter->rsrc_id = ZEND_REGISTER_RESOURCE(NULL, filter, jonj_file_le_stream_filter()));
	} else {
		RETURN_FALSE;
	}
}
...
JONJ_FUNCTION(stream_filter_remove)
{
	...
	filter = zend_fetch_resource(&zfilter TSRMLS_CC, -1, NULL, NULL, 1, jonj_file_le_stream_filter());
	if (!filter) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid resource given, not a stream filter");
		RETURN_FALSE;
	}

	if (jonj_stream_filter_flush(filter, 1) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to flush filter, not removing");
		RETURN_FALSE;
	}

	if (zend_list_delete(Z_LVAL_P(zfilter)) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not invalidate filter, not removing");
		RETURN_FALSE;
	} else {
		jonj_stream_filter_remove(filter, 1 TSRMLS_CC);
		RETURN_TRUE;
...
JONJ_FUNCTION(stream_get_line)
{
	...
	if (max_length < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The maximum allowed length must be greater than or equal to zero");
		RETURN_FALSE;
	}
	if (!max_length) {
		max_length = JONJ_SOCK_CHUNK_SIZE;
	}

	jonj_stream_from_zval(stream, &zstream);

	if ((buf = jonj_stream_get_record(stream, max_length, &buf_size, str, str_len TSRMLS_CC))) {
...
JONJ_FUNCTION(stream_set_blocking)
{
	...
	if (jonj_stream_set_option(stream, JONJ_STREAM_OPTION_BLOCKING, block == 0 ? 0 : 1, NULL) == -1) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(stream_set_timeout)
{
	...
	jonj_stream_from_zval(stream, &socket);
	...
	if (JONJ_STREAM_OPTION_RETURN_OK == jonj_stream_set_option(stream, JONJ_STREAM_OPTION_READ_TIMEOUT, 0, &t)) {
		RETURN_TRUE;
	}

	RETURN_FALSE;
}
#endif /* HAVE_SYS_TIME_H || defined(JONJ_WIN32) */
/* }}} */
...
JONJ_FUNCTION(stream_set_write_buffer)
{
	...
	if (buff == 0) {
		ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_WRITE_BUFFER, JONJ_STREAM_BUFFER_NONE, NULL);
	} else {
		ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_WRITE_BUFFER, JONJ_STREAM_BUFFER_FULL, &buff);
	}
...
JONJ_FUNCTION(stream_set_chunk_size)
{
	...
	if (csize <= 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The chunk size must be a positive integer, given %ld", csize);
		RETURN_FALSE;
	}
	/* stream.chunk_size is actually a size_t, but jonj_stream_set_option
	 ...
	 */
	if (csize > INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The chunk size cannot be larger than %d", INT_MAX);
		RETURN_FALSE;
	}

	jonj_stream_from_zval(stream, &zstream);

	ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_SET_CHUNK_SIZE, (int)csize, NULL);
...
JONJ_FUNCTION(stream_set_read_buffer)
{
	...
	/* if buff is 0 then set to non-buffered */
	if (buff == 0) {
		ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_READ_BUFFER, JONJ_STREAM_BUFFER_NONE, NULL);
	} else {
		ret = jonj_stream_set_option(stream, JONJ_STREAM_OPTION_READ_BUFFER, JONJ_STREAM_BUFFER_FULL, &buff);
	}
...
JONJ_FUNCTION(stream_socket_enable_crypto)
{
	...
	jonj_stream *stream, *sessstream = NULL;
	...
	if (enable) {
		if (ZEND_NUM_ARGS() < 3 || cryptokindnull) {
			...
			if (!GET_CTX_OPT(stream, "ssl", "crypto_method", val)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "When enabling encryption you must specify the crypto type");
				RETURN_FALSE;
			}
		...
		if (zsessstream) {
			jonj_stream_from_zval(sessstream, &zsessstream);
		}

		if (jonj_stream_xport_crypto_setup(stream, cryptokind, sessstream TSRMLS_CC) < 0) {
			RETURN_FALSE;
		}
	}

	ret = jonj_stream_xport_crypto_enable(stream, enable TSRMLS_CC);
	switch (ret) {
		case -1:
			RETURN_FALSE;
...
JONJ_FUNCTION(stream_is_local)
{
	...
	if (Z_TYPE_PP(zstream) == IS_RESOURCE) {
		...
		wrapper = jonj_stream_locate_url_wrapper(Z_STRVAL_PP(zstream), NULL, 0 TSRMLS_CC);
	}
...
JONJ_FUNCTION(stream_supports_lock)
{
	...
	if (!jonj_stream_supports_lock(stream)) {
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(stream_socket_shutdown)
{
	...
	RETURN_BOOL(jonj_stream_xport_shutdown(stream, (stream_shutdown_t)how TSRMLS_CC) == 0);
}
/* }}} */
#endif
