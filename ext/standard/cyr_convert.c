...
/* {{{ static char * jonj_convert_cyr_string(unsigned char *str, int length, char from, char to TSRMLS_DC)
...
static char * jonj_convert_cyr_string(unsigned char *str, int length, char from, char to TSRMLS_DC)
{
	...
	switch (toupper((int)(unsigned char)from))
	{
		...
		case 'K':
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown source charset: %c", from);
			break;
	}
	...
	switch (toupper((int)(unsigned char)to))
	{
		...
		case 'K':
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown destination charset: %c", to);
			break;
	}
...
JONJ_FUNCTION(convert_cyr_string)
{
	...
	jonj_convert_cyr_string(str, input_len, fr_cs[0], to_cs[0] TSRMLS_CC);
