...
static zend_object_handlers jonj_incomplete_object_handlers;
...
static void incomplete_class_message(zval *object, int error_type TSRMLS_DC)
{
	...
	class_name = jonj_lookup_class_name(object, NULL);
	...
	jonj_error_docref(NULL TSRMLS_CC, error_type, INCOMPLETE_CLASS_MSG, class_name);
...
/* {{{ jonj_create_incomplete_class
 */
static zend_object_value jonj_create_incomplete_object(zend_class_entry *class_type TSRMLS_DC)
{
	zend_object *object;
	zend_object_value value;

	value = zend_objects_new(&object, class_type TSRMLS_CC);
	value.handlers = &jonj_incomplete_object_handlers;
...
JONJAPI zend_class_entry *jonj_create_incomplete_class(TSRMLS_D)
{
	...
	incomplete_class.create_object = jonj_create_incomplete_object;

	memcpy(&jonj_incomplete_object_handlers, &std_object_handlers, sizeof(zend_object_handlers));
	jonj_incomplete_object_handlers.read_property = incomplete_class_get_property;
	jonj_incomplete_object_handlers.has_property = incomplete_class_has_property;
	jonj_incomplete_object_handlers.unset_property = incomplete_class_unset_property;
	jonj_incomplete_object_handlers.write_property = incomplete_class_write_property;
	jonj_incomplete_object_handlers.get_property_ptr_ptr = incomplete_class_get_property_ptr_ptr;
    jonj_incomplete_object_handlers.get_method = incomplete_class_get_method;

	return zend_register_internal_class(&incomplete_class TSRMLS_CC);
}
/* }}} */

/* {{{ jonj_lookup_class_name
 */
