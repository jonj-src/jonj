			  ...
			  zval *type, char *arg_sep, int enc_type TSRMLS_DC)
{
	...
	for (zend_hash_internal_pointer_reset(ht);
		...
		if (zend_hash_get_current_data_ex(ht, (void **)&zdata, NULL) == FAILURE || !zdata || !(*zdata)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error traversing form data array");
			return FAILURE;
		}
		if (Z_TYPE_PP(zdata) == IS_ARRAY || Z_TYPE_PP(zdata) == IS_OBJECT) {
			if (key_type == HASH_KEY_IS_STRING) {
				if (enc_type == JONJ_QUERY_RFC3986) {
					ekey = jonj_raw_url_encode(key, key_len, &ekey_len);
				} else {
					ekey = jonj_url_encode(key, key_len, &ekey_len);
				}
			...
			ht->nApplyCount++;
			jonj_url_encode_hash_ex(HASH_OF(*zdata), formstr, NULL, 0, newprefix, newprefix_len, "%5D", 3, (Z_TYPE_PP(zdata) == IS_OBJECT ? *zdata : NULL), arg_sep, enc_type TSRMLS_CC);
			ht->nApplyCount--;
			efree(newprefix);
			...
			switch (Z_TYPE_PP(zdata)) {
				case IS_STRING:
					if (enc_type == JONJ_QUERY_RFC3986) {
						ekey = jonj_raw_url_encode(Z_STRVAL_PP(zdata), Z_STRLEN_PP(zdata), &ekey_len);
					} else {
						ekey = jonj_url_encode(Z_STRVAL_PP(zdata), Z_STRLEN_PP(zdata), &ekey_len);						
					}
					break;
				case IS_LONG:
				case IS_BOOL:
				...
				default:
					...
					*copyzval = **zdata;
					...
					if (enc_type == JONJ_QUERY_RFC3986) {
						ekey = jonj_raw_url_encode(Z_STRVAL_P(copyzval), Z_STRLEN_P(copyzval), &ekey_len);
					} else {
						ekey = jonj_url_encode(Z_STRVAL_P(copyzval), Z_STRLEN_P(copyzval), &ekey_len);
					}
...
JONJ_FUNCTION(http_build_query)
{
	...
	smart_str formstr = {0};
	long enc_type = JONJ_QUERY_RFC1738;
	...
	if (Z_TYPE_P(formdata) != IS_ARRAY && Z_TYPE_P(formdata) != IS_OBJECT) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parameter 1 expected to be Array or Object.  Incorrect value given");
		RETURN_FALSE;
	}

	if (jonj_url_encode_hash_ex(HASH_OF(formdata), &formstr, prefix, prefix_len, NULL, 0, NULL, 0, (Z_TYPE_P(formdata) == IS_OBJECT ? formdata : NULL), arg_sep, enc_type TSRMLS_CC) == FAILURE) {
		if (formstr.c) {
			efree(formstr.c);
		}
		RETURN_FALSE;
