...
{
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The general Levenshtein support is not there yet");
	/* not there yet */
...
JONJ_FUNCTION(levenshtein)
{
	...
	if (distance < 0 && /* TODO */ ZEND_NUM_ARGS() != 3) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument string(s) too long");
	}
