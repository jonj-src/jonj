 ...
 *
 * Side effects:
 *     May set jonj_error based on abnormal conditions.
 *
 * Parameters :
 ...
 *----------------------------------------------------------------------
*/
JONJAPI int ValidateFormat(char *format, int numVars, int *totalSubs)
{
...
notXpg:
		...
		if (gotXpg) {
mixedXPG:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", "cannot mix \"%\" and \"%n$\" conversion specifiers");
			goto error;
		}
...
xpgCheckDone:
		...
		switch (*ch) {
			...
			case 'c':
				...
				/*
				if (flags & SCAN_WIDTH) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Field width may not be specified in %c conversion");
					goto error;
				}
				*/
				break;
			...
			case '[':
				...
				break;
badSet:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unmatched [ in format string");
				goto error;

			default: {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Bad scan conversion character \"%c\"", *ch);
				goto error;
			}
		}
	...
	for (i = 0; i < numVars; i++) {
		if (nassign[i] > 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", "Variable is assigned by multiple \"%n$\" conversion specifiers");
			goto error;
			 ...
			 */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Variable is not assigned by any conversion specifiers");
			goto error;
		}
	}
...
badIndex:
	if (gotXpg) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", "\"%n$\" argument index out of range");
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Different numbers of variable names and field specifiers");
	}
...
JONJAPI int jonj_sscanf_internal( char *string, char *format,
	...
	if (numVars) {
		for (i = varStart;i < argCount;i++){
			if ( ! PZVAL_IS_REF( *args[ i ] ) ) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parameter %d must be passed by reference", i);
				scan_set_error_return(numVars, return_value);
...
addToInt:
				 ...
				 * to a string since JONJ only supports signed values.
				 */
				if (!(flags & SCAN_SUPPRESS)) {
					*end = '\0';
