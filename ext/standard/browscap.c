...
static void convert_browscap_pattern(zval *pattern, int persistent) /* {{{ */
{
	...
	jonj_strtolower(Z_STRVAL_P(pattern), Z_STRLEN_P(pattern));
...
static void jonj_browscap_parser_cb(zval *arg1, zval *arg2, zval *arg3, int callback_type, void *arg TSRMLS_DC) /* {{{ */
{
...
static int browscap_read_file(char *filename, browser_data *browdata, int persistent TSRMLS_DC) /* {{{ */
{
	...
	zend_parse_ini_file(&fh, 1, ZEND_INI_SCANNER_RAW,
			(zend_ini_parser_cb_t) jonj_browscap_parser_cb, browdata TSRMLS_CC);
...
JONJ_INI_MH(OnChangeBrowscap)
{
	if (stage == JONJ_INI_STAGE_STARTUP) {
		...
		return SUCCESS;
	} else if (stage == JONJ_INI_STAGE_ACTIVATE) {
...
JONJ_MSHUTDOWN_FUNCTION(browscap) /* {{{ */
{
...
JONJ_FUNCTION(get_browser)
{
	...
	if (BROWSCAP_G(activation_bdata).filename[0] != '\0') {
		...
		if (!global_bdata.htab) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "browscap ini directive not set");
			RETURN_FALSE;
		}
		bdata = &global_bdata;
	}
	...
	if (agent_name == NULL) {
		...
		if (!PG(http_globals)[TRACK_VARS_SERVER] ||
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "HTTP_USER_AGENT variable is not set, cannot determine user agent name");
			RETURN_FALSE;
		}
	...
	jonj_strtolower(lookup_browser_name, agent_name_len);
