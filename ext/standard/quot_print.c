...
*/
static char jonj_hex2int(int c) /* {{{ */
{
	if (isdigit(c)) {
		return c - '0';
	}
...
JONJAPI unsigned char *jonj_quot_print_encode(const unsigned char *str, size_t length, size_t *ret_length) /* {{{ */
{
	...
	ret = safe_emalloc(3, length + (((3 * length)/(JONJ_QPRINT_MAXL-9)) + 1), 1);
	d = ret;
		...
		if (((c = *str++) == '\015') && (*str == '\012') && length > 0) {
			...
			*d++ = *str++;
			...
			if (iscntrl (c) || (c == 0x7f) || (c & 0x80) || (c == '=') || ((c == ' ') && (*str == '\015'))) {
				if ((((lp+= 3) > JONJ_QPRINT_MAXL) && (c <= 0x7f))
            || ((c > 0x7f) && (c <= 0xdf) && ((lp + 3) > JONJ_QPRINT_MAXL))
            || ((c > 0xdf) && (c <= 0xef) && ((lp + 6) > JONJ_QPRINT_MAXL))
            || ((c > 0xef) && (c <= 0xf4) && ((lp + 9) > JONJ_QPRINT_MAXL))) {
					*d++ = '=';
					*d++ = '\015';
					*d++ = '\012';
					lp = 3;
...
JONJ_FUNCTION(quoted_printable_decode)
{
	...
	while (str_in[i]) {
		...
		case '=':
			...
			{
				str_out[j++] = (jonj_hex2int((int) str_in[i + 1]) << 4)
						+ jonj_hex2int((int) str_in[i + 2]);
				i += 3;
...
JONJ_FUNCTION(quoted_printable_encode)
{
	...
	new_str = (char *)jonj_quot_print_encode((unsigned char *)str, (size_t)str_len, &new_str_len);
