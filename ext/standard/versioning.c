...
/* {{{ jonj_canonicalize_version() */

JONJAPI char *
jonj_canonicalize_version(const char *version)
{
...
/* }}} */
/* {{{ jonj_version_compare() */

JONJAPI int
jonj_version_compare(const char *orig_ver1, const char *orig_ver2)
{
	char *ver1;
	char *ver2;
	char *p1, *p2, *n1, *n2;
	...
	if (orig_ver1[0] == '#') {
		...
		ver1 = jonj_canonicalize_version(orig_ver1);
	}
	if (orig_ver2[0] == '#') {
		ver2 = estrdup(orig_ver2);
	} else {
		ver2 = jonj_canonicalize_version(orig_ver2);
	}
	p1 = n1 = ver1;
	p2 = n2 = ver2;
	while (*p1 && *p2 && n1 && n2) {
	...
	if (compare == 0) {
		if (n1 != NULL) {
			if (isdigit(*p1)) {
				...
				compare = jonj_version_compare(p1, "#N#");
			}
		} else if (n2 != NULL) {
			if (isdigit(*p2)) {
				compare = -1;
			} else {
				compare = jonj_version_compare("#N#", p2);
			}
		}
	}
	efree(ver1);
...
/* {{{ proto int version_compare(string ver1, string ver2 [, string oper])
  Compares two "JONJ-standardized" version number strings */
...
JONJ_FUNCTION(version_compare)
{
	...
	compare = jonj_version_compare(v1, v2);
	if (argc == 2) {
		RETURN_LONG(compare);
	}
