...
static inline void var_push(jonj_unserialize_data_t *var_hashx, zval **rval)
{
...
static inline zval **get_var_push_dtor_slot(jonj_unserialize_data_t *var_hashx)
{
...
JONJAPI void var_push_dtor(jonj_unserialize_data_t *var_hashx, zval **rval)
{
...
JONJAPI void var_replace(jonj_unserialize_data_t *var_hashx, zval *ozval, zval **nzval)
{
	long i;
...
static int var_access(jonj_unserialize_data_t *var_hashx, long id, zval ***store)
{
	var_entries *var_hash = (*var_hashx)->first;
#if VAR_ENTRIES_DBG
...
JONJAPI void var_destroy(jonj_unserialize_data_t *var_hashx)
{
	void *next;
	long i;
	var_entries *var_hash = (*var_hashx)->first;
...
#define UNSERIALIZE_PARAMETER zval **rval, const unsigned char **p, const unsigned char *max, jonj_unserialize_data_t *var_hash TSRMLS_DC
...
static inline int process_nested_data(UNSERIALIZE_PARAMETER, HashTable *ht, long elements, int objprops)
{
	while (elements-- > 0) {
		...
		if (!jonj_var_unserialize(&key, p, max, NULL TSRMLS_CC)) {
		...
		if (!jonj_var_unserialize(&data, p, max, var_hash TSRMLS_CC)) {
            var_push_dtor_no_addref(var_hash, &key);
...
static inline int object_common2(UNSERIALIZE_PARAMETER, long elements)
{
	...
	if (Z_OBJCE_PP(rval) != JONJ_IC_ENTRY &&
...
JONJAPI int jonj_var_unserialize(UNSERIALIZE_PARAMETER)
{
...
"d:" ("NAN" | "-"? "INF") ";"	{
	*p = YYCURSOR;
	...
	if (!strncmp(start + 2, "NAN", 3)) {
		ZVAL_DOUBLE(*rval, jonj_get_nan());
	} else if (!strncmp(start + 2, "INF", 3)) {
		ZVAL_DOUBLE(*rval, jonj_get_inf());
...
object ":" uiv ":" ["]	{
	...
	do {
		...
		if (call_user_function_ex(CG(function_table), NULL, user_func, &retval_ptr, 1, args, 0, NULL TSRMLS_CC) != SUCCESS) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "defined (%s) but not found", user_func->value.str.val);
			incomplete_class = 1;
			ce = JONJ_IC_ENTRY;
			zval_ptr_dtor(&user_func);
			zval_ptr_dtor(&arg_func_name);
		...
		if (zend_lookup_class(class_name, len2, &pce TSRMLS_CC) == SUCCESS) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Function %s() hasn't defined the class it was called for", user_func->value.str.val);
			incomplete_class = 1;
			ce = JONJ_IC_ENTRY;
		}
		BG(serialize_lock)--;
	...
	*p = YYCURSOR;
	...
	if (custom_object) {
		...
		if (ret && incomplete_class) {
			jonj_store_class_name(*rval, class_name, len2);
		}
		efree(class_name);
		return ret;
	}
...
"}" {
	...
	jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Unexpected end of serialized data");
