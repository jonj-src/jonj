...
#define JONJ_URL_REDIRECT_MAX		20
...
jonj_stream *jonj_stream_url_wrap_http_ex(jonj_stream_wrapper *wrapper, 
		const char *path, const char *mode, int options, char **opened_path, 
		jonj_stream_context *context, int redirect_max, int flags STREAMS_DC TSRMLS_DC) /* {{{ */
{
	...
	jonj_stream_filter *transfer_encoding = NULL;
	int response_code;

	tmp_line[0] = '\0';

	if (redirect_max < 1) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "Redirection limit reached, aborting");
		return NULL;
	}
	...
	if (strncasecmp(resource->scheme, "http", sizeof("http")) && strncasecmp(resource->scheme, "https", sizeof("https"))) {
		if (!context ||
			jonj_stream_context_get_option(context, wrapper->wops->label, "proxy", &tmpzval) == FAILURE ||
			Z_TYPE_PP(tmpzval) != IS_STRING ||
			Z_STRLEN_PP(tmpzval) <= 0) {
			jonj_url_free(resource);
			return jonj_stream_open_wrapper_ex(path, mode, REPORT_ERRORS, NULL, context);
		}
		...
		if (strpbrk(mode, "awx+")) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "HTTP wrapper does not support writeable connections");
			jonj_url_free(resource);
			return NULL;
		}
		...
		if (context &&
			jonj_stream_context_get_option(context, wrapper->wops->label, "proxy", &tmpzval) == SUCCESS &&
			Z_TYPE_PP(tmpzval) == IS_STRING &&
			Z_STRLEN_PP(tmpzval) > 0) {
			use_proxy = 1;
			transport_len = Z_STRLEN_PP(tmpzval);
	...
	if (context && jonj_stream_context_get_option(context, wrapper->wops->label, "timeout", &tmpzval) == SUCCESS) {
	...
	stream = jonj_stream_xport_create(transport_string, transport_len, options,
			STREAM_XPORT_CLIENT | STREAM_XPORT_CONNECT,
			NULL, &timeout, context, &errstr, NULL);

	if (stream) {
		jonj_stream_set_option(stream, JONJ_STREAM_OPTION_READ_TIMEOUT, 0, &timeout);
	}

	if (errstr) {
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "%s", errstr);
		efree(errstr);
		errstr = NULL;
	}
	...
	if (stream && use_proxy && use_ssl) {
		...
		if (!context || jonj_stream_context_get_option(context, "ssl", "peer_name", &tmpzval) == FAILURE) {
			MAKE_STD_ZVAL(ssl_proxy_peer_name);
			ZVAL_STRING(ssl_proxy_peer_name, resource->host, 1);
			jonj_stream_context_set_option(stream->context, "ssl", "peer_name", ssl_proxy_peer_name);
		}
		...
		if (context && jonj_stream_context_get_option(context, "http", "header", &tmpzval) == SUCCESS) {
			char *s, *p;
...
finish:
		...
		if (jonj_stream_write(stream, header.c, header.len) != header.len) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "Cannot connect to HTTPS server through proxy");
 	 	...
 	 	if (stream) {
			...
			/* get response header */
			while (jonj_stream_gets(stream, header_line, HTTP_HEADER_BLOCK_SIZE-1) != NULL) {
				if (header_line[0] == '\n' ||
				    header_line[0] == '\r' ||
				    header_line[0] == '\0') {
				  break;
		...
		if (stream) {
			if (jonj_stream_xport_crypto_setup(stream, STREAM_CRYPTO_METHOD_SSLv23_CLIENT, NULL TSRMLS_CC) < 0 ||
	...
	if (options & STREAM_WILL_CAST)
		chunk_size = jonj_stream_set_chunk_size(stream, 1);

	/* avoid problems with auto-detecting when reading the headers -> the headers
	 * are always in canonical \r\n format */
	eol_detect = stream->flags & (JONJ_STREAM_FLAG_DETECT_EOL | JONJ_STREAM_FLAG_EOL_MAC);
	stream->flags &= ~(JONJ_STREAM_FLAG_DETECT_EOL | JONJ_STREAM_FLAG_EOL_MAC);

	jonj_stream_context_set(stream, context);

	jonj_stream_notify_info(context, JONJ_STREAM_NOTIFY_CONNECT, NULL, 0);

	if (header_init && context && jonj_stream_context_get_option(context, "http", "max_redirects", &tmpzval) == SUCCESS) {
		SEPARATE_ZVAL(tmpzval);
		convert_to_long_ex(tmpzval);
		redirect_max = Z_LVAL_PP(tmpzval);
	}

	if (context && jonj_stream_context_get_option(context, "http", "method", &tmpzval) == SUCCESS) {
	...
	if (context && jonj_stream_context_get_option(context, "http", "protocol_version", &tmpzval) == SUCCESS) {
		SEPARATE_ZVAL(tmpzval);
		convert_to_double_ex(tmpzval);
	...
	if (!request_fulluri &&
		context &&
		jonj_stream_context_get_option(context, "http", "request_fulluri", &tmpzval) == SUCCESS) {
		zval ztmp = **tmpzval;
	...
	/* send it */
	jonj_stream_write(stream, scratch, strlen(scratch));
	...
	if (context && jonj_stream_context_get_option(context, "http", "header", &tmpzval) == SUCCESS) {
		...
		if (Z_TYPE_PP(tmpzval) == IS_ARRAY) {
			...
			if (tmpstr.c) {
				tmp = jonj_trim(tmpstr.c, strlen(tmpstr.c), NULL, 0, NULL, 3 TSRMLS_CC);
				smart_str_free(&tmpstr);
			}
		}
		if (Z_TYPE_PP(tmpzval) == IS_STRING && Z_STRLEN_PP(tmpzval)) {
			/* Remove newlines and spaces from start and end jonj_trim will estrndup() */
			tmp = jonj_trim(Z_STRVAL_PP(tmpzval), Z_STRLEN_PP(tmpzval), NULL, 0, NULL, 3 TSRMLS_CC);
		}
		if (tmp && strlen(tmp) > 0) {
			char *s;
			...
			jonj_strtolower(tmp, strlen(tmp));
	...
	if (((have_header & HTTP_HEADER_AUTH) == 0) && resource->user) {
		...
		jonj_url_decode(resource->user, strlen(resource->user));
		...
		if (resource->pass) {
			jonj_url_decode(resource->pass, strlen(resource->pass));
			strcat(scratch, resource->pass);
		}

		tmp = (char*)jonj_base64_encode((unsigned char*)scratch, strlen(scratch), NULL);

		if (snprintf(scratch, scratch_len, "Authorization: Basic %s\r\n", tmp) > 0) {
			jonj_stream_write(stream, scratch, strlen(scratch));
			jonj_stream_notify_info(context, JONJ_STREAM_NOTIFY_AUTH_REQUIRED, NULL, 0);
		}
	...
	if ((have_header & HTTP_HEADER_CONNECTION) == 0) {
		jonj_stream_write_string(stream, "Connection: close\r\n");
	}

	if (context &&
	    jonj_stream_context_get_option(context, "http", "user_agent", &ua_zval) == SUCCESS &&
...
#define _UA_HEADER "User-Agent: %s\r\n"
		...
		if (ua_len > sizeof(_UA_HEADER)) {
			...
			if ((ua_len = slprintf(ua, ua_len, _UA_HEADER, ua_str)) > 0) {
				ua[ua_len] = 0;
				jonj_stream_write(stream, ua, ua_len);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot construct User-agent header");
			}
	...
	if (user_headers) {
		...
		if (
				...
				context &&
				!(have_header & HTTP_HEADER_CONTENT_LENGTH) &&
				jonj_stream_context_get_option(context, "http", "content", &tmpzval) == SUCCESS &&
				Z_TYPE_PP(tmpzval) == IS_STRING && Z_STRLEN_PP(tmpzval) > 0
		) {
			scratch_len = slprintf(scratch, scratch_len, "Content-Length: %d\r\n", Z_STRLEN_PP(tmpzval));
			jonj_stream_write(stream, scratch, scratch_len);
			have_header |= HTTP_HEADER_CONTENT_LENGTH;
		}

		jonj_stream_write(stream, user_headers, strlen(user_headers));
		jonj_stream_write(stream, "\r\n", sizeof("\r\n")-1);
		efree(user_headers);
	}
	...
	if (header_init && context &&
		...
		if (!(have_header & HTTP_HEADER_TYPE)) {
			jonj_stream_write(stream, "Content-Type: application/x-www-form-urlencoded\r\n",
				sizeof("Content-Type: application/x-www-form-urlencoded\r\n") - 1);
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Content-type not specified assuming application/x-www-form-urlencoded");
		}
		jonj_stream_write(stream, "\r\n", sizeof("\r\n")-1);
		jonj_stream_write(stream, Z_STRVAL_PP(tmpzval), Z_STRLEN_PP(tmpzval));
	} else {
		jonj_stream_write(stream, "\r\n", sizeof("\r\n")-1);
	}
...

	{
		...
		if(zend_hash_find(EG(active_symbol_table), "http_response_header", sizeof("http_response_header"), (void **) &rh) != SUCCESS || Z_TYPE_PP(rh) != IS_ARRAY) {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "HTTP request failed, http_response_header overwritten");
			goto out;
		}
		response_header = *rh;
	...
	if (!jonj_stream_eof(stream)) {
		...
		if (jonj_stream_get_line(stream, tmp_line, sizeof(tmp_line) - 1, &tmp_line_len) != NULL) {
			zval *http_response;
			...
			if (context && SUCCESS==jonj_stream_context_get_option(context, "http", "ignore_errors", &tmpzval)) {
			...
			if (response_code >= 100 && response_code < 200) {
				...
				while (
					...
					&& jonj_stream_get_line(stream, tmp_line, sizeof(tmp_line) - 1, &tmp_line_len) != NULL
			...
			if (response_code >= 200 && response_code < 400) {
				...
				switch(response_code) {
					case 403:
						jonj_stream_notify_error(context, JONJ_STREAM_NOTIFY_AUTH_RESULT,
					...
					default:
						...
						jonj_stream_notify_error(context, JONJ_STREAM_NOTIFY_FAILURE,
								tmp_line, response_code);
				}
			}
		...
		jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "HTTP request failed, unexpected end of socket!");
		goto out;
	}
	...
	while (!body && !jonj_stream_eof(stream)) {
		size_t http_header_line_length;
		if (jonj_stream_get_line(stream, http_header_line, HTTP_HEADER_BLOCK_SIZE, &http_header_line_length) && *http_header_line != '\n' && *http_header_line != '\r') {
			char *e = http_header_line + http_header_line_length - 1;
			if (*e != '\n') {
				do { /* partial header */
					if (jonj_stream_get_line(stream, http_header_line, HTTP_HEADER_BLOCK_SIZE, &http_header_line_length) == NULL) {
						jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "Failed to read HTTP headers");
						goto out;
					}
			...
			if (!strncasecmp(http_header_line, "Location: ", 10)) {
				if (context && jonj_stream_context_get_option(context, "http", "follow_location", &tmpzval) == SUCCESS) {
					SEPARATE_ZVAL(tmpzval);
					convert_to_long_ex(tmpzval);
				...
				jonj_stream_notify_info(context, JONJ_STREAM_NOTIFY_MIME_TYPE_IS, http_header_line + 14, 0);
			} else if (!strncasecmp(http_header_line, "Content-Length: ", 16)) {
				file_size = atoi(http_header_line + 16);
				jonj_stream_notify_file_size(context, file_size, http_header_line, 0);
				...
				if (!(options & STREAM_ONLY_GET_HEADERS)) {
					...
					if (context && jonj_stream_context_get_option(context, "http", "auto_decode", &tmpzval) == SUCCESS) {
						SEPARATE_ZVAL(tmpzval);
						convert_to_boolean(*tmpzval);
						decode = Z_LVAL_PP(tmpzval);
					}
					if (decode) {
						transfer_encoding = jonj_stream_filter_create("dechunk", NULL, jonj_stream_is_persistent(stream) TSRMLS_CC);
	...
	if (!reqok || (location[0] != '\0' && follow_location)) {
		...
		if (location[0] != '\0')
			jonj_stream_notify_info(context, JONJ_STREAM_NOTIFY_REDIRECTED, location, 0);
		...
		if (location[0] != '\0') {
			...
			*new_path='\0';
			...
			if ((resource = jonj_url_parse(new_path)) == NULL) {
				jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "Invalid redirect URL! %s", new_path);
				goto out;
			}
...
#define CHECK_FOR_CNTRL_CHARS(val) { \
	if (val) { \
		unsigned char *s, *e; \
		int l; \
		l = jonj_url_decode(val, strlen(val)); \
		...
		while (s < e) { \
			...
			stream = jonj_stream_url_wrap_http_ex(wrapper, new_path, mode, options, opened_path, context, --redirect_max, HTTP_WRAPPER_REDIRECTED STREAMS_CC TSRMLS_CC);
		} else {
			jonj_stream_wrapper_log_error(wrapper, options TSRMLS_CC, "HTTP request failed! %s", tmp_line);
		}
	}
out:
	if (protocol_version) {
	...
	if (stream) {
		...
		if (options & STREAM_WILL_CAST)
			jonj_stream_set_chunk_size(stream, chunk_size);
		...
		if (transfer_encoding) {
			jonj_stream_filter_append(&stream->readfilters, transfer_encoding);
		}
	} else {
		if(response_header) {
			Z_DELREF_P(response_header);
		...
		if (transfer_encoding) {
			jonj_stream_filter_free(transfer_encoding TSRMLS_CC);
		}
	}
...
{
	return jonj_stream_url_wrap_http_ex(wrapper, path, mode, options, opened_path, context, JONJ_URL_REDIRECT_MAX, HTTP_WRAPPER_HEADER_INIT STREAMS_CC TSRMLS_CC);
}
/* }}} */

static int jonj_stream_http_stream_stat(jonj_stream_wrapper *wrapper, jonj_stream *stream, jonj_stream_statbuf *ssb TSRMLS_DC) /* {{{ */
{
...
static jonj_stream_wrapper_ops http_stream_wops = {
	jonj_stream_url_wrap_http,
	NULL, /* stream_close */
	jonj_stream_http_stream_stat,
	NULL, /* stat_url */
	NULL, /* opendir */
	"http",
	NULL, /* unlink */
...
JONJAPI jonj_stream_wrapper jonj_stream_http_wrapper = {
	&http_stream_wops,
	NULL,
	1 /* is_url */
};
