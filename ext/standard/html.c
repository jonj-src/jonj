 ...
 * Public interface for get_next_char used with UTF-8 */
 JONJAPI unsigned int jonj_next_utf8_char(
		const unsigned char *str,
		size_t str_len,
		size_t *cursor,
		int *status)
...
det_charset:
	...
	if (charset_hint) {
		...
		if (!found) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "charset `%s' not supported, assuming utf-8",
					charset_hint);
		}
	}
	return charset;
...
/* {{{ jonj_utf32_utf8 */
static inline size_t jonj_utf32_utf8(unsigned char *buf, unsigned k)
{
	size_t retval = 0;
...
/* {{{ jonj_mb2_int_to_char
 * Convert back big endian int representation of sequence of one or two 8-bit code units. */
static inline size_t jonj_mb2_int_to_char(unsigned char *buf, unsigned k)
{
	assert(k <= 0xFFFFU);
	/* one or two bytes */
	if (k <= 0xFFU) { /* 1 */
...
/* {{{ jonj_mb3_int_to_char
 * Convert back big endian int representation of sequence of one to three 8-bit code units.
 * For EUC-JP. */
static inline size_t jonj_mb3_int_to_char(unsigned char *buf, unsigned k)
{
	assert(k <= 0xFFFFFFU);
	/* one to three bytes */
	if (k <= 0xFFU) { /* 1 */
...
static inline size_t write_octet_sequence(unsigned char *buf, enum entity_charset charset, unsigned code) {
	...
	switch (charset) {
	case cs_utf_8:
		return jonj_utf32_utf8(buf, code);
...
#if 0
		return jonj_mb2_int_to_char(buf, code);
#else
#if ZEND_DEBUG
		assert(code <= 0xFFU);
#endif
 ...
 * Auxiliary function to jonj_unescape_html_entities().
...
{
	return jonj_escape_html_entities_ex(old, oldlen, newlen, all, flags, hint_charset, 1 TSRMLS_CC);
}
...
JONJAPI char *jonj_escape_html_entities_ex(unsigned char *old, size_t oldlen, size_t *newlen, int all, int flags, char *hint_charset, zend_bool double_encode TSRMLS_DC)
{
	...
	if (all) { /* replace with all named entities */
		if (CHARSET_PARTIAL_SUPPORT(charset)) {
			jonj_error_docref0(NULL TSRMLS_CC, E_STRICT, "Only basic entities "
...
/* {{{ jonj_html_entities
 */
static void jonj_html_entities(INTERNAL_FUNCTION_PARAMETERS, int all)
{
	...
	replaced = jonj_escape_html_entities_ex(str, str_len, &new_len, all, (int) flags, hint_charset, double_encode TSRMLS_CC);
...
{
	jonj_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
JONJ_FUNCTION(htmlspecialchars_decode)
{
	...
	replaced = jonj_unescape_html_entities(str, str_len, &new_len, 0 /*!all*/, quote_style, NULL TSRMLS_CC);
	if (replaced) {
...
JONJ_FUNCTION(html_entity_decode)
{
	...
	replaced = jonj_unescape_html_entities(str, str_len, &new_len, 1 /*all*/, quote_style, hint_charset TSRMLS_CC);
...
{
	jonj_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
