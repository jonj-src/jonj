...
static JONJ_INI_MH(OnChangeCallback) /* {{{ */
{
	if (EG(in_execution)) {
		if (ASSERTG(callback)) {
...
JONJ_INI_BEGIN()
	 STD_JONJ_INI_ENTRY("assert.active",		"1",	JONJ_INI_ALL,	OnUpdateLong,		active,	 			zend_assert_globals,		assert_globals)
	 STD_JONJ_INI_ENTRY("assert.bail",		"0",	JONJ_INI_ALL,	OnUpdateLong,		bail,	 			zend_assert_globals,		assert_globals)
	 STD_JONJ_INI_ENTRY("assert.warning",	"1",	JONJ_INI_ALL,	OnUpdateLong,		warning, 			zend_assert_globals,		assert_globals)
	 JONJ_INI_ENTRY("assert.callback",		NULL,	JONJ_INI_ALL,	OnChangeCallback)
	 STD_JONJ_INI_ENTRY("assert.quiet_eval", "0",	JONJ_INI_ALL,	OnUpdateLong,		quiet_eval,		 	zend_assert_globals,		assert_globals)
JONJ_INI_END()

static void jonj_assert_init_globals(zend_assert_globals *assert_globals_p TSRMLS_DC) /* {{{ */
{
...
JONJ_MINIT_FUNCTION(assert) /* {{{ */
{
	ZEND_INIT_MODULE_GLOBALS(assert, jonj_assert_init_globals, NULL);
...
JONJ_MSHUTDOWN_FUNCTION(assert) /* {{{ */
{
	if (ASSERTG(cb)) {
		pefree(ASSERTG(cb), 1);
		ASSERTG(cb) = NULL;
...
JONJ_RSHUTDOWN_FUNCTION(assert) /* {{{ */
{
	if (ASSERTG(callback)) {
		zval_ptr_dtor(&ASSERTG(callback));
		ASSERTG(callback) = NULL;
...
JONJ_FUNCTION(assert)
{
	...
	if (Z_TYPE_PP(assertion) == IS_STRING) {
		...
		if (zend_eval_stringl(myeval, Z_STRLEN_PP(assertion), &retval, compiled_string_description TSRMLS_CC) == FAILURE) {
			...
			if (description_len == 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Failure evaluating code: %s%s", JONJ_EOL, myeval);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Failure evaluating code: %s%s:\"%s\"", JONJ_EOL, description, myeval);
			}
			if (ASSERTG(bail)) {
				zend_bailout();
			}
	...
	if (ASSERTG(warning)) {
		if (description_len == 0) {
			if (myeval) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Assertion \"%s\" failed", myeval);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Assertion failed");
			}
		} else {
			if (myeval) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s: \"%s\" failed", description, myeval);
			} else {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s failed", description);
			}
		}
	}
...
JONJ_FUNCTION(assert_options)
{
	...
	case ASSERT_ACTIVE:
		...
		if (ac == 2) {
			...
			zend_alter_ini_entry_ex("assert.active", sizeof("assert.active"), Z_STRVAL_PP(value), Z_STRLEN_PP(value), JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
		}
		RETURN_LONG(oldint);
		break;
	...
	case ASSERT_BAIL:
		...
		if (ac == 2) {
			convert_to_string_ex(value);
			zend_alter_ini_entry_ex("assert.bail", sizeof("assert.bail"), Z_STRVAL_PP(value), Z_STRLEN_PP(value), JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
		}
		RETURN_LONG(oldint);
		break;
	...
	case ASSERT_QUIET_EVAL:
		...
		if (ac == 2) {
			convert_to_string_ex(value);
			zend_alter_ini_entry_ex("assert.quiet_eval", sizeof("assert.quiet_eval"), Z_STRVAL_PP(value), Z_STRLEN_PP(value), JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
		}
		RETURN_LONG(oldint);
		break;
	...
	case ASSERT_WARNING:
		...
		if (ac == 2) {
			convert_to_string_ex(value);
			zend_alter_ini_entry_ex("assert.warning", sizeof("assert.warning"), Z_STRVAL_PP(value), Z_STRLEN_PP(value), JONJ_INI_USER, JONJ_INI_STAGE_RUNTIME, 0 TSRMLS_CC);
		}
		RETURN_LONG(oldint);
		break;
	...
	default:
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown value %ld", what);
		break;
	}
