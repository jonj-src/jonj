...
#ifdef ZTS
#define DIRG(v) TSRMG(dir_globals_id, jonj_dir_globals *, v)
int dir_globals_id;
#else
#define DIRG(v) (dir_globals.v)
jonj_dir_globals dir_globals;
#endif
	...
	if (ZEND_NUM_ARGS() == 0) { \
		...
		if (myself) { \
			if (zend_hash_find(Z_OBJPROP_P(myself), "handle", sizeof("handle"), (void **)&tmp) == FAILURE) { \
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find my handle property"); \
				RETURN_FALSE; \
			} \
			ZEND_FETCH_RESOURCE(dirp, jonj_stream *, tmp, -1, "Directory", jonj_file_le_stream()); \
		} else { \
			ZEND_FETCH_RESOURCE(dirp, jonj_stream *, 0, DIRG(default_dir), "Directory", jonj_file_le_stream()); \
		} \
	} else { \
		dirp = (jonj_stream *) zend_fetch_resource(&id TSRMLS_CC, -1, "Directory", NULL, 1, jonj_file_le_stream()); \
		if (!dirp) \
			RETURN_FALSE; \
	}
...
static const zend_function_entry jonj_dir_class_functions[] = {
	JONJ_FALIAS(close,	closedir,		arginfo_dir)
	JONJ_FALIAS(rewind,	rewinddir,		arginfo_dir)
	JONJ_NAMED_FE(read,  jonj_if_readdir, arginfo_dir)
	{NULL, NULL, NULL}
};


static void jonj_set_default_dir(int id TSRMLS_DC)
{
...
JONJ_MINIT_FUNCTION(dir)
{
	...
	INIT_CLASS_ENTRY(dir_class_entry, "Directory", jonj_dir_class_functions);
	dir_class_entry_ptr = zend_register_internal_class(&dir_class_entry TSRMLS_CC);

#ifdef ZTS
	ts_allocate_id(&dir_globals_id, sizeof(jonj_dir_globals), NULL, NULL);
#endif
	...
	REGISTER_LONG_CONSTANT("SCANDIR_SORT_ASCENDING",  JONJ_SCANDIR_SORT_ASCENDING,  CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SCANDIR_SORT_DESCENDING", JONJ_SCANDIR_SORT_DESCENDING, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SCANDIR_SORT_NONE",       JONJ_SCANDIR_SORT_NONE,       CONST_CS | CONST_PERSISTENT);
...
/* {{{ internal functions */
static void _jonj_do_opendir(INTERNAL_FUNCTION_PARAMETERS, int createobject)
{
	char *dirname;
	int dir_len;
	zval *zcontext = NULL;
	...
	dirp = jonj_stream_opendir(dirname, REPORT_ERRORS, context);

	if (dirp == NULL) {
		RETURN_FALSE;
	}

	dirp->flags |= JONJ_STREAM_FLAG_NO_FCLOSE;

	jonj_set_default_dir(dirp->rsrc_id TSRMLS_CC);

	if (createobject) {
		object_init_ex(return_value, dir_class_entry_ptr);
		add_property_stringl(return_value, "path", dirname, dir_len, 1);
		add_property_resource(return_value, "handle", dirp->rsrc_id);
		jonj_stream_auto_cleanup(dirp); /* so we don't get warnings under debug */
	} else {
		jonj_stream_to_zval(dirp, return_value);
	}
}
/* }}} */
...
{
	_jonj_do_opendir(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	_jonj_do_opendir(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
JONJ_FUNCTION(closedir)
{
	...
	if (!(dirp->flags & JONJ_STREAM_FLAG_IS_DIR)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%d is not a valid Directory resource", dirp->rsrc_id);
		RETURN_FALSE;
	}
	...
	if (rsrc_id == DIRG(default_dir)) {
		jonj_set_default_dir(-1 TSRMLS_CC);
	}
}
/* }}} */
...
JONJ_FUNCTION(chroot)
{
	...
	ret = chroot(str);
	if (ret != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%s (errno %d)", strerror(errno), errno);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(chdir)
{
	...
	if (jonj_check_open_basedir(str TSRMLS_CC)) {
		RETURN_FALSE;
	}
	ret = VCWD_CHDIR(str);
...
JONJ_NAMED_FUNCTION(jonj_if_readdir)
{
	...
	if (jonj_stream_readdir(dirp, &entry)) {
...
no_results:
		...
		if (PG(open_basedir) && *PG(open_basedir)) {
			if (jonj_check_open_basedir_ex(pattern, 0 TSRMLS_CC)) {
				RETURN_FALSE;
			}
		}
#endif
	...
	for (n = 0; n < globbuf.gl_pathc; n++) {
		if (PG(open_basedir) && *PG(open_basedir)) {
			if (jonj_check_open_basedir_ex(globbuf.gl_pathv[n], 0 TSRMLS_CC)) {
				basedir_limit = 1;
				continue;
			}
		}
...
JONJ_FUNCTION(scandir)
{
	...
	if (dirn_len < 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Directory name cannot be empty");
		RETURN_FALSE;
	}
	...
	if (flags == JONJ_SCANDIR_SORT_ASCENDING) {
		n = jonj_stream_scandir(dirn, &namelist, context, (void *) jonj_stream_dirent_alphasort);
	} else if (flags == JONJ_SCANDIR_SORT_NONE) {
		n = jonj_stream_scandir(dirn, &namelist, context, NULL);
	} else {
		n = jonj_stream_scandir(dirn, &namelist, context, (void *) jonj_stream_dirent_alphasortr);
	}
	if (n < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "(errno %d): %s", errno, strerror(errno));
		RETURN_FALSE;
	}
