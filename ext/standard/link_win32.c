...
/*
TODO:
- Create jonj_readlink (done), jonj_link and jonj_symlink in win32/link.c
- Expose them (JONJAPI) so extensions developers can use them
- define link/readlink/symlink to their jonj_ equivalent and use them in ext/standart/link.c
...
JONJ_FUNCTION(readlink)
{
	...
	if (jonj_sys_readlink(link, target, MAXPATHLEN) == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "readlink failed to read the symbolic link (%s), error %d)", link, GetLastError());
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(symlink)
{
	...
	if (kernel32) {
		...
		if (pCreateSymbolicLinkA == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't call CreateSymbolicLinkA");
			RETURN_FALSE;
		}
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't call get a handle on kernel32.dll");
		RETURN_FALSE;
	}
	...
	len = jonj_dirname(dirname, strlen(dirname));
	...
	if (jonj_stream_locate_url_wrapper(source_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC) ||
		jonj_stream_locate_url_wrapper(dest_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC) ) 
	{
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to symlink to a URL");
		RETURN_FALSE;
	}
	...
	if ((attr = GetFileAttributes(topath)) == INVALID_FILE_ATTRIBUTES) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not fetch file information(error %d)", GetLastError());
			RETURN_FALSE;
	}
	...
	if (!ret) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create symlink, error code(%d)", GetLastError());
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(link)
{
	...
	if (jonj_stream_locate_url_wrapper(source_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC) ||
		jonj_stream_locate_url_wrapper(dest_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY TSRMLS_CC) ) 
	{
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to link to a URL");
		RETURN_FALSE;
	}
