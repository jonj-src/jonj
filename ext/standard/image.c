...
/* file type markers */
JONJAPI const char jonj_sig_gif[3] = {'G', 'I', 'F'};
JONJAPI const char jonj_sig_psd[4] = {'8', 'B', 'P', 'S'};
JONJAPI const char jonj_sig_bmp[2] = {'B', 'M'};
JONJAPI const char jonj_sig_swf[3] = {'F', 'W', 'S'};
JONJAPI const char jonj_sig_swc[3] = {'C', 'W', 'S'};
JONJAPI const char jonj_sig_jpg[3] = {(char) 0xff, (char) 0xd8, (char) 0xff};
JONJAPI const char jonj_sig_png[8] = {(char) 0x89, (char) 0x50, (char) 0x4e, (char) 0x47,
                                    (char) 0x0d, (char) 0x0a, (char) 0x1a, (char) 0x0a};
JONJAPI const char jonj_sig_tif_ii[4] = {'I','I', (char)0x2A, (char)0x00};
JONJAPI const char jonj_sig_tif_mm[4] = {'M','M', (char)0x00, (char)0x2A};
JONJAPI const char jonj_sig_jpc[3]  = {(char)0xff, (char)0x4f, (char)0xff};
JONJAPI const char jonj_sig_jp2[12] = {(char)0x00, (char)0x00, (char)0x00, (char)0x0c,
                                     (char)0x6a, (char)0x50, (char)0x20, (char)0x20,
                                     (char)0x0d, (char)0x0a, (char)0x87, (char)0x0a};
JONJAPI const char jonj_sig_iff[4] = {'F','O','R','M'};
JONJAPI const char jonj_sig_ico[4] = {(char)0x00, (char)0x00, (char)0x01, (char)0x00};

/* REMEMBER TO ADD MIME-TYPE TO FUNCTION jonj_image_type_to_mime_type */
...
static struct gfxinfo *jonj_handle_gif (jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	unsigned char dim[5];

	if (jonj_stream_seek(stream, 3, SEEK_CUR))
		return NULL;

	if (jonj_stream_read(stream, dim, sizeof(dim)) != sizeof(dim))
		return NULL;
...
/* {{{ jonj_handle_psd
 */
static struct gfxinfo *jonj_handle_psd (jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	unsigned char dim[8];

	if (jonj_stream_seek(stream, 11, SEEK_CUR))
		return NULL;
...
/* {{{ jonj_handle_bmp
 */
static struct gfxinfo *jonj_handle_bmp (jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
...
static unsigned long int jonj_swf_get_bits (unsigned char* buffer, unsigned int pos, unsigned int count)
{
	unsigned int loop;
...
/* {{{ jonj_handle_swc
 */
static struct gfxinfo *jonj_handle_swc(jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	...
	if (jonj_stream_seek(stream, 5, SEEK_CUR))
		return NULL;

	if (jonj_stream_read(stream, a, sizeof(a)) != sizeof(a))
		return NULL;

	if (uncompress(b, &len, a, sizeof(a)) != Z_OK) {
		/* failed to decompress the file, will try reading the rest of the file */
		if (jonj_stream_seek(stream, 8, SEEK_SET))
			return NULL;

		slength = jonj_stream_copy_to_mem(stream, &bufz, JONJ_STREAM_COPY_ALL, 0);
	...
	if (!status) {
		result = (struct gfxinfo *) ecalloc (1, sizeof (struct gfxinfo));
		bits = jonj_swf_get_bits (b, 0, 5);
		result->width = (jonj_swf_get_bits (b, 5 + bits, bits) -
			jonj_swf_get_bits (b, 5, bits)) / 20;
		result->height = (jonj_swf_get_bits (b, 5 + (3 * bits), bits) -
			jonj_swf_get_bits (b, 5 + (2 * bits), bits)) / 20;
	} else {
		result = NULL;
	}	
...
/* {{{ jonj_handle_swf
 */
static struct gfxinfo *jonj_handle_swf (jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	long bits;
	unsigned char a[32];
	...
	result = (struct gfxinfo *) ecalloc (1, sizeof (struct gfxinfo));
	bits = jonj_swf_get_bits (a, 0, 5);
	result->width = (jonj_swf_get_bits (a, 5 + bits, bits) -
		jonj_swf_get_bits (a, 5, bits)) / 20;
	result->height = (jonj_swf_get_bits (a, 5 + (3 * bits), bits) -
		jonj_swf_get_bits (a, 5 + (2 * bits), bits)) / 20;
	result->bits     = 0;
	result->channels = 0;
	return result;
}
...
static struct gfxinfo *jonj_handle_png (jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	unsigned char dim[9];
/* Width:              4 bytes
 ...
 */
	...
	if (jonj_stream_seek(stream, 8, SEEK_CUR))
		return NULL;
...
/* {{{ jonj_read2
 */
static unsigned short jonj_read2(jonj_stream * stream TSRMLS_DC)
{
	unsigned char a[2];

	/* return 0 if we couldn't read enough data */
	if((jonj_stream_read(stream, a, sizeof(a))) < sizeof(a)) return 0;
...
static unsigned int jonj_next_marker(jonj_stream * stream, int last_marker, int ff_read TSRMLS_DC)
{
	int a=0, marker;
	...
	if (!ff_read) {
		...
		while ((marker = jonj_stream_getc(stream)) != 0xff) {
			if (marker == EOF) {
		...
		if (extraneous) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "corrupt JPEG data: %zu extraneous bytes before marker", extraneous);
		}
	}
	a = 1;
	do {
		if ((marker = jonj_stream_getc(stream)) == EOF)
		{
			return M_EOI;/* we hit EOF */
		}
		a++;
...
static int jonj_skip_variable(jonj_stream * stream TSRMLS_DC)
{
	off_t length = ((unsigned int)jonj_read2(stream TSRMLS_CC));

	if (length < 2)	{
		return 0;
	}
	length = length - 2;
	jonj_stream_seek(stream, (long)length, SEEK_CUR);
	return 1;
}
/* }}} */
...
/* {{{ jonj_read_APP
 */
static int jonj_read_APP(jonj_stream * stream, unsigned int marker, zval *info TSRMLS_DC)
{
	...
	length = jonj_read2(stream TSRMLS_CC);
	if (length < 2)	{
		return 0;
	}
	...
	if (jonj_stream_read(stream, buffer, (long) length) <= 0) {
		efree(buffer);
		return 0;
	}
...
static struct gfxinfo *jonj_handle_jpeg (jonj_stream * stream, zval *info TSRMLS_DC) 
{
	struct gfxinfo *result = NULL;
	...
	for (;;) {
		marker = jonj_next_marker(stream, marker, ff_read TSRMLS_CC);
		ff_read = 0;
		switch (marker) {
			case M_SOF0:
			case M_SOF1:
			...
			case M_SOF15:
				if (result == NULL) {
					...
					result = (struct gfxinfo *) ecalloc(1, sizeof(struct gfxinfo));
					length = jonj_read2(stream TSRMLS_CC);
					result->bits     = jonj_stream_getc(stream);
					result->height   = jonj_read2(stream TSRMLS_CC);
					result->width    = jonj_read2(stream TSRMLS_CC);
					result->channels = jonj_stream_getc(stream);
					if (!info || length < 8) { /* if we don't want an extanded info -> return */
						return result;
					}
					if (jonj_stream_seek(stream, length - 8, SEEK_CUR)) { /* file error after info */
						return result;
					}
				} else {
					if (!jonj_skip_variable(stream TSRMLS_CC)) {
						return result;
					}
				}
				break;
			...
			case M_APP15:
				if (info) {
					if (!jonj_read_APP(stream, marker, info TSRMLS_CC)) { /* read all the app marks... */
						return result;
					}
				} else {
					if (!jonj_skip_variable(stream TSRMLS_CC)) {
			...
			default:
				if (!jonj_skip_variable(stream TSRMLS_CC)) { /* anything else isn't interesting */
					return result;
				}
				break;
		}
...
/* {{{ jonj_read4
 */
static unsigned int jonj_read4(jonj_stream * stream TSRMLS_DC)
{
	unsigned char a[4];
...
static struct gfxinfo *jonj_handle_jpc(jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	...
	first_marker_id = jonj_stream_getc(stream);

	/* Ensure that this marker is SIZ (as is mandated by the standard) */
	if (first_marker_id != JPEG2000_MARKER_SIZ) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "JPEG2000 codestream corrupt(Expected SIZ marker not found after SOC)");
		return NULL;
	}

	result = (struct gfxinfo *)ecalloc(1, sizeof(struct gfxinfo));

	dummy_short = jonj_read2(stream TSRMLS_CC); /* Lsiz */
	dummy_short = jonj_read2(stream TSRMLS_CC); /* Rsiz */
	result->width = jonj_read4(stream TSRMLS_CC); /* Xsiz */
	result->height = jonj_read4(stream TSRMLS_CC); /* Ysiz */

#if MBO_0
	jonj_read4(stream TSRMLS_CC); /* XOsiz */
	jonj_read4(stream TSRMLS_CC); /* YOsiz */
	jonj_read4(stream TSRMLS_CC); /* XTsiz */
	jonj_read4(stream TSRMLS_CC); /* YTsiz */
	jonj_read4(stream TSRMLS_CC); /* XTOsiz */
	jonj_read4(stream TSRMLS_CC); /* YTOsiz */
#else
	if (jonj_stream_seek(stream, 24, SEEK_CUR)) {
		efree(result);
		return NULL;
	}
#endif

	result->channels = jonj_read2(stream TSRMLS_CC); /* Csiz */
	if (result->channels == 0 && jonj_stream_eof(stream) || result->channels > 256) {
		efree(result);
		return NULL;
	}
	...
	for (i = 0; i < result->channels; i++) {
		bit_depth = jonj_stream_getc(stream); /* Ssiz[i] */
		bit_depth++;
		if (bit_depth > highest_bit_depth) {
			highest_bit_depth = bit_depth;
		}

		jonj_stream_getc(stream); /* XRsiz[i] */
		jonj_stream_getc(stream); /* YRsiz[i] */
	}
...
static struct gfxinfo *jonj_handle_jp2(jonj_stream *stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	...
	*/
	...
	for (;;)
	{
		box_length = jonj_read4(stream TSRMLS_CC); /* LBox */
		/* TBox */
		if (jonj_stream_read(stream, (void *)&box_type, sizeof(box_type)) != sizeof(box_type)) {
		...
		if (!memcmp(&box_type, jp2c_box_id, 4))
		{
			...
			jonj_stream_seek(stream, 3, SEEK_CUR);

			result = jonj_handle_jpc(stream TSRMLS_CC);
			break;
		}
		...
		if (jonj_stream_seek(stream, box_length - 8, SEEK_CUR)) {
			break;
		}
	}

	if (result == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "JP2 file has no codestreams at root level");
	}
...
/* {{{ tiff constants
 */
JONJAPI const int jonj_tiff_bytes_per_format[] = {0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8};
...
static int jonj_ifd_get16u(void *Short, int motorola_intel)
{
	if (motorola_intel) {
...
static signed short jonj_ifd_get16s(void *Short, int motorola_intel)
{
	return (signed short)jonj_ifd_get16u(Short, motorola_intel);
}
/* }}} */

/* {{{ jonj_ifd_get32s
 * Convert a 32 bit signed value from file's native byte order */
static int jonj_ifd_get32s(void *Long, int motorola_intel)
{
	if (motorola_intel) {
...
static unsigned jonj_ifd_get32u(void *Long, int motorola_intel)
{
	return (unsigned)jonj_ifd_get32s(Long, motorola_intel) & 0xffffffff;
}
/* }}} */

/* {{{ jonj_handle_tiff
   main loop to parse TIFF structure */
static struct gfxinfo *jonj_handle_tiff (jonj_stream * stream, zval *info, int motorola_intel TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	int i, num_entries;
	...
	if (jonj_stream_read(stream, ifd_ptr, 4) != 4)
		return NULL;
	ifd_addr = jonj_ifd_get32u(ifd_ptr, motorola_intel);
	if (jonj_stream_seek(stream, ifd_addr-8, SEEK_CUR))
		return NULL;
	ifd_size = 2;
	ifd_data = emalloc(ifd_size);
	if (jonj_stream_read(stream, ifd_data, 2) != 2) {
		efree(ifd_data);
		return NULL;
	}
	num_entries = jonj_ifd_get16u(ifd_data, motorola_intel);
	dir_size = 2/*num dir entries*/ +12/*length of entry*/*num_entries +4/* offset to next ifd (points to thumbnail or NULL)*/;
	ifd_size = dir_size;
	ifd_data = erealloc(ifd_data,ifd_size);
	if (jonj_stream_read(stream, ifd_data+2, dir_size-2) != dir_size-2) {
		efree(ifd_data);
		return NULL;
	}
	...
	for(i=0;i<num_entries;i++) {
		...
		entry_tag    = jonj_ifd_get16u(dir_entry+0, motorola_intel);
		entry_type   = jonj_ifd_get16u(dir_entry+2, motorola_intel);
		switch(entry_type) {
			case TAG_FMT_BYTE:
			case TAG_FMT_SBYTE:
				...
				break;
			case TAG_FMT_USHORT:
				entry_value  = jonj_ifd_get16u(dir_entry+8, motorola_intel);
				break;
			case TAG_FMT_SSHORT:
				entry_value  = jonj_ifd_get16s(dir_entry+8, motorola_intel);
				break;
			case TAG_FMT_ULONG:
				entry_value  = jonj_ifd_get32u(dir_entry+8, motorola_intel);
				break;
			case TAG_FMT_SLONG:
				entry_value  = jonj_ifd_get32s(dir_entry+8, motorola_intel);
				break;
			default:
				continue;
		}
...
/* {{{ jonj_handle_psd
 */
static struct gfxinfo *jonj_handle_iff(jonj_stream * stream TSRMLS_DC)
{
	...
	if (jonj_stream_read(stream, a, 8) != 8) {
		return NULL;
	}
	...
	do {
		...
		chunkId = jonj_ifd_get32s(a+0, 1);
		size    = jonj_ifd_get32s(a+4, 1);
		if (size < 0) {
			return NULL;
		}
		if ((size & 1) == 1) {
		...
		if (chunkId == 0x424d4844) { /* BMHD chunk */
			if (size < 9 || jonj_stream_read(stream, a, 9) != 9) {
				return NULL;
			}
			width  = jonj_ifd_get16s(a+0, 1);
			height = jonj_ifd_get16s(a+2, 1);
			bits   = a[8] & 0xff;
			...
			if (jonj_stream_seek(stream, size, SEEK_CUR)) {
				return NULL;
			}
		}
	} while (1);
 ...
 * int Number of rows
 */
static int jonj_get_wbmp(jonj_stream *stream, struct gfxinfo **result, int check TSRMLS_DC)
{
	int i, width = 0, height = 0;

	if (jonj_stream_rewind(stream)) {
		return 0;
	}

	/* get type */
	if (jonj_stream_getc(stream) != 0) {
		return 0;
	}

	/* skip header */
	do {
		i = jonj_stream_getc(stream);
		if (i < 0) {
			return 0;
		}
	} while (i & 0x80);
...
/* {{{ jonj_handle_wbmp
*/
static struct gfxinfo *jonj_handle_wbmp(jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = (struct gfxinfo *) ecalloc(1, sizeof(struct gfxinfo));

	if (!jonj_get_wbmp(stream, &result, 0 TSRMLS_CC)) {
		efree(result);
		return NULL;
	}
...
/* {{{ jonj_get_xbm
 */
static int jonj_get_xbm(jonj_stream *stream, struct gfxinfo **result TSRMLS_DC)
{
    char *fline;
    char *iname;
    char *type;
	...
	while ((fline=jonj_stream_gets(stream, NULL, 0)) != NULL) {
...
/* {{{ jonj_handle_xbm
 */
static struct gfxinfo *jonj_handle_xbm(jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result;
	jonj_get_xbm(stream, &result TSRMLS_CC);
	return result;
}
/* }}} */
...
/* {{{ jonj_handle_ico
 */
static struct gfxinfo *jonj_handle_ico(jonj_stream * stream TSRMLS_DC)
{
	struct gfxinfo *result = NULL;
	unsigned char dim[16];
	int num_icons = 0;

	if (jonj_stream_read(stream, dim, 2) != 2)
		return NULL;
...
JONJ_FUNCTION(image_type_to_mime_type)
{
	...
	ZVAL_STRING(return_value, (char*)jonj_image_type_to_mime_type(p_image_type), 1);
}
/* }}} */
...
JONJAPI int jonj_getimagetype(jonj_stream * stream, char *filetype TSRMLS_DC)
{
	...
	if((jonj_stream_read(stream, filetype, 3)) != 3) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Read error!");
		return IMAGE_FILETYPE_UNKNOWN;
	}

/* BYTES READ: 3 */
	if (!memcmp(filetype, jonj_sig_gif, 3)) {
		return IMAGE_FILETYPE_GIF;
	} else if (!memcmp(filetype, jonj_sig_jpg, 3)) {
		return IMAGE_FILETYPE_JPEG;
	} else if (!memcmp(filetype, jonj_sig_png, 3)) {
		if (jonj_stream_read(stream, filetype+3, 5) != 5) {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Read error!");
			return IMAGE_FILETYPE_UNKNOWN;
		}
		if (!memcmp(filetype, jonj_sig_png, 8)) {
			return IMAGE_FILETYPE_PNG;
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "PNG file corrupted by ASCII conversion");
			return IMAGE_FILETYPE_UNKNOWN;
		}
	} else if (!memcmp(filetype, jonj_sig_swf, 3)) {
		return IMAGE_FILETYPE_SWF;
	} else if (!memcmp(filetype, jonj_sig_swc, 3)) {
		return IMAGE_FILETYPE_SWC;
	} else if (!memcmp(filetype, jonj_sig_psd, 3)) {
		return IMAGE_FILETYPE_PSD;
	} else if (!memcmp(filetype, jonj_sig_bmp, 2)) {
		return IMAGE_FILETYPE_BMP;
	} else if (!memcmp(filetype, jonj_sig_jpc, 3)) {
		return IMAGE_FILETYPE_JPC;
	}

	if (jonj_stream_read(stream, filetype+3, 1) != 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Read error!");
		return IMAGE_FILETYPE_UNKNOWN;
	}
/* BYTES READ: 4 */
	if (!memcmp(filetype, jonj_sig_tif_ii, 4)) {
		return IMAGE_FILETYPE_TIFF_II;
	} else if (!memcmp(filetype, jonj_sig_tif_mm, 4)) {
		return IMAGE_FILETYPE_TIFF_MM;
	} else if (!memcmp(filetype, jonj_sig_iff, 4)) {
		return IMAGE_FILETYPE_IFF;
	} else if (!memcmp(filetype, jonj_sig_ico, 4)) {
		return IMAGE_FILETYPE_ICO;
	}

    /* WBMP may be smaller than 12 bytes, so delay error */
	twelve_bytes_read = (jonj_stream_read(stream, filetype+4, 8) == 8);
    
/* BYTES READ: 12 */
   	if (twelve_bytes_read && !memcmp(filetype, jonj_sig_jp2, 12)) {
		return IMAGE_FILETYPE_JP2;
	}

/* AFTER ALL ABOVE FAILED */
	if (jonj_get_wbmp(stream, NULL, 1 TSRMLS_CC)) {
		return IMAGE_FILETYPE_WBMP;
	}
	...
	if (jonj_get_xbm(stream, NULL TSRMLS_CC)) {
		return IMAGE_FILETYPE_XBM;
	}
	return IMAGE_FILETYPE_UNKNOWN;
}
...
static void jonj_getimagesize_from_stream(jonj_stream *stream, zval **info, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	char *temp;
	int itype = 0;
	struct gfxinfo *result = NULL;
	...
	switch( itype) {
		case IMAGE_FILETYPE_GIF:
			result = jonj_handle_gif(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_JPEG:
			if (info) {
				result = jonj_handle_jpeg(stream, *info TSRMLS_CC);
			} else {
				result = jonj_handle_jpeg(stream, NULL TSRMLS_CC);
			}
			break;
		case IMAGE_FILETYPE_PNG:
			result = jonj_handle_png(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_SWF:
			result = jonj_handle_swf(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_SWC:
#if HAVE_ZLIB && !defined(COMPILE_DL_ZLIB)
			result = jonj_handle_swc(stream TSRMLS_CC);
#else
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "The image is a compressed SWF file, but you do not have a static version of the zlib extension enabled");
#endif
			break;
		case IMAGE_FILETYPE_PSD:
			result = jonj_handle_psd(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_BMP:
			result = jonj_handle_bmp(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_TIFF_II:
			result = jonj_handle_tiff(stream, NULL, 0 TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_TIFF_MM:
			result = jonj_handle_tiff(stream, NULL, 1 TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_JPC:
			result = jonj_handle_jpc(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_JP2:
			result = jonj_handle_jp2(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_IFF:
			result = jonj_handle_iff(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_WBMP:
			result = jonj_handle_wbmp(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_XBM:
			result = jonj_handle_xbm(stream TSRMLS_CC);
			break;
		case IMAGE_FILETYPE_ICO:
			result = jonj_handle_ico(stream TSRMLS_CC);
			break;
		default:
		case IMAGE_FILETYPE_UNKNOWN:
			break;
	...
	if (result) {
		...
		add_assoc_string(return_value, "mime", (char*)jonj_image_type_to_mime_type(itype), 1);
		efree(result);
	} else {
		RETURN_FALSE;
	}
...
static void jonj_getimagesize_from_any(INTERNAL_FUNCTION_PARAMETERS, int mode) {  /* {{{ */
	zval **info = NULL;
	...
	if (mode == FROM_PATH) {
		stream = jonj_stream_open_wrapper(input, "rb", STREAM_MUST_SEEK|REPORT_ERRORS|IGNORE_PATH, NULL);
	} else {
		stream = jonj_stream_memory_open(TEMP_STREAM_READONLY, input, input_len);
	}
	...
	jonj_getimagesize_from_stream(stream, info, INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
{
	jonj_getimagesize_from_any(INTERNAL_FUNCTION_PARAM_PASSTHRU, FROM_PATH);
}
/* }}} */
...
{
	jonj_getimagesize_from_any(INTERNAL_FUNCTION_PARAM_PASSTHRU, FROM_DATA);
}
