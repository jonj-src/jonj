...
#define INC_OUTPUTPOS(a,b) \
	if ((a) < 0 || ((INT_MAX - outputpos)/((int)b)) < (a)) { \
		...
		efree(formatcodes);	\
		efree(formatargs);	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: integer overflow in format string", code); \
		RETURN_FALSE; \
	} \
	outputpos += (a)*(b);
...
/* {{{ jonj_pack
 */
static void jonj_pack(zval **val, int size, int *map, char *output)
{
	int i;
	char *v;
...
JONJ_FUNCTION(pack)
{
	...
	for (i = 0; i < formatlen; formatcount++) {
		...
		switch ((int) code) {
			...
			case '@':
				if (arg < 0) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: '*' ignored", code);
					arg = 1;
				}
				break;
			...
			case 'H':
				if (currentarg >= num_args) {
					...
					efree(formatcodes);
					efree(formatargs);
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough arguments", code);
					RETURN_FALSE;
				}
			...
			case 'P':
					...
					efree(formatcodes);
					efree(formatargs);
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "64-bit format codes are not available for 32-bit versions of JONJ");
					RETURN_FALSE;
#endif
			case 'c': 
			case 'C': 
			...
			case 'd':
				...
				if (currentarg > num_args) {
					...
					efree(formatcodes);
					efree(formatargs);
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: too few arguments", code);
					RETURN_FALSE;
				}
				break;
			...
			default:
				...
				efree(formatcodes);
				efree(formatargs);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: unknown format code", code);
				RETURN_FALSE;
		}
	...
	if (currentarg < num_args) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "%d arguments unused", (num_args - currentarg));
	}
	...
	for (i = 0; i < formatcount; i++) {
		...
		switch ((int) code) {
			...
			case 'X':
				...
				if (outputpos < 0) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: outside of string", code);
					outputpos = 0;
				}
				break;
		...
		switch ((int) code) {
			...
			case 'H': {
				...
				if(arg > Z_STRLEN_PP(val)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough characters in string", code);
					arg = Z_STRLEN_PP(val);
				}
				...
				while (arg-- > 0) {
					...
					if (n >= '0' && n <= '9') {
						...
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: illegal hex digit %c", code, n);
						n = 0;
					}
			...
			case 'C':
				while (arg-- > 0) {
					jonj_pack(argv[currentarg++], 1, byte_map, &output[outputpos]);
					outputpos++;
				}
				break;
			...
			case 'v': {
				...
				while (arg-- > 0) {
					jonj_pack(argv[currentarg++], 2, map, &output[outputpos]);
					outputpos += 2;
				}
				break;
			}
			...
			case 'I':
				while (arg-- > 0) {
					jonj_pack(argv[currentarg++], sizeof(int), int_map, &output[outputpos]);
			...
			case 'V': {
				...
				while (arg-- > 0) {
					jonj_pack(argv[currentarg++], 4, map, &output[outputpos]);
					outputpos += 4;
				}
				break;
			}
			...
			case 'P': {
				...
				while (arg-- > 0) {
					jonj_pack(argv[currentarg++], 8, map, &output[outputpos]);
					outputpos += 8;
				}
				break;
			}
...
/* {{{ jonj_unpack
 */
static long jonj_unpack(char *data, int size, int issigned, int *map)
{
	long result;
...
JONJ_FUNCTION(unpack)
{
	...
	while (formatlen-- > 0) {
		...
		switch ((int) type) {
			...
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid format type %c", type);
				zval_dtor(return_value);
				RETURN_FALSE;
				break;
		}
		...
		if (size != 0 && size != -1 && size < 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: integer overflow", type);
			zval_dtor(return_value);
			RETURN_FALSE;
		}
			...
			if ((inputpos + size) <= inputlen) {
				switch ((int) type) {
					...
					case 'C': {
						...
						long v = jonj_unpack(&input[inputpos], 1, issigned, byte_map);
					...
					case 'v': {
						...
						v = jonj_unpack(&input[inputpos], 2, issigned, map);
						add_assoc_long(return_value, n, v);
						break;
					}
					...
					case 'I': {
						...
						v = jonj_unpack(&input[inputpos], sizeof(int), issigned, int_map);
						add_assoc_long(return_value, n, v);
						break;
					}
					...
					case 'V': {
						...
						v |= jonj_unpack(&input[inputpos], 4, issigned, map);
						if (sizeof(long) > 4) {
 							if (type == 'l') {
								v = (signed int) v;
							} else {
					...
					case 'P': {
						...
						v = jonj_unpack(&input[inputpos], 8, issigned, map);
					...
					case 'X':
						if (inputpos < size) {
							...
							if (arg >= 0) {
								jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: outside of string", type);
							}
						}
						break;
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough input, need %d, have %d", type, size, inputlen - inputpos);
				zval_dtor(return_value);
				RETURN_FALSE;
			}
		}
