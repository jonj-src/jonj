...
typedef struct ps_sd {
	struct ps_sd *next;
	jonj_uint32 hv;		/* hash value of key */
...
typedef struct {
	MM *mm;
	ps_sd **hash;
	jonj_uint32 hash_max;
	jonj_uint32 hash_cnt;
	pid_t owner;
} ps_mm;
...
static inline jonj_uint32 ps_sd_hash(const char *data, int len)
{
	jonj_uint32 h;
...
static ps_sd *ps_sd_new(ps_mm *data, const char *key)
{
	...
	if (!sd) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mm_malloc failed, avail %ld, err %s", mm_available(data->mm), mm_error());
		return NULL;
	}
...
JONJ_MINIT_FUNCTION(ps_mm)
{
	...
	jonj_session_register_module(&ps_mod_mm);
	return SUCCESS;
}
...
PS_WRITE_FUNC(mm)
{
	...
	if (sd) {
		if (vallen >= sd->alloclen) {
			...
			if (!sd->data) {
				ps_sd_destroy(data, sd);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot allocate new data segment");
				sd = NULL;
			}
		}
		if (sd) {
