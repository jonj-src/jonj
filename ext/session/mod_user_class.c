/*
   ...
   +----------------------------------------------------------------------+
   | Author: Arpad Ray <arpad@jonj.tk>                                    |
   +----------------------------------------------------------------------+
 */
...
#define PS_SANITY_CHECK						\
	if (PS(default_mod) == NULL) {				\
		jonj_error_docref(NULL TSRMLS_CC, E_CORE_ERROR, "Cannot call default session handler"); \
		RETURN_FALSE;						\
	}							
...
#define PS_SANITY_CHECK_IS_OPEN				\
	...
	if (!PS(mod_user_is_open)) {			\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Parent session handler is not open");	\
		RETURN_FALSE;						\
	}							

/* {{{ proto bool SessionHandler::open(string save_path, string session_name)
   Wraps the old open handler */
JONJ_METHOD(SessionHandler, open)
{
...
JONJ_METHOD(SessionHandler, close)
{
...
JONJ_METHOD(SessionHandler, read)
{
	char *key, *val;
	int key_len, val_len;
...
JONJ_METHOD(SessionHandler, write)
{
	char *key, *val;
	int key_len, val_len;
...
JONJ_METHOD(SessionHandler, destroy)
{
	char *key;
	int key_len;
...
JONJ_METHOD(SessionHandler, create_sid)
{
	char *id;
