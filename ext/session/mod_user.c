...
PS_CREATE_SID_FUNC(user)
{
	...
	if (PSF(create_sid) != NULL) {
		...
		if (retval) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "No session id returned by function");
			return NULL;
		}

		if (!id) {
			jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Session id must be a string");
			return NULL;
		}
	...
	return jonj_session_create_id(mod_data, newlen TSRMLS_CC);
}
