...
#ifndef JONJ_SESSION_H
#define JONJ_SESSION_H
...
#define JONJ_SESSION_API 20020330

/* To check jonj_session_valid_key()/jonj_session_reset_id() */
#define JONJ_SESSION_STRICT 1
...
#define PS_MOD(x) \
	#x, ps_open_##x, ps_close_##x, ps_read_##x, ps_write_##x, \
	 ps_delete_##x, ps_gc_##x, jonj_session_create_id
...
typedef enum {
	jonj_session_disabled,
	jonj_session_none,
	jonj_session_active
} jonj_session_status;

typedef struct _jonj_session_rfc1867_progress {
...
} jonj_session_rfc1867_progress;

typedef struct _jonj_ps_globals {
	char *save_path;
	char *session_name;
	char *id;
	...
	void *mod_data;
	jonj_session_status session_status;
	long gc_probability;
	long gc_divisor;
	long gc_maxlifetime;
	int module_number;
	...
	jonj_session_rfc1867_progress *rfc1867_progress;
	...
	zend_bool use_strict_mode; /* whether or not JONJ accepts unknown session ids */
	unsigned char session_data_hash[16]; /* binary MD5 hash length */
} jonj_ps_globals;

typedef jonj_ps_globals zend_ps_globals;

extern zend_module_entry session_module_entry;
#define jonjext_session_ptr &session_module_entry

#ifdef ZTS
#define PS(v) TSRMG(ps_globals_id, jonj_ps_globals *, v)
#else
...
JONJAPI void session_adapt_url(const char *, size_t, char **, size_t * TSRMLS_DC);
...
JONJAPI int jonj_session_register_module(ps_module *);

JONJAPI int jonj_session_register_serializer(const char *name,
	        int (*encode)(PS_SERIALIZER_ENCODE_ARGS),
	        int (*decode)(PS_SERIALIZER_DECODE_ARGS));

JONJAPI void jonj_session_set_id(char *id TSRMLS_DC);
...
#define PS_ADD_VARL(name,namelen) do {										\
	jonj_add_session_var(name, namelen TSRMLS_CC);							\
} while (0)
...
#define PS_ENCODE_LOOP(code) do {									\
		...
		for (zend_hash_internal_pointer_reset(_ht);					\
			...
			if (key_type == HASH_KEY_IS_LONG) {						\
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Skipping numeric key %ld", num_key);	\
				continue;											\
			}														\
			key_length--;											\
			if (jonj_get_session_var(key, key_length, &struc TSRMLS_CC) == SUCCESS) {	\
				code;		 										\
			} 														\
		}															\
	} while(0)

JONJAPI ZEND_EXTERN_MODULE_GLOBALS(ps)

void jonj_session_auto_start(void *data);

#define PS_CLASS_NAME "SessionHandler"
extern zend_class_entry *jonj_session_class_entry;

#define PS_IFACE_NAME "SessionHandlerInterface"
extern zend_class_entry *jonj_session_iface_entry;

#define PS_SID_IFACE_NAME "SessionIdInterface"
extern zend_class_entry *jonj_session_id_iface_entry;

extern JONJ_METHOD(SessionHandler, open);
extern JONJ_METHOD(SessionHandler, close);
extern JONJ_METHOD(SessionHandler, read);
extern JONJ_METHOD(SessionHandler, write);
extern JONJ_METHOD(SessionHandler, destroy);
extern JONJ_METHOD(SessionHandler, gc);
extern JONJ_METHOD(SessionHandler, create_sid);
