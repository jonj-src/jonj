...
static void ps_files_open(ps_files *data, const char *key TSRMLS_DC)
{
	...
	if (data->fd < 0 || !data->lastkey || strcmp(key, data->lastkey)) {
		...
		if (jonj_session_valid_key(key) == FAILURE) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "The session id is too long or contains illegal characters, valid characters are a-z, A-Z, 0-9 and '-,'");
			return;
		}

		if (!ps_files_path_create(buf, sizeof(buf), data, key)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to create session data file path. Too short session ID, invalid save_path or path lentgth exceeds MAXPATHLEN(%d)", MAXPATHLEN);
			return;
		}
		...
		if(PG(open_basedir) && lstat(buf, &sbuf) == 0 && S_ISLNK(sbuf.st_mode) && jonj_check_open_basedir(buf TSRMLS_CC)) {
			return;
		}
		...
		if (data->fd != -1) {
			...
			if (fstat(data->fd, &sbuf) || (sbuf.st_uid != 0 && sbuf.st_uid != getuid() && sbuf.st_uid != geteuid())) {
				close(data->fd);
				data->fd = -1;
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Session data file is not created by your uid");
				return;
			}
#endif
			do {
...
# endif
			if (fcntl(data->fd, F_SETFD, FD_CLOEXEC)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "fcntl(%d, F_SETFD, FD_CLOEXEC) failed: %s (%d)", data->fd, strerror(errno), errno);
			}
#endif
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "open(%s, O_RDWR) failed: %s (%d)", buf, strerror(errno), errno);
		}
	}
}
...
static int ps_files_cleanup_dir(const char *dirname, int maxlifetime TSRMLS_DC)
{
	...
	dir = opendir(dirname);
	if (!dir) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "ps_files_cleanup_dir: opendir(%s) failed: %s (%d)", dirname, strerror(errno), errno);
		return (0);
	}
	...
	buf[dirname_len] = JONJ_DIR_SEPARATOR;

	while (jonj_readdir_r(dir, (struct dirent *) dentry, &entry) == 0 && entry) {
...
PS_OPEN_FUNC(files)
{
	...
	if (*save_path == '\0') {
		...
		save_path = jonj_get_temporary_directory(TSRMLS_C);

		if (jonj_check_open_basedir(save_path TSRMLS_CC)) {
			return FAILURE;
		}
	}
	...
	if (argc > 1) {
		...
		if (errno == ERANGE) {
			jonj_error(E_WARNING, "The first parameter in session.save_path is invalid");
			return FAILURE;
		}
	}
	...
	if (argc > 2) {
		...
		if (errno == ERANGE || filemode < 0 || filemode > 07777) {
			jonj_error(E_WARNING, "The second parameter in session.save_path is invalid");
			return FAILURE;
		}
	}
...
PS_READ_FUNC(files)
{
	...
	*val = emalloc(sbuf.st_size);
	...
	if (n != sbuf.st_size) {
		if (n == -1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "read failed: %s (%d)", strerror(errno), errno);
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "read returned less bytes than requested");
		}
		efree(*val);
		return FAILURE;
	}
...
PS_WRITE_FUNC(files)
{
	...
	if (vallen < (int)data->st_size) {
		jonj_ignore_value(ftruncate(data->fd, 0));
	}
	...
	if (n != vallen) {
		if (n == -1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "write failed: %s (%d)", strerror(errno), errno);
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "write wrote less bytes than requested");
		}
		return FAILURE;
	}
...
PS_CREATE_SID_FUNC(files)
{
	...
	do {
		sid = jonj_session_create_id((void **)&data, newlen TSRMLS_CC);
		/* Check collision */
