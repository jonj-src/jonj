...
typedef struct _jonj_mcrypt_filter_data {
	MCRYPT module;
	char encrypt;
	int blocksize;
	char *block_buffer;
	int block_used;
	char persistent;
} jonj_mcrypt_filter_data;

static jonj_stream_filter_status_t jonj_mcrypt_filter(
	jonj_stream *stream,
	...
	int flags TSRMLS_DC)
{
	...
	data = (jonj_mcrypt_filter_data *)(thisfilter->abstract);
	while(buckets_in->head) {
		...
		if (data->blocksize) {
			...
			newbucket = jonj_stream_bucket_new(stream, outchunk, n, 1, data->persistent TSRMLS_CC);
			jonj_stream_bucket_append(buckets_out, newbucket TSRMLS_CC);
	...
	if ((flags & PSFS_FLAG_FLUSH_CLOSE) && data->blocksize && data->block_used) {
		...
		newbucket = jonj_stream_bucket_new(stream, data->block_buffer, data->blocksize, 0, data->persistent TSRMLS_CC);
		jonj_stream_bucket_append(buckets_out, newbucket TSRMLS_CC);
...
static void jonj_mcrypt_filter_dtor(jonj_stream_filter *thisfilter TSRMLS_DC)
{
	if (thisfilter && thisfilter->abstract) {
		jonj_mcrypt_filter_data *data = (jonj_mcrypt_filter_data*)thisfilter->abstract;
...
static jonj_stream_filter_ops jonj_mcrypt_filter_ops = {
    jonj_mcrypt_filter,
    jonj_mcrypt_filter_dtor,
    "mcrypt.*"
};

/* {{{ jonj_mcrypt_filter_create
 * Instantiate mcrypt filter
 */
static jonj_stream_filter *jonj_mcrypt_filter_create(const char *filtername, zval *filterparams, int persistent TSRMLS_DC)
{
	...
	if (!filterparams || Z_TYPE_P(filterparams) != IS_ARRAY) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter parameters for %s must be an array", filtername);
		return NULL;
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "mode", sizeof("mode"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "mode is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "algorithms_dir", sizeof("algorithms_dir"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "algorithms_dir is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "modes_dir", sizeof("modes_dir"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "modes_dir is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "key", sizeof("key"), (void**)&tmpzval) == SUCCESS &&
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key not specified or is not a string");
		return NULL;
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "iv", sizeof("iv"), (void**) &tmpzval) == FAILURE ||
		Z_TYPE_PP(tmpzval) != IS_STRING) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter parameter[iv] not provided or not of type: string");
	...
	data = pemalloc(sizeof(jonj_mcrypt_filter_data), persistent);
	...
	return jonj_stream_filter_alloc(&jonj_mcrypt_filter_ops, data, persistent);
}
/* }}} */

jonj_stream_filter_factory jonj_mcrypt_filter_factory = {
	jonj_mcrypt_filter_create
};
