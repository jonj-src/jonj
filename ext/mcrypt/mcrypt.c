...
#include "ext/standard/jonj_rand.h"
#include "ext/standard/jonj_smart_str.h"
#include "jonj_mcrypt_filter.h"

static int le_mcrypt;

typedef struct _jonj_mcrypt {
	MCRYPT td;
	zend_bool init;
} jonj_mcrypt;
...
const zend_function_entry mcrypt_functions[] = { /* {{{ */
	JONJ_DEP_FE(mcrypt_ecb, 				arginfo_mcrypt_ecb)
	JONJ_DEP_FE(mcrypt_cbc, 				arginfo_mcrypt_cbc)
	JONJ_DEP_FE(mcrypt_cfb, 				arginfo_mcrypt_cfb)
	JONJ_DEP_FE(mcrypt_ofb, 				arginfo_mcrypt_ofb)
	JONJ_FE(mcrypt_get_key_size, 	arginfo_mcrypt_get_key_size)
	JONJ_FE(mcrypt_get_block_size, 	arginfo_mcrypt_get_block_size)
	JONJ_FE(mcrypt_get_cipher_name, 	arginfo_mcrypt_get_cipher_name)
	JONJ_FE(mcrypt_create_iv, 		arginfo_mcrypt_create_iv)

	JONJ_FE(mcrypt_list_algorithms, 	arginfo_mcrypt_list_algorithms)
	JONJ_FE(mcrypt_list_modes, 		arginfo_mcrypt_list_modes)
	JONJ_FE(mcrypt_get_iv_size, 		arginfo_mcrypt_get_iv_size)
	JONJ_FE(mcrypt_encrypt, 			arginfo_mcrypt_encrypt)
	JONJ_FE(mcrypt_decrypt, 			arginfo_mcrypt_decrypt)

	JONJ_FE(mcrypt_module_open, 		arginfo_mcrypt_module_open)
	JONJ_FE(mcrypt_generic_init, 	arginfo_mcrypt_generic_init)
	JONJ_FE(mcrypt_generic, 			arginfo_mcrypt_generic)
	JONJ_FE(mdecrypt_generic, 		arginfo_mdecrypt_generic)
	JONJ_DEP_FALIAS(mcrypt_generic_end, mcrypt_generic_deinit, arginfo_mcrypt_generic_deinit)
	JONJ_FE(mcrypt_generic_deinit, 	arginfo_mcrypt_generic_deinit)

	JONJ_FE(mcrypt_enc_self_test, 	arginfo_mcrypt_enc_self_test)
	JONJ_FE(mcrypt_enc_is_block_algorithm_mode, arginfo_mcrypt_enc_is_block_algorithm_mode)
	JONJ_FE(mcrypt_enc_is_block_algorithm, 	arginfo_mcrypt_enc_is_block_algorithm)
	JONJ_FE(mcrypt_enc_is_block_mode, 		arginfo_mcrypt_enc_is_block_mode)
	JONJ_FE(mcrypt_enc_get_block_size, 		arginfo_mcrypt_enc_get_block_size)
	JONJ_FE(mcrypt_enc_get_key_size, 		arginfo_mcrypt_enc_get_key_size)
	JONJ_FE(mcrypt_enc_get_supported_key_sizes, arginfo_mcrypt_enc_get_supported_key_sizes)
	JONJ_FE(mcrypt_enc_get_iv_size, 			arginfo_mcrypt_enc_get_iv_size)
	JONJ_FE(mcrypt_enc_get_algorithms_name, 	arginfo_mcrypt_enc_get_algorithms_name)
	JONJ_FE(mcrypt_enc_get_modes_name, 		arginfo_mcrypt_enc_get_modes_name)
	JONJ_FE(mcrypt_module_self_test, 		arginfo_mcrypt_module_self_test)

	JONJ_FE(mcrypt_module_is_block_algorithm_mode, 	arginfo_mcrypt_module_is_block_algorithm_mode)
	JONJ_FE(mcrypt_module_is_block_algorithm, 		arginfo_mcrypt_module_is_block_algorithm)
	JONJ_FE(mcrypt_module_is_block_mode, 			arginfo_mcrypt_module_is_block_mode)
	JONJ_FE(mcrypt_module_get_algo_block_size, 		arginfo_mcrypt_module_get_algo_block_size)
	JONJ_FE(mcrypt_module_get_algo_key_size, 		arginfo_mcrypt_module_get_algo_key_size)
	JONJ_FE(mcrypt_module_get_supported_key_sizes, 	arginfo_mcrypt_module_get_supported_key_sizes)

	JONJ_FE(mcrypt_module_close, 					arginfo_mcrypt_module_close)
	JONJ_FE_END
};
/* }}} */

static JONJ_MINFO_FUNCTION(mcrypt);
static JONJ_MINIT_FUNCTION(mcrypt);
static JONJ_MSHUTDOWN_FUNCTION(mcrypt);
static JONJ_GINIT_FUNCTION(mcrypt);
static JONJ_GSHUTDOWN_FUNCTION(mcrypt);
...
zend_module_entry mcrypt_module_entry = {
	...
	"mcrypt",
	mcrypt_functions,
	JONJ_MINIT(mcrypt), JONJ_MSHUTDOWN(mcrypt),
	NULL, NULL,
	JONJ_MINFO(mcrypt),
	NO_VERSION_YET,
	JONJ_MODULE_GLOBALS(mcrypt),
	JONJ_GINIT(mcrypt),
	JONJ_GSHUTDOWN(mcrypt),
	NULL,
...
#define MCRYPT_GET_TD_ARG										\
	...
	ZEND_FETCH_RESOURCE (pm, jonj_mcrypt *, &mcryptind, -1, "MCrypt", le_mcrypt);
...
#define JONJ_MCRYPT_INIT_CHECK	\
	if (!pm->init) {	\
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Operation disallowed prior to mcrypt_generic_init().");	\
		RETURN_FALSE;	\
	}	\

JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("mcrypt.algorithms_dir", NULL, JONJ_INI_ALL, OnUpdateString, algorithms_dir, zend_mcrypt_globals, mcrypt_globals)
	STD_JONJ_INI_ENTRY("mcrypt.modes_dir",      NULL, JONJ_INI_ALL, OnUpdateString, modes_dir, zend_mcrypt_globals, mcrypt_globals)
JONJ_INI_END()

static void jonj_mcrypt_module_dtor(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	jonj_mcrypt *pm = (jonj_mcrypt *) rsrc->ptr;
	if (pm) {
...
static JONJ_MINIT_FUNCTION(mcrypt) /* {{{ */
{
	le_mcrypt = zend_register_list_destructors_ex(jonj_mcrypt_module_dtor, NULL, "mcrypt", module_number);
...
/* modes */
	...
	jonj_stream_filter_register_factory("mcrypt.*", &jonj_mcrypt_filter_factory TSRMLS_CC);
	jonj_stream_filter_register_factory("mdecrypt.*", &jonj_mcrypt_filter_factory TSRMLS_CC);
...
static JONJ_MSHUTDOWN_FUNCTION(mcrypt) /* {{{ */
{
	jonj_stream_filter_unregister_factory("mcrypt.*" TSRMLS_CC);
	jonj_stream_filter_unregister_factory("mdecrypt.*" TSRMLS_CC);
...
JONJ_MINFO_FUNCTION(mcrypt) /* {{{ */
{
	...
	jonj_info_print_table_header(2, "mcrypt support", "enabled");
	jonj_info_print_table_header(2, "mcrypt_filter support", "enabled");
	jonj_info_print_table_row(2, "Version", LIBMCRYPT_VERSION);
	jonj_info_print_table_row(2, "Api No", mcrypt_api_no);
	jonj_info_print_table_row(2, "Supported ciphers", tmp1.c);
	jonj_info_print_table_row(2, "Supported modes", tmp2.c);
	smart_str_free(&tmp1);
	smart_str_free(&tmp2);
...
JONJ_FUNCTION(mcrypt_module_open)
{
	...
	if (td == MCRYPT_FAILED) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not open encryption module");
		RETURN_FALSE;
	} else {
		pm = emalloc(sizeof(jonj_mcrypt));
		pm->td = td;
		pm->init = 0;
...
JONJ_FUNCTION(mcrypt_generic_init)
{
	...
	if (key_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Key size is 0");
	}
	...
	if (key_len > max_key_size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Key size too large; supplied length: %d, max: %d", key_len, max_key_size);
	...
	if (iv_len != iv_size) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Iv size incorrect; supplied length: %d, needed: %d", iv_len, iv_size);
		if (iv_len > iv_size) {
			iv_len = iv_size;
		}
	}
	...
	if (result < 0) {
		...
		switch (result) {
			case -3:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Key length incorrect");
				break;
			case -4:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Memory allocation error");
				break;
			case -1:
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown error");
				break;
		}
	} else {
		pm->init = 1;
...
JONJ_FUNCTION(mcrypt_generic)
{
	...
	ZEND_FETCH_RESOURCE(pm, jonj_mcrypt *, &mcryptind, -1, "MCrypt", le_mcrypt);
	JONJ_MCRYPT_INIT_CHECK

	if (data_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "An empty string was passed");
		RETURN_FALSE
	}
	...
	if (mcrypt_enc_is_block_mode(pm->td) == 1) { /* It's a block algorithm */
		...
		if (data_size <= 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Integer overflow in data size");
			RETURN_FALSE;
		}
...
JONJ_FUNCTION(mcrypt_generic_deinit)
{
	...
	if (mcrypt_generic_deinit(pm->td) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not terminate encryption specifier");
		RETURN_FALSE
	}
	pm->init = 0;
	RETURN_TRUE
...
JONJ_FUNCTION(mcrypt_list_algorithms)
{
	...
	if (count == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No algorithms found in module dir");
	}
	for (i = 0; i < count; i++) {
...
JONJ_FUNCTION(mcrypt_list_modes)
{
	...
	if (count == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No modes found in module dir");
	}
	for (i = 0; i < count; i++) {
		add_index_string(return_value, i, modules[i], 1);
	}
...
JONJ_FUNCTION(mcrypt_get_key_size)
{
	...
	if (td != MCRYPT_FAILED) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, MCRYPT_OPEN_MODULE_FAILED);
		RETURN_FALSE;
	}
}
/* }}} */
...
static char *jonj_mcrypt_get_key_size_str(
...
static zend_bool jonj_mcrypt_is_valid_key_size(
...
static int jonj_mcrypt_ensure_valid_key_size(MCRYPT td, int key_size TSRMLS_DC) /* {{{ */
{
	int key_size_count;
	int max_key_size = mcrypt_enc_get_key_size(td);
	int *key_sizes = mcrypt_enc_get_supported_key_sizes(td, &key_size_count);

	zend_bool is_valid_key_size = jonj_mcrypt_is_valid_key_size(
		key_size, max_key_size, key_sizes, key_size_count
	);
	if (!is_valid_key_size) {
		char *key_size_str = jonj_mcrypt_get_key_size_str(
...
static int jonj_mcrypt_ensure_valid_iv(MCRYPT td, const char *iv, int iv_size TSRMLS_DC) /* {{{ */
{
...
static void jonj_mcrypt_do_crypt(char* cipher, const char *key, int key_len, const char *data, int data_len, char *mode, const char *iv, int iv_len, int dencrypt, zval* return_value TSRMLS_DC) /* {{{ */
{
	char *cipher_dir_string;
	char *module_dir_string;
	...
	if (jonj_mcrypt_ensure_valid_key_size(td, key_len TSRMLS_CC) == FAILURE) {
		mcrypt_module_close(td);
		RETURN_FALSE;
	}

	if (jonj_mcrypt_ensure_valid_iv(td, iv, iv_len TSRMLS_CC) == FAILURE) {
		mcrypt_module_close(td);
		RETURN_FALSE;
	}
	...
	if (mcrypt_generic_init(td, (void *) key, key_len, (void *) iv) < 0) {
		efree(data_s);
		jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Mcrypt initialisation failed");
		mcrypt_module_close(td);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(mcrypt_encrypt)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, mode, iv, iv_len, MCRYPT_ENCRYPT, return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_decrypt)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, mode, iv, iv_len, MCRYPT_DECRYPT, return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_ecb)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, "ecb", iv, iv_len, Z_LVAL_PP(mode), return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_cbc)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, "cbc", iv, iv_len, Z_LVAL_PP(mode), return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_cfb)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, "cfb", iv, iv_len, Z_LVAL_PP(mode), return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_ofb)
{
	...
	jonj_mcrypt_do_crypt(cipher, key, key_len, data, data_len, "ofb", iv, iv_len, Z_LVAL_PP(mode), return_value TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(mcrypt_create_iv)
{
	...
	if (size <= 0 || size >= INT_MAX) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create an IV with a size of less than 1 or greater than %d", INT_MAX);
		RETURN_FALSE;
	}
	...
	if (source == RANDOM || source == URANDOM) {
		...
		BYTE *iv_b = (BYTE *) iv;
		if (jonj_win32_get_random_bytes(iv_b, (size_t) size) == FAILURE){
			efree(iv);
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not gather sufficient random data");
			RETURN_FALSE;
		}
		n = size;
#else
		...
		if (*fd < 0) {
			*fd = open(source == RANDOM ? "/dev/random" : "/dev/urandom", O_RDONLY);
			if (*fd < 0) {
				efree(iv);
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot open source device");
				RETURN_FALSE;
			}
		}
		...
		n = size;
		while (size) {
			iv[--size] = (char) (255.0 * jonj_rand(TSRMLS_C) / RAND_MAX);
		}
	}
