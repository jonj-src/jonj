...
JONJ_ARG_WITH(pgsql,for PostgreSQL support,
[  --with-pgsql[=DIR]        Include PostgreSQL support.  DIR is the PostgreSQL
                          base install directory or the path to pg_config])

if test "$JONJ_PGSQL" != "no"; then
  JONJ_EXPAND_PATH($PGSQL_INCLUDE, PGSQL_INCLUDE)

  AC_MSG_CHECKING(for pg_config)
  for i in $JONJ_PGSQL $JONJ_PGSQL/bin /usr/local/pgsql/bin /usr/local/bin /usr/bin ""; do
  ...
  if test -n "$PG_CONFIG"; then
    ...
    if test "$JONJ_PGSQL" = "yes"; then
      PGSQL_SEARCH_PATHS="/usr /usr/local /usr/local/pgsql"
    else
      PGSQL_SEARCH_PATHS=$JONJ_PGSQL
    fi
    ...
    for i in $PGSQL_SEARCH_PATHS; do
      ...
      for j in lib $JONJ_LIBDIR/pgsql $JONJ_LIBDIR/postgres $JONJ_LIBDIR/postgresql ""; do
  ...
  JONJ_ADD_LIBRARY_WITH_PATH(pq, $PGSQL_LIBDIR, PGSQL_SHARED_LIBADD)
  JONJ_SUBST(PGSQL_SHARED_LIBADD)

  JONJ_ADD_INCLUDE($PGSQL_INCLUDE)

  JONJ_NEW_EXTENSION(pgsql, pgsql.c, $ext_shared)
fi
