...
/* If you declare any globals in jonj_enchant.h uncomment this:*/
/*ZEND_DECLARE_MODULE_GLOBALS(enchant)*/

#define JONJ_ENCHANT_MYSPELL 1
#define JONJ_ENCHANT_ISPELL 2
...
zend_function_entry enchant_functions[] = {
	JONJ_FE(enchant_broker_init, 			arginfo_enchant_broker_init)
	JONJ_FE(enchant_broker_free, 			arginfo_enchant_broker_free)
	JONJ_FE(enchant_broker_get_error, 		arginfo_enchant_broker_free)
	JONJ_FE(enchant_broker_set_dict_path,	arginfo_enchant_broker_set_dict_path)
	JONJ_FE(enchant_broker_get_dict_path,	arginfo_enchant_broker_get_dict_path)
	JONJ_FE(enchant_broker_list_dicts, 		arginfo_enchant_broker_free)
	JONJ_FE(enchant_broker_request_dict,		arginfo_enchant_broker_request_dict)
	JONJ_FE(enchant_broker_request_pwl_dict, arginfo_enchant_broker_request_pwl_dict)
	JONJ_FE(enchant_broker_free_dict, 		arginfo_enchant_broker_free_dict)
	JONJ_FE(enchant_broker_dict_exists, 		arginfo_enchant_broker_request_dict)
	JONJ_FE(enchant_broker_set_ordering, 	arginfo_enchant_broker_set_ordering)
	JONJ_FE(enchant_broker_describe, 		arginfo_enchant_broker_free)
	JONJ_FE(enchant_dict_check, 				arginfo_enchant_dict_check)
	JONJ_FE(enchant_dict_suggest, 			arginfo_enchant_dict_check)
	JONJ_FE(enchant_dict_add_to_personal, 	arginfo_enchant_dict_check)
	JONJ_FE(enchant_dict_add_to_session, 	arginfo_enchant_dict_check)
	JONJ_FE(enchant_dict_is_in_session, 		arginfo_enchant_dict_check)
	JONJ_FE(enchant_dict_store_replacement, 	arginfo_enchant_dict_store_replacement)
	JONJ_FE(enchant_dict_get_error, 			arginfo_enchant_broker_free_dict)
	JONJ_FE(enchant_dict_describe, 			arginfo_enchant_broker_free_dict)
	JONJ_FE(enchant_dict_quick_check, 		arginfo_enchant_dict_quick_check)
	JONJ_FE_END
};
/* }}} */
...
zend_module_entry enchant_module_entry = {
	...
	enchant_functions,
	JONJ_MINIT(enchant),
	JONJ_MSHUTDOWN(enchant),
	NULL,	/* Replace with NULL if there's nothing to do at request start */
	NULL,	/* Replace with NULL if there's nothing to do at request end */
	JONJ_MINFO(enchant),
#if ZEND_MODULE_API_NO >= 20010901
	JONJ_ENCHANT_VERSION,
#endif
...
static void jonj_enchant_list_dicts_fn( const char * const lang_tag,
...
static void jonj_enchant_broker_free(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
{
	if (rsrc->ptr) {
...
static void jonj_enchant_dict_free(zend_rsrc_list_entry *rsrc TSRMLS_DC) /* {{{ */
...
{
	le_enchant_broker = zend_register_list_destructors_ex(jonj_enchant_broker_free, NULL, "enchant_broker", module_number);
	le_enchant_dict = zend_register_list_destructors_ex(jonj_enchant_dict_free, NULL, "enchant_dict", module_number);
	REGISTER_LONG_CONSTANT("ENCHANT_MYSPELL", JONJ_ENCHANT_MYSPELL, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("ENCHANT_ISPELL", JONJ_ENCHANT_ISPELL, CONST_CS | CONST_PERSISTENT);
	return SUCCESS;
}
/* }}} */
...
static void __enumerate_providers_fn (const char * const name,
                        ...
                        void * ud) /* {{{ */
{
	jonj_info_print_table_row(3, name, desc, file);
}
/* }}} */
...
JONJ_MINFO_FUNCTION(enchant)
{
	...
	jonj_info_print_table_header(2, "enchant support", "enabled");
	jonj_info_print_table_row(2, "Version", JONJ_ENCHANT_VERSION);
#ifdef ENCHANT_VERSION_STRING
	jonj_info_print_table_row(2, "Libenchant Version", ENCHANT_VERSION_STRING);
#elif defined(HAVE_ENCHANT_BROKER_SET_PARAM)
	jonj_info_print_table_row(2, "Libenchant Version", "1.5.0 or later");
#endif
...
JONJ_FUNCTION(enchant_broker_list_dicts)
{
	...
	enchant_broker_list_dicts(pbroker->pbroker, jonj_enchant_list_dicts_fn, (void *)return_value);
}
/* }}} */
...
JONJ_FUNCTION(enchant_broker_request_dict)
{
	...
	if (taglen == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Tag cannot be empty");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(enchant_broker_request_pwl_dict)
{
	...
	if ((PG(safe_mode) && (!jonj_checkuid(pwl, NULL, CHECKUID_CHECK_FILE_AND_DIR))) || jonj_check_open_basedir(pwl TSRMLS_CC)) {
#else
	if (jonj_check_open_basedir(pwl TSRMLS_CC)) {
#endif
		RETURN_FALSE;
	}
...
/* {{{ proto array enchant_broker_describe(resource broker)
	Enumerates the Enchant providers and tells you some rudimentary information about them. The same info is provided through jonjinfo() */
