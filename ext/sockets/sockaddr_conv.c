...
extern int jonj_string_to_if_index(const char *val, unsigned *out TSRMLS_DC);
...
int jonj_set_inet6_addr(struct sockaddr_in6 *sin6, char *string, jonj_socket *jonj_sock TSRMLS_DC) /* {{{ */
{
	...
	if (inet_pton(AF_INET6, string, &tmp)) {
		...
		if (!addrinfo) {
#ifdef JONJ_WIN32
			JONJ_SOCKET_ERROR(jonj_sock, "Host lookup failed", WSAGetLastError());
#else
			JONJ_SOCKET_ERROR(jonj_sock, "Host lookup failed", (-10000 - h_errno));
#endif
			return 0;
		}
		if (addrinfo->ai_family != PF_INET6 || addrinfo->ai_addrlen != sizeof(struct sockaddr_in6)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Host lookup failed: Non AF_INET6 domain returned on AF_INET6 socket");
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Host lookup failed: getaddrinfo() not available on this system");
		return 0;
#endif
	...
	if (scope++) {
		...
		if (IS_LONG == is_numeric_string(scope, strlen(scope), &lval, &dval, 0)) {
			...
			jonj_string_to_if_index(scope, &scope_id TSRMLS_CC);
		}
...
int jonj_set_inet_addr(struct sockaddr_in *sin, char *string, jonj_socket *jonj_sock TSRMLS_DC) /* {{{ */
{
	...
	if (inet_aton(string, &tmp)) {
		...
		if (host_entry->h_addrtype != AF_INET) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Host lookup failed: Non AF_INET domain returned on AF_INET socket");
			return 0;
		}
...
{
	if (jonj_sock->type == AF_INET) {
		struct sockaddr_in t = {0};
		if (jonj_set_inet_addr(&t, string, jonj_sock TSRMLS_CC)) {
...
#if HAVE_IPV6
	else if (jonj_sock->type == AF_INET6) {
		struct sockaddr_in6 t = {0};
		if (jonj_set_inet6_addr(&t, string, jonj_sock TSRMLS_CC)) {
			memcpy(ss, &t, sizeof t);
