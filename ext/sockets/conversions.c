...
{
	if (err->msg != NULL) {
		jonj_error_docref0(NULL TSRMLS_CC, err->level, "%s", err->msg);
		if (err->should_free) {
			efree(err->msg);
		}
	}
...
double_case:
	...
	default:
		do_from_zval_err(ctx, "%s", "expected an integer, either of a JONJ "
...
void from_zval_write_int(const zval *arr_value, char *field, ser_context *ctx)
{
	...
	if (lval > INT_MAX || lval < INT_MIN) {
		do_from_zval_err(ctx, "%s", "given JONJ integer is out of bounds "
				"for a native int");
		return;
	}
...
static void from_zval_write_sin_addr(const zval *zaddr_str, char *inaddr, ser_context *ctx)
{
	...
	res = jonj_set_inet_addr(&saddr, Z_STRVAL_P(zaddr_str), ctx->sock TSRMLS_CC);
	if (res) {
...
static void from_zval_write_sin6_addr(const zval *zaddr_str, char *addr6, ser_context *ctx)
{
	...
	res = jonj_set_inet6_addr(&saddr6,
...
static void to_zval_read_control_array(const char *msghdr_c, zval *zv, res_context *ctx)
{
	...
	/*if (msg->msg_flags & MSG_CTRUNC) {
		jonj_error_docref0(NULL, E_WARNING, "The MSG_CTRUNC flag is present; will not "
...
static void from_zval_write_fd_array_aux(zval **elem, unsigned i, void **args, ser_context *ctx)
{
	...
	if (Z_TYPE_PP(elem) == IS_RESOURCE) {
		...
		ZEND_FETCH_RESOURCE_NO_RETURN(sock, jonj_socket *, elem, -1,
				NULL, jonj_sockets_le_socket());
		if (sock) {
			iarr[i] = sock->bsd_socket;
			return;
		}

		ZEND_FETCH_RESOURCE2_NO_RETURN(stream, jonj_stream *, elem, -1,
				NULL, jonj_file_le_stream(), jonj_file_le_pstream());
		if (stream == NULL) {
			do_from_zval_err(ctx, "resource is not a stream or a socket");
			return;
		}

		if (jonj_stream_cast(stream, JONJ_STREAM_AS_FD, (void **)&iarr[i - 1],
...
void to_zval_read_fd_array(const char *data, zval *zv, res_context *ctx)
{
	...
	for (i = 0; i < num_elems; i++) {
		...
		if (S_ISSOCK(statbuf.st_mode)) {
			jonj_socket *sock = socket_import_file_descriptor(fd TSRMLS_CC);
			zend_register_resource(elem, sock, jonj_sockets_le_socket() TSRMLS_CC);
		} else {
			jonj_stream *stream = jonj_stream_fopen_from_fd(fd, "rw", NULL);
			jonj_stream_to_zval(stream, elem);
		}
