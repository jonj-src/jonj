...
static int _jonj_mcast_join_leave(jonj_socket *sock, int level, struct sockaddr *group, socklen_t group_len, unsigned int if_index, int join TSRMLS_DC);
#ifdef HAS_MCAST_EXT
static int _jonj_mcast_source_op(jonj_socket *sock, int level, struct sockaddr *group, socklen_t group_len, struct sockaddr *source, socklen_t source_len, unsigned int if_index, enum source_op sop TSRMLS_DC);
#endif

#ifdef RFC3678_API
static int _jonj_source_op_to_rfc3678_op(enum source_op sop);
#elif HAS_MCAST_EXT
static const char *_jonj_source_op_to_string(enum source_op sop);
static int _jonj_source_op_to_ipv4_op(enum source_op sop);
#endif
...
static int jonj_get_if_index_from_zval(zval *val, unsigned *out TSRMLS_DC)
{
	int ret;
	...
	if (Z_TYPE_P(val) == IS_LONG) {
		...
		ret = jonj_string_to_if_index(Z_STRVAL_P(val), out TSRMLS_CC);
		zval_ptr_dtor(&val);
	}
...
static int jonj_get_if_index_from_array(const HashTable *ht, const char *key,
	jonj_socket *sock, unsigned int *if_index TSRMLS_DC)
{
	zval **val;
	...
	return jonj_get_if_index_from_zval(*val, if_index TSRMLS_CC);
}

static int jonj_get_address_from_array(const HashTable *ht, const char *key,
	jonj_socket *sock, jonj_sockaddr_storage *ss, socklen_t *ss_len TSRMLS_DC)
{
	zval **val,
		 *valcp;

	if (zend_hash_find(ht, key, strlen(key) + 1, (void **)&val) == FAILURE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no key \"%s\" passed in optval", key);
		return FAILURE;
	}
	valcp = *val;
	zval_add_ref(&valcp);
	convert_to_string_ex(val);
	if (!jonj_set_inet46_addr(ss, ss_len, Z_STRVAL_P(valcp), sock TSRMLS_CC)) {
		zval_ptr_dtor(&valcp);
		return FAILURE;
	}
	zval_ptr_dtor(&valcp);
...
static int jonj_do_mcast_opt(jonj_socket *jonj_sock, int level, int optname, zval **arg4 TSRMLS_DC)
{
	HashTable		 		*opt_ht;
	unsigned int			if_index;
	int						retval;
	int (*mcast_req_fun)(jonj_socket *, int, struct sockaddr *, socklen_t,
		unsigned TSRMLS_DC);
#ifdef HAS_MCAST_EXT
	int (*mcast_sreq_fun)(jonj_socket *, int, struct sockaddr *, socklen_t,
		struct sockaddr *, socklen_t, unsigned TSRMLS_DC);
#endif

	switch (optname) {
	case JONJ_MCAST_JOIN_GROUP:
		mcast_req_fun = &jonj_mcast_join;
		goto mcast_req_fun;
	case JONJ_MCAST_LEAVE_GROUP:
		{
			jonj_sockaddr_storage	group = {0};
			socklen_t				glen;

			mcast_req_fun = &jonj_mcast_leave;
mcast_req_fun:
			convert_to_array_ex(arg4);
			opt_ht = HASH_OF(*arg4);

			if (jonj_get_address_from_array(opt_ht, "group", jonj_sock, &group,
				&glen TSRMLS_CC) == FAILURE) {
					return FAILURE;
			}
			if (jonj_get_if_index_from_array(opt_ht, "interface", jonj_sock,
				&if_index TSRMLS_CC) == FAILURE) {
					return FAILURE;
			}

			retval = mcast_req_fun(jonj_sock, level, (struct sockaddr*)&group,
				glen, if_index TSRMLS_CC);
			break;
		}

#ifdef HAS_MCAST_EXT
	case JONJ_MCAST_BLOCK_SOURCE:
		mcast_sreq_fun = &jonj_mcast_block_source;
		goto mcast_sreq_fun;
	case JONJ_MCAST_UNBLOCK_SOURCE:
		mcast_sreq_fun = &jonj_mcast_unblock_source;
		goto mcast_sreq_fun;
	case JONJ_MCAST_JOIN_SOURCE_GROUP:
		mcast_sreq_fun = &jonj_mcast_join_source;
		goto mcast_sreq_fun;
	case JONJ_MCAST_LEAVE_SOURCE_GROUP:
		{
			jonj_sockaddr_storage	group = {0},
									source = {0};
			socklen_t				glen,
			...
			mcast_sreq_fun = &jonj_mcast_leave_source;
		mcast_sreq_fun:
			convert_to_array_ex(arg4);
			opt_ht = HASH_OF(*arg4);
			...
			if (jonj_get_address_from_array(opt_ht, "source", jonj_sock, &source,
			...
			retval = mcast_sreq_fun(jonj_sock, level, (struct sockaddr*)&group,
	...
	default:
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
			"unexpected option in jonj_do_mcast_opt (level %d, option %d). "
							   ...
							   zval **arg4 TSRMLS_DC)
{
	...
	case JONJ_MCAST_LEAVE_SOURCE_GROUP:
#endif
		if (jonj_do_mcast_opt(jonj_sock, level, optname, arg4 TSRMLS_CC) == FAILURE) {
			return FAILURE;
		} else {
			return SUCCESS;
		}
	...
	case IP_MULTICAST_IF:
		if (jonj_get_if_index_from_zval(*arg4, &if_index TSRMLS_CC) == FAILURE) {
			return FAILURE;
		}

		if (jonj_if_index_to_addr4(if_index, jonj_sock, &if_addr TSRMLS_CC) == FAILURE) {
			return FAILURE;
		}
		opt_ptr = &if_addr;
...
{
	return _jonj_mcast_join_leave(sock, level, group, group_len, if_index, 1 TSRMLS_CC);
}
...
int jonj_mcast_leave(
	...
	unsigned int if_index TSRMLS_DC)
{
	return _jonj_mcast_join_leave(sock, level, group, group_len, if_index, 0 TSRMLS_CC);
}
...
int jonj_mcast_join_source(
	...
	unsigned int if_index TSRMLS_DC)
{
	return _jonj_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, JOIN_SOURCE TSRMLS_CC);
}
...
int jonj_mcast_leave_source(
	...
	unsigned int if_index TSRMLS_DC)
{
	return _jonj_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, LEAVE_SOURCE TSRMLS_CC);
}
...
int jonj_mcast_block_source(
	...
	unsigned int if_index TSRMLS_DC)
{
	return _jonj_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, BLOCK_SOURCE TSRMLS_CC);
}
...
int jonj_mcast_unblock_source(
	...
	unsigned int if_index TSRMLS_DC)
{
	return _jonj_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, UNBLOCK_SOURCE TSRMLS_CC);
}
#endif /* HAS_MCAST_EXT */


static int _jonj_mcast_join_leave(
	jonj_socket *sock,
	int level,
	...
	int join TSRMLS_DC)
{
	...
	if (sock->type == AF_INET) {
		...
		if (if_index != 0) {
			if (jonj_if_index_to_addr4(if_index, sock, &addr TSRMLS_CC) ==
					FAILURE)
...
#ifdef HAS_MCAST_EXT
static int _jonj_mcast_source_op(
	jonj_socket *sock,
	int level,
	struct sockaddr *group,
	socklen_t group_len,
	...
	enum source_op sop TSRMLS_DC)
{
	...
	return setsockopt(sock->bsd_socket, level,
			_jonj_source_op_to_rfc3678_op(sop), (char*)&gsreq, sizeof(gsreq));
#else
	if (sock->type == AF_INET) {
		...
		return setsockopt(sock->bsd_socket, level,
				_jonj_source_op_to_ipv4_op(sop), (char*)&mreqs, sizeof(mreqs));
	}
#if HAVE_IPV6
	else if (sock->type == AF_INET6) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
			"This platform does not support %s for IPv6 sockets",
			_jonj_source_op_to_string(sop));
		return -2;
	}
#endif
	else {
...
#if JONJ_WIN32
int jonj_if_index_to_addr4(unsigned if_index, jonj_socket *jonj_sock, struct in_addr *out_addr TSRMLS_DC)
{
...
int jonj_add4_to_if_index(struct in_addr *addr, jonj_socket *jonj_sock, unsigned *if_index TSRMLS_DC)
{
	MIB_IPADDRTABLE *addr_table;
    ULONG size;
    DWORD retval;
...
#define ifr_ifindex ifr_index
	...
	if (ioctl(jonj_sock->bsd_socket, SIOCGIFNAME, &if_req) == -1) {
...
#error Neither SIOCGIFNAME nor if_indextoname are available
	...
	if (ioctl(jonj_sock->bsd_socket, SIOCGIFADDR, &if_req) == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
			"Failed obtaining address for interface %u: error %d", if_index, errno);
		return FAILURE;
	}
...
int jonj_add4_to_if_index(struct in_addr *addr, jonj_socket *jonj_sock, unsigned *if_index TSRMLS_DC)
{
	...
	for(;;) {
		...
		if (ioctl(jonj_sock->bsd_socket, SIOCGIFCONF, (char*)&if_conf) == -1 &&
	...
	for (p = if_conf.ifc_buf;
		...
		if ((((struct sockaddr*)&cur_req->ifr_addr)->sa_family == AF_INET) &&
			...
			if (ioctl(jonj_sock->bsd_socket, SIOCGIFINDEX, (char*)cur_req)
					== -1) {
