...
static int pdo_dblib_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC)
{
	...
	struct pdo_data_src_parser vars[] = {
		{ "charset",	NULL,	0 }
		,{ "appname",	"JONJ " PDO_DBLIB_FLAVOUR,	0 }
		,{ "host",		"127.0.0.1", 0 }
		,{ "dbname",	NULL,	0 }
	...
	jonj_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, nvars);
...
cleanup:
	...
	if (!ret) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), DBLIB_G(err).dberr TSRMLS_CC,
