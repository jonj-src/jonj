...
static char *pdo_sqlite_last_insert_id(pdo_dbh_t *dbh, const char *name, unsigned int *len TSRMLS_DC)
{
	...
	id = jonj_pdo_int64_to_str(sqlite3_last_insert_rowid(H->db) TSRMLS_CC);
	*len = strlen(id);
	return id;
}
...
static void jonj_sqlite3_func_callback(sqlite3_context *context, int argc,
	sqlite3_value **argv)
{
...
static void jonj_sqlite3_func_step_callback(sqlite3_context *context, int argc,
	sqlite3_value **argv)
{
	struct pdo_sqlite_func *func = (struct pdo_sqlite_func*)sqlite3_user_data(context);
	TSRMLS_FETCH();
...
static void jonj_sqlite3_func_final_callback(sqlite3_context *context)
{
	struct pdo_sqlite_func *func = (struct pdo_sqlite_func*)sqlite3_user_data(context);
	TSRMLS_FETCH();
...
static int jonj_sqlite3_collation_callback(void *context,
...
static JONJ_METHOD(SQLite, sqliteCreateFunction)
{
	struct pdo_sqlite_func *func;
	zval *callback;
	char *func_name;
	...
	if (!zend_is_callable(callback, 0, &cbname TSRMLS_CC)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "function '%s' is not callable", cbname);
		efree(cbname);
		RETURN_FALSE;
	}
	efree(cbname);
	...
	ret = sqlite3_create_function(H->db, func_name, argc, SQLITE_UTF8,
			func, jonj_sqlite3_func_callback, NULL, NULL);
	if (ret == SQLITE_OK) {
...
static JONJ_METHOD(SQLite, sqliteCreateAggregate)
{
	struct pdo_sqlite_func *func;
	...
	ret = sqlite3_create_function(H->db, func_name, argc, SQLITE_UTF8,
			func, NULL, jonj_sqlite3_func_step_callback, jonj_sqlite3_func_final_callback);
	if (ret == SQLITE_OK) {
		func->funcname = estrdup(func_name);
...
static JONJ_METHOD(SQLite, sqliteCreateCollation)
{
	struct pdo_sqlite_collation *collation;
	zval *callback;
	char *collation_name;
	...
	ret = sqlite3_create_collation(H->db, collation_name, SQLITE_UTF8, collation, jonj_sqlite3_collation_callback);
	if (ret == SQLITE_OK) {
...
static const zend_function_entry dbh_methods[] = {
	JONJ_ME(SQLite, sqliteCreateFunction, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(SQLite, sqliteCreateAggregate, NULL, ZEND_ACC_PUBLIC)
	JONJ_ME(SQLite, sqliteCreateCollation, NULL, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
...
static int pdo_sqlite_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC) /* {{{ */
{
	...
	if (!filename) {
		zend_throw_exception_ex(jonj_pdo_get_exception(), 0 TSRMLS_CC,
