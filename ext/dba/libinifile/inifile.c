...
inifile * inifile_alloc(jonj_stream *fp, int readonly, int persistent TSRMLS_DC)
{
	...
	if (!readonly) {
		if (!jonj_stream_truncate_supported(fp)) {
...
static int inifile_read(inifile *dba, line_type *ln TSRMLS_DC) {
	...
	inifile_val_free(&ln->val);
	while ((fline = jonj_stream_gets(dba->fp, NULL, 0)) != NULL) {
		if (fline) {
			if (fline[0] == '[') {
				...
				if (pos) {
					*pos = '\0';
					...
					ln->key.name = estrdup("");
					ln->pos = jonj_stream_tell(dba->fp);
					efree(fline);
					return 1;
				} else {
					efree(fline);
...
val_type inifile_fetch(inifile *dba, const key_type *key, int skip TSRMLS_DC) {
	...
	if (skip == -1 && dba->next.key.group && dba->next.key.name && !inifile_key_cmp(&dba->next.key, key TSRMLS_CC)) {
		...
		jonj_stream_seek(dba->fp, dba->next.pos, SEEK_SET);
	...
	while(inifile_read(dba, &ln TSRMLS_CC)) {
		if (!(res=inifile_key_cmp(&ln.key, key TSRMLS_CC))) {
			if (!skip) {
				...
				inifile_line_free(&dba->next);
				dba->next = ln;
				dba->next.pos = jonj_stream_tell(dba->fp);
				return val;
			}
			skip--;
		} else if (res == 1) {
...
int inifile_nextkey(inifile *dba TSRMLS_DC) {
	...
	/*inifile_line_free(&dba->next); ??? */
	jonj_stream_seek(dba->fp, dba->curr.pos, SEEK_SET);
...
static int inifile_truncate(inifile *dba, size_t size TSRMLS_DC)
{
	...
	if ((res=jonj_stream_truncate_set_size(dba->fp, size)) != 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error in ftruncate: %d", res);
		return FAILURE;
	}
	jonj_stream_seek(dba->fp, size, SEEK_SET);
	return SUCCESS;
}
/* }}} */
...
static int inifile_find_group(inifile *dba, const key_type *key, size_t *pos_grp_start TSRMLS_DC) 
{
	...
	jonj_stream_seek(dba->fp, 0, SEEK_SET);
	inifile_line_free(&dba->curr);
	inifile_line_free(&dba->next);
	...
	if (key->group && strlen(key->group)) {
		...
		while(inifile_read(dba, &ln TSRMLS_CC)) {
			...
			*pos_grp_start = jonj_stream_tell(dba->fp);
		}
		inifile_line_free(&ln);
	} else {
		*pos_grp_start = 0;
...
static int inifile_copy_to(inifile *dba, size_t pos_start, size_t pos_end, inifile **ini_copy TSRMLS_DC)
{
	...
	if ((fp = jonj_stream_temp_create(0, 64 * 1024)) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not create temporary stream");
		*ini_copy = NULL;
		return FAILURE;
	}
	...
	jonj_stream_seek(dba->fp, pos_start, SEEK_SET);
	if (SUCCESS != jonj_stream_copy_to_stream_ex(dba->fp, fp, pos_end - pos_start, NULL)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not copy group [%zu - %zu] to temporary stream", pos_start, pos_end);
		return FAILURE;
	} 
	return SUCCESS;
}
...
static int inifile_filter(inifile *dba, inifile *from, const key_type *key TSRMLS_DC) 
{
	...
	jonj_stream_seek(from->fp, 0, SEEK_SET);
	jonj_stream_seek(dba->fp, 0, SEEK_END);
	while(inifile_read(from, &ln TSRMLS_CC)) {
		switch(inifile_key_cmp(&ln.key, key TSRMLS_CC)) {
		case 0:
			pos_curr = jonj_stream_tell(from->fp);
			if (pos_start != pos_next) {
				jonj_stream_seek(from->fp, pos_start, SEEK_SET);
				if (SUCCESS != jonj_stream_copy_to_stream_ex(from->fp, dba->fp, pos_next - pos_start, NULL)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not copy [%zu - %zu] from temporary stream", pos_next, pos_start);
					ret = FAILURE;
				}
				jonj_stream_seek(from->fp, pos_curr, SEEK_SET);
			}
			pos_next = pos_start = pos_curr;
			break;
		case 1:
			pos_next = jonj_stream_tell(from->fp);
			break;
		case 2:
...
static int inifile_delete_replace_append(inifile *dba, const key_type *key, const val_type *value, int append TSRMLS_DC) 
{
	...
	/* 4 */
	if (ret == SUCCESS) {
		fp_tmp = jonj_stream_temp_create(0, 64 * 1024);
		if (!fp_tmp) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not create temporary stream");
			ret = FAILURE;
		} else {
			jonj_stream_seek(dba->fp, 0, SEEK_END);
			if (pos_grp_next != (size_t)jonj_stream_tell(dba->fp)) {
				jonj_stream_seek(dba->fp, pos_grp_next, SEEK_SET);
				if (SUCCESS != jonj_stream_copy_to_stream_ex(dba->fp, fp_tmp, JONJ_STREAM_COPY_ALL, NULL)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not copy remainder to temporary stream");
					ret = FAILURE;
				}
			}
		}
	...
	if (ret == SUCCESS) {
		if (key->name && strlen(key->name)) {
			...
			if (value) {
				if (pos_grp_start == pos_grp_next && key->group && strlen(key->group)) {
					jonj_stream_printf(dba->fp TSRMLS_CC, "[%s]\n", key->group);
				}
				jonj_stream_printf(dba->fp TSRMLS_CC, "%s=%s\n", key->name, value->value ? value->value : "");
			}
		}
		 ...
		 * however next operation must be done.
		 */
		if (fp_tmp && jonj_stream_tell(fp_tmp)) {
			jonj_stream_seek(fp_tmp, 0, SEEK_SET);
			jonj_stream_seek(dba->fp, 0, SEEK_END);
			if (SUCCESS != jonj_stream_copy_to_stream_ex(fp_tmp, dba->fp, JONJ_STREAM_COPY_ALL, NULL)) {
				jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Could not copy from temporary stream - ini file truncated");
				ret = FAILURE;
			}
		}
	}
	...
	if (ini_tmp) {
		jonj_stream_close(ini_tmp->fp);
		inifile_free(ini_tmp, 0);
	}
	if (fp_tmp) {
		jonj_stream_close(fp_tmp);
	}
	jonj_stream_flush(dba->fp);
	jonj_stream_seek(dba->fp, 0, SEEK_SET);
