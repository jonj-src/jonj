...
static void jonj_dba_db4_errcall_fcn(
...
/* This code suppresses a BDB 4.8+ error message, thus keeping JONJ test compatibility */
	{
	...
	jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "%s%s", errpfx?errpfx:"", msg);
}
...
DBA_OPEN_FUNC(db4)
{
	...
	if ((err=db_create(&dbp, NULL, 0)) == 0) {
	    dbp->set_errcall(dbp, jonj_dba_db4_errcall_fcn);
	    if (
