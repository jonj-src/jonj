...
static int cdb_make_write(struct cdb_make *c, char *buf, uint32 sz TSRMLS_DC) {
	return jonj_stream_write(c->fp, buf, sz) == sz ? 0 : -1;
}
...
/* {{{ cdb_make_start */
int cdb_make_start(struct cdb_make *c, jonj_stream * f TSRMLS_DC)
{
	c->head = 0;
	c->split = 0;
	c->hash = 0;
	...
	if (jonj_stream_seek(f, c->pos, SEEK_SET) == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Fseek failed");
		return -1;
	}
	return jonj_stream_tell(c->fp);
}
/* }}} */
...
int cdb_make_finish(struct cdb_make *c TSRMLS_DC)
{
	...
	if (jonj_stream_flush(c->fp) != 0)
		return -1;
	jonj_stream_rewind(c->fp);
	if (jonj_stream_tell(c->fp) != 0)
		return -1;
	if (cdb_make_write(c, c->final, sizeof(c->final) TSRMLS_CC) != 0)
		return -1;
	return jonj_stream_flush(c->fp);
}
/* }}} */
