...
#if DBA_CDB_BUILTIN
# define jonj_cdb_read(cdb, buf, len, pos) cdb_read(cdb, buf, len, pos TSRMLS_CC)
# define jonj_cdb_findnext(cdb, key, len) cdb_findnext(cdb, key, len TSRMLS_CC)
# define jonj_cdb_find(cdb, key, len) cdb_find(cdb, key, len TSRMLS_CC)
#else
# define jonj_cdb_read(cdb, buf, len, pos) cdb_read(cdb, buf, len, pos)
# define jonj_cdb_findnext(cdb, key, len) cdb_findnext(cdb, key, len)
# define jonj_cdb_find(cdb, key, len) cdb_find(cdb, key, len)
#endif
...
#endif
	if (jonj_cdb_find(&cdb->c, key, keylen) == 1) {
		while(skip--) {
			if (jonj_cdb_findnext(&cdb->c, key, keylen) != 1) {
				return NULL;
			}
		}
		...
		if (jonj_cdb_read(&cdb->c, new_entry, len, cdb_datapos(&cdb->c)) == -1) {
			efree(new_entry);
			return NULL;
		}
		new_entry[len] = 0;
...
/* {{{ cdb_file_read */
#if DBA_CDB_BUILTIN
# define cdb_file_read(fildes, buf, size) jonj_stream_read(fildes, buf, size)
#else
...
/* {{{ cdb_file_lseek 
 jonj_stream_seek does not return actual position */
#if DBA_CDB_BUILTIN
int cdb_file_lseek(jonj_stream *fp, off_t offset, int whence TSRMLS_DC) {
	jonj_stream_seek(fp, offset, whence);
	return jonj_stream_tell(fp);
}
#else
