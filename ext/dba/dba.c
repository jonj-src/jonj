...
#include "jonj_db3.h"
#include "jonj_db4.h"
#include "jonj_flatfile.h"
#include "jonj_inifile.h"
#include "jonj_qdbm.h"
#include "jonj_tcadb.h"
...
const zend_function_entry dba_functions[] = {
	JONJ_FE(dba_open, arginfo_dba_open)
	JONJ_FE(dba_popen, arginfo_dba_popen)
	JONJ_FE(dba_close, arginfo_dba_close)
	JONJ_FE(dba_delete, arginfo_dba_delete)
	JONJ_FE(dba_exists, arginfo_dba_exists)
	JONJ_FE(dba_fetch, arginfo_dba_fetch)
	JONJ_FE(dba_insert, arginfo_dba_insert)
	JONJ_FE(dba_replace, arginfo_dba_replace)
	JONJ_FE(dba_firstkey, arginfo_dba_firstkey)
	JONJ_FE(dba_nextkey, arginfo_dba_nextkey)
	JONJ_FE(dba_optimize, arginfo_dba_optimize)
	JONJ_FE(dba_sync, arginfo_dba_sync)
	JONJ_FE(dba_handlers, arginfo_dba_handlers)
	JONJ_FE(dba_list, arginfo_dba_list)
	JONJ_FE(dba_key_split, arginfo_dba_key_split)
	JONJ_FE_END
};
/* }}} */

JONJ_MINIT_FUNCTION(dba);
JONJ_MSHUTDOWN_FUNCTION(dba);
JONJ_MINFO_FUNCTION(dba);
...
static JONJ_GINIT_FUNCTION(dba);
...
/* {{{ jonj_dba_myke_key */
static size_t jonj_dba_make_key(zval *key, char **key_str, char **key_free TSRMLS_DC)
{
	if (Z_TYPE_P(key) == IS_ARRAY) {
		...
		if (zend_hash_num_elements(Z_ARRVAL_P(key)) != 2) {
			jonj_error_docref(NULL TSRMLS_CC, E_RECOVERABLE_ERROR, "Key does not have exactly two elements: (key, name)");
			return -1;
		}
...
#define DBA_GET2 												\
	...
	if ((key_len = jonj_dba_make_key(key, &key_str, &key_free TSRMLS_CC)) == 0) {\
		RETURN_FALSE;											\
	}
...
#define DBA_WRITE_CHECK \
	if(info->mode != DBA_WRITER && info->mode != DBA_TRUNC && info->mode != DBA_CREAT) { \
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "You cannot perform a modification to a database without proper access"); \
		RETURN_FALSE; \
	}
...
/* {{{ dba_fetch_resource
JONJAPI void dba_fetch_resource(dba_info **pinfo, zval **id TSRMLS_DC)
{
	dba_info *info;
	DBA_ID_FETCH
	*pinfo = info;
...
/* {{{ dba_get_handler
JONJAPI dba_handler *dba_get_handler(const char* handler_name)
{
	dba_handler *hptr;
...
static void dba_close(dba_info *info TSRMLS_DC)
{
	...
	if (info->fp && info->fp!=info->lock.fp) {
		if(info->flags&DBA_PERSISTENT) {
			jonj_stream_pclose(info->fp);
		} else {
			jonj_stream_close(info->fp);
		}
	}
	if (info->lock.fp) {
		if(info->flags&DBA_PERSISTENT) {
			jonj_stream_pclose(info->lock.fp);
		} else {
			jonj_stream_close(info->lock.fp);
		}
	}
	if (info->lock.name) {
...
ZEND_INI_MH(OnUpdateDefaultHandler)
{
	...
	if (!hptr->name) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No such handler: %s", new_value);
		return FAILURE;
	}
...
JONJ_INI_BEGIN()
    STD_JONJ_INI_ENTRY("dba.default_handler", DBA_DEFAULT, JONJ_INI_ALL, OnUpdateDefaultHandler, default_handler,    zend_dba_globals, dba_globals)
JONJ_INI_END()
/* }}} */
...
JONJ_MINFO_FUNCTION(dba)
{
	...
	jonj_info_print_table_start();
 	jonj_info_print_table_row(2, "DBA support", "enabled");
	if (handlers.c) {
		smart_str_0(&handlers);
		jonj_info_print_table_row(2, "Supported handlers", handlers.c);
		smart_str_free(&handlers);
	} else {
		jonj_info_print_table_row(2, "Supported handlers", "none");
	}
...
/* {{{ jonj_dba_update
 */
static void jonj_dba_update(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	int val_len;
	zval *id;
	dba_info *info = NULL;
...
/* {{{ jonj_find_dbm
 */
dba_info *jonj_dba_find(const char* path TSRMLS_DC)
{
...
/* {{{ jonj_dba_open
 */
static void jonj_dba_open(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	zval ***args = (zval ***) NULL;
	int ac = ZEND_NUM_ARGS();
	dba_mode_t modenr;
	...
	if (ac==2) {
		...
		if (!hptr) {
			jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "No default handler selected");
			FREENOW;
			RETURN_FALSE;
		}
	} else {
	...
	if (!hptr->name) {
		jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "No such handler: %s", Z_STRVAL_PP(args[2]));
		FREENOW;
		RETURN_FALSE;
	}
	...
	if (pmode[0] && (pmode[1]=='d' || pmode[1]=='l' || pmode[1]=='-')) { /* force lock on db file or lck file or disable locking */
		...
		case 'l':
			...
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_NOTICE, "Handler %s does locking internally", hptr->name);
			}
			break;
		default:
		case '-':
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Locking cannot be disabled for handler %s", hptr->name);
				FREENOW;
				RETURN_FALSE;
			}
			lock_flag = 0;
	...
	switch (*pmode++) {
		...
		case 'n':
			...
			break;
		default:
			jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Illegal DBA mode");
			FREENOW;
			RETURN_FALSE;
	}
	if (!lock_file_mode) {
	...
	if (*pmode=='t') {
		pmode++;
		if (!lock_flag) {
			jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "You cannot combine modifiers - (no lock) and t (test lock)");
			FREENOW;
			RETURN_FALSE;
		}
		if (!lock_mode) {
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Handler %s uses its own locking which doesn't support mode modifier t (test lock)", hptr->name);
				FREENOW;
				RETURN_FALSE;
			} else {
				jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Handler %s doesn't uses locking for this mode which makes modifier t (test lock) obsolete", hptr->name);
				FREENOW;
				RETURN_FALSE;
			}
		} else {
	...
	if (hptr->flags & DBA_LOCK_ALL) {
		if ((other = jonj_dba_find(info->path TSRMLS_CC)) != NULL) {
	...
	if (!error && lock_mode) {
		if (lock_dbf) {
			...
			if (!strcmp(file_mode, "r")) {
				...
				lock_file_mode = "rb";
				info->lock.fp = jonj_stream_open_wrapper(lock_name, lock_file_mode, STREAM_MUST_SEEK|IGNORE_PATH|persistent_flag, &opened_path);
			}
			if (!info->lock.fp) {
		...
		if (!info->lock.fp) {
			info->lock.fp = jonj_stream_open_wrapper(lock_name, lock_file_mode, STREAM_MUST_SEEK|REPORT_ERRORS|IGNORE_PATH|persistent_flag, &opened_path);
			if (info->lock.fp) {
				if (lock_dbf) {
		...
		if (!jonj_stream_supports_lock(info->lock.fp)) {
			error = "Stream does not support locking";
		}
		if (jonj_stream_lock(info->lock.fp, lock_mode)) {
	...
	if (!error && (hptr->flags&DBA_STREAM_OPEN)==DBA_STREAM_OPEN) {
		if (info->lock.fp && lock_dbf) {
			...
			info->fp = jonj_stream_open_wrapper(info->path, file_mode, STREAM_MUST_SEEK|REPORT_ERRORS|IGNORE_PATH|persistent_flag, NULL);
		}
		if (!info->fp) {
			dba_close(info TSRMLS_CC);
			/* stream operation already wrote an error message */
		...
		if (hptr->flags & (DBA_NO_APPEND|DBA_CAST_AS_FD)) {
			 ...
			 */
			if (SUCCESS != jonj_stream_cast(info->fp, JONJ_STREAM_AS_FD, (void*)&info->fd, 1)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not cast stream");
				dba_close(info TSRMLS_CC);
				FREENOW;
				RETURN_FALSE;
#ifdef F_SETFL
	...
	if (error || hptr->open(info, &error TSRMLS_CC) != SUCCESS) {
		dba_close(info TSRMLS_CC);
		jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Driver initialization failed for handler: %s%s%s", hptr->name, error?": ":"", error?error:"");
		FREENOW;
		RETURN_FALSE;
	}
	...
	if (persistent) {
		...
		if (zend_hash_update(&EG(persistent_list), key, keylen+1, &new_le, sizeof(zend_rsrc_list_entry), NULL) == FAILURE) {
			dba_close(info TSRMLS_CC);
			jonj_error_docref2(NULL TSRMLS_CC, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Could not register persistent resource");
			FREENOW;
			RETURN_FALSE;
		}
	}
...
JONJ_FUNCTION(dba_popen)
{
	jonj_dba_open(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ proto resource dba_open(string path, string mode [, string handlername, string ...])
   Opens path using the specified handler in mode*/
JONJ_FUNCTION(dba_open)
{
	jonj_dba_open(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ proto void dba_close(resource handle)
   Closes database */
JONJ_FUNCTION(dba_close)
{
	zval *id;
	dba_info *info = NULL;
...
JONJ_FUNCTION(dba_fetch)
{
	...
	if (ac==3) {
		if (!strcmp(info->hnd->name, "cdb")) {
			if (skip < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Handler %s accepts only skip values greater than or equal to zero, using skip=0", info->hnd->name);
				skip = 0;
			}
			 ...
			 */
			if (skip < -1) {
				jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Handler %s accepts only skip value -1 and greater, using skip=0", info->hnd->name);
				skip = 0;
			}
		} else {
			jonj_error_docref(NULL TSRMLS_CC, E_NOTICE, "Handler %s does not support optional skip parameter, the value will be ignored", info->hnd->name);
			skip = 0;			
		}
	} else {
		skip = 0; 
...
JONJ_FUNCTION(dba_insert)
{
	jonj_dba_update(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
/* {{{ proto bool dba_replace(string key, string value, resource handle)
   ...
   If inifile: remove all other key lines */
JONJ_FUNCTION(dba_replace)
{
	jonj_dba_update(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ proto bool dba_optimize(resource handle)
   Optimizes (e.g. clean up, vacuum) database */
JONJ_FUNCTION(dba_optimize)
{
	zval *id;
	dba_info *info = NULL;
