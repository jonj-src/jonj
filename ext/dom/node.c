...
const zend_function_entry jonj_dom_node_class_functions[] = { /* {{{ */
	JONJ_FALIAS(insertBefore, dom_node_insert_before, arginfo_dom_node_insert_before)
	JONJ_FALIAS(replaceChild, dom_node_replace_child, arginfo_dom_node_replace_child)
	JONJ_FALIAS(removeChild, dom_node_remove_child, arginfo_dom_node_remove_child)
	JONJ_FALIAS(appendChild, dom_node_append_child, arginfo_dom_node_append_child)
	JONJ_FALIAS(hasChildNodes, dom_node_has_child_nodes, arginfo_dom_node_has_child_nodes)
	JONJ_FALIAS(cloneNode, dom_node_clone_node, arginfo_dom_node_clone_node)
	JONJ_FALIAS(normalize, dom_node_normalize, arginfo_dom_node_normalize)
	JONJ_FALIAS(isSupported, dom_node_is_supported, arginfo_dom_node_is_supported)
	JONJ_FALIAS(hasAttributes, dom_node_has_attributes, arginfo_dom_node_has_attributes)
	JONJ_FALIAS(compareDocumentPosition, dom_node_compare_document_position, arginfo_dom_node_compare_document_position)
	JONJ_FALIAS(isSameNode, dom_node_is_same_node, arginfo_dom_node_is_same_node)
	JONJ_FALIAS(lookupPrefix, dom_node_lookup_prefix, arginfo_dom_node_lookup_prefix)
	JONJ_FALIAS(isDefaultNamespace, dom_node_is_default_namespace, arginfo_dom_node_is_default_namespace)
	JONJ_FALIAS(lookupNamespaceUri, dom_node_lookup_namespace_uri, arginfo_dom_node_lookup_namespace_uri)
	JONJ_FALIAS(isEqualNode, dom_node_is_equal_node, arginfo_dom_node_is_equal_node)
	JONJ_FALIAS(getFeature, dom_node_get_feature, arginfo_dom_node_get_feature)
	JONJ_FALIAS(setUserData, dom_node_set_user_data, arginfo_dom_node_set_user_data)
	JONJ_FALIAS(getUserData, dom_node_get_user_data, arginfo_dom_node_get_user_data)
	JONJ_ME(domnode, getNodePath, arginfo_dom_node_getNodePath, ZEND_ACC_PUBLIC)
	JONJ_ME(domnode, getLineNo, arginfo_dom_node_getLineNo, ZEND_ACC_PUBLIC)
	JONJ_ME(domnode, C14N, arginfo_dom_node_C14N, ZEND_ACC_PUBLIC)
	JONJ_ME(domnode, C14NFile, arginfo_dom_node_C14NFile, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
/* }}} */
...
int dom_node_node_name_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	switch (nodep->type) {
		...
		case XML_TEXT_NODE:
			...
			break;
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid Node Type");
	}
...
int dom_node_parent_node_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object(nodeparent, &ret, *retval, obj TSRMLS_CC))) {
...
int dom_node_child_nodes_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (dom_node_children_valid(nodep) == FAILURE) {
		...
		jonj_dom_create_interator(*retval, DOM_NODELIST TSRMLS_CC);
...
int dom_node_first_child_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object(first, &ret, *retval, obj TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_last_child_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object(last, &ret, *retval, obj TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_previous_sibling_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object(prevsib, &ret, *retval, obj TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_next_sibling_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object(nextsib, &ret, *retval, obj TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_attributes_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (nodep->type == XML_ELEMENT_NODE) {
		jonj_dom_create_interator(*retval, DOM_NAMEDNODEMAP TSRMLS_CC);
		intern = (dom_object *)zend_objects_get_address(*retval TSRMLS_CC);
...
int dom_node_owner_document_read(dom_object *obj, zval **retval TSRMLS_DC)
{
	...
	if (NULL == (*retval = jonj_dom_create_object((xmlNodePtr) docp, &ret, *retval, obj TSRMLS_CC))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
		...
		case XML_ATTRIBUTE_NODE:
			...
			if (nsnode && nodep->ns != NULL && !xmlStrEqual(nodep->ns->prefix, (xmlChar *)prefix)) {
				...
				if (ns == NULL) {
					...
					jonj_dom_throw_error(NAMESPACE_ERR, dom_get_strict_error(obj->document) TSRMLS_CC);
					return FAILURE;
				}
...
static xmlNodePtr _jonj_dom_insert_fragment(xmlNodePtr nodep, xmlNodePtr prevsib, xmlNodePtr nextsib, xmlNodePtr fragment, dom_object *intern, dom_object *childobj TSRMLS_DC) /* {{{ */
{
		...
		while (node != NULL) {
			...
			if (node->doc != nodep->doc) {
				...
				if (node->_private != NULL) {
					...
					jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)childobj, NULL TSRMLS_CC);
				}
			}
...
JONJ_FUNCTION(dom_node_insert_before)
{
	...
	if (dom_node_is_read_only(parentp) == SUCCESS ||
		...
		jonj_dom_throw_error(NO_MODIFICATION_ALLOWED_ERR, stricterror TSRMLS_CC);
		RETURN_FALSE;
	}

	if (dom_hierarchy(parentp, child) == FAILURE) {
		jonj_dom_throw_error(HIERARCHY_REQUEST_ERR, stricterror TSRMLS_CC);
		RETURN_FALSE;
	}

	if (child->doc != parentp->doc && child->doc != NULL) {
		jonj_dom_throw_error(WRONG_DOCUMENT_ERR, stricterror TSRMLS_CC);
		RETURN_FALSE;
	}

	if (child->type == XML_DOCUMENT_FRAG_NODE && child->children == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Document Fragment is empty");
		RETURN_FALSE;
	}
	...
	if (ref != NULL) {
		...
		if (refp->parent != parentp) {
			jonj_dom_throw_error(NOT_FOUND_ERR, stricterror TSRMLS_CC);
			RETURN_FALSE;
		}
		...
		if (child->type == XML_TEXT_NODE && (refp->type == XML_TEXT_NODE || 
			...
			if (lastattr != NULL && lastattr->type != XML_ATTRIBUTE_DECL) {
				if (lastattr != (xmlAttrPtr) child) {
					...
					jonj_libxml_node_free_resource((xmlNodePtr) lastattr TSRMLS_CC);
				} else {
			...
			new_child = _jonj_dom_insert_fragment(parentp, refp->prev, refp, child, intern, childobj TSRMLS_CC);
		}
		...
		if (child->type == XML_TEXT_NODE && parentp->last != NULL && parentp->last->type == XML_TEXT_NODE) {
			...
			new_child = _jonj_dom_insert_fragment(parentp, parentp->last, NULL, child, intern, childobj TSRMLS_CC);
		}
		if (new_child == NULL) {
	...
	if (NULL == new_child) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Couldn't add newnode as the previous sibling of refnode");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(dom_node_replace_child)
{
	...
	if (foundoldchild) {
		if (newchild->type == XML_DOCUMENT_FRAG_NODE) {
			...
			newchild = _jonj_dom_insert_fragment(nodep, prevsib, nextsib, newchild, intern, newchildobj TSRMLS_CC);
			if (newchild) {
			...
			if (newchild->doc == NULL && nodep->doc != NULL) {
				...
				jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)newchildobj, NULL TSRMLS_CC);
			}
		...
		jonj_dom_throw_error(NOT_FOUND_ERR, dom_get_strict_error(intern->document) TSRMLS_CC);
		RETURN_FALSE;
	}
}
...
JONJ_FUNCTION(dom_node_append_child)
{
	...
	if (child->type == XML_TEXT_NODE && nodep->last != NULL && nodep->last->type == XML_TEXT_NODE) {
		...
		new_child = _jonj_dom_insert_fragment(nodep, nodep->last, NULL, child, intern, childobj TSRMLS_CC);
	}

	if (new_child == NULL) {
		new_child = xmlAddChild(nodep, child);
		if (new_child == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Couldn't append node");
			RETURN_FALSE;
		}
	}
...
static void dom_canonicalization(INTERNAL_FUNCTION_PARAMETERS, int mode) /* {{{ */
{
	...
	if (! docp) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Node must be associated with a document");
		RETURN_FALSE;
	}
	...
	if (xpath_array == NULL) {
		if (nodep->type != XML_DOCUMENT_NODE) {
			...
			if (xpathobjp && xpathobjp->type == XPATH_NODESET) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "XPath query did not return a nodeset.");
				RETURN_FALSE;
			}
		}
	} else {
		...
		if (zend_hash_find(ht, "query", sizeof("query"), (void**)&tmp) == SUCCESS &&
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'query' missing from xpath array or is not a string");
			RETURN_FALSE;
		}
