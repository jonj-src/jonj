...
#define JONJ_DOM_XPATH_QUERY 0
#define JONJ_DOM_XPATH_EVALUATE 1
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_dom_xpath_register_jonj_functions, 0, 0, 0)
ZEND_END_ARG_INFO();
/* }}} */

const zend_function_entry jonj_dom_xpath_class_functions[] = {
	JONJ_ME(domxpath, __construct, arginfo_dom_xpath_construct, ZEND_ACC_PUBLIC)
	JONJ_FALIAS(registerNamespace, dom_xpath_register_ns, arginfo_dom_xpath_register_ns)
	JONJ_FALIAS(query, dom_xpath_query, arginfo_dom_xpath_query)
	JONJ_FALIAS(evaluate, dom_xpath_evaluate, arginfo_dom_xpath_evaluate)
	JONJ_FALIAS(registerPhpFunctions, dom_xpath_register_jonj_functions, arginfo_dom_xpath_register_jonj_functions)
	JONJ_FE_END
};


static void dom_xpath_ext_function_jonj(xmlXPathParserContextPtr ctxt, int nargs, int type) /* {{{ */
{
	zval **args = NULL;
	zval *retval;
	int result, i, ret;
	...
	if (! zend_is_executing(TSRMLS_C)) {
		...
		"xmlExtFunctionTest: Function called from outside of JONJ\n");
		error = 1;
	} else {
		...
		if (intern == NULL) {
			...
			xmlGenericError(xmlGenericErrorContext,
			"xmlExtFunctionTest: JONJ Object did not register JONJ functions\n");
			error = 1;
		}
	}
				...
				if (type == 1) {
					...
					if (obj->nodesetval && obj->nodesetval->nodeNr > 0) {
						for (j = 0; j < obj->nodesetval->nodeNr; j++) {
							...
							child = jonj_dom_create_object(node, &ret, child, (dom_object *)intern TSRMLS_CC);
...
static void dom_xpath_ext_function_string_jonj(xmlXPathParserContextPtr ctxt, int nargs) /* {{{ */
{
	dom_xpath_ext_function_jonj(ctxt, nargs, 1);
}
/* }}} */

static void dom_xpath_ext_function_object_jonj(xmlXPathParserContextPtr ctxt, int nargs) /* {{{ */
{
	dom_xpath_ext_function_jonj(ctxt, nargs, 2);
}
/* }}} */
...
JONJ_METHOD(domxpath, __construct)
{
	...
	if (intern != NULL) {
		...
		xmlXPathRegisterFuncNS (ctx, (const xmlChar *) "functionString",
					   (const xmlChar *) "http://jonj.tk/xpath",
					   dom_xpath_ext_function_string_jonj);
		xmlXPathRegisterFuncNS (ctx, (const xmlChar *) "function",
					   (const xmlChar *) "http://jonj.tk/xpath",
					   dom_xpath_ext_function_object_jonj);

		intern->ptr = ctx;
		ctx->userData = (void *)intern;
		intern->document = docobj->document;
		jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)intern, docp TSRMLS_CC);
	}
}
...
JONJ_FUNCTION(dom_xpath_register_ns)
{
	...
	if (ctxp == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid XPath Context");
		RETURN_FALSE;
	}
...
static void jonj_xpath_eval(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	...
	if (docp == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid XPath Document Pointer");
		RETURN_FALSE;
	}
	...
	if (nodep && docp != nodep->doc) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Node From Wrong Document");
		RETURN_FALSE;
	}
...
{
	jonj_xpath_eval(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_DOM_XPATH_QUERY);
}
/* }}} end dom_xpath_query */
...
{
	jonj_xpath_eval(INTERNAL_FUNCTION_PARAM_PASSTHRU, JONJ_DOM_XPATH_EVALUATE);
}
/* }}} end dom_xpath_evaluate */

/* {{{ proto void dom_xpath_register_jonj_functions() */
...
/* }}} end dom_xpath_register_jonj_functions */
