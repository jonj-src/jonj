...
const zend_function_entry jonj_dom_domexception_class_functions[] = {
	JONJ_FE_END
};
...
void jonj_dom_throw_error_with_message(int error_code, char *error_message, int strict_error TSRMLS_DC) /* {{{ */
{
	if (strict_error == 1) {
		...
		jonj_libxml_issue_error(E_WARNING, error_message TSRMLS_CC);
	}
}
/* }}} */

/* {{{ jonj_dom_throw_error */
void jonj_dom_throw_error(int error_code, int strict_error TSRMLS_DC)
{
	...
	jonj_dom_throw_error_with_message(error_code, error_message, strict_error TSRMLS_CC);
}
/* }}} end jonj_dom_throw_error */
