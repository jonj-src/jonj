...
const zend_function_entry jonj_dom_documentfragment_class_functions[] = {
	JONJ_ME(domdocumentfragment, __construct, arginfo_dom_documentfragement_construct, ZEND_ACC_PUBLIC)
	JONJ_ME(domdocumentfragment, appendXML, arginfo_dom_documentfragement_appendXML, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
...
JONJ_METHOD(domdocumentfragment, __construct)
{
	...
	if (intern != NULL) {
		...
		/* jonj_dom_set_object(intern, nodep TSRMLS_CC); */
...
/* jonj_dom_xmlSetTreeDoc is a custom implementation of xmlSetTreeDoc
 needed for hack in appendXML due to libxml bug - no need to share this function */
static void jonj_dom_xmlSetTreeDoc(xmlNodePtr tree, xmlDocPtr doc) /* {{{ */
{
    xmlAttrPtr prop;
	xmlNodePtr cur;
		...
		if(tree->type == XML_ELEMENT_NODE) {
			...
			while (prop != NULL) {
				...
				if (prop->children) {
					cur = prop->children;
					while (cur != NULL) {
						jonj_dom_xmlSetTreeDoc(cur, doc);
						cur = cur->next;
					}
				}
				prop = prop->next;
...
JONJ_METHOD(domdocumentfragment, appendXML) {
	...
	if (data) {
		...
		jonj_dom_xmlSetTreeDoc(lst, nodep->doc);
		/* End stupid hack */
