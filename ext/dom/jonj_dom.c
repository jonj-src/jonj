...
static void dom_copy_doc_props(jonj_libxml_ref_obj *source_doc, jonj_libxml_ref_obj *dest_doc)
{
...
zend_class_entry *dom_get_doc_classmap(jonj_libxml_ref_obj *document, zend_class_entry *basece TSRMLS_DC)
{
	dom_doc_propsptr doc_props;
...
{
	if (obj && obj->ptr != NULL) {
		return ((jonj_libxml_node_ptr *)obj->ptr)->node;
	} else {
		return NULL;
	}
}
...
/* {{{ dom_object *jonj_dom_object_get_data(xmlNodePtr obj) */
JONJ_DOM_EXPORT dom_object *jonj_dom_object_get_data(xmlNodePtr obj)
{
	if (obj && obj->_private != NULL) {
		return (dom_object *) ((jonj_libxml_node_ptr *) obj->_private)->_private;
	} else {
		return NULL;
	}
}
/* }}} end jonj_dom_object_get_data */

/* {{{ dom_read_na */
static int dom_read_na(dom_object *obj, zval **retval TSRMLS_DC)
{
	*retval = NULL;
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Cannot read property");
	return FAILURE;
}
/* }}} */
...
{
	jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Cannot write property");
	return FAILURE;
}
/* }}} */
...
zval *dom_read_property(zval *object, zval *member, int type, const zend_literal *key TSRMLS_DC)
{
	...
	if (obj->prop_handler != NULL) {
		...
		jonj_error(E_WARNING, "Couldn't fetch %s. Node no longer exists", obj->std.ce->name);
	}
	if (ret == SUCCESS) {
...
void *jonj_dom_export_node(zval *object TSRMLS_DC) /* {{{ */
{
	jonj_libxml_node_object *intern;
	xmlNodePtr nodep = NULL;

	intern = (jonj_libxml_node_object *)zend_object_store_get_object(object TSRMLS_CC);
...
JONJ_FUNCTION(dom_import_simplexml)
{
	zval *node;
	xmlNodePtr nodep = NULL;
	jonj_libxml_node_object *nodeobj;
	int ret;
	...
	nodeobj = (jonj_libxml_node_object *)zend_object_store_get_object(node TSRMLS_CC);
...
zend_object_value dom_objects_store_clone_obj(zval *zobject TSRMLS_DC) /* {{{ */
{
	...
	if (obj->clone == NULL) {
		jonj_error(E_ERROR, "Trying to clone an uncloneable object of class %s", Z_OBJCE_P(zobject)->name);
	}
...
static const zend_function_entry dom_functions[] = {
	JONJ_FE(dom_import_simplexml, arginfo_dom_import_simplexml)
	JONJ_FE_END
};
...
JONJ_MINIT_FUNCTION(dom)
{
	...
	INIT_CLASS_ENTRY(ce, "DOMException", jonj_dom_domexception_class_functions);
	...
	REGISTER_DOM_CLASS(ce, "DOMStringList", NULL, jonj_dom_domstringlist_class_functions, dom_domstringlist_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMNameList", NULL, jonj_dom_namelist_class_functions, dom_namelist_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMImplementationList", NULL, jonj_dom_domimplementationlist_class_functions, dom_domimplementationlist_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMImplementationSource", NULL, jonj_dom_domimplementationsource_class_functions, dom_domimplementationsource_class_entry);
	REGISTER_DOM_CLASS(ce, "DOMImplementation", NULL, jonj_dom_domimplementation_class_functions, dom_domimplementation_class_entry);

	REGISTER_DOM_CLASS(ce, "DOMNode", NULL, jonj_dom_node_class_functions, dom_node_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMDocumentFragment", dom_node_class_entry, jonj_dom_documentfragment_class_functions, dom_documentfragment_class_entry);
	zend_hash_add(&classes, ce.name, ce.name_length + 1, &dom_node_prop_handlers, sizeof(dom_node_prop_handlers), NULL);

	REGISTER_DOM_CLASS(ce, "DOMDocument", dom_node_class_entry, jonj_dom_document_class_functions, dom_document_class_entry);
	...
	INIT_CLASS_ENTRY(ce, "DOMNodeList", jonj_dom_nodelist_class_functions);
	ce.create_object = dom_nnodemap_objects_new;
	dom_nodelist_class_entry = zend_register_internal_class_ex(&ce, NULL, NULL TSRMLS_CC);
	dom_nodelist_class_entry->get_iterator = jonj_dom_get_iterator;
	...
	INIT_CLASS_ENTRY(ce, "DOMNamedNodeMap", jonj_dom_namednodemap_class_functions);
	ce.create_object = dom_nnodemap_objects_new;
	dom_namednodemap_class_entry = zend_register_internal_class_ex(&ce, NULL, NULL TSRMLS_CC);
	dom_namednodemap_class_entry->get_iterator = jonj_dom_get_iterator;
	...
	REGISTER_DOM_CLASS(ce, "DOMCharacterData", dom_node_class_entry, jonj_dom_characterdata_class_functions, dom_characterdata_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMAttr", dom_node_class_entry, jonj_dom_attr_class_functions, dom_attr_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMElement", dom_node_class_entry, jonj_dom_element_class_functions, dom_element_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMText", dom_characterdata_class_entry, jonj_dom_text_class_functions, dom_text_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMComment", dom_characterdata_class_entry, jonj_dom_comment_class_functions, dom_comment_class_entry);
	zend_hash_add(&classes, ce.name, ce.name_length + 1, &dom_characterdata_prop_handlers, sizeof(dom_typeinfo_prop_handlers), NULL);

	REGISTER_DOM_CLASS(ce, "DOMTypeinfo", NULL, jonj_dom_typeinfo_class_functions, dom_typeinfo_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMUserDataHandler", NULL, jonj_dom_userdatahandler_class_functions, dom_userdatahandler_class_entry);
	REGISTER_DOM_CLASS(ce, "DOMDomError", NULL, jonj_dom_domerror_class_functions, dom_domerror_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMErrorHandler", NULL, jonj_dom_domerrorhandler_class_functions, dom_domerrorhandler_class_entry);
	REGISTER_DOM_CLASS(ce, "DOMLocator", NULL, jonj_dom_domlocator_class_functions, dom_domlocator_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMConfiguration", NULL, jonj_dom_domconfiguration_class_functions, dom_domconfiguration_class_entry);
	REGISTER_DOM_CLASS(ce, "DOMCdataSection", dom_text_class_entry, jonj_dom_cdatasection_class_functions, dom_cdatasection_class_entry);
	zend_hash_add(&classes, ce.name, ce.name_length + 1, &dom_text_prop_handlers, sizeof(dom_documenttype_prop_handlers), NULL);

	REGISTER_DOM_CLASS(ce, "DOMDocumentType", dom_node_class_entry, jonj_dom_documenttype_class_functions, dom_documenttype_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMNotation", dom_node_class_entry, jonj_dom_notation_class_functions, dom_notation_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMEntity", dom_node_class_entry, jonj_dom_entity_class_functions, dom_entity_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMEntityReference", dom_node_class_entry, jonj_dom_entityreference_class_functions, dom_entityreference_class_entry);
	zend_hash_add(&classes, ce.name, ce.name_length + 1, &dom_node_prop_handlers, sizeof(dom_entity_prop_handlers), NULL);

	REGISTER_DOM_CLASS(ce, "DOMProcessingInstruction", dom_node_class_entry, jonj_dom_processinginstruction_class_functions, dom_processinginstruction_class_entry);
	...
	REGISTER_DOM_CLASS(ce, "DOMStringExtend", NULL, jonj_dom_string_extend_class_functions, dom_string_extend_class_entry);

#if defined(LIBXML_XPATH_ENABLED)
	INIT_CLASS_ENTRY(ce, "DOMXPath", jonj_dom_xpath_class_functions);
	...
	/* DOMException Codes */
	REGISTER_LONG_CONSTANT("DOM_JONJ_ERR",				JONJ_ERR,				CONST_CS | CONST_PERSISTENT);
	...
	jonj_libxml_register_export(dom_node_class_entry, jonj_dom_export_node);
...
JONJ_MINFO_FUNCTION(dom)
{
	...
	jonj_info_print_table_row(2, "DOM/XML", "enabled");
	jonj_info_print_table_row(2, "DOM/XML API Version", DOM_API_VERSION);
	jonj_info_print_table_row(2, "libxml Version", LIBXML_DOTTED_VERSION);
#if defined(LIBXML_HTML_ENABLED)
	jonj_info_print_table_row(2, "HTML Support", "enabled");
#endif
#if defined(LIBXML_XPATH_ENABLED)
	jonj_info_print_table_row(2, "XPath Support", "enabled");
#endif
#if defined(LIBXML_XPTR_ENABLED)
	jonj_info_print_table_row(2, "XPointer Support", "enabled");
#endif
#ifdef LIBXML_SCHEMAS_ENABLED
	jonj_info_print_table_row(2, "Schema Support", "enabled");
	jonj_info_print_table_row(2, "RelaxNG Support", "enabled");
#endif
...
void node_list_unlink(xmlNodePtr node TSRMLS_DC)
{
	...
	while (node != NULL) {
		...
		wrapper = jonj_dom_object_get_data(node);
...
void dom_xpath_objects_free_storage(void *object TSRMLS_DC)
{
	...
	if (intern->ptr != NULL) {
		...
		jonj_libxml_decrement_doc_ref((jonj_libxml_node_object *) intern TSRMLS_CC);
		intern->ptr = NULL;
	}

	if (intern->registered_jonjfunctions) {
...
void dom_objects_free_storage(void *object TSRMLS_DC)
{
	...
	if (intern->ptr != NULL && ((jonj_libxml_node_ptr *)intern->ptr)->node != NULL) {
		if (((xmlNodePtr) ((jonj_libxml_node_ptr *)intern->ptr)->node)->type != XML_DOCUMENT_NODE && ((xmlNodePtr) ((jonj_libxml_node_ptr *)intern->ptr)->node)->type != XML_HTML_DOCUMENT_NODE) {
			jonj_libxml_node_decrement_resource((jonj_libxml_node_object *) intern TSRMLS_CC);
		} else {
			jonj_libxml_decrement_node_ptr((jonj_libxml_node_object *) intern TSRMLS_CC);
			retcount = jonj_libxml_decrement_doc_ref((jonj_libxml_node_object *)intern TSRMLS_CC);
		}
		intern->ptr = NULL;
	}
	...
	if (instanceof_function(intern->std.ce, dom_node_class_entry TSRMLS_CC)) {
		...
		if (node != NULL) {
			...
			if (cloned_node != NULL) {
				...
				jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)clone, cloned_node->doc TSRMLS_CC);
				jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)clone, cloned_node, (void *)clone TSRMLS_CC);
...
JONJ_DOM_EXPORT zval *jonj_dom_create_object(xmlNodePtr obj, int *found, zval *return_value, dom_object *domobj TSRMLS_DC)
{
	...
	*found = 0;
	...
	if ((intern = (dom_object *) jonj_dom_object_get_data((void *) obj))) {
	...
	switch (obj->type) {
		...
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unsupported node type: %d", Z_TYPE_P(obj));
			ZVAL_NULL(wrapper);
			return wrapper;
	}
	...
	if (obj->doc != NULL) {
		...
		jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)intern, obj->doc TSRMLS_CC);
	}

	jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)intern, obj, (void *)intern TSRMLS_CC);
	return (wrapper);
}
/* }}} end jonj_domobject_new */
			...
			case XML_TEXT_NODE:
				...
				while (nextp != NULL) {
					if (nextp->type == XML_TEXT_NODE) {
						...
						xmlFree(strContent);
						xmlUnlinkNode(nextp);
						jonj_libxml_node_free_resource(nextp TSRMLS_CC);
						nextp = newnextp;
					} else {
						break;
					}
