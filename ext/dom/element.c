...
const zend_function_entry jonj_dom_element_class_functions[] = { /* {{{ */
	JONJ_FALIAS(getAttribute, dom_element_get_attribute, arginfo_dom_element_get_attribute)
	JONJ_FALIAS(setAttribute, dom_element_set_attribute, arginfo_dom_element_set_attribute)
	JONJ_FALIAS(removeAttribute, dom_element_remove_attribute, arginfo_dom_element_remove_attribute)
	JONJ_FALIAS(getAttributeNode, dom_element_get_attribute_node, arginfo_dom_element_get_attribute_node)
	JONJ_FALIAS(setAttributeNode, dom_element_set_attribute_node, arginfo_dom_element_set_attribute_node)
	JONJ_FALIAS(removeAttributeNode, dom_element_remove_attribute_node, arginfo_dom_element_remove_attribute_node)
	JONJ_FALIAS(getElementsByTagName, dom_element_get_elements_by_tag_name, arginfo_dom_element_get_elements_by_tag_name)
	JONJ_FALIAS(getAttributeNS, dom_element_get_attribute_ns, arginfo_dom_element_get_attribute_ns)
	JONJ_FALIAS(setAttributeNS, dom_element_set_attribute_ns, arginfo_dom_element_set_attribute_ns)
	JONJ_FALIAS(removeAttributeNS, dom_element_remove_attribute_ns, arginfo_dom_element_remove_attribute_ns)
	JONJ_FALIAS(getAttributeNodeNS, dom_element_get_attribute_node_ns, arginfo_dom_element_get_attribute_node_ns)
	JONJ_FALIAS(setAttributeNodeNS, dom_element_set_attribute_node_ns, arginfo_dom_element_set_attribute_node_ns)
	JONJ_FALIAS(getElementsByTagNameNS, dom_element_get_elements_by_tag_name_ns, arginfo_dom_element_get_elements_by_tag_name_ns)
	JONJ_FALIAS(hasAttribute, dom_element_has_attribute, arginfo_dom_element_has_attribute)
	JONJ_FALIAS(hasAttributeNS, dom_element_has_attribute_ns, arginfo_dom_element_has_attribute_ns)
	JONJ_FALIAS(setIdAttribute, dom_element_set_id_attribute, arginfo_dom_element_set_id_attribute)
	JONJ_FALIAS(setIdAttributeNS, dom_element_set_id_attribute_ns, arginfo_dom_element_set_id_attribute_ns)
	JONJ_FALIAS(setIdAttributeNode, dom_element_set_id_attribute_node, arginfo_dom_element_set_id_attribute_node)
	JONJ_ME(domelement, __construct, arginfo_dom_element_construct, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
/* }}} */
...
JONJ_METHOD(domelement, __construct)
{
	...
	if (name_valid != 0) {
		jonj_dom_throw_error(INVALID_CHARACTER_ERR, 1 TSRMLS_CC);
		RETURN_FALSE;
	}
	...
	if (uri_len > 0) {
		...
		if (errorcode != 0) {
			...
			jonj_dom_throw_error(errorcode, 1 TSRMLS_CC);
			RETURN_FALSE;
		}
	} else {
	    ...
	    if (prefix != NULL) {
			xmlFree(localname);
			xmlFree(prefix);
	        jonj_dom_throw_error(NAMESPACE_ERR, 1 TSRMLS_CC);
	        RETURN_FALSE;
	    }
	...
	if (intern != NULL) {
		...
		if (oldnode != NULL) {
			jonj_libxml_node_free_resource(oldnode  TSRMLS_CC);
		}
		jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)intern, nodep, (void *)intern TSRMLS_CC);
	}
}
...
JONJ_FUNCTION(dom_element_set_attribute)
{
	...
	if (dom_node_is_read_only(nodep) == SUCCESS) {
		jonj_dom_throw_error(NO_MODIFICATION_ALLOWED_ERR, dom_get_strict_error(intern->document) TSRMLS_CC);
		RETURN_FALSE;
	}
	...
	if (!attr) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No such attribute '%s'", name);
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(dom_element_remove_attribute)
{
	...
	switch (attrp->type) {
		case XML_ATTRIBUTE_NODE:
			if (jonj_dom_object_get_data(attrp) == NULL) {
...
JONJ_FUNCTION(dom_element_set_attribute_node)
{
	...
	if (attrp->type != XML_ATTRIBUTE_NODE) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Attribute node is required");
		RETURN_FALSE;
	}
	...
	if (existattrp != NULL && existattrp->type != XML_ATTRIBUTE_DECL) {
		if ((oldobj = jonj_dom_object_get_data((xmlNodePtr) existattrp)) != NULL && 
			((jonj_libxml_node_ptr *)oldobj->ptr)->node == (xmlNodePtr) attrp)
		{
			RETURN_NULL();
		}
	...
	if (attrp->doc == NULL && nodep->doc != NULL) {
		...
		jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)attrobj, NULL TSRMLS_CC);
	}
...
JONJ_FUNCTION(dom_element_set_attribute_ns)
{
	...
	if (errorcode != 0) {
		jonj_dom_throw_error(errorcode, stricterror TSRMLS_CC);
	}
...
JONJ_FUNCTION(dom_element_remove_attribute_ns)
{
	...
	if (attrp && attrp->type != XML_ATTRIBUTE_DECL) {
		if (jonj_dom_object_get_data((xmlNodePtr) attrp) == NULL) {
			node_list_unlink(attrp->children TSRMLS_CC);
			xmlUnlinkNode((xmlNodePtr) attrp);
			xmlFreeProp(attrp);
		} else {
...
static void jonj_set_attribute_id(xmlAttrPtr attrp, zend_bool is_id) /* {{{ */
{
...
JONJ_FUNCTION(dom_element_set_id_attribute)
{
	...
	if (attrp == NULL || attrp->type == XML_ATTRIBUTE_DECL) {
		...
		jonj_set_attribute_id(attrp, is_id);
	}
