...
const zend_function_entry jonj_dom_domimplementation_class_functions[] = {
	JONJ_ME(domimplementation, getFeature, arginfo_dom_implementation_get_feature, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_ME(domimplementation, hasFeature, arginfo_dom_implementation_has_feature, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_ME(domimplementation, createDocumentType, arginfo_dom_implementation_create_documenttype, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_ME(domimplementation, createDocument, arginfo_dom_implementation_create_document, ZEND_ACC_PUBLIC|ZEND_ACC_ALLOW_STATIC)
	JONJ_FE_END
};
...
JONJ_METHOD(domimplementation, createDocumentType)
{
	...
	if (name_len == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "qualifiedName is required");
		RETURN_FALSE;
	}
	...
	jonj_dom_throw_error(INVALID_CHARACTER_ERR, TSRMLS_CC);
	*/
	...
	if (doctype == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to create DocumentType");
		RETURN_FALSE;
	}
...
JONJ_METHOD(domimplementation, createDocument)
{
	...
	if (node != NULL) {
		...
		if (doctype->type == XML_DOCUMENT_TYPE_NODE) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid DocumentType object");
			RETURN_FALSE;
		}
		if (doctype->doc != NULL) {
			jonj_dom_throw_error(WRONG_DOCUMENT_ERR, 1 TSRMLS_CC);
			RETURN_FALSE;
		}
	} else {
		doctobj = NULL;
	...
	if (localname != NULL) {
		...
		if (!nodep) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unexpected Error");
			RETURN_FALSE;
		}
	...
	if (doctobj != NULL) {
		doctobj->document = ((dom_object *)((jonj_libxml_node_ptr *)docp->_private)->_private)->document;
		jonj_libxml_increment_doc_ref((jonj_libxml_node_object *)doctobj, docp TSRMLS_CC);
	}
}
