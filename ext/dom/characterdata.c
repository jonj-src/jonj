...
const zend_function_entry jonj_dom_characterdata_class_functions[] = {
	JONJ_FALIAS(substringData, dom_characterdata_substring_data, arginfo_dom_characterdata_substring_data)
	JONJ_FALIAS(appendData, dom_characterdata_append_data, arginfo_dom_characterdata_append_data)
	JONJ_FALIAS(insertData, dom_characterdata_insert_data, arginfo_dom_characterdata_insert_data)
	JONJ_FALIAS(deleteData, dom_characterdata_delete_data, arginfo_dom_characterdata_delete_data)
	JONJ_FALIAS(replaceData, dom_characterdata_replace_data, arginfo_dom_characterdata_replace_data)
	JONJ_FE_END
};
...
JONJ_FUNCTION(dom_characterdata_substring_data)
{
	...
	if (offset < 0 || count < 0 || offset > length) {
		xmlFree(cur);
		jonj_dom_throw_error(INDEX_SIZE_ERR, dom_get_strict_error(intern->document) TSRMLS_CC);
		RETURN_FALSE;
	}
