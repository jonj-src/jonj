...
const zend_function_entry jonj_dom_namednodemap_class_functions[] = { /* {{{ */
	JONJ_FALIAS(getNamedItem, dom_namednodemap_get_named_item, arginfo_dom_namednodemap_get_named_item)
	JONJ_FALIAS(setNamedItem, dom_namednodemap_set_named_item, arginfo_dom_namednodemap_set_named_item)
	JONJ_FALIAS(removeNamedItem, dom_namednodemap_remove_named_item, arginfo_dom_namednodemap_remove_named_item)
	JONJ_FALIAS(item, dom_namednodemap_item, arginfo_dom_namednodemap_item)
	JONJ_FALIAS(getNamedItemNS, dom_namednodemap_get_named_item_ns, arginfo_dom_namednodemap_get_named_item_ns)
	JONJ_FALIAS(setNamedItemNS, dom_namednodemap_set_named_item_ns, arginfo_dom_namednodemap_set_named_item_ns)
	JONJ_FALIAS(removeNamedItemNS, dom_namednodemap_remove_named_item_ns, arginfo_dom_namednodemap_remove_named_item_ns)
	JONJ_FE_END
};
/* }}} */
	...
	if (index >= 0) {
		...
		if (objmap != NULL) {
			if ((objmap->nodetype == XML_NOTATION_NODE) || 
				...
				if (objmap->ht) {
					if (objmap->nodetype == XML_ENTITY_NODE) {
						itemnode = jonj_dom_libxml_hash_iter(objmap->ht, index);
					} else {
						itemnode = jonj_dom_libxml_notation_iter(objmap->ht, index);
					}
				}
			} else {
				nodep = dom_object_get_node(objmap->baseobj);
