...
const zend_function_entry jonj_dom_attr_class_functions[] = {
	JONJ_FALIAS(isId, dom_attr_is_id, arginfo_dom_attr_is_id)
	JONJ_ME(domattr, __construct, arginfo_dom_attr_construct, ZEND_ACC_PUBLIC)
	JONJ_FE_END
};
...
JONJ_METHOD(domattr, __construct)
{
	...
	if (intern != NULL) {
		...
		jonj_libxml_increment_node_ptr((jonj_libxml_node_object *)intern, (xmlNodePtr)nodep, (void *)intern TSRMLS_CC);
	}
}
