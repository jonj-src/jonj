...
typedef struct filter_list_entry {
	const char *name;
	int    id;
	void (*function)(JONJ_INPUT_FILTER_PARAM_DECL);
} filter_list_entry;

/* {{{ filter_list */
static const filter_list_entry filter_list[] = {
	{ "int",             FILTER_VALIDATE_INT,           jonj_filter_int             },
	{ "boolean",         FILTER_VALIDATE_BOOLEAN,       jonj_filter_boolean         },
	{ "float",           FILTER_VALIDATE_FLOAT,         jonj_filter_float           },

	{ "validate_regexp", FILTER_VALIDATE_REGEXP,        jonj_filter_validate_regexp },
	{ "validate_url",    FILTER_VALIDATE_URL,           jonj_filter_validate_url    },
	{ "validate_email",  FILTER_VALIDATE_EMAIL,         jonj_filter_validate_email  },
	{ "validate_ip",     FILTER_VALIDATE_IP,            jonj_filter_validate_ip     },
	{ "validate_mac",    FILTER_VALIDATE_MAC,           jonj_filter_validate_mac    },

	{ "string",          FILTER_SANITIZE_STRING,        jonj_filter_string          },
	{ "stripped",        FILTER_SANITIZE_STRING,        jonj_filter_string          },
	{ "encoded",         FILTER_SANITIZE_ENCODED,       jonj_filter_encoded         },
	{ "special_chars",   FILTER_SANITIZE_SPECIAL_CHARS, jonj_filter_special_chars   },
	{ "full_special_chars",   FILTER_SANITIZE_FULL_SPECIAL_CHARS, jonj_filter_full_special_chars   },
	{ "unsafe_raw",      FILTER_UNSAFE_RAW,             jonj_filter_unsafe_raw      },
	{ "email",           FILTER_SANITIZE_EMAIL,         jonj_filter_email           },
	{ "url",             FILTER_SANITIZE_URL,           jonj_filter_url             },
	{ "number_int",      FILTER_SANITIZE_NUMBER_INT,    jonj_filter_number_int      },
	{ "number_float",    FILTER_SANITIZE_NUMBER_FLOAT,  jonj_filter_number_float    },
	{ "magic_quotes",    FILTER_SANITIZE_MAGIC_QUOTES,  jonj_filter_magic_quotes    },

	{ "callback",        FILTER_CALLBACK,               jonj_filter_callback        },
};
/* }}} */
...
static unsigned int jonj_sapi_filter(int arg, char *var, char **val, unsigned int val_len, unsigned int *new_val_len TSRMLS_DC);
static unsigned int jonj_sapi_filter_init(TSRMLS_D);
...
static const zend_function_entry filter_functions[] = {
	JONJ_FE(filter_input,		arginfo_filter_input)
	JONJ_FE(filter_var,		arginfo_filter_var)
	JONJ_FE(filter_input_array,	arginfo_filter_input_array)
	JONJ_FE(filter_var_array,	arginfo_filter_var_array)
	JONJ_FE(filter_list,		arginfo_filter_list)
	JONJ_FE(filter_has_var,		arginfo_filter_has_var)
	JONJ_FE(filter_id,		arginfo_filter_id)
	JONJ_FE_END
};
/* }}} */
...
zend_module_entry filter_module_entry = {
	...
	filter_functions,
	JONJ_MINIT(filter),
	JONJ_MSHUTDOWN(filter),
	NULL,
	JONJ_RSHUTDOWN(filter),
	JONJ_MINFO(filter),
	"0.11.0",
...
static JONJ_INI_MH(UpdateDefaultFilter) /* {{{ */
{
...
/* {{{ JONJ_INI
 */
static JONJ_INI_MH(OnUpdateFlags)
{
	if (!new_value) {
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("filter.default",   "unsafe_raw", JONJ_INI_SYSTEM|JONJ_INI_PERDIR, UpdateDefaultFilter, default_filter, zend_filter_globals, filter_globals)
	JONJ_INI_ENTRY("filter.default_flags", NULL,     JONJ_INI_SYSTEM|JONJ_INI_PERDIR, OnUpdateFlags)
JONJ_INI_END()
/* }}} */

static void jonj_filter_init_globals(zend_filter_globals *filter_globals) /* {{{ */
{
...
{
	ZEND_INIT_MODULE_GLOBALS(filter, jonj_filter_init_globals, NULL);
	...
	sapi_register_input_filter(jonj_sapi_filter, jonj_sapi_filter_init TSRMLS_CC);
...
JONJ_MINFO_FUNCTION(filter)
{
	...
	jonj_info_print_table_row( 2, "Input Validation and Filtering", "enabled" );
...
static filter_list_entry jonj_find_filter(long id) /* {{{ */
{
	int i, size = sizeof(filter_list) / sizeof(filter_list_entry);
...
static void jonj_zval_filter(zval **value, long filter, long flags, zval *options, char* charset, zend_bool copy TSRMLS_DC) /* {{{ */
{
	filter_list_entry  filter_func;

	filter_func = jonj_find_filter(filter);

	if (!filter_func.id) {
		/* Find default filter */
		filter_func = jonj_find_filter(FILTER_DEFAULT);
	}
...
#define PARSE_CASE(s,a,t)                    \
	...
	*/
	...
	if (array_ptr) {
		...
		jonj_register_variable_ex(var, &raw_var, array_ptr TSRMLS_CC);
	}
	...
	if (val_len) {
		...
		if (IF_G(default_filter) != FILTER_UNSAFE_RAW) {
			...
			jonj_zval_filter(&tmp_new_var, IF_G(default_filter), IF_G(default_filter_flags), NULL, NULL/*charset*/, 0 TSRMLS_CC);
		} else {
			Z_STRVAL(new_var) = estrndup(*val, val_len);
		}
	} else { /* empty string */
	...
	if (orig_array_ptr) {
		jonj_register_variable_ex(var, &new_var, orig_array_ptr TSRMLS_CC);
	}
...
static void jonj_zval_filter_recursive(zval **value, long filter, long flags, zval *options, char *charset, zend_bool copy TSRMLS_DC) /* {{{ */
{
	if (Z_TYPE_PP(value) == IS_ARRAY) {
		...
		for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_PP(value), &pos);
			...
			if (Z_TYPE_PP(element) == IS_ARRAY) {
				...
				jonj_zval_filter_recursive(element, filter, flags, options, charset, copy TSRMLS_CC);
				Z_ARRVAL_PP(element)->nApplyCount--;
			} else {
				jonj_zval_filter(element, filter, flags, options, charset, copy TSRMLS_CC);
			}
		}
	} else {
		jonj_zval_filter(value, filter, flags, options, charset, copy TSRMLS_CC);
	}
}
/* }}} */

static zval *jonj_filter_get_storage(long arg TSRMLS_DC)/* {{{ */

{
	...
	switch (arg) {
		...
		case PARSE_SESSION:
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "INPUT_SESSION is not yet implemented");
			break;
		case PARSE_REQUEST:
			/* FIXME: Implement request source */
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "INPUT_REQUEST is not yet implemented");
			break;
	}
...
JONJ_FUNCTION(filter_has_var)
{
	...
	array_ptr = jonj_filter_get_storage(arg TSRMLS_CC);
...
static void jonj_filter_call(zval **filtered, long filter, zval **filter_args, const int copy, long filter_flags TSRMLS_DC) /* {{{ */
{
	zval  *options = NULL;
	zval **option;
	char  *charset = NULL;
	...
	if (filter_args && Z_TYPE_PP(filter_args) != IS_ARRAY) {
		...
		JONJ_FILTER_GET_LONG_OPT(filter_args, lval);
		...
		if (zend_hash_find(HASH_OF(*filter_args), "filter", sizeof("filter"), (void **)&option) == SUCCESS) {
			JONJ_FILTER_GET_LONG_OPT(option, filter);
		}

		if (zend_hash_find(HASH_OF(*filter_args), "flags", sizeof("flags"), (void **)&option) == SUCCESS) {
			JONJ_FILTER_GET_LONG_OPT(option, filter_flags);
	...
	if (Z_TYPE_PP(filtered) == IS_ARRAY) {
		...
		jonj_zval_filter_recursive(filtered, filter, filter_flags, options, charset, copy TSRMLS_CC);
		return;
	}
	...
	jonj_zval_filter(filtered, filter, filter_flags, options, charset, copy TSRMLS_CC);
...
static void jonj_filter_array_handler(zval *input, zval **op, zval *return_value, zend_bool add_empty TSRMLS_DC) /* {{{ */
{
	char *arg_key;
	uint arg_key_len;
	ulong index;
	...
	if (!op) {
		...
		jonj_filter_call(&return_value, FILTER_DEFAULT, NULL, 0, FILTER_REQUIRE_ARRAY TSRMLS_CC);
	} else if (Z_TYPE_PP(op) == IS_LONG) {
		zval_dtor(return_value);
		MAKE_COPY_ZVAL(&input, return_value);
		jonj_filter_call(&return_value, Z_LVAL_PP(op), NULL, 0, FILTER_REQUIRE_ARRAY TSRMLS_CC);
			...
			zend_hash_move_forward_ex(Z_ARRVAL_PP(op), &pos))
		{
			if (zend_hash_get_current_key_ex(Z_ARRVAL_PP(op), &arg_key, &arg_key_len, &index, 0, &pos) != HASH_KEY_IS_STRING) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Numeric keys are not allowed in the definition array");
				zval_dtor(return_value);
				RETURN_FALSE;
	 		}
			if (arg_key_len < 2) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty keys are not allowed in the definition array");
				zval_dtor(return_value);
				RETURN_FALSE;
			}
			if (zend_hash_find(Z_ARRVAL_P(input), arg_key, arg_key_len, (void **)&tmp) != SUCCESS) {
				...
				jonj_filter_call(&nval, -1, arg_elm, 0, FILTER_REQUIRE_SCALAR TSRMLS_CC);
...
JONJ_FUNCTION(filter_input)
{
	...
	if (!JONJ_FILTER_ID_EXISTS(filter)) {
		RETURN_FALSE;
	}

	input = jonj_filter_get_storage(fetch_from TSRMLS_CC);
	...
	jonj_filter_call(&return_value, filter, filter_args, 1, FILTER_REQUIRE_SCALAR TSRMLS_CC);
}
/* }}} */
...
JONJ_FUNCTION(filter_input_array)
{
	...
	if (op
		...
		&& (Z_TYPE_PP(op) == IS_LONG && !JONJ_FILTER_ID_EXISTS(Z_LVAL_PP(op)))
		) {
		RETURN_FALSE;
	}

	array_input = jonj_filter_get_storage(fetch_from TSRMLS_CC);
	...
	jonj_filter_array_handler(array_input, op, return_value, add_empty TSRMLS_CC);
}
/* }}} */
