...
/* {{{ HELPER FUNCTIONS */
static void jonj_filter_encode_html(zval *value, const unsigned char *chars)
{
	smart_str str = {0};
...
static void jonj_filter_encode_url(zval *value, const unsigned char* chars, const int char_len, int high, int low, int encode_nul)
{
	unsigned char *str, *p;
...
static void jonj_filter_strip(zval *value, long flags)
{
...
void jonj_filter_string(JONJ_INPUT_FILTER_PARAM_DECL)
{
	...
	jonj_filter_encode_html(value, enc);

	/* strip tags, implicitly also removes \0 chars */
	new_len = jonj_strip_tags_ex(Z_STRVAL_P(value), Z_STRLEN_P(value), NULL, NULL, 0, 1);
...
void jonj_filter_encoded(JONJ_INPUT_FILTER_PARAM_DECL)
{
	...
	jonj_filter_strip(value, flags);
	/* urlencode */
	jonj_filter_encode_url(value, (unsigned char *)DEFAULT_URL_ENCODE, sizeof(DEFAULT_URL_ENCODE)-1, flags & FILTER_FLAG_ENCODE_HIGH, flags & FILTER_FLAG_ENCODE_LOW, 1);
}
/* }}} */

/* {{{ jonj_filter_special_chars */
...
/* {{{ jonj_filter_full_special_chars */
void jonj_filter_full_special_chars(JONJ_INPUT_FILTER_PARAM_DECL)
{
	...
	buf = jonj_escape_html_entities_ex(Z_STRVAL_P(value), Z_STRLEN_P(value), &len, 1, quotes, SG(default_charset), 0 TSRMLS_CC);
	str_efree(Z_STRVAL_P(value));
...
/* {{{ jonj_filter_magic_quotes */
void jonj_filter_magic_quotes(JONJ_INPUT_FILTER_PARAM_DECL)
{
	char *buf;
	int   len;
	
	/* just call jonj_addslashes quotes */
	buf = jonj_addslashes(Z_STRVAL_P(value), Z_STRLEN_P(value), &len, 0 TSRMLS_CC);
