/*
  ...
  +----------------------------------------------------------------------+
  | Authors: Derick Rethans <derick@jonj.tk>                             |
...
#define FETCH_LONG_OPTION(var_name, option_name)                                                                         \
	...
	if (option_array) {                                                                                                  \
		if (zend_hash_find(HASH_OF(option_array), option_name, sizeof(option_name), (void **) &option_val) == SUCCESS) { \
			JONJ_FILTER_GET_LONG_OPT(option_val, var_name);								\
			var_name##_set = 1;                                                                                          \
		}                                                                                                                \
	}
/* }}} */
...
static int jonj_filter_parse_int(const char *str, unsigned int str_len, long *ret TSRMLS_DC) { /* {{{ */
	long ctx_value;
	int sign = 0, digit = 0;
...
static int jonj_filter_parse_octal(const char *str, unsigned int str_len, long *ret TSRMLS_DC) { /* {{{ */
...
static int jonj_filter_parse_hex(const char *str, unsigned int str_len, long *ret TSRMLS_DC) { /* {{{ */
	unsigned long ctx_value = 0;
	const char *end = str + str_len;
	unsigned long n;
...
void jonj_filter_int(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	JONJ_FILTER_TRIM_DEFAULT(p, len);

	if (*p == '0') {
		p++; len--;
		if (allow_hex && (*p == 'x' || *p == 'X')) {
			p++; len--;
			if (jonj_filter_parse_hex(p, len, &ctx_value TSRMLS_CC) < 0) {
				error = 1;
			}
		} else if (allow_octal) {
			if (jonj_filter_parse_octal(p, len, &ctx_value TSRMLS_CC) < 0) {
				error = 1;
			}
		} else if (len != 0) {
			error = 1;
		...
		if (jonj_filter_parse_int(p, len, &ctx_value TSRMLS_CC) < 0) {
			error = 1;
		}
	}
...
void jonj_filter_boolean(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	JONJ_FILTER_TRIM_DEFAULT_EX(str, len, 0);
...
void jonj_filter_float(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	JONJ_FILTER_TRIM_DEFAULT(str, len);
	end = str + len;
	...
	if (decimal_set) {
		if (decimal_len != 1) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "decimal separator must be one char");
			RETURN_VALIDATION_FAILED
		} else {
			dec_sep = *decimal;
		}
...
void jonj_filter_validate_regexp(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	if (!regexp_set) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "'regexp' option missing");
		RETURN_VALIDATION_FAILED
	}
...
void jonj_filter_validate_url(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	jonj_filter_url(value, flags, option_array, charset TSRMLS_CC);
	...
	url = jonj_url_parse_ex(Z_STRVAL_P(value), Z_STRLEN_P(value));
...
static int _jonj_filter_validate_ipv4(char *str, int str_len, int *ip) /* {{{ */
{
	const char *end = str + str_len;
	int num, m;
	int n = 0;
...
static int _jonj_filter_validate_ipv6(char *str, int str_len TSRMLS_DC) /* {{{ */
{
	int compressed = 0;
	int blocks = 0;
	int n;
	...
	if (ipv4) {
		...
		if (!_jonj_filter_validate_ipv4(ipv4, (str_len - (ipv4 - str)), ip4elm)) {
			return 0;
		}
...
void jonj_filter_validate_ip(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	switch (mode) {
		case FORMAT_IPV4:
			if (!_jonj_filter_validate_ipv4(Z_STRVAL_P(value), Z_STRLEN_P(value), ip)) {
				RETURN_VALIDATION_FAILED
			}
		...
		case FORMAT_IPV6:
			{
				int res = 0;
				res = _jonj_filter_validate_ipv6(Z_STRVAL_P(value), Z_STRLEN_P(value) TSRMLS_CC);
				if (res < 1) {
					RETURN_VALIDATION_FAILED
				}
				/* Check flags */
...
void jonj_filter_validate_mac(JONJ_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	if (exp_separator_set && exp_separator_len != 1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Separator must be exactly one character long");
		RETURN_VALIDATION_FAILED;
	}
	...
	for (i = 0; i < tokens; i++) {
		...
		if (jonj_filter_parse_hex(input + offset, length, &ret TSRMLS_CC) < 0) {
