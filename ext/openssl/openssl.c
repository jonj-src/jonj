/*
   ...
   |          Pierre-Alain Joye <pierre@jonj.tk>                          |
...
#include "jonj.h"
#include "jonj_ini.h"
#include "jonj_openssl.h"

/* JONJ Includes */
#include "ext/standard/file.h"
#include "ext/standard/info.h"
#include "ext/standard/jonj_fopen_wrappers.h"
 ...
 * against jonj constants. Also sorry to break the
 * enum principles here, BC...
 */
enum jonj_openssl_key_type {
	OPENSSL_KEYTYPE_RSA,
	OPENSSL_KEYTYPE_DSA,
	OPENSSL_KEYTYPE_DH,
...
enum jonj_openssl_cipher_type {
	JONJ_OPENSSL_CIPHER_RC2_40,
	JONJ_OPENSSL_CIPHER_RC2_128,
	JONJ_OPENSSL_CIPHER_RC2_64,
	JONJ_OPENSSL_CIPHER_DES,
	JONJ_OPENSSL_CIPHER_3DES,
	JONJ_OPENSSL_CIPHER_AES_128_CBC,
	JONJ_OPENSSL_CIPHER_AES_192_CBC,
	JONJ_OPENSSL_CIPHER_AES_256_CBC,

	JONJ_OPENSSL_CIPHER_DEFAULT = JONJ_OPENSSL_CIPHER_RC2_40
};

JONJ_FUNCTION(openssl_get_md_methods);
JONJ_FUNCTION(openssl_get_cipher_methods);

JONJ_FUNCTION(openssl_digest);
JONJ_FUNCTION(openssl_encrypt);
JONJ_FUNCTION(openssl_decrypt);
JONJ_FUNCTION(openssl_cipher_iv_length);

JONJ_FUNCTION(openssl_dh_compute_key);
JONJ_FUNCTION(openssl_random_pseudo_bytes);
...
const zend_function_entry openssl_functions[] = {
	JONJ_FE(openssl_get_cert_locations, arginfo_openssl_get_cert_locations)

/* spki functions */
	JONJ_FE(openssl_spki_new, arginfo_openssl_spki_new)
	JONJ_FE(openssl_spki_verify, arginfo_openssl_spki_verify)
	JONJ_FE(openssl_spki_export, arginfo_openssl_spki_export)
	JONJ_FE(openssl_spki_export_challenge, arginfo_openssl_spki_export_challenge)

/* public/private key functions */
	JONJ_FE(openssl_pkey_free,			arginfo_openssl_pkey_free)
	JONJ_FE(openssl_pkey_new,			arginfo_openssl_pkey_new)
	JONJ_FE(openssl_pkey_export,			arginfo_openssl_pkey_export)
	JONJ_FE(openssl_pkey_export_to_file,	arginfo_openssl_pkey_export_to_file)
	JONJ_FE(openssl_pkey_get_private,	arginfo_openssl_pkey_get_private)
	JONJ_FE(openssl_pkey_get_public,		arginfo_openssl_pkey_get_public)
	JONJ_FE(openssl_pkey_get_details,	arginfo_openssl_pkey_get_details)

	JONJ_FALIAS(openssl_free_key,		openssl_pkey_free, 			arginfo_openssl_pkey_free)
	JONJ_FALIAS(openssl_get_privatekey,	openssl_pkey_get_private,	arginfo_openssl_pkey_get_private)
	JONJ_FALIAS(openssl_get_publickey,	openssl_pkey_get_public,	arginfo_openssl_pkey_get_public)

/* x.509 cert funcs */
	JONJ_FE(openssl_x509_read,				arginfo_openssl_x509_read)
	JONJ_FE(openssl_x509_free,          		arginfo_openssl_x509_free)
	JONJ_FE(openssl_x509_parse,			 	arginfo_openssl_x509_parse)
	JONJ_FE(openssl_x509_checkpurpose,		arginfo_openssl_x509_checkpurpose)
	JONJ_FE(openssl_x509_check_private_key,	arginfo_openssl_x509_check_private_key)
	JONJ_FE(openssl_x509_export,				arginfo_openssl_x509_export)
	JONJ_FE(openssl_x509_fingerprint,			arginfo_openssl_x509_fingerprint)
	JONJ_FE(openssl_x509_export_to_file,		arginfo_openssl_x509_export_to_file)

/* PKCS12 funcs */
	JONJ_FE(openssl_pkcs12_export,			arginfo_openssl_pkcs12_export)
	JONJ_FE(openssl_pkcs12_export_to_file,	arginfo_openssl_pkcs12_export_to_file)
	JONJ_FE(openssl_pkcs12_read,				arginfo_openssl_pkcs12_read)

/* CSR funcs */
	JONJ_FE(openssl_csr_new,				arginfo_openssl_csr_new)
	JONJ_FE(openssl_csr_export,			arginfo_openssl_csr_export)
	JONJ_FE(openssl_csr_export_to_file,	arginfo_openssl_csr_export_to_file)
	JONJ_FE(openssl_csr_sign,			arginfo_openssl_csr_sign)
	JONJ_FE(openssl_csr_get_subject,		arginfo_openssl_csr_get_subject)
	JONJ_FE(openssl_csr_get_public_key,	arginfo_openssl_csr_get_public_key)

	JONJ_FE(openssl_digest,				arginfo_openssl_digest)
	JONJ_FE(openssl_encrypt,				arginfo_openssl_encrypt)
	JONJ_FE(openssl_decrypt,				arginfo_openssl_decrypt)
	JONJ_FE(openssl_cipher_iv_length,	arginfo_openssl_cipher_iv_length)
	JONJ_FE(openssl_sign,				arginfo_openssl_sign)
	JONJ_FE(openssl_verify,				arginfo_openssl_verify)
	JONJ_FE(openssl_seal,				arginfo_openssl_seal)
	JONJ_FE(openssl_open,				arginfo_openssl_open)

#if OPENSSL_VERSION_NUMBER >= 0x10000000L
	JONJ_FE(openssl_pbkdf2,	arginfo_openssl_pbkdf2)
#endif

/* for S/MIME handling */
	JONJ_FE(openssl_pkcs7_verify,		arginfo_openssl_pkcs7_verify)
	JONJ_FE(openssl_pkcs7_decrypt,		arginfo_openssl_pkcs7_decrypt)
	JONJ_FE(openssl_pkcs7_sign,			arginfo_openssl_pkcs7_sign)
	JONJ_FE(openssl_pkcs7_encrypt,		arginfo_openssl_pkcs7_encrypt)

	JONJ_FE(openssl_private_encrypt,		arginfo_openssl_private_encrypt)
	JONJ_FE(openssl_private_decrypt,		arginfo_openssl_private_decrypt)
	JONJ_FE(openssl_public_encrypt,		arginfo_openssl_public_encrypt)
	JONJ_FE(openssl_public_decrypt,		arginfo_openssl_public_decrypt)

	JONJ_FE(openssl_get_md_methods,		arginfo_openssl_get_md_methods)
	JONJ_FE(openssl_get_cipher_methods,	arginfo_openssl_get_cipher_methods)

	JONJ_FE(openssl_dh_compute_key,      arginfo_openssl_dh_compute_key)

	JONJ_FE(openssl_random_pseudo_bytes,    arginfo_openssl_random_pseudo_bytes)
	JONJ_FE(openssl_error_string, arginfo_openssl_error_string)
	JONJ_FE_END
};
/* }}} */
...
int jonj_openssl_get_x509_list_id(void) /* {{{ */
{
	return le_x509;
}
/* }}} */
...
static void jonj_pkey_free(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
static void jonj_x509_free(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
	X509 *x509 = (X509 *)rsrc->ptr;
	X509_free(x509);
}

static void jonj_csr_free(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
...
inline static int jonj_openssl_open_base_dir_chk(char *filename TSRMLS_DC)
{
...
jonj_stream* jonj_openssl_get_stream_from_ssl_handle(const SSL *ssl)
{
	return (jonj_stream*)SSL_get_ex_data(ssl, ssl_stream_data_index);
}

int jonj_openssl_get_ssl_stream_data_index()
{
	return ssl_stream_data_index;
}

/* openssl -> JONJ "bridging" */
/* true global; readonly after module startup */
static char default_ssl_conf_filename[MAXPATHLEN];

struct jonj_x509_request { /* {{{ */
...
static X509 * jonj_openssl_x509_from_zval(zval ** val, int makeresource, long * resourceval TSRMLS_DC);
static EVP_PKEY * jonj_openssl_evp_from_zval(zval ** val, int public_key, char * passphrase, int makeresource, long * resourceval TSRMLS_DC);
static int jonj_openssl_is_private_key(EVP_PKEY* pkey TSRMLS_DC);
static X509_STORE * setup_verify(zval * calist TSRMLS_DC);
static STACK_OF(X509) * load_all_certs_from_file(char *certfile);
static X509_REQ * jonj_openssl_csr_from_zval(zval ** val, int makeresource, long * resourceval TSRMLS_DC);
static EVP_PKEY * jonj_openssl_generate_private_key(struct jonj_x509_request * req TSRMLS_DC);
...
*/
	...
	if (ASN1_STRING_type(timestr) != V_ASN1_UTCTIME && ASN1_STRING_type(timestr) != V_ASN1_GENERALIZEDTIME) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "illegal ASN1 data type for timestamp");
		return (time_t)-1;
	}

	if (ASN1_STRING_length(timestr) != strlen((const char*)ASN1_STRING_data(timestr))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "illegal length in timestamp");
		return (time_t)-1;
	}

	if (ASN1_STRING_length(timestr) < 13) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to parse time string %s correctly", timestr->data);
		return (time_t)-1;
	}
...
#if OPENSSL_VERSION_NUMBER >= 0x10000002L
static inline int jonj_openssl_config_check_syntax(const char * section_label, const char * config_filename, const char * section, LHASH_OF(CONF_VALUE) * config TSRMLS_DC) /* {{{ */
#else
static inline int jonj_openssl_config_check_syntax(const char * section_label, const char * config_filename, const char * section, LHASH * config TSRMLS_DC)
#endif
{
	X509V3_CTX ctx;
	...
	if (!X509V3_EXT_add_conf(config, &ctx, (char *)section, NULL)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error loading %s section %s of %s",
				section_label,
				section,
				config_filename);
		return FAILURE;
...
static int add_oid_section(struct jonj_x509_request * req TSRMLS_DC) /* {{{ */
{
	char * str;
	...
	if (sktmp == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "problem loading oid section %s", str);
		return FAILURE;
	}
	for (i = 0; i < sk_CONF_VALUE_num(sktmp); i++) {
		cnf = sk_CONF_VALUE_value(sktmp, i);
		if (OBJ_create(cnf->value, cnf->name, cnf->name) == NID_undef) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "problem creating object %s=%s", cnf->name, cnf->value);
			return FAILURE;
		}
	}
	return SUCCESS;
...
#define JONJ_SSL_REQ_INIT(req)		memset(req, 0, sizeof(*req))
#define JONJ_SSL_REQ_DISPOSE(req)	jonj_openssl_dispose_config(req TSRMLS_CC)
#define JONJ_SSL_REQ_PARSE(req, zval)	jonj_openssl_parse_config(req, zval TSRMLS_CC)

#define JONJ_SSL_CONFIG_SYNTAX_CHECK(var) if (req->var && jonj_openssl_config_check_syntax(#var, \
...
static const EVP_CIPHER * jonj_openssl_get_evp_cipher_from_algo(long algo);

int openssl_spki_cleanup(const char *src, char *dest);

static int jonj_openssl_parse_config(struct jonj_x509_request * req, zval * optional_args TSRMLS_DC) /* {{{ */
{
	char * str;
	zval ** item;
	...
	if (str && !jonj_openssl_open_base_dir_chk(str TSRMLS_CC)) {
	...
	if (req->priv_key_encrypt && optional_args && zend_hash_find(Z_ARRVAL_P(optional_args), "encrypt_key_cipher", sizeof("encrypt_key_cipher"), (void**)&item) == SUCCESS 
		...
		const EVP_CIPHER* cipher = jonj_openssl_get_evp_cipher_from_algo(cipher_algo);
		if (cipher == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown cipher algorithm for private key.");
			return FAILURE;
		} else  {
	...
	JONJ_SSL_CONFIG_SYNTAX_CHECK(extensions_section);

	/* set the string mask */
	str = CONF_get_string(req->req_config, req->section_name, "string_mask");
	if (str && !ASN1_STRING_set_default_mask_asc(str)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid global string mask setting %s", str);
		return FAILURE;
	}

	JONJ_SSL_CONFIG_SYNTAX_CHECK(request_extensions_section);
	
	return SUCCESS;
}
/* }}} */

static void jonj_openssl_dispose_config(struct jonj_x509_request * req TSRMLS_DC) /* {{{ */
{
	if (req->priv_key) {
...
#ifdef JONJ_WIN32
#define JONJ_OPENSSL_RAND_ADD_TIME() ((void) 0)
#else
#define JONJ_OPENSSL_RAND_ADD_TIME() jonj_openssl_rand_add_timeval()

static inline void jonj_openssl_rand_add_timeval()  /* {{{ */
{
	struct timeval tv;
...
static int jonj_openssl_load_rand_file(const char * file, int *egdsocket, int *seeded TSRMLS_DC) /* {{{ */
{
	...
	*seeded = 0;
	...
	if (file == NULL || !RAND_load_file(file, -1)) {
		if (RAND_status() == 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to load random state; not enough random data!");
			return FAILURE;
		}
		return FAILURE;
	}
...
static int jonj_openssl_write_rand_file(const char * file, int egdsocket, int seeded) /* {{{ */
{
	char buffer[MAXPATHLEN];
	...
	JONJ_OPENSSL_RAND_ADD_TIME();
	if (file == NULL || !RAND_write_file(file)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to write random state");
		return FAILURE;
	}
	return SUCCESS;
}
...
static EVP_MD * jonj_openssl_get_evp_md_from_algo(long algo) { /* {{{ */
	EVP_MD *mdtype;
...
static const EVP_CIPHER * jonj_openssl_get_evp_cipher_from_algo(long algo) { /* {{{ */
	switch (algo) {
		...
		case JONJ_OPENSSL_CIPHER_RC2_40:
			return EVP_rc2_40_cbc();
			break;
		case JONJ_OPENSSL_CIPHER_RC2_64:
			return EVP_rc2_64_cbc();
			break;
		case JONJ_OPENSSL_CIPHER_RC2_128:
			return EVP_rc2_cbc();
			break;
#endif

#ifndef OPENSSL_NO_DES
		case JONJ_OPENSSL_CIPHER_DES:
			return EVP_des_cbc();
			break;
		case JONJ_OPENSSL_CIPHER_3DES:
			return EVP_des_ede3_cbc();
			break;
#endif

#ifndef OPENSSL_NO_AES
		case JONJ_OPENSSL_CIPHER_AES_128_CBC:
			return EVP_aes_128_cbc();
			break;
		case JONJ_OPENSSL_CIPHER_AES_192_CBC:
			return EVP_aes_192_cbc();
			break;
		case JONJ_OPENSSL_CIPHER_AES_256_CBC:
...
/* {{{ INI Settings */
JONJ_INI_BEGIN()
	JONJ_INI_ENTRY("openssl.cafile", NULL, JONJ_INI_PERDIR, NULL)
	JONJ_INI_ENTRY("openssl.capath", NULL, JONJ_INI_PERDIR, NULL)
JONJ_INI_END()
/* }}} */
...
JONJ_MINIT_FUNCTION(openssl)
{
	...
	le_key = zend_register_list_destructors_ex(jonj_pkey_free, NULL, "OpenSSL key", module_number);
	le_x509 = zend_register_list_destructors_ex(jonj_x509_free, NULL, "OpenSSL X.509", module_number);
	le_csr = zend_register_list_destructors_ex(jonj_csr_free, NULL, "OpenSSL X.509 CSR", module_number);
	 ...
	 * OpenSSL callbacks */
	ssl_stream_data_index = SSL_get_ex_new_index(0, "JONJ stream index", NULL, NULL, NULL);
	...
	/* Ciphers */
#ifndef OPENSSL_NO_RC2
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_RC2_40", JONJ_OPENSSL_CIPHER_RC2_40, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_RC2_128", JONJ_OPENSSL_CIPHER_RC2_128, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_RC2_64", JONJ_OPENSSL_CIPHER_RC2_64, CONST_CS|CONST_PERSISTENT);
#endif
#ifndef OPENSSL_NO_DES
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_DES", JONJ_OPENSSL_CIPHER_DES, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_3DES", JONJ_OPENSSL_CIPHER_3DES, CONST_CS|CONST_PERSISTENT);
#endif
#ifndef OPENSSL_NO_AES
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_AES_128_CBC", JONJ_OPENSSL_CIPHER_AES_128_CBC, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_AES_192_CBC", JONJ_OPENSSL_CIPHER_AES_192_CBC, CONST_CS|CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("OPENSSL_CIPHER_AES_256_CBC", JONJ_OPENSSL_CIPHER_AES_256_CBC, CONST_CS|CONST_PERSISTENT);
#endif
	...
	jonj_stream_xport_register("ssl", jonj_openssl_ssl_socket_factory TSRMLS_CC);
	jonj_stream_xport_register("sslv3", jonj_openssl_ssl_socket_factory TSRMLS_CC);
#ifndef OPENSSL_NO_SSL2
	jonj_stream_xport_register("sslv2", jonj_openssl_ssl_socket_factory TSRMLS_CC);
#endif
	jonj_stream_xport_register("tls", jonj_openssl_ssl_socket_factory TSRMLS_CC);
	jonj_stream_xport_register("tlsv1.0", jonj_openssl_ssl_socket_factory TSRMLS_CC);
#if OPENSSL_VERSION_NUMBER >= 0x10001001L
	jonj_stream_xport_register("tlsv1.1", jonj_openssl_ssl_socket_factory TSRMLS_CC);
	jonj_stream_xport_register("tlsv1.2", jonj_openssl_ssl_socket_factory TSRMLS_CC);
#endif

	/* override the default tcp socket provider */
	jonj_stream_xport_register("tcp", jonj_openssl_ssl_socket_factory TSRMLS_CC);

	jonj_register_url_stream_wrapper("https", &jonj_stream_http_wrapper TSRMLS_CC);
	jonj_register_url_stream_wrapper("ftps", &jonj_stream_ftp_wrapper TSRMLS_CC);
...
JONJ_MINFO_FUNCTION(openssl)
{
	...
	jonj_info_print_table_row(2, "OpenSSL support", "enabled");
	jonj_info_print_table_row(2, "OpenSSL Library Version", SSLeay_version(SSLEAY_VERSION));
	jonj_info_print_table_row(2, "OpenSSL Header Version", OPENSSL_VERSION_TEXT);
	jonj_info_print_table_row(2, "Openssl default config", default_ssl_conf_filename);
...
JONJ_MSHUTDOWN_FUNCTION(openssl)
{
	...
	jonj_unregister_url_stream_wrapper("https" TSRMLS_CC);
	jonj_unregister_url_stream_wrapper("ftps" TSRMLS_CC);

	jonj_stream_xport_unregister("ssl" TSRMLS_CC);
#ifndef OPENSSL_NO_SSL2
	jonj_stream_xport_unregister("sslv2" TSRMLS_CC);
#endif
	jonj_stream_xport_unregister("sslv3" TSRMLS_CC);
	jonj_stream_xport_unregister("tls" TSRMLS_CC);
	jonj_stream_xport_unregister("tlsv1.0" TSRMLS_CC);
#if OPENSSL_VERSION_NUMBER >= 0x10001001L
	jonj_stream_xport_unregister("tlsv1.1" TSRMLS_CC);
	jonj_stream_xport_unregister("tlsv1.2" TSRMLS_CC);
#endif

	/* reinstate the default tcp handler */
	jonj_stream_xport_register("tcp", jonj_stream_generic_socket_factory TSRMLS_CC);
...
static X509 * jonj_openssl_x509_from_zval(zval ** val, int makeresource, long * resourceval TSRMLS_DC)
{
	...
	if (Z_STRLEN_PP(val) > 7 && memcmp(Z_STRVAL_PP(val), "file://", sizeof("file://") - 1) == 0) {
		...
		if (jonj_openssl_open_base_dir_chk(Z_STRVAL_PP(val) + (sizeof("file://") - 1) TSRMLS_CC)) {
			return NULL;
		}
...
JONJ_FUNCTION(openssl_x509_export_to_file)
{
	...
	cert = jonj_openssl_x509_from_zval(zcert, 0, &certresource TSRMLS_CC);
	if (cert == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot get cert from parameter 1");
		return;
	}

	if (jonj_openssl_open_base_dir_chk(filename TSRMLS_CC)) {
		return;
	}
	...
	if (bio_out) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error opening file %s", filename);
	}
...
JONJ_FUNCTION(openssl_spki_new)
{
	...
	pkey = jonj_openssl_evp_from_zval(&zpkey, 0, challenge, 1, &keyresource TSRMLS_CC);

	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to use supplied private key");
		goto cleanup;
	}
	...
	if (method != NULL) {
		if (Z_TYPE_P(method) == IS_LONG) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Algorithm must be of supported type");
			goto cleanup;
		}
	}
	mdtype = jonj_openssl_get_evp_md_from_algo(algo);

	if (!mdtype) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown signature algorithm");
		goto cleanup;
	}

	if ((spki = NETSCAPE_SPKI_new()) == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to create new SPKAC");
		goto cleanup;
	}
	...
	if (!NETSCAPE_SPKI_set_pubkey(spki, pkey)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to embed public key");
		goto cleanup;
	}

	if (!NETSCAPE_SPKI_sign(spki, pkey, mdtype)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to sign with specified algorithm");
		goto cleanup;
	}

	spkstr = NETSCAPE_SPKI_b64_encode(spki);
	if (!spkstr){
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to encode SPKAC");
		goto cleanup;
	}
...
JONJ_FUNCTION(openssl_spki_verify)
{
	...
	if (spkstr == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to use supplied SPKAC");
		goto cleanup;
	}
	...
	if (strlen(spkstr_cleaned)<=0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid SPKAC");
		goto cleanup;
	}

	spki = NETSCAPE_SPKI_b64_decode(spkstr_cleaned, strlen(spkstr_cleaned));
	if (spki == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to decode supplied SPKAC");
		goto cleanup;
	}

	pkey = X509_PUBKEY_get(spki->spkac->pubkey);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to acquire signed public key");
		goto cleanup;
	}
...
JONJ_FUNCTION(openssl_spki_export_challenge)
{
	...
	spki = NETSCAPE_SPKI_b64_decode(spkstr_cleaned, strlen(spkstr_cleaned));
	if (spki == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to decode SPKAC");
		goto cleanup;
	}
...
int jonj_openssl_x509_fingerprint(X509 *peer, const char *method, zend_bool raw, char **out, int *out_len TSRMLS_DC)
{
	...
	if (!(mdtype = EVP_get_digestbyname(method))) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "Could not generate signature");
		return FAILURE;
	}
...
JONJ_FUNCTION(openssl_x509_fingerprint)
{
	...
	if (jonj_openssl_x509_fingerprint(cert, method, raw_output, &fingerprint, &fingerprint_len TSRMLS_CC) == SUCCESS) {
...
JONJ_FUNCTION(openssl_x509_check_private_key)
{
	...
	key = jonj_openssl_evp_from_zval(zkey, 0, "", 1, &keyresource TSRMLS_CC);
	if (key) {
...
static STACK_OF(X509) * load_all_certs_from_file(char *certfile)
{
	...
	if(!(stack = sk_X509_new_null())) {
		jonj_error_docref(NULL TSRMLS_CC, E_ERROR, "memory allocation failure");
		goto end;
	}

	if (jonj_openssl_open_base_dir_chk(certfile TSRMLS_CC)) {
		sk_X509_free(stack);
		goto end;
	}

	if(!(in=BIO_new_file(certfile, "r"))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error opening the file, %s", certfile);
		sk_X509_free(stack);
		goto end;
	}
	...
	if(!(sk=PEM_X509_INFO_read_bio(in, NULL, NULL, NULL))) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error reading the file, %s", certfile);
		sk_X509_free(stack);
		goto end;
	}
	...
	if(!sk_X509_num(stack)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no certificates in file, %s", certfile);
		sk_X509_free(stack);
		goto end;
	}
	ret=stack;
	...
	if (calist && (Z_TYPE_P(calist) == IS_ARRAY)) {
		...
		for (;; zend_hash_move_forward_ex(HASH_OF(calist), &pos)) {
			...
			if (VCWD_STAT(Z_STRVAL_PP(item), &sb) == -1) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to stat %s", Z_STRVAL_PP(item));
				continue;
			}
			...
			if ((sb.st_mode & S_IFREG) == S_IFREG) {
				...
				if (file_lookup == NULL || !X509_LOOKUP_load_file(file_lookup, Z_STRVAL_PP(item), X509_FILETYPE_PEM)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error loading file %s", Z_STRVAL_PP(item));
				} else {
					nfiles++;
				}
				file_lookup = NULL;
				...
				if (dir_lookup == NULL || !X509_LOOKUP_add_dir(dir_lookup, Z_STRVAL_PP(item), X509_FILETYPE_PEM)) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error loading directory %s", Z_STRVAL_PP(item));
				} else { 
					ndirs++;
				}
				dir_lookup = NULL;
...
JONJ_FUNCTION(openssl_x509_read)
{
	...
	x509 = jonj_openssl_x509_from_zval(cert, 1, &Z_LVAL_P(return_value) TSRMLS_CC);

	if (x509 == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied parameter cannot be coerced into an X509 certificate!");
		RETURN_FALSE;
	}
}
/* }}} */
...
static void jonj_sk_X509_free(STACK_OF(X509) * sk) /* {{{ */
{
	for (;;) {
...
static STACK_OF(X509) * jonj_array_to_X509_sk(zval ** zcerts TSRMLS_DC) /* {{{ */
{
	HashPosition hpos;
	zval ** zcertval;
	...
	if (Z_TYPE_PP(zcerts) == IS_ARRAY) {
		...
		while(zend_hash_get_current_data_ex(HASH_OF(*zcerts), (void**)&zcertval, &hpos) == SUCCESS) {
			...
			cert = jonj_openssl_x509_from_zval(zcertval, 0, &certresource TSRMLS_CC);
			if (cert == NULL) {
				goto clean_exit;
			}
		...
		cert = jonj_openssl_x509_from_zval(zcerts, 0, &certresource TSRMLS_CC);
...
JONJ_FUNCTION(openssl_pkcs12_export_to_file)
{
	...
	priv_key = jonj_openssl_evp_from_zval(&zpkey, 0, "", 1, &keyresource TSRMLS_CC);
	if (priv_key == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot get private key from parameter 3");
		goto cleanup;
	}
	if (cert && !X509_check_private_key(cert, priv_key)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "private key does not correspond to cert");
		goto cleanup;
	}
	if (jonj_openssl_open_base_dir_chk(filename TSRMLS_CC)) {
		goto cleanup;
	...
	*/
	...
	if (args && zend_hash_find(Z_ARRVAL_P(args), "extracerts", sizeof("extracerts"), (void**)&item) == SUCCESS)
		ca = jonj_array_to_X509_sk(item TSRMLS_CC);
...
/* {{{ jonj_openssl_make_REQ */
static int jonj_openssl_make_REQ(struct jonj_x509_request * req, X509_REQ * csr, zval * dn, zval * attribs TSRMLS_DC)
{
	...
	if (X509_REQ_set_version(csr, 0L)) {
		...
		while(zend_hash_get_current_data_ex(HASH_OF(dn), (void**)&item, &hpos) == SUCCESS) {
			...
			if (strindex) {
				...
				if (nid != NID_undef) {
					...
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "dn: %s is not a recognized name", strindex);
				}
			}
		...
		for(i = 0; i < sk_CONF_VALUE_num(dn_sk); i++) {
			...
			if (!X509_NAME_add_entry_by_txt(subj, type, MBSTRING_UTF8, (unsigned char*)v->value, -1, -1, 0)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "add_entry_by_txt %s -> %s (failed)", type, v->value);
				return FAILURE;
			}
			if (!X509_NAME_entry_count(subj)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "no objects specified in config file");
				return FAILURE;
			}
		}
		if (attribs) {
				...
				if (strindex) {
					...
					if (nid != NID_undef) {
						if (!X509_NAME_add_entry_by_NID(subj, nid, MBSTRING_UTF8, (unsigned char*)Z_STRVAL_PP(item), -1, -1, 0)) {
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "attribs: add_entry_by_NID %d -> %s (failed)", nid, Z_STRVAL_PP(item));
							return FAILURE;
						}
					} else {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "dn: %s is not a recognized name", strindex);
...
JONJ_FUNCTION(openssl_csr_export_to_file)
{
	...
	csr = jonj_openssl_csr_from_zval(&zcsr, 0, &csr_resource TSRMLS_CC);
	if (csr == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot get CSR from parameter 1");
		return;
	}
...
JONJ_FUNCTION(openssl_csr_sign)
{
	...
	if (zcert) {
		cert = jonj_openssl_x509_from_zval(zcert, 0, &certresource TSRMLS_CC);
		if (cert == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot get cert from parameter 2");
			goto cleanup;
		}
	}
	priv_key = jonj_openssl_evp_from_zval(zpkey, 0, "", 1, &keyresource TSRMLS_CC);
	...
	if (cert && !X509_check_private_key(cert, priv_key)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "private key does not correspond to signing cert");
		goto cleanup;
	}
	
	if (JONJ_SSL_REQ_PARSE(&req, args) == FAILURE) {
		goto cleanup;
	}
	/* Check that the request matches the signature */
	key = X509_REQ_get_pubkey(csr);
	if (key == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error unpacking public key");
		goto cleanup;
	}
	i = X509_REQ_verify(csr, key);

	if (i < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Signature verification problems");
		goto cleanup;
	}
	else if (i == 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Signature did not match the certificate request");
		goto cleanup;
	}
	...
	new_cert = X509_new();
	if (new_cert == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "No memory");
		goto cleanup;
	}
	/* Version 3 cert */
	...
	if (!X509_sign(new_cert, priv_key, req.digest)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to sign it");
		goto cleanup;
	}
...
JONJ_FUNCTION(openssl_csr_new)
{
	...
	if (JONJ_SSL_REQ_PARSE(&req, args) == SUCCESS) {
		/* Generate or use a private key */
		if (Z_TYPE_P(out_pkey) != IS_NULL) {
			req.priv_key = jonj_openssl_evp_from_zval(&out_pkey, 0, NULL, 0, &key_resource TSRMLS_CC);
			if (req.priv_key != NULL) {
				we_made_the_key = 0;
			}
		}
		if (req.priv_key == NULL) {
			jonj_openssl_generate_private_key(&req TSRMLS_CC);
		}
		if (req.priv_key == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to generate a private key");
		} else {
			csr = X509_REQ_new();
			if (csr) {
				if (jonj_openssl_make_REQ(&req, csr, dn, attribs TSRMLS_CC) == SUCCESS) {
					X509V3_CTX ext_ctx;
					...
					{
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error loading extension section %s", req.request_extensions_section);
					} else {
						RETVAL_TRUE;
						...
						if (X509_REQ_sign(csr, req.priv_key, req.digest)) {
							...
							jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error signing request");
						}
...
#define TMP_CLEAN \
	...
	if (Z_TYPE_PP(val) == IS_ARRAY) {
		...
		if (zend_hash_index_find(HASH_OF(*val), 1, (void **)&zphrase) == FAILURE) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key array must be of the form array(0 => key, 1 => phrase)");
			return NULL;
		}
	...
	if (Z_TYPE_PP(val) == IS_RESOURCE) {
		...
		if (type == le_x509) {
			...
			is_priv = jonj_openssl_is_private_key((EVP_PKEY*)what TSRMLS_CC);

			/* check whether it is actually a private key if requested */
			if (!public_key && !is_priv) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied key param is a public key");
				TMP_CLEAN;
			}

			if (public_key && is_priv) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Don't know how to get public key from this private key");
				TMP_CLEAN;
			} else {
				if (Z_TYPE(tmp) == IS_STRING) {
					zval_dtor(&tmp);
		...
		if (public_key) {
			cert = jonj_openssl_x509_from_zval(val, 0, &cert_res TSRMLS_CC);
...
/* {{{ jonj_openssl_generate_private_key */
static EVP_PKEY * jonj_openssl_generate_private_key(struct jonj_x509_request * req TSRMLS_DC)
{
	char * randfile = NULL;
	...
	if (req->priv_key_bits < MIN_KEY_LENGTH) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "private key length is too short; it needs to be at least %d bits, not %d",
				MIN_KEY_LENGTH, req->priv_key_bits);
		return NULL;
	}

	randfile = CONF_get_string(req->req_config, req->section_name, "RANDFILE");
	jonj_openssl_load_rand_file(randfile, &egdsocket, &seeded TSRMLS_CC);
...
#endif
			default:
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unsupported private key type");
		}
	}

	jonj_openssl_write_rand_file(randfile, egdsocket, seeded);
...
#endif
		default:
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key type not supported in this JONJ build!");
			break;
	}
	return 1;
}
...
/* {{{ jonj_openssl_pkey_init_dsa */
zend_bool jonj_openssl_pkey_init_dsa(DSA *dsa)
{
...
/* {{{ jonj_openssl_pkey_init_dh */
zend_bool jonj_openssl_pkey_init_dh(DH *dh)
{
	if (!dh->p || !dh->g) {
		return 0;
	}
...
JONJ_FUNCTION(openssl_pkey_new)
{
		...
		if (zend_hash_find(Z_ARRVAL_P(args), "rsa", sizeof("rsa"), (void**)&data) == SUCCESS &&
		    ...
		    if (pkey) {
				...
				if (dsa) {
					...
					if (jonj_openssl_pkey_init_dsa(dsa)) {
		    ...
		    if (pkey) {
				...
				if (dh) {
					...
					if (jonj_openssl_pkey_init_dh(dh)) {
	...
	if (JONJ_SSL_REQ_PARSE(&req, args) == SUCCESS)
	{
		if (jonj_openssl_generate_private_key(&req TSRMLS_CC)) {
...
JONJ_FUNCTION(openssl_pkey_export_to_file)
{
	...
	key = jonj_openssl_evp_from_zval(zpkey, 0, passphrase, 0, &key_resource TSRMLS_CC);

	if (key == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot get key from parameter 1");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(openssl_pkey_get_public)
{
	...
	Z_TYPE_P(return_value) = IS_RESOURCE;
	pkey = jonj_openssl_evp_from_zval(cert, 1, NULL, 1, &Z_LVAL_P(return_value) TSRMLS_CC);
...
JONJ_FUNCTION(openssl_pkey_get_private)
{
	...
	Z_TYPE_P(return_value) = IS_RESOURCE;
	pkey = jonj_openssl_evp_from_zval(cert, 0, passphrase, 1, &Z_LVAL_P(return_value) TSRMLS_CC);
...
JONJ_FUNCTION(openssl_pkcs7_verify)
{
	...
	if (datafilename) {
		...
		if (jonj_openssl_open_base_dir_chk(datafilename TSRMLS_CC)) {
			goto clean_exit;
		}
	...
	if (PKCS7_verify(p7, others, store, datain, dataout, flags)) {
		...
		if (signersfilename) {
			...
			if (jonj_openssl_open_base_dir_chk(signersfilename TSRMLS_CC)) {
				goto clean_exit;
			}
			...
			if (certout) {
				...
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "signature OK, but cannot open %s for writing", signersfilename);
				RETVAL_LONG(-1);
			}
		}
		goto clean_exit;
...
JONJ_FUNCTION(openssl_pkcs7_encrypt)
{
	...
	long cipherid = JONJ_OPENSSL_CIPHER_DEFAULT;
	uint strindexlen;
	ulong intindex;
	char * strindex;
	...
	if (jonj_openssl_open_base_dir_chk(infilename TSRMLS_CC) || jonj_openssl_open_base_dir_chk(outfilename TSRMLS_CC)) {
		return;
	}
	...
	if (Z_TYPE_PP(zrecipcerts) == IS_ARRAY) {
		...
		cert = jonj_openssl_x509_from_zval(zrecipcerts, 0, &certresource TSRMLS_CC);
		if (cert == NULL) {
			goto clean_exit;
		}
	...
	cipher = jonj_openssl_get_evp_cipher_from_algo(cipherid);
	if (cipher == NULL) {
		/* shouldn't happen */
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to get cipher");
		goto clean_exit;
	}
...
JONJ_FUNCTION(openssl_pkcs7_sign)
{
	...
	privkey = jonj_openssl_evp_from_zval(zprivkey, 0, "", 0, &keyresource TSRMLS_CC);
	if (privkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error getting private key");
		goto clean_exit;
	}

	cert = jonj_openssl_x509_from_zval(zcert, 0, &certresource TSRMLS_CC);
	if (cert == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error getting cert");
		goto clean_exit;
	}
	...
	infile = BIO_new_file(infilename, "r");
	if (infile == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error opening input file %s!", infilename);
		goto clean_exit;
	}

	outfile = BIO_new_file(outfilename, "w");
	if (outfile == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error opening output file %s!", outfilename);
		goto clean_exit;
	}

	p7 = PKCS7_sign(cert, privkey, others, infile, flags);
	if (p7 == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "error creating PKCS7 structure!");
		goto clean_exit;
	}
...
JONJ_FUNCTION(openssl_pkcs7_decrypt)
{
	...
	cert = jonj_openssl_x509_from_zval(recipcert, 0, &certresval TSRMLS_CC);
	if (cert == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to coerce parameter 3 to x509 cert");
		goto clean_exit;
	}

	key = jonj_openssl_evp_from_zval(recipkey ? recipkey : recipcert, 0, "", 0, &keyresval TSRMLS_CC);
	if (key == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to get private key");
		goto clean_exit;
	}
...
JONJ_FUNCTION(openssl_private_encrypt)
{
	...
	pkey = jonj_openssl_evp_from_zval(key, 0, "", 0, &keyresource TSRMLS_CC);

	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key param is not a valid private key");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(openssl_private_decrypt)
{
	...
	pkey = jonj_openssl_evp_from_zval(key, 0, "", 0, &keyresource TSRMLS_CC);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key parameter is not a valid private key");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(openssl_public_encrypt)
{
	...
	pkey = jonj_openssl_evp_from_zval(key, 1, NULL, 0, &keyresource TSRMLS_CC);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "key parameter is not a valid public key");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(openssl_sign)
{
	...
	pkey = jonj_openssl_evp_from_zval(key, 0, "", 0, &keyresource TSRMLS_CC);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied key param cannot be coerced into a private key");
		RETURN_FALSE;
	}
	...
	if (method == NULL || Z_TYPE_P(method) == IS_LONG) {
		...
		mdtype = jonj_openssl_get_evp_md_from_algo(signature_algo);
...
JONJ_FUNCTION(openssl_verify)
{
	...
	pkey = jonj_openssl_evp_from_zval(key, 1, NULL, 0, &keyresource TSRMLS_CC);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied key param cannot be coerced into a public key");
		RETURN_FALSE;
	}
...
JONJ_FUNCTION(openssl_seal)
{
	...
	if (!nkeys) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Fourth argument to openssl_seal() must be a non-empty array");
		RETURN_FALSE;
	}
	...
	if (method) {
		...
		if (EVP_CIPHER_iv_length(cipher) > 0) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Ciphers with modes requiring IV are not supported");
			RETURN_FALSE;
		}
	} else {
		cipher = EVP_rc4();
	...
	while (zend_hash_get_current_data_ex(pubkeysht, (void **) &pubkey,
				&pos) == SUCCESS) {
		pkeys[i] = jonj_openssl_evp_from_zval(pubkey, 1, NULL, 0, &key_resources[i] TSRMLS_CC);
		if (pkeys[i] == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "not a public key (%dth member of pubkeys)", i+1);
			RETVAL_FALSE;
			goto clean_exit;
		}
...
JONJ_FUNCTION(openssl_open)
{
	...
	pkey = jonj_openssl_evp_from_zval(privkey, 0, "", 0, &keyresource TSRMLS_CC);
	if (pkey == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to coerce parameter 4 into a private key");
		RETURN_FALSE;
	}
...
static zend_bool jonj_openssl_validate_iv(char **piv, int *piv_len, int iv_required_len TSRMLS_DC)
{
	char *iv_new;
	...
	if (*piv_len < iv_required_len) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "IV passed is only %d bytes long, cipher expects an IV of precisely %d bytes, padding with \\0", *piv_len, iv_required_len);
	...
	jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "IV passed is %d bytes long which is longer than the %d expected by selected cipher, truncating", *piv_len, iv_required_len);
...
JONJ_FUNCTION(openssl_encrypt)
{
	...
	if (!cipher_type) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown cipher algorithm");
		RETURN_FALSE;
	}
	...
	if (iv_len <= 0 && max_iv_len > 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Using an empty Initialization Vector (iv) is potentially insecure and not recommended");
	}
	free_iv = jonj_openssl_validate_iv(&iv, &iv_len, max_iv_len TSRMLS_CC);
	...
	if (EVP_EncryptFinal(&cipher_ctx, (unsigned char *)outbuf + i, &i)) {
		...
		if (options & OPENSSL_RAW_DATA) {
			...
			base64_str = (char*)jonj_base64_encode(outbuf, outlen, &base64_str_len);
			efree(outbuf);
			if (!base64_str) {
				RETVAL_FALSE;
			} else {
...
JONJ_FUNCTION(openssl_decrypt)
{
	...
	if (!(options & OPENSSL_RAW_DATA)) {
		base64_str = (char*)jonj_base64_decode((unsigned char*)data, data_len, &base64_str_len);
		if (!base64_str) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to base64 decode the input");
			RETURN_FALSE;
		}
	...
	free_iv = jonj_openssl_validate_iv(&iv, &iv_len, EVP_CIPHER_iv_length(cipher_type) TSRMLS_CC);
...
JONJ_FUNCTION(openssl_random_pseudo_bytes)
{
	...
	if (jonj_win32_get_random_bytes(buffer, (size_t) buffer_length) == FAILURE){
		efree(buffer);
		if (zstrong_result_returned) {
			ZVAL_BOOL(zstrong_result_returned, 0);
		}
