/*
  ...
  |          Daniel Lowrey <rdlowrey@jonj.tk>                            |
  |          Chris Wright <daverandom@jonj.tk>                           |
  +----------------------------------------------------------------------+
*/
...
#define GET_VER_OPT(name)               (stream->context && SUCCESS == jonj_stream_context_get_option(stream->context, "ssl", name, &val))
...
#define JONJ_X509_NAME_ENTRY_TO_UTF8(ne, i, out) ASN1_STRING_to_UTF8(&out, X509_NAME_ENTRY_get_data(X509_NAME_get_entry(ne, i)))

extern jonj_stream* jonj_openssl_get_stream_from_ssl_handle(const SSL *ssl);
extern int jonj_openssl_x509_fingerprint(X509 *peer, const char *method, zend_bool raw, char **out, int *out_len TSRMLS_DC);
extern int jonj_openssl_get_ssl_stream_data_index();
extern int jonj_openssl_get_x509_list_id(void);
...
static size_t jonj_openssl_sockop_io(int read, jonj_stream *stream, char *buf, size_t count TSRMLS_DC);

jonj_stream_ops jonj_openssl_socket_ops;

/* Certificate contexts used for server-side SNI selection */
typedef struct _jonj_openssl_sni_cert_t {
	char *name;
	SSL_CTX *ctx;
} jonj_openssl_sni_cert_t;

/* Provides leaky bucket handhsake renegotiation rate-limiting  */
typedef struct _jonj_openssl_handshake_bucket_t {
	long prev_handshake;
	long limit;
	long window;
	float tokens;
	unsigned should_close;
} jonj_openssl_handshake_bucket_t;

/* This implementation is very closely tied to the that of the native
 * sockets implemented in the core.
 * Don't try this technique in other extensions!
 * */
typedef struct _jonj_openssl_netstream_data_t {
	jonj.tkstream_data_t s;
	SSL *ssl_handle;
	SSL_CTX *ctx;
	...
	jonj_openssl_handshake_bucket_t *reneg;
	jonj_openssl_sni_cert_t *sni_certs;
	unsigned sni_cert_count;
	char *url_name;
	unsigned state_set:1;
	unsigned _spare:31;
} jonj_openssl_netstream_data_t;
...
static int is_http_stream_talking_to_iis(jonj_stream *stream TSRMLS_DC) /* {{{ */
{
...
static int handle_ssl_error(jonj_stream *stream, int nr_bytes, zend_bool is_init TSRMLS_DC) /* {{{ */
{
	jonj_openssl_netstream_data_t *sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
		...
		case SSL_ERROR_SYSCALL:
			if (ERR_peek_error() == 0) {
				if (nr_bytes == 0) {
					...
					char *estr = jonj_socket_strerror(jonj_socket_errno(), NULL, 0);
		...
		default:
			...
			switch (ERR_GET_REASON(ecode)) {
				case SSL_R_NO_SHARED_CIPHER:
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL_R_NO_SHARED_CIPHER: no suitable shared cipher could be used.  This could be because the server is missing an SSL certificate (local_cert context option)");
					retry = 0;
					break;
...
static int verify_callback(int preverify_ok, X509_STORE_CTX *ctx) /* {{{ */
{
	...
	stream = (jonj_stream*)SSL_get_ex_data(ssl, jonj_openssl_get_ssl_stream_data_index());
...
static int jonj_x509_fingerprint_cmp(X509 *peer, const char *method, const char *expected TSRMLS_DC)
{
	char *fingerprint;
	int fingerprint_len;
	int result = -1;

	if (jonj_openssl_x509_fingerprint(peer, method, 0, &fingerprint, &fingerprint_len TSRMLS_CC) == SUCCESS) {
...
static zend_bool jonj_x509_fingerprint_match(X509 *peer, zval *val TSRMLS_DC)
{
	if (Z_TYPE_P(val) == IS_STRING) {
		...
		return method && jonj_x509_fingerprint_cmp(peer, method, Z_STRVAL_P(val) TSRMLS_CC) == 0;
		...
		if (!zend_hash_num_elements(Z_ARRVAL_P(val))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid peer_fingerprint array; [algo => fingerprint] form required");
			return 0;
		}
		...
		for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(val), &pos);
			...
			if (jonj_x509_fingerprint_cmp(peer, key, Z_STRVAL_PP(current) TSRMLS_CC) != 0) {
				return 0;
			}
		}
...
static zend_bool matches_common_name(X509 *peer, const char *subject_name TSRMLS_DC) /* {{{ */
{
	...
	if (cert_name_len == -1) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to locate peer certificate CN");
	} else if (cert_name_len != strlen(buf)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Peer certificate CN=`%.*s' is malformed", cert_name_len, buf);
	} else if (matches_wildcard_name(subject_name, buf)) {
		is_match = 1;
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Peer certificate CN=`%.*s' did not match expected CN=`%s'", cert_name_len, buf, subject_name);
	}
...
static int apply_peer_verification_policy(SSL *ssl, X509 *peer, jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	zval **val = NULL;
	char *peer_name = NULL;
	int err,
	...
	if ((must_verify_peer || must_verify_peer_name || must_verify_fingerprint) && peer == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Could not get peer certificate");
		return FAILURE;
	}
	...
	if (must_verify_fingerprint) {
		if (Z_TYPE_PP(val) == IS_STRING || Z_TYPE_PP(val) == IS_ARRAY) {
			if (!jonj_x509_fingerprint_match(peer, *val TSRMLS_CC)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
...
{
	jonj_stream *stream = (jonj_stream *)data;
	zval **val = NULL;
	char *passphrase = NULL;
	/* TODO: could expand this to make a callback into JONJ user-space */
...
#if defined(JONJ_WIN32) && OPENSSL_VERSION_NUMBER >= 0x00907000L
...
static int win_cert_verify_callback(X509_STORE_CTX *x509_store_ctx, void *arg) /* {{{ */
{
	...
	jonj_stream *stream;
	jonj_openssl_netstream_data_t *sslsock;
	zval **val;
	zend_bool is_self_signed = 0;

	TSRMLS_FETCH();

	stream = (jonj_stream*)arg;
	sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
	...
	{ /* First convert the x509 struct back to a DER encoded buffer and let Windows decode it into a form it can work with */
		...
		if (der_len < 0) {
			...
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error encoding X509 certificate: %d: %s", err_code, ERR_error_string(err_code, err_buf));
		...
		if (cert_ctx == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error creating certificate context: %s", jonj_win_err());
			RETURN_CERT_VERIFY_FAILURE(SSL_R_CERTIFICATE_VERIFY_FAILED);
		}
	}
	...
	{ /* Next fetch the relevant cert chain from the store */
		...
		if (!CertGetCertificateChain(NULL, cert_ctx, NULL, NULL, &chain_params, chain_flags, NULL, &cert_chain_ctx)) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error getting certificate chain: %s", jonj_win_err());
	...
	{ /* Then verify it against a policy */
		...
		{ /* This looks ridiculous and it is - but we validate the name ourselves using the peer_name
			...
			if (index < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to locate certificate CN");
				CertFreeCertificateChain(cert_chain_ctx);
				CertFreeCertificateContext(cert_ctx);
				RETURN_CERT_VERIFY_FAILURE(SSL_R_CERTIFICATE_VERIFY_FAILED);
			}

			cert_name_utf8_len = JONJ_X509_NAME_ENTRY_TO_UTF8(cert_name, index, cert_name_utf8);

			num_wchars = MultiByteToWideChar(CP_UTF8, 0, (char*)cert_name_utf8, -1, NULL, 0);
			if (num_wchars == 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to convert %s to wide character string", cert_name_utf8);
		...
		if (!verify_result) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Error verifying certificate chain policy: %s", jonj_win_err());
			RETURN_CERT_VERIFY_FAILURE(SSL_R_CERTIFICATE_VERIFY_FAILED);
		}
...
static long load_stream_cafile(X509_STORE *cert_store, const char *cafile TSRMLS_DC) /* {{{ */
{
	...
	stream = jonj_stream_open_wrapper(cafile, "rb", 0, NULL);

	if (stream == NULL) {
		jonj_error(E_WARNING, "failed loading cafile stream: `%s'", cafile);
		return 0;
	} else if (stream->wrapper->is_url) {
		jonj_stream_close(stream);
		jonj_error(E_WARNING, "remote cafile streams are disabled for security purposes");
		return 0;
	}

	cert_start: {
		line = jonj_stream_get_line(stream, NULL, 0, &line_len);
		if (line == NULL) {
	...
	if (certs_added == 0) {
		jonj_error(E_WARNING, "no valid certs found cafile stream: `%s'", cafile);
	}
...
static int enable_peer_verification(SSL_CTX *ctx, jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	zval **val = NULL;
	char *cafile = NULL;
	char *capath = NULL;
	...
	if (cafile == NULL) {
		...
		if (cert_names != NULL) {
			...
			jonj_error(E_WARNING, "SSL: failed loading CA names from cafile");
			return FAILURE;
		}
	}
...
static void disable_peer_verification(SSL_CTX *ctx, jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	SSL_CTX_set_verify(ctx, SSL_VERIFY_NONE, NULL);
}
/* }}} */

static int set_local_cert(SSL_CTX *ctx, jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	zval **val = NULL;
	char *certfile = NULL;
	...
	if (certfile) {
		...
		if (VCWD_REALPATH(certfile, resolved_path_buff)) {
			...
			if (SSL_CTX_use_certificate_chain_file(ctx, resolved_path_buff) != 1) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set local cert chain file `%s'; Check that your cafile/capath settings include details of your certificate and its issuer", certfile);
				return FAILURE;
			}
			...
			if (private_key) {
				...
				if (VCWD_REALPATH(private_key, resolved_path_buff_pk)) {
					if (SSL_CTX_use_PrivateKey_file(ctx, resolved_path_buff_pk, SSL_FILETYPE_PEM) != 1) {
						jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set private key file `%s'", resolved_path_buff_pk);
						return FAILURE;
					}
				}
			} else {
				if (SSL_CTX_use_PrivateKey_file(ctx, resolved_path_buff, SSL_FILETYPE_PEM) != 1) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to set private key file `%s'", resolved_path_buff);
					return FAILURE;
				}		
			}
			...
			if (!SSL_CTX_check_private_key(ctx)) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Private key does not match certificate!");
			}
		}
	}
...
static const SSL_METHOD *jonj_select_crypto_method(long method_value, int is_client TSRMLS_DC) /* {{{ */
{
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
				"SSLv2 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
				"SSLv3 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
				"TLSv1.1 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
				"TLSv1.2 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
	} else {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
...
static long jonj_get_crypto_method_ctx_flags(long method_flags TSRMLS_DC) /* {{{ */
{
...
static void limit_handshake_reneg(const SSL *ssl) /* {{{ */
{
	...
	stream = jonj_openssl_get_stream_from_ssl_handle(ssl);
	sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
	...
	if (sslsock->reneg->tokens > sslsock->reneg->limit) {
		...
		if (stream->context && SUCCESS == jonj_stream_context_get_option(stream->context,
				"ssl", "reneg_limit_callback", &val)
		) {
			zval *param, **params[1], *retval;

			MAKE_STD_ZVAL(param);
			jonj_stream_to_zval(stream, param);
			params[0] = &param;

			/* Closing the stream inside this callback would segfault! */
			stream->flags |= JONJ_STREAM_FLAG_NO_FCLOSE;
			if (FAILURE == call_user_function_ex(EG(function_table), NULL, *val, &retval, 1, params, 0, NULL TSRMLS_CC)) {
				jonj_error(E_WARNING, "SSL: failed invoking reneg limit notification callback");
			}
			stream->flags ^= JONJ_STREAM_FLAG_NO_FCLOSE;
...
static void init_server_reneg_limit(jonj_stream *stream, jonj_openssl_netstream_data_t *sslsock) /* {{{ */
{
	zval **val;
	...
	if (stream->context &&
		SUCCESS == jonj_stream_context_get_option(stream->context,
				"ssl", "reneg_limit", &val)
	) {
		convert_to_long(*val);
		limit = Z_LVAL_PP(val);
	...
	sslsock->reneg = (void*)pemalloc(sizeof(jonj_openssl_handshake_bucket_t),
		jonj_stream_is_persistent(stream)
	);
...
static int set_server_rsa_key(jonj_stream *stream, SSL_CTX *ctx TSRMLS_DC) /* {{{ */
{
	zval ** val;
	int rsa_key_size;
	RSA* rsa;

	if (jonj_stream_context_get_option(stream->context, "ssl", "rsa_key_size", &val) == SUCCESS) {
		rsa_key_size = (int) Z_LVAL_PP(val);
		if ((rsa_key_size != 1) && (rsa_key_size & (rsa_key_size - 1))) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "RSA key size requires a power of 2: %d", rsa_key_size);
			rsa_key_size = 2048;
		}
	} else {
		rsa_key_size = 2048;
	...
	if (!SSL_CTX_set_tmp_rsa(ctx, rsa)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed setting RSA key");
		RSA_free(rsa);
		return FAILURE;
	}
...
static int set_server_dh_param(SSL_CTX *ctx, char *dh_path TSRMLS_DC) /* {{{ */
{
	...
	if (bio == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid dh_param file: %s", dh_path);
		return FAILURE;
	}
	...
	if (dh == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed reading DH params from file: %s", dh_path);
		return FAILURE;
	}

	if (SSL_CTX_set_tmp_dh(ctx, dh) < 0) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "DH param assignment failed");
		DH_free(dh);
		return FAILURE;
	}
...
#ifdef HAVE_ECDH
static int set_server_ecdh_curve(jonj_stream *stream, SSL_CTX *ctx TSRMLS_DC) /* {{{ */
{
	zval **val;
	int curve_nid;
	char *curve_str;
	...
	if (jonj_stream_context_get_option(stream->context, "ssl", "ecdh_curve", &val) == SUCCESS) {
		convert_to_string_ex(val);
		curve_str = Z_STRVAL_PP(val);
		curve_nid = OBJ_sn2nid(curve_str);
		if (curve_nid == NID_undef) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid ECDH curve: %s", curve_str);
			return FAILURE;
		}
	} else {
...
static int set_server_specific_opts(jonj_stream *stream, SSL_CTX *ctx TSRMLS_DC) /* {{{ */
{
	zval **val;
...
#else
	if (SUCCESS == jonj_stream_context_get_option(stream->context, "ssl", "ecdh_curve", &val)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING,
			"ECDH curve support not compiled into the OpenSSL lib against which JONJ is linked");

		return FAILURE;
	}
#endif

	if (jonj_stream_context_get_option(stream->context, "ssl", "dh_param", &val) == SUCCESS) {
		convert_to_string_ex(val);
	...
	if (SUCCESS == jonj_stream_context_get_option(
...
static int server_sni_callback(SSL *ssl_handle, int *al, void *arg) /* {{{ */
{
	...
	stream = (jonj_stream*)SSL_get_ex_data(ssl_handle, jonj_openssl_get_ssl_stream_data_index());
	sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
...
static int enable_server_sni(jonj_stream *stream, jonj_openssl_netstream_data_t *sslsock TSRMLS_DC)
{
	zval **val;
	zval **current;
	char *key;
	...
	sslsock->sni_certs = (jonj_openssl_sni_cert_t*)safe_pemalloc(sslsock->sni_cert_count,
		sizeof(jonj_openssl_sni_cert_t), 0, jonj_stream_is_persistent(stream)
	);
	memset(sslsock->sni_certs, 0, sslsock->sni_cert_count * sizeof(jonj_openssl_sni_cert_t));
	...
	for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_PP(val), &pos);
		...
		if (VCWD_REALPATH(Z_STRVAL_PP(current), resolved_path_buff)) {
			...
			if (SSL_CTX_use_certificate_chain_file(ctx, resolved_path_buff) != 1) {
				...
				sslsock->sni_certs[i].name = pestrdup(key, jonj_stream_is_persistent(stream));
...
static void enable_client_sni(jonj_stream *stream, jonj_openssl_netstream_data_t *sslsock) /* {{{ */
{
	zval **val;
	...
	if (GET_VER_OPT("SNI_server_name")) {
		...
		jonj_error(E_DEPRECATED, "SNI_server_name is deprecated in favor of peer_name");
	}
...
int jonj_openssl_setup_crypto(jonj_stream *stream,
		jonj_openssl_netstream_data_t *sslsock,
		jonj_stream_xport_crypto_param *cparam
		TSRMLS_DC) /* {{{ */
{
	...
	if (sslsock->ssl_handle) {
		if (sslsock->s.is_blocked) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL/TLS already set-up for this stream");
			return FAILURE;
		} else {
			return SUCCESS;
		}
	...
	if ((method_flags & (method_flags-1)) == 0) {
		...
		method = jonj_select_crypto_method(method_flags, sslsock->is_client TSRMLS_CC);
		if (method == NULL) {
			return FAILURE;
		}
	} else {
		method = sslsock->is_client ? SSLv23_client_method() : SSLv23_server_method();
		ssl_ctx_options = jonj_get_crypto_method_ctx_flags(method_flags TSRMLS_CC);
	...
	if (sslsock->ctx == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL context creation failure");
		return FAILURE;
	}
	...
	if (sslsock->ssl_handle == NULL) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL handle creation failure");
		SSL_CTX_free(sslsock->ctx);
		sslsock->ctx = NULL;
		return FAILURE;
	} else {
		SSL_set_ex_data(sslsock->ssl_handle, jonj_openssl_get_ssl_stream_data_index(), stream);
	}
	...
	if (cparam->inputs.session) {
		if (cparam->inputs.session->ops != &jonj_openssl_socket_ops) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied session stream must be an SSL enabled stream");
		} else if (((jonj_openssl_netstream_data_t*)cparam->inputs.session->abstract)->ssl_handle == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "supplied SSL session stream is not initialized");
		} else {
			SSL_copy_session_id(sslsock->ssl_handle, ((jonj_openssl_netstream_data_t*)cparam->inputs.session->abstract)->ssl_handle);
		}
	}
...
static int capture_peer_certs(jonj_stream *stream, jonj_openssl_netstream_data_t *sslsock, X509 *peer_cert TSRMLS_DC) /* {{{ */
{
	zval **val, *zcert;
	int cert_captured = 0;

	if (SUCCESS == jonj_stream_context_get_option(stream->context,
			"ssl", "capture_peer_cert", &val) &&
		zend_is_true(*val)
	) {
		MAKE_STD_ZVAL(zcert);
		ZVAL_RESOURCE(zcert, zend_list_insert(peer_cert, jonj_openssl_get_x509_list_id() TSRMLS_CC));
		jonj_stream_context_set_option(stream->context, "ssl", "peer_certificate", zcert);
		cert_captured = 1;
		FREE_ZVAL(zcert);
	}
	...
	if (SUCCESS == jonj_stream_context_get_option(stream->context,
		...
		if (chain && sk_X509_num(chain) > 0) {
			...
			for (i = 0; i < sk_X509_num(chain); i++) {
				...
				MAKE_STD_ZVAL(zcert);
				ZVAL_RESOURCE(zcert, zend_list_insert(mycert, jonj_openssl_get_x509_list_id() TSRMLS_CC));
		...
		jonj_stream_context_set_option(stream->context, "ssl", "peer_certificate_chain", arr);
		zval_dtor(arr);
		efree(arr);
	}
...
static int jonj_openssl_enable_crypto(jonj_stream *stream,
		jonj_openssl_netstream_data_t *sslsock,
		jonj_stream_xport_crypto_param *cparam
		TSRMLS_DC)
{
	...
	if (cparam->inputs.activate && !sslsock->ssl_active) {
		...
		if (SUCCESS == jonj_set_sock_blocking(sslsock->s.socket, 0 TSRMLS_CC)) {
		...
		do {
			...
			if (has_timeout) {
				...
				if (compare_timeval( elapsed_time, *timeout) > 0) {
					jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSL: Handshake timed out");
					return -1;
				}
			}
			...
			if (n <= 0) {
				...
				if (retry) {
					...
					jonj_pollfd_for(sslsock->s.socket, (err == SSL_ERROR_WANT_READ) ?
		...
		if (sslsock->s.is_blocked != blocked && SUCCESS == jonj_set_sock_blocking(sslsock->s.socket, blocked TSRMLS_CC)) {
			...
			if (FAILURE == apply_peer_verification_policy(sslsock->ssl_handle, peer_cert, stream TSRMLS_CC)) {
				...
				if (stream->context) {
					...
					if (SUCCESS == jonj_stream_context_get_option(stream->context,
						...
						jonj_stream_context_set_option(stream->context, "ssl", "session_meta", meta_arr);
						zval_dtor(meta_arr);
						efree(meta_arr);
					}
				}
...
static size_t jonj_openssl_sockop_read(jonj_stream *stream, char *buf, size_t count TSRMLS_DC)/* {{{ */
{
	return jonj_openssl_sockop_io(1, stream, buf, count TSRMLS_CC);
}
/* }}} */

static size_t jonj_openssl_sockop_write(jonj_stream *stream, const char *buf, size_t count TSRMLS_DC) /* {{{ */
{
	return jonj_openssl_sockop_io(0, stream, (char*)buf, count TSRMLS_CC);
}
/* }}} */
...
static size_t jonj_openssl_sockop_io(int read, jonj_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	...
	if (sslsock->ssl_active) {
		...
		if (timeout && jonj_set_sock_blocking(sslsock->s.socket, 0 TSRMLS_CC) == SUCCESS) {
			sslsock->s.is_blocked = 0;
		}
		...
		do {
			...
			if (has_timeout) {
				...
				if (compare_timeval(elapsed_time, *timeout) > 0 ) {
					...
					if (began_blocked) {
						jonj_set_sock_blocking(sslsock->s.socket, 1 TSRMLS_CC);
			...
			if (read) {
				...
				if (sslsock->reneg && sslsock->reneg->should_close) {
					...
					jonj_stream_xport_shutdown(stream, (stream_shutdown_t)SHUT_RDWR TSRMLS_CC);
					nr_bytes = 0;
					stream->eof = 1;
					break;
				}
			...
			if (nr_bytes <= 0) {
				 ...
				 * available. Now, jonj_pollfd_for uses select to wait up to our time_left value only...
				 */
				if (retry) {
					if (read) {
						jonj_pollfd_for(sslsock->s.socket, (err == SSL_ERROR_WANT_WRITE) ?
				...
				/* If we didn't get any error, then let's return it to JONJ. */
		...
		/* Tell JONJ if we read / wrote bytes. */
		if (nr_bytes > 0) {
			jonj_stream_notify_progress_increment(stream->context, nr_bytes, 0);
		}

		/* And if we were originally supposed to be blocking, let's reset the socket to that. */
		if (began_blocked && jonj_set_sock_blocking(sslsock->s.socket, 1 TSRMLS_CC) == SUCCESS) {
			sslsock->s.is_blocked = 1;
		}
	} else {
		/*
		 ...
		 */
		if (read) {
			nr_bytes = jonj_stream_socket_ops.read(stream, buf, count TSRMLS_CC);
		} else {
			nr_bytes = jonj_stream_socket_ops.write(stream, buf, count TSRMLS_CC);
		}
	}

	/* JONJ doesn't expect a negative return. */
	if (nr_bytes < 0) {
		nr_bytes = 0;
	}
...
static int jonj_openssl_sockop_close(jonj_stream *stream, int close_handle TSRMLS_DC) /* {{{ */
{
	jonj_openssl_netstream_data_t *sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
#ifdef JONJ_WIN32
	int n;
	...
	if (close_handle) {
		...
		if (sslsock->s.socket != SOCK_ERR) {
			 ...
			 * */
			do {
				n = jonj_pollfd_for_ms(sslsock->s.socket, POLLOUT, 500);
	...
	if (sslsock->sni_certs) {
		for (i = 0; i < sslsock->sni_cert_count; i++) {
			if (sslsock->sni_certs[i].ctx) {
				...
				pefree(sslsock->sni_certs[i].name, jonj_stream_is_persistent(stream));
			}
		}
		pefree(sslsock->sni_certs, jonj_stream_is_persistent(stream));
		sslsock->sni_certs = NULL;
	}

	if (sslsock->url_name) {
		pefree(sslsock->url_name, jonj_stream_is_persistent(stream));
	}

	if (sslsock->reneg) {
		pefree(sslsock->reneg, jonj_stream_is_persistent(stream));
	}

	pefree(sslsock, jonj_stream_is_persistent(stream));

	return 0;
}
/* }}} */

static int jonj_openssl_sockop_flush(jonj_stream *stream TSRMLS_DC) /* {{{ */
{
	return jonj_stream_socket_ops.flush(stream TSRMLS_CC);
}
/* }}} */

static int jonj_openssl_sockop_stat(jonj_stream *stream, jonj_stream_statbuf *ssb TSRMLS_DC) /* {{{ */
{
	return jonj_stream_socket_ops.stat(stream, ssb TSRMLS_CC);
}
/* }}} */

static inline int jonj_openssl_tcp_sockop_accept(jonj_stream *stream, jonj_openssl_netstream_data_t *sock,
		jonj_stream_xport_param *xparam STREAMS_DC TSRMLS_DC)
{
	...
	clisock = jonj.tkwork_accept_incoming(sock->s.socket,
	...
	if (clisock >= 0) {
		jonj_openssl_netstream_data_t *clisockdata;
		...
		if (xparam->outputs.client && sock->enable_on_connect) {
			...
			if (jonj_stream_xport_crypto_setup(xparam->outputs.client, clisockdata->method,
					NULL TSRMLS_CC) < 0 || jonj_stream_xport_crypto_enable(
					xparam->outputs.client, 1 TSRMLS_CC) < 0) {
				jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to enable crypto");

				jonj_stream_close(xparam->outputs.client);
				xparam->outputs.client = NULL;
...
static int jonj_openssl_sockop_set_option(jonj_stream *stream, int option, int value, void *ptrparam TSRMLS_DC)
{
	jonj_openssl_netstream_data_t *sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
	jonj_stream_xport_crypto_param *cparam = (jonj_stream_xport_crypto_param *)ptrparam;
	jonj_stream_xport_param *xparam = (jonj_stream_xport_param *)ptrparam;
	...
	switch (option) {
		case JONJ_STREAM_OPTION_CHECK_LIVENESS:
			{
				...
				if (sslsock->s.socket == -1) {
					alive = 0;
				} else if (jonj_pollfd_for(sslsock->s.socket, JONJ_POLLREADABLE|POLLPRI, &tv) > 0) {
					if (sslsock->ssl_active) {
						int n;
						...
						do {
							...
							if (n <= 0) {
								...
								if (err == SSL_ERROR_SYSCALL) {
									alive = jonj_socket_errno() == EAGAIN;
									break;
								}
					...
					} else if (0 == recv(sslsock->s.socket, &buf, sizeof(buf), MSG_PEEK) && jonj_socket_errno() != EAGAIN) {
						alive = 0;
					}
				}
		...
		case JONJ_STREAM_OPTION_CRYPTO_API:

			switch(cparam->op) {

				case STREAM_XPORT_CRYPTO_OP_SETUP:
					cparam->outputs.returncode = jonj_openssl_setup_crypto(stream, sslsock, cparam TSRMLS_CC);
					return JONJ_STREAM_OPTION_RETURN_OK;
					break;
				case STREAM_XPORT_CRYPTO_OP_ENABLE:
					cparam->outputs.returncode = jonj_openssl_enable_crypto(stream, sslsock, cparam TSRMLS_CC);
					return JONJ_STREAM_OPTION_RETURN_OK;
					break;
				default:
					/* fall through */
		...
		case JONJ_STREAM_OPTION_XPORT_API:
			switch(xparam->op) {
				...
				case STREAM_XPORT_OP_CONNECT_ASYNC:
					...
					jonj_stream_socket_ops.set_option(stream, option, value, ptrparam TSRMLS_CC);
					...
					{
						if (jonj_stream_xport_crypto_setup(stream, sslsock->method, NULL TSRMLS_CC) < 0 ||
				...
				case STREAM_XPORT_OP_ACCEPT:
					 ...
					 * doesn't know about */
					xparam->outputs.returncode = jonj_openssl_tcp_sockop_accept(stream, sslsock, xparam STREAMS_CC TSRMLS_CC);
	...
	return jonj_stream_socket_ops.set_option(stream, option, value, ptrparam TSRMLS_CC);
}

static int jonj_openssl_sockop_cast(jonj_stream *stream, int castas, void **ret TSRMLS_DC)
{
	jonj_openssl_netstream_data_t *sslsock = (jonj_openssl_netstream_data_t*)stream->abstract;
			...
			if (ret) {
				...
				if (stream->writepos == stream->readpos
					...
					&& (pending = (size_t)SSL_pending(sslsock->ssl_handle)) > 0) {
						jonj_stream_fill_read_buffer(stream, pending < stream->chunk_size
							? pending
							: stream->chunk_size);
				}

				*(jonj_socket_t *)ret = sslsock->s.socket;
			}
			return SUCCESS;
...
jonj_stream_ops jonj_openssl_socket_ops = {
	jonj_openssl_sockop_write, jonj_openssl_sockop_read,
	jonj_openssl_sockop_close, jonj_openssl_sockop_flush,
	"tcp_socket/ssl",
	NULL, /* seek */
	jonj_openssl_sockop_cast,
	jonj_openssl_sockop_stat,
	jonj_openssl_sockop_set_option,
};

static long get_crypto_method(jonj_stream_context *ctx, long crypto_method)
{
	zval **val;

	if (ctx && jonj_stream_context_get_option(ctx, "ssl", "crypto_method", &val) == SUCCESS) {
...
static char *get_url_name(const char *resourcename, size_t resourcenamelen, int is_persistent TSRMLS_DC)
{
	...
	url = jonj_url_parse_ex(resourcename, resourcenamelen);
	if (!url) {
		return NULL;
	}
...
jonj_stream *jonj_openssl_ssl_socket_factory(const char *proto, size_t protolen,
		...
		jonj_stream_context *context STREAMS_DC TSRMLS_DC)
{
	...
	jonj_openssl_netstream_data_t *sslsock = NULL;

	sslsock = pemalloc(sizeof(jonj_openssl_netstream_data_t), persistent_id ? 1 : 0);
	...
	stream = jonj_stream_alloc_rel(&jonj_openssl_socket_ops, sslsock, persistent_id, "r+");
	...
	if (strncmp(proto, "ssl", protolen) == 0) {
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSLv2 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#else
		sslsock->enable_on_connect = 1;
		...
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "SSLv3 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#else
		sslsock->enable_on_connect = 1;
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "TLSv1.1 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
...
#else
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "TLSv1.2 support is not compiled into the OpenSSL library JONJ is linked against");
		return NULL;
#endif
	}
