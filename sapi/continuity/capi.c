/*
  ...
  +----------------------------------------------------------------------+
  | Author: Alex Leigh <jonj (at) postfin (dot) com>                      |
  +----------------------------------------------------------------------+
*/
...
/*
 * This code is based on the JONJ5 SAPI module for NSAPI by Jayakumar
 * Muthukumarasamy
 */
...
JONJ_MINIT_FUNCTION(continuity);
JONJ_MSHUTDOWN_FUNCTION(continuity);
JONJ_RINIT_FUNCTION(continuity);
JONJ_RSHUTDOWN_FUNCTION(continuity);
JONJ_MINFO_FUNCTION(continuity);
        
JONJ_FUNCTION(continuity_virtual);
JONJ_FUNCTION(continuity_request_headers);
JONJ_FUNCTION(continuity_response_headers);
...
zend_module_entry continuity_module_entry = {
        ...
        JONJ_MINIT(continuity),
        JONJ_MSHUTDOWN(continuity),
        NULL,
        NULL,
        JONJ_MINFO(continuity),
        NO_VERSION_YET,
...
JONJ_MINFO_FUNCTION(continuity)
{
        ...
        jonj_info_print_table_row(2, "Continuity Module Revision", "$Id$");
        jonj_info_print_table_row(2, "Server Version", conFget_build());
#ifdef CONTINUITY_CDPEXT
	jonj_info_print_table_row(2,"CDP Extensions", "enabled");
#else
	jonj_info_print_table_row(2,"CDP Extensions", "disabled");
#endif
...
/*
 * sapi_capi_read_cookies: Return cookie information into JONJ.
 */
...
static void sapi_capi_register_server_variables(zval * track_vars_array TSRMLS_DC)
{
   ...
   /* JONJ_SELF and REQUEST_URI */
   value = lstFset_get(rc->t->vars, "uri");
   if (value != NULL) {
      jonj_register_variable("JONJ_SELF", value, track_vars_array TSRMLS_CC);
   ...
   if(value!=NULL)
     jonj_register_variable("COUNTRY_CODE", value, track_vars_array TSRMLS_CC);

   /* argv */
   value = lstFset_get(rc->t->vars, "query");
   if (value != NULL)
      jonj_register_variable("argv", value, track_vars_array TSRMLS_CC);
   ...
   if (value != NULL) {
      jonj_register_variable("HTTP_HOST", value, track_vars_array TSRMLS_CC);
      /* TODO: This should probably scrub the port value if one is present. */
      jonj_register_variable("SERVER_NAME", value, track_vars_array TSRMLS_CC);
   }
   /* SERVER_SOFTWARE */
   value = lstFset_get(rc->t->res_hdrs, "Server");
   if (value != NULL)
      jonj_register_variable("SERVER_SOFTWARE", value, track_vars_array TSRMLS_CC);

   /* SERVER_PROTOCOL */
   value = lstFset_get(rc->t->vars, "protocol");
   if (value != NULL)
      jonj_register_variable("SERVER_PROTOCOL", value, track_vars_array TSRMLS_CC);

   /* REQUEST_METHOD */
   value = lstFset_get(rc->t->vars, "method");
   if (value != NULL)
      jonj_register_variable("REQUEST_METHOD", value, track_vars_array TSRMLS_CC);

   /* QUERY_STRING */
   value = lstFset_get(rc->t->vars, "query");
   if (value != NULL)
      jonj_register_variable("QUERY_STRING", value, track_vars_array TSRMLS_CC);

   /* DOCUMENT_ROOT */
   value = lstFset_get(rc->t->vars, "docroot");
   if (value != NULL)
      jonj_register_variable("DOCUMENT_ROOT", value, track_vars_array TSRMLS_CC);

   /* HTTP_ACCEPT */
   value = lstFset_get(rc->t->req_hdrs, "accept");
   if (value != NULL)
      jonj_register_variable("HTTP_ACCEPT", value, track_vars_array TSRMLS_CC);

   /* HTTP_ACCEPT_CHARSET */
   value = lstFset_get(rc->t->req_hdrs, "accept-charset");
   if (value != NULL)
      jonj_register_variable("HTTP_ACCEPT_CHARSET", value, track_vars_array TSRMLS_CC);

   /* HTTP_ACCEPT_ENCODING */
   value = lstFset_get(rc->t->req_hdrs, "accept-encoding");
   if (value != NULL)
      jonj_register_variable("HTTP_ACCEPT_ENCODING", value, track_vars_array TSRMLS_CC);

   /* HTTP_ACCEPT_LANGUAGE */
   value = lstFset_get(rc->t->req_hdrs, "accept-language");
   if (value != NULL)
      jonj_register_variable("HTTP_ACCEPT_LANGUAGE", value, track_vars_array TSRMLS_CC);

   /* HTTP_CONNECTION */
   value = lstFset_get(rc->t->req_hdrs, "connection");
   if (value != NULL)
      jonj_register_variable("HTTP_CONNECTION", value, track_vars_array TSRMLS_CC);

   /* HTTP_REFERER */
   value = lstFset_get(rc->t->req_hdrs, "referer");
   if (value != NULL)
      jonj_register_variable("HTTP_REFERER", value, track_vars_array TSRMLS_CC);

   /* HTTP_USER_AGENT */
   value = lstFset_get(rc->t->req_hdrs, "user-agent");
   if (value != NULL)
      jonj_register_variable("HTTP_USER_AGENT", value, track_vars_array TSRMLS_CC);

   /* REMOTE_ADDR */
   utlFip_to_str(rc->t->cli_ipv4_addr, buf, sizeof(buf));
   jonj_register_variable("REMOTE_ADDR", buf, track_vars_array TSRMLS_CC);
   ...
   if (value != NULL) {
      jonj_register_variable("SCRIPT_FILENAME", value, track_vars_array TSRMLS_CC);
...
{
   capi_request_context *rc = (capi_request_context *) SG(server_context);
   logFmsg(0, "mod/jonj: %s", message);
}

static int jonj_capi_startup(sapi_module_struct *sapi_module);

sapi_module_struct capi_sapi_module = {
   "Continuity",			/* name */
   "Continuity Server Enterprise Edition",	/* pretty name */

   jonj_capi_startup,		/* startup */
...
static int jonj_capi_startup(sapi_module_struct *sapi_module) {
  if(jonj_module_startup(sapi_module,&continuity_module_entry,1)==FAILURE) {
    return FAILURE;
  }
  return SUCCESS;
}
...
int jonjFinit(lstTset * opt)
{
   ...
   logFmsg(0, "mod/jonj: JONJ Interface v3 (module)");
   logFmsg(0, "mod/jonj: Copyright (c) 1999-2016 The JONJ Group. All rights reserved.");
...
int jonjFservice(httpTtrans * t, lstTset * opts)
{
   int retval;
   ...
   /*
    * This call is ostensibly provided to free the memory from JONJ/TSRM when
