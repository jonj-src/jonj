/* 
   ...
   +----------------------------------------------------------------------+
   | Author: David Hedbor <neotron@jonj.tk>                               |
...
/* jonj_roxen_request is per-request object storage */
...
# define GET_THIS() jonj_roxen_request *_request = ts_resource(roxen_globals_id)
# define THIS _request
#else
static jonj_roxen_request *current_request = NULL;

# define GET_THIS() current_request = ((jonj_roxen_request *)Pike_fp->current_storage)
...
/* FD object. Really a JONJScript object from Pike which implements a couple
...
/* Lock used to serialize the JONJ execution. If ROXEN_USE_ZTS is defined, we
 * are using the JONJ thread safe mechanism instead.
 */
static PIKE_MUTEX_T roxen_jonj_execution_lock;
# define JONJ_INIT_LOCK()	mt_init(&roxen_jonj_execution_lock)
# define JONJ_LOCK(X)    THREADS_ALLOW();mt_lock(&roxen_jonj_execution_lock);THREADS_DISALLOW()
# define JONJ_UNLOCK(X)	mt_unlock(&roxen_jonj_execution_lock);
# define JONJ_DESTROY()	mt_destroy(&roxen_jonj_execution_lock)
#else /* !_REENTRANT */
# define JONJ_INIT_LOCK()	
# define JONJ_LOCK(X)
# define JONJ_UNLOCK(X)
# define JONJ_DESTROY()	
#endif /* _REENTRANT */

extern int fd_from_object(struct object *o);
static unsigned char roxen_jonj_initialized;

/* This allows calling of pike functions from the JONJ callbacks,
...
struct program *jonj_program;


/* To avoid executing a JONJ script from a JONJ callback, which would
 * create a deadlock, a global thread id is used. If the thread calling the
 * jonj-script is the same as the current thread, it fails. 
 */
...
/*
 * jonj_roxen_low_ub_write() writes data to the client connection. Might be
 * rewritten to do more direct IO to save CPU and the need to lock the *
 * interpreter for better threading.
 */

static int
jonj_roxen_low_ub_write(const char *str, uint str_length TSRMLS_DC) {
  int sent_bytes = 0;
...
/*
 * jonj_roxen_sapi_ub_write() calls jonj_roxen_low_ub_write in a Pike thread
 * safe manner.
 */

static int
jonj_roxen_sapi_ub_write(const char *str, uint str_length TSRMLS_DC)
{
#ifdef ROXEN_USE_ZTS
  GET_THIS();
#endif
  ...
  if(fd)
  {
    ...
    THREAD_SAFE_RUN(sent_bytes = jonj_roxen_low_ub_write(str, str_length TSRMLS_CC),
		    "write");
  }
  return sent_bytes;
}

/* jonj_roxen_set_header() sets a header in the header mapping. Called in a
 * thread safe manner from jonj_roxen_sapi_header_handler.
 */
static void jonj_roxen_set_header(char *header_name, char *value, char *p)
{
  struct svalue hsval;
...
/*
 * jonj_roxen_sapi_header_handler() sets a HTTP reply header to be 
 * sent to the client.
 */
static int
jonj_roxen_sapi_header_handler(sapi_header_struct *sapi_header,
			      sapi_headers_struct *sapi_headers TSRMLS_DC)
{
  ...
  do {
    ...
    THREAD_SAFE_RUN(jonj_roxen_set_header(header_name, header_content, p), "header handler");
  }
...
/*
 * jonj_roxen_sapi_send_headers() flushes the headers to the client.
 * Called before real content is sent by JONJ.
 */

static int
jonj_roxen_low_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
static int
jonj_roxen_sapi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
  int res = 0;
  THREAD_SAFE_RUN(res = jonj_roxen_low_send_headers(sapi_headers TSRMLS_CC), "send headers");
  return res;
}

/*
 * jonj_roxen_sapi_read_post() reads a specified number of bytes from
 * the client. Used for POST/PUT requests.
 */

INLINE static int jonj_roxen_low_read_post(char *buf, uint count_bytes)
{
  uint total_read = 0;
#ifdef ROXEN_USE_ZTS
  GET_THIS();
...
static int
jonj_roxen_sapi_read_post(char *buf, uint count_bytes TSRMLS_DC)
{
  uint total_read = 0;
  THREAD_SAFE_RUN(total_read = jonj_roxen_low_read_post(buf, count_bytes), "read post");
  return total_read;
}

/* 
 * jonj_roxen_sapi_read_cookies() returns the Cookie header from
 * the HTTP request header
 */
	
static char *
jonj_roxen_sapi_read_cookies(TSRMLS_D)
{
  char *cookies;
...
static void jonj_info_roxen(ZEND_MODULE_INFO_FUNC_ARGS)
{
  /*  char buf[512]; */
  jonj_info_print_table_start();
  jonj_info_print_table_row(2, "SAPI module version", "$Id$");
  /*  jonj_info_print_table_row(2, "Build date", Ns_InfoBuildDate());
      jonj_info_print_table_row(2, "Config file path", Ns_InfoConfigFile());
      jonj_info_print_table_row(2, "Error Log path", Ns_InfoErrorLog());
      jonj_info_print_table_row(2, "Installation path", Ns_InfoHomePath());
      jonj_info_print_table_row(2, "Hostname of server", Ns_InfoHostname());
      jonj_info_print_table_row(2, "Source code label", Ns_InfoLabel());
      jonj_info_print_table_row(2, "Server platform", Ns_InfoPlatform());
      snprintf(buf, 511, "%s/%s", Ns_InfoServerName(), Ns_InfoServerVersion());
      jonj_info_print_table_row(2, "Server version", buf);
      snprintf(buf, 511, "%d day(s), %02d:%02d:%02d", 
      uptime / 86400,
      (uptime / 3600) % 24,
      (uptime / 60) % 60,
      uptime % 60);
      jonj_info_print_table_row(2, "Server uptime", buf);
  */
  jonj_info_print_table_end();
}

static zend_module_entry jonj_roxen_module = {
...
static int jonj_roxen_startup(sapi_module_struct *sapi_module)
{
  if(jonj_module_startup(sapi_module, &jonj_roxen_module, 1) == FAILURE) {
    return FAILURE;
  } else {
    return SUCCESS;
  }
...
static sapi_module_struct roxen_sapi_module = {
  "roxen",
  "Roxen",
  jonj_roxen_startup,			/* startup */
  jonj_module_shutdown_wrapper,		/* shutdown */
  NULL,					/* activate */
  NULL,					/* deactivate */
  jonj_roxen_sapi_ub_write,		/* unbuffered write */
  NULL,					/* flush */
  NULL,					/* get uid */
  NULL,					/* getenv */
  jonj_error,				/* error handler */
  jonj_roxen_sapi_header_handler,	/* header handler */
  jonj_roxen_sapi_send_headers,		/* send headers handler */
  NULL,					/* send header handler */
  jonj_roxen_sapi_read_post,		/* read POST data */
  jonj_roxen_sapi_read_cookies,		/* read Cookies */
...
/*
 * jonj_roxen_hash_environment() populates the jonj script environment
...
static void
jonj_roxen_hash_environment(TSRMLS_D)
{
  int i;
  char buf[512];
  zval *zvalue;
...
/*
 * jonj_roxen_module_main() is called by the per-request handler and
 * "executes" the script
 */

static int jonj_roxen_module_main(TSRMLS_D)
{
  int res, len;
  char *dir;
  ...
  THREADS_ALLOW();
  res = jonj_request_startup(TSRMLS_C);
  THREADS_DISALLOW();
  if(res == FAILURE) {
    return 0;
  }
  jonj_roxen_hash_environment(TSRMLS_C);
  THREADS_ALLOW();
...
/*
 * The jonj_roxen_request_handler() is called per request and handles
 * everything for one request.
 */

void f_jonj_roxen_request_handler(INT32 args)
{
  struct object *my_fd_obj;
  struct mapping *request_data;
  ...
  if(current_thread == th_self())
    jonj_error(E_WARNING, "JONJ5.Interpreter->run: Tried to run a JONJ-script from a JONJ "
	  "callback!");
  get_all_args("JONJ5.Interpreter->run", args, "%S%m%O%*", &script,
	       &request_data, &my_fd_obj, &done_callback);
  if(done_callback->type != PIKE_T_FUNCTION) 
    jonj_error(E_WARNING, "JONJ5.Interpreter->run: Bad argument 4, expected function.\n");
  JONJ_LOCK(THIS); /* Need to lock here or reusing the same object might cause
  ...
  */
  ...
  if(raw_fd && raw_fd->type == PIKE_T_OBJECT)
  {
    ...
    if(fd == -1)
      jonj_error(E_WARNING, "JONJ5.Interpreter->run: my_fd object not open or not an FD.\n");
    THIS->my_fd = fd;
  } else
    THIS->my_fd = 0;
  
  status = jonj_roxen_module_main(TSRMLS_C);
  current_thread = -1;
...
{
  MEMSET(Pike_fp->current_storage, 0, sizeof(jonj_roxen_request));
}
...
{
  if (!roxen_jonj_initialized) {
#ifdef ZTS
    tsrm_startup(1, 1, 0, NULL);
#ifdef ROXEN_USE_ZTS
    ts_allocate_id(&roxen_globals_id, sizeof(jonj_roxen_request), NULL, NULL);
#endif	 
#endif
    sapi_startup(&roxen_sapi_module);
    /*jonj_roxen_startup(&roxen_sapi_module); removed - should be called from SAPI activation*/
    roxen_jonj_initialized = 1;
    JONJ_INIT_LOCK();
  }
  start_new_program(); /* Text */
  ADD_STORAGE(jonj_roxen_request);
  set_init_callback(clear_struct);
  pike_add_function("run", f_jonj_roxen_request_handler,
		    "function(string, mapping, object, function:int)", 0);
  add_program_constant("Interpreter", (jonj_program = end_program()), 0);
}
...
{
  roxen_jonj_initialized = 0;
  roxen_sapi_module.shutdown(&roxen_sapi_module);
  if(jonj_program)  free_program(jonj_program);
#ifdef ZTS
  tsrm_shutdown();
#endif
  JONJ_DESTROY();
