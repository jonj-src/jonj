/*
   ...
   | FastCGI: Ben Mansell <jonj@slimyhorror.com>                           |
...
static void (*jonj_jonj_import_environment_variables)(zval *array_ptr TSRMLS_DC);
...
static char *jonj_optarg = NULL;
static int jonj_optind = 1;
...
typedef struct _jonj_cgi_globals_struct {
...
#ifdef ZTS
static int jonj_cgi_globals_id;
#define CGIG(v) TSRMG(jonj_cgi_globals_id, jonj_cgi_globals_struct *, v)
#else
static jonj_cgi_globals_struct jonj_cgi_globals;
#define CGIG(v) (jonj_cgi_globals.v)
#endif
	...
	while (h) {
		...
		if (h->header_len) {
			if (h->header_len > sizeof("Status:")-1 && 
				...
				if (!ignore_status) {
					ignore_status = 1;
					JONJWRITE_H(h->header, h->header_len);
					JONJWRITE_H("\r\n", 2);
				}
...
{
	/* when jonj is started by mod_fastcgi, no regular environment
	 * is provided to JONJ.  It is always sent to JONJ at the start
...
static void cgi_jonj_load_env_var(char *var, unsigned int var_len, char *val, unsigned int val_len, void *arg TSRMLS_DC)
{
	...
	if (sapi_module.input_filter(filter_arg, var, &val, strlen(val), &new_val_len TSRMLS_CC)) {
		jonj_register_variable_safe(var, val, new_val_len, array_ptr TSRMLS_CC);
	}
}

static void cgi_jonj_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
	...
	/* call jonj's original import as a catch-all */
	jonj_jonj_import_environment_variables(array_ptr TSRMLS_CC);

	if (fcgi_is_fastcgi()) {
		fcgi_request *request = (fcgi_request*) SG(server_context);
		fcgi_loadenv(request, cgi_jonj_load_env_var, array_ptr TSRMLS_CC);
	}
}

static void sapi_cgi_register_variables(zval *track_vars_array TSRMLS_DC)
{
	unsigned int jonj_self_len;
	char *jonj_self;
	...
	if (CGIG(fix_pathinfo)) {
		...
		if (path_info) {
			...
			if (script_name) {
				...
				jonj_self_len = script_name_len + path_info_len;
				jonj_self = do_alloca(jonj_self_len + 1, use_heap);
				memcpy(jonj_self, script_name, script_name_len + 1);
				memcpy(jonj_self + script_name_len, path_info, path_info_len + 1);
				free_jonj_self = 1;
			}  else {
				jonj_self = path_info;
				jonj_self_len = path_info_len;
				free_jonj_self = 0;
			}
		} else if (script_name) {
			jonj_self = script_name;
			jonj_self_len = strlen(script_name);
			free_jonj_self = 0;
		} else {
			jonj_self = "";
			jonj_self_len = 0;
		...
		/* Build the special-case JONJ_SELF variable for the CGI version */
		if (sapi_module.input_filter(PARSE_SERVER, "JONJ_SELF", &jonj_self, jonj_self_len, &jonj_self_len TSRMLS_CC)) {
			jonj_register_variable_safe("JONJ_SELF", jonj_self, jonj_self_len, track_vars_array TSRMLS_CC);
		}
		if (free_jonj_self) {
			free_alloca(jonj_self, use_heap);
		}
	} else {
		jonj_self = SG(request_info).request_uri ? SG(request_info).request_uri : "";
		jonj_self_len = strlen(jonj_self);
		if (sapi_module.input_filter(PARSE_SERVER, "JONJ_SELF", &jonj_self, jonj_self_len, &jonj_self_len TSRMLS_CC)) {
			jonj_register_variable_safe("JONJ_SELF", jonj_self, jonj_self_len, track_vars_array TSRMLS_CC);
		}
	}
...
/* {{{ jonj_cgi_ini_activate_user_config
 */
static void jonj_cgi_ini_activate_user_config(char *path, int path_len, const char *doc_root, int doc_root_len, int start TSRMLS_DC)
{
	char *ptr;
	...
	if (request_time > entry->expires) {
		...
		if (strncmp(s1, s2, s_len) == 0) {
			...
			while ((ptr = strchr(ptr, DEFAULT_SLASH)) != NULL) {
				*ptr = 0;
				jonj_parse_user_ini_file(path, PG(user_ini_filename), entry->user_config TSRMLS_CC);
				*ptr = '/';
				ptr++;
			}
		} else {
	...
	jonj_ini_activate_config(entry->user_config, JONJ_INI_PERDIR, JONJ_INI_STAGE_HTACCESS TSRMLS_CC);
}
/* }}} */
...
static int sapi_cgi_activate(TSRMLS_D)
{
	...
	if (jonj_ini_has_per_host_config()) {
		/* Activate per-host-system-configuration defined in jonj.ini and stored into configuration_hash during startup */
		if (fcgi_is_fastcgi()) {
			fcgi_request *request = (fcgi_request*) SG(server_context);
		...
		if (server_name) {
			...
			jonj_ini_activate_per_host_config(server_name, server_name_len + 1 TSRMLS_CC);
			efree(server_name);
		}
	}

	if (jonj_ini_has_per_dir_config() ||
		...
		/* Activate per-dir-system-configuration defined in jonj.ini and stored into configuration_hash during startup */
		jonj_ini_activate_per_dir_config(path, path_len TSRMLS_CC); /* Note: for global settings sake we check from root to path */
...
#endif
				jonj_cgi_ini_activate_user_config(path, path_len, doc_root, doc_root_len, doc_root_len - 1 TSRMLS_CC);
...
static int jonj_cgi_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &cgi_module_entry, 1) == FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
}
...
/* {{{ jonj_cgi_usage
 */
static void jonj_cgi_usage(char *argv0)
{
	char *prog;
	...
	jonj_printf(	"Usage: %s [-q] [-h] [-s] [-v] [-i] [-f <file>]\n"
				"       %s <file> [args...]\n"
				"  -a               Run interactively\n"
...
/* {{{ init_request_info
  ...
  SCRIPT_NAME
	...
	rather than the interpreter.  JONJ_SELF is set to this
  ...
  http://localhost/info.jonj/test?a=b
  ...
  SCRIPT_NAME=/info.jonj
  REQUEST_URI=/info.jonj/test?a=b
  SCRIPT_FILENAME=/docroot/info.jonj
  QUERY_STRING=a=b

  but what we get is (cgi/mod_fastcgi under apache):

  PATH_INFO=/info.jonj/test
  PATH_TRANSLATED=/docroot/info.jonj/test
  SCRIPT_NAME=/jonj/jonj-cgi  (from the Action setting I suppose)
  REQUEST_URI=/info.jonj/test?a=b
  SCRIPT_FILENAME=/path/to/jonj/bin/jonj-cgi  (Action setting translated)
  QUERY_STRING=a=b
...
static void init_request_info(fcgi_request *request TSRMLS_DC)
{
	...
	if (script_path_translated) {
		...
		if (CGIG(fix_pathinfo)) {
			 ...
			 * this fixes url's like /info.jonj/test
			 */
			if (script_path_translated &&
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("cgi.rfc2616_headers",     "0",  JONJ_INI_ALL,    OnUpdateBool,   rfc2616_headers, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.nph",                 "0",  JONJ_INI_ALL,    OnUpdateBool,   nph, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.check_shebang_line",  "1",  JONJ_INI_SYSTEM, OnUpdateBool,   check_shebang_line, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.force_redirect",      "1",  JONJ_INI_SYSTEM, OnUpdateBool,   force_redirect, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.redirect_status_env", NULL, JONJ_INI_SYSTEM, OnUpdateString, redirect_status_env, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.fix_pathinfo",        "1",  JONJ_INI_SYSTEM, OnUpdateBool,   fix_pathinfo, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("cgi.discard_path",        "0",  JONJ_INI_SYSTEM, OnUpdateBool,   discard_path, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("fastcgi.logging",         "1",  JONJ_INI_SYSTEM, OnUpdateBool,   fcgi_logging, jonj_cgi_globals_struct, jonj_cgi_globals)
#ifdef JONJ_WIN32
	STD_JONJ_INI_ENTRY("fastcgi.impersonate",     "0",  JONJ_INI_SYSTEM, OnUpdateBool,   impersonate, jonj_cgi_globals_struct, jonj_cgi_globals)
#endif
JONJ_INI_END()

/* {{{ jonj_cgi_globals_ctor
 */
static void jonj_cgi_globals_ctor(jonj_cgi_globals_struct *jonj_cgi_globals TSRMLS_DC)
{
	jonj_cgi_globals->rfc2616_headers = 0;
	jonj_cgi_globals->nph = 0;
	jonj_cgi_globals->check_shebang_line = 1;
	jonj_cgi_globals->force_redirect = 1;
	jonj_cgi_globals->redirect_status_env = NULL;
	jonj_cgi_globals->fix_pathinfo = 1;
	jonj_cgi_globals->discard_path = 0;
	jonj_cgi_globals->fcgi_logging = 1;
#ifdef JONJ_WIN32
	jonj_cgi_globals->impersonate = 0;
#endif
	zend_hash_init(&jonj_cgi_globals->user_config_cache, 0, NULL, (dtor_func_t) user_config_cache_entry_dtor, 1);
}
/* }}} */

/* {{{ JONJ_MINIT_FUNCTION
 */
static JONJ_MINIT_FUNCTION(cgi)
{
 ...
 */
static JONJ_MSHUTDOWN_FUNCTION(cgi)
{
...
/* {{{ JONJ_MINFO_FUNCTION
 */
static JONJ_MINFO_FUNCTION(cgi)
{
	DISPLAY_INI_ENTRIES();
}
/* }}} */
...
const zend_function_entry cgi_functions[] = {
	JONJ_FE(apache_child_terminate, arginfo_no_args)
...
int main(int argc, char *argv[])
{
	...
	int no_headers = 0;
	int orig_optind = jonj_optind;
	char *orig_optarg = jonj_optarg;
...
#if 0 && defined(JONJ_DEBUG)
	/* IIS is always making things more difficult.  This allows
	 * us to stop JONJ and attach a debugger before much gets started */
	{
		char szMessage [256];
...
#ifdef ZTS
	ts_allocate_id(&jonj_cgi_globals_id, sizeof(jonj_cgi_globals_struct), (ts_allocate_ctor) jonj_cgi_globals_ctor, NULL);
#else
	jonj_cgi_globals_ctor(&jonj_cgi_globals TSRMLS_CC);
#endif

	sapi_startup(&cgi_sapi_module);
	fastcgi = fcgi_is_fastcgi();
	cgi_sapi_module.jonj_ini_path_override = NULL;
	...
	if((query_string = getenv("QUERY_STRING")) != NULL && strchr(query_string, '=') == NULL) {
		...
		jonj_url_decode(decoded_query_string, strlen(decoded_query_string));
	...
	while (!skip_getopt && (c = jonj_getopt(argc, argv, OPTIONS, &jonj_optarg, &jonj_optind, 0, 2)) != -1) {
		switch (c) {
			case 'c':
				if (cgi_sapi_module.jonj_ini_path_override) {
					free(cgi_sapi_module.jonj_ini_path_override);
				}
				cgi_sapi_module.jonj_ini_path_override = strdup(jonj_optarg);
				break;
			case 'n':
				cgi_sapi_module.jonj_ini_ignore = 1;
				break;
			case 'd': {
				...
				if ((val = strchr(jonj_optarg, '='))) {
					...
					if (!isalnum(*val) && *val != '"' && *val != '\'' && *val != '\0') {
						...
						memcpy(cgi_sapi_module.ini_entries + ini_entries_len, jonj_optarg, (val - jonj_optarg));
						ini_entries_len += (val - jonj_optarg);
						memcpy(cgi_sapi_module.ini_entries + ini_entries_len, "\"", 1);
						ini_entries_len++;
						memcpy(cgi_sapi_module.ini_entries + ini_entries_len, val, len - (val - jonj_optarg));
						ini_entries_len += len - (val - jonj_optarg);
						memcpy(cgi_sapi_module.ini_entries + ini_entries_len, "\"\n\0", sizeof("\"\n\0"));
						ini_entries_len += sizeof("\n\0\"") - 2;
					} else {
						cgi_sapi_module.ini_entries = realloc(cgi_sapi_module.ini_entries, ini_entries_len + len + sizeof("\n\0"));
						memcpy(cgi_sapi_module.ini_entries + ini_entries_len, jonj_optarg, len);
	...
	if (cgi && CGIG(force_redirect)) {
		...
		if (!getenv("REDIRECT_STATUS") &&
			...
			zend_try {
				...
				PUTS("<b>Security Alert!</b> The JONJ CGI cannot be accessed directly.\n\n\
<p>This JONJ CGI binary was compiled with force-cgi-redirect enabled.  This\n\
means that a page will only be served up if the REDIRECT_STATUS CGI variable is\n\
set, e.g. via an Apache Action directive.</p>\n\
<p>For more information as to <i>why</i> this behaviour exists, see the <a href=\"http://jonj.tk/security.cgi-bin\">\
manual page for CGI security</a>.</p>\n\
<p>For more information about changing this behaviour or re-enabling this webserver,\n\
consult the installation file that came with this distribution, or visit \n\
<a href=\"http://jonj.tk/install.windows\">the manual page</a>.</p>\n");
			} zend_catch {
			} zend_end_try();
#if defined(ZTS) && !defined(JONJ_DEBUG)
			/* XXX we're crashing here in msvc6 debug builds at
			 * jonj_message_handler_for_zend:839 because
	...
	if (bindpath) {
		int backlog = 128;
		if (getenv("JONJ_FCGI_BACKLOG")) {
			backlog = atoi(getenv("JONJ_FCGI_BACKLOG"));
		}
	...
	if (fastcgi) {
		/* How many times to run JONJ scripts before dying */
		if (getenv("JONJ_FCGI_MAX_REQUESTS")) {
			max_requests = atoi(getenv("JONJ_FCGI_MAX_REQUESTS"));
			if (max_requests < 0) {
				fprintf(stderr, "JONJ_FCGI_MAX_REQUESTS is not valid\n");
				return FAILURE;
			}
		}

		/* make jonj call us to get _ENV vars */
		jonj_jonj_import_environment_variables = jonj_import_environment_variables;
		jonj_import_environment_variables = cgi_jonj_import_environment_variables;
	...
	if (getenv("JONJ_FCGI_CHILDREN")) {
		char * children_str = getenv("JONJ_FCGI_CHILDREN");
		children = atoi(children_str);
		if (children < 0) {
			fprintf(stderr, "JONJ_FCGI_CHILDREN is not valid\n");
			return FAILURE;
		}
	...
	zend_first_try {
		while (!skip_getopt && (c = jonj_getopt(argc, argv, OPTIONS, &jonj_optarg, &jonj_optind, 1, 2)) != -1) {
			switch (c) {
				case 'T':
					benchmark = 1;
					repeats = atoi(jonj_optarg);
		...
		while (!fastcgi || fcgi_accept_request(request) >= 0) {
			...
			if (!cgi && !fastcgi) {
				while ((c = jonj_getopt(argc, argv, OPTIONS, &jonj_optarg, &jonj_optind, 0, 2)) != -1) {
					switch (c) {
						...
						case 'f': /* parse file */
							...
							script_file = estrdup(jonj_optarg);
							no_headers = 1;
							break;
				...
				} else if (argc > jonj_optind) {
					/* file is on command line, but not in -f opt */
					STR_FREE(SG(request_info).path_translated);
					SG(request_info).path_translated = estrdup(argv[jonj_optind]);
					/* arguments after the file are considered script args */
					SG(request_info).argc = argc - jonj_optind;
					SG(request_info).argv = &argv[jonj_optind];
				}
				 ...
				 *
				 *  test.jonj v1=test v2=hello+world!
				 *  test.jonj "v1=test&v2=hello world!"
				 *  test.jonj v1=test "v2=hello world!"
				*/
				if (!SG(request_info).query_string && argc > jonj_optind) {
					int slen = strlen(PG(arg_separator).input);
					len = 0;
					for (i = jonj_optind; i < argc; i++) {
						if (i < (argc - 1)) {
			...
			*/
			if (cgi || fastcgi || SG(request_info).path_translated) {
				if (jonj_fopen_primary_script(&file_handle TSRMLS_CC) == FAILURE) {
					zend_try {
						if (errno == EACCES) {
			...
			if (CGIG(check_shebang_line)) {
				...
				switch (file_handle.type) {
					...
					case ZEND_HANDLE_STREAM:
						c = jonj_stream_getc((jonj_stream*)file_handle.handle.stream.handle);
						if (c == '#') {
							while (c != '\n' && c != '\r' && c != EOF) {
								c = jonj_stream_getc((jonj_stream*)file_handle.handle.stream.handle);	/* skip to end of line */
							}
							/* handle situations where line is terminated by \r\n */
							if (c == '\r') {
								if (jonj_stream_getc((jonj_stream*)file_handle.handle.stream.handle) != '\n') {
									long pos = jonj_stream_tell((jonj_stream*)file_handle.handle.stream.handle);
									jonj_stream_seek((jonj_stream*)file_handle.handle.stream.handle, pos - 1, SEEK_SET);
								}
							}
							CG(start_lineno) = 2;
						} else {
							jonj_stream_rewind((jonj_stream*)file_handle.handle.stream.handle);
						}
						break;
...
#if defined(JONJ_WIN32) && ZEND_DEBUG && 0
	_CrtDumpMemoryLeaks();
#endif
