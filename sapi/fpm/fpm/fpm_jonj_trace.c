	...
	/* $Id: fpm_jonj_trace.c,v 1.27.2.1 2008/11/15 00:57:24 anight Exp $ */
...
static int fpm_jonj_trace_dump(struct fpm_child_s *child, FILE *slowlog TSRMLS_DC) /* {{{ */
{
...
void fpm_jonj_trace(struct fpm_child_s *child) /* {{{ */
{
	TSRMLS_FETCH();
	...
	if (0 > fpm_jonj_trace_dump(child, slowlog TSRMLS_CC)) {
