...
/* 
 * Please keep the same order as in fpm_conf.h and in jonj-fpm.conf.in
 */
...
int fpm_worker_pool_config_free(struct fpm_worker_pool_config_s *wpc) /* {{{ */
{
	...
	for (kv = wpc->jonj_values; kv; kv = kv_next) {
		kv_next = kv->next;
		free(kv->key);
		free(kv->value);
		free(kv);
	}
	for (kv = wpc->jonj_admin_values; kv; kv = kv_next) {
		kv_next = kv->next;
		free(kv->key);
		free(kv->value);
		free(kv);
...
static int fpm_evaluate_full_path(char **path, struct fpm_worker_pool_s *wp, char *default_prefix, int expand) /* {{{ */
{
	...
	if (prefix == NULL) {
		prefix = default_prefix ? default_prefix : JONJ_PREFIX;
	}
...
static int fpm_conf_process_all_pools() /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		if (!wp->config->security_limit_extensions) {
			wp->config->security_limit_extensions = strdup(".jonj .phar");
		}
		...
		/* env[], jonj_value[], jonj_admin_values[] */
		if (!wp->config->chroot) {
			struct key_value_s *kv;
			char *options[] = FPM_JONJ_INI_TO_EXPAND;
			char **p;
...
static int fpm_conf_post_process(int force_daemon TSRMLS_DC) /* {{{ */
{
	...
	if (fpm_global_config.pid_file) {
		fpm_evaluate_full_path(&fpm_global_config.pid_file, NULL, JONJ_LOCALSTATEDIR, 0);
	}
	...
	if (!fpm_global_config.error_log) {
		fpm_global_config.error_log = strdup("log/jonj-fpm.log");
	}
	...
	if (!fpm_global_config.syslog_ident) {
		fpm_global_config.syslog_ident = strdup("jonj-fpm");
	}
...
#endif
	{
		fpm_evaluate_full_path(&fpm_global_config.error_log, NULL, JONJ_LOCALSTATEDIR, 0);
	}
...
static void fpm_conf_ini_parser_array(zval *name, zval *key, zval *value, void *arg TSRMLS_DC) /* {{{ */
{
	...
	} else if (!strcmp("jonj_value", Z_STRVAL_P(name))) {
		config = (char *)current_wp->config + WPO(jonj_values);
		err = fpm_conf_set_array(key, value, &config, 0);

	} else if (!strcmp("jonj_admin_value", Z_STRVAL_P(name))) {
		config = (char *)current_wp->config + WPO(jonj_admin_values);
		err = fpm_conf_set_array(key, value, &config, 0);

	} else if (!strcmp("jonj_flag", Z_STRVAL_P(name))) {
		config = (char *)current_wp->config + WPO(jonj_values);
		err = fpm_conf_set_array(key, value, &config, 1);

	} else if (!strcmp("jonj_admin_flag", Z_STRVAL_P(name))) {
		config = (char *)current_wp->config + WPO(jonj_admin_values);
		err = fpm_conf_set_array(key, value, &config, 1);
...
static void fpm_conf_dump() /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		for (kv = wp->config->jonj_values; kv; kv = kv->next) {
			zlog(ZLOG_NOTICE, "\tjonj_value[%s] = %s", kv->key, kv->value);
		}

		for (kv = wp->config->jonj_admin_values; kv; kv = kv->next) {
			zlog(ZLOG_NOTICE, "\tjonj_admin_value[%s] = %s", kv->key, kv->value);
		}
		zlog(ZLOG_NOTICE, " ");
	}
}
...
int fpm_conf_init_main(int test_conf, int force_daemon) /* {{{ */
{
	...
	if (fpm_globals.config == NULL) {
		...
		if (fpm_globals.prefix == NULL) {
			spprintf(&tmp, 0, "%s/jonj-fpm.conf", JONJ_SYSCONFDIR);
		} else {
			spprintf(&tmp, 0, "%s/etc/jonj-fpm.conf", fpm_globals.prefix);
		}
