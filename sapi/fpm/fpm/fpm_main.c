		...
		if (request_body_fd == -1) {
			...
			if (request_body_filename && *request_body_filename) {
				...
				if (0 > request_body_fd) {
					jonj_error(E_WARNING, "REQUEST_BODY_FILE: open('%s') failed: %s (%d)",
...
void cgi_jonj_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
...
static void sapi_cgi_register_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	if (CGIG(fix_pathinfo)) {
		...
		jonj_self = emalloc(jonj_self_len + 1);

		/* Concat script_name and path_info into jonj_self */
		if (script_name) {
...
void sapi_cgi_log_fastcgi(int level, char *message, size_t len)
{
	...
	/* ensure we want:
	 * - to log (fastcgi.logging in jonj.ini)
...
{
	zlog(ZLOG_NOTICE, "JONJ message: %s", message);
}
/* }}} */
...
static void jonj_cgi_usage(char *argv0)
{
	...
	jonj_printf(	"Usage: %s [-n] [-e] [-h] [-i] [-m] [-v] [-t] [-p <prefix>] [-g <pid>] [-c <file>] [-d foo[=bar]] [-y <file>] [-D] [-F [-O]]\n"
...
#define APACHE_PROXY_BALANCER_PREFIX "proxy:balancer://"
		 ...
		 *     proxy:fcgi://localhost:9000/some-dir/info.jonj/test?foo=bar
		 *     proxy:balancer://localhost:9000/some-dir/info.jonj/test?foo=bar
		 * should be changed to:
		 *     /some-dir/info.jonj/test
		 * See: http://bugs.jonj.tk/bug.jonj?id=54152
		 *      http://bugs.jonj.tk/bug.jonj?id=62172
	...
	/* INI stuff */
	ini = sapi_cgibin_getenv("JONJ_VALUE", sizeof("JONJ_VALUE") - 1 TSRMLS_CC);
	if (ini) {
	...
	ini = sapi_cgibin_getenv("JONJ_ADMIN_VALUE", sizeof("JONJ_ADMIN_VALUE") - 1 TSRMLS_CC);
	if (ini) {
...
static void fastcgi_ini_parser(zval *arg1, zval *arg2, zval *arg3, int callback_type, void *arg TSRMLS_DC) /* {{{ */
{
	...
	kv.value = value;
	kv.next = NULL;
	if (fpm_jonj_apply_defines_ex(&kv, *mode) == -1) {
...
JONJ_INI_BEGIN()
	...
	STD_JONJ_INI_ENTRY("fastcgi.error_header",    NULL, JONJ_INI_SYSTEM, OnUpdateString, error_header, jonj_cgi_globals_struct, jonj_cgi_globals)
	STD_JONJ_INI_ENTRY("fpm.config",    NULL, JONJ_INI_SYSTEM, OnUpdateString, fpm_config, jonj_cgi_globals_struct, jonj_cgi_globals)
JONJ_INI_END()
...
static void jonj_cgi_globals_ctor(jonj_cgi_globals_struct *jonj_cgi_globals TSRMLS_DC)
{
	...
	jonj_cgi_globals->error_header = NULL;
	jonj_cgi_globals->fpm_config = NULL;
}
/* }}} */
...
static JONJ_MINFO_FUNCTION(cgi)
{
	...
	jonj_info_print_table_row(2, "jonj-fpm", "active");
...
JONJ_FUNCTION(fastcgi_finish_request) /* {{{ */
{
	fcgi_request *request = (fcgi_request*) SG(server_context);
...
static const zend_function_entry cgi_fcgi_sapi_functions[] = {
	JONJ_FE(fastcgi_finish_request,              NULL)
	{NULL, NULL, NULL}
};
...
int main(int argc, char *argv[])
{
	...
	int force_daemon = -1;
	int force_stderr = 0;
	int jonj_information = 0;
	int jonj_allow_to_run_as_root = 0;
	...
	cgi_sapi_module.jonj_ini_ignore_cwd = 1;
	...
	while ((c = jonj_getopt(argc, argv, OPTIONS, &jonj_optarg, &jonj_optind, 0, 2)) != -1) {
		switch (c) {
			...
			case 'y':
				fpm_config = jonj_optarg;
				break;

			case 'p':
				fpm_prefix = jonj_optarg;
				break;
	...
	if (jonj_information) {
		cgi_sapi_module.jonjinfo_as_text = 1;
		cgi_sapi_module.startup(&cgi_sapi_module);
		if (jonj_request_startup(TSRMLS_C) == FAILURE) {
			SG(server_context) = NULL;
			jonj_module_shutdown(TSRMLS_C);
...
<a href=\"http://jonj.tk/install.windows\">the manual page</a>.</p>\n");
	...
	if (0 > fpm_init(argc, argv, fpm_config ? fpm_config : CGIG(fpm_config), fpm_prefix, fpm_pid, test_conf, jonj_allow_to_run_as_root, force_daemon, force_stderr)) {
	...
	zend_first_try {
		while (fcgi_accept_request(&request) >= 0) {
			...
			if (fpm_jonj_limit_extensions(SG(request_info).path_translated)) {
			 ...
			 * jonj_fopen_primary_script seems to delete SG(request_info).path_translated on failure
			 */
