	...
	/* $Id: fpm_jonj.c,v 1.22.2.4 2008/12/13 03:21:18 anight Exp $ */
...
static int fpm_jonj_zend_ini_alter_master(char *name, int name_length, char *new_value, int new_value_length, int mode, int stage TSRMLS_DC) /* {{{ */
{
...
static void fpm_jonj_disable(char *value, int (*zend_disable)(char *, uint TSRMLS_DC) TSRMLS_DC) /* {{{ */
{
	char *s = 0, *e = value;
...
int fpm_jonj_apply_defines_ex(struct key_value_s *kv, int mode) /* {{{ */
{
	...
	if (!strcmp(name, "extension") && *value) {
		zval zv;
		jonj_dl(value, MODULE_PERSISTENT, &zv, 1 TSRMLS_CC);
		return Z_BVAL(zv) ? 1 : -1;
	}

	if (fpm_jonj_zend_ini_alter_master(name, name_len+1, value, value_len, mode, JONJ_INI_STAGE_ACTIVATE TSRMLS_CC) == FAILURE) {
		return -1;
	}
	...
	if (!strcmp(name, "disable_functions") && *value) {
		...
		fpm_jonj_disable(v, zend_disable_function TSRMLS_CC);
		return 1;
	}
	...
	if (!strcmp(name, "disable_classes") && *value) {
		...
		fpm_jonj_disable(v, zend_disable_class TSRMLS_CC);
		return 1;
	}
...
static int fpm_jonj_apply_defines(struct fpm_worker_pool_s *wp) /* {{{ */
{
	struct key_value_s *kv;

	for (kv = wp->config->jonj_values; kv; kv = kv->next) {
		if (fpm_jonj_apply_defines_ex(kv, ZEND_INI_USER) == -1) {
			zlog(ZLOG_ERROR, "Unable to set jonj_value '%s'", kv->key);
		}
	}

	for (kv = wp->config->jonj_admin_values; kv; kv = kv->next) {
		if (fpm_jonj_apply_defines_ex(kv, ZEND_INI_SYSTEM) == -1) {
			zlog(ZLOG_ERROR, "Unable to set jonj_admin_value '%s'", kv->key);
		}
	}
...
static int fpm_jonj_set_allowed_clients(struct fpm_worker_pool_s *wp) /* {{{ */
{
...
static int fpm_jonj_set_fcgi_mgmt_vars(struct fpm_worker_pool_s *wp) /* {{{ */
{
...
static void fpm_jonj_cleanup(int which, void *arg) /* {{{ */
{
	TSRMLS_FETCH();
...
{
	if (0 > fpm_cleanup_add(FPM_CLEANUP_PARENT, fpm_jonj_cleanup, 0)) {
		return -1;
	}
	return 0;
}
...
{
	if (0 > fpm_jonj_apply_defines(wp) ||
		0 > fpm_jonj_set_allowed_clients(wp)) {
		return -1;
	}
