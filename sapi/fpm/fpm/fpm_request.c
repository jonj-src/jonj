...
void fpm_request_info() /* {{{ */
{
	TSRMLS_FETCH();
	struct fpm_scoreboard_proc_s *proc;
	char *request_uri = fpm_jonj_request_uri(TSRMLS_C);
	char *request_method = fpm_jonj_request_method(TSRMLS_C);
	char *script_filename = fpm_jonj_script_filename(TSRMLS_C);
	char *query_string = fpm_jonj_query_string(TSRMLS_C);
	char *auth_user = fpm_jonj_auth_user(TSRMLS_C);
	size_t content_length = fpm_jonj_content_length(TSRMLS_C);
	struct timeval now;
