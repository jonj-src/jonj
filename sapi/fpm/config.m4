...
if test "$JONJ_FPM" != "no"; then
  AC_MSG_RESULT($JONJ_FPM)
  ...
  JONJ_ARG_WITH(fpm-user,,
  [  --with-fpm-user[=USER]    Set the user for jonj-fpm to run as. (default: nobody)], nobody, no)

  JONJ_ARG_WITH(fpm-group,,
  [  --with-fpm-group[=GRP]    Set the group for jonj-fpm to run as. For a system user, this 
                          should usually be set to match the fpm username (default: nobody)], nobody, no)

  JONJ_ARG_WITH(fpm-systemd,,
  [  --with-fpm-systemd      Activate systemd integration], no, no)

  JONJ_ARG_WITH(fpm-acl,,
  [  --with-fpm-acl          Use POSIX Access Control Lists], no, no)

  if test "$JONJ_FPM_SYSTEMD" != "no" ; then
    ...
    if test $HAVE_SD_DAEMON_H = "no" || test -z "${SYSTEMD_LIBS}"; then
      ...
      JONJ_FPM_SD_FILES="fpm/fpm_systemd.c"
      JONJ_EVAL_LIBLINE($SYSTEMD_LIBS)
      JONJ_EVAL_INCLINE($SYSTEMD_INCS)
      jonj_fpm_systemd=notify
    fi
  else
    jonj_fpm_systemd=simple
  fi

  if test "$JONJ_FPM_ACL" != "no" ; then
    AC_CHECK_HEADERS([sys/acl.h])
    AC_CHECK_LIB(acl, acl_free, [
      JONJ_ADD_LIBRARY(acl)
  ...
  JONJ_SUBST_OLD(jonj_fpm_systemd)
  AC_DEFINE_UNQUOTED(JONJ_FPM_SYSTEMD, "$jonj_fpm_systemd", [fpm systemd service type])

  if test -z "$JONJ_FPM_USER" -o "$JONJ_FPM_USER" = "yes" -o "$JONJ_FPM_USER" = "no"; then
    jonj_fpm_user="nobody"
  else
    jonj_fpm_user="$JONJ_FPM_USER"
  fi

  if test -z "$JONJ_FPM_GROUP" -o "$JONJ_FPM_GROUP" = "yes" -o "$JONJ_FPM_GROUP" = "no"; then
    jonj_fpm_group="nobody"
  else
    jonj_fpm_group="$JONJ_FPM_GROUP"
  fi

  JONJ_SUBST_OLD(jonj_fpm_user)
  JONJ_SUBST_OLD(jonj_fpm_group)
  jonj_fpm_sysconfdir=`eval echo $sysconfdir`
  JONJ_SUBST_OLD(jonj_fpm_sysconfdir)
  jonj_fpm_localstatedir=`eval echo $localstatedir`
  JONJ_SUBST_OLD(jonj_fpm_localstatedir)
  jonj_fpm_prefix=`eval echo $prefix`
  JONJ_SUBST_OLD(jonj_fpm_prefix)

  AC_DEFINE_UNQUOTED(JONJ_FPM_USER, "$jonj_fpm_user", [fpm user name])
  AC_DEFINE_UNQUOTED(JONJ_FPM_GROUP, "$jonj_fpm_group", [fpm group name])

  JONJ_ADD_BUILD_DIR(sapi/fpm/fpm)
  JONJ_ADD_BUILD_DIR(sapi/fpm/fpm/events)
  JONJ_OUTPUT(sapi/fpm/jonj-fpm.conf sapi/fpm/init.d.jonj-fpm sapi/fpm/jonj-fpm.service sapi/fpm/jonj-fpm.8 sapi/fpm/status.html)
  JONJ_ADD_MAKEFILE_FRAGMENT([$abs_srcdir/sapi/fpm/Makefile.frag])

  SAPI_FPM_PATH=sapi/fpm/jonj-fpm
  
  if test "$fpm_trace_type" && test -f "$abs_srcdir/sapi/fpm/fpm/fpm_trace_$fpm_trace_type.c"; then
    JONJ_FPM_TRACE_FILES="fpm/fpm_trace.c fpm/fpm_trace_$fpm_trace_type.c"
  fi
  
  JONJ_FPM_CFLAGS="-I$abs_srcdir/sapi/fpm"
 
  JONJ_FPM_FILES="fpm/fastcgi.c \
    fpm/fpm.c \
    fpm/fpm_children.c \
    fpm/fpm_cleanup.c \
    fpm/fpm_clock.c \
  ...
  JONJ_SELECT_SAPI(fpm, program, $JONJ_FPM_FILES $JONJ_FPM_TRACE_FILES $JONJ_FPM_SD_FILES, $JONJ_FPM_CFLAGS, '$(SAPI_FPM_PATH)')

  case $host_alias in
      *aix*)
        BUILD_FPM="echo '\#! .' > jonj.sym && echo >>jonj.sym && nm -BCpg \`echo \$(JONJ_GLOBAL_OBJS) \$(JONJ_BINARY_OBJS) \$(JONJ_FPM_OBJS) | sed 's/\([A-Za-z0-9_]*\)\.lo/\1.o/g'\` | \$(AWK) '{ if (((\$\$2 == \"T\") || (\$\$2 == \"D\") || (\$\$2 == \"B\")) && (substr(\$\$3,1,1) != \".\")) { print \$\$3 } }' | sort -u >> jonj.sym && \$(LIBTOOL) --mode=link \$(CC) -export-dynamic \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) -Wl,-brtl -Wl,-bE:jonj.sym \$(JONJ_RPATHS) \$(JONJ_GLOBAL_OBJS) \$(JONJ_BINARY_OBJS) \$(JONJ_FPM_OBJS) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(ZEND_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
        ;;
      *darwin*)
        BUILD_FPM="\$(CC) \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) \$(NATIVE_RPATHS) \$(JONJ_GLOBAL_OBJS:.lo=.o) \$(JONJ_BINARY_OBJS:.lo=.o) \$(JONJ_FPM_OBJS:.lo=.o) \$(JONJ_FRAMEWORKS) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(ZEND_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
      ;;
      *)
        BUILD_FPM="\$(LIBTOOL) --mode=link \$(CC) -export-dynamic \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) \$(JONJ_RPATHS) \$(JONJ_GLOBAL_OBJS) \$(JONJ_BINARY_OBJS) \$(JONJ_FPM_OBJS) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(ZEND_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
      ;;
  esac

  JONJ_SUBST(SAPI_FPM_PATH)
  JONJ_SUBST(BUILD_FPM)
