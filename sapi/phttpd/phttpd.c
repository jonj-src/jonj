...
static int
jonj_phttpd_startup(sapi_module_struct *sapi_module)
{
	fprintf(stderr,"***jonj_phttpd_startup\n");

    if (jonj_module_startup(sapi_module, NULL, 0)) {
        return FAILURE;
    } else {
        return SUCCESS;
    }
...
static int
jonj_phttpd_sapi_ub_write(const char *str, uint str_length TSRMLS_DC)
{
    int sent_bytes;
...
static int
jonj_phttpd_sapi_header_handler(sapi_header_struct *sapi_header, sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
static int
jonj_phttpd_sapi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
static char *
jonj_phttpd_sapi_read_cookies(TSRMLS_D)
{
...
/*
    ...
    return http_cookie;
*/
	fprintf(stderr,"***jonj_phttpd_sapi_read_cookies\n");
 
	return 0;
}

static int
jonj_phttpd_sapi_read_post(char *buf, uint count_bytes TSRMLS_DC)
{
/*
    uint max_read;
    uint total_read = 0;
    ...
    return total_read;
*/
	fprintf(stderr,"***jonj_phttpd_sapi_read_post\n");
	return 0;
}
...
static sapi_module_struct phttpd_sapi_module = {
    ...
    jonj_phttpd_startup,                     /* startup */
    jonj_module_shutdown_wrapper,            /* shutdown */
 
	NULL,									/* activate */
	NULL,									/* deactivate */

    jonj_phttpd_sapi_ub_write,               /* unbuffered write */
	NULL,									/* flush */
	NULL,									/* get uid */
 	NULL,									/* getenv */
    ...
    jonj_phttpd_sapi_header_handler,         /* header handler */
    jonj_phttpd_sapi_send_headers,           /* send headers handler */
    NULL,                                   /* send header handler */
 
    jonj_phttpd_sapi_read_post,              /* read POST data */
    jonj_phttpd_sapi_read_cookies,           /* read Cookies */
...
static void
jonj_phttpd_request_ctor(TSRMLS_D TSRMLS_DC)
{
...
static void
jonj_phttpd_request_dtor(TSRMLS_D TSRMLS_DC)
{
    free(SG(request_info).path_translated);
}


int jonj_doit(TSRMLS_D)
{
	struct stat sb;
...
/*
	jonj_phttpd_hash_environment(TSRMLS_C);
*/
...
int pm_request(struct connectioninfo *cip)
{
	...
	if (strcasecmp(hip->method, "GET") == 0 || 
		...
		jonj_phttpd_request_ctor(TSRMLS_C);
		status = jonj_doit(TSRMLS_C);
		jonj_phttpd_request_dtor(TSRMLS_C);
