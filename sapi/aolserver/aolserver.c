...
/* conflict between JONJ and AOLserver headers */
#define Debug jonj_Debug
#include "jonj.h"
#undef Debug
...
/* jonj_ns_context is per-server (thus only once at all) */
...
static jonj_ns_context *global_context;

static void jonj_ns_config(jonj_ns_context *ctx, char global);

/*
 * jonj_ns_sapi_ub_write() writes data to the client connection.
 */

static int
jonj_ns_sapi_ub_write(const char *str, uint str_length TSRMLS_DC)
{
	int n;
	uint sent = 0;
...
/*
 * jonj_ns_sapi_header_handler() sets a HTTP reply header to be 
 * sent to the client.
 */

static int
jonj_ns_sapi_header_handler(sapi_header_struct *sapi_header, sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
/*
 * jonj_ns_sapi_send_headers() flushes the headers to the client.
 * Called before real content is sent by JONJ.
 */

static int
jonj_ns_sapi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
/*
 * jonj_ns_sapi_read_post() reads a specified number of bytes from
 * the client. Used for POST/PUT requests.
 */

static int
jonj_ns_sapi_read_post(char *buf, uint count_bytes TSRMLS_DC)
{
	uint max_read;
	uint total_read = 0;
...
/* 
 * jonj_ns_sapi_read_cookies() returns the Cookie header from
 * the HTTP request header
 */
	
static char *jonj_ns_sapi_read_cookies(TSRMLS_D)
{
	int i;
...
static void jonj_info_aolserver(ZEND_MODULE_INFO_FUNC_ARGS)
{
	char buf[512];
	...
	jonj_info_print_table_row(2, "HTTP Request", NSG(conn)->request->line);
	for (i = 0; i < Ns_SetSize(NSG(conn)->headers); i++) {
		jonj_info_print_table_row(2, Ns_SetKey(NSG(conn)->headers, i), Ns_SetValue(NSG(conn)->headers, i));
	}

	jonj_info_print_table_colspan_header(2, "HTTP Response Headers");
	for (i = 0; i < Ns_SetSize(NSG(conn)->outputheaders); i++) {
		jonj_info_print_table_row(2, Ns_SetKey(NSG(conn)->outputheaders, i), Ns_SetValue(NSG(conn)->outputheaders, i));
	}
	jonj_info_print_table_end();
}

JONJ_FUNCTION(getallheaders);
...
static const zend_function_entry aolserver_functions[] = {
	JONJ_FE(getallheaders, arginfo_aolserver_getallheaders)
	{NULL, NULL, NULL}
};

static zend_module_entry jonj_aolserver_module = {
...
static int
jonj_ns_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &jonj_aolserver_module, 1) == FAILURE) {
		return FAILURE;
	} else {
		return SUCCESS;
	}
...
/*
 * jonj_ns_sapi_register_variables() populates the jonj script environment
 * with a number of variables. HTTP_* variables are created for
 * the HTTP header data, so that a script can access these.
 */

#define ADD_STRINGX(name, buf)										\
	jonj_register_variable(name, buf, track_vars_array TSRMLS_CC)

#define ADD_STRING(name)										\
	ADD_STRINGX(name, buf)

static void
jonj_ns_sapi_register_variables(zval *track_vars_array TSRMLS_DC)
{
	int i;
	...
	ADD_STRINGX("JONJ_SELF", SG(request_info).request_uri);
...
static sapi_module_struct aolserver_sapi_module = {
	...
	jonj_ns_startup,							/* startup */
	jonj_module_shutdown_wrapper,			/* shutdown */

	NULL,									/* activate */
	NULL,									/* deactivate */

	jonj_ns_sapi_ub_write,					/* unbuffered write */
	NULL,									/* flush */
	NULL,									/* get uid */
	NULL,									/* getenv */
	...
	jonj_ns_sapi_header_handler,				/* header handler */
	jonj_ns_sapi_send_headers,				/* send headers handler */
	NULL,									/* send header handler */

	jonj_ns_sapi_read_post,					/* read POST data */
	jonj_ns_sapi_read_cookies,				/* read Cookies */

	jonj_ns_sapi_register_variables,
	NULL,									/* Log message */
...
/*
 * jonj_ns_module_main() is called by the per-request handler and
 * "executes" the script
 */

static int
jonj_ns_module_main(TSRMLS_D)
{
	...
	jonj_ns_config(global_context, 0);
...
/*
 * jonj_ns_request_ctor() initializes the per-request data structure
 * and fills it with data provided by the web server
 */

static void 
jonj_ns_request_ctor(TSRMLS_D)
{
	char *server;
	Ns_DString ds;
	char *root;
...
/*
 * jonj_ns_request_dtor() destroys all data associated with
 * the per-request structure 
 */

static void
jonj_ns_request_dtor(TSRMLS_D)
{
...
/*
 * The jonj_ns_request_handler() is called per request and handles
 * everything for one request.
 */

static int
jonj_ns_request_handler(void *context, Ns_Conn *conn)
{
	int status = NS_OK;
	TSRMLS_FETCH();
	...
	jonj_ns_request_ctor(TSRMLS_C);
	
	status = jonj_ns_module_main(TSRMLS_C);
	
	jonj_ns_request_dtor(TSRMLS_C);

	return status;
}

/*
 * jonj_ns_config() fetches the configuration data.
 *
 * It understands the "map" and "jonj_value" command.
 */

static void 
jonj_ns_config(jonj_ns_context *ctx, char global)
{
	int i;
	char *path;
	Ns_Set *set;
	...
	for (i = 0; set && i < Ns_SetSize(set); i++) {
		...
		if (global && !strcasecmp(key, "map")) {
			Ns_Log(Notice, "Registering JONJ for \"%s\"", value);
			Ns_RegisterRequest(ctx->ns_server, "GET", value, jonj_ns_request_handler, NULL, ctx, 0);
			Ns_RegisterRequest(ctx->ns_server, "POST", value, jonj_ns_request_handler, NULL, ctx, 0);
			Ns_RegisterRequest(ctx->ns_server, "HEAD", value, jonj_ns_request_handler, NULL, ctx, 0);
	 ...
	 */
		...
		} else if (!global && !strcasecmp(key, "jonj_value")) {
			Ns_Log(Notice, "jonj_value has been deactivated temporarily. Please use a jonj.ini file to pass directives to JONJ. Thanks.");
#if 0
			char *val;
			...
			if (val) {
				...
				Ns_Log(Debug, "JONJ configuration option '%s=%s'", new_key, val);
				zend_alter_ini_entry(new_key, strlen(new_key) + 1, val, 
						strlen(val) + 1, JONJ_INI_SYSTEM, JONJ_INI_STAGE_ACTIVATE);
...
/*
 * jonj_ns_server_shutdown() performs the last steps before the
 * server exits. Shutdowns basic services and frees memory
 */

static void
jonj_ns_server_shutdown(void *context)
{
	jonj_ns_context *ctx = (jonj_ns_context *) context;
...
int Ns_ModuleInit(char *server, char *module)
{
	...
	jonj_ns_config(ctx, 1);

	global_context = ctx;

	/* register shutdown handler */
	Ns_RegisterServerShutdown(server, jonj_ns_server_shutdown, ctx);
