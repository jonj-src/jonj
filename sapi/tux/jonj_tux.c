...
static jonj_tux_globals tux_globals;
...
static void sapi_tux_register_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	jonj_register_variable("SERVER_SOFTWARE", TUXAPI_version, track_vars_array TSRMLS_CC);
	...
	jonj_register_variable("DOCUMENT_ROOT", TUXAPI_docroot, track_vars_array TSRMLS_CC);
	jonj_register_variable("SERVER_NAME", TUXAPI_servername, track_vars_array TSRMLS_CC);
...
#define CONDADD(name, field) 							\
	if (TG(req)->field[0]) {								\
		jonj_register_variable(#name, TG(req)->field, track_vars_array TSRMLS_CC); \
	}
...
static int jonj_tux_startup(sapi_module_struct *sapi_module)
{
...
int TUXAPI_handle_events(user_req_t *req)
{
	...
	if (req->event == JONJ_TUX_BACKGROUND_CONN) {
...
void tux_set_dont_close(void)
{
	...
	TG(req)->event = JONJ_TUX_BACKGROUND_CONN;
