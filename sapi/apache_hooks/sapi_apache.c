...
int apache_jonj_module_hook(request_rec *r, jonj_handler *handler, zval **ret TSRMLS_DC)
{
    ...
    if(AP(current_hook) == AP_RESPONSE) {
        if (jonj_request_startup_for_hook(TSRMLS_C) == FAILURE)
            return FAILURE;
    }
    else {
        if (jonj_request_startup_for_hook(TSRMLS_C) == FAILURE)
    ...
    req = jonj_apache_request_new(r);
    jonj_register_variable_ex("request", req, PG(http_globals)[TRACK_VARS_SERVER] TSRMLS_CC);

    switch(handler->type) {
        case AP_HANDLER_TYPE_FILE:
            jonj_register_variable("JONJ_SELF_HOOK", handler->name, PG(http_globals)[TRACK_VARS_SERVER] TSRMLS_CC);
	        memset(&file_handle, 0, sizeof(file_handle));
	        file_handle.type = ZEND_HANDLE_FILENAME;
	        file_handle.filename = handler->name;
	        (void) jonj_execute_simple_script(&file_handle, ret TSRMLS_CC);
            break;
        case AP_HANDLER_TYPE_METHOD:
            if( (tmp = strstr(handler->name, "::")) != NULL &&  *(tmp+2) != '\0' ) {
                ...
                *tmp = ':';
                ...
                jonj_error(E_ERROR, "Unable to call %s - not a Class::Method\n", handler->name);
                /* not a class::method */
            }
            break;
        default:
