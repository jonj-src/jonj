...
static inline const char *jonjdbg_command_name(const jonjdbg_command_t *command, char *buffer) {
	size_t pos = 0;
...
JONJDBG_API const char *jonjdbg_get_param_type(const jonjdbg_param_t *param TSRMLS_DC) /* {{{ */
{
	switch (param->type) {
		case STACK_PARAM:
			return "stack";
...
JONJDBG_API void jonjdbg_clear_param(jonjdbg_param_t *param TSRMLS_DC) /* {{{ */
{
	if (param) {
		switch (param->type) {
			case FILE_PARAM:
...
JONJDBG_API void jonjdbg_copy_param(const jonjdbg_param_t* src, jonjdbg_param_t* dest TSRMLS_DC) /* {{{ */
{
...
JONJDBG_API zend_ulong jonjdbg_hash_param(const jonjdbg_param_t *param TSRMLS_DC) /* {{{ */
{
...
JONJDBG_API zend_bool jonjdbg_match_param(const jonjdbg_param_t *l, const jonjdbg_param_t *r TSRMLS_DC) /* {{{ */
{
	if (l && r) {
		if (l->type == r->type) {
			switch (l->type) {
...
JONJDBG_API void jonjdbg_stack_free(jonjdbg_param_t *stack) {
	if (stack && stack->next) {
		jonjdbg_param_t *remove = stack->next;
		
		while (remove) {
			jonjdbg_param_t *next = NULL;
...
JONJDBG_API void jonjdbg_stack_push(jonjdbg_param_t *stack, jonjdbg_param_t *param) {
	jonjdbg_param_t *next = calloc(1, sizeof(jonjdbg_param_t));
...
JONJDBG_API int jonjdbg_stack_verify(const jonjdbg_command_t *command, jonjdbg_param_t **stack, char **why TSRMLS_DC) {
	if (command) {
		char buffer[128] = {0,};
		const jonjdbg_param_t *top = (stack != NULL) ? *stack : NULL;
		...
		if (!(arg) || !(*arg)) {
			...
			asprintf(why,
				...
				jonjdbg_command_name(command, buffer));
			return FAILURE;
		}
...
#define verify_arg(e, a, t) if (!(a)) { \
	...
	asprintf(why, \
		...
		jonjdbg_command_name(command, buffer), \
		(e),\
		jonjdbg_get_param_type((a) TSRMLS_CC), \
		current); \
	return FAILURE; \
}
...
JONJDBG_API const jonjdbg_command_t* jonjdbg_stack_resolve(const jonjdbg_command_t *commands, const jonjdbg_command_t *parent, jonjdbg_param_t **top, char **why) {
	const jonjdbg_command_t *command = commands;
	jonjdbg_param_t *name = *top;
	const jonjdbg_command_t *matched[3] = {NULL, NULL, NULL};
	ulong matches = 0L;
	...
	if (command->subs && (*top) && ((*top)->type == STR_PARAM)) {
		return jonjdbg_stack_resolve(command->subs, command, top, why);
	} else {
		return command;
	}
...
JONJDBG_API int jonjdbg_stack_execute(jonjdbg_param_t *stack, char **why TSRMLS_DC) {
	jonjdbg_param_t *top = NULL;
	const jonjdbg_command_t *handler = NULL;
	...
	top = (jonjdbg_param_t*) stack->next;
	
	switch (top->type) {
		case EVAL_PARAM:
			return JONJDBG_COMMAND_HANDLER(ev)(top TSRMLS_CC);

		case RUN_PARAM:
			return JONJDBG_COMMAND_HANDLER(run)(top TSRMLS_CC);
		
		case SHELL_PARAM:
			return JONJDBG_COMMAND_HANDLER(sh)(top TSRMLS_CC);
		
		case STR_PARAM: {
			handler = jonjdbg_stack_resolve(
				jonjdbg_prompt_commands, NULL, &top, why);
			
			if (handler) {
				if (jonjdbg_stack_verify(handler, &top, why TSRMLS_CC) == SUCCESS) {
...
disconnect:
			if (0) {
				JONJDBG_G(flags) |= (JONJDBG_IS_QUITTING|JONJDBG_IS_DISCONNECTED);
				zend_bailout();
				return NULL;
			}
...
#if !defined(HAVE_LIBREADLINE) && !defined(HAVE_LIBEDIT)
			if (!(JONJDBG_G(flags) & JONJDBG_IS_REMOTE)) {
				if (!jonjdbg_write("%s", jonjdbg_get_prompt(TSRMLS_C))) {
					goto disconnect;
				}
			}
			...
			/* note: EOF is ignored */
readline:	
			if (!fgets(buf, JONJDBG_MAX_CMD, JONJDBG_G(io)[JONJDBG_STDIN])) {
...
readline:
			...
			} else cmd = readline(jonjdbg_get_prompt(TSRMLS_C));
	...
	if (buffer && strlen(buffer)) {
		...
		JONJDBG_G(buffer) = estrdup(buffer);
	} else {
		if (JONJDBG_G(buffer)) {
			buffer = estrdup(JONJDBG_G(buffer));
		}
	}
...
JONJDBG_API void jonjdbg_destroy_input(char **input TSRMLS_DC) /*{{{ */
{
	efree(*input);
} /* }}} */
