...
static inline zend_uint jonjdbg_decode_literal(zend_op_array *ops, zend_literal *literal TSRMLS_DC) /* {{{ */
{
	int iter = 0;
...
static inline char *jonjdbg_decode_op(zend_op_array *ops, znode_op *op, zend_uint type, HashTable *vars TSRMLS_DC) /* {{{ */
{
	char *decode = NULL;
	...
	switch (type &~ EXT_TYPE_UNUSED) {
		...
		case IS_CONST:
			asprintf(&decode, "C%u", jonjdbg_decode_literal(ops, op->literal TSRMLS_CC));
		break;
...
char *jonjdbg_decode_opline(zend_op_array *ops, zend_op *op, HashTable *vars TSRMLS_DC) /*{{{ */
{
	...
	case ZEND_JMPZNZ:
			decode[1] = jonjdbg_decode_op(ops, &op->op1, op->op1_type, vars TSRMLS_CC);
			asprintf(
	...
	case ZEND_RECV_INIT:
		...
		default: {
			decode[1] = jonjdbg_decode_op(ops, &op->op1, op->op1_type, vars TSRMLS_CC);
			decode[2] = jonjdbg_decode_op(ops, &op->op2, op->op2_type, vars TSRMLS_CC);
result:
			decode[3] = jonjdbg_decode_op(ops, &op->result, op->result_type, vars TSRMLS_CC);
format:
			asprintf(
				&decode[0],
				"%-20s %-20s %-20s",
...
void jonjdbg_print_opline_ex(zend_execute_data *execute_data, HashTable *vars, zend_bool ignore_flags TSRMLS_DC) /* {{{ */
{
	...
	if (ignore_flags ||
		...
		char *decode = jonjdbg_decode_opline(execute_data->op_array, opline, vars TSRMLS_CC);

		if (ignore_flags || (!(JONJDBG_G(flags) & JONJDBG_IS_QUIET) || (JONJDBG_G(flags) & JONJDBG_IS_STEPPING))) {
			/* output line info */
			jonjdbg_notice("L%-5u %16p %-30s %s %s",
			   opline->lineno,
			   opline,
		...
		if (!ignore_flags && JONJDBG_G(oplog)) {
			jonjdbg_log_ex(JONJDBG_G(oplog), "L%-5u %16p %-30s %s %s",
				opline->lineno,
				opline,
				jonjdbg_decode_opcode(opline->opcode),
				decode,
...
{
	jonjdbg_print_opline_ex(execute_data, NULL, ignore_flags TSRMLS_CC);
} /* }}} */

const char *jonjdbg_decode_opcode(zend_uchar opcode) /* {{{ */
{
