...
/* {{{ Commands Table */
#define JONJDBG_COMMAND_HELP_D(name, tip, alias, action) \
	{JONJDBG_STRL(#name), tip, sizeof(tip)-1, alias, action, NULL, 0}

const jonjdbg_command_t jonjdbg_help_commands[] = {
	JONJDBG_COMMAND_HELP_D(aliases,    "show alias list", 'a', jonjdbg_do_help_aliases),
	JONJDBG_COMMAND_HELP_D(options,    "command line options", 0, NULL),
	JONJDBG_COMMAND_HELP_D(overview,   "help overview", 0, NULL),
	JONJDBG_COMMAND_HELP_D(jonjdbginit, "jonjdbginit file format", 0, NULL),
	JONJDBG_COMMAND_HELP_D(syntax,     "syntax overview", 0, NULL),
	JONJDBG_END_COMMAND
};  /* }}} */
...
void pretty_print(char *text TSRMLS_DC)
{
	...
	const char  *prompt_escape = jonjdbg_get_prompt(TSRMLS_C);
	unsigned int prompt_escape_len = strlen(prompt_escape);
	unsigned int prompt_len = strlen(JONJDBG_G(prompt)[0]);

	const char  *bold_on_escape  = JONJDBG_G(flags) & JONJDBG_IS_COLOURED ? "\033[1m" : "";
	const char  *bold_off_escape = JONJDBG_G(flags) & JONJDBG_IS_COLOURED ? "\033[0m" : "";
	unsigned int bold_escape_len = strlen(bold_on_escape);

	unsigned int term_width = jonjdbg_get_terminal_width(TSRMLS_C);
	unsigned int size = 0;
	 ...
	 *
	 * ** toggles bold on and off if JONJDBG_IS_COLOURED flag is set
	...
	*q++ = '\0';
	...
	if ((q-new)>size) {
		jonjdbg_error("Output overrun of %lu bytes", ((q-new) - size));
	}
...
void summary_print(jonjdbg_command_t const * const cmd TSRMLS_DC)
{
	char *summary;
...
/* {{{ get_help. Retries and formats text from the jonjdbg help text table */
static char *get_help(const char * const key TSRMLS_DC)
{
	jonjdbg_help_text_t *p;

	/* Note that jonjdbg_help_text is not assumed to be collated in key order.  This is an
	   inconvience that means that help can't be logically grouped Not worth
	   the savings */

	for (p = jonjdbg_help_text; p->key; p++) {
		if (!strcmp(p->key, key)) {
			return p->text;
		}
	}
...
static int get_command(
	...
	jonjdbg_command_t const **command, /* address of first matching command  */
	jonjdbg_command_t const * commands /* command table to be scanned */
	TSRMLS_DC)
{
	const jonjdbg_command_t *c;
...
JONJDBG_COMMAND(help) /* {{{ */
{
	...
	if (param && param->type == STR_PARAM) {
	    n = get_command(param->str, param->len, &cmd, jonjdbg_prompt_commands TSRMLS_CC);
		...
		if (n==1) {
			...
			if (param->len > 1) {
				for (cmd=jonjdbg_prompt_commands; cmd->name; cmd++) {
				...
				jonjdbg_error("Internal help error, non-unique alias \"%c\"", param->str[0]);
				return FAILURE;
			}

		} else { /* no prompt command found so try help topic */
		    n = get_command( param->str, param->len, &cmd, jonjdbg_help_commands TSRMLS_CC);
...
{
	const jonjdbg_command_t *c, *c_sub;
	int len;

	/* Print out aliases for all commands except help as this one comes last */
	jonjdbg_writeln("Below are the aliased, short versions of all supported commands");
	for(c = jonjdbg_prompt_commands; c->name; c++) {
		if (c->alias && c->alias != 'h') {
			jonjdbg_writeln(" %c     %-20s  %s", c->alias, c->name, c->tip);
			if (c->subs) {
				len = 20 - 1 - c->name_len;
				for(c_sub = c->subs; c_sub->alias; c_sub++) {
					if (c_sub->alias) {
						jonjdbg_writeln(" %c %c   %s %-*s  %s",
	...
	get_command("h", 1, &c, jonjdbg_prompt_commands TSRMLS_CC);
	jonjdbg_writeln(" %c     %-20s  %s\n", c->alias, c->name, c->tip);
 ...
 *   $P insert an bold jonjdbg> prompt.
 ...
 */
#define CR "\n"
jonjdbg_help_text_t jonjdbg_help_text[] = {

/******************************** General Help Topics ********************************/
{"overview!", CR
"**jonjdbg** is a lightweight, powerful and easy to use debugging platform for JONJ5.4+" CR
"It supports the following commands:" CR CR

"**Information**" CR
"  **list**     list JONJ source" CR
...
"**Miscellaneous**" CR
"  **set**      set the jonjdbg configuration" CR
"  **source**   execute a jonjdbginit script" CR
"  **register** register a jonjdbginit function as a command alias" CR
"  **sh**       shell a command" CR
"  **ev**       evaluate some code" CR
"  **quit**     exit jonjdbg" CR CR
...
"Type **help aliases** to show a full alias list, including any registered jonjdginit functions" CR
"Type **help syntax** for a general introduction to the command syntax." CR
"Type **help options** for a list of jonjdbg command line options." CR
"Type **help jonjdbginit** to show how to customise the debugger environment."
},
{"options", CR
"Below are the command line options supported by jonjdbg" CR CR
...
"  **-c**      **-c**/my/jonj.ini       Set jonj.ini file to load" CR
"  **-d**      **-d**memory_limit=4G   Set a jonj.ini directive" CR
"  **-n**                          Disable default jonj.ini" CR
"  **-q**                          Supress welcome banner" CR
"  **-v**                          Enable oplog output" CR
"  **-s**                          Enable stepping" CR
"  **-b**                          Disable colour" CR
"  **-i**      **-i**my.init           Set .jonjdbginit file" CR
"  **-I**                          Ignore default .jonjdbginit" CR
...
"  **--**      **--** arg1 arg2        Use to delimit jonjdbg arguments and jonj $argv; append any $argv "
...
"bind address using the **-a** option. If **-a** is specied without an argument, then jonjdbg "
...
{"jonjdbginit", CR
"Phpdgb uses an debugger script file to initialize the debugger context.  By default, jonjdbg looks "
"for the file named **.jonjdbginit** in the current working directory.  This location can be "
...
"A script file can contain a sequence of valid debugger commands, comments and embedded JONJ "
"code. " CR CR 
...
"JONJ code is delimited by the start and end escape tags **<:** and **:>**. JONJ code can be used "
"to define application context for a debugging session and also to extend the debugger by defining "
"and **register** JONJ functions as new commands." CR CR

"Also note that executing a **clear** command will cause the current **jonjdbginit** to be reparsed "
"/ reloaded."
},
...
"    " CR
"     $P break 200" CR
"     $P b my_source.jonj:200" CR
"     Break at line 200 in the current source and in file **my_source.jonj**. " CR CR
...
"     Comments introduced by the **#** character are only allowed in **jonjdbginit** script files."
},
...
"is a valid JONJ expression which will trigger the break in "
...
"**Examples**" CR CR
"    $P break test.jonj:100" CR
"    $P b test.jonj:100" CR
"    Break execution at line 100 of test.jonj" CR CR

"    $P break 200" CR
"    $P b 200" CR
"    Break execution at line 200 of the currently JONJ script file" CR CR
...
"    $P break test.jonj:#3" CR
"    $P b test.jonj:#3" CR
"    Break at opline #3 in test.jonj" CR CR

"    $P break if $cnt > 10" CR
"    $P b if $cnt > 10" CR
"    Break when the condition ($cnt > 10) evaluates to true" CR CR

"    $P break at jonjdbg::isGreat if $opt == 'S'" CR
"    $P break @ jonjdbg::isGreat if $opt == 'S'" CR
"    Break at any opcode in jonjdbg::isGreat when the condition ($opt == 'S') is true" CR CR

"    $P break at test.jonj:20 if !isset($x)" CR
"    Break at every opcode on line 20 of test.jonj when the condition evaluates to true" CR CR
...
{"clean",
"Classes, constants or functions can only be declared once in JONJ.  You may experience errors "
"during a debug session if you attempt to recompile a JONJ source.  The clean command clears "
...
"Note: if all breakpoints are cleared, then the JONJ script will run until normal completion."
},
...
"    $P ev $variable = \"Hello jonjdbg :)\"" CR
"    Will set $variable in the current scope" CR CR

"Note that **ev** allows any valid JONJ expression including assignments, function calls and "
"other write statements.  This enables you to change the environment during execution, so care "
"is needed here.  You can even call JONJ functions which have breakpoints defined. " CR CR
...
"    $P exec /tmp/script.jonj" CR
"    $P e /tmp/script.jonj" CR
"    Set the execution context to **/tmp/script.jonj**"
},
...
{"info",
"**info** commands provide quick access to various types of information about the JONJ environment" CR
...
"Register any global function for use as a command in jonjdbg console" CR CR

"**Examples**" CR CR
"    $P register scandir" CR
"    $P R scandir" CR
"    Will register the scandir function for use in jonjdbg" CR CR
...
{"set",
"The **set** command is used to configure how jonjdbg looks and behaves.  Specific set commands "
...
"**Examples**" CR CR
"    $P sh ls /usr/src/jonj-src" CR
"    Will execute ls /usr/src/jonj-src, displaying the output in the console"
//*********** what does this mean????Note: read only commands please!
},

{"source",
"Sourcing a **jonjdbginit** script during your debugging session might save some time." CR CR

"**Examples**" CR CR

"    $P source /my/init" CR
"    $P < /my/init" CR
"    Will execute the jonjdbginit file at /my/init" CR CR
},
...
"                You then you can continue, jonjdbg will remove the write protection, so that the program can continue." CR
"                If jonjdbg could not handle that segfault, the same segfault is triggered again and this time jonjdbg will abort."
},
