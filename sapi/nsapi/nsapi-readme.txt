Configuration of your Netscape/iPlanet/Sun Webserver for JONJ5
-----------------------------------------------------------------
...
<path-to-server>/https-servername/config/magnus.conf file:
    ...
    Init fn="load-modules" funcs="jonj5_init,jonj5_execute,jonj5_auth_trans" shlib="/path/to/jonjlibrary"
    Init fn=jonj5_init errorString="Failed to initialize JONJ!" [jonj_ini="/path/to/jonj.ini"]

The "shlib" will vary depending on your OS:

	Unix: "<path-to-server>/bin/libjonj5.so".
	Windows: "c:/path/to/jonj5/jonj5nsapi.dll"
...
In obj.conf (for virtual server classes [Sun 6.0+] in their vserver.obj.conf):
    ...
    # all 'AddLog' lines
    # You can modify some entries in jonj.ini request specific by adding it to the Service
    ...
    Service fn="jonj5_execute" type="magnus-internal/x-httpd-jonj" [inikey=value ...]
    .
    .
    .
    </Object>
...
JONJ scripts (same like a cgi-bin directory):

    <Object name="x-httpd-jonj">
    ObjectType fn="force-type" type="magnus-internal/x-httpd-jonj"
    Service fn="jonj5_execute" [inikey=value ...]
    </Object>

After that you can configure a directory in the Administration server and assign it
the style "x-httpd-jonj". All files in it will get executed as JONJ. This is nice to
hide JONJ usage by renaming files to .html

Note: The stacksize that JONJ uses depends on the configuration of the webserver. If you get
crashes with very large JONJ scripts, it is recommended to raise it with the Admin Server
...
JONJ authentication cannot be used with any other authentication. ALL
AUTHENTICATION IS PASSED TO YOUR JONJ SCRIPT.  To configure JONJ
...
To use JONJ Authentication on a single directory, add the following:
...
You can use JONJ to generate the error pages for "404 Not Found"
or similar. Add the following line to the object in obj.conf for
every error page you want to overwrite:

    Error fn="jonj5_execute" code=XXX script="/path/to/script.jonj" [inikey=value inikey=value...]
...
Just generate a JONJ script which displays a directory listing and
replace the corresponding default Service line for
type="magnus-internal/directory" in obj.conf with the following:

    Service fn="jonj5_execute" type="magnus-internal/directory" script="/path/to/script.jonj" [inikey=value inikey=value...]
...
Add the following parameter to jonj5_init in magnus.conf:
