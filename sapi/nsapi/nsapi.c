...
/*
 * If neither XP_UNIX not XP_WIN32 is defined use JONJ_WIN32
 */
...
/* this parameters to "Service"/"Error" are NSAPI ones which should not be jonj.ini keys and are excluded */
...
JONJ_MINIT_FUNCTION(nsapi);
JONJ_MSHUTDOWN_FUNCTION(nsapi);
JONJ_RINIT_FUNCTION(nsapi);
JONJ_RSHUTDOWN_FUNCTION(nsapi);
JONJ_MINFO_FUNCTION(nsapi);

JONJ_FUNCTION(nsapi_virtual);
JONJ_FUNCTION(nsapi_request_headers);
JONJ_FUNCTION(nsapi_response_headers);
...
const zend_function_entry nsapi_functions[] = {
	JONJ_FE(nsapi_virtual,	arginfo_nsapi_virtual)						/* Make subrequest */
	JONJ_FALIAS(virtual, nsapi_virtual, arginfo_nsapi_virtual)			/* compatibility */
	JONJ_FE(nsapi_request_headers, arginfo_nsapi_request_headers)		/* get request headers */
	JONJ_FALIAS(getallheaders, nsapi_request_headers, arginfo_nsapi_request_headers)	/* compatibility */
	JONJ_FALIAS(apache_request_headers, nsapi_request_headers, arginfo_nsapi_request_headers)	/* compatibility */
	JONJ_FE(nsapi_response_headers, arginfo_nsapi_response_headers)		/* get response headers */
	JONJ_FALIAS(apache_response_headers, nsapi_response_headers, arginfo_nsapi_response_headers)	/* compatibility */
	{NULL, NULL, NULL}
};
/* }}} */
 ...
 */
JONJ_INI_BEGIN()
    STD_JONJ_INI_ENTRY("nsapi.read_timeout", "60", JONJ_INI_ALL, OnUpdateLong, read_timeout, zend_nsapi_globals, nsapi_globals)
JONJ_INI_END()
/* }}} */
...
/* {{{ jonj_nsapi_init_dynamic_symbols
 */
static void jonj_nsapi_init_dynamic_symbols(void)
{
	...
	if (nsapi_dll) {
		...
		if (!module) {
			log_error(LOG_WARN, "jonj5_init", NULL, NULL, "Cannot find DLL specified by server_lib parameter: %s", nsapi_dll);
		}
	} else {
...
/* {{{ jonj_nsapi_init_globals
 */
static void jonj_nsapi_init_globals(zend_nsapi_globals *nsapi_globals)
{
...
{
	jonj_nsapi_init_dynamic_symbols();
	ZEND_INIT_MODULE_GLOBALS(nsapi, jonj_nsapi_init_globals, NULL);
...
JONJ_MINFO_FUNCTION(nsapi)
{
	...
	jonj_info_print_table_row(2, "NSAPI Module Revision", "$Id$");
	jonj_info_print_table_row(2, "Server Software", system_version());
	jonj_info_print_table_row(2, "Sub-requests with nsapi_virtual()",
...
JONJ_FUNCTION(nsapi_virtual)
{
	...
	if (!nsapi_servact_service) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to include uri '%s' - Sub-requests not supported on this platform", uri);
		RETURN_FALSE;
	} else if (zend_ini_long("zlib.output_compression", sizeof("zlib.output_compression"), 0)) {
		jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to include uri '%s' - Sub-requests do not work with zlib.output_compression", uri);
		RETURN_FALSE;
	} else {
		...
		if ((rq = request_restart_internal(uri, NULL)) == NULL) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to include uri '%s' - Internal request creation failed", uri);
			RETURN_FALSE;
		}
		...
		if (rq->status_num != 200) {
			jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to include uri '%s' - HTTP status code %d during subrequest", uri, rq->status_num);
			request_free(rq);
			RETURN_FALSE;
		}
...
static int jonj_nsapi_remove_header(sapi_header_struct *sapi_header TSRMLS_DC)
{
	char *header_name, *p;
	nsapi_request_context *rc = (nsapi_request_context *)SG(server_context);
...
static int sapi_nsapi_header_handler(sapi_header_struct *sapi_header, sapi_header_op_enum op, sapi_headers_struct *sapi_headers TSRMLS_DC)
{
	...
	switch(op) {
		case SAPI_HEADER_DELETE_ALL:
			/* this only deletes headers set or overwritten by JONJ, headers previously set by NSAPI are left intact */
			zend_llist_apply(&sapi_headers->headers, (llist_apply_func_t) jonj_nsapi_remove_header TSRMLS_CC);
			return 0;

		case SAPI_HEADER_DELETE:
			/* reuse the zend_llist_apply callback function for this, too */
			jonj_nsapi_remove_header(sapi_header TSRMLS_CC);
			return 0;
...
static void sapi_nsapi_register_server_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	for (i = 0; i < nsapi_reqpb_size; i++) {
		...
		if (value) {
			jonj_register_variable((char *)nsapi_reqpb[i].env_var, value, track_vars_array TSRMLS_CC);
		}
	}
	...
	for (i=0; i < rc->rq->headers->hsize; i++) {
		...
		while (entry) {
			...
			if (value) {
				...
				jonj_register_variable(value, entry->param->value, track_vars_array TSRMLS_CC);
				efree(value);
			}
			entry=entry->next;
		}
	...
	for (i = 0; i < nsapi_vars_size; i++) {
		...
		if (value) {
			jonj_register_variable((char *)nsapi_vars[i].env_var, value, track_vars_array TSRMLS_CC);
		}
	}
	...
	for (i = 0; i < nsapi_client_size; i++) {
		...
		if (value) {
			jonj_register_variable((char *)nsapi_client[i].env_var, value, track_vars_array TSRMLS_CC);
		}
	}

	if (value = session_dns(rc->sn)) {
		jonj_register_variable("REMOTE_HOST", value, track_vars_array TSRMLS_CC);
		nsapi_free(value);
	}

	slprintf(buf, sizeof(buf), "%d", conf_getglobals()->Vport);
	jonj_register_variable("SERVER_PORT", buf, track_vars_array TSRMLS_CC);
	jonj_register_variable("SERVER_NAME", conf_getglobals()->Vserver_hostname, track_vars_array TSRMLS_CC);

	value = http_uri2url_dynamic("", "", rc->sn, rc->rq);
	jonj_register_variable("SERVER_URL", value, track_vars_array TSRMLS_CC);
	nsapi_free(value);

	jonj_register_variable("SERVER_SOFTWARE", system_version(), track_vars_array TSRMLS_CC);
	if (security_active) {
		jonj_register_variable("HTTPS", "ON", track_vars_array TSRMLS_CC);
	}
	jonj_register_variable("GATEWAY_INTERFACE", "CGI/1.1", track_vars_array TSRMLS_CC);

	/* DOCUMENT_ROOT */
	if (value = request_translate_uri("/", rc->sn)) {
		pos = strlen(value);
		jonj_register_variable_safe("DOCUMENT_ROOT", value, pos-1, track_vars_array TSRMLS_CC);
		nsapi_free(value);
	}
	...
	if (rc->path_info) {
		if (value = request_translate_uri(rc->path_info, rc->sn)) {
			jonj_register_variable("PATH_TRANSLATED", value, track_vars_array TSRMLS_CC);
			nsapi_free(value);
		}
		jonj_register_variable("PATH_INFO", rc->path_info, track_vars_array TSRMLS_CC);
	}
	...
	if (SG(request_info).request_uri) {
		...
		if (SG(request_info).query_string) {
			...
			if (value) {
				jonj_register_variable("REQUEST_URI", value, track_vars_array TSRMLS_CC);
				efree(value);
			}
		} else {
			jonj_register_variable_safe("REQUEST_URI", SG(request_info).request_uri, pos, track_vars_array TSRMLS_CC);
		}
		...
		jonj_register_variable_safe("SCRIPT_NAME", SG(request_info).request_uri, pos, track_vars_array TSRMLS_CC);
	}
	jonj_register_variable("SCRIPT_FILENAME", SG(request_info).path_translated, track_vars_array TSRMLS_CC);

	/* special variables in error mode */
	if (rc->http_error) {
		slprintf(buf, sizeof(buf), "%d", rc->http_error);
		jonj_register_variable("ERROR_TYPE", buf, track_vars_array TSRMLS_CC);
	}
}
...
static void nsapi_log_message(char *message TSRMLS_DC)
{
	...
	if (rc) {
		...
		log_error(LOG_INFORM, "jonj5", NULL, NULL, "%s", message);
	}
}
...
static int jonj_nsapi_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &nsapi_module_entry, 1)==FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
}
...
static void nsapi_jonj_ini_entries(NSLS_D TSRMLS_DC)
{
	struct pb_entry *entry;
	register int i,j,ok;
		...
		while (entry) {
			...
			if (ok) {
				...
				if (zend_alter_ini_entry(entry->param->name, strlen(entry->param->name)+1,
				 ...
				 JONJ_INI_SYSTEM, JONJ_INI_STAGE_ACTIVATE)==FAILURE) {
					log_error(LOG_WARN, pblock_findval("fn", NSG(pb)), NSG(sn), NSG(rq), "Cannot change jonj.ini key \"%s\" to \"%s\"", entry->param->name, entry->param->value);
				}
			}
			entry=entry->next;
		}
...
void NSAPI_PUBLIC jonj5_close(void *vparam)
{
	if (nsapi_sapi_module.shutdown) {
		nsapi_sapi_module.shutdown(&nsapi_sapi_module);
	}

	if (nsapi_sapi_module.jonj_ini_path_override) {
		free(nsapi_sapi_module.jonj_ini_path_override);
	}
	...
	log_error(LOG_INFORM, "jonj5_close", NULL, NULL, "Shutdown JONJ Module");
}

/*********************************************************
/ init SAF
/
/ Init fn="jonj5_init" [jonj_ini="/path/to/jonj.ini"] [server_lib="ns-httpdXX.dll"]
/   Initialize the NSAPI module in magnus.conf
/
/ jonj_ini: gives path to jonj.ini file
/ server_lib: (only Win32) gives name of DLL (without path) to look for
/  servact_* functions
/
/*********************************************************/
int NSAPI_PUBLIC jonj5_init(pblock *pb, Session *sn, Request *rq)
{
	...
	/* look if jonj_ini parameter is given to jonj5_init */
	if (strval = pblock_findval("jonj_ini", pb)) {
		nsapi_sapi_module.jonj_ini_path_override = strdup(strval);
	}
	
#ifdef JONJ_WIN32
	/* look if server_lib parameter is given to jonj5_init
	...
	daemon_atrestart(&jonj5_close, NULL);

	log_error(LOG_INFORM, pblock_findval("fn", pb), sn, rq, "Initialized JONJ Module (%d threads expected)", threads);
	return REQ_PROCEED;
}
...
/
/ Service fn="jonj5_execute" type=... method=... [inikey=inivalue inikey=inivalue...]
/
/ use in Service for a directory to supply a jonj-made directory listing instead of server default:
/
/ Service fn="jonj5_execute" type="magnus-internal/directory" script="/path/to/script.jonj" [inikey=inivalue inikey=inivalue...]
/
/ use in Error SAF to display jonj script as error page:
/
/ Error fn="jonj5_execute" code=XXX script="/path/to/script.jonj" [inikey=inivalue inikey=inivalue...]
/ Error fn="jonj5_execute" reason="Reason" script="/path/to/script.jonj" [inikey=inivalue inikey=inivalue...]
/
/*********************************************************/
int NSAPI_PUBLIC jonj5_execute(pblock *pb, Session *sn, Request *rq)
{
	int retval;
	...
	/* check if this uri was included in an other JONJ script with nsapi_virtual()
	   by looking for a request context in the current thread */
	if (SG(server_context)) {
		/* send 500 internal server error */
		log_error(LOG_WARN, pblock_findval("fn", pb), sn, rq, "Cannot make nesting JONJ requests with nsapi_virtual()");
		if (error_directive) {
			return REQ_NOACTION;
		} else {
	...
	if (!request_context) {
		log_error(LOG_CATASTROPHE, pblock_findval("fn", pb), sn, rq, "Insufficient memory to process JONJ request!");
		return REQ_ABORTED;
	}
	...
	nsapi_jonj_ini_entries(NSLS_C TSRMLS_CC);

	jonj_handle_auth_data(pblock_findval("authorization", rq->headers) TSRMLS_CC);
	...
	if (fst && S_ISREG(fst->st_mode)) {
		if (jonj_request_startup(TSRMLS_C) == SUCCESS) {
			...
			/* send 500 internal server error */
			log_error(LOG_WARN, pblock_findval("fn", pb), sn, rq, "Cannot prepare JONJ engine!");
			if (error_directive) {
				retval=REQ_NOACTION;
			} else {
				protocol_status(sn, rq, 500, NULL);
		...
		log_error(LOG_WARN, pblock_findval("fn", pb), sn, rq, "Cannot execute JONJ script: %s (File not found)", SG(request_info).path_translated);
		if (error_directive) {
			retval=REQ_NOACTION;
		} else {
...
/ will pass authentication through to jonj, and allow us to
/ check authentication with our scripts.
/
/ jonj5_auth_trans
/   main function called from netscape server to authenticate
/   a line in obj.conf:
/		funcs=jonj5_auth_trans shlib="path/to/this/jonjnsapi.dll"
/	and:
/		<Object ppath="path/to/be/authenticated/by/jonj/*">
/		AuthTrans fn="jonj5_auth_trans"
/*********************************************************/
int NSAPI_PUBLIC jonj5_auth_trans(pblock * pb, Session * sn, Request * rq)
{
	/* This is a DO NOTHING function that allows authentication
	 * information
	 * to be passed through to JONJ scripts.
	 */
	return REQ_PROCEED;
}
