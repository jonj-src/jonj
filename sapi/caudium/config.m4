...
RESULT=no
JONJ_ARG_WITH(caudium,,
[  --with-caudium[=DIR]      Build JONJ as a Pike module for use with Caudium.
                          DIR is the Caudium server dir [/usr/local/caudium/server]], no, no)

AC_MSG_CHECKING([for Caudium support])

if test "$JONJ_CAUDIUM" != "no"; then
  if test "$prefix" = "NONE"; then CPREF=/usr/local/; fi
  if test ! -d $JONJ_CAUDIUM ; then
    if test "$prefix" = "NONE"; then
      JONJ_CAUDIUM=/usr/local/caudium/server/
    else
      JONJ_CAUDIUM=$prefix/caudium/server/
    fi
  fi
  if test -f $JONJ_CAUDIUM/bin/caudium; then
    PIKE=$JONJ_CAUDIUM/bin/caudium
  elif test -f $JONJ_CAUDIUM/bin/pike; then
    PIKE=$JONJ_CAUDIUM/bin/pike
  else
    AC_MSG_ERROR([Could not find a pike in $JONJ_CAUDIUM/bin/])
  fi
  if $PIKE -e 'float v; int rel;sscanf(version(), "Pike v%f release %d", v, rel);v += rel/10000.0; if(v < 7.0268) exit(1); exit(0);'; then
    ...
    if test -f $PIKE_C_INCLUDE/version.h; then
      ...
      if test "$PIKE_TEST_VER" = "${PIKE_CMAJOR_VERSION}.${PIKE_CMINOR_VERSION}.${PIKE_CBUILD_VERSION}"; then
        JONJ_ADD_INCLUDE($PIKE_C_INCLUDE)
    ...
    AC_MSG_ERROR([Caudium JONJ5 requires Pike 7.0 or newer])
  fi
  PIKE_VERSION=`$PIKE -e 'string v; int rel;sscanf(version(), "Pike v%s release %d", v, rel); write(v+"."+rel);'`   
  AC_DEFINE(HAVE_CAUDIUM,1,[Whether to compile with Caudium support])
  JONJ_SELECT_SAPI(caudium, shared, caudium.c)
  INSTALL_IT="\$(INSTALL) -m 0755 $SAPI_SHARED $JONJ_CAUDIUM/lib/$PIKE_VERSION/JONJ5.so"
