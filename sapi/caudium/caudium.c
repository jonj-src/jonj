...
/* Ok, we are now using Pike level threads to handle JONJ5 since
...
#ifndef ZTS
/* Need thread safety */
#error You need to compile JONJ with threads.
#endif

#ifndef PIKE_THREADS
#error The JONJ5 module requires that your Pike has thread support.
#endif
...
/* jonj_caudium_request is per-request object storage */
...
typedef struct
{
  ...
  int written;
  TSRMLS_D;
} jonj_caudium_request;


void pike_module_init(void);
void pike_module_exit(void);
static void free_struct(TSRMLS_D);
void f_jonj_caudium_request_handler(INT32 args);
...
#define GET_THIS() jonj_caudium_request *_request = ts_resource(caudium_globals_id)
#define THIS _request
#define PTHIS ((jonj_caudium_request *)(Pike_fp->current_storage))
...
static unsigned char caudium_jonj_initialized;
...
/*
 * jonj_caudium_low_ub_write() writes data to the client connection. Might be
 * rewritten to do more direct IO to save CPU and the need to lock the 
 * interpreter for better threading.
 */

INLINE static int
jonj_caudium_low_ub_write(const char *str, uint str_length TSRMLS_DC) {
  int sent_bytes = 0;
...
/*
 * jonj_caudium_sapi_ub_write() calls jonj_caudium_low_ub_write in a Pike thread
 * safe manner or writes directly to the output FD if RXML post-parsing is
 * disabled. 
 */

static int
jonj_caudium_sapi_ub_write(const char *str, uint str_length TSRMLS_DC)
{
  GET_THIS();
  ...
  if(fd)
  {
    ...
    THREAD_SAFE_RUN(sent_bytes = jonj_caudium_low_ub_write(str, str_length TSRMLS_CC),
		    "write");
  }
  return sent_bytes;
}

/* jonj_caudium_set_header() sets a header in the header mapping. Called in a
 * thread safe manner from jonj_caudium_sapi_header_handler.
 */
INLINE static void
jonj_caudium_set_header(char *header_name, char *value, char *p)
{
  struct svalue hsval;
...
/*
 * jonj_caudium_sapi_header_handler() sets a HTTP reply header to be 
 * sent to the client.
 */
static int
jonj_caudium_sapi_header_handler(sapi_header_struct *sapi_header,
			      sapi_headers_struct *sapi_headers TSRMLS_DC)
{
  ...
  do {
    ...
    THREAD_SAFE_RUN(jonj_caudium_set_header(header_name, header_content, p), "header handler");
  }
...
/*
 * jonj_caudium_sapi_send_headers() flushes the headers to the client.
 * Called before real content is sent by JONJ.
 */

INLINE static int
jonj_caudium_low_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
static int
jonj_caudium_sapi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
  int res = 0;
  THREAD_SAFE_RUN(res = jonj_caudium_low_send_headers(sapi_headers TSRMLS_CC), "send headers");
  return res;
}

/*
 * jonj_caudium_sapi_read_post() reads a specified number of bytes from
 * the client. Used for POST/PUT requests.
 */

INLINE static int jonj_caudium_low_read_post(char *buf, uint count_bytes)
{
  uint total_read = 0;
  GET_THIS();
  TSRMLS_FETCH();
...
static int
jonj_caudium_sapi_read_post(char *buf, uint count_bytes TSRMLS_DC)
{
  uint total_read = 0;
  THREAD_SAFE_RUN(total_read = jonj_caudium_low_read_post(buf, count_bytes), "read post");
  return total_read;
}

/* 
 * jonj_caudium_sapi_read_cookies() returns the Cookie header from
 * the HTTP request header
 */
	
static char *
jonj_caudium_sapi_read_cookies(TSRMLS_D)
{
  char *cookies;
...
static void jonj_info_caudium(ZEND_MODULE_INFO_FUNC_ARGS)
{
  /*  char buf[512]; */
...
static zend_module_entry jonj_caudium_module = {
...
INLINE static void low_sapi_caudium_register_variables(zval *track_vars_array TSRMLS_DC)   
{
  ...
  struct svalue *val;
  GET_THIS();
  jonj_register_variable("JONJ_SELF", SG(request_info).request_uri,
			track_vars_array TSRMLS_CC);
  jonj_register_variable("GATEWAY_INTERFACE", "CGI/1.1",
			track_vars_array TSRMLS_CC);
  jonj_register_variable("REQUEST_METHOD",
			(char *) SG(request_info).request_method,
			track_vars_array TSRMLS_CC);
  jonj_register_variable("REQUEST_URI", SG(request_info).request_uri,
			track_vars_array TSRMLS_CC);
  jonj_register_variable("PATH_TRANSLATED", SG(request_info).path_translated,
			track_vars_array TSRMLS_CC);
  ...
  if(headers && headers->type == PIKE_T_MAPPING) {
	...
	jonj_register_variable(ind->u.string->str, val->u.string->str,
			      track_vars_array TSRMLS_CC );
      }
    }
  }
...
static int jonj_caudium_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &jonj_caudium_module, 1)==FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
}
...
static sapi_module_struct caudium_sapi_module = {
  "caudium",
  "Caudium",
  jonj_caudium_startup,			/* startup */
  jonj_module_shutdown_wrapper,		/* shutdown */
  NULL,					/* activate */
  NULL,					/* deactivate */
  jonj_caudium_sapi_ub_write,		/* unbuffered write */
  NULL,					/* flush */
  NULL,					/* get uid */
  NULL,					/* getenv */
  jonj_error,				/* error handler */
  jonj_caudium_sapi_header_handler,	/* header handler */
  jonj_caudium_sapi_send_headers,	/* send headers handler */
  NULL,					/* send header handler */
  jonj_caudium_sapi_read_post,		/* read POST data */
  jonj_caudium_sapi_read_cookies,	/* read cookies */
...
/*
 * jonj_caudium_module_main() is called by the per-request handler and
 * "executes" the script
 */

static void jonj_caudium_module_main(jonj_caudium_request *ureq)
{
  int res;
  ...
  */
  ...
  /* Let JONJ5 handle the deconding of the AUTH */
  jonj_handle_auth_data(lookup_string_header("HTTP_AUTHORIZATION", NULL), TSRMLS_C);
...
/*
 * The jonj_caudium_request_handler() is called per request and handles
...
void f_jonj_caudium_request_handler(INT32 args)
{
  ...
  struct pike_string *ind;
  jonj_caudium_request *_request;
  THIS = malloc(sizeof(jonj_caudium_request));
  if(THIS == NULL)
  ...
  if(done_callback->type != PIKE_T_FUNCTION) 
    Pike_error("JONJ5.Interpreter->run: Bad argument 4, expected function.\n");
  ...
  jonj_caudium_module_main(THIS);
#else
  th_farm((void (*)(void *))jonj_caudium_module_main, THIS);
#endif
  pop_n_elems(args);
}
...
static void free_struct(TSRMLS_D)
{
  ...
  MEMSET(THIS, 0, sizeof(jonj_caudium_request));
}
...
void pike_module_init( void )
{
  if (!caudium_jonj_initialized) {
    caudium_jonj_initialized = 1;
    tsrm_startup(1, 1, 0, NULL);
    ts_allocate_id(&caudium_globals_id, sizeof(jonj_caudium_request), NULL, NULL);
    sapi_startup(&caudium_sapi_module);
    sapi_module.startup(&caudium_sapi_module);
  }
  start_new_program(); /* Text */
  pike_add_function("run", f_jonj_caudium_request_handler,
