...
#ifdef ZTS
int jonj_apache2_info_id;
#else
jonj_apache2_info_struct jonj_apache2_info;
#endif

#define SECTION(name)  PUTS("<h2>" name "</h2>\n")

static request_rec *jonj_apache_lookup_uri(char *filename TSRMLS_DC)
{
...
JONJ_FUNCTION(virtual)
{
	...
	if (!(rr = jonj_apache_lookup_uri(filename TSRMLS_CC))) {
...
static char *jonj_apache_get_version()
{
...
{
	char *apv = jonj_apache_get_version();
...
JONJ_MINFO_FUNCTION(apache)
{
	...
	int n, max_requests;
	char *p;
	server_rec *serv = ((jonj_struct *) SG(server_context))->r->server;
#if !defined(WIN32) && !defined(WINNT) && !defined(NETWARE)
	...
	jonj_info_print_table_row(2, "Apache API Version", tmp);
	
	if (serv->server_admin && *(serv->server_admin)) {
		jonj_info_print_table_row(2, "Server Administrator", serv->server_admin);
	}
	
	snprintf(tmp, sizeof(tmp), "%s:%u", serv->server_hostname, serv->port);
	jonj_info_print_table_row(2, "Hostname:Port", tmp);
...
#endif
	jonj_info_print_table_row(2, "User/Group", tmp);
#endif
	...
	jonj_info_print_table_row(2, "Max Requests", tmp);
	...
	jonj_info_print_table_row(2, "Timeouts", tmp);
	
	jonj_info_print_table_row(2, "Virtual Server", (serv->is_virtual ? "Yes" : "No"));
	jonj_info_print_table_row(2, "Server Root", ap_server_root);
	jonj_info_print_table_row(2, "Loaded Modules", tmp1.c);
	...
	{
		const apr_array_header_t *arr = apr_table_elts(((jonj_struct *) SG(server_context))->r->subprocess_env);
		char *key, *val;
		...
		APR_ARRAY_FOREACH_OPEN(arr, key, val)
			...
			jonj_info_print_table_row(2, key, val);
		APR_ARRAY_FOREACH_CLOSE()
		...
		jonj_info_print_table_row(2, "HTTP Request", ((jonj_struct *) SG(server_context))->r->the_request);
		
		arr = apr_table_elts(((jonj_struct *) SG(server_context))->r->headers_in);
		APR_ARRAY_FOREACH_OPEN(arr, key, val)
			if (!val) {
				val = "";
			}
		...
		arr = apr_table_elts(((jonj_struct *) SG(server_context))->r->headers_out);
		APR_ARRAY_FOREACH_OPEN(arr, key, val)
			if (!val) {
				val = "";
			}
...
static const zend_function_entry apache_functions[] = {
	JONJ_FE(apache_lookup_uri, 		arginfo_apache2handler_lookup_uri)
	JONJ_FE(virtual, 				arginfo_apache2handler_virtual) 
	JONJ_FE(apache_request_headers, 	arginfo_apache2handler_getallheaders)
	JONJ_FE(apache_response_headers, arginfo_apache2handler_response_headers)
	JONJ_FE(apache_setenv, 		arginfo_apache2handler_setenv)
	JONJ_FE(apache_getenv, 		arginfo_apache2handler_getenv)
	JONJ_FE(apache_note, 		arginfo_apache2handler_note)
	JONJ_FE(apache_get_version, 	arginfo_apache2handler_get_version)
	JONJ_FE(apache_get_modules, 	arginfo_apache2handler_get_modules)
	JONJ_FALIAS(getallheaders, 	apache_request_headers, arginfo_apache2handler_getallheaders)
	{NULL, NULL, NULL}
};

JONJ_INI_BEGIN()
	STD_JONJ_INI_ENTRY("xbithack",		"0",	JONJ_INI_ALL,	OnUpdateLong,	xbithack,	jonj_apache2_info_struct, jonj_apache2_info)
	STD_JONJ_INI_ENTRY("engine",		"1",	JONJ_INI_ALL,	OnUpdateLong,	engine, 	jonj_apache2_info_struct, jonj_apache2_info)
	STD_JONJ_INI_ENTRY("last_modified",	"0",	JONJ_INI_ALL,	OnUpdateLong,	last_modified,	jonj_apache2_info_struct, jonj_apache2_info)
JONJ_INI_END()

static JONJ_MINIT_FUNCTION(apache)
{
#ifdef ZTS
	ts_allocate_id(&jonj_apache2_info_id, sizeof(jonj_apache2_info_struct), (ts_allocate_ctor) NULL, NULL);
#endif
...
zend_module_entry jonj_apache_module = {
