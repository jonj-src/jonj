/*
   ...
   |         Xinchen Hui       <laruence@jonj.tk>                         |
   +----------------------------------------------------------------------+
*/

/* $Id: jonj_cli.c 306938 2011-01-01 02:17:06Z felipe $ */
...
#include "ext/standard/file.h" /* for jonj_set_sock_blocking() :-( */
#include "ext/standard/jonj_smart_str.h"
#include "ext/standard/html.h"
#include "ext/standard/url.h" /* for jonj_raw_url_decode() */
#include "ext/standard/jonj_string.h" /* for jonj_dirname() */
#include "jonj.tkwork.h"

#include "jonj_http_parser.h"
...
typedef struct jonj_cli_server_poller {
	fd_set rfds, wfds;
	struct {
		fd_set rfds, wfds;
	} active;
	jonj_socket_t max_fd;
} jonj_cli_server_poller;

typedef struct jonj_cli_server_request {
	enum jonj_http_method request_method;
	...
	size_t ext_len;
	struct stat sb;
} jonj_cli_server_request;

typedef struct jonj_cli_server_chunk {
	struct jonj_cli_server_chunk *next;
	enum jonj_cli_server_chunk_type {
		JONJ_CLI_SERVER_CHUNK_HEAP,
		JONJ_CLI_SERVER_CHUNK_IMMORTAL
	} type;
	union {
		struct { void *block; char *p; size_t len; } heap;
		struct { const char *p; size_t len; } immortal;
	} data;
} jonj_cli_server_chunk;

typedef struct jonj_cli_server_buffer {
	jonj_cli_server_chunk *first;
	jonj_cli_server_chunk *last;
} jonj_cli_server_buffer;

typedef struct jonj_cli_server_content_sender {
	jonj_cli_server_buffer buffer;
} jonj_cli_server_content_sender;

typedef struct jonj_cli_server_client {
	struct jonj_cli_server *server;
	jonj_socket_t sock;
	struct sockaddr *addr;
	socklen_t addr_len;
	char *addr_str;
	size_t addr_str_len;
	jonj_http_parser parser;
	unsigned int request_read:1;
	char *current_header_name;
	size_t current_header_name_len;
	unsigned int current_header_name_allocated:1;
	size_t post_read_offset;
	jonj_cli_server_request request;
	unsigned int content_sender_initialized:1;
	jonj_cli_server_content_sender content_sender;
	int file_fd;
} jonj_cli_server_client;

typedef struct jonj_cli_server {
	jonj_socket_t server_sock;
	jonj_cli_server_poller poller;
	int is_running;
	char *host;
	int port;
	int address_family;
...
typedef struct jonj_cli_server_http_response_status_code_pair {
	int code;
	const char *str;
} jonj_cli_server_http_response_status_code_pair;
...
static jonj_cli_server_http_response_status_code_pair status_map[] = {
	{ 100, "Continue" },
...
static jonj_cli_server_http_response_status_code_pair template_map[] = {
...
static int jonj_cli_output_is_tty = OUTPUT_NOT_CHECKED;

static size_t jonj_cli_server_client_send_through(jonj_cli_server_client *client, const char *str, size_t str_len);
static jonj_cli_server_chunk *jonj_cli_server_chunk_heap_new_self_contained(size_t len);
static void jonj_cli_server_buffer_append(jonj_cli_server_buffer *buffer, jonj_cli_server_chunk *chunk);
static void jonj_cli_server_logf(const char *format TSRMLS_DC, ...);
static void jonj_cli_server_log_response(jonj_cli_server_client *client, int status, const char *message TSRMLS_DC);

ZEND_DECLARE_MODULE_GLOBALS(cli_server);

/* {{{ static char jonj_cli_server_css[]
 * copied from ext/standard/info.c
 */
static const char jonj_cli_server_css[] = "<style>\n" \
...
#ifdef JONJ_WIN32
int jonj_cli_server_get_system_time(char *buf) {
...
int jonj_cli_server_get_system_time(char *buf) {
	...
	jonj_localtime_r(&tv.tv_sec, &tm);
	jonj_asctime_r(&tm, buf);
	return 0;
}
#endif
...
{
	const jonj_cli_server_http_response_status_code_pair *pa = (const jonj_cli_server_http_response_status_code_pair *) a;
	const jonj_cli_server_http_response_status_code_pair *pb = (const jonj_cli_server_http_response_status_code_pair *) b;
...
{
	jonj_cli_server_http_response_status_code_pair needle, *result = NULL;
...
{
	size_t e = (sizeof(template_map) / sizeof(jonj_cli_server_http_response_status_code_pair));
	size_t s = 0;
...
static void append_essential_headers(smart_str* buffer, jonj_cli_server_client *client, int persistent) /* {{{ */
{
	{
		char **val;
...
{
	jonj_cli_server_ext_mime_type_pair *pair;
...
JONJ_INI_BEGIN()
	STD_JONJ_INI_BOOLEAN("cli_server.color", "0", JONJ_INI_ALL, OnUpdateBool, color, zend_cli_server_globals, cli_server_globals)
JONJ_INI_END()

static JONJ_MINIT_FUNCTION(cli_server)
{
...
static JONJ_MSHUTDOWN_FUNCTION(cli_server)
{
	UNREGISTER_INI_ENTRIES();
	return SUCCESS;
}

static JONJ_MINFO_FUNCTION(cli_server)
{
	DISPLAY_INI_ENTRIES();
}
...
zend_module_entry cli_server_module_entry = {
	...
	"cli_server",
	NULL,
	JONJ_MINIT(cli_server),
	JONJ_MSHUTDOWN(cli_server),
	NULL,
	NULL,
	JONJ_MINFO(cli_server),
	JONJ_VERSION,
...
const zend_function_entry server_additional_functions[] = {
	...
	JONJ_FE(apache_request_headers, arginfo_no_args)
	JONJ_FE(apache_response_headers, arginfo_no_args)
	JONJ_FALIAS(getallheaders, apache_request_headers, arginfo_no_args)
	{NULL, NULL, NULL}
};

static int sapi_cli_server_startup(sapi_module_struct *sapi_module) /* {{{ */
{
	if (jonj_module_startup(sapi_module, &cli_server_module_entry, 1) == FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
} /* }}} */
...
{
	jonj_cli_server_client *client = SG(server_context);
	if (!client) {
		return 0;
	}
	return jonj_cli_server_client_send_through(client, str, str_length);
} /* }}} */

static void sapi_cli_server_flush(void *server_context) /* {{{ */
{
	jonj_cli_server_client *client = server_context;
	TSRMLS_FETCH();
...
static int sapi_cli_server_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC) /* {{{ */
{
	...
	jonj_cli_server_client_send_through(client, buffer.c, buffer.len);
...
static void sapi_cli_server_register_variable(zval *track_vars_array, const char *key, const char *val TSRMLS_DC) /* {{{ */
{
	...
	if (sapi_module.input_filter(PARSE_SERVER, (char*)key, &new_val, strlen(val), &new_val_len TSRMLS_CC)) {
		jonj_register_variable_safe((char *)key, new_val, new_val_len, track_vars_array TSRMLS_CC);
	}
} /* }}} */
...
static void sapi_cli_server_register_variables(zval *track_vars_array TSRMLS_DC) /* {{{ */
{
	...
	{
		char *tmp;
		spprintf(&tmp, 0, "JONJ %s Development Server", JONJ_VERSION);
	...
	if (client->request.path_info_len) {
		...
		sapi_cli_server_register_variable(track_vars_array, "JONJ_SELF", tmp TSRMLS_CC);
		efree(tmp);
	} else {
		sapi_cli_server_register_variable(track_vars_array, "JONJ_SELF", client->request.vpath TSRMLS_CC);
	}
...
static void sapi_cli_server_log_message(char *msg TSRMLS_DC) /* {{{ */
{
	...
	if (jonj_cli_server_get_system_time(buf) != 0) {
...
static int jonj_cli_server_poller_ctor(jonj_cli_server_poller *poller) /* {{{ */
{
	FD_ZERO(&poller->rfds);
	FD_ZERO(&poller->wfds);
	poller->max_fd = -1;
...
static void jonj_cli_server_poller_add(jonj_cli_server_poller *poller, int mode, int fd) /* {{{ */
{
	if (mode & POLLIN) {
		JONJ_SAFE_FD_SET(fd, &poller->rfds);
	}
	if (mode & POLLOUT) {
		JONJ_SAFE_FD_SET(fd, &poller->wfds);
	}
	if (fd > poller->max_fd) {
		poller->max_fd = fd;
	}
...
static void jonj_cli_server_poller_remove(jonj_cli_server_poller *poller, int mode, int fd) /* {{{ */
{
	if (mode & POLLIN) {
		JONJ_SAFE_FD_CLR(fd, &poller->rfds);
	}
	if (mode & POLLOUT) {
		JONJ_SAFE_FD_CLR(fd, &poller->wfds);
	}
#ifndef JONJ_WIN32
	if (fd == poller->max_fd) {
		while (fd > 0) {
			fd--;
			if (JONJ_SAFE_FD_ISSET(fd, &poller->rfds) || JONJ_SAFE_FD_ISSET(fd, &poller->wfds)) {
				break;
			}
		}
		poller->max_fd = fd;
...
static int jonj_cli_server_poller_poll(jonj_cli_server_poller *poller, struct timeval *tv) /* {{{ */
{
	memmove(&poller->active.rfds, &poller->rfds, sizeof(poller->rfds));
	memmove(&poller->active.wfds, &poller->wfds, sizeof(poller->wfds));
	return jonj_select(poller->max_fd + 1, &poller->active.rfds, &poller->active.wfds, NULL, tv);
} /* }}} */

static int jonj_cli_server_poller_iter_on_active(jonj_cli_server_poller *poller, void *opaque, int(*callback)(void *, int fd, int events)) /* {{{ */
{
	int retval = SUCCESS;
#ifdef JONJ_WIN32
	struct socket_entry {
...
#else
	jonj_socket_t fd;
	const jonj_socket_t max_fd = poller->max_fd;

	for (fd=0 ; fd<=max_fd ; fd++)  {
		if (JONJ_SAFE_FD_ISSET(fd, &poller->active.rfds)) {
                if (SUCCESS != callback(opaque, fd, POLLIN)) {
                    retval = FAILURE;
                }
		}
		if (JONJ_SAFE_FD_ISSET(fd, &poller->active.wfds)) {
...
static size_t jonj_cli_server_chunk_size(const jonj_cli_server_chunk *chunk) /* {{{ */
{
	switch (chunk->type) {
	case JONJ_CLI_SERVER_CHUNK_HEAP:
		return chunk->data.heap.len;
	case JONJ_CLI_SERVER_CHUNK_IMMORTAL:
		return chunk->data.immortal.len;
	}
	return 0;
} /* }}} */

static void jonj_cli_server_chunk_dtor(jonj_cli_server_chunk *chunk) /* {{{ */
{
	switch (chunk->type) {
	case JONJ_CLI_SERVER_CHUNK_HEAP:
...
static void jonj_cli_server_buffer_dtor(jonj_cli_server_buffer *buffer) /* {{{ */
{
	jonj_cli_server_chunk *chunk, *next;
	for (chunk = buffer->first; chunk; chunk = next) {
		next = chunk->next;
		jonj_cli_server_chunk_dtor(chunk);
		pefree(chunk, 1);
	}
} /* }}} */

static void jonj_cli_server_buffer_ctor(jonj_cli_server_buffer *buffer) /* {{{ */
{
	buffer->first = NULL;
	buffer->last = NULL;
} /* }}} */
...
static void jonj_cli_server_buffer_prepend(jonj_cli_server_buffer *buffer, jonj_cli_server_chunk *chunk) /* {{{ */
{
	jonj_cli_server_chunk *last;
	for (last = chunk; last->next; last = last->next);
...
static size_t jonj_cli_server_buffer_size(const jonj_cli_server_buffer *buffer) /* {{{ */
{
	jonj_cli_server_chunk *chunk;
	size_t retval = 0;
	for (chunk = buffer->first; chunk; chunk = chunk->next) {
		retval += jonj_cli_server_chunk_size(chunk);
	}
	return retval;
} /* }}} */

static jonj_cli_server_chunk *jonj_cli_server_chunk_immortal_new(const char *buf, size_t len) /* {{{ */
{
	jonj_cli_server_chunk *chunk = pemalloc(sizeof(jonj_cli_server_chunk), 1);
	if (!chunk) {
		return NULL;
	}

	chunk->type = JONJ_CLI_SERVER_CHUNK_IMMORTAL;
	chunk->next = NULL;
...
static jonj_cli_server_chunk *jonj_cli_server_chunk_heap_new(char *block, char *buf, size_t len) /* {{{ */
{
	jonj_cli_server_chunk *chunk = pemalloc(sizeof(jonj_cli_server_chunk), 1);
	if (!chunk) {
		return NULL;
	...
	chunk->type = JONJ_CLI_SERVER_CHUNK_HEAP;
	chunk->next = NULL;
...
static jonj_cli_server_chunk *jonj_cli_server_chunk_heap_new_self_contained(size_t len) /* {{{ */
{
	jonj_cli_server_chunk *chunk = pemalloc(sizeof(jonj_cli_server_chunk) + len, 1);
	if (!chunk) {
		return NULL;
	}
...
static void jonj_cli_server_content_sender_dtor(jonj_cli_server_content_sender *sender) /* {{{ */
{
	jonj_cli_server_buffer_dtor(&sender->buffer);
} /* }}} */

static void jonj_cli_server_content_sender_ctor(jonj_cli_server_content_sender *sender) /* {{{ */
{
	jonj_cli_server_buffer_ctor(&sender->buffer);
} /* }}} */

static int jonj_cli_server_content_sender_send(jonj_cli_server_content_sender *sender, jonj_socket_t fd, size_t *nbytes_sent_total) /* {{{ */
{
	jonj_cli_server_chunk *chunk, *next;
...
static int jonj_cli_server_content_sender_pull(jonj_cli_server_content_sender *sender, int fd, size_t *nbytes_read) /* {{{ */
{
	ssize_t _nbytes_read;
	jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_heap_new_self_contained(131072);

	_nbytes_read = read(fd, chunk->data.heap.p, chunk->data.heap.len);
	if (_nbytes_read < 0) {
		char *errstr = get_last_error();
		TSRMLS_FETCH();
		jonj_cli_server_logf("%s" TSRMLS_CC, errstr);
		pefree(errstr, 1);
		jonj_cli_server_chunk_dtor(chunk);
		pefree(chunk, 1);
		return 1;
	...
	jonj_cli_server_buffer_append(&sender->buffer, chunk);
	*nbytes_read = _nbytes_read;
	return 0;
} /* }}} */

#if HAVE_UNISTD_H
static int jonj_cli_is_output_tty() /* {{{ */
{
	if (jonj_cli_output_is_tty == OUTPUT_NOT_CHECKED) {
		jonj_cli_output_is_tty = isatty(STDOUT_FILENO);
	}
	return jonj_cli_output_is_tty;
} /* }}} */
#endif
...
static void jonj_cli_server_log_response(jonj_cli_server_client *client, int status, const char *message TSRMLS_DC) /* {{{ */
{
...
#if HAVE_UNISTD_H
	if (CLI_SERVER_G(color) && jonj_cli_is_output_tty() == OUTPUT_IS_TTY) {
	...
	if (color) {
		jonj_cli_server_logf("\x1b[3%dm%s%s%s\x1b[0m" TSRMLS_CC, color, basic_buf, message_buf, error_buf);
	} else {
		jonj_cli_server_logf("%s%s%s" TSRMLS_CC, basic_buf, message_buf, error_buf);
	}
...
static int jonj.tkwork_listen_socket(const char *host, int *port, int socktype, int *af, socklen_t *socklen, char **errstr TSRMLS_DC) /* {{{ */
{
	int retval = SOCK_ERR;
	int err = 0;
	struct sockaddr *sa = NULL, **p, **sal;

	int num_addrs = jonj.tkwork_getaddresses(host, socktype, &sal, errstr TSRMLS_CC);
	if (num_addrs == 0) {
		return -1;
	}
	for (p = sal; *p; p++) {
...
out:
	...
	if (err) {
		...
		if (errstr) {
			*errstr = jonj_socket_strerror(err, NULL, 0);
		}
		return SOCK_ERR;
	}
	return retval;
...
static int jonj_cli_server_request_ctor(jonj_cli_server_request *req) /* {{{ */
{
...
static void jonj_cli_server_request_dtor(jonj_cli_server_request *req) /* {{{ */
{
	if (req->request_uri) {
...
static void jonj_cli_server_request_translate_vpath(jonj_cli_server_request *request, const char *document_root, size_t document_root_len) /* {{{ */
{
	struct stat sb;
	static const char *index_files[] = { "index.jonj", "index.html", NULL };
...
static void normalize_vpath(char **retval, size_t *retval_len, const char *vpath, size_t vpath_len, int persistent) /* {{{ */
{
	...
	*retval = NULL;
	...
	decoded_vpath_end = decoded_vpath + jonj_raw_url_decode(decoded_vpath, vpath_len);
...
/* {{{ jonj_cli_server_client_read_request */
static int jonj_cli_server_client_read_request_on_message_begin(jonj_http_parser *parser)
{
	return 0;
}

static int jonj_cli_server_client_read_request_on_path(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
	{
		char *vpath;
		size_t vpath_len;
...
static int jonj_cli_server_client_read_request_on_query_string(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
...
static int jonj_cli_server_client_read_request_on_url(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
...
static int jonj_cli_server_client_read_request_on_fragment(jonj_http_parser *parser, const char *at, size_t length)
{
	return 0;
}

static int jonj_cli_server_client_read_request_on_header_field(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
...
static int jonj_cli_server_client_read_request_on_header_value(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
...
static int jonj_cli_server_client_read_request_on_headers_complete(jonj_http_parser *parser)
{
	jonj_cli_server_client *client = parser->data;
	if (client->current_header_name_allocated) {
		pefree(client->current_header_name, 1);
...
static int jonj_cli_server_client_read_request_on_body(jonj_http_parser *parser, const char *at, size_t length)
{
	jonj_cli_server_client *client = parser->data;
	if (!client->request.content) {
...
static int jonj_cli_server_client_read_request_on_message_complete(jonj_http_parser *parser)
{
	jonj_cli_server_client *client = parser->data;
	client->request.protocol_version = parser->http_major * 100 + parser->http_minor;
	jonj_cli_server_request_translate_vpath(&client->request, client->server->document_root, client->server->document_root_len);
	{
...
static int jonj_cli_server_client_read_request(jonj_cli_server_client *client, char **errstr TSRMLS_DC)
{
	char buf[16384];
	static const jonj_http_parser_settings settings = {
		jonj_cli_server_client_read_request_on_message_begin,
		jonj_cli_server_client_read_request_on_path,
		jonj_cli_server_client_read_request_on_query_string,
		jonj_cli_server_client_read_request_on_url,
		jonj_cli_server_client_read_request_on_fragment,
		jonj_cli_server_client_read_request_on_header_field,
		jonj_cli_server_client_read_request_on_header_value,
		jonj_cli_server_client_read_request_on_headers_complete,
		jonj_cli_server_client_read_request_on_body,
		jonj_cli_server_client_read_request_on_message_complete
	};
	...
	nbytes_consumed = jonj_http_parser_execute(&client->parser, &settings, buf, nbytes_read);
...
static size_t jonj_cli_server_client_send_through(jonj_cli_server_client *client, const char *str, size_t str_len) /* {{{ */
{
	...
	do {
		...
		if (nbytes_sent < 0) {
			int err = jonj_socket_errno();
			if (err == SOCK_EAGAIN) {
				int nfds = jonj_pollfd_for(client->sock, POLLOUT, &tv);
				if (nfds > 0) {
					continue;
				} else if (nfds < 0) {
					/* error */
...
static void jonj_cli_server_client_populate_request_info(const jonj_cli_server_client *client, sapi_request_info *request_info) /* {{{ */
{
	char **val;

	request_info->request_method = jonj_http_method_str(client->request.request_method);
...
static int jonj_cli_server_client_ctor(jonj_cli_server_client *client, jonj_cli_server *server, int client_sock, struct sockaddr *addr, socklen_t addr_len TSRMLS_DC) /* {{{ */
{
	...
	client->addr_len = addr_len;
	{
		char *addr_str = 0;
		long addr_str_len = 0;
		jonj.tkwork_populate_name_from_sockaddr(addr, addr_len, &addr_str, &addr_str_len, NULL, 0 TSRMLS_CC);
		client->addr_str = pestrndup(addr_str, addr_str_len, 1);
		client->addr_str_len = addr_str_len;
		efree(addr_str);
	}
	jonj_http_parser_init(&client->parser, JONJ_HTTP_REQUEST);
	client->request_read = 0;
	client->current_header_name = NULL;
	client->current_header_name_len = 0;
	client->current_header_name_allocated = 0;
	client->post_read_offset = 0;
	if (FAILURE == jonj_cli_server_request_ctor(&client->request)) {
		return FAILURE;
	}
...
static void jonj_cli_server_client_dtor(jonj_cli_server_client *client) /* {{{ */
{
	jonj_cli_server_request_dtor(&client->request);
	if (client->file_fd >= 0) {
	...
	if (client->content_sender_initialized) {
		jonj_cli_server_content_sender_dtor(&client->content_sender);
	}
} /* }}} */

static void jonj_cli_server_close_connection(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
#ifdef DEBUG
	jonj_cli_server_logf("%s Closing" TSRMLS_CC, client->addr_str);
#endif
	zend_hash_index_del(&server->clients, client->sock);
} /* }}} */

static int jonj_cli_server_send_error_page(jonj_cli_server *server, jonj_cli_server_client *client, int status TSRMLS_DC) /* {{{ */
{
	...
	jonj_cli_server_content_sender_ctor(&client->content_sender);
	client->content_sender_initialized = 1;

	escaped_request_uri = jonj_escape_html_entities_ex((unsigned char *)client->request.request_uri, client->request.request_uri_len, &escaped_request_uri_len, 0, ENT_QUOTES, NULL, 0 TSRMLS_CC);

	{
		static const char prologue_template[] = "<!doctype html><html><head><title>%d %s</title>";
		jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_heap_new_self_contained(strlen(prologue_template) + 3 + strlen(status_string) + 1);
		if (!chunk) {
			goto fail;
		}
		...
		jonj_cli_server_buffer_append(&client->content_sender.buffer, chunk);
	}
	{
		jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_immortal_new(jonj_cli_server_css, sizeof(jonj_cli_server_css) - 1);
		if (!chunk) {
			goto fail;
		}
		jonj_cli_server_buffer_append(&client->content_sender.buffer, chunk);
	}
	{
		...
		jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_immortal_new(template, sizeof(template) - 1);
		if (!chunk) {
			goto fail;
		}
		jonj_cli_server_buffer_append(&client->content_sender.buffer, chunk);
	...
	{
		jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_heap_new_self_contained(strlen(content_template) + escaped_request_uri_len + 3 + strlen(status_string) + 1);
		if (!chunk) {
			goto fail;
		}
	...
	}
	{
		...
		jonj_cli_server_chunk *chunk = jonj_cli_server_chunk_immortal_new(epilogue_template, sizeof(epilogue_template) - 1);
		if (!chunk) {
			goto fail;
		}
		jonj_cli_server_buffer_append(&client->content_sender.buffer, chunk);
...

	{
		...
		smart_str_append_generic_ex(&buffer, jonj_cli_server_buffer_size(&client->content_sender.buffer), 1, size_t, _unsigned);
		smart_str_appendl_ex(&buffer, "\r\n", 2, 1);
		smart_str_appendl_ex(&buffer, "\r\n", 2, 1);

		chunk = jonj_cli_server_chunk_heap_new(buffer.c, buffer.c, buffer.len);
		if (!chunk) {
			smart_str_free_ex(&buffer, 1);
			goto fail;
		}
		jonj_cli_server_buffer_prepend(&client->content_sender.buffer, chunk);
	}

	jonj_cli_server_log_response(client, status, errstr ? errstr : "?" TSRMLS_CC);
	jonj_cli_server_poller_add(&server->poller, POLLOUT, client->sock);
	if (errstr) {
		pefree(errstr, 1);
	}
...
static int jonj_cli_server_dispatch_script(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	if (strlen(client->request.path_translated) != client->request.path_translated_len) {
		/* can't handle paths that contain nul bytes */
		return jonj_cli_server_send_error_page(server, client, 400 TSRMLS_CC);
	}
	{
	...
	jonj_cli_server_log_response(client, SG(sapi_headers).http_response_code, NULL TSRMLS_CC);
	return SUCCESS;
} /* }}} */

static int jonj_cli_server_begin_send_static(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	int fd;
	int status = 200;
	...
	if (client->request.path_translated &&
		('.' == client->request.path_translated[client->request.path_translated_len-1] ||
		 ' ' == client->request.path_translated[client->request.path_translated_len-1])) {
		return jonj_cli_server_send_error_page(server, client, 500 TSRMLS_CC);
	}
#endif

	fd = client->request.path_translated ? open(client->request.path_translated, O_RDONLY): -1;
	if (fd < 0) {
		return jonj_cli_server_send_error_page(server, client, 404 TSRMLS_CC);
	}
...

	{
		...
		if (!buffer.c) {
			/* out of memory */
			jonj_cli_server_log_response(client, 500, NULL TSRMLS_CC);
			return FAILURE;
		}
		append_essential_headers(&buffer, client, 1);
	...
	jonj_cli_server_log_response(client, 200, NULL TSRMLS_CC);
	jonj_cli_server_poller_add(&server->poller, POLLOUT, client->sock);
	return SUCCESS;
}
/* }}} */

static int jonj_cli_server_request_startup(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) { /* {{{ */
	char **auth;
	jonj_cli_server_client_populate_request_info(client, &SG(request_info));
	...
	if (FAILURE == jonj_request_startup(TSRMLS_C)) {
...
static int jonj_cli_server_request_shutdown(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) { /* {{{ */
	jonj_request_shutdown(0);
	jonj_cli_server_close_connection(server, client TSRMLS_CC);
	destroy_request_info(&SG(request_info));
...
static int jonj_cli_server_dispatch_router(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	int decline = 0;
	zend_file_handle zfd;
	char *old_cwd;
	...
	old_cwd[0] = '\0';
	jonj_ignore_value(VCWD_GETCWD(old_cwd, MAXPATHLEN - 1));
...
static int jonj_cli_server_dispatch(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	int is_static_file  = 0;

	SG(server_context) = client;
	if (client->request.ext_len != 3 || memcmp(client->request.ext, "jonj", 3) || !client->request.path_translated) {
		is_static_file = 1;
	}

	if (server->router || !is_static_file) {
		if (FAILURE == jonj_cli_server_request_startup(server, client TSRMLS_CC)) {
			SG(server_context) = NULL;
			jonj_cli_server_close_connection(server, client TSRMLS_CC);
			destroy_request_info(&SG(request_info));
			return SUCCESS;
	...
	if (server->router) {
		if (!jonj_cli_server_dispatch_router(server, client TSRMLS_CC)) {
			jonj_cli_server_request_shutdown(server, client TSRMLS_CC);
			return SUCCESS;
		}
	}

	if (!is_static_file) {
		if (SUCCESS == jonj_cli_server_dispatch_script(server, client TSRMLS_CC)
				|| SUCCESS != jonj_cli_server_send_error_page(server, client, 500 TSRMLS_CC)) {
		...
		if (SUCCESS != jonj_cli_server_begin_send_static(server, client TSRMLS_CC)) {
			jonj_cli_server_close_connection(server, client TSRMLS_CC);
		}
		SG(server_context) = NULL;
		return SUCCESS;
...
static void jonj_cli_server_dtor(jonj_cli_server *server TSRMLS_DC) /* {{{ */
{
...
static void jonj_cli_server_client_dtor_wrapper(jonj_cli_server_client **p) /* {{{ */
{
	closesocket((*p)->sock);
	jonj_cli_server_poller_remove(&(*p)->server->poller, POLLIN | POLLOUT, (*p)->sock);
	jonj_cli_server_client_dtor(*p);
	pefree(*p, 1);
} /* }}} */

static int jonj_cli_server_ctor(jonj_cli_server *server, const char *addr, const char *document_root, const char *router TSRMLS_DC) /* {{{ */
{
	int retval = SUCCESS;
	char *host = NULL;
	char *errstr = NULL;
	...
	int err = 0;
	int port = 3000;
	jonj_socket_t server_sock = SOCK_ERR;
	char *p = NULL;
	...
	server_sock = jonj.tkwork_listen_socket(host, &port, SOCK_STREAM, &server->address_family, &server->socklen, &errstr TSRMLS_CC);
	if (server_sock == SOCK_ERR) {
		jonj_cli_server_logf("Failed to listen on %s:%d (reason: %s)" TSRMLS_CC, host, port, errstr ? errstr: "?");
		efree(errstr);
		retval = FAILURE;
		goto out;
	}
	...
	err = jonj_cli_server_poller_ctor(&server->poller);
	if (SUCCESS != err) {
		goto out;
	}

	jonj_cli_server_poller_add(&server->poller, POLLIN, server_sock);

	server->host = host;
	server->port = port;

	zend_hash_init(&server->clients, 0, NULL, (void(*)(void*))jonj_cli_server_client_dtor_wrapper, 1);
...
static int jonj_cli_server_recv_event_read_request(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	char *errstr = NULL;
	int status = jonj_cli_server_client_read_request(client, &errstr TSRMLS_CC);
	if (status < 0) {
		jonj_cli_server_logf("%s Invalid request (%s)" TSRMLS_CC, client->addr_str, errstr);
		efree(errstr);
		jonj_cli_server_close_connection(server, client TSRMLS_CC);
		return FAILURE;
	} else if (status == 1 && client->request.request_method == JONJ_HTTP_NOT_IMPLEMENTED) {
		return jonj_cli_server_send_error_page(server, client, 501 TSRMLS_CC);
	} else if (status == 1) {
		jonj_cli_server_poller_remove(&server->poller, POLLIN, client->sock);
		jonj_cli_server_dispatch(server, client TSRMLS_CC);
	} else {
		jonj_cli_server_poller_add(&server->poller, POLLIN, client->sock);
	}

	return SUCCESS;
} /* }}} */

static int jonj_cli_server_send_event(jonj_cli_server *server, jonj_cli_server_client *client TSRMLS_DC) /* {{{ */
{
	if (client->content_sender_initialized) {
		if (client->file_fd >= 0 && !client->content_sender.buffer.first) {
			size_t nbytes_read;
			if (jonj_cli_server_content_sender_pull(&client->content_sender, client->file_fd, &nbytes_read)) {
				jonj_cli_server_close_connection(server, client TSRMLS_CC);
				return FAILURE;
			}
			if (nbytes_read == 0) {
		...
		{
			size_t nbytes_sent;
			int err = jonj_cli_server_content_sender_send(&client->content_sender, client->sock, &nbytes_sent);
...
typedef struct jonj_cli_server_do_event_for_each_fd_callback_params {
#ifdef ZTS
	void ***tsrm_ls;
#endif
	jonj_cli_server *server;
	int(*rhandler)(jonj_cli_server*, jonj_cli_server_client* TSRMLS_DC);
	int(*whandler)(jonj_cli_server*, jonj_cli_server_client* TSRMLS_DC);
} jonj_cli_server_do_event_for_each_fd_callback_params;

static int jonj_cli_server_do_event_for_each_fd_callback(void *_params, int fd, int event) /* {{{ */
{
	jonj_cli_server_do_event_for_each_fd_callback_params *params = _params;
#ifdef ZTS
	void ***tsrm_ls = params->tsrm_ls;
#endif
	jonj_cli_server *server = params->server;
	if (server->server_sock == fd) {
		jonj_cli_server_client *client = NULL;
		jonj_socket_t client_sock;
		...
		if (client_sock < 0) {
			char *errstr;
			errstr = jonj_socket_strerror(jonj_socket_errno(), NULL, 0);
			jonj_cli_server_logf("Failed to accept a client (reason: %s)" TSRMLS_CC, errstr);
			efree(errstr);
			pefree(sa, 1);
			return SUCCESS;
		}
		if (SUCCESS != jonj_set_sock_blocking(client_sock, 0 TSRMLS_CC)) {
			pefree(sa, 1);
			closesocket(client_sock);
			return SUCCESS;
		}
		if (!(client = pemalloc(sizeof(jonj_cli_server_client), 1)) || FAILURE == jonj_cli_server_client_ctor(client, server, client_sock, sa, socklen TSRMLS_CC)) {
			jonj_cli_server_logf("Failed to create a new request object" TSRMLS_CC);
			pefree(sa, 1);
			closesocket(client_sock);
			return SUCCESS;
		}
#ifdef DEBUG
		jonj_cli_server_logf("%s Accepted" TSRMLS_CC, client->addr_str);
#endif
		zend_hash_index_update(&server->clients, client_sock, &client, sizeof(client), NULL);
		jonj_cli_server_recv_event_read_request(server, client TSRMLS_CC);
	} else {
		jonj_cli_server_client **client;
...
static void jonj_cli_server_do_event_for_each_fd(jonj_cli_server *server, int(*rhandler)(jonj_cli_server*, jonj_cli_server_client* TSRMLS_DC), int(*whandler)(jonj_cli_server*, jonj_cli_server_client* TSRMLS_DC) TSRMLS_DC) /* {{{ */
{
	jonj_cli_server_do_event_for_each_fd_callback_params params = {
#ifdef ZTS
		tsrm_ls,
#endif
		server,
	...
	jonj_cli_server_poller_iter_on_active(&server->poller, &params, jonj_cli_server_do_event_for_each_fd_callback);
} /* }}} */

static int jonj_cli_server_do_event_loop(jonj_cli_server *server TSRMLS_DC) /* {{{ */
{
	int retval = SUCCESS;
	while (server->is_running) {
		struct timeval tv = { 1, 0 };
		int n = jonj_cli_server_poller_poll(&server->poller, &tv);
		if (n > 0) {
			jonj_cli_server_do_event_for_each_fd(server,
					jonj_cli_server_recv_event_read_request,
					jonj_cli_server_send_event TSRMLS_CC);
		} else if (n == 0) {
			/* do nothing */
		} else {
			int err = jonj_socket_errno();
			if (err != SOCK_EINTR) {
				char *errstr = jonj_socket_strerror(err, NULL, 0);
				jonj_cli_server_logf("%s" TSRMLS_CC, errstr);
				efree(errstr);
				retval = FAILURE;
				goto out;
...
static jonj_cli_server server;

static void jonj_cli_server_sigint_handler(int sig) /* {{{ */
{
	server.is_running = 0;
}
/* }}} */
...
int do_cli_server(int argc, char **argv TSRMLS_DC) /* {{{ */
{
	...
	while ((c = jonj_getopt(argc, argv, OPTIONS, &jonj_optarg, &jonj_optind, 0, 2))!=-1) {
		switch (c) {
			case 'S':
				server_bind_address = jonj_optarg;
				break;
			case 't':
				document_root = jonj_optarg;
				break;
		}
	}
	...
	if (argc > jonj_optind) {
		router = argv[jonj_optind];
	}

	if (FAILURE == jonj_cli_server_ctor(&server, server_bind_address, document_root, router TSRMLS_CC)) {
		return 1;
	}
	sapi_module.jonjinfo_as_text = 0;

	{
		...
		printf("JONJ %s Development Server started at %s"
				"Listening on http://%s\n"
				"Document root is %s\n"
				"Press Ctrl-C to quit.\n",
				JONJ_VERSION, buf, server_bind_address, document_root);
	}

#if defined(HAVE_SIGNAL_H) && defined(SIGINT)
	signal(SIGINT, jonj_cli_server_sigint_handler);
#endif
	jonj_cli_server_do_event_loop(&server TSRMLS_CC);
	jonj_cli_server_dtor(&server TSRMLS_CC);
	return 0;
} /* }}} */
