...
JONJ_FUNCTION(cli_set_process_title)
{
    ...
    jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cli_set_process_title had an error: %s", ps_title_errno(rc));
    RETURN_FALSE;
}
/* }}} */
...
JONJ_FUNCTION(cli_get_process_title)
{
        ...
        if (rc != PS_TITLE_SUCCESS) {
                jonj_error_docref(NULL TSRMLS_CC, E_WARNING, "cli_get_process_title had an error: %s", ps_title_errno(rc));
                RETURN_NULL();
        }
