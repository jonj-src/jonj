...
static void jonj_info_pi3web(ZEND_MODULE_INFO_FUNC_ARGS)
{
	...
	PUTS("<tr><th colspan=2 bgcolor=\"" JONJ_HEADER_COLOR "\">Pi3Web Server Information</th></tr>\n");
	jonj_info_print_table_header(2, "Information Field", "Value");
	jonj_info_print_table_row(2, "Pi3Web SAPI module version", "$Id$");
	jonj_info_print_table_row(2, "Server Name Stamp", HTTPCore_getServerStamp());
	snprintf(variable_buf, 511, "%d", HTTPCore_debugEnabled());
	jonj_info_print_table_row(2, "Debug Enabled", variable_buf);
	PIPlatform_getCurrentDirectory( variable_buf, PI3WEB_SERVER_VAR_BUF_SIZE);
	jonj_info_print_table_row(2, "Current Path", variable_buf);
	if (lpCB->GetServerVariable(lpCB->ConnID, "SERVER_NAME", variable_buf, &variable_len)
		&& variable_buf[0]) {
		jonj_info_print_table_row(2, "Main Virtual Hostname", variable_buf);
	};
	snprintf(variable_buf, 511, "%d", PIPlatform_getProcessId());
	jonj_info_print_table_row(2, "Server PID", variable_buf);
	jonj_info_print_table_row(2, "Server Platform", PIPlatform_getDescription());

	PUTS("</table><br />");	

	PUTS("<table border=0 cellpadding=3 cellspacing=1 width=600 align=center>\n");
	PUTS("<tr><th colspan=2 bgcolor=\"" JONJ_HEADER_COLOR "\">HTTP Request Information</th></tr>\n");
	jonj_info_print_table_row(2, "HTTP Request Line", lpCB->lpszReq);
	PUTS("<tr><th colspan=2 bgcolor=\"" JONJ_HEADER_COLOR "\">HTTP Headers</th></tr>\n");
	...
	{	
		...
		if (lpCB->GetServerVariable(lpCB->ConnID, pKey, variable_buf, &variable_len)
			&& variable_buf[0]) {
			jonj_info_print_table_row(2, pKey, variable_buf);
			...
			if (lpCB->GetServerVariable(lpCB->ConnID, pKey, tmp_variable_buf, &variable_len)
				&& variable_buf[0]) {
				jonj_info_print_table_row(2, pKey, tmp_variable_buf);
			}
...
static zend_module_entry jonj_pi3web_module = {
...
static int jonj_pi3web_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &jonj_pi3web_module, 1)==FAILURE) {
		return FAILURE;
	} else {
		return SUCCESS;
	}
...
static void sapi_pi3web_register_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	{	
		...
		if (lpCB->GetServerVariable(lpCB->ConnID, pKey, static_variable_buf, &variable_len)
			&& (variable_len > 1)) {
			jonj_register_variable(pKey, static_variable_buf, track_vars_array TSRMLS_CC);
		} else if (PIPlatform_getLastError()==PIAPI_EINVAL) {
			variable_buf = (char *) emalloc(variable_len);
			if (lpCB->GetServerVariable(lpCB->ConnID, pKey, variable_buf, &variable_len)) {
				jonj_register_variable(pKey, variable_buf, track_vars_array TSRMLS_CC);
			}
			efree(variable_buf);
		}
...
MODULE_API DWORD JONJ5_wrapper(LPCONTROL_BLOCK lpCB)
{
	...
	zend_first_try {
		...
		switch ( lpCB->dwBehavior ) {
			case JONJ_MODE_STANDARD:
				iRet = ( jonj_execute_script( &file_handle TSRMLS_CC ) ) ?
			...
			case JONJ_MODE_INDENT: {
				...
				break;
			case JONJ_MODE_LINT:
				iRet = (jonj_lint_script(&file_handle TSRMLS_CC) == SUCCESS) ?
					PIAPI_COMPLETED : PIAPI_ERROR;
				break;
			default:
				iRet = PIAPI_ERROR;;
...
MODULE_API BOOL JONJ5_shutdown() {
