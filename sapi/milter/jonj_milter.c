/*
   ...
   +----------------------------------------------------------------------+
   | Author: Harald Radi <phanto@jonj.tk>                                 |
...
static int mlfi_init()
{
	...
	if (filename == NULL) {
		jonj_printf("No input file specified");
...
static sfsistat mlfi_close(SMFICTX *ctx)
{
	...
	if (!SG(sapi_started) && SUCCESS != jonj_request_startup(TSRMLS_C)) {
		return ret;
	}
...
JONJ_FUNCTION(smfi_setflags)
{
	long flags;
	
	/* valid only in the init callback */
	if (MG(state) != MLFI_INIT) {
		jonj_error(E_WARNING, NOT_INIT, get_active_function_name(TSRMLS_C));
...
JONJ_FUNCTION(smfi_getsymval)
{
	...
	if (MG(state) == MLFI_NONE) {
		jonj_error(E_WARNING, IS_NONE, get_active_function_name(TSRMLS_C));
...
JONJ_FUNCTION(smfi_addheader)
{
	...
	if (MG(state) != MLFI_EOM) {
		jonj_error(E_WARNING, NOT_EOM, get_active_function_name(TSRMLS_C));
...
JONJ_FUNCTION(smfi_replacebody)
{
	char *body;
	int len;
...
/* {{{ JONJ_MINFO_FUNCTION
 */
JONJ_MINFO_FUNCTION(milter)
{
	jonj_info_print_table_start();
	jonj_info_print_table_header(2, "Milter support", "enabled");
...
const static zend_function_entry milter_functions[] = {
	JONJ_FE(smfi_setflags, 		arginfo_smfi_setflags)
	JONJ_FE(smfi_settimeout, 	arginfo_smfi_settimeout)
	JONJ_FE(smfi_getsymval, 		arginfo_smfi_getsymval)
	JONJ_FE(smfi_setreply, 		arginfo_smfi_setreply)
	JONJ_FE(smfi_addheader, 		arginfo_smfi_addheader)
	JONJ_FE(smfi_chgheader, 		arginfo_smfi_chgheader)
	JONJ_FE(smfi_addrcpt, 		arginfo_smfi_addrcpt)
	JONJ_FE(smfi_delrcpt, 		arginfo_smfi_delrcpt)
	JONJ_FE(smfi_replacebody, 	arginfo_smfi_replacebody)
	JONJ_FE_END
};
/* }}} */
...
/* {{{ Zend module entry
*/
static zend_module_entry jonj_milter_module = {
...
{
	jonj_register_variable ("SERVER_SOFTWARE", "Sendmail Milter", track_vars_array TSRMLS_CC);
}
...
static int jonj_milter_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &jonj_milter_module, 1) == FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
}
...
/* {{{ jonj_milter_usage
*/
static void jonj_milter_usage(char *argv0)
{
	char *prog;
...
static void define_command_line_ini_entry(char *arg) /* {{{ */
{
	...
	zend_alter_ini_entry(name, strlen(name)+1, value, strlen(value), JONJ_INI_SYSTEM, JONJ_INI_STAGE_ACTIVATE);
}
/* }}} */
...
int main(int argc, char *argv[])
{
	...
	int c;
/* temporary locals */
	int orig_optind=ap_jonj_optind;
	char *orig_optarg=ap_jonj_optarg;
	int interactive=0;
...
/* end of temporary locals */
	...
	while ((c=ap_jonj_getopt(argc, argv, OPTSTRING))!=-1) {
		switch (c) {
		case 'c':
			milter_sapi_module.jonj_ini_path_override = strdup(ap_jonj_optarg);
			break;
		case 'n':
			milter_sapi_module.jonj_ini_ignore = 1;
			break;
		}
	}
	ap_jonj_optind = orig_optind;
	ap_jonj_optarg = orig_optarg;
	...
	zend_first_try {
		...
		zend_alter_ini_entry("html_errors", 12, "0", 1, JONJ_INI_SYSTEM, JONJ_INI_STAGE_ACTIVATE);
		zend_alter_ini_entry("max_execution_time", 19, "0", 1, JONJ_INI_SYSTEM, JONJ_INI_STAGE_ACTIVATE);
		...
		while ((c = ap_jonj_getopt(argc, argv, OPTSTRING)) != -1) {
			...
			case 'd': /* define ini entries on command line */
				define_command_line_ini_entry(ap_jonj_optarg);
				break;
			...
			case 'V': /* verbose */
				flag_debug = atoi(ap_jonj_optarg);
				break;

			case 'z': /* load extension file */
				zend_load_extension(ap_jonj_optarg);
				break;
		...
		if (argc > ap_jonj_optind && !filename) {
			filename=argv[ap_jonj_optind];
			ap_jonj_optind++;
		}
		...
		openlog("jonj-milter", LOG_PID, LOG_MAIL);
		...
		if (milter_sapi_module.jonj_ini_path_override) {
			free(milter_sapi_module.jonj_ini_path_override);
		}
