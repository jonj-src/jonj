...
/**
 * ParseTestFile
 * parse a single jonjt file and add it to the arrays
 */
...
/**
 * GetTestFiles
 * Recurse through the path and subdirectories, parse each jonjt file
 */
BOOL GetTestFiles(const char *path)
{
	// find all files .jonjt under testpath\tests
	...
	if (fh != INVALID_HANDLE_VALUE) {
		do {
			...
			} else if (strstr(fd.cFileName, ".jonjt")) {
...
static void _usage(char *argv0)
{
	...
	printf("Usage: %s -m <isapi.dll> -d|-l <file> [-t <numthreads>] [-i <numiterations>]\n"
				"  -m             path to isapi dll\n"
				"  -d <directory> jonj directory (to run jonj test files).\n"
...
int main(int argc, char* argv[])
{
	...
	if (!hDll) {
		...
		fprintf(stderr,"Error: Dll 'jonj5isapi.dll' not found -%d\n%s\n", GetLastError(), lpMsgBuf);
		free (module);
		free(filelist);
		LocalFree( lpMsgBuf );
		return -1;
				...
				const char *matchdata) 
{
	...
	// first arg = filename
	// this is added for testing jonj from command line
