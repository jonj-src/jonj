...
#define SPECIAL_VAR_JONJ_SELF	2
...
static void jonj_info_isapi(ZEND_MODULE_INFO_FUNC_ARGS)
{
	char **p;
	...
	jonj_info_print_table_header(2, "Server Variable", "Value");
	...
	while (*server_variable_names) {
		...
		while (*p) {
			...
			if (lpECB->GetServerVariable(lpECB->ConnID, *p, variable_buf, &variable_len)
				&& variable_buf[0]) {
				jonj_info_print_table_row(2, *p, variable_buf);
				...
				if (lpECB->GetServerVariable(lpECB->ConnID, *p, tmp_variable_buf, &variable_len)
					&& variable_buf[0]) {
					jonj_info_print_table_row(2, *p, tmp_variable_buf);
				}
...
static zend_module_entry jonj_isapi_module = {
...
static int jonj_isapi_startup(sapi_module_struct *sapi_module)
{
	if (jonj_module_startup(sapi_module, &jonj_isapi_module, 1)==FAILURE) {
		return FAILURE;
	} else {
...
static void sapi_isapi_register_zeus_ssl_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array TSRMLS_DC)
{
	...
	jonj_register_variable( "SSL_CLIENT_DN", static_cons_buf, track_vars_array TSRMLS_CC );
	...
	jonj_register_variable( "SSL_CLIENT_I_DN", static_cons_buf, track_vars_array TSRMLS_CC );	
}
...
static void sapi_isapi_register_zeus_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array TSRMLS_DC)
{
	 ...
	 * belongs in JONJ's version of PATH_INFO
	 */
	lpECB->GetServerVariable(lpECB->ConnID, "SCRIPT_NAME", static_variable_buf, &scriptname_len);

	/* Adjust Zeus' version of PATH_INFO, set JONJ_SELF,
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_INFO", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		/* JONJ_SELF is just PATH_INFO */
		jonj_register_variable( "JONJ_SELF", static_variable_buf, track_vars_array TSRMLS_CC );

		/* Chop off filename to get just the 'real' PATH_INFO' */
		pathinfo_len = variable_len - scriptname_len;
		jonj_register_variable( "PATH_INFO", static_variable_buf + scriptname_len - 1, track_vars_array TSRMLS_CC );
		...
		if ( strlen(lpECB->lpszQueryString) + variable_len + 1 < ISAPI_SERVER_VAR_BUF_SIZE ) {
			...
			jonj_register_variable( "URL", static_variable_buf, track_vars_array TSRMLS_CC );
			jonj_register_variable( "REQUEST_URI", static_variable_buf, track_vars_array TSRMLS_CC );
		}
	}

	/* Get and adjust PATH_TRANSLATED to what JONJ wants */
	variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_TRANSLATED", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		jonj_register_variable( "PATH_TRANSLATED", static_variable_buf, track_vars_array TSRMLS_CC );
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_USER", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		jonj_register_variable( "JONJ_AUTH_USER", static_variable_buf, track_vars_array TSRMLS_CC );
	}
	variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_PASSWORD", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		jonj_register_variable( "JONJ_AUTH_PW", static_variable_buf, track_vars_array TSRMLS_CC );
	}
	variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_TYPE", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		jonj_register_variable( "AUTH_TYPE", static_variable_buf, track_vars_array TSRMLS_CC );
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "SERVER_SOFTWARE", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		jonj_register_variable( "SERVER_SIGNATURE", static_variable_buf, track_vars_array TSRMLS_CC );
	}
}
#else
...
static void sapi_isapi_register_iis_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array TSRMLS_DC)
{
	...
	/* Get SCRIPT_NAME, we use this to work out which bit of the URL
	 * belongs in JONJ's version of PATH_INFO.  SCRIPT_NAME also becomes JONJ_SELF.
	 */
	lpECB->GetServerVariable(lpECB->ConnID, "SCRIPT_NAME", static_variable_buf, &scriptname_len);
	jonj_register_variable("SCRIPT_FILENAME", SG(request_info).path_translated, track_vars_array TSRMLS_CC);

	/* Adjust IIS' version of PATH_INFO, set JONJ_SELF,
	 * and generate REQUEST_URI
	 * Get and adjust PATH_TRANSLATED to what JONJ wants
	 */
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_INFO", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		/* Chop off filename to get just the 'real' PATH_INFO' */
		jonj_register_variable( "ORIG_PATH_INFO", static_variable_buf, track_vars_array TSRMLS_CC );
		pathinfo_len = variable_len - scriptname_len;
		strncpy(path_info_buf, static_variable_buf + scriptname_len - 1, sizeof(path_info_buf)-1);
		jonj_register_variable( "PATH_INFO", path_info_buf, track_vars_array TSRMLS_CC );
		/* append query string to give url... extra byte for '?' */
		if ( strlen(lpECB->lpszQueryString) + variable_len + 1 < ISAPI_SERVER_VAR_BUF_SIZE ) {
			/* append query string only if it is present... */
			if ( strlen(lpECB->lpszQueryString) ) {
		...
		variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
		if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_TRANSLATED", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
			jonj_register_variable( "ORIG_PATH_TRANSLATED", static_variable_buf, track_vars_array TSRMLS_CC );
		}
		if (lpECB->ServerSupportFunction(lpECB->ConnID, HSE_REQ_MAP_URL_TO_PATH_EX, path_info_buf, &pathinfo_len, (LPDWORD) &humi)) {
			...
			jonj_register_variable("PATH_TRANSLATED", humi.lpszPath, track_vars_array TSRMLS_CC);
		}
	}
	...
	if (lpECB->ServerSupportFunction(lpECB->ConnID, HSE_REQ_MAP_URL_TO_PATH_EX, static_variable_buf, &variable_len, (LPDWORD) &humi)) {
		...
		jonj_register_variable("DOCUMENT_ROOT", humi.lpszPath, track_vars_array TSRMLS_CC);
	}
	...
	if (!SG(request_info).auth_user || !SG(request_info).auth_password || 
		...
		if (lpECB->GetServerVariable(lpECB->ConnID, "HTTP_AUTHORIZATION", static_variable_buf, &variable_len)
			&& static_variable_buf[0]) {
			jonj_handle_auth_data(static_variable_buf TSRMLS_CC);
		}
	}

	if (SG(request_info).auth_user)  {
		jonj_register_variable("JONJ_AUTH_USER", SG(request_info).auth_user, track_vars_array TSRMLS_CC );
	}
	if (SG(request_info).auth_password) {
		jonj_register_variable("JONJ_AUTH_PW", SG(request_info).auth_password, track_vars_array TSRMLS_CC );
	}
}
#endif
...
static void sapi_isapi_register_server_variables2(char **server_variables, LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array, char **recorded_values TSRMLS_DC)
{
	...
	while (*p) {
		...
		if (lpECB->GetServerVariable(lpECB->ConnID, *p, static_variable_buf, &variable_len)
			&& static_variable_buf[0]) {
			jonj_register_variable(*p, static_variable_buf, track_vars_array TSRMLS_CC);
			if (recorded_values) {
			...
			if (lpECB->GetServerVariable(lpECB->ConnID, *p, variable_buf, &variable_len)
				&& variable_buf[0]) {
				jonj_register_variable(*p, variable_buf, track_vars_array TSRMLS_CC);
			}
			if (recorded_values) {
			...
			jonj_register_variable(*p, "", track_vars_array TSRMLS_CC);
		}
		p++;
	}
}
...
static void sapi_isapi_register_server_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	if (SG(request_info).cookie_data) {
		jonj_register_variable("HTTP_COOKIE", SG(request_info).cookie_data, track_vars_array TSRMLS_CC);
	}
	...
	/* JONJ_SELF support */
	if (isapi_special_server_variables[SPECIAL_VAR_JONJ_SELF]) {
		jonj_register_variable("JONJ_SELF", isapi_special_server_variables[SPECIAL_VAR_JONJ_SELF], track_vars_array TSRMLS_CC);
		efree(isapi_special_server_variables[SPECIAL_VAR_JONJ_SELF]);
	}

	if (isapi_special_server_variables[SPECIAL_VAR_ALL_HTTP]) {
		/* Register the internal bits of ALL_HTTP */
		variable = jonj_strtok_r(isapi_special_server_variables[SPECIAL_VAR_ALL_HTTP], "\r\n", &strtok_buf);
		while (variable) {
			...
			if (colon) {
				...
				*colon = 0;
				jonj_register_variable(variable, value, track_vars_array TSRMLS_CC);
				*colon = ':';
			}
			variable = jonj_strtok_r(NULL, "\r\n", &strtok_buf);
		}
...
static void jonj_isapi_report_exception(char *message, int message_len TSRMLS_DC)
{
	if (!SG(headers_sent)) {
		HSE_SEND_HEADER_EX_INFO header_info;
		LPEXTENSION_CONTROL_BLOCK lpECB = (LPEXTENSION_CONTROL_BLOCK) SG(server_context);
	...
	zend_first_try {
		...
		__try {
			...
			if (file_handle.filename)
				retval = jonj_fopen_primary_script(&file_handle TSRMLS_CC);
			...
			if (_exception_code()==EXCEPTION_STACK_OVERFLOW) {
				...
				_snprintf(buf, sizeof(buf)-1,"JONJ has encountered a Stack overflow");
				jonj_isapi_report_exception(buf, strlen(buf) TSRMLS_CC);
			} else if (_exception_code()==EXCEPTION_ACCESS_VIOLATION) {
				_snprintf(buf, sizeof(buf)-1,"JONJ has encountered an Access Violation at %p", e->ExceptionRecord->ExceptionAddress);
				jonj_isapi_report_exception(buf, strlen(buf) TSRMLS_CC);
				my_endthread();
			} else {
				_snprintf(buf, sizeof(buf)-1,"JONJ has encountered an Unhandled Exception Code %d at %p", e->ExceptionRecord->ExceptionCode , e->ExceptionRecord->ExceptionAddress);
				jonj_isapi_report_exception(buf, strlen(buf) TSRMLS_CC);
				my_endthread();
			}
		}
