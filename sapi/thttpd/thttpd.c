...
#define JONJ_SYS_CALL(x) do { x } while (n == -1 && errno == EINTR)
...
#define TG(v) TSRMG(thttpd_globals_id, jonj_thttpd_globals *, v)
#else
static jonj_thttpd_globals thttpd_globals;
...
static int sapi_thttpd_ub_write(const char *str, uint str_length TSRMLS_DC)
{
	...
	while (str_length > 0) {
		JONJ_SYS_CALL(n = send(TG(hc)->conn_fd, str, str_length, 0););
...
static int do_writev(struct iovec *vec, int nvec, int len TSRMLS_DC)
{
	...
	if (TG(sbuf).c == 0) {
		JONJ_SYS_CALL(n = writev(TG(hc)->conn_fd, vec, nvec););
...
static void sapi_thttpd_register_variables(zval *track_vars_array TSRMLS_DC)
{
	...
	jonj_register_variable("JONJ_SELF", SG(request_info).request_uri, track_vars_array TSRMLS_CC);
	jonj_register_variable("SERVER_SOFTWARE", SERVER_SOFTWARE, track_vars_array TSRMLS_CC);
	jonj_register_variable("GATEWAY_INTERFACE", "CGI/1.1", track_vars_array TSRMLS_CC);
	jonj_register_variable("REQUEST_METHOD", (char *) SG(request_info).request_method, track_vars_array TSRMLS_CC);
	jonj_register_variable("REQUEST_URI", SG(request_info).request_uri, track_vars_array TSRMLS_CC);
	jonj_register_variable("PATH_TRANSLATED", SG(request_info).path_translated, track_vars_array TSRMLS_CC);

	if (TG(hc)->one_one) {
		jonj_register_variable("SERVER_PROTOCOL", "HTTP/1.1", track_vars_array TSRMLS_CC);
	} else {
		jonj_register_variable("SERVER_PROTOCOL", "HTTP/1.0", track_vars_array TSRMLS_CC);
	}
...
#define CONDADD(name, field) 							\
	if (TG(hc)->field[0]) {								\
		jonj_register_variable(#name, TG(hc)->field, track_vars_array TSRMLS_CC); \
	}
	...
	if (TG(hc)->authorization[0])
		jonj_register_variable("AUTH_TYPE", "Basic", track_vars_array TSRMLS_CC);
}

static JONJ_MINIT_FUNCTION(thttpd)
{
	return SUCCESS;
}

static zend_module_entry jonj_thttpd_module = {
...
static int jonj_thttpd_startup(sapi_module_struct *sapi_module)
{
#if JONJ_API_VERSION >= 20020918
	if (jonj_module_startup(sapi_module, &jonj_thttpd_module, 1) == FAILURE) {
#else
	if (jonj_module_startup(sapi_module) == FAILURE
			|| zend_startup_module(&jonj_thttpd_module) == FAILURE) {
#endif
		return FAILURE;
	}
	return SUCCESS;
...
static void thttpd_request_ctor(TSRMLS_D)
{
	...
	jonj_handle_auth_data(TG(hc)->authorization TSRMLS_CC);
}
...
static off_t thttpd_real_jonj_request(httpd_conn *hc, int TSRMLS_DC);
...
/*		do_work = 50; */
		...
		thttpd_real_jonj_request(hc, 0 TSRMLS_CC);
...
static off_t thttpd_real_jonj_request(httpd_conn *hc, int show_source TSRMLS_DC)
{
	TG(hc) = hc;
	hc->bytes_sent = 0;
...
#else
	TSRMLS_FETCH();
	return thttpd_real_jonj_request(hc, show_source TSRMLS_CC);
#endif
}
...
void thttpd_jonj_init(void)
{
	...
	ts_allocate_id(&thttpd_globals_id, sizeof(jonj_thttpd_globals), NULL, NULL);
	qr_lock = tsrm_mutex_alloc();
	thttpd_register_on_close(remove_dead_conn);
#endif

	if ((ini = getenv("JONJ_INI_PATH"))) {
		thttpd_sapi_module.jonj_ini_path_override = ini;
	}
