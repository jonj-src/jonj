...
#define WEBJAMES_JONJ_ONLY
#include "jonj_webjames.h"
...
} jonj_webjames_globals;

static jonj_webjames_globals webjames_globals;
...
#define ADD_STRING(name,string)\
	jonj_register_variable(name, string, track_vars_array TSRMLS_CC)
...
#define ADD_FIELD(name, field) \
	if (WG(conn)->field) { \
		jonj_register_variable(name, WG(conn)->field, track_vars_array TSRMLS_CC); \
	}
...
static void webjames_jonj_close(struct connection *conn, int force)
...
/*called by WebJames to start handler*/
{
	...
	conn->close=webjames_jonj_close;
...
static void jonj_info_webjames(ZEND_MODULE_INFO_FUNC_ARGS)
{
	jonj_info_print_table_start();
	jonj_info_print_table_row(2, "SAPI module version", WEBJAMES_SAPI_VERSION);
	jonj_info_print_table_row(2, "WebJames version", WEBJAMES_VERSION " (" WEBJAMES_DATE ")");
	jonj_info_print_table_end();
}

static zend_module_entry jonj_webjames_module = {
...
static int jonj_webjames_startup(sapi_module_struct *sapi_module)
{
  if(jonj_module_startup(sapi_module, &jonj_webjames_module, 1) == FAILURE) {
    return FAILURE;
  } else {
    return SUCCESS;
  }
...
/*called when WebJames initialises*/
{
	...
	if (strcmp(configuration.webjames_h_revision,WEBJAMES_H_REVISION)!=0) {
		...
		webjames_writelog(0,"JONJ module is compiled for WebJames (%s) and was linked with a different version (%s)",WEBJAMES_H_REVISION,configuration.webjames_h_revision);
